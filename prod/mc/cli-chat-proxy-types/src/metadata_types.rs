...
/// v1.18: Added `shell_version`: the ainxt-shell agent binary version, distinct
///        from `client_version` (the UI client's version). They coincide for the
///        TUI but differ for embedding clients like ainxt-desktop.
...
/// OS-level sandbox state for a trace turn (local `ainxt-sandbox`, not cloud sandbox).
...
pub struct PromptMetadata {
    ...
    /// Whether the prompt was sent in unchanged mode (skipping `<user_query>` wrapping).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub prompt_verbatim: Option<bool>,
    /// Current working directory of the session.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub cwd: Option<String>,
    /// The agent type / harness name for this session (e.g. "ainxt-build", "codex").
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub agent_type: Option<String>,
    /// Version of the ainxt-shell agent binary that handled this turn
    /// (`ainxt_version::VERSION`). Self-reported by the agent, so it reflects
    ...
    /// like ainxt-desktop the bundled shell differs from the app version.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
mod tests {
    ...
    fn cwd_round_trips() {
        let mut meta: PromptMetadata = serde_json::from_str(minimal_json()).unwrap();
        meta.cwd = Some("/root/code/ainxt".into());
        let json = serde_json::to_string(&meta).unwrap();
        let deserialized: PromptMetadata = serde_json::from_str(&json).unwrap();
        assert_eq!(deserialized.cwd.as_deref(), Some("/root/code/ainxt"));
    }
    #[test]
