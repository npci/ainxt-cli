...
#
# Intentionally uses "latest" rather than a pinned version+checksum, unlike
# every artifact dotslash itself then manages (e.g. bin/protoc, which IS
# hash-pinned): this is the one-time bootstrap of the tool that enforces
# hash-pinning for everything downstream, fetched over HTTPS from its
# official, single-source GitHub Releases page. Pin this if your threat model
# requires reproducible bootstrap, not just reproducible managed artifacts.
