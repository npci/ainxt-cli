...
struct WaiterGuard<'a> {
    demux: &'a Demux,
    request_id: &'a ainxt_tool_protocol::RequestId,
}
...
/// Explicit overrides are honored unchanged; keep them comfortably above
...
pub struct ConnectionConfig {
    ...
    pub server_id: Option<ainxt_tool_protocol::ServerId>,
...
struct HubConnectionInner {
    ...
    server_id: Option<ainxt_tool_protocol::ServerId>,
...
impl HubConnection {
    ...
    pub async fn call_request<P>(
        &self,
        request_id: ainxt_tool_protocol::RequestId,
    ...
    pub fn try_alloc_request_id(&self) -> Result<ainxt_tool_protocol::RequestId, ClientError> {
        let value = self
            .inner
            .next_request_id
            .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        ainxt_tool_protocol::RequestId::new(format!("c{value}")).map_err(ClientError::from)
    }
    ...
    pub async fn serve(
        ...
        params: ainxt_tool_protocol::ServeParams,
    ) -> Result<ainxt_tool_protocol::ServeResult, ClientError> {
...
async fn open_socket(
    ...
    ainxt_tracing::http_client::attach_trace_to_http_request(headers);
...
async fn run_handshake(
    ...
    (
        ...
        ainxt_tool_protocol::HelloAckMsg,
    ),
    ClientError,
> {
    let ack = send_hello(
    ...
    'actor: loop {
        ...
        .await
        {
            ...
            ConnectedExit::SocketClosed(cause) => {
                ...
                warn!(
                    url = % url, cause = outage.disconnect_cause.label(), close_code = ? outage
                    .disconnect_cause.close_code(), error_detail = ? outage.disconnect_cause.detail(),
                    ...
                    match outcome {
                        ...
                        Err(err) => {
                            ...
                            warn!(
                                ?err,
                                attempt,
                                cause = outage.disconnect_cause.label(),
...
async fn reconnect_and_replay(
    ...
    if inner.kind == ConnectionKind::Harness {
        for sid in &sessions {
            let req = ainxt_tool_protocol::JsonRpcRequest {
                ...
                id: ainxt_tool_protocol::JsonRpcId::new_uuid_v7(),
    ...
    info!(
        attempt, sessions_replayed, cause = outage.disconnect_cause.label(), close_code = ? outage
        .disconnect_cause.close_code(), error_detail = ? outage.disconnect_cause.detail(), prev_connection_id =
    ...
    crate::metrics::reconnect_cause(outage.disconnect_cause.label());
...
mod tests {
    ...
    /// `Duration::ZERO`; a positive override is honored unchanged.
    #[test]
    ...
    async fn serve_send_failure_fails_fast_without_retry() {
        ...
        let result = tokio::time::timeout(
            ...
            conn.serve(session, ainxt_tool_protocol::ServeParams { tools: vec![] }),
        )
        .await
        .expect("serve must fail bounded, not park");
        assert!(matches!(result, Err(ClientError::NetworkError(_))));
        let request_id = ainxt_tool_protocol::RequestId::new("c1").expect("valid");
        assert!(
    ...
    async fn serve_times_out_bounded_and_reclaims_every_attempt_waiter() {
        ...
        let result = conn
            .serve(session, ainxt_tool_protocol::ServeParams { tools: vec![] })
            .await;
        assert!(matches!(result, Err(ClientError::NetworkError(_))));
        for id in ["c1", "c2", "c3"] {
            ...
            let request_id = ainxt_tool_protocol::RequestId::new(id).expect("valid");
            assert!(
                demux.take_response_waiter(&request_id).is_none(),
    ...
    /// (matches `RecordingSink` for the writer half).
    fn test_inbound() -> (InboundTx, InboundRx) {
        futures::channel::mpsc::unbounded()
    }
    /// A zero or unset liveness deadline resolves to 2.5× the effective
    /// ping cadence; a positive override is honored unchanged. Matches the
