...
//! variant-keyed family lookup (`ToolFamily`). Matches the toolbox
//! `AinxtToolDyn` / `AinxtToolFamily` tests but against the runtime's
//! typed `Tool` trait.
...
use ainxt_tool_protocol::{ToolCapabilities, ToolId};
...
async fn tool_dyn_preserves_custom_model_output() {
    ...
    match item {
        ToolStreamItem::Terminal(Ok(typed)) => {
            ...
            // Custom model output preserved unchanged — no JSON fallback.
...
async fn tool_dyn_delegates_id_description_capabilities() {
    ...
    assert_eq!(
        tool.description(&ainxt_tool_runtime::ListToolsContext::default())
            .name,
        "blocking_echo"
    );
