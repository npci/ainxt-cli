...
use ainxt_tool_protocol::StreamingSpec;
...
/// It is copied into the payload unchanged and is intentionally distinct from
...
pub fn stream_chunk(
    ...
    *last_total = if gap {
    ...
    let payload = PartialResultPayload {
        delta,
        total_bytes: total,
        // The caller's cumulative upstream-loss flag passes through unchanged;
