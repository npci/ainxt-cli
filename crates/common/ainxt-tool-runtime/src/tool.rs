...
/// Rich content block for `ToolProgress::Content`. Matches the wire-side
...
/// `filename`, `path`, `metadata`) that the ainxt SLOP converter uses to
...
pub enum ContentBlock {
    ...
    ///
    /// Optional metadata fields are used by the ainxt SLOP converter to
    ...
    /// - `path`: file path on the ainxt Computer filesystem
