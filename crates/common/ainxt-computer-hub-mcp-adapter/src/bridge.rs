...
use ainxt_tool_protocol::{McpBlock, SessionId, ToolId, ToolOutputWire};
use ainxt_tool_runtime::{ToolCallContext, ToolError, ToolStream, TypedToolOutput, terminal_only};
...
/// [`ainxt_computer_hub_sdk::ToolServerBuilder`] to register them
/// with the hub.
///
...
impl McpBridge {
    ...
    /// Handlers to register with a [`ainxt_computer_hub_sdk::ToolServerBuilder`].
    ///
...
#[async_trait]
impl ainxt_computer_hub_sdk::ToolServerHandler for McpToolHandler {
...
mod tests {
    ...
    async fn bridge_handles_transport_error() {
        ...
        let item = stream.next().await.unwrap();
        match item {
            ainxt_tool_runtime::ToolStreamItem::Terminal(Err(ref e))
                if e.kind == ainxt_tool_runtime::ToolErrorKind::Execution =>
            {
                assert!(
