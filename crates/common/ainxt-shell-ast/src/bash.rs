...
pub fn is_setup_command(cmd: &[String]) -> bool {
    if cmd.is_empty() {
        return true;
    }
...
pub fn strip_wrapper_command(cmd: &[String]) -> Option<&[String]> {
...
pub fn is_wrapper_command(words: &[String]) -> bool {
    matches!(
...
/// wrapper is present.
pub fn unwrap_wrappers(words: &[String]) -> &[String] {
...
pub fn wrapper_has_chdir(words: &[String]) -> bool {
    let mut current = words;
    for _ in 0..8 {
...
///
/// ```bash
/// AINXT_API_KEY='ainxt-some-key' cargo run --bin ainxt-pager
/// ```
///
...
///   prefix: ["AINXT_API_KEY=ainxt-some-key"]
///   highlighted_words: ["cargo", "run", "--bin", "ainxt-pager"]
///   suffix: []
...
mod tests {
    ...
    fn test_parse_plain_commands_from_script() {
        ...
        let environment_key_command = "AINXT_API_KEY='ainxt-some-key' cargo run --bin ainxt-pager";
        assert_eq!(
            primary_command_from_script(environment_key_command),
            Some(BashCommandHighlights {
                prefix: vec!["AINXT_API_KEY=ainxt-some-key".to_owned()],
                highlighted_words: vec![
                    "cargo".to_owned(),
                    "run".to_owned(),
                    "--bin".to_owned(),
                    "ainxt-pager".to_owned()
                ],
                suffix: vec![],
            })
        );
        ...
        let redirection_command = "cargo build --bin ainxt-pager 2>&1";
        assert_eq!(
        ...
        let another_long_command = "cargo test --package ainxt-shell --lib -- permission::bash_command_splitting::tests::test_parse_plain_commands_from_script --exact --nocapture 2>&1";
        assert_eq!(
