...
pub struct SubagentDescriptor {
    ...
    /// unchanged after the description; may itself contain product-specific
    /// template variables (e.g. the CLI's `${{ tools.by_kind.* }}`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub tools: Option<String>,
}

/// A built-in subagent type shared by the CLI (`ainxt-agent`) and other
...
mod tests {
    ...
    fn task_tool_input_model_parses_explicit() {
        let input: TaskToolInput =
            serde_json::from_str(r#"{"description": "d", "prompt": "p", "model": "ainxt-3"}"#)
                .unwrap();
        assert_eq!(input.model.as_deref(), Some("ainxt-3"));
    }
    ...
    fn sanitize_optional_arg_moves_when_no_trim() {
        assert_eq!(
            sanitize_optional_arg(Some("ainxt-3".into())).as_deref(),
            Some("ainxt-3")
        );
        assert_eq!(
            sanitize_optional_arg(Some("  ainxt-3  ".into())).as_deref(),
    ...
    /// rendering reproduces the placeholder kinds unchanged.
    ...
    fn render_tools_substitutes_naming_with_bare_kind_fallback() {
        // Bare-kind naming reproduces the placeholder kinds unchanged.
        assert_eq!(
    ...
    fn build_task_description_preserves_template_placeholders() {
        ...
        // unchanged for its downstream TemplateRenderer to resolve.
        let subagents = vec![SubagentDescriptor {
            name: "explore".into(),
            description: "Explore.".into(),
    ...
    // match what the ainxt-shell MiniJinja templates render for the default
    // ainxt-build toolset (monitor + task + bash + read present, POSIX). The
