...
pub trait ConnectionClient: Send + Sync + std::fmt::Debug {
    ...
    /// response envelope unchanged, including method-level error outcomes.
...
fn map_block(block: ainxt_tool_protocol::McpBlock) -> ContentBlock {
    use ainxt_tool_protocol::McpBlock;
    match block {
...
pub fn error_from_envelope(err: ainxt_tool_protocol::JsonRpcError) -> ToolError {
