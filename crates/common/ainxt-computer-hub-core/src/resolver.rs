...
use ainxt_tool_protocol::{SessionId, ToolCapabilities, ToolId, ToolRegistration};
            ...
            ToolStreamItem::Terminal(Ok(out)) => match serde_json::to_value(&out) {
                Ok(value) => {
                    ...
                    let model_output = if custom.is_empty() {
                        ainxt_tool_runtime::extract_content_blocks(&value)
                    } else {
                        custom
                    };
...
impl CompoundResolver {
    ...
    /// handle, returning the tool's stream unchanged. Misses produce a
