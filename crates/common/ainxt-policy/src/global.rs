...
mod tests {
    ...
    // ENGINE is a process-global static (ArcSwapOption), and cargo test runs
    // tests within one binary on separate threads by default. Without
    // #[serial], scoped_install_and_clear's temporary Block install can be
    // live on another thread exactly when this test reads active(), which
    // would then see Block instead of the uninstalled-default Off.
    #[test]
    #[serial(policy_global_engine)]
