...
pub struct ToolCallResult {
    ...
    /// this crate must not depend on `ainxt-tool-runtime`. Sampler-side wire
...
/// JSON value forwarded unchanged without decoding.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
...
/// `ainxt_computer_hub_core::registry::ToolSessionBindOutcome`, with one
...
pub struct ToolsListResult {
    pub tools: Vec<ainxt_tool_types::ToolDescription>,
}
...
pub struct SessionBindServerResult {
    ...
    /// [`SessionBindResult::unserved_tool_ids`], forwarded unchanged.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub unserved_tool_ids: Vec<String>,
    /// [`SessionBindResult::resolve_error`], forwarded unchanged.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub struct HookFrame {
    ...
    pub event: HookEvent,
    /// W3C `traceparent` (matches [`ToolCallParams::trace_context`]).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub trace_context: Option<String>,
}
