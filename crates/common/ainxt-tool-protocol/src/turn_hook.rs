...
/// Default `session_relationship` wire value (matches
/// `ainxt_file_utils::events::SessionRelationship::Primary`).
pub const DEFAULT_SESSION_RELATIONSHIP: &str = "primary";

/// Default `schema_version` wire value. Bare literal (not the
/// `ainxt-file-utils` constant) to avoid a dependency cycle.
...
pub struct BeforeTurnPayload {
    ...
    pub turn_number: u64,
    /// Model being used for this turn (e.g. "ainxt-3").
    pub model_id: String,
    /// Whether the session is in YOLO / auto-approve mode.
    #[serde(default)]
    pub yolo_mode: bool,
    // ── Extended fields (workspace matches these into `events.jsonl`);
    // all `#[serde(default)]` for old-shell / old-workspace interop. ──
    /// Matches `Event::TurnStarted::conversation_message_count`.
    #[serde(default)]
    pub conversation_message_count: usize,
    /// Snake-case mirror of `Event::TurnStarted::session_relationship`
    /// (`"primary"` | `"subagent"`). A `String`, not the `ainxt-file-utils`
    ...
    /// Matches `Event::TurnStarted::schema_version`.
    #[serde(default = "default_schema_version")]
    pub schema_version: String,
}

impl Default for BeforeTurnPayload {
    /// Matches the per-field serde defaults so producers that don't yet track a
...
pub struct AfterTurnPayload {
    ...
    /// the workspace decodes it into the `ainxt-file-utils`
    ...
    /// unchanged by the workspace. `None` when there is no context.
    #[serde(default, skip_serializing_if = "Option::is_none")]
