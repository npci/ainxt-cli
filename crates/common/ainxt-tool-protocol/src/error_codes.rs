...
//! envelope code; the string is the ainxt stable identifier.
//!
...
/// Structured payload placed in the wire `details` object. `code` matches the
    ...
    fn builder_emits_custom_with_code_in_details_for_every_reason_and_phase() {
        for reason in REASONS {
            for phase in PHASES {
                ...
                // details.code matches the subcode (round-trip identity).
