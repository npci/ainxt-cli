...
//! harness implements [`CompactionItem`] for its `AinxtTurn`;
//! ainxt-build implements it for `ainxt_sampling_types::ConversationItem`.
//!
...
//! `<ainxt_user_queries>` blocks, drop tool content from assistant turns,
//! wrap an LLM summary into a carrier item).

/// Harness-agnostic role of a single conversation item.
///
/// This is the common denominator of `AinxtRole` (ainxt chat) and the
/// `ConversationItem` variants (ainxt-build).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CompactionRole {
    /// System prompt.
    System,
    /// Developer prompt (ainxt chat) — maps to System on harnesses without a
...
/// extraction (`<ainxt_file id=".." name=".." />` lines in the
/// `<ainxt_user_queries>` preamble).
...
///
/// Implementors:
/// - ainxt chat: `AinxtTurn`
/// - ainxt-build: `ConversationItem`
pub trait CompactionItem {
    ...
    ///
    /// Returns an owned `String` because some harnesses (ainxt chat's
    /// `AinxtTurn`) compute the flattened text on demand rather than storing a
    /// borrowable slice.
    ...
    /// Whether this item carries a *prior compaction summary* (ainxt chat: a
    ...
    /// strips their `<ainxt_user_queries>` blocks before sampling.
    ///
    ...
    /// File attachments on a (user) item, for the `<ainxt_file>` lines in the
    /// `<ainxt_user_queries>` preamble. Empty for items without attachments.
    ///
...
pub trait CompactionItemBuilder: CompactionItem + Clone {
    ...
    /// conversation (ainxt chat: a `Developer` turn with category
    ...
    /// requests/results (ainxt chat: keep only `Channel` contents of an
...
/// constructs the typed harness items that make up ainxt-build's rebuilt
/// history.
///
/// This is a sibling of [`CompactionItemBuilder`], not a part of it, on
/// purpose. `CompactionItemBuilder` is already implemented by ainxt chat's
/// `AinxtTurn`; adding these constructors to it as required methods would break
/// that impl. They are also ainxt-build-specific (ainxt chat's tail-keep path
...
///
/// The ainxt-build implementor (`ConversationItem`) maps each constructor to the
...
/// operate over `&[Arc<T>]` (ainxt chat stores turns as `Arc<AinxtTurn>`).
