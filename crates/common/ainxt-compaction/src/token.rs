...
//!
//! - ainxt chat has a real tokenizer (`TextTokenizer` / `ImageTokenizer`) and
//!   counts whole turns via `AinxtTurn::get_num_tokens`.
//! - ainxt-build estimates with `bytes / 4`.
//!
...
//! (ainxt chat: `ItemTokenCounter<Arc<AinxtTurn>>`), so exactly one mechanism
//! is in play.
