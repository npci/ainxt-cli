...
//!
//! Behavior is exactly identical for ainxt chat (`T = Arc<AinxtTurn>`).
...
///   summaries so their `<ainxt_user_queries>` sections can be split out.
...
/// Extracts **all** `<ainxt_user_queries>...</ainxt_user_queries>` blocks
...
pub fn split_prior_compaction_text(text: &str) -> (Option<String>, String) {
    let start_tag = "<ainxt_user_queries>";
    let end_tag = "</ainxt_user_queries>";
...
/// Extract a `<ainxt_user_queries>` XML block from `User` items in `turns`.
///
/// Walks `turns`, finds `User` items, and formats each as a `<ainxt_query>`
/// element with text content (from [`CompactionItem::text`]) and any
/// `<ainxt_file id="..." name="..." />` lines for the item's attachment
...
pub fn extract_user_queries_from_turns<T: CompactionItem>(
    ...
    let mut result = String::from("<ainxt_user_queries>\n");
    ...
    result.push_str("</ainxt_user_queries>");
    Some(result)
}

/// Walk `turns`, find any prior compaction summary items, extract their
/// `<ainxt_user_queries>` blocks via [`split_prior_compaction_text`], and
/// concatenate them.
///
...
pub struct SeparatedHistoryTurns<T> {
    /// `turns` with the `<ainxt_user_queries>` block stripped from every
    ...
    /// Concatenation of every `<ainxt_user_queries>` block found (in
    /// document order, joined by `\n`). `None` if no prior compaction
    /// item contained a user-queries block. Preserved unchanged so it
    ...
    /// regardless of whether it contained a `<ainxt_user_queries>` block.
...
/// `<ainxt_user_queries>` block (preserved unchanged for the next summary)
...
///   loop, so the LLM never sees `<ainxt_user_queries>` from earlier rounds.
...
///   would see the prior `<ainxt_user_queries>` and tend to copy it into the
...
pub fn separate_prior_user_queries<T: CompactionItemBuilder>(
    ...
    for turn in turns {
        if turn.is_compaction_summary() {
            ...
            // `<ainxt_user_queries>` block (and therefore stripped to an empty
...
mod tests {
    ...
    fn extract_user_queries_wraps_single_user_turn() {
        ...
        assert!(out.starts_with("<ainxt_user_queries>"));
        assert!(out.ends_with("</ainxt_user_queries>"));
        assert!(out.contains("<ainxt_query>hello world</ainxt_query>"));
    }
    ...
    fn extract_prior_user_queries_concatenates_blocks() {
        let inner = "<ainxt_user_queries>\n<ainxt_query>first</ainxt_query>\n</ainxt_user_queries>";
        let inner2 = "<ainxt_user_queries>\n<ainxt_query>second</ainxt_query>\n</ainxt_user_queries>";
    ...
    fn separate_strips_user_queries_from_summary_item() {
        let prior = "<ainxt_user_queries>\n<ainxt_query>Q1</ainxt_query>\n</ainxt_user_queries>\n\n<chunk_summary index=\"0\">S1</chunk_summary>";
        ...
        assert!(prior.contains("<ainxt_user_queries>"));
        ...
        match &sep.turns_for_llm[0] {
            MockItem::Developer { text, summary } => {
                ...
                assert!(!text.contains("<ainxt_user_queries>"));
    ...
    fn separate_drops_summary_item_with_no_rest() {
        let only_queries = "<ainxt_user_queries>\n<ainxt_query>Q</ainxt_query>\n</ainxt_user_queries>";
