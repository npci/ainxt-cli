//! Compacted-history assembly (ainxt-build's rebuild structure, generic).
//!
//! Moved from `ainxt-chat-state::compaction_utils::build_compacted_history` and
...
//! ```
//!
//! ainxt-build is the canonical harness. The summary carrier text is built by
...
pub struct CompactedHistoryParts<T> {
    ...
    /// Messages retained unchanged from after the last real user turn.
...
/// 3. **AGENTS.md reminder** (if any) -- project instructions re-injected unchanged.
/// 4. **Last user query** (if any) -- wrapped in `<user_query>` tags.
/// 5. **Recent messages** (if any) -- retained unchanged from after the last
///    real user turn.
...
pub fn assemble_compacted_history<T: CompactionItemFactory>(
    ...
    // compaction unchanged (not dependent on the summarizer). The
    ...
    // ainxt-build keeps the legacy `<user_query>`-wrapped continuation text and
...
mod tests {
    ...
    #[test]
    fn ainxt_build_order_recent_before_summary() {
