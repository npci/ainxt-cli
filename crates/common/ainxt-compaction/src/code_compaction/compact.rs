//! ainxt-build's full-replace compaction pass.
//!
//! ainxt-build does not select a tail to keep; it summarizes the whole
...
//! ```
//!
//! Per-harness concerns stay in the product host (for example `ainxt-shell`): the triggers, the
//! conversation *gathering / sanitization* that produces `llm_turns`, the
//! unchanged→fitted→lossy input ladder, the live LLM transport (the
...
pub struct FullReplaceContext<T> {
    /// The original system message, carried over unchanged.
    ...
    /// The last real user query (raw), kept unchanged post-compaction.
    pub last_user_query: Option<String>,
    /// Working tail retained unchanged (tool/subagent results from the current
    /// turn). ainxt-build keeps this; pass empty to drop it.
...
///
/// Returned by [`sample_full_replace_summary`] for harnesses (ainxt-build's
...
/// Run ainxt-build's full-replace compaction pass and return the rebuilt
...
///
/// The **input ladder** (unchanged → fitted → lossy) stays in the product host: on a
...
///
/// This is the seam ainxt-build's shell uses: it drives the unchanged → fitted →
    ...
    O: FullReplaceObserver + ?Sized,
{
    ...
    let prompt = CompactionPrompt {
        // ainxt-build appends the summarization prompt as the final user
...
mod tests {
    ...
    /// returns a structured summary must produce ainxt-build's exact compacted
    /// history shape, with the LLM output cleaned and the agent-state reminder
    /// carried through as the final item.
    #[tokio::test]
    async fn full_replace_produces_ainxt_build_history_shape() {
        let llm_turns = vec![
        ...
        // Agent-state reminder carried through unchanged as the final item.
        assert_eq!(
            out[6],
