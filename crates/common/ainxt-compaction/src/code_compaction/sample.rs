...
//!
//! The canonical `sample → classify → retry` loop, used by **both** ainxt-build's
//! full-replace pass ([`sample_full_replace_summary`](super::sample_full_replace_summary))
//! and ainxt chat's intra `Shared` summarizer
...
//! the winning summary (ainxt-build cleans in its assembler; intra cleans via
...
pub enum SampleRetryError {
    ...
    Failure {
        ...
        /// signal the ainxt-build host uses to step down its input size).
