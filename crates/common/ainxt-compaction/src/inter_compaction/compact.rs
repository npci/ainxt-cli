...
//! extraction, chunk summarisation, and the final `<ainxt_user_queries>`
//! + `<chunk_summary>` assembly — is shared. The harness supplies the
//! candidate items, the *current* user-queries preamble (ainxt chat
...
pub struct ChunkedCompactionOutput {
    /// `<ainxt_user_queries>` preamble + `<chunk_summary index="i">` blocks.
...
/// 2. [`separate_prior_user_queries`] — split prior `<ainxt_user_queries>`
...
/// round's user messages (ainxt chat: unchanged from the raw request, with
/// attachment refs). `conversation_id` / `response_id` are threaded
/// through for log correlation only.
///
/// Observer events (the ainxt chat observer maps them to the
...
pub async fn sample_compaction_chunked<T: CompactionItemBuilder + Send + Sync>(
    ...
    // Step 2 — split prior `<ainxt_user_queries>` out of every prior
    // compaction summary item. The LLM never sees them (it would re-emit
    // them unchanged and snowball across rounds); they are reattached to
