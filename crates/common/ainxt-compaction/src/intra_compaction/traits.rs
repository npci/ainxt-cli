...
pub enum CompactionTarget {
    ...
    /// with a single summary — ainxt-build's full-replace strategy. No tail is
...
/// stream processor. Implemented by ainxt chat's
/// `StreamProcessor` (`Item = Arc<AinxtTurn>`).
///
...
pub trait CompactionStreamProc: Send + Sync {
    ...
    /// item type (`Arc<AinxtTurn>`) already satisfies it.
