...
// The `Shared` summarizer reuses ainxt-build's full-replace summarization core
...
/// (e.g. running sub-agents) appended unchanged to the summary on the
...
/// `FullReplace` strategy (default): ainxt-build's full-replace — summarize the
...
/// `<ainxt_user_queries>` preamble: the shared `code_compaction` summarizer
/// (always [`IntraSummarizer::Shared`] here, regardless of `policy.summarizer`)
/// preserves user intent itself, matching ainxt-build. The reduction and
/// `min_compactable_tokens` guards are kept for equivalence with the partial modes.
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 2. Summarize the whole conversation through ainxt-build's shared core.
    ...
    //     harness-supplied `<system-reminder>` (unchanged ids) to the summary so
    //     the model can keep polling/cancelling them. Empty/None → no change.
    //     Shared with ainxt chat inter-compaction via `append_reminder_block` so
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 3a. For `History` target, split prior `<ainxt_user_queries>` blocks
    //     out of any prior compaction summary items before sampling — same
    //     primitive inter-compaction uses, so the LLM never sees
    //     `<ainxt_user_queries>` and won't re-emit it (which would snowball
    ...
    //     and the `<ainxt_user_queries>` preamble below — stays intra's.
    let summary_text = match policy.summarizer {
        ...
        // New (default): ainxt-build's shared summarization core from
    ...
    // 3c. For `History` target, prepend a `<ainxt_user_queries>` preamble so
...
/// — ainxt-build's summarization core (`build_summary_prompt` + bounded retry +
...
/// intra and ainxt-build stay in lock-step. Outcome mapping:
...
where
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    // ainxt-build appends the summarization prompt as the final user message;
    ...
    .await
    {
        // ainxt-build returns the raw summary and cleans it in its assembler;
...
/// metric label. Keep these in sync with the doc string on ainxt chat's
...
/// the literal error messages produced by the ainxt chat sampler —
...
mod tests {
    ...
    fn compaction_sample_error_to_intra_maps_empty_response() {
        // The literal message emitted by the ainxt chat sampler when the
    ...
    async fn full_replace_appends_active_reminder_to_summary() {
        ...
        // appended unchanged to the FullReplace summary developer turn.
        let turns: Vec<_> = (0..6).map(|_| MockItem::user(&"x".repeat(2000))).collect();
        let sp = MockStreamProc::with_turns(turns);
        let sampler = MockSampler::returns(&long_summary());
        let obs = RecordingObserver::default();
        ...
        // The committed summary developer turn carries the unchanged id.
        assert!(
    ...
    async fn legacy_summarizer_accepts_short_uncleaned_summary() {
        ...
        // summary is accepted unchanged (would be rejected under `Shared`).
        let turns: Vec<_> = (0..6).map(|_| MockItem::user(&"x".repeat(400))).collect();
        let sp = MockStreamProc::with_turns(turns);
    ...
    /// — guards the forwarding that ainxt chat (`Arc<AinxtTurn>`) relies on.
    #[test]
