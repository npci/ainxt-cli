...
///
/// - `FullReplace` (default): ainxt-build's full-replace strategy — summarize
...
///
/// Uses **percentage** thresholds (borrowed from ainxt-shell's
...
/// time (token threshold only, matching ainxt-build) and also ignores
...
pub struct IntraCompactionConfig {
    ...
    /// as ainxt-build full-replace auto-compact). Worthless early passes are
    ...
    /// `<ainxt_user_queries>` preamble prepended to the history compaction
    /// summary. Matches the inter-compaction Basic threshold. Has no
...
pub const DEFAULT_COMPACTION_MODEL_NAME: &str = "ainxt-4.20";
