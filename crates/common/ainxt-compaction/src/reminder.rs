...
//!
//! - ainxt chat intra FullReplace ([`crate::intra_compaction`]) and inter
//!   (appends after sampling via [`append_reminder_block`])
//! - ainxt-build full-replace ([`crate::code_compaction`] assemble's
//!   `system_reminder`)
//!
...
/// Still-running background task. `task_id` is rendered unchanged.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
/// Still-running sub-agent. `subagent_id` is rendered unchanged.
///
