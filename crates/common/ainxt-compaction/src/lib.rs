...
//! stay in each product host (for example `ainxt-shell`).
//!
//! The crate depends on **neither** a conversation-type crate nor
//! `ainxt-sampling-types`. It is decoupled from both ainxt chat and
//! ainxt-build hosts through a small set of trait seams:
//!
...
//!
//! - [`code_compaction`] — ainxt-build's whole-session **full-replace**
//!   subsystem (prompt/summary/failure/config, assemble, orchestration).
//! - [`intra_compaction`] — ainxt chat's tail-keep, per-step pass.
//! - [`inter_compaction`] — ainxt chat's chunked, between-turn pass.
//!
...
//! (active-agent-state `<system-reminder>` formatting shared by ainxt chat and
//! ainxt-build; hosts still own snapshotting and host-only sections).
...
// ainxt-build's full-replace subsystem now lives under `code_compaction`;
