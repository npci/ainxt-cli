    ...
    B: Backend<Error = io::Error>,
{
        ...
        B: Write,
    {
        ...
        // plain cell diff + draw — exactly identical to `flush` with zero
...
mod inline_resize_tests {
    ...
    /// [`TestBackend`] uses [`std::convert::Infallible`] as its `Backend::Error`
    /// (ratatui 0.30+), but our `Terminal<B>` is bounded on `Backend<Error =
    /// io::Error>` to match every real backend we ship with (`CrosstermBackend`).
    /// This thin wrapper re-exposes `TestBackend` as `io::Error`-fallible so it
    /// stays usable in tests; since `Infallible` is uninhabited, the `match {}`
    /// conversions below can never actually run.
    ...
    impl Backend for IoTestBackend {
        ...
        fn get_cursor_position(&mut self) -> io::Result<Position> {
            match self.0.get_cursor_position() {
                Ok(p) => Ok(p),
                Err(e) => match e {},
            }
        }
        ...
        fn set_cursor_position<P: Into<Position>>(&mut self, position: P) -> io::Result<()> {
            match self.0.set_cursor_position(position) {
                Ok(()) => Ok(()),
                Err(e) => match e {},
            }
        }
        ...
        fn clear_region(&mut self, clear_type: ClearType) -> io::Result<()> {
            match self.0.clear_region(clear_type) {
                Ok(()) => Ok(()),
                Err(e) => match e {},
            }
        }
    ...
    fn full_height_inline(width: u16, height: u16) -> Terminal<IoTestBackend> {
        Terminal::with_options(
            IoTestBackend::new(width, height),
            TerminalOptions {
    ...
    fn small_inline_viewport_is_not_forced_full() {
        let backend = IoTestBackend::new(80, 24);
