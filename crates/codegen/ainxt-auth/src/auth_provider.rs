...
pub trait AuthCredentialProvider: HttpAuth + Send + Sync + 'static {
    ...
    /// (`ainxt-desktop`, `ainxt login`). The `token` field MUST mirror
    ...
    /// Whether `X-Ainxt-Token-Auth` should be sent with the bearer token.
    /// `false` for deployment keys (bare Bearer), `true` for user/OAuth tokens.
    /// See `AinxtAuthCredentials::apply()` for the wire format contract.
