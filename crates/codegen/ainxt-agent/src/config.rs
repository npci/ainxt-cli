//! Agent definition types — parsed from `.ainxt/agents/*.md` files.
...
use ainxt_tools::implementations::codex;
use ainxt_tools::implementations::ainxt_build;
use ainxt_tools::implementations::ainxt_build_concise;
use ainxt_tools::implementations::memory;
use ainxt_tools::implementations::opencode;
use ainxt_tools::implementations::search_tool;
use ainxt_tools::implementations::use_tool;
use ainxt_tools::registry::types::{ToolConfig, ToolServerConfig};
...
/// Orchestrator-specific prompt body appended to the standard AinxtBuild
...
fn bash_tool_config() -> ToolConfig {
    ToolConfig::from(&ainxt_build::BashTool)
...
fn task_tool_config() -> ToolConfig {
    ToolConfig::from(&ainxt_build::TaskTool)
...
fn task_output_tool_config() -> ToolConfig {
    ToolConfig::from(&ainxt_build::TaskOutputTool).with_name("get_command_or_subagent_output")
}
/// `wait_tasks` → `wait_commands_or_subagents`.
fn wait_tasks_tool_config() -> ToolConfig {
    ToolConfig::from(&ainxt_build::WaitTasksTool).with_name("wait_commands_or_subagents")
}
/// `kill_task` → `kill_command_or_subagent`.
fn kill_task_tool_config() -> ToolConfig {
    ToolConfig::from(&ainxt_build::KillTaskTool).with_name("kill_command_or_subagent")
}
/// Complete workspace-executable toolset for hub registration.
///
/// Extends `default_ainxt_build_toolset()` with tools that are dynamically
...
/// zero local dispatch.
pub fn workspace_ainxt_build_toolset() -> ToolServerConfig {
    let mut tools = default_ainxt_build_toolset().tools;
    tools.push((&opencode::OpenCodeWriteTool).into());
    tools.push((&ainxt_build::EnterPlanModeTool).into());
    tools.push((&ainxt_build::ExitPlanModeTool).into());
    tools.push((&ainxt_build::AskUserQuestionTool).into());
    tools.push((&ainxt_build::WebSearchTool).into());
    tools.push((&ainxt_build::WebFetchTool).into());
    tools.push((&memory::search_tool::MemorySearchImpl).into());
    tools.push((&memory::get_tool::MemoryGetImpl).into());
    tools.push((&ainxt_build::LspTool).into());
    ToolServerConfig {
        tools,
        behavior_preset: None,
    }
}
/// Toolset for the `ainxt-computer` (workspace/sandbox) preset.
fn ainxt_computer_toolset() -> ToolServerConfig {
    #[allow(unused_mut)]
    let mut tools = vec![
        bash_tool_config(),
        (&ainxt_build::ReadFileTool).into(),
        (&ainxt_build::SearchReplaceTool).into(),
        (&opencode::OpenCodeWriteTool).into(),
        (&ainxt_build::ListDirTool).into(),
        (&ainxt_build::GrepTool).into(),
        (&ainxt_build::KillTerminalCommandTool).into(),
        (&ainxt_build::GetTerminalCommandOutputTool).into(),
    ];
    ToolServerConfig {
        tools,
        behavior_preset: None,
...
fn native_toolset_presets() -> Vec<(&'static str, ToolServerConfig)> {
    vec![
        ("ainxt-build", workspace_ainxt_build_toolset()),
        ("ainxt-build-concise", ainxt_build_concise_toolset()),
        ("ainxt-build-plan", ainxt_build_plan_toolset()),
        ("codex", codex_toolset()),
        ("explore", explore_toolset()),
        ("plan", plan_toolset()),
        ("ainxt-computer", ainxt_computer_toolset()),
    ]
}
...
fn default_ainxt_build_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&ainxt_build::ReadFileTool).into(),
            ...
            (&ainxt_build::TodoWriteTool).into(),
            task_output_tool_config(),
            wait_tasks_tool_config(),
            task_tool_config(),
            (&ainxt_build::SchedulerCreateTool).into(),
            (&ainxt_build::SchedulerDeleteTool).into(),
            (&ainxt_build::SchedulerListTool).into(),
            (&ainxt_build::MonitorTool).into(),
...
fn ainxt_build_concise_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            (&ainxt_build_concise::BashConciseTool).into(),
            (&ainxt_build_concise::ReadFileConciseTool).into(),
            (&ainxt_build_concise::SearchReplaceConciseTool).into(),
            (&ainxt_build::ListDirTool).into(),
            (&ainxt_build::GrepTool).into(),
            kill_task_tool_config(),
            (&ainxt_build::TodoWriteTool).into(),
...
pub fn ainxt_build_hashline_toolset(
    hashline_tools: Vec<ainxt_tools::registry::types::ToolConfig>,
) -> ToolServerConfig {
    let mut tools: Vec<ainxt_tools::registry::types::ToolConfig> = vec![bash_tool_config()];
    ...
    tools.extend([
        ...
        wait_tasks_tool_config(),
        task_tool_config(),
        (&ainxt_build::WebSearchTool).into(),
        (&ainxt_build::SchedulerCreateTool).into(),
        (&ainxt_build::SchedulerDeleteTool).into(),
        (&ainxt_build::SchedulerListTool).into(),
        (&ainxt_build::MonitorTool).into(),
...
/// ainxt + plan mode toolset.
///
/// Extends the default `ainxt-build` toolset with plan mode tools:
...
fn ainxt_build_plan_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&ainxt_build::ReadFileTool).into(),
...
/// ainxt + plan mode toolset WITHOUT subagent tools.
///
/// Same as `ainxt_build_plan_toolset` but excludes `TaskTool`,
/// `TaskOutputTool`, and `KillTaskTool`. Use this when the shell
/// does not have subagent infrastructure wired up.
fn ainxt_build_plan_no_subagents_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&ainxt_build::ReadFileTool).into(),
...
/// Default ainxt toolset + `ask_user_question`.
///
/// Same as `default_ainxt_build_toolset` with the `AskUserQuestionTool` added,
/// allowing the agent to ask structured questions without full plan mode.
fn ainxt_build_ask_user_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&ainxt_build::ReadFileTool).into(),
...
/// - `model: ainxt-3-fast` → `Override("ainxt-3-fast")`
...
pub enum BuiltinAgentName {
    ...
    Plan,
    BrowserUse,
    #[strum(serialize = "ainxt-build-orchestrator")]
    AinxtBuildOrchestrator,
}
...
impl BuiltinAgentName {
    ...
    pub fn definition(self) -> AgentDefinition {
        match self {
            Self::AinxtBuild => AgentDefinition::default_ainxt_build(),
            Self::AinxtBuildConcise => AgentDefinition::ainxt_build_concise(),
            Self::AinxtBuildPlan => AgentDefinition::ainxt_build_plan(),
            Self::AinxtBuildPlanNoSubagents => AgentDefinition::ainxt_build_plan_no_subagents(),
            Self::AinxtBuildAskUser => AgentDefinition::ainxt_build_ask_user(),
            ...
            Self::AinxtBuildOrchestrator => AgentDefinition::ainxt_build_orchestrator(),
        }
    }
...
/// Portable agent identity — parsed from .ainxt/agents/*.md.
...
pub struct AgentDefinition {
    ...
    #[serde(default = "default_ainxt_build_toolset")]
    ...
    pub capability_mode: Option<ainxt_tool_types::SubagentCapabilityMode>,
    #[serde(default)]
    ...
    /// unchanged with only the subagent strip applied.
    #[serde(default = "default_true")]
...
pub enum AgentScope {
    /// .ainxt/agents/ (project-level, highest priority)
    Project,
    /// ~/.ainxt/agents/ (user-level)
    User,
    /// ~/.ainxt/bundled/agents/ (lowest-priority bundled cache)
    Bundled,
    /// Built-in agent (e.g., default_ainxt_build(), browser_use()).
    #[default]
    BuiltIn,
}
impl AgentScope {
...
pub enum MemoryScope {
    /// `~/.ainxt/agent-memory/<name>/`
    User,
    /// `<project>/.ainxt/agent-memory/<name>/`
    Project,
    /// `<project>/.ainxt/agent-memory-local/<name>/`
    Local,
}
impl MemoryScope {
    ...
    pub fn resolve_dir(self, agent_name: &str, project_cwd: &std::path::Path) -> ResolvedMemoryDir {
        match self {
            Self::User => ResolvedMemoryDir {
                path: ainxt_config::ainxt_home()
                    .join("agent-memory")
                    .join(agent_name),
                is_project_scoped: false,
            },
            Self::Project => ResolvedMemoryDir {
                path: project_cwd.join(".ainxt/agent-memory").join(agent_name),
                is_project_scoped: true,
            },
            Self::Local => ResolvedMemoryDir {
                path: project_cwd
                    .join(".ainxt/agent-memory-local")
                    .join(agent_name),
                is_project_scoped: true,
            },
        }
...
/// these into `ainxt_tools::registry::types::ToolsetConfig.bash`
...
impl AgentDefinition {
    ...
    fn scope_from_path(path: &Path) -> AgentScope {
        ...
        let ainxt = ainxt_config::user_ainxt_home();
        let home = dirs::home_dir();
        for (dir, scope) in crate::discovery::user_agent_dirs(home.as_deref(), ainxt.as_deref()) {
            if path.starts_with(&dir) {
                return scope;
            }
        }
        if path_str.contains(".ainxt/agents/") || path_str.contains(".ainxt\\agents\\") {
            return AgentScope::Project;
        }
        if path_str.contains(".ainxt/bundled/agents/")
            || path_str.contains(".ainxt\\bundled\\agents\\")
        {
...
impl AgentDefinition {
    ...
    /// (`!inject_default_tools`). Stock `ainxt-build*` agents leave all
    ...
    pub fn override_file_tools(
        &mut self,
        file_tools: Vec<ainxt_tools::registry::types::ToolConfig>,
    ) {
        const FILE_TOOL_SLOTS: &[[&str; 2]] = &[
            ["AinxtBuild:read_file", "AinxtBuildHashline:hashline_read"],
            [
                "AinxtBuild:search_replace",
                "AinxtBuildHashline:hashline_edit",
            ],
            ["AinxtBuild:grep", "AinxtBuildHashline:hashline_grep"],
        ];
    ...
    pub fn builtin_defaults(name: &str, description: &str) -> Self {
        Self {
            ...
            tool_config: default_ainxt_build_toolset(),
    ...
    pub fn default_ainxt_build() -> Self {
        Self::base(
            BuiltinAgentName::AinxtBuild,
            "ainxt agent for software engineering tasks.",
        )
    }
    /// ainxt Concise agent definition — concise output format for SFT/RL.
    pub fn ainxt_build_concise() -> Self {
        Self {
            tool_config: ainxt_build_concise_toolset(),
            agents_md: false,
            ..Self::base(
                BuiltinAgentName::AinxtBuildConcise,
                "ainxt agent with concise output format.",
            )
        }
    }
    /// ainxt agent with plan mode tools.
    pub fn ainxt_build_plan() -> Self {
        Self {
            tool_config: ainxt_build_plan_toolset(),
            ..Self::base(
                BuiltinAgentName::AinxtBuildPlan,
                "ainxt agent with plan mode support.",
            )
        }
    }
    /// ainxt + plan mode WITHOUT subagent tools.
    pub fn ainxt_build_plan_no_subagents() -> Self {
        Self {
            tool_config: ainxt_build_plan_no_subagents_toolset(),
            ..Self::base(
                BuiltinAgentName::AinxtBuildPlanNoSubagents,
                "ainxt agent with plan mode (no subagents).",
            )
        }
    }
    /// Default ainxt agent with the `ask_user_question` tool.
    pub fn ainxt_build_ask_user() -> Self {
        Self {
            tool_config: ainxt_build_ask_user_toolset(),
            ..Self::base(
                BuiltinAgentName::AinxtBuildAskUser,
                "ainxt agent with ask-user-question tool.",
            )
        }
    }
    pub fn codex() -> Self {
    ...
    pub fn general_purpose() -> Self {
        ...
        Self {
            description: ainxt_tool_types::GENERAL_PURPOSE_SUBAGENT
                .description
                .to_string(),
    ...
    pub fn explore() -> Self {
        use crate::prompt::subagent_prompts;
        Self {
            description: ainxt_tool_types::EXPLORE_SUBAGENT.description.to_string(),
    ...
    pub fn plan() -> Self {
        use crate::prompt::subagent_prompts;
        Self {
            description: ainxt_tool_types::PLAN_SUBAGENT.description.to_string(),
    ...
    /// ainxt Orchestrator — GBL model with full AinxtBuild tools
    ...
    pub fn ainxt_build_orchestrator() -> Self {
        Self {
            tool_config: orchestrator_toolset(),
            inject_default_tools: false,
            prompt_body: Some(ORCHESTRATOR_PROMPT_BODY.to_string()),
            ..Self::base(
                BuiltinAgentName::AinxtBuildOrchestrator,
                "AinxtBuild orchestrator that delegates coding to specialized subagents",
            )
        }
    }
    ...
    pub fn from_json(value: &serde_json::Value) -> Result<Self, AgentBuildError> {
        ...
        if !value.get("toolConfig").is_some_and(|v| v.is_object()) {
            def.tool_config = default_ainxt_build_toolset();
        }
...
mod tests {
    ...
    fn presets_select_distinct_toolsets_by_size() {
        let gb = toolset_for_preset("ainxt-build").unwrap();
    ...
    fn ainxt_computer_exclusive_ids() -> Vec<String> {
        #[allow(unused_mut)]
        let mut ids: Vec<String> = vec![
            ToolConfig::from(&ainxt_build::GetTerminalCommandOutputTool).id,
            ToolConfig::from(&ainxt_build::KillTerminalCommandTool).id,
        ];
        ids
    }
    #[test]
    fn ainxt_computer_preset_is_curated_ainxt_build_subset() {
        let gc = toolset_for_preset("ainxt-computer").unwrap();
        let gb = toolset_for_preset("ainxt-build").unwrap();
        let gb_ids: std::collections::HashSet<&str> =
            gb.tools.iter().map(|t| t.id.as_str()).collect();
        let exclusive_ids = ainxt_computer_exclusive_ids();
        ...
        for t in &gc.tools {
            ...
            assert!(
                ...
                "ainxt-computer tool `{}` must also ship in the ainxt-build preset",
                t.id
            );
        }
        assert!(
            gc.tools.len() < gb.tools.len(),
            "ainxt-computer should be a curated subset of ainxt-build"
        );
    }
    #[test]
    fn ainxt_computer_uses_subagent_free_background_task_tools() {
        let gc = toolset_for_preset("ainxt-computer").unwrap();
        let ids: std::collections::HashSet<&str> = gc.tools.iter().map(|t| t.id.as_str()).collect();
        assert!(
            ids.contains(
                ToolConfig::from(&ainxt_build::GetTerminalCommandOutputTool)
                    .id
                    .as_str()
            )
        );
        assert!(
            ids.contains(
                ToolConfig::from(&ainxt_build::KillTerminalCommandTool)
                    .id
                    .as_str()
            )
        );
        assert!(!ids.contains(ToolConfig::from(&ainxt_build::TaskOutputTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&ainxt_build::KillTaskTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&ainxt_build::TaskTool).id.as_str()));
        for t in &gc.tools {
            if t.id == ToolConfig::from(&ainxt_build::GetTerminalCommandOutputTool).id
                || t.id == ToolConfig::from(&ainxt_build::KillTerminalCommandTool).id
            {
                assert!(t.name_override.is_none(), "tool `{}` must not rename", t.id);
            }
        }
    }
    /// The ainxt-computer preset must ship a full-file write tool (legacy
    /// `write_file` equivalence) — the same OpenCode `write` tool the ainxt-build
    ...
    #[test]
    fn ainxt_computer_preset_includes_write_tool() {
        let gc = toolset_for_preset("ainxt-computer").unwrap();
        let write_id = ToolConfig::from(&opencode::OpenCodeWriteTool).id;
        assert!(
            gc.tools.iter().any(|t| t.id == write_id),
            "ainxt-computer preset must include the `{write_id}` tool"
        );
    }
    #[test]
    fn ainxt_computer_preset_excludes_plan_and_lsp() {
        let gc = toolset_for_preset("ainxt-computer").unwrap();
        let gc_ids: std::collections::HashSet<&str> =
            gc.tools.iter().map(|t| t.id.as_str()).collect();
        for excluded in [
            ToolConfig::from(&ainxt_build::LspTool).id,
            ToolConfig::from(&ainxt_build::EnterPlanModeTool).id,
            ToolConfig::from(&ainxt_build::ExitPlanModeTool).id,
        ] {
            assert!(
                !gc_ids.contains(excluded.as_str()),
                "ainxt-computer preset must not advertise `{excluded}`"
            );
        }
        let full = workspace_ainxt_build_toolset();
        ...
        for present in [
            ...
            assert!(
                ...
                "workspace_ainxt_build_toolset must ship `{present}`"
            );
        }
    }
    ...
    fn expected_strict_harness(name: BuiltinAgentName) -> bool {
        match name {
            BuiltinAgentName::Codex | BuiltinAgentName::AinxtBuildOrchestrator => true,
            BuiltinAgentName::AinxtBuild
            | BuiltinAgentName::AinxtBuildConcise
            | BuiltinAgentName::AinxtBuildPlan
            | BuiltinAgentName::AinxtBuildPlanNoSubagents
    ...
    fn is_strict_harness_agent_type_classifies_by_name() {
        for strict in ["codex", "ainxt-build-orchestrator"] {
            assert!(
        ...
        for non_strict in [
            ...
            "ainxt-build-plan",
            "ainxt-build-concise",
            "ainxt-build-ask-user",
            "opencode",
            "browser-use",
            "custom-user-agent",
            "",
            "ainxt-build-totally-made-up",
        ] {
            assert!(
...
"#;
    ...
    fn test_model_override_display_shows_id() {
        ...
        assert_eq!(
            ModelOverride::Override("ainxt-3-fast".to_string()).to_string(),
            "ainxt-3-fast"
        );
    }
    #[test]
...
"#;
        ...
        assert_eq!(def.model, ModelOverride::Override("ainxt-3".to_string()));
    }
    #[test]
...
"#;
    ...
    fn memory_scope_resolve_dir() {
        ...
        assert_eq!(
            proj.path,
            std::path::PathBuf::from("/project/.ainxt/agent-memory/a")
        );
        assert!(proj.is_project_scoped);
        let local = MemoryScope::Local.resolve_dir("a", cwd);
        assert_eq!(
            local.path,
            std::path::PathBuf::from("/project/.ainxt/agent-memory-local/a")
        );
...
"#;
    ...
    #[test]
    fn test_default_tool_config_has_ainxt_build_tools() {
        let content = r#"---
name: default-tools
...
"#;
        ...
        assert!(
            ...
            "default tool_config should have ainxt_build tools"
        );
    }
    #[test]
    ...
    fn test_model_override_serde_explicit_model_id() {
        let yaml = "\"ainxt-3-fast\"";
        let m: ModelOverride = serde_yaml::from_str(yaml).unwrap();
        assert_eq!(m, ModelOverride::Override("ainxt-3-fast".to_string()));
    }
    #[test]
    ...
    fn test_model_override_serialize_override() {
        let m = ModelOverride::Override("ainxt-3-fast".to_string());
        let s = serde_json::to_string(&m).unwrap();
        assert_eq!(s, "\"ainxt-3-fast\"");
    }
    #[test]
    fn test_model_override_in_frontmatter() {
        ...
        assert_eq!(
            def.model,
            ModelOverride::Override("ainxt-3-fast".to_string())
        );
    }
    #[test]
    ...
    fn test_model_override_in_json() {
        let json = serde_json::json!(
            { "name" : "test", "description" : "Test", "model" : "ainxt-code-fast-1" }
        );
        let def = AgentDefinition::from_json(&json).unwrap();
        assert_eq!(
            def.model,
            ModelOverride::Override("ainxt-code-fast-1".to_string())
        );
    }
    #[test]
    fn test_builtin_agent_name_strum_round_trip() {
        use std::str::FromStr;
        for (s, expected) in [
            ("ainxt-build", BuiltinAgentName::AinxtBuild),
            ("ainxt-build-concise", BuiltinAgentName::AinxtBuildConcise),
            ("ainxt-build-ask-user", BuiltinAgentName::AinxtBuildAskUser),
    ...
    fn def_with_template(tpl: crate::prompt::context::TemplateOverride) -> AgentDefinition {
        let mut def = AgentDefinition::default_ainxt_build();
