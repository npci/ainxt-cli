...
/// Maximum byte size of a plugin manifest file accepted for parsing.
///
/// Manifests are small JSON documents — a 64 KiB ceiling is generous for any
/// legitimate plugin while preventing a crafted oversized file from causing
/// excessive memory allocation during `serde_json` deserialization.
...
pub fn load_manifest(plugin_root: &Path) -> Result<ManifestLoadResult, ManifestError> {
    for rel_path in MANIFEST_PATHS {
        ...
        if manifest_path.is_file() {
            ...
            // Security (CWE-502): reject oversized manifests before
            // deserialization to prevent a crafted file from causing
            // excessive memory allocation or stack overflow during parsing.
...
///
/// Replaces `${AINXT_PLUGIN_ROOT}`, `${CLAUDE_PLUGIN_ROOT}`,
/// `${AINXT_PLUGIN_DATA}`, and `${CLAUDE_PLUGIN_DATA}` with the provided values.
///
/// Delegates to [`ainxt_tools::util::substitute_plugin_tokens`], the single
...
pub fn substitute_env_vars(s: &str, plugin_root: &str, plugin_data: &str) -> String {
    ainxt_tools::util::substitute_plugin_tokens(s, Some(plugin_root), Some(plugin_data))
}
...
pub enum ManifestError {
    ...
    #[error("manifest {path} is too large ({size} bytes; limit is {limit} bytes)")]
    TooLarge {
        path: PathBuf,
        size: usize,
        limit: usize,
...
mod tests {
    ...
    fn load_manifest_fallback_paths() {
        ...
        std::fs::create_dir_all(plugin_root.join(".ainxt-plugin")).unwrap();

        // Write manifest in .ainxt-plugin/ fallback location
        std::fs::write(
            plugin_root.join(".ainxt-plugin/plugin.json"),
    ...
    fn load_manifest_rejects_oversized_file() {
        let tmp = tempfile::tempdir().unwrap();
        let plugin_root = tmp.path().join("big-plugin");
        std::fs::create_dir_all(&plugin_root).unwrap();

        // Write a manifest that exceeds MAX_MANIFEST_BYTES.
        // Pad with a large description string inside valid JSON so the
        // file is syntactically correct but over the size limit.
        let padding = "x".repeat(MAX_MANIFEST_BYTES + 1);
        let oversized = format!(r#"{{"name":"big-plugin","description":"{padding}"}}"#);
        std::fs::write(plugin_root.join("plugin.json"), &oversized).unwrap();

        match load_manifest(&plugin_root) {
            Err(ManifestError::TooLarge { size, limit, .. }) => {
    ...
    fn substitute_env_vars_replaces_all() {
        let input = "${AINXT_PLUGIN_ROOT}/bin:${CLAUDE_PLUGIN_ROOT}/lib:${AINXT_PLUGIN_DATA}/cache";
