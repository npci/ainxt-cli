...
//! `ainxt-hooks` runtime.  It pre-filters unsupported events from plugin
...
//! `ainxt-hooks` crate's parser and runtime.

use std::collections::HashMap;
use std::path::{Path, PathBuf};

use ainxt_hooks::config::{HookSpec, parse_hook_file};
...
fn process_hooks_content(
    ...
    for spec in &mut specs {
        ...
        // takes (matches what managed_mcp does for MCP server commands).
        if let Some(cmd) = &spec.command {
            ...
            // `ainxt-hooks::runner::command` for env vars whose
            // values are already known at load time.
            let substituted = substitute_env_vars(&cmd_str, plugin_root, plugin_data);
            let expanded = ainxt_config::expand_env_vars_in_string(&substituted);
            if expanded != cmd_str {
...
mod tests {
    ...
    fn parse_plugin_hooks_from_file() {
        ...
        assert_eq!(
            specs[0].extra_env.get("AINXT_PLUGIN_ROOT").unwrap(),
            "/path/to/plugin"
        );
        assert_eq!(
        ...
        assert_eq!(
            specs[0].extra_env.get("AINXT_PLUGIN_DATA").unwrap(),
            "/path/to/data"
        );
    ...
    /// `${CLAUDE_PLUGIN_ROOT}` (or its `AINXT_PLUGIN_ROOT` alias) must be
    ...
    fn parse_plugin_hooks_substitutes_plugin_root_in_command() {
            ...
            "hooks": {
                "PreToolUse": [
                    {"hooks": [
                        ...
                        {"type": "command", "command": "${AINXT_PLUGIN_ROOT}/hooks/alias.sh"},
        ...
        assert!(
            raws.contains(&"${AINXT_PLUGIN_ROOT}/hooks/alias.sh"),
            "command_raw must preserve the source string verbatim, got {raws:?}"
        );
        assert!(
    ...
    /// `ainxt-hooks::runner::command`, which can fail for hooks
    ...
    /// (CLAUDE_PLUGIN_ROOT, AINXT_PLUGIN_ROOT, CLAUDE_PLUGIN_DATA,
    /// AINXT_PLUGIN_DATA). This preserves the plugin contract while still
    ...
    fn parse_plugin_hooks_user_env_merged_with_plugin_precedence() {
                    ...
                    {"hooks": [
                        {
                            ...
                            "env": {
                                ...
                                "AINXT_PLUGIN_ROOT": "/user/wins?",
                                "CLAUDE_PLUGIN_DATA": "/user/wins?",
                                "AINXT_PLUGIN_DATA": "/user/wins?"
                            }
                        }
                    ]}
                ]
        ...
        // User-declared key the plugin doesn't own: preserved unchanged.
        assert_eq!(
        ...
        // overridden. CLAUDE_PLUGIN_ROOT and AINXT_PLUGIN_ROOT both map
        // to plugin_root; CLAUDE_PLUGIN_DATA and AINXT_PLUGIN_DATA both
        // map to plugin_data.
        for (key, expected) in [
            ("CLAUDE_PLUGIN_ROOT", "/actual/plugin/root"),
            ("AINXT_PLUGIN_ROOT", "/actual/plugin/root"),
            ("CLAUDE_PLUGIN_DATA", "/actual/plugin/data"),
            ("AINXT_PLUGIN_DATA", "/actual/plugin/data"),
        ] {
            assert_eq!(
