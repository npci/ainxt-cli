...
fn promote_tmp_to_dest(tmp: &Path, dest: &Path) -> std::io::Result<()> {
    #[cfg(test)]
    {
        if std::env::var_os("AINXT_TEST_TEST_FAIL_REFRESH_PROMOTE").is_some() {
...
mod tests {
    ...
    ///
    /// # Safety of `set_var` / `remove_var`
    ///
    /// `std::env::set_var` and `std::env::remove_var` are `unsafe` in Rust 1.92+
    /// because concurrent mutation of the process environment is not thread-safe.
    /// Every test that constructs an `EnvVarGuard` is annotated with
    /// `#[serial(home_env)]`, which serialises those tests so they never run
    /// concurrently with each other. The env vars touched here (`HOME`,
    /// `AINXT_TEST_TEST_FAIL_REFRESH_PROMOTE`) are only read by the code under
    /// test, not by any background thread, making the mutation safe in this
    ...
    impl EnvVarGuard {
        fn set(key: &'static str, value: impl AsRef<std::ffi::OsStr>) -> Self {
            ...
            // SAFETY: tests using EnvVarGuard are serialised via
            // #[serial(home_env)], preventing concurrent env mutation.
    ...
    impl Drop for EnvVarGuard {
        fn drop(&mut self) {
            match self.prev.take() {
                // SAFETY: same serial-test guarantee as EnvVarGuard::set.
    ...
    fn refresh_local_install_picks_up_new_agent() {
        ...
        let mut registry = InstallRegistry::empty(home.join(".ainxt").join("installed-plugins"));
        ...
        let trust = TrustStore::load_from(home.join(".ainxt").join("trusted-plugins"));
    ...
    fn refresh_promote_failure_rolls_back_to_prior_snapshot() {
        ...
        let trust = TrustStore::load_from(home.join(".ainxt").join("trusted-plugins"));
        let summary = {
            let _fail = EnvVarGuard::set("AINXT_TEST_TEST_FAIL_REFRESH_PROMOTE", "1");
    ...
    fn refresh_preserves_install_subdir_scope() {
        ...
        let mut registry = InstallRegistry::empty(home.join(".ainxt/installed-plugins"));
    ...
    fn refresh_keeps_stale_when_legacy_subdir_scope_lost() {
        ...
        // Snapshot the full source (matches the install-time copy).
        let install_dir = home.join(".ainxt").join("installed-plugins");
