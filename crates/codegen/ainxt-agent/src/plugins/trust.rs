...
//!
//! Plugins from project directories (`.ainxt/plugins/`, `.claude/plugins/`)
...
//!
//! **Trust storage**: `~/.ainxt/trusted-plugins` (one canonical path per line).
//!
...
/// Name of the trust-store file under `~/.ainxt/`.
...
impl TrustStore {
    ...
    ///
    /// If `~/.ainxt/trusted-plugins` does not exist, returns an empty store.
    /// If the file cannot be read, logs a warning and returns an empty store.
    pub fn load() -> Self {
        // Gate on user_ainxt_home() so a project's `.ainxt/trusted-plugins` is never
        // read as the user trust store when neither AINXT_HOME nor a home dir resolves.
        let Some(ainxt) = ainxt_config::user_ainxt_home() else {
            return Self {
                trusted: HashSet::new(),
                file_path: PathBuf::new(),
            };
        };
        let file_path = ainxt.join(TRUST_FILE_NAME);
    ...
    ///
    /// Canonicalizes the path and appends it to `~/.ainxt/trusted-plugins`.
    ...
    pub fn grant_trust(&mut self, plugin_root: &Path) -> Result<(), TrustError> {
        ...
        // Explicitly release the file handle as soon as the write
        // completes, rather than relying solely on end-of-scope Drop.
        drop(file);
    ...
    /// rewrites `~/.ainxt/trusted-plugins` without the revoked entry.
    ...
    fn rewrite_trust_file(&self) -> Result<(), TrustError> {
        ...
        // Explicitly release the file handle now that all entries are
        // written, rather than relying solely on end-of-scope Drop.
        drop(file);
    ...
    /// trust via `~/.ainxt/trusted-plugins`.
    ...
    fn read_trust_file(path: &Path) -> HashSet<PathBuf> {
        ...
        let trusted: HashSet<PathBuf> = reader
            .lines()
            .filter_map(|line| {
                let line = line.ok()?;
                ...
                // Entries may predate dunce (Windows \\?\ unchanged form); simplify so lookups match.
                Some(dunce::simplified(Path::new(trimmed)).to_path_buf())
            })
            .collect();
        // `lines()` takes ownership of the reader, so the underlying file
        // handle is already released when the iterator is dropped at the end
        // of the `collect()` above.
        trusted
    }
}
...
mod tests {
    ...
    /// Legacy entries written under std canonicalize use the unchanged `\\?\`
