...
//! primitive consumed cross-crate by `ainxt-workspace`, not agent-definition
//! discovery.
...
/// code-exec markers (`.mcp.json`, `.ainxt/config.toml`, `.claude/settings.json`,
...
impl RepoDirChain {
    ...
    pub fn resolve(cwd: &Path) -> Self {
        let git_root = git2::Repository::discover(cwd)
            ...
            // home-level `.ainxt`/`.mcp.json`/plugins would look repo-local. Drop
...
/// from `ainxt-workspace`, which depends on THIS crate) to keep the dep edge
...
mod tests {
    ...
    /// concurrently with each other. The env var touched here (`HOME`) is only
    /// read by the code under test, not by any background thread, making the
    /// mutation safe in this controlled test context.
    struct EnvVarGuard {
        key: &'static str,
