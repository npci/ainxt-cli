...
use std::sync::Arc;
use ainxt_tools::bridge::ToolBridge;
use ainxt_tools::computer::types::{AsyncFileSystem, TerminalBackend};
...
/// The ainxt [`ToolKind`] a vendor-compat `tools:` allowlist entry resolves to, so
/// a plugin's upstream allowlist still binds. Backed by the shared vendor-to-ainxt
/// tool registry in `ainxt-tools` (also used by the hook matcher).
fn claude_tool_kind(name: &str) -> Option<ToolKind> {
    ainxt_tools::types::kind_for(name)
}
...
pub struct AgentBuilder {
    ...
    /// path (e.g., `~/.ainxt/worktrees/project/fork-...-overlay`) that must stay
    ...
    parent_scheduler_handle:
        Option<ainxt_tools::implementations::ainxt_build::scheduler::types::SchedulerHandle>,
    ...
    memory_backend: Option<Arc<dyn ainxt_tools::types::memory_backend::MemoryBackend>>,
    web_search_config: ainxt_tools::implementations::web_search::WebSearchConfig,
    ...
    backend_search: bool,
    web_fetch_config: ainxt_tools::implementations::ainxt_build::web_fetch::WebFetchConfig,
    lsp: Option<std::sync::Arc<dyn ainxt_tools::implementations::lsp::LspBackend>>,
    image_gen_config: ainxt_tools::implementations::ainxt_build::image_gen::ImageGenConfig,
    video_gen_config: ainxt_tools::implementations::ainxt_build::video_gen::VideoGenConfig,
    ...
    compat: ainxt_tools::types::compat::CompatConfig,
    ...
    api_key_provider: Option<ainxt_tools::types::SharedApiKeyProvider>,
    attribution_callback: Option<ainxt_tools::SharedAttributionCallback>,
    ...
    /// `resolve_max_mcp_output_bytes_for_cwd` in ainxt-shell).
    ...
    preloaded_skills: Option<Vec<ainxt_tools::implementations::skills::types::SkillInfo>>,
}
/// Ensure plan mode tools (`enter_plan_mode`, `exit_plan_mode`,
/// `ask_user_question`) are present in the tool config.
fn ensure_plan_mode_tools(tool_config: &mut ainxt_tools::registry::types::ToolServerConfig) {
    use ainxt_tools::implementations::ainxt_build;
    let existing: std::collections::HashSet<&str> =
        tool_config.tools.iter().map(|tc| tc.id.as_str()).collect();
    let missing_enter = !existing.contains("AinxtBuild:enter_plan_mode");
    let missing_exit = !existing.contains("AinxtBuild:exit_plan_mode");
    let missing_ask = !existing.contains("AinxtBuild:ask_user_question");
    drop(existing);
    if missing_enter {
        tool_config
            .tools
            .push((&ainxt_build::EnterPlanModeTool).into());
    }
    if missing_exit {
        tool_config
            .tools
            .push((&ainxt_build::ExitPlanModeTool).into());
    }
    if missing_ask {
        tool_config
            .tools
            .push((&ainxt_build::AskUserQuestionTool).into());
    }
}
...
fn merge_tool_params(
    tool_config: &mut ainxt_tools::registry::types::ToolServerConfig,
    ids: &[&str],
...
impl AgentBuilder {
    pub fn new(
        ...
        Self {
            ...
            terminal_backend,
            fs_backend: Arc::new(ainxt_tools::computer::local::LocalFs),
            notification_handle,
    ...
    pub fn with_preloaded_skills(
        mut self,
        skills: Vec<ainxt_tools::implementations::skills::types::SkillInfo>,
    ) -> Self {
    ...
    pub fn with_memory_backend(
        mut self,
        backend: Arc<dyn ainxt_tools::types::memory_backend::MemoryBackend>,
    ) -> Self {
    ...
    pub fn with_parent_scheduler_handle(
        mut self,
        handle: ainxt_tools::implementations::ainxt_build::scheduler::types::SchedulerHandle,
    ) -> Self {
    ...
    pub fn with_web_search_config(
        mut self,
        config: ainxt_tools::implementations::web_search::WebSearchConfig,
    ) -> Self {
    ...
    /// `AINXT_WEB_FETCH` env var.
    pub fn with_web_fetch_config(
        mut self,
        config: ainxt_tools::implementations::ainxt_build::web_fetch::WebFetchConfig,
    ) -> Self {
    ...
    pub fn with_lsp(
        mut self,
        handle: std::sync::Arc<dyn ainxt_tools::implementations::lsp::LspBackend>,
    ) -> Self {
    ...
    /// allowing image generation via the ainxt ainxt image API with session
    /// credentials. When `Disabled` (default), the tool is not registered.
    pub fn with_image_gen_config(
        mut self,
        config: ainxt_tools::implementations::ainxt_build::image_gen::ImageGenConfig,
    ) -> Self {
    ...
    /// allowing video generation via the ainxt Video Generation API with
    /// session credentials. When `Disabled` (default), the tool is not
    /// registered.
    pub fn with_video_gen_config(
        mut self,
        config: ainxt_tools::implementations::ainxt_build::video_gen::VideoGenConfig,
    ) -> Self {
    ...
    pub fn with_app_builder_deployer_config(
        mut self,
        config: ainxt_tools::implementations::ainxt_build::deploy_app::AppBuilderDeployerConfig,
    ) -> Self {
    ...
    pub fn with_api_key_provider(
        mut self,
        provider: ainxt_tools::types::SharedApiKeyProvider,
    ) -> Self {
    ...
    /// Set public model slugs advertised in the AinxtBuild Task description.
    ...
    ///
    /// When disabled, `AinxtBuild:ask_user_question` is stripped from the
    ...
    /// Without this, only auto-discovered skills (cwd/.ainxt/skills, ~/.ainxt/skills)
    /// are included — custom paths added via `ainxt.dev/skills/add` would be ignored.
    ...
    pub async fn build(mut self) -> Result<Agent, AgentBuildError> {
        ...
        if definition.inject_default_tools {
            ...
            if self.web_search_config.is_enabled() {
                use ainxt_tools::implementations::ainxt_build;
                tool_config.tools.push((&ainxt_build::WebSearchTool).into());
            }
            if self.web_fetch_config.is_enabled() {
                use ainxt_tools::implementations::ainxt_build;
                tool_config.tools.push((&ainxt_build::WebFetchTool).into());
            }
            if self.lsp.is_some() {
                tool_config
                    .tools
                    .push((&ainxt_tools::implementations::ainxt_build::LspTool).into());
            }
            ...
            if self.write_file_enabled && !has_write_tool {
                tool_config
                    .tools
                    .push((&ainxt_tools::implementations::opencode::OpenCodeWriteTool).into());
            }
            ensure_plan_mode_tools(&mut tool_config);
        }
        if self.memory_backend.is_none() {
            let ainxt_build_ns = ainxt_tools::types::tool::ToolNamespace::AinxtBuild.to_string();
            let mem_search_id = format!(
                "{ainxt_build_ns}:{}",
                ainxt_tools::implementations::memory::MEMORY_SEARCH_TOOL_NAME
            );
            let mem_get_id = format!(
                "{ainxt_build_ns}:{}",
                ainxt_tools::implementations::memory::MEMORY_GET_TOOL_NAME
            );
            tool_config
                .tools
        ...
        if !self.ask_user_question_enabled {
            let ask_user_id = format!(
                "{}:ask_user_question",
                ainxt_tools::types::tool::ToolNamespace::AinxtBuild,
            );
        ...
        if task_stripped {
            use ainxt_tools::types::tool::ToolNamespace;
            ...
            if !has_satisfier(ToolNamespace::AinxtBuild, "run_terminal_cmd", true)
                && !has_satisfier(ToolNamespace::AinxtBuildConcise, "run_terminal_cmd", true)
        ...
        if let ainxt_tools::implementations::ainxt_build::web_fetch::WebFetchConfig::Enabled {
            ref params,
        } = self.web_fetch_config
            && let Ok(params_value) = serde_json::to_value(params)
            && let Some(obj) = params_value.as_object()
        {
            merge_tool_params(&mut tool_config, &["AinxtBuild:web_fetch"], obj);
        }
        if let Some(ref bash_params) = self.bash_params_json {
            merge_tool_params(
                ...
                &[
                    ...
                    "AinxtBuildConcise:run_terminal_cmd",
                ],
                bash_params,
            );
        }
        ...
        if !definition.tools.is_empty() {
            ...
            if unresolved.is_empty() {
                ...
                tracing::warn!(
                    ...
                    definition.tools,
                    "tools allowlist had unmappable entries; keeping full ainxt toolset"
                );
            }
        }
        tool_config
        ...
        if let Some(bytes) = self.mcp_max_output_bytes {
            tool_bridge.toolset().resources.lock().await.insert(
                ainxt_tools::types::resources::TruncationCfg(
                    ainxt_tools::types::context::TruncationConfig {
        ...
        if use_backend_search {
            if web_search_enabled && definition.hosted_tool_allowed("web_search") {
                hosted_tools.push(ainxt_sampling_types::HostedTool::WebSearch {
                    allowed_domains: None,
                });
            }
            if definition.hosted_tool_allowed("x_search") {
                hosted_tools.push(ainxt_sampling_types::HostedTool::XSearch);
            }
        }
...
/// CLI naming for the shared [`ainxt_tool_types::build_task_description`] builder.
const TASK_TOOL_NAMING: ainxt_tool_types::TaskToolNaming<'static> = ainxt_tool_types::TaskToolNaming {
...
/// CLI [`ainxt_tool_types::SubagentToolNaming`]: each kind maps to its
...
const SUBAGENT_TOOL_NAMING: ainxt_tool_types::SubagentToolNaming<'static> =
    ainxt_tool_types::SubagentToolNaming {
...
/// shared [`ainxt_tool_types`] catalog and rendered with [`SUBAGENT_TOOL_NAMING`]
...
fn builtin_tools_fragment(name: BuiltinAgentName) -> String {
    let subagent = match name {
        BuiltinAgentName::GeneralPurpose => ainxt_tool_types::GENERAL_PURPOSE_SUBAGENT,
        BuiltinAgentName::Explore => ainxt_tool_types::EXPLORE_SUBAGENT,
        BuiltinAgentName::Plan => ainxt_tool_types::PLAN_SUBAGENT,
        _ => return String::new(),
    };
...
/// [`ainxt_tool_types::SubagentDescriptor`] and defers to
/// [`ainxt_tool_types::build_task_description`] so the CLI and the prod chat
...
/// `description` is used unchanged (markdown is fine — it's model-facing text).
pub(crate) fn build_task_description(
    subagents: &[SubagentEntry],
    model_slugs: &[String],
) -> String {
    let descriptors: Vec<ainxt_tool_types::SubagentDescriptor> = subagents
        .iter()
        .map(|entry| {
            ...
            ainxt_tool_types::SubagentDescriptor {
        ...
        .collect();
    let mut description = ainxt_tool_types::build_task_description(&descriptors, &TASK_TOOL_NAMING);
...
fn resolve_shell_for_prompt() -> String {
    ...
    #[cfg(not(unix))]
    {
        ainxt_config::shell::detect_windows_shell()
            .name()
            .to_string()
    }
}
...
mod tests {
    ...
    fn entry(name: &str, desc: &str, source: SubagentSource) -> SubagentEntry {
        SubagentEntry {
            ...
            config_source: ainxt_tools::types::config_source::ConfigSource::Builtin,
        }
    }
    #[test]
    fn build_task_description_builtin_includes_tools() {
        ...
        assert!(
            desc.contains(ainxt_tool_types::GENERAL_PURPOSE_SUBAGENT.tools_template),
            "should include general-purpose tool names"
        );
        assert!(
            desc.contains(ainxt_tool_types::EXPLORE_SUBAGENT.tools_template),
    ...
    fn build_task_description_shadowed_builtin_uses_user_desc() {
        let subagents = vec![SubagentEntry {
            ...
            config_source: ainxt_tools::types::config_source::ConfigSource::Project {
    ...
    fn build_task_description_lists_public_model_slugs() {
        ...
        assert!(!desc.contains(concat!("ainxt", " models")));
    }
    #[test]
    ...
    fn task_model_guidance_resolves_model_param_override() {
        use ainxt_tools::types::template_renderer::TemplateRenderer;
        use ainxt_tools::types::tool::ToolKind;
    ...
    async fn build_pager_agent(
        ...
        use ainxt_tools::computer::local::LocalTerminalBackend;
        use ainxt_tools::notification::ToolNotificationHandle;
        AgentBuilder::new(
            std::env::temp_dir(),
    ...
    async fn pager_flag_combinations_satisfy_tool_invariants() {
        ...
        let cases: &[PagerFlagCase] = &[
            PagerFlagCase {
                label: "ainxt-build / subagents+ask_user",
                profile: AgentDefinition::default_ainxt_build,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "ainxt-build / subagents / no-ask-user",
                profile: AgentDefinition::default_ainxt_build,
                subagents: true,
                ask_user: false,
            },
            PagerFlagCase {
                label: "ainxt-build / no-subagents / ask_user",
                profile: AgentDefinition::default_ainxt_build,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "ainxt-build / no-subagents / no-ask-user",
                profile: AgentDefinition::default_ainxt_build,
                subagents: false,
                ask_user: false,
            },
            PagerFlagCase {
                label: "ainxt-build-ask-user / subagents",
                profile: AgentDefinition::ainxt_build_ask_user,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "ainxt-build-ask-user / no-subagents",
                profile: AgentDefinition::ainxt_build_ask_user,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "ainxt-build-plan",
                profile: AgentDefinition::ainxt_build_plan,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "ainxt-build-plan / no-ask-user",
                profile: AgentDefinition::ainxt_build_plan,
                subagents: true,
                ask_user: false,
            },
            PagerFlagCase {
                label: "ainxt-build-plan-no-subagents",
                profile: AgentDefinition::ainxt_build_plan_no_subagents,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "ainxt-build-plan-no-subagents / no-ask-user",
                profile: AgentDefinition::ainxt_build_plan_no_subagents,
                subagents: false,
                ask_user: false,
            },
    ...
    async fn curated_empty_toolset_fails_agent_build() {
        use ainxt_tools::computer::local::LocalTerminalBackend;
        use ainxt_tools::notification::ToolNotificationHandle;
        let mut profile = crate::config::AgentDefinition::default_ainxt_build();
    ...
    async fn plan_mode_injected_ask_user_question_receives_params() {
        use ainxt_tools::computer::local::LocalTerminalBackend;
        use ainxt_tools::implementations::ainxt_build::ask_user_question::AskUserQuestionParams;
        use ainxt_tools::notification::ToolNotificationHandle;
        use ainxt_tools::types::resources::Params;
        let profile = crate::config::AgentDefinition::default_ainxt_build();
        assert!(
            !profile
                .tool_config
                .tools
                .iter()
                .any(|tc| tc.id == "AinxtBuild:ask_user_question"),
    ...
    async fn build_with_tools(tools: Vec<String>, disallowed: Vec<String>) -> crate::agent::Agent {
        use ainxt_tools::computer::local::LocalTerminalBackend;
        use ainxt_tools::notification::ToolNotificationHandle;
        let mut def = crate::config::AgentDefinition::default_ainxt_build();
        def.tools = tools;
    ...
    fn session_tools_allowed_clamp() {
        ...
        assert!(def.session_tools_allowed("AinxtBuild:read_file"));
    ...
    async fn spawning_blocked_disables_all_background_bash_modes() {
        use ainxt_tools::computer::local::LocalTerminalBackend;
        use ainxt_tools::implementations::ainxt_build::bash::BashParams;
        use ainxt_tools::notification::ToolNotificationHandle;
        use ainxt_tools::types::resources::Params;
        let mut definition = crate::config::AgentDefinition::default_ainxt_build();
    ...
    /// Compat allowlist names (`Read`, `Bash`, `Grep`) map to their ainxt
    /// equivalents by `ToolKind` — a real restricted toolset, not zero tools.
    #[tokio::test]
    async fn claude_tool_names_map_to_ainxt_equivalents() {
    ...
    /// Shell, LSP, ask, and task-lifecycle tool names resolve to their ainxt
    ...
    /// treat `mcp__*` as always-on, so ainxt keeps the MCP meta-tools
    ...
    async fn requested_enabled_web_tools_survive_allowlist() {
        ...
        use ainxt_tools::implementations::web_search::WebSearchConfig;
        use ainxt_tools::notification::ToolNotificationHandle;
        let mut definition = crate::config::AgentDefinition::default_ainxt_build();
    ...
    /// ainxt-build toolsets have no Skill tool — skills are read from
    ...
    /// Compat `ToolSearch` meta-tool maps to ainxt's `search_tool` (MCP
    ...
    async fn disallowed_web_search_strips_function_and_hosted_tools() {
        ...
        assert!(
            !hosted
                .iter()
                .any(|t| matches!(t, ainxt_sampling_types::HostedTool::WebSearch { .. })),
            "hosted WebSearch must be removed when web_search is disallowed, got: {hosted:?}"
        );
        assert!(
            hosted
                .iter()
                .any(|t| matches!(t, ainxt_sampling_types::HostedTool::XSearch)),
