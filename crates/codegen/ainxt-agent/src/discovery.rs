...
//!
//! Searches `.ainxt/agents/` and `.claude/agents/` from cwd to repo root,
//! then `~/.ainxt/agents/`, then `~/.claude/agents/`. Name-based dedup keeps
//! highest priority.
...
/// Project-level agent directories to scan (`.ainxt/agents/` + `.claude/agents/` compat).
const PROJECT_AGENT_SUBDIRS: &[&str] = &[".ainxt/agents", ".claude/agents"];

/// Existing project-level agent dirs (`.ainxt/agents` / `.claude/agents`), walked
/// from `cwd` up to the git worktree root (inclusive). Returns
/// `(existing dirs, git_root)`. Matches [`crate::plugins::project_plugin_dirs`].
...
/// Existing project agent dirs (`.ainxt/agents` / `.claude/agents`) under each
...
pub fn all_subagents(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<SubagentEntry> {
    ...
    all_subagents_with_home(cwd, toggle, dirs::home_dir().as_deref(), ainxt.as_deref())
}
...
fn all_subagents_with_home(
    ...
    let discovered = discover_with_home(cwd, home, ainxt_home);
...
fn merge_subagents(
    ...
    //
    // A user-level ~/.ainxt/agents/explore.md does NOT shadow built-in explore
...
/// 1. `.ainxt/agents/` walking from `cwd` up to repo root
/// 2. `~/.ainxt/agents/` (user-level)
/// 3. `~/.claude/agents/` (compat user-level)
/// 4. `~/.ainxt/bundled/agents/` (bundled, lowest priority)
///
/// Deduplicates by name — higher-priority definitions win.
/// User-level agent directories in priority order: user ainxt agents, `.claude`
/// compat agents, then bundled. `.ainxt` dirs resolve from `ainxt_home`
/// (AINXT_HOME-aware) plus the legacy literal `~/.ainxt` when AINXT_HOME points
/// elsewhere; `.claude` resolves from `home`.
pub(crate) fn user_agent_dirs(
    home: Option<&Path>,
    ainxt_home: Option<&Path>,
) -> Vec<(std::path::PathBuf, AgentScope)> {
    // Legacy literal ~/.ainxt, included only when it differs from ainxt_home
    // (i.e. AINXT_HOME points elsewhere) so agents left in the old location are
    // still discovered and stay consistent with scope_from_path classification.
    let legacy_ainxt = home
        .map(|h| h.join(".ainxt"))
        .filter(|legacy| ainxt_home != Some(legacy.as_path()));

    let mut dirs = Vec::new();
    if let Some(g) = ainxt_home {
        dirs.push((g.join("agents"), AgentScope::User));
    }
    if let Some(l) = &legacy_ainxt {
...
pub fn discover(cwd: &Path) -> Vec<AgentDefinition> {
    let ainxt = ainxt_config::user_ainxt_home();
    discover_with_home(cwd, dirs::home_dir().as_deref(), ainxt.as_deref())
}
...
fn discover_with_home(
    ...
    for (dir, scope) in user_agent_dirs(home, ainxt_home) {
        if dir.is_dir() {
...
pub fn by_name(name: &str) -> Option<AgentDefinition> {
    let ainxt = ainxt_config::user_ainxt_home();
    by_name_with_home(name, dirs::home_dir().as_deref(), ainxt.as_deref())
}
...
fn by_name_with_home(
    ...
    {
        let home_dirs = user_agent_dirs(home, ainxt_home);
...
///
/// Project-level `.ainxt/agents/` has highest priority, then falls back
/// to built-ins, user-level, and finally bundled definitions.
pub fn by_name_in_cwd(name: &str, cwd: &Path) -> Option<AgentDefinition> {
    let ainxt = ainxt_config::user_ainxt_home();
    by_name_in_cwd_with_home(name, cwd, dirs::home_dir().as_deref(), ainxt.as_deref())
}
...
fn by_name_in_cwd_with_home(
    ...
    by_name_with_home(name, home, ainxt_home)
}
...
fn by_name_in_cwd_with_plugins_and_home(
    ...
    if let Some(def) = by_name_in_cwd_with_home(name, cwd, home, ainxt_home) {
        return Some(def);
    }
...
/// Expand `${CLAUDE_PLUGIN_ROOT}` / `${CLAUDE_PLUGIN_DATA}` (and the ainxt
...
/// Load project agent definitions from every `.ainxt/agents` / `.claude/agents`
...
mod tests {
    ...
    fn test_origin_for_scope(scope: PluginScope) -> crate::plugins::PluginOrigin {
        ...
        match scope {
            ...
            PluginScope::Project => PluginOrigin::ProjectAinxt,
            PluginScope::User => PluginOrigin::UserAinxt,
    ...
    #[test]
    fn user_agent_dirs_includes_legacy_ainxt_when_ainxt_home_differs() {
        let home = Path::new("/home/u");
        let ainxt = Path::new("/custom/ainxthome");
        let paths: Vec<_> = user_agent_dirs(Some(home), Some(ainxt))
            .into_iter()
            .map(|(p, _)| p)
            .collect();
        assert!(paths.contains(&ainxt.join("agents")));
        assert!(paths.contains(&home.join(".ainxt").join("agents")));
        assert!(paths.contains(&home.join(".claude").join("agents")));
        assert!(paths.contains(&ainxt.join("bundled").join("agents")));
        assert!(paths.contains(&home.join(".ainxt").join("bundled").join("agents")));
    }

    #[test]
    fn user_agent_dirs_dedups_legacy_when_ainxt_home_is_dot_ainxt() {
        let home = Path::new("/home/u");
        let ainxt = home.join(".ainxt");
        let count = user_agent_dirs(Some(home), Some(&ainxt))
            .into_iter()
            .filter(|(p, _)| *p == ainxt.join("agents"))
            .count();
        assert_eq!(
            count, 1,
            "no duplicate ~/.ainxt/agents when ainxt_home == ~/.ainxt"
        );
    }
    ...
    #[test]
    fn test_by_name_builtin_ainxt_build() {
        let def = by_name("ainxt-build");
        assert!(def.is_some());
        assert_eq!(def.unwrap().name, "ainxt-build");
    }
    ...
    fn test_discover_dedup_by_name() {
        ...
        let agents_dir_1 = tmp.path().join(".ainxt").join("agents");
        let agents_dir_2 = inner_dir.join(".ainxt").join("agents");
    ...
    fn test_discover_includes_bundled_agents_at_lowest_priority() {
        ...
        let bundled_dir = home.join(".ainxt").join("bundled").join("agents");
        ...
        let defs = discover_with_home(&cwd, Some(&home), Some(&home.join(".ainxt")));
        assert_eq!(defs.len(), 1);
    ...
    fn test_by_name_in_cwd_uses_bundled_when_no_higher_priority_match_exists() {
        ...
        let def =
            by_name_in_cwd_with_home("bundled-only", &cwd, Some(&home), Some(&home.join(".ainxt")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_user_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let user_dir = home.join(".ainxt").join("agents");
        let bundled_dir = home.join(".ainxt").join("bundled").join("agents");
        fs::create_dir_all(&cwd).unwrap();
        ...
        let def =
            by_name_in_cwd_with_home("reviewer", &cwd, Some(&home), Some(&home.join(".ainxt")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_builtin_beats_bundled_for_builtin_names() {
        ...
        let def = by_name_in_cwd_with_home("explore", &cwd, Some(&home), Some(&home.join(".ainxt")))
            .unwrap();
    ...
    fn test_by_name_in_cwd_project_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let project_dir = cwd.join(".ainxt").join("agents");
        let bundled_dir = home.join(".ainxt").join("bundled").join("agents");
    ...
    fn test_by_name_in_cwd_project_shadows_builtin() {
        ...
        // Create a project-level "ainxt-build" that shadows the built-in
        write_agent_file(
            &agents_dir,
            "ainxt-build.md",
            "ainxt-build",
        ...
        let def = by_name_in_cwd("ainxt-build", tmp.path());
        assert!(def.is_some());
        let def = def.unwrap();
        assert_eq!(def.name, "ainxt-build");
        assert_eq!(def.description, "Custom ainxt-build");
    }

    #[test]
    fn test_by_name_in_cwd_falls_back_to_builtin() {
        let tmp = tempfile::tempdir().unwrap();
        // No .ainxt/agents/ directory — should fall back to built-in
    ...
    fn test_orchestrator_from_str_resolves() {
        use std::str::FromStr;
        let variant = BuiltinAgentName::from_str("ainxt-build-orchestrator")
            .expect("from_str must resolve ainxt-build-orchestrator");
        assert_eq!(variant, BuiltinAgentName::AinxtBuildOrchestrator);
        let def = variant.definition();
        assert_eq!(def.name, "ainxt-build-orchestrator");
        assert!(
    ...
    fn test_orchestrator_by_name_in_cwd() {
        let tmp = tempfile::tempdir().unwrap();
        let def = by_name_in_cwd("ainxt-build-orchestrator", tmp.path())
            .expect("by_name_in_cwd must find ainxt-build-orchestrator");
        assert_eq!(def.name, "ainxt-build-orchestrator");
    ...
    fn test_merge_user_level_builtin_name_is_skipped() {
        // A user-level (~/.ainxt/agents/) agent named "explore" should NOT shadow
    ...
    fn test_plugin_agent_body_resolves_plugin_root() {
        ...
        fs::create_dir_all(&plugin_agents).unwrap();
        // Matches how enterprise plugin-dev agents reference the plugin root.
