...
//! `TemplateRenderer` in `ainxt-tools`. This struct does NOT own a
...
pub struct PromptContext {
    ...
    /// Identity in the primary ainxt-build system prompt (`You are <label>…`).
    /// Not the UI picker name. Defaults to [`DEFAULT_SYSTEM_PROMPT_LABEL`].
    #[serde(default = "default_system_prompt_label")]
    pub system_prompt_label: String,
}
/// Default assistant identity used in the system prompt when the gateway does
/// not supply a per-model `systemPromptLabel`. The gateway's value always wins;
/// this is only the fallback so the assistant still has a name offline.
pub const DEFAULT_SYSTEM_PROMPT_LABEL: &str = "AiNxt CLI";
...
mod tests {
    ...
    fn test_placeholders_system_prompt_label_override() {
        let mut ctx = test_context();
        ctx.system_prompt_label = "ainxt Internal".into();
        let p = ctx.placeholders();
        assert_eq!(p["system_prompt_label"], "ainxt Internal");
    }
    #[test]
    ...
    fn test_agents_md_paths_use_display_cwd_in_rendered_section() {
        ...
        let overlay_path = "/root/.ainxt/worktrees/my-project/ab-123-a-overlay";
    ...
    fn trimmed_prompts_no_redundant_identity() {
        ...
        for (name, prompt) in [
            ...
            assert!(
                !prompt.contains("You are a ainxt agent"),
