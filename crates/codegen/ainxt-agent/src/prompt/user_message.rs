...
//!
//! Matches `prompt::context::PromptContext` but for the first user message
...
//! - `Default` -- the legacy ainxt prefix (built by the shell layer).
...
use ainxt_tools::types::skill_discovery_tracker::{XmlRenderMode, format_announcement_xml};
...
pub enum UserMessageTemplate {
    /// Legacy ainxt prefix: `<user_info>` + optional `<git_status>`.
...
/// One discovered rule file (AGENTS.md / Claude.md / .ainxt/rules/*.md).
///
...
pub struct UserMessageContext {
    ...
    /// string should use `ainxt_shell::util::uname::os_kernel_and_release`
    /// (or equivalent).
    ...
    /// User-scoped rule files (~/.ainxt/, ~/.claude/).
    ...
    /// (`~/.ainxt/projects/<encoded-cwd>/mcps`). Surfaced in
...
struct UserMessagePlaceholders<'a> {
    ...
    /// just substitutes this unchanged. See `render_skill_listing_xml` for
