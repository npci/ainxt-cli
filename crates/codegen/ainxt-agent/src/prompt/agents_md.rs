...
//!
//! Searches from cwd to repo root, plus `~/.ainxt/`. Also discovers
//! `*.md` files in `.ainxt/rules/` and `.claude/rules/` directories.
...
/// Find `*.md` files in `.ainxt/rules/`, `.claude/rules/`, and `.cursor/rules/`,
...
/// Read Agents.md from ~/.ainxt/, git repo root, and session cwd.
...
async fn read_agents_config_with_options(
    ...
    let global_dir = ainxt_tools::util::ainxt_home::ainxt_home();
    ...
    // Always include ainxt_home (~/.ainxt/) first, then ~/.claude/ and ~/.cursor/
...
/// Unchanged leading bytes [`render_agents_md`] emits for every reminder block.
/// Used by `ainxt-shell` to structurally detect legacy untagged AGENTS.md
...
fn render_agents_md(configs: &[AgentConfigFile]) -> Option<String> {
    ...
    for config in configs {
        ...
        // .ainxt/rules/*.md) so globs/paths metadata doesn't leak into the
        // system prompt as raw YAML.
        let is_rules_file = config.file_path.contains("/.ainxt/rules/")
            || config.file_path.contains("/.claude/rules/");
        let content = if is_rules_file {
            ainxt_tools::implementations::skills::skill::extract_skill_body(&config.content)
        } else {
...
mod tests {
    ...
    fn format_agents_md_section_delivers_full_content() {
        ...
        let section = format_agents_md_section(&configs).unwrap();
        // No cap: the full content is delivered unchanged, with no truncation marker.
        assert!(
