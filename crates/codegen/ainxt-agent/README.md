...
This crate extracts a first-class `Agent` type from `ainxt-shell`.
...
`ainxt-shell`, another in-process host, or a headless batch runner.
...
in `.ainxt/agents/` (project-level) or `~/.ainxt/agents/` (user-level).

```rust
use ainxt_agent::{AgentDefinition, AgentBuilder};
use ainxt_tools::notification::ToolNotificationHandle;

// 1. Parse the definition file
let def = AgentDefinition::from_file(".ainxt/agents/code-reviewer.md")?;
...
```rust
use ainxt_agent::discovery;

// Find all .md files in .ainxt/agents/ directories
...
└────────▲─────────┘
         │ depends on
┌────────┴─────────┐
│  ainxt-shell  │  uses AgentBuilder to create
│  (session host)  │  Agent during session setup
└──────────────────┘
```

- **`ainxt-tools`**: Provides `ToolBridge`, `ToolRegistry`,
  `ToolState`, `SystemReminderLayer`, and tool implementations.
  `ainxt-agent` depends on it for tool setup.
- **`ainxt-shell`**: The application shell. Uses `AgentBuilder`
  to construct an `Agent` during session creation. The shell
  re-exports some modules from `ainxt-agent` (AGENTS.md
...
|---|---|---|
| `ainxt-build` | extend | Default agent for software engineering tasks |
...
```bash
# Check
cargo check -p ainxt-agent

# Test
cargo test -p ainxt-agent

# Clippy
cargo clippy -p ainxt-agent --fix --allow-dirty
