...
//!
//! `ainxt_hooks` pulls in `git2`/`reqwest`/`ainxt-tools`, too heavy for
...
/// Wire mirror of `ainxt_hooks::discovery::HookRegistry`.
///
...
/// Wire mirror of `ainxt_hooks::config::HookSpec`.
///
...
/// `ainxt-hooks`, so a server-side test (`ainxt-workspace`'s
...
/// Wire mirror of `ainxt_hooks::event::HookEventName`.
///
...
pub enum HookEventNameWire {
    ...
    PreCompact,
    PostCompact,
    /// An event string this client does not know, preserved unchanged.
    Unknown(String),
}
...
impl<'de> Deserialize<'de> for HookEventNameWire {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        Ok(match s.as_str() {
            ...
            // Forward-tolerant: preserve an unknown event unchanged.
            _ => Self::Unknown(s),
        })
    }
}
...
mod tests {
    ...
    fn hook_event_name_wire_snake_case_round_trip() {
        // All 15 variants (matches upstream `event_name_deser_all_variants`).
        for (variant, wire) in [
