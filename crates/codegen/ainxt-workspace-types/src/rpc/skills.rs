...
//! `ainxt-tools/src/implementations/skills/types.rs` (the type the
...
pub enum SkillScope {
    /// cwd/.ainxt/skills
    Local,
    /// repo_root/.ainxt/skills
    Repo,
    /// ~/.ainxt/skills
    User,
    /// ~/.ainxt/server-skills (synced from the skill store)
    Server,
    /// platform built-in skills
    Bundled,
    /// plugin-provided skills
    Plugin,
    /// A scope value this client does not know, preserved unchanged.
    Unknown(String),
}
...
mod tests {
    ...
    fn skill_info_deserializes_minimal_payload() {
        let raw = serde_json::json!({
            ...
            "path": "/workspace/.ainxt/skills/my-skill/SKILL.md",
            "scope": "local",
        });
    ...
    // Fixture mirrored field-for-field from the ainxt-tools SkillInfo
    ...
    fn skill_info_deserializes_full_payload() {
        let raw = serde_json::json!({
            ...
            "metadata": {"team": "infra"},
            "path": "/root/.ainxt/server-skills/deploy/SKILL.md",
            "scope": "server",
            "config_source": {"type": "user", "path": "/root/.ainxt/skills"},
            "plugin_name": "infra-plugin",
            "plugin_version": "1.0.0",
            "plugin_root": "/root/.ainxt/plugins/infra-plugin",
            "plugin_data": "/root/.ainxt/plugin-data/infra-plugin",
            "allowed_tools": ["bash"],
            "model": "ainxt-4",
            "effort": "high",
            "user_invocable": true,
