...
//! the shell's `ainxt.dev/fs/*` ACP methods.
...
// types exactly so the ACP wire contract is unchanged.
...
// workspace server (`ainxt-workspace`) and the ainxt.dev backend
...
// `workspace.fs_*` ops above already serve the shell's `ainxt.dev/fs/*`
...
/// Filesystem node kind. Wire values match the shell's `ainxt.dev/fs/list`
/// node `type` strings.
...
/// ACP-compatible list request (camelCase wire format, matches
/// `ainxt.dev/fs/list` plus `offset` pagination).
