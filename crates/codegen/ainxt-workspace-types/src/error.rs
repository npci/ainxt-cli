...
//!   [`IoKind`] enum that matches every currently-stable variant of
//!   [`std::io::ErrorKind`]. Conversion from `std::io::Error` happens
//!   manually at the workspace-crate boundary via
//!   [`WorkspaceError::from_io`].
//!
//! - **`Tool`**: a natural definition would be `Tool(#[from] ainxt_tools::ToolError)`.
//!   That coupling would force this crate to depend on
//!   `ainxt-tools`, which would defeat the lightweight
...
/// notably `std::io::Error` and `ainxt_tools::ToolError`) happens
/// at the workspace-crate boundary -- see this module's doc comment.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize, Error)]
#[serde(tag = "type", content = "data", rename_all = "snake_case")]
pub enum WorkspaceError {
    /// Filesystem I/O failure. Matches `std::io::Error` shape.
    #[error("io: {message}")]
    Io {
