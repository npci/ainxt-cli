//! Wire types for the `ainxt-workspace` API.
//!
...
//! `ainxt-workspace-grpc` crate. The Rust types defined here are
//! the source of truth.
...
///
/// Lives in `ainxt-workspace-types` (instead of `ainxt-workspace` or
/// `ainxt-mcp`) so both the permission-validation layer and the MCP
/// transport layer can depend on it without dragging the full workspace or
/// rmcp into each other. Re-exported by `ainxt_workspace::permission`
