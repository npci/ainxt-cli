...
use crate::paths::{system_config_dir, user_ainxt_home};
...
/// ainxt config files; use [`load_toml_file`] directly for unrelated TOML.
...
pub fn load_from_disk() -> std::io::Result<toml::Value> {
    load_user_config_layer(user_ainxt_home().as_deref(), "config.toml")
}
...
pub fn load_managed_config() -> std::io::Result<toml::Value> {
    load_user_config_layer(user_ainxt_home().as_deref(), MANAGED_CONFIG_FILENAME)
}
...
/// `.ainxt/<filename>` (the cwd-fallback would silently promote an untrusted
/// project `.ainxt` to the user tier).
...
pub struct ManagedConfigLayer {
    ...
    /// `true` for the root-owned system layer (`/etc/ainxt`), derived from the
    /// load directory.
    pub is_system: bool,
}
...
pub fn managed_config_layers() -> Vec<ManagedConfigLayer> {
    managed_config_layers_at(system_config_dir().as_deref(), user_ainxt_home().as_deref())
}
...
impl ConfigLayers {
    ...
    /// wraps this with the `AINXT_CAMPAIGNS_OVERRIDE` env layer.
...
/// `AINXT_CAMPAIGNS=0` or `[features] campaigns = false` on pre-campaign base.
pub fn campaigns_application_disabled(base_effective: &toml::Value) -> bool {
    if crate::env_bool("AINXT_CAMPAIGNS") == Some(false) {
        return true;
    }
    base_effective
        .get("features")
...
/// (`ainxt_shell::util::config::load_effective_config`) when remote campaigns
/// or `AINXT_CAMPAIGNS_OVERRIDE` must be honored. The name matches the
...
/// can't be parsed (broken `AINXT_TEST_VERSION` in dev), silently strips
...
pub fn apply_version_overrides_with_registered(value: &mut toml::Value) -> std::io::Result<()> {
    match ainxt_version::installed_semver() {
...
mod tests {
    ...
    fn parse_error_keeps_kind_but_not_snippet() {
        ...
        std::fs::write(
            &path,
            "api_key = \"ainxt-secretmustnotleak\"\napi_key = \"ainxt-secretmustnotleak2\"\n",
        )
        .unwrap();
        ...
        assert!(
            !msg.contains("ainxt-secretmustnotleak"),
    ...
    /// `AINXT_CAMPAIGNS=0` disables campaign application regardless of config.
    /// `AINXT_CAMPAIGNS` is process-global, so this test serializes itself with a
    ...
    fn kill_switch_env_var_disables() {
        ...
        let prior = std::env::var_os("AINXT_CAMPAIGNS");
        let empty = toml::Value::Table(Default::default());

        // SAFETY: ENV_GUARD serializes this against itself; no other test in the
        // crate mutates or reads AINXT_CAMPAIGNS concurrently.
        unsafe { std::env::set_var("AINXT_CAMPAIGNS", "0") };
        assert!(campaigns_application_disabled(&empty));

        unsafe { std::env::remove_var("AINXT_CAMPAIGNS") };
        assert!(!campaigns_application_disabled(&empty));

        match prior {
            Some(v) => unsafe { std::env::set_var("AINXT_CAMPAIGNS", v) },
            None => unsafe { std::env::remove_var("AINXT_CAMPAIGNS") },
        }
    }
}
