//! Filesystem locations for ainxt config files and binaries.

use std::path::PathBuf;
use std::sync::OnceLock;

static AINXT_HOME: OnceLock<PathBuf> = OnceLock::new();
...
/// The default user ainxt directory (`~/.ainxt`, canonicalized) used when
/// `AINXT_HOME` is unset. Exposed so callers (e.g. display helpers) can detect
/// whether [`ainxt_home()`] is the default without duplicating the computation.
///
/// Uses [`dunce::canonicalize`] instead of [`std::fs::canonicalize`]: on
/// Windows, std returns a unchanged path (`\\?\C:\Users\...`) which external
/// tools choke on — e.g. `git clone` rejects `\\?\` destinations with
/// "Invalid argument", breaking marketplace cache clones under
/// `~/.ainxt/marketplace-cache`. `dunce` strips the prefix whenever the path
...
/// `ainxt_fast_worktree::db::resolve_ainxt_home` (deliberately standalone crate).
pub fn default_ainxt_home() -> PathBuf {
    #[allow(deprecated)]
    let home = std::env::home_dir().unwrap_or_else(|| PathBuf::from("."));
    dunce::canonicalize(&home).unwrap_or(home).join(".ainxt")
}

/// Per-user config directory: `$AINXT_HOME` or `~/.ainxt`. Created if needed.
pub fn ainxt_home() -> PathBuf {
    AINXT_HOME
        .get_or_init(|| {
            let ainxt_home = if let Ok(v) = std::env::var("AINXT_HOME") {
                PathBuf::from(v)
            } else {
                default_ainxt_home()
            };
...
/// The user-global ainxt home, but only when one genuinely resolves: `Some` when
/// `$AINXT_HOME` is set or a home directory is found, `None` otherwise. Unlike
/// [`ainxt_home()`], this never falls back to a cwd-relative `.ainxt`, so callers
/// that *scan* user-global ainxt resources (hooks, marketplace sources, ...) don't
/// mistake a project's `.ainxt` tree for the user-global one when no home resolves.
pub fn user_ainxt_home() -> Option<PathBuf> {
    #[allow(deprecated)]
    let resolvable = std::env::var_os("AINXT_HOME").is_some() || std::env::home_dir().is_some();
    resolvable.then(ainxt_home)
}

/// Canonical ainxt application path: `$AINXT_HOME/bin/ainxt` (Unix) or `ainxt.exe` (Windows).
pub fn ainxt_application() -> PathBuf {
    ainxt_application_in(&ainxt_home())
}

/// [`ainxt_application`] under an explicit home instead of `$AINXT_HOME`.
pub fn ainxt_application_in(home: &std::path::Path) -> PathBuf {
    let name = if cfg!(windows) { "ainxt.exe" } else { "ainxt" };
    home.join("bin").join(name)
}

/// System-wide config directory: `/etc/ainxt/` on Unix, `None` on Windows.
pub fn system_config_dir() -> Option<PathBuf> {
    if cfg!(unix) {
        Some(PathBuf::from("/etc/ainxt"))
    } else {
        None
    }
}
...
/// `ainxt_home()/sessions/{encode_cwd_dirname(cwd)}`.
///
...
pub fn sessions_cwd_dir(cwd: &str) -> PathBuf {
    ainxt_home().join("sessions").join(encode_cwd_dirname(cwd))
}
...
pub fn ensure_sessions_cwd_dir(cwd: &str) -> std::io::Result<PathBuf> {
    ...
    let dir = ainxt_home().join("sessions").join(&encoded_name);
...
mod tests {
    ...
    #[test]
    fn default_ainxt_home_has_no_verbatim_prefix() {
        // On Windows, std::fs::canonicalize returns `\\?\C:\...` unchanged
        ...
        let home = default_ainxt_home();
        assert!(!home.to_string_lossy().starts_with(r"\\?\"));
        assert!(home.ends_with(".ainxt"));
    }
