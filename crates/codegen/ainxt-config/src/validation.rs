...
/// [`fail_closed_flag_from_str`]. Env tightening (file vs `AINXT_MANAGED_CONFIG_FAIL_CLOSED`)
...
/// with `AINXT_MANAGED_CONFIG_URL`; only applies to `requirements.toml`.
pub(crate) const FAIL_CLOSED_ENV: &str = "AINXT_MANAGED_CONFIG_FAIL_CLOSED";
...
impl RequirementsSource {
    ...
    /// id (`dev.ainxt:…`). For diagnostics and matching only; the MDM layer has no
...
pub struct RequirementsLayer {
    ...
    /// not re-derive from the source (`AINXT_HOME`-influenced, could carry `..`).
    pub is_system: bool,
}
...
pub fn requirements_layers() -> Vec<RequirementsLayer> {
    let mut out = Vec::new();
    if let Some(user_path) = user_ainxt_home().map(|g| g.join("requirements.toml"))
...
pub(crate) fn load_requirements() -> Option<toml::Value> {
    load_user_requirements(user_ainxt_home().as_deref())
}

/// User requirements layer from `<home>/requirements.toml`, or `None` with no
/// resolvable user home (rather than reading a cwd-relative `.ainxt`).
...
fn validate_requirements_value(
    ...
    let Ok(version) = ainxt_version::installed_semver() else {
        return Ok(());
    };
...
pub fn validate_requirements() -> Result<(), RequirementsError> {
    validate_user_requirements(user_ainxt_home().as_deref())?;
    if let Some(dir) = system_config_dir() {
...
/// no-op (no cwd-relative `.ainxt/requirements.toml` is read or enforced).
...
mod tests {
    ...
    fn load_requirements_layer_soft_fails_on_invalid_version_overrides() {
        ...
        let dir = std::env::temp_dir().join(format!("ainxt-vo-soft-{}", std::process::id()));
    ...
    fn validate_requirements_layer_errs_on_fail_closed_violation() {
        ...
        let dir = std::env::temp_dir().join(format!("ainxt-vo-validate-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("requirements.toml");
        let mut f = std::fs::File::create(&path).unwrap();
        f.write_all(
    ...
    fn validate_requirements_layer_ok_without_fail_closed() {
        ...
        let dir = std::env::temp_dir().join(format!("ainxt-vo-soft2-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("requirements.toml");
        let mut f = std::fs::File::create(&path).unwrap();
        f.write_all(b"\n[[version_overrides]]\nminimum_version = \"not-a-version\"\n")
    ...
    fn fail_closed_key_is_stripped_from_returned_layer() {
        ...
        let dir = std::env::temp_dir().join(format!("ainxt-vo-strip-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("requirements.toml");
        let mut f = std::fs::File::create(&path).unwrap();
        f.write_all(b"fail_closed = true\n[features]\ntelemetry = true\n\n")
            .unwrap();
        drop(f);
    ...
    fn load_user_requirements_reads_layer_when_home_present() {
        ...
        let dir = std::env::temp_dir().join(format!("ainxt-req-load-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let mut f = std::fs::File::create(dir.join("requirements.toml")).unwrap();
        f.write_all(b"[features]\ntelemetry = true\n\n").unwrap();
        drop(f);
    ...
    fn validate_user_requirements_errs_on_fail_closed_violation() {
        ...
        let dir = std::env::temp_dir().join(format!("ainxt-req-validate-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let mut f = std::fs::File::create(dir.join("requirements.toml")).unwrap();
        f.write_all(
            b"\nfail_closed = true\n[[version_overrides]]\nminimum_version = \"not-a-version\"\n",
