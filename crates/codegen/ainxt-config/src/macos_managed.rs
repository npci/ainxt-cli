...
//! preference domain `dev.ainxt` (`requirements_toml_base64`). Only admin-*forced*
...
#[cfg(target_os = "macos")]
const MANAGED_PREFERENCES_DOMAIN: &str = "dev.ainxt";
#[cfg(target_os = "macos")]
...
pub const MDM_REQUIREMENTS_SOURCE: &str = "dev.ainxt:requirements_toml_base64";
...
/// used **unchanged** — `$VAR`/`${VAR}` are deliberately NOT expanded: this is
...
fn read_forced_requirements() -> Option<String> {
    ...
    // per-user domain, which a local user can set (`defaults write dev.ainxt`)
...
mod tests {
    ...
    /// The forced payload is used unchanged: `$VAR`/`${VAR}` must NOT be expanded,
    ...
    fn forced_payload_is_not_env_expanded() {
        ...
        let prior = std::env::var("AINXT_MDM_NO_EXPAND_TEST").ok();
        unsafe { std::env::set_var("AINXT_MDM_NO_EXPAND_TEST", "attacker") };
        let decoded = decode_managed_toml(&b64("base_url = \"${AINXT_MDM_NO_EXPAND_TEST}/v1\"\n"));
        unsafe {
            match prior {
                Some(p) => std::env::set_var("AINXT_MDM_NO_EXPAND_TEST", p),
                None => std::env::remove_var("AINXT_MDM_NO_EXPAND_TEST"),
            }
        }
        assert_eq!(
            decoded
                ...
                .and_then(|v| v.as_str()),
            Some("${AINXT_MDM_NO_EXPAND_TEST}/v1"),
