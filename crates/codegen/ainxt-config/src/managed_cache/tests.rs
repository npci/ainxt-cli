...
fn compromised_only_when_opted_in_and_deleted_or_substituted() {
    // Use `tempfile::tempdir()` (a private, auto-cleaned-up directory under
    // the OS temp root, already used elsewhere in this file) instead of
    // hand-building a fixed, PID-suffixed path directly under the shared
    // temp directory.
    let tmp = tempfile::tempdir().unwrap();
    let dir = tmp.path().to_path_buf();
...
fn confirmed_switch_requires_two_known_differing_sides() {
    ...
    // is returned unchanged for logging).
    assert_eq!(
