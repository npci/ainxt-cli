...
//!
//! These tests exercise the real `ainxt-pager` binary through a PTY using
...
/// enterprise deploy report: with `AINXT_GOAL=1`, `/goal` must
...
/// explicitly off (`AINXT_GOAL=0`; goal mode defaults on), `/goal` must stay
...
/// Full folder-trust session: with `AINXT_FOLDER_TRUST=1` and a git repo that
...
/// also pins `AINXT_CONTEXTUAL_HINTS=0` (belt-and-suspenders); the same planning
