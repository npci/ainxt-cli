...
#[ignore = "PTY e2e; run with cargo test -p ainxt-pager --test leader_pty_e2e -- --ignored --test-threads=1"]
async fn campaign_leader_mode_remote_dismiss_on_model_pick() {
    ...
    // socket under the shared AINXT_HOME so every spawn elects/attaches to the
    // same leader (matches `LeaderCluster`).
    let ainxt_home = content.home().join(".ainxt");
    std::fs::create_dir_all(&ainxt_home).expect("create AINXT_HOME");
    std::fs::write(
    ...
    // Session (OAuth) auth, not the harness's default AINXT_API_KEY: the
    ...
    let state_path = ainxt_home.join("campaigns_state.json");
    ...
    let config = std::fs::read_to_string(ainxt_home.join("config.toml")).expect("read config.toml");
    assert!(
