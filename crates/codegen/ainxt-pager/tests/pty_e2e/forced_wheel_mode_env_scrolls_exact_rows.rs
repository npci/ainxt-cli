...
//
// `AINXT_SCROLL_MODE=wheel` + `AINXT_SCROLL_LINES=1` on a `TERM_PROGRAM=zed`
...
// - Without AINXT_SCROLL_LINES=1, Zed's wheel profile prices the burst at
//   3 lines/event → 9 rows.
// - Without AINXT_SCROLL_MODE=wheel, Auto finalizes a 3-event ZERO-interval
...
// no acceleration, and the harness's hermetic AINXT_HOME pins scroll_speed at
// its default (50 → 1.0x).
...
/// **Env-forced wheel pricing e2e.** The `AINXT_SCROLL_MODE` /
/// `AINXT_SCROLL_LINES` overrides must reach the live config: a 3-event burst
...
async fn forced_wheel_mode_env_scrolls_exact_rows() {
    let (mut harness, _content, top_before) = spawn_bottom_pinned_marker_scrollback_with_env(
        ...
        &[
            ("TERM_PROGRAM", "zed"),
            ("AINXT_SCROLL_MODE", "wheel"),
            ("AINXT_SCROLL_LINES", "1"),
        ],
    )
    .await;
