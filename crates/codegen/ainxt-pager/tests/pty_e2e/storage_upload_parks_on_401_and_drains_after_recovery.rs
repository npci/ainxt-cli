...
async fn storage_upload_parks_on_401_and_drains_after_recovery() {
    ...
    // Trace uploads are gated on first-party ainxt OAuth (`is_ainxt_auth()`:
    // AuthMode::Oidc + ainxt issuer); the harness's AINXT_API_KEY is ApiKey mode
    ...
    env_refs.retain(|(k, _)| *k != "AINXT_TRACE_UPLOAD");
    env_refs.push(("AINXT_TRACE_UPLOAD", "true"));
    env_refs.push(("AINXT_TELEMETRY_TRACE_UPLOAD", "true"));
    env_refs.push(("AINXT_UPLOAD_QUEUE_AUTH_PROBE_SECS", "2"));
    ...
    //
    // Accounting (ainxt-file-utils upload queue):
    ...
    //   not fire inside this 3s window; with `AINXT_UPLOAD_QUEUE_AUTH_PROBE_SECS=2`
