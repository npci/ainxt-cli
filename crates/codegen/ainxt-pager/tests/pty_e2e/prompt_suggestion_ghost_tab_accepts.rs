...
/// turn and the turn-end `ainxt.dev/suggestPrompt` call (the mock's fixed mode
/// answers every request), so after the turn the ghost text matches it.
...
async fn prompt_suggestion_ghost_tab_accepts() {
    // The suggestion model (`ainxt-build-0.1`, the shell's built-in default)
    ...
    env.retain(|(k, _)| k != "AINXT_PROMPT_SUGGESTIONS");
    env.push(("AINXT_PROMPT_SUGGESTIONS".into(), "true".into()));
