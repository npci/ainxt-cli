...
async fn reasoning_efforts_from_config_toml_menu() {
    let content = ContentController::start_with_models(vec![
        MockModel::new("ainxt-4.5").with_supports_reasoning_effort(true),
    ])
    .await
    .expect("start content");
    content.set_response(format!("{MOCK_RESPONSE_SENTINEL} turn."));

    // Seed `~/.ainxt/config.toml` with a per-model reasoning-effort menu.
    let ainxt_home = content.home().join(".ainxt");
    std::fs::create_dir_all(&ainxt_home).expect("create .ainxt");
    // Quote the dotted model id: bare `[model.ainxt-4.5]` is TOML key-path syntax (model.ainxt-4.5), not the id "ainxt-4.5".
    std::fs::write(
