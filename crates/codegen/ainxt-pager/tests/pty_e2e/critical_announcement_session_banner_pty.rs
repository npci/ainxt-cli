...
const PROMO_URL: &str = "https://ainxt.example.test/zz-promo-cta";
...
fn spawn_with_announcements(content: &ContentController, override_json: &str) -> PtyHarness {
    ...
    env.push((
        "AINXT_ANNOUNCEMENTS_OVERRIDE".into(),
...
async fn critical_announcement_hide_button_click_hides_banner() {
    ...
    let long_msg = format!(
        "{CRIT_MSG} elevated error rates persist across regions check status.ainxt.dev for updates and retry your request later"
    );
...
/// safe-open path — observed via the `AINXT_TEST_OPEN_URL_FILE` seam, never a
...
async fn promo_cta_click_opens_link_and_hide_roundtrip() {
    ...
    let extra_env = [
        ("AINXT_TEST_OPEN_URL_FILE", url_file_str.as_str()),
...
/// Pinned (`dismissible:false`) promo seeded via `AINXT_ANNOUNCEMENTS_OVERRIDE`.
...
/// `AINXT_TEST_OPEN_URL_FILE` seam + a `TERM_PROGRAM` pin for OSC 8).
