...
use ainxt_pager_pty_harness::PtyHarness;
pub(crate) use ainxt_pager_pty_harness::{SGR_SCROLL_DOWN, SGR_SCROLL_UP};
...
/// (`ainxt-pager-pty-harness/src/scroll_matrix/runner.rs`) ports this
...
/// `ainxt-pager-pty-harness/src/scroll_matrix/session.rs` is a port of
...
/// (e.g. `AINXT_SCROLL_SPEED`) appended to the controller's env.
