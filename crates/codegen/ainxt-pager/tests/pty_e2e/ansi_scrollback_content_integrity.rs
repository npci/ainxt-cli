...
///
/// Regression guard for the `ainxt-ratatui-inline` rewrite (termwiz →
/// anstyle-parse). Minimal mode commits finalized assistant blocks into the
/// terminal's NATIVE scrollback through `ainxt_ratatui_inline::Terminal::
...
async fn ansi_scrollback_content_integrity() {
    ...
    // Code-block rows render unchanged (no markdown reflow) and syntect
