...
//!
//! The stack is `tmux -> nvim :terminal -> ainxt`: nvim/tmux repaint ainxt's
//! pane out-of-band (no ainxt PTY resize), and ainxt's diff renderer only
...
//! into the virtual terminal, bypassing ainxt) and then check whether ainxt
//! heals it. `NVIM` is set in ainxt's env so it sees the embedded-editor
//! context the fix keys off (mirroring a real nvim `:terminal`).
//!
//! The fix: ainxt forces a full clear+repaint on `FocusGained` in
//! editor/multiplexer contexts, so the injected row is gone after refocus.
//! The final assertion guards that heal.

use super::common::*;

/// Unique sentinel that ainxt would never render on its own.
...
async fn out_of_band_stale_row_heals_on_focus_gained() {
    ...
    env.push(("NVIM".into(), "/tmp/ainxt-pty-harness-fake-nvim.sock".into()));
    ...
    // virtual screen at col 1 — the static left margin ainxt's diff renderer
    ...
    // ainxt must not self-heal out-of-band content via ordinary diff redraws:
    // it only rewrites cells whose own model changed, so this row is stranded.
    h.update(Duration::from_millis(300));
    assert!(
        h.contains_text(STALE_MARKER),
        "stale row should survive a normal redraw (ainxt's diff renderer doesn't own it)\nscreen:\n{}",
        h.screen_contents()
    );

    // A FocusGained (CSI I) forces a full clear+repaint that re-asserts ainxt's
