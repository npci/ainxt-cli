...
//
// Bug: when the user has scrolled UP into ainxt's scrollback (NOT pinned to the
// bottom / not following) and the terminal is resized HORIZONTALLY, ainxt loses
...
// in follow/tail mode ainxt re-pins to the bottom (no jump).
//
...
//
// Runs FULLSCREEN (alt-screen): ainxt's mode-independent scroll-anchor logic is
// what's under test. `Viewport::Fullscreen` is autoresized on resize with no DSR
// cursor probe, so ainxt itself re-wraps at the new width, and the alt-screen grid
// is not reflowed by the terminal — so the marker's position reflects ainxt's own
...
///
/// ainxt renders markdown with SOFT line breaks (a single `\n` becomes a space),
...
async fn resize_preserves_scroll_position() {
    ...
    // terminal resize with NO DSR cursor probe, so ainxt re-wraps the scrollback
    // at the new width and the stale `scroll_offset` surfaces the jump in ainxt's
    // OWN rendered output. The alt-screen grid is not reflowed by the terminal on
    // resize, so this isolates ainxt's scroll-anchor logic with no harness confound.
