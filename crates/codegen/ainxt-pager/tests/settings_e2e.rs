...
fn make_state() -> SettingsModalState {
    ...
    ainxt_pager::app::set_voice_mode_enabled_for_test(true);
...
/// inside the sub-sheet. Matches `build_rows`'s group-child skip.
...
/// Mouse equivalence for the group: clicking the group row's value column opens the
...
// ---------------------------------------------------------------------------
// Per-setting MOUSE paths (keyboard ↔ mouse equivalence)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Mouse equivalence under active filter
// ---------------------------------------------------------------------------
...
fn render_with_filter_active_and_small_viewport_clamps_scroll() {
    ...
    ainxt_pager::views::settings_modal::render_settings_modal(
...
fn defaults_round_trip_through_registry() {
    use ainxt_pager::settings::{SettingValue, current_value_for};
    let reg = SettingsRegistry::defaults();
    ...
    ainxt_pager::appearance::cache::set_keep_text_selection(
        ainxt_pager::appearance::TextSelection::Flash,
    );
    ainxt_pager::appearance::cache::set_show_thinking_blocks(true);
    ainxt_pager::appearance::cache::set_prompt_suggestions(true);
    ainxt_pager::appearance::cache::set_group_tool_verbs(true);
    ainxt_pager::appearance::cache::set_scroll_mode(
        ainxt_pager::appearance::ScrollMode::Auto,
    );
    ainxt_pager::appearance::cache::set_invert_scroll(false);
    // 3 = the registry default shown while the profile is in charge.
    ainxt_pager::appearance::cache::set_scroll_lines(3);
    ...
    let expected = |key: &str| -> SettingValue {
        match key {
            ...
            "theme" => SettingValue::Enum("ainxtnight"),
            "auto_dark_theme" => SettingValue::Enum("ainxtnight"),
            "auto_light_theme" => SettingValue::Enum("ainxtday"),
    ...
    for meta in reg.all() {
        ...
        let default_value = ainxt_pager::settings::default_value_for(meta);
...
/// Release events are dropped (kitty-keyboard protocol equivalence).
#[test]
...
// `crates/codegen/ainxt-pager/src/app/dispatch.rs::tests` (next to
...
fn pr4_mouse_click_in_theme_picker_is_no_op() {
    ...
    s.mode = SettingsModalMode::PickingEnum {
        key: "theme",
        choices_idx: 0,
        original_value: SettingValue::Enum("ainxtnight"),
        supports_preview: true,
    };
    synth_rects(&mut s);
    let outcome = handle_settings_mouse(
...
fn pr6_current_value_for_reads_pager_snapshot() {
    use ainxt_pager::settings::current_value_for;
...
fn pr6_permission_mode_picker_enter_dispatches_set_permission_mode_commit() {
    use ainxt_pager::app::actions::PermissionModeKind;
    let reg = SettingsRegistry::defaults();
...
// ---------------------------------------------------------------------------
// Mouse path tests for permission_mode (keyboard ↔ mouse equivalence)
// ---------------------------------------------------------------------------
...
fn reset_overlay_dims_all_rows_except_target() {
    ...
    use ainxt_pager::views::settings_modal::ResetConfirmOverlay;
...
/// "Ask ainxt" tip footer at the bottom of the content area in
...
fn docs_footer_renders_for_browse_and_picker() {
    ...
    for fixture_label in ["browse", "picker"] {
        ...
        assert!(
            all_text.contains("Ask ainxt"),
            "[{fixture_label}] docs footer (`Ask ainxt`) must appear in the rendered modal:\n\
             {all_text}"
        );
        assert!(
            all_text.contains("change theme to ainxtday"),
...
fn render_modal_to_string(s: &mut SettingsModalState, width: u16, height: u16) -> String {
    ...
    let mut buf = Buffer::empty(area);
    ainxt_pager::views::settings_modal::render_settings_modal(&mut buf, area, s, false, None);
...
fn restart_pill_hidden_when_edited_but_collapsed() {
    use ainxt_pager::settings::{PagerLocalSnapshot, SettingsRegistry};
    use ainxt_shell::agent::config::UiConfig;
...
/// lines. The wrapped output covers the description unchanged
...
/// row) toggles expansion — keyboard ↔ mouse equivalence for the
...
fn reset_confirm_prompt_helper_builds_well_formed_string_for_every_setting() {
    use ainxt_pager::settings::{PagerLocalSnapshot, SettingsRegistry};
    use ainxt_pager::views::modal::{ActiveModal, ModalConfirmation, reset_confirm_prompt};
    use ainxt_shell::agent::config::UiConfig;
    let reg = SettingsRegistry::defaults();
    for meta in reg.all() {
...
fn pr14_default_model_picker_commits_resolved_model_id() {
    let snapshot = PagerLocalSnapshot {
        available_models: vec![
            (
                "ainxt 4.5".to_string(),
                agent_client_protocol::ModelId::new(std::sync::Arc::from("ainxt-4.5")),
            ),
            (
                "ainxt 3".to_string(),
                agent_client_protocol::ModelId::new(std::sync::Arc::from("ainxt-3")),
            ),
        ],
        ..PagerLocalSnapshot::default()
    };
    ...
    // Walk down past row 0 ("(no override)") to row 1 ("ainxt 4.5").
    let outcome = handle_settings_key(&mut s, &press(KeyCode::Down));
    assert!(
        matches!(outcome, SettingsKeyOutcome::Changed),
...
/// Mouse click on `default_model` opens picker (keyboard ↔ mouse equivalence).
#[test]
...
fn pr8_default_model_and_max_thoughts_width_defaults_roundtrip() {
    ...
    assert_eq!(
        ainxt_pager::settings::default_value_for(dm_meta),
    ...
    assert_eq!(
        ainxt_pager::settings::default_value_for(mt_meta),
...
fn pr9_privacy_slash_command_parses_aliases() {
    use ainxt_pager::slash::commands::privacy::parse_privacy_arg;
...
fn pr10_plan_mode_picker_enter_dispatches_set_commit() {
    use ainxt_pager::app::actions::PlanModeKind;
...
// ---------------------------------------------------------------------------
// Mouse path tests for plan_mode (keyboard ↔ mouse equivalence).
//
// Matches the permission_mode / coding_data_sharing mouse tests. Every
// keyboard interaction has a mouse equivalent.
// ---------------------------------------------------------------------------

/// First mouse-click on a DIFFERENT (non-selected) `plan_mode` row
/// only SELECTS the row (no picker entry, no Action). Matches the
...
fn render_mermaid_picker_enter_dispatches_set_commit() {
    use ainxt_pager::appearance::RenderMermaid;
...
// equivalence). Matches the plan_mode mouse tests.
// ---------------------------------------------------------------------------
...
// time. Matches the render_mermaid enum tests (keyboard ↔ mouse equivalence).
// ---------------------------------------------------------------------------
...
/// commit forwards `choice.to_string()` unchanged, so these canonicals are the
...
/// click (mouse ↔ keyboard equivalence).
#[test]
...
fn pr14_string_settings_use_known_model_validator() {
    use ainxt_pager::settings::DynamicEnumSource;
    let reg = SettingsRegistry::defaults();
...
/// Non-baseline `fork_secondary_model` surfaces unchanged.
#[test]
...
//
// Matches `render_mermaid`: `supports_preview: false`, Enter opens picker,
...
fn keep_text_selection_picker_enter_dispatches_set_commit() {
    use ainxt_pager::appearance::TextSelection;
    ...
    ainxt_pager::appearance::cache::set_keep_text_selection(TextSelection::Flash);
    let mut s = make_state();
    navigate_to(&mut s, "keep_text_selection");
    let _ = handle_settings_key(&mut s, &press(KeyCode::Enter));
...
fn keep_text_selection_hold_snapshot_seeds_picker_at_hold() {
    ...
    ainxt_pager::appearance::cache::set_keep_text_selection(TextSelection::Hold);
    let ui = UiConfig {
...
// no preview (matches keep_text_selection).
// ---------------------------------------------------------------------------
...
fn invert_scroll_enter_dispatches_typed_setter() {
    ainxt_pager::appearance::cache::set_invert_scroll(true);
    let mut s = make_state();
    navigate_to(&mut s, "invert_scroll");
    let outcome = handle_settings_key(&mut s, &press(KeyCode::Enter));
...
fn show_thinking_blocks_space_dispatches_typed_setter() {
    ...
    ainxt_pager::appearance::cache::set_show_thinking_blocks(false);
    let mut s = make_state();
...
fn prompt_suggestions_space_dispatches_typed_setter() {
    // Pin off so space toggles to true.
    ainxt_pager::appearance::cache::set_prompt_suggestions(false);
    let mut s = make_state();
...
fn group_tool_verbs_cache_off_dispatches_on() {
    ...
    ainxt_pager::appearance::cache::set_group_tool_verbs(false);
    let mut s = SettingsModalState::new(
        Arc::new(SettingsRegistry::defaults()),
        UiConfig::default(),
        PagerLocalSnapshot::default(),
...
fn collapsed_edit_blocks_space_dispatches_typed_setter() {
    ...
    ainxt_pager::appearance::cache::set_collapsed_edit_blocks(false);
    let mut s = make_state();
...
fn collapsed_edit_blocks_cache_on_dispatches_off() {
    ...
    ainxt_pager::appearance::cache::set_collapsed_edit_blocks(true);
    let mut s = SettingsModalState::new(
        Arc::new(SettingsRegistry::defaults()),
        UiConfig::default(),
        PagerLocalSnapshot::default(),
