...
//   1. Brotli-compresses the built binary into `../ainxt-<platform>/bin/<bin>.br`
...
// (`@ainxt/ainxt`) lists all six as `optionalDependencies` pinned to
// the same version; npm installs only the one matching the host's
// `os` + `cpu` filters.
//
// Why brotli? npm's tarball ceiling is ~200 MB and the raw ainxt binary is
...
const ainxtRoot = process.env.AINXT_ROOT || path.resolve(__dirname, '..', '..', '..', '..', '..');
const npmRoot = path.resolve(__dirname, '..', '..');

const NOTICES_SOURCE = path.resolve(
    npmRoot, '..', '..', 'ainxt-tools', 'THIRD_PARTY_NOTICES.md');
...
async function packPlatform({ platform, arch, envVar, defaultSource, binName }) {
    const pkgDir = path.join(npmRoot, `ainxt-${platform}-${arch}`);
    ...
    console.log(
        `[assemble] ainxt-${platform}-${arch}@${VERSION}: ` +
...
async function main() {
    const targets = [
        {
            platform: 'darwin', arch: 'arm64', binName: 'ainxt',
            envVar: 'AINXT_DARWIN_ARM64',
            defaultSource: path.join(ainxtRoot, 'target', 'release', 'ainxt-pager'),
        },
        {
            platform: 'darwin', arch: 'x64', binName: 'ainxt',
            envVar: 'AINXT_DARWIN_X64',
            defaultSource: path.join(ainxtRoot, 'target', 'x86_64-apple-darwin', 'release', 'ainxt-pager'),
        },
        {
            platform: 'linux', arch: 'x64', binName: 'ainxt',
            envVar: 'AINXT_LINUX_X64',
            defaultSource: path.join(ainxtRoot, 'target',
                'explorer_cross_x86_64-unknown-linux-gnu',
                'x86_64-unknown-linux-gnu', 'release', 'ainxt-pager'),
        },
        {
            platform: 'linux', arch: 'arm64', binName: 'ainxt',
            envVar: 'AINXT_LINUX_ARM64',
            defaultSource: path.join(ainxtRoot, 'target',
                'explorer_cross_aarch64-unknown-linux-gnu',
                'aarch64-unknown-linux-gnu', 'release', 'ainxt-pager'),
        },
        {
            platform: 'win32', arch: 'x64', binName: 'ainxt.exe',
            envVar: 'AINXT_WIN32_X64',
            defaultSource: path.join(ainxtRoot, 'target', 'x86_64-pc-windows-msvc', 'release', 'ainxt-pager.exe'),
        },
        {
            platform: 'win32', arch: 'arm64', binName: 'ainxt.exe',
            envVar: 'AINXT_WIN32_ARM64',
            defaultSource: path.join(ainxtRoot, 'target', 'aarch64-pc-windows-msvc', 'release', 'ainxt-pager.exe'),
        },
    ];
