#!/usr/bin/env node
// Runs once after npm install/update. Reads the ainxt binary from the
// matching per-platform optional dependency (@ainxt/ainxt-<platform>)
// and installs it to ~/.ainxt/bin/ using versioned filenames:
//
//   Unix:    ainxt-<version>  +  ainxt  (symlink)
//   Windows: ainxt-<version>.exe  +  ainxt.exe  (copy)
//
...
const CANONICAL_DIR = path.join(os.homedir(), '.ainxt', 'bin');
...
if (!SUPPORTED.has(key)) {
    console.error(`@ainxt/ainxt: unsupported platform ${key}`);
    process.exit(0);
}
...
function resolvePlatformPackageDir() {
    const platformPkg = `@ainxt/ainxt-${key}`;
    try {
...
if (!version) {
    console.error('@ainxt/ainxt: unable to determine version');
    process.exit(0);
}
...
function installBinary(binName, sourceDir, vendorSubpath) {
    ...
    if (fs.existsSync(brPath)) {
        ...
        console.error(`@ainxt/ainxt: missing binary at ${brPath}`);
        return false;
    }
    ...
    if (IS_WINDOWS) {
        ...
        try {
            ...
            try {
                ...
                console.error(`@ainxt/ainxt: failed to update ${canonicalPath}: ${e2.message}`);
                console.error('Close all running ainxt processes and try again.');
                return false;
            }
        }
    } else {
...
// Uses an exact prefix match + hyphen + digit to avoid ainxt-* matching ainxt-pager-*.
...
if (!platformDir) {
    console.error(`@ainxt/ainxt: platform package @ainxt/ainxt-${key} not installed.`);
    console.error('  This usually means npm was invoked with --no-optional, or the install failed.');
    console.error('  Try: npm install -g @ainxt/ainxt');
    process.exit(0);
}

installBinary('ainxt', platformDir, `ainxt${EXE}`);
cleanupOldVersions('ainxt');
cleanupOldVersions('ainxt-pager');

// Write installer config
const configDir = path.join(os.homedir(), '.ainxt');
...
// Persist the npm registry so `ainxt update` and the trampoline use the same one.
const npmRegistry = process.env.AINXT_NPM_REGISTRY
    || (() => {
        try {
            const resolved = execSync(
                'npm config get @ainxt:registry',
...
// Set AINXT_INSTALL_COMPLETIONS=1 to auto-generate to ~/.ainxt/completions.
const AINXT_PATH = path.join(CANONICAL_DIR, `ainxt${EXE}`);
if (process.env.AINXT_INSTALL_COMPLETIONS === '1' && !IS_WINDOWS) {
    try {
        const { spawnSync } = require('child_process');
        const completionsDir = path.join(os.homedir(), '.ainxt', 'completions');
        const bashPath = path.join(completionsDir, 'bash', 'ainxt.bash');
        const zshPath = path.join(completionsDir, 'zsh', '_ainxt');
        fs.mkdirSync(path.dirname(bashPath), { recursive: true });
        fs.mkdirSync(path.dirname(zshPath), { recursive: true });
        const bashRes = spawnSync(AINXT_PATH, ['completions', 'bash'], { encoding: 'utf8' });
        if (bashRes.status === 0) fs.writeFileSync(bashPath, bashRes.stdout);
        const zshRes = spawnSync(AINXT_PATH, ['completions', 'zsh'], { encoding: 'utf8' });
        if (zshRes.status === 0) fs.writeFileSync(zshPath, zshRes.stdout);
        console.log('Completions generated to ~/.ainxt/completions (bash/zsh)');
    } catch {}
} else if (!IS_WINDOWS) {
    console.log('Tip: ainxt completions bash > ~/.local/share/bash-completion/completions/ainxt');
    console.log('     ainxt completions zsh  > ~/.zsh/completions/_ainxt');
}
