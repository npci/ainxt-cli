#
# AiNxt CLI installer for PowerShell (Windows).
#
# Installs the `ainxt` binary from this repository's GitHub Releases. There is
# no third-party fallback origin (see the note at the BaseUrl block, CLI-026).
#
# Usage — one line:
#   irm https://raw.githubusercontent.com/npci/ainxt-cli/main/crates/codegen/ainxt-pager/scripts/install.ps1 | iex
#
# Pin a version:
#   $env:AINXT_VERSION="0.2.101"; irm https://raw.githubusercontent.com/npci/ainxt-cli/main/crates/codegen/ainxt-pager/scripts/install.ps1 | iex
#
# Your own artifact host:
#   $env:AINXT_BASE_URL="https://artifacts.example"; irm ... | iex
#
# NOTE ON `irm | iex`. Piping a remote script into a shell makes the transport
# part of the trust boundary. Accepted here for a one-command install, and
# mitigated by HTTPS-only, a single project-controlled origin, no fallback
# origin, and checksum verification of the binary (mandatory when
# $env:AINXT_REQUIRE_CHECKSUM=1). Prefer not to? Download and read this file
# first, or build from source with ./setup.sh — no artifact host needed.
#
...
if ($PSVersionTable.Platform -and $PSVersionTable.Platform -ne 'Win32NT') {
    Write-Error "This installer is for Windows. On macOS/Linux, use: curl -fsSL https://raw.githubusercontent.com/npci/ainxt-cli/main/crates/codegen/ainxt-pager/scripts/install.sh | bash"
    exit 1
}
...
# Artifact-name OS token. MUST match what scripts/build-release.sh emits:
#   dist/ainxt-<version>-win32-<arch>.exe
# Until 2026-08-29 this script requested `windows-<arch>`, which the builder
# never produces, so every Windows download 404'd. See CLI-027.
$platform = "win32-$arch"
...
$Channel = if ($env:AINXT_CHANNEL) { $env:AINXT_CHANNEL } else { 'stable' }

# Where binaries come from. Default: this repository's own GitHub Releases — an
# origin the publishing project controls, needing no infrastructure.
#
# There is deliberately NO third-party fallback origin. Until 2026-08-29 this
# script fell back to a hardcoded 'storage.googleapis.com/...' bucket, with no
# environment override at all, whenever the primary was unreachable — and since
# the primary default was the reserved domain 'ainxt.example.test', that was
# ALWAYS. An unconfigured run therefore downloaded and executed a binary from a
# bucket this project does not necessarily control. See CLI-026.
# Default source repository for GitHub Releases. A plain constant, not a
# conditional on who is publishing: override it with AINXT_GITHUB_REPO and
# nothing in this script behaves differently.
$DefaultGhRepo = 'npci/ainxt-cli'
$GhRepo = $env:AINXT_GITHUB_REPO
if (-not $GhRepo) { $GhRepo = $DefaultGhRepo }
$BaseUrl = $env:AINXT_BASE_URL

if ($BaseUrl) {
    # Operator-hosted flat layout: <base>/<channel> and <base>/ainxt-<ver>-<platform>
    if ($Version) {
        $resolvedVersion = $Version
    } else {
        Write-Host "Fetching latest $Channel version from $BaseUrl..." -ForegroundColor DarkGray
    ...
    if ($Version) {
        $resolvedVersion = $Version
        $artifactDir = "https://github.com/$GhRepo/releases/download/v$resolvedVersion"
    } else {
        Write-Host "Fetching latest release of $GhRepo..." -ForegroundColor DarkGray
        $tag = $null
        try {
            $rel = Download-String "https://api.github.com/repos/$GhRepo/releases/latest"
            if ($rel -match '"tag_name"\s*:\s*"v?([^"]+)"') { $tag = $Matches[1] }
        } catch { }
        if (-not $tag) {
            Write-Error @"
Could not determine the latest release of $GhRepo.
No releases published yet? Build from source instead:
  git clone https://github.com/$GhRepo.git; cd ainxt-cli; ./setup.sh
Or point this installer at your own host: `$env:AINXT_BASE_URL = 'https://...'
"@
            exit 1
        }
        $resolvedVersion = $tag
        $artifactDir = "https://github.com/$GhRepo/releases/latest/download"
    }
}
...
$artifactBase = "$artifactDir/ainxt-$resolvedVersion-$platform"
