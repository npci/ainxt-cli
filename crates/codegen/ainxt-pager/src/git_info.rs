...
///   - [`update_from_notification`] — the `ainxt.dev/git_head_changed` ACP
...
pub fn update_from_notification(dir: &Path, branch: Option<&str>, main_repo: Option<String>) {
    if let Ok(mut cache) = CWD_GIT_CACHE.lock() {
        ...
        // A branch-switch notification doesn't carry a status count. Preserve
        // any previously-computed count rather than blanking it; the next lazy
        // refresh recomputes it via `compute_snapshot`.
        let changed_files = cache
            .get(dir)
            .and_then(|(info, _)| info.as_ref())
            .and_then(|i| i.changed_files);
        let info = CwdGitInfo {
...
struct GitSnapshot {
    ...
    /// Count of files with working-tree or index changes (excludes ignored).
    /// `None` when not in a repo. Computed via `git2` statuses in the same
    /// off-thread snapshot as the branch, so it stays off the render path.
...
pub struct CwdGitInfo {
    ...
    /// Count of files with working-tree or index changes (excludes ignored).
    /// `None` when the count could not be computed (e.g. a `git_head_changed`
    /// notification-only refresh, which doesn't carry a status count).
...
/// Cheap changed-file count for `cwd`, read from the same per-cwd cache as
/// the branch ([`cwd_git_info_lazy`]). Returns `None` when `cwd` is not a git
/// repo (or the count hasn't been computed yet); `Some(0)` for a clean tree.
///
/// Delegates to [`cwd_git_info_lazy`], so it inherits the same throttled
/// off-thread refresh and never spawns a `git` subprocess or blocks on I/O —
/// safe to call every frame from the status-bar renderer.
pub fn cwd_changed_files_lazy(cwd: &Path) -> Option<usize> {
    cwd_git_info_lazy(cwd).and_then(|i| i.changed_files)
}
...
/// Count files with working-tree or index changes, excluding ignored and
/// clean entries. Cheap-ish `git2` status walk — called only from the
/// off-thread snapshot ([`compute_snapshot`]), never per frame. Returns
/// `None` if the status walk fails so callers don't render a bogus `0`.
///
/// Untracked files are included (they're "changes" to the working tree) but
/// ignored files are not, matching what a user thinks of as "changed files".
fn count_changed_files(repo: &git2::Repository) -> Option<usize> {
...
pub fn worktree_label_index() -> std::collections::HashMap<PathBuf, String> {
    ...
    let Ok(db) = ainxt_fast_worktree::db::WorktreeDb::open_default() else {
        return map;
    };
    ...
    for rec in records {
        let Some(label) = rec
            .metadata
            .as_ref()
            .and_then(|m| m.get(ainxt_shell::session::worktree::META_KEY_LABEL))
            .and_then(|v| v.as_str())
            .filter(|s| !s.is_empty())
        else {
            continue;
...
fn lookup_worktree_label(cwd: &Path) -> Option<String> {
    let db = ainxt_fast_worktree::db::WorktreeDb::open_default().ok()?;
    ...
    for ancestor in cwd.ancestors() {
        if let Ok(Some(record)) = db.get(&ancestor.to_string_lossy()) {
            let label = record
                .metadata
                .as_ref()?
                .get(ainxt_shell::session::worktree::META_KEY_LABEL)?
                .as_str()
                .filter(|s| !s.is_empty())
                .map(String::from);
            return label;
...
///
/// `AINXT_NERD_FONTS=1` forces Powerline; `AINXT_NERD_FONTS=0` forces the fallback.
pub(crate) fn branch_icon() -> &'static str {
    static ICON: OnceLock<&str> = OnceLock::new();
    ICON.get_or_init(|| {
        decide_branch_icon(
            std::env::var("AINXT_NERD_FONTS").ok().as_deref(),
            HostOs::current(),
...
///
/// An explicit `AINXT_NERD_FONTS` override (`0`/`false` → off, anything else →
...
mod tests {
    ...
    fn cwd_git_info_lazy_non_repo_is_none() {
        let p = Path::new("/nonexistent-ainxt-git-info-lazy-test-zzz");
