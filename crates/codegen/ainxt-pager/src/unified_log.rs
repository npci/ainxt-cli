...
//! `ainxt.dev/log` ACP notifications. Call [`init`] once at startup with
...
use ainxt_telemetry::unified_log::{
...
fn push_entry(lvl: LogLevel, msg: &str, sid: Option<&str>, ctx: Option<serde_json::Value>) {
    let entry = ClientLogEntry {
        ...
        ver: Some(ainxt_version::VERSION.to_owned()),
        lvl,
...
fn send_entries(entries: Vec<ClientLogEntry>) {
    ...
    handle.spawn(async move {
        let _ = ainxt_acp_lib::acp_send(notification, &tx).await;
    });
}
