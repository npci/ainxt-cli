...
/// Extract user-guide docs to `<ainxt_home>/docs/user-guide/`.
///
/// Called from the pager binary startup so the model can read them from disk.
pub fn extract_user_guide_docs(ainxt_home: &std::path::Path) {
    let docs_dir = ainxt_home.join("docs").join("user-guide");
