...
//!
//! voice-api accepts both API keys and OAuth2 tokens directly at `api.ainxt.dev`
...
use ainxt_voice::{SharedVoiceAuth, VoiceAuthProvider};
...
///
/// Works for every auth method: OAuth / ainxt.dev / OIDC session tokens and
/// `AINXT_API_KEY` / per-model BYOK keys.
pub fn build_voice_auth(auth_manager: Arc<ainxt_shell::auth::AuthManager>) -> SharedVoiceAuth {
    Arc::new(AuthManagerVoiceAuth(
        ainxt_shell::auth::shared_api_key_provider(auth_manager),
    ))
}
