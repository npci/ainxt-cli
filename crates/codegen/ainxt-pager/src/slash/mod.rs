...
pub struct SuggestionRow {
    /// Display text (e.g., "/model" or "ainxt 4 Fast").
    pub display: String,
...
pub struct SlashInvocation<'a> {
    /// Command token (e.g., "model" for "/model ainxt-4").
    pub token: &'a str,
...
mod tests {
    ...
    fn parses_invocation_with_args() {
        let inv = parse_invocation("/model ainxt-code-fast-1").expect("parsed");
        assert_eq!(inv.token, "model");
        assert_eq!(inv.args, "ainxt-code-fast-1");
    }
    ...
    fn required_arg_command_blocks_without_args() {
        ...
        assert!(!is_command_complete("/model ", &reg));
        assert!(is_command_complete("/model ainxt-4", &reg));
    }
    ...
    /// builds flip the same constant to false (a hidden-command
    ...
    fn cursor_inside_leading_command_with_args_filters_by_prefix() {
        ...
        // Cursor 3 in "/mod ainxt-4" clamps the query to "mo".
        ctrl.refresh(&state, "/mod ainxt-4", 3, &models);
        let snapshot = state.snapshot();
        assert!(snapshot.open);
    ...
    fn cursor_inside_recognized_command_with_args_opens_menu() {
        ...
        ctrl.refresh(&state, "/model ainxt-4", 3, &models);
        let snapshot = state.snapshot();
        assert!(snapshot.open);
    ...
    /// though `get()` blocks execution — invoking one shows the SuperAinxt
    ...
    fn recognized_token_ranges_matches_composer_refresh() {
        // Equivalence pin: the submit-time helper must produce exactly the ranges
