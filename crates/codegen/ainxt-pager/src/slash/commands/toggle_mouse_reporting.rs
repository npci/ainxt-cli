...
/// Toggle terminal mouse reporting (mouse capture). Matches the `Ctrl+R`
...
impl SlashCommand for ToggleMouseReportingCommand {
    ...
    fn description(&self) -> &str {
        "Switch terminal mouse reporting on or off (for native click-drag copy/paste)"
    }
        ...
        if crate::app::mouse_reporting_toggle_enabled() {
            ...
            CommandResult::Message(
                "Mouse reporting toggle is off. Set `[ui] mouse_reporting_toggle = true` \
                 in ~/.ainxt/config.toml to enable it."
                    .to_string(),
            )
        }
    }
