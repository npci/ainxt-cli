...
use ainxt_shell::sampling::types::supports_reasoning_effort_meta;
...
impl SlashCommand for ModelCommand {
    ...
    fn suggest_args(&self, ctx: &AppCtx, args_query: &str) -> Option<Vec<ArgItem>> {
        ...
        if !ctx.models.is_empty()
            && let Some(model_id) = detect_effort_phase(ctx.models, args_query)
        {
            return Some(build_effort_items(ctx.models, &model_id));
        }
        let mut items = build_model_items(ctx.models);
        // Always reachable, even with zero configured models — the primary
        // discoverability path into the model editor (no slash command to
        // memorize). See `dispatch_open_model_editor`.
        items.push(add_model_item());
        Some(items)
    }

    fn run(&self, ctx: &mut CommandExecCtx, args: &str) -> CommandResult {
        let trimmed = args.trim();
        if trimmed == ADD_MODEL_SENTINEL {
            return CommandResult::Action(Action::OpenModelEditor);
        }
        if trimmed.is_empty() {
        ...
        // often contain spaces ("ainxt 4.5"); if we split on the last token
        // first, a shorter catalog entry ("ainxt") would steal the prefix and
        ...
        // level (e.g. `none` on ainxt-4.5) surfaces the effort error with the
...
/// Sentinel `insert_text` for the synthetic "+ Add new model..." row. This
/// round-trips through the prompt text buffer (formatted as `/model
/// <insert_text>`), so it must be plain printable text — not a NUL/control
/// byte, which a text widget could mishandle. Collision with a real model
/// id/name is not realistically possible.
const ADD_MODEL_SENTINEL: &str = "__ainxt_add_new_model__";

fn add_model_item() -> ArgItem {
    ArgItem {
        display: "+ Add new model...".into(),
        match_text: "add new model configure".into(),
        insert_text: ADD_MODEL_SENTINEL.into(),
        description: "Set up a [model.*] entry without editing config.toml".into(),
    }
}
...
mod tests {
    ...
    fn empty_query_returns_one_row_per_logical_model() {
        ...
        let (pid, pinfo) = plain_model("ainxt-4.5", "ainxt 4.5");
        ...
        // 2 logical models + the always-appended "+ Add new model..." row.
        assert_eq!(items.len(), 3, "model phase: one row per logical model, plus add-model");
        ...
        let plain = items.iter().find(|i| i.match_text == "ainxt 4.5").unwrap();
        assert_eq!(plain.insert_text, "ainxt 4.5");
    }
    ...
    fn partial_model_query_stays_in_model_phase() {
        ...
        // 1 logical model + the always-appended "+ Add new model..." row.
    ...
    fn run_prefers_full_multi_word_model_name_over_prefix_plus_effort() {
        // Catalog has both "ainxt" (reasoning) and "ainxt 4.5". `/model ainxt 4.5`
        // must select the full name, not treat "4.5" as an effort on "ainxt".
        let mut state = ModelState::default();
        let (short_id, short_info) = model_with_reasoning("ainxt", "ainxt");
        let (long_id, long_info) = model_with_reasoning("ainxt-4.5", "ainxt 4.5");
        state.available.insert(short_id, short_info);
        state.available.insert(long_id.clone(), long_info);
        let mut ctx = dummy_exec_ctx(&state);
        let result = ModelCommand.run(&mut ctx, "ainxt 4.5");
        match result {
            CommandResult::Action(Action::SetDefaultModel(resolved_id)) => {
                assert_eq!(resolved_id, long_id);
            }
            other => panic!("expected SetDefaultModel(ainxt 4.5), got {other:?}"),
        }
    }
    ...
    fn run_rejects_effort_for_non_reasoning_model() {
        let mut state = ModelState::default();
        let (id, info) = plain_model("ainxt-4.5", "ainxt 4.5");
        state.available.insert(id, info);
        let mut ctx = dummy_exec_ctx(&state);
        let result = ModelCommand.run(&mut ctx, "ainxt 4.5 high");
    ...
    /// Case-insensitive matching against the catalog: `/model ainxt 4.5`
    /// resolves to the same `ModelId` as `/model ainxt 4.5`.
    #[test]
