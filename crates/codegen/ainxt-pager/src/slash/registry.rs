...
pub struct CommandRegistry {
    ...
    /// gate (the hidden-command mechanism) for state the command object cannot
...
impl CommandRegistry {
    ...
    /// but disappear from `get()` — invoking one shows the SuperAinxt upsell
    ...
    fn rebuild_triggers(&mut self) {
        ...
        for (idx, command) in self.commands.iter().enumerate() {
            ...
            // invoking one shows the SuperAinxt upsell instead.
