...
pub enum CommandResult {
    ...
    /// Included for TUI equivalence; no phase-1 command uses this directly.
    Handled,
    /// Command handled but was a no-op (e.g., model already selected).
    /// Included for TUI equivalence. Dispatch treats it identically to Handled.
    HandledNoOp,
...
pub trait SlashCommand: Send + Sync {
    ...
    /// minimal by default (the mode is converging toward equivalence). Override to
