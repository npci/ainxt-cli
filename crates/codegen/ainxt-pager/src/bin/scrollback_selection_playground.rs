...
use ainxt_pager::scrollback::text_selection::{
...
use ainxt_pager::scrollback::types::slice_display_cols;
use ainxt_pager::scrollback::{RenderBlock, ScratchBuffer, ScrollbackPane, ScrollbackState};
...
fn selection_highlight_duration_ms() -> u64 {
    ...
    if ainxt_pager::appearance::cache::load_keep_text_selection().holds() {
        0
    } else {
        PLAYGROUND_FLASH_MS
    }
...
fn double_click_action_label() -> &'static str {
    if ainxt_pager::appearance::cache::load_keep_text_selection().selects_word() {
        "word_select"
    } else {
        "toggle_fold"
    }
...
impl App {
    fn new() -> Self {
        ...
        scrollback.push_block(RenderBlock::agent_message(
            "I'm ainxt, an interactive CLI agent built to help with software engineering tasks like coding, debugging, refactoring, and exploring codebases.",
        ));
        scrollback.push_block(RenderBlock::agent_message(
