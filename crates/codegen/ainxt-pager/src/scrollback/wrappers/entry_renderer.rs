...
impl<'a> EntryRenderer<'a> {
    ...
    ///
    /// Matches `desired_height` but derives the content line count from the block's
    /// raw source text (`searchable_text`) instead of laying it out. Lets the layout
    /// cache size off-screen entries on a bulk load (`ainxt -r`) without word-wrapping
    ...
    ///
    /// EXACT for blocks whose searchable text matches their selectable rendered
...
mod tests {
    ...
    fn gutter_keeps_code_block_background_on_no_background_block() {
        ...
        // carries no code background while this test's local `ainxtnight()`
    ...
    fn affordance_row_adds_one_row_per_diagram() {
        ...
        // invariant a bulk load (`ainxt -r`) relies on to avoid clipping.
