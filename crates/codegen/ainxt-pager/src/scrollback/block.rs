...
impl RenderBlock {
    ...
    pub fn context_info(
        snapshot: ainxt_shell::session::ContextInfo,
    ...
    /// return their stored source fields unchanged. Returns `None` for blocks
    ...
    pub fn with_hyperlinks<R>(
        &self,
        f: impl FnOnce(&[ainxt_markdown::HyperlinkTarget]) -> R,
    ) -> R {
        match self {
...
mod searchable_text_tests {
    ...
    use ainxt_shell::session::ContextInfo;
    ...
    fn subagent_failed_indexes_metadata_and_error() {
        ...
        assert!(text.contains("ainxt-test"), "got: {text:?}");
    ...
    fn context_info_indexes_model_only() {
        ...
        let block = RenderBlock::context_info(snapshot, "ainxt-4.5");
        // Only the model name is source text; the rest is a numeric breakdown.
        assert_eq!(block.searchable_text().as_deref(), Some("ainxt-4.5"));
    }
