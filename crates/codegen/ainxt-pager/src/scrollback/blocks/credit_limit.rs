...
//! (SuperAinxt Heavy). Instead of offering "Upgrade tier" + PAYG / buy-credits
...
mod tests {
    ...
    fn output_payg_off_mentions_enabling() {
        ...
        // Asserts the URL handed to the constructor above is rendered. Read
        // "ainxt.dev?_s=usage" until 2026-08-29: the constructor argument was
        // renamed to the reserved example.test domain but this assertion was
        // not, so it required the block to render a domain nobody passed in.
        assert!(all_text.contains("ainxt.example.test?_s=usage"));
    }
    ...
    fn output_structure_and_content() {
        let url = "https://ainxt.example.test?_s=usage";
