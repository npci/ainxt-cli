...
pub enum SessionEvent {
    ...
    /// Message comes from agent via AinxtSessionUpdate::HookAnnotation.
    HookAnnotation {
        /// The hook message
        message: String,
    },
...
impl SessionEventBlock {
    ...
    ///
    /// Matches [`OtherToolCallBlock`](super::OtherToolCallBlock): a bold
    ...
    ///
    /// Text selection matches [`ThinkingBlock`](super::ThinkingBlock): the
...
impl BlockContent for SessionEventBlock {
    ...
    fn is_selectable(&self) -> bool {
        ...
        // has body content (matches `is_foldable`), so j/k never lands on the
...
mod tests {
    ...
    fn model_unavailable_with_switch() {
        let event = SessionEvent::ModelUnavailable {
            previous_model_id: "ainxt-4.5".into(),
            new_model_id: "ainxt-build".into(),
            reason: "Model \"ainxt-4.5\" is no longer available.".into(),
        };
        assert_eq!(
            event.message(),
            "Model \"ainxt-4.5\" is no longer available. Switched to \"ainxt-build\"."
        );
    }
    ...
    fn recap_loading_or_empty_is_not_selectable_or_foldable() {
        ...
        // that can't fold and offers no interaction (matches `is_foldable`).
        for summary in ["", "   \n  "] {
            let block = SessionEventBlock::new(SessionEvent::Recap {
    ...
    fn marker_with_stop_hooks_is_interactive_and_starts_collapsed() {
        ...
        // All-skipped batches change nothing (matches has_content()).
        use super::super::tool::HookRunStatus;
    ...
    fn still_running_text_is_the_work_only_status_form() {
        // The between-turns status line reuses the marker suffix unchanged
