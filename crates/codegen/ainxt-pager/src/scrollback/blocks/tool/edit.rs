...
mod tests {
    ...
    fn wrapped_header_path_provenance_survives_all_wrap_shapes() {
        ...
        for path in cases {
            for width in 8..=48 {
                // Matches production: expanded headers are prefix + path only
    ...
    /// Pins AinxtNight via the shared test-lock guard; hold it for the whole
    /// test so a concurrent theme flip can't skew the compared highlighter walks.
    fn pin_ainxtnight_syntect() -> std::sync::MutexGuard<'static, ()> {
        let guard = crate::theme::cache::pin_theme();
        assert!(
            !Theme::ainxtnight().diff_uses_line_fg(),
            "AinxtNight must be banded for Equal-line syntect path in render"
        );
        guard
    }
    ...
    /// Caller pins the theme via `pin_ainxtnight_syntect` (lock is not reentrant).
...
"
    ...
    fn syntax_highlight_splits_keyword_and_string_fg() {
        let _guard = pin_ainxtnight_syntect();
