...
//! the generic [`CodeBlockSpan`](ainxt_markdown::CodeBlockSpan) API) and
...
use ainxt_markdown::MarkdownRenderView;
...
pub struct MermaidBlock {
    ...
    /// from [`CodeBlockSpan::body`](ainxt_markdown::CodeBlockSpan::body). For
    ...
    /// [`CodeBlockSpan::output_line_range`](ainxt_markdown::CodeBlockSpan::output_line_range).
...
) -> impl Iterator<Item = &'a ainxt_markdown::CodeBlockSpan> {
    view.code_blocks
        .iter()
...
///
/// `AinxtDay` is the only light theme; every other concrete theme (and the
/// `AinxtNight` default that `Auto` resolves to before it reaches the cache) is
/// dark. The render worker maps this to `ainxt_mermaid::MermaidTheme`; it
...
pub fn theme_is_dark(theme: ThemeKind) -> bool {
    !matches!(theme, ThemeKind::AinxtDay)
}
...
mod tests {
    ...
    #[test]
    fn theme_is_dark_maps_ainxtday_to_light_only() {
        assert!(
            !theme_is_dark(ThemeKind::AinxtDay),
            "AinxtDay is the light theme"
        );
        for dark in [
            ThemeKind::AinxtNight,
