...
impl ScrollbackState {
    ...
    pub fn set_execute_output(&mut self, id: EntryId, output: &str) -> bool {
        ...
        {
            // Replace output entirely — ainxt-shell sends full accumulated buffer each tick.
            // The shell now sends clean output (no ANSI codes) when the client sets
            // ainxt.dev/bashOutputNoColor: true, so no stripping is needed.
    ...
    pub fn clear_all_pending_user_input(&mut self) {
        // Real transitions are structural (matches `set_pending_user_input`):
