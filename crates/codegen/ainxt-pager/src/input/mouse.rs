...
//! `scroll_mode` setting (or `AINXT_SCROLL_MODE`) pins [`ScrollInputMode`], and `invert_scroll` /
...
// `ainxt-pager-pty-harness/src/scroll_matrix/gestures.rs` (the harness
// deliberately has no pager dependency). Its gesture tables and invariants
// are shaped around these default values, so retune the matches together
// with any change here. Runtime may override via `set_redraw_cadence`
// (`AINXT_SCROLL_CADENCE_MS` from the event loop); the harness still models
// the 16ms default.
...
pub struct ScrollDebugSnapshot {
    ...
    pub flush_cap: i32,
    /// Effective scroll flush cadence in ms (`AINXT_SCROLL_CADENCE_MS` / default 16).
    pub cadence_ms: u64,
}
...
///   redraw cadence (default 16ms / ~60Hz; `AINXT_SCROLL_CADENCE_MS`) to avoid floods.
...
pub struct MouseScrollState {
    ...
    /// `AINXT_SCROLL_LOG` flight recorder ([`crate::input::scroll_log`]).
...
impl MouseScrollState {
    ...
    /// Inject flush cadence (`AINXT_SCROLL_CADENCE_MS`); Default stays 16ms.
    ...
    /// Emit a flight-recorder line; a single branch when `AINXT_SCROLL_LOG`
