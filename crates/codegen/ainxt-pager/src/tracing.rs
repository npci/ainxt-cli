...
/// firehose (`AINXT_DEBUG_LOG` / `AINXT_LOG_FILE`).
pub use ainxt_telemetry::debug_log::ACP_UPDATE_PAYLOAD_TARGET;
/// Target for the always-on compact ACP update summary line (kind, ids,
/// status, payload sizes). Cheap to format at streaming rate.
///
/// Defined in `ainxt-telemetry` so the firehose directives and the pager
/// filter share one constant (re-exported here for callsites).
pub use ainxt_telemetry::debug_log::ACP_UPDATE_TARGET;
...
pub fn init_tracing() -> TracingHandle {
    ...
    let directives = format!(
        "ainxt_shell=info,ainxt_pager=trace,ainxt_tools=info,ainxt_acp_lib=info,{RMCP_SSE_NOISE_TARGET}=error,sampling_log=off,{ACP_UPDATE_TARGET}=debug,{ACP_UPDATE_PAYLOAD_TARGET}={payload_level}"
    );
    ...
    let otel_layer = ainxt_telemetry::otel_layer::build_otel_layer(
    ...
    let instrumentation_layer = ainxt_telemetry::instrumentation::layer();
    let sampling_log_layer = ainxt_telemetry::sampling_log::layer();
    let hooks_log_layer = ainxt_telemetry::hooks_log::layer();
    let registry = tracing_subscriber::registry()
        ...
        .with(otel_layer);
    ainxt_telemetry::debug_log::install_firehose(registry, "tui");
