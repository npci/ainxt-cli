...
fn execute_hook(
    ...
    cmd.arg("-c")
        .arg(command)
        .env("AINXT_EVENT", event_str)
        .env("AINXT_MESSAGE", message)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    if let Some(sid) = session_id {
        cmd.env("AINXT_SESSION_ID", sid);
    }
...
mod tests {
    ...
    fn sets_environment_variables() {
        ...
        let command = format!(
            "printf 'AINXT_EVENT=%s\\nAINXT_MESSAGE=%s\\nAINXT_SESSION_ID=%s\\n' \
             \"$AINXT_EVENT\" \"$AINXT_MESSAGE\" \"$AINXT_SESSION_ID\" > {}",
            out.display()
        );
        ...
        assert!(
            content.contains("AINXT_EVENT=Turn complete"),
            "missing AINXT_EVENT: {content}"
        );
        assert!(
            content.contains("AINXT_MESSAGE=hello world"),
            "missing AINXT_MESSAGE: {content}"
        );
        assert!(
            content.contains("AINXT_SESSION_ID=sess-42"),
            "missing AINXT_SESSION_ID: {content}"
        );
    }
    ...
    fn omits_session_id_when_none() {
        ...
        let content = std::fs::read_to_string(&out).unwrap();
        assert!(
            !content.contains("AINXT_SESSION_ID"),
            "AINXT_SESSION_ID should not be set: {content}"
        );
    }
    ...
    fn run_hook_passes_correct_env_via_thread() {
        ...
        assert!(content.contains("AINXT_EVENT=Turn complete"));
        assert!(content.contains("AINXT_MESSAGE=test body payload"));
        assert!(content.contains("AINXT_SESSION_ID=test-session-123"));
    }
}
