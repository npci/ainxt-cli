...
impl NotificationConfig {
    ...
    ///
    /// Matches the pattern used by `RawAppearanceConfig::to_toml_with_comments()`
...
# AINXT_SESSION_RECAP=0. Manual /recap uses only the shell flag.
session_recap = true
...
# session-name, cwd, model, turn-timer, ainxt
...
mod tests {
    ...
    fn config_round_trips_through_toml() {
        let config = NotificationConfig {
            ...
            title: TitleConfig {
                enabled: false,
                items: vec![TitleItem::Ainxt, TitleItem::Cwd],
            },
