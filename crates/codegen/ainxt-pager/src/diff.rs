...
use ainxt_tools::types::output::SearchReplaceEditDetail;
...
fn stitch_hunk_pair(a: &DiffHunk, b: &DiffHunk) -> Option<DiffHunk> {
    ...
    while i < b.len() {
        ...
        if row.ln > max_ln {
            ...
            // tail, so splice its remaining rows in unchanged (rendered rows
...
mod tests {
    ...
    fn extract_edit_hunks_from_diff_meta_structured() {
        ...
        use agent_client_protocol as acp;
        use std::sync::Arc;
        use ainxt_tools::types::output::SearchReplaceEditContextInformation;
