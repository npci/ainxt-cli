...
pub struct PickerConfig<'a> {
    ...
    /// gate this to `load_vim_mode()`. Matches scrollback vim-mode.
...
pub fn render_picker(
    ...
    if config.disable_search {
        ...
        // Shortcuts" for the cheatsheet modal). Matches the search-bar
    ...
    if let Event::Key(key) = ev {
        ...
        if (config.show_search_hint || config.vim_normal_first) && state.search_active {
            if key.code == KeyCode::Esc {
                ...
                // query in one step (matches scrollback vim-mode).
        ...
        if state.tabs_focused {
            ...
            // a search query (matches behavior from non-active search hint).
        ...
        // way into search (printable chars don't type until then). Matches
        ...
        if !config.show_search_hint && !config.disable_search && !config.vim_normal_first {
            ...
            // this condition matches the renderer's: the advertised chord
...
mod tests {
    ...
    // exercise the dormant nav mode; everything else matches a basic picker.
    ...
    fn underline_last_desc_underlines_only_the_link_line() {
        ...
        // Render <-> hit-test equivalence: the recorded band equals the painted rows.
