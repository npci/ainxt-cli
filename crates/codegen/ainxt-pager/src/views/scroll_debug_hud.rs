...
//! loop. Recipe: `AINXT_FPS=1 AINXT_SCROLL_DEBUG=1 ainxt --resume <session>`,
//! then flip `scroll_mode` / `scroll_lines` / `invert_scroll` /
//! `scroll_speed` in `/settings` to compare variants live. For event-exact
//! capture beyond this per-frame sampling, add `AINXT_SCROLL_LOG=1` — the
...
//! precedent): dev instrumentation alters the frame pipeline
...
/// Runtime enablement for the HUD. Matches `FrameMetrics`' env machinery:
/// `AINXT_SCROLL_DEBUG` (nonempty and not `"0"`) enables at startup, and the
...
impl ScrollDebugHud {
    pub fn new() -> Self {
        let env_on = std::env::var("AINXT_SCROLL_DEBUG").is_ok_and(|v| !v.is_empty() && v != "0");
        Self { enabled: env_on }
    }
...
pub struct ScrollDebugPanel {
    ...
    /// Rows left free for FPS overlays stacked above (the dev `AINXT_FPS`
