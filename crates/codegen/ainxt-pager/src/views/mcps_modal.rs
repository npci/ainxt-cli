...
/// Display label for a section header, e.g. `"Managed by your deployment (3)"`.
/// Named for the deployment rather than a vendor domain: this build has no
/// hosted backend, and the previous label showed `ainxt.dev`, which is not a
/// domain the project owns (see the R42 note in ainxt-env).
pub fn section_label(section: &McpSectionId, count: usize) -> String {
    match section {
        McpSectionId::Managed => format!("Managed by your deployment ({count})"),
        McpSectionId::Plugin(name) => format!("Plugin: {name} ({count})"),
        McpSectionId::Local => format!("Local ({count})"),
    }
}

/// Base connectors URL (no team). Prefer [`managed_connectors_url`] when opening.
///
/// Resolved through `ainxt_env` so a fork or a self-hosted deployment points at its
/// own host via `AINXT_URL_CONNECTORS` instead of patching this constant.
pub fn managed_section_connectors_url() -> String {
    ainxt_env::url_connectors().into_owned()
}

/// Connectors deep link, appending percent-encoded `teamId` when the session is a team principal.
///
/// Returns an empty string when no connectors URL is configured. The
/// open-source build ships without one (audit risk R42), and a bare
/// `?teamId=team-1` is not a link -- callers check with `url_is_set` and omit
/// the affordance rather than offering one that goes nowhere.
pub fn managed_connectors_url(team_id: Option<&str>) -> String {
    let base = managed_section_connectors_url();
    if !ainxt_env::url_is_set(&base) {
        return String::new();
    }
    match team_id.filter(|id| !id.is_empty()) {
        Some(id) => format!("{base}?teamId={}", urlencoding::encode(id)),
        None => base,
    }
}
...
pub fn section_description_lines(section: &McpSectionId, team_id: Option<&str>) -> Vec<String> {
    match section {
        McpSectionId::Managed => {
            ...
            if url.is_empty() {
                // No managed-connectors service in this build. Say so, rather
                // than printing an empty pair of brackets under the header.
                return vec![
                    "No managed-connectors service is configured for this build.".into(),
                    "Set AINXT_URL_CONNECTORS to your deployment's connectors page.".into(),
                ];
            }
            vec![
...
///
/// Priority: `ainxt_com_` prefix or managed wire source → Managed; else plugin
...
pub fn section_for(server: &McpServerInfo) -> McpSectionId {
    if server.name.starts_with("ainxt_com_") || server.wire_source == McpWireSource::Managed {
        McpSectionId::Managed
...
pub fn is_removable(server: &McpServerInfo) -> bool {
    server.wire_source == McpWireSource::Local && !server.name.starts_with("ainxt_com_")
}
...
/// Patch a single server row in-place from an `ainxt.dev/mcp/server_status`
/// push.
///
...
mod tests {
    ...
    #[test]
    fn section_description_lines_managed_reflect_configuration() {
        // These assert against the RESOLVED base rather than a literal, because
        // the open-source build ships no connectors URL and a fork may point at
        // its own. Hardcoding a vendor host here made the test a statement about
        // branding rather than about behaviour.
        let base = managed_section_connectors_url();
        let lines = section_description_lines(&McpSectionId::Managed, None);
        assert_eq!(lines.len(), 2);

        if base.is_empty() {
            // Unconfigured: explain, and never render an empty `[]`.
            assert!(
                lines.iter().any(|l| l.contains("AINXT_URL_CONNECTORS")),
                "should name the variable that enables this: {lines:?}"
            );
            assert!(
                !lines.iter().any(|l| l.contains("[]")),
                "must not render empty brackets: {lines:?}"
            );
            let with_team = section_description_lines(&McpSectionId::Managed, Some("team-1"));
            assert!(
                !with_team.iter().any(|l| l.contains("teamId")),
                "a bare query string is not a link: {with_team:?}"
            );
        } else {
            assert!(
            ...
            let shown = base.strip_prefix("https://").unwrap_or(&base);
            assert_eq!(lines[1], format!("[{shown}]"));
            assert!(
            ...
            let with_team = section_description_lines(&McpSectionId::Managed, Some("team-1"));
            assert_eq!(with_team[1], format!("[{shown}?teamId=team-1]"));
        }
    }
    ...
    fn managed_connectors_url_display_strips_scheme() {
        let base = managed_section_connectors_url();
        let shown = base.strip_prefix("https://").unwrap_or(&base);
        assert_eq!(managed_connectors_url_display(None), shown);
        if !base.is_empty() {
            assert_eq!(
                managed_connectors_url_display(Some("team-uuid-1")),
                format!("{shown}?teamId=team-uuid-1")
            );
        }
    }
    ...
    fn managed_connectors_url_appends_team_id_when_present() {
        // Compare against the resolved base rather than a literal, so these keep
        // holding when AINXT_URL_CONNECTORS points the deployment somewhere else.
        let base = managed_section_connectors_url();
        assert_eq!(managed_connectors_url(None), base);
        assert_eq!(managed_connectors_url(Some("")), base);
        if !base.is_empty() {
            assert_eq!(
                managed_connectors_url(Some("team-uuid-1")),
                format!("{base}?teamId=team-uuid-1")
            );
            assert_eq!(
                managed_connectors_url(Some("a b/c")),
                format!("{base}?teamId={}", urlencoding::encode("a b/c"))
            );
        }
    }
    ...
    #[test]
    fn section_for_ainxt_com_with_plugin_label_is_managed() {
    ...
    #[test]
    fn is_removable_rejects_ainxt_com_prefix() {
        let server = server_from_wire("ainxt_com_slack", Some("local"), None);
        assert!(!is_removable(&server));
    }
    ...
    fn convert_list_response_classifies_managed_gateway_only_for_gateway_rows() {
        ...
        let legacy_managed = server_from_wire("ainxt_com_slack", Some("managed"), None);
