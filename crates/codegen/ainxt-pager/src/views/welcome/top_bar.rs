...
mod tests {
    ...
    fn format_cwd_plain_repo() {
        assert_eq!(format_cwd_parts("~/ainxt", None), "~/ainxt");
    }
    ...
    fn format_cwd_worktree_shows_main_repo() {
        assert_eq!(
            format_cwd_parts("~/wt/session-1", Some("~/ainxt")),
            "~/wt/session-1 (worktree of ~/ainxt)"
        );
    }
    ...
    /// is outside `$HOME`, so `collapse_home` leaves it unchanged.)
    #[test]
    fn format_cwd_display_shows_subdir_not_repo_root() {
        ...
        assert_eq!(
            format_cwd_display(Path::new("/work/ainxt/frontend/apps"), Some(&info)),
            "/work/ainxt/frontend/apps",
        );
    }
    ...
    fn format_cwd_display_cache_miss_shows_raw_cwd() {
        assert_eq!(
            format_cwd_display(Path::new("/work/ainxt/frontend/apps"), None),
            "/work/ainxt/frontend/apps",
        );
    }
}
