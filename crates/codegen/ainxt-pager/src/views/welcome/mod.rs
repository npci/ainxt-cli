...
struct WelcomeLayoutInput<'a> {
    ...
    prompt_compact: bool,
    announcement: Option<&'a ainxt_announcements::RemoteAnnouncement>,
...
pub(super) enum VersionBadgeMode<'a> {
    /// Full badge: team | tier | api_key | **ainxt** VERSION+channel **Beta** (right-aligned).
    Full { subscription_tier: Option<&'a str> },
    /// Hero footer: team | api_key | ainxt Beta [channel] (right-aligned, gray).
    HeroFooter,
    /// Hero inline: **ainxt Beta**  VERSION (left-aligned).
    HeroInline,
}
...
pub(super) fn render_version_badge(
    ...
    let channel = ainxt_update::channel_label();
    ...
    if show_api_key && is_api_key_auth {
        ...
        // In HeroFooter mode this separator precedes only the channel badge,
        // which is omitted entirely when there's no channel to show (see
        // below) — skip it here too so we don't leave a dangling separator
        // with nothing after it.
        let more_follows =
            !matches!(&mode, VersionBadgeMode::HeroFooter) || !channel.trim().is_empty();
        if more_follows {
            spans.push(sep);
        }
    }
    match &mode {
        VersionBadgeMode::Full { .. } => {
            ...
            spans.push(Span::styled(
                format!("{}{}", ainxt_version::VERSION, channel),
                Style::default().fg(theme.gray),
            ));
        }
...
pub struct WelcomeRenderParams<'a> {
    ...
    pub announcement: Option<&'a ainxt_announcements::RemoteAnnouncement>,
    ...
    pub foreign_resume_hint: Option<&'a ainxt_workspace::foreign_sessions::RecentForeignSession>,
    pub is_api_key_auth: bool,
    pub session_picker_content_results:
        Option<&'a [ainxt_shell::extensions::session_search::SearchSessionHit]>,
    ...
    pub gate: Option<&'a ainxt_shell::auth::GateInfo>,
...
pub fn render_welcome(
    ...
    let mut result = match params.auth_state {
        AuthState::Pending { error } => {
            let label = params.login_label.unwrap_or("ainxt.dev");
        ...
        AuthState::Done if params.is_zdr_blocked => {
            ...
            let (menu_rects, post_flush_escapes) = render_welcome_blocked(
                ...
                buf,
                Some((
                    "ainxt is not yet available for this account.",
                    theme.gray_bright,
                )),
                &menu,
                params.selected,
        ...
        // created, when the cwd has untrusted repo-local config. Matches the
...
/// Render the folder-trust question. Matches [`render_welcome_blocked`]'s
/// stacked layout (logo + message + menu + version badge), but the message is a
/// multi-line block showing the workspace path and the warning that ainxt
...
/// `menu_rects` are returned for equivalence with the other welcome arms.
fn render_welcome_trust(
    ...
    let lines = vec![
        ...
        Line::from(Span::styled(
            "ainxt may run or modify contents in this directory,",
            Style::default().fg(theme.gray),
        ))
        .alignment(Alignment::Center),
        Line::from(Span::styled(
...
fn render_announcement_section(
    ...
    buf: &mut Buffer,
    theme: &Theme,
    announcement: &ainxt_announcements::RemoteAnnouncement,
    min_width_hint: u16,
    content_height: u16,
    expanded: bool,
    mouse_pos: Option<(u16, u16)>,
...
fn render_welcome_done(
    ...
    let gate_menu;
    let owned_menu;
    let _ = (key_g, cta); // upgrade CTA intentionally not shown in ainxt
    let menu_items: &[(&str, &str)] = if !p.has_access {
        // ainxt has no upsell: when access is gated, offer only Logout / Quit.
    ...
    let (cursor_pos, post_flush_escapes) = if show_picker {
        ...
        let gate_text = p
            ...
            .unwrap_or("SuperAinxt subscription required");
        let msg = Line::from(Span::styled(
            gate_text,
            Style::default().fg(theme.gray_bright),
        ))
        ...
        if centered.height > 2 {
            ...
            let gate_link = p
                ...
                .unwrap_or("https://ainxt.example.test/subscribe");
...
mod tests {
    ...
    fn foreign_resume_tip_names_each_tool_and_age() {
        use ainxt_workspace::foreign_sessions::ForeignSessionTool;
        ...
        for (tool, label) in [
            ...
            let hint = ainxt_workspace::foreign_sessions::RecentForeignSession {
                tool,
    ...
    fn grouped_entries_insert_headers() {
        let entries = vec![
            make_entry("s1", "Fix auth", "ainxt"),
            make_entry("s2", "Add streaming", "ainxt"),
        ...
        // Groups are sorted alphabetically, so "ainxt" (2 rows) precedes "fw-1"
        // (1 row):  0 header "ainxt", 1-2 rows, 3 header "fw-1", 4 row.
        //
        // This block asserted the mirror image — header, row, header, row, row —
        // with the comment "fw-1 before ainxt". That held when the group was named
        // "grok" ("fw-1" < "grok"); renaming it to "ainxt" inverted the sort and
        // the fixture layout, but the expected positions were never updated.
        assert_eq!(non_sel.len(), 5);
        assert!(non_sel[0], "entry 0 should be the \"ainxt\" header");
        assert!(!non_sel[1], "entry 1 should be a selectable row");
        assert!(!non_sel[2], "entry 2 should be a selectable row");
        assert!(non_sel[3], "entry 3 should be the \"fw-1\" header");
        assert!(!non_sel[4], "entry 4 should be a selectable row");

        // Verify headers
        assert!(
            matches!(&result[0], crate::views::picker::PickerEntry::Header { label } if label == &"ainxt")
        );
        assert!(
    ...
    fn grouped_entries_rows_are_indented() {
        let entries = vec![make_entry("s1", "Fix auth", "ainxt")];
    ...
    fn extract_user_code_parses_verification_url() {
        assert_eq!(
            extract_user_code("https://accounts.example.test/oauth2/device?user_code=ABCD-EFGH"),
            Some("ABCD-EFGH"),
        );
        // Trailing params after the code are ignored.
        assert_eq!(
            extract_user_code("https://ainxt.example.test/oauth2/device?user_code=WXYZ-1234&foo=bar"),
            Some("WXYZ-1234"),
        );
        // A param whose name merely ends in `user_code` must not be matched.
        assert_eq!(
            extract_user_code("https://ainxt.example.test/d?foo_user_code=BAD&user_code=GOOD"),
            Some("GOOD"),
        );
        // No code param, empty code, and unexpected characters all yield None.
        assert_eq!(extract_user_code("https://ainxt.example.test/oauth2/device"), None);
        assert_eq!(extract_user_code("https://ainxt.example.test/d?user_code="), None);
        assert_eq!(extract_user_code("https://ainxt.example.test/d?user_code=AB%20CD"), None);
    }
    ...
    fn device_auth_arm_shows_url_and_no_paste_box() {
        ...
        let mut buf = Buffer::empty(area);
        let theme = Theme::current();
        let url = "https://accounts.example.test/oauth2/device?user_code=ABCD-EFGH";
    ...
    fn device_auth_arm_raw_url_mode_shows_full_url() {
        ...
        let mut buf = Buffer::empty(area);
        let theme = Theme::current();
        let url = "https://accounts.example.test/oauth2/device?user_code=WXYZ-1234";
    ...
    fn raw_url_mode_uses_full_width_for_long_urls() {
        ...
        let url = "https://accounts.example.test/oauth2/device?user_code=WXYZ-1234&extra=0123456789";
    ...
    fn command_auth_arm_shows_url_and_waiting() {
        ...
        let mut buf = Buffer::empty(area);
        let theme = Theme::current();
        let url = "https://accounts.example.test/oauth2/authorize?client_id=ainxt";
    ...
    fn long_ann() -> ainxt_announcements::RemoteAnnouncement {
...
the usual channels. "
    ...
    fn announcement_equal_for_short_message() {
        let area = Rect::new(0, 0, 120, 60);
        let a = ainxt_announcements::RemoteAnnouncement {
            title: Some("FYI".into()),
