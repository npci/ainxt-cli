//! Logo component — renders the ainxt logo art (braille glyphs, U+28xx).
//!
//! Braille packs a 2x4 dot grid per character cell, giving the mark far more
//! effective resolution than half-blocks at the same line count, so the
//! interlocking "A" stays legible in the welcome header.
//!
//! Hidden entirely on legacy Windows consoles: the braille glyphs are not
//! reliably covered by ConHost raster fonts and would render as tofu.
