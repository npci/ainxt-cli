...
const HERO_SUBTITLE: &str = "Thanks for trying ainxt, give feedback with /feedback!";
...
fn right_col_height(menu_height: u16, info_height: u16) -> u16 {
    ...
    1u16.saturating_add(subtitle_rows(info_height))
        .saturating_add(info_gap)
...
pub(super) fn compute_hero_box(
    ...
    announcement: Option<&ainxt_announcements::RemoteAnnouncement>,
    expanded: bool,
    ...
    let right_header_rows = 1u16
        .saturating_add(subtitle_rows(info_height))
        .saturating_add(info_gap)
        .saturating_add(info_height)
        .saturating_add(1);
...
mod tests {
    ...
    /// shows when expanded — matches the enterprise-policy case from the bug.
...
managed devices and accounts. Report security incidents";
    ...
    fn upgrade_cta_reserves_rows_and_returns_button_rect() {
        ...
        let a = ann(None, Some("ainxt 4.5 is here. Upgrade now."));
        ...
        let mut pinned = ann(None, Some("ainxt 4.5 is here. Upgrade now."));
        pinned.dismissible = Some(false);
        pinned.cta = Some(ainxt_announcements::AnnouncementCta {
        ...
        let mut dismissible = ann(None, Some("ainxt 4.5 is here. Upgrade now."));
        dismissible.cta = Some(ainxt_announcements::AnnouncementCta {
            label: Some("Upgrade Account".into()),
            url: Some("https://ainxt.example.test/subscribe".into()),
            caption: Some("or use Ctrl+O".into()),
        });
