...
//! are written to `.ainxt/config.toml`.
...
use ainxt_shell::claude_import::{ImportPlan, ImportableItem, PathKind, find_project_root};
...
fn build_rows(
    ...
    if !plan.global_items.is_empty() {
        ...
        let label = "Global  ~/.ainxt/config.toml".to_string();
    ...
    if !plan.project_items.is_empty() {
        ...
        let project_config = find_project_root(cwd).join(".ainxt").join("config.toml");
...
mod tests {
    use super::*;
    use ainxt_shell::claude_import::PathKind;
    use ainxt_workspace::permission::types::{PatternMode, PermissionRule, ToolFilter};
    ...
    fn filtered_plan_respects_per_item_selection_after_grouping() {
        use ainxt_shell::claude_import::ImportableItem;
