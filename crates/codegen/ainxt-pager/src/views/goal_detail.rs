...
use ainxt_shell::tools::{TodoItem, TodoStatus};
...
/// is returned unchanged (control-stripped, since it's a raw passthrough).
...
mod tests {
    ...
    fn render_per_model_breakdown_shows_each_model() {
        let mut goal = make_goal();
        goal.live_tokens_by_model = vec![("ainxt-4".into(), 12_300), ("ainxt-3-mini".into(), 8_000)];
        let text = render_to_text(&goal);
        assert!(text.contains("ainxt-4"), "first model must render:\n{text}");
        assert!(
            text.contains("ainxt-3-mini"),
    ...
    fn render_per_model_truncates_wide_model_id_by_display_width() {
        ...
        // far short. Banded (not exact) to stay equivalence-robust.
    ...
    fn humanize_event_timestamp_relative_and_fallback() {
        ...
        // Non-RFC3339 (legacy / fixture) returned unchanged.
    ...
    fn humanizers_strip_control_chars_independently() {
        ...
        // detail, unknown-event name, unchanged-timestamp fallback) must strip
        ...
        // Non-RFC3339 → unchanged fallback, still stripped.
