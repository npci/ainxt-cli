...
use ainxt_acp_lib::AcpResult;
use ainxt_markdown::StreamingMarkdownRenderer;
pub use ainxt_tools::implementations::ainxt_build::ask_user_question::{
...
/// driven by an ACP `ainxt.dev/ask_user_question` request).
///
...
pub enum LocalQuestionKind {
    ...
    CreditLimitUpsell {
        choices: Vec<ainxt_telemetry::events::CreditLimitChoice>,
    },
    /// SuperAinxt upsell modal: the free-usage paywall (429 +
    ...
    /// `id`.
    FreeUsageUpsell {
        /// Telemetry source for `SuperAinxtUpsellClicked` — distinguishes
        /// the paywall from the restricted-command upsell.
        source: ainxt_telemetry::events::SuperAinxtUpsell,
    },
...
///
/// Created when an `ainxt.dev/ask_user_question` ext-method request arrives;
...
pub struct QuestionViewState {
    ...
    /// instead of by an ACP `ainxt.dev/ask_user_question` request. `None` for
...
impl QuestionViewState {
    ...
    ///
    /// Called by the `ExtMethod` handler when a blocking `ainxt.dev/ask_user_question`
...
impl QuestionViewState {
    ...
    /// questions (e.g. the SuperAinxt upsell) have no freeform row, so
...
impl QuestionViewState {
    ...
    /// - Preview included for single-select only, unchanged from the option.
    /// - Notes included when freeform text is non-empty and selected.
    pub fn build_accepted_response(
        &self,
    ) -> ainxt_tools::implementations::ainxt_build::ask_user_question::AskUserQuestionExtResponse
    {
    ...
    pub fn send_ext_response(
        &mut self,
        response: ainxt_tools::implementations::ainxt_build::ask_user_question::AskUserQuestionExtResponse,
    ) -> bool {
...
pub fn render_question_view(
    ...
    // toward `bg_light` so the panel visually recedes. Matches the
...
mod tests {
    ...
    /// `no_freeform` questions (e.g. the SuperAinxt upsell) have no "Other"
