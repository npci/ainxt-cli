...
//! `ainxt.dev/suggest` request through the Effect pipeline. Stale responses
...
/// Parsed ghost suggestion from an ACP `ainxt.dev/suggest` response.
#[derive(Debug, Clone)]
...
/// A single completion item from an ACP `ainxt.dev/suggest` response.
...
// (e.g. ainxt-pager-minimal's) compiling when optional fields are added.
#[derive(Debug, Clone, Default)]
...
/// Parsed response from an ACP `ainxt.dev/suggest` request.
#[derive(Debug, Clone)]
...
impl SuggestResponseParsed {
    /// Parse a raw JSON value from an ACP `ainxt.dev/suggest` response.
...
/// Wire `limit` for `ainxt.dev/suggest` fetches. Matches the shell file
...
pub struct SuggestionController {
    ...
    /// `AINXT_SUGGESTIONS` env var. Tab-triggered completion in bash mode
    ...
    /// Resolved at construction from `AINXT_SUGGESTIONS_AI` env var.
    pub ai_enabled: bool,
    /// Model to use for AI suggestions. Sent in the `ainxt.dev/suggest` request.
    /// Resolved at construction from `AINXT_SUGGESTIONS_AI_MODEL` env var.
...
impl SuggestionController {
    pub fn new() -> Self {
        Self {
            ...
            tab_pending: None,
            enabled: ainxt_config::env_bool("AINXT_SUGGESTIONS").unwrap_or(false),
            dropdown: CompletionDropdownState::default(),
            ai_enabled: ainxt_config::env_bool("AINXT_SUGGESTIONS_AI").unwrap_or(false),
            ai_model: std::env::var("AINXT_SUGGESTIONS_AI_MODEL")
                .ok()
                .filter(|s| !s.is_empty()),
        }
    }
    ...
    /// Called when an ACP `ainxt.dev/suggest` response arrives, with the text and
