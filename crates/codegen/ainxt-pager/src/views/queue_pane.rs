...
pub enum QueueRowOrigin {
    ...
    /// while running). Edits route to the agent as `ainxt.dev/queue/*`.
    Server,
}
...
pub struct QueuePane {
    ...
    /// `bg_highlight` — e.g. AinxtNight's dark band leaking into AinxtDay).
...
impl QueuePane {
    ...
    pub fn render(
        ...
        // at construction (default AinxtNight, dark) after the user switches —
        // painting a dark band on a light AinxtDay canvas.
...
mod tests {
    ...
    /// re-selected the wrong — often the just-deleted — row). Each step matches
