...
fn handle_browse(state: &mut SettingsModalState, key: &KeyEvent) -> SettingsKeyOutcome {
    match key.code {
        ...
        KeyCode::Backspace => {
            ...
            // re-broaden the filter without switching modes. Matches
...
///
/// Matches `memory_modal::handle_memory_mouse` for equivalence:
...
pub fn handle_settings_mouse(
    ...
    // Mouse hover highlight (equivalence with scrollback).
    ...
    match kind {
        MouseEventKind::Down(crossterm::event::MouseButton::Left) => {
            ...
            if on_triangle && let Some((key, _meta)) = state.focused_setting() {
                // Toggle expansion. Matches the keyboard
...
fn handle_picker_mouse(
    ...
    if target_idx == current_idx {
        ...
        // (kept for equivalence with the row-list's "already-focused
...
fn handle_editor_mouse(
    ...
    // (`handle_editing_value`). Matches the picker's choice-click
    // approach.
