...
/// `action_for_bool` matches `current_value_for`: every registered
...
/// so the next render paints the highlight. Matches the
...
/// to `None`. Matches the scrollback's behaviour when the mouse
...
/// with the theme's `bg_hover` color. (Matches the existing
...
fn editor_render_fixture(buffer: &str, cursor_byte: usize) -> SettingsModalState {
    ...
    let snapshot = PagerLocalSnapshot {
        available_models: vec![(
            "ainxt Test".to_string(),
            acp::ModelId::new(Arc::from("ainxt-test")),
        )],
        ..PagerLocalSnapshot::default()
    };
...
fn render_editing_value_cursor_at_logical_position_when_buffer_fits() {
    let mut s = editor_render_fixture("ainxt Test", 4); // cursor between "ainxt" and " Test"
    let area = Rect {
        x: 0,
        y: 0,
        width: 60,
...
fn picking_enum_esc_returns_to_browse() {
    ...
    match outcome {
        ...
        other => panic!("expected Action::PreviewTheme(\"ainxtnight\") on Esc, got {other:?}"),
    }
    assert!(matches!(s.mode, SettingsModalMode::Browse));
}
...
/// low-contrast theme compatibility (equivalence with `cancel_turn_panel`).
#[test]
...
fn picker_visual_smoke_debug() {
    let entries = vec![SettingMeta {
        ...
        description: "Controls whether ainxt may retain and train on coding data.",
        keywords: &["test"],
        kind: SettingKind::Enum {
            default: "opt-out",
            choices: &[
                EnumChoice {
                    canonical: "opt-in",
                    display: "Opt in",
                    description: "Allow ainxt to retain and use coding session data for training and product improvement.",
                },
                EnumChoice {
                    canonical: "opt-out",
                    display: "Opt out",
...
/// focus to that choice (same as clicking line 1). Matches the
...
fn picker_multi_line_choice_hit_rect_spans_all_lines() {
    ...
    let entries = vec![SettingMeta {
        ...
        owner: SettingOwner::Shared,
        label: "Coding data sharing",
        description: "Controls whether ainxt may retain coding data.",
        keywords: &["test"],
        kind: SettingKind::Enum {
            default: "opt-in",
            choices: &[
...
fn editing_value_chars_mutate_buffer_and_invalid_enter_is_noop() {
    ...
    // (catalog has 'ainxt 4 Fast'; "a" doesn't match) is
...
fn editing_value_esc_returns_to_browse() {
    ...
    s.mode = SettingsModalMode::EditingValue {
        key: "default_model",
        buffer: "ainxt-4".to_string(),
        cursor_byte: "ainxt-4".len(),
        validation_error: None,
    };
    let outcome = handle_settings_key(&mut s, &KeyEvent::new(KeyCode::Esc, KeyModifiers::NONE));
    assert!(matches!(outcome, SettingsKeyOutcome::Changed));
...
//    the "Tip · Ask ainxt…" docs footer and the keybindings hints.
//
...
fn section_header_style_matches_palette() {
    ...
    // render in `theme.gray_dim` for palette equivalence. Walk the
    // row and find the first `─` cell; assert its fg color.
    // Matches the `search_bar_renders_divider_below` pattern.
...
fn docs_footer_tip_is_centered() {
    ...
    // SHORT = "Tip · Ask ainxt to change a setting" (34 cells);
    ...
    assert!(
        !row_short.contains("ainxtday"),
        "width=40 must NOT render LONG path (no `ainxtday`): {row_short:?}",
    );
    assert!(
...
fn tip_line_has_blank_row_above() {
    ...
    for y in 0..area.height {
        let txt = buf_row_text(&buf, y, area.x, area.width);
        if txt.contains("Tip") && txt.contains("Ask ainxt") {
            tip_y = Some(y);
            break;
        }
    }
...
fn click_settings_breadcrumb_collapses_picker_to_browse() {
    ...
    // `"ainxtnight"`. Tightened from the previous `Action(_) |
    ...
    match outcome {
        ...
        other => panic!(
            "expected Action(PreviewTheme(\"ainxtnight\")) — the keyboard \
...
fn click_settings_breadcrumb_after_nav_reverts_to_original() {
    ...
    // Pick a different index. The default theme is `ainxtnight`
...
fn d_key_in_picking_enum_dispatches_open_reset_confirm() {
    ...
    match outcome {
        SettingsKeyOutcome::ActionPair(
            ...
            // Default theme is `ainxtnight`; entering the picker
            // captures `original_value = current value = ainxtnight`,
...
fn search_bar_renders_divider_below() {
    ...
    // would fail this — palette equivalence is "full-width divider".
    assert!(
...
/// misleading: on dark themes (AinxtNight, TokyoNight, RosePine
/// Moon) `bg_visual` is actually *lighter* than `bg_highlight`;
/// only on the AinxtDay light theme is title darker. The
...
fn max_thoughts_width_preview_title_styling_distinguishes_from_content() {
    ...
    let raw_theme = match crate::theme::Theme::current_kind() {
        crate::theme::ThemeKind::AinxtNight => crate::theme::Theme::ainxtnight(),
        crate::theme::ThemeKind::TokyoNight => crate::theme::Theme::tokyonight(),
        crate::theme::ThemeKind::AinxtDay => crate::theme::Theme::ainxtday(),
        ...
        crate::theme::ThemeKind::Auto => crate::theme::Theme::ainxtnight(),
    };
    assert_ne!(
