...
pub(super) fn render_docs_footer(buf: &mut Buffer, area: Rect, theme: &Theme) {
    const LONG: &str =
        "Tip · Ask ainxt: \"change theme to ainxtday\" or \"what does compact mode do?\"";
    const SHORT: &str = "Tip · Ask ainxt to change a setting";
...
/// Wrapped description height for scroll math (matches render path).
...
/// (parallel to the group's children) for mouse routing. Matches the enum
...
/// Matches `question_view::compute_max_label_w` semantics.
...
/// canonical unchanged if the lookup misses (defense-in-depth: a
...
fn display_for_enum_canonical<'a>(kind: &'a SettingKind, canonical: &'a str) -> &'a str {
    ...
    // Fallback: render the canonical unchanged. Defensive — catches a
...
fn render_setting_group_row(
    ...
    // Triangle prefix matches normal rows: "▾" expanded, "▸" collapsed
