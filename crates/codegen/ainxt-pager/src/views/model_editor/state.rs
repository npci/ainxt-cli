...
impl ModelEditorForm {
    ...
    /// Label for the `CredentialValue` row, specific to the chosen
    /// `credential_mode` — "Credential value" alone doesn't say whether it
    /// wants a variable *name* or the secret itself, and typing the wrong
    /// one into the wrong mode defeats the point of offering env-var mode.
    pub fn credential_value_label(&self) -> &'static str {
        match self.credential_mode {
            CredentialMode::EnvKey => "Env var name",
            CredentialMode::ApiKey => "API key",
            CredentialMode::None => "Credential value",
        }
    }

    /// Hint shown under the `CredentialValue` row while focused.
    pub fn credential_value_hint(&self) -> Option<&'static str> {
        match self.credential_mode {
            CredentialMode::EnvKey => Some(
                "Name of an environment variable that holds the key, e.g. \"OPENAI_API_KEY\" — not the key itself.",
            ),
            CredentialMode::ApiKey => Some("Pasted directly into config.toml in plaintext."),
