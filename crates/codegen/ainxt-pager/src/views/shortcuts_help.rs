...
pub fn build_entries(
    ...
    for (cat_idx, &(cat, label)) in CATEGORY_ORDER.iter().enumerate() {
        ...
        for def in defs {
            ...
            // off (remote kill switch / `AINXT_VOICE_MODE=0`). Unlike the old
...
/// matches; this matches the palette's `filter_palette_entries` behavior.
...
pub fn render_detail_body<'a>(
    ...
    // Rows are already wrapped to width, so render unchanged (no second wrap pass).
...
pub fn handle_input(
    ...
    if !searching {
        // `i` matches the vim-nav pickers' "press i to search" affordance.
...
///
/// The signature matches the destructured `ActiveModal::ShortcutsHelp`
...
/// picker `handle_input`. Matches the agent view's per-modal
...
mod tests {
    ...
    /// Mouse equivalence with the keyboard path: clicking a hint while searching opens
