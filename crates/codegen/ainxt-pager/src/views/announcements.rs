...
use ainxt_announcements::visible_announcements;
...
fn is_critical(a: &ainxt_announcements::RemoteAnnouncement) -> bool {
    a.severity.as_deref() == Some("critical")
}

fn is_promo(a: &ainxt_announcements::RemoteAnnouncement) -> bool {
...
fn is_live_critical(
    a: &ainxt_announcements::RemoteAnnouncement,
    now: chrono::DateTime<chrono::Utc>,
) -> bool {
    is_critical(a) && !ainxt_announcements::is_expired_at(a, now)
}
...
fn is_live_promo(
    ...
    is_promo(a) && !ainxt_announcements::is_expired_at(a, now)
}
...
pub fn is_dismissible(a: &ainxt_announcements::RemoteAnnouncement) -> bool {
...
fn is_hidden(
    ...
    is_dismissible(a) && hidden_ids.contains(&ainxt_announcements::announcement_hide_key(a))
}
...
fn usable_cta(a: &ainxt_announcements::RemoteAnnouncement) -> Option<(&str, &str)> {
...
pub(crate) fn usable_cta_caption(a: &ainxt_announcements::RemoteAnnouncement) -> Option<&str> {
...
fn first_critical_session_announcement<'a>(
    announcements: &'a [ainxt_announcements::RemoteAnnouncement],
    hidden_ids: &BTreeSet<String>,
) -> Option<&'a ainxt_announcements::RemoteAnnouncement> {
...
pub fn promo_cta_target<'a>(
    announcements: &'a [ainxt_announcements::RemoteAnnouncement],
    hidden_ids: &BTreeSet<String>,
) -> Option<(&'a ainxt_announcements::RemoteAnnouncement, &'a str)> {
...
pub fn session_announcement_hide_keys_at(
    ...
    visible_announcements(announcements)
        ...
        .map(ainxt_announcements::announcement_hide_key)
        .collect()
}
...
/// button). Hover matches the turn-status [stop] affordance: error red on
...
fn render_critical_rows(
    area: Rect,
    buf: &mut Buffer,
    ann: &ainxt_announcements::RemoteAnnouncement,
    hide_hovered: bool,
) -> BannerHits {
    use unicode_width::UnicodeWidthStr;
...
mod tests {
    ...
    fn promo(id: &str, message: &str, cta: Option<(&str, &str)>) -> RemoteAnnouncement {
        RemoteAnnouncement {
            ...
            cta: cta.map(|(label, url)| ainxt_announcements::AnnouncementCta {
    ...
    fn session_hide_keys_cover_live_criticals_and_promos_only() {
        ...
        let list = vec![
            ...
            promo("promo-1", "upsell", Some(("Go", "https://ainxt.example.test"))),
            expired_promo,
        ];
    ...
    /// Promo selection matches the critical gate: severity filter, hidden
    ...
    fn first_promo_selection_filters_severity_and_hidden() {
        let list = vec![
            ...
            promo("p-a", "A promo", Some(("Go", "https://ainxt.example.test"))),
    ...
    fn first_session_announcement_prefers_critical_over_promo() {
        let list = vec![
            promo("p", "upsell", Some(("Go", "https://ainxt.example.test"))),
            RemoteAnnouncement {
                id: Some("c".into()),
                severity: Some("critical".into()),
    ...
    fn render_promo_row_non_dismissible_shows_configured_caption() {
        let mut ann = promo("p", &"M".repeat(60), Some(("Go", "https://ainxt.example.test")));
        ...
        let mut bare = promo("p", &"M".repeat(60), Some(("Go", "https://ainxt.example.test")));
    ...
    fn promo_cta_target_requires_usable_pair() {
        let full = vec![promo("p", "msg", Some(("Go", " https://ainxt.example.test/promo ")))];
        let (a, url) = promo_cta_target(&full, &no_hidden()).expect("usable target");
        assert_eq!(a.id.as_deref(), Some("p"));
        assert_eq!(url, "https://ainxt.example.test/promo");

        let mut label_only = promo("p", "msg", None);
        label_only.cta = Some(ainxt_announcements::AnnouncementCta {
            label: Some("Go".into()),
            url: None,
            caption: None,
        });
    ...
    fn usable_cta_caption_trims_and_never_resurrects_unusable_cta() {
        let mut p = promo("p", "msg", Some(("Go", "https://ainxt.example.test")));
    ...
    fn promo_cta_returns_label_and_pinned_flag() {
        let mut pinned = promo("p", "msg", Some(("Upgrade Account", "https://ainxt.example.test/promo")));
        ...
        let dismissible = [promo("d", "msg", Some(("Go", "https://ainxt.example.test")))];
    ...
    fn promo_cta_target_yields_to_critical_slot_owner() {
        let promo_ann = promo("p", "upsell", Some(("Go", "https://ainxt.example.test/promo")));
        ...
        assert_eq!(
            ...
            Some((Some("p"), "https://ainxt.example.test/promo"))
        );
    ...
    fn usable_cta_rejects_unsafe_schemes() {
        ...
        for good in ["https://ainxt.example.test/promo", "http://ainxt.example.test/promo"] {
    ...
    fn render_promo_row_button_and_hide_affordances() {
        // Derive the button geometry from the label so a copy change cannot
        // silently invalidate the assertion (it already did once: the expected
        // width below said 15, the size of an earlier 13-character label, while
        // this fixture renders `[Subscribe]` = 11).
        const CTA_LABEL: &str = "Subscribe";
        let cta_width = CTA_LABEL.chars().count() as u16 + 2; // surrounding [ ]
        let mut ann = promo(
            "p",
            "New promo",
            Some((CTA_LABEL, "https://ainxt.example.test/subscribe")),
        );
        ann.cta.as_mut().unwrap().caption = Some("or use Ctrl+O".into());
        let anns = [ann];
        ...
        let row0 = buf_row(&buf, area, 0);
        assert!(
            row0.starts_with(&format!("[{CTA_LABEL}]")),
            "row0={row0:?}"
        );
        assert!(
        ...
        // [Label] is `cta_width` cols at x 0; [hide] right-aligned at 80−6=74;
        // the hide CTA ends gap-adjacent to it.
        assert_eq!(
            hits.cta,
            Some(Rect::new(0, 0, cta_width, 1)),
            "[Label] hit rect"
        );
    ...
    fn render_promo_row_hover_styles() {
        let anns = [promo("p", "msg", Some(("Go", "https://ainxt.example.test")))];
        let area = Rect::new(0, 0, 80, 1);
        let theme = Theme::current();
    ...
    fn render_promo_row_truncates_button_never_affordances() {
        let anns = [promo(
            ...
            "msg",
            Some((
                "Upgrade to SuperAinxt Heavy for the exclusive preview",
                "https://ainxt.example.test",
            )),
        )];
        let area = Rect::new(0, 0, 50, 1);
        let mut buf = Buffer::empty(area);
    ...
    fn render_promo_row_narrow_width_drops_hide_cta_text() {
        let anns = [promo("p", "msg body", Some(("Go", "https://ainxt.example.test")))];
