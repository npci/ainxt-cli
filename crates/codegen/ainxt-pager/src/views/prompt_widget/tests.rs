#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn cmd_a_image_chip_selection_is_path_free() {
        ...
        img.source_path = Some(PathBuf::from("/tmp/ainxt-test-image.png"));
        ...
        assert!(
            !full.contains("/tmp/ainxt-test-image.png"),
            "source path must not appear in the buffer chip: {full:?}"
        );
        assert_eq!(
            pw.images[0].source_path.as_deref(),
            Some(std::path::Path::new("/tmp/ainxt-test-image.png")),
        ...
        assert!(selected.contains("[Image #1]"));
        assert!(
            !selected.contains("/tmp/ainxt-test-image.png"),
    ...
    // ── Image preview activation (paste-chip equivalence) ─────────────────
    ...
    fn accept_inside_command_with_args_absorbs_existing_separator() {
        ...
        pw.textarea.insert_str("/mod ainxt-4");
        pw.textarea.set_cursor(3);
        pw.refresh_slash(&models);
    ...
    fn accept_arg_completion_replaces_arg_range() {
        ...
        let model_id = agent_client_protocol::ModelId::new(Arc::from("ainxt-4.5"));
        models.available.insert(
            model_id.clone(),
            agent_client_protocol::ModelInfo::new(model_id, "ainxt 4.5".to_string()),
        );

        // Type "/model ai" and position cursor at end (in args). The prefix must
        // actually match a model in `models.available` for arg suggestions to
        // open. This read "gr" — the prefix of the upstream "Grok" model names —
        // and matched nothing once the only available model became "ainxt 4.5",
        // so the suggestion popup never opened and the assertion below failed.
        pw.textarea.insert_str("/model ai");
        pw.refresh_slash(&models);
        ...
        // Accept arg completion → should replace "ai" with "ainxt 4.5".
        pw.accept_slash_completion(&models);
        let text = pw.textarea.text().to_string();
        assert!(
            text.contains("ainxt 4.5"),
    ...
    fn test_image() -> PastedImage {
        PastedImage {
            element_id: ainxt_ratatui_textarea::ElementId::from_raw(0), // overwritten by insert_image
            display_number: 0,
    ...
    fn fuzzy_result(path: &str, is_dir: bool) -> ainxt_workspace::file_system::FuzzyMatchResult {
        ainxt_workspace::file_system::FuzzyMatchResult {
    ...
    /// after a turn ends with `ainxt.dev/suggestPrompt` resolved.
