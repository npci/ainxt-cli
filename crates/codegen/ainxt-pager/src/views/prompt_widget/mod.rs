...
//!  ainxt-3 · yolo                             ← info line (optional)
//! ```
//!
...
use ainxt_ratatui_textarea::{ElementId, ElementKind, TextArea, TextAreaState, TextElement};
...
pub struct PromptWidget {
    ...
    hovered_image_element_id: Option<ainxt_ratatui_textarea::ElementId>,
    /// Image shown immediately after insertion without moving the edit cursor.
    post_insert_image_preview: Option<(ainxt_ratatui_textarea::ElementId, usize)>,
        ...
        let applied = if snap.cursor_in_command {
            ...
            if let Some(ref range) = snap.command_range {
                    ...
                    && self.textarea.text().is_char_boundary(range.end)
                {
                    ...
                    // `/mod ainxt-4` yields `/model ainxt-4`, not `/model  ainxt-4`
            ...
            // Replace the args range (e.g., "gr" → "ainxt 4 Fast").
...
impl PromptWidget {
    ...
    fn handle_key_inner(&mut self, key: &KeyEvent) -> PromptEvent {
        ...
        if changed {
            ...
            if is_backspace_key && !old_text.is_empty() {
                use ainxt_telemetry::events::BackspaceNoEffect;
    ...
    pub fn handle_mouse(&mut self, mouse: &crossterm::event::MouseEvent) -> PromptEvent {
        use ainxt_ratatui_textarea::{MouseAction, TextElementEventKind};
    ...
    fn sync_images_with_textarea(&mut self) {
        ...
        // files cleaned up (matches what `clear()` does on submit).
    ...
    fn image_element_near_cursor(&self) -> Option<&ainxt_ratatui_textarea::TextElement> {
    ...
    pub fn textarea(&self) -> &ainxt_ratatui_textarea::TextArea {
        &self.textarea
    }
...
impl PromptWidget {
    ...
    pub(crate) fn render_info_line(
        ...
        // Trailing pad matches the leading pad above.
...
fn strip_all_elements(
    text: &str,
    cursor: usize,
    textarea: &ainxt_ratatui_textarea::TextArea,
) -> (String, usize) {
...
fn map_clean_offset_to_raw(
    text: &str,
    clean_offset: usize,
    elements: &[ainxt_ratatui_textarea::TextElement],
) -> usize {
    if elements.is_empty() {
