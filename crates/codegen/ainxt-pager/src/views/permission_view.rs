...
use ainxt_workspace::permission::bash_command_splitting::{
...
///
/// Matches [`QuestionFocus`](crate::views::question_view::QuestionFocus) from
...
pub struct PermissionViewState {
    ...
    pub request: ainxt_acp_lib::AcpArgs<acp::RequestPermissionRequest>,
    ...
    /// permissions). Imported from `ainxt-shell`, NOT duplicated locally.
...
///
/// Matches `render_question_view`: bg_light background, accent `┃` line,
...
pub fn render_permission_view(
    ...
    // visually recedes. Matches the unfocused prompt widget pattern
...
/// ```text
///  3 [ ] Tell ainxt what to do differently
...
/// Build the RejectOnce row as a freeform input line (matches question view).
...
mod tests {
    ...
    fn empty_view_state(mcp_scope: Option<McpScopeState>) -> PermissionViewState {
        ...
        let perm = ainxt_acp_lib::AcpArgs {
            request,
            response_tx,
        };
        PermissionViewState {
    ...
    fn render_shows_planned_mcp_args() {
        ...
        state.description = vec![
            ...
            "  \"body\": \"hello from ainxt\"".to_string(),
            "}".to_string(),
        ];
        ...
        assert!(
            text.contains("\"body\": \"hello from ainxt\""),
            "args JSON not rendered:\n{text}"
        );
    ...
    ///
    /// ```text
    /// PERMISSION_UI_RENDER_REVIEW=1 cargo test -p ainxt-pager --lib \
    ...
    fn historic_bash_fixture_path() -> std::path::PathBuf {
        // CARGO_MANIFEST_DIR = crates/codegen/ainxt-pager
