...
/// Ticks-per-USD scaling for the session cost (`cost_usd_ticks = USD * 1e10`,
/// see `ainxt_chat_state::usage`).
const COST_TICKS_PER_USD: f64 = 1e10;

/// Format a session cost (USD ticks) for the status bar.
///
/// - `0` ticks (all in-house / free models) => `"free"`.
/// - `< $0.50`               => 4 decimals, e.g. `"$0.0123"`.
/// - `>= $0.50`              => 2 decimals, e.g. `"$1.42"`.
pub fn fmt_session_cost(cost_usd_ticks: i64) -> String {
    if cost_usd_ticks <= 0 {
        return "free".to_string();
    }
    let dollars = cost_usd_ticks as f64 / COST_TICKS_PER_USD;
    if dollars < 0.50 {
        format!("${dollars:.4}")
    } else {
        format!("${dollars:.2}")
...
/// Format a session cost, marking it approximate when part of it was priced
/// from published rates instead of by the gateway.
///
/// The `~` prefix is the whole point: an estimate must be visually distinct
/// from a figure that reconciles with the platform's billing. `free` is left
/// unprefixed — an in-house model has no per-token price to approximate, so the
/// zero is certain either way.
pub fn fmt_session_cost_marked(cost_usd_ticks: i64, estimated: bool) -> String {
    let text = fmt_session_cost(cost_usd_ticks);
    if estimated && cost_usd_ticks > 0 {
        format!("~{text}")
    } else {
        text
    }
...
/// True when a model id is an in-house / self-hosted model (the gateway
/// prefixes those with `local:`, e.g. `local:gemma-4-31B-it`).
pub fn is_in_house_model(model_id: &str) -> bool {
    model_id.trim().starts_with("local:")
}

/// Build the session-cost status-bar segment as a `Line<'static>`.
///
/// Suppressed only for gateway/chat-kind sessions (they don't own local
/// billing telemetry). Otherwise:
/// - A known cost (`Some(ticks)`) renders as the dollar amount, or `free` for
///   `Some(0)` (in-house models settle at zero). When `estimated` is set, part
///   of the total was priced from published rates because the gateway did not
///   price every call, and the amount is prefixed `~`.
/// - `None` means no call has landed yet, so the segment shows a placeholder
///   from the model naming: in-house (`local:*`) => `free`, else `cloud`. It is
///   a hint about the model class, not a claim about spend.
pub fn session_cost_line(
    ...
    let text = match cost_usd_ticks {
        Some(ticks) => fmt_session_cost_marked(ticks, estimated),
        None if is_in_house_model(model_id) => "free".to_string(),
...
pub fn context_bar_line_for_session(
    ...
    let pct = ainxt_token_estimation::usage_percentage(used, total);
...
mod tests {
    ...
    #[test]
    fn test_fmt_session_cost_free_and_amounts() {
        // Zero (all in-house/free models) => "free".
        assert_eq!(fmt_session_cost(0), "free");
        // Negative shouldn't occur, but is treated as free defensively.
        assert_eq!(fmt_session_cost(-1), "free");
        // < $0.50 => 4 decimals. $0.0123 = 123_000_000 ticks.
        assert_eq!(fmt_session_cost(123_000_000), "$0.0123");
        // Just under $0.50 keeps 4 decimals.
        assert_eq!(fmt_session_cost(4_999_000_000), "$0.4999");
        // >= $0.50 => 2 decimals. $1.42 = 14_200_000_000 ticks.
        assert_eq!(fmt_session_cost(14_200_000_000), "$1.42");
        // Exactly $0.50 => 2 decimals.
        assert_eq!(fmt_session_cost(5_000_000_000), "$0.50");
    }

    #[test]
    fn test_fmt_session_cost_marks_estimates() {
        // An estimated amount is prefixed so it cannot pass for a real bill.
        assert_eq!(fmt_session_cost_marked(123_000_000, true), "~$0.0123");
        // A gateway figure is shown plainly.
        assert_eq!(fmt_session_cost_marked(123_000_000, false), "$0.0123");
        // "free" needs no marker: an in-house model has no price to approximate.
        assert_eq!(fmt_session_cost_marked(0, true), "free");
    }

    #[test]
    fn test_session_cost_line_visibility() {
        let theme = Theme::default();
        // No cost yet + in-house (local:) model renders "free".
        let free_local = session_cost_line(None, false, "local:gemma-4-31B-it", &theme, false)
            .expect("in-house free");
        assert_eq!(line_text(&free_local), "free");
        // No cost yet + cloud model renders "cloud" (not a misleading "free").
        let cloud =
            session_cost_line(None, false, "claude-sonnet-4-6", &theme, false).expect("cloud");
        assert_eq!(line_text(&cloud), "cloud");
        // Gateway/chat suppresses even with a value.
        assert!(session_cost_line(Some(123_000_000), false, "gpt-5.5", &theme, true).is_none());
        assert!(session_cost_line(None, false, "local:qwen-3.6-27B", &theme, true).is_none());
        // A gateway-reported zero renders "free" regardless of model class.
        let free = session_cost_line(Some(0), false, "claude-sonnet-4-6", &theme, false)
            .expect("reported free");
        assert_eq!(line_text(&free), "free");
        // A gateway-reported positive cost renders a plain dollar amount.
        let paid = session_cost_line(Some(123_000_000), false, "gpt-5.5", &theme, false)
            .expect("paid line");
        assert_eq!(line_text(&paid), "$0.0123");
        // A locally estimated total renders with the `~` marker.
        let estimated = session_cost_line(Some(123_000_000), true, "gpt-5.5", &theme, false)
            .expect("estimated line");
        assert_eq!(line_text(&estimated), "~$0.0123");
    }

    #[test]
    fn in_house_model_detection() {
        // Gateway's own convention: self-hosted models are prefixed `local:`.
        assert!(is_in_house_model("local:gemma-4-31B-it"));
        assert!(is_in_house_model("local:qwen-3.6-35B-A3B"));
        assert!(is_in_house_model("local:kimi-k2.7-code"));
        assert!(!is_in_house_model("gpt-5.5"));
        assert!(!is_in_house_model("claude-sonnet-4-6"));
        assert!(!is_in_house_model("gemini-3.5-flash"));
    }
