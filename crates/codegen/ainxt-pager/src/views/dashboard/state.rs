...
/// `~/.ainxt/config.toml`. Lenient — corrupted fields fall back to
...
pub struct DashboardState {
    ...
    /// row, or the reject-feedback slot in question mode). Matches
    ...
    /// exists"). Rendered unchanged by `paint_dispatch_feedback_badge`;
    ...
    /// [`STOP_CONFIRM_WINDOW`] closes the agent. Matches the session-close
    ...
    /// when the dialog opens; consumed on confirm. Matches the `attach` flag of
...
/// Mode staged for the next agent the dashboard spawns. Matches the agent
...
pub struct PendingDispatchModel {
    ...
    pub effort: Option<ainxt_shell::sampling::types::ReasoningEffort>,
    pub display: String,
}
...
impl DashboardState {
    ...
    /// start. Matches the in-memory lifetime of section collapse.
    ...
    /// Top-level input handler. Matches `AgentView::handle_input` —
    ...
    pub(crate) fn handle_input_with_paste_provenance(
        ...
        // a parallel say. Matches how `agent_view` short-circuits
    ...
    /// synchronously. Matches [`Self::handle_paste_key_deferred`] for the
    ...
    pub(crate) fn complete_clipboard_attachment_paste(
        ...
        // reply buffer or a different agent's reply (matches the agent's
    ...
    fn handle_key(&mut self, key: &KeyEvent, registry: &ActionRegistry) -> InputOutcome {
        ...
        // image/file-url probe defers off the event loop. Matches `AgentView`
        ...
        // matching the `j`/`k` nav equivalence). Up/Down nav reaches the
        ...
        // command). Matches `agent_view::handle_prompt_key`. The model
        ...
        if matches!(key.code, KeyCode::Esc) {
            ...
            // j/k, Space) right away. Matches Tab's focus toggle. The
            ...
            if !self.list_focused {
                ...
                // cursor once the list takes focus (matches Tab).
                self.clear_manual_scroll();
                return InputOutcome::Changed;
            }
        ...
        if old != new {
            ...
            // dispatch unchanged). `parse_filter` still honours the
    ...
    fn handle_mouse(&mut self, mouse: &crossterm::event::MouseEvent) -> InputOutcome {
        ...
        if matches!(mouse.kind, MouseEventKind::Moved) {
            ...
            // completion list tracks the pointer while open (matches
        ...
        if matches!(mouse.kind, MouseEventKind::Down(MouseButton::Left)) {
            ...
            if self.upgrade_cta_hit.contains(mouse.column, mouse.row) {
                return InputOutcome::Action(Action::AnnouncementsOpenCta(
                    ainxt_telemetry::events::AnnouncementCtaSurface::Dashboard,
                ));
            }
            ...
            // focus it and flip show-all (matches the section-header
    ...
    fn handle_worktree_dialog_input(&mut self, ev: &Event) -> InputOutcome {
        ...
        match dialog.handle_key(key) {
            ...
            NewWorktreeDialogOutcome::Cancelled => {
                ...
                // input so they can resend it instead of losing it. Matches the
    ...
    fn handle_shortcuts_modal_input(&mut self, ev: &Event) -> InputOutcome {
        ...
        match ev {
            ...
            Event::Mouse(mouse) => {
                ...
                // button inert. Matches `agent_view::handle_modal_mouse`.
...
/// Strict Enter swap for compose surfaces (agent prompt equivalence).
...
/// Load the full persisted dashboard from `~/.ainxt/config.toml`.
///
...
/// `~/.ainxt/config.toml` (`[ui]`, `[hints]`, `[mcpServers]`, …).
///
...
pub fn write_persisted(p: &PersistedDashboard) -> std::io::Result<()> {
    let path = config_path()
        .ok_or_else(|| std::io::Error::new(std::io::ErrorKind::NotFound, "no ainxt home"))?;
...
mod tests {
    ...
    /// (`✗`/`x`) so the unchanged-rendering badge marks it as an error.
    #[test]
    ...
    /// `s:` empty now matches `a:` empty: returns None,
    /// not Substring("").
    #[test]
    ...
    fn compact_cwd_strips_home() {
        let p = Path::new("/Users/alice/projects/ainxt");
        assert_eq!(compact_cwd(p, Some("/Users/alice")), "~/projects/ainxt");
    }
    ...
    /// silently discarding the user's typed text. Matches the restore in
    ...
    fn peek_test_image() -> crate::prompt_images::PastedImage {
        crate::prompt_images::PastedImage {
            element_id: ainxt_ratatui_textarea::ElementId::from_raw(0),
            display_number: 0,
    ...
    /// The (peek-less) dispatch input matches the peek: with content,
    ...
    /// Right matches Enter on the question-picker navigation surface:
    ...
    /// input (matches the dispatch box's click-to-focus) and routes the
    /// event to the widget.
    #[test]
    ...
    /// dispatches unchanged. This pins the bug fix: prefixed prompts
    ...
    /// NOT hijacked as row navigation). Matches the agent scrollback.
    #[test]
    ...
    /// dispatching (agent prompt equivalence).
    #[test]
    ...
    fn completion_reports_full_miss_for_unreadable_file_url() {
        ...
        let completion = state.complete_clipboard_attachment_paste(
            completion_ctx(None, false),
            crate::app::actions::ProbedAttachment::NoRaster,
            Some("file:///definitely/missing/ainxt-primary-paste.png".to_owned()),
        );
    ...
    /// now matches click-to-open list semantics from gh-dash / k9s.
    #[test]
    ...
    /// list tracks the pointer (agent-view equivalence).
    #[test]
    ...
    /// per-key serial lock. The `AINXT_AGENT_DASHBOARD` key means this
    /// test runs serially with any other test that decorates itself
    /// with `#[serial_test::serial(AINXT_AGENT_DASHBOARD)]` — see the
    ...
    fn env_var_force_disables() {
        ...
        // same `AINXT_AGENT_DASHBOARD` key reads it concurrently.
