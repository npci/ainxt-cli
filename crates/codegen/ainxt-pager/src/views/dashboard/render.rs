...
/// Render the dashboard. Matches `agent_view::draw` — returns the
...
pub fn render_dashboard(
    ...
    // any sub-renderer runs. Matches `welcome::render` and
    ...
    // on `DashboardState` (matches `agent_view`'s `active_modal`);
...
/// formula). The cursor matches that chrome math; anything else parks
...
fn render_header(
    ...
    // Build right-aligned chips. Order matches `RowState::group_priority`
...
///
/// ```text
///   ◆ Add responsiveness to /context · ainxt my-branch-2 worktree       4 mins
...
fn render_row(
    ...
    if title_avail > 0 {
        ...
        // Subtitle: ` · ainxt my-branch-2 worktree`.
...
/// **top border** (e.g. `✗ Session no longer exists`, `✓ Theme: ainxt
...
/// Matches `agent_view`'s slash dropdown chrome. No-op (and clears the
...
fn render_footer(
    ...
    let hints: Vec<HintItem> = if peek_active {
        ...
        // Matches `DashboardState::handle_peek_key`.
...
mod tests {
    ...
    fn dashboard_visual_preview() {
        ...
        let rows = vec![
            DashboardRow {
                ...
                subtitle: Some("ainxt my-branch-2 worktree".to_string()),
            ...
            DashboardRow {
                ...
                subtitle: Some("ainxt my-branch-3 worktree".to_string()),
            ...
            DashboardRow {
                ...
                subtitle: Some("ainxt main".to_string()),
            ...
            DashboardRow {
                ...
                label: "Add buttons for /models".to_string(),
                subtitle: Some("ainxt mybranch worktree".to_string()),
                state: RowState::Working,
    ...
    fn header_location_renders_from_staged_cwd() {
        ...
        let mut state = DashboardState::new();
        // A distinct absolute path outside $HOME (rendered unchanged) that
        // differs from the process cwd. No git cache entry → no branch span.
        state.cwd = std::path::PathBuf::from("/ainxt-staged-cwd-marker");
        ...
        let top_row: String = (0..area.width)
            ...
            .collect();
        assert!(
            top_row.contains("/ainxt-staged-cwd-marker"),
    ...
    /// row. Matches the agent view's "drop bottom_vpad on short
    /// terminals" behaviour.
    #[test]
    ...
    fn feedback_badge_renders_verbatim_in_neutral_color() {
        ...
        state.error_toast = Some(format!("{check} Theme: ainxt Day"));
        let _ = render_dispatch(&mut buf, Rect::new(0, 0, 80, 3), &theme, &mut state, None);

        let top_row: String = (0..80).map(|x| buf[(x, 0)].symbol().to_string()).collect();
        assert!(
            top_row.contains(&format!("{check} Theme: ainxt Day")),
    ...
    /// and it does NOT reserve a column — the row content is exactly
    ...
    fn render_location_picker_shows_worktree_toggle_in_repo() {
        ...
        let repo = std::env::temp_dir().join("ainxt-loc-wt-toggle-repo-test");
    ...
    fn render_location_picker_hides_worktree_toggle_outside_repo() {
        ...
        let mut modal = LocationPickerState::new(
            vec![],
            std::path::PathBuf::from("/ainxt-not-a-repo-xyz-12345"),
            std::collections::HashMap::new(),
        );
        let mut buf = Buffer::empty(area);
        render_location_picker(&mut buf, area, &theme, &mut modal);
    ...
    fn render_footer_row_selected_empty_prompt_shows_enter_open() {
        ...
        // Two-focus discoverability — matches the `[+ New Agent]`
    ...
    /// the Enter ↔ Shift/Alt+Enter behavior (agent keybar equivalence).
    #[test]
