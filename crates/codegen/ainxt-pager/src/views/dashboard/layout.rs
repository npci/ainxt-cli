...
pub fn compute_layout_with_dispatch(
    ...
    // cleanup.
    let peek_h: u16 = 0;
    // Sum the row-budget components in `u32` first: each addend is a
    // terminal-row count (already bounded well under `u16::MAX`), but
    // widening avoids any narrowing-conversion concern from summing several
    // `u16`s before the final `saturating_sub`/clamp back to a row count
    // that fits the terminal height.
    let reserved_rows_u32: u32 = u32::from(top_margin_h)
        + u32::from(header_h)
        + u32::from(header_gap_h)
        + u32::from(footer_h)
        + u32::from(dispatch_h)
        + u32::from(peek_h)
        + u32::from(dispatch_gap_h)
        + u32::from(shortcuts_gap_h)
        + u32::from(bottom_margin_h);
    let reserved_rows: u16 = reserved_rows_u32.min(u32::from(u16::MAX)) as u16;
    let remaining = area.height.saturating_sub(reserved_rows);
    ...
    // render_dashboard. Matches the dispatch inset pattern but with a
    ...
    // box (matches `prompt_gap` in `views::agent::AgentViewLayout`).
    y += dispatch_gap_h;
    ...
    // shortcuts footer (matches `shortcuts_gap`).
    y += shortcuts_gap_h;
...
mod tests {
    ...
    /// Matches the agent view's `bottom_vpad` (`outer_vpad = 1`,
    ...
    /// from the footer. Matches `prompt_gap` + `shortcuts_gap` in the
    /// agent view's layout.
    #[test]
    ...
    /// row list isn't starved. Threshold matches the top-margin
