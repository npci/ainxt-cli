...
/// scrolls internally (the widget keeps the caret visible). Matches the
...
pub struct PeekFields {
    ...
    /// feedback message (matches the chat permission panel).
...
pub struct PeekPanelState {
    ...
    /// bottom border (matches the dispatch box's config badge). `None`
...
fn paint_peek_config_badge(
    ...
    // permission mode (the gate in ainxt-shell), so `plan` alone is the
...
pub fn render_peek_panel(
    ...
    if let Some(ref q) = panel.question {
        ...
        // types after selecting it (matches the chat permission panel).
        let mut y = inner.y;
        ...
        for (i, (_, label)) in panel.options.iter().enumerate().take(9) {
            ...
            if is_reject {
                // Freeform feedback row (matches the chat permission
    ...
    // (matches the dispatch box — a focused input keeps its text area
...
pub fn extract_last_response_type(agent: &AgentView) -> String {
    ...
    // what the agent is doing RIGHT NOW — matches the agent view's turn-status
...
mod tests {
    ...
    fn render_peek_shows_model_and_auto_approve_on_bottom_border() {
        ...
        panel.model_name = Some("ainxt 4 Fast".to_string());
        panel.auto_approve = true;
        let bottom = badge_row(&panel, 6);
        assert!(
            bottom.contains("ainxt 4 Fast"),
        ...
        qpanel.model_name = Some("ainxt 4 Fast".to_string());
        let qbottom = badge_row(&qpanel, 8);
        assert!(
            qbottom.contains("ainxt 4 Fast"),
        ...
        plain.model_name = Some("ainxt 4 Fast".to_string());
        ...
        planp.model_name = Some("ainxt 4 Fast".to_string());
    ...
    fn render_peek_reject_option_shows_feedback_hint() {
        ...
        // Matches the chat permission panel's placeholder unchanged.
        assert!(
    ...
    /// content above the reply (agent prompt equivalence). Without an
