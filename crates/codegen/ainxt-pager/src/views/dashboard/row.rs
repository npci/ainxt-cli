...
pub struct DashboardRow {
    ...
    /// dim text (e.g. `"ainxt my-branch-2 worktree"`). `None` when the
...
/// Matches the agent view's idle "watching" cue
...
/// caller then falls back to a bare `"Working"`). Matches the agent
...
mod tests {
    ...
    /// replayed messages) and an optional current model — matches a
    ...
    fn idle_local_agent_without_message_has_blank_secondary() {
        let agent = make_idle_agent_with_model(Some("ainxt-4.5"));
    ...
    fn subtitle_worktree_shows_label_branch_and_marker() {
        ...
        agent.session.cwd = PathBuf::from("/home/me/.ainxt/worktrees/x/location-picker");
    ...
    fn subtitle_non_worktree_shows_branch_and_folder() {
        let mut agent = make_idle_agent_with_model(None);
        agent.session.cwd = PathBuf::from("/home/me/ainxt/crates/foo");
