...
use ainxt_tools::implementations::skills::types::SkillInfo;
...
pub fn fuzzy_matches_hook(hook: &ainxt_hooks_plugins_types::HookInfo, query: &str) -> bool {
    if query.is_empty() {
        return true;
    }
...
pub(crate) fn source_has_matching_plugin(
    source: &ainxt_hooks_plugins_types::MarketplaceScanResult,
    query: &str,
) -> bool {
...
pub fn filtered_marketplace_count(
    sources: &[ainxt_hooks_plugins_types::MarketplaceScanResult],
    query: &str,
) -> usize {
...
pub(crate) fn test_plugin_info(
    name: &str,
    origin: Option<ainxt_hooks_plugins_types::PluginOrigin>,
) -> ainxt_hooks_plugins_types::PluginInfo {
    ainxt_hooks_plugins_types::PluginInfo {
        name: name.to_string(),
        id: format!("user/abcd1234/{name}"),
        root: format!("/tmp/{name}"),
        scope: ainxt_hooks_plugins_types::PluginScope::User,
        trusted: true,
        enabled: true,
        version: None,
        description: None,
        ...
        agent_count: 0,
        agent_names: vec![],
        hook_status: ainxt_hooks_plugins_types::HookStatus::None,
        hook_count: 0,
        mcp_server_count: 0,
        mcp_status: ainxt_hooks_plugins_types::McpStatus::None,
...
type GroupedPlugins<'a> = std::collections::BTreeMap<
    (u8, String, String),
    Vec<(usize, &'a ainxt_hooks_plugins_types::PluginInfo)>,
>;
...
/// sensible groups.
pub fn plugin_group(plugin: &ainxt_hooks_plugins_types::PluginInfo) -> PluginGroup {
    use ainxt_hooks_plugins_types::{PluginOrigin, PluginScope};

    match &plugin.origin {
        Some(PluginOrigin::ProjectAinxt) => PluginGroup::new(0, "origin:project", "Project"),
        Some(PluginOrigin::ProjectClaude) => {
            PluginGroup::new(1, "origin:project-claude", "Project (Claude)")
        }
        Some(PluginOrigin::UserAinxt) => PluginGroup::new(2, "origin:user", "User"),
...
pub fn resolve_marketplace_index<'a>(
    sources: &'a [ainxt_hooks_plugins_types::MarketplaceScanResult],
    target: usize,
    query: &str,
...
pub enum MarketplaceHit<'a> {
    ...
    Plugin {
        source_index: usize,
        source: &'a ainxt_hooks_plugins_types::MarketplaceScanResult,
        plugin: &'a ainxt_hooks_plugins_types::MarketplacePluginEntry,
    },
}
...
fn build_hook_groups<'a>(
    hooks: &'a [ainxt_hooks_plugins_types::HookInfo],
    filter: StatusFilter,
    query: &str,
) -> Vec<(&'a str, Vec<usize>)> {
...
pub fn next_visible_hook(
    hooks: &[ainxt_hooks_plugins_types::HookInfo],
    current: usize,
...
impl ExtensionsTab {
    ...
    pub fn telemetry_tab(self) -> ainxt_telemetry::events::ExtensionsModalTab {
        use ainxt_telemetry::events::ExtensionsModalTab;
        match self {
...
pub enum ButtonAction {
    ...
    HooksAction(ainxt_hooks_plugins_types::HooksAction),
    /// Execute a plugins action via ACP (no args needed).
    PluginsAction(ainxt_hooks_plugins_types::PluginsAction),
    ...
    RefreshMcpList,
    /// Open ainxt.dev connectors page (MCP tab: press `o`).
    ...
    MarketplaceAction(ainxt_hooks_plugins_types::MarketplaceAction),
...
pub struct ActionResultNotice {
    /// Full result text (used unchanged for the tab-wide status line).
    pub message: String,
...
pub fn resolve_key(tab: ExtensionsTab, ch: char) -> Option<ButtonAction> {
    use ainxt_hooks_plugins_types::{HooksAction, MarketplaceAction, PluginsAction};
...
pub fn build_action_from_input(
    ...
    use ainxt_hooks_plugins_types::{HooksAction, PluginsAction};
    ...
    match command_prefix {
        ...
        "marketplace_add_source" => Some(ButtonAction::MarketplaceAction(
            ainxt_hooks_plugins_types::MarketplaceAction::AddSource {
                url: first.to_string(),
            },
        )),
        "mcp_add" => {
...
fn parse_mcp_add_fields(name: &str, url_or_cmd: &str) -> Option<ButtonAction> {
    use ainxt_shell::util::config::{McpServerConfig, McpServerTransportConfig};
...
pub struct ExtensionsModalState {
    ...
    pub hooks_data: TabDataState<ainxt_hooks_plugins_types::HooksListResponse>,
    /// Plugins list data (fetched from shell).
    pub plugins_data: TabDataState<ainxt_hooks_plugins_types::PluginsListResponse>,
    ...
    pub last_plugins_action: Option<ainxt_hooks_plugins_types::PluginsAction>,
    ...
    pub marketplace_data: TabDataState<ainxt_hooks_plugins_types::MarketplaceListResponse>,
...
impl ExtensionsModalState {
    ...
    pub fn seed_plugin_groups_once(&mut self, plugins: &[ainxt_hooks_plugins_types::PluginInfo]) {
...
/// Whether an MCP section's child servers are hidden (matches render logic).
...
pub fn derive_source_label(source_dir: &str) -> (String, bool) {
    let ainxt = ainxt_config::ainxt_home();
    let source_path = std::path::Path::new(source_dir);
    // Plugin / installed-plugin dirs, under the user ainxt home (AINXT_HOME-aware)
    // or a project-scoped `{cwd}/.ainxt/<subdir>/`. Returns the first path
    ...
    let plugin_name = |subdir: &str| -> Option<String> {
        ...
        // User ainxt home (AINXT_HOME-aware).
        if let Ok(rest) = source_path.strip_prefix(ainxt.join(subdir))
            && let Some(name) = first_comp(rest)
        {
            return Some(name);
        }
        // Project-scoped `.ainxt/<subdir>/<name>` anywhere in the path.
        ...
        comps
            .windows(3)
            .find(|w| w[0] == ".ainxt" && w[1] == subdir && !w[2].is_empty())
            .map(|w| w[2].clone())
    };
    if let Some(name) = plugin_name("plugins").or_else(|| plugin_name("installed-plugins")) {
        return (format!("Plugin: {name}"), false);
    }
    // Global hooks under $AINXT_HOME/hooks
    let global_hooks = ainxt.join("hooks");
    ...
    // Project hooks
    if source_dir.ends_with("/.ainxt/hooks") || source_dir.contains("/.ainxt/hooks/") {
        return ("Project hooks".into(), false);
    }
    // Custom directory — removable
    let display = {
        if let Ok(rest) = source_path.strip_prefix(&ainxt) {
            let prefix = crate::util::display_ainxt_home_prefix();
...
fn skill_source_str(skill: &SkillInfo) -> String {
    if let Some(ref cs) = skill.config_source {
        match cs {
            ainxt_tools::types::config_source::ConfigSource::User { path } => {
                if crate::util::is_under_user_ainxt_home(path) {
                    crate::util::display_user_ainxt_path("skills")
            ...
            ainxt_tools::types::config_source::ConfigSource::Project { path } => {
            ...
            ainxt_tools::types::config_source::ConfigSource::Plugin { plugin_name, .. } => {
...
fn build_plugin_fields(plugin: &ainxt_hooks_plugins_types::PluginInfo) -> Vec<String> {
    use ainxt_hooks_plugins_types::McpStatus;
...
fn component_categories(
    components: &ainxt_hooks_plugins_types::PluginComponents,
) -> [(&'static str, &[ainxt_hooks_plugins_types::ComponentItem]); 6] {
    use ainxt_hooks_plugins_types::ComponentCategory;
...
pub(crate) fn marketplace_components_summary(
    plugin: &ainxt_hooks_plugins_types::MarketplacePluginEntry,
) -> Option<String> {
    plugin
        .components
        .as_ref()
...
pub fn render_extensions_modal(
            ...
            ExtensionsTab::Hooks => {
                if let TabDataState::Loaded(ref data) = state.hooks_data {
                    ...
                    let mut groups: std::collections::BTreeMap<
                        String,
                        Vec<(usize, &ainxt_hooks_plugins_types::HookInfo)>,
                    > = std::collections::BTreeMap::new();
    ...
    // Is there actually a connectors URL to link to?
    //
    // This build ships `URL_CONNECTORS` empty (audit risk R42), in which case
    // `section_description_lines` deliberately returns two plain sentences
    // ("No managed-connectors service is configured...", "Set
    // AINXT_URL_CONNECTORS to...") rather than a bracketed URL. `mcps_modal`
    // states the contract for that case: callers "check with `url_is_set` and
    // omit the affordance rather than offering one that goes nowhere."
    //
    // This call site did neither, which caused two real defects:
    //   1. The `debug_assert!` below asserted the last line is always bracketed,
    //      so **any debug build panicked when this modal rendered** — and
    //      `./setup.sh` produces a debug build by default. It was reached by 12
    //      unit tests, which is how it surfaced.
    //   2. `underline_last_desc` was set for the Managed section
    //      unconditionally, so with no URL the modal underlined the *hint text*
    //      as a link and recorded a click band over it — an affordance that
    //      opens nothing.
    let managed_connectors_configured = ainxt_env::url_is_set(
        &crate::views::mcps_modal::managed_section_connectors_url(),
    );
    ...
    // Only meaningful when a URL is configured — see above.
    debug_assert!(
        !managed_connectors_configured
        ...
        .map(|(i, label)| {
            if entry_is_header[i] {
                ...
                picker::PickerEntry::Row(picker::PickerRow {
                    ...
                    // Managed section only, and only when a connectors URL is
                    // actually configured — otherwise the last description line
                    // is explanatory prose, not a link (see the note above).
                    underline_last_desc: managed_connectors_configured
...
/// row of `footer_area`, joined by `  |  ` separators. Matches the
...
mod tests {
    ...
    fn derive_source_label_detects_project_scoped_plugins() {
        // Regression: project-scoped `{cwd}/.ainxt/plugins/<name>/` must label as
        // a (non-removable) plugin, not a removable "Custom" source. The user
        // ainxt-home branch is AINXT_HOME-aware; this covers the project fallback.
        let (label, is_custom) = derive_source_label("/repo/work/.ainxt/plugins/my-plugin/hooks");
        assert_eq!(label, "Plugin: my-plugin");
        assert!(!is_custom);

        let (label, is_custom) =
            derive_source_label("/repo/work/.ainxt/installed-plugins/vendor-abc123/skills");
    ...
    fn mcp_collapsed_managed_section_omits_server_rows() {
        ...
        let servers = vec![
            make_mcp_server_for_rows("ainxt_com_linear", McpWireSource::Managed, vec![]),
        ...
        assert!(
            rows.labels
                .iter()
                .any(|l| l.starts_with("Managed by your deployment")),
            "managed section header must appear"
        );
        assert!(
            !rows.labels.iter().any(|l| l == "ainxt_com_linear"),
    ...
    fn make_skill(
        ...
        ainxt_tools::implementations::skills::types::SkillInfo {
            ...
            metadata: None,
            path: "test".to_string(),
            scope: ainxt_tools::implementations::skills::types::SkillScope::User,
            config_source: None,
            plugin_name: None,
            plugin_version: None,
            plugin_root: None,
    ...
    fn make_plugin_skill(
        ...
        skill.scope = ainxt_tools::implementations::skills::types::SkillScope::Plugin;
        skill.config_source = Some(ainxt_tools::types::config_source::ConfigSource::Plugin {
    ...
    fn make_plugin(name: &str) -> ainxt_hooks_plugins_types::PluginInfo {
        test_plugin_info(name, None)
    }

    fn make_plugin_with_origin(
        name: &str,
        origin: ainxt_hooks_plugins_types::PluginOrigin,
    ) -> ainxt_hooks_plugins_types::PluginInfo {
    ...
    fn make_plugin_with_enabled(name: &str, enabled: bool) -> ainxt_hooks_plugins_types::PluginInfo {
    ...
    fn space_footer_desc_is_contextual_enable_or_disable() {
        ...
        state.picker_state.selected = 0;
        state.plugins_data = TabDataState::Loaded(ainxt_hooks_plugins_types::PluginsListResponse {
            plugins: vec![
    ...
    fn mcp_add_url_derives_name() {
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                assert!(matches!(
                    config.transport,
                    ainxt_shell::util::config::McpServerTransportConfig::StreamableHttp { .. }
                ));
            }
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                match config.transport {
                    ainxt_shell::util::config::McpServerTransportConfig::Stdio {
                        command,
                        args,
                        ..
                    } => {
    ...
    fn build_action_single_field_plugins() {
        ...
        assert!(matches!(
            ...
            Some(ButtonAction::PluginsAction(
                ainxt_hooks_plugins_types::PluginsAction::Install { .. }
            ))
        ));
    }
    ...
    fn make_hook(
        ...
        source_dir: &str,
        disabled: bool,
    ) -> ainxt_hooks_plugins_types::HookInfo {
        ainxt_hooks_plugins_types::HookInfo {
            name: name.to_string(),
            event: ainxt_hooks_plugins_types::HookEvent::PreToolUse,
            handler_type: ainxt_hooks_plugins_types::HookHandlerType::Command,
            matcher: None,
    ...
    fn superpowers_source() -> ainxt_hooks_plugins_types::MarketplaceScanResult {
        ainxt_hooks_plugins_types::MarketplaceScanResult {
    ...
    fn local_plugins_source() -> ainxt_hooks_plugins_types::MarketplaceScanResult {
        ainxt_hooks_plugins_types::MarketplaceScanResult {
            source_name: "local-plugins".into(),
            source_kind: "local".into(),
            source_url_or_path: "/home/user/.ainxt/marketplace/local".into(),
            plugins: vec![
                TestPlugin {
                    name: "my-linter",
                    version: Some("0.1.0"),
    ...
    struct TestPlugin {
        ...
        components: Option<ainxt_hooks_plugins_types::PluginComponents>,
    }
    ...
    impl TestPlugin {
        fn build(self) -> ainxt_hooks_plugins_types::MarketplacePluginEntry {
            ...
            ainxt_hooks_plugins_types::MarketplacePluginEntry {
    ...
    fn marketplace_add_source_builds_action() {
        ...
        match action {
            Some(ButtonAction::MarketplaceAction(
                ainxt_hooks_plugins_types::MarketplaceAction::AddSource { url },
            )) => {
    ...
    fn marketplace_key_r_dispatches_refresh() {
        ...
        assert!(matches!(
            action,
            Some(ButtonAction::MarketplaceAction(
                ainxt_hooks_plugins_types::MarketplaceAction::Refresh {
                    source_url_or_path: None
                }
            ))
        ));
    ...
    fn marketplace_modal_state_with_loaded_data() {
        ...
        state.marketplace_data =
            TabDataState::Loaded(ainxt_hooks_plugins_types::MarketplaceListResponse {
    ...
    fn marketplace_error_source_has_zero_plugins() {
        let error_source = ainxt_hooks_plugins_types::MarketplaceScanResult {
    ...
    fn component(name: &str, desc: Option<&str>) -> ainxt_hooks_plugins_types::ComponentItem {
        ainxt_hooks_plugins_types::ComponentItem::new(name, desc.map(str::to_string))
    }

    fn sample_components() -> ainxt_hooks_plugins_types::PluginComponents {
        ainxt_hooks_plugins_types::PluginComponents {
            skills: vec![
    ...
    fn marketplace_summary_empty_components_is_none() {
        let plugin = TestPlugin {
            name: "empty",
            components: Some(ainxt_hooks_plugins_types::PluginComponents::default()),
            ..Default::default()
        }
        .build();
    ...
    fn render_components_fields_caps_names_per_category() {
        let components = ainxt_hooks_plugins_types::PluginComponents {
            skills: (0..12)
    ...
    fn plugins_modal_state(
        plugins: Vec<ainxt_hooks_plugins_types::PluginInfo>,
    ) -> ExtensionsModalState {
        let mut state = ExtensionsModalState::new(ExtensionsTab::Plugins);
        state.plugins_data =
            TabDataState::Loaded(ainxt_hooks_plugins_types::PluginsListResponse { plugins });
        state
    }
    ...
    fn plugin_group_maps_each_origin_variant() {
        use ainxt_hooks_plugins_types::PluginOrigin;
        for (origin, rank, key, label) in [
            (PluginOrigin::ProjectAinxt, 0, "origin:project", "Project"),
            (
            ...
            (PluginOrigin::UserAinxt, 2, "origin:user", "User"),
            (
            ...
            (
                PluginOrigin::MarketplaceInstall {
                    source_name: Some("ainxt Official".into()),
                    git_url: Some("https://example.com/r.git".into()),
                },
                5,
                "ainxt-mp:ainxt Official",
                "ainxt Official",
            ),
            (
                PluginOrigin::MarketplaceInstall {
    ...
    fn plugin_group_fallback_without_origin() {
        ...
        project.scope = ainxt_hooks_plugins_types::PluginScope::Project;
        ...
        cli.scope = ainxt_hooks_plugins_types::PluginScope::Cli;
        assert_eq!(plugin_group(&cli).key, "origin:cli");

        let mut config = make_plugin("cfg-tool");
        config.scope = ainxt_hooks_plugins_types::PluginScope::Config;
        assert_eq!(plugin_group(&config).key, "origin:config");

        let mut mp = make_plugin("mp-tool");
        mp.marketplace_source = Some("ainxt Official".into());
        let group = plugin_group(&mp);
        assert_eq!(group.key, "ainxt-mp:ainxt Official");
        assert_eq!(group.label, "ainxt Official");
    ...
    fn plugin_group_unknown_origin_uses_scope_fallback() {
        let mut unknown = make_plugin_with_origin(
            "future-tool",
            ainxt_hooks_plugins_types::PluginOrigin::Unknown,
        );
        assert_eq!(plugin_group(&unknown).key, "origin:user");

        unknown.marketplace_source = Some("ainxt Official".into());
        assert_eq!(plugin_group(&unknown).key, "ainxt-mp:ainxt Official");
    }
    ...
    fn plugins_render_groups_with_headers_in_rank_order() {
        ...
        let mut state = plugins_modal_state(vec![
            ...
            make_plugin_with_origin("user-tool", PluginOrigin::UserAinxt),
    ...
    fn plugins_render_multiple_plugins_under_one_group() {
        use ainxt_hooks_plugins_types::PluginOrigin;
        let mut state = plugins_modal_state(vec![
            make_plugin_with_origin("solo-tool", PluginOrigin::UserAinxt),
            make_plugin_with_origin(
                "catalog-tool",
                PluginOrigin::ClaudeMarketplace {
                    marketplace: "claude-market".into(),
    ...
    fn plugins_collapsed_group_hides_rows_and_search_forces_open() {
        use ainxt_hooks_plugins_types::PluginOrigin;
        let mut plugin = make_plugin_with_origin("user-tool", PluginOrigin::UserAinxt);
    ...
    fn plugins_fallback_grouping_without_origin() {
        ...
        assert_eq!(buffer_count(&buf, "User (1 plugin)"), 1);
        assert_eq!(buffer_count(&buf, "ainxt Official (1 plugin)"), 1);
    ...
    fn marketplace_placeholders_render_only_when_expanded() {
        ...
        source.plugins[0].components = Some(ainxt_hooks_plugins_types::PluginComponents::default());
