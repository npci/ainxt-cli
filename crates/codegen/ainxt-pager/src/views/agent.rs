...
pub fn build_hints(
    ...
    // Ctrl+M means "model picker" everywhere except when the prompt is
    // focused, where the same chord is intercepted for multiline toggle
    // (see ActionId::ToggleMultiline's long_help). Only hint it outside the
    // prompt to avoid advertising the wrong behavior for the common case.
    if !matches!(active_pane, ActivePane::Prompt)
        && let Some(key) = registry.key_for(ActionId::ModelPicker)
    {
        hints.push(HintItem::new(key, "models"));
    }
    hints
}
#[cfg(test)]
