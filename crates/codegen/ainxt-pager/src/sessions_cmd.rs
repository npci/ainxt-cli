...
use ainxt_shell::auth::{AuthManager, try_ensure_fresh_auth};
use ainxt_shell::session::merge::MergedSession;
...
pub async fn run(args: SessionsArgs, agent_config: &AgentConfig) -> Result<()> {
    ...
    // deployments that only configure a deployment_key + custom ainxt_api_base_url.
    // If the user has previously run the interactive `ainxt` TUI (which succeeds
    ...
    let auth = try_ensure_fresh_auth(&agent_config.ainxt_com_config).await;
    ...
    let client = ainxt_shell::agent::session_registry_client::SessionRegistryClient::new(
    ...
    match args.command {
        SessionsCommand::List { limit } => {
            let sessions = ainxt_shell::session::merge::fetch_merged(
                Some(&client),
                cwd.to_str(),
                None,
                limit,
        ...
        SessionsCommand::Search { query, limit } => {
            ...
            use ainxt_shell::session::merge::REMOTE_TIMEOUT;
            use ainxt_shell::session::storage::search::{SessionSearchRequest, execute_search};
        ...
        SessionsCommand::Delete { id } => {
            ...
            let deletion = ainxt_shell::session::persistence::delete_session_history(
                &id,
                None,
                needs_remote,
                auth_manager.clone(),
