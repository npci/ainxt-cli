//! Headless single-turn mode (`ainxt -p "prompt"`).
//!
//! Runs the agent in-process via
//! `spawn_ainxt_shell`, sends the ACP lifecycle (init → auth → session → prompt),
...
use ainxt_acp_lib::{AcpAgentTx, AcpClientMessageBox, AcpClientRx, acp_send};
use ainxt_shell::agent::auth_method::AuthMethodKind;
use ainxt_shell::agent::config::Config as AgentConfig;
use ainxt_shell::extensions::task::{CancelSubagentRequest, KillTaskRequest};
use ainxt_shell::sampling::error::{RATE_LIMITED_ERROR_CODE, rate_limited_user_message};
use ainxt_shell::sampling::types::{
    REASONING_EFFORT_META_KEY, parse_canonical_effort_token, reasoning_effort_meta_value,
};
use ainxt_shell::util::config as cli_config;

use crate::acp::model_state::{EffortTokenError, ModelState};
use crate::acp::spawn::spawn_ainxt_shell;
...
pub struct HeadlessOptions {
    ...
    /// Force the API backend for all models (messages|chat_completions|responses).
...
pub fn parse_permission_rules_strict(
    allow: &[String],
    deny: &[String],
) -> anyhow::Result<Vec<ainxt_workspace::permission::types::PermissionRule>> {
...
pub fn parse_permission_rules_lenient(
    allow: &[String],
    deny: &[String],
) -> Vec<ainxt_workspace::permission::types::PermissionRule> {
    let (rules, errors) = parse_permission_rules_inner(allow, deny);
...
pub(crate) fn parse_permission_rules_inner(
    ...
    use ainxt_workspace::permission::rules::parse_permission_rule;
    use ainxt_workspace::permission::types::RuleAction;
...
fn parse_cli_agents(
    json: &str,
) -> anyhow::Result<Vec<ainxt_shell::agent::config::AgentDefinition>> {
    ...
    for (name, mut value) in map {
        ...
        let mut def = ainxt_shell::agent::config::AgentDefinition::from_json(&value)
...
fn apply_agent_flag(agent: &Option<String>, config: &mut ainxt_shell::agent::config::Config) {
...
fn attach_result_usage(result: &mut serde_json::Value, usage: &serde_json::Value) {
    ainxt_shell::extensions::notification::attach_result_usage_fail_closed(result, usage);
}
...
fn auth_required_message(interactive: bool) -> String {
    if interactive {
        "Not signed in. Run `ainxt login` to authenticate \
         (or `ainxt login --device-code` if no browser is available)."
            .to_string()
    } else {
        "Not signed in. To authenticate without a browser, run:\n  \
         ainxt login --device-code\n\n\
         Alternatively, set the AINXT_API_KEY environment variable \
         or run `ainxt login` on a machine with a browser."
            .to_string()
    }
}
...
async fn authenticate(
    ...
    let method_id = crate::acp::select_eager_auth_method(auths, default_auth_method_id)
        .ok_or_else(|| {
            ...
            let interactive = std::io::stdin().is_terminal()
                && !ainxt_shell::util::clipboard::is_remote_session();
    ...
    if kind.needs_interactive_login() {
        use std::io::IsTerminal;
        let interactive =
            std::io::stdin().is_terminal() && !ainxt_shell::util::clipboard::is_remote_session();
...
async fn open_session(
    ...
    let mcp_servers =
        cli_config::load_mcp_servers(cwd, &ainxt_tools::types::compat::CompatConfig::default());
            ...
            acp::LoadSessionRequest::new(acp::SessionId::new(sid.to_string()), cwd.to_path_buf())
                ...
                .meta({
                    ...
                    if let Some(true) = restore_code {
                        m.insert("ainxt.dev/restore_code".into(), serde_json::Value::Bool(true));
                    }
                    Some(m)
                }),
            acp_tx,
...
async fn apply_headless_model_and_effort(
    ...
    .map_err(|e| {
        if let Some(name) = model_name {
            anyhow::anyhow!(
                "Couldn't set model '{}': {}. Run 'ainxt models' to see available models.",
                name,
                e
            )
        } else {
...
pub async fn run_single_turn(
    ...
    ainxt_shell::http::set_process_client_mode_headless();
    ...
    agent_config.mode = ainxt_shell::agent::config::AgentMode::Headless;
    ...
    // hub_url lives on `ainxt workspace` / WorkspaceStartArgs only).
    ...
    agent_config.cli_agent_overrides = ainxt_shell::agent::config::CliAgentOverrides {
    ...
    if options.trust {
        ainxt_shell::agent::folder_trust::grant_folder_trust(&cwd);
    }

    let cancel = CancellationToken::new();
    let memory_config = agent_config.memory_config.clone();
    let spawned = match spawn_ainxt_shell(agent_config, &cancel, memory_config).await {
        Ok(s) => s,
        Err(e) => {
    ...
    // Fail fast, and loudly, when the model catalogue is empty.
    //
    // Without this, nothing downstream can succeed but nothing says so: the
    // sampler falls back to an empty model id and the request is retried for the
    // whole `AINXT_MAX_RETRIES` budget (default 15, ~6 min with the 30s backoff
    // cap) while headless mode writes **nothing** to stdout or stderr. Measured
    // against an unreachable gateway: 339 s of total silence, then exit 1. In CI
    // that is indistinguishable from a hang, and `ainxt -p` is documented as the
    // CI entrypoint.
    //
    // The agent already knows immediately — `agent/models.rs` logs
    // "no models available in catalog; the ainxt gateway returned no models"
    // within ~100 ms — but only through `tracing`, which is off unless RUST_LOG
    // is set. So the diagnosis existed all along and was simply never shown.
    //
    // Why an empty catalogue is a safe signal: models declared in config.toml
    // are part of it, so a `[model.*]` entry pointing at a local Ollama reports
    // one entry and does not trip this. Empty means neither a gateway nor local
    // config supplied anything.
    //
    // Scoped deliberately: skipped when `--model` was passed, because
    // `apply_headless_model_and_effort` intentionally accepts an id that does not
    // resolve against the catalogue (`unwrap_or_else(|| ModelId::new(name))`).
    // That escape hatch lets you drive a gateway whose /models endpoint is broken
    // but whose /messages endpoint works, and this check must not close it.
    if session_models.is_empty() && options.model.is_none() {
        let msg = concat!(
            "No models are available, so there is nothing to send this prompt to.\n",
            "\n",
            "The model catalogue came back empty. Usual causes:\n",
            "  * AINXT_GATEWAY_URL is unset, unreachable, or is not an AiNxt gateway.\n",
            "    The CLI appends /ainxt/v1/api, so it requests\n",
            "    $AINXT_GATEWAY_URL/ainxt/v1/api/models . Ollama, vLLM, LiteLLM and\n",
            "    plain OpenAI-compatible servers serve /v1/... and will 404 there;\n",
            "    configure those as a [model.*] entry in config.toml instead.\n",
            "  * Not signed in: run `ainxt login`, or set AINXT_TOKEN / AINXT_API_KEY.\n",
            "\n",
            "Next steps:\n",
            "  ainxt models                                   # what the CLI can see\n",
            "  curl \"$AINXT_GATEWAY_URL/ainxt/v1/api/models\"   # what the gateway serves\n",
            "  RUST_LOG=ainxt_shell=debug,ainxt_sampler=debug  # full detail\n",
            "\n",
            "Setup guide: INSTALL.md . To use a specific model id anyway, without a\n",
            "catalogue, pass --model <id>.",
        );
        emitter.on_error(msg);
        cancel.cancel();
        anyhow::bail!("no models available: the model catalogue is empty");
    }

    // Debug: track headless sessions in active_sessions.json when env var is set.
    let track_active = std::env::var("AINXT_TRACK_HEADLESS").is_ok();
    if track_active {
        let _ = ainxt_shell::active_sessions::register(
            ainxt_shell::active_sessions::ActiveSession {
    ...
    if options.self_verify {
        prompt_blocks.push(acp::ContentBlock::Text(acp::TextContent::new(
            skill_body(ainxt_shell::builtin::CHECK_SKILL_MD).to_string(),
        )));
    }
        ...
        let n = n.clamp(2, 10);
        {
            prompt_blocks.insert(
                ...
                acp::ContentBlock::Text(acp::TextContent::new(format!(
                    ...
                    skill_body(ainxt_shell::builtin::BEST_OF_N_SKILL_MD)
                ))),
            );
        }
    }
    ...
    // Pending background work: bash/monitor via ainxt.dev/task_backgrounded +
    // task_completed; background subagents via SubagentSpawned + SubagentFinished
    // on ainxt.dev/session_notification (prefixed `subagent:{id}` in pending_bg).
    ...
    // Handle result
    if track_active {
        // Non-blocking flock so a slow/network ~/.ainxt can't hang exit.
        let _ = ainxt_shell::active_sessions::try_unregister(&session_id);
    }
    cancel.cancel();
    match prompt_result {
        Some(Ok(resp)) => {
        ...
        Some(Err(err)) => {
            ...
            if let Some(usage) = ainxt_shell::sampling::error::prompt_usage_from_error(&err)
...
fn handle_ext_notification(
    ...
    if method == "ainxt.dev/task_completed" {
        #[derive(serde::Deserialize)]
    ...
    if method == "ainxt.dev/monitor_event" {
        return ExtEvent::MonitorEvent;
    }

    match method {
        "ainxt.dev/session_notification" | "ainxt.dev/session/update" => {}
    ...
    let Ok(ainxt_notif) = serde_json::from_str::<AinxtNotif>(notif.request.params.get()) else {
        return ExtEvent::None;
    };

    match ainxt_notif.update {
        AinxtUpdate::AutoCompactStarted { percentage } => match format {
            OutputFormat::StreamingJson => {
                println!(
                    "{}",
        ...
        AinxtUpdate::AutoCompactCompleted {} => match format {
            OutputFormat::StreamingJson => {
        ...
        AinxtUpdate::AutoCompactFailed { error } => match format {
            OutputFormat::StreamingJson => {
                println!(
                    "{}",
        ...
        AinxtUpdate::AutoCompactCancelled {} => match format {
            OutputFormat::StreamingJson => {
        ...
        AinxtUpdate::AutoContinueCompleted { total_tokens } => match format {
            OutputFormat::StreamingJson => {
                println!(
                    "{}",
        ...
        AinxtUpdate::ImageCompressed { message } => match format {
            OutputFormat::StreamingJson => {
                println!(
                    "{}",
        ...
        AinxtUpdate::SubagentSpawned { subagent_id } => {
            return ExtEvent::SubagentSpawned { subagent_id };
        }
        AinxtUpdate::SubagentFinished { subagent_id, .. } => {
...
mod tests {
    ...
    fn reap_request_for_task_kills_with_session_scope() {
        ...
        assert_eq!(request.method.as_ref(), "ainxt.dev/task/kill");
    ...
    fn reap_request_for_subagent_cancels_with_stripped_id() {
        ...
        assert_eq!(request.method.as_ref(), "ainxt.dev/subagent/cancel");
        let params: serde_json::Value = serde_json::from_str(request.params.get()).unwrap();
        assert_eq!(params["subagentId"], "sub-7");
    }

    use super::*;
    use ainxt_workspace::permission::types::{RuleAction, ToolFilter};
    ...
    fn domain_mode_web_fetch() {
        ...
        assert_eq!(
            ...
            ainxt_workspace::permission::types::PatternMode::Domain
        );
    ...
    ) -> ainxt_acp_lib::AcpArgsBox<acp::ExtNotification> {
    ...
    fn headless_task_backgrounded_parses_task_id() {
        ...
        // the inner update object (matching the real `ainxt.dev/task_backgrounded`
        // wire shape: `{ "update": { "sessionUpdate": ..., "task_id": ... } }`).
        let notif = make_ext_notif(
            "ainxt.dev/task_backgrounded",
            serde_json::json!({
    ...
    fn headless_session_update_unknown_method_is_none() {
        ...
        let raw = serde_json::value::to_raw_value(&payload).unwrap();
        let (tx, _rx) = tokio::sync::oneshot::channel();
        let notif = ainxt_acp_lib::AcpArgs {
            request: acp::ExtNotification::new("ainxt.dev/other", raw.into()),
            response_tx: tx,
        }
        .boxed();
        assert!(matches!(
