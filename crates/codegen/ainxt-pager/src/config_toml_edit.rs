...
/// Set `[hints].<key>` to `value` in `~/.ainxt/config.toml`, preserving every
...
/// Set the ainxt gateway base URL in `~/.ainxt/config.toml` by writing BOTH
/// derived `[endpoints]` keys — `cli_chat_proxy_base_url` and
/// `ainxt_api_base_url` — each normalised to `{url}/ainxt/v1/api`. Mirrors the
/// derivation in `EndpointsConfig::default` (ainxt-shell), so persisting these
/// makes the new gateway take effect on the next launch. Preserves every other
/// key and table, creates the file/parent dir when missing, and no-ops when the
/// existing file is non-empty but unparseable. Performs blocking I/O.
pub(crate) fn set_gateway_url(url: &str) -> std::io::Result<()> {
    let path = ainxt_tools::util::ainxt_home::ainxt_home().join("config.toml");
    set_gateway_url_at(&path, url)
}

/// Path-injectable core of [`set_gateway_url`].
fn set_gateway_url_at(path: &Path, url: &str) -> std::io::Result<()> {
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent)?;
    }
    let Some(mut doc) = read_config_document_for_edit(path) else {
    ...
    let api_base = format!("{base}/ainxt/v1/api");
    doc["endpoints"]["cli_chat_proxy_base_url"] = toml_edit::value(api_base.clone());
    doc["endpoints"]["ainxt_api_base_url"] = toml_edit::value(api_base);
    std::fs::write(path, doc.to_string())
}
...
mod tests {
    ...
    #[test]
    fn set_gateway_url_writes_both_derived_endpoint_keys() {
        let dir = tempdir().unwrap();
        let path = dir.path().join("config.toml");
        fs::write(&path, "[ui]\ncompact_mode = false\n").unwrap();

        set_gateway_url_at(&path, "https://gateway.example.com/").unwrap();

        let doc = read_config_document_for_edit(&path).expect("reparse");
        let endpoints = doc.get("endpoints").expect("[endpoints] table");
        assert_eq!(
            endpoints
                .get("cli_chat_proxy_base_url")
                .and_then(|v| v.as_str()),
            Some("https://gateway.example.com/ainxt/v1/api"),
        );
        assert_eq!(
            endpoints.get("ainxt_api_base_url").and_then(|v| v.as_str()),
            Some("https://gateway.example.com/ainxt/v1/api"),
        );
        assert!(
            fs::read_to_string(&path).unwrap().contains("compact_mode"),
    ...
    #[test]
    fn set_gateway_url_creates_missing_file() {
        let dir = tempdir().unwrap();
        let path = dir.path().join("nested/config.toml");
        set_gateway_url_at(&path, "http://localhost:8000").unwrap();
        assert!(path.exists(), "missing file and parent dir should be created");
    }

    #[test]
    fn set_gateway_url_leaves_unparseable_file_untouched() {
        let dir = tempdir().unwrap();
        let path = dir.path().join("config.toml");
        let bad = "this is [not valid toml\n";
        fs::write(&path, bad).unwrap();
