...
//!
//! Simplified to only support AinxtShell (in-process) mode.
...
/// Spawn a AinxtShell agent in a background thread.
///
/// Returns the ACP client channel for communication and a cancellation token.
pub async fn spawn_ainxt_shell(
    agent_config: AgentConfig,
    cancel: &CancellationToken,
    memory_config: Option<ainxt_shell::config::MemoryConfig>,
) -> Result<SpawnedAgent> {
    ...
    // skills to ~/.ainxt/skills/), and model catalog construction.
    let (agent_config, models_manager) =
        ainxt_shell::agent::init::bootstrap(&agent_config, &auth_manager, None)
