...
//! ainxt-shell hasn't been updated or meta is absent.
...
pub struct NotificationMeta {
    ...
    /// Per-session cost in USD ticks (`sessionCostTicks`, USD * 1e10) as
    /// computed by the shell's session ledger. `None` before any call has
    /// completed; `Some(0)` when everything was free (in-house models).
    ///
    /// The pager TRUSTS this value and never derives cost from a client-side
    /// model list — the ledger owns the arithmetic, including the fallback for
    /// calls the gateway did not price. See [`Self::session_cost_estimated`].
    pub session_cost_ticks: Option<i64>,
    /// Whether `session_cost_ticks` includes a locally estimated portion
    /// (`sessionCostEstimated`), because the gateway did not price every call.
    /// The status bar must mark such a figure approximate; absent means the
    /// whole total came from the gateway.
    ...
    /// (`"{sessionId}-{counter}"`, see `ainxt-shell util::event_id`). The
...
/// replayed notifications (`_meta.isReplay`, stamped by ainxt-shell's
...
pub mod user_prompt_meta {
    ...
    /// block's `text`; only meaningful when that text is displayed unchanged
...
pub mod user_message_chunk_meta {
    ...
    /// ([`ainxt_shell::session::PromptOrigin::hide_user_echo_from_scrollback`]).
...
impl NotificationMeta {
    ...
    pub fn from_json(meta: Option<&serde_json::Map<String, serde_json::Value>>) -> Self {
        ...
        Self {
            ...
            session_cost_ticks: m.get("sessionCostTicks").and_then(|v| v.as_i64()),
            session_cost_estimated: m
                .get("sessionCostEstimated")
...
mod tests {
    ...
    fn parse_full_meta() {
        let meta_json = json!({
            "totalTokens": 5000u64,
            "sessionCostTicks": 12_345_678i64,
        ...
        assert_eq!(meta.session_cost_ticks, Some(12_345_678));
    ...
    fn parse_missing_new_fields() {
        // Simulate old ainxt-shell that doesn't send streamStartMs/turnStartMs
        let meta_json = json!({
            "totalTokens": 1000u64,
            "agentTimestampMs": 1700000000000i64,
        });
    ...
    fn parse_none_meta() {
        ...
        assert_eq!(meta.session_cost_ticks, None);
