...
use ainxt_tools::types::output::{ReadFileOutput, SearchToolOutput, WebFetchOutput};
use ainxt_tools::util::strip_redundant_session_cd;
...
pub struct AcpUpdateTracker {
    ...
    /// `ainxt.dev/task_backgrounded` creates a `BgTask` block.
...
impl AcpUpdateTracker {
    ...
    fn handle_user_message(
        ...
        let mut block = if let Some(dt) = display_override {
            ...
            let skill_display =
                ainxt_tools::implementations::skills::skill::extract_skill_display_text(&text);
...
fn user_message_hidden_from_scrollback(
    ...
    if let Some(pid) = meta.prompt_id.as_deref()
        && ainxt_shell::session::PromptOrigin::from_prompt_id(pid)
...
/// rather than a media file (the free / X Basic SuperAinxt-upsell short-circuit).
...
/// `kill_*`, todo/goal/scheduler). Matches the title/variant matches in
...
///
/// These are deferred from scrollback — the `ainxt.dev/task_backgrounded`
...
fn extract_edit_error(tc: &acp::ToolCall) -> String {
    use ainxt_tools::types::output::SearchReplaceOutput;
    if let Some(ref raw) = tc.raw_output
...
fn extract_listdir_content(raw: &Option<serde_json::Value>) -> Option<String> {
    let val = raw.as_ref()?;
    match serde_json::from_value::<ToolOutput>(val.clone()) {
        Ok(ToolOutput::ListDir(ainxt_tools::types::output::ListDirOutput::Content(c))) => {
            Some(c.content)
        }
        _ => None,
    }
...
fn extract_use_tool_output(raw: &Option<serde_json::Value>) -> Option<String> {
    ...
    if let Ok(output) = serde_json::from_value::<ToolOutput>(val.clone()) {
        let text = match output {
            ToolOutput::MCP(mcp) => {
                use ainxt_tools::types::output::MCPOutputDetails;
                match mcp.output() {
...
mod tests {
    use super::*;
    use std::sync::Arc;
    /// Default meta with no timestamps (simulates old ainxt-shell or tests that
    ...
    fn test_search_tool_call_flow() {
        use ainxt_tools::types::output::{GrepFileMatch, GrepLineMatch, GrepSearchOutput};
        let mut tracker = AcpUpdateTracker::new();
    ...
    /// No stream_start_ms (old ainxt-shell) should not break anything.
    #[test]
    ...
    /// wait is not shown (display matches interject eligibility).
    #[test]
    ...
    #[test]
    fn is_task_tool_recognizes_ainxt_build_variant() {
...
[CI] IN_PROGRESS=5 SUCCESS=38\n\
    ...
    fn video_tool_variants_use_typed_path_not_generic_scrape() {
        ...
        let cases: &[(&str, ToolOutput)] = &[
            (
                "ImageToVideo",
                ToolOutput::ImageToVideo(ainxt_tools::types::output::MediaGenOutput::new(
                    video_path.clone(),
                )),
            ),
            (
                "ReferenceToVideo",
                ToolOutput::ReferenceToVideo(ainxt_tools::types::output::MediaGenOutput::new(
                    video_path.clone(),
                )),
            ),
        ];
    ...
    fn media_gen_ref_skips_uploaded_only_video() {
        let output =
            ToolOutput::ImageToVideo(ainxt_tools::types::output::MediaGenOutput::uploaded(
    ...
    /// SuperAinxt upsell as `ToolOutput::Text` on a `Completed` status. The media
    ...
    fn tier_restricted_media_shows_upsell_text_not_error() {
        let upsell = "Image generation is not enabled for this account.";
        let output = ToolOutput::Text(ainxt_tools::types::output::TextOutput::from(upsell));
        let tc = acp::ToolCall::new(
        ...
        // Assert the upsell text itself reaches the card body. This looked for
        // the literal "SuperAinxt", which the tracker never injects and the real
        // product string never contains: `image_gen::TIER_RESTRICTED_UPSELL`
        // deliberately names no tier ("... is not enabled for this account. Let
        // the user know their plan does not currently include ..."). The tracker
        // only passes the text through, so the brand name could only have come
        // from the fixture — it was a proxy for "the text is shown" that stopped
        // matching when the product copy was reworded. Assert the pass-through
        // directly instead.
        assert!(
            block
                .output
                .as_deref()
                .unwrap_or_default()
                .contains("not enabled for this account"),
