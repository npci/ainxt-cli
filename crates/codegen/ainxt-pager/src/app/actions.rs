...
use ainxt_shell::sampling::types::ReasoningEffort;

/// Payload for a successful `TaskResult::GitStatusLoaded` — the changed-file
/// rows plus the branch name, ready to hand to the source-control panel
/// (Phase B). Kept here (not the view module) so the actions enum stays the
/// single source of truth for the async boundary.
#[derive(Debug, Clone)]
pub struct GitStatusPayload {
    pub branch: Option<String>,
    pub files: Vec<crate::views::source_control_panel::ChangeRow>,
}
...
pub enum SwitchModelError {
    ...
    /// `acp::Error.data`.
    IncompatibleAgent {
        error: ainxt_shell::agent::config::ModelSwitchIncompatibleAgentError,
...
pub enum Action {
    ...
    /// Open ainxt.dev in the browser for SuperAinxt subscription upsell.
    OpenSubscribeUrl,
    /// Re-check subscription status via the shell's `ainxt.dev/auth/check_subscription`.
    CheckSubscription,
    ...
    /// Open ainxt.dev managed connectors, appending session teamId when set.
    ...
    /// `prompt_id`. Routed to the agent as `ainxt.dev/queue/remove`;
    /// the resulting `ainxt.dev/queue/changed` rebroadcast is the source of truth.
    QueueRemoveShared {
        id: String,
        expected_version: u64,
    },
    /// Reorder the server-authoritative (shared) queued prompts to match
    /// `ordered_ids`. Routed as `ainxt.dev/queue/reorder`.
    QueueReorderShared {
        ordered_ids: Vec<String>,
    },
    /// Clear the caller's server-authoritative (shared) queued prompts.
    /// Routed as `ainxt.dev/queue/clear`.
    QueueClearShared,
    /// Replace the text of a server-authoritative (shared) queued prompt.
    /// Routed to the agent as `ainxt.dev/queue/edit`; the rebroadcast of
    /// `ainxt.dev/queue/changed` is the source of truth. Last write wins via the
    ...
    /// merges its text into the in-flight turn. Routed as `ainxt.dev/queue/interject`;
    /// the `ainxt.dev/session/interjection` + `ainxt.dev/queue/changed` rebroadcasts are
    /// the source of truth (no optimistic client-side block). Matches the local
    ...
    OpenTranscriptPager,
    /// Minimal mode (`ainxt --minimal`): re-print the most-recently committed
    ...
    OpenExtensionsModal {
        ...
        trigger: ainxt_telemetry::events::ExtensionsModalTrigger,
    },
    ...
    /// Open the in-TUI `[model.*]` add/edit/remove screen. Reachable from
    /// the `/model` picker's "+ Add new model..." row, the command palette,
    /// and (when no model is configured) the Welcome screen.
    OpenModelEditor,
    /// Persist an add/edit of one `[model.<id>]` entry. `original_key` is
    /// `Some` (and possibly different from `model_key`) when editing an
    /// existing entry, including a rename.
    SaveModelOverride {
        original_key: Option<String>,
        model_key: String,
        fields: ainxt_shell::agent::config_model_override_write::ModelOverrideFormFields,
    },
    /// Delete `[model.<model_key>]` entirely.
    DeleteModelOverride { model_key: String },
    ...
    ExecuteHooksAction(ainxt_hooks_plugins_types::HooksAction),
    /// Execute a plugins management action from the modal.
    ExecutePluginsAction(ainxt_hooks_plugins_types::PluginsAction),
    /// Execute a marketplace management action from the modal.
    ExecuteMarketplaceAction(ainxt_hooks_plugins_types::MarketplaceAction),
    /// Add or update an MCP server via ainxt.dev/mcp/upsert.
    UpsertMcpServer {
        name: String,
        config: Box<ainxt_shell::util::config::McpServerConfig>,
    },
    /// Delete an MCP server via ainxt.dev/mcp/delete.
    DeleteMcpServer {
        server_name: String,
    },
    ...
    /// Toggle a skill enable/disable via ainxt.dev/skills/toggle.
    ToggleSkill {
        skill_name: String,
        enabled: bool,
    },
    ...
    DemoteToBackground,
    /// Request current bundle cache status via `ainxt.dev/bundle/status`.
    RequestBundleStatus,
    ...
    AnnouncementsOpenCta(ainxt_telemetry::events::AnnouncementCtaSurface),
    ...
    /// Commit the active theme (canonical name, e.g. `"ainxtnight"`, `"auto"`).
    SetTheme(String),
    /// Commit the ainxt gateway base URL. Persists the derived
    /// `[endpoints]` keys to `config.toml`; takes effect on next launch
    /// (the gateway URL is frozen at startup — no live hot-swap).
    ...
    ShowSessionInfo,
    /// Show session usage stats (`/stats`): active model, token breakdown,
    /// and cost, sourced from the session's own usage ledger. Distinct from
    /// `ShowUsage` (`/usage`), which is a platform billing/credit-limit summary.
    ShowSessionUsage,
    ...
    ShowContextInfo,
    /// Open the source-control view (clickable changed-files indicator /
    /// `/changes`). Phase A shows a "coming soon" toast; Phase B opens the
    /// real changed-files panel + diff.
    OpenSourceControl,
    /// Flip the source-control changed-files indicator between counting the
    /// git working tree and the current session's changes (Phase B).
    SourceControlToggleMode,
    /// Select the file at the given list index in the source-control panel and
    /// open its diff (Phase B).
    SourceControlSelectFile(usize),
    /// Go back one level in the source-control panel: diff → list, or close the
    /// panel when already at the list (Phase B).
    SourceControlBack,
    /// Toggle stage/unstage on the selected changed file (Phase C, `space`).
    /// Fires an `Effect::GitStage`/`GitUnstage`, then re-fetches status.
    SourceControlToggleStage,
    /// Open the commit sub-view (Phase C, `c`) and request an AI-suggested
    /// commit message.
    SourceControlOpenCommit,
    /// Commit the current (edited) message (Phase C, `Ctrl+S`). Fires an
    /// `Effect::GitCommit`.
    SourceControlCommit,
    /// A key event routed to the commit-view message editor (Phase C).
    SourceControlEditMessage(crossterm::event::KeyEvent),
    ...
    /// Open the Agent Dashboard view (`/dashboard`, `Ctrl+\`, `ainxt dashboard`).
    OpenDashboard,
...
///
/// Both variants write to `~/.ainxt/config.toml` and route ACP
/// `ainxt.dev/yolo_mode_changed` notifications. The ACP notification is
...
pub enum Effect {
    ...
    ScanForeignSessions {
        cwd: std::path::PathBuf,
        compat: ainxt_workspace::foreign_sessions::EnabledForeignSessionSources,
        ainxt_home: std::path::PathBuf,
    ...
    FetchSessionList {
        /// Text search pushed down to `ainxt.dev/session/list` as `query` (chat
    ...
    /// `ainxt.dev/sessions/list`. Only issued in leader mode while the
    /// dashboard is open.
    FetchRoster,
    /// Fetch the local on-disk session list (dormant/idle sessions) for the
    /// dashboard via `ainxt.dev/session/list` — the non-leader fallback for the
    ...
    /// Cancel a subagent via `ainxt.dev/subagent/cancel`.
    KillSubagent {
        session_id: acp::SessionId,
        subagent_id: String,
    },
    ...
    /// Persist the dashboard's `[dashboard]` configuration to `~/.ainxt/config.toml`.
    ...
    /// Persist a typed setting to `~/.ainxt/config.toml`. On failure,
    ...
    /// Write one `[model.<id>]` entry (add or edit). `agent_id` routes the
    /// `TaskResult` back to the agent tab whose `ModelEditor` modal is
    /// waiting on it (it may no longer be the active tab by the time the
    /// write completes).
    SaveModelOverride {
        agent_id: AgentId,
        original_key: Option<String>,
    ...
    /// Delete `[model.<model_key>]`. See `SaveModelOverride` for `agent_id`.
    DeleteModelOverride {
        agent_id: AgentId,
        model_key: String,
    },
    ...
    /// `ainxt.dev/queue/remove`. The agent re-broadcasts the authoritative queue.
    QueueRemove {
        session_id: acp::SessionId,
        id: String,
        expected_version: u64,
    },
    /// Reorder server-owned queued prompts: fire-and-forget `ainxt.dev/queue/reorder`.
    QueueReorder {
        session_id: acp::SessionId,
        ordered_ids: Vec<String>,
    },
    ...
    /// `ainxt.dev/queue/edit`. The session actor's serialized mailbox makes this
    /// last-writer-wins for concurrent edits; the rebroadcast of
    /// `ainxt.dev/queue/changed` is the truth signal.
    QueueEdit {
        session_id: acp::SessionId,
        id: String,
        new_text: String,
    ...
    /// fire-and-forget `ainxt.dev/queue/interject`. The session actor atomically
    ...
    /// Resolve the running agent name for a session (`ainxt.dev/session/info`).
    ...
    /// Fetch MCP server list from the shell (ainxt.dev/mcp/list).
    FetchMcpsList {
        agent_id: AgentId,
        session_id: acp::SessionId,
        cache: bool,
    },
    /// Trigger MCP OAuth for a server (ainxt.dev/mcp/auth_trigger).
    McpAuthTrigger {
        agent_id: AgentId,
        session_id: acp::SessionId,
        server_name: String,
    },
    /// Fetch hooks list from the shell (ainxt.dev/hooks/list).
    FetchHooksList {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    /// Fetch plugins list from the shell (ainxt.dev/plugins/list).
    FetchPluginsList {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    ...
    HooksAction {
        agent_id: AgentId,
        session_id: acp::SessionId,
        action: ainxt_hooks_plugins_types::HooksAction,
    },
    /// Execute a plugins management action via ACP.
    PluginsAction {
        agent_id: AgentId,
        session_id: acp::SessionId,
        action: ainxt_hooks_plugins_types::PluginsAction,
    },
    ...
    /// Fetch skills list from the shell (ainxt.dev/skills/list).
    FetchSkillsList {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    /// Toggle a skill via ainxt.dev/skills/toggle (enable/disable without restart).
    ToggleSkill {
        agent_id: AgentId,
        session_id: acp::SessionId,
        skill_name: String,
    ...
    MarketplaceAction {
        agent_id: AgentId,
        session_id: acp::SessionId,
        action: ainxt_hooks_plugins_types::MarketplaceAction,
    },
    /// Install a plugin from the inline CTA via `ainxt.dev/marketplace/action`,
    ...
    /// Reload plugins after a CTA install via `ainxt.dev/plugins/action`
    ...
    /// Read the MCP server list after a CTA install via `ainxt.dev/mcp/list`,
    ...
    /// the same `ainxt.dev/mcp/list` fetch as `FetchPluginCtaMcps`, reported back via
    ...
    /// Upsert an MCP server via ainxt.dev/mcp/upsert.
    UpsertMcpServer {
        agent_id: AgentId,
        session_id: acp::SessionId,
        name: String,
    ...
    /// Live-toggle an MCP server via ainxt.dev/mcp/toggle (no restart needed).
    ToggleMcpServer {
        agent_id: AgentId,
        session_id: acp::SessionId,
        server_name: String,
    ...
    /// Toggle a single MCP tool via ainxt.dev/mcp/toggle_tool.
    ToggleMcpTool {
        agent_id: AgentId,
        session_id: acp::SessionId,
        server_name: String,
    ...
    /// Fetch and display session info via ainxt.dev/session/info.
    ShowSessionInfo {
        agent_id: AgentId,
        session_id: acp::SessionId,
        show_resolved_model: bool,
    },
    /// Fetch and display detailed context usage via ainxt.dev/session/info.
    ShowContextInfo {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    /// Fetch and display session usage stats (`/stats`) via
    /// ainxt.dev/session/usage: active model, token breakdown, and cost from
    /// the session's own usage ledger.
    ShowSessionUsage {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    /// Fetch current bundle cache status via `ainxt.dev/bundle/status`.
    FetchBundleStatus,
    /// Fetch a bundled entry's raw content via `ainxt.dev/bundle/entry/get`.
    ...
    /// Send raw note to ainxt.dev/memory/rewrite for LLM-powered reformatting.
    ...
    /// Fire a /btw side question via ainxt.dev/btw ext method.
    SendBtw {
        agent_id: AgentId,
        session_id: acp::SessionId,
        question: String,
    },
    /// Fetch the git working-tree status (changed files) via the
    /// `ainxt.dev/git/status` ext method, for the source-control panel (Phase B).
    /// The reply becomes a `TaskResult::GitStatusLoaded`.
    GitStatus {
        agent_id: AgentId,
        session_id: acp::SessionId,
        /// Generation guard echoed back in the reply to drop stale results.
        generation: u64,
        include_untracked: bool,
        include_stats: bool,
    },
    /// Fetch the working-tree-vs-HEAD diff for one file via the
    /// `ainxt.dev/git/diffs` ext method (Phase B). The reply becomes a
    ...
    /// Stage the given paths via `ainxt.dev/git/stage` (Phase C). The reply
    /// becomes a `TaskResult::GitStaged` which re-fetches status on success.
    GitStage {
        agent_id: AgentId,
        session_id: acp::SessionId,
        generation: u64,
    ...
    /// Unstage the given paths via `ainxt.dev/git/unstage` (Phase C).
    GitUnstage {
        agent_id: AgentId,
        session_id: acp::SessionId,
        generation: u64,
    ...
    /// Commit the staged changes with `message` via `ainxt.dev/git/commit`
    /// (Phase C). The reply becomes a `TaskResult::GitCommitDone`.
    GitCommit {
        agent_id: AgentId,
        session_id: acp::SessionId,
        generation: u64,
    ...
    /// Ask the shell for an AI-suggested commit message via
    /// `ainxt.dev/git/suggest_commit_message` (Phase C). The reply becomes a
    ...
    /// Request a session recap via the ainxt.dev/recap ext method. Fire-and-forget:
    /// the recap arrives later as a `SessionRecap` notification.
    SendRecap {
        session_id: acp::SessionId,
        auto: bool,
    },
    /// Send a mid-turn interjection via ainxt.dev/interject ext method.
    SendInterject {
        agent_id: AgentId,
        session_id: acp::SessionId,
        text: String,
        /// Client-minted id echoed back on the `ainxt.dev/session/interjection`
    ...
    /// Log out via `ainxt.dev/auth/logout` (shell clears auth.json + in-memory state).
    Logout,
    /// Re-check subscription status via `ainxt.dev/auth/check_subscription`.
    ...
    /// registry (`~/.ainxt/active_sessions.json`).
    ...
    /// Call `ainxt.dev/session/fork` to create a peer session that resumes
    ...
    /// Fetch billing/credit usage from the agent's `ainxt.dev/billing` extension.
    ...
    /// Send an ACP `ainxt.dev/suggest` request to the shell. `agent_id` is echoed
    ...
    /// Send an ACP `ainxt.dev/suggestPrompt` request to the shell — predict the
    /// user's likely next prompt after a completed turn (tab autocomplete
    /// ghost text).
    FetchPromptSuggestion {
        agent_id: AgentId,
        generation: u64,
        /// Suggestion model resolved by the pager (`ainxt-build-0.1` when the
...
/// Outcome of an `ainxt.dev/subagent/cancel` request, telling dispatch whether the
...
pub enum TaskResult {
    ...
    WorktreeForked {
        ...
        restore_degree: Option<ainxt_workspace::session::git::RestoreDegree>,
    },
    ...
    SessionLoaded {
        ...
        /// `_meta["ainxt.dev/runningPromptId"]`), present only when the session was
    ...
    SessionListLoaded {
        ...
        /// Degraded conversations lane (`_meta["ainxt.dev/partial"]`), surfaced
    ...
    ForeignResumeHintDetected {
        canonical_cwd: std::path::PathBuf,
        launch_token: u64,
        hint: Option<ainxt_workspace::foreign_sessions::RecentForeignSession>,
    },
    ...
    /// Leader session roster loaded via `ainxt.dev/sessions/list`.
    RosterLoaded {
    ...
    CancelComplete,
    /// Response to `ainxt.dev/subagent/cancel`; see [`SubagentKillOutcome`].
    ...
    BgTaskKilled {
        session_id: String,
        task_id: String,
        outcome: Option<ainxt_tools::types::KillOutcome>,
    },
    ...
    ChangelogFetched {
        ...
        entries: Vec<ainxt_shell::util::changelog::ChangelogEntry>,
    },
    ...
    AuthUrlReady {
        ...
        external: bool,
        /// Presentation mode from `ainxt.dev/auth/get_url`; `None` on older agents.
        mode: Option<String>,
    },
    ...
    HooksListLoaded {
        agent_id: AgentId,
        result: Result<ainxt_hooks_plugins_types::HooksListResponse, String>,
    },
    /// Plugins list fetched from shell.
    PluginsListLoaded {
        agent_id: AgentId,
        result: Result<ainxt_hooks_plugins_types::PluginsListResponse, String>,
    },
    /// Hooks action completed.
    HooksActionResult {
        agent_id: AgentId,
        result: Result<ainxt_hooks_plugins_types::ActionOutcome, String>,
    },
    ...
    MarketplaceListLoaded {
        agent_id: AgentId,
        result: Result<ainxt_hooks_plugins_types::MarketplaceListResponse, String>,
    },
    ...
    SkillsListLoaded {
        agent_id: AgentId,
        result: Result<Vec<ainxt_tools::implementations::skills::types::SkillInfo>, String>,
    },
    ...
    SessionInfoComplete {
        agent_id: AgentId,
        info: Box<ainxt_shell::session::SessionInfoResponse>,
        text: String,
    },
    ...
    /// Session usage stats (`/stats`) fetched successfully.
    SessionUsageComplete {
        agent_id: AgentId,
        text: String,
    },
    /// Session usage stats fetch failed.
    SessionUsageFailed {
        agent_id: AgentId,
        error: String,
    },
    ...
    /// Git working-tree status for the source-control panel arrived (Phase B).
    GitStatusLoaded {
        agent_id: AgentId,
        generation: u64,
        result: Result<GitStatusPayload, String>,
    },
    /// A single file's working-tree-vs-HEAD diff for the panel arrived (Phase B).
    GitDiffLoaded {
        agent_id: AgentId,
        generation: u64,
        path: String,
        /// `Ok((old_text, new_text))` or an error message (e.g. size exceeded).
        result: Result<(String, String), String>,
    },
    /// A stage/unstage op finished (Phase C). On success the panel re-fetches
    /// status so the staged/unstaged state refreshes.
    GitStaged {
        agent_id: AgentId,
        generation: u64,
        /// `Ok(())` on success, else a human error message.
        result: Result<(), String>,
    },
    /// A commit op finished (Phase C).
    GitCommitDone {
        agent_id: AgentId,
        generation: u64,
        /// `Ok(Option<shorthash>)` on success (hash if the backend returned one),
        /// else a human error message shown in the commit view.
        result: Result<Option<String>, String>,
    },
    /// An AI-suggested commit message arrived (Phase C). Prefills the editor
    /// only if the user hasn't typed yet (never clobbers user input).
    CommitMessageSuggested {
        agent_id: AgentId,
        generation: u64,
        result: Result<String, String>,
    },
    /// `ainxt.dev/recap` request acknowledged (fire-and-forget). The recap itself
    ...
    LogoutComplete,
    /// Shell responded to `ainxt.dev/auth/check_subscription`. `verify` echoes
    ...
    AuthCopiedTimeout,
    DeepSearchResults {
        results: Vec<ainxt_shell::extensions::session_search::SearchSessionHit>,
        seq: u64,
    },
    /// `ainxt.dev/session/fork` completed (no-worktree path). The pager adopts
    ...
    /// `ainxt.dev/session/fork` failed. The placeholder agent stays in
    ...
    /// Shell suggestions loaded from ACP `ainxt.dev/suggest`. `request_text` /
    ...
    /// Predicted next prompt loaded from ACP `ainxt.dev/suggestPrompt`.
    ...
    /// `[model.<id>]` write succeeded (add or edit).
    ModelOverrideSaved { agent_id: AgentId, model_key: String },
    /// `[model.<id>]` write failed. The editor stays on the form (no
    /// in-memory rollback needed — nothing was applied optimistically).
    ModelOverrideSaveFailed { agent_id: AgentId, error: String },
    /// `[model.<id>]` delete succeeded.
    ModelOverrideDeleted { agent_id: AgentId, model_key: String },
    /// `[model.<id>]` delete failed.
    ModelOverrideDeleteFailed { agent_id: AgentId, error: String },
