...
use ainxt_shell::sampling::error::{
    RATE_LIMITED_ERROR_CODE, rate_limited_user_message,
};
use ainxt_shell::session::ExtMethodResult;
...
/// Run the post-CTA-install `ainxt.dev/mcp/list` read (uncached, which also nudges
...
pub(super) fn format_acp_error(err: &acp::Error, is_api_key_auth: bool) -> String {
    ...
    if err.code == acp::ErrorCode::InvalidParams && let Some(data) = &err.data
        && let Some(msg) = ainxt_shell::sampling::error::error_detail_from_data(data)
        && !msg.is_empty()
    {
        return msg;
    }
...
) -> (bool, Option<String>, Option<ainxt_workspace::session::git::RestoreDegree>) {
...
/// CANONICAL wire parser for `LoadSessionResponse._meta["ainxt.dev/runningPromptId"]`.
///
...
pub(crate) fn parse_session_load_running_prompt_id(
    ...
    resp_meta
        .and_then(|m| m.get("ainxt.dev/runningPromptId"))
        .and_then(|v| v.as_str())
        .map(String::from)
}
...
pub(crate) fn sanitize_user_error(raw: &str) -> String {
    if raw.contains(ainxt_fast_worktree::OUT_OF_DISK_CONTEXT)
        || raw.contains(ainxt_fast_worktree::ENOSPC_OS_MESSAGE)
    {
        return "Out of disk space.".to_string();
    }
    if raw.trim() == "builder error" {
        return "Login request could not be built. Try again, or check AINXT_GATEWAY_URL / \
                AINXT_API_BASE_URL for a malformed value."
            .to_string();
    }
...
/// |-------|-----------|----------|--------------------------------|--------------------|
/// | false | false     | false    | `ainxt-build` (default)         | `false`            |
/// | false | true      | false    | `ainxt-build` (default)         | `false`            |
/// | false | false     | true     | `ainxt-build-ask-user`          | omitted (shell gate) |
/// | false | true      | true     | `ainxt-build-ask-user`          | omitted (shell gate) |
/// | true  | false     | false    | `ainxt-build-plan-no-subagents` | `false`            |
/// | true  | true      | false    | `ainxt-build-plan`              | `false`            |
/// | true  | false     | true     | `ainxt-build-plan-no-subagents` | omitted (shell gate) |
/// | true  | true      | true     | `ainxt-build-plan`              | omitted (shell gate) |
///
/// When [`Self::chat_mode`] is set (gateway light-frontend / `--chat`), Build
/// `agentProfile` injection is omitted (K12) and `_meta["ainxt.dev/session"].kind`
...
pub(crate) struct SessionFlags {
    ...
    /// Injected as `ainxt.dev/restore_code` into `LoadSession` meta, or passed
...
impl SessionFlags {
    ...
    ///
    /// Returns `None` for the default `ainxt-build` profile (no `_meta`
    ...
    pub(super) fn agent_profile(&self) -> Option<&'static str> {
        ...
        match (self.plan_mode, self.subagents, self.ask_user) {
            (true, true, _) => Some("ainxt-build-plan"),
            (true, false, _) => Some("ainxt-build-plan-no-subagents"),
            (false, _, true) => Some("ainxt-build-ask-user"),
            (false, _, false) => None,
        }
    }
    ...
    /// `AINXT_AGENT` — the env var chooses the *agent*, but the tool-strip is
    /// independent. Chat mode additionally stamps `ainxt.dev/session.kind`.
    pub(super) fn to_meta(&self) -> Option<acp::Meta> {
        let mut meta = serde_json::Map::new();
        if self.chat_mode {
            if self.plan_mode || self.agent_override.is_some()
                || std::env::var("AINXT_AGENT").ok().is_some_and(|s| !s.trim().is_empty())
            {
                tracing::warn!(
        ...
        } else if std::env::var("AINXT_AGENT").ok().is_some_and(|s| !s.trim().is_empty())
        {} else if let Some(profile) = self.agent_profile() {
            meta.insert("agentProfile".into(), serde_json::json!(profile));
        }
        if self.chat_mode {
            meta.insert("ainxt.dev/session".into(), serde_json::json!({ "kind" : "chat" }));
        }
        if !self.ask_user {
...
pub(super) const CHAT_FORBIDDEN_WORKSPACE_BIND_KEYS: &[&str] = &[
    "envId",
    "ainxt.dev/cloud_server_id",
    "ainxt.dev/cloud_existing_workspace",
];
/// Stamp `_meta["ainxt.dev/session"].kind = "chat"` and strip Build `agentProfile` (K12).
pub(super) fn apply_chat_kind_meta(meta: &mut Option<acp::Meta>) {
    let obj = meta.get_or_insert_with(acp::Meta::new);
    obj.insert("ainxt.dev/session".into(), serde_json::json!({ "kind" : "chat" }));
...
pub(super) fn extract_first_user_prompt(
    info: &ainxt_shell::session::info::Info,
) -> Option<String> {
    use std::io::BufRead;
    let history_path = ainxt_shell::session::persistence::session_dir(info)
...
pub(super) fn count_chat_history_stats(history_path: &Path) -> (usize, usize) {
    use std::io::BufRead;
    use ainxt_shell::sampling::{AssistantItem, ConversationItem, UserItem};
...
/// Degraded conversations lane on `ainxt.dev/session/list`, parsed from the
/// response's `_meta["ainxt.dev/partial"]` envelope.
...
/// Read `_meta["ainxt.dev/partial"]` from a session-list payload. `None` when the
...
pub(super) fn parse_session_list_partial(
    ...
    let partial = payload.get("_meta")?.get("ainxt.dev/partial")?;
...
/// Parse the `ainxt.dev/session/list` response payload (the unwrapped
    ...
    entries
        ...
        .filter_map(|v| {
            ...
            let is_conversation = v
                .get("_meta")
                .and_then(|m| m.get("ainxt.dev/session"))
            ...
            use ainxt_tools::implementations::skills::skill::extract_skill_display_text;
        ...
        .filter_map(|mut e| {
            ...
            if e.source == "remote"
                && ainxt_shell::session::resolve_local_session_any_cwd(&e.id)
                    .is_some()
            {
...
pub(super) async fn send_check_subscription(
    ...
    let req = acp::ExtRequest::new(
        "ainxt.dev/auth/check_subscription",
        serde_json::value::to_raw_value(&serde_json::json!({}))
...
pub(crate) async fn persist_setting(
    ...
    match key {
        "compact_mode" => {
            ...
            ainxt_shell::util::config::set_compact_mode(b)
                .await
        ...
        "show_timestamps" => {
            ...
            ainxt_shell::util::config::set_show_timestamps(b)
                .await
                .map_err(|e| e.to_string())
        }
        "show_timeline" => {
            ...
            ainxt_shell::util::config::set_show_timeline(b)
                .await
                .map_err(|e| e.to_string())
        }
        "simple_mode" => {
            ...
            ainxt_shell::util::config::set_simple_mode(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.undo" => {
            ...
            ainxt_shell::util::config::set_contextual_hint_undo(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.plan_mode" => {
            ...
            ainxt_shell::util::config::set_contextual_hint_plan_mode(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.image_input" => {
            ...
            ainxt_shell::util::config::set_contextual_hint_image_input(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.send_now" => {
            ...
            ainxt_shell::util::config::set_contextual_hint_send_now(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.small_screen" => {
            ...
            ainxt_shell::util::config::set_contextual_hint_small_screen(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.word_select" => {
            ...
            ainxt_shell::util::config::set_contextual_hint_word_select(b)
                .await
                .map_err(|e| e.to_string())
        }
        "theme" => {
            ...
            ainxt_shell::util::config::set_theme(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "gateway_url" => {
            let SettingValue::String(s) = value else {
                return Err(kind_mismatch("gateway_url", "String", &value));
            };
            // Self-contained round-trip via toml_edit — writes both derived
            // `[endpoints]` keys so the new gateway takes effect next launch.
            crate::config_toml_edit::set_gateway_url(&s).map_err(|e| e.to_string())
        }
        "auto_dark_theme" => {
            let SettingValue::Enum(s) = value else {
                return Err(kind_mismatch("auto_dark_theme", "Enum", &value));
            };
            ainxt_shell::util::config::set_auto_dark_theme(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "auto_light_theme" => {
            ...
            ainxt_shell::util::config::set_auto_light_theme(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "default_model" => {
            ...
            ainxt_shell::util::config::set_default_model(s)
                .await
                .map_err(|e| e.to_string())
        }
        "scroll_speed" => {
            ...
            ainxt_shell::util::config::set_scroll_speed(i)
                .await
                .map_err(|e| e.to_string())
        }
        "scroll_mode" => {
            ...
            ainxt_shell::util::config::set_scroll_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "invert_scroll" => {
            ...
            ainxt_shell::util::config::set_invert_scroll(b)
                .await
                .map_err(|e| e.to_string())
        }
        "display_refresh_auto_cadence" => {
            ...
            ainxt_shell::util::config::set_display_refresh_auto_cadence(b)
                .await
                .map_err(|e| e.to_string())
        }
        "scroll_lines" => {
            ...
            ainxt_shell::util::config::set_scroll_lines(i)
                .await
                .map_err(|e| e.to_string())
        }
        "default_selected_permission" => {
            ...
            ainxt_shell::util::config::set_default_selected_permission(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "cancel_subagents_on_turn_cancel" => {
            ...
            ainxt_shell::util::config::set_cancel_subagents_on_turn_cancel(
                    s.to_string(),
                )
                .await
                .map_err(|e| e.to_string())
        ...
        "vim_mode" => {
            ...
            ainxt_shell::util::config::set_vim_mode(b)
                .await
                .map_err(|e| e.to_string())
        }
        "remember_tool_approvals" => {
            ...
            ainxt_shell::util::config::set_remember_tool_approvals(b)
                .await
                .map_err(|e| e.to_string())
        }
        "toolset.ask_user_question.timeout_enabled" => {
            ...
            ainxt_shell::util::config::set_ask_user_question_timeout_enabled(b)
                .await
                .map_err(|e| e.to_string())
        }
        "show_thinking_blocks" => {
            ...
            ainxt_shell::util::config::set_show_thinking_blocks(b)
                .await
                .map_err(|e| e.to_string())
        }
        "group_tool_verbs" => {
            ...
            ainxt_shell::util::config::set_group_tool_verbs(b)
                .await
                .map_err(|e| e.to_string())
        }
        "collapsed_edit_blocks" => {
            ...
            ainxt_shell::util::config::set_collapsed_edit_blocks(b)
                .await
                .map_err(|e| e.to_string())
        }
        "prompt_suggestions" => {
            ...
            ainxt_shell::util::config::set_prompt_suggestions(b)
                .await
                .map_err(|e| e.to_string())
        }
        "keep_text_selection" => {
            ...
            ainxt_shell::util::config::set_keep_text_selection(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        ...
        "render_mermaid" => {
            ...
            ainxt_shell::util::config::set_render_mermaid(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "hunk_tracker_mode" => {
            ...
            ainxt_shell::util::config::set_hunk_tracker_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "screen_mode" => {
            ...
            ainxt_shell::util::config::set_screen_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "voice_capture_mode" => {
            ...
            ainxt_shell::util::config::set_voice_capture_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "voice_stt_language" => {
            ...
            ainxt_shell::util::config::set_voice_stt_language(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "max_thoughts_width" => {
            ...
            ainxt_shell::util::config::set_max_thoughts_width(i)
                .await
                .map_err(|e| e.to_string())
        }
        "show_tips" => {
            ...
            ainxt_shell::util::config::set_show_tips(b)
                .await
                .map_err(|e| e.to_string())
        }
        "auto_update" => {
            ...
            ainxt_shell::util::config::set_auto_update(b)
                .await
                .map_err(|e| e.to_string())
        }
        "fork_secondary_model" => {
            ...
            ainxt_shell::util::config::set_fork_secondary_model(s)
                .await
                .map_err(|e| e.to_string())
        }
...
/// 2. Fire ACP `ainxt.dev/yolo_mode_changed` (gated on disk success for
...
pub(crate) async fn persist_permission_mode_and_notify(
    ...
    let disk_result = ainxt_shell::util::config::update_config(|cfg| {
...
/// Whether to fire the ACP `ainxt.dev/yolo_mode_changed` notification.
...
pub(super) fn marketplace_outcome_succeeded(
    outcome: &ainxt_hooks_plugins_types::ActionOutcome,
) -> bool {
    outcome.status == ainxt_hooks_plugins_types::OutcomeStatus::Success
}
/// Extract the typed kill outcome from an `ainxt.dev/task/kill` ext response.
///
...
/// serializes (`ainxt_shell::extensions::task::KillTaskResponse` +
/// `ainxt_shell::session::result::ExtMethodResult`) so the contract stays
...
pub(super) fn parse_kill_outcome(
    resp: &str,
) -> Option<ainxt_tools::types::KillOutcome> {
    use ainxt_shell::extensions::task::KillTaskResponse;
    use ainxt_shell::session::result::ExtMethodResult;
    serde_json::from_str::<ExtMethodResult<KillTaskResponse>>(resp)
        .ok()
        .and_then(|envelope| envelope.result)
        .map(|payload| payload.outcome)
}
/// Map an `ainxt.dev/subagent/cancel` response (payload under `result`) to a kill
...
pub(super) fn parse_subagent_kill_outcome(resp: &str) -> SubagentKillOutcome {
    use ainxt_shell::extensions::task::{
...
pub(super) fn credit_balance_from_config(
    c: ainxt_shell::extensions::billing::BillingConfig,
...
/// Fetch the user's auto top-up rule via the `ainxt.dev/auto-topup-rule` extension.
...
/// Map an `ainxt.dev/auto-topup-rule` payload to an [`AutoTopupFetch`]. A body that
...
pub(super) fn parse_auto_topup_response(
    ...
    use ainxt_shell::extensions::billing::GetAutoTopupRuleResponse;
...
/// A blocking flock on the shared, possibly-network `~/.ainxt` lock must never
...
pub(super) fn unregister_active_session_best_effort_in(
    ...
    match ainxt_shell::active_sessions::try_unregister_in(root, session_id) {
        Ok(true) => {}
        Ok(false) => {
            tracing::debug!(
