...
use ainxt_acp_lib::{AcpAgentTx, acp_send};
use actions::{
...
use ainxt_shell::sampling::error::http_status_from_error;
use ainxt_shell::session::{ExtMethodResult, SessionInfoResponse};

/// Best-effort human message from an `ExtMethodResult.error` value, which may be
/// a bare string or an `{ message, code, data }` object.
fn git_err_string(err: &serde_json::Value) -> String {
...
/// Parse an `ainxt.dev/git/status` reply into a [`actions::GitStatusPayload`]
/// for the source-control panel (Phase B).
///
/// The reply is the standard `{ result: GitStatusExtResponse, error?: ... }`
/// envelope. We surface the working-tree (unstaged) changes; a leading `error`
/// in the envelope is reported as an `Err`.
fn parse_git_status_reply(raw: &str) -> Result<actions::GitStatusPayload, String> {
    use ainxt_workspace::session::git::{GitStatusData};
    use ainxt_workspace::workspace_ops::GitStatusExtResponse;
    let envelope: ExtMethodResult<GitStatusExtResponse> = serde_json::from_str(raw)
        .map_err(|e| sanitize_user_error(&format!("couldn't parse git status: {e}")))?;
    if let Some(err) = envelope.error {
        return Err(sanitize_user_error(&git_err_string(&err)));
    }
    let data: GitStatusData = envelope
        .result
        .and_then(|r| r.data)
        .ok_or_else(|| "git status returned no data".to_string())?;
    // Working-tree changes (unstaged) are what the read-only panel shows.
    // Fall back to staged if unstaged is empty (e.g. everything already staged).
    let mut rows = Vec::new();
    for change in data.unstaged.iter().chain(data.staged.iter()) {
        // De-dup by path so a file staged+unstaged shows once.
        if rows
            .iter()
            .any(|r: &crate::views::source_control_panel::ChangeRow| r.path == change.path)
        {
            continue;
        }
        rows.push(crate::views::source_control_panel::ChangeRow {
    ...
    Ok(actions::GitStatusPayload {
        branch: data.branch,
        files: rows,
    })
}

/// Parse an `ainxt.dev/git/diffs` reply into `(old_text, new_text)` for `path`.
///
/// The envelope carries a `GitDiffsData { files }`; we pick the file matching
/// `path` (or the sole file) and return its `old_text` / `new_text`. A size-limit
/// or other backend error surfaces as `Err` so the panel shows a message rather
/// than crashing.
fn parse_git_diff_reply(raw: &str, path: &str) -> Result<(String, String), String> {
    use ainxt_workspace::session::git::GitDiffsData;
    let envelope: ExtMethodResult<GitDiffsData> = serde_json::from_str(raw)
        .map_err(|e| sanitize_user_error(&format!("couldn't parse git diff: {e}")))?;
    if let Some(err) = envelope.error {
        return Err(sanitize_user_error(&git_err_string(&err)));
    }
    let data = envelope
        .result
        .ok_or_else(|| "git diff returned no data".to_string())?;
    let file = data
        .files
        .iter()
        .find(|f| f.path == path)
        .or_else(|| data.files.first())
        .ok_or_else(|| "no diff for file (unchanged?)".to_string())?;
    let old_text = file.old_text.clone().unwrap_or_default();
    let new_text = file.new_text.clone().unwrap_or_default();
    Ok((old_text, new_text))
}

/// Parse a bare `{ result?: {}, error?: ... }` envelope for a git write op that
/// returns no meaningful data (stage/unstage). An envelope-level `error` maps to
/// `Err`; anything else is `Ok(())`.
fn parse_git_ack_reply(raw: &str) -> Result<(), String> {
    let value: serde_json::Value = serde_json::from_str(raw)
        .map_err(|e| sanitize_user_error(&format!("couldn't parse git reply: {e}")))?;
...
/// Parse an `ainxt.dev/git/commit` reply into `Ok(Option<shorthash>)`.
///
/// The envelope carries a `CommitData { commitHash, output }`. A leading
/// `error` maps to `Err`. The short hash (first 7 chars) is returned when the
/// backend provided a commit hash so the UI can toast "Committed <hash>".
fn parse_git_commit_reply(raw: &str) -> Result<Option<String>, String> {
    let value: serde_json::Value = serde_json::from_str(raw)
        .map_err(|e| sanitize_user_error(&format!("couldn't parse git commit: {e}")))?;
    if let Some(err) = value.get("error").filter(|v| !v.is_null()) {
        return Err(sanitize_user_error(&git_err_string(err)));
    }
...
/// Parse an `ainxt.dev/git/suggest_commit_message` reply into the suggested
/// message. An envelope-level `error` (e.g. "couldn't suggest…") maps to `Err`.
fn parse_suggest_commit_reply(raw: &str) -> Result<String, String> {
    let value: serde_json::Value = serde_json::from_str(raw)
        .map_err(|e| sanitize_user_error(&format!("couldn't parse suggestion: {e}")))?;
    if let Some(err) = value.get("error").filter(|v| !v.is_null()) {
        return Err(sanitize_user_error(&git_err_string(err)));
    }
    let data = value.get("result").unwrap_or(&value);
    let msg = data
        ...
        .ok_or_else(|| "no suggestion returned".to_string())?;
    Ok(msg)
}
...
pub(crate) fn execute(
    ...
    match effect {
        Effect::RegisterActiveSession { session_id, cwd } => {
            ...
            if let Err(e) = ainxt_shell::active_sessions::register(ainxt_shell::active_sessions::ActiveSession {
                session_id,
                pid: std::process::id(),
                cwd,
        ...
        Effect::CreateSession {
            ...
            let tx = acp_tx.clone();
            let compat = ainxt_tools::types::compat::CompatConfig::default();
            let mcp_servers = ainxt_shell::util::config::load_mcp_servers(
                &session_cwd,
                &compat,
            );
            ...
            tasks
                .spawn(async move {
                    if let Some(sid) = load_session_id {
                        ...
                        let wt_type = ainxt_shell::util::config::worktree_type();
                        ...
                        let ext_req = acp::ExtRequest::new(
                            "ainxt.dev/git/worktree/resume_session",
                    ...
                    let ext_req = acp::ExtRequest::new(
                        "ainxt.dev/git/worktree/create_from_worktree_sync",
                    ...
                    let mcp_servers = ainxt_shell::util::config::load_mcp_servers(
                        &session_cwd,
                        &ainxt_tools::types::compat::CompatConfig::default(),
                    );
                    let result = acp_send(
                            acp::NewSessionRequest::new(session_cwd.clone())
                                .mcp_servers(mcp_servers)
        ...
        Effect::LoadSession { agent_id, session_id, session_cwd, chat_kind } => {
            ...
            if let Some(true) = session_flags.restore_code {
                meta.get_or_insert_with(acp::Meta::new)
                    .insert("ainxt.dev/restore_code".into(), serde_json::Value::Bool(true));
            }
        ...
        Effect::ScanForeignSessions { cwd, compat, ainxt_home, coordinator, seq } => {
            ...
            let abort_handle = tasks
                .spawn(async move {
                    ...
                    let summaries = tokio::task::spawn_blocking(move || {
                            let _permit = permit;
                            ainxt_workspace::foreign_sessions::scan_foreign_sessions(
                                &cwd,
                                enabled,
                            )
                        })
            ...
            tasks
                .spawn(async move {
                    ...
                    let recent = crate::app::foreign_sessions::with_gated_sources_async(
                            ...
                            &ainxt_home,
                            |enabled| async move {
                                tokio::task::spawn_blocking(move || ainxt_workspace::foreign_sessions::most_recent_foreign_session(
                                        &cwd_for_scan,
                                        enabled,
                    ...
                    match acp_send(request, &tx).await {
                        Ok(resp) => {
                            ...
                            match parsed {
                                ...
                                None => {
                                    tracing::warn!(
                                        "failed to parse ainxt.dev/sessions/list response"
                                    );
        ...
        Effect::RestoreAndLoadSession { agent_id, session_id, session_cwd: _ } => {
            use ainxt_shell::agent::session_registry_client::SessionRegistryClient;
            use ainxt_shell::session::restore::restore_session_with_storage;
            let setup_started = std::time::Instant::now();
            let raw_config = ainxt_shell::config::load_effective_config();
            let setup = raw_config
                .ok()
                .and_then(|raw| {
                    let cfg = ainxt_shell::agent::config::Config::new_from_toml_cfg(
                            &raw,
                        )
                        .ok()?;
                    ...
                    let storage = ainxt_shell::auth::credential_provider::build_storage_client_for_proxy(
                        &proxy_base,
                        deployment_key,
                        alpha_test_key,
            ...
            tasks
                .spawn(async move {
                    ...
                    let progress: Option<
                        ainxt_shell::session::restore::ProgressCallback,
                    > = {
                        use ainxt_shell::session::restore::{PhaseStep, RestorePhase};
                        Some(
                            Box::new(move |event| {
            ...
            tasks
                .spawn(async move {
                    ...
                    let detail = tokio::task::spawn_blocking(move || {
                            let info = ainxt_shell::session::info::Info {
                                id: acp::SessionId::new(session_id),
                                cwd,
                            };
                            let history_path = ainxt_shell::session::persistence::session_dir(
                                    &info,
                                )
        ...
        Effect::SendBashCommand { agent_id, session_id, command, prompt_id } => {
            ...
            tasks
                .spawn(async move {
                    use ainxt_shell::extensions::prompt_meta::PromptBlockMeta;
                    ulog::info(
                        "prompt.acp_send.start",
                        Some(&session_id.0),
                        Some(
            ...
            tasks
                .spawn(async move {
                    ...
                    let req = acp::ExtRequest::new(
                        "ainxt.dev/compact_conversation",
                        serde_json::value::to_raw_value(&params)
        ...
        Effect::KillBgTask { session_id, task_id } => {
            ...
            tasks
                .spawn(async move {
                    let params = ainxt_shell::extensions::task::KillTaskRequest {
        ...
        Effect::SwitchModel {
            ...
            tasks
                .spawn(async move {
                    let meta = effort
                        .map(|eff| {
                            use ainxt_shell::sampling::types::{
                    ...
                    let result = acp_send(req, &tx)
                        ...
                        .map(|_| ())
                        .map_err(|e| {
                            use ainxt_shell::agent::config::ModelSwitchIncompatibleAgentError;
            ...
            tasks
                .spawn(async move {
                    let changelog = tokio::task::spawn_blocking(|| {
                            ainxt_shell::util::changelog::ChangelogManager::new()
                                .fetch()
                        })
                        .await
                        .unwrap_or_else(|e| {
                            tracing::warn!(error = % e, "changelog fetch task failed");
                            ainxt_shell::util::changelog::Changelog {
                                markdown: None,
                                entries: None,
                            }
                        });
        ...
        Effect::PersistAnnouncementsHidden { hidden_ids } => {
            tasks
                .spawn(async move {
                    ainxt_announcements::write_hidden_announcement_ids(&hidden_ids)
                        .await;
        ...
        Effect::PersistPreferredModel { model_id, reasoning_effort } => {
            ...
            tasks
                .spawn(async move {
                    let result = ainxt_shell::util::config::persist_models_default(
                            Some(model_id_str),
                            reasoning_effort,
                        )
                        .await
        ...
        Effect::SaveModelOverride {
            ...
            tasks.spawn(async move {
                match ainxt_shell::agent::config_model_override_write::save_model_override(
                    original_key,
                    model_key.clone(),
                    fields,
                )
                .await
                {
                    Ok(()) => TaskResult::ModelOverrideSaved { agent_id, model_key },
                    Err(error) => TaskResult::ModelOverrideSaveFailed {
                        agent_id,
        ...
        Effect::DeleteModelOverride { agent_id, model_key } => {
            tasks.spawn(async move {
                match ainxt_shell::agent::config_model_override_write::remove_model_override(
                    model_key.clone(),
                )
                .await
                {
                    Ok(()) => TaskResult::ModelOverrideDeleted { agent_id, model_key },
                    Err(error) => TaskResult::ModelOverrideDeleteFailed {
                        agent_id,
                        error: error.to_string(),
                    },
                }
                ...
                .spawn(async move {
                    ...
                    let result = match acp_send(req, &tx).await {
                        Ok(resp) => {
                            ...
                            let inner = wrapper.get("result").unwrap_or(&wrapper);
                            serde_json::from_value::<
                                ainxt_hooks_plugins_types::HooksListResponse,
                            >(inner.clone())
                ...
                .spawn(async move {
                    ...
                    let result = match acp_send(req, &tx).await {
                        Ok(resp) => {
                            ...
                            let inner = wrapper.get("result").unwrap_or(&wrapper);
                            serde_json::from_value::<
                                ainxt_hooks_plugins_types::PluginsListResponse,
                            >(inner.clone())
        ...
        Effect::HooksAction { agent_id, session_id, action } => {
            ...
            tasks
                .spawn(async move {
                    let req_body = ainxt_hooks_plugins_types::HooksActionRequest {
                        session_id: session_id.0.to_string(),
                        action,
                    };
                    let req = acp::ExtRequest::new(
                        "ainxt.dev/hooks/action",
                    ...
                    let result = match acp_send(req, &tx).await {
                        Ok(resp) => {
                            ...
                            let inner = wrapper.get("result").unwrap_or(&wrapper);
                            serde_json::from_value::<
                                ainxt_hooks_plugins_types::ActionOutcome,
                            >(inner.clone())
        ...
        Effect::PluginsAction { agent_id, session_id, action } => {
            ...
            tasks
                .spawn(async move {
                    let req_body = ainxt_hooks_plugins_types::PluginsActionRequest {
                        session_id: session_id.0.to_string(),
                        action,
                    };
                    let req = acp::ExtRequest::new(
                        "ainxt.dev/plugins/action",
                        serde_json::value::to_raw_value(&req_body)
                ...
                .spawn(async move {
                    ...
                    let result = match acp_send(req, &tx).await {
                        Ok(resp) => {
                            ...
                            let inner = wrapper.get("result").unwrap_or(&wrapper);
                            serde_json::from_value::<
                                ainxt_hooks_plugins_types::MarketplaceListResponse,
                            >(inner.clone())
                ...
                .spawn(async move {
                    ...
                    let result = match acp_send(req, &tx).await {
                        Ok(resp) => {
                            ...
                            serde_json::from_value::<
                                Vec<
                                    ainxt_tools::implementations::skills::types::SkillInfo,
                                >,
                        ...
                        Ok(resp) => {
                            ...
                            if parsed.is_ok() {
                                let refresh = acp::ExtRequest::new(
                                    "ainxt.dev/skills/refresh-baseline",
                                    serde_json::value::to_raw_value(&serde_json::json!({}))
            ...
            tasks
                .spawn(async move {
                    ...
                    for (name, old, new, source_url, rel_path) in outdated {
                        let action = ainxt_hooks_plugins_types::MarketplaceAction::Update {
                            source_url_or_path: source_url.clone(),
                            plugin_relative_path: rel_path.clone(),
                        };
                        let req_body = ainxt_hooks_plugins_types::MarketplaceActionRequest {
                            session_id: session_id.0.to_string(),
                            action,
                        };
                        let update_req = acp::ExtRequest::new(
                            "ainxt.dev/marketplace/action",
                            serde_json::value::to_raw_value(&req_body)
                    ...
                    if !succeeded.is_empty() {
                        ...
                        let notify_req = acp::ExtRequest::new(
                            "ainxt.dev/plugins/notify-updates",
            ...
            tasks
                .spawn(async move {
                    let plugin_name = plugin_relative_path
                        ...
                        .to_string();
                    let action = ainxt_hooks_plugins_types::MarketplaceAction::Install {
                        source_url_or_path,
                        plugin_relative_path,
                    };
                    let req_body = ainxt_hooks_plugins_types::MarketplaceActionRequest {
        ...
        Effect::ReloadPluginsForCta { agent_id, session_id, plugin_name } => {
            ...
            tasks
                .spawn(async move {
                    let req_body = ainxt_hooks_plugins_types::PluginsActionRequest {
                        session_id: session_id.0.to_string(),
                        action: ainxt_hooks_plugins_types::PluginsAction::Reload,
                    };
                    let req = acp::ExtRequest::new(
                        "ainxt.dev/plugins/action",
                        serde_json::value::to_raw_value(&req_body)
            ...
            tasks
                .spawn(async move {
                    ...
                    struct McpUpsertRequest {
                        ...
                        server_name: String,
                        #[serde(flatten)]
                        config: ainxt_shell::util::config::McpServerConfig,
                    }
        ...
        Effect::ShareSession { agent_id, session_id } => {
            use ainxt_shell::session::{ShareSessionRequest, ShareSessionResponse};
            let tx = acp_tx.clone();
            tasks
                .spawn(async move {
                    let request = acp::ExtRequest::new(
                        "ainxt.dev/share_session",
        ...
        Effect::SetCodingDataSharing { agent_id, opted_in, rollback_to_opted_in } => {
            ...
            tasks
                .spawn(async move {
                    let request = acp::ExtRequest::new(
                        "ainxt.dev/privacy/setCodingDataRetention",
                        serde_json::value::to_raw_value(
                                &serde_json::json!(
        ...
        Effect::ShowSessionUsage { agent_id, session_id } => {
            let tx = acp_tx.clone();
            tasks
                .spawn(async move {
                    match fetch_session_usage(&session_id, &tx).await {
                        Ok(usage) => {
                            ...
                            TaskResult::SessionUsageComplete { agent_id, text }
                        }
                        Err(error) => {
                            TaskResult::SessionUsageFailed { agent_id, error }
                        }
                    }
                });
        }
        Effect::SendFeedback { agent_id, session_id, feedback_text } => {
            use ainxt_shell::session::ClientType;
            use ainxt_shell::session::acp_types::ClientFeedbackInput;
        ...
        Effect::SaveMemoryNote { agent_id, text, cwd } => {
            tasks
                .spawn(async move {
                    let result = tokio::task::spawn_blocking(move || {
                            let storage = ainxt_shell::session::memory::MemoryStorage::new(
                                &cwd,
                                None,
                            );
                            storage
                                .append_to_memory(
                                    ainxt_shell::session::memory::MemoryScope::Global,
                                    &text,
                                )
                        })
                        .await
        ...
        Effect::GitStatus {
            ...
            tasks.spawn(async move {
                ...
                let request = acp::ExtRequest::new(
                    "ainxt.dev/git/status",
                    serde_json::value::to_raw_value(&params)
                        .expect("serialize git/status params")
                        .into(),
                );
                let result = match acp_send(request, &tx).await {
                    Ok(resp) => parse_git_status_reply(resp.0.get()),
                    Err(e) => Err(sanitize_user_error(&format!("git status failed: {e}"))),
                };
                TaskResult::GitStatusLoaded {
                    agent_id,
                    generation,
                    result,
                }
        ...
        Effect::GitDiff {
            ...
            let tx = acp_tx.clone();
            tasks.spawn(async move {
                // Working-tree vs HEAD for a single file, with full old/new
                // content so we can feed the standalone diff renderer.
                let params = serde_json::json!({
                    "sessionId": session_id.0.to_string(),
                    "from": "HEAD",
                    "to": "working",
                ...
                let request = acp::ExtRequest::new(
                    "ainxt.dev/git/diffs",
                    serde_json::value::to_raw_value(&params)
                        .expect("serialize git/diffs params")
                        .into(),
                );
                let result = match acp_send(request, &tx).await {
                    Ok(resp) => parse_git_diff_reply(resp.0.get(), &path),
                    Err(e) => Err(sanitize_user_error(&format!("git diff failed: {e}"))),
                };
                TaskResult::GitDiffLoaded {
                    agent_id,
                    generation,
                    path,
                    result,
        ...
        Effect::GitStage {
            ...
            tasks.spawn(async move {
                ...
                let request = acp::ExtRequest::new(
                    "ainxt.dev/git/stage",
                    serde_json::value::to_raw_value(&params)
                        .expect("serialize git/stage params")
                        .into(),
                );
                let result = match acp_send(request, &tx).await {
                    Ok(resp) => parse_git_ack_reply(resp.0.get()),
                    Err(e) => Err(sanitize_user_error(&format!("git stage failed: {e}"))),
                };
                TaskResult::GitStaged {
                    agent_id,
                    generation,
                    result,
                }
        ...
        Effect::GitUnstage {
            ...
            tasks.spawn(async move {
                ...
                let request = acp::ExtRequest::new(
                    "ainxt.dev/git/unstage",
                    serde_json::value::to_raw_value(&params)
                        .expect("serialize git/unstage params")
                        .into(),
                );
                let result = match acp_send(request, &tx).await {
                    Ok(resp) => parse_git_ack_reply(resp.0.get()),
                    Err(e) => Err(sanitize_user_error(&format!("git unstage failed: {e}"))),
                };
                TaskResult::GitStaged {
                    agent_id,
                    generation,
        ...
        Effect::GitCommit {
            ...
            tasks.spawn(async move {
                ...
                let request = acp::ExtRequest::new(
                    "ainxt.dev/git/commit",
                    serde_json::value::to_raw_value(&params)
                        .expect("serialize git/commit params")
                        .into(),
                );
                let result = match acp_send(request, &tx).await {
                    Ok(resp) => parse_git_commit_reply(resp.0.get()),
                    Err(e) => Err(sanitize_user_error(&format!("git commit failed: {e}"))),
                };
                TaskResult::GitCommitDone {
                    agent_id,
                    generation,
                    result,
                }
        ...
        Effect::SuggestCommitMessage {
            ...
            let tx = acp_tx.clone();
            tasks.spawn(async move {
                // The shell gathers the staged diff itself (HEAD → staged) when
                // no diff text is supplied, so we only pass the session id.
                let params = serde_json::json!({
                    "sessionId": session_id.0.to_string(),
                });
                let request = acp::ExtRequest::new(
                    "ainxt.dev/git/suggest_commit_message",
                    serde_json::value::to_raw_value(&params)
                        .expect("serialize git/suggest_commit_message params")
                        .into(),
                );
                let result = match acp_send(request, &tx).await {
                    Ok(resp) => parse_suggest_commit_reply(resp.0.get()),
                    Err(e) => Err(sanitize_user_error(&format!("couldn't suggest: {e}"))),
                };
                TaskResult::CommitMessageSuggested {
                    agent_id,
                    generation,
                    result,
                }
                            ...
                            Ok(Ok(resp)) => {
                                ...
                                if let Some(hits) = payload.get("results") {
                                    results = serde_json::from_value::<
                                        Vec<
                                            ainxt_shell::extensions::session_search::SearchSessionHit,
                                        >,
                                    >(hits.clone())
                                        .unwrap_or_default();
                                }
        ...
        Effect::HydrateSessionTitleFromDisk { agent_id, session_id, cwd } => {
            tasks
                .spawn(async move {
                    ...
                    let path = ainxt_shell::session::persistence::session_dir(&info)
                        .join("summary.json");
                    let title = tokio::task::spawn_blocking(move || -> Option<
                            (String, bool),
                        > {
                            let raw = std::fs::read_to_string(path).ok()?;
                            let summary: ainxt_shell::session::persistence::Summary = serde_json::from_str(
                                    &raw,
                                )
                                .ok()?;
        ...
        Effect::FetchBilling { agent_id, silent } => {
            ...
            tasks
                .spawn(async move {
                    use ainxt_shell::extensions::billing::BillingConfigResponse;
                    let req = acp::ExtRequest::new(
                        "ainxt.dev/billing",
                        serde_json::value::to_raw_value(&serde_json::json!({}))
            ...
            tasks
                .spawn(async move {
                    let settings = tokio::task::spawn_blocking(|| {
                            if !ainxt_shell::util::config::resolve_remote_fetch_enabled() {
                                return None;
                            }
                            let ainxt_home = ainxt_shell::util::ainxt_home::ainxt_home();
                            let store = ainxt_shell::auth::read_auth_json(
                                    &ainxt_home.join("auth.json"),
                                )
                                .ok()?;
                            let scope = ainxt_shell::auth::AinxtComConfig::default()
                                .auth_scope();
                            let auth = ainxt_shell::auth::lookup_auth(
                                &store,
                                &scope,
                            )?;
                            let proxy_base = std::env::var(
                                ...
                                .unwrap_or_else(|_| {
                                    ainxt_shell::agent::config::CLI_CHAT_PROXY_BASE_URL_DEFAULT
                                        .to_owned()
                                });
                            ainxt_shell::remote::fetch_settings_blocking(
                                &proxy_base,
                                &auth,
                                None,
                            )
...
/// Fetch session info from ACP via `ainxt.dev/session/info`.
async fn fetch_session_info(
    session_id: &acp::SessionId,
    tx: &AcpAgentTx,
) -> Result<SessionInfoResponse, String> {
    let request = acp::ExtRequest::new(
        "ainxt.dev/session/info",
        serde_json::value::to_raw_value(
...
/// Fetch session usage stats from ACP via `ainxt.dev/session/usage`.
async fn fetch_session_usage(
    session_id: &acp::SessionId,
    tx: &AcpAgentTx,
) -> Result<ainxt_shell::session::SessionUsageResponse, String> {
    let request = acp::ExtRequest::new(
        "ainxt.dev/session/usage",
        serde_json::value::to_raw_value(
                &serde_json::json!({ "sessionId" : session_id.0.to_string() }),
            )
    ...
    let resp = acp_send(request, tx)
        .await
        .map_err(|e| sanitize_user_error(&format!("couldn't fetch session usage: {e}")))?;
    let envelope: ExtMethodResult<ainxt_shell::session::SessionUsageResponse> =
        serde_json::from_str(resp.0.get()).map_err(|e| {
            tracing::debug!("typed session usage deser failed: {e}");
    ...
    envelope.result.ok_or_else(|| "session usage response missing result".to_string())
}

/// Format session usage stats into a human-readable string for `/stats`.
///
/// Model + token/cost breakdown from the session's usage ledger — see
/// [`crate::views::context_bar::fmt_session_cost`] for the cost formatting
/// convention shared with the status-bar cost indicator. Distinct from
/// `format_session_info`, which reports current context-window occupancy
/// rather than cumulative session token/cost totals.
///
/// A cost the gateway priced is shown plainly, because it reconciles with the
/// platform's billing. When the gateway did not price every call, the ledger
/// fills the gap from published rates and this view labels the figure
/// `estimated` — an approximation the user can act on is more useful than a
/// blank, as long as it never impersonates the real bill.
fn format_session_usage(usage: &ainxt_shell::session::SessionUsageResponse) -> String {
    let model = usage.data.model.as_deref().unwrap_or("unknown");
    let model_display = usage
        .data
        .model_display_name
        .as_deref()
        .unwrap_or(model);
    let totals = &usage.data.usage.totals;

    let incomplete_note = if usage.data.usage.usage_is_incomplete {
        " (partial — some usage may be missing)"
    } else {
        ""
    };

    // `approx_cost_usd_ticks` is present exactly when some calls reached the
    // ledger unpriced by the gateway and were filled from published rates. It
    // already covers the whole session (gateway figures included), so it is
    // preferred over the partial exact sum, which would understate the spend.
    let cost_line = match (totals.approx_cost_usd_ticks, totals.cost_usd_ticks) {
        (Some(ticks), _) => format!(
            "  Cost: ~{} (estimated — the gateway did not price every call)",
            crate::views::context_bar::fmt_session_cost(ticks)
        ),
        (None, Some(ticks)) => format!(
            "  Cost: {}",
            crate::views::context_bar::fmt_session_cost(ticks)
        ),
        (None, None) if crate::views::context_bar::is_in_house_model(model) => {
            "  Cost: free (in-house model)".to_string()
        }
        (None, None) => "  Cost: unavailable (no completed calls yet)".to_string(),
    };

    // `input_tokens` is the FULL prompt (fresh + cache reads + cache writes),
    // so printing it next to a cache column reads as if the cached tokens were
    // billed twice. Show the fresh remainder instead; the columns then add up
    // to the prompt total.
    let fresh_input = totals
        .input_tokens
    ...
    let mut lines = vec![
        ...
        format!(
            ...
            fresh_input, totals.output_tokens, totals.cached_read_tokens,
            totals.cache_write_tokens, totals.total_tokens,
        ),
    ];
    if totals.reasoning_tokens > 0 {
        lines.push(format!("  Reasoning tokens: {}", totals.reasoning_tokens));
    }
    lines.push(cost_line);
    lines.push(format!("  Model calls: {}  ·  Turns: {}", totals.model_calls, usage.data.usage.num_turns));

    if usage.data.usage.model_usage.len() > 1 {
        lines.push("  Per-model breakdown:".to_string());
        for (model_id, m) in &usage.data.usage.model_usage {
            // Mirrors the totals line: a gateway figure as-is, an estimate
            // marked with `~`, and only a genuine absence of data reported as
            // no cost at all.
            let per_model_cost = match (m.approx_cost_usd_ticks, m.cost_usd_ticks) {
                (Some(ticks), _) => {
                    format!("~{}", crate::views::context_bar::fmt_session_cost(ticks))
                }
                (None, Some(ticks)) => crate::views::context_bar::fmt_session_cost(ticks),
                (None, None) => "cost unavailable".to_string(),
            };
            // Cache columns appear only when the model actually used the
            // prompt cache, so rows for non-caching models stay readable.
            let mut cache_part = String::new();
            if m.cached_read_tokens > 0 {
                cache_part.push_str(&format!(" / {} cache-read", m.cached_read_tokens));
            }
            if m.cache_write_tokens > 0 {
                cache_part.push_str(&format!(" / {} cache-write", m.cache_write_tokens));
            }
            // Fresh input only: the cache columns are already part of the
            // model's prompt total, so showing the full figure here would
            // count them twice.
            let fresh = m
                .input_tokens
                .saturating_sub(m.cached_read_tokens)
                .saturating_sub(m.cache_write_tokens);
            lines.push(format!(
                "    {model_id}: {fresh} in / {} out{cache_part}, {} calls, {per_model_cost}",
                m.output_tokens, m.model_calls,
            ));
        }
    }

    format!("Session usage stats{incomplete_note}\n{}", lines.join("\n"))
}
...
async fn lookup_session_title(session_id: &acp::SessionId) -> Option<String> {
    let summaries = ainxt_shell::session::persistence::list_summaries(None)
        .await
        .ok()?;
    summaries
        .into_iter()
...
///
/// Matches the TUI's `render_session_info` for pager display.
fn format_session_info(
    ...
    let model_display = ainxt_shell::session::model_display_name(
    ...
    let model_hash_line = if ainxt_shell::session::should_show_model_fingerprint(
    ...
    let backend_line = info
        ...
        .map(|b| format!("\n  API Backend: {b}"))
        .unwrap_or_default();
    let sandbox_line = match ainxt_sandbox::profile_name() {
        Some(profile) => {
            let net = if ainxt_sandbox::should_restrict_child_network() {
                " (network: restricted)"
            } else {
                ""
            };
    ...
    let conversation_line = info
        ...
        .map(|id| format!("\n  Conversation ID: {id}"))
        .unwrap_or_default();
    let version_display = ainxt_version::display_version(
...
/// offsets index this block's `text`, which is displayed unchanged — this
...
/// Build the `ainxt.dev/interject` params. The optional structured `content`
