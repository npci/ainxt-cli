...
//!
//! Pager-side cache of what `ainxt-shell` reports from
//! `ainxt.dev/bundle/status`. The shell now performs the actual bundle download in
...
///
/// Populated from `ainxt.dev/bundle/status` ACP responses.
...
/// Deserialized response from `ainxt.dev/bundle/status`.
...
/// Deserialized response from `ainxt.dev/bundle/entry/get`.
#[derive(Debug, Deserialize, PartialEq)]
#[serde(rename_all = "camelCase")]
