...
use ainxt_workspace::foreign_sessions::{
...
impl ForeignPickerSource {
    ...
    fn skill_paths(self, ainxt_home: &Path) -> [PathBuf; 2] {
        ...
        [
            ainxt_home
                ...
                .join(skill)
                .join("SKILL.md"),
            ainxt_home.join("skills").join(skill).join("SKILL.md"),
        ]
    }
}
...
{
    let enabled = EnabledForeignSessionSources::default();

    // Third-party session sources (Claude / Cursor / Codex) are intentionally
    // hidden from the ainxt UI: the discovery/resume machinery is retained, but
    // no source is ever advertised in the session picker. Return the empty
    // (all-disabled) set unconditionally. Remove this early return to re-enable
    // third-party session discovery.
    if true {
        return enabled;
    }
    ...
    for source in ForeignPickerSource::ALL {
        ...
        for path in source.skill_paths(ainxt_home) {
...
pub(crate) async fn gated_sources_async(
    ...
    gated_sources_async_with(compat, ainxt_home, |path| async move {
...
{
    let enabled = gated_sources_async_with(compat, ainxt_home, metadata_exists).await;
...
pub(crate) fn scan_effect(
    ...
    (compat.claude || compat.codex || compat.cursor).then(|| Effect::ScanForeignSessions {
        cwd: cwd.to_path_buf(),
        compat,
        ainxt_home: ainxt_home.to_path_buf(),
        coordinator,
        seq,
    })
}
...
mod tests {
    ...
    use ainxt_workspace::foreign_sessions::ForeignSessionSource;
    ...
    async fn async_gate_missing_skill_prevents_store_work() {
        ...
        .await;
        // Third-party sources are hidden: the gate returns the empty set, so no
        // store work runs and no source is available to resume.
        assert!(result.is_none());
        let probed = probed.borrow();
        // The gate short-circuits before probing any skill paths.
        assert!(probed.is_empty());
        assert_eq!(store_calls.get(), 0);
    }

    #[tokio::test]
    async fn async_gate_hides_all_third_party_sources() {
        // Even when every third-party skill is present, the gate reports all
        // sources disabled so none appear in the ainxt session picker.
        let enabled = gated_sources_async_with(compat_all(), Path::new("/ainxt"), |path| {
