...
impl AgentView {
    ...
    /// reconnect cursor and both dedup highwaters (ACP + ainxt) when the id
    ...
    pub(crate) fn bind_session_id(&mut self, session_id: agent_client_protocol::SessionId) {
        if self.session.session_id.as_ref() != Some(&session_id) {
            ...
            self.last_applied_ainxt_event_seq = None;
        }
    ...
    pub fn new(session: AgentSession, scrollback: ScrollbackState) -> Self {
        ...
        let mut view = Self {
            ...
            last_applied_ainxt_event_seq: None,
            ...
            context_state: None,
            session_cost_ticks: None,
            session_cost_estimated: false,
            chat_kind: false,
            app_chat_mode: false,
            credit_balance: None,
            auto_topup: None,
            ...
            hit_changes: Default::default(),
            source_control_mode: Default::default(),
            ...
            source_control: None,
            last_source_control_area: Rect::default(),
            hit_source_control_close: Default::default(),
            toast: None,
    ...
    pub(crate) fn begin_session_reload(&mut self, generation: u64) {
        ...
        self.session_reload = Some(SessionReload {
            ...
            last_applied_ainxt_event_seq: self.last_applied_ainxt_event_seq,
            saw_replay: false,
    ...
    /// `session/load` response meta (`ainxt.dev/runningPromptId`): enter
    ...
    fn apply_reload_outcome(&mut self, reload: SessionReload, success: bool) -> bool {
        ...
        if success && reload.saw_replay {
            ...
            self.last_applied_ainxt_event_seq = reload.last_applied_ainxt_event_seq;
            dropped_heavy = true;
        }
    ...
    pub fn apply_full_context_info(&mut self, next: ainxt_shell::session::ContextInfo) {
        if self.chat_kind {
    ...
    pub fn apply_context_used(&mut self, used: u64, total: u64) {
        ...
        match self.context_state.as_mut() {
            Some(snap) => {
                ...
                snap.usage_pct = ainxt_token_estimation::usage_percentage_u8(used, snap.total);
                snap.free_tokens = ainxt_token_estimation::free_tokens(snap.total, used);
            }
            None => {
                self.context_state = Some(ainxt_shell::session::ContextInfo::from_notification(
                    used, total,
                ));
            }
        }
    }
    /// Store the shell-reported per-session cost (USD ticks, USD * 1e10) and
    /// whether any of it was estimated locally rather than priced by the
    /// gateway.
    ///
    /// Mirrors [`Self::apply_context_used`]'s chat-kind gate: gateway/chat
    /// sessions don't own local billing telemetry, so the cost segment stays
    /// hidden there. The value is trusted as-is — `Some(0)` renders as `free`
    /// (all in-house/free models), `None` hides the segment entirely.
    pub fn apply_session_cost(&mut self, cost_usd_ticks: i64, estimated: bool) {
        if self.chat_kind {
            self.session_cost_ticks = None;
            self.session_cost_estimated = false;
            return;
        }
        self.session_cost_ticks = Some(cost_usd_ticks);
        self.session_cost_estimated = estimated;
    }
