...
impl AgentView {
    ...
    pub(super) fn try_send_now_queued_from_prompt(&mut self) -> Option<InputOutcome> {
        ...
        // accepting the hint — matches the undo / image-input funnels so the
    ...
    pub(in crate::app) fn force_interject_queue_row(&mut self, id: u64) -> InputOutcome {
        ...
        if is_server {
            // Server row: the agent promotes it to run next (`ainxt.dev/queue/interject`); any kind may send now.
                ...
                && let Some(server_id) = row.server_id.clone()
            {
                ...
                // the intent; the confirming `ainxt.dev/queue/changed` broadcast
    ...
    /// `ainxt.dev/queue/changed` broadcast (pre-merge entries — the mirrored
    ...
    /// QUEUED — the caller fires `ainxt.dev/queue/interject` with that
    ...
    pub(crate) fn flush_pending_adoption_updates(&mut self, prompt_id: &str) {
        ...
        for (pid, update, mut meta) in std::mem::take(&mut self.pending_adoption_updates) {
            if pid == prompt_id {
                // Forward-only: the ainxt rail shares this cursor and may have
    ...
    pub(in crate::app) fn handle_queue_key(
        ...
        if let Some(event) = self.queue.handle_key(key, registry) {
            ...
            // Server-origin rows go to the agent as `ainxt.dev/queue/*`
    ...
    /// Reorder payload for `ainxt.dev/queue/reorder`. Omit only running; include
...
mod queue_edit_routing_tests {
    ...
    /// a server row remains (matches the delete path's visibility treatment).
    #[test]
    ...
    fn delete_last_then_requeue_auto_shows_pane() {
        let mut agent = running_agent_local_only();
        let registry = ActionRegistry::defaults();
        // Prime prev_len via sync (matches a rendered frame with one row).
        agent.queue.sync_from_merged(
            &agent.session.pending_prompts,
            &agent.shared_queue,
            agent.session.current_prompt_id.as_deref(),
    ...
    fn force_interject_local_expanded_row_keeps_it_queued() {
        let mut agent =
            running_agent_with_local_skill("/image a cat", "<expanded image instructions>");
        let registry = non_vscode_registry();
