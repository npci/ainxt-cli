...
impl McpInitProgress {
    ...
    ///   `ainxt.dev/mcp_initialized` clears the progress.
...
/// Which source the status-bar "changed files" indicator counts from.
///
/// Phase A only ever renders the [`SourceControlMode::Git`] working-tree
/// count; [`SourceControlMode::Session`] is reserved for Phase B, when the
/// source-control panel can attribute changes to the current agent session.
/// The `SourceControlToggleMode` action flips between them.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum SourceControlMode {
    /// Count every changed file in the git working tree (default).
    #[default]
    Git,
    /// Count only files changed by the current agent session (Phase B).
    Session,
}
...
pub(crate) struct SessionReload {
    ...
    /// Live dedup highwaters (ACP + ainxt) as of window open (same restore
    /// rationale).
    last_applied_event_seq: Option<u64>,
    last_applied_ainxt_event_seq: Option<u64>,
...
pub struct PluginCtaState {
    ...
    pub candidates: Vec<ainxt_hooks_plugins_types::MarketplacePluginEntry>,
    ...
    pub phase: CtaPhase,
    /// Generation counter for prompt-change debouncing (matches suggestions).
...
/// (`ainxt.dev/follow_ups`). Streaming-only: never persisted, does not survive a
...
pub struct AgentView {
    ...
    ///
    /// ACP stream only — the ainxt stream keeps its own highwater
    /// ([`Self::last_applied_ainxt_event_seq`]) because the two streams are not
    /// delivered in one id order: ACP lines ride the agent's FIFO event
    /// pipeline while ainxt lines are emitted direct-to-gateway, so a fresh ainxt
    ...
    /// ainxt-stream sibling of [`Self::last_applied_event_seq`] (see there for
    /// why the highwaters are split). Same drop rule, replay-exempt.
    pub last_applied_ainxt_event_seq: Option<u64>,
    /// Raw `eventId` of the most recent update APPLIED to this root session —
    /// replay or live, on both the ACP and ainxt paths; dropped updates (dedup,
    ...
    pub context_state: Option<ainxt_shell::session::ContextInfo>,
    /// Cached per-session cost in USD ticks (USD * 1e10) from the shell's
    /// session ledger. `None` until the shell reports any cost; `Some(0)` when
    /// every call was free (in-house models). The value always comes from the
    /// shell — the pager never derives cost from a model list.
    pub session_cost_ticks: Option<i64>,
    /// Whether [`Self::session_cost_ticks`] includes a locally estimated part
    /// (the gateway did not price every call), in which case the status bar
    /// renders it with a `~` so it is never read as a billable amount.
    pub session_cost_estimated: bool,
    ...
    pub chat_kind: bool,
    /// Process-wide `--chat` (matches `AppView::chat_mode`; set via
    ...
    /// (optimistic echo). The shell broadcasts `ainxt.dev/session/interjection` to
    ...
    /// Clickable "changed files" indicator in the status bar (source control).
    /// Click opens the source-control view (Phase B); Phase A shows a toast.
    pub hit_changes: HitArea,
    /// Which source the changed-files indicator counts from. Phase A always
    /// renders the Git working-tree count; the toggle exists so Phase B can
    /// swap in a session-scoped count without further plumbing.
    pub source_control_mode: SourceControlMode,
    ...
    /// Active source-control changed-files panel (Phase B). When `Some`,
    /// renders as a dismissible centered overlay and captures keyboard input
    /// (j/k to move, Enter/l to open a diff, h/Esc to go back / close,
    /// g to toggle mode). Read-only: staging/commit is Phase C.
    pub source_control: Option<crate::views::source_control_panel::SourceControlState>,
    /// Panel rect the source-control overlay last painted into (for mouse
    /// row hit-testing and diff-scroll clamping).
    pub(crate) last_source_control_area: Rect,
    /// Hit area for the source-control panel's close/hint affordance.
    pub(crate) hit_source_control_close: HitArea,
    ...
    pub(crate) deferred_session_mode: Option<ainxt_tools::types::SessionMode>,
    ...
    /// MCP servers, cleared when `ainxt.dev/mcp_initialized` arrives.
    ...
    /// on save/cancel. Matches `plan_approval_view.stashed_prompt` so
    ...
    /// Running agent definition for this session (`ainxt.dev/session/info` `agentName`).
    ...
    /// Whether the `/share` slash command is available (matches
    ...
    /// Armed when an `ainxt.dev/session/prompt_complete` broadcast arrives for the
    ...
    /// `ainxt.dev/queue/changed` broadcast has confirmed the row yet. Inserted by
    ...
    /// optimistic echo. Firing `ainxt.dev/queue/interject` then would race the
    ...
    /// Parked here and fired from the confirming `ainxt.dev/queue/changed`
    ...
    /// Follow-up suggestion chips for the latest assistant response
    /// (`ainxt.dev/follow_ups`). `None` when no chips are shown. Set by
    ...
    /// Stamped `ainxt.dev/follow_ups` that arrived for a turn that is NOT yet the
...
fn translate_local_submit(
    ...
    match kind {
        ...
        LocalQuestionKind::CreditLimitUpsell { choices } => {
            ...
            // Fall back to the RESOLVER, not the compile-time const: the const
            // ignores the documented AINXT_URL_USAGE override and is empty in
            // this build (audit risk R42).
            let payg_fallback = super::dispatch::upsell_url_payg();
            let url = q
                .and_then(|q| q.options.get(*idx))
                .and_then(|o| o.id.as_deref())
                .unwrap_or(payg_fallback.as_ref());
            let choice = choices
                .get(*idx)
                .copied()
                .unwrap_or(ainxt_telemetry::events::CreditLimitChoice::PayAsYouGo);
            ainxt_telemetry::session_ctx::log_event(
                ainxt_telemetry::events::CreditLimitUpsellClicked {
        ...
        LocalQuestionKind::FreeUsageUpsell { source } => {
            // Resolver, not const — see the note above.
            let upgrade_fallback = super::dispatch::upsell_url_upgrade();
            let url = qv
                .questions
                .first()
                .and_then(|q| q.options.get(*idx))
                .and_then(|o| o.id.as_deref())
                .unwrap_or(upgrade_fallback.as_ref());
            ainxt_telemetry::session_ctx::log_event(
                ainxt_telemetry::events::SuperAinxtUpsellClicked {
                    source,
                    auth_method: None,
                },
            );
...
fn translate_project_select(
    ...
    use ainxt_telemetry::events::{ProjectPickerOutcome, ProjectPickerSelected};
    let project_dir_options = resolved_paths.len().saturating_sub(1);
    let emit = |outcome: ProjectPickerOutcome| {
        ainxt_telemetry::session_ctx::log_event(ProjectPickerSelected {
            outcome,
...
pub(super) fn apply_model_editor_outcome(
    agent: &mut AgentView,
    outcome: crate::views::model_editor::ModelEditorKeyOutcome,
) -> InputOutcome {
    use crate::views::model_editor::ModelEditorKeyOutcome;
    match outcome {
        ModelEditorKeyOutcome::Close => {
            agent.active_modal = None;
            InputOutcome::Changed
        }
        ModelEditorKeyOutcome::Action(a) => InputOutcome::Action(a),
        ModelEditorKeyOutcome::Changed => InputOutcome::Changed,
        ModelEditorKeyOutcome::Unchanged => InputOutcome::Unchanged,
    }
}
...
fn remember_mode_enabled() -> bool {
    let path = ainxt_tools::util::ainxt_home::ainxt_home().join("config.toml");
...
fn question_visible_h(
    ...
    prompt_height: u16,
    question: &ainxt_tools::implementations::ainxt_build::ask_user_question::Question,
    content_w: usize,
    preview: Option<&str>,
    fullscreen: bool,
    desc_cap: u16,
...
pub(super) mod test_fixtures {
    ...
    /// FIX 4 (b): after adopting a NEW turn, a buffer-replayed `ainxt.dev/follow_ups`
    ...
    /// FINDING B (stamped path): a LATE FIRST-TIME (never-seen) `ainxt.dev/follow_ups`
    ...
    /// upstream by the `_meta["ainxt.dev/replayed"]` gate); it is pinned here so the
    ...
    /// FIX (buffer-before-adoption): a stamped `ainxt.dev/follow_ups` for a turn that
