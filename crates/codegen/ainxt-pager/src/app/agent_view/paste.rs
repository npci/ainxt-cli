                ...
                if let Err(msg) = self.prompt.insert_image(pasted) {
                    ...
                    if ctx.source.tip_showing() {
                        ainxt_telemetry::session_ctx::log_event(
                            ainxt_telemetry::events::ContextualTip {
...
pub(super) mod paste_key_tests {
    ...
    /// Matches the strongest Cmd+V drop-path test, but exercises the
    ...
    -> crate::views::question_view::QuestionViewState {
        let question =
            ainxt_tools::implementations::ainxt_build::ask_user_question::Question {
                question: "Pick one?".to_string(),
                options: vec![
                ainxt_tools::implementations::ainxt_build::ask_user_question::QuestionOption
    ...
    /// plan-nudge fire is armed (matches a real keypress without the
    ...
    fn agent_completion_inserts_unreadable_file_url_as_path_text() {
        ...
        assert_eq!(
            agent.prompt.text(),
            "/definitely/missing/ainxt-primary-paste.png "
        );
        assert!(agent.prompt.images.is_empty());
    }
    #[test]
