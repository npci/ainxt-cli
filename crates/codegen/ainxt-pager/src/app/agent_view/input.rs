...
impl AgentView {
    ...
    /// on an empty prompt would exit the overlay instead of reaching
    /// video (seek/close), or image (close).
    ...
    fn handle_input_inner(
        ...
        // ── Source-control panel (Phase B) ──
        // When open, the panel captures all input: j/k or arrows move the file
        // selection, Enter/l open the selected file's diff, h/Esc go back
        // (diff → list, else close), g toggles Git/Session mode. In the diff
        // view Up/Down/PgUp/PgDn scroll. This block takes precedence over the
        // btw/goal handlers below (mirrors the btw Esc guard).
        if self.source_control.is_some() {
            ...
            let sc_view = self
                ...
                .as_ref()
                .map(|s| s.view)
                .unwrap_or(PanelView::List);
            // ── Commit sub-view (Phase C) ──
            // The message editor is a full text field, so it must receive keys
            // *with* modifiers too. Ctrl+S commits, Esc cancels back to the
            // list; everything else is routed into the editor.
            if sc_view == PanelView::Commit {
                if let Event::Key(key) = ev
                    && key.kind != KeyEventKind::Release
                {
                    if key.code == KeyCode::Esc && key.modifiers.is_empty() {
                        return InputOutcome::Action(Action::SourceControlBack);
                    }
                    if key.code == KeyCode::Char('s')
                        && key.modifiers.contains(KeyModifiers::CONTROL)
                    {
                        return InputOutcome::Action(Action::SourceControlCommit);
                    }
                    // Route everything else to the editor.
                    return InputOutcome::Action(Action::SourceControlEditMessage(*key));
                }
                // Ignore mouse in the commit view.
                if let Event::Mouse(_) = ev {
                    return InputOutcome::Changed;
                }
            }
                ...
                && key.modifiers.is_empty()
            {
                ...
                match (view, key.code) {
                    ...
                    (PanelView::List, KeyCode::Down | KeyCode::Char('j')) => {
                        if let Some(sc) = self.source_control.as_mut() {
                            sc.move_selection(1);
                        }
                        return InputOutcome::Changed;
                    }
                    (PanelView::List, KeyCode::Up | KeyCode::Char('k')) => {
                        if let Some(sc) = self.source_control.as_mut() {
                            sc.move_selection(-1);
                        }
                        return InputOutcome::Changed;
                    ...
                    (PanelView::List, KeyCode::Enter | KeyCode::Char('l') | KeyCode::Right) => {
                        let idx = self
                            .source_control
                            .as_ref()
                            .map(|s| s.selected)
                        ...
                        if has_files {
                            return InputOutcome::Action(Action::SourceControlSelectFile(idx));
                        }
                        return InputOutcome::Changed;
                    }
                    // Stage / unstage the selected file (Phase C).
                    (PanelView::List, KeyCode::Char(' ')) => {
                        let has_files = self
                            .source_control
                            .as_ref()
                            .is_some_and(|s| !s.files.is_empty());
                        if has_files {
                            return InputOutcome::Action(Action::SourceControlToggleStage);
                        }
                        return InputOutcome::Changed;
                    }
                    // Open the commit view (Phase C).
                    (PanelView::List, KeyCode::Char('c')) => {
                        return InputOutcome::Action(Action::SourceControlOpenCommit);
                    }
                    // Diff view scrolling.
                    (PanelView::Diff, KeyCode::Down | KeyCode::Char('j')) => {
                        let max = crate::views::source_control_panel::diff_max_scroll(
                            self.source_control.as_ref().unwrap(),
                            self.last_source_control_area,
                        );
                        if let Some(sc) = self.source_control.as_mut() {
                            sc.diff_scroll_down(1, max);
                        }
                        return InputOutcome::Changed;
                    }
                    (PanelView::Diff, KeyCode::Up | KeyCode::Char('k')) => {
                        if let Some(sc) = self.source_control.as_mut() {
                            sc.diff_scroll_up(1);
                        }
                        return InputOutcome::Changed;
                    }
                    (PanelView::Diff, KeyCode::PageDown) => {
                        let page = self.last_source_control_area.height.saturating_sub(3).max(1)
                            as usize;
                        let max = crate::views::source_control_panel::diff_max_scroll(
                            self.source_control.as_ref().unwrap(),
                            self.last_source_control_area,
                        ...
                        if let Some(sc) = self.source_control.as_mut() {
                            sc.diff_scroll_down(page, max);
                        }
                        return InputOutcome::Changed;
                    }
                    (PanelView::Diff, KeyCode::PageUp) => {
                        let page = self.last_source_control_area.height.saturating_sub(3).max(1)
                            as usize;
                        if let Some(sc) = self.source_control.as_mut() {
                            sc.diff_scroll_up(page);
                        }
                        return InputOutcome::Changed;
                    }
                    // Toggle Git/Session mode.
                    (_, KeyCode::Char('g')) => {
                        return InputOutcome::Action(Action::SourceControlToggleMode);
                    }
                    // Back one level (diff → list) or close.
                    (_, KeyCode::Esc | KeyCode::Char('h') | KeyCode::Left) => {
                        return InputOutcome::Action(Action::SourceControlBack);
                    }
                    (_, KeyCode::Char('q')) => {
            ...
            // Mouse: click a list row to open its diff; click the close hint to
            // close; hover updates the close affordance.
            if let Event::Mouse(mouse) = ev {
                if matches!(mouse.kind, MouseEventKind::Down(MouseButton::Left)) {
                    if self
                        .hit_source_control_close
                        .contains(mouse.column, mouse.row)
                    {
                        return InputOutcome::Action(Action::SourceControlBack);
                    }
                    if let Some(sc) = self.source_control.as_ref()
                        && let Some(idx) = crate::views::source_control_panel::source_control_row_at(
                            sc,
                            self.last_source_control_area,
                            mouse.column,
                            mouse.row,
        ...
        if self.active_modal.is_some() {
            return match ev {
                ...
                Event::Paste(text) => {
                    if let Some(ActiveModal::ModelEditor { state }) = self.active_modal.as_mut() {
                        if crate::views::model_editor::handle_model_editor_paste(state, text) {
                            return InputOutcome::Changed;
                        }
                    }
                    InputOutcome::Changed
                ...
                Event::Mouse(mouse) => {
                    ...
                    match mouse.kind {
                        MouseEventKind::Moved => {
                            ...
                            changed |= self.hit_changes.update_hover(mouse.column, mouse.row);
            ...
            && registry.lookup(key, When::AgentScreen) == Some(ActionId::OpenExtensions)
        {
            return InputOutcome::Action(Action::OpenExtensionsModal {
                ...
                trigger: ainxt_telemetry::events::ExtensionsModalTrigger::KeyboardShortcut,
            });
        }
        if let Event::Key(key) = ev
            && key.kind != KeyEventKind::Release
