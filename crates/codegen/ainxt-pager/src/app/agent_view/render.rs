...
impl AgentView {
    ...
    pub fn draw(
        ...
        banner_height: u16,
        banner_announcements: &[ainxt_announcements::RemoteAnnouncement],
        hidden_announcement_ids: &std::collections::BTreeSet<String>,
        tip: Option<&str>,
        bundle_state: &crate::app::bundle::BundleState,
        in_dashboard_overlay: bool,
        link_spans_out: &mut Vec<ainxt_ratatui_inline::LinkSpan>,
        ...
        // Source-control "changed files" indicator, sourced from the same
        // cheap per-cwd git cache as the branch below (never a per-frame git
        // call). Shown only when the cwd is a git repo (count is `Some`);
        // `Some(0)` still renders `0` so a clean tree reads intentionally.
        //
        // Phase A always uses the Git working-tree count regardless of
        // `source_control_mode`. TODO(Phase B): when `mode == Session`, render
        // the session-attributed changed-file count with a distinct prefix.
        if let Some(changed) = crate::git_info::cwd_changed_files_lazy(&self.session.cwd) {
            let glyph = crate::git_info::branch_icon();
            let mut changes_style = Style::default().fg(theme.gray_dim).bg(theme.bg_base);
            if self.hit_changes.hovered {
                changes_style = changes_style.add_modifier(ratatui::style::Modifier::BOLD);
            }
            status.push(
                "changes",
                Line::from(Span::styled(format!("{glyph} {changed}"), changes_style)),
            );
        }
        let ctx_used = self.context_state.as_ref().map(|c| c.used);
        let model_window = self.session.models.get_context_window();
        // ainxt runs many models with different context windows. The active
        // model's window is authoritative for the indicator's total, so prefer
        // it — this makes `/model` switches reflect immediately instead of
        // lingering on the previous model's shell-reported total. Fall back to
        // the shell-reported total only when the model advertises no window.
        let shell_total = self
            .context_state
            .as_ref()
            .and_then(|c| (c.total > 0).then_some(c.total));
        let ctx_total = model_window.or(shell_total);
        ...
        // Session cost sits next to the context indicator. A gateway-reported
        // cost renders as-is; otherwise we use the gateway's model naming
        // (in-house `local:*` => `free`, else `cloud`) so the paid tier stays
        // visible and never mislabels a cloud model as free.
        let cost_model_id = self
            .session
            .models
            .current_model_name()
            .unwrap_or_default();
        if let Some(cost_line) = context_bar::session_cost_line(
            self.session_cost_ticks,
            self.session_cost_estimated,
            &cost_model_id,
            &theme,
            self.chat_kind,
        ) {
            status.push("cost", cost_line);
        }
        ...
        self.hit_changes.rect = areas.get("changes").copied();
        ...
        if let Some(profile) = ainxt_sandbox::profile_name() {
        ...
        // Drawn last, on top of the agent view (scrollback stays visible
        // behind it), like the goal-detail overlay. Records its rect for mouse
        // row hit-testing and diff scrolling, and pushes it as an occluder so
        // link hit targets underneath are suppressed.
        if self.source_control.is_some() {
            let overlay_rect =
                crate::views::source_control_panel::panel_area(area);
            let tick = self.scrollback.animation_tick();
            let focused = true;
            if let Some(sc) = self.source_control.as_mut() {
                let close_rect =
                    crate::views::source_control_panel::render_source_control_panel(
                        buf,
                        sc,
                        overlay_rect,
                        tick,
                ...
                self.hit_source_control_close.rect = close_rect;
            }
            self.last_source_control_area = overlay_rect;
            self.frame_occluder_rects.push(overlay_rect);
        } else {
            self.hit_source_control_close.clear();
            self.last_source_control_area = Rect::default();
        }
        self.pane_areas = layout.pane_areas();
        {
            ...
            if route.emit_osc8 {
                ...
                *link_spans_out = self
                    ...
                    .filter_map(|link| {
                        ...
                        .map(|url| ainxt_ratatui_inline::LinkSpan {
                            row: link.screen_row,
