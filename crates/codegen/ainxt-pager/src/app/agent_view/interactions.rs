...
impl AgentView {
    ...
    ///
    /// Two modes (matches question view pattern):
    ...
    fn submit_question_answers(&mut self, skipped: bool) -> InputOutcome {
        use ainxt_tools::implementations::ainxt_build::ask_user_question::AskUserQuestionExtResponse;
        self.swap_question_freeform();
        ...
        ainxt_telemetry::session_ctx::log_event(ainxt_telemetry::events::PlanSubmit {
    ...
    ///
    /// Matches the agent view's own Enter handling but sources the
...
mod permission_scope_key_tests {
    ...
    fn setup_bash_permission(agent: &mut AgentView) {
        ...
        perm.bash_highlights = Some(
            ainxt_workspace::permission::bash_command_splitting::BashCommandHighlights {
                prefix: vec![],
...
mod question_no_freeform_tests {
    ...
    //! the SuperAinxt upsell. Regression tests for the bug where clicking
    ...
    fn upsell_question() -> Question {
        ...
        Question {
            ...
            options: vec![
                opt("Upgrade to SuperAinxt", "For everyday coding"),
                opt("Upgrade to SuperAinxt Heavy", "Highest usage limits"),
            ],
