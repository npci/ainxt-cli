...
impl AgentView {
    ...
    pub(crate) fn log_prompt_suggestion_shown_if_visible(&mut self) {
        ...
        ainxt_telemetry::session_ctx::log_event(ainxt_telemetry::events::PromptSuggestion {
            action: ainxt_telemetry::events::PromptSuggestionAction::Shown,
            chars,
            words,
        });
    }
    ...
    /// Apply an `ainxt.dev/follow_ups` notification, keyed by `response_id`
    ...
    /// on each `ainxt.dev/follow_ups` notification (the same `promptId` it stamps on
    ...
    /// - A buffer-replayed `ainxt.dev/follow_ups` for a PRIOR turn's `response_id`
    ...
    /// Buffer a stamped `ainxt.dev/follow_ups` for a turn that is not yet current,
    ...
    /// Flush a buffered `ainxt.dev/follow_ups` for `prompt_id` (a turn that has just
    ...
    /// the running turn's `ainxt.dev/follow_ups` arrive on the ext channel DURING
    ...
    pub(in crate::app) fn connect_matched_plugin(&mut self) {
        ...
        ainxt_telemetry::session_ctx::log_event(
            ainxt_telemetry::events::PluginCtaConnectClicked {
        ...
        self.pending_effects
            .push(super::actions::Effect::InstallPluginFromCta {
                agent_id: self.session.id,
                session_id,
                source_url_or_path: ainxt_plugin_marketplace::official_source_git_url()
                    .into_owned(),
                plugin_relative_path,
            });
    }
...
mod plugin_cta_notify_tests {
    ...
    fn cta_entry(name: &str) -> ainxt_hooks_plugins_types::MarketplacePluginEntry {
