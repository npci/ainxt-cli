...
impl AgentView {
    ...
    fn log_extensions_modal_action(
        &self,
        action: &str,
        input_method: ainxt_telemetry::events::ExtensionsInputMethod,
    ) {
    ...
    fn log_extensions_modal_action_with(
        ...
        if let Some(ref state) = self.extensions_modal {
            ainxt_telemetry::session_ctx::log_event(
                ainxt_telemetry::events::ExtensionsModalAction {
    ...
    pub(super) fn handle_extensions_modal_key(
        ...
        // Ctrl+O opens ainxt.dev connectors on the MCP servers tab.
            ...
            })
        {
            self.log_extensions_modal_action(
                "open_connectors",
                ainxt_telemetry::events::ExtensionsInputMethod::Keyboard,
            );
            return self.execute_modal_button_action(
        ...
        // Route chrome keys through ModalWindow first (matches the mouse path).
    ...
    pub(super) fn handle_extensions_modal_mouse(
        ...
        if let Some(ch) = chrome_shortcut_ch {
            ...
            // (matches the keyboard guard in handle_extensions_modal_key).
            if self
                .extensions_modal
                .as_ref()
                .is_some_and(|s| s.pending_action.is_some())
                ...
                .and_then(|s| crate::views::extensions_modal::resolve_key(s.active_tab, ch))
            {
                self.log_extensions_modal_resolved_action(
                    ch,
                    &action,
                    ainxt_telemetry::events::ExtensionsInputMethod::Mouse,
                );
        ...
        // Open the connectors URL on mouse-down (equivalence with Ctrl+O). A section-row
    ...
    fn extensions_modal_click_opens_connectors(&self, entry_idx: usize, mouse_row: u16) -> bool {
        self.extensions_modal.as_ref().is_some_and(|state| {
            // Equivalence with the Ctrl+O guard: don't open while the search bar has focus.
    ...
    fn execute_modal_button_action(
        ...
        match action {
            ButtonAction::HooksAction(hooks_action) => {
                if let Some(ref mut state) = self.extensions_modal {
                    state.modal_message = None;
                    if matches!(hooks_action, ainxt_hooks_plugins_types::HooksAction::Reload) {
            ...
            ButtonAction::PluginsAction(plugins_action) => {
                if let Some(ref mut state) = self.extensions_modal {
                    ...
                    if matches!(
                        plugins_action,
                        ainxt_hooks_plugins_types::PluginsAction::Reload
                    ) {
                        // Reload rebuilds the entire plugin registry -- show
                        // tab-level "Loading..." instead of a single-entry badge.
                        state.pending_action = Some("Reloading...".into());
                        ...
                        let label = if matches!(
                            plugins_action,
                            ainxt_hooks_plugins_types::PluginsAction::Update { .. }
                        ) {
                            "Updating..."
                        } else {
                            "Processing..."
                        ...
                        && let Some(hook) = data.hooks.get(idx)
                    {
                        ...
                        return self.execute_modal_button_action(
                            crate::views::extensions_modal::ButtonAction::HooksAction(
                                ainxt_hooks_plugins_types::HooksAction::Remove { path },
                            ),
                        );
                    }
                }
            ...
            ButtonAction::ToggleSelectedHook => {
                    ...
                    && let Some(hook) = data.hooks.get(idx)
                {
                    ...
                    if is_collapsed {
                        ...
                        let group_hooks: Vec<&ainxt_hooks_plugins_types::HookInfo> = data
                            .hooks
                            .iter()
                        ...
                        let action = ainxt_hooks_plugins_types::HooksAction::ToggleSource {
                            hook_names,
                            disable: any_enabled,
                        };
                        ...
                        let action = if hook.disabled {
                            ainxt_hooks_plugins_types::HooksAction::Enable {
                                hook_name: hook.name.clone(),
                            }
                        } else {
                            ainxt_hooks_plugins_types::HooksAction::Disable {
                                hook_name: hook.name.clone(),
                            }
                        };
                        return self.execute_modal_button_action(ButtonAction::HooksAction(action));
            ...
            ButtonAction::ToggleSelectedPlugin => {
                    ...
                    && let Some(plugin) = data.plugins.get(idx)
                {
                    let action = if plugin.enabled {
                        ainxt_hooks_plugins_types::PluginsAction::Disable {
                            plugin_id: plugin.id.clone(),
                        }
                    } else {
                        ainxt_hooks_plugins_types::PluginsAction::Enable {
                            plugin_id: plugin.id.clone(),
                        }
                    };
            ...
            ButtonAction::UninstallSelectedPlugin => {
                if let Some(ref mut state) = self.extensions_modal
                    ...
                    && let Some(plugin) = data.plugins.get(idx)
                {
                    let action = ainxt_hooks_plugins_types::PluginsAction::Uninstall {
                        plugin_id: plugin.id.clone(),
                        confirmed: false,
                    };
                    return self.execute_modal_button_action(ButtonAction::PluginsAction(action));
            ...
            ButtonAction::UpdateSelectedPlugin => {
                ...
                if let Some(ref state) = self.extensions_modal
                    ...
                    && let Some(plugin) = data.plugins.get(idx)
                {
                    let action = ainxt_hooks_plugins_types::PluginsAction::Update {
            ...
            ButtonAction::InstallSelectedMarketplacePlugin => self
                .execute_selected_marketplace_plugin_action(
                    ...
                    |source_url_or_path, plugin_relative_path| {
                        ainxt_hooks_plugins_types::MarketplaceAction::Install {
                            source_url_or_path,
            ...
            ButtonAction::UpdateSelectedMarketplacePlugin => self
                .execute_selected_marketplace_plugin_action(
                    "Updating...",
                    |source_url_or_path, plugin_relative_path| {
                        ainxt_hooks_plugins_types::MarketplaceAction::Update {
                            source_url_or_path,
                            plugin_relative_path,
                        }
                    },
            ...
            ButtonAction::UninstallSelectedMarketplacePlugin => self
                .execute_selected_marketplace_plugin_action(
                    "Uninstalling...",
                    |source_url_or_path, plugin_relative_path| {
                        ainxt_hooks_plugins_types::MarketplaceAction::Uninstall {
                            source_url_or_path,
                            plugin_relative_path,
                        }
                    },
                        ...
                        if let Some(source) = source {
                            ...
                            state.modal_message = Some(
                                crate::views::extensions_modal::ModalMessage::MarketplaceConfirmation {
                                    message: msg,
                                    action:
                                        ainxt_hooks_plugins_types::MarketplaceAction::RemoveSource {
    ...
    fn execute_selected_marketplace_plugin_action(
        ...
        make_action: impl FnOnce(String, String) -> ainxt_hooks_plugins_types::MarketplaceAction,
    ) -> InputOutcome {
        if let Some(ref mut state) = self.extensions_modal {
            use crate::views::extensions_modal::TabDataState;
            if let TabDataState::Loaded(ref response) = state.marketplace_data
    ...
    fn update_selected_marketplace_plugin_dispatches_update_and_sets_pending_state() {
        ...
        modal.marketplace_data =
            TabDataState::Loaded(ainxt_hooks_plugins_types::MarketplaceListResponse {
                sources: vec![ainxt_hooks_plugins_types::MarketplaceScanResult {
...
mod extensions_action_target_tests {
    ...
    fn plugin_info(name: &str, enabled: bool) -> ainxt_hooks_plugins_types::PluginInfo {
    ...
    fn plugins_toggle_and_uninstall_resolve_name_and_state() {
        ...
        modal.plugins_data = TabDataState::Loaded(ainxt_hooks_plugins_types::PluginsListResponse {
    ...
    fn update_selected_plugin_dispatches_update_with_selected_id_and_pending_state() {
        ...
        // Telemetry target resolves to the selected plugin (equivalence with toggle/uninstall).
        let (target, enabled) =
        ...
        match outcome {
            crate::app::app_view::InputOutcome::Action(
                crate::app::actions::Action::ExecutePluginsAction(
                    ainxt_hooks_plugins_types::PluginsAction::Update { plugin_id },
                ),
    ...
    fn skills_toggle_resolves_name_and_resulting_state() {
        let skill = ainxt_tools::implementations::skills::types::SkillInfo {
            name: "my-skill".into(),
            enabled: false,
            ..Default::default()
        };
    ...
    fn hooks_modal(hooks: Vec<ainxt_hooks_plugins_types::HookInfo>) -> ExtensionsModalState {
        let mut modal = ExtensionsModalState::new(ExtensionsTab::Hooks);
        modal.hooks_data = TabDataState::Loaded(ainxt_hooks_plugins_types::HooksListResponse {
            hooks,
    ...
    fn loading_data_yields_no_target() {
        ...
        for action in [
            ...
            ButtonAction::ToggleSelectedMcpServer,
            ButtonAction::InstallSelectedMarketplacePlugin,
            ButtonAction::PluginsAction(ainxt_hooks_plugins_types::PluginsAction::Reload),
        ] {
            let (target, enabled) = AgentView::extensions_action_target(&modal, &action);
...
mod connectors_url_click_tests {
    ...
    /// These tests are about *clicking the connectors link*, so a link has to
    /// exist. `URL_CONNECTORS` ships empty in this build (audit risk R42), and
    /// `render_extensions_modal` now correctly records no link band and no
    /// underline in that case — so the affordance under test only exists once a
    /// deployment configures one. Supplied here through the documented override.
    const TEST_CONNECTORS_URL: &str = "https://connectors.example.test/manage";

    fn managed_server() -> McpServerInfo {
        McpServerInfo {
            name: "ainxt_com_linear".into(),
            display_name: None,
    ...
    #[test]
    #[serial_test::serial(AINXT_URL_CONNECTORS)]
    fn mouse_down_on_url_row_opens_connectors() {
        let _env =
            crate::test_util::EnvVarGuard::set("AINXT_URL_CONNECTORS", TEST_CONNECTORS_URL);
    ...
    fn mouse_down_on_url_row_while_searching_does_not_open() {
        let _env =
            crate::test_util::EnvVarGuard::set("AINXT_URL_CONNECTORS", TEST_CONNECTORS_URL);
        // Equivalence with the Ctrl+O guard: opening is suppressed while search is active.
        let mut agent = rendered_agent();
        let (col, band) = managed_url_hit(&agent);
        agent
            .extensions_modal
