    ...
    pub(in crate::app) fn handle_prompt_key(
            ...
            && self.prompt.prompt_suggestion_visible()
        {
            if key!(Tab).matches(key) || key!(Right).matches(key) {
                if self.prompt.accept_prompt_suggestion() {
                    ...
                    ainxt_telemetry::session_ctx::log_event(
                        ainxt_telemetry::events::PromptSuggestion {
                            action: ainxt_telemetry::events::PromptSuggestionAction::Accepted,
                            chars,
                            words,
                        },
                    );
                ...
                && self.prompt.text().is_empty()
            {
                ...
                ainxt_telemetry::session_ctx::log_event(
                    ainxt_telemetry::events::PromptSuggestion {
                        action: ainxt_telemetry::events::PromptSuggestionAction::Dismissed,
                        chars,
                        words,
                    },
                );
...
mod prompt_suggestion_key_tests {
    ...
    /// right after a turn ends with `ainxt.dev/suggestPrompt` resolved. Pins the
