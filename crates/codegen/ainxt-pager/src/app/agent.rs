...
use ainxt_shell::extensions::notification::GoalClassifierVerdict;
...
impl QueuedPrompt {
    ...
    /// when the payload was expanded client-side (`/image`, `/loop`):
...
pub enum AgentCommand {
    ...
    /// `ainxt.dev/session/fork` request is in flight.
    ForkSession,
}
impl AgentCommand {
...
impl GoalDisplayStatus {
    ...
    /// self-driving `Active` one. Matches the shell's
...
pub struct AgentSession {
    ...
    /// (`ainxt.dev/prompt_history` scoped via `filter_session_id`). Most-recent-first.
    /// Fetched on session create/load; prompts sent in this session are
    /// additionally front-inserted locally on send.
    pub prompt_history: Vec<String>,
    /// True until the session's startup/load `ainxt.dev/prompt_history` fetch completes.
...
pub struct ChipElement {
    ...
    pub kind: ainxt_ratatui_textarea::ElementKind,
...
mod tests {
    ...
    /// payloads (`/image`, `/loop`) that must run as their own turn.
    #[test]
    fn wire_matches_display_classifies_payload_shapes() {
        ...
        let expanded = QueuedPrompt {
            ...
            ..QueuedPrompt::plain(3, "/image cat", QueueEntryKind::Prompt)
        };
