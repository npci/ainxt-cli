...
/// consequence text is unchanged-pinned because the toast is the
...
/// Matches `yolo_toast(false)` precedent — restoring the safe
...
/// NOT the persisted canonical ("opt-in" / "opt-out"). Matches
...
/// sanitised error messages unchanged — the typical happy-path
...
fn dispatch_confirm_reset_setting_reset_dispatches_typed_setter_for_shared_enum() {
    ...
    // Reset → SetTheme("ainxtnight") (the registered default).
    assert_eq!(effects.len(), 1);
    match &effects[0] {
        Effect::PersistSetting { key, value, .. } => {
            assert_eq!(*key, "theme");
    ...
    assert_eq!(app.current_ui.theme.as_deref(), Some("ainxtnight"));
}
