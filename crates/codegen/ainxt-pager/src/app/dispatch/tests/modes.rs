...
///    `ainxt.dev/yolo_mode_changed` ACP notification back to the shell.
...
fn enable_always_approve_sends_response_and_flips_yolo_and_persists() {
    ...
    //     ~/.ainxt/config.toml AND fires ainxt.dev/yolo_mode_changed.
    let persist = effects
        .iter()
        .find_map(|e| match e {
...
/// guard MUST precede the `ainxt_telemetry::log_event` call
...
//
// Matches the `coding_data_sharing` and `yolo` test
...
/// State stays unchanged. Matches `set_coding_data_sharing_idempotent_opt_in`.
#[test]
...
/// toasts but emits NO Effect. Matches `set_coding_data_sharing_idempotent_opt_out`.
#[test]
...
/// produce `"✓ Plan mode: <on|off>"`. Matches `set_compact_mode_toast_format`.
