...
fn test_app() -> AppView {
    ...
    AppView {
        ...
        auth_methods: vec![acp::AuthMethod::Agent(acp::AuthMethodAgent::new(
            acp::AuthMethodId::new("ainxt.dev"),
            "ainxt".to_string(),
        ))],
...
/// overrides for equivalence with `dispatch_new_session_inner`.
...
fn cta_entry(name: &str, status: &str) -> ainxt_hooks_plugins_types::MarketplacePluginEntry {
...
fn cta_outcome(
    status: ainxt_hooks_plugins_types::OutcomeStatus,
    message: &str,
...
/// Plant a Build session under the process `ainxt_home()` (OnceLock-cached;
/// do not rely on setting `AINXT_HOME` mid-process). Caller must remove `sess_dir`.
fn plant_local_build_session(cwd: &std::path::Path, session_id: &str) -> std::path::PathBuf {
    ...
    let encoded = ainxt_shell::util::ainxt_home::encode_cwd_dirname(&cwd.to_string_lossy());
...
/// `ainxt.dev/ask_user_question` ext-method request. Returns the args
...
fn make_ask_user_question_args(
    ...
    let req = AskUserQuestionExtRequest {
        ...
        mode: ainxt_tools::implementations::ainxt_build::ask_user_question::AskUserQuestionMode::Default,
        questions: vec![
...
/// a "opt-reject-once" (RejectOnce, position 2). Matches the
        ...
        vec![
            acp::PermissionOption::new(
                acp::PermissionOptionId::new(Arc::from(
                    ainxt_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID,
                )),
...
const POLICY_WARNING: &str =
    ainxt_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
...
/// the in-memory theme state (single mutable global). Matches the
...
fn push_synthetic_permission(
    ...
    let request = ainxt_acp_lib::AcpArgs {
