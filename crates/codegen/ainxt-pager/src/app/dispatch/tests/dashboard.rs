...
fn voice_final_appends_to_dashboard_dispatch() {
    ...
    crate::voice::handle_voice_event(
        &mut app,
        ainxt_voice::VoiceEvent::UtteranceFinal {
            text: "the build".into(),
        },
    );
    assert_eq!(
...
fn voice_dashboard_dispatch_submit_tears_down_voice() {
    ...
    assert!(matches!(
        rx.try_recv(),
        Ok(ainxt_voice::VoiceCommand::PttRelease)
    ));
...
fn voice_target_bound_at_start_dispatch_vs_peek() {
    if !ainxt_voice::AUDIO_SUPPORTED {
        return;
    }
    use crate::views::dashboard::DashboardRowId;
    use crate::views::dashboard::peek::{PeekFields, PeekPanelState};
...
/// `ainxt dashboard` before login: the startup hook consumes the
/// `AINXT_OPEN_DASHBOARD_AT_STARTUP` env var and stashes
...
fn auth_complete_opens_deferred_dashboard() {
    ...
    assert!(
        ...
        "deferred `ainxt dashboard` must open the dashboard after login",
    );
    assert!(
...
fn dashboard_confirm_worktree_applies_pending_model_and_plan() {
    let mut app = test_app();
    seed_model(&mut app, "ainxt-4.5", "ainxt 4.5");
    open_dashboard(&mut app);
    app.cwd_has_git_ancestor = true;
    ...
    if let Some(d) = app.dashboard.as_mut() {
        d.pending_model = Some(crate::views::dashboard::PendingDispatchModel {
            id: model_id.clone(),
            effort: Some(ainxt_shell::sampling::types::ReasoningEffort::High),
            display: "ainxt 4.5".to_string(),
        });
    ...
    assert_eq!(
        ...
        Some((
            model_id,
            Some(ainxt_shell::sampling::types::ReasoningEffort::High)
        )),
        "effort must be stashed for the shell",
    );
    assert_eq!(
        agent.deferred_session_mode,
        Some(ainxt_tools::types::SessionMode::Plan),
    );
...
/// `ainxt dashboard`, and the startup hook.
#[serial_test::serial(AINXT_AGENT_DASHBOARD)]
#[test]
...
fn dashboard_slash_model_stages_pending_model() {
    ...
    seed_model(&mut app, "ainxt-4.5", "ainxt 4.5");
    open_dashboard(&mut app);
    let effects = dispatch_dashboard_dispatch_slash(&mut app, "/model ainxt-4.5".into());
    assert!(
        effects.is_empty(),
    ...
    assert_eq!(pending.id.0.as_ref(), "ainxt-4.5");
    assert_eq!(pending.display, "ainxt 4.5");
...
fn dashboard_slash_restricted_command_upsells_via_toast() {
    let _env = crate::test_util::EnvVarGuard::set(
        "AINXT_URL_SUBSCRIBE",
        "https://upsell.example.test/superainxt",
    );
    let mut app = test_app();
    app.tier_restricted_commands = vec!["image".to_string()];
    open_dashboard(&mut app);

    let effects = dispatch_dashboard_dispatch_slash(&mut app, "/image a sunset".into());
    ...
    // The restricted token this test registers is "image" (see above) and the
    // toast names the command the user typed, so it reads "/image". The
    // assertion required "/imagine" — a name neither the setup nor the product
    // uses on this path.
    assert!(
        toast.contains("/image") && toast.contains("SuperAinxt"),
        "toast must carry the upsell: {toast}"
    );
    assert!(
        toast.contains("upsell.example.test"),
        "toast must carry the configured upgrade URL, not a dangling \"upgrade at \": {toast}"
    );
}
...
/// toast unchanged in a neutral colour, so without the prefix an
...
fn dashboard_open_reseeds_pending_model_and_mode() {
    ...
    if let Some(d) = app.dashboard.as_mut() {
        d.pending_model = Some(crate::views::dashboard::PendingDispatchModel {
            id: acp::ModelId::new(std::sync::Arc::from("ainxt-4.5")),
            effort: None,
            display: "ainxt 4.5".to_string(),
        });
...
fn dashboard_attach_subagent_lazily_replays_deferred_transcript() {
    ...
    crate::app::subagent::set_replay_ainxt_home_for_tests(Some(home.path().to_path_buf()));
...
fn dashboard_overlay_stop_closes_session_and_returns_to_dashboard() {
    ...
    // Success feedback matches the externally-closed "Session
    // closed" toast.
    assert_eq!(
        app.dashboard.as_ref().unwrap().error_toast.as_deref(),
...
fn dashboard_overlay_cycle_non_overlay_noop_when_dashboard_disabled() {
    let _guard =
        crate::test_util::EnvVarGuard::set("AINXT_AGENT_DASHBOARD", std::path::Path::new("0"));
    let mut app = test_app_with_agent();
    let id1 = AgentId(0);
    mark_agent_nonempty(&mut app, id1);
    let id2 = AgentId(1);
...
/// `DashboardCloseShortcutsHelp` clears the modal. Matches the
...
/// Up-arrow on the button is a no-op (no wrap). Matches the
...
fn dashboard_peek_box_grows_for_multiline_reply() {
    ...
    let box_height_for = |reply_text: &str| -> u16 {
        ...
        // live (matches real usage: select row → peek opens → type).
        render(&mut app);
        app.dashboard
            .as_mut()
            .unwrap()
