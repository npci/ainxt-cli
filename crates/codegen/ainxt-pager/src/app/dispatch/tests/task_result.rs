...
) -> ainxt_workspace::foreign_sessions::RecentForeignSession {
    ainxt_workspace::foreign_sessions::RecentForeignSession {
        tool,
...
fn foreign_resume_results_require_launch_token_and_canonical_cwd() {
    ...
    dispatch(
        Action::TaskComplete(TaskResult::ForeignResumeHintDetected {
            ...
            hint: Some(foreign_resume_hint(
                ainxt_workspace::foreign_sessions::ForeignSessionTool::Cursor,
            )),
        }),
        &mut launch,
    );
    assert_eq!(
        ...
        Some(ainxt_workspace::foreign_sessions::ForeignSessionTool::Cursor)
    );
    ...
    dispatch(
        Action::TaskComplete(TaskResult::ForeignResumeHintDetected {
            ...
            hint: Some(foreign_resume_hint(
                ainxt_workspace::foreign_sessions::ForeignSessionTool::Codex,
            )),
        }),
        &mut stale,
    );
...
fn marketplace_list_loaded_sanitizes_components_at_ingestion() {
    ...
    entry.components = Some(ainxt_hooks_plugins_types::PluginComponents {
        skills: vec![ainxt_hooks_plugins_types::ComponentItem {
...
fn plugins_action_success_sets_result_notice_and_autoreload_preserves_it() {
    ...
    dispatch(
        Action::TaskComplete(TaskResult::PluginsActionResult {
            agent_id: id,
            result: Ok(ainxt_hooks_plugins_types::ActionOutcome {
...
fn uninstall_result_notice_is_footer_only_not_row_anchored() {
    ...
    let id = AgentId(0);
    {
        ...
        modal.last_plugins_action = Some(ainxt_hooks_plugins_types::PluginsAction::Uninstall {
...
/// Regression (Bugbot): a failed `ainxt.dev/subagent/cancel` RPC must NOT
...
fn switch_to_non_reasoning_model_clears_persisted_effort() {
    ...
    app.agents
        ...
        .insert(
            model_id.clone(),
            acp::ModelInfo::new(model_id.clone(), "ainxt".to_string()),
        );
    app.agents
        .get_mut(&id)
        .unwrap()
...
fn switch_model_incompatible_agent_shows_question_modal() {
    ...
    let err = ainxt_shell::agent::config::ModelSwitchIncompatibleAgentError {
        code: "MODEL_SWITCH_INCOMPATIBLE_AGENT".into(),
        active_agent_type: "ainxt-build".into(),
...
fn same_agent_type_switch_no_modal() {
    ...
    let mut app = test_app_with_agent();
    let id = AgentId(0);
    let model_a = acp::ModelId::new(std::sync::Arc::from("ainxt-build-a"));
    let model_b = acp::ModelId::new(std::sync::Arc::from("ainxt-build-b"));

    // Add both models to the catalog (no agentType → both use ainxt-build).
    let agent = app.agents.get_mut(&id).unwrap();
    agent.session.models.available.insert(
        model_a.clone(),
        acp::ModelInfo::new(model_a.clone(), "ainxt A".to_string()),
    );
    agent.session.models.set_current(model_a, None);
    agent.session.models.available.insert(
        model_b.clone(),
        acp::ModelInfo::new(model_b.clone(), "ainxt B".to_string()),
    );
    agent.session.model_switch_pending = true;
...
fn delete_both_session_clears_modal_and_welcome_content_hits() {
    ...
    let hit = ainxt_shell::extensions::session_search::SearchSessionHit {
...
fn gate_refreshed_emits_check_subscription_on_gate_lift() {
    ...
    app.gate = Some(ainxt_shell::auth::GateInfo {
        message: "SuperAinxt subscription required".into(),
    ...
    let settings = ainxt_shell::util::config::RemoteSettings::default();
...
fn gate_refreshed_no_effect_when_still_gated() {
    ...
    let settings = ainxt_shell::util::config::RemoteSettings {
...
fn test_gate() -> ainxt_shell::auth::GateInfo {
    ainxt_shell::auth::GateInfo {
        message: "Subscribe".into(),
        url: None,
        label: None,
    }
...
fn verify_check_with_meta_resolves_pending_gate() {
    ...
    let meta = serde_json::to_value(ainxt_shell::auth::AuthMeta::default()).unwrap();
    dispatch_task_result(
...
fn verify_check_with_gated_meta_shows_gate() {
    ...
    let meta = serde_json::to_value(ainxt_shell::auth::AuthMeta {
        gate: Some(test_gate()),
        ..Default::default()
    })
    .unwrap();
