...
/// build auto-trusts and persists nothing). Matches this module's raw env idiom.
...
fn voice_on_welcome_creates_session_and_records() {
    ...
    assert!(matches!(
        rx.try_recv(),
        Ok(ainxt_voice::VoiceCommand::PttPress)
    ));
}
#[test]
...
fn worktree_session_created_sets_session_and_cwd() {
    ...
    let id = AgentId(0);
    let worktree_path = PathBuf::from("/tmp/ainxt-worktrees/pager-123");
...
fn worktree_session_preserves_subdirectory_offset() {
    let mut app = test_app();
    app.cwd = PathBuf::from("/repo/crates/codegen/ainxt-pager");
    ...
    let id = AgentId(0);
    let worktree_root = PathBuf::from("/home/user/.ainxt/worktrees/repo/pager-123");
    let session_cwd = worktree_root.join("crates/codegen/ainxt-pager");
    let effects = dispatch(
        Action::TaskComplete(TaskResult::WorktreeSessionCreated {
            agent_id: id,
...
fn worktree_session_created_drains_queued_prompts() {
    ...
    assert_eq!(app.agents[&id].session.queue_len(), 1);
    let worktree_path = PathBuf::from("/tmp/ainxt-worktrees/pager-abc");
    let effects = dispatch(
        Action::TaskComplete(TaskResult::WorktreeSessionCreated {
            agent_id: id,
            session_id: acp::SessionId::new("wt-drain-1"),
            worktree_path,
            session_cwd: PathBuf::from("/tmp/ainxt-worktrees/pager-abc"),
            models: None,
        }),
        &mut app,
    );
...
/// trust. AINXT_HOME-isolated so the write hits a temp store, not the real one.
...
fn bg_task_killed_no_op_for_unknown_session() {
    ...
    let effects = dispatch(
        Action::TaskComplete(TaskResult::BgTaskKilled {
            ...
            outcome: Some(ainxt_tools::types::KillOutcome::AlreadyExited),
        }),
        &mut app,
    );
    assert!(effects.is_empty());
