...
/// Turn end with prompt suggestions enabled fires the `ainxt.dev/suggestPrompt`
...
fn turn_end_fetches_prompt_suggestion_when_enabled() {
    ...
    // No `ainxt-build-0.1` in the test catalog and no env override →
    // `None` on the wire; the shell then uses its own `ainxt-build-0.1`
...
/// coming — no suggestion fetch (matches the local `pending_prompts` guard).
#[test]
...
/// `ainxt.dev/queue/changed` rebroadcast — not client-side prediction — updates
...
/// The always-on pipeline end to end, with `AINXT_SUGGESTIONS` semantics
