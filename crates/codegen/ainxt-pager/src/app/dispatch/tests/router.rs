...
fn critical_announcement(id: &str) -> ainxt_announcements::RemoteAnnouncement {
...
fn promo_announcement(id: &str) -> ainxt_announcements::RemoteAnnouncement {
    ainxt_announcements::RemoteAnnouncement {
        id: Some(id.into()),
        message: Some(format!("{id} message")),
        severity: Some("promo".into()),
        cta: Some(ainxt_announcements::AnnouncementCta {
            label: Some("Go".into()),
            url: Some(format!("https://ainxt.example.test/{id}")),
            caption: None,
        }),
        ..Default::default()
    }
...
/// the promo url (observed via the `AINXT_TEST_OPEN_URL_FILE` seam, from every
...
fn announcements_open_cta_opens_promo_and_noops_under_critical() {
    ...
    // `tempfile::tempdir()` (a private, auto-cleaned-up directory) instead
    // of a hand-built, PID-suffixed path directly under the shared temp
    // directory.
    let tmp = tempfile::tempdir().unwrap();
    let url_file = tmp.path().join("ainxt-cta-open.txt");
    ...
    for surface in [
        ...
        assert!(
            opened().lines().any(|l| l == "https://ainxt.example.test/promo-open"),
...
fn acp_command_colliding_with_builtin_skipped_in_autocomplete() {
    ...
    // The invariant is that the BUILTIN wins over a colliding ACP command --
    // assert that, not the builtin's exact help text. This required
    // "Quit the application"; the builtin's copy has since been reworded to
    // "Close ainxt", so the test failed while the behaviour it names was fine.
    assert_ne!(
        exit_cmd.description(),
        "ACP exit (should be skipped)",
        "the ACP command must not shadow the builtin /exit"
    );
    // A non-colliding ACP command is still registered.
...
fn tick_propagates_available_commands_to_bootstrap() {
    ...
    let skill_meta = serde_json::json!(
        { "scope" : "user", "path" : "/home/user/.ainxt/skills/pick-best/SKILL.md", }
    );
