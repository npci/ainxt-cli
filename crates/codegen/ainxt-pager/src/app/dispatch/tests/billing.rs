...
use super::*;
// Runtime URL resolvers — assertions compare against these, not the
// compile-time `UPSELL_URL_*` consts, so they stay correct whether or not a
// deployment has configured `AINXT_URL_SUBSCRIBE` / `AINXT_URL_USAGE`.
use crate::app::dispatch::billing::{upsell_url_payg, upsell_url_upgrade};

/// Reserved-domain stand-ins for a deployment that has configured its billing
/// pages. `URL_SUBSCRIBE` / `URL_USAGE` ship EMPTY in this build (audit risk
/// R42), so any test that needs a real URL must supply one through the
/// documented `AINXT_URL_*` override rather than assume a compiled-in value.
const TEST_URL_SUBSCRIBE: &str = "https://billing.example.test/superainxt";
const TEST_URL_USAGE: &str = "https://billing.example.test/credits?_s=usage";
...
fn is_max_tier_positive_match() {
    assert!(is_max_tier(Some("superainxt_heavy")));
    assert!(is_max_tier(Some("SuperAinxt Heavy")));
    assert!(is_max_tier(Some("SUPERAINXT_HEAVY")));
}

#[test]
fn is_max_tier_non_max_and_unknown() {
    assert!(!is_max_tier(Some("superainxt")));
...
fn is_max_tier_rejects_partial_matches() {
    assert!(!is_max_tier(Some("superainxt_heav")));
    assert!(!is_max_tier(Some("superainxt_heavy_plus")));
...
fn upsell_non_max_shows_qa_with_two_options() {
    ...
    assert_eq!(q.options[0].id.as_deref(), Some(upsell_url_upgrade().as_ref()));
    assert_eq!(q.options[1].label, "Pay as you go");
    assert_eq!(q.options[1].id.as_deref(), Some(upsell_url_payg().as_ref()));
}
...
#[test]
#[serial_test::serial(AINXT_URL_SUBSCRIBE)]
fn upsell_non_max_upgrade_url_is_superainxt() {
    let _env = crate::test_util::EnvVarGuard::set("AINXT_URL_SUBSCRIBE", TEST_URL_SUBSCRIBE);
    let mut app = test_app_with_agent();
    open_upsell_qa(
        &mut app,
        CreditLimitUpsellMode::LegacyPayg { enabled: false },
    ...
    let url = agent_qv(&app).questions[0].options[0]
        ...
        .as_deref()
        .unwrap();
    // The Q&A upsell option carries the configured subscribe URL verbatim.
    assert_eq!(url, TEST_URL_SUBSCRIBE, "got: {url}");

    // This assertion used to also require `referrer=ainxt-build` here. It does
    // not hold on this path and never can: funnel attribution is appended by
    // `dispatch_open_subscribe_url` (billing.rs, via `ensure_query_param`), which
    // is the welcome-screen button, not the credit-limit Q&A modal. The option id
    // built at `billing.rs:247` uses the bare resolver. That asymmetry means Q&A
    // upsell clicks are not attributed — see CLI-038; it is a product decision,
    // not something a test should silently assert into existence.
    // The referrer behaviour is covered on its real path by
    // `open_subscribe_url_appends_funnel_referrer` below.
}

#[test]
#[serial_test::serial(AINXT_URL_USAGE)]
fn upsell_non_max_payg_url_is_usage() {
    let _env = crate::test_util::EnvVarGuard::set("AINXT_URL_USAGE", TEST_URL_USAGE);
    let mut app = test_app_with_agent();
    open_upsell_qa(
        &mut app,
        CreditLimitUpsellMode::LegacyPayg { enabled: false },
...
fn is_credit_limit_error_matches_legacy_403_and_pool_402() {
    ...
    assert!(is_credit_limit_error(
        None,
        "API error (status 402 Payment Required): ainxt usage balance exhausted"
    ));
    assert!(is_credit_limit_error(
        None,
        "status 403: run out of credits"
...
fn upsell_max_tier_pushes_scrollback_card_payg_off() {
    ...
    assert_eq!(blk.url, upsell_url_payg().as_ref());
}
...
fn upsell_max_tier_scrollback_card_url_is_payg() {
    ...
    assert_eq!(
        last_credit_limit_block(&app, before).url,
        upsell_url_payg().as_ref()
    );
}
...
fn billing_fetched_updates_subscription_tier() {
    let mut app = test_app_with_agent();
    dispatch_billing(&mut app, None, true, Some("superainxt_heavy".into()));
    assert_eq!(app.subscription_tier.as_deref(), Some("superainxt_heavy"));
}
    ...
    assert!(matches!(
        ...
        Some(
            crate::views::question_view::LocalQuestionKind::FreeUsageUpsell {
                source: ainxt_telemetry::events::SuperAinxtUpsell::FreeUsagePaywall,
            }
        )
    ));
    let q = &qv.questions[0];
    assert_eq!(q.question, "You hit your free usage limit.");
    // Bind the Cow first: `upsell_url_upgrade().as_ref()` inside the array would
    // borrow a temporary that is dropped at the end of the `let` statement.
    let upgrade_url = upsell_url_upgrade();
    let expected = [
        (
            "Upgrade to SuperAinxt",
            "For everyday coding and productivity tasks",
            Some(upgrade_url.as_ref()),
        ),
        (
            "Upgrade to SuperAinxt Heavy",
            "Get the most out of ainxt. Highest usage limits.",
            Some(upgrade_url.as_ref()),
        ),
    ];
...
fn free_usage_failure_opens_paywall_modal() {
    ...
    use ainxt_shell::extensions::notification::{RetryState, SessionUpdate};
    ...
    for idx in [0, 1] {
        ...
        match translate_local_submit_for_test(&qv, kind(), false) {
            InputOutcome::Action(Action::OpenUrl(url)) => {
                assert_eq!(url, upsell_url_upgrade().as_ref())
            }
...
/// Submitting a tier-restricted command opens the two-option SuperAinxt
...
fn restricted_command_submit_opens_two_option_upsell() {
    ...
    app.agents
        .get_mut(&id)
        .unwrap()
        .set_restricted_commands(&["image".to_string()]);

    let effects = dispatch(Action::SendPrompt("/image a sunset".into()), &mut app);
    ...
    assert!(matches!(
        ...
        Some(
            crate::views::question_view::LocalQuestionKind::FreeUsageUpsell {
                source: ainxt_telemetry::events::SuperAinxtUpsell::RestrictedCommand,
            }
        )
    ));
    let q = &qv.questions[0];
    assert_eq!(q.question, "Unlock all features with SuperAinxt.");
    assert_eq!(q.options.len(), 2);
    assert_eq!(q.options[0].label, "Upgrade to SuperAinxt");
    assert_eq!(q.options[0].id.as_deref(), Some(upsell_url_upgrade().as_ref()));
    assert_eq!(q.options[1].label, "Upgrade to SuperAinxt Heavy");
    assert_eq!(q.options[1].id.as_deref(), Some(upsell_url_upgrade().as_ref()));
}
...
fn restricted_command_with_open_modal_keeps_composer_text() {
    ...
    {
        let agent = app.agents.get_mut(&id).unwrap();
        agent.set_restricted_commands(&["image".to_string()]);
        ...
        agent.prompt.set_text("/image a sunset");
    }
...
/// `AINXT_TEST_OPEN_URL_FILE` seam), `Action::OpenUrl` for a billing CTA
...
#[serial_test::serial(AINXT_TEST_OPEN_URL_FILE)]
#[test]
fn open_url_shows_manual_url_when_browser_unavailable() {
    ...
    let bad = std::env::temp_dir().join(format!(
        "ainxt-open-url-missing-{}/out.txt",
        std::process::id()
    ));
    // SAFETY: serialized via `serial_test` so no other test races the env var.
    unsafe { std::env::set_var("AINXT_TEST_OPEN_URL_FILE", &bad) };

    let mut app = test_app_with_agent();
    let before = agent_scrollback_len(&app);
    // Subject under test is the browser-unavailable fallback, not which URL.
    // Was `UPSELL_URL_UPGRADE`, a `const` equal to the now-empty
    // `ainxt_env::URL_SUBSCRIBE`; dispatching an empty URL opens nothing and
    // pushes no message, so this asserted on a message that never existed.
    let url = TEST_URL_SUBSCRIBE;
    ...
    unsafe { std::env::remove_var("AINXT_TEST_OPEN_URL_FILE") };
}
...
fn open_url_does_not_show_fallback_when_opener_succeeds() {
    let url_file =
        std::env::temp_dir().join(format!("ainxt-open-url-ok-{}.txt", std::process::id()));
    let _ = std::fs::remove_file(&url_file);
    // SAFETY: serialized via `serial_test`.
    unsafe { std::env::set_var("AINXT_TEST_OPEN_URL_FILE", &url_file) };

    let mut app = test_app_with_agent();
    let before = agent_scrollback_len(&app);
    // As above: the subject is the opener seam, not which URL. Was
    // `UPSELL_URL_PAYG` (== the empty `ainxt_env::URL_USAGE`).
    let url = TEST_URL_USAGE;
...
fn credit_limit_upsell_submit_shows_url_when_browser_unavailable() {
    ...
    let bad = std::env::temp_dir().join(format!(
        "ainxt-open-url-upsell-missing-{}/out.txt",
        std::process::id()
    ));
    // SAFETY: serialized via `serial_test`.
    unsafe { std::env::set_var("AINXT_TEST_OPEN_URL_FILE", &bad) };

    // The "Buy more credits" option resolves through `ainxt_env::url_usage()`,
    // which is empty in this build; with no URL there is nothing to open and
    // nothing lands in scrollback, so the assertion below had no message to read.
    let _env = crate::test_util::EnvVarGuard::set("AINXT_URL_USAGE", TEST_URL_USAGE);
    ...
    assert_eq!(url, upsell_url_payg().as_ref());
...
/// Funnel attribution: `dispatch_open_subscribe_url` (the welcome-screen upgrade
/// button) must append `referrer=ainxt-build` to whatever subscribe URL is
/// configured. This is the only code path that adds it — see the note in
/// `upsell_non_max_upgrade_url_is_superainxt`.
#[serial_test::serial(AINXT_TEST_OPEN_URL_FILE)]
#[test]
fn open_subscribe_url_appends_funnel_referrer() {
    let url_file = std::env::temp_dir().join(format!(
        "ainxt-open-url-referrer-{}.txt",
        std::process::id()
    ));
    let _ = std::fs::remove_file(&url_file);
    // SAFETY: serialized via `serial_test`.
    ...
    let mut app = test_app_with_agent();
    let _ = crate::app::dispatch::billing::dispatch_open_subscribe_url(&mut app);

    let recorded = std::fs::read_to_string(&url_file).unwrap_or_default();
    assert!(
        recorded.contains("referrer=ainxt-build"),
        "funnel referrer missing from opened URL; got {recorded:?}"
    );
    assert!(
        recorded.contains("billing.example.test"),
        "configured subscribe host missing; got {recorded:?}"
    );
