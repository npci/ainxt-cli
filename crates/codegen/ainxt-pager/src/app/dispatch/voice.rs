...
/// Show the SuperAinxt upsell when a tier-restricted (free / X Basic) user tries
/// to start voice via the Ctrl+Space / F8 keybinding, which bypasses the slash
/// registry (`/voice` is instead hidden + upsold via the deny list). Matches the
    ...
    match app.active_view {
        ...
        ActiveView::AgentDashboard => {
            if let Some(d) = app.dashboard.as_mut() {
                // Resolver, not const — see dashboard.rs.
                d.set_error_toast(&format!(
                    "/voice requires SuperAinxt — upgrade at {}",
...
/// (free / X Basic), it shows the SuperAinxt upsell instead of starting a session
...
pub(super) fn dispatch_enable_voice_mode(app: &mut AppView, from_hold: bool) -> Vec<Effect> {
    ...
    if !app.voice_mode_enabled || !ainxt_voice::AUDIO_SUPPORTED {
        return vec![];
    }
    ...
    // SuperAinxt upsell instead of starting a doomed session (`/voice` itself is
...
pub(super) fn dispatch_voice_toggle(app: &mut AppView) -> Vec<Effect> {
    ...
    // Not recording: start. Matches `/voice` so the banner surfaces with one
