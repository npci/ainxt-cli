...
use super::billing::dispatch_open_subscribe_url;
use super::ctx::{
...
use super::model_editor::{
    dispatch_delete_model_override, dispatch_open_model_editor, dispatch_save_model_override,
};
use super::modes::{
...
use super::settings::setters::{
    ...
    set_display_refresh_auto_cadence, set_fork_secondary_model, set_gateway_url, set_group_tool_verbs,
...
use super::status::{
    dispatch_copy_session_id, dispatch_share_session,
    dispatch_show_context_info, dispatch_show_privacy_info, dispatch_show_queue,
    dispatch_show_release_notes, dispatch_show_session_info, dispatch_show_session_usage,
    dispatch_show_tasks, dispatch_show_usage, set_coding_data_sharing,
};
use super::source_control::{
    dispatch_open_source_control, dispatch_source_control_back,
    dispatch_source_control_commit, dispatch_source_control_edit_message,
    dispatch_source_control_open_commit, dispatch_source_control_select_file,
    dispatch_source_control_toggle_mode, dispatch_source_control_toggle_stage,
};
    ...
    let effects = match action {
        Action::Quit | Action::QuitConfirmed => {
            if let Some(tx) = &app.voice_cmd_tx {
                let _ = tx.try_send(ainxt_voice::VoiceCommand::Shutdown);
            }
        ...
        Action::OpenBlockViewer => {
            ...
            let mut credit_card: Option<(String, ainxt_telemetry::events::CreditLimitChoice)> =
                None;
            with_scrollback(app, |s| {
            ...
            if let Some((url, choice)) = credit_card {
                log_event(ainxt_telemetry::events::CreditLimitUpsellClicked {
        ...
        Action::OpenModelEditor => dispatch_open_model_editor(app),
        Action::SaveModelOverride {
            original_key,
            model_key,
            fields,
        } => dispatch_save_model_override(app, original_key, model_key, fields),
        Action::DeleteModelOverride { model_key } => dispatch_delete_model_override(app, model_key),
        ...
        Action::AnnouncementsOpenCta(surface) => {
            if let Some((promo, url)) = crate::views::announcements::promo_cta_target(
                ...
                log_event(ainxt_telemetry::events::AnnouncementCtaClicked {
                    id: promo_id,
                    source: surface,
                });
                open_url_or_show(app, &url);
        ...
        Action::ShowSessionUsage => dispatch_show_session_usage(app),
        ...
        Action::OpenSourceControl => dispatch_open_source_control(app),
        Action::SourceControlToggleMode => dispatch_source_control_toggle_mode(app),
        Action::SourceControlSelectFile(idx) => dispatch_source_control_select_file(app, idx),
        Action::SourceControlBack => dispatch_source_control_back(app),
        Action::SourceControlToggleStage => dispatch_source_control_toggle_stage(app),
        Action::SourceControlOpenCommit => dispatch_source_control_open_commit(app),
        Action::SourceControlCommit => dispatch_source_control_commit(app),
        Action::SourceControlEditMessage(key) => dispatch_source_control_edit_message(app, key),
        ...
        Action::SetGatewayUrl(v) => set_gateway_url(app, v),
        ...
        Action::OpenSubscribeUrl => dispatch_open_subscribe_url(app),
        Action::OpenUrl(url) => {
...
pub(super) fn dispatch_action_result(
    ...
    use ainxt_hooks_plugins_types::OutcomeStatus;
            ...
            OutcomeStatus::Success => {
                    ...
                    && modal.result_notice.is_none()
                {
                    let entry_index = match modal.last_plugins_action {
                        Some(ainxt_hooks_plugins_types::PluginsAction::Uninstall { .. }) => None,
            ...
            OutcomeStatus::ConfirmationRequired => {
                if let Some(ref mut modal) = agent.extensions_modal {
                    let confirmed_action = modal.last_plugins_action.as_ref().map(|a| {
                        ...
                        if let ainxt_hooks_plugins_types::PluginsAction::Uninstall {
                            ref mut confirmed,
                            ..
                        } = action
                        {
