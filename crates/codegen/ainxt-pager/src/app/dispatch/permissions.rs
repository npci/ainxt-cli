...
///
/// Special case for [`ainxt_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID`]:
...
/// "always-approve"` to `~/.ainxt/config.toml`, and fire the
/// `ainxt.dev/yolo_mode_changed` ACP notification. See the option-id constant
...
pub(super) fn dispatch_permission_select(
    ...
    let enable_always_approve =
        option_id.0.as_ref() == ainxt_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID;
    ...
    let steers_next_cursor = !enable_always_approve
        && option_id.0.as_ref() != ainxt_workspace::permission::ALLOW_EDITS_SESSION_OPTION_ID;
