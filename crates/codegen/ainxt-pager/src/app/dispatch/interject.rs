...
//! `ainxt.dev/interject` effect, and prompt-history recording. Split out of
//! `dispatch.rs` unchanged (pure code motion).
...
/// prompt, and fires the `ainxt.dev/interject` ext method carrying a client-minted
/// id.
///
/// The shell broadcasts `ainxt.dev/session/interjection` to every attached pane so
...
pub(super) fn dispatch_interject(
    ...
    // record its id so the broadcast echo (`ainxt.dev/session/interjection`) is
...
pub(super) fn dispatch_send_prompt_now(
    ...
    // Mid-outage guard (matches the plain prompt path): the producers already
    ...
    crate::unified_log::info(
        "prompt.send_now",
        Some(&crate::session_log_tag(&sid_str)),
