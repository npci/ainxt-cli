...
///
/// Does **not** invent `ainxt.dev` when no interactive method is advertised
...
/// disappears once the user returns to the session. Matches the
...
pub(super) fn dispatch_cancel_login(app: &mut AppView) -> Vec<Effect> {
    ...
    // silently resubmit it. Matches the strip in the `AuthComplete` path.
...
pub(super) fn handle_auth_complete(
        ...
        && *current_seq == request_seq
    {
        if let Some(meta_val) = meta.as_ref()
            && let Ok(auth_meta) =
                serde_json::from_value::<ainxt_shell::auth::AuthMeta>(meta_val.clone())
        {
        ...
        if let Some(return_view) = app.auth_return_view.take() {
            ...
            // a clean session. Matches the credit-limit upsell's
            // stale-block strip.
        ...
        // Fetch changelog (matches startup path for interactive login).
