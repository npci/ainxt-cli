...
pub(in crate::app::dispatch) fn set_voice_stt_language(
    ...
    refresh_open_settings_modals(app);
    let effective = ainxt_voice::language_for_api(canonical);
    tracing::info!(
        target: "settings",
    ...
    let toast = if canonical == ainxt_voice::STT_LANGUAGE_AUTO {
        format!("\u{2713} Voice language: System ({effective})")
    } else {
        let name = ainxt_voice::stt_language_by_code(canonical).map_or(canonical, |l| l.name);
...
/// `AINXT_PROMPT_SUGGESTIONS` env var overrides the effective value.
...
pub(super) fn set_contextual_hint_inner(
    app: &mut AppView,
    write: fn(&mut ainxt_shell::agent::config::ContextualHints, Option<bool>),
    new: bool,
) {
    write(&mut app.current_ui.contextual_hints, Some(new));
    let resolved = ainxt_shell::util::config::resolve_contextual_hints(
...
// ── gateway_url (commit path) ───────────────────────────────────────

/// State + persist for the ainxt gateway base URL.
///
/// The gateway URL is frozen at process startup (`EndpointsConfig` reads
/// `[endpoints]` once at launch), so there is no live hot-swap here — we
/// only persist and toast that the change applies on restart. The value
/// PERSISTED is the raw base URL the user typed (trailing `/` stripped);
/// the config writer derives the `/ainxt/v1/api` suffix.
pub(in crate::app::dispatch) fn set_gateway_url(app: &mut AppView, new: String) -> Vec<Effect> {
    let normalized = new.trim_end_matches('/').to_string();
    tracing::info!(
        target: "settings",
        key = "gateway_url",
        value = %normalized,
    ...
    app.show_toast(&format!(
        "Gateway set to {normalized}. Restart ainxt to apply."
    ));
    vec![Effect::PersistSetting {
        key: "gateway_url",
        value: crate::settings::SettingValue::String(normalized),
        // No previously-tracked in-memory value to roll back to (the live
        // endpoint is frozen at startup); an empty String is the neutral
        // rollback.
...
pub(in crate::app::dispatch) fn set_auto_dark_theme(app: &mut AppView, new: String) -> Vec<Effect> {
    let prev_canonical: &'static str = app
        ...
        .and_then(crate::theme::canonical_name)
        .filter(|s| *s != "auto")
        // No prior config: fall back to AinxtNight (the default).
        .unwrap_or_else(|| crate::theme::ThemeKind::AinxtNight.display_name());
    let new_canonical = match crate::theme::canonical_name(&new) {
...
pub(in crate::app::dispatch) fn set_auto_light_theme(
    ...
    let prev_canonical: &'static str = app
        ...
        .and_then(crate::theme::canonical_name)
        .filter(|s| *s != "auto")
        .unwrap_or_else(|| crate::theme::ThemeKind::AinxtDay.display_name());
    let new_canonical = match crate::theme::canonical_name(&new) {
        Some(c) if c != crate::theme::ThemeKind::Auto.display_name() => c,
        _ => {
            tracing::error!(
...
pub(in crate::app::dispatch) fn set_default_model_inner(
    ...
    // the next `ainxt.dev/models/update` roundtrip.
...
pub(in crate::app::dispatch) fn set_default_model(
    ...
    // so persisting the human-readable name (e.g. "ainxt")
    // would silently fail to resolve on the next startup.
    //
    // Chat (`--chat` / AINXT_CHAT_MODE) catalogs use opaque `/rest/modes`
...
/// Outer dispatcher for `Action::ClearForkSecondaryModel`. Resets the persisted
/// override to the built-in baseline — the empty-string "no opinion" sentinel
/// (there is no bundled default model; the catalog is gateway-only).
