...
/// Open the command palette (the `/help` slash command path). Matches the
...
pub(in crate::app::dispatch) fn dispatch_open_settings(app: &mut AppView) -> Vec<Effect> {
    ...
    // hot. Matches the shortcuts-cheatsheet precedent at
...
pub(in crate::app::dispatch) fn apply_setting_rollback(
    ...
    match (key, rollback_value) {
        ...
        // fork_secondary_model: the baseline IS the empty-string sentinel
        // (no bundled default model), so the rollback value is used unchanged.
        ("fork_secondary_model", SettingValue::String(s)) => {
