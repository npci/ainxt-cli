...
/// Start (if needed) and submit the initial prompt from `ainxt "<prompt>"`.
///
...
pub(super) fn dispatch_show_undo_tip(app: &mut AppView) -> Vec<Effect> {
    ...
    // Emit the impression only when the tip actually took the slot (matches
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(ainxt_telemetry::events::ContextualTip {
            tip: ainxt_telemetry::events::ContextualTipKind::Undo,
...
pub(in crate::app) fn show_small_screen_tip(app: &mut AppView) {
    ...
    // Impression only when the tip actually takes the slot (matches undo/plan).
    if agent.show_ephemeral_tip(
        crate::tips::small_screen::small_screen_tip(),
        &mut app.tip_seen_counts,
    ) {
        log_event(ainxt_telemetry::events::ContextualTip {
            tip: ainxt_telemetry::events::ContextualTipKind::SmallScreen,
            action: ainxt_telemetry::events::ContextualTipAction::Shown,
        });
    }
}
...
pub(super) fn dispatch_show_plan_nudge(app: &mut AppView) -> Vec<Effect> {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(ainxt_telemetry::events::ContextualTip {
            tip: ainxt_telemetry::events::ContextualTipKind::PlanMode,
            action: ainxt_telemetry::events::ContextualTipAction::Shown,
        });
    }
    vec![]
...
pub(super) fn dispatch_show_word_select_tip(app: &mut AppView) -> Vec<Effect> {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(ainxt_telemetry::events::ContextualTip {
            tip: ainxt_telemetry::events::ContextualTipKind::WordSelect,
            action: ainxt_telemetry::events::ContextualTipAction::Shown,
        });
    }
...
pub(super) fn dispatch_accept_word_select_tip(app: &mut AppView) -> Vec<Effect> {
    ...
    log_event(ainxt_telemetry::events::ContextualTip {
        tip: ainxt_telemetry::events::ContextualTipKind::WordSelect,
        action: ainxt_telemetry::events::ContextualTipAction::Accepted,
    });
...
fn maybe_show_send_now_tip(app: &mut AppView) {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(ainxt_telemetry::events::ContextualTip {
            tip: ainxt_telemetry::events::ContextualTipKind::SendNow,
            action: ainxt_telemetry::events::ContextualTipAction::Shown,
        });
    }
}
...
pub(super) fn dispatch_send_prompt_inner(
    ...
    // Restricted commands (`/usage`, `/image`, …) are hidden from the
            ...
            .is_restricted(invocation.token)
    {
        ...
        // another question modal already up, `open_subscribe_upsell` would
    ...
    if !literal && trimmed.starts_with('/') {
        ...
        // `ainxt.dev/queue/changed`. We render an optimistic echo into the shared
        ...
        // exactly idle experience. Image/skill/editing/non-running cases
    ...
    if let Some(agent) = app.agents.get_mut(&agent_id) {
            ...
            && agent.session.current_prompt_id.as_deref() != Some(response_pid)
        {
                ...
                && crate::app::acp_handler::is_server_initiated_prompt(response_pid)
            {
                ...
                // later `ainxt.dev/queue/changed` broadcast can't re-pin a
        ...
        // from the scrollback (matches reauth), not a session flag.
