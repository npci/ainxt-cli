...
pub(super) fn dispatch_open_dashboard(app: &mut AppView) -> Vec<Effect> {
    ...
    // stay put (matches the auth gate above) so the question is resolved first.
...
pub(super) fn dispatch_dashboard_attach(
    ...
    // (matches the subagent fullscreen takeover). Input goes
...
/// chrome and return to the dashboard view. Matches the popup
...
///   work in flight.
///
/// Matches `dispatch_dashboard_stop`'s second press, except the user
...
pub(super) fn dispatch_dashboard_overlay_stop(app: &mut AppView) -> Vec<Effect> {
        ...
        && let Some(d) = app.dashboard.as_mut()
    {
        ...
        // Success feedback — the close path itself is silent. Matches the
...
/// Matches `dispatch_dashboard_dispatch`'s new-session arm with `attach=true`,
...
/// + `Always` contexts. Matches `ActionId::ShortcutsHelp`'s agent-view handler.
...
pub(super) fn dispatch_dashboard_dispatch(
    ...
    if attach {
        ...
        // Matches `dispatch_dashboard_attach` and
...
///     stored unchanged (they carry their own glyph).
...
pub(super) fn dispatch_dashboard_dispatch_slash(app: &mut AppView, text: String) -> Vec<Effect> {
    ...
    let result = {
        ...
        {
            use ainxt_telemetry::events::{PagerCommandSource, PagerSlashCommand};
        ...
        if reg.is_restricted(invocation.token) {
            ...
            if let Some(d) = app.dashboard.as_mut() {
                d.dispatch.set_text("");
                // Resolver, not const: the const ignores AINXT_URL_SUBSCRIBE and
                // is empty here, which rendered "… — upgrade at " with a dangling
                // preposition and no link.
                d.set_error_toast(&format!(
                    "/{token} requires SuperAinxt — upgrade at {}",
    ...
    match result {
        ...
        CommandResult::Error(msg) => {
            if let Some(d) = app.dashboard.as_mut() {
                ...
                // own — route through `set_error_toast` so the unchanged
                // badge shows the `✗` error marker. `Message` results
                // below stay unchanged: they carry their own glyph
                // (e.g. `✓ Theme: …`).
...
fn stage_dashboard_model(
    ...
    effort: Option<ainxt_shell::sampling::types::ReasoningEffort>,
) {
    let display = app
        .models
        .available
...
pub(super) fn apply_pending_dispatch_config(
    ...
    match pending_mode {
        ...
        DashboardDispatchMode::Plan => {
            agent.session.yolo_mode = false;
            agent.deferred_session_mode = Some(ainxt_tools::types::SessionMode::Plan);
...
/// it matches what the user sees: the next visible row below `closed`
...
pub(super) fn dispatch_dashboard_permission_select(
        ...
        .filter(|_| option_id.0.as_ref() == "allow-always-mcp")
    {
        let selection = match scope.selected {
            crate::views::permission_view::McpScope::Tool => {
                ainxt_workspace::permission::McpScopeSelection::Tool {
                    tool_name: scope.tool_name.clone(),
                }
            }
            crate::views::permission_view::McpScope::Server => match &scope.server_prefix {
                Some(prefix) => ainxt_workspace::permission::McpScopeSelection::Server {
                    server: prefix.clone(),
                },
                None => ainxt_workspace::permission::McpScopeSelection::Tool {
                    tool_name: scope.tool_name.clone(),
                },
            },
        };
        ...
        && perm.bash_selection_count > 0
    {
        ...
        serde_json::to_value(ainxt_workspace::permission::BashCommandSelectedTerms {
            command_parts: parts,
        })
        .ok()
        .and_then(|v| v.as_object().cloned())
...
///
/// Matches [`dispatch_permission_followup`](super::permissions::dispatch_permission_followup) (resolve the front request
