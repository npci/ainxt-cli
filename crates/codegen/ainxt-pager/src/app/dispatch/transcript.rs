...
pub(crate) fn dispatch_open_transcript_pager(app: &mut AppView) {
    ...
    let path = std::env::temp_dir().join(format!("ainxt-transcript-{}.md", uuid::Uuid::new_v4()));
...
pub(super) fn dispatch_dump_input_log(app: &mut AppView) -> Vec<Effect> {
    ...
    let context_tag = agent.session.session_id.as_ref().map(|s| s.0.to_string());
    ...
    let logs_dir = ainxt_home.join("logs");
...
pub(super) fn handle_skills_toggle_done(
    ...
    // The toggle effect already called ainxt.dev/skills/refresh-baseline
