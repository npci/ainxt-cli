...
/// Open the local worktree question modal for `/new`. Matches
...
/// model that requires a different agent harness. Matches
...
/// the workspace (writes `~/.ainxt/trusted_folders.toml`), mark trust resolved,
...
pub(in crate::app::dispatch) fn dispatch_trust_folder(app: &mut AppView) -> Vec<Effect> {
    if let TrustState::Pending { workspace } = &app.trust_state {
        ainxt_workspace::folder_trust::grant_folder_trust(workspace);
    }
    finish_trust(app)
}
...
/// (`--resume` / `--worktree` / initial-prompt / `ainxt dashboard`). Extracted
