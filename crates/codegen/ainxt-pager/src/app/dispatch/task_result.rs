    ...
    match result {
        ...
        TaskResult::ForeignResumeCwdCanonicalized {
                ...
                && let Some(canonical_cwd) = accepted_cwd
            {
                vec![Effect::DetectForeignResumeHint {
                    ...
                    ainxt_home: ainxt_tools::util::ainxt_home::ainxt_home(),
                    launch_token,
                }]
            } else {
                vec![]
        ...
        TaskResult::ChangelogFetched { markdown, entries } => {
            ...
            app.changelog_bullets =
                ainxt_shell::util::changelog::bullets_from_entries(&entries, 3);
            vec![]
        }
        ...
        TaskResult::GitStatusLoaded {
            ...
            generation,
            result,
        } => super::source_control::handle_git_status_loaded(app, agent_id, generation, result),
        TaskResult::GitDiffLoaded {
            agent_id,
            generation,
            path,
            result,
        } => super::source_control::handle_git_diff_loaded(app, agent_id, generation, path, result),
        TaskResult::GitStaged {
            agent_id,
            generation,
            result,
        } => super::source_control::handle_git_staged(app, agent_id, generation, result),
        TaskResult::GitCommitDone {
            agent_id,
            generation,
            result,
        } => super::source_control::handle_git_commit_done(app, agent_id, generation, result),
        TaskResult::CommitMessageSuggested {
            agent_id,
            generation,
            result,
        } => super::source_control::handle_commit_message_suggested(
            app, agent_id, generation, result,
        ),
        ...
        TaskResult::ModelOverrideSaved { agent_id, model_key } => {
            if let Some(agent) = app.agents.get_mut(&agent_id)
                && let Some(crate::views::modal::ActiveModal::ModelEditor { state }) =
                    &mut agent.active_modal
            {
                state.handle_save_success(&model_key);
            }
            app.show_toast(&format!(
                "Saved model '{model_key}' — if you set an env var name, make sure it's \
                 actually exported in your shell, then reopen /model to select it"
            ));
            vec![]
        }
        TaskResult::ModelOverrideSaveFailed { agent_id, error } => {
            let scrubbed = scrub_error_for_toast(&error);
            if let Some(agent) = app.agents.get_mut(&agent_id)
                && let Some(crate::views::modal::ActiveModal::ModelEditor { state }) =
                    &mut agent.active_modal
            {
                state.handle_save_failure(scrubbed);
            }
            vec![]
        }
        TaskResult::ModelOverrideDeleted { agent_id, model_key } => {
            if let Some(agent) = app.agents.get_mut(&agent_id)
                && let Some(crate::views::modal::ActiveModal::ModelEditor { state }) =
                    &mut agent.active_modal
            {
                state.handle_delete_success(&model_key);
            }
            app.show_toast(&format!("Removed model '{model_key}'"));
            vec![]
        }
        TaskResult::ModelOverrideDeleteFailed { agent_id, error } => {
            let scrubbed = scrub_error_for_toast(&error);
            if let Some(agent) = app.agents.get_mut(&agent_id)
                && let Some(crate::views::modal::ActiveModal::ModelEditor { state }) =
                    &mut agent.active_modal
            {
                state.handle_delete_failure(scrubbed);
            }
            vec![]
        }
    }
