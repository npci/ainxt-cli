...
/// Show session info: fetch via ainxt.dev/session/info and display in scrollback.
///
...
/// Show session usage stats (`/stats`): active model, token breakdown, and
/// cost via `ainxt.dev/session/usage`.
///
/// Produces Effect::ShowSessionUsage which spawns an async ACP ext request.
/// On completion, TaskResult::SessionUsageComplete shows the formatted stats.
/// Distinct from `dispatch_show_usage` (`/usage`), which fetches the
/// platform's billing/credit-limit summary instead.
pub(super) fn dispatch_show_session_usage(app: &mut AppView) -> Vec<Effect> {
    let ActiveView::Agent(id) = app.active_view else {
        return vec![];
    };
    let Some(agent) = app.agents.get_mut(&id) else {
...
pub(super) fn dispatch_show_privacy_info(app: &mut AppView) -> Vec<Effect> {
    ...
    if app.is_zdr {
        ...
        lines.push("  Usage and code data may be used by ainxt to improve the product.");
        lines.push("");
        lines.push("  Use /privacy opt-out to enable privacy mode.");
    }

    lines.push("");
    // `lines` is a `Vec<&str>`, so the formatted string has to outlive the push — inlining
    // `&format!(…)` borrows a temporary that is dropped at the end of the statement (E0716).
    // Omit the pointer entirely when this build has no legal page, rather than
    // rendering a dangling "  Learn more: ".
    let legal = ainxt_env::url_legal();
    let learn_more = format!("  Learn more: {legal}");
    if ainxt_env::url_is_set(&legal) {
        lines.push(&learn_more);
    }
    let text = lines.join("\n");
...
/// (persists via ACP ext-request, NOT `~/.ainxt/config.toml`).
...
/// Show context info: fetch via ainxt.dev/session/info and display rich breakdown.
///
...
///
/// When the remote settings `ainxt_build_usage_redirect_url` flag is set (delivered via
