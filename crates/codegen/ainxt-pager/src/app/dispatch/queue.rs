...
//! action arm. Split out of `dispatch.rs` unchanged (pure code motion).
...
/// immediately, before the confirming `ainxt.dev/queue/changed` broadcast.
...
/// that bridges the round-trip before the confirming `ainxt.dev/queue/changed`
...
///
/// Delegates to the shared implementation in `ainxt_tools::reminders`.
...
fn format_cron_prompt(prompt: &str, task_id: &str, human_schedule: &str) -> String {
    ainxt_tools::reminders::format_scheduled_task_prompt(prompt, task_id, human_schedule)
}
...
///
/// Matches the matching arm of [`maybe_drain_queue`] EXCEPT it does NOT mint a
...
pub(crate) fn apply_turn_start_shim(
    ...
    // seen ring, so a buffer-replayed `ainxt.dev/follow_ups` for an older response
...
mod tests {
    ...
    /// `ainxt.dev/follow_ups` for a PRIOR turn's response stays rejected (its
