...
fn effective_cancel_subagents_preference(
    ...
    ui: &ainxt_shell::agent::config::UiConfig,
) -> Option<bool> {
...
pub(super) fn do_cancel_turn(app: &mut AppView, cancel_subagents: bool) -> Vec<Effect> {
    ...
    // FRONT queued prompt as the next turn — its `ainxt.dev/queue/changed`
...
/// Grace window between a driver-side `ainxt.dev/session/prompt_complete`
...
/// Finish turns whose end was announced by `ainxt.dev/session/prompt_complete`
...
pub(crate) fn reconcile_overdue_turn_ends(app: &mut AppView) -> Option<Vec<Effect>> {
    ...
    for id in overdue {
        ...
        // FIFO handoff (matches the PromptResponse arm): adopt the next
...
// TODO: Add dispatch_cancel_command() once ainxt-shell supports proper
