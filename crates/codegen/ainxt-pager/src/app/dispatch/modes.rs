...
/// from `~/.ainxt/sessions/<urlencoded_cwd>/<session_id>/plan.md`.
...
pub(super) fn set_plan_mode(
    ...
    // active. Matches `dispatch_cycle_mode`'s `in_plan` read so
    ...
    let mode_id = acp::SessionModeId::new(if new {
        ainxt_tools::types::SessionMode::Plan.as_id()
    } else {
...
/// the gate is on, the current UI mode is Auto, and yolo is not winning. Matches
...
pub(super) fn set_yolo_mode_inner(app: &mut AppView, new: bool) {
    ...
    // Global matches update unconditionally (even if the user navigated
    ...
    if previous_state != new {
        ainxt_telemetry::session_ctx::log_event(ainxt_telemetry::events::YoloToggled {
            enabled: new,
            previous_state,
            trigger: ainxt_telemetry::events::YoloTrigger::Pager,
        });
...
/// modal. Matches `set_yolo_mode` but preserves the canonical string
...
fn dispatch_cycle_mode_inner(app: &mut AppView) -> Vec<Effect> {
    ...
    match (in_plan, in_auto, in_yolo) {
        ...
        (false, false, false) => {
            ...
            vec![Effect::SetSessionMode {
                session_id,
                mode_id: acp::SessionModeId::new(ainxt_tools::types::SessionMode::Plan.as_id()),
            }]
        }
