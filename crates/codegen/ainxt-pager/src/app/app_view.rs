...
/// [`ainxt_shell::util::config::resolve_hints`]; this type is the pager's
/// in-memory mirror.
...
impl From<ainxt_shell::util::config::WorktreeHintMode> for WorktreeMode {
    fn from(mode: ainxt_shell::util::config::WorktreeHintMode) -> Self {
        use ainxt_shell::util::config::WorktreeHintMode;
        match mode {
...
impl WorktreeMode {
    ...
    pub fn from_config_str(s: &str) -> Self {
        ainxt_shell::util::config::WorktreeHintMode::from_config_str(s).into()
    }
    ...
    pub fn resolve_from_hints_value(hints: Option<&toml::Value>) -> (Self, Self) {
        let (new_session, fork) =
            ainxt_shell::util::config::WorktreeHintMode::resolve_pair(hints);
...
///
/// Matches [`AuthState`]: a welcome sub-state that drives the "Do you trust the
...
/// [`ainxt_workspace::folder_trust::decide`] verdict; when the feature flag
...
impl PendingAction {
    ...
    /// the dashboard-overlay stop (Ctrl+X), which matches the
...
/// Cap for the `AINXT_ESC_DOUBLE_PRESS_MS` override; the pty_e2e suite sets
/// exactly this value.
pub const ESC_DOUBLE_PRESS_TEST_MS: u64 = 60_000;
/// Idle-Esc double-press confirm window, `AINXT_ESC_DOUBLE_PRESS_MS`-overridable
...
pub(crate) fn esc_double_press_ttl() -> Duration {
    ...
    *TTL.get_or_init(|| parse_esc_ttl(std::env::var("AINXT_ESC_DOUBLE_PRESS_MS").ok()))
}
...
/// (toolset) gate via [`ainxt_shell::tier::is_restricted_tier_name`] so
...
fn is_restricted_tier(tier: Option<&str>) -> bool {
    match tier {
        None => true,
        Some(t) => ainxt_shell::tier::is_restricted_tier_name(t),
    }
}
...
pub struct AppView {
    ...
    /// stays sans-IO.
    pub current_ui: ainxt_shell::agent::config::UiConfig,
    /// Working directory.
    pub cwd: PathBuf,
    /// Whether the project picker question has already been shown this session.
    pub project_picker_shown: bool,
    /// "Don't ask me again" opt-out from [`ainxt_shell::util::config::resolve_hints`];
    ...
    /// Current appearance config (hot-reloadable from ~/.ainxt/pager.toml).
    ...
    /// Scroll-diagnostics HUD (`AINXT_SCROLL_DEBUG` env / `/scroll-debug`).
    ...
    /// Release-safe FPS HUD (`/debug fps`; `AINXT_FPS` env on release
    /// builds, where the dev overlay is compiled out) — see the module doc.
    pub fps_hud: crate::views::fps_hud::FpsHud,
    pub active_announcements: Vec<ainxt_announcements::RemoteAnnouncement>,
    ...
    pub announcement: Option<ainxt_announcements::RemoteAnnouncement>,
    ...
    /// Whether the plugin marketplace CTA is enabled. Env `AINXT_PLUGIN_CTA`
    ...
    /// Dashboard entry points (`/dashboard`, `Ctrl+\`, `ainxt dashboard`, the
    ...
    /// `ainxt.dev/sessions/list` polls and `ainxt.dev/sessions/changed` broadcasts.
    ...
    /// poll outside leader mode, so we fetch the same `ainxt.dev/session/list` the
    ...
    /// Reconciled from `ainxt.dev/queue/changed` broadcasts so
    ...
    /// confirming `ainxt.dev/queue/changed` broadcast has not yet arrived. Keyed by
    ...
    /// non-selectable headers. Gated by `AINXT_SESSION_PICKER_GROUPED` env var
    ...
    /// back into the input box. Gated by `AINXT_CANCEL_REWIND` env /
    ...
    /// When false, the pager must not request recaps (zero `ainxt.dev/recap` traffic).
    ...
    /// empty and inert outside `--minimal`; the `ainxt-pager-minimal` crate
    ...
    pub session_picker_content_results:
        Option<Vec<ainxt_shell::extensions::session_search::SearchSessionHit>>,
    ...
    pub(crate) foreign_session_compat:
        ainxt_workspace::foreign_sessions::EnabledForeignSessionSources,
    ...
    /// `ainxt_shell::util::config::resolve_auto_permission_mode_enabled`.
    ...
    /// Stamps `_meta["ainxt.dev/session"].kind = "chat"` and omits Build agent
    ...
    /// at startup and on settings toggles from `AINXT_CONTEXTUAL_HINTS` (master)
    /// > `[ui.contextual_hints]` user config > remote tier > default.
    pub contextual_hints: ainxt_shell::util::config::ResolvedContextualHints,
    ...
    pub remote_contextual_hints: Option<ainxt_shell::util::config::ContextualHintsRemote>,
    ...
    /// Folder-trust state for the welcome screen. Matches [`AppView::auth_state`]:
    ...
    /// Login button label from `AuthMethod.name` (e.g., "ainxt.dev", "Acme Corp").
    ...
    /// `ainxt_build_usage_redirect_url`, targeted at personal-team users).
    ...
    pub announcement_cta_impressions_logged:
        std::collections::BTreeSet<(String, ainxt_telemetry::events::AnnouncementCtaSurface)>,
    /// Access gate from `ainxt_build_access_gate`. `Some` = blocked.
    pub gate: Option<ainxt_shell::auth::GateInfo>,
    /// User-friendly subscription tier name (e.g. "SuperAinxt", "Free").
    ...
    pub pending_gate_verification: Option<ainxt_shell::auth::GateInfo>,
    ...
    /// `~/.ainxt/config.toml`. `None` when the file/section is absent
    ...
    /// kill switch or `AINXT_VOICE_MODE=0` — the STT pipeline is not started and
    ...
    pub voice_config: ainxt_voice::VoiceConfig,
    /// Auth for STT (OAuth session via shell `AuthManager`, or `AINXT_API_KEY`).
    /// `None` until the pipeline is first started (lazy on `/voice`).
    pub voice_auth: Option<ainxt_voice::SharedVoiceAuth>,
    /// Commands into the voice pipeline (start/stop capture — toggle, not hold).
    pub voice_cmd_tx: Option<tokio::sync::mpsc::Sender<ainxt_voice::VoiceCommand>>,
...
impl AppView {
    ...
    /// trust must be resolved. Matches the auth gate at the session-creating
    ...
    pub fn gate_from_settings(
        rs: &ainxt_shell::util::config::RemoteSettings,
    ) -> Option<ainxt_shell::auth::GateInfo> {
        let msg = rs.gate_message.as_ref()?;
        if msg.is_empty() {
            return None;
        }
        Some(ainxt_shell::auth::GateInfo {
            message: msg.clone(),
    ...
    pub fn apply_auth_meta(&mut self, meta: &ainxt_shell::auth::AuthMeta) {
        ...
        if was_gated && self.gate.is_none() {
            ...
            ainxt_telemetry::session_ctx::log_event(
                ainxt_telemetry::events::SubscriptionActivated {
    ...
    pub fn new(
        ...
        Self {
            ...
            current_ui: ainxt_shell::agent::config::UiConfig::default(),
            ...
            auto_mode_gate: ainxt_shell::util::config::auto_permission_mode_enabled_from_disk(),
            ...
            voice_config: ainxt_voice::VoiceConfig::default(),
            voice_auth: None,
            voice_cmd_tx: None,
    ...
    pub fn deferred_model_switch_from_cli(
        ...
        acp::ModelId,
        Option<ainxt_shell::sampling::types::ReasoningEffort>,
    )> {
    ...
    pub fn voice_can_start_pipeline(&self) -> bool {
        self.voice_mode_enabled && ainxt_voice::AUDIO_SUPPORTED
    }
    /// Sync voice availability into slash surfaces, cheatsheet, and settings.
    /// Matches `apply_session_recap_available` for `/recap`.
    ...
    /// `/always-approve` and `/auto` stay offered as true toggles. Matches
    ...
    /// lockstep. Matches [`Self::apply_voice_mode_enabled`].
    ///
    /// Called from [`Self::apply_auth_meta`] (startup / login) and from the
    /// `ainxt.dev/settings/update` handler when the subscription tier changes, so
    ...
    fn voice_send(&self, cmd: ainxt_voice::VoiceCommand) {
    ...
    pub(crate) fn voice_begin_recording(&mut self, target: VoiceTarget, hold: bool) {
        self.voice_send(ainxt_voice::VoiceCommand::PttPress);
    ...
    pub(crate) fn voice_stop_keeping_final(&mut self) {
        ...
        self.voice_send(ainxt_voice::VoiceCommand::PttRelease);
    ...
    /// `ainxt.dev/queue/changed` broadcast. The broadcast is
    ...
    /// `ainxt.dev/queue/changed` broadcast replaces it (matched by `id`) rather than
    ...
    /// Rows the dev `AINXT_FPS` overlay occupies (0 in non-dev builds), so
...
struct WelcomeInputCtx<'a> {
    ...
    sp_content_results:
        &'a Option<Vec<ainxt_shell::extensions::session_search::SearchSessionHit>>,
        ...
        if matches!(ctx.auth_state, AuthState::Done) {
            if ctx.upgrade_cta_keyboard && key!('o', CONTROL).matches(key) {
                return InputOutcome::Action(Action::AnnouncementsOpenCta(
                    ainxt_telemetry::events::AnnouncementCtaSurface::Keyboard,
                ));
            }
        ...
        match mouse.kind {
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                if let Some(rect) = ctx.gate_url_rect
                    && rect.contains(ratatui::layout::Position::new(mouse.column, mouse.row))
                {
                    return InputOutcome::Action(Action::OpenSubscribeUrl);
                }
                if let Some(rect) = ctx.upgrade_cta_rect
                    && rect.contains(ratatui::layout::Position::new(mouse.column, mouse.row))
                {
                    return InputOutcome::Action(Action::AnnouncementsOpenCta(
                        ainxt_telemetry::events::AnnouncementCtaSurface::Welcome,
                    ));
                }
...
fn dispatch_access_gate_menu_action(index: usize) -> InputOutcome {
    match index {
        0 => InputOutcome::Action(Action::OpenSubscribeUrl),
...
impl AppView {
    ...
    fn draw_inner(&mut self, terminal: &mut PagerTerminal) {
            ...
            )
        {
            ...
            if crate::terminal::terminal_context()
                ...
                .osc22_cursor
            {
                ainxt_shell::util::with_locked_stderr(|stderr| {
                ...
                match *active_view {
                    ActiveView::Welcome => {
                        ...
                        if !has_access && !self.access_gate_shown_logged {
                            ...
                            ainxt_telemetry::session_ctx::log_event(
                                ainxt_telemetry::events::SuperAinxtUpsellShown {
                                    source:
                                        ainxt_telemetry::events::SuperAinxtUpsell::WelcomeScreen,
                                    auth_method: self
                                        .login_method_id
                                        .as_ref()
    ...
    /// Log [`ainxt_telemetry::events::AnnouncementCtaShown`] for each
    ...
    pub(crate) fn log_announcement_cta_impressions(&mut self) {
        use ainxt_telemetry::events::AnnouncementCtaSurface;
        ...
        let key = ainxt_announcements::announcement_hide_key(owner);
        ...
        for (surface, _) in surfaces.into_iter().filter(|(_, painted)| *painted) {
            ...
            {
                ainxt_telemetry::session_ctx::log_event(
                    ainxt_telemetry::events::AnnouncementCtaShown {
                        id: id.clone(),
                        source: surface,
                    },
                );
...
impl AppView {
    ...
    pub fn apply_contextual_hints(
        &mut self,
        resolved: ainxt_shell::util::config::ResolvedContextualHints,
    ) {
    ...
    fn apply_clipboard_probe(
        ...
        if agent.show_ephemeral_tip(
            ...
            ainxt_telemetry::session_ctx::log_event(ainxt_telemetry::events::ContextualTip {
                tip: ainxt_telemetry::events::ContextualTipKind::ImageInput,
                action: ainxt_telemetry::events::ContextualTipAction::Shown,
            });
            return true;
        }
        false
...
pub(crate) mod tests {
    ...
    fn app_draw_drains_deferred_release_after_flush() {
        ...
        let mut terminal = ainxt_ratatui_inline::Terminal::with_options(
            backend,
            TerminalOptions {
    ...
    fn slash_gate_resyncs_when_critical_expires_between_pushes() {
        ...
        app.active_announcements = vec![ainxt_announcements::RemoteAnnouncement {
    ...
    fn needs_animation_gates_todo_badge_flash() {
        ...
        app.agents
            ...
            .unwrap()
            .todo
            .update_todos(vec![ainxt_shell::tools::TodoItem {
                content: "do the thing".into(),
                priority: Default::default(),
                status: ainxt_shell::tools::TodoStatus::InProgress,
                meta: None,
            }]);
        assert!(
    ...
    fn apply_auth_meta_hides_usage_for_team_users() {
        ...
        let meta = ainxt_shell::auth::AuthMeta {
    ...
    fn apply_auth_meta_shows_usage_for_personal_users() {
        ...
        let meta = ainxt_shell::auth::AuthMeta::default();
        app.apply_auth_meta(&meta);
        assert!(app.usage_visible);
    }
    #[test]
    fn apply_auth_meta_clears_api_key_flag_and_shows_usage_on_personal_login() {
        ...
        app.usage_visible = false;
        app.apply_auth_meta(&ainxt_shell::auth::AuthMeta::default());
    ...
    fn apply_auth_meta_api_key_enables_voice_and_skips_tier_gate() {
        ...
        app.apply_auth_meta(&ainxt_shell::auth::AuthMeta {
    ...
    /// fail-closed hiding:
    /// - `/image`, `/video` are `required_tools()`-gated, so advertise
    ...
    fn apply_auth_meta_lifts_restrictions_for_paid_tiers_and_teams() {
        ...
        advertise_media_tools(&mut app);
        let meta = ainxt_shell::auth::AuthMeta {
            subscription_tier: Some("SuperAinxt".into()),
            ..Default::default()
        };
        app.apply_auth_meta(&meta);
        assert!(app.tier_restricted_commands.is_empty());
        ...
        app.apply_auth_meta(&ainxt_shell::auth::AuthMeta::default());
        assert!(!app.tier_restricted_commands.is_empty());
        app.subscription_tier = Some("SuperAinxt".into());
    ...
    fn is_restricted_tier_classification() {
        ...
        assert!(!is_restricted_tier(Some("SuperAinxt")));
        assert!(!is_restricted_tier(Some("SuperAinxt Heavy")));
    ...
    fn apply_auth_meta_clears_gate_on_subscription() {
        let mut app = test_app();
        app.gate = Some(ainxt_shell::auth::GateInfo {
            message: "Subscribe to use ainxt".into(),
            url: Some("https://ainxt.example.test/subscribe".into()),
            label: None,
        });
    ...
    fn apply_auth_meta_gate_unchanged_when_still_gated() {
        let mut app = test_app();
        let gate = ainxt_shell::auth::GateInfo {
    ...
    fn welcome_ctrl_u_update_keeps_priority_over_foreign_resume() {
        ...
        app.apply_foreign_resume_detection(
            launch_token,
            &canonical_cwd,
            Some(ainxt_workspace::foreign_sessions::RecentForeignSession {
                tool: ainxt_workspace::foreign_sessions::ForeignSessionTool::Cursor,
    ...
    /// dashboard session overlay — matches the `q` shortcut and
    ...
    fn overlay_left_arrow_file_search_open_does_not_exit() {
        let (mut app, id) = neutral_overlay_app();
        {
            ...
            agent.prompt.file_search.set_test_state(
                ctx,
                vec![ainxt_workspace::file_system::FuzzyMatchResult {
