...
struct AgentLoadOutcome {
    ...
    success: bool,
    /// `ainxt.dev/runningPromptId` from the reload response: the turn another
    /// client is driving mid-reconnect, adopted at finalize (matches the
...
/// (matches the `unwrap_or((false, _))` at the finalize site).
...
/// [`decide`](ainxt_workspace::folder_trust::decide) precedence.
///
...
pub(crate) async fn run(
    ...
    if args.log_sampling {
        ...
        unsafe { std::env::set_var("AINXT_LOG_SAMPLING", "1") };
    }
    ...
    let launch_effective_ui = ainxt_shell::config::load_effective_config()
        .ok()
    ...
    let toml_owns_mode = launch_effective_ui
        .as_ref()
        .and_then(ainxt_shell::util::config::permission_mode_from_ui_if_set)
        .is_some();
    ...
    app.require_plan_approval = ainxt_shell::util::config::load_require_plan_approval();
    ...
    if let Some(ref agent) = args.agent {
        match crate::headless::resolve_agent_arg(agent) {
            crate::headless::ResolvedAgent::FilePath(path) => {
                match ainxt_shell::agent::config::AgentDefinition::from_file(&path) {
    ...
    app.sharing_enabled = remote_settings
        ...
        .unwrap_or(false);
    app.plugin_cta_enabled = ainxt_config::env_bool("AINXT_PLUGIN_CTA")
        .or_else(|| remote_settings.as_ref().and_then(|s| s.plugin_cta))
        .unwrap_or(false);
    // Voice is applied after auth_meta so API-key detection is accurate.
    app.session_picker_grouped = std::env::var("AINXT_SESSION_PICKER_GROUPED")
        .ok()
    ...
    if needs_interactive_login {
        ...
        if connection.needs_login {
            ...
            // --force-login: find the ainxt.dev method from the advertised list
            let ainxt_com = connection
                .auth_methods
                .iter()
                .find(|m| m.id().0.as_ref() == "ainxt.dev");
            if let Some(method) = ainxt_com {
                ...
                // No ainxt.dev method available, use the first method as fallback
        ...
        // fail-closed: do not invent ainxt.dev / auto-start OIDC.
        tracing::info!(
    ...
    let mut post_render_effects = if needs_interactive_login {
        if connection.auth_methods.is_empty() {
            ...
            app.auth_state = super::app_view::AuthState::Pending {
                error: Some(
                    ainxt_shell::agent::auth_method::PREFERRED_API_KEY_UNAVAILABLE.to_string(),
                ),
            };
            vec![]
        } else {
    ...
    if let Some(meta) = connection.auth_meta.as_ref() {
        match serde_json::from_value::<ainxt_shell::auth::AuthMeta>(meta.clone()) {
        ...
        app.is_api_key_auth = app.auth_methods.iter().any(|m| {
            m.id().0.as_ref() == ainxt_shell::agent::auth_method::AINXT_API_KEY_METHOD_ID
        });
    ...
    app.hidden_announcement_ids = ainxt_announcements::read_hidden_announcement_ids().await;

    // Load config layers once, resolve announcements, tips, and feature flags.
    let requirements = ainxt_shell::config::load_merged_requirements();
    let user_config = ainxt_shell::config::load_from_disk().ok();
    let managed_config = ainxt_shell::config::load_managed_config().ok();

    // Full merge when every layer parses; partial merge below if any layer fails.
    let effective_config = match ainxt_shell::config::load_effective_config() {
        Ok(raw) => Some(raw),
        Err(e) => {
    ...
    let compat = ainxt_shell::agent::config::resolve_compat_sessions_from_raw(
    ...
    if let Some(ref raw) = effective_config {
        ...
        if let Some(table) = raw.as_table() {
            ...
            // (config > AINXT_API_BASE_URL env > default).
            let endpoints_base =
                ainxt_shell::agent::config::EndpointsConfig::from_config_value(raw)
                    .ainxt_api_base_url;
            app.voice_config =
                ainxt_voice::VoiceConfig::from_config_table(table, Some(&endpoints_base));
        }
    }
    // Stamp request-identity headers so the STT handshake attributes voice usage
    // to ainxt-cli server-side (matches sampler / imagine). Done after
    ...
    app.zdr_access_enabled = ainxt_shell::util::config::resolve_zdr_access_enabled(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    crate::appearance::cache::set_show_thinking_blocks(
        ainxt_shell::util::config::resolve_show_thinking_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_group_tool_verbs(
        ainxt_shell::util::config::resolve_group_tool_verbs(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_collapsed_edit_blocks(
        ainxt_shell::util::config::resolve_collapsed_edit_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    {
        use ainxt_shell::util::config::{resolve_announcements, resolve_tips};
        ...
        app.active_announcements = ainxt_announcements::filter_expired(announcements);
        ...
        if !app.tips.is_empty() {
            let ainxt_home = ainxt_tools::util::ainxt_home::ainxt_home();
            app.tip = ainxt_shell::util::tips::pick_and_advance(&app.tips, &ainxt_home);
        }
    }

    let hints = ainxt_shell::util::config::resolve_hints(
    ...
    // Apply initial config (may come from existing ~/.ainxt/pager.toml).
    ...
    let display_mode: &'static str = if launch_auto {
        ...
        ainxt_shell::util::config::clamped_display_permission_mode(
            ainxt_shell::util::config::parse_permission_mode_canonical(cli),
        )
    } else {
        ainxt_shell::util::config::resolved_display_permission_mode(
    ...
    let resolved_hints = ainxt_shell::util::config::resolve_contextual_hints(
    ...
    let mouse_toggle = ainxt_shell::util::config::resolve_mouse_reporting_toggle(
        effective_config.as_ref(),
        &app.current_ui,
    );
    ...
    // AINXT_SCROLL_SPEED/_MODE/_LINES + AINXT_INVERT_SCROLL env overrides
    // apply on first load).
    ...
    let mut voice_rx = None::<tokio::sync::mpsc::Receiver<ainxt_voice::VoiceEvent>>;
    ...
    // Initial prompt from the CLI positional (`ainxt "fix the bug"`). When
    ...
    // accounts cannot start a session, so drop the prompt — this matches the
    ...
    // `ainxt dashboard` startup: open the dashboard view immediately. The
    // CLI subcommand wrote a `AINXT_OPEN_DASHBOARD_AT_STARTUP=1` env var
    // so we don't have to thread a flag through every arg struct.
    if std::env::var("AINXT_OPEN_DASHBOARD_AT_STARTUP").as_deref() == Ok("1") {
        ...
        if app.session_startup_allowed() {
            ...
            // Defer to the `AuthComplete` handler (matches
    ...
    loop {
        ...
        if let VoiceState::ColdStart { hold, target } = app.voice_state {
            if app.voice_cmd_tx.is_none() && app.voice_can_start_pipeline() {
                ...
                tokio::spawn(ainxt_voice::run_voice_pipeline(
                    voice_config,
                    voice_auth.clone(),
                    cmd_rx,
                    event_tx,
                ...
                app.voice_state = VoiceState::Idle;
                app.voice_ui_active = false;
                app.show_toast("Voice pipeline could not start — restart ainxt");
            } else {
        ...
        tokio::select! {
            ...
            Ok(()) = config_watcher.changed() => {
                ...
                // Preserve fields persisted via `~/.ainxt/config.toml [ui]`
                // rather than `~/.ainxt/pager.toml`. The watcher only knows
...
pub(crate) fn load_initial_ui_config() -> ainxt_shell::agent::config::UiConfig {
    use ainxt_shell::agent::config::UiConfig;
    let Ok(root) = ainxt_shell::config::load_effective_config() else {
...
/// Config `Option<bool>` matches seeded once at startup. `None` = no
