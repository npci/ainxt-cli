...
/// Deserialize shape of the `ainxt.dev/follow_ups` params emitted by the shell
/// translator: `{ response_id, suggestions: [{ label, .. }] }`. The keys are
/// prost-derived snake_case — NOT camelCase like most other pager
/// notification payloads — so this struct must match snake_case unchanged.
...
pub(super) struct FollowUpsParams {
    ...
    /// change. Parsed loosely as a JSON value to read the `"ainxt.dev/replayed"`
    /// key (a slash-bearing key prost cannot model as a field).
    #[serde(default, rename = "_meta")]
    meta: Option<serde_json::Value>,
}

/// A single `ainxt.dev/follow_ups` suggestion. Only the human-facing `label` is
...
/// Handle `ainxt.dev/follow_ups` — render follow-up suggestion chips for the
/// latest assistant response.
///
/// Newest-response-wins keying lives in [`AgentView::apply_follow_ups`]. The
/// reserved `_meta["ainxt.dev/replayed"] == true` marker suppresses rendering (it
...
pub(super) fn handle_follow_ups(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    if params
        .meta
        .as_ref()
        .and_then(|m| m.get("ainxt.dev/replayed"))
