#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// layer-injected seam so the developer's real `~/.ainxt` cannot leak in.
    #[test]
    ...
    /// through the layer-injected seam (no real `~/.ainxt` reads).
    #[test]
    ...
    /// layer-injected seam (no real `~/.ainxt` reads).
    #[test]
