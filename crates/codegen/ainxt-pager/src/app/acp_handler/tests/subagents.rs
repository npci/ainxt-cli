#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// Regression: replay from `updates.jsonl` emits `ainxt.dev/session/update` (not
    ...
    fn subagent_resume_finished_then_open_shows_full_transcript() {
        with_replay_disk_home(|_| {
            ...
            // The lazy load reapplies the "Worked for" footer (live equivalence).
    ...
    fn ext_session_notification_and_update_equivalent_for_subagent_spawned() {
        ...
        let (spawn_notif, finish_notif) =
            run_subagent_lifecycle_via_method("ainxt.dev/session_notification", child_sid);
        let (spawn_update, finish_update) =
            run_subagent_lifecycle_via_method("ainxt.dev/session/update", child_sid);
    ...
    fn ext_session_update_malformed_params_returns_false() {
        ...
        let request = acp::ExtNotification::new("ainxt.dev/session/update", raw.into());
        let msg = AcpClientMessage::ExtNotification(ainxt_acp_lib::AcpArgs {
            request,
            response_tx: tx,
        });
        ...
        assert!(
            !affected,
            "malformed ainxt.dev/session/update params must not redraw"
        );
        assert!(
    ...
    fn ext_session_notification_for_inactive_agent_updates_its_context_used() {
        // AutoCompactCompleted on the ainxt ext path resets the context bar
