#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn settings_non_api_key_tier_clears_stale_api_key_flag() {
        ...
        assert!(app.voice_mode_enabled);
        assert!(handle_ext_notification(
            &tier_settings_update("SuperAinxt"),
            &mut app
        ));
        assert!(!app.is_api_key_auth);
        assert!(app.voice_mode_enabled);
    ...
    /// The live-refresh flip matches `set_group_tool_verbs_inner`'s stale
    ...
    /// gen-ordered `ainxt.dev/announcements/update` for every settings writer, and a
