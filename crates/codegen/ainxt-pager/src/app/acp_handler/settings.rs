...
/// Handle `ainxt.dev/models/update` — model list changed (etag-triggered refresh).
pub(super) fn handle_models_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    if let Ok(model_state) = serde_json::from_str::<acp::SessionModelState>(notif.params.get()) {
        ...
        tracing::info!(
            ...
            "models updated via ainxt.dev/models/update"
        );
        ...
        tracing::warn!("Failed to parse ainxt.dev/models/update");
        false
    }
}

/// Handle `ainxt.dev/settings/update` — remote settings refreshed on `/new`.
pub(super) fn handle_settings_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(update) = serde_json::from_str::<PagerSettingsUpdate>(notif.params.get()) else {
        tracing::warn!("Failed to parse ainxt.dev/settings/update");
        return false;
    };
    ...
    if let Some(v) = update.auto_permission_mode_enabled {
        ...
        // without a restart.
        ainxt_shell::util::config::cache_remote_auto_permission_mode_enabled(Some(v));
        app.auto_mode_gate = ainxt_shell::util::config::auto_permission_mode_enabled_from_disk();
        ...
        && app.permission_mode_from_soft_default
    {
        ...
        let root = ainxt_shell::config::load_effective_config().ok();
    ...
    // Always recompute is_api_key_auth from the tier so a later Free/SuperAinxt
    ...
    if let Some(v) = update.subscription_tier_display {
        ...
        if was_api_key
            ...
            && app
                .subscription_tier
                .as_deref()
                .is_some_and(ainxt_shell::tier::is_restricted_tier_name)
        {
            app.voice_reset();
    ...
    // Respect env var > config > remote precedence (matches event_loop.rs startup).
    if let Some(remote_val) = update.session_picker_grouped {
        let resolved = std::env::var("AINXT_SESSION_PICKER_GROUPED")
            .ok()
        ...
        && !msg.is_empty()
    {
        ...
        let effs = app.impose_gate(ainxt_shell::auth::GateInfo {
            message: msg.clone(),
    ...
    let (requirements, user_config, managed_config) = (
        ainxt_shell::config::load_merged_requirements(),
        ainxt_shell::config::load_from_disk().ok(),
        ainxt_shell::config::load_managed_config().ok(),
    );

    // Local layers may beat remote — re-resolve the full chain into the render
    // cache (matches the event_loop.rs startup resolve). Runs on None too: the
    ...
    // until restart.
    let remote = ainxt_shell::util::config::RemoteSettings {
        group_tool_verbs: update.group_tool_verbs,
        ..Default::default()
    };
    let resolved = ainxt_shell::util::config::resolve_group_tool_verbs(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    .value;
    // On a real flip, re-fold every live transcript (matches dispatch's
    ...
    let resolved = ainxt_shell::util::config::resolve_collapsed_edit_blocks(
        requirements.as_ref(),
        user_config.as_ref(),
        managed_config.as_ref(),
        Some(&remote),
    ...
    // in every live transcript (matches dispatch's
    ...
    if let Some(remote_tips) = update.tips {
        use ainxt_shell::util::config::resolve_tips;
    ...
    tracing::info!("settings updated via ainxt.dev/settings/update");
    true
}
...
pub(super) fn apply_soft_default_permission_mode(
    ...
    let mode = ainxt_shell::util::config::resolve_permission_mode(effective_ui, remote);
    ...
    app.current_ui.permission_mode = Some(if auto {
        ...
        ainxt_shell::util::config::resolved_display_permission_mode(effective_ui, remote)
            .to_string()
    });
}

/// Tell live sessions to leave Auto on the mid-session kill-switch: fire the
/// `ainxt.dev/yolo_mode_changed` notification the agent maps to
...
pub(super) fn notify_sessions_leave_auto(app: &AppView, session_ids: &[acp::SessionId]) {
    ...
    let args = ainxt_acp_lib::AcpArgs {
...
/// Handle `ainxt.dev/sessions/changed` — the leader broadcasts roster
...
pub(super) fn handle_sessions_changed(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(changed) = serde_json::from_str::<crate::app::roster::RosterChanged>(notif.params.get())
        ...
        tracing::warn!("Failed to parse ainxt.dev/sessions/changed");
        return false;
    };
...
pub(super) fn handle_announcements_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(parsed) =
        serde_json::from_str::<ainxt_announcements::AnnouncementsRefreshed>(notif.params.get())
    else {
        return false;
    };
...
/// `resolve_announcements` honors `AINXT_ANNOUNCEMENTS_OVERRIDE` first, so a
/// backend push can't reintroduce announcements when the override is set.
pub(super) fn apply_announcements_update(
    app: &mut AppView,
    next_gen: u64,
    remote: &[ainxt_announcements::RemoteAnnouncement],
    requirements: Option<&toml::Value>,
    user_config: Option<&toml::Value>,
    managed_config: Option<&toml::Value>,
) {
    let merged = ainxt_shell::util::config::resolve_announcements(
        requirements,
        user_config,
        managed_config,
        Some(remote),
    );
    let announcements = ainxt_announcements::filter_expired(merged);
    ...
    if ainxt_announcements::prune_hidden_announcement_ids(
...
) -> Option<ainxt_announcements::RemoteAnnouncement> {
...
/// Deserialization type for the `ainxt.dev/settings/update` notification payload.
///
/// This is intentionally a separate struct from `SettingsUpdateNotification` in
/// `ainxt-shell/src/agent/mvp_agent.rs`. The shell side derives `Serialize`
...
/// `ainxt-shell/src/agent/mvp_agent.rs` when adding fields that both sides
/// need.
...
pub(super) struct PagerSettingsUpdate {
    ...
    // remote_settings also emits gen-ordered `ainxt.dev/announcements/update`
