...
/// Wire payload of `ainxt.dev/session/prompt_complete`, emitted by
...
pub(super) fn handle_queue_changed(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(changed) =
        ...
        tracing::warn!("Failed to parse ainxt.dev/queue/changed");
        return false;
    };
    ...
    tracing::debug!(
        ...
        "received ainxt.dev/queue/changed broadcast",
    );
...
/// durable [`AinxtSessionUpdate::TurnCompleted`]): it parses the payload and
/// delegates the turn-finalize to
/// [`finalize_turn_from_terminal`](super::super::turn_completion::finalize_turn_from_terminal),
/// which carries the driver-arm / viewer-finish behavior unchanged.
///
...
pub(super) fn handle_prompt_complete(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(payload) = serde_json::from_str::<PromptCompletePayload>(notif.params.get()) else {
        tracing::warn!("Failed to parse ainxt.dev/session/prompt_complete");
        return false;
    };
