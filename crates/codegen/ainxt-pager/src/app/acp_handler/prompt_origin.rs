...
pub(crate) fn is_server_initiated_prompt(prompt_id: &str) -> bool {
    ainxt_shell::session::PromptOrigin::from_prompt_id(prompt_id).is_synthetic()
}
...
/// `ainxt.dev/session/prompt_complete` turn-end signal. A viewer can thus safely
...
pub(crate) fn is_scheduler_fired_prompt(prompt_id: &str) -> bool {
    matches!(
        ainxt_shell::session::PromptOrigin::from_prompt_id(prompt_id),
        ainxt_shell::session::PromptOrigin::SchedulerFired
    )
}
...
pub(crate) fn is_wake_prompt(prompt_id: &str) -> bool {
    matches!(
        ainxt_shell::session::PromptOrigin::from_prompt_id(prompt_id),
        ainxt_shell::session::PromptOrigin::TaskCompleted { .. }
            | ainxt_shell::session::PromptOrigin::SubagentCompleted { .. }
            | ainxt_shell::session::PromptOrigin::NotificationDrain
    )
}
...
/// adoptable iff the turn emits a terminal `ainxt.dev/session/prompt_complete`, the
