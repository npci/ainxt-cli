...
//! and ainxt session extension notifications (`ainxt.dev/session_notification` and
//! replay-path `ainxt.dev/session/update`).
...
use ainxt_acp_lib::AcpClientMessage;

use super::actions::Effect;
use ainxt_shell::extensions::notification::{
    SessionNotification, SessionUpdate as AinxtSessionUpdate, is_reauthable_failure,
};
use ainxt_shell::tools::todo::todo_item_from_plan_entry;
use ainxt_workspace::permission::bash_command_splitting::BashCommandHighlights;
        ...
        AcpClientMessage::SessionNotification(notif) => {
            ...
            let affected = match find_session_match(app, &notif.request.session_id) {
                Some(SessionMatch::Root(id)) => {
                    ...
                    // time and drained FIFO. The ainxt stream is direct-emitted
                    // and keeps a SEPARATE highwater (see the ainxt dedup in
                    ...
                    if !dedup_drop {
                        ...
                        // The per-session cost rides the same `_meta` as
                        // `totalTokens` (see `NotificationMeta`). Absent =>
                        // leave the last known value / no segment.
                        if let Some(ticks) = meta.session_cost_ticks {
                            agent.apply_session_cost(ticks, meta.session_cost_estimated);
                        }
...
/// Handle an ainxt extension notification.
///
/// Dispatches on method string:
/// - `ainxt.dev/session_notification` / `ainxt.dev/session/update` → per-agent session updates
fn handle_ext_notification(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    match notif.method.as_ref() {
        ...
        "ainxt.dev/follow_ups" => handle_follow_ups(notif, app),
        "ainxt.dev/task_backgrounded" => handle_task_backgrounded(notif, app),
        "ainxt.dev/task_completed" => handle_task_completed(notif, app),
        "ainxt.dev/models/update" => handle_models_update(notif, app),
        "ainxt.dev/settings/update" => handle_settings_update(notif, app),
        "ainxt.dev/sessions/changed" => handle_sessions_changed(notif, app),
        "ainxt.dev/queue/changed" => handle_queue_changed(notif, app),
        ...
        "ainxt.dev/session/prompt_complete" => handle_prompt_complete(notif, app),
        "ainxt.dev/session/interjection" => handle_interjection(notif, app),
        "ainxt.dev/monitor_event" => handle_monitor_event(notif, app),
        "ainxt.dev/scheduled_task_created" => handle_scheduled_task_created(notif, app),
        "ainxt.dev/scheduled_task_fired" => handle_scheduled_task_fired(notif, app),
        "ainxt.dev/scheduled_task_deleted" => handle_scheduled_task_deleted(notif, app),
        "ainxt.dev/scheduled_task_inject_prompt" => handle_scheduled_task_inject_prompt(notif, app),
        "ainxt.dev/announcements/update" => handle_announcements_update(notif, app),
        "ainxt.dev/git_head_changed" => handle_git_head_changed(notif, app),
        "ainxt.dev/mcp/init_progress" => handle_mcp_init_progress(notif, app),
        "ainxt.dev/mcp/tools_changed" | "ainxt.dev/mcp_initialized" => handle_mcp_tools_changed(notif, app),
        "ainxt.dev/mcp/server_status" if push_server_status_enabled() => {
            handle_mcp_server_status(notif, app)
        }
        "ainxt.dev/mcp/servers_updated" => handle_mcp_servers_updated(notif, app),
        _ => false,
    }
}

/// Handle `ainxt.dev/session/interjection` — the leader broadcasts this
...
fn handle_interjection(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(parsed) = serde_json::from_str::<serde_json::Value>(notif.params.get()) else {
        tracing::warn!("Failed to parse ainxt.dev/session/interjection");
        return false;
    };
    ...
    // emits the queue-emptying `ainxt.dev/queue/changed` right after it).
...
fn handle_ext_method(ext: ainxt_acp_lib::AcpArgs<acp::ExtRequest>, app: &mut AppView) -> bool {
    match ext.request.method.as_ref() {
        "ainxt.dev/ask_user_question" => handle_ask_user_question(ext, app),
        "ainxt.dev/exit_plan_mode" => handle_exit_plan_mode(ext, app),
        unknown => {
