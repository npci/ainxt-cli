...
///
/// Resolution matches the `mcp.liveness_watchers` flag pattern but
...
/// `AINXT_MCP_PUSH_SERVER_STATUS=0` to disable.
pub(super) fn push_server_status_enabled() -> bool {
    use std::sync::OnceLock;
    static ENABLED: OnceLock<bool> = OnceLock::new();
    *ENABLED.get_or_init(|| {
        ainxt_shell::util::config::resolve_mcp_push_server_status(
...
/// Handle `ainxt.dev/mcp/tools_changed` and `ainxt.dev/mcp_initialized`.
///
/// Routing rules (verified against the four shell emit sites in
/// `ainxt-shell/src/session/acp_session.rs` — toggle-tool ~L6661,
...
///    [`ainxt_shell::extensions::mcp::McpToolsChanged`] struct), and
...
pub(super) fn handle_mcp_tools_changed(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    // Both `ainxt.dev/mcp_initialized` and (newer shell)
    // `ainxt.dev/mcp/tools_changed` carry `sessionId`. Route by it so a
    ...
    if method == "ainxt.dev/mcp_initialized"
...
/// Handle `ainxt.dev/mcp/server_status`.
///
...
/// ([`ainxt_shell::extensions::mcp::McpServerStatusPayload`] +
/// [`ainxt_shell::extensions::mcp::McpServerStatus`]) instead of
...
pub(super) fn handle_mcp_server_status(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    use ainxt_shell::extensions::mcp::{McpServerStatus, McpServerStatusPayload, McpToolEntry};

    let Ok(payload) = serde_json::from_str::<McpServerStatusPayload>(notif.params.get()) else {
        tracing::warn!(
            "Failed to parse ainxt.dev/mcp/server_status: {}",
            &notif.params.get()
        ...
        match serde_json::from_value::<Vec<McpToolEntry>>(raw) {
            ...
            Err(e) => {
                tracing::warn!(
                    server = %payload.name,
                    error = %e,
                    "ainxt.dev/mcp/server_status: tools field present but not Vec<McpToolEntry>; status still applied"
                );
                None
            }
        }
...
/// Handle `ainxt.dev/mcp/servers_updated`.
///
/// Emitted by the shell from `MvpAgent` on managed-config resolve and
/// on config reload (`crates/codegen/ainxt-shell/src/agent/mvp_agent.rs`
