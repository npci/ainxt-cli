...
use super::agent_view::{
    AgentView, active_contexts_for_pane, apply_model_editor_outcome, apply_settings_outcome,
};
...
impl AgentView {
    ...
    pub(super) fn handle_modal_key(&mut self, key: &KeyEvent) -> InputOutcome {
        ...
        // ModelEditor: route through ModalWindow chrome, then delegate.
        // Sub-modes (field editing / choice picking) own their own
        // Esc/keystroke semantics, same short-circuit as Settings above.
        if let ActiveModal::ModelEditor { state } = modal {
            if matches!(
                &state.mode,
                crate::views::model_editor::ModelEditorMode::Editing {
                    field_edit: Some(_),
                    ..
                }
            ) {
                let out = crate::views::model_editor::handle_model_editor_key(state, key);
                return apply_model_editor_outcome(self, out);
            }
            let chrome_cfg = mw::ModalWindowConfig {
                title: "",
                tabs: None,
                        ...
                        match cmd {
                            ...
                            PaletteCommand::OpenExtensionsTab(tab) => {
                                ...
                                InputOutcome::Action(Action::OpenExtensionsModal {
                                    tab,
                                    trigger: ainxt_telemetry::events::ExtensionsModalTrigger::CommandPalette,
                                })
                            }
                            ...
                            PaletteCommand::OpenModelEditor => {
                                self.active_modal = None;
                                InputOutcome::Action(Action::OpenModelEditor)
                            }
    ...
    pub(super) fn handle_modal_mouse(
        ...
        // ModelEditor: route through ModalWindow chrome (close button,
        // hover). Clicking the close button mirrors Esc's semantics: a
        // sub-mode actively editing a field owns Esc first, same
        // short-circuit as the key handler above.
        if let Some(ActiveModal::ModelEditor { state }) = &mut self.active_modal {
            let outcome =
                mw::handle_modal_mouse(&mut state.window, mouse.kind, mouse.column, mouse.row);
            return match outcome {
                ModalWindowOutcome::CloseRequested => {
                    if matches!(
                        ...
                        let out = crate::views::model_editor::handle_model_editor_key(state, &esc);
                        apply_model_editor_outcome(self, out)
                    } else {
                        self.active_modal = None;
                        InputOutcome::Changed
                    }
                ...
                ModalWindowOutcome::ShortcutActivated(id) => {
                    use crate::views::model_editor::{
                        SHORTCUT_CONFIRM_NO_ID, SHORTCUT_CONFIRM_YES_ID, SHORTCUT_DELETE_ID,
                        SHORTCUT_ENTER_ID, SHORTCUT_ESC_ID, SHORTCUT_SAVE_ID,
                    };
                    let synthetic = if id == SHORTCUT_ENTER_ID {
                        KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE)
                    } else if id == SHORTCUT_DELETE_ID {
                        KeyEvent::new(KeyCode::Char('d'), KeyModifiers::NONE)
                    } else if id == SHORTCUT_ESC_ID || id == SHORTCUT_CONFIRM_NO_ID {
                        KeyEvent::new(KeyCode::Esc, KeyModifiers::NONE)
                    } else if id == SHORTCUT_SAVE_ID {
                        KeyEvent::new(KeyCode::Char('s'), KeyModifiers::NONE)
                    } else if id == SHORTCUT_CONFIRM_YES_ID {
                        KeyEvent::new(KeyCode::Char('y'), KeyModifiers::NONE)
                    } else {
                        return InputOutcome::Changed;
                    };
                    let out = crate::views::model_editor::handle_model_editor_key(state, &synthetic);
                    apply_model_editor_outcome(self, out)
                }
            ...
            &mut self.active_modal
        {
            ...
            return match outcome {
                ModalWindowOutcome::CloseRequested => {
                    // Close-button (X) click → Cancel. Matches Esc /
    ...
    pub(crate) fn draw_active_modal(
        ...
        if let Some(ref mut active_modal) = self.active_modal {
            ...
            } else if let modal::ActiveModal::ModelEditor {
                state: model_editor_state,
            } = active_modal
            {
                crate::views::model_editor::render_model_editor(
                    buf,
                    area,
                    model_editor_state,
                    compact,
