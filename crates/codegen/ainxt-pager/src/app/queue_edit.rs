...
//! pattern as ainxt-shell's `compaction.rs`): entry from the queue pane,
...
pub enum PromptMode {
    ...
    EditingQueued {
        ...
        /// `ainxt.dev/queue/changed` rebroadcast for the visual result) instead
...
impl AgentView {
    ...
    fn interject_edited_queued(
        ...
        match server_id {
            Some(server_id) => {
                // Server rows: the queue wire (`ainxt.dev/queue/interject` newText)
