    ...
    pub(super) fn handle_mouse(&mut self, mouse: &MouseEvent) -> InputOutcome {
        match mouse.kind {
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                if self.hit_changes.contains(mouse.column, mouse.row) {
                    return InputOutcome::Action(Action::OpenSourceControl);
                }
                    ...
                    && !self.pos_occluded(mouse.column, mouse.row)
                {
                    return InputOutcome::Action(Action::AnnouncementsOpenCta(
                        ainxt_telemetry::events::AnnouncementCtaSurface::Banner,
                    ));
                }
                if self
                    .plugin_cta
                        ...
                        &self.plugin_cta.phase
                {
                    ...
                    if let Err(e) = ainxt_shell::config::add_dismissed_plugin_cta(&plugin_id) {
                        tracing::warn!(
                    ...
                    ainxt_telemetry::session_ctx::log_event(
                        ainxt_telemetry::events::PluginCtaDismissed {
                ...
                {
                    return InputOutcome::Action(Action::AnnouncementsOpenCta(
                        ainxt_telemetry::events::AnnouncementCtaSurface::Header,
                    ));
                }
                        ...
                        if let Some(idx) = hit_idx {
                                    ...
                                    .entry_screen_area(idx, self.pane_areas.scrollback)
                            {
                                ...
                                if click_row >= url_row {
                                    ...
                                    ainxt_telemetry::session_ctx::log_event(ainxt_telemetry::events::CreditLimitUpsellClicked {
