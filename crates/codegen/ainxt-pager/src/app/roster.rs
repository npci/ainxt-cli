...
//!
//! - Request/response `ainxt.dev/sessions/list` → [`RosterListResponse`].
//! - Broadcast notification `ainxt.dev/sessions/changed` → [`RosterChanged`].
//!
...
/// Response to `ainxt.dev/sessions/list`.
#[derive(Debug, Clone, Default, Deserialize)]
...
/// Broadcast payload for `ainxt.dev/sessions/changed`.
#[derive(Debug, Clone, Default, Deserialize)]
...
/// Parse an `ainxt.dev/sessions/list` ext-response body into a [`RosterListResponse`].
///
/// The agent serializes the response through
/// `ExtMethodResult::success(..).to_ext_response()` (see
/// `ainxt-shell/src/agent/handlers/session.rs::handle_roster_list`), which
...
/// `ainxt.dev/sessions/changed` broadcast. Matches how `Effect::FetchSessionList`
/// unwraps `result` for `ainxt.dev/session/list`.
...
mod tests {
    ...
    fn agent_entry() -> ainxt_shell::agent::roster::RosterEntry {
        use ainxt_shell::agent::roster as agent;
        agent::RosterEntry {
    ...
    fn roster_list_response_survives_result_envelope() {
        ...
        // Exact wire bytes the agent emits for `ainxt.dev/sessions/list`.
        ...
        assert_eq!(e.model_id.as_deref(), Some("ainxt-4"));
        assert!(e.yolo);
    ...
    /// `ainxt.dev/sessions/changed` round-trip: serialize the agent's `RosterChanged`
