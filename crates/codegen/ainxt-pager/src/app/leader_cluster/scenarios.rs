...
#[test]
#[ignore = "leader-cluster: needs single-process isolation (process-global env + ainxt_home OnceLock in the shared lib test binary); run: cargo test -p ainxt-pager --lib -- app::leader_cluster --ignored --test-threads=1"]
...
fn reattach_completion_roundtrips_durable_log() {
    ...
    tokio::task::LocalSet::new().block_on(&rt, async {
        ...
        // (equivalence with the PTY case; the in-process server also survives).
...
/// turns work. The reload-driver matches the event loop's reconnect arm
...
/// Locate `<ainxt_home>/sessions/<enc-cwd>/<sid>/updates.jsonl` without the
...
fn find_session_updates_file(sid: &str) -> Option<PathBuf> {
    let sessions = effective_ainxt_home().join("sessions");
