...
pub use ainxt_prompt_queue as prompt_queue;
mod acp_handler;
mod csi_filter;
mod dispatch;
...
pub(crate) use dispatch::{acp_error_is_free_usage_exhausted, free_usage_user_message};
...
/// (`[ui] mouse_reporting_toggle` / `AINXT_MOUSE_REPORTING_TOGGLE`). Seeded once
...
pub(crate) fn voice_mode_requirement_pin() -> Option<bool> {
    ainxt_config::load_merged_requirements().and_then(|req| {
        req.get("features")
...
pub(crate) fn voice_mode_config_value() -> Option<bool> {
    ainxt_shell::config::load_effective_config()
        .ok()
        .and_then(|cfg| {
            cfg.get("features")
                .and_then(|f| f.get("voice_mode"))
...
pub(crate) fn resolve_voice_mode_enabled(
    ...
    use ainxt_shell::agent::config::{BoolFlag, ConfigSource};
    let resolved = BoolFlag::env("AINXT_VOICE_MODE")
...
///
/// Uses [`ainxt_ratatui_inline::Terminal`] instead of stock `ratatui::Terminal`
...
pub(crate) enum ScreenMode {
    ...
    ///
    /// All minimal-mode rendering lives in the sibling `ainxt-pager-minimal`
...
impl ScreenMode {
    ...
    /// allowlist (`ainxt-telemetry`'s `KNOWN_SCREEN_MODES`); renaming one
...
pub fn resolve_use_leader(
    ...
    _remote_settings: Option<&ainxt_shell::util::config::RemoteSettings>,
    eligible: bool,
...
pub fn join_early_prefetch(
    handle: Option<ainxt_shell::agent::models::EarlyPrefetchHandle>,
) -> Option<ainxt_shell::util::config::RemoteSettings> {
    let handle = handle?;
...
/// should print a message telling the user to relaunch `ainxt`.
pub async fn run(
    args: PagerArgs,
    bg_update_rx: Option<
        tokio::sync::oneshot::Receiver<Option<ainxt_update::auto_update::UpdateAvailable>>,
    >,
) -> anyhow::Result<bool> {
    ainxt_tty_utils::redirect_native_stderr();
    ...
    let ainxt_com_config =
        match ainxt_shell::agent::config::Config::new_from_toml_cfg(&raw_config) {
            Ok(c) => c.ainxt_com_config,
            Err(e) => {
                tracing::warn!(
                    error = % e, "failed to parse config for auth refresh, using defaults"
                );
                ainxt_shell::auth::AinxtComConfig::default()
            }
        };
    let refreshed_auth = ainxt_shell::auth::try_ensure_fresh_auth(&ainxt_com_config).await;
    let early_prefetch =
        ainxt_shell::agent::models::start_early_prefetch_with_auth(refreshed_auth);
    ...
    ainxt_shell::util::config::cache_remote_auto_mode(
    ...
    if let Some(reason) = policy_disable_reason {
        tokio::spawn(ainxt_shell::leader::kill_stale_reachable_leaders(reason));
    }
    if let Some(err) =
    ...
    {
        let summaries = ainxt_shell::session::persistence::list_summaries(None).await?;
    ...
    let env_hunk_tracker_mode = std::env::var("AINXT_HUNK_TRACKER").ok();
    ...
    let launch_auto = ainxt_shell::util::config::effective_auto_for_launch(
        args.yolo,
        args.permission_mode_flag.as_deref(),
        remote_permission_mode,
    );
    ...
    cancel.cancel();
    ainxt_tty_utils::global_process_scope().kill_all();
    match result {
        Ok(run_result) => {
...
fn print_exit_resume_hint(session_id: &str, minimal: bool, w: &mut impl Write) {
    ...
    if minimal {
        let _ = writeln!(w, "  ainxt --minimal --resume {session_id}");
    } else {
        let _ = writeln!(w, "  ainxt --resume {session_id}");
    }
}
...
fn disable_mouse_paste_raw() {
    ainxt_shell::util::with_locked_stderr(|stderr| {
        let _ = stderr.write_all(ainxt_crash_handler::terminal::MOUSE_PASTE_RESET);
        let _ = stderr.flush();
    });
}
...
pub(crate) mod win_native_selection {
    ...
    /// reporting off, and window-resize events on (equivalence with the capture
...
fn init_terminal(
    ...
    ainxt_crash_handler::enable_terminal_escape_restore();
    ...
    (move || -> io::Result<(PagerTerminal, ScreenMode)> {
        ...
        let mouse_leaks = crate::terminal::terminal_context().mouse_reporting_leaks_as_raw_text();
        ainxt_shell::util::with_locked_stderr(|stderr| {
            if !want_minimal && !mouse_leaks {
                execute!(stderr, event::EnableMouseCapture)?;
            } else if want_minimal && mouse_leaks {
                let _ = stderr.write_all(ainxt_crash_handler::terminal::MOUSE_TRACKING_RESET);
            }
            execute!(
                stderr,
        ...
        MOUSE_CAPTURE_ENABLED.store(!want_minimal && !mouse_leaks, Ordering::Release);
        set_panic_hook(mode);
        ...
        if mode.is_fullscreen() {
            ...
            Ok((
                ainxt_ratatui_inline::Terminal::new(backend)?,
            ...
            if let Ok(term) = ainxt_ratatui_inline::Terminal::with_options(
                probe_backend,
            ...
            if want_minimal {
                ...
                MOUSE_CAPTURE_ENABLED.store(!mouse_leaks, Ordering::Release);
            ...
            let backend =
                CrosstermBackend::new(crate::render::draw::TermWriter::new(frame_tx, writer_sync));
            let term = ainxt_ratatui_inline::Terminal::with_options(
                backend,
                ratatui::TerminalOptions {
    ...
    .inspect_err(|_| {
        ...
        ainxt_crash_handler::disable_terminal_escape_restore();
    })
}
...
/// BEL/ESC (titles can arrive from ainxt.dev conversation metadata) would
...
mod tests {
    ...
    fn terminal_title_strips_control_characters() {
        assert_eq!(
            ...
            "evil]52;c;payloadtitle - ainxt"
        );
        assert_eq!(terminal_title_string("\x07\x1b\x00"), "ainxt");
        assert_eq!(terminal_title_string(""), "ainxt");
        assert_eq!(terminal_title_string("My chat"), "My chat - ainxt");
    }
    #[test]
    ...
    fn cli_leader_and_no_leader_conflict() {
        let result = try_parse_pager(&["ainxt-pager", "--leader", "--no-leader"]);
        assert!(result.is_err());
    }
    #[test]
    fn cli_leader_flag_parses() {
        let args = try_parse_pager(&["ainxt-pager", "--leader"]).unwrap();
        assert!(args.leader);
        assert!(!args.no_leader);
    }
    #[test]
    fn cli_no_leader_flag_parses() {
        let args = try_parse_pager(&["ainxt-pager", "--no-leader"]).unwrap();
        assert!(!args.leader);
        assert!(args.no_leader);
    }
    #[test]
    fn cli_neither_leader_flag_defaults_false() {
        let args = try_parse_pager(&["ainxt-pager"]).unwrap();
        assert!(!args.leader);
        assert!(!args.no_leader);
    }
    #[test]
    ...
    fn cli_top_level_leader_with_agent_subcommand_parses_flag() {
        let args = try_parse_pager(&["ainxt-pager", "--leader", "agent"]).unwrap();
        assert!(args.leader);
        assert!(matches!(args.command, Some(Command::Agent(_))));
    }
    #[test]
    fn cli_top_level_no_leader_with_agent_subcommand_parses_flag() {
        let args = try_parse_pager(&["ainxt-pager", "--no-leader", "agent"]).unwrap();
        assert!(args.no_leader);
        assert!(matches!(args.command, Some(Command::Agent(_))));
    }
    #[test]
    ...
    fn remote_settings_leader_mode_true_enables_leader() {
        let rs = ainxt_shell::util::config::RemoteSettings {
    ...
    fn cli_short_r_no_id_sets_empty_sentinel() {
        let args = try_parse_pager(&["ainxt-pager", "-r"]).unwrap();
        assert_eq!(args.resume_session.as_deref(), Some(""));
        assert!(args.resume_most_recent());
    }
    #[test]
    ...
    fn cli_continue_conflicts_with_resume() {
        let result = try_parse_pager(&["ainxt-pager", "--continue", "--resume", "abc"]);
        assert!(result.is_err());
    }
    #[test]
    fn cli_continue_conflicts_with_load() {
        let result = try_parse_pager(&["ainxt-pager", "--continue", "--load", "abc"]);
        assert!(result.is_err());
    }
    #[test]
    ...
    fn cli_chat_flag_rejected_without_feature() {
        assert!(try_parse_pager(&["ainxt-pager", "--chat"]).is_err());
    }
    #[test]
    ...
    fn cli_worktree_flag_parses() {
        let args = try_parse_pager(&["ainxt-pager", "--worktree"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
    }
    #[test]
    fn cli_worktree_short_w_parses() {
        let args = try_parse_pager(&["ainxt-pager", "-w"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
    }
    #[test]
    fn cli_worktree_with_label() {
        let args = try_parse_pager(&["ainxt-pager", "-w", "my-label"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some("my-label"));
    }
    #[test]
    fn cli_worktree_long_with_label() {
        let args = try_parse_pager(&["ainxt-pager", "--worktree", "fix-bug"]).unwrap();
    ...
    fn cli_worktree_with_resume_parses() {
        let args = try_parse_pager(&["ainxt-pager", "-w", "--resume", "abc"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
        assert_eq!(args.session_to_resume(), Some("abc"));
    }
    #[test]
    fn cli_worktree_label_with_resume() {
        let args = try_parse_pager(&["ainxt-pager", "-w", "my-label", "--resume", "abc"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some("my-label"));
        assert_eq!(args.session_to_resume(), Some("abc"));
    }
    #[test]
    ...
    fn cli_session_id_parses() {
        let args = try_parse_pager(&["ainxt-pager", "--session-id", "my-id"]).unwrap();
    ...
    fn cli_session_id_short_s_parses() {
        let args = try_parse_pager(&["ainxt-pager", "-s", "my-id"]).unwrap();
        assert_eq!(args.session_id.as_deref(), Some("my-id"));
    }
    #[test]
    fn cli_session_id_with_resume_requires_fork() {
        let args = try_parse_pager(&["ainxt-pager", "-s", "a", "--resume", "b"]).unwrap();
        assert!(args.session_startup_intent().is_err());
    }
    #[test]
    fn cli_session_id_with_continue_requires_fork() {
        let args = try_parse_pager(&["ainxt-pager", "-s", "a", "--continue"]).unwrap();
        assert!(args.session_startup_intent().is_err());
    }
    #[test]
    fn cli_session_id_with_resume_and_fork_ok() {
        let args =
            try_parse_pager(&["ainxt-pager", "-s", "a", "--resume", "b", "--fork-session"]).unwrap();
    ...
    fn cli_no_alt_screen_flag_parses() {
        let args = try_parse_pager(&["ainxt-pager", "--no-alt-screen"]).unwrap();
    ...
    #[test]
    fn cli_command_name_is_ainxt() {
        use clap::CommandFactory;
        assert_eq!(PagerArgs::command().get_name(), "ainxt");
    }
    #[test]
    fn cli_help_output_header() {
        ...
        assert_eq!(
            ...
            vec![
                "ainxt TUI",
                "",
                "Usage: ainxt [OPTIONS] [PROMPT] [COMMAND]",
                "",
                "Arguments:",
            ]
        );
    ...
    fn cli_completions_parses() {
        ...
        let args = try_parse_pager(&["ainxt-pager", "completions", "zsh"]).unwrap();
        assert!(matches!(
            args.command,
            Some(Command::Completions { shell: Shell::Zsh })
        ));
        let args = try_parse_pager(&["ainxt-pager", "completions", "bash"]).unwrap();
        assert!(matches!(
            args.command,
