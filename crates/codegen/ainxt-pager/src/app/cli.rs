...
pub enum Command {
    /// Run ainxt without the interactive UI
    Agent(Box<AgentArgs>),
    /// Show the configuration ainxt discovers for this directory
    Inspect {
    ...
    Logout,
    /// Sign in to ainxt (paste an access token by default, or --password for email/password)
    Login {
        /// Ignored (kept for backwards compatibility).
        #[arg(long, hide = true)]
        legacy: bool,
        /// Sign in with email + password instead of pasting a token.
        #[arg(long = "password", visible_alias = "email-login")]
        password: bool,
        /// Email address for --password sign-in (prompted if omitted).
        #[arg(long = "email")]
        email: Option<String>,
        /// Ignored (kept for backwards compatibility).
        #[arg(long = "oauth", alias = "oidc", hide = true)]
        oauth: bool,
        /// Ignored (kept for backwards compatibility).
        #[arg(long = "device-auth", visible_alias = "device-code", hide = true)]
        device_auth: bool,
    ...
    /// Inspect what the security policy permits and why
    Policy(crate::policy_cmd::PolicyArgs),
...
Examples:
  ainxt wrap docker exec -it my-container bash
  ainxt wrap kubectl exec -it my-pod -- bash

See ~/.ainxt/README.md for more information.
")]
    Wrap(WrapArgs),
    ...
    /// `AINXT_WORKSPACE_COMMAND=1` to enable it locally for testing.
    #[command(hide = true)]
    ...
    /// `~/.ainxt/config.toml` or when the `AINXT_AGENT_DASHBOARD=0` env
    /// var is set.
    Dashboard,
}
...
/// Targets a running leader process by PID (used by `ainxt leader` / `ainxt workspace`).
#[derive(Debug, clap::Args, Clone, Default)]
pub struct LeaderTargetArgs {
    /// Leader process ID from `ainxt leader list`.
    #[arg(long)]
    pub pid: Option<u32>,
}
#[derive(Debug, clap::Args, Clone)]
...
pub struct AgentArgs {
    ...
    /// API backend / wire protocol for all models: chat_completions (default),
    /// messages, or responses
    #[arg(long = "api-backend", value_name = "BACKEND")]
    ...
    /// Override the public ainxt API base URL.
    #[arg(long = "ainxt-api-base-url")]
    pub ainxt_api_base_url: Option<String>,
    /// Agent runtime mode
    #[command(subcommand)]
    ...
    pub fn canonical_plugin_dirs(&self) -> Vec<PathBuf> {
        self.plugin_dirs
            ...
            .filter_map(|p| match dunce::canonicalize(p) {
                ...
                Ok(_) => {
                    eprintln!(
                        "ainxt: --plugin-dir {}: not a directory; skipping",
                        p.display()
                    );
                    None
                }
                Err(e) => {
                    eprintln!("ainxt: --plugin-dir {}: {e}; skipping", p.display());
                    None
                }
            })
            .collect()
...
pub enum AgentCmd {
    ...
    Stdio,
    /// Run the agent headlessly over the ainxt WebSocket relay
...
pub struct HeadlessArgs {
    #[arg(long = "ainxt-ws-origin")]
    pub ainxt_ws_origin: Option<String>,
    #[arg(long = "ainxt-ws-url")]
    pub ainxt_ws_url: Option<String>,
}
...
pub struct ServeArgs {
    ...
    #[arg(long, env = "AINXT_AGENT_SECRET")]
...
pub struct LeaderArgs {
    ...
    /// Defer the ainxt.dev relay WebSocket until the first headless IPC client
...
fn version_with_channel() -> &'static str {
    ...
    V.get_or_init(|| {
        let label = ainxt_update::channel_label();
        ainxt_version::display_version_with_commit(env!("VERSION_WITH_COMMIT"), label)
    })
}
...
pub struct PagerArgs {
    ...
    #[arg(long)]
    pub cwd: Option<PathBuf>,
    /// Use a custom leader socket path instead of the default `~/.ainxt/leader.sock`.
    #[arg(
        long = "leader-socket",
        value_name = "PATH",
        global = true,
    ...
    /// API backend / wire protocol for all models: chat_completions (default),
    /// messages, or responses.
    #[clap(long = "api-backend", value_name = "BACKEND")]
    pub api_backend: Option<String>,
    /// Reasoning effort for reasoning models
    #[clap(
        long = "reasoning-effort",
    ...
    /// persists per-segment markdown to grep. Sets `AINXT_COMPACTION_MODE`.
    #[clap(long = "compaction-mode", value_name = "MODE", hide = true)]
    pub compaction_mode: Option<String>,
    /// Segments unchanged detail [none|minimal|balanced|verbose] (default
    /// `verbose`). Only affects `--compaction-mode segments`. Sets
    /// `AINXT_COMPACTION_DETAIL`.
    ...
    #[arg(
        ...
        value_parser = clap::builder::PossibleValuesParser::new(
            ainxt_shell::agent::config::PermissionMode::VALID_VALUES
        )
    )]
    ...
    /// Default for all `ainxt -p` runs is to wait (up to `--background-wait-timeout`)
    ...
    #[arg(long, env = "AINXT_SANDBOX", value_name = "PROFILE")]
    ...
    /// Session-scoped only — does not write config. To default plain `ainxt` to
    /// minimal, set `[ui] screen_mode = "minimal"` in ~/.ainxt/config.toml.
    #[arg(long = "minimal")]
    pub minimal: bool,
    ...
    pub fullscreen: bool,
    /// Write sampling events to ~/.ainxt/logs/sampling.jsonl.
    #[arg(long = "log-sampling", env = "AINXT_LOG_SAMPLING", hide = true)]
    ...
    pub no_leader: bool,
    /// Initial prompt for the interactive session, e.g. `ainxt "fix the bug"` or `ainxt --worktree=feat "create this feature"`.
    #[arg(
        value_name = "PROMPT",
        conflicts_with_all = &["single",
        "prompt_json",
...
impl PagerArgs {
    ...
    /// explicit `--sandbox`/`AINXT_SANDBOX` that differs from the saved profile is
    ...
    pub fn saved_resume_profile(&self) -> Option<String> {
        ...
        match self.resume_target() {
            ResumeTarget::SessionId(id) => {
                ainxt_shell::session::persistence::resumed_session_sandbox_profile(
                    Some(&id),
                    cwd,
                )
            }
            ResumeTarget::MostRecentForCwd => {
                ainxt_shell::session::persistence::resumed_session_sandbox_profile(None, cwd)
            }
    ...
    fn resolve_startup_sandbox(explicit: Option<&str>, saved: Option<String>) -> SandboxStartup {
        match (explicit, saved) {
            (Some(x), Some(s))
                if x.parse::<ainxt_sandbox::ProfileName>().ok()
                    != s.parse::<ainxt_sandbox::ProfileName>().ok() =>
            {
    ...
    /// whitespace. This is the `ainxt "<prompt>"` launch form; the headless
...
mod tests {
    ...
    fn version_flag_exits_zero() {
        let err = PagerArgs::try_parse_from(["ainxt", "--version"]).unwrap_err();
    ...
    fn version_short_flag_exits_zero() {
        let err = PagerArgs::try_parse_from(["ainxt", "-v"]).unwrap_err();
        assert_eq!(err.kind(), clap::error::ErrorKind::DisplayVersion);
        assert!(
            err.exit_code() == 0,
            "-v must exit 0; got {}",
    ...
    fn resume_target_classifies_flags() {
        assert_eq!(
            PagerArgs::try_parse_from(["ainxt"]).unwrap().resume_target(),
            ResumeTarget::None
        );
        assert_eq!(
            PagerArgs::try_parse_from(["ainxt", "-c"])
                .unwrap()
                .resume_target(),
            ResumeTarget::MostRecentForCwd
        );
        assert_eq!(
            PagerArgs::try_parse_from(["ainxt", "--resume"])
                .unwrap()
                .resume_target(),
            ResumeTarget::MostRecentForCwd
        );
        assert_eq!(
            PagerArgs::try_parse_from(["ainxt", "--resume", "sess-1"])
                .unwrap()
                .resume_target(),
            ResumeTarget::SessionId("sess-1".to_string())
        );
        assert_eq!(
            PagerArgs::try_parse_from(["ainxt", "-s", "sess-2"])
                .unwrap()
                .resume_target(),
            ResumeTarget::None
        );
        assert_eq!(
            PagerArgs::try_parse_from(["ainxt", "-r", "old", "--fork-session"])
                .unwrap()
                .resume_target(),
    ...
    fn minimal_and_fullscreen_flags_conflict() {
        let args = PagerArgs::try_parse_from(["ainxt", "--minimal"]).unwrap();
        assert!(args.minimal && !args.fullscreen);
        let args = PagerArgs::try_parse_from(["ainxt", "--fullscreen"]).unwrap();
        assert!(args.fullscreen && !args.minimal);
        let err = PagerArgs::try_parse_from(["ainxt", "--minimal", "--fullscreen"]).unwrap_err();
    ...
    fn startup_sandbox_profile_no_resume() {
        assert_eq!(
            PagerArgs::try_parse_from(["ainxt", "--sandbox", "strict"])
                .unwrap()
                .startup_sandbox_profile(None),
            SandboxStartup::Apply(Some("strict".to_string()))
        );
        assert_eq!(
            PagerArgs::try_parse_from(["ainxt", "--sandbox", ""])
                .unwrap()
                .startup_sandbox_profile(None),
            SandboxStartup::Apply(None)
        );
        assert_eq!(
            PagerArgs::try_parse_from(["ainxt"])
                .unwrap()
                .startup_sandbox_profile(None),
            SandboxStartup::Apply(None)
        );
    ...
    fn leader_socket_flag_parses_at_root() {
        let args = PagerArgs::try_parse_from(["ainxt", "--leader-socket", "/tmp/leader-x.sock"])
    ...
    fn leader_socket_flag_defaults_to_none() {
        let args = PagerArgs::try_parse_from(["ainxt"]).expect("bare ainxt parses");
        assert!(args.leader_socket.is_none());
    }
    #[test]
    fn leader_mgmt_list_info_kill_parse() {
        let list = PagerArgs::try_parse_from(["ainxt", "leader", "list", "--json"])
            .expect("ainxt leader list --json");
        assert!(matches!(
            list.command,
        ...
        let info = PagerArgs::try_parse_from(["ainxt", "leader", "info", "--pid", "42"])
            .expect("ainxt leader info --pid");
        assert!(matches!(
            info.command,
            Some(Command::Leader(LeaderMgmtArgs {
        ...
        let kill = PagerArgs::try_parse_from(["ainxt", "leader", "kill"]).expect("ainxt leader kill");
        assert!(matches!(
            kill.command,
            Some(Command::Leader(LeaderMgmtArgs {
        ...
        assert!(PagerArgs::try_parse_from(["ainxt", "leader", "profile"]).is_err());
    }
    #[test]
    fn debug_file_flag_parses_and_is_global() {
        let root = PagerArgs::try_parse_from(["ainxt", "--debug-file", "/tmp/fire.txt"])
        ...
        let sub =
            PagerArgs::try_parse_from(["ainxt", "agent", "stdio", "--debug-file", "/tmp/f.txt"])
    ...
    fn worktree_flag_and_initial_prompt_combine() {
        ...
        let c = PagerArgs::try_parse_from(["ainxt", "-w", "x"]).expect("-w x parses");
        assert_eq!(c.worktree.as_deref(), Some("x"));
        assert_eq!(c.initial_prompt(), None);
    }
    #[test]
    fn trust_flag_parses_on_pager_and_alias() {
        let bare = PagerArgs::try_parse_from(["ainxt"]).expect("bare ainxt parses");
        assert!(!bare.trust);
        let long = PagerArgs::try_parse_from(["ainxt", "--trust"]).expect("--trust parses");
        assert!(long.trust);
        let alias =
            PagerArgs::try_parse_from(["ainxt", "--trust-folder"]).expect("--trust-folder parses");
        assert!(alias.trust);
    }
    #[test]
    fn reasoning_effort_and_effort_alias_parse_same_field() {
        let long = PagerArgs::try_parse_from(["ainxt", "--reasoning-effort", "high"])
            .expect("--reasoning-effort parses");
        assert_eq!(long.reasoning_effort.as_deref(), Some("high"));
        let alias =
            PagerArgs::try_parse_from(["ainxt", "--effort", "high"]).expect("--effort alias parses");
        assert_eq!(alias.reasoning_effort.as_deref(), Some("high"));
    }
    #[test]
    fn reasoning_effort_accepts_max_and_remapped_ids() {
        let max = PagerArgs::try_parse_from(["ainxt", "--effort", "max"]).expect("max parses");
        assert_eq!(max.reasoning_effort.as_deref(), Some("max"));
        let deep =
            PagerArgs::try_parse_from(["ainxt", "--reasoning-effort", "deep"]).expect("deep parses");
        assert_eq!(deep.reasoning_effort.as_deref(), Some("deep"));
    }
    #[test]
    fn reasoning_effort_last_flag_wins_when_both_names_set() {
        let args =
            PagerArgs::try_parse_from(["ainxt", "--reasoning-effort", "low", "--effort", "high"])
                .expect("both effort flag names parse");
        assert_eq!(args.reasoning_effort.as_deref(), Some("high"));
        let reverse =
            PagerArgs::try_parse_from(["ainxt", "--effort", "high", "--reasoning-effort", "low"])
    ...
    fn agent_args_effort_alias_parses() {
        let args = PagerArgs::try_parse_from(["ainxt", "agent", "--effort", "max", "stdio"])
