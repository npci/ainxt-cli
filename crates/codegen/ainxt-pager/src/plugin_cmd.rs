//! `ainxt plugin` CLI subcommand — manage plugins and marketplace sources.
//!
...
//!
//! Business logic lives in `ainxt_shell::plugin` (shared orchestration)
//! and lower crates (`ainxt-agent`, `ainxt-plugin-marketplace`). This
...
use ainxt_agent::plugins::install_registry::{InstallKind, InstallRegistry};
use ainxt_agent::plugins::manifest::{ManifestLoadResult, PluginManifest, load_manifest};
use ainxt_plugin_marketplace::SourceKind;
use ainxt_plugin_marketplace::git::SourceCacheLease;
use ainxt_shell::plugin::{self, RepoUpdateOutcome, UninstallError};

// ── JSON output types ───────────────────────────────────────────────

/// Typed entry for `ainxt plugin list --json`. The `status` field acts as a
...
/// Typed entry for `ainxt plugin marketplace list --json`.
#[derive(Serialize)]
...
pub enum PluginCommand {
    ...
    Uninstall {
        /// Plugin name (as shown by `ainxt plugin list`).
        name: String,
...
fn trust_prompt(subject: &str, source_arg: &str) -> String {
    format!(
        "Installing {subject} requires confirmation.\n\
         ...
         \n\
         To proceed, re-run with --trust:\n  ainxt plugin install {source_arg} --trust"
    )
}
...
fn cmd_list(json: bool, available: bool) -> Result<()> {
    ...
    if json {
        ...
        println!("No plugins installed. Run `ainxt plugin install --help` to get started.");
    } else {
...
fn installed_plugins(
    repos: &[(
        &str,
        &ainxt_agent::plugins::install_registry::InstalledRepo,
    )],
) -> Vec<PluginEntry> {
    repos
        .iter()
...
fn available_plugins(registry: &InstallRegistry) -> Vec<PluginEntry> {
    let config = ainxt_shell::config::load_effective_config()
        .ok()
        .unwrap_or(toml::Value::Table(toml::map::Map::new()));
    let mut sources = ainxt_plugin_marketplace::load_sources(&config);
    sources.extend(ainxt_plugin_marketplace::load_extra_sources_from_settings(&sources));
    ...
    for source in &sources {
        ...
        for plugin in ainxt_plugin_marketplace::scan_marketplace(&root).entries {
            let already_installed =
                ainxt_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    registry,
                    &identity,
...
fn source_identity(source: &ainxt_plugin_marketplace::MarketplaceSource) -> String {
    match &source.kind {
...
fn resolve_marketplace_root(
    ...
    match &source.kind {
        ...
        SourceKind::Git { url, branch } => {
            let cache = ainxt_plugin_marketplace::git::default_cache_root();
            ainxt_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache,
...
fn install_kind(is_git: bool) -> ainxt_telemetry::events::InstallKind {
    if is_git {
        ainxt_telemetry::events::InstallKind::Git
    } else {
        ainxt_telemetry::events::InstallKind::Local
    }
}

fn log_plugin_installed(
    install_kind: ainxt_telemetry::events::InstallKind,
    success: bool,
    error_category: Option<String>,
) {
    ainxt_telemetry::session_ctx::log_event(ainxt_telemetry::events::PluginInstalled {
        install_kind,
        success,
        trust: true,
        error_category,
...
fn cmd_install(source: &str, trust: bool) -> Result<()> {
    if let Some(mref) = ainxt_plugin_marketplace::install_resolve::parse_marketplace_ref(source)
    {
    ...
    if !trust {
        use ainxt_agent::plugins::git_install::{self, InstallSource};
...
fn cmd_install_marketplace(
    source: &str,
    mref: &ainxt_plugin_marketplace::install_resolve::MarketplaceRef,
    trust: bool,
) -> Result<()> {
    if !trust {
            ...
            if outcome.already_installed {
                ...
                println!(
                    "Plugin \"{}\" is already installed from {}. \
                     Run `ainxt plugin update {}` to update it.",
    ...
    match plugin::uninstall_plugin(name, confirm, keep_data) {
        Ok(outcome) => {
            ainxt_telemetry::session_ctx::log_event(
                ainxt_telemetry::events::PluginUninstalled {
                    confirmed: true,
                    success: true,
                },
            );
        ...
        Err(UninstallError::NeedsConfirm {
            ...
            "Plugin \"{name}\" belongs to repo \"{repo_key}\" which also contains:\n\
             ...
             Uninstalling will remove all {total} plugin(s). To proceed:\n\
               ainxt plugin uninstall {name} --confirm",
            other_plugins
                .iter()
                .map(|p| format!("  - {p}"))
                .collect::<Vec<_>>()
    ...
    if registry.find_plugin(name).is_none() {
        bail!(
            "Plugin \"{name}\" not found.\n\
               Run `ainxt plugin list` to see installed plugins."
        );
    }
    if let Err(e) = ainxt_shell::config::remove_disabled_plugin(name) {
        tracing::warn!("failed to remove from disabled list: {e}");
    }
    ainxt_shell::config::add_enabled_plugin(name)
...
fn cmd_disable(name: &str) -> Result<()> {
    ...
    if let Err(e) = ainxt_shell::config::remove_enabled_plugin(name) {
        tracing::warn!("failed to remove from enabled list: {e}");
    }
    ainxt_shell::config::add_disabled_plugin(name)
...
fn cmd_validate(path: &str) -> Result<()> {
    ...
    match load_manifest(&root) {
        ...
        Ok(ManifestLoadResult::NotFound) => {
            println!(
                "No plugin.json found. ainxt discovers skills, agents, and hooks \
...
fn cmd_tag(path: &str, push: bool, force: bool, dry_run: bool) -> Result<()> {
    ...
    let version = match load_manifest(&root) {
        Ok(ManifestLoadResult::Found(m)) => m.version.ok_or_else(|| {
            anyhow::anyhow!(
                "No `version` field in plugin.json. Set a version to use `ainxt plugin tag`."
            )
        })?,
    ...
    if json {
        ...
        println!(
            "No marketplace sources configured.\n\
             Run `ainxt plugin marketplace add --help` to get started."
        );
    } else {
        for s in sources {
            let id = match &s.kind {
...
fn marketplace_add(
    ...
    use ainxt_shell::plugin::MarketplaceAddInput;
...
fn marketplace_remove(
    ...
    let local_input = match plugin::classify_marketplace_add_input(url, &cwd) {
        ainxt_shell::plugin::MarketplaceAddInput::LocalPath(p) => Some(p),
        _ => None,
    };
...
fn marketplace_update(
    ...
    marketplace_update_with_cache_root(
        sources,
        name,
        &ainxt_plugin_marketplace::git::default_cache_root(),
    )
}
...
fn marketplace_update_with_cache_root(
    ...
    for source in sources {
        ...
        if let SourceKind::Git { url, branch } = &source.kind {
            match ainxt_plugin_marketplace::git::force_sync_source_cache(
                url,
                branch.as_deref(),
                cache_root,
            ) {
...
mod tests {
    use super::*;
    use ainxt_plugin_marketplace::MarketplaceSource;

    #[test]
    fn trust_prompt_marketplace_has_no_error_framing() {
        let msg = trust_prompt(
            "\"sentry\" from marketplace \"ainxt Official\"",
            "sentry@ainxt-org/plugin-marketplace",
        );
        assert!(
            msg.starts_with(
                "Installing \"sentry\" from marketplace \"ainxt Official\" requires confirmation."
            ),
            "{msg}"
        );
        assert!(msg.contains("hooks, MCP servers, and skills"));
        assert!(msg.contains(
            "To proceed, re-run with --trust:\n  ainxt plugin install sentry@ainxt-org/plugin-marketplace --trust"
        ));
    ...
    fn trust_prompt_git_and_local_subjects() {
        ...
        assert!(git.ends_with("  ainxt plugin install u/r --trust"), "{git}");
    ...
    fn marketplace_update_force_syncs_fresh_git_cache() {
        ...
        let cache_dir = ainxt_plugin_marketplace::git::sync_source_cache(
            &url,
            Some("main"),
            cache_root.path(),
        )
    ...
    fn current_head(repo: &Path) -> String {
        let output = ainxt_plugin_marketplace::git::git_command()
            .current_dir(repo)
            .args(["rev-parse", "HEAD"])
            .output()
            .unwrap();
