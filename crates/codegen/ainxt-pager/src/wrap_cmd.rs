//! `ainxt wrap` — run any command in a local PTY that forwards its clipboard.
//!
//! Generalizes the `ainxt ssh` wrapper: spawns an arbitrary command inside a
...
//! run — a single shell-quoted string (`ainxt wrap "mycli ssh host"`) or a shell
...
pub fn run(args: &WrapArgs) -> Result<()> {
    ...
    let program = args
        .command
        .first()
        .ok_or_else(|| anyhow::anyhow!("ainxt wrap: no command given"))?;
    ...
    let (wrapped, fallback) = {
        ...
        let in_path = ainxt_config::shell::is_command_available(program);
        (
    ...
    if should_wrap() {
        match crate::pty_wrap::run_wrapped_command(&wrapped.program, &wrapped.args) {
            ...
            Err(e) => {
                ...
                eprintln!("ainxt wrap: wrapped mode failed, running without PTY wrapping: {e}");
...
/// The program and argv `ainxt wrap` will actually spawn.
#[derive(Clone)]
struct SpawnPlan {
    program: String,
    args: Vec<String>,
...
fn derive_spawn(
    ...
    // (`ainxt wrap "mycli ssh host"`), not a program name: hand it to the shell
    // unchanged so it does word-splitting, alias expansion, pipes, etc.
    ...
    // name, and an empty one (`ainxt wrap "$PROG" ...` with `$PROG` unset)
...
/// deliberately not `ainxt_config::shell::unix_shell_path`, which coerces
...
///
/// Unlike `ainxt ssh`, `ainxt wrap` does not gate on the terminal brand: the user
