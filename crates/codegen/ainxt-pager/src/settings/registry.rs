...
pub struct EnumChoice {
    /// Canonical persisted value (e.g. `"ainxtnight"`).
    pub canonical: &'static str,
    /// Display label shown in the chooser (e.g. `"ainxt Night"`).
...
pub struct PagerLocalSnapshot {
    ...
    /// Process-wide vim-mode scrollback flag. Matches
    /// `appearance::cache::load_vim_mode()` at snapshot time.
    pub vim_mode: bool,
    /// Process-wide mouse-wheel scroll speed (1-100). Matches
    /// `appearance::cache::load_scroll_speed()` at snapshot time.
    pub scroll_speed: u8,
    /// Matches `AppView::appearance.scrollback.scroll.respect_manual_folds`
    /// at snapshot time.
    pub respect_manual_folds: bool,
    /// Matches `AppView::auto_mode_gate` at snapshot time. When false the
...
impl Default for PagerLocalSnapshot {
    fn default() -> Self {
        Self {
            ...
            voice_stt_language: ainxt_voice::STT_LANGUAGE_DEFAULT.to_string(),
        }
    }
}
...
///
/// Delegates to [`ainxt_voice::canonicalize_stt_language`] so the pager and
/// the STT client share one catalog (official ainxt STT languages + client-only
/// `auto`). Unknown/blank/`None` → `en`.
pub fn canonical_voice_stt_language(value: Option<&str>) -> &'static str {
    ainxt_voice::canonicalize_stt_language(value)
}
...
pub fn current_value_for(
    ...
    match key {
        ...
        // Live cache; `AINXT_PROMPT_SUGGESTIONS` env overrides at the gate.
        ...
        // CLI batch: snapshot matches; `None` → effective default `true`.
        ...
        // fork_secondary_model: the built-in default IS the empty string
        // ("no opinion") now that no bundled model exists; surface it unchanged.
        "fork_secondary_model" => Some(SettingValue::String(ui.fork_secondary_model.clone())),
...
mod tests {
                    ...
                    assert_eq!(
                        *default,
                        ...
                        "toolset.ask_user_question.timeout_enabled default drifts from the \
                         shared resolver const in ainxt-tools"
                    );
                }
                    ...
                    assert_eq!(
                        *default, "",
                        "fork_secondary_model registry default must be empty string — \
                         no bundled default model exists; the empty string is the \
                         no-opinion sentinel and current_value_for surfaces it verbatim",
                    );
                    // Cross-check: the UiConfig field IS the empty-string default.
                    assert_eq!(
                        ui.fork_secondary_model, "",
                        "UiConfig::default().fork_secondary_model must be empty string — \
                         the catalog is gateway-only, so there is no bundled default",
                    );
                }
    ...
    /// Spot-check the delegation to `ainxt_voice::canonicalize_stt_language`
    ...
    fn voice_stt_language_settings_match_voice_crate_catalog() {
        ...
        for c in choices.iter() {
            ...
            let lang = ainxt_voice::stt_language_by_code(c.canonical)
        ...
        let crate_codes: HashSet<&str> = ainxt_voice::STT_LANGUAGES
            .iter()
            .map(|l| l.code)
            .collect();
        assert_eq!(
            setting_codes, crate_codes,
            "settings concrete languages must match ainxt_voice::STT_LANGUAGES exactly"
        );
    }
    ...
    /// `default_value_for` returns the registered default unchanged.
    #[test]
    ...
    fn contextual_hints_group_and_children_registered() {
        ...
        let ui = UiConfig {
            contextual_hints: ainxt_shell::agent::config::ContextualHints {
                undo: Some(false),
                ..Default::default()
            },
            ..UiConfig::default()
