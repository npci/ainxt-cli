//! Unit tests for the `ainxt wrap` spawn planning in [`super`] (`wrap_cmd`),
...
fn single_arg_with_whitespace_always_routes_via_shell() {
    ...
    // arg as a command line to hand to the shell unchanged.
    let plan = derive_spawn(
        &cmd(&["x\tssh\nhost"]),
        "/bin/sh",
        false,
...
fn direct_route_passes_args_verbatim_without_quoting() {
    ...
    // program must receive its argv elements exactly.
    let plan = derive_spawn(
        &cmd(&["rsync", "a b", "don't"]),
        "/bin/zsh",
        true,
