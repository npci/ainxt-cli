...
ainxt supports **hooks** (event-driven shell commands) and **plugins** (bundles of skills, agents, hooks, and MCP servers). Both are managed through a unified modal interface.
...
- **Global hooks** — from `~/.ainxt/hooks/`
- **Project hooks** — from `.ainxt/hooks/` in your repo
...
Plugin hooks automatically receive `AINXT_PLUGIN_ROOT` and `AINXT_PLUGIN_DATA` environment variables (see the [Plugins guide](user-guide/09-plugins.md#environment-variables-in-plugin-hooks)).
...
2. **settings.json** — `extraKnownMarketplaces` from `~/.ainxt/settings.json` or `~/.claude/settings.json`
...
Press `a` on the Marketplace tab (or run `ainxt plugin marketplace add <source>`)
...
Sources land in `~/.ainxt/config.toml`:
...
Or in `~/.ainxt/settings.json` / `~/.claude/settings.json`:
...
- [Hook Examples](../../ainxt-hooks/examples/README.md) — ready-to-use sample hooks
