...
> **Status: alpha.** The schema below is versioned (`ainxt_code.schema.version = v1`);
> additive changes may occur without notice, renames/removals will bump the
> version and be called out in the changelog.

ainxt CLI can export usage **metrics** and **events** to your organization's
...
any data flowing through ainxt.
...
- **Structurally separate** from ainxt-internal telemetry: its exporters carry
  only the headers you configure, never ainxt credentials.
- **Independent of ainxt data-retention opt-outs**: it works even when
  `telemetry` is disabled and for ZDR (zero-data-retention) teams — those
  settings govern ainxt-side retention; the external stream is governed solely
  by your own OTEL configuration.

## Quick start

```bash
export AINXT_EXTERNAL_OTEL=1                  # master switch
...
`AINXT_EXTERNAL_OTEL=1` alone enables **nothing** — you must also select at
...
|---|---|---|
| `AINXT_EXTERNAL_OTEL` | `0` | Master switch. Distinct from `AINXT_TELEMETRY_ENABLED`, which controls ainxt-internal product analytics — the two govern opposite-pointing data flows. |
...
| `OTEL_EXPORTER_OTLP_LOGS_ENDPOINT` / `..._METRICS_ENDPOINT` | — | Signal-specific overrides, used unchanged. For gRPC these should normally be collector endpoints without `/v1/...` paths. |
...
| `OTEL_LOG_USER_PROMPTS` | `0` | Content gate: prompt text on `ainxt_code.user_prompt` (60 KB cap, secret-scrubbed). |
| `OTEL_LOG_TOOL_DETAILS` | `0` | Content gate: tool parameters (4 KB cap), full file paths, unchanged MCP/skill/plugin names. Bash command text is **never** exported in v1, even with this gate. |
...
> `AINXT_EXTERNAL_OTEL` is set, product analytics ignores those vars, and the
...
standard OTEL names** (`AINXT_EXTERNAL_OTEL`, `OTEL_*`) for ecosystem
...
the `[telemetry]` `otel_*` keys through `ainxt setup` managed config /
...
## Metrics (meter scope `ai.ainxt.ainxt_code`)

| Metric | Unit | Attributes |
|---|---|---|
| `ainxt_code.session.count` | `{session}` | base attrs only |
| `ainxt_code.token.usage` | `{token}` | `type` = `input` \| `output` \| `reasoning` \| `cache_read`; `model` |
| `ainxt_code.turn.count` | `{turn}` | `outcome` = `completed` \| `cancelled` \| `error`; `model` |
| `ainxt_code.tool.decision` | `{decision}` | `tool_name`, `decision` = `allow` \| `deny` \| `cancelled` \| `followup`, `access_kind`, `permission_mode` |
| `ainxt_code.tool.usage` | `{call}` | `tool_name`, `outcome` |
| `ainxt_code.error.count` | `{error}` | `error_category`, `model` |

There is no `cost.usage` metric: join `ainxt_code.token.usage` with your own
price sheet. `lines_of_code.count` and `active_time.total` are planned for a
later phase.

`tool_name` values: built-in tool names pass unchanged; MCP tools collapse to
...
|---|---|
| `ainxt_code.session_start` | `model`, `permission_mode`, `mcp_server_count`, `plugin_count`, `skill_count`, `hook_count`, `memory_enabled`, `is_git_repo`, `client_identifier` |
| `ainxt_code.session_end` | `duration_secs`, `turn_count`, `tool_call_count`, `compaction_count`, `model` |
| `ainxt_code.user_prompt` | `prompt_length`, `model`, `screen_mode?` (`fullscreen` \| `inline` \| `minimal` \| `headless` \| `other`); `prompt` (**prompts**) |
| `ainxt_code.turn_completed` | `outcome`, `duration_ms`, `tool_call_count`, `model`, `error_category?`, `cancellation_category?` |
| `ainxt_code.api_request` | `model`, `duration_ms`, `stop_reason?`, `input_tokens`, `output_tokens`, `reasoning_tokens`, `cache_read_tokens` |
| `ainxt_code.api_error` | `error_category`, `model`, `status_code?`, `duration_ms?` |
| `ainxt_code.tool_result` | `tool_name`, `outcome`, `success`, `duration_ms`, `file_extension`; `tool_parameters`, `file_path` (**details**) |
| `ainxt_code.tool_decision` | `tool_name`, `decision`, `access_kind`, `permission_mode`, `source` |
| `ainxt_code.mcp_server_connection` | `status`, `transport_type`, `duration_ms`, `tool_count?`, `error_type?`; `mcp_server.name` (**details**; collapsed to `mcp_server` otherwise) |
| `ainxt_code.permission_mode_changed` | `to_mode`, `trigger` |
| `ainxt_code.skill_activated` | `skill_source`; `skill.name` (**details**) |
| `ainxt_code.plugin_loaded` | `install_kind?`, `success`, `error_category?`; `plugin_name` (**details**) |
| `ainxt_code.compaction` | `duration_ms`, `tokens_before`, `tokens_after`, `model?` |
| `ainxt_code.subagent` | `phase` = `launched` \| `completed`, `subagent_type?`, `outcome?`, `duration_ms?` |
| `ainxt_code.auth` | `auth_method` |
| `ainxt_code.internal_error` | `error_type` (class only — no message, no location) |
| `ainxt_code.model_switched` | `from_model`, `to_model`, `success`, `error_code?` |
...
sum by (model, type) (rate(ainxt_code_token_usage_total[1h]))

# Sessions per team per day
sum by (team_id) (increase(ainxt_code_session_count_total[1d]))

# Tool-permission denial ratio
sum(rate(ainxt_code_tool_decision_total{decision="deny"}[1h]))
  / sum(rate(ainxt_code_tool_decision_total[1h]))
```
