...
Agent mode runs ainxt as an ACP (Agent Client Protocol) server for integration with IDEs, editors, and custom tooling. Unlike single-prompt mode (`ainxt -p`, which prints one response and exits), agent mode keeps a persistent process running and communicates through structured JSON-RPC messages.
...
These options belong to the `ainxt agent` command and apply to every mode. Pass them before the mode name, for example `ainxt agent --model ainxt-build stdio`. The `stdio` subcommand itself takes no options.

| Flag                       | Description                                                       |
| -------------------------- | ---------------------------------------------------------------- |
| `-m, --model <MODEL>`      | Set the model ID (for example, `ainxt-build`).                    |
...
```bash
ainxt agent serve --bind 127.0.0.1:2419 --secret <token>
```

Clients connect over WebSocket and authenticate with the secret token. If you omit `--secret`, the agent generates a token and prints it at startup; you can also supply one through the `AINXT_AGENT_SECRET` environment variable. The agent persists across reconnections, so a client can disconnect and later resume in-flight work.
...
Beyond the base ACP protocol, ainxt defines extension methods under the `ainxt.dev/` prefix for ainxt-specific functionality. These cover:

| Category                   | Prefix               | Examples                                         |
| -------------------------- | -------------------- | ------------------------------------------------ |
| **Filesystem**             | `ainxt.dev/fs/*`          | `list`, `exists`, `read_file`, `write_file`      |
| **Git**                    | `ainxt.dev/git/*`         | `status`, `stage`, `commit`, `diffs`, `discard`  |
| **Git Worktree**           | `ainxt.dev/git/worktree/*`| `create`, `remove`, `apply`, `list`, `gc`        |
| **Search**                 | `ainxt.dev/search/*`      | `fuzzy/open`, `fuzzy/change`, `content`          |
| **Terminal**               | `ainxt.dev/terminal/*`    | `create`, `kill`, `output`, `wait_for_exit`      |
| **Session Management**     | `ainxt.dev/session/*`     | `fork`, `resolve_local_for_worktree_resume`      |
| **Conversation & History** | `ainxt.dev/*`             | `prompt_history`, `rewind/*`, `compact_conversation` |
| **Authentication**         | `ainxt.dev/auth/*`        | `get_url`, `submit_code`                         |
| **Feedback & Telemetry**   | `ainxt.dev/*`             | `feedback`, `telemetry/*`                        |

The tables here show representative methods in each category. The `ainxt.dev/*` set is ainxt-specific and may expand across releases, so treat it as non-exhaustive and discover the available methods from the agent's `initialize` response.
...
| -------------------------- | ------------------------------------ |
| `ainxt.dev/search/fuzzy/status` | Fuzzy search results update          |
| `ainxt.dev/git/worktree/status` | Worktree creation progress           |
| `ainxt.dev/fs_notify`           | Filesystem change notification       |
| `ainxt.dev/fs/index`            | Full file index update               |
| `ainxt.dev/fs/index/delta`      | Incremental file index update        |
| `ainxt.dev/session_notification`| Session-specific updates (diff review, retry state, auto-compact) |
| `ainxt.dev/session/update`      | Session update (tool calls, content) |
