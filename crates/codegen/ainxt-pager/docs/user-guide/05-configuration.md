...
ainxt reads configuration from local config files, environment variables, and
...
2. **Environment variables** (e.g., `AINXT_API_KEY`, `AINXT_MEMORY`)
3. **config.toml** (`~/.ainxt/config.toml`)
...
Location: `~/.ainxt/config.toml`

If the file does not exist, ainxt uses built-in defaults. Specify only the values you want to override.
...
**Disable vim input mode**); ainxt writes your choice to `[ui] simple_mode` in
`config.toml`.
...
The setting can also be overridden with the `AINXT_DEFAULT_SELECTED_PERMISSION`
...
(`/settings` → **Vim scrollback navigation**). ainxt writes the change to
`[ui] vim_mode` in `~/.ainxt/config.toml` immediately and applies it to every
...
`ainxt` launches. Configure it from `/settings` → **Default screen mode**
...
`AINXT_SCROLL_SPEED`, `AINXT_SCROLL_MODE`, `AINXT_INVERT_SCROLL`
(`1`/`true`/`0`/`false`), and `AINXT_SCROLL_LINES`. Precedence: env var →
...
allowed_domains = ["docs.rs", "ainxt.dev"]           # override the built-in allowlist
```
...
(`AINXT_ASK_USER_QUESTION_TIMEOUT_ENABLED` /
`AINXT_ASK_USER_QUESTION_TIMEOUT_SECS`) → user config → managed →
...
env_key = "AINXT_API_KEY"               # env var(s) holding the API key; string or array (first set, non-empty wins)
...
Credential resolution: `api_key` > `env_key` > signed-in session token > `AINXT_API_KEY`.
...
MCP servers can also be configured per-project in `.ainxt/config.toml`. Project-scoped config contributes `[mcp_servers]`, `[plugins]`, and `[permission]` rules; other sections load only from `~/.ainxt/config.toml`.

Priority for `[mcp_servers]` and `[plugins]`: `.ainxt/config.toml` (current dir) > `<repo-root>/.ainxt/config.toml` > `~/.ainxt/config.toml`. `[permission]` rules are not overridden by priority; they merge across all files with `deny` > `ask` > `allow` (see [22-permissions-and-safety.md](22-permissions-and-safety.md)).

### Memory

Persist knowledge across sessions (requires `--experimental-memory` or `AINXT_MEMORY=1`).
...
[subagents.models]
explore = "ainxt-build"               # route to different models
```
...
`ainxt inspect` reports cells that still need session-start resolution as
...
The `[hints]` table holds small persisted UI preferences — mostly "stop asking me" opt-outs. ainxt writes these for you when you pick a "don't ask again" / "reset in config.toml" option in the TUI, but you can edit or remove them by hand. Deleting a key restores the default behavior.

`[hints]` is read from the **effective config merge** (same precedence as other settings): system managed → user `managed_config.toml` → user `config.toml` → user `requirements.toml` → system `requirements.toml`. Higher-priority layers override lower ones. The TUI only **writes** opt-outs to user `~/.ainxt/config.toml`.
...
enabled = true
items = ["action-required", "spinner", "activity", "session-name", "ainxt"]
```
...
| `title.items` | array | (see above) | Items shown in the title bar. Options: `action-required`, `spinner`, `activity`, `session-name`, `cwd`, `model`, `turn-timer`, `ainxt`. |
...
| ainxt Desktop | None (native) | N/A | N/A |
| Unknown | BEL | No | No |

When `method = "auto"`, ainxt detects the terminal brand and selects the best
...
`$AINXT_EVENT`, `$AINXT_MESSAGE`, and `$AINXT_SESSION_ID`.

```toml
# macOS native notification
[[ui.notifications.hooks]]
command = "terminal-notifier -title 'ainxt' -message '$AINXT_MESSAGE'"
events = ["turn_complete", "approval_required"]
only_unfocused = true
timeout_secs = 10
...
# Push to ntfy server
[[ui.notifications.hooks]]
command = "curl -s -d '$AINXT_MESSAGE' ntfy.sh/my-ainxt-alerts"
...
fires, try `condition = "always"` as a fallback. ainxt supports focus tracking
...
Set these only to point telemetry at your own infrastructure or to turn parts of it off. The built-in endpoint and credentials are managed by ainxt; leave them unset to use the defaults.
...
```toml
[telemetry]
otel_enabled = true                                       # external OTEL master switch (= AINXT_EXTERNAL_OTEL)
...
All environment variables are read at **runtime** unless marked `[BUILD-TIME]`
(those are baked into the binary via `option_env!()` during `cargo build` and
cannot be changed after compilation).

> **Quick reference:** [`env.example`](../../../../../env.example) at the repo root
> documents every variable with descriptions, defaults and ready-to-use values.
...
**Source a file (per-session):**
```sh
source env.example
```

**Inline for CI / one-off runs:**
```sh
AINXT_GATEWAY_URL=https://gateway.example.com \
AINXT_TOKEN=$SECRET \
ainxt -p "run the test suite" --output-format json
```
...
|----------|--------|---------|
| 1 | **CLI flags** | `--model`, `--api-backend`, `--sandbox`, `--always-approve` |
| 2 | **Environment variables** | `AINXT_GATEWAY_URL`, `AINXT_TOKEN`, `AINXT_URL_*` |
| 3 | **`~/.ainxt/config.toml`** | `[models] default`, `[features] telemetry` |
| 4 | **`requirements.toml`** | Org-managed, pushed by gateway (internal deployments) |
| 5 | **`managed_config.toml`** | System-level (`/etc/ainxt/` or `~/.ainxt/`) |
| 6 | **Built-in defaults** | — |

> **Important:** The most critical deployment settings — `AINXT_GATEWAY_URL`,
> `AINXT_TOKEN`, `AINXT_DEPLOYMENT_KEY`, `AINXT_URL_*`, `AINXT_MARKETPLACE_*`,
> `AINXT_ALLOW_INSECURE` — are **env-var only**. There are no CLI flags for them.
> Set them in your shell profile or source an env file.
...
|----------|-------------|---------|
| `AINXT_GATEWAY_URL` | **Primary config.** Gateway base URL — all API paths (`/ainxt/v1/api/...`) are derived from this automatically. | `http://localhost:8000/ainxt/v1/api` |
| `AINXT_API_BASE_URL` | Override the model API base URL independently of the gateway. | Derived from `AINXT_GATEWAY_URL` |
| `AINXT_CLI_CHAT_PROXY_BASE_URL` | Override the chat proxy base URL independently. | Derived from `AINXT_GATEWAY_URL` |
| `AINXT_MODELS_LIST_URL` | Override the model list endpoint (full URL). | Derived from gateway |
| `AINXT_ASSET_SERVER_URL` | Override the asset server URL. | `https://assets.ainxt.invalid` |
| `AINXT_FEEDBACK_BASE_URL` | Override the feedback submission endpoint. | Derived from gateway |
| `AINXT_MANAGED_CONFIG_URL` | URL for org-wide managed config pushed by the gateway. | — |
...
|----------|-------------|
| `AINXT_TOKEN` | Bearer token for non-interactive / CI use. Obtained via `ainxt login` or issued by your gateway admin. |
| `AINXT_DEPLOYMENT_KEY` | Enterprise deployment key. Takes priority over `AINXT_TOKEN` when set. |
| `AINXT_API_KEY` | API key (alternative to `AINXT_TOKEN` for some gateway configurations). |
| `AINXT_OAUTH_ENABLED` | Enable/disable OAuth2 browser login flow. Default: `true`. |
| `AINXT_OAUTH2_ISSUER` | OAuth2 provider issuer URL (e.g. `https://auth.example.com`). |
| `AINXT_OAUTH2_CLIENT_ID` | OAuth2 client ID registered with your identity provider. |
| `AINXT_OAUTH2_SCOPES` | OAuth2 scopes (comma-separated). |
| `AINXT_OAUTH2_PRINCIPAL_TYPE` | OAuth2 principal type (`user` or `team`). |
| `AINXT_OAUTH2_PRINCIPAL_ID` | OAuth2 principal ID. |
| `AINXT_OIDC_ISSUER` | OIDC issuer URL (alternative to OAuth2 config). |
| `AINXT_OIDC_CLIENT_ID` | OIDC client ID. |
| `AINXT_OIDC_SCOPES` | OIDC scopes (comma-separated). |
| `AINXT_OIDC_AUDIENCE` | OIDC audience claim. |
| `AINXT_AUTH_PROVIDER_COMMAND` | Path to an external auth binary that provides tokens. |
| `AINXT_AUTH_PROVIDER_LABEL` | Display name shown on the TUI login screen. |
| `AINXT_AUTH_TOKEN_TTL` | Token lifetime in seconds. |
| `AINXT_AUTH_EARLY_INVALIDATION_SECS` | Seconds before expiry to refresh the token (default: 300). |
...
| Variable | Description | Default |
|----------|-------------|---------|
| `AINXT_HOME` | Config and state directory. | `~/.ainxt` |
| `AINXT_WORKSPACE_HOME` | Per-project workspace state directory. | Derived from `AINXT_HOME` |

---

### UI / Branding URLs

These appear in billing prompts, the `/docs` command, and the `/usage` command.
Override them to point at your own portal or documentation — no rebuild needed.

| Variable | Default | Description |
|----------|---------|-------------|
| `AINXT_URL_SUBSCRIBE` | `https://ainxt.example.test/subscribe` | Subscription / upgrade page shown in billing prompts. |
| `AINXT_URL_USAGE` | `https://ainxt.example.test?_s=usage` | Pay-as-you-go / usage management page (`/usage manage`). |
| `AINXT_URL_DOCS` | *(unset)* | Developer docs root, opened by `/docs web`. Empty by default: this project publishes no hosted docs site, so `/docs web` reports that none is configured. Set it to your own if you host one. |
| `AINXT_URL_LEGAL` | `https://ainxt.example.test/legal` | Legal / terms of service page (shown in `/privacy`). |
| `AINXT_URL_PROMO` | `https://ainxt.example.test/promo` | Promo / announcement CTA URL. |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `AINXT_MARKETPLACE_SOURCE_URL` | `https://github.com/ainxt-org/plugin-marketplace.git` | Git URL of the plugin marketplace repository. |
| `AINXT_MARKETPLACE_ORG` | `ainxt-org/plugin-marketplace` | `owner/repo` slug for canonical URL matching. |
| `AINXT_OFFICIAL_MARKETPLACE_AUTO_REGISTER` | `true` | Auto-register the marketplace source on first run. |
...
|----------|--------|-------------|
| `AINXT_INSTALLER` | `gateway`, `gh-release`, `internal`, `npm` | Update channel. Auto-detected: `gateway` when `AINXT_GATEWAY_URL` is set, else `internal`. |
| `AINXT_DISABLE_AUTOUPDATER` | any non-empty | Disable the background update check entirely. |
| `AINXT_FORCE_UPDATE_CHECK` | any non-empty | Force an update check on this launch. |
| `AINXT_NPM_REGISTRY` | URL | Custom npm registry for npm-installed updates. |
...
| Variable | Values | Description |
|----------|--------|-------------|
| `AINXT_API_BACKEND` | `messages` (default), `chat_completions`, `responses` | Force the API wire protocol for all models. Can also be set via `--api-backend` flag or `[models] api_backend` in `config.toml`. |
...
| Variable | Description |
|----------|-------------|
| `AINXT_ALLOW_INSECURE=1` | Allow plaintext `http://` gateway connections. **Dev/local only — never production.** Credentials are sent in cleartext. |
| ~~`AINXT_TLS_INSECURE=1`~~ | **Removed — no longer implemented.** The runtime TLS-verification bypass was deleted from this codebase; setting this has no effect. Add your CA to the system trust store instead, which keeps server identity verified. |
| ~~`AINXT_TLS_INSECURE_QUIET=1`~~ | **Removed** along with `AINXT_TLS_INSECURE`. |
...
| Variable | Values | Description |
|----------|--------|-------------|
| `AINXT_SANDBOX` | `off`, `workspace`, `container`, or a custom profile name | Sandbox profile for tool execution. Also settable via `--sandbox` flag. |
| `AINXT_SANDBOX_AUTO_ALLOW_BASH` | `true` / `false` | Auto-approve bash commands in sandbox without prompting. |
| `AINXT_SANDBOX_PROFILE` | profile name | Sandbox profile name (alternative to `AINXT_SANDBOX`). |
...
| Variable | Description |
|----------|-------------|
| `AINXT_SUGGESTIONS` | Enable (`true`) or disable (`false`) prompt suggestions. |
| `AINXT_SUGGESTIONS_AI` | Enable AI-powered prompt suggestions. |
| `AINXT_WEB_FETCH` | Enable (`1`) or disable (`0`) the web_fetch tool. |
| `AINXT_WEB_FETCH_PROXY` | HTTP proxy for web fetch requests. |
| `AINXT_DISABLE_WEB_FETCH` | Disable web fetch entirely (any non-empty value). |
| `AINXT_COMPACTION_MODE` | Compaction mode: `auto` or `disabled`. |
| `AINXT_FOLDER_TRUST` | Folder trust mode: `auto`, `trusted`, or `untrusted`. |
| `AINXT_RESPECT_GITIGNORE` | Force gitignore filtering on (`1`) or off (`0`); overrides `[tools] respect_gitignore`. |
| `AINXT_LSP_TOOLS` | Enable the LSP tool (any non-empty value). |
| `AINXT_DOOM_LOOP_RECOVERY` | Enable doom-loop recovery (any non-empty value). |
| `AINXT_TWO_PASS_COMPACTION` | Enable two-pass compaction (any non-empty value). |
| `AINXT_AGENT` | Custom agent definition path or name. |
| `AINXT_FEEDBACK_ENABLED` | Enable/disable feedback submission. |
| `AINXT_AUTO_WAKE` | Enable auto-wake on new prompt (any non-empty value). |
...
| Variable | Description |
|----------|-------------|
| `AINXT_TELEMETRY_ENABLED` | Enable/disable telemetry. Values: `true`, `false`, `session-metrics`. |
| `AINXT_TELEMETRY_TRACE_UPLOAD` | Enable/disable session trace upload. |
| `AINXT_DEPLOYMENT_KEY` | Management API key for enterprise deployments. |
| `AINXT_INTERNAL_OTLP_TRACES_ENDPOINT` | Internal OTLP traces endpoint. |
| `AINXT_INTERNAL_OTLP_HEADERS` | Headers for the internal OTLP endpoint (e.g. auth token). |
| `AINXT_TRACE_UPLOAD_URL` | Trace upload base URL. |
| `AINXT_TRACE_UPLOAD_BUCKET` | GCS bucket for trace upload. |
| `AINXT_TRACE_UPLOAD_REGION` | GCS region for trace upload. |
| `AINXT_TRACE_UPLOAD_CREDENTIALS_FILE` | Path to GCS credentials file. |
| `OTEL_EXPORTER_OTLP_ENDPOINT` | Standard OTLP endpoint (external collector). |
| `OTEL_EXPORTER_OTLP_HEADERS` | Standard OTLP headers (e.g. `Authorization=Bearer <token>`). |
| `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT` | OTLP traces endpoint (overrides base endpoint for traces). |
| `OTEL_TRACES_EXPORTER` | OTLP traces exporter type (`otlp`, `console`, `none`). |
...
| Variable | Description |
|----------|-------------|
| `AINXT_DEBUG_LOG` | Enable debug logging. Set to `1` for the default path, or a file path to write there. |
| `AINXT_LOG_FILE` | Write logs to this specific file path. |
| `RUST_LOG` | Log level filter (e.g. `ainxt_shell=debug,ainxt_sampler=debug`). Controls both `AINXT_LOG_FILE` output and headless stderr. |

---

### Build-Time Variables (`option_env!()`)

These are set as environment variables **during `cargo build`** and baked into
the binary. They cannot be changed at runtime.

| Variable | Description |
|----------|-------------|
| `AINXT_BUILD_CLI_CHAT_PROXY_BASE_URL` | Compile-time default API proxy URL. |
| `AINXT_BUILD_API_BASE_URL` | Compile-time default model API URL. |
| `AINXT_BUILD_ASSET_SERVER_URL` | Compile-time default asset server URL. |
| `AINXT_VERSION` | Version string stamped into the binary. |
| `AINXT_TELEMETRY_BUILD_EVENTS_URL` | Telemetry endpoint (OSS builds: leave unset → telemetry disabled). |
| `AINXT_TELEMETRY_BUILD_EVENTS_API_KEY` | Telemetry API key (OSS builds: leave unset). |
| `AINXT_TELEMETRY_BUILD_MIXPANEL_TOKEN` | Mixpanel token (OSS builds: leave unset). |
| `SENTRY_DSN` | Sentry error reporting DSN (OSS builds: leave unset). |
| `AINXT_POLICY_AUTHORITY_HEX` | Ed25519 authority key for policy enforcement. **Setting this makes the build "managed" (internal).** OSS builds leave this unset → permissive engine, no enforcement. |
| `AINXT_POLICY_BUNDLE_B64` | Base64-encoded signed policy bundle embedded in the binary (optional). |
...
| Path | Description |
|------|-------------|
| `~/.ainxt/config.toml` | Main configuration file |
| `~/.ainxt/pager.toml` | TUI appearance configuration |
...
| `~/.ainxt/lsp.json` | LSP server configuration (user-scoped) |
| `~/.ainxt/logs/` | Internal log files (for example `unified.jsonl`, MCP server logs) |
| `.ainxt/config.toml` | Project-scoped MCP servers, plugins, and permission rules |
...
Some configuration can be set per-project by placing files in `.ainxt/` within your repository:

| File | What it configures |
|------|--------------------|
| `.ainxt/config.toml` | MCP servers, plugins, permission rules, and the `[mcp] max_output_bytes` tool-result cap (other sections load only from `~/.ainxt/config.toml`) |
| `.ainxt/skills/` | Project-specific skills |
| `.ainxt/hooks/` | Project-specific lifecycle hooks |
| `.ainxt/agents/` | Project-specific agent definitions |
| `.ainxt/lsp.json` | LSP server configuration |
| `.ainxt/sandbox.toml` | Custom sandbox profiles |
...
|--------|----------|-------|
| User | `~/.ainxt/lsp.json` | All projects |
| Project | `.ainxt/lsp.json` | Current repository |
...
1. **Project** -- `.ainxt/lsp.json`
2. **User** -- `~/.ainxt/lsp.json`
