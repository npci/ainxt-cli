...
- **`ainxt dashboard`** — launches the TUI directly into the dashboard.
...
- **Ctrl+\\** — same as the slash command, two keystrokes. Configurable
  in `~/.ainxt/config.toml` under `[keybindings]` like every other shortcut.
...
```
 ainxt · Dashboard — 4 agents · 2 awaiting
...
The state icon matches ainxt's sibling views (
`tasks_pane`):
...
The registry-lookup rules and the mouse-event intercept matrix are not
restated here; the keybinding registry in the source
(`When::DashboardFocused`, below) is the authoritative list.

All shortcuts are registered under `When::DashboardFocused` and can be
rebound via `~/.ainxt/config.toml`.
...
- Free text → creates a new top-level session, seeded with the prompt.
  ...
  `/`, `s:`, `a:`, or `#` and still dispatches unchanged (filtering is
...
Set `AINXT_AGENT_DASHBOARD=0` to force-disable the feature for a single
...
process. The plan's Phase 4 ("supervisor / `ainxt --bg`") would list
