...
Memory lets ainxt recall facts, decisions, and patterns from earlier sessions. ainxt indexes the information you save and searches it automatically, so a new session can reuse relevant context.

---

## What Is Memory?

Without memory, each ainxt session starts fresh: the model knows nothing about previous sessions. When you enable memory, ainxt can:
...
3. `AINXT_MEMORY` env var: `1`/`true` enables, `0`/`false` disables
...
|----------|-------|-------------|
| `~/.ainxt/memory/MEMORY.md` | Global | Facts that apply across all your projects |
| `~/.ainxt/memory/<project-slug>-<hash8>/MEMORY.md` | Workspace | Project-specific conventions and context |
| `~/.ainxt/memory/<project-slug>-<hash8>/sessions/` | Sessions | Per-session summaries and logs |
...
When a session ends, ainxt saves a structured metadata summary to that session's daily log. The summary contains:
...
ainxt builds the summary from conversation metadata without an LLM call, without added latency. ainxt skips the save for trivial sessions -- those with fewer than three substantive prompts, or fewer than 50 bytes of user text.
...
Ask ainxt to remember something, and it appends the note to a `MEMORY.md` file -- the workspace file for project-specific items, or the global `~/.ainxt/memory/MEMORY.md` for cross-project preferences:

```
> remember to always open PR links after pushing
```

ainxt records entries as durable statements under organized headings, such as `## Preferences`, `## Project Context`, or `## Debugging`. The file watcher reindexes the change on the next memory search, so the new entry is searchable within the current session.
...
Run `/remember` with no text to enter remember mode, where the next line you type becomes the note. Either way, ainxt opens a review panel showing the note (with an optional rewritten version you can toggle with `Tab`); the note is written only after you confirm. On save, ainxt shows `Memory saved to ~/.ainxt/memory/MEMORY.md`.

### Forget

Ask ainxt to forget something, and it finds and removes the matching entry:

```
> forget the snake_case convention
```

Forget is best-effort: the model searches memory and removes entries that match. For guaranteed removal, edit the files under `~/.ainxt/memory/` directly and delete the entry yourself. To locate a file, open the `/memory` browser and press `y` to copy its path.

### Recall

Ask what ainxt remembers:

```
> what do you remember?
```

ainxt searches across all memory files and summarizes what it knows, grouped by source: global preferences, project-specific knowledge, and session history. Use `/memory` to browse the raw files.

### Direct Editing

You can edit memory files directly under `~/.ainxt/memory/`. The file watcher reindexes your changes on the next memory search. Use `/flush` to save the current session now, and `/dream` to consolidate session logs into organized topics.
...
When you save a note with `/remember`, ainxt confirms in the scrollback:

```
Memory saved to ~/.ainxt/memory/MEMORY.md
```

Background saves — flush, dream, and session-end — run silently and do not post a scrollback message. Use `/memory` at any time to browse what ainxt has stored.
...
Dream also runs automatically. By default, ainxt checks the consolidation gates when a session ends and runs Dream once enough time has passed and enough sessions have accumulated:
...
On the first turn of each session, ainxt automatically searches memory for content relevant to the current project and injects it as context. This means ainxt starts with knowledge from previous sessions without a reminder.
...
ainxt searches memory automatically, but you can also trigger searches manually in the chat:
...
The `ainxt memory` command manages memory from the shell. It has one subcommand, `clear`:
...
ainxt memory clear --workspace

# Clear the global MEMORY.md
ainxt memory clear --global

# Clear both workspace and global memory
ainxt memory clear --all

# Skip the confirmation prompt (-y is the short form)
ainxt memory clear --yes
```

To edit memory from the shell, open the files in your editor directly -- for example, `$EDITOR ~/.ainxt/memory/MEMORY.md`.
...
| `watcher.enabled` | `true` | Watch `~/.ainxt/memory/` for external edits and reindex |
...
| `min_score` | unset | Score threshold for first-turn results. When unset, ainxt applies no threshold, which is equivalent to `0.0`. |
...
| `flush_model` | unset | Model for the flush turn. When unset, ainxt uses the session's primary model. |
...
When a session memory is old, ainxt attaches a staleness note to it in search results. Older results get a stronger reminder to verify the current state before you rely on them. These notes help you spot stored facts that might no longer be accurate. Global and workspace memories never receive staleness notes, because they hold curated long-term knowledge.

---

## File Watcher

By default, ainxt watches `~/.ainxt/memory/` for external file changes. If you edit memory files directly (e.g., in your editor), the changes are picked up automatically on the next memory search:
...
1. Verify memory is enabled: check `ainxt inspect` output.
2. Check the flag: `ainxt --experimental-memory` or `AINXT_MEMORY=1`.
3. Check for `--no-memory` or `AINXT_MEMORY=0` overriding your config.
...
```bash
ls ~/.ainxt/memory/
cat ~/.ainxt/memory/MEMORY.md
$EDITOR ~/.ainxt/memory/MEMORY.md
```
