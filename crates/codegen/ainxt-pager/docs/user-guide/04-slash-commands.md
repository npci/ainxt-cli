...
- **Shell builtins** -- handled by the agent backend (ainxt-shell)
- **Pager builtins** -- handled by the TUI frontend (ainxt-pager)
...
When the context window fills up, ainxt auto-compacts at 85% usage (configurable via `[session] auto_compact_threshold_percent` in config.toml).
...
`--fullscreen` CLI flags are likewise session-scoped. To make plain `ainxt` open
...
The `/flush`, `/dream`, and `/memory` commands require `--experimental-memory` or `AINXT_MEMORY=1`. `/remember` is always available.
...
```
/image a golden sunset over a calm ocean with silhouetted palm trees
```
...
```
/video a cat playing piano in a jazz club
```
...
Run a prompt on a recurring interval. Specify the interval as `30m`, `1 hour`, or `every 2 days`. If you omit it, ainxt prompts you.
...
Set, manage, or check an autonomous goal. ainxt works toward the objective across turns and reports progress.
...
- `/docs web` opens `AINXT_URL_DOCS` in the browser. **Unset by default** — this
  project has no hosted docs site, so the command reports that none is configured.
  The guide you are reading is the documentation.
...
Any enabled skill with `user-invocable: true` in its SKILL.md frontmatter appears as a slash command. (A skill turned off via `/skills` is not advertised.) For example, if you have a skill at `~/.ainxt/skills/commit/SKILL.md`, you can invoke it with:
