...
Headless mode runs ainxt non-interactively from the command line. It accepts a single prompt, executes it with full tool access, and returns the result. Use it to automate tasks, script workflows, build integrations, and parse output programmatically.
...
ainxt processes the prompt, runs any necessary tools, and prints the result to stdout. The process exits when the response is complete.
...
| `--unchanged`            | Send prompt exactly as given                          |
...
- `estimated_total_cost_usd` (with `estimated_total_cost_usd_ticks`, and
  `modelUsage.*.estimatedCostUSD` per row) appears when the server did **not**
  price every call and the CLI filled the gap from the models' published public
  rates. It is a whole-session approximation — the server's own figures plus the
  estimate for what it skipped — so it is the number to show a human who just
  wants a budget signal. It is **not** billable: real prices vary with
  deployment-specific rates and cache-token ratios, so reconciliation must use
  `total_cost_usd` / `total_cost_usd_ticks` and ignore the estimated keys. The
  two never overlap: the estimated keys appear only when an estimate was needed,
  and `total_cost_usd` never contains a locally computed amount.
...
On failure, ainxt emits an error object (process exit non-zero). Prompt-level
...
ainxt may also emit `max_turns_reached` and `auto_compact_*` events; treat the list as non-exhaustive and switch on `type`.

---

## Session Management in Headless Mode

By default, each `ainxt -p` invocation creates a fresh session. To maintain context across calls, use session flags.
...
ainxt -p "Review the changes in this PR" --output-format json | jq -r '.sessionId'

# Continue in the same session
ainxt -p "Now check for security issues" --resume "<id>"

# Optional: create with a client-chosen UUID (must not already exist)
ainxt -p "hello" --session-id "$(uuidgen | tr '[:upper:]' '[:lower:]')" --output-format json
```
...
ainxt -p "What's the secret number?" --resume abc123
```
...
ainxt -p "Write a concise commit message for these changes:

$(git diff --staged)"

# Or read the prompt from a file
ainxt --prompt-file ./prompt.txt
```
...
for file in src/*.js; do
  ainxt -p "Migrate $file from CommonJS to ES modules." --yolo
done
```
...
ainxt's headless mode can be wrapped as an OpenAI-compatible chat completion API:
...
RESULT=$(ainxt -p "Review this PR for bugs. Output JSON with 'issues' array." \
...
ainxt -p "Format all files" --yolo

# Run tests and fix failures
ainxt -p "Run the tests and fix any failures" --cwd ~/projects/my-app --yolo
```
...
| ------------------------------- | ------------------------------------------------------------- |
| `AINXT_API_KEY`        | API key for authentication (required when no browser login)   |
| `AINXT_HOME`                    | Override config directory (default: `~/.ainxt`)                |
| `AINXT_MAX_RETRIES`             | **Set this in CI.** Retry budget for API calls. Default `15`, which with the 30s backoff cap is ~6 minutes of retrying — during which headless mode prints *nothing*. `2` is a good CI value. |
| `AINXT_LOG_FILE`                | Path to a log file (used unchanged as the path; works in headless and TUI, honors `RUST_LOG`) |
| `RUST_LOG`                     | Log level filter (e.g. `debug`). Headless logs to stderr.     |

> **Empty catalogue fails fast.** If the gateway is unreachable or returns no
> models, `ainxt -p` now detects it before sending the prompt and exits non-zero
> within a few seconds, printing what to check. Under `--output-format json` the
> same error arrives on stdout as `{"type":"error","message":...}`, so a CI job
> can parse it.
>
> **Other failures are still silent.** If the catalogue loads but requests fail,
> the retry loop emits nothing for the whole budget. Any CI job running headless
> ainxt should still set `AINXT_MAX_RETRIES` and a job-level timeout, and
> `RUST_LOG=ainxt_shell=debug,ainxt_sampler=debug` when diagnosing.

For CI environments without browser access, set `AINXT_API_KEY`. There is no
hosted AiNxt console to obtain one from: the key is issued by your own gateway,
or is your own model provider's key:

```bash
export AINXT_API_KEY="ainxt-..."
ainxt -p "Run the test suite" --yolo
```
...
- **`AINXT_API_KEY`** — simplest for CI. See [Environment Variables](#environment-variables-for-headless) above.
- **`ainxt login --device-auth`** (or `--device-code`) — no browser needed on the target machine.
  See [Authentication > Device Code Flow](02-authentication.md#device-code-flow).
- **`ainxt login`** — browser-based OAuth2 on machines with a GUI.
...
- Combine `--yolo` with `--rules` to set guardrails: `ainxt -p "..." --yolo --rules "Never delete files"`.
- For debugging, raise the log level and capture stderr: `RUST_LOG=debug ainxt -p "..." 2> debug.log`.

---

## Project Root Discovery

When ainxt starts, it discovers the project root by walking upward from `--cwd`
...
ainxt discovers that repository as the project root and scopes its discovery (AGENTS.md, skills, git history) to it, which can make
...
ainxt stores data in `~/.ainxt` (override with `AINXT_HOME`; see [Environment Variables for Headless](#environment-variables-for-headless)):
...
For containers or CI, mount `~/.ainxt` read-only:

- Pre-populate `auth.json` or use `AINXT_API_KEY`
- Session persistence fails silently (ephemeral)
- Update checks log a warning and skip

```bash
export AINXT_API_KEY="ainxt-..."
export AINXT_DISABLE_AUTOUPDATER=1
ainxt -p "..." --no-auto-update
```
...
| `AINXT_DISABLE_AUTOUPDATER=1`    | Process   |
| Non-TTY stderr (auto-detected)  | Automatic |
| `[cli] auto_update = false`     | Persistent|

`AINXT_DISABLE_AUTOUPDATER` set to a falsy value (`0`, `false`, `off`, `no`, or empty, any
case) counts as not set. The agent SDKs
inject `AINXT_DISABLE_AUTOUPDATER=1` for the non-leader agents they spawn (a falsy value in
the SDK's isolation env keeps updates on), and the stdio agent skips its background update
unless it runs from the managed install (`$AINXT_HOME/bin/ainxt`).
...
These flags supplement the [Command-Line Options](#command-line-options) table above. Flags already listed there (`--prompt-json`, `--prompt-file`, `--unchanged`, `--sandbox`, `--no-auto-update`) are not repeated here.
...
- Resume: `ainxt -p "continue" --resume "<id>"` or `ainxt -p "continue" --continue`
