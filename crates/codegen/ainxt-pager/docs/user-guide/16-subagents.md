...
| **How you set them** | At startup, or with agent definitions (`.md` files in `.ainxt/agents/` or `~/.ainxt/agents/`) | In `config.toml` (`[subagents.personas]`) or `.toml` files under `.ainxt/personas/`; applied during subagent resolution |
...
| **Examples** | `ainxt-build`, `explore`, `plan` | `researcher`, `concise` |
...
ainxt discovers file-based personas from these locations, in priority order:

- `.ainxt/personas/*.toml` (project)
- `~/.ainxt/personas/*.toml` (user)
...
> **Note:** ainxt applies personas through subagent resolution and roles, not through a `spawn_subagent` parameter. The main agent does not pass a persona name when it spawns a child.
...
When a persona applies, ainxt resolves the effective model and reasoning effort in this order, highest priority first:
...
ainxt manages worktrees through the `ainxt.dev/git/worktree/*` extension methods, including an apply operation that merges changes back into the main working directory.
...
[subagents.models]
explore = "ainxt-build"               # route explore to a specific model
```
...
# instructions_file = ".ainxt/personas/concise.md"  # or load from a file
```

ainxt also discovers roles from `.ainxt/roles/*.toml` and personas from `.ainxt/personas/*.toml`. Inline `config.toml` definitions take precedence over files.

---

## The Tasks Pane (TUI)

ainxt shows running and finished work in side panes on the agent screen:
...
- `Subagent running: "do the thing" (Implementer · ainxt-3) — Thinking`
