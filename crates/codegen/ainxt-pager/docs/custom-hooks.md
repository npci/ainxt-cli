...
Hooks let you run custom scripts or HTTP requests at key moments during a ainxt session — for example, before or after a tool runs, when a session starts or ends, or when the agent sends a notification.
...
2. Create a simple hook file, e.g. `~/.ainxt/hooks/session-start.json`:
   ```json
   {
     "hooks": {
       "SessionStart": [
...
|-----------|-----------------------------------|--------------|-------|
| Global    | `~/.ainxt/hooks/*.json`            | Always       | Best for personal hooks |
...
**Trusting a project**: Open the hooks modal (`Ctrl+L` on non–VS Code family, or `/hooks` on any terminal including VS Code family) or run `/hooks-trust` (the same folder-trust gate as `--trust`, recorded in `~/.ainxt/trusted_folders.toml`) the first time you open a project with hooks. This prevents untrusted repos from running arbitrary code.
...
**Tool name aliases**: Claude-style names like `Bash`, `Edit`, `Read` automatically match ainxt's internal names (`run_terminal_cmd`, `search_replace`, `read_file`).
...
ainxt injects the following variables into every hook process:

- `AINXT_HOOK_EVENT` — the event name (e.g. `pre_tool_use`, `session_start`, `post_tool_use`)
- `AINXT_HOOK_NAME` — the full configured name of this hook
- `AINXT_SESSION_ID` — the current session identifier
- `AINXT_WORKSPACE_ROOT` — absolute path to the workspace root

For hooks provided by plugins, the following are also set:

- `AINXT_PLUGIN_ROOT` — absolute path to the plugin's installation directory
- `AINXT_PLUGIN_DATA` — absolute path to the plugin's writable data directory

These runner- and plugin-injected variables always take precedence. Attempts to override the reserved runner keys via the `env` field are stripped at load time (with a warning logged). For plugin hooks, `AINXT_PLUGIN_ROOT` and `AINXT_PLUGIN_DATA` similarly override any user-supplied values for those keys.
...
`AINXT_PLUGIN_ROOT` and `AINXT_PLUGIN_DATA`. These keys override any user-declared
...
{
  "type": "command",
  "command": "${HOME}/.config/ainxt-hooks/check.sh"
}
```

Lookup order for each reference:
1. The handler's own `env` map.
2. The current process environment (the env ainxt itself sees).

If a reference is unset in both, it's **preserved unchanged** (e.g. `${UNSET}`
...
`${AINXT_PLUGIN_ROOT}/check` resolve against the plugin's actual path.
...
left unchanged for the runtime `sh -c` branch to handle. This avoids subtle
...
- **The values of the `env` map itself** — these are stored unchanged and
...
Hooks from `~/.ainxt/hooks/` appear under **Global**, project ones under **Project**, etc.
...
- Global hooks (`~/.ainxt/...`) run with your user permissions — treat them like shell scripts.
...
- **See errors?** → Check the pager logs (usually in the tracing pane or `~/.ainxt/logs`).

## More Examples

See the built-in examples in the `ainxt-hooks` crate:

- [Safe Shell Guard](../../ainxt-hooks/examples/hooks/safe-shell.json)
- [No Recursive Grep](../../ainxt-hooks/examples/hooks/no-recursive-grep.json) — hard-blocks `grep -r`/`grep -R`/`rgrep` (OOM guard)
- [Session Audit Log](../../ainxt-hooks/examples/hooks/session-log.json)
- [Tool Activity Logger](../../ainxt-hooks/examples/hooks/tool-logger.json)

Copy them to `~/.ainxt/hooks/` and customize.
