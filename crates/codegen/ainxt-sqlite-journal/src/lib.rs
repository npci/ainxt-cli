...
//! do not provide. When `$HOME` (and thus `~/.ainxt`) is NFS-mounted on
...
impl JournalMode {
    ...
    /// yet), so callers must create it first. `AINXT_SQLITE_JOURNAL_MODE`
    /// (`wal`|`truncate`) overrides detection as a field kill-switch.
    pub fn for_db_path(db_path: &Path) -> Self {
        let env = std::env::var("AINXT_SQLITE_JOURNAL_MODE").ok();
        match mode_from_env(env.as_deref()) {
            EnvOverride::Mode(mode) => {
                ...
                tracing::info!(
                    ...
                    mode = mode.as_str(),
                    source = "env",
                    "sqlite journal mode forced by AINXT_SQLITE_JOURNAL_MODE"
                );
                return mode;
            }
            EnvOverride::Invalid => {
                ...
                tracing::warn!(
                    ...
                    "invalid AINXT_SQLITE_JOURNAL_MODE (accepted: wal, truncate); using detection"
                );
            }
            EnvOverride::Unset => {}
        }
    ...
    /// `set_journal_mode` in `ainxt-fast-worktree`).
...
/// Parse result of the `AINXT_SQLITE_JOURNAL_MODE` kill-switch.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
enum EnvOverride {
    Unset,
...
/// `AINXT_SQLITE_JOURNAL_MODE` override parsing (pure for testability).
...
/// Matches macOS `libc::MNT_LOCAL` so the pure classifier is testable on
...
/// `\\?\UNC\server\share`; the `\\.\` and `\\?\C:\` device/unchanged-local
...
mod tests {
    ...
    fn windows_unc_classifies() {
        assert!(is_windows_unc(r"\\server\share\ainxt"));
        assert!(is_windows_unc(r"\\?\UNC\server\share\ainxt"));
        assert!(is_windows_unc(r"\\?\unc\server\share"));
        assert!(!is_windows_unc(r"\\?\C:\Users\x"));
        assert!(!is_windows_unc(r"\\.\pipe\ainxt"));
        assert!(!is_windows_unc(r"C:\Users\x"));
    ...
    fn for_db_path_defaults_to_wal_on_local_fs() {
        ...
        if std::env::var("AINXT_SQLITE_JOURNAL_MODE").is_ok() {
            return;
        }
        let tmp = TempDir::new().unwrap();
