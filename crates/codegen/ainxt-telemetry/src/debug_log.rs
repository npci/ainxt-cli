...
//! - PerSession (`AINXT_DEBUG_LOG=1`): a routing layer fans each session's
//!   firehose to `~/.ainxt/debug/<session_id>.txt` (one file per session), with a
...
//! - SingleFile (explicit path via `AINXT_LOG_FILE` or `AINXT_DEBUG_LOG=<path>`):
...
impl DebugSource {
    fn label(self) -> &'static str {
        match self {
            Self::AinxtLogFile => "AINXT_LOG_FILE",
            Self::AinxtDebugLog => "AINXT_DEBUG_LOG",
        }
    }
}
...
///
/// Lives here (not in `ainxt-pager`) so the firehose directives below and
...
/// safe. See `ainxt-pager/src/tracing.rs` for the consumer side.
...
// Broad firehose filter for the routing/AINXT_DEBUG_LOG sources: capture our
// crates at debug regardless of a narrowing RUST_LOG, with deps at info so they
// don't flood. Curated first-party allowlist: new ainxt crates default to `info`
// until added here.
const FIREHOSE_BASE_DIRECTIVES: &str = "info,ainxt_pager=debug,ainxt_shell=debug,ainxt_tools=debug,ainxt_telemetry=debug,ainxt_agent=debug,ainxt_mcp=debug,ainxt_acp_lib=debug,ainxt_update=debug,sampling_log=off";
...
// AINXT_DEBUG_LOG single-file source (matches `default_file_filter`).
fn firehose_filter() -> EnvFilter {
    EnvFilter::new(firehose_directives())
}

// RUST_LOG-respecting filter for the AINXT_LOG_FILE source: DEBUG default, honor
// RUST_LOG, silence sampling_log (preserves AINXT_LOG_FILE back-compat).
...
/// filter by source (AINXT_LOG_FILE respects RUST_LOG). Open failures warn AFTER
    ...
    match resolve_debug_target() {
        ...
        Some(DebugTarget::SingleFile { path, src }) => {
            let filter = match src {
                DebugSource::AinxtLogFile => default_file_filter(),
                DebugSource::AinxtDebugLog => firehose_filter(),
            };
...
pub(crate) enum DebugTarget {
    /// `AINXT_DEBUG_LOG=1` → route per session into `<dir>` (`~/.ainxt/debug`).
    PerSession { dir: PathBuf },
    /// An explicit path → one flat `fmt` file, routing bypassed.
    SingleFile { path: PathBuf, src: DebugSource },
}

/// Resolve the debug target, honoring precedence: explicit AINXT_LOG_FILE wins
/// (single file, RUST_LOG filter); else AINXT_DEBUG_LOG — a truthy bool routes
/// per session into `~/.ainxt/debug`, an explicit path writes a single file.
///
/// Read via `var_os` (not `var`) so a non-UTF-8 path isn't silently dropped.
pub(crate) fn resolve_debug_target() -> Option<DebugTarget> {
    let ainxt_log_file = std::env::var_os("AINXT_LOG_FILE");
    let ainxt_debug_log = std::env::var_os("AINXT_DEBUG_LOG");
    resolve_debug_target_inner(
        ainxt_log_file.as_deref(),
        ainxt_debug_log.as_deref(),
        &ainxt_home().join("debug"),
    )
}
...
fn resolve_debug_target_inner(
    ainxt_log_file: Option<&OsStr>,
    ainxt_debug_log: Option<&OsStr>,
    debug_dir: &Path,
) -> Option<DebugTarget> {
    if let Some(raw) = ainxt_log_file
        && !is_blank(raw)
    {
        return Some(DebugTarget::SingleFile {
            path: os_path(raw),
            src: DebugSource::AinxtLogFile,
        });
    }
    let raw = ainxt_debug_log?;
...
/// `~/.ainxt/debug` older than [`LOG_RETENTION`] so the dir doesn't grow
...
pub(crate) fn sweep_old_logs() {
    prune_old_logs(&ainxt_home().join("debug"), LOG_RETENTION);
}
...
mod tests {
        ...
        for v in ["0", "false", "off", "no", "", "  "] {
            assert!(
                resolve_debug_target_inner(None, Some(OsStr::new(v)), Path::new("/debug"))
                    .is_none(),
                "expected None for AINXT_DEBUG_LOG={v:?}"
            );
        }
    }
    ...
    fn resolve_target_debug_log_enabled_is_per_session_dir() {
        for v in ["1", "true", "on", "yes"] {
            ...
            assert_eq!(
                ...
                "expected PerSession for AINXT_DEBUG_LOG={v:?}"
            );
        }
    }
    ...
    fn resolve_target_empty_log_file_falls_through_to_debug_log() {
        // Empty / whitespace AINXT_LOG_FILE is treated as unset (matches AINXT_DEBUG_LOG).
        for blank in ["", "   "] {
            let target = resolve_debug_target_inner(
    ...
    fn resolve_target_non_utf8_debug_log_path_is_single_file() {
        ...
        // A non-UTF-8 AINXT_DEBUG_LOG value is a path, not a bool keyword, and its
        ...
        match target {
            DebugTarget::SingleFile { path, src } => {
                assert_eq!(src, DebugSource::AinxtDebugLog);
    ...
    fn layer_routes_event_by_session_span() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            // `%` matches production's `info_span!("session", session_id = %...)`.
            tracing::info_span!("session", session_id = %"sess-xyz").in_scope(|| {
                tracing::info!(target: "ainxt_shell", "inside session");
            });
            tracing::info!(target: "ainxt_shell", "outside session");
        });
        crate::appender::flush_file_log_guards();
    ...
    fn layer_routes_under_real_firehose_filter() {
        ...
        // `ainxt_telemetry::session_ctx`) survives the real filter so
        ...
        tracing::subscriber::with_default(subscriber, || {
            tracing::info_span!(
                target: "ainxt_telemetry::session_ctx",
                "session",
                session_id = %"sid-real"
            )
            .in_scope(|| {
                tracing::debug!(target: "ainxt_shell", "filtered routing works");
            });
        });
        crate::appender::flush_file_log_guards();
    ...
    fn layer_routes_two_sessions_to_distinct_files() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            tracing::info_span!("session", session_id = %"sid-one").in_scope(|| {
                ...
                tracing::info!(target: "ainxt_shell", "one first");
                tracing::info!(target: "ainxt_shell", "one second");
            });
            tracing::info_span!("session", session_id = %"sid-two").in_scope(|| {
                tracing::info!(target: "ainxt_shell", "two only");
            });
        });
        crate::appender::flush_file_log_guards();
    ...
    fn prune_old_logs_missing_dir_is_noop() {
        ...
        prune_old_logs(
            Path::new("/no/such/ainxt/debug/dir"),
