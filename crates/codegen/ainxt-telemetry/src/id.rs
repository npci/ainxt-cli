...
//!
//! Extracted from `ainxt-shell::agent::unique_identifier` so the
//! telemetry engine can stamp events without depending on shell internals.
//! `$AINXT_HOME` is resolved through `ainxt-config::ainxt_home`.
...
/// This function caches the result in `$AINXT_HOME/agent_id` so subsequent calls
...
fn load_or_compute_agent_id() -> String {
    let cache_path = ainxt_config::ainxt_home().join("agent_id");
...
/// Returns true when workspace marker env vars (`AINXT_ROOT` and `AINXT_USER`) are set.
///
...
pub fn has_workspace_env_markers() -> bool {
    std::env::var("AINXT_ROOT").is_ok() && std::env::var("AINXT_USER").is_ok()
}

/// Opt-in special-user gate for telemetry.
///
/// Enabled only when `AINXT_TELEMETRY_SPECIAL_USER=1` (or `true`). There is no
/// hardcoded username allowlist.
pub fn is_special_user() -> bool {
    matches!(
        std::env::var("AINXT_TELEMETRY_SPECIAL_USER").as_deref(),
