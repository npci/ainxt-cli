...
//! file logger that writes to `~/.ainxt/logs/hooks.log`.
//!
//! ## When to use
//!
//! Use regular `tracing::info!` / `tracing::debug!` / `tracing::warn!` with
//! targets `ainxt_hooks` or `ainxt_agent::plugins` at key lifecycle
//! points — discovery, dispatch, execution, errors.
//!
//! ## Enabling
//!
//! ```bash
//! AINXT_HOOKS_LOG=1 ainxt              # enable, write to ~/.ainxt/logs/hooks.log
//! AINXT_HOOKS_LOG=/tmp/h.log ainxt     # write to custom path
//! AINXT_HOOKS_LOG=0 ainxt              # explicitly disable
//! tail -f ~/.ainxt/logs/hooks.log     # watch in another terminal
//! ```
...
const ENV_HOOKS_LOG: &str = "AINXT_HOOKS_LOG";
...
///
/// Writes to `~/.ainxt/logs/hooks.log` (or custom path via `AINXT_HOOKS_LOG`).
/// Filters to hooks (`ainxt_hooks`) and plugins (`ainxt_agent::plugins`) targets.
/// Set `AINXT_HOOKS_LOG=0` to disable, `AINXT_HOOKS_LOG=/path` to redirect.
    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let filter = tracing_subscriber::filter::EnvFilter::new(
        "ainxt_hooks=debug,ainxt_agent::plugins=debug",
    );
...
fn resolve_log_path() -> Option<PathBuf> {
    let default_path = || ainxt_home().join("logs").join("hooks.log");
