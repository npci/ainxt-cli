...
impl ExternalEventName {
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            Self::SessionStart => "ainxt_code.session_start",
            Self::SessionEnd => "ainxt_code.session_end",
            Self::UserPrompt => "ainxt_code.user_prompt",
            Self::TurnCompleted => "ainxt_code.turn_completed",
            Self::ApiRequest => "ainxt_code.api_request",
            Self::ApiError => "ainxt_code.api_error",
            Self::ToolResult => "ainxt_code.tool_result",
            Self::ToolDecision => "ainxt_code.tool_decision",
            Self::McpServerConnection => "ainxt_code.mcp_server_connection",
            Self::PermissionModeChanged => "ainxt_code.permission_mode_changed",
            Self::SkillActivated => "ainxt_code.skill_activated",
            Self::PluginLoaded => "ainxt_code.plugin_loaded",
            Self::Compaction => "ainxt_code.compaction",
            Self::Subagent => "ainxt_code.subagent",
            Self::Auth => "ainxt_code.auth",
            Self::InternalError => "ainxt_code.internal_error",
            Self::ModelSwitched => "ainxt_code.model_switched",
            Self::ContextualTip => "ainxt_code.contextual_tip",
        }
    }
}
...
/// key with a default attr (e.g. unchanged vs. sanitized `tool_name`), the
...
pub enum MetricIncrement {
    /// `ainxt_code.session.count`.
    SessionCount,
    /// `ainxt_code.token.usage` — `token_type` ∈ `input|output|reasoning|cache_read`.
    TokenUsage {
        token_type: &'static str,
        model: String,
        count: u64,
    },
    /// `ainxt_code.turn.count`.
    TurnCount {
        outcome: &'static str,
        model: String,
    },
    /// `ainxt_code.tool.decision`.
    ToolDecision {
        tool_name: String,
        decision: &'static str,
    ...
    /// `ainxt_code.tool.usage`.
    ToolUsage {
        tool_name: String,
        outcome: &'static str,
    },
    /// `ainxt_code.error.count`.
    ErrorCount {
...
pub(crate) const METRIC_SESSION_COUNT: &str = "ainxt_code.session.count";
pub(crate) const METRIC_TOKEN_USAGE: &str = "ainxt_code.token.usage";
pub(crate) const METRIC_TURN_COUNT: &str = "ainxt_code.turn.count";
pub(crate) const METRIC_TOOL_DECISION: &str = "ainxt_code.tool.decision";
pub(crate) const METRIC_TOOL_USAGE: &str = "ainxt_code.tool.usage";
pub(crate) const METRIC_ERROR_COUNT: &str = "ainxt_code.error.count";
...
/// must never pass unchanged — unknown values collapse to `"other"`.
...
/// metadata (`_meta.screenMode`), so it must never pass unchanged — unknown
...
/// Built-in tool names (closed enum surface in `ainxt-tools`) that pass
/// unchanged through `tool_name` sanitization. Pinned by test; everything else
...
/// Default `tool_name` reduction: built-ins pass unchanged, MCP-qualified
...
/// The unchanged name rides the `ToolDetails` gate.
...
fn tool_outcome_label(o: &ainxt_file_utils::events::types::ToolOutcome) -> &'static str {
    use ainxt_file_utils::events::types::ToolOutcome;
    match o {
...
/// `SessionHarness` → `ainxt_code.session_start`. Emitted from a spawn outside
...
/// `SessionNew` → `ainxt_code.session.count` (metric only; the flagship
...
/// `SessionEnded` → `ainxt_code.session_end`.
...
/// `PromptSubmitted` → `ainxt_code.user_prompt`. Prompt text rides the
...
/// `TurnCompleted` → `ainxt_code.turn_completed` + `turn.count`
...
/// `ModelResponseReceived` → `ainxt_code.api_request` + `token.usage`.
...
/// `RateLimitHit` → `ainxt_code.api_error` (`error_category = rate_limit`).
...
/// `ApiError` → `ainxt_code.api_error`. Category/class enums only — no
...
/// `ToolCallCompleted` → `ainxt_code.tool_result` + `tool.usage`.
...
/// `PermissionDecisionPayload` → `ainxt_code.tool_decision` + `tool.decision`.
...
/// `McpServerConnected` → `ainxt_code.mcp_server_connection` (`status=connected`).
...
/// `McpServerFailed` → `ainxt_code.mcp_server_connection` (`status=failed`).
...
/// `PlanModeToggled` → `ainxt_code.permission_mode_changed`.
...
/// `ContextualTip` → `ainxt_code.contextual_tip`. Labels only (no user
...
/// `YoloToggled` → `ainxt_code.permission_mode_changed`.
...
/// `SkillDispatched` → `ainxt_code.skill_activated`. Skill names are
...
/// `PluginInstalled` → `ainxt_code.plugin_loaded`.
...
/// `PluginUsed` → `ainxt_code.plugin_loaded` (plugin name details-gated).
...
/// `CompactionCompleted` → `ainxt_code.compaction`.
...
/// `CompactionTriggered` → `ainxt_code.compaction` trigger attrs ride on the
/// completion event instead (one event per compaction); not mapped.
/// `SubagentLaunched` → `ainxt_code.subagent` (`phase=launched`).
...
/// `SubagentCompleted` → `ainxt_code.subagent` (`phase=completed`).
...
/// `InternalError` → `ainxt_code.internal_error`. Error class only — no
...
/// `ModelSwitched` → `ainxt_code.model_switched`.
