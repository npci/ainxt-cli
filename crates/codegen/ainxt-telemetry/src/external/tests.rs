...
fn event_names_are_pinned() {
    ...
    let expected: &[(E, &str)] = &[
        (E::SessionStart, "ainxt_code.session_start"),
        (E::SessionEnd, "ainxt_code.session_end"),
        (E::UserPrompt, "ainxt_code.user_prompt"),
        (E::TurnCompleted, "ainxt_code.turn_completed"),
        (E::ApiRequest, "ainxt_code.api_request"),
        (E::ApiError, "ainxt_code.api_error"),
        (E::ToolResult, "ainxt_code.tool_result"),
        (E::ToolDecision, "ainxt_code.tool_decision"),
        (E::McpServerConnection, "ainxt_code.mcp_server_connection"),
        (
            E::PermissionModeChanged,
            "ainxt_code.permission_mode_changed",
        ),
        (E::SkillActivated, "ainxt_code.skill_activated"),
        (E::PluginLoaded, "ainxt_code.plugin_loaded"),
        (E::Compaction, "ainxt_code.compaction"),
        (E::Subagent, "ainxt_code.subagent"),
        (E::Auth, "ainxt_code.auth"),
        (E::InternalError, "ainxt_code.internal_error"),
        (E::ModelSwitched, "ainxt_code.model_switched"),
        (E::ContextualTip, "ainxt_code.contextual_tip"),
    ];
...
fn client_identifier_allowlist_is_pinned() {
    ...
    assert_eq!(
        schema::sanitize_client_identifier("ainxt-pager"),
        "ainxt-pager"
    );
    assert_eq!(
...
fn session_start_snapshot_counts_not_names() {
    ...
    let ev = &events[0];
    assert_eq!(ev.0, "ainxt_code.session_start");
...
fn session_new_increments_session_count_only() {
    ...
    assert_eq!(names, vec!["ainxt_code.session.count".to_owned()]);
}
...
fn api_request_snapshot_and_token_usage() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "ainxt_code.api_request");
...
fn one_failed_turn_increments_error_count_exactly_once() {
    ...
    let names: Vec<String> = exported_events(&stream)
        ...
        .collect();
    assert_eq!(
        names.iter().filter(|n| *n == "ainxt_code.api_error").count(),
        2
    );
    ...
    let total: u64 = stream
        ...
        .filter(|m| m.name() == "ainxt_code.error.count")
        .map(|m| match m.data() {
...
fn turn_error_increments_error_count() {
    ...
    names.sort();
    assert_eq!(names, vec!["ainxt_code.error.count", "ainxt_code.turn.count"]);
}
...
fn tool_result_gates_off_collapses_and_reduces() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "ainxt_code.tool_result");
...
fn user_prompt_gates_off_drops_text() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "ainxt_code.user_prompt");
...
fn mcp_connection_collapses_server_name_by_default() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "ainxt_code.mcp_server_connection");
...
fn tool_decision_snapshot() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "ainxt_code.tool_decision");
...
fn contextual_tip_maps_every_tip_and_action() {
    ...
    for (tip, action, tip_label, action_label) in cases {
        ...
        let events = exported_events(&stream);
        let ev = &events[0];
        assert_eq!(ev.0, "ainxt_code.contextual_tip");
...
/// ainxt-workspace sampler events, which live outside this crate and have
...
fn validating_metric_exporter_drops_export_on_bad_attr_key() {
    ...
    let rogue = meter.u64_counter("ainxt_code.session.count").build();
    rogue.add(
        1,
...
fn redacting_log_exporter_drops_record_with_closed_gate_key() {
    ...
    record.set_event_name("ainxt_code.user_prompt");
...
fn redacting_log_exporter_drops_record_with_unknown_key() {
    ...
    let logger = stream.ext.logger.as_ref().unwrap();
    let mut record = logger.create_log_record();
    record.set_event_name("ainxt_code.api_request");
...
fn metric_increments_pass_model_through_scrub_and_attach_session_id() {
    ...
    let exported = stream.metrics.get_finished_metrics().unwrap();
    let blob = format!("{exported:?}");
    assert!(blob.contains("ainxt_code.token.usage"));
    assert!(
