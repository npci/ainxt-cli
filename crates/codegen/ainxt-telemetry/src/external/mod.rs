...
//!
//! Enterprise customers point the ainxt CLI at *their own* OpenTelemetry
//! collector (standard `OTEL_*` env vars + the `AINXT_EXTERNAL_OTEL` master
...
//!   headers.
//! - Default **off**: with `AINXT_EXTERNAL_OTEL` unset (or no exporter
...
//!   data-retention opt-outs (user-confirmed): those govern ainxt-side
...
impl IdentityAttrs {
    pub fn from_snapshot(snapshot: &ainxt_auth::CredentialSnapshot) -> Self {
        Self {
...
/// Returns `true` when `endpoint` uses the `http://` scheme and its host is
/// **not** a loopback address (`localhost`, `127.0.0.1`, `::1`).
///
/// Loopback endpoints are the OpenTelemetry spec defaults for a local
/// collector sidecar and are intentionally excluded from the warning.
/// Any other `http://` host sends data in cleartext over the network.
fn is_cleartext_remote_endpoint(endpoint: &str) -> bool {
    if !endpoint.starts_with("http://") {
        return false;
    }
    // Parse the host. If parsing fails we conservatively return false
    // (don't warn on a malformed URL — that will surface as a connection
    // error at export time, which is a clearer signal).
...
#[cfg(test)]
mod cleartext_endpoint_tests {
    use super::is_cleartext_remote_endpoint;

    // ── should warn (remote http://) ─────────────────────────────────────────

    #[test]
    fn warns_for_remote_http_host() {
        assert!(is_cleartext_remote_endpoint(
            "http://collector.corp.example:4318/v1/logs"
        ));
    }

    #[test]
    fn warns_for_remote_http_ip() {
        assert!(is_cleartext_remote_endpoint("http://10.0.0.5:4318/v1/metrics"));
    }

    #[test]
    fn warns_for_remote_http_no_port() {
        assert!(is_cleartext_remote_endpoint("http://otel.internal/v1/logs"));
    }

    // ── should NOT warn (loopback http:// — spec default) ────────────────────

    #[test]
    fn no_warn_for_localhost() {
        assert!(!is_cleartext_remote_endpoint(
    ...
    #[test]
    fn no_warn_for_ipv6_loopback() {
        assert!(!is_cleartext_remote_endpoint("http://::1:4318/v1/logs"));
    }

    // ── should NOT warn (https:// — encrypted) ───────────────────────────────

    #[test]
    fn no_warn_for_https_remote() {
        assert!(!is_cleartext_remote_endpoint(
    ...
    #[test]
    fn no_warn_for_https_localhost() {
        assert!(!is_cleartext_remote_endpoint(
    ...
    #[test]
    fn no_warn_for_grpc_endpoint() {
        // gRPC endpoints use http:// in the URL but are handled by the gRPC
        // stack — still warn if remote, but localhost is safe.
        assert!(!is_cleartext_remote_endpoint("http://localhost:4317"));
    }

    #[test]
    fn no_warn_for_malformed_url() {
        // Malformed URLs fail silently — the export will error at connection time.
        assert!(!is_cleartext_remote_endpoint("not-a-url"));
        assert!(!is_cleartext_remote_endpoint(""));
    }
}
    ...
    if cfg.internal_pipeline_consumed_otel_vars {
        tracing::warn!(
            "external otel: refusing to activate — the internal trace pipeline consumed \
             ...
             AINXT_INTERNAL_OTLP_* to use the external stream."
        );
        return None;
    }

    // Security (CWE-319): warn when telemetry — which may carry session IDs,
    // user IDs, and OTEL auth headers — is being sent to a non-localhost
    // endpoint over an unencrypted http:// channel.
    //
    // We only warn for non-loopback hosts: http://localhost and
    // http://127.0.0.1 are the OpenTelemetry spec defaults for a local
    // collector sidecar and are intentionally left as-is. A remote http://
    // endpoint is the dangerous case.
    for (label, endpoint) in [
        ("logs", cfg.logs_endpoint.as_str()),
        ("metrics", cfg.metrics_endpoint.as_str()),
    ] {
        if is_cleartext_remote_endpoint(endpoint) {
            tracing::warn!(
                signal = label,
                endpoint = endpoint,
                "external otel: {label} endpoint uses http:// with a non-localhost host — \
                 telemetry data (session IDs, user IDs, OTEL auth headers) will be \
                 transmitted in cleartext. Use https:// to protect data in transit."
            );
        }
    }
