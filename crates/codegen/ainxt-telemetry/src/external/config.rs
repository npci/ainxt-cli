...
//! `AINXT_EXTERNAL_OTEL=1` *and* at least one of `OTEL_METRICS_EXPORTER` /
...
/// Master switch env var. Deliberately *not* `AINXT_ENABLE_TELEMETRY`: that
/// would be a word-order typo away from the long-standing
/// `AINXT_TELEMETRY_ENABLED` (product events/Mixpanel mode), and the two control
/// opposite-pointing data flows (to ainxt vs. to the customer's collector).
pub const ENV_MASTER_SWITCH: &str = "AINXT_EXTERNAL_OTEL";
...
pub struct ContentGates {
    /// `OTEL_LOG_USER_PROMPTS=1`: prompt text on `ainxt_code.user_prompt`
    /// (60 KB cap, secret-scrubbed).
    pub log_user_prompts: bool,
    /// `OTEL_LOG_TOOL_DETAILS=1`: gated tool params / full paths / unchanged
...
mod tests {
    ...
    fn master_switch_alone_enables_nothing() {
        // RQ7: AINXT_EXTERNAL_OTEL=1 without an explicit exporter is inert.
        assert!(
            ExternalOtelConfig::resolve_with(env(&[("AINXT_EXTERNAL_OTEL", "1")]), None).is_none()
        );
    }
    ...
    fn endpoint_resolution_follows_otlp_http_spec() {
        ...
        .unwrap();
        // Signal-specific endpoint used unchanged; base + spec path otherwise.
    ...
    fn file_config_layered_under_env() {
        ...
        let cfg =
            ExternalOtelConfig::resolve_with(env(&[("AINXT_EXTERNAL_OTEL", "0")]), Some(&file));
        assert!(cfg.is_none());
    }
