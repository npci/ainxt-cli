...
//!
//! Extracted from `ainxt-shell::agent::telemetry`.
...
pub enum EmitterOrigin {
    /// `ainxt-shell` (and the pager/TUI that emit through it).
    Shell,
    /// `ainxt-workspace` (remote sampler / workspace server).
    Workspace,
}
...
impl EmitterOrigin {
    ...
    pub fn event_prefix(self) -> &'static str {
        match self {
            EmitterOrigin::Shell => "ainxt-shell-",
            EmitterOrigin::Workspace => "ainxt-workspace-",
        }
    }
}
...
/// event-name prefix (`ainxt-shell-*` vs `ainxt-workspace-*`).
///
...
mod tests {
    ...
    fn event_prefix_is_stable_per_origin() {
        assert_eq!(EmitterOrigin::Shell.event_prefix(), "ainxt-shell-");
        assert_eq!(EmitterOrigin::Workspace.event_prefix(), "ainxt-workspace-");
    }

    /// The `Shell` reroute must reproduce the historical
    /// `format!("ainxt-shell-{suffix}")` event name exactly, since every
    ...
    fn shell_origin_event_name_matches_legacy_format() {
        ...
        let legacy = format!("ainxt-shell-{suffix}");
    ...
    fn workspace_origin_event_name_uses_workspace_prefix() {
        ...
        assert_eq!(name, "ainxt-workspace-turn");
    }
