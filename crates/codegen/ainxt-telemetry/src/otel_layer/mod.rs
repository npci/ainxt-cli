...
//!
//! `ainxt-pager` uses this module to set up OTLP
...
const ENV_OTEL_FILTER: &str = "AINXT_OTEL_FILTER";
...
/// (`AuthManager`, `EndpointsConfig`, `AinxtComConfig`).
///
/// Built by the binaries (`ainxt-pager`) from their own
/// configuration. The credentials provider is constructed by shell's
/// `ainxt_shell::auth::credential_provider::build_otel_credential_provider`.
pub struct OtelLayerConfig {
    ...
    /// Value for the `X-Ainxt-Token-Auth` header (typically `"ainxt-cli"`).
...
pub struct OtelClientInfo {
    /// Binary name (`ainxt-pager`) -> `client.name`.
...
pub struct OtelExporterConfig {
    /// Full OTLP traces endpoint URL (e.g. `https://api.example.test/v1/traces`).
...
///
/// - `client_name`: binary name (e.g. `"ainxt-tui"`, `"ainxt-pager"`) -- stored as
///   `client.name` resource attribute for dashboards to distinguish client types.
/// - `client_version`: `CARGO_PKG_VERSION` -- sent in the `x-ainxt-client-version` header.
    ...
    S: tracing::Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let tracer = provider.tracer("ainxt-cli");
    ...
    let otel_filter = tracing_subscriber::filter::EnvFilter::try_new(&otel_filter)
        .unwrap_or_else(|e| {
            eprintln!(
                "[otel] Invalid AINXT_OTEL_FILTER '{}': {}. Using default '{}'.",
...
struct RefreshableSpanExporter {
    ...
    /// Value for `X-Ainxt-Token-Auth`. Only sent when `credentials.needs_token_auth_header()`.
...
fn build_export_headers(
    ...
    snapshot: &ainxt_auth::CredentialSnapshot,
    ...
    if let Some(value) = token_auth_header {
        headers.insert("X-Ainxt-Token-Auth".to_string(), value.to_string());
    }
...
fn build_server_provider(client: OtelClientInfo, config: OtelLayerConfig) -> SdkTracerProvider {
    ...
    if config.exporter.enabled {
        ...
        static_headers.insert(
            "x-ainxt-client-version".to_string(),
...
mod tests {
    ...
    use std::sync::Mutex;
    use ainxt_auth::{AuthCredentialProvider, CredentialSnapshot, HttpAuth};
    ...
    fn make_exporter(
        ...
        RefreshableSpanExporter {
            ...
            token_header_value: Arc::from("ainxt-cli"),
