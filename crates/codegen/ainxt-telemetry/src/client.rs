...
//!
//! Extracted from `ainxt-shell::agent::telemetry::track`. The HTTP client is
...
/// whichever [`EmitterOrigin`] prefix it carries (`ainxt-shell-` /
/// `ainxt-workspace-`). Unprefixed names pass through unchanged. Kept in
...
/// format for Mixpanel. Handles both CCP display names ("SuperAinxt Heavy")
/// and JWT-derived keys ("superainxt_heavy").
fn normalize_tier(tier: &str) -> String {
    match tier {
        "SuperAinxt Heavy" | "superainxt_heavy" => "superainxt_heavy",
        "SuperAinxt" | "superainxt" => "superainxt",
        "SuperAinxt Lite" | "superainxt_lite" => "superainxt_lite",
...
pub async fn track(event_name: &str, request_id: &str, ctx: &UserContext, mut metadata: Metadata) {
    ...
    if let (Some(url), Some(api_key)) = (&client.events_url, &client.events_api_key) {
        // Security (CWE-200): the api_key is transmitted only via the
        // `x-api-key` HTTP header below. Including it in the JSON body
        // is redundant and increases exposure if the body is logged by
        // a proxy or the receiving server.
        let body = json!({
            "viewer_context": {
    ...
    if let Some(ref mixpanel) = client.mixpanel {
        ...
        props.insert("app_name".into(), json!("ainxt"));
...
mod tests {
    ...
    /// Shell events must still strip to their bare suffix, exactly
    /// identical to the previous `strip_prefix("ainxt-shell-")` behavior.
    #[test]
    fn event_value_strips_shell_prefix() {
        assert_eq!(event_value("ainxt-shell-turn"), "turn");
        assert_eq!(
            event_value("ainxt-shell-trace_upload_attempted"),
    ...
    fn event_value_strips_workspace_prefix() {
        assert_eq!(event_value("ainxt-workspace-turn"), "turn");
    }
    ...
    fn event_value_strips_only_leading_prefix() {
        assert_eq!(event_value("ainxt-shell-workspace-x"), "workspace-x");
    }
    ...
    fn normalize_tier_maps_display_and_claim_names() {
        ...
        assert_eq!(normalize_tier("free"), "free");
        assert_eq!(normalize_tier("SuperAinxt"), "superainxt");
        assert_eq!(normalize_tier("SuperAinxt Heavy"), "superainxt_heavy");
        assert_eq!(normalize_tier("superainxt_heavy"), "superainxt_heavy");
        ...
        assert_eq!(normalize_tier("SuperAinxt Lite"), "superainxt_lite");
    ...
    /// (e.g. a future `"ainxt-shell-ext-"` origin), an earlier `ALL` entry could
