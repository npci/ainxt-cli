...
//! over ACP (`ainxt.dev/log` notifications); shell receives them in
...
pub const LOG_METHOD: &str = "ainxt.dev/log";
...
pub enum LogSource {
    ...
    #[serde(rename = "shell")]
    Shell,
    #[strum(serialize = "ainxt-pager")]
    #[serde(rename = "ainxt-pager")]
    AinxtPager,
    #[strum(serialize = "ainxt-desktop")]
    #[serde(rename = "ainxt-desktop")]
    AinxtDesktop,
}
...
/// Wire format for the `ainxt.dev/log` ACP notification params.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
fn log_path() -> PathBuf {
    ainxt_home().join(LOG_DIR).join(LOG_FILE)
}
...
///
/// Every caller of [`trim_file`] passes a path built from `ainxt_home()` plus
/// fixed path segments (this module's own log, or `sampling_log.rs`'s), or,
/// in tests, a freshly created temp directory -- neither ever contains `..`.
/// Kept as an explicit, enforced invariant (rather than one that's merely
/// true by construction) directly at the read/write sites, since `trim_file`
/// is `pub` and could in principle be called with an arbitrary path.
fn has_no_traversal_component(path: &std::path::Path) -> bool {
    !path
        .components()
...
pub fn trim_file(path: &std::path::Path) {
    if !has_no_traversal_component(path) {
        tracing::warn!(path = %path.display(), "trim_file: refusing path containing '..'");
        return;
    }
    ...
    if !has_no_traversal_component(&tmp) {
        tracing::warn!(path = %tmp.display(), "trim_file: refusing temp path containing '..'");
        return;
    }
...
///
/// Called by the `ainxt.dev/log` notification handler. Entries from
