//! Tracing layer for `target: "sampling_log"` → `~/.ainxt/logs/sampling.jsonl`.
//! Enable with `--log-sampling` or `AINXT_LOG_SAMPLING=1`.
...
const ENV_VAR: &str = "AINXT_LOG_SAMPLING";
    ...
    S: Subscriber + for<'span> LookupSpan<'span> + Send + Sync + 'static,
{
    ...
    let path = ainxt_home().join(crate::unified_log::LOG_DIR).join(LOG_FILE);
