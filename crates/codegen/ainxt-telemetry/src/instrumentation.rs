...
const ENV_ENABLED: &str = "AINXT_INSTRUMENTATION";
const ENV_LOG_PATH: &str = "AINXT_INSTRUMENTATION_LOG";
const DEFAULT_LOG_DIR: &str = "logs";
const DEFAULT_LOG_FILE: &str = "instrumentation.log";
const DEFAULT_TRACE_FILE: &str = "instrumentation.trace.json";

pub const TARGET: &str = "ainxt_instrumentation";
...
fn default_log_path() -> PathBuf {
    ainxt_home().join(DEFAULT_LOG_DIR).join(DEFAULT_LOG_FILE)
}
...
fn default_output_path(mode: InstrumentationMode) -> PathBuf {
    let root = ainxt_home().join(DEFAULT_LOG_DIR);
    match mode {
...
pub fn generate_chrome_trace(options: ChromeTraceOptions) -> Result<PathBuf> {
    ...
    // `lines()` takes ownership of the reader, so the input file's handle is
    // already released once the loop above finishes consuming the iterator.
    ...
    // Explicitly release the output file handle now that the trace has been
