...
//!
//! These structs were extracted from `ainxt-shell` so they can be
...
/// A login method was chosen from the picker. `method` is "ainxt" or "api_key";
...
/// A login flow completed successfully. `method` is "ainxt" or "api_key";
...
impl CompactionScope {
    pub fn begin(
        ...
        let percentage = ainxt_token_estimation::usage_percentage_u8(tokens_used, context_window);
...
pub struct ToolCallCompleted {
    pub tool_name: String,
    pub outcome: ainxt_file_utils::events::types::ToolOutcome,
    pub duration_ms: u64,
...
// ---------------------------------------------------------------------------
// Pager events (called from ainxt-pager via log_event)
// ---------------------------------------------------------------------------
...
/// first prompt when ainxt is launched from a non-project directory).
...
pub enum SuperAinxtUpsell {
    ...
    /// (`/usage`, `/image`, …) is invoked on the free / X Basic tiers.
    RestrictedCommand,
}

#[derive(Serialize)]
pub struct SuperAinxtUpsellShown {
    pub source: SuperAinxtUpsell,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub auth_method: Option<String>,
}

#[derive(Serialize)]
pub struct SuperAinxtUpsellClicked {
    pub source: SuperAinxtUpsell,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub auth_method: Option<String>,
}
...
/// Modeled on [`SuperAinxtUpsell`]; lets the funnel attribute the click to the
...
/// is lifted — i.e. they subscribed (externally on ainxt.dev) and came back.
/// This is the actual conversion signal for SuperAinxt Heavy subscriptions
/// attributed to ainxt: the user saw the gate in ainxt, went and
...
pub struct SubscriptionActivated {
    ...
    /// is strongly attributable to ainxt's upsell surface.
...
/// The kind of bearer that was rejected. Matches shell's `TokenType` as a
...
/// `ainxt-shell-manual_auth` (the `manual_auth` binding gets the `ainxt-shell-`
...
telemetry_event!(SuperAinxtUpsellShown, "superainxt_upsell_shown");
telemetry_event!(SuperAinxtUpsellClicked, "superainxt_upsell_clicked");
