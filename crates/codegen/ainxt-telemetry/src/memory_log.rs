...
//!
//! Provides a dedicated tracing target (`ainxt_memory`) with an optional
//! file logger that writes to `~/.ainxt/logs/memory.log`.
//!
//! ## When to use
//!
...
//! AINXT_MEMORY_LOG=0 ainxt                # disable even when enabled
//! tail -f ~/.ainxt/logs/memory.log      # watch in another terminal
//! ```

/// Tracing target for all memory system operations.
pub const TARGET: &str = "ainxt_memory";
...
mod inner {
    ...
    const ENV_MEMORY_LOG: &str = "AINXT_MEMORY_LOG";
    ...
    ///
    /// Writes to `~/.ainxt/logs/memory.log`. Filters to `ainxt_memory=trace`.
    /// Set `AINXT_MEMORY_LOG=0` to disable, `AINXT_MEMORY_LOG=/path` to redirect.
    ...
    fn resolve_log_path() -> Option<PathBuf> {
        let default_path = || ainxt_home().join("logs").join("memory.log");
