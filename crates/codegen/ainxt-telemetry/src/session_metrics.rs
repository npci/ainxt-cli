...
pub enum TraceUploadReason {
    ...
    FeatureOff,
    /// No ainxt.dev auth or deployment key.
    NoCredentials,
    /// Direct-to-bucket S3 upload.
    DirectS3,
    /// Proxy mode via ainxt.dev auth.
    Proxy,
...
impl TraceUploadReason {
    ...
    pub fn from_upload_method(method: &Option<ainxt_file_utils::UploadMethod>) -> Self {
        match method {
            Some(ainxt_file_utils::UploadMethod::Proxy { .. }) => Self::Proxy,
            Some(ainxt_file_utils::UploadMethod::S3 { .. }) => Self::DirectS3,
            Some(ainxt_file_utils::UploadMethod::Direct { .. }) => Self::DirectGcs,
...
#[cfg(test)]
mod tests {
    use ainxt_file_utils::UploadMethod;

    use super::TraceUploadReason;

    /// The `ainxt-shell-doom_loop_recovery` Mixpanel event's name and
