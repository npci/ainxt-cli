...
fn external_stream_end_to_end() {
    ...
    let mut cfg = ainxt_telemetry::external::ExternalOtelConfig::resolve_with(
        |name| match name {
    ...
    cfg.client = ainxt_telemetry::external::config::ExternalClientInfo {
        service_version: "0.0.0-test".into(),
        client_version: "0.0.0-test".into(),
        app_entrypoint: "cli".into(),
    };

    ainxt_telemetry::external::init(Some(cfg));
    assert!(ainxt_telemetry::external::is_active());
    ...
    ainxt_telemetry::log_event(ainxt_telemetry::events::SessionNew {
    ...
    assert!(
        resource_service_name
            .as_deref()
            .is_some_and(|s| s.contains("ainxt-cli")),
        "service.name=ainxt-cli is a wire commitment: {resource_service_name:?}"
    );
    for expected in [
        "ainxt_code.session_start",
        "ainxt_code.user_prompt",
        "ainxt_code.api_request",
    ] {
        assert!(
    ...
    assert_eq!(
        event_names
            .iter()
            .filter(|n| *n == "ainxt_code.session_start")
            .count(),
        1
    );
            ...
            for sm in &rm.scope_metrics {
                for metric in &sm.metrics {
                    ...
                    if let Some(Data::Sum(sum)) = &metric.data {
                        ...
                        if metric.name == "ainxt_code.session.count" {
    ...
    assert!(
        metric_names.iter().any(|n| n == "ainxt_code.session.count"),
        "missing session.count in {metric_names:?}"
    );
    assert!(metric_names.iter().any(|n| n == "ainxt_code.token.usage"));
    assert_eq!(
    ...
    ainxt_telemetry::external::shutdown();
    assert!(
