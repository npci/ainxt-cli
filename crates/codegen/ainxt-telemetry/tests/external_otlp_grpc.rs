//! gRPC transport coverage for the external OTEL stream. This matches the
...
fn external_stream_grpc_end_to_end() {
    ...
    assert!(
        metrics.iter().any(|p| p.name == "ainxt_code.session.count"),
        "missing session.count in {metrics:?}"
    );
    assert!(
        metrics.iter().any(|p| p.name == "ainxt_code.token.usage"),
