...
/// MCP server name used as the key in client/tool maps (e.g. `"github"`, `"ainxt_com_linear"`).
...
impl InitProgress {
    ...
    /// finished (matches the pre-refactor `try_start_init` contract).
...
/// SDK-side id echoed back in `ainxt.dev/mcp/sdk_call`. A named struct (rather than a
/// `(String, String)` tuple) so callers can't transpose the two strings.
///
/// `Deserialize`d straight from a `_meta["ainxt.dev/mcp/servers"]` entry, so the
...
/// The session's in-process SDK MCP servers (declared via `_meta["ainxt.dev/mcp/servers"]`,
...
struct AcpMcpRegistry {
    ...
    /// `ainxt.dev/mcp/sdk_call` over the ACP connection).
...
pub struct McpState {
    ...
    /// Persisted to `~/.ainxt/config.toml` under `[mcp_servers.<name>].disabled_tools`.
    ...
    event_writer: ainxt_file_utils::events::EventWriter,
    ...
    /// `ainxt.dev/mcp/server_status` notifications.
    ///
...
impl McpState {
    ...
    pub fn new_with_meta(configs: Vec<acp::McpServer>, meta_config_map: McpMetaConfigMap) -> Self {
        Self {
            ...
            event_writer: ainxt_file_utils::events::EventWriter::noop(),
    ...
    /// `AinxtClientHandler` cloned during `try_handshake` reads
    ...
    pub fn set_event_writer(&mut self, writer: ainxt_file_utils::events::EventWriter) {
        self.event_writer = writer;
    }

    pub fn event_writer(&self) -> &ainxt_file_utils::events::EventWriter {
        &self.event_writer
    }
        ...
        I: IntoIterator<Item = (&'a str, &'a HashMap<String, String>)>,
    {
        ...
        for (client_name, client) in &mut self.owned_clients {
            ...
            *client = Arc::new(McpClient::new_http(
            ...
            tracing::debug!(server = %client_name, "Rebuilt managed MCP HTTP client due to header change");
        }
    }
...
/// MCP initialization strategy. Defined in `ainxt-telemetry`; re-exported
/// here so existing call sites continue to work.
pub use ainxt_telemetry::enums::McpInitStrategy;
...
impl McpError {
    ...
    pub fn error_category(&self) -> ainxt_file_utils::events::McpErrorCategory {
        use ainxt_file_utils::events::McpErrorCategory;
        match self {
...
///   the frontend via `ainxt.dev/mcp/tools_changed` notifications and callable via
///   `ainxt.dev/mcp/call`.
...
/// `ainxt_tool_runtime::Tool` directly instead of going through typed wrappers.
...
impl ainxt_tool_runtime::Tool for McpErasedTool {
    ...
    fn id(&self) -> ainxt_tool_protocol::ToolId {
        ...
        ainxt_tool_protocol::ToolId::new(&qualified)
            .unwrap_or_else(|_| ainxt_tool_protocol::ToolId::new("mcp_tool").expect("valid"))
    }
    ...
    fn description(
        ...
        ainxt_tool_types::ToolDescription::new(&self.tool.name, &self.tool.description)
    }
    ...
    async fn run(
        ...
        event_writer.emit(ainxt_file_utils::events::Event::McpToolCallStarted {
            ...
            .await
        {
            ...
            Err(first_err) if client.has_auth() => {
                ...
                ew.emit(ainxt_file_utils::events::Event::McpAuthRetry {
                    server_name: server.clone(),
                ...
                if reauth_ok {
                    self.try_call_tool(&client, &raw, &mut reconnect_attempted, &mut is_timeout, ew)
                        .await
                        .map_err(|e| {
                            ainxt_tool_runtime::ToolError::custom("process_manager", e.to_string())
                        })
                } else {
                    Err(first_err)
                }
        ...
        let call_result = match dispatch_result {
            Ok(result) => result,
            Err(e) => {
                ew.emit(ainxt_file_utils::events::Event::McpToolCallCompleted {
                    server_name: server.clone(),
                    tool_name: tool.clone(),
        ...
        event_writer.emit(ainxt_file_utils::events::Event::McpToolCallCompleted {
            server_name: server.clone(),
            tool_name: tool.clone(),
            call_id: qualified_name.clone(),
            duration_ms,
        ...
        ainxt_telemetry::session_ctx::log_event(ainxt_telemetry::events::McpToolCalled {
            server_name: server.clone(),
            tool_name: tool.clone(),
            qualified_name,
            success,
...
impl McpErasedTool {
    async fn try_call_tool(
        ...
        is_timeout: &mut bool,
        ew: &ainxt_file_utils::events::EventWriter,
    ) -> Result<rmcp::model::CallToolResult, ainxt_tool_runtime::ToolError> {
        let mcp_service = client
            .ensure_initialized()
            .await
            .map_err(|e| ainxt_tool_runtime::ToolError::custom("process_manager", e.to_string()))?;
        ...
        match result {
            ...
            Ok(Err(e)) => Err(ainxt_tool_runtime::ToolError::custom(
                "process_manager",
                e.to_string(),
            )),
            Err(_) => {
                *is_timeout = true;
                ...
                Err(ainxt_tool_runtime::ToolError::custom(
                    "process_manager",
                    format!(
    ...
    async fn recover_and_retry(
        ...
        *reconnect_attempted = true;
        ...
        ew.emit(ainxt_file_utils::events::Event::McpTransportError {
        ...
        let mcp_service = match client.recover().await {
            Ok(service) => {
                ew.emit(ainxt_file_utils::events::Event::McpTransportReconnect {
                    server_name: self.tool.server_name.clone(),
                    success: true,
                    error: None,
                });
            ...
            Err(e) => {
                ...
                return Err(ainxt_tool_runtime::ToolError::custom(
                    "process_manager",
        ...
        match tokio::time::timeout(timeout_duration, mcp_service.call_tool(params)).await {
            Ok(Ok(call_result)) => Ok(call_result),
            Ok(Err(retry_err)) => Err(ainxt_tool_runtime::ToolError::custom(
                "process_manager",
                retry_err.to_string(),
            )),
            Err(_) => {
    ...
    W: AsyncWrite,
{
    ...
    /// drop the writer — matches rmcp's own `AsyncRwTransport`.
    ...
    W: AsyncWrite + Send + Unpin + 'static,
{
    ...
    fn record_decode_error(&self, line: &[u8], err: &serde_json::Error) {
        ...
        self.event_writer
            .emit(ainxt_file_utils::events::Event::McpTransportDecodeError {
...
enum PendingTransport {
    ...
    /// (`ainxt.dev/mcp/sdk_call`). Rebuildable from its `server_id` + invoker, so handshake
...
/// Uses [`AinxtClientHandler`] rather than rmcp's default `ClientInfo`
...
pub type McpService = Arc<RunningService<RoleClient, AinxtClientHandler>>;
...
/// inspection (see [`McpClient::state_kind`]). Matches the variants
...
///    (`TransportClosed`).
/// 2. [`AinxtClientHandler`] when the server pushes a notification we
...
///
/// Consumers fan these out to ACP `ainxt.dev/mcp/server_status` after 50 ms
...
pub enum McpClientEvent {
    ...
    /// stringified error, surfaced unchanged to the client (no
...
pub struct McpClient {
    ...
    /// [`AinxtClientHandler`]** constructed by
...
/// and the [`AinxtClientHandler`] it constructs during
...
/// External-config overrides for an MCP server, surfaced to ainxt-mcp
...
impl McpClient {
    ...
    /// channel. `server_id` is the id the agent echoes back in `ainxt.dev/mcp/sdk_call`; the
    ...
    pub async fn ensure_initialized(&self) -> Result<McpService, McpError> {
        ...
        tracing::info!(target: ainxt_telemetry::instrumentation::TARGET, event = "timing", name = "mcp_try_handshake", elapsed_us = handshake_elapsed);
    ...
    async fn try_handshake(
        &self,
        pending: PendingTransport,
    ) -> Result<rmcp::service::RunningService<RoleClient, AinxtClientHandler>, McpError> {
        ...
        match pending {
            ...
            PendingTransport::Acp { server_id, invoker } => {
                // Per-reverse-call backstop on `ainxt.dev/mcp/sdk_call`: the larger of the
    ...
    fn make_client_info(server_name: &str) -> ClientInfo {
        ...
        ClientInfo::new(
            capabilities,
            Implementation::new(
                format!("ainxt-shell-{server_name}"),
    ...
    /// Build the [`AinxtClientHandler`] that drives `client.serve(...)`.
    ///
    ...
    fn make_client_handler(&self) -> AinxtClientHandler {
        AinxtClientHandler {
    ...
    /// references (the [`AinxtClientHandler`] handed to
    ...
    pub async fn get_tool_registrations(
        ...
        let _ensure_init_timer =
            ainxt_telemetry::instrumentation::timer("mcp_ensure_initialized");
        let mcp_service = self.ensure_initialized().await?;

        let mut all_tools = Vec::new();
        let mut cursor: Option<String> = None;

        let _list_tools_timer = ainxt_telemetry::instrumentation::timer("mcp_list_tools");
        loop {
...
fn is_figma_mcp(server_name: &str, url: &str) -> bool {
    ...
    // Legacy direct managed name (`ainxt_com_figma`); newer clients use gateway tools (`managed_mcp_gateway_tools_enabled`).
    const MANAGED_PREFIX: &str = "ainxt_com_";
...
fn ensure_figma_user_agent(headers: &mut reqwest::header::HeaderMap, server_name: &str, url: &str) {
    ...
    headers.insert(
        ...
        reqwest::header::HeaderValue::from_static("ainxt-cli"),
    );
}
...
pub async fn start_mcp_server(
    ...
    let _per_server_timer = ainxt_telemetry::instrumentation::timer("mcp_start_one_server");
    match mcp_server {
        acp::McpServer::Stdio(acp::McpServerStdio {
            ...
            let _stdio_spawn_timer = ainxt_telemetry::instrumentation::timer("mcp_stdio_spawn");
                ...
                SafeTokioChildProcess::spawn(cmd, name.clone(), event_writer.clone()).map_err(
                    |e| {
                        ...
                        ainxt_telemetry::session_ctx::log_event(
                            ainxt_telemetry::events::McpServerFailed {
                                server_name: name.clone(),
                                error_type: ainxt_telemetry::events::McpErrorType::SpawnFailed,
        ...
        | acp::McpServer::Sse(acp::McpServerSse {
            ...
            let auth_prep = if has_existing_auth {
                ...
                let _auth_discovery_timer =
                    ainxt_telemetry::instrumentation::timer("mcp_http_auth_discovery");
                ...
                .await
                {
                    ...
                    Err(_) => {
                        ...
                        event_writer.emit(
                            ainxt_file_utils::events::Event::McpOAuthDiscoveryTimeout {
                                server_name: name.clone(),
                                url: url.clone(),
                            },
                        );
...
pub async fn start_mcp_servers(
    ...
    let _mcp_start_timer = ainxt_telemetry::instrumentation::timer("mcp_start_servers");
...
/// `ainxt.dev/mcp/server_status` events.
///
...
impl ClientHandler for AinxtClientHandler {
    ...
    // See the [`AinxtClientHandler`] doc-comment for the full RPIT
    // contract.
...
mod tests {
    ...
    async fn resilient_transport_skips_undecodable_line_and_keeps_stream_alive() {
        ...
        let mut transport = ResilientRwTransport::new(
            ...
            tokio::io::sink(),
            "fwbuild".to_string(),
            ainxt_file_utils::events::EventWriter::noop(),
        );
    ...
    fn is_figma_mcp_matches_name_and_host() {
        ...
        assert!(is_figma_mcp("Figma", "https://example.com/mcp"));
        assert!(is_figma_mcp("ainxt_com_figma", "https://example.com/mcp"));
        assert!(is_figma_mcp("AINXT_COM_FIGMA", "https://example.com/mcp"));
        assert!(is_figma_mcp("ainxt_com_FIGMA", "https://example.com/mcp"));
        ...
        assert!(!is_figma_mcp("ainxt_com_linear", "https://example.com/mcp"));
    ...
    #[test]
    fn ensure_figma_user_agent_sets_ainxt_cli_when_missing() {
        let mut headers = reqwest::header::HeaderMap::new();
    ...
    fn test_same_raw_name_different_servers_no_local_registry_collision() {
        use ainxt_computer_hub_sdk::LocalRegistry;
        use ainxt_tool_runtime::Tool;
    ...
    async fn recover_and_retry_surfaces_original_error_when_recover_fails() {
        ...
        let mut reconnect_attempted = false;
        let mut is_timeout = false;
        let ew = ainxt_file_utils::events::EventWriter::noop();
    ...
    async fn try_call_tool_http_mcperror_recovers_then_retry_succeeds() {
        ...
        let ew = ainxt_file_utils::events::EventWriter::open(tmp.path());
    ...
    async fn try_call_tool_reconnects_then_succeeds_after_retriable_transport_error() {
        ...
        async fn dead_service() -> McpService {
            ...
            let handler = AinxtClientHandler {
    ...
    fn is_auth_rejection_message_matches_auth_signals() {
        // The unchanged string captured in production for a managed handshake.
        assert!(is_auth_rejection_message(
            "MCP server 'ainxt_com_notion' handshake failed: Auth required, when send initialize request"
        ));
    ...
    fn auth_required_records_as_auth_not_init_failed_and_maps_category() {
        ...
        assert!(matches!(
            err.error_category(),
            ainxt_file_utils::events::McpErrorCategory::AuthRequired
        ));
    }
    ...
    // Matches the post-construction wiring on the `ensure_initialized`
