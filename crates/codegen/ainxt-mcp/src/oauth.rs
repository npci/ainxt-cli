...
const MCP_OAUTH_CLIENT_NAME: &str = "ainxt";
...
// mode disabled, multiple `ainxt` invocations).
//
// Layer 1 (cross-process): filesystem lock at $AINXT_HOME/mcp_auth_{safe_name}.lock
...
fn auth_lock_path(server_name: &str) -> std::path::PathBuf {
    let safe: String = server_name
        ...
        .collect();
    ainxt_config::ainxt_home().join(format!("mcp_auth_{safe}.lock"))
}
