...
//!
//! Credentials are stored in `$AINXT_HOME/mcp_credentials.json`, keyed by a
//! composite key derived from the server name and URL. This keeps MCP OAuth
//! tokens isolated from the user's ainxt auth (`auth.json`).
//!
...
/// File name for the credential store inside `$AINXT_HOME`.
const CREDENTIALS_FILENAME: &str = "mcp_credentials.json";

/// On-disk credential store: `$AINXT_HOME/mcp_credentials.json`.
///
...
impl McpCredentialStore {
    ...
    /// Load the credential store from the default path (`$AINXT_HOME/mcp_credentials.json`).
    ///
    ...
    pub fn save_default(&self) -> Result<()> {
        let path = Self::default_path().ok_or_else(|| {
            McpCredentialError::Other("no user ainxt home (set $AINXT_HOME or $HOME)".into())
        })?;
        self.save_to(&path)
    }
    ...
    /// Default path: `$AINXT_HOME/mcp_credentials.json`.
    fn default_path() -> Option<PathBuf> {
        Some(ainxt_config::user_ainxt_home()?.join(CREDENTIALS_FILENAME))
    }
}
...
mod tests {
    ...
    /// `$AINXT_HOME/mcp_credentials.json`. Existing credential files must keep
    ...
    fn save_and_load_from_file() {
        let dir = std::env::temp_dir().join("ainxt-mcp-credentials-test");
