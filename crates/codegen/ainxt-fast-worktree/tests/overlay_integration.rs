...
fn force_cleanup_snapshot(path: &Path) {
    if path.exists() {
        // `--` stops option parsing so a test-generated path can never be
        // interpreted as a `btrfs subvolume delete` flag.
...
fn test_snapshot_overlay_upper_creates_btrfs_subvolume() {
    ...
    // Create snapshot. `--` stops option parsing so test-generated paths
    // can never be interpreted as `btrfs subvolume snapshot` flags.
    ...
    // Verify it's a subvolume. `--` stops option parsing so a
    // test-generated path can never be interpreted as a flag.
...
fn test_overlay_mount_fuse_lower_btrfs_upper() {
    ...
    // Step 1: Snapshot the upper. `--` stops option parsing so
    // test-generated paths can never be interpreted as flags.
...
fn test_cleanup_orphaned_overlay_snapshots() {
    ...
    // Run orphan cleanup.
    let report = ainxt_fast_worktree::cleanup_orphaned_overlay_snapshots();
