...
///
/// This matches the skip list in `copy::gitdir::SKIP_TOP_LEVEL` +
...
/// Record the source repo root in `<worktree>/.git/ainxt-worktree-source`.
///
...
fn record_main_repo_marker(source: &Path, worktree: &Path) {
    ...
    let marker = git_dir.join("ainxt-worktree-source");
    if marker.exists() {
        return;
    }
...
/// without root+btrfs (matches `btrfs::snapshot::expose_or_reclaim_snapshot`).
#[cfg(target_os = "linux")]
...
fn execute_btrfs_worktree(
    ...
    // symlinks). Matches the overlay path's post-mount cleanup.
    let post_snapshot = || -> Result<String> {
        // Clean up stale lock files and transient git state from the snapshot.
        // This MUST happen before any git operations because even `git reset --hard`
        // will block indefinitely if the source had an `index.lock` at snapshot time.
...
mod tests {
    ...
    fn test_marker_written_for_standalone_git_dir() {
        ...
        ainxt_test_utils::git::init_git_repo(&source);
        ...
        let marker = dest.join(".git/ainxt-worktree-source");
    ...
    fn test_marker_skipped_for_linked_worktree() {
        ...
        assert!(!dest.join(".git/ainxt-worktree-source").exists());
    }
