...
/// registration directory path parsed out of a worktree's `.git` file
/// content) in a log record. Values are filtered character-by-character at
/// format time via `Display::fmt`, rather than materialized into a separate
/// sanitized `String` ahead of the call, so a log site can never
/// accidentally interpolate the raw value instead of the sanitized one.
...
fn remove_worktree_from_disk(
    ...
    {
        tracing::debug!(
            registration_dir = %LogSafe(&reg_dir.display().to_string()),
...
/// handle `~/.ainxt/worktrees/<repo>/<session>/`) and calls `remove_worktree()`
...
fn try_btrfs_remove(
    ...
    if worktree_path.is_symlink() {
        ...
        if let Ok(Some(_)) = btrfs::is_btrfs_subvolume(&resolved) {
            ...
            // `btrfs` refuses non-subvolumes, the snapshot dir is ainxt-owned, and
    ...
    if btrfs_info.bind_mount_source.is_some() {
        ...
        ainxt_tty_utils::detach_std_command(&mut umount_cmd);
        ...
        for subdir in btrfs::BTRFS_SNAPSHOT_SUBDIRS {
            ...
            for dir_entry in dir_entries.flatten() {
                ...
                if meta.mount_target.is_symlink() {
                    ...
                    ainxt_tty_utils::detach_std_command(&mut umount_cmd);
                    umount_cmd.stdin(std::process::Stdio::null());
                    // `--` stops option parsing so a mount_target value read
                    // back from on-disk metadata can never be interpreted as
                    // a umount flag (e.g. a path starting with `-`).
                    let _ = umount_cmd.arg("--").arg(&meta.mount_target).output();
...
pub mod gc {
    ...
    /// symlinked `$AINXT_HOME` or custom worktree path would otherwise never match.
...
mod tests {
    ...
    fn test_cleanup_worktrees_in_with_nested_dirs() {
        ...
        // Create ~/.ainxt/worktrees/<repo>/<session>/ structure.
        let worktrees_dir = tmp.path().join("worktrees");
    ...
    fn test_cleanup_worktrees_in_removes_nested_dangling_symlink() {
        // Dangling symlink one level deeper (~/.ainxt/worktrees/<repo>/<session>):
    ...
    fn remove_with_delegate_deregisters_plain_worktree_without_calling_delegate() {
        ainxt_test_utils::require_git!();
        use ainxt_test_utils::git::{git_commit_all, init_git_repo};
        // Isolate AINXT_HOME so the post-removal unregister writes to a private DB.
        #[cfg(feature = "metadata")]
        let _fx = crate::db::AinxtHomeFixture::new();
    ...
    fn delete_fallback_invokes_delegate_when_direct_delete_fails() {
        ...
        let report = delete_snapshot_with_delegate_fallback(
            ...
            Path::new("/home/u/.ainxt/worktrees/repo/wt"),
            Some(&delegate),
    ...
    fn test_cleanup_orphaned_btrfs_skips_when_mount_target_parent_missing() {
        ...
        let mount_target = unrestored_home.join(".ainxt/worktrees/x/wt-live");
        assert!(
    ...
    fn test_cleanup_orphaned_btrfs_skips_active() {
        ...
        let mount_target = std::path::PathBuf::from("/home/user/.ainxt/worktrees/active-wt");
    ...
    mod metadata_integration {
        ...
        fn register_worktree_writes_correct_fields() {
            // Isolate AINXT_HOME so register_worktree's open_default write lands
            // in our own DB (lock + private tmp + restore via the fixture).
            let fx = crate::db::AinxtHomeFixture::new();

            // Unique basename → unique id, so a concurrent open_default writer
            // (AINXT_HOME is process-global) can't INSERT-OR-REPLACE our row.
        ...
        fn gc_with_delegate_removes_expired_and_unregisters() {
            ...
            // AINXT_HOME == the gc DB dir so remove_worktree's open_default
