...
/// `tracing::` log call sites that embed externally influenced data (e.g.
/// `/proc/self/mountinfo` contents, `findmnt` output) in a log record. Values
/// are filtered character-by-character at format time via `Display::fmt`,
/// rather than materialized into a separate sanitized `String` ahead of the
/// call, so a log site can never accidentally interpolate the raw value
/// instead of the sanitized one.
...
pub fn get_bind_mount_info(path: &Path) -> Result<Option<BindMountInfo>> {
    ...
    let output = match output {
        ...
        Ok(_) => {
            tracing::debug!(path = %LogSafe(&path.display().to_string()), "findmnt failed or path not mounted");
            return Ok(None);
        }
        Err(e) => {
            tracing::debug!(path = %LogSafe(&path.display().to_string()), error = %e, "failed to run findmnt");
            return Ok(None);
        }
    };
    ...
    if parts.len() < 3 {
        tracing::debug!(path = %LogSafe(&path.display().to_string()), line = %line, "unexpected findmnt output format");
        return Ok(None);
    }
    ...
    if !is_bind {
        tracing::debug!(
            path = %LogSafe(&path.display().to_string()),
            source = %LogSafe(source),
            "not a bind mount"
        );
        return Ok(None);
    }
    ...
    if let Some(actual_source) = actual_source {
        tracing::debug!(
            path = %LogSafe(&path.display().to_string()),
            source = %LogSafe(&actual_source.display().to_string()),
            "detected bind mount"
        );
        ...
        if let Some(fstype_idx) = fstype_idx {
            ...
            if *fstype == "btrfs" {
                ...
                match resolve_via_subvol_mount(source, root, &mountinfo) {
                    ...
                    Ok(None) => {
                        tracing::debug!(
                            device = %LogSafe(source),
                            root = %LogSafe(root),
                            target = %LogSafe(&target_str),
...
/// root `/repo/.ainxt-snapshots/wt-123`, this returns
/// `/workspace/repo/.ainxt-snapshots/wt-123`.
...
pub fn is_btrfs_subvolume(path: &Path) -> Result<Option<BtrfsInfo>> {
    ...
    if !on_btrfs {
            ...
            && check_is_subvolume_cmd(&bind_info.source)
        {
            ...
            tracing::info!(
                path = %LogSafe(&path.display().to_string()),
                source = %LogSafe(&bind_info.source.display().to_string()),
        ...
        tracing::debug!(path = %LogSafe(&path.display().to_string()), "not a BTRFS subvolume");
        return Ok(None);
    }
    ...
    // IS on btrfs), but the snapshot destination (e.g. `~/.ainxt/worktrees/...`)
...
fn check_is_subvolume_cmd(path: &Path) -> bool {
    ...
    cmd.stdin(Stdio::null());
    // OsStr arg: a non-UTF-8 path must not silently collapse to ".".
    // `--` stops option parsing so `path` (sourced from the live mount
    // table via `get_bind_mount_info`) can never be interpreted as a
    // `btrfs subvolume show` flag.
    cmd.arg("subvolume")
        .arg("show")
        .arg("--")
        .arg(path)
...
mod tests {
    ...
    fn test_resolve_via_subvol_mount_nested_path() {
        // Simulates: target root /repo/.ainxt-snapshots/wt-123 resolved via
        // mount with root=/repo at /workspace/repo
        let mountinfo =
            "8267 8961 0:813 /repo /workspace/repo rw,relatime - btrfs /dev/loop0 rw,ssd";
        let result =
            resolve_via_subvol_mount("/dev/loop0", "/repo/.ainxt-snapshots/wt-123", mountinfo)
                .unwrap();
        assert_eq!(
            result,
            Some(PathBuf::from("/workspace/repo/.ainxt-snapshots/wt-123"))
        );
    }
