...
/// path derived from a repo-controlled `.git`/`commondir` file) in a log
...
fn link_shared_indexes(source_git_dir: &Path, dest_git_dir: &Path) -> Result<()> {
    ...
    if linked > 0 {
        tracing::debug!(
            source_common_dir = %LogSafe(&source_common_dir.display().to_string()),
            dest_git_dir = %LogSafe(&dest_git_dir.display().to_string()),
            linked,
