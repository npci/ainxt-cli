...
//! matching the old scanner's even/odd equivalence rule. Content inside inline code
//! spans and fenced code blocks is left unchanged (so LaTeX-in-backticks stays
        ...
        while i < n {
            match self.state {
                ...
                State::InlineCode { run } => {
                    // Single-line span: copy unchanged until a matching-length
                ...
                State::Fenced { ch, len } => {
                    ...
                    // Copy the rest of this line unchanged (fenced content).
                    let start = i;
                    while i < n && bytes[i] != b'\n' {
                        i += 1;
                    }
