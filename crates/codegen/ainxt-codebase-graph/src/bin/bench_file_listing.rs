...
use ainxt_codebase_graph::LanguageRegistry;

fn main() {
    let args: Vec<String> = std::env::args().collect();
    let path_str = if let Some(p) = args.get(1) {
        p.clone()
    } else if let Ok(p) = std::env::var("BENCH_REPO_ROOT").or_else(|_| std::env::var("AINXT_ROOT")) {
        p
    } else {
    ...
    if let Ok(statuses) = repo.statuses(Some(&mut status_opts)) {
        for status_entry in statuses.iter() {
            if status_entry.status().is_wt_new()
                && let Some(path_str) = status_entry.path().ok()
