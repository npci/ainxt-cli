...
mod tests {
    ...
    /// (0600) applied atomically at open time via `OpenOptions::mode`, rather
    /// than as a follow-up `set_permissions()` call. These test fixtures live
    /// in an ephemeral `tempdir()`-scoped directory with no sensitive
    /// content, but we harden them anyway for consistency with production
    /// file-creation call sites (see `auth/gateway_auth.rs::write_owner_only`
    /// for the same pattern on a real secret). Best-effort mode on non-Unix
    /// platforms, which lack POSIX mode bits.
    ...
    fn test_get_symbol_at_position() {
        ...
        let mut file = create_owner_only_file(&file_path);
        file.write_all(b"fn hello_world() {\n    println!(\"Hello\");\n}\n")
            .unwrap();
        drop(file);
    ...
    fn test_typescript_for_of_array_destructuring() {
        ...
        let mut file = create_owner_only_file(&file_path);
        file.write_all(
            br#"const myMap = new Map<string, { sessionId: string }>();
...
const result = myObject.value;"#,
    ...
    fn test_typescript_function_parameter_destructuring() {
        ...
        let mut file = create_owner_only_file(&file_path);
        file.write_all(
            br#"function process([first, second]: number[], { name }: { name: string }) {
  console.log(first, second, name);
    ...
    fn test_typescript_simple_function_parameters() {
        ...
        let mut file = create_owner_only_file(&file_path);
        file.write_all(
            br#"function greet(name: string, age: number) {
  console.log(name, age);
    ...
    fn test_typescript_react_dependency_array_references() {
        ...
        let mut file = create_owner_only_file(&file_path);
        file.write_all(
            br#"import { useCallback, useEffect } from 'react';
