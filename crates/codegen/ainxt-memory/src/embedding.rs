...
pub(super) fn build_middleware_client(
    ...
    ainxt_http::with_auth_retry(ainxt_http::shared_client(), credentials)
}

fn build_static_middleware_client(
    api_key: Option<String>,
) -> reqwest_middleware::ClientWithMiddleware {
    let provider: std::sync::Arc<dyn ainxt_auth::AuthCredentialProvider> = std::sync::Arc::new(
        ainxt_auth::StaticAuthCredentialProvider::new(Box::new(NoopHttpAuth), api_key),
    );
...
impl ainxt_auth::HttpAuth for NoopHttpAuth {
    ...
    async fn embed_batch(
        ...
        for batch in texts.chunks(self.max_batch_size) {
            ...
            for attempt in 0..MAX_RETRIES {
                ...
                let request = ainxt_http::shared_client()
