...
//! `~/.ainxt/memory/{project-slug}-{hash8}/` to avoid polluting the user's repo.

use std::path::{Path, PathBuf};

use ainxt_tools::util::ainxt_home::ainxt_home;

/// Scope for a memory write operation.
/// Write-operation scope. Distinct from `ainxt_agent::config::MemoryScope` (agent memory dir).
...
/// `~/.ainxt/memory/`. Workspace-scoped files live under a directory
/// named `{project-slug}-{hash8}`, e.g. `~/.ainxt/memory/ainxt-a3f7b2c9/`.
#[derive(Debug, Clone)]
pub struct MemoryStorage {
    /// `~/.ainxt/memory/`
    global_dir: PathBuf,
    /// `~/.ainxt/memory/{project-slug}-{hash8}/`
...
impl MemoryStorage {
    /// Create a new `MemoryStorage` rooted at `~/.ainxt/memory/`.
    ///
    ...
    fn new_inner(cwd: &Path, root_override: Option<&Path>, use_workspace_hash: bool) -> Self {
        let global_dir = root_override
            .map(|p| p.to_path_buf())
            .unwrap_or_else(|| ainxt_home().join("memory"));
    ...
    pub fn total_chunk_count(&self) -> usize {
        ...
        // mounts (SIGBUS); see ainxt_sqlite_journal::JournalMode::open_readonly.
        ainxt_sqlite_journal::JournalMode::for_db_path(&db_path)
    ...
    ///
    /// File path: `~/.ainxt/memory/{project}-{hash8}/sessions/YYYY-MM-DD-{slug}-{sid8}.md`
    ///
    /// - `date`: e.g. `"2026-02-23"`
    ...
    pub fn ensure_initialized(&self) -> std::io::Result<()> {
        ...
        if !global_file.exists() {
            std::fs::write(
                ...
                "# Global Memory\n\
                 \n\
                 > This file is automatically managed by ainxt's memory system.\n\
...
mod tests {
    ...
    fn test_compute_workspace_hash_human_readable() {
        let name = compute_workspace_hash(Path::new("/users/me/work/ainxt"));
        assert!(
            name.starts_with("ainxt-"),
