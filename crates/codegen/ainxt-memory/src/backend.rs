...
use ainxt_tools::types::memory_backend::{MemoryBackend, MemorySearchResult};
...
pub struct MemoryBackendParams {
    ...
    pub embed_config: Option<ainxt_config_types::MemoryEmbeddingConfig>,
    ...
    pub search_config: ainxt_config_types::MemorySearchConfig,
    ...
    pub auth_credentials: Option<Arc<dyn ainxt_auth::AuthCredentialProvider>>,
}
...
async fn build_embedding_provider(
    config: Option<&ainxt_config_types::MemoryEmbeddingConfig>,
    auth_credentials: Option<&Arc<dyn ainxt_auth::AuthCredentialProvider>>,
...
pub struct MemoryBackendImpl {
    ...
    embed_config: Option<ainxt_config_types::MemoryEmbeddingConfig>,
    ...
    search_config: ainxt_config_types::MemorySearchConfig,
    ...
    /// Session ID for telemetry events.
    session_id: String,
    /// Telemetry label for search events — matches [`MemoryBackendParams::search_source`].
...
impl MemoryBackendImpl {
    ...
    pub fn new(db_path: PathBuf, storage: MemoryStorage) -> Self {
        Self {
            ...
            embed_api_key: None,
            search_config: ainxt_config_types::MemorySearchConfig::default(),
            watcher: None,
            stale_claim_secs: 60,
    ...
    pub fn with_embedding(
        mut self,
        config: ainxt_config_types::MemoryEmbeddingConfig,
        base_url: String,
    ...
    pub fn with_search_config(mut self, config: ainxt_config_types::MemorySearchConfig) -> Self {
    ...
    fn open_readonly(&self) -> Result<rusqlite::Connection, rusqlite::Error> {
        ...
        ainxt_sqlite_journal::JournalMode::for_db_path(&self.db_path).open_readonly(&self.db_path)
    }
...
impl MemoryBackendImpl {
    ...
    pub fn search_config_for_test(&self) -> &ainxt_config_types::MemorySearchConfig {
        &self.search_config
    }
}
...
impl MemoryBackend for MemoryBackendImpl {
    ...
    async fn search(
        ...
        let mut index = super::index::MemoryIndex::open_or_create(
            &self.db_path,
            self.storage.clone(),
            ainxt_config_types::MemoryIndexConfig::default(),
            embed_dims,
        )
        ...
        if needs_release {
            ...
            if let Some((dirty_count, reindexed_count, sync_start)) = watcher_sync_stats {
                ainxt_telemetry::session_ctx::log_event(
                    ainxt_telemetry::memory_telemetry::MemoryWatcherSync {
        ...
        // out of the base FTS results. Matches hybrid_search() in search.rs.
        let evergreen = index
        ...
        if results.is_empty() {
            ainxt_telemetry::session_ctx::log_event(
                ainxt_telemetry::memory_telemetry::MemorySearchEmpty {
                    session_id: self.session_id.clone(),
            ...
            ainxt_telemetry::session_ctx::log_event(
                ainxt_telemetry::memory_telemetry::MemorySearch {
                    session_id: self.session_id.clone(),
                    query_length: query.len(),
                    keyword_count,
...
mod factory_tests {
    ...
    use ainxt_config_types::{MemoryEmbeddingConfig, MemorySearchConfig};
    ...
    fn test_factory_wires_non_overridable_search_config_fields() {
        ...
        let custom_search = MemorySearchConfig {
            max_results: 7,
            mmr: ainxt_config_types::MmrConfig {
                enabled: true,
                lambda: 0.42,
            },
            temporal_decay: ainxt_config_types::TemporalDecayConfig {
                enabled: true,
                half_life_days: 14.0,
            },
            ..Default::default()
    ...
    /// start successfully.
    ///
    /// This matches the ordering enforced in `spawn_session_actor`:
    ...
    async fn make_embedding_provider_uses_async_api_key_resolution() {
        ...
        let probe: ainxt_tools::types::SharedApiKeyProvider = Arc::new(AsyncProbe {
...
mod tests {
    ...
    use crate::index::{MemoryIndex, init_sqlite_vec};
    use tempfile::TempDir;
    use ainxt_config_types::MemoryIndexConfig;
