...
impl MemoryIndex {
    ...
    /// a stale one).  Used by `ainxt memory doctor` to detect stuck states.
    ...
    ///
    /// Used by `ainxt memory doctor` to detect orphaned chunks (chunks whose
...
mod tests {
    ...
    /// 3. Run the same orphan-removal logic as `ainxt memory reindex`:
    ...
    ///
    /// This proves that `ainxt memory reindex`'s Phase 1 actually fixes the
    /// state that `ainxt memory doctor` warns about.
    #[test]
    fn test_reindex_maintenance_removes_orphaned_chunks() {
        ...
        // Simulate `ainxt memory reindex` Phase 1: compare indexed vs current.
    ...
    ///
    /// Verifies that `ainxt memory reindex` Phase 0 correctly bails when a
    ...
    /// `ainxt memory reindex` Phase 3 resets the stale reindex claim.
    ///
    /// Verifies that `release_claim()` clears `meta.reindex_claim` so that
    /// `ainxt memory doctor` no longer reports a stale lock after reindex runs.
    #[test]
    fn test_reindex_maintenance_resets_stale_claim() {
        ...
        // Simulate `ainxt memory reindex` Phase 3: release the claim.
        idx.release_claim();
