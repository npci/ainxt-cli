//! Shared announcement types, persistence, and formatting for ainxt CLI apps.
//!
//! This crate provides the common logic used by `ainxt-shell` and
//! `ainxt-pager` for handling announcements (banner notifications).
...
/// Payload for `ainxt.dev/announcements/update` ACP notification.
...
/// Read hidden announcement ids from `~/.ainxt/announcements.json`.
...
/// Write hidden announcement ids to `~/.ainxt/announcements.json`.
...
fn announcements_state_path() -> PathBuf {
    ainxt_tools::util::ainxt_home::ainxt_home().join("announcements.json")
}
...
///
/// Precedence: `AINXT_ANNOUNCEMENTS_OVERRIDE` env var (JSON override) → remote announcements.
...
pub fn resolve_startup(
    ...
    if let Ok(raw) = std::env::var("AINXT_ANNOUNCEMENTS_OVERRIDE") {
        match serde_json::from_str::<Vec<RemoteAnnouncement>>(&raw) {
            Ok(list) => return Some(list),
            Err(e) => {
                tracing::warn!(error = %e, "invalid AINXT_ANNOUNCEMENTS_OVERRIDE JSON; ignoring override");
            }
        }
    }
...
mod tests {
    ...
    fn resolve_startup_env_override() {
        ...
        unsafe {
            std::env::set_var("AINXT_ANNOUNCEMENTS_OVERRIDE", r#"[{"id":"test"}]"#);
        }
        ...
        // SAFETY: test-only
        unsafe {
            std::env::remove_var("AINXT_ANNOUNCEMENTS_OVERRIDE");
        }
    }
    ...
    fn cta_parses_nested_partial_and_absent() {
        let full: RemoteAnnouncement = serde_json::from_str(
            r#"{"id":"p","severity":"promo","cta":{"label":"Subscribe","url":"https://ainxt.example.test/subscribe","caption":"or use Ctrl+O"}}"#,
        )
        .unwrap();
        let cta = full.cta.as_ref().expect("cta present");
        assert_eq!(cta.label.as_deref(), Some("Subscribe"));
        assert_eq!(cta.url.as_deref(), Some("https://ainxt.example.test/subscribe"));
