...
# are the single source of truth for the `ainxt.dev/announcements/update` payload.
# Pipeline:
...
DESKTOP_DIR="$REPO_ROOT/frontend/apps/ainxt-desktop"
...
for f in "${bindings[@]}"; do
  {
    ...
    echo "// Regenerate via generate.sh in the ainxt-announcements package."
    cat "$f"
  } >"$DST/$(basename "$f")"
done
