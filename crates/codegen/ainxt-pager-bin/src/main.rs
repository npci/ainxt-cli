...
use ainxt_pager::app::{WorkspaceMgmtArgs, WorkspaceMgmtCommand, WorkspaceStartArgs};
use ainxt_pager::client_identity::PAGER_CLIENT_VERSION;
use ainxt_shell::agent::app::{run_headless, run_leader, run_stdio_agent};
use ainxt_shell::agent::config::Config as AgentConfig;
use ainxt_shell::leader::{
...
use ainxt_update::{UpdateConfig, auto_update, enforce_minimum_version_or_exit};
...
fn apply_headless_args_to_config(args: &HeadlessArgs, config: &mut AgentConfig) {
    if let Some(v) = &args.ainxt_ws_origin {
        config.ainxt_com_config.ainxt_ws_origin = v.clone();
    }
    if let Some(v) = &args.ainxt_ws_url {
        config.ainxt_com_config.ainxt_ws_url = v.clone();
    }
}
/// Apply global endpoint CLI args to an existing config.
fn apply_agent_endpoint_args(
    agent_args: &ainxt_pager::app::AgentArgs,
    config: &mut AgentConfig,
) {
    if let Some(v) = &agent_args.cli_chat_proxy_base_url {
        config.endpoints.cli_chat_proxy_base_url = Some(v.clone());
    }
    if let Some(v) = &agent_args.ainxt_api_base_url {
        config.endpoints.ainxt_api_base_url = v.clone();
    }
}
...
///
/// The secret itself is never printed to the terminal (which can be
/// captured by session recorders, terminal multiplexer scrollback, or
/// screen-sharing); only a short prefix is shown so an operator who already
/// knows the secret can sanity-check they're looking at the right value.
fn print_serve_startup_info(bind_addr: SocketAddr, secret: &str) {
    let masked = mask_secret_for_display(secret);
    eprintln!();
    eprintln!("   ainxt agent server starting...");
    eprintln!();
    eprintln!("   Address:  {}:{}", bind_addr.ip(), bind_addr.port());
    eprintln!("   Secret:   {masked} (see --secret / AINXT_AGENT_SECRET to recover the full value)");
    eprintln!();
    eprintln!("   WebSocket URL: ws://{bind_addr}/ws?server-key=<secret>");
    eprintln!();
}

/// Render a secret as a short, non-reversible display form: the first 4
/// characters followed by an ellipsis, or fully masked when too short for a
/// partial prefix to be safe.
...
/// Entrypoint tag for `ainxt -p`; keys the quiet stderr default in `init_tracing_simple`.
...
fn init_tracing_simple(app_entrypoint: &'static str) {
    ...
    use ainxt_telemetry::debug_log::RMCP_SSE_NOISE_TARGET;
    ...
    let registry = tracing_subscriber::registry()
        ...
        .with(ainxt_telemetry::sampling_log::layer())
        .with(ainxt_telemetry::instrumentation::layer())
        .with(ainxt_telemetry::hooks_log::layer())
        .with(ainxt_telemetry::otel_layer::build_otel_layer(
            ainxt_telemetry::otel_layer::OtelClientInfo {
                client_name: "ainxt-pager",
                client_version: ainxt_version::VERSION,
                service_version: env!("VERSION_WITH_COMMIT"),
                app_entrypoint,
            },
            ainxt_shell::auth::credential_provider::build_default_otel_layer_config(),
        ));
    ainxt_telemetry::debug_log::install_firehose(registry, app_entrypoint);
    ainxt_telemetry::external::init(
        ainxt_shell::agent::config::resolve_external_otel_config(
            ainxt_telemetry::external::config::ExternalClientInfo {
                service_version: env!("VERSION_WITH_COMMIT").to_owned(),
                client_version: ainxt_version::VERSION.to_owned(),
                app_entrypoint: app_entrypoint.to_owned(),
            },
        ),
    );
}
/// `ainxt setup`: rendering + exit codes only; fetch logic lives in `ainxt_shell::managed_config`.
/// `json` prints the served configuration instead of installing it.
async fn run_setup_command(json: bool) {
    use ainxt_shell::managed_config::{self, SetupOutcome};
    if !managed_config::has_principal() {
        eprintln!("No deployment key or team sign-in found.");
        eprintln!();
        eprintln!("To install managed configuration, sign in with a team using `ainxt login`,");
        eprintln!("or set a deployment key:");
        eprintln!();
        if cfg!(unix) {
            eprintln!("  export AINXT_DEPLOYMENT_KEY=<your-key>");
        } else {
            eprintln!("  $env:AINXT_DEPLOYMENT_KEY=\"<your-key>\"");
        }
        eprintln!("  ainxt setup");
        eprintln!();
        eprintln!("Or add the key to ~/.ainxt/config.toml:");
        eprintln!();
        eprintln!("  [endpoints]");
        eprintln!("  deployment_key = \"<your-key>\"");
        eprintln!();
        eprintln!(
            "If you don't have a deployment key, contact your organization's ainxt administrator."
        );
        std::process::exit(1);
    }
    if json {
        match managed_config::fetch_setup_report().await {
            Ok(report) => {
                ...
                if !report.configured {
                    eprintln!(
                        "Your team doesn't have a managed configuration yet. A team admin can set one up at console.ainxt.dev."
                    );
                }
            }
            Err(e) => {
    ...
    match managed_config::run_setup().await {
        ...
        SetupOutcome::Skipped => {
            eprintln!(
                "Managed configuration was not applied this run (another process held the apply lock, or the credential changed during the fetch). Run `ainxt setup` again."
            );
        }
...
async fn run_leader_mgmt(args: LeaderMgmtArgs) -> Result<()> {
    match args.command {
        ...
        LeaderMgmtCommand::List { json } => {
            let leaders = ainxt_shell::leader::discover_leaders().await;
            if json {
...
async fn kill_leaders() -> Result<()> {
    ...
    for d in &leaders {
        ...
        if !ainxt_shell::util::is_ainxt_process(pid) {
            if let Some(ref lock) = d.lock_path {
                eprintln!("  PID {pid} is not a ainxt process, removing stale lock");
        ...
        if let Err(e) = ainxt_shell::util::kill_process_by_pid(pid) {
...
fn resolve_target(args: &LeaderTargetArgs) -> LeaderTarget {
    match args.pid {
        ...
        None => LeaderTarget::Environment(ainxt_shell::env::AinxtBuildEnvironment::Production),
    }
}
async fn connect_to_leader(
    args: &LeaderTargetArgs,
) -> Result<(LeaderDescriptor, ainxt_shell::leader::LeaderClient)> {
    let target = resolve_target(args);
    let selection = ainxt_shell::leader::resolve_leader_target(target)
        .await
        .map_err(|e| anyhow::anyhow!("{}", e.message))?;
    let socket_path = selection
        .socket_path()
        .ok_or_else(|| anyhow::anyhow!("resolved leader target did not include a socket path"))?;
    let client = ainxt_shell::leader::LeaderClient::connect(
        socket_path.to_path_buf(),
        "ainxt-pager-leader-cli",
        ClientMode::Stdio,
...
fn leader_info_json(
    ...
    info: Option<&ainxt_shell::leader::ControlPayload>,
...
/// Env override for the `ainxt workspace` gate: any truthy value enables the
/// command locally, a falsy one disables it — bypassing the remote settings flag.
const WORKSPACE_COMMAND_ENV: &str = "AINXT_WORKSPACE_COMMAND";
/// Resolution of the `ainxt workspace` gate. `Unknown` is kept separate from
...
/// The `AINXT_WORKSPACE_COMMAND` override, if set (`Some(true)`/`Some(false)`);
...
fn workspace_command_gate(
    ...
    remote_settings: Option<&ainxt_shell::util::config::RemoteSettings>,
) -> WorkspaceGate {
...
/// Truthy parse for ainxt on/off env vars: everything enables except the common
...
fn fetch_remote_settings() -> Option<ainxt_shell::util::config::RemoteSettings> {
    join_early_prefetch(ainxt_shell::agent::models::start_early_prefetch(None))
}
async fn run_workspace_mgmt(args: WorkspaceMgmtArgs) -> Result<()> {
    ...
    match workspace_command_gate(env_override, remote_settings.as_ref()) {
        ...
        WorkspaceGate::Disabled => {
            anyhow::bail!(
                "`ainxt workspace` is not enabled for this account \
             (gated by a server-side feature flag that is currently off)."
            )
        }
        WorkspaceGate::Unknown => {
            anyhow::bail!(
                "Could not load your settings for `ainxt workspace`. Check your \
             network connection (run `ainxt login` if you are signed out), then \
             try again."
            )
        }
    }
...
async fn connect_workspace_control(
    ...
    let ws_url = &agent_config.ainxt_com_config.ainxt_ws_url;
    ...
    .map_err(|e| {
        anyhow::anyhow!(
            "no running leader for this environment ({e}). \
             Start a ainxt session, or run `ainxt workspace start`."
        )
    })
}
async fn workspace_control(
    ...
    let raw_config = ainxt_shell::config::load_effective_config_disk_only()
...
async fn workspace_start(
    ...
    use ainxt_shell::auth::ensure_authenticated;
    ainxt_shell::util::config::set_remote_campaigns_from_settings(remote_settings.as_ref());
    let raw_config = ainxt_shell::config::load_effective_config()
        .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
    let agent_config = AgentConfig::new_from_toml_cfg(&raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
    ...
    if !use_leader {
        anyhow::bail!(
            "`ainxt workspace` requires leader mode (the workspace is shared via the leader).\n\
             Enable it with `[cli] use_leader = true` in ~/.ainxt/config.toml, or pass --leader."
        );
    }
    ensure_authenticated(
        &agent_config.ainxt_com_config,
        false,
        Some("No cached credentials found. Run `ainxt login` first."),
    )
    .await?;
    let env_urls = LeaderEnvUrls::from(&agent_config.ainxt_com_config);
...
struct CachedSession {
    /// Unchanged `session/load` request JSON (preferred replay form: preserves
...
struct StdioReplayState {
    ...
    /// `ainxt.dev/leader_reconnected` as the primary restored session.
...
fn cache_outgoing_acp_state(msg: &str, state: &std::sync::Mutex<StdioReplayState>) {
    ...
    match method {
        ...
        "ainxt.dev/session/close" | "_ainxt.dev/session/close" => {
...
const REPLAY_LOAD_REQUEST_ID: &str = "ainxt.dev/leader-replay/session-load";
...
/// response". Everything that is not the response itself is forwarded unchanged
...
/// unchanged client request when available, else a synthesized load from the
...
/// `ainxt.dev/leader_reconnected` with empty params in that case, signalling the
...
fn shutdown_and_flush_telemetry(exit_code: i32) -> ! {
    ainxt_telemetry::sentry::flush_on_shutdown();
    ainxt_telemetry::otel_layer::shutdown_otel();
    ainxt_telemetry::debug_log::flush();
    std::process::exit(exit_code);
}
/// Emitted by both leader guards (server mode and leader-connect) so the two sites
/// can't drift.
const PLUGIN_DIR_LEADER_WARNING: &str = "ainxt: --plugin-dir is ignored in leader mode; run with --no-leader to \
     load per-process plugins";
/// Run the `agent` subcommand, dispatching to the appropriate mode.
async fn run_agent_command(
    agent_args: Box<ainxt_pager::app::AgentArgs>,
    ...
    let _otel_guard = ainxt_telemetry::otel_layer::otel_guard();
    ainxt_telemetry::instrumentation::install_panic_hook();
    if trust {
        match std::env::current_dir() {
            Ok(cwd) => ainxt_shell::agent::folder_trust::grant_folder_trust(&cwd),
            Err(e) => {
                tracing::warn!(
    ...
    let early_prefetch = ainxt_shell::agent::models::start_early_prefetch(None);
    ainxt_shell::agent::mvp_agent::warm_async_http_client();
    ...
    if !is_stdio && !is_leader {
        eprintln!(
            "ainxt (pager) - v{}",
            ainxt_version::display_version_with_commit(
    ...
    // Global API backend override: flag > env > config. Absent all three,
    // leave it unset so dispatch falls back to each model's own configured
    // `api_backend` instead of forcing one backend on every model — see
    // `ainxt_sampler::effective_api_backend`.
    if let Some(backend) = agent_args
        .api_backend
        .clone()
        .or_else(|| std::env::var("AINXT_API_BACKEND").ok())
        .or_else(|| agent_config.models.api_backend.clone())
        .as_deref()
        .and_then(ainxt_shell::sampling::parse_api_backend)
    {
        ainxt_shell::sampling::set_forced_api_backend(backend);
    }
    agent_config.reasoning_effort_override = agent_args
        .reasoning_effort
        .as_deref()
        .and_then(ainxt_shell::sampling::types::parse_canonical_effort_token);
    let launch_yolo = ainxt_shell::util::config::effective_yolo_for_launch(
        agent_args.yolo,
        permission_mode_flag.as_deref(),
        None,
    );
    if let Some(warning) = launch_yolo.blocked_warning {
        eprintln!("ainxt: {warning}");
    }
    agent_config.default_yolo_mode = launch_yolo.yolo;
    agent_config.default_auto_mode = ainxt_shell::util::config::effective_auto_for_launch(
        agent_args.yolo,
        permission_mode_flag.as_deref(),
        None,
    );
    ...
    agent_config.resolve_runtime_fields(&ainxt_shell::agent::config::RuntimeResolutionContext {
    ...
    let managed_install = is_managed_install(
        ...
        &ainxt_shell::util::ainxt_home::ainxt_home(),
    );
    ...
    if use_leader {
        ...
        let env_urls = ainxt_shell::leader::LeaderEnvUrls::from(&agent_config.ainxt_com_config);
        ...
        match mode {
            ClientMode::Stdio => {
                ...
                let stdin_task = tokio::spawn(async move {
                    let mut stdin_lines = ainxt_acp_lib::spawn_stdin_line_reader();
                    loop {
                        tokio::select! {
                                    ...
                                    .await
                                {
                                    Ok((new_tx, mut new_rx, _disconnect_rx)) => {
                                        ...
                                        let notification = format!(
                                            r#"{{"jsonrpc":"2.0","method":"ainxt.dev/leader_reconnected","params":{params}}}"#
                                        );
    ...
    match agent_args.mode {
        ...
        Some(AgentCmd::Serve(a)) => {
            ...
            let server_config = ainxt_shell::agent::ServerConfig {
                bind_addr: a.bind,
                secret: secret.clone(),
            };
            print_serve_startup_info(a.bind, &secret);
            ainxt_shell::agent::run_agent_server(server_config, agent_config).await
        }
        Some(AgentCmd::Leader(a)) => {
            ...
            let leader_auto_update = if !should_check_for_updates(
                ...
                Some(ainxt_shell::agent::app::LeaderAutoUpdateConfig {
                    check_interval: std::time::Duration::from_secs(60 * 60),
                    check_fn: Box::new(move || {
                        let uc = update_config_for_leader.clone();
                        Box::pin(async move {
                            let current_config = ainxt_shell::util::config::load_config().await;
                            ...
                            match auto_update::ensure_latest_on_disk(&uc).await {
                                Ok(outcome) => {
                                    if let Some(v) = &outcome.installed {
                                        if let Err(e) = ainxt_shell::managed_config::sync().await
                                        {
                                            tracing::warn!(
...
/// Sets `AINXT_OPEN_DASHBOARD_AT_STARTUP=1` if the user asked for
/// `ainxt dashboard`, and clears `args.command` so the regular
...
/// the leader roster — so `ainxt dashboard` does NOT force leader mode and
/// is compatible with `--no-leader`.
///
/// The only gate is the feature flag: a disabled dashboard
/// (`[dashboard].enabled = false` / `AINXT_AGENT_DASHBOARD=0`) is a CLI
...
fn flag_dashboard_at_startup_if_requested(args: &mut PagerArgs) -> Result<()> {
    ...
    if !ainxt_pager::views::dashboard::dashboard_enabled() {
        anyhow::bail!(
            "the Agent Dashboard is disabled. Enable it by removing \
             `[dashboard] enabled = false` from ~/.ainxt/config.toml and \
             unsetting AINXT_AGENT_DASHBOARD=0."
        );
    }
    args.command = None;
    unsafe { std::env::set_var("AINXT_OPEN_DASHBOARD_AT_STARTUP", "1") };
    Ok(())
}
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_allocator_stats() -> Option<ainxt_pager::memory_trace::AllocatorStats> {
    ...
    unsafe {
        ...
        Some(ainxt_pager::memory_trace::AllocatorStats {
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_heap_stats() -> Option<ainxt_shell::heap_profile::JemallocStats> {
    unsafe {
        tikv_jemalloc_ctl::raw::write(b"epoch\0", 1u64).ok()?;
        ...
        Some(ainxt_shell::heap_profile::JemallocStats {
            allocated,
            resident,
        })
    }
...
fn install_heap_profile_hooks() {
    ainxt_shell::heap_profile::install(ainxt_shell::heap_profile::HeapProfileHooks {
...
/// Stack for the thread that runs `real_main`.
///
/// Windows gives a process's main thread only a 1 MiB stack by default, while
/// Linux and macOS give 8 MiB. The synchronous bootstrap plus the large
/// `async_main` future that `block_on` polls inline overflow 1 MiB (`thread
/// 'main' has overflowed its stack` at startup). Running the real entry point
/// on an explicitly-sized thread makes the available stack identical on every
/// platform instead of depending on the OS default or linker flags.
const MAIN_STACK_SIZE: usize = 32 * 1024 * 1024;

fn main() {
    let worker = std::thread::Builder::new()
        .name("ainxt-main".to_string())
    ...
    match worker.join() {
        Ok(()) => {}
        // `real_main` panicked. Its panic hook already printed the message and
        // any backtrace on the worker thread, so just exit with the standard
        // Rust panic status instead of re-printing via `resume_unwind`.
        Err(_) => std::process::exit(101),
    }
}

fn real_main() {
    ainxt_pager_minimal::install();
    #[cfg(all(feature = "jemalloc", unix))]
    ainxt_pager::memory_release::install_release_hook(purge_jemalloc_retained_pages);
    #[cfg(all(feature = "jemalloc", unix))]
    {
        ainxt_pager::memory_trace::install_allocator_stats_provider(jemalloc_allocator_stats);
        ainxt_pager::memory_trace::install_allocator_dump_provider(jemalloc_stats_dump);
    }
    #[cfg(all(feature = "jemalloc", unix))]
    install_heap_profile_hooks();
    if let Some(code) = ainxt_pager::app::mermaid_worker::maybe_run_render_subprocess() {
        std::process::exit(code);
    }
    ainxt_pager::memory_trace::start(
        ainxt_shell::util::ainxt_home::ainxt_home().join("memtrace"),
    );
    raise_fd_limit();
    if let Err(e) = ainxt_config::validate_requirements() {
        eprintln!("Couldn't start ainxt: {e}");
        eprintln!();
        eprintln!(
            "Update ainxt to a version the policy allows, or ask your administrator \
             to fix the managed requirements."
        );
        std::process::exit(2);
    }
    // Security policy bootstrap (INV-7: no silent degradation). Resolves the
    // compile-time build manifest + signed bundle and publishes the process
    // policy engine. A managed (`require_policy`) build with a missing, invalid,
    // or rolled-back bundle fails here and must refuse to start rather than run
    // unprotected. OSS builds resolve a permissive engine and proceed unchanged.
    match ainxt_policy::bootstrap::initialize(&ainxt_shell::util::ainxt_home::ainxt_home()) {
        Ok(outcome) => {
            // Attestation reports these on every outbound request, so they are
            // captured once here rather than re-read from disk per call.
            ainxt_pep::bootstrap::record_accepted_versions(
                outcome.bundle_version,
                outcome.overlay_version,
            );
            tracing::info!(
                enforcement = ?outcome.base_policy.enforcement,
                bundle_version = ?outcome.bundle_version,
                overlay_version = ?outcome.overlay_version,
                "security policy initialized"
            );
        }
        Err(e) => {
            eprintln!("Couldn't start ainxt: security policy failed to load: {e}");
            eprintln!();
            eprintln!(
                "This build requires a valid signed policy bundle. Ask your \
                 administrator to install or repair the managed policy."
            );
            std::process::exit(2);
        }
    }
    // Audit chain + enforcement point, installed immediately after the policy
    // engine and before anything can run a tool. Until the sink exists the
    // platform produces no evidence at all, so any decision taken before this
    // point is permanently uninvestigable.
    {
        use ainxt_pep::bootstrap::{AuditStart, install_audit, install_pep};
        let home = ainxt_shell::util::ainxt_home::ainxt_home();

        match install_audit(&home) {
            AuditStart::Fresh => tracing::info!("audit chain started"),
            AuditStart::Resumed { records } => tracing::info!(records, "audit chain resumed"),
            AuditStart::Broken { detail, records } => {
                // Deliberately not fatal. The log lives in a user-writable
                // directory, so refusing to start would hand any local user a
                // denial of service. Loud instead, and reported to the gateway
                // on the next attestation.
                tracing::error!(
                    records,
                    %detail,
                    "audit chain integrity check failed; starting a new chain"
                );
                eprintln!(
                    "warning: the local audit chain does not verify ({detail}); \
                     a new chain has been started and this has been reported."
                );
            }
        }

        if let Err(e) = install_pep(&home, Default::default()) {
            // Fail closed only when the policy is actually enforcing: without a
            // risk ledger there are no budgets, and an enforcing deployment
            // that silently loses them is worse than one that refuses to run.
            if ainxt_policy::global::active().is_enforcing() {
                eprintln!("Couldn't start ainxt: {e}");
                std::process::exit(2);
            }
            tracing::warn!(error = %e, "risk ledger unavailable; enforcement point not installed");
        }
    }

    let _sentry_guard = ainxt_telemetry::sentry::init(ainxt_telemetry::sentry::Config {
        client: "ainxt-pager",
        client_version: PAGER_CLIENT_VERSION,
        release: env!("VERSION_WITH_COMMIT"),
        disabled: ainxt_shell::agent::config::is_error_reporting_disabled_sync(),
    });
    ainxt_pager::docs::extract_user_guide_docs(&ainxt_shell::util::ainxt_home::ainxt_home());
    ainxt_crash_handler::install_terminal_restore_only();
    if ainxt_shell::util::config::load_crash_handler_enabled_sync() {
        let crash_dir = ainxt_shell::util::ainxt_home::ainxt_home().join("crash");
        if let Some(report) = ainxt_crash_handler::check_previous_crash(&crash_dir) {
            eprintln!("ainxt crashed during your last session.");
            eprintln!("  Signal:  {}", report.signal_name);
            eprintln!("  Version: {}", report.app_version);
            eprintln!("  Report:  {}", report.report_path.display());
            eprintln!();
        }
        if !ainxt_crash_handler::install(ainxt_crash_handler::CrashHandlerConfig {
    ...
    let crashed = ainxt_shell::active_sessions::collect_crashed().unwrap_or_default();
    if !crashed.is_empty() {
        tracing::info!(
            count = crashed.len(),
    ...
    ainxt_telemetry::debug_log::flush();
    if let Err(e) = result {
        ainxt_tty_utils::restore_native_stderr();
        eprintln!("Error: {e:#}");
        drop(_sentry_guard);
        std::process::exit(1);
    }
...
async fn async_main() -> Result<()> {
    ...
    if let Some(ref mode) = args.compaction_mode {
        unsafe { std::env::set_var("AINXT_COMPACTION_MODE", mode) };
    }
    if let Some(ref detail) = args.compaction_detail {
        unsafe { std::env::set_var("AINXT_COMPACTION_DETAIL", detail) };
    }
    if args.chat() {
        unsafe {
            std::env::set_var(ainxt_shell::agent::chat_modes::AINXT_CHAT_MODE_ENV, "1");
        }
    }
    if let Some(ref socket) = args.leader_socket {
        unsafe { std::env::set_var(ainxt_shell::leader::LEADER_SOCKET_ENV, socket) };
    }
    if let Some(ref path) = args.debug_file {
        unsafe {
            std::env::set_var("AINXT_DEBUG_LOG", path);
            std::env::remove_var("AINXT_LOG_FILE");
        }
    }
    if args.debug || args.debug_file.is_some() {
        ...
        set_if_unset("AINXT_DEBUG_LOG", "1");
        set_if_unset("AINXT_HOOKS_LOG", "1");
    }
    if let Some(Command::Completions { shell }) = &args.command {
        ainxt_pager::completions_cmd::run(*shell);
        return Ok(());
    }
    if let Some(Command::Wrap(ref wrap_args)) = args.command {
        return ainxt_pager::wrap_cmd::run(wrap_args);
    }
    ...
    let sandbox_profile_arg = match args.startup_sandbox_profile(saved_profile.as_deref()) {
        ainxt_pager::app::cli::SandboxStartup::Apply(profile) => profile,
        ainxt_pager::app::cli::SandboxStartup::Conflict { requested, saved } => {
            eprintln!(
    ...
    ainxt_shell::config::apply_sandbox(
        None,
    ...
    ainxt_shell::http::set_client_name(if is_interactive {
        ainxt_workspace::permission::ClientType::AinxtPager
    } else {
        ainxt_workspace::permission::ClientType::Generic
    });
    ...
    if let Some(command) = args.command.take() {
        match command {
            Command::Version { json } => {
                if json {
                    let payload = serde_json::json!(
                        ...
                        ainxt_update::channel_name().unwrap_or("unknown"), }
                    );
                    println!("{}", serde_json::to_string(&payload)?);
                } else {
                    println!(
                ...
                if args.leader || args.no_leader {
                    ...
                    anyhow::bail!(
                        "top-level {flag} applies to the pager TUI, not the agent subcommand. \
                         Use `ainxt-pager agent {flag}` instead."
                    );
                }
            ...
            Command::Inspect { json } => {
                ...
                ainxt_shell::inspect::inspect(&cwd, json).await?;
                return Ok(());
            }
            Command::Setup { json } => {
            ...
            Command::Mcp(mcp_args) => {
                init_tracing_simple("cli");
                return ainxt_pager::mcp_cmd::run(mcp_args).await;
            }
            Command::Policy(policy_args) => {
                // Read-only diagnostic; mirrors `Inspect` in not initialising
                // tracing, so the output stays clean enough to paste into a
                // ticket.
                return ainxt_pager::policy_cmd::run(policy_args).await;
            }
            Command::Plugin(plugin_args) => {
                init_tracing_simple("cli");
                let _otel_guard = ainxt_telemetry::otel_layer::otel_guard();
                return ainxt_pager::plugin_cmd::run(plugin_args).await;
            }
            Command::Models => {
                init_tracing_simple("cli");
                let _otel_guard = ainxt_telemetry::otel_layer::otel_guard();
                let config = ainxt_shell::config::load_effective_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return ainxt_pager::models::list_available_models(&agent_config).await;
            }
            ...
            Command::Worktree(worktree_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return ainxt_pager::worktree_cmd::run(worktree_args, &agent_config).await;
            }
            ...
            Command::Sessions(sessions_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return ainxt_pager::sessions_cmd::run(sessions_args, &agent_config).await;
            }
            Command::Share(ref share_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return ainxt_pager::share_cmd::run(share_args, &agent_config).await;
            }
            Command::Export(export_args) => {
                init_tracing_simple("cli");
                return ainxt_pager::export_cmd::run(export_args);
            }
            Command::Trace(trace_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return ainxt_pager::trace_cmd::run(trace_args, &agent_config).await;
            }
            Command::Memory(memory_args) => {
                return ainxt_pager::memory_cmd::run(memory_args);
            }
            Command::Update {
                check,
                json,
            ...
            Command::Login {
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                // Resolve the gateway URL from the configured proxy endpoint,
                // overridable via AINXT_GATEWAY_URL.
                let gateway = ainxt_shell::auth::gateway_auth::resolve_gateway_url(
                    &config.endpoints.proxy_url(),
                );
                // Default: token sign-in (paste an ainxt access token). Use
                // --password for the email/password sign-in flow.
                let result = if password {
                    ainxt_shell::auth::gateway_auth::interactive_password_login(
                        &gateway,
                        email.as_deref(),
                    )
                    .await
                } else {
                    ainxt_shell::auth::gateway_auth::interactive_token_login(&gateway).await
                };
                match result {
                    Ok(()) => ainxt_shell::instrumentation::finalize_and_exit(0),
                    Err(e) => {
                        eprintln!("Login failed: {e}");
                        ainxt_shell::instrumentation::finalize_and_exit(1);
                    }
                }
            }
            Command::Logout => {
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                // Clear the gateway credentials (credentials.json) and any legacy
                // OAuth auth.json state.
                ainxt_shell::auth::gateway_auth::logout();
                let _ = ainxt_shell::auth::run_cli_logout(&config);
                println!("Signed out.");
                ainxt_shell::instrumentation::finalize_and_exit(0);
            }
    ...
    let headless_prompt = ainxt_pager::headless::HeadlessPrompt::from_args(
    ...
    if let Some(prompt) = headless_prompt {
        ...
        let json_schema = args
            .json_schema
            .as_deref()
            .map(ainxt_pager::headless::parse_json_schema)
            .transpose()?;
        if json_schema.is_some() {
            if args.output_format == ainxt_pager::headless::OutputFormat::Plain {
                args.output_format = ainxt_pager::headless::OutputFormat::Json;
            }
            if args.self_verify {
                anyhow::bail!(
        ...
        return ainxt_pager::headless::run_single_turn(
            prompt,
            args.verbatim,
            ainxt_pager::headless::HeadlessOptions {
    ...
    let result = ainxt_pager::app::run(args, bg_update_rx).await;
    ainxt_sandbox::flush();
    match result {
        Ok(true) => {
            let adopted = bg_update_wait.lock().await.take();
            // finish_update_on_exit delegates to `auto_update::run_update`,
            // which prints its own accurate outcome ("Already up to date
            // (x.y.z)." or "✓ ainxt vX installed successfully!"). Only add a
            // relaunch hint on success and a retry hint on failure — do not
            // claim "Update installed" here, since that would contradict an
            // "Already up to date" line printed a moment earlier.
            if finish_update_on_exit(adopted, &update_config).await {
                eprintln!("Update installed. Run `ainxt` to start.");
            } else {
                eprintln!("Update did not complete. Run `ainxt update` to retry.");
            }
            Ok(())
        }
        Ok(false) => Ok(()),
...
/// Complete the update after a quit-for-update exit (Ctrl+U or `/update`).
/// Returns `true` when an update path completed without a reported failure.
///
/// Prefers awaiting the parked waiter for the background `ainxt update` child
/// spawned at startup — the download is usually already done or in flight.
/// When there is no waiter (spawn failed, the version cache was fresh so no
/// check ran, a debug build skipped the startup check, or the target was
/// already on disk) or the child failed, this falls back to a blocking
/// [`auto_update::run_update`].
///
/// IMPORTANT: the fallback deliberately uses `run_update` and NOT
/// `run_update_if_available`. The latter short-circuits with `Ok(false)` on
/// the version-cache TTL, an `auto_update = false` config, and a missing
/// installer — so an explicit user-initiated update would silently do nothing
/// while the caller's `.is_ok()` still reported success. `run_update` skips
/// those gates, always queries the gateway, and prints its own accurate status
/// ("Already up to date (x.y.z)." or "✓ ainxt vX installed successfully!").
async fn finish_update_on_exit(
    ...
    let run_blocking = |reason: Option<String>| {
        ...
        async move {
            ...
            match auto_update::run_update(false, None, None, &mut cfg).await {
                // run_update prints its own accurate status line; a returned
                // version means the target is installed (or already on disk).
                Ok(Some(_)) => true,
                // No installer detected — run_update already explained why.
                Ok(None) => false,
                Err(e) => {
...
fn build_update_config() -> UpdateConfig {
    let environment = ainxt_shell::env::AinxtBuildEnvironment::from_flags(false, false);
    let mut config = UpdateConfig::from_environment(&environment);
    cryptify::flow_stmt!({
        {
            config.deployment_key =
                ainxt_shell::agent::config::EndpointsConfig::default().deployment_key;
        }
    });
    config.npm_registry = std::env::var(obfstr::obfstr!("AINXT_NPM_REGISTRY"))
        .ok()
        .or_else(ainxt_shell::util::config::load_npm_registry_sync);
    if let Ok(root) = ainxt_shell::config::load_effective_config_disk_only()
        && let Some(ch) = ainxt_shell::util::config::channel_from_toml_opt(&root)
    {
        config.channel = ch;
    }
    config
...
fn should_check_for_updates(no_auto_update_flag: bool) -> bool {
    // Debug builds skip the update check by default to avoid noise during
    // development. Set AINXT_FORCE_UPDATE_CHECK=1 to override.
    if cfg!(debug_assertions)
        && !std::env::var_os("AINXT_FORCE_UPDATE_CHECK")
    ...
    !std::env::var_os("AINXT_DISABLE_AUTOUPDATER")
        .is_some_and(|v| env_flag_enabled(&v.to_string_lossy()))
}
...
/// True when `exe` is the binary `<ainxt_home>/bin/ainxt` resolves to, the
...
/// npm shim hardcodes `~/.ainxt`, so a custom `AINXT_HOME` skips here too.
fn is_managed_install(exe: Option<std::path::PathBuf>, ainxt_home: &std::path::Path) -> bool {
    if ainxt_home.as_os_str().is_empty() {
        return false;
    }
    let Some(exe) = exe else {
        return false;
    };
    let managed = ainxt_config::ainxt_application_in(ainxt_home);
...
/// Handle `ainxt-pager update [--check] [--json] [--force-reinstall] [--version X] [--alpha|--stable|--enterprise]`.
...
/// After a successful `ainxt update`, ask any running leader on this machine that
...
async fn signal_leaders_to_relaunch(installed_version: &str) {
    for d in ainxt_shell::leader::discover_leaders().await {
        if d.classification != ainxt_shell::leader::LeaderDiscoveryState::Reachable {
            continue;
        }
        ...
        let client = match ainxt_shell::leader::LeaderClient::connect(
            socket_path,
            "ainxt-pager-update",
            ClientMode::Stdio,
            ClientCapabilities::default(),
        ...
        match client
            ...
            .await
        {
            Ok(Ok(ainxt_shell::leader::ControlPayload::Relaunching {
                from_version,
                to_version,
                ..
            })) => {
            ...
            Ok(Ok(ainxt_shell::leader::ControlPayload::RelaunchDeclined { reason })) => {
...
mod tests {
    use super::*;
    // Holds the owning `TempDir` guard alongside the dump file path so the
    // private, auto-cleaned-up directory (rather than a fixed name directly
    // under the shared OS temp root) lives exactly as long as the struct.
    #[cfg(all(feature = "jemalloc", unix))]
    struct TempHeapDump {
    ...
    impl TempHeapDump {
        fn new(label: &str) -> Self {
            let dir = tempfile::Builder::new()
                .prefix("ainxt-jemalloc-")
                .tempdir()
                .expect("create temp dir for heap dump");
            let path = dir.path().join(format!("{label}.heap"));
            Self { _dir: dir, path }
        }
        ...
        fn assert_nonempty_dump(&self) {
            let meta = std::fs::metadata(&self.path).expect("dump file missing after prof.dump");
    ...
    fn require_opt_prof() -> bool {
        ...
        eprintln!(
            ...
            ainxt_shell::heap_profile::LG_PROF_SAMPLE
        );
        false
    }
    #[cfg(all(feature = "jemalloc", unix))]
    ...
    #[cfg(all(feature = "jemalloc", unix))]
    fn assert_stats_sane(stats: ainxt_shell::heap_profile::JemallocStats) {
    ...
    fn jemalloc_dump_rejects_interior_nul_path() {
        ...
        let path = Path::new(OsStr::from_bytes(b"/tmp/ainxt-jemalloc-\0.heap"));
    ...
    fn install_heap_profile_hooks_wires_shell_apis() {
        install_heap_profile_hooks();
        assert_stats_sane(
            ainxt_shell::heap_profile::stats().expect("shell stats after install"),
        );
        if !require_opt_prof() {
            assert!(!ainxt_shell::heap_profile::prof_available());
            return;
        }
        assert!(ainxt_shell::heap_profile::prof_available());
        assert_prof_active(false);
        {
            let _guard = ProfActiveGuard::set(true);
            assert_prof_active(true);
            assert!(ainxt_shell::heap_profile::set_prof_active(true));
            assert_prof_active(true);
        }
        assert_prof_active(false);
        assert!(ainxt_shell::heap_profile::set_prof_active(false));
        assert_prof_active(false);
        let dump = TempHeapDump::new("shell");
        ainxt_shell::heap_profile::dump_to_path(dump.path()).expect("shell dump");
        dump.assert_nonempty_dump();
    }
    #[cfg(unix)]
    #[test]
    fn is_managed_install_matches_only_the_bin_ainxt_target() {
        // `tempfile::tempdir()` (a private, auto-cleaned-up directory under
        // the OS temp root) instead of a hand-built, PID-suffixed path
        // directly under the shared temp directory.
        let tmp = tempfile::tempdir().unwrap();
        let home = tmp.path().to_path_buf();
        std::fs::create_dir_all(home.join("bin")).unwrap();
        std::fs::create_dir_all(home.join("downloads")).unwrap();
        assert!(!is_managed_install(
            Some(home.join("bin").join("ainxt")),
            &home
        ));
        ...
        let target = home.join("downloads").join("ainxt-1.2.3");
        std::fs::write(&target, b"binary").unwrap();
        std::os::unix::fs::symlink(&target, home.join("bin").join("ainxt")).unwrap();
        assert!(is_managed_install(
            Some(home.join("bin").join("ainxt")),
            &home
        ));
        assert!(is_managed_install(Some(target.clone()), &home));
        let pinned = home.join("bin").join("ainxt-9.9.9");
    ...
    fn dashboard_subcommand_flags_startup_without_forcing_leader() {
        ...
        assert_eq!(
            std::env::var("AINXT_OPEN_DASHBOARD_AT_STARTUP").as_deref(),
            Ok("1"),
            "startup hook flag must be set",
        );
        unsafe { std::env::remove_var("AINXT_OPEN_DASHBOARD_AT_STARTUP") };
    }
    /// `ainxt dashboard --no-leader` is allowed — the dashboard does not
    /// require a leader, so the combination launches into the dashboard in
    /// non-leader mode.
    #[serial_test::serial(AINXT_AGENT_DASHBOARD)]
    #[test]
    fn dashboard_subcommand_allows_no_leader() {
        let mut args = PagerArgs::try_parse_from(["ainxt", "--no-leader", "dashboard"]).unwrap();
        flag_dashboard_at_startup_if_requested(&mut args)
    ...
    #[serial_test::serial(AINXT_WORKSPACE_COMMAND)]
    #[test]
    fn workspace_command_env_override_parsing() {
        unsafe { std::env::remove_var("AINXT_WORKSPACE_COMMAND") };
        assert_eq!(workspace_command_env_override(), None);
        unsafe { std::env::set_var("AINXT_WORKSPACE_COMMAND", "1") };
        assert_eq!(workspace_command_env_override(), Some(true));
        unsafe { std::env::set_var("AINXT_WORKSPACE_COMMAND", "off") };
    ...
    fn cache_session_close_stops_replaying_it() {
        ...
        cache_outgoing_acp_state(
            r#"{"jsonrpc":"2.0","id":3,"method":"_ainxt.dev/session/close","params":{"sessionId":"s1"}}"#,
            &state,
        );
        let s = state.lock().unwrap();
    ...
    ///   2. forward interleaved notifications to the client unchanged,
    ...
    async fn replay_waits_for_load_response_through_notifications() {
        ...
        let responder = tokio::spawn(async move {
            ...
            response_tx
                .send(
                    r#"{"jsonrpc":"2.0","method":"ainxt.dev/leader/version_mismatch","params":{}}"#
                        .to_string(),
                )
                .unwrap();
            response_tx
    ...
    /// `ainxt.dev/leader_reconnected` with empty params and the external client
