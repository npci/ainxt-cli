//! Memory-system configuration value types, extracted from ainxt-shell
...
//! `ainxt-shell` — `resolve()` depends on `toml` and on shell-internal
...
///
/// When enabled, watches `~/.ainxt/memory/` for `.md` file changes (create,
...
///
/// On session init, directories under `~/.ainxt/memory/` are scanned:
