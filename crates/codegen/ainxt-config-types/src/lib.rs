...
use ainxt_announcements::RemoteAnnouncement;
...
pub struct DoomLoopRecoverySettings {
    /// Send the `x-ainxt-doom-loop-check` header and parse the reported
...
pub struct RemoteSettings {
    ...
    /// `app::subscription` module). Forwarded from the `ainxt_build_settings`
    ...
    /// OAuth2 provider issuer URL (e.g., "https://auth.example.test"). When present
    ...
    /// default (on). Sits below env `AINXT_FOLDER_TRUST`, user
    ...
    /// remote settings `ainxt_build_settings.max_mcp_output_bytes` — global default
    ...
    /// `ainxt_build_settings` remote settings key) to enable it. See
    ...
    /// `ainxt_telemetry::external::apply_remote_policy`.
    #[serde(default)]
    ...
    /// `AINXT_TWO_PASS_COMPACTION` / default (off).
    #[serde(default)]
    ...
    /// Controlled via remote settings (`non_git_warning` in `ainxt_build_settings`).
    ...
    /// remote settings gate for first-run auto-registration of the official ainxt
    ...
    /// ghost text), from the `ainxt_build_settings` remote settings flag. Sits below
    /// env (`AINXT_PROMPT_SUGGESTIONS_MODEL`) and `[models] prompt_suggestion`
    /// in config.toml, above the client hint and the built-in
    /// `ainxt-build-0.1` default. The effective model is catalog-guarded: when
    ...
    /// remote settings flag: optional ainxt image model override for `image_gen`.
    /// When present and non-empty, `image_gen` uses this model slug
    /// (e.g. `ainxt-image`) instead of the default quality model
    /// (`ainxt-image-quality`). Absent/empty → default model.
    #[serde(default)]
    ...
    /// Overridable locally via `AINXT_VOICE_MODE`. Free-tier SuperAinxt upsell
    ...
    /// User's subscription tier from remote settings `ainxt_build_access_gate`.
    /// E.g. "free", "premium", "superainxt", "superainxt_heavy".
    ...
    /// Whether the user is allowed to use ainxt. Set by remote settings
    /// `ainxt_build_access_gate` targeting rules. `None` = no server response
    ...
    /// (e.g. "SuperAinxt", "X Premium+", "Free", "API Key"). Set by CCP
    ...
    /// Server-controlled via the remote settings `ainxt_build_usage_redirect_url`
    ...
    /// `ainxt_build_settings`. Per-model override on `ModelInfo`
    /// (`ainxt_build_models`) takes precedence; user config and env var
    ...
    /// `0` disables. Env (`AINXT_COMPACTION_WALL_CLOCK_SECS`) overrides it.
    ...
    /// Env (`AINXT_COMPACTION_MODE`) and user config override it.
    #[serde(default)]
    pub compaction_mode: Option<String>,
    /// Segments unchanged detail (`none` | `minimal` | `balanced` | `verbose`)
    /// from remote settings. Env (`AINXT_COMPACTION_DETAIL`) and config override it.
    #[serde(default)]
    pub compaction_detail: Option<String>,
    /// remote settings unchanged-input flag; env (`AINXT_COMPACTION_VERBATIM_INPUT`) and config override it. `None` = default (true).
    #[serde(default)]
    ...
    /// remote settings gate for the `ainxt workspace` CLI command (Computer Hub
    /// workspace exposure), from `ainxt_build_settings.workspace_command_enabled`.
...
/// Remote enable tier for the per-tip contextual hints (matches the client's
...
/// only guaranteed to work with a compatible harness (cursor vs ainxt-build).
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GoalRoleModel {
    /// Model id, e.g. "ainxt-4". Resolved against available models at
    /// spawn time; unknown/unauthorized ⇒ fail-open to current model.
    pub model: String,
    /// Harness `agent_type` (e.g. "cursor", "ainxt-build-plan") whose
    /// `AgentDefinition` decides the role subagent's harness flavor (system
    /// prompt + cursor-vs-ainxt-build toolset), applied REGARDLESS of the
...
mod tests {
    ...
    fn remote_settings_image_description_model_round_trip() {
        let json = r#"{"image_description_model": "ainxt-build"}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(s.image_description_model.as_deref(), Some("ainxt-build"));
    ...
    fn remote_settings_prompt_suggestion_model_round_trip() {
        let json = r#"{"prompt_suggestion_model": "ainxt-build-0.1"}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(s.prompt_suggestion_model.as_deref(), Some("ainxt-build-0.1"));
        let out = serde_json::to_string(&s).unwrap();
        let s2: RemoteSettings = serde_json::from_str(&out).unwrap();
    ...
    fn remote_settings_goal_planner_model_round_trip() {
        let json =
            r#"{"goal_planner_model": {"model": "ainxt-4", "agent_type": "general-purpose"}}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
    ...
    fn remote_settings_goal_strategist_model_round_trip() {
        let json = r#"{"goal_strategist_model": {"model": "ainxt-4.5", "agent_type": "cursor"}}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
    ...
    fn remote_settings_goal_skeptic_models_fully_valid_pool_round_trips() {
        let json = r#"{"goal_skeptic_models": [
            {"model": "ainxt-4", "agent_type": "general-purpose"},
            {"model": "ainxt-3", "agent_type": "cursor"}
        ]}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
    ...
    fn remote_settings_goal_skeptic_models_extra_unknown_fields_kept() {
        let json = r#"{"goal_skeptic_models": [
            {"model": "ainxt-4", "agent_type": "general-purpose", "reasoning_effort": "high"}
        ]}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
            s.goal_skeptic_models,
    ...
    fn remote_settings_goal_role_models_malformed_pair_does_not_drop_other_fields() {
        let json = r#"{
            ...
            "goal_strategist_model": {"model": "ainxt-4.5", "agent_type": "cursor"},
            "default_model": "ainxt-4"
        }"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(s.goal_planner_model, None);
        assert_eq!(
        ...
        assert_eq!(s.default_model.as_deref(), Some("ainxt-4"));
    }
    #[test]
    fn remote_settings_goal_role_model_extra_unknown_fields_kept_single_pair() {
        let json = r#"{"goal_planner_model": {"model": "ainxt-4", "agent_type": "general-purpose", "future": true}}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
            s.goal_planner_model,
            Some(GoalRoleModel {
