//! Build script for bundling ripgrep for the ainxt-tools crate.
//!
//! - If `AINXT_TOOLS_BUNDLE_RG_PATH` is set, always bundle it
...
/// `AINXT_TOOLS_BUNDLE_<NAME>_PATH` points at one (supplied by the release
/// pipeline). Emits
...
/// bundled (the runtime resolver falls back to `~/.ainxt/vendor` / `$PATH`);
...
fn bundle_search_tool(
    ...
    let override_env = format!("AINXT_TOOLS_BUNDLE_{name_uc}_PATH");
    ...
    // Windows target is dead weight — skip (matches the ripgrep Windows skip).
    ...
    println!("cargo:rustc-env=AINXT_TOOLS_{name_uc}_VER={ver}");
    println!("cargo:rustc-env=AINXT_TOOLS_{name_uc}_TARGET=override");
    Ok(())
}
...
fn bundle_rg() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("cargo:rerun-if-env-changed=AINXT_TOOLS_BUNDLE_RG_PATH");
    ...
    let path_override = env::var("AINXT_TOOLS_BUNDLE_RG_PATH").ok();
    ...
    // AINXT_TOOLS_BUNDLE_RG_PATH still bundles regardless of target.
    ...
    println!("cargo:rustc-env=AINXT_TOOLS_RG_VER={}", RG_VER);
    ...
    if let Some(path) = path_override {
        ...
        println!("cargo:rustc-env=AINXT_TOOLS_RG_TARGET=override");
        let _ = fs::remove_file(&dest);
        fs::copy(PathBuf::from(path.clone()), &dest).map_err(|e| {
            format!(
                "Failed copying AINXT_TOOLS_BUNDLE_RG_PATH: {e} from path {path} to dest {}",
                dest.display()
            )
        })?;
        return Ok(());
    ...
    let asset_triple = match (target_os.as_str(), target_arch.as_str()) {
        ...
        _ => {
            return Err(format!(
                "Unsupported target for ripgrep bundling: {os}-{arch}. Set AINXT_TOOLS_BUNDLE_RG_PATH to a local rg binary for offline or unsupported builds.",
                os = target_os,
                arch = target_arch
            ).into());
        }
    ...
    println!("cargo:rustc-env=AINXT_TOOLS_RG_TARGET={}", asset_triple);
    ...
    let bytes: Vec<u8> = {
        let resp = reqwest::blocking::get(&url).map_err(|e| {
            format!(
                "Failed to download ripgrep: {}\nSet AINXT_TOOLS_BUNDLE_RG_PATH to a local rg for offline builds.",
                e
            )
        })?;
        if !resp.status().is_success() {
            return Err(format!(
                "HTTP {} downloading ripgrep. Set AINXT_TOOLS_BUNDLE_RG_PATH for offline builds.",
                resp.status()
            )
            .into());
        }
    ...
    if !found {
        return Err(format!(
            "Could not find 'rg' in ripgrep archive {}. Set AINXT_TOOLS_BUNDLE_RG_PATH for offline builds.",
            url
        )
        .into());
    }
