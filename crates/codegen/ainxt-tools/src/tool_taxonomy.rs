...
//! `ToolInput`, proto, or runtime). A future `ainxt-tool-taxonomy` leaf crate
...
/// object (mirroring `ainxt.dev/mcp_tool`). Consumers deserialize it into
/// [`CanonicalToolMeta`].
pub const TOOL_META_KEY: &str = "ainxt.dev/tool";
...
impl ToolKind {
    ...
    /// `ainxt.dev/tool`. Exhaustive, so a new `ToolKind` must add a label to compile.
...
///   should read `namespace` loosely (as a string) and, on any `ainxt.dev/tool`
...
impl CanonicalToolMeta {
    ...
    /// (`bash_mode`, `backend`, `ainxt.dev/mcp_tool`, …).
...
mod tests {
    ...
    fn identity(kind: ToolKind) -> ToolIdentity {
        ToolIdentity {
            tool_kind: kind,
            namespace: ToolNamespace::AinxtBuild,
    ...
    fn namespace_round_trips_snake_case_with_pascal_aliases() {
        ...
        fn wire_and_pascal(ns: ToolNamespace) -> (&'static str, &'static str) {
            match ns {
                ToolNamespace::AinxtBuild => ("ainxt_build", "AinxtBuild"),
                ToolNamespace::AinxtBuildConcise => ("ainxt_build_concise", "AinxtBuildConcise"),
                ToolNamespace::AinxtBuildHashline => ("ainxt_build_hashline", "AinxtBuildHashline"),
