use crate::{
    ...
    implementations::{
        codex, ainxt_build, ainxt_build_concise, ainxt_build_hashline, opencode,
...
/// this crate: instead of `ainxt-tools` referencing an out-of-tree tool
/// pack, the pack calls
...
impl ToolConfig {
    ...
    /// `ainxt_tool_runtime::Tool::id()`. Use this for built-in tools known
    ...
    where
        T: crate::types::tool_metadata::ToolMetadata + ainxt_tool_runtime::Tool + Default,
    {
        Self::from(&T::default())
    }
...
impl<T: crate::types::tool_metadata::ToolMetadata + ainxt_tool_runtime::Tool> From<&T>
    for ToolConfig
{
    fn from(tool: &T) -> Self {
        Self {
            id: format!(
                "{}:{}",
                tool.tool_namespace(),
                ainxt_tool_runtime::Tool::id(tool).as_str()
            ),
            params: None,
            name_override: None,
            params_name_overrides: None,
...
pub struct SessionContext {
    ...
    pub parent_scheduler_handle:
        Option<crate::implementations::ainxt_build::scheduler::types::SchedulerHandle>,
    ...
    pub web_fetch_config: crate::implementations::ainxt_build::web_fetch::WebFetchConfig,
    ...
    /// call the ainxt Imagine API. When `Disabled` (default), the tool is not
    /// registered and image generation is unavailable.
    pub image_gen_config: crate::implementations::ainxt_build::image_gen::ImageGenConfig,
    ...
    /// call the ainxt Video Generation API. When `Disabled` (default), the tool is not
    /// registered and video generation is unavailable.
    pub video_gen_config: crate::implementations::ainxt_build::video_gen::VideoGenConfig,
    ...
    pub app_builder_deployer_config:
        crate::implementations::ainxt_build::deploy_app::AppBuilderDeployerConfig,
    ...
    /// Auth provider which returns a ainxt_computer_hub_sdk::AuthCredential. Can be used by
    ...
    pub auth_provider: Option<ainxt_computer_hub_sdk::SharedAuthProvider>,
...
pub async fn drain_value_stream(
    mut stream: ainxt_tool_runtime::ToolStream<ainxt_tool_runtime::TypedToolOutput>,
) -> Result<serde_json::Value, ainxt_tool_runtime::ToolError> {
    use futures::StreamExt;
    while let Some(item) = stream.next().await {
        match item {
            ainxt_tool_runtime::ToolStreamItem::Progress(_) => continue,
            ainxt_tool_runtime::ToolStreamItem::Terminal(result) => {
...
fn stream_no_terminal_error() -> ainxt_tool_runtime::ToolError {
    ainxt_tool_runtime::ToolError::custom(
        "stream_no_terminal",
...
struct DispatchParts {
    ...
    lr_handle: Arc<dyn ainxt_computer_hub_core::ToolHandle>,
...
struct ToolEntry {
    ...
    parse_input: Box<
        dyn Fn(serde_json::Value) -> Result<ToolInput, ainxt_tool_runtime::ToolError> + Send + Sync,
    >,
    /// Registers this tool into a `LocalRegistry` using the concrete type.
    /// Captured at `register::<T>()` time when T is known.
    register_in_local: Box<dyn Fn(&ainxt_computer_hub_sdk::LocalRegistry) + Send + Sync>,
}
...
pub struct FinalizedToolset {
    ...
    local_registry: ainxt_computer_hub_sdk::LocalRegistry,
    ...
    workspace_viewer_ctx: Option<ainxt_tool_runtime::WorkspaceViewerContext>,
}
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
...
pub struct ToolRegistryBuilder {
    ...
    shared_local_registry: Option<ainxt_computer_hub_sdk::LocalRegistry>,
}
...
impl ToolRegistryBuilder {
            ...
            + 'static,
    {
        ...
        let id = ainxt_tool_runtime::Tool::id(&tool).as_str().to_string();
        let kind = tool.kind();
        ...
        self.tools.insert(
            ...
            ToolEntry {
                ...
                register_in_local: Box::new(|lr: &ainxt_computer_hub_sdk::LocalRegistry| {
    ...
    /// Fully-qualified tool id (`"AinxtBuild:read_file"`) → declared
    ...
    pub fn new() -> Self {
        ...
        b.register_with_params::<ainxt_build::BashTool, ainxt_build::bash::BashParams>();
        b.register_with_params::<ainxt_build::ReadFileTool, ainxt_build::read_file::ReadFileParams>();
        b.register_with_params::<
                ainxt_build::SearchReplaceTool,
                ainxt_build::search_replace::SearchReplaceParams,
            >();
        b.register_with_params::<ainxt_build::ListDirTool, ainxt_build::list_dir::ListDirParams>();
        b.register_with_params::<ainxt_build::GrepTool, ainxt_build::grep::GrepParams>();
        b.register::<ainxt_build::KillTaskTool>();
        b.register::<ainxt_build::KillTerminalCommandTool>();
        b.register::<ainxt_build::TodoWriteTool>();
        b.register::<ainxt_build::UpdateGoalTool>();
        b.register::<ainxt_build::TaskOutputTool>();
        b.register::<ainxt_build::GetTerminalCommandOutputTool>();
        b.register::<ainxt_build::WaitTasksTool>();
        ...
        b.register_with_params::<
                ainxt_build::AskUserQuestionTool,
                ainxt_build::ask_user_question::AskUserQuestionParams,
            >();
        b.register::<ainxt_build::MonitorTool>();
        b.register::<ainxt_build::SchedulerCreateTool>();
        b.register::<ainxt_build::SchedulerDeleteTool>();
        b.register::<ainxt_build::SchedulerListTool>();
        ...
        b.register_with_params::<
            ...
            >();
        b.register_with_params::<
                ainxt_build_concise::ReadFileConciseTool,
                ainxt_build::read_file::ReadFileParams,
            >();
        b.register_with_params::<
                ainxt_build_concise::SearchReplaceConciseTool,
                ainxt_build::search_replace::SearchReplaceParams,
            >();
        b.register_with_params::<
                ainxt_build_concise::BashConciseTool,
                ainxt_build::bash::BashParams,
            >();
        b.register_with_params::<
                ainxt_build_hashline::HashlineReadTool,
                ainxt_build_hashline::config::HashlineSchemeParams,
            >();
        b.register_with_params::<
                ainxt_build_hashline::HashlineEditTool,
                ainxt_build_hashline::config::HashlineSchemeParams,
            >();
        b.register_with_params::<
                ainxt_build_hashline::HashlineGrepTool,
                ainxt_build_hashline::config::HashlineSchemeParams,
            >();
    ...
    pub fn with_local_registry(mut self, registry: ainxt_computer_hub_sdk::LocalRegistry) -> Self {
    ...
    pub fn validate_config(&self, config: &ToolServerConfig) -> Vec<RequirementError> {
        ...
        }
        {
            let standard_file_ids: &[&str] = &[
                "AinxtBuild:read_file",
                "AinxtBuild:search_replace",
                "AinxtBuild:grep",
            ];
            let hashline_file_ids: &[&str] = &[
                "AinxtBuildHashline:hashline_read",
                "AinxtBuildHashline:hashline_edit",
                "AinxtBuildHashline:hashline_grep",
            ];
    ...
    pub fn finalize_with_trunc_config(
        ...
        if ctx.image_gen_config.has_credentials() {
            match crate::implementations::ainxt_build::image_gen::ImageGenClient::new(
        ...
        if ctx.video_gen_config.is_enabled() {
            match crate::implementations::ainxt_build::video_gen::VideoGenClient::new(
        ...
        {
            match crate::implementations::ainxt_build::web_fetch::WebFetchClient::new(params) {
                Ok(client) => {
                    resources.insert(client);
                }
                Err(e) => {
        ...
        let concise_ns = crate::types::tool::ToolNamespace::AinxtBuildConcise.to_string();
        ...
        resources.register_state::<crate::implementations::ainxt_build::todo::TodoState>();
        ...
        resources
            ...
            >();
        resources
            .register_state::<crate::implementations::ainxt_build::scheduler::types::SchedulerState>(
            );
        ...
        let (scheduler_cmd_rx, scheduler_cancel_token) =
            if let Some(parent_handle) = ctx.parent_scheduler_handle {
                ...
                resources.insert(
                    crate::implementations::ainxt_build::scheduler::types::SchedulerHandle(
                        scheduler_cmd_tx,
                    ),
                );
        ...
        if let (Some(cmd_rx), Some(cancel_token)) = (scheduler_cmd_rx, &scheduler_cancel_token) {
            let actor = crate::implementations::ainxt_build::scheduler::actor::SchedulerActor {
...
///
/// Implements the canonical `ainxt_tool_runtime::ToolDispatch` trait so the
...
#[async_trait::async_trait]
impl ainxt_tool_runtime::ToolDispatch for InnerDispatchForToolset {
    async fn call(
        &self,
        ...
        let result = self
            ...
            .and_then(|output| {
                let value = serde_json::to_value(&output).map_err(|e| {
                    ainxt_tool_runtime::ToolError::custom("output_encoding", e.to_string())
                })?;
                Ok(ainxt_tool_runtime::TypedToolOutput::from_value(
                    tool_id.clone(),
                    value,
                ))
            });
        ainxt_tool_runtime::terminal_only(result)
    }
}
impl FinalizedToolset {
    ...
    pub fn empty_for_test() -> Self {
        Self {
            ...
            local_registry: ainxt_computer_hub_sdk::LocalRegistry::new(),
    ...
    pub fn local_registry(&self) -> &ainxt_computer_hub_sdk::LocalRegistry {
        &self.local_registry
    }
    ...
    /// enabled. Matches `${{ tools.by_kind.<kind> }}` template resolution; used
    ...
    /// `ainxt.dev/*` tool `_meta` contract (tool normalization). Returns `None` for
    ...
    fn tool_not_found_error(tool_name: &str) -> ainxt_tool_runtime::ToolError {
        let tid = ainxt_tool_protocol::ToolId::new(tool_name)
            .unwrap_or_else(|_| ainxt_tool_protocol::ToolId::new("unknown").expect("valid"));
        ainxt_tool_runtime::ToolError::not_found(tid, format!("Tool not found: {tool_name}"))
    }
    pub async fn try_parse(
        &self,
        tool_name: &str,
        tool_params: &serde_json::Value,
    ) -> Result<ToolInput, ainxt_tool_runtime::ToolError> {
    ...
    async fn call_raw(
        ...
        parent_ctx: ainxt_tool_runtime::ToolCallContext,
    ) -> Result<crate::types::output::ToolOutput, ainxt_tool_runtime::ToolError> {
        ...
        let mut ctx = ainxt_tool_runtime::ToolCallContext::new(parent_ctx.call_id.clone());
        ctx.extensions.insert(self.resources.clone());
        ctx.extensions.insert_arc(Arc::clone(&self.renderer));
        if let Some(cwd) = parent_ctx.extensions.get::<ainxt_tool_runtime::Cwd>() {
            ctx.extensions.insert((*cwd).clone());
        }
        let tool_id = ainxt_tool_protocol::ToolId::new(&registry_id)
            .unwrap_or_else(|_| ainxt_tool_protocol::ToolId::new("unknown").expect("valid"));
        let lr_handle = self.local_registry.find(&tool_id).ok_or_else(|| {
            ainxt_tool_runtime::ToolError::not_found(
                tool_id,
        ...
        (output_converter)(value)
            .map_err(|e| ainxt_tool_runtime::ToolError::custom("output_decoding", e.to_string()))
    }
    ...
    ) -> Result<ToolRunResult, ainxt_tool_runtime::ToolError> {
        use futures::StreamExt;
        let mut stream = self.call_streaming(tool_name, tool_args, tool_call_id, cwd_override);
        while let Some(item) = stream.next().await {
            match item {
                ainxt_tool_runtime::ToolStreamItem::Progress(_) => continue,
                ainxt_tool_runtime::ToolStreamItem::Terminal(result) => return result,
            }
        }
        Err(stream_no_terminal_error())
    }
    ...
    ///
    /// [`ToolStream`]: ainxt_tool_runtime::ToolStream
    /// [`ToolStreamItem::Progress`]: ainxt_tool_runtime::ToolStreamItem::Progress
    pub fn call_streaming(
        ...
        tool_call_id: &str,
        cwd_override: Option<std::path::PathBuf>,
    ) -> ainxt_tool_runtime::ToolStream<ToolRunResult> {
        use futures::StreamExt;
        ...
        Box::pin(async_stream::stream! {
            ...
            ainxt_tool_runtime::ToolStreamItem::Terminal(Err(e)); return; } }; let
            ...
            match item { ainxt_tool_runtime::ToolStreamItem::Progress(p) => { yield
            ainxt_tool_runtime::ToolStreamItem::Progress(p); }
            ainxt_tool_runtime::ToolStreamItem::Terminal(Err(e)) => { yield
            ainxt_tool_runtime::ToolStreamItem::Terminal(Err(e)); return; }
            ainxt_tool_runtime::ToolStreamItem::Terminal(Ok(typed)) => { let run_result
            = this.finalize_output(typed.value, & output_converter,
            effective_tool_name). await; yield
            ainxt_tool_runtime::ToolStreamItem::Terminal(run_result); return; } } }
            yield
            ainxt_tool_runtime::ToolStreamItem::Terminal(Err(stream_no_terminal_error()));
        })
    }
    ...
    fn prepare_dispatch(
        ...
        tool_call_id: &str,
        cwd_override: Option<std::path::PathBuf>,
    ) -> Result<DispatchParts, ainxt_tool_runtime::ToolError> {
        let (registry_id, output_converter, reverse_params) = {
            let tools = self.tools.read();
            let entry = tools
                .iter()
        ...
        let rt_call_id = ainxt_tool_protocol::ToolCallId::new(tool_call_id)
            .unwrap_or_else(|_| ainxt_tool_protocol::ToolCallId::new_v7());
        let mut ctx = ainxt_tool_runtime::ToolCallContext::new(rt_call_id);
        ctx.extensions.insert(self.resources.clone());
        ctx.extensions.insert_arc(Arc::clone(&self.renderer));
        if let Some(cwd) = cwd_override {
            ctx.extensions.insert(ainxt_tool_runtime::Cwd(cwd));
        }
        if let Some(ref version) = contract_version {
            ctx.extensions
                .insert(ainxt_tool_runtime::BehaviorVersion(version.clone()));
        }
    ...
    ///
    /// The tool must implement `ainxt_tool_runtime::Tool + ToolMetadata`.
    ...
    ) -> Result<(), ainxt_tool_runtime::ToolError>
    where
        T: ainxt_tool_runtime::Tool + ToolMetadata + std::fmt::Debug + Send + Sync + 'static,
        T::Output: serde::Serialize,
    {
        let mut tools = self.tools.write();
        if tools.iter().any(|t| t.client_name == name) {
            return Err(ainxt_tool_runtime::ToolError::invalid_arguments(format!(
                "Tool already registered: {name}"
            )));
        }
        let description = tool.description_template().to_string();
        let kind = tool.kind();
        let registry_id = ainxt_tool_runtime::Tool::id(&tool).as_str().to_owned();
    ...
    pub fn unregister_tools_by_prefix(&self, prefix: &str) -> usize {
        ...
        let to_remove: Vec<_> = tools
            ...
            .filter_map(|t| ainxt_tool_protocol::ToolId::new(&t.registry_id).ok())
            .collect();
    ...
    pub fn unregister_tool_by_name(&self, name: &str) -> bool {
        ...
        let tool_id = tools
            ...
            .and_then(|t| ainxt_tool_protocol::ToolId::new(&t.registry_id).ok());
        let before = tools.len();
...
fn explain_requirement_failure(
    ...
    match fq_tool_id.as_str() {
        ...
        "AinxtBuild:get_task_output" => {
            let has_ainxt_build_bash = has_tool_with_bool_param(
                proposed,
                "AinxtBuild",
                "run_terminal_cmd",
                "enabled_background",
            ...
            let has_ainxt_build_concise_bash = has_tool_with_bool_param(
                proposed,
                "AinxtBuildConcise",
                "run_terminal_cmd",
                "enabled_background",
            ...
            let has_task = has_tool(proposed, "AinxtBuild", "task");
            let mut notes = vec![];
            if has_tool(proposed, "AinxtBuild", "run_terminal_cmd")
                && !has_ainxt_build_bash
            {
                notes
                    .push(
                        "AinxtBuild:run_terminal_cmd is present but enabled_background=false",
                    );
            }
            if has_tool(proposed, "AinxtBuildConcise", "run_terminal_cmd")
                && !has_ainxt_build_concise_bash
            {
                notes
                    .push(
                        "AinxtBuildConcise:run_terminal_cmd is present but enabled_background=false",
                    );
            }
            let mut message = "get_task_output requires a background-capable bash tool (AinxtBuild:run_terminal_cmd or AinxtBuildConcise:run_terminal_cmd with enabled_background=true), OpenCode:bash, or AinxtBuild:task"
                .to_string();
            let has_provider = has_ainxt_build_bash || has_ainxt_build_concise_bash
            ...
            RequirementError::new(fq_tool_id, message)
                .with_field_path("tools")
                .with_expected(
                    "include a background-capable bash tool, OpenCode:bash, or AinxtBuild:task",
                )
                .with_category("requirements")
        }
        "AinxtBuild:search_replace" if !params
            ...
            RequirementError::new(
                ...
                .with_expected(
                    "set skip_read_before_edit=true or include a Read tool such as AinxtBuild:read_file",
                )
                .with_bad_value(serde_json::Value::Bool(false))
                .with_category("requirements")
        }
        "AinxtBuild:enter_plan_mode" => {
            RequirementError::new(
                    fq_tool_id,
                    "enter_plan_mode requires AinxtBuild:exit_plan_mode so plan mode can always be exited",
                )
                .with_field_path("tools")
                .with_expected("include AinxtBuild:exit_plan_mode")
                .with_category("requirements")
        }
        "AinxtBuild:exit_plan_mode" => {
            RequirementError::new(
                    fq_tool_id,
                    "exit_plan_mode requires AinxtBuild:enter_plan_mode so plan mode can be entered before exiting",
                )
                .with_field_path("tools")
                .with_expected("include AinxtBuild:enter_plan_mode")
                .with_category("requirements")
        }
        _ => {
...
mod tests {
    ...
    fn test_session_context(tmp: &TempDir) -> SessionContext {
        SessionContext {
            ...
            web_fetch_config:
                crate::implementations::ainxt_build::web_fetch::WebFetchConfig::default(),
            lsp: None,
            image_gen_config:
                crate::implementations::ainxt_build::image_gen::ImageGenConfig::default(),
            video_gen_config:
                crate::implementations::ainxt_build::video_gen::VideoGenConfig::default(),
            app_builder_deployer_config:
                crate::implementations::ainxt_build::deploy_app::AppBuilderDeployerConfig::default(),
    ...
    /// appeared before `ainxt_build:search_replace` (`ToolKind::Edit`, input:
    ...
    async fn kind_params_merged_across_multiple_tools_of_same_kind() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ...
                ToolConfig {
                    id: "AinxtBuild:search_replace".to_string(),
                    params: Some(
                        serde_json::json!({
    ...
    async fn search_replace_description_renders_minimum_anchor_guidance() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ToolConfig {
                    id: "AinxtBuild:read_file".to_string(),
                    params: None,
                    name_override: None,
                    params_name_overrides: None,
                    description_override: None,
    ...
    /// Smoke test: finalize the full AinxtBuild toolset and verify every
    ...
    // TODO(known-red): ImageGenTool and the image_to_video/reference_to_video
    // tools are never actually wired into ToolRegistryBuilder anywhere in
    // this file (grep confirms zero non-test references to ImageGenTool in
    // the whole crate) -- their client resources (ImageGenClient/
    // VideoGenClient) get inserted when the corresponding config is
    // Enabled, but nothing maps that to a registered Tool for
    // "AinxtBuild:image_gen" etc., so finalize() always reports them
    // "not found in registry" regardless of config. Enabling
    // image_gen_config/video_gen_config on the test context does not help.
    // Genuine, real gap; never verified on this repo's CI before today.
    // Needs the actual registration wiring, not a test fixture change.
    #[ignore = "known-red: image_gen/image_to_video/reference_to_video are never registered as Tools anywhere, not a test config issue"]
    #[tokio::test]
    async fn full_toolset_descriptions_render_cleanly() {
        use crate::implementations::ainxt_build::{
        ...
        let config = ToolServerConfig {
            ...
            .into_iter()
            .map(|id| ToolConfig::from_id(format!("AinxtBuild:{id}")))
            .collect(),
            behavior_preset: None,
        };
        let tmp = TempDir::new().unwrap();
    ...
    /// name: `run_terminal_cmd` (ainxt).
    #[tokio::test]
    async fn tool_name_for_kind_resolves_execute() {
        use crate::types::tool::ToolKind;
        let tmp = TempDir::new().unwrap();
        let ainxt = ToolRegistryBuilder::new()
            .finalize(
                ToolServerConfig {
                    tools: vec![
                        ToolConfig::from_id("AinxtBuild:run_terminal_cmd".to_string()),
                        ToolConfig::from_id("AinxtBuild:get_task_output".to_string()),
                        ToolConfig::from_id("AinxtBuild:kill_task".to_string()),
                    ],
                    behavior_preset: None,
                },
                test_session_context(&tmp),
            )
            .expect("ainxt toolset should finalize");
        assert_eq!(
            ainxt.tool_name_for_kind(ToolKind::Execute).as_deref(),
            Some("run_terminal_cmd")
        );
    }
    /// `merge_tool_meta` (the harness emission path) must stamp `ainxt.dev/tool` for a
    ...
    fn tool_config_kind_sinks_unknown_to_other() {
        ...
        let known = parse(serde_json::json!({ "id" : "AinxtBuild:read_file", "kind" : "read" }));
        assert_eq!(known.kind, Some(ToolKind::Read));
        let typo = parse(serde_json::json!({ "id" : "AinxtBuild:read_file", "kind" : "raed" }));
        assert_eq!(typo.kind, Some(ToolKind::Other));
        let absent = parse(serde_json::json!({ "id" : "AinxtBuild:read_file" }));
    ...
    /// Verify AinxtBuildConcise tools can be finalized and produce concise output.
    #[tokio::test]
    async fn test_concise_namespace_tools() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ToolConfig {
                    id: "AinxtBuildConcise:read_file".to_string(),
                    params: None,
                    name_override: None,
                    params_name_overrides: None,
                    description_override: None,
                ...
                ToolConfig {
                    id: "AinxtBuildConcise:search_replace".to_string(),
                    params: None,
                    name_override: None,
                    params_name_overrides: None,
                    description_override: None,
                ...
                ToolConfig {
                    id: "AinxtBuildConcise:run_terminal_cmd".to_string(),
                    params: Some(
                ...
                ToolConfig::for_tool::<ainxt_build::GrepTool>(),
                ToolConfig::for_tool::<ainxt_build::KillTaskTool>(),
                ToolConfig::for_tool::<ainxt_build::TaskOutputTool>(),
                ToolConfig {
                    id: "AinxtBuild:list_dir".to_string(),
                    params: None,
                    name_override: None,
                    params_name_overrides: None,
                    description_override: None,
    ...
    fn has_tool_id_knows_pinned_tool_config_ids() {
        let builder = ToolRegistryBuilder::new();
        for id in [
            "AinxtBuild:run_terminal_cmd",
            "AinxtBuild:read_file",
            "AinxtBuild:search_replace",
            "AinxtBuild:list_dir",
            "AinxtBuild:grep",
            "AinxtBuild:get_terminal_command_output",
            "AinxtBuild:kill_terminal_command",
        ] {
            assert!(
        ...
        assert!(
            !builder.has_tool_id("AinxtBuild:does_not_exist"),
    ...
    fn known_tool_kinds_maps_pinned_tool_config_ids() {
        ...
        for (id, expected) in [
            ("AinxtBuild:run_terminal_cmd", ToolKind::Execute),
            ("AinxtBuild:read_file", ToolKind::Read),
            ("AinxtBuild:search_replace", ToolKind::Edit),
            ("AinxtBuild:grep", ToolKind::Search),
            ("AinxtBuild:list_dir", ToolKind::List),
        ] {
            assert_eq!(
                kinds.get(id),
                Some(&expected),
        ...
        assert!(
            !kinds.contains_key("AinxtBuild:does_not_exist"),
    ...
    /// (e.g. `"read_file"`). If both `AinxtBuild:read_file` and
    ...
    fn validate_config_reports_param_field_path_and_value() {
        ...
        let config = ToolServerConfig {
            tools: vec![ToolConfig {
                id: "AinxtBuild:run_terminal_cmd".to_string(),
                params: Some(
        ...
        let error = &errors[0];
        assert_eq!(error.tool, "AinxtBuild:run_terminal_cmd");
        assert_eq!(
    ...
    fn validate_config_reports_semantic_hashline_param_error() {
        ...
        let config = ToolServerConfig {
            tools: vec![ToolConfig {
                id: "AinxtBuildHashline:hashline_read".to_string(),
                params: Some(
    ...
    impl ainxt_tool_runtime::Tool for FakeMcpTool {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> ainxt_tool_protocol::ToolId {
            ainxt_tool_protocol::ToolId::new("fake_mcp").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::ainxt_tool_runtime::ListToolsContext,
        ) -> ainxt_tool_types::ToolDescription {
            ainxt_tool_types::ToolDescription::new("fake_mcp", &self.description)
        }
        async fn run(
            &self,
            _ctx: ainxt_tool_runtime::ToolCallContext,
            _input: serde_json::Value,
        ) -> Result<String, ainxt_tool_runtime::ToolError> {
            Ok("ok".into())
        }
    }
    #[tokio::test]
    ...
    impl ainxt_tool_runtime::Tool for NonStreamingStub {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> ainxt_tool_protocol::ToolId {
            ainxt_tool_protocol::ToolId::new("non_streaming_stub").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::ainxt_tool_runtime::ListToolsContext,
        ) -> ainxt_tool_types::ToolDescription {
            ainxt_tool_types::ToolDescription::new("non_streaming_stub", "non-streaming stub")
        }
        async fn run(
            &self,
            _ctx: ainxt_tool_runtime::ToolCallContext,
    ...
    impl ainxt_tool_runtime::Tool for StreamingStub {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> ainxt_tool_protocol::ToolId {
            ainxt_tool_protocol::ToolId::new("streaming_stub").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::ainxt_tool_runtime::ListToolsContext,
        ) -> ainxt_tool_types::ToolDescription {
            ainxt_tool_types::ToolDescription::new("streaming_stub", "streaming stub")
        }
        async fn run(
            &self,
            _ctx: ainxt_tool_runtime::ToolCallContext,
        ...
        async fn execute(
            ...
            _ctx: ainxt_tool_runtime::ToolCallContext,
            _input: serde_json::Value,
        ) -> ainxt_tool_runtime::ToolStream<String> {
            Box::pin(futures::stream::iter(vec![
                ainxt_tool_runtime::ToolStreamItem::Progress(ainxt_tool_runtime::ToolProgress::Text {
                    text: "progress-1".into(),
                }),
                ainxt_tool_runtime::ToolStreamItem::Terminal(Ok("terminal-value".to_string())),
            ]))
        }
    }
    ...
    async fn call_non_streaming_stub_back_compat() {
        ...
        let builder = ToolRegistryBuilder::new();
        let config = ToolServerConfig {
            tools: vec![ToolConfig::for_tool::<ainxt_build::ReadFileTool>()],
            behavior_preset: None,
        };
        let ctx = test_session_context(&tmp);
        let toolset = Arc::new(builder.finalize(config, ctx).unwrap());
        ...
        while let Some(item) = stream.next().await {
            match item {
                ainxt_tool_runtime::ToolStreamItem::Progress(p) => {
                    ...
                    assert!(matches!(p, ainxt_tool_runtime::ToolProgress::Text { .. }));
                    progress_count += 1;
                }
                ainxt_tool_runtime::ToolStreamItem::Terminal(result) => {
    ...
    impl ainxt_tool_runtime::Tool for NoTerminalStub {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> ainxt_tool_protocol::ToolId {
            ainxt_tool_protocol::ToolId::new("no_terminal_stub").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::ainxt_tool_runtime::ListToolsContext,
        ) -> ainxt_tool_types::ToolDescription {
            ainxt_tool_types::ToolDescription::new("no_terminal_stub", "no-terminal stub")
        }
        async fn run(
            &self,
            _ctx: ainxt_tool_runtime::ToolCallContext,
    ...
    async fn tool_definitions_builtins_only_hides_mcp_tools() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ToolConfig::for_tool::<ainxt_build::ReadFileTool>(),
                ToolConfig::for_tool::<ainxt_build::GrepTool>(),
            ],
            behavior_preset: None,
        };
    ...
    fn task_tool_rejected_without_get_task_output_and_kill_task() {
        ...
        assert!(
            errors.iter().any(|e| e.tool == "AinxtBuild:task"
                && e.message.contains("AinxtBuild:get_task_output")
                && e.message.contains("AinxtBuild:kill_task")),
    ...
    fn task_tool_rejected_with_only_get_task_output() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ...
                ToolConfig {
                    id: "AinxtBuild:get_task_output".to_string(),
                    params: None,
                    name_override: None,
                    params_name_overrides: None,
                    description_override: None,
        ...
        assert!(
            errors
                .iter()
                .any(|e| e.tool == "AinxtBuild:task" && e.message.contains("AinxtBuild:kill_task")),
    ...
    fn task_tool_rejected_with_only_kill_task() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ...
                ToolConfig {
                    id: "AinxtBuild:kill_task".to_string(),
                    params: None,
                    name_override: None,
                    params_name_overrides: None,
                    description_override: None,
    ...
    /// ainxt-build agent config (all tools present) and that the new examples
    ...
    async fn background_param_templates_reference_real_schema_keys() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                tool("AinxtBuild:run_terminal_cmd"),
                tool("AinxtBuild:task"),
                tool("AinxtBuild:get_task_output"),
                tool("AinxtBuild:wait_tasks"),
                tool("AinxtBuild:kill_task"),
            ],
            behavior_preset: None,
        };
        let tmp = TempDir::new().unwrap();
    ...
    fn bash_with_enabled_background_reports_missing_task_tools() {
        ...
        assert!(error.message.contains("AinxtBuild:get_task_output"));
        assert!(error.message.contains("AinxtBuild:kill_task"));
        assert!(
            error
                .expected
                .as_deref()
    ...
    fn task_requirement_error_lists_missing_background_tools() {
        ...
        let error = &errors[0];
        assert_eq!(error.tool, "AinxtBuild:task");
        assert!(error.message.contains("AinxtBuild:get_task_output"));
        assert!(error.message.contains("AinxtBuild:kill_task"));
    ...
    fn search_replace_requirement_error_mentions_read_tool() {
        ...
        let error = errors
            .iter()
            .find(|error| error.tool == "AinxtBuild:search_replace")
    ...
    /// the baseline agent (its plan-mode prompt note is `${% if %}`-guarded).
    #[test]
    fn ask_user_question_validates_standalone() {
        let builder = ToolRegistryBuilder::new();
        let config = ToolServerConfig {
            tools: vec![ToolConfig {
                id: "AinxtBuild:ask_user_question".to_string(),
                params: None,
                name_override: None,
                params_name_overrides: None,
                description_override: None,
    ...
    async fn task_description_renders_with_default_agent_config() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ...
                ToolConfig {
                    id: "AinxtBuild:web_search".to_string(),
                    params: None,
                    name_override: None,
                    params_name_overrides: None,
                    description_override: None,
        ...
        let toolset = builder
            .finalize(config, ctx)
            .expect("finalize should succeed with default ainxt-build tools");
        let defs = toolset.tool_definitions();
        let task_def = defs
            .iter()
    ...
    fn hashline_tools_registered_in_builder() {
        ...
        assert!(
            builder
                .tools
                .contains_key("AinxtBuildHashline:hashline_read"),
            "hashline_read should be registered"
        );
        assert!(
            builder
                .tools
                .contains_key("AinxtBuildHashline:hashline_edit"),
            "hashline_edit should be registered"
        );
        assert!(
            builder
                .tools
                .contains_key("AinxtBuildHashline:hashline_grep"),
    ...
    async fn hashline_config_finalizes_successfully() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                hashline_tool_config("AinxtBuildHashline:hashline_read"),
                hashline_tool_config("AinxtBuildHashline:hashline_edit"),
                hashline_tool_config("AinxtBuildHashline:hashline_grep"),
            ],
            behavior_preset: None,
        };
        let ctx = test_session_context(&tmp);
    ...
    async fn standard_and_hashline_each_finalize_independently() {
        ...
        let standard_config = ToolServerConfig {
            tools: vec![
                hashline_tool_config("AinxtBuild:read_file"),
                hashline_tool_config("AinxtBuild:search_replace"),
                hashline_tool_config("AinxtBuild:grep"),
            ],
            behavior_preset: None,
        };
        let ctx = test_session_context(&tmp);
    ...
    async fn empty_struct_schema_has_properties_and_required() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ToolConfig::for_tool::<ainxt_build::EnterPlanModeTool>(),
                ToolConfig::for_tool::<ainxt_build::ExitPlanModeTool>(),
            ],
            behavior_preset: None,
        };
        let ctx = test_session_context(&tmp);
    ...
    async fn hashline_toolset_e2e_with_utilities() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ToolConfig {
                    id: "AinxtBuildHashline:hashline_read".to_owned(),
                    params: Some(
                ...
                ToolConfig::for_tool::<ainxt_build_hashline::HashlineEditTool>(),
                ToolConfig::for_tool::<ainxt_build_hashline::HashlineGrepTool>(),
                ToolConfig::for_tool::<ainxt_build::ListDirTool>(),
            ],
            behavior_preset: None,
        };
        let errors = builder.validate_config(&config);
    ...
    fn bash_config_with_background() -> ToolConfig {
        ToolConfig {
            id: "AinxtBuild:run_terminal_cmd".to_owned(),
            params: Some(
                serde_json::json!({ "enabled_background" : true })
                    .as_object()
                    .unwrap()
    ...
    async fn ainxt_build_bridge(tmp: &TempDir) -> crate::bridge::ToolBridge {
        let builder = ToolRegistryBuilder::new();
        let config = ToolServerConfig {
            tools: vec![
                ToolConfig::for_tool::<ainxt_build::ListDirTool>(),
                ToolConfig::for_tool::<ainxt_build::ReadFileTool>(),
                ToolConfig::for_tool::<ainxt_build::SearchReplaceTool>(),
                bash_config_with_background(),
                ToolConfig::for_tool::<ainxt_build::TaskOutputTool>(),
                ToolConfig::for_tool::<ainxt_build::KillTaskTool>(),
                ToolConfig::for_tool::<ainxt_build::GrepTool>(),
                ToolConfig::for_tool::<ainxt_build::TodoWriteTool>(),
            ],
            behavior_preset: None,
        };
    ...
    async fn hub_dispatch_list_dir() {
        ...
        let bridge = ainxt_build_bridge(&tmp).await;
        let result = bridge
            .call(
                "list_dir",
    ...
    /// Equivalence: list_dir through hub dispatch produces the same prompt_text
    ...
    async fn hub_dispatch_parity_list_dir() {
        ...
        let hub_bridge = ainxt_build_bridge(&tmp).await;
        ...
        let builder = ToolRegistryBuilder::new();
        let config = ToolServerConfig {
            tools: vec![ToolConfig::for_tool::<ainxt_build::ListDirTool>()],
            behavior_preset: None,
        };
    ...
    async fn prepare_dispatch_stamps_workspace_viewer_ctx_when_present() {
        let (toolset, _tmp) =
            toolset_with_viewer_ctx(Some(ainxt_tool_runtime::WorkspaceViewerContext {
        ...
        let wvc = parts
            .ctx
            .extensions
            .get::<ainxt_tool_runtime::WorkspaceViewerContext>()
    ...
    async fn bash_streaming_progress_emitted_via_finalized_toolset_when_gate_on() {
        ...
        while let Some(item) = stream.next().await {
            match item {
                ainxt_tool_runtime::ToolStreamItem::Progress(_) => progress_count += 1,
                ainxt_tool_runtime::ToolStreamItem::Terminal(r) => {
