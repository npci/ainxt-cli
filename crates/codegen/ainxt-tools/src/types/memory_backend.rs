...
//!
//! `MemoryBackend` is defined in `ainxt-tools` to keep the tool crate
...
///
/// Use `tracing::info!(target: MEMORY_LOG_TARGET, ...)` in `ainxt-tools`.
/// Matches `ainxt_shell::session::memory_log::TARGET`.
pub const MEMORY_LOG_TARGET: &str = "ainxt_memory";
