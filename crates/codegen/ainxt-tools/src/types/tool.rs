...
//!
//! The tool runtime contract (`Tool` trait) lives in `ainxt_tool_runtime`.
...
///
/// Serializes to snake_case (`ainxt_build`, `mcp`, …) for the
...
/// (e.g. `"AinxtBuild:read_file"`); only the serde form goes on the wire.
#[derive(
    Debug,
    Clone,
    Copy,
...
pub enum ToolNamespace {
    #[serde(alias = "AinxtBuild")]
    AinxtBuild,
    #[serde(alias = "AinxtBuildConcise")]
    AinxtBuildConcise,
    #[serde(alias = "AinxtBuildHashline")]
    AinxtBuildHashline,
    #[serde(alias = "Codex")]
    Codex,
...
/// [`crate::tool_taxonomy`]) matches that openness: an advisory string, not a
/// closed enum.
#[derive(
    Debug,
    Clone,
