...
//!
//! `ToolSearchIndex` is defined in `ainxt-tools` to keep the tool crate
//! backend-agnostic. The concrete implementation lives in `ainxt-shell`
...
/// in `ainxt-shell` maps `mcp_initialized` to `is_ready`.
...
/// serialized), injected by `ainxt-shell` after MCP initialization.
#[derive(Clone)]
