...
impl<'a> SkillEntry<'a> {
    ...
    // ── Budgeted XML rendering (ainxt build harness) ─────────────
    ...
    /// - Body content: no XML escaping (passed through unchanged)
...
impl<'a> SkillListing<'a> {
    ...
    /// Render as unchanged XML, returning `None` if empty.
    ///
...
///
/// `"/path/.ainxt/skills/my-skill/SKILL.md"` -> `"/path/.ainxt/skills/"`
...
/// Used by the budgeted (ainxt build) XML rendering path.
...
pub enum XmlRenderMode {
    /// Unchanged rendering: every skill with its full description; no
    /// budget cap, minimal escaping.
    Verbatim,
    /// ainxt build harness: budget-capped three-tier rendering with full
    /// XML entity escaping.
    Budgeted {
        ...
        skills
            ...
            .filter(|s| {
                s.enabled
                    ...
                    // Compat mode renders all skills unchanged; only the ainxt
...
/// startup `<system-reminder>` exactly (standard header, `Use when:`
...
mod tests {
    ...
    fn verbatim_bypasses_is_listable_filter() {
        // ainxt-build drops a description-less skill; compat mode renders all.
    ...
    fn smoke_table_first_skills_do_not_flatten_into_listing() {
        ...
        let skills = parse_skill_files(vec![
            ...
            // Authored description → shown unchanged.
            write(
                "authored",
    ...
    /// - Attribute values: only `"` is escaped to `&quot;`
    /// - Body content: NO escaping (passed through unchanged)
    #[test]
    ...
    // ── budgeted mode: ainxt build harness (budgeted XML) ───────────
    ...
    fn xml_verbatim_preserves_full_description_no_extraction() {
        ...
        let text =
            format_announcement_xml(&skills, &mut announced, None, None, XmlRenderMode::Verbatim)
                .unwrap();
        // vendor-compat must render the full original description unchanged
        assert!(
