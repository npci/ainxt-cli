//! Canonical external-settings tool name ↔ ainxt tool correspondence: one table
//! replacing two that drifted apart.
//!
//! Two consumers read it independently. The hook matcher (`ainxt-hooks`) needs the
//! ainxt tool **names** an external settings term maps to (and the reverse, for regex
//! matchers); the agent builder (`ainxt-agent`) needs the [`ToolKind`] a `tools:`
...
//!
//! The `ainxt` names are test-checked against the live registry.

use super::tool::ToolKind;
use ToolKind::*;

/// One Claude tool's correspondence to ainxt, read via the accessor functions below.
struct ClaudeTool {
    claude: &'static str,
    /// ainxt [`ToolKind`] for allowlist resolution; `None` for names that are matchable
    /// (spawn/plan-mode directives) but must not resolve an allowlist.
    kind: Option<ToolKind>,
    /// ainxt tool names this Claude tool maps to (empty when there is no direct
    /// ainxt tool — the entry then only contributes a `kind`).
    ainxt: &'static [&'static str],
}

/// Row that resolves an allowlist (carries a [`ToolKind`]) — the common case.
const fn k(claude: &'static str, kind: ToolKind, ainxt: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: Some(kind),
        ainxt,
...
const fn match_only(claude: &'static str, ainxt: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: None,
        ainxt,
...
const CLAUDE_TOOLS: &[ClaudeTool] = &[
    ...
    k("Skill",           Read,                 &["skill"]),                           // matcher: opencode's `skill` tool; allowlist Read (ainxt-build reads SKILL.md)
...
/// The ainxt [`ToolKind`] a Claude allowlist entry resolves to, if any.
...
/// The ainxt tool names a Claude matcher term fires on.
pub fn ainxt_names_for(claude: &str) -> impl Iterator<Item = &'static str> {
    CLAUDE_TOOLS
        .iter()
        .find(|t| t.claude == claude)
        .map(|t| t.ainxt)
...
/// The Claude names that map to `ainxt_name` (reverse lookup, for regex matchers).
pub fn claude_names_for(ainxt_name: &str) -> impl Iterator<Item = &'static str> + '_ {
    CLAUDE_TOOLS
        .iter()
        .filter(move |t| t.ainxt.contains(&ainxt_name))
        .map(|t| t.claude)
}

/// Every distinct ainxt name the table references, for the `ainxt-agent` drift-check
/// test that asserts each is a real client tool name.
pub fn ainxt_names() -> impl Iterator<Item = &'static str> {
    let mut seen = std::collections::HashSet::new();
    CLAUDE_TOOLS
        .iter()
        .flat_map(|t| t.ainxt.iter().copied())
...
mod tests {
    ...
    fn every_row_contributes() {
        // A row with neither a kind nor a ainxt name is dead weight (and signals a typo).
        for t in CLAUDE_TOOLS {
            assert!(
                t.kind.is_some() || !t.ainxt.is_empty(),
                "dead row: {}",
                t.claude
            );
        }
