//! `ToolMetadata` — ainxt-tools-specific metadata for tools.
//!
//! Each tool implements two traits:
//! 1. `ainxt_tool_runtime::Tool` — typed Args/Output, `run()` with actual logic
...
//! ## Context helpers
//!
//! Tools access session state through `ainxt_tool_runtime::ToolCallContext`
...
/// ainxt-tools-specific metadata trait.
///
/// Each tool struct implements this alongside `ainxt_tool_runtime::Tool`.
...
pub trait ToolMetadata: Send + Sync {
    ...
    /// Namespace grouping (AinxtBuild, Cursor, OpenCode, ...).
...
) -> Result<SharedResources, ainxt_tool_runtime::ToolError> {
    ctx.extensions
        .get::<SharedResources>()
        .map(|arc| (*arc).clone())
        .ok_or_else(|| {
...
pub async fn resolve_cwd(
...
) -> Result<PathBuf, ainxt_tool_runtime::ToolError> {
    if let Some(cwd) = ctx.extensions.get::<ainxt_tool_runtime::Cwd>() {
    ...
    res.get::<crate::types::resources::Cwd>()
        .map(|c| c.0.clone())
        .ok_or_else(|| {
            ainxt_tool_runtime::ToolError::custom("missing_cwd", "Cwd not available in Resources")
        })
}
...
pub fn test_ctx(resources: SharedResources) -> ainxt_tool_runtime::ToolCallContext {
    ...
    ctx.extensions
        .insert(ainxt_tool_runtime::WorkspaceViewerContext {
...
pub fn test_ctx_with_call_id(
    ...
    let id = ainxt_tool_protocol::ToolCallId::new(call_id)
        .unwrap_or_else(|_| ainxt_tool_protocol::ToolCallId::new_v7());
    let mut ctx = ainxt_tool_runtime::ToolCallContext::new(id);
    ctx.extensions.insert(resources);
    ctx.extensions
        .insert(ainxt_tool_runtime::WorkspaceViewerContext {
            stream_tool_progress: true,
...
pub fn behavior_version(ctx: &ainxt_tool_runtime::ToolCallContext) -> Option<String> {
    ctx.extensions
