...
use ainxt_tool_types::SubagentCompletedOutput;
...
use crate::implementations::ainxt_build::todo::{TodoItem, TodoState};
...
pub(crate) fn typed_tool_output_preserving_cco(
    ...
    source: &impl ainxt_tool_runtime::ToolOutput,
) -> ainxt_tool_runtime::TypedToolOutput {
    let cco = source.chat_completion_output();
    let value = serde_json::to_value(payload).unwrap_or(serde_json::Value::Null);
    ainxt_tool_runtime::TypedToolOutput::from_value(tool_id, value).with_chat_completion_output(cco)
}
...
/// protocol boundary in `ainxt-shell`.
#[derive(Debug, Clone, Serialize, Deserialize, schemars::JsonSchema)]
...
pub struct BashOutput {
    ...
    /// Set by the ainxt_build `run_terminal_cmd` implementation when the
    /// command was detected as a bare `echo "<msg>"` (or close variant:
    /// echo -n, echo -e, simple printf for literal output, etc.).
    ///
    /// Used for:
    /// - Telemetry / statistics on this pattern for the ainxt_build backend.
...
pub struct BackgroundTaskStarted {
    ...
    /// uses this string unchanged instead of the default
...
pub struct WebFetchContent {
    ...
    pub bytes: usize,
    /// Internal path to the complete converted body when AinxtBuild persisted overflow.
    #[serde(skip)]
    #[schemars(skip)]
...
use ainxt_tool_types::KillTaskOutput;
...
impl ainxt_tool_runtime::ToolOutput for BashToolOutput {
    fn chat_completion_output(&self) -> Option<ainxt_tool_runtime::ToolChatCompletionResponse> {
        match self {
            Self::Bash(bash) => ainxt_tool_runtime::ToolOutput::chat_completion_output(bash),
...
pub enum ToolOutput {
    ...
    Monitor(crate::implementations::ainxt_build::monitor::types::MonitorOutput),
    SchedulerCreate(crate::implementations::ainxt_build::scheduler::create::SchedulerCreateOutput),
    SchedulerDelete(crate::implementations::ainxt_build::scheduler::delete::SchedulerDeleteOutput),
    SchedulerList(crate::implementations::ainxt_build::scheduler::list::SchedulerListOutput),
    UpdateGoal(crate::implementations::ainxt_build::update_goal::UpdateGoalOutput),
...
pub enum TodoWriteOutput {
    ...
    DuplicateId(String),
    /// Argument validation failed (model-facing message is returned unchanged).
...
impl ainxt_tool_runtime::ToolOutput for ToolOutput {
    fn chat_completion_output(&self) -> Option<ainxt_tool_runtime::ToolChatCompletionResponse> {
        match self {
            Self::Bash(bash) => ainxt_tool_runtime::ToolOutput::chat_completion_output(bash),
            _ => None,
...
impl ainxt_tool_runtime::ToolOutput for BashOutput {
    fn chat_completion_output(&self) -> Option<ainxt_tool_runtime::ToolChatCompletionResponse> {
        ...
        Some(ainxt_tool_runtime::ToolChatCompletionResponse {
            result: Some(ainxt_tool_runtime::ToolChatCompletion {
                sender: "assistant".into(),
                message_tag: Some("raw_function_result".into()),
                code_execution_result: Some(ainxt_tool_runtime::ToolCodeExecutionResult {
                    stdout,
                    stderr: String::new(),
...
impl ainxt_tool_runtime::ToolOutput for GrepSearchOutput {}
impl ainxt_tool_runtime::ToolOutput for ReadFileOutput {}
impl ainxt_tool_runtime::ToolOutput for ListDirOutput {}
impl ainxt_tool_runtime::ToolOutput for SearchReplaceOutput {}
impl ainxt_tool_runtime::ToolOutput for TodoWriteOutput {}
impl ainxt_tool_runtime::ToolOutput for WebSearchOutput {}
impl ainxt_tool_runtime::ToolOutput for WebFetchOutput {}
impl ainxt_tool_runtime::ToolOutput for SkillOutput {}
impl ainxt_tool_runtime::ToolOutput for ApplyPatchOutput {}
impl ainxt_tool_runtime::ToolOutput for CodexGrepFilesOutput {}
impl ainxt_tool_runtime::ToolOutput for SearchToolOutput {}
impl ainxt_tool_runtime::ToolOutput for EnterPlanModeOutput {}
...
mod tests {
    use super::*;
    use crate::implementations::ainxt_build::todo::{TodoPriority, TodoStatus};
    use serde_json::json;
    use ainxt_tool_types::KillTaskResult;
    use ainxt_tool_types::TaskOutputResult;
    ...
    fn subagent_completed_prompt_format_with_worktree() {
        let output = ToolOutput::SubagentCompleted(SubagentCompletedOutput {
            ...
            turns: 1,
            duration_ms: 2000,
            worktree_path: Some("/tmp/ainxt-worktree/wt-agent".into()),
            persona: None,
        ...
        let rendered = output.to_prompt_format();
        assert!(
            rendered.contains("<worktree_path>/tmp/ainxt-worktree/wt-agent</worktree_path>"),
    ...
    fn assert_cer(
        resp: &ainxt_tool_runtime::ToolChatCompletionResponse,
        stdout: &str,
        exit_code: i32,
        timed_out: bool,
    ) {
    ...
    fn bash_output_chat_completion_carries_exit_and_stdout() {
        let resp = ainxt_tool_runtime::ToolOutput::chat_completion_output(&sample_bash(
            0, b"hello\n", false,
        ))
        .unwrap();
    ...
    fn bash_output_chat_completion_empty_stdout_still_emits() {
        let resp =
            ainxt_tool_runtime::ToolOutput::chat_completion_output(&sample_bash(0, b"", false))
                .unwrap();
    ...
    fn bash_output_chat_completion_truncation_marker_and_extra() {
        ...
        let resp = ainxt_tool_runtime::ToolOutput::chat_completion_output(&bash).unwrap();
        let result = resp.result.as_ref().unwrap();
    ...
    fn bash_tool_output_foreground_delegates_background_skips() {
        let resp = ainxt_tool_runtime::ToolOutput::chat_completion_output(&BashToolOutput::Bash(
            sample_bash(0, b"ok", false),
        ))
        .unwrap();
        assert_cer(&resp, "ok", 0, false);
        assert!(
            ainxt_tool_runtime::ToolOutput::chat_completion_output(
    ...
    fn aggregate_tool_output_bash_delegates_background_skips() {
        let resp = ainxt_tool_runtime::ToolOutput::chat_completion_output(&ToolOutput::Bash(
        ...
        assert!(
            ainxt_tool_runtime::ToolOutput::chat_completion_output(&ToolOutput::Text(
    ...
    fn bash_tool_id() -> ainxt_tool_protocol::ToolId {
        ainxt_tool_protocol::ToolId::new("bash").unwrap()
    }
    #[test]
    fn into_typed_tool_output_preserves_bash_foreground_cco() {
        ...
        let dropped = ainxt_tool_runtime::TypedToolOutput::from_value(typed.tool_id, expected_value);
    ...
    fn into_typed_tool_output_non_bash_cco_is_none() {
        ...
        let typed = run.into_typed_tool_output(ainxt_tool_protocol::ToolId::new("text").unwrap());
