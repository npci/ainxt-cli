...
pub enum ConfigSource {
    ...
    Builtin,
    /// Bundled skill shipped with the binary (extracted to ~/.ainxt/skills/
    /// or injected via bundled skill dirs).
    Bundled { path: PathBuf },
    /// Server-synced (e.g. ~/.ainxt/server-skills from the skill store).
    Server { path: PathBuf },
    /// Project-scoped: cwd/.ainxt/ or cwd/.claude/.
    Project { path: PathBuf },
    /// User-scoped: ~/.ainxt/ or ~/.claude/.
    User { path: PathBuf },
