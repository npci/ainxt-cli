...
///
/// Use with `#[schemars(with = "AinxtIntegerSchema")]` on `Option<usize>` fields.
pub struct AinxtIntegerSchema;
impl schemars::JsonSchema for AinxtIntegerSchema {
    fn schema_name() -> std::borrow::Cow<'static, str> {
        "ainxt_integer_schema".into()
    }
...
/// Lenient boolean deserializers (shared via `ainxt-tool-types`), re-exported so
/// fields reference them under the same `crate::types::schema::` path as above.
pub use ainxt_tool_types::{deserialize_lenient_bool, deserialize_lenient_option_bool};
#[cfg(test)]
mod tests {
    use super::*;
