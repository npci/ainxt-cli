...
/// `compat_default_matches_legacy_constants` asserts equivalence against.
#[cfg(test)]
...
#[cfg(test)]
pub(crate) const RULES_DIRS: &[&str] = &[".ainxt/rules", ".claude/rules", ".cursor/rules"];
    ...
    pub async fn check_path(&mut self, target_path: &Path) -> Vec<PathBuf> {
        ...
        while let Some(dir) = current {
            ...
            if !self.checked_dirs.contains(&dir_buf) {
                ...
                // Check for rules files in .ainxt/rules/, .claude/rules/, and
