...
///
/// `skills` is the standard layout (`.ainxt/skills/`, `.claude/skills/`,
/// `.cursor/skills/`). The product-specific `skills-cursor/` layout is no
/// longer scanned — it pulled vendor default skills into ainxt sessions.
...
/// name (e.g. `~/.ainxt/skills/shell`) is NOT dropped — only skills physically
...
pub fn read_frontmatter_only(path: &Path) -> std::io::Result<(String, usize)> {
    ...
    // Explicitly `.zeroize()`-d at the bottom of every loop iteration (not
    // just relying on end-of-scope `Drop`), immediately after this line's
    // content has been copied into `frontmatter`.
...
pub fn parse_skill_files(skill_files: Vec<(PathBuf, SkillScope)>) -> Vec<SkillInfo> {
    ...
    // per-vendor toggle, so vendor builtins never leak into ainxt.
...
/// `cwd` (exclusive). At each directory, checks for `.ainxt/skills/`,
...
/// visits known config roots (`.ainxt`, `.agents`, `.claude`, …); those are
...
/// historically, and preserving that keeps default behavior exactly. The
...
pub fn discover_skills_for_paths(
    ...
    // `.ainxt` and `.agents` are always scanned; `.claude` is gated on the
    // claude-vendor skills cell. (`.cursor` is excluded here by design — see fn docs.)
    let mut config_dir_names: Vec<&str> = vec![".ainxt", ".agents"];
...
";
    ...
    fn is_vendor_default_skill_spares_user_skill_outside_vendor_dir() {
        // A user's own "shell" skill in ~/.ainxt is NOT a vendor builtin.
        assert!(!is_vendor_default_skill(
            "/home/u/.ainxt/skills/shell/SKILL.md",
            "shell"
        ));
    }
    ...
    fn parse_skill_files_drops_denylisted_cursor_skill() {
        ...
        .unwrap();
        // Same name under /.ainxt/ → kept (user content).
        let ainxt_shell = tmp.path().join(".ainxt").join("skills").join("shell");
        std::fs::create_dir_all(&ainxt_shell).unwrap();
        std::fs::write(
            ainxt_shell.join("SKILL.md"),
        ...
        let skills = parse_skill_files(vec![
            ...
            (ainxt_shell.join("SKILL.md"), SkillScope::User),
        ]);
        assert_eq!(skills.len(), 1, "cursor builtin must be dropped");
        assert!(skills[0].path.contains("/.ainxt/"));
    }
    ...
    fn discover_skills_for_paths_gates_claude_dir() {
        ...
        // A .claude skill and a .ainxt skill in an intermediate dir.
        ...
        .unwrap();
        let ainxt_skill = sub.join(".ainxt").join("skills").join("ainxt-dyn");
        std::fs::create_dir_all(&ainxt_skill).unwrap();
        std::fs::write(ainxt_skill.join("SKILL.md"), "---\nname: ainxt-dyn\n---\n").unwrap();
        ...
        assert!(
            names_on.contains(&"ainxt-dyn"),
            "ainxt-dyn missing: {names_on:?}"
        );
        assert!(
        ...
        // claude.skills OFF → only ainxt-dyn discovered.
        ...
        assert!(
            names_off.contains(&"ainxt-dyn"),
            "ainxt-dyn missing: {names_off:?}"
        );
        assert!(
