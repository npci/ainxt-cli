...
// New implementation is in `ainxt_build/skill/`.
...
///
/// Supported variables (ainxt-native names + compat aliases):
///
...
/// | `${AINXT_PLUGIN_ROOT}` | `${CLAUDE_PLUGIN_ROOT}` | Plugin root dir (plugin-backed skills) |
/// | `${AINXT_PLUGIN_DATA}` | `${CLAUDE_PLUGIN_DATA}` | Plugin data dir (plugin-backed skills) |
///
...
/// `load_skill_content` in `ainxt_build/skill/mod.rs` is a duplicate
/// of this.
...
It has multiple lines."#;
    ...
    fn test_build_skill_message_exact_format() {
        let skill = SkillInfo {
            ...
            author: None,
            argument_hint: None,
            path: "/home/user/.ainxt/skills/commit/SKILL.md".to_string(),
            scope: SkillScope::User,
            config_source: None,
            plugin_name: None,
            plugin_version: None,
        ...
        let expected = "\
<skill name=\"commit\" description=\"Create a git commit\" path=\"/home/user/.ainxt/skills/commit/SKILL.md\">
# Git Commit Skill
...
You are helping the user create a commit.
    ...
    fn test_build_skill_message_special_chars_in_fields() {
        ...
        // are inserted unchanged (no escaping) so the test breaks if we add
        // escaping later.
        let skill = SkillInfo {
...
<skill name=\"empty\" description=\"An empty skill\" path=\"/skills/empty/SKILL.md\">
    ...
    fn test_build_skill_message_multiline_content() {
        let skill = SkillInfo {
            ...
            path: "/repo/.ainxt/skills/review/SKILL.md".to_string(),
            ..SkillInfo::default()
        };
        ...
        let expected = "\
<skill name=\"review\" description=\"Review code\" path=\"/repo/.ainxt/skills/review/SKILL.md\">
# Code Review
...
- No warnings
    ...
    fn test_substitutions_skill_dir() {
        ...
        apply_substitutions(
            ...
            None,
            &SubstitutionContext {
                skill_dir: Some("/home/user/.ainxt/skills/deploy"),
                ..Default::default()
            },
        );
        assert_eq!(
            content,
            "Config at /home/user/.ainxt/skills/deploy/config.json"
        );
    }
    ...
    #[test]
    fn test_ainxt_plugin_aliases_substitution() {
        let mut content = "Root ${AINXT_PLUGIN_ROOT}, data ${AINXT_PLUGIN_DATA}".to_string();
        apply_substitutions(
            &mut content,
            None,
            &SubstitutionContext {
...
Review code.
    ...
    fn test_build_skill_information_single() {
        ...
        let refs = vec![SkillRef {
            name: "commit",
            path: "/home/user/.ainxt/skills/commit/SKILL.md",
        }];
        ...
        assert!(
            result.contains(
                "<skill name=\"commit\" path=\"/home/user/.ainxt/skills/commit/SKILL.md\"/>"
            )
        );
    ...
    fn test_build_skill_information_multi() {
        ...
        let refs = vec![
            SkillRef {
                name: "review",
                path: "/project/.ainxt/skills/review/SKILL.md",
            },
            SkillRef {
                name: "lint",
                path: "/project/.ainxt/skills/lint/SKILL.md",
            },
        ];
        let result = build_skill_information(&blocks, &refs);
        assert!(result.starts_with("<skill_information>\n"));
