...
${%- endfor %}
${%- else %}
(No skills available. Skills can be added in ~/.ainxt/skills/ or .ainxt/skills/)
${%- endif %}
</available_skills>"#;
...
impl ainxt_tool_runtime::Tool for SkillTool {
    type Args = SkillInput;
    type Output = SkillOutput;

    fn id(&self) -> ainxt_tool_protocol::ToolId {
        ainxt_tool_protocol::ToolId::new("skill").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: SkillInput,
    ) -> Result<SkillOutput, ainxt_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn tool_metadata() {
        ...
        let tool = SkillTool;
        assert_eq!(ainxt_tool_runtime::Tool::id(&tool).as_str(), "skill");
