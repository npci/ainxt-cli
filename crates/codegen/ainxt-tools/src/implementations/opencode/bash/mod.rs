...
impl ainxt_tool_runtime::Tool for BashTool {
    ...
    fn id(&self) -> ainxt_tool_protocol::ToolId {
        ainxt_tool_protocol::ToolId::new("bash").expect("valid tool id")
    }
        ...
        let result = match backend.run(request).await {
            ...
            Err(e) => {
                ...
                return Err(ainxt_tool_runtime::ToolError::execution(
                    ainxt_tool_protocol::ToolId::new("run_terminal_command").expect("valid"),
                    e.to_string(),
                ));
            }
        };
...
mod tests {
    ...
    fn rt_ctx_with_call_id(
        ...
        let id = ainxt_tool_protocol::ToolCallId::new(call_id).unwrap();
    ...
    async fn tool_metadata() {
        ...
        let tool = BashTool;
        assert_eq!(ainxt_tool_runtime::Tool::id(&tool).as_str(), "bash");
    ...
    // ─── Description template shell-awareness equivalence tests ───
    //
    // Same shape as ainxt_build/bash: the opencode tool inherits the same
