//! LSP server configuration from `.ainxt/lsp.json`.
...
/// This is the canonical merge function — both session startup and `ainxt inspect` call it.
...
pub fn load_servers_with_plugins_sourced(
    ...
    let user_path = crate::util::ainxt_home::ainxt_home().join("lsp.json");
    let project_path = cwd.join(".ainxt").join("lsp.json");
...
/// Load LSP server configs from `~/.ainxt/lsp.json` and `<cwd>/.ainxt/lsp.json`.
