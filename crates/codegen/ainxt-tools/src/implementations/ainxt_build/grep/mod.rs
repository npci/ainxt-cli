...
// Re-export the shared AinxtIntegerSchema from types module
pub use crate::types::AinxtIntegerSchema;
use ripgrep::rg_path;
...
crate::register_resource!("ainxt_build", "Grep", GrepParams);
...
fn grep_timeout() -> Duration {
    Duration::from_secs(grep_timeout_secs(ainxt_tty_utils::is_wsl()))
}
...
static GREP_CAPABILITIES: LazyLock<ainxt_tool_protocol::ToolCapabilities> =
...
impl ainxt_tool_runtime::Tool for GrepTool {
    type Args = GrepSearchInput;
    type Output = GrepSearchOutput;

    fn id(&self) -> ainxt_tool_protocol::ToolId {
        ainxt_tool_protocol::ToolId::new("grep").expect("valid tool id")
    }
    ...
    /// Streaming entry point. Gate OFF (default): exactly the blocking
    ...
    async fn execute(
    ...
    ) -> ainxt_tool_runtime::ToolStream<GrepSearchOutput> {
        ...
        let Some(spec) = admitted_spec else {
            return ainxt_tool_runtime::terminal_only(self.run(ctx, input).await);
        };
    ...
    async fn run(
        ...
        ctx: ainxt_tool_runtime::ToolCallContext,
        input: GrepSearchInput,
    ) -> Result<GrepSearchOutput, ainxt_tool_runtime::ToolError> {
...
fn grep_progress_stream(
    ctx: ainxt_tool_runtime::ToolCallContext,
    input: GrepSearchInput,
    spec: &'static ainxt_tool_protocol::StreamingSpec,
    span: tracing::Span,
) -> ainxt_tool_runtime::ToolStream<GrepSearchOutput> {
    Box::pin(async_stream::stream! {
        ...
        let GrepReady {
            ...
            Ok(GrepStep::Early(out)) => {
                ...
                yield ainxt_tool_runtime::ToolStreamItem::Terminal(Ok(out));
                return;
            }
            Err(e) => {
                yield ainxt_tool_runtime::ToolStreamItem::Terminal(Err(e));
                return;
            }
        };
        ...
        if timed_out {
            ...
            if stdout_buf.is_empty() {
                yield ainxt_tool_runtime::ToolStreamItem::Terminal(Ok(grep_timeout_output(secs)));
            } else {
                ...
                output.exit_code = -1;
                yield ainxt_tool_runtime::ToolStreamItem::Terminal(Ok(output));
            }
            return;
        }
...
async fn prepare_grep(
    ctx: &ainxt_tool_runtime::ToolCallContext,
    input: &GrepSearchInput,
) -> Result<GrepStep, ainxt_tool_runtime::ToolError> {
...
struct BodyStreamer<'a> {
    spec: &'a ainxt_tool_protocol::StreamingSpec,
...
impl<'a> BodyStreamer<'a> {
    fn new(spec: &'a ainxt_tool_protocol::StreamingSpec, config: &'a GrepFormatConfig) -> Self {
        Self {
            spec,
            config,
            body: String::new(),
    ...
    fn feed(&mut self, bytes: &[u8]) -> Vec<ainxt_tool_runtime::ToolProgress> {
    ...
    /// Flush the final non-`\n`-terminated segment unchanged at EOF (matches
    /// `str::lines()`, which keeps a trailing `\r`).
    fn finish(&mut self) -> Option<ainxt_tool_runtime::ToolProgress> {
    ...
    fn push_line(&mut self, line: &[u8]) -> Option<ainxt_tool_runtime::ToolProgress> {
        ...
        ainxt_tool_runtime::stream_chunk(
            self.spec,
            self.body.as_bytes(),
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        let tool = GrepTool;
        assert_eq!(ainxt_tool_runtime::Tool::id(&tool).as_str(), "grep");
    ...
    async fn tool_grep_no_matches() {
        ...
        let tool = GrepTool;
        let output = ainxt_tool_runtime::Tool::run(
            &tool,
    ...
    async fn deny_read_globs_exclude_denied_files() {
        ...
        let run = |resources: Resources, input| async {
            let out =
                ainxt_tool_runtime::Tool::run(&GrepTool, test_ctx(resources.into_shared()), input)
                    .await
                    .unwrap();
    ...
    fn read_grep_delta(p: &ainxt_tool_runtime::ToolProgress) -> String {
        match p {
    ...
    async fn grep_streaming_body_matches_card_body() {
        ...
        let mut terminal: Option<Result<GrepSearchOutput, ainxt_tool_runtime::ToolError>> = None;
    ...
    /// emitted (exactly pre-streaming) while the terminal still surfaces.
    #[tokio::test]
    async fn grep_streaming_suppressed_when_gate_off() {
        ...
        let tool = GrepTool;
        let mut stream =
            ainxt_tool_runtime::Tool::execute(&tool, ctx, make_grep_input("findme")).await;

        let mut progress = 0usize;
        let mut terminal: Option<Result<GrepSearchOutput, ainxt_tool_runtime::ToolError>> = None;
        while let Some(item) = stream.next().await {
            match item {
                ainxt_tool_runtime::ToolStreamItem::Progress(_) => progress += 1,
                ainxt_tool_runtime::ToolStreamItem::Terminal(r) => {
                    assert!(terminal.is_none(), "more than one Terminal yielded");
                    terminal = Some(r);
                }
    ...
    async fn grep_streaming_body_matches_card_body_when_truncated() {
        ...
        let tool = GrepTool;
        let mut stream =
            ainxt_tool_runtime::Tool::execute(&tool, test_ctx(resources.into_shared()), input).await;
