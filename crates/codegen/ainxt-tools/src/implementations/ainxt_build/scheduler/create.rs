...
impl ainxt_tool_runtime::ToolOutput for SchedulerCreateOutput {}
...
impl ainxt_tool_runtime::Tool for SchedulerCreateTool {
    type Args = SchedulerCreateInput;
    type Output = SchedulerCreateOutput;

    fn id(&self) -> ainxt_tool_protocol::ToolId {
        ainxt_tool_protocol::ToolId::new(SCHEDULER_CREATE_TOOL_NAME).expect("valid tool id")
    }
    ...
    ) -> Result<SchedulerCreateOutput, ainxt_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn loop_schedule_instruction_holds_invariants() {
        ...
        // Raw request forwarded unchanged for the model to parse.
