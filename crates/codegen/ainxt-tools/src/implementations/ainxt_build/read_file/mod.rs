...
mod versions;
use crate::types::schema::AinxtIntegerSchema;
...
crate::register_resource!("ainxt_build", "ReadFile", ReadFileParams);
...
/// deltas reproduce the terminal `content` exactly).
...
/// `PlainText` / `Append`.
static READ_FILE_CAPABILITIES: LazyLock<ainxt_tool_protocol::ToolCapabilities> =
...
/// an empty window (harness-aligned; not a ainxt-line clamp).
...
impl ainxt_tool_runtime::Tool for ReadFileTool {
    type Args = ReadFileInput;
    type Output = ReadFileOutput;
    fn id(&self) -> ainxt_tool_protocol::ToolId {
        ainxt_tool_protocol::ToolId::new("read_file").expect("valid tool id")
    }
    fn description(
        &self,
    ...
    /// reproduces the card exactly; image/PDF/PPTX stay terminal-only.
    /// Gated by `WorkspaceViewerContext::stream_tool_progress`.
    async fn execute(
        &self,
        ctx: ainxt_tool_runtime::ToolCallContext,
        input: ReadFileInput,
    ) -> ainxt_tool_runtime::ToolStream<ReadFileOutput> {
        ...
        let Some(spec) = admitted_spec else {
            ...
            return Box::pin(async_stream::stream! {
                yield ainxt_tool_runtime::ToolStreamItem::Terminal(this.run(ctx, input)
                . await);
            });
        };
        Box::pin(async_stream::stream! {
            ...
            ainxt_tool_runtime::stream_chunk(spec, & content[..window_end], window_end
            as u64, & mut last_total, false,) { yield
            ainxt_tool_runtime::ToolStreamItem::Progress(p); } window_start =
            window_end; } } yield
            ainxt_tool_runtime::ToolStreamItem::Terminal(Ok(output)); } Err(e) => yield
            ainxt_tool_runtime::ToolStreamItem::Terminal(Err(e)), }
        })
    }
...
impl ReadFileTool {
    ...
    async fn read_with_streamability(
        ctx: &ainxt_tool_runtime::ToolCallContext,
        input: ReadFileInput,
    ) -> Result<(ReadFileOutput, bool), ainxt_tool_runtime::ToolError> {
        let resources = crate::types::tool_metadata::shared_resources(ctx)?;
        let cwd_override = ctx
            .extensions
            .get::<ainxt_tool_runtime::Cwd>()
            .map(|c| c.0.clone());
...
mod tests {
    ...
    async fn read_file_basic() {
        ...
        let result = ainxt_tool_runtime::Tool::run(&tool, test_ctx(shared.clone()), input)
            .await
            .unwrap();
        match result {
    ...
    async fn legacy_read_file_not_found_returns_exact_historical_message() {
        ...
        let result = ainxt_tool_runtime::Tool::run(&tool, ctx, input)
            .await
            .unwrap();
        let expected = format!(
            "Failed to read file: {}",
    ...
    async fn current_read_file_not_found_returns_structured_not_found() {
        ...
        let result = ainxt_tool_runtime::Tool::run(&tool, test_ctx(resources.into_shared()), input)
            .await
            .unwrap();
        match result {
            ReadFileOutput::FileNotFound(msg) => {
...
";
    ...
    async fn skill_file_ignores_offset_and_limit() {
        let tmp = TempDir::new().unwrap();
        let skill_dir = tmp.path().join(".ainxt/skills/commit");
        ...
        let resources = test_resources(tmp.path());
        let input = ReadFileInput {
            path: ".ainxt/skills/commit/SKILL.md".to_string(),
            offset: Some(3),
            limit: Some(1),
            pages: None,
            format: None,
    ...
    async fn run_read_file_on(filename: &str, content: &[u8]) -> ReadFileOutput {
        ...
        ainxt_tool_runtime::Tool::run(&ReadFileTool, test_ctx(resources.into_shared()), input)
            .await
            .unwrap()
    }
    #[tokio::test]
    ...
    async fn execute_collect(
        ...
        let mut stream = ainxt_tool_runtime::Tool::execute(&ReadFileTool, ctx, input).await;
    ...
    /// Harness equivalence: offset=-1 on a file with no trailing `\n` resolves to the
    /// phantom field only (start past any `split_inclusive` line), so ainxt
