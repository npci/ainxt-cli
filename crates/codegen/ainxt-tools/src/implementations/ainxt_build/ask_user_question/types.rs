...
//!
//! - **`ainxt-tools`** — tool blocks on a oneshot, formats the result.
//! - **`ainxt-shell`** — coordinator receives requests over mpsc, calls the
//!   client via ACP `ext_method`, sends results back over the oneshot.
//! - **`ainxt-pager`** — handles the `ExtMethod`, renders UI, returns a
//!   typed response.
//!
//! All three crates import these types from `ainxt-tools`.
...
/// - `preview`: unchanged `Option.preview` of the selected option (single-select only).
...
register_resource!("ainxt_build", "UserQuestionSender", UserQuestionSender);
