...
use ainxt_tool_types::{TaskOutputOutput, TaskOutputToolInput};
...
impl ainxt_tool_runtime::Tool for GetTerminalCommandOutputTool {
    type Args = TaskOutputToolInput;
    type Output = TaskOutputOutput;

    fn id(&self) -> ainxt_tool_protocol::ToolId {
        ainxt_tool_protocol::ToolId::new("get_terminal_command_output").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        ainxt_tool_runtime::Tool::run(&TaskOutputTool, ctx, input).await
    }
}
...
mod tests {
    use super::*;
    use crate::implementations::ainxt_build::task_output::test_helpers::{
