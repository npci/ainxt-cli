...
use crate::implementations::ainxt_build::task::TaskTool;
use crate::implementations::ainxt_build::task::backend::SubagentBackendResource;
use crate::implementations::ainxt_build::task::types::{SubagentSnapshot, SubagentSnapshotStatus};
use crate::implementations::ainxt_build_concise::BashConciseTool;
...
/// `TaskOutput` cap. Env override: `AINXT_MAX_WAIT_BLOCK_MS`.
const MAX_WAIT_BLOCK: Duration = Duration::from_secs(600);

fn max_wait_block() -> Duration {
    std::env::var("AINXT_MAX_WAIT_BLOCK_MS")
        .ok()
...
pub(crate) fn background_bash_requires_exprs() -> Vec<Expr<ToolRequirement>> {
    ...
    let ainxt_build_bash = Expr::Value(ToolRequirement::Tool {
        namespace: ToolMetadata::tool_namespace(&BashTool).to_string(),
        id: ainxt_tool_runtime::Tool::id(&BashTool).as_str().to_string(),
    ...
    let ainxt_build_concise_bash = Expr::Value(ToolRequirement::Tool {
        namespace: ToolMetadata::tool_namespace(&BashConciseTool).to_string(),
        id: ainxt_tool_runtime::Tool::id(&BashConciseTool)
            .as_str()
            .to_string(),
        if_params: Some(Expr::Value(ToolParamsRequirement {
            key: "enabled_background".to_string(),
    ...
    let opencode_bash = Expr::Value(ToolRequirement::Tool {
        ...
        id: ainxt_tool_runtime::Tool::id(&OpenCodeBashTool)
            .as_str()
            .to_string(),
        if_params: None,
    });
    vec![ainxt_build_bash, ainxt_build_concise_bash, opencode_bash]
}
...
pub(crate) fn task_output_requires_expr() -> Expr<ToolRequirement> {
    ...
    let task_tool = Expr::Value(ToolRequirement::Tool {
        ...
        id: ainxt_tool_runtime::Tool::id(&TaskTool).as_str().to_string(),
        if_params: None,
    });
...
impl TaskOutputTool {
    async fn run_single_task(
    ...
    ) -> Result<TaskOutputOutput, ainxt_tool_runtime::ToolError> {
        let contract_version = ctx
            .extensions
            .get::<ainxt_tool_runtime::BehaviorVersion>()
            .map(|v| v.0.clone());
        ...
        let waits = ainxt_tool_types::task_output_waits(timeout_ms);
...
pub(crate) use ainxt_tool_types::MAX_MULTI_WAIT_IDS;
...
fn format_subagent_snapshot(snap: &SubagentSnapshot) -> TaskOutputOutput {
    ...
    match &snap.status {
        ...
        SubagentSnapshotStatus::Completed {
            ...
            output.push_str("\n\n");
            output.push_str(&ainxt_tool_types::format_resume_footer(
                &snap.subagent_id,
                &snap.subagent_type,
...
impl crate::types::tool_metadata::ToolMetadata for TaskOutputTool {
    ...
    fn description_template(&self) -> &str {
        ...
        // fallback matches the default ainxt-build toolset.
        static DESC: std::sync::LazyLock<String> = std::sync::LazyLock::new(|| {
            ainxt_tool_types::build_task_output_description(&ainxt_tool_types::TaskOutputToolNaming {
...
/// [`ainxt_tool_types::build_task_output_description`] builder so the CLI and
...
impl ainxt_tool_runtime::Tool for TaskOutputTool {
    type Args = TaskOutputToolInput;
    type Output = TaskOutputOutput;

    fn id(&self) -> ainxt_tool_protocol::ToolId {
        ainxt_tool_protocol::ToolId::new("get_task_output").expect("valid tool id")
    }
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        // The static fallback is the shared builder's default ainxt-build
    ...
    // ── Legacy message equivalence fixture tests ────────────────────────
    //
    ...
    async fn multi_task_ids_poll_returns_multi_result_mode_poll() {
        let resources = resources_with_terminal(None);
        let tool = TaskOutputTool;
        let out = ainxt_tool_runtime::Tool::run(
            &tool,
            test_ctx(resources.into_shared()),
            TaskOutputToolInput {
    ...
    fn resources_with_backend_query() -> (
        ...
        tokio::sync::mpsc::UnboundedReceiver<
            crate::implementations::ainxt_build::task::types::SubagentEvent,
        >,
    ) {
        use crate::implementations::ainxt_build::task::backend::{
            ChannelBackend, SubagentBackendResource,
        };
        use crate::implementations::ainxt_build::task::types::SubagentEvent;
    ...
    fn unwrap_query(
        event: crate::implementations::ainxt_build::task::types::SubagentEvent,
    ) -> crate::implementations::ainxt_build::task::types::SubagentQueryRequest {
        match event {
            crate::implementations::ainxt_build::task::types::SubagentEvent::Query(q) => q,
