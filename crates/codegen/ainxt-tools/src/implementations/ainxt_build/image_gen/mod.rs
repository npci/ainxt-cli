//! `image_gen` tool — generates images via the ainxt ainxt image API and saves
...
/// Default ainxt image model for `image_gen`. Used unless an explicit
/// `model_override` is supplied via `ImageGenConfig::Enabled`.
const AINXT_IMAGINE_MODEL: &str = "ainxt-image-quality";
// Some ainxt image models (e.g. `ainxt-image`, selectable via `model_override`)
...
/// SuperAinxt upsell modal instead; this covers the natural-language path.
pub(crate) const TIER_RESTRICTED_UPSELL: &str = "Image generation is not enabled for this account. Let the user know their plan does not currently include image or video generation. Do not retry this tool.";

/// HTTP client for ainxt ainxt image API. Cloned per-request; shares `Arc` state.
#[derive(Clone)]
pub struct ImageGenClient {
    http: reqwest::Client,
    base_url: String,
    /// ainxt image model slug used by `generate()`. Selected at construction
    /// from `ImageGenConfig::model_override` (falling back to
    /// [`AINXT_IMAGINE_MODEL`]). `image_edit` uses its own model and is
    /// unaffected.
    model: String,
    writer: super::storage::SessionFileWriter,
    api_key_provider: Option<SharedApiKeyProvider>,
    /// Optional 401-attribution hook. Hosts wire this so a 401 from the
    /// ainxt image API emits an `auth_401_attribution` event with
    ...
    /// HTTP call and return the SuperAinxt upsell prose instead. See
...
impl ImageGenClient {
    pub fn new(
        ...
        let model = model_override
            ...
            .unwrap_or_else(|| AINXT_IMAGINE_MODEL.to_owned());
    ...
    /// ainxt image server-side. `image_gen` / `image_edit` use this to short-circuit
    /// with the SuperAinxt upsell instead of issuing a doomed request.
    ...
    pub async fn generate(
        ...
        if !status.is_success() {
            ...
            tracing::warn!(http_status = %status, "ainxt image API error: {truncated}");
        ...
        let resp_json: ImageGenResponse = serde_json::from_str(&body).map_err(|e| {
            ...
            tracing::warn!("ainxt image API returned unparseable body: {preview}");
            ainxt_tool_runtime::ToolError::invalid_arguments(format!(
...
pub enum ImageGenConfig {
    ...
    Enabled {
        ...
        /// Optional ainxt image model override for `image_gen`. When `Some(non-empty)`,
        /// `image_gen` calls that model instead of the default quality model
        /// ([`AINXT_IMAGINE_MODEL`]). Driven by the remote
        ...
        /// SuperAinxt upsell prose instead of a doomed request. Set by the
...
impl ImageGenConfig {
    ...
    /// default quality model ([`AINXT_IMAGINE_MODEL`]) is used.
...
impl ainxt_tool_runtime::Tool for ImageGenTool {
    type Args = ImageGenInput;
    type Output = ToolOutput;

    fn id(&self) -> ainxt_tool_protocol::ToolId {
        ainxt_tool_protocol::ToolId::new("image_gen").expect("valid tool id")
    }
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        assert_eq!(ainxt_tool_runtime::Tool::id(&tool).as_str(), "image_gen");
        assert!(
    ...
    fn per_tool_gates_are_independent() {
        let cfg = ImageGenConfig::Enabled {
            ...
            image_edit_enabled: true,
            model_override: Some("ainxt-image".into()),
            tier_restricted: false,
        };
        assert!(cfg.has_credentials());
        assert!(!cfg.image_gen_enabled());
        assert!(cfg.image_edit_enabled());
        assert_eq!(cfg.model_override(), Some("ainxt-image"));
    ...
    fn client_selects_model_from_override() {
        ...
        assert_eq!(
            ImageGenClient::new(&mk(Some("ainxt-image")), None)
                .unwrap()
                .model,
            "ainxt-image"
        );
    ...
    async fn tier_restricted_short_circuits_with_upsell() {
        // A free / X Basic user's image_gen call returns the SuperAinxt upsell
        ...
        match result {
            ToolOutput::Text(t) => {
                // The upsell text was de-branded for the OSS release (no
                // "SuperAinxt" name or referrer link); assert on the
                // TIER_RESTRICTED_UPSELL constant it actually returns.
