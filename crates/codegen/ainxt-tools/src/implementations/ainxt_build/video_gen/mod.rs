...
//! the ainxt Video Generation API and save them to the local filesystem so the
...
const AINXT_VIDEO_BASE_MODEL: &str = "ainxt-video";
const AINXT_VIDEO_QUALITY_MODEL: &str = "ainxt-video-preview";
...
const DEFAULT_ZDR_VIDEO_KEY_PREFIX: &str = "ainxt-videos/";
...
pub use ainxt_tools_api::slash_commands::{
...
impl S3AccessCredentials {
    ...
    fn to_static(&self) -> ainxt_file_utils::s3::S3StaticCredentials {
        ainxt_file_utils::s3::S3StaticCredentials {
...
/// HTTP client for ainxt Video Generation API. Cloned per-request; shares `Arc` state.
#[derive(Clone)]
pub struct VideoGenClient {
    ...
    /// When `true`, the user is on a tier the ainxt image server zero-limits
    /// (free / X Basic). The video tools short-circuit before any HTTP call
    /// and return the SuperAinxt upsell prose. See [`VideoGenClient::is_tier_restricted`].
...
impl VideoGenClient {
    pub fn new(
        ...
        api_key_provider: Option<SharedApiKeyProvider>,
    ) -> Result<Self, ainxt_tool_runtime::ToolError> {
        let VideoGenConfig::Enabled {
            ...
            return Err(ainxt_tool_runtime::ToolError::invalid_arguments(
        ...
        headers.insert(
            ...
            HeaderValue::from_str(&format!("Bearer {api_key}")).map_err(|e| {
                ainxt_tool_runtime::ToolError::invalid_arguments(format!(
        ...
        extra_headers.into_iter().try_for_each(|(key, value)| {
            ...
            Ok::<(), ainxt_tool_runtime::ToolError>(())
        })?;
    ...
    /// ainxt image server-side. The video tools use this to short-circuit with the
    /// SuperAinxt upsell instead of issuing a doomed request.
    ...
    pub async fn generate_with_images(
    ...
    ) -> Result<VideoOutcome, ainxt_tool_runtime::ToolError> {
        ...
        if !status.is_success() {
            ...
            return Err(ainxt_tool_runtime::ToolError::new(
                ainxt_tool_runtime::ToolErrorKind::Custom,
        ...
        tracing::info!(job_id = %request_id, "Video generation started, polling for completion");
                ...
                "done" => {
                    ...
                    return match presigned {
                        ...
                        None if video_url.is_empty() => {
                            Err(ainxt_tool_runtime::ToolError::invalid_arguments(
    ...
    async fn download_video(&self, url: &str) -> Result<Vec<u8>, ainxt_tool_runtime::ToolError> {
        let response = self.download_http.get(url).send().await.map_err(|e| {
            ainxt_tool_runtime::ToolError::invalid_arguments(format!("Failed to download video: {e}"))
        })?;
    ...
    async fn finish_zdr_video(
        ...
        let config = self.zdr_video_output_s3.as_ref().ok_or_else(|| {
            ainxt_tool_runtime::ToolError::invalid_arguments(
    ...
    async fn presign_zdr_output_urls(
    ...
    ) -> Result<ZdrPresignedUrls, ainxt_tool_runtime::ToolError> {
        ...
        let upload_url = ainxt_file_utils::s3::presign_put_url(
            &config.region,
            endpoint,
    ...
    async fn presign_and_download(
        ...
        urls: &ZdrPresignedUrls,
        request_id: &str,
    ) -> Result<Vec<u8>, ainxt_tool_runtime::ToolError> {
        let get_url = self
    ...
    async fn presign_zdr_get_url(
        ...
        let url = ainxt_file_utils::s3::presign_get_url(
            &config.region,
            endpoint,
            &creds.to_static(),
            &config.bucket,
...
pub enum VideoGenConfig {
    ...
    Enabled {
        ...
        /// `true` when the user is on a tier the ainxt image server zero-limits
        /// (free / X Basic). The video tools stay advertised but short-circuit
        /// at call time with the SuperAinxt upsell prose. Set by the host from
...
/// the deliberate `/imagine-video` slash command shows the SuperAinxt upsell
/// modal instead.
pub(crate) const TIER_RESTRICTED_UPSELL: &str = "Video generation is not enabled for this account. Let the user know their plan does not currently include image or video generation. Do not retry this tool.";
...
async fn resolve_image_reference(value: &str) -> Result<String, ainxt_tool_runtime::ToolError> {
    ...
    if value.starts_with("data:image/") {
        let comma = value.find(',').ok_or_else(|| {
            ainxt_tool_runtime::ToolError::invalid_arguments("malformed data URL in image reference")
        })?;
    ...
    let (_w, _h, mime) =
        crate::util::image_validate::validate_image_bytes(&raw_bytes).map_err(|e| {
            ainxt_tool_runtime::ToolError::invalid_arguments(format!("invalid image reference: {e}"))
        })?;
...
fn validate_one_of(
    ...
    Err(ainxt_tool_runtime::ToolError::invalid_arguments(format!(
        "`{field}` must be one of: {}. Got {value}.",
        allowed.join(", ")
    )))
}

fn validate_imagine_duration(duration: Option<u32>) -> Result<(), ainxt_tool_runtime::ToolError> {
...
) -> Result<(VideoGenClient, std::path::PathBuf), ainxt_tool_runtime::ToolError> {
...
async fn save_video_bytes(
    ...
    let absolute_path = client
        ...
        .await
        .map_err(|e| ainxt_tool_runtime::ToolError::invalid_arguments(e.to_string()))?;
...
) -> Result<MediaGenOutput, ainxt_tool_runtime::ToolError> {
    match outcome {
...
impl ainxt_tool_runtime::Tool for ImageToVideoTool {
    type Args = ImageToVideoInput;
    type Output = ToolOutput;

    fn id(&self) -> ainxt_tool_protocol::ToolId {
        ainxt_tool_protocol::ToolId::new(IMAGE_TO_VIDEO_TOOL_NAME).expect("valid tool id")
    }
    ...
    async fn run(
        ...
        // Free / X Basic users are zero-limited on ainxt image server-side; return
...
impl ainxt_tool_runtime::Tool for ReferenceToVideoTool {
    type Args = ReferenceToVideoInput;
    type Output = ToolOutput;

    fn id(&self) -> ainxt_tool_protocol::ToolId {
        ainxt_tool_protocol::ToolId::new(REFERENCE_TO_VIDEO_TOOL_NAME).expect("valid tool id")
    }
...
mod tests {
    ...
    fn image_and_reference_payload_fields_are_serialized() {
        let payload = GenerateVideoPayload {
            model: AINXT_VIDEO_QUALITY_MODEL,
            prompt: "animate",
    ...
    async fn image_to_video_rejects_bad_duration() {
        ...
        let err = ainxt_tool_runtime::Tool::run(
            &tool,
