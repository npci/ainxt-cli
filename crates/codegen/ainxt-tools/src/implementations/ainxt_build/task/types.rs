...
//! (in `ainxt-tools`) and the subagent coordinator (in `ainxt-shell`).
//!
//! ## Resource types
//!
...
use ainxt_tool_types::{SubagentCapabilityMode, SubagentIsolationMode, WaitMode};
...
pub struct SubagentRuntimeOverrides {
    ...
    /// `"ainxt-build-plan"`) whose `AgentDefinition` decides the child's harness
...
/// Re-export of [`ainxt_tool_types::is_not_sentinel`] for existing call sites.
pub use ainxt_tool_types::is_not_sentinel;
...
/// defense-in-depth check in `ainxt-shell`'s subagent coordinator.
...
fn is_background_capable_bash_tool(tc: &crate::registry::types::ToolConfig) -> bool {
    match tc.id.as_str() {
        "AinxtBuild:run_terminal_cmd" | "AinxtBuildConcise:run_terminal_cmd" => tc
            .params
            .as_ref()
...
///
/// Matches [`SubagentValidateTypeOutcome`] but, on success, additionally
...
/// the cross-crate drain loop in `ainxt-shell` relies on
...
register_resource!("ainxt_build", "SubagentEventSender", SubagentEventSender);
...
pub struct MonitorEventNotification {
    ...
    /// events inside another session's turn. `None` for legacy / non-ainxt-build
...
impl MonitorEventNotification {
    ...
    /// is `my_owner`. Matches `task_owned_by_session`: an event surfaces only
...
pub type MonitorEventBuffer = ainxt_interjection_core::EventQueue<MonitorEventNotification>;

register_resource!("ainxt_build", "MonitorEventBuffer", MonitorEventBuffer);
...
/// ainxt-shell produces it, the channel protocol carries it, and the
/// compaction pipeline in ainxt-chat-state (via `RunningSubagentSummary`)
...
register_resource!("ainxt_build", "SubagentDepthCounter", SubagentDepthCounter);
...
register_resource!("ainxt_build", "TaskModelValidator", TaskModelValidator);
...
register_resource!("ainxt_build", "SessionIdResource", SessionIdResource);

/// Carries the current parent prompt/turn ID for TaskTool subagent scoping.
///
/// Set by ainxt-shell immediately before a prompt turn begins executing so
...
/// True while a `/goal` loop is active. Set by ainxt-shell at turn start.
...
register_resource!("ainxt_build", "GoalLoopActive", GoalLoopActive);
...
mod tests {
    ...
    fn read_only_filter_prunes_orphaned_background_task_tools() {
        let mut config = ToolServerConfig {
            tools: vec![
                tc("AinxtBuild:run_terminal_cmd", ToolKind::Execute),
                tc("AinxtBuild:read_file", ToolKind::Read),
                tc("AinxtBuild:list_dir", ToolKind::List),
                tc("AinxtBuild:grep", ToolKind::Search),
                tc("AinxtBuild:kill_task", ToolKind::KillTaskAction),
                tc("AinxtBuild:get_task_output", ToolKind::BackgroundTaskAction),
            ],
    ...
    fn read_only_filter_keeps_background_task_tools_when_task_tool_remains() {
        let mut config = ToolServerConfig {
            tools: vec![
                ...
                tc("AinxtBuild:kill_task", ToolKind::KillTaskAction),
                tc("AinxtBuild:get_task_output", ToolKind::BackgroundTaskAction),
                tc("AinxtBuild:task", ToolKind::Task),
            ],
            behavior_preset: None,
        };
    ...
    fn read_write_filter_keeps_background_capable_bash_when_explicitly_enabled() {
        let mut bash = tc("AinxtBuild:run_terminal_cmd", ToolKind::Execute);
        bash.params = Some(
