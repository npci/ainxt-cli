//! `image_edit` tool — edits or transforms images via the ainxt ainxt image
...
use crate::implementations::ainxt_build::image_gen::{ImageGenClient, ImageGenResponse};
...
/// Size/dimension limits for reference images sent to the ainxt image API.
...
/// Compress a reference image to fit within ainxt image API limits.
///
/// Returns `(bytes, mime)`. Small JPEG/PNG inputs pass through unchanged.
fn compress_reference(
    raw_bytes: Vec<u8>,
) -> Result<(Vec<u8>, &'static str), ainxt_tool_runtime::ToolError> {
    ...
    let img = ImageReader::new(Cursor::new(&raw_bytes))
        ...
        .ok_or_else(|| {
            ainxt_tool_runtime::ToolError::invalid_arguments("failed to decode image reference")
        })?;

    let params = ReEncodeParams {
        max_bytes: MAX_REF_RAW_BYTES,
        max_side_px: MAX_REF_DIMENSION,
        // ainxt image backend limits are side-based; no pixel-area cap applies.
        max_pixels: u64::MAX,
    ...
    let (buf, _w, _h, mime) = re_encode_under_limit(&img, &params).map_err(|e| {
        ainxt_tool_runtime::ToolError::invalid_arguments(format!(
            "could not compress image reference small enough for ainxt image API: {e}"
        ))
    })?;
...
/// into a compressed data URL for the ainxt image API.
async fn resolve_to_data_url(value: &str) -> Result<String, ainxt_tool_runtime::ToolError> {
...
impl ainxt_tool_runtime::Tool for ImageEditTool {
    type Args = ImageEditInput;
    type Output = ToolOutput;

    fn id(&self) -> ainxt_tool_protocol::ToolId {
        ainxt_tool_protocol::ToolId::new("image_edit").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        if !status.is_success() {
            ...
            tracing::warn!(http_status = %status, "ainxt image edit API error: {truncated}");
        ...
        let resp_json: ImageGenResponse = serde_json::from_str(&body).map_err(|e| {
            ...
            tracing::warn!("ainxt image edit API returned unparseable body: {preview}");
            ainxt_tool_runtime::ToolError::invalid_arguments(format!(
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        assert_eq!(ainxt_tool_runtime::Tool::id(&tool).as_str(), "image_edit");
