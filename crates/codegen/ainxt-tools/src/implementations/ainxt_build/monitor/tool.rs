...
impl ainxt_tool_runtime::Tool for MonitorTool {
    type Args = MonitorInput;
    type Output = MonitorOutput;

    fn id(&self) -> ainxt_tool_protocol::ToolId {
        ainxt_tool_protocol::ToolId::new("monitor").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: MonitorInput,
    ) -> Result<MonitorOutput, ainxt_tool_runtime::ToolError> {
...
mod tests {
    ...
    async fn persistent_monitor_released_when_session_drops_backend() {
        ...
        // Session-owned terminal backend (matches multi-session hosts that
