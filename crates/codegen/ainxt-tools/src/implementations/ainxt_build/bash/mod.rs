...
use regex::Regex;
use ainxt_config::shell::AmpersandSemantics;
...
static BASH_CAPABILITIES: LazyLock<ainxt_tool_protocol::ToolCapabilities> =
    LazyLock::new(|| ainxt_tool_protocol::ToolCapabilities {
        is_read_only: false,
        tool_scope: Some(ainxt_tool_protocol::ToolScope::Write),
        streaming: Some(ainxt_tool_protocol::StreamingSpec {
...
pub struct BashParams {
    ...
    /// ainxt-build opts up to 10h via ainxt-shell's `BashToolConfig`.
    ///
    /// When set:
...
impl crate::types::resources::ResourceType for BashParams {
    const ID: &'static str = "ainxt_build.Bash";
...
impl ainxt_tool_runtime::ToolOutput for BashToolOutput {
    fn chat_completion_output(&self) -> Option<ainxt_tool_runtime::ToolChatCompletionResponse> {
        match self {
            Self::Foreground(bash) => ainxt_tool_runtime::ToolOutput::chat_completion_output(bash),
...
// particular production ainxt-build sets it to 10h in ainxt-shell's
...
/// [`ainxt_config::shell::ampersand_semantics`] reports POSIX `&` semantics
/// (Unix + Git Bash).
...
/// should use `background: true`. Env override: `AINXT_MAX_FOREGROUND_BLOCK_MS`.
const MAX_FOREGROUND_BLOCK: Duration = Duration::from_secs(300); // 5 minutes

fn max_foreground_block() -> Duration {
    std::env::var("AINXT_MAX_FOREGROUND_BLOCK_MS")
        .ok()
...
// ───────────────────────────────────────────────────────────────────────────
// Bare `echo "<msg>"` detection (for statistics + hints in ainxt_build bash)
// ───────────────────────────────────────────────────────────────────────────
...
/// communication. We want statistics on this pattern for the ainxt_build
...
impl BashTool {
    ...
    ///   the terminal backend default applies (`AINXT_FOREGROUND_BLOCK_BUDGET_MS`
    ...
    /// still honors `AINXT_FOREGROUND_BLOCK_BUDGET_MS` via `None` on the request.
    ///
...
${%- endif %}"#
    ...
    fn get_prefixed_command(cmd_prefix: &Option<String>, command: &str) -> String {
        match cmd_prefix {
            Some(prefix) => {
                let sep = ainxt_config::shell::chain_separator();
...
impl ainxt_tool_runtime::Tool for BashTool {
    type Args = BashToolInput;
    type Output = BashToolOutput;

    fn id(&self) -> ainxt_tool_protocol::ToolId {
        ainxt_tool_protocol::ToolId::new("run_terminal_cmd").expect("valid tool id")
    }
    ...
    async fn execute(
        &self,
        mut ctx: ainxt_tool_runtime::ToolCallContext,
        input: BashToolInput,
    ) -> ainxt_tool_runtime::ToolStream<BashToolOutput> {
        ...
        if input.is_background {
            ...
            return Box::pin(async_stream::stream! {
                yield ainxt_tool_runtime::ToolStreamItem::Terminal(this.run(ctx, input).await);
            });
        }
        ...
        // emission that would be dropped anyway. Matches the
        ...
        let Some(spec) = BASH_CAPABILITIES.streaming.as_ref() else {
            return Box::pin(async_stream::stream! {
                yield ainxt_tool_runtime::ToolStreamItem::Terminal(Err(
            ...
            loop {
                tokio::select! {
                    ...
                    maybe = rx.recv() => {
                        match maybe {
                            Some(ToolNotification::BashOutputChunk(chunk)) => {
                                ...
                                {
                                    yield ainxt_tool_runtime::ToolStreamItem::Progress(p);
                                }
                            }
                            Some(notif) => {
                    ...
                    result = &mut run_fut => {
                        ...
                        yield ainxt_tool_runtime::ToolStreamItem::Terminal(result);
                        break;
                    }
                }
            }
    ...
    async fn run(
        ...
        input: BashToolInput,
    ) -> Result<BashToolOutput, ainxt_tool_runtime::ToolError> {
        ...
        let ampersand = ainxt_config::shell::ampersand_semantics();
        if let Some(violation) = should_reject_background_op(
            ...
            return Err(ainxt_tool_runtime::ToolError::invalid_arguments(message));
        }
        ...
        if input.is_background {
            ...
            let request = TerminalRunRequest {
                ...
                // existing ainxt_build callers that never opted in are
                // unaffected.
            ...
            // We tag it for statistics (ainxt_build backend) and can surface an educational
...
mod tests {
    ...
    fn read_chunk_progress(p: &ainxt_tool_runtime::ToolProgress) -> (String, usize, bool, bool) {
        match p {
            ainxt_tool_runtime::ToolProgress::Custom { subkind, payload } => {
    ...
    fn bash_output_chunk_progress_caps_oversized_delta() {
        ...
        while last < total {
            let p = bash_output_chunk_progress(spec, &chunk, &mut last).unwrap();
            let ainxt_tool_runtime::ToolProgress::Custom { payload, .. } = p else {
    ...
    async fn bash_streaming_progress_suppressed_when_gate_off() {
        ...
        let tool = BashTool;
        let mut stream = ainxt_tool_runtime::Tool::execute(
            &tool,
            ctx,
        ...
        let mut terminal: Option<Result<BashToolOutput, ainxt_tool_runtime::ToolError>> = None;
        while let Some(item) = stream.next().await {
            match item {
                ainxt_tool_runtime::ToolStreamItem::Progress(_) => {
                    progress += 1;
                }
    ...
    async fn background_injects_python_unbuffered() {
        ...
        let _ = ainxt_tool_runtime::Tool::run(
            &tool,
            test_ctx(resources.into_shared()),
    ...
    async fn errors_when_terminal_not_in_resources() {
        ...
        let result =
            ainxt_tool_runtime::Tool::run(&tool, test_ctx(resources.into_shared()), make_input("ls"))
                .await;
        assert!(result.is_err());
        assert!(
            result
    ...
    fn foreground_chat_completion_emits_code_execution_result() {
        let resp = ainxt_tool_runtime::ToolOutput::chat_completion_output(
    ...
    fn background_chat_completion_is_none() {
        ...
        assert!(ainxt_tool_runtime::ToolOutput::chat_completion_output(&out).is_none());
    }
    ...
    mod resolve_effective_timeout_tests {
        ...
        fn foreground_positive_clamped_to_default_max() {
            ...
            // ainxt-shell opts up to 10h via max_timeout_secs. Background unaffected.
    ...
    mod gate_decision {
        ...
        /// otherwise it delegates unchanged to `detect_background_op_violation` (whose
    ...
    mod versioned_background_check {
        ...
        ) -> ainxt_tool_runtime::ToolCallContext {
            let mut ctx =
                ainxt_tool_runtime::ToolCallContext::new(ainxt_tool_protocol::ToolCallId::new_v7());
            ctx.extensions.insert(resources);
            ctx.extensions.insert(ainxt_tool_runtime::BehaviorVersion(
