...
//! `LspTool` (Tool trait impl) under the `AinxtBuild` namespace.
...
impl ainxt_tool_runtime::ToolOutput for LspToolOutput {}
...
impl ainxt_tool_runtime::Tool for LspTool {
    type Args = LspToolInput;
    type Output = LspToolOutput;

    fn id(&self) -> ainxt_tool_protocol::ToolId {
        ainxt_tool_protocol::ToolId::new("lsp").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: LspToolInput,
    ) -> Result<LspToolOutput, ainxt_tool_runtime::ToolError> {
            ...
            handle = res
                ...
                .ok_or_else(|| {
                    ainxt_tool_runtime::ToolError::custom(
                        "process_manager",
                        "LSP tool is unavailable. Configure ~/.ainxt/lsp.json or <cwd>/.ainxt/lsp.json and ensure the language server can start.",
                    )
                })?
                .clone();
        }
