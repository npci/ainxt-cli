...
register_resource!("ainxt_build", "SearchReplace", SearchReplaceParams);
...
) -> Result<SearchReplaceOutput, ainxt_tool_runtime::ToolError> {
            ...
            let output = match e.io_error_kind() {
                ...
                _ => {
                    return Err(ainxt_tool_runtime::ToolError::execution(
                        ainxt_tool_protocol::ToolId::new("search_replace").expect("valid"),
                        e.to_string(),
                    ));
                }
            };
...
impl ainxt_tool_runtime::Tool for SearchReplaceTool {
    type Args = SearchReplaceInput;
    type Output = SearchReplaceOutput;
    fn id(&self) -> ainxt_tool_protocol::ToolId {
        ainxt_tool_protocol::ToolId::new("search_replace").expect("valid tool id")
    }
    fn description(
        &self,
...
mod tests {
    ...
    async fn consecutive_edits_succeed_without_prior_read() {
        ...
        let result1 = ainxt_tool_runtime::Tool::run(
            &tool,
        ...
        let result2 = ainxt_tool_runtime::Tool::run(
            &tool,
            test_ctx(shared),
...
";
    ...
    /// The terminal fallback names only installed script tools (matches the
