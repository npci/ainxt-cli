...
crate::register_resource!("ainxt_build", "ListDir", ListDirParams);
...
impl ainxt_tool_runtime::Tool for ListDirTool {
    type Args = ListDirInput;
    type Output = ListDirOutput;
    fn id(&self) -> ainxt_tool_protocol::ToolId {
        ainxt_tool_protocol::ToolId::new("list_dir").expect("valid tool id")
    }
    fn description(
        &self,
    ...
    async fn run(
        ...
        input: ListDirInput,
    ) -> Result<ListDirOutput, ainxt_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn budget_collapses_last_oversized_sibling() {
        ...
        {
            use std::os::unix::fs::PermissionsExt;
            fs::set_permissions(&small, fs::Permissions::from_mode(0o700)).unwrap();
        }
    ...
    fn budget_skips_multiple_oversized_siblings_interleaved() {
        ...
        {
            use std::os::unix::fs::PermissionsExt;
            fs::set_permissions(&aaa, fs::Permissions::from_mode(0o700)).unwrap();
        }
        fs::create_dir(&ccc).unwrap();
        #[cfg(unix)]
        {
            use std::os::unix::fs::PermissionsExt;
            fs::set_permissions(&ccc, fs::Permissions::from_mode(0o700)).unwrap();
        }
        for i in 0..50 {
        ...
        {
            use std::os::unix::fs::PermissionsExt;
            fs::set_permissions(&bbb, fs::Permissions::from_mode(0o700)).unwrap();
        }
        fs::create_dir(&ddd).unwrap();
        #[cfg(unix)]
        {
            use std::os::unix::fs::PermissionsExt;
            fs::set_permissions(&ddd, fs::Permissions::from_mode(0o700)).unwrap();
        }
    ...
    fn depth1_seed_survives_real_walk_truncation() {
        ...
        {
            use std::os::unix::fs::PermissionsExt;
            fs::set_permissions(&zzz, fs::Permissions::from_mode(0o700)).unwrap();
        }
        for i in 0..30 {
            File::create(aaa.join(format!("f{}.rs", i))).unwrap();
        }
    ...
    fn depth1_seed_via_build_tree_includes_all_siblings() {
        ...
        {
            use std::os::unix::fs::PermissionsExt;
            fs::set_permissions(&early, fs::Permissions::from_mode(0o700)).unwrap();
        }
        fs::create_dir(&late).unwrap();
        #[cfg(unix)]
        {
            use std::os::unix::fs::PermissionsExt;
            fs::set_permissions(&late, fs::Permissions::from_mode(0o700)).unwrap();
        }
        for i in 0..30 {
    ...
    fn tool_name_and_description() {
        ...
        let tool = ListDirTool;
        assert_eq!(ainxt_tool_runtime::Tool::id(&tool).as_str(), "list_dir");
