...
//!
//! Reuses the core file-reading logic from [`ainxt_build::read_file::run_read_file`]
...
use crate::implementations::ainxt_build::read_file::{ReadFileInput, run_read_file};
...
impl ainxt_tool_runtime::Tool for HashlineReadTool {
    type Args = ReadFileInput;
    type Output = ReadFileOutput;

    fn id(&self) -> ainxt_tool_protocol::ToolId {
        ainxt_tool_protocol::ToolId::new("hashline_read").expect("valid tool id")
    }
...
mod tests {
    ...
    use crate::implementations::ainxt_build::read_file::{MAX_LINES_READ, ReadFileTool};
    ...
    fn tool_metadata() {
        ...
        assert_eq!(ainxt_tool_runtime::Tool::id(&tool).as_str(), "hashline_read");
        assert_eq!(ToolMetadata::kind(&tool), ToolKind::Read);
        assert!(ainxt_tool_runtime::Tool::capabilities(&tool).is_read_only);
        assert!(matches!(
    ...
    /// file content independently of ainxt_build `read_file`).
    #[tokio::test]
    ...
    /// hashline reformats `fc.content` (which keeps original URIs unchanged);
