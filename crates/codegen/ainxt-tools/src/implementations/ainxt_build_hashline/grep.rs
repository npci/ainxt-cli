...
use crate::implementations::ainxt_build::grep::{GrepSearchInput, GrepTool, OutputMode};
...
impl ainxt_tool_runtime::Tool for HashlineGrepTool {
    type Args = GrepSearchInput;
    type Output = GrepSearchOutput;

    fn id(&self) -> ainxt_tool_protocol::ToolId {
        ainxt_tool_protocol::ToolId::new("hashline_grep").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        let mut rt_ctx = ainxt_tool_runtime::ToolCallContext::new(call_id);
        rt_ctx.extensions.insert(resources.clone());
        rt_ctx.extensions.insert(ainxt_tool_runtime::Cwd(cwd));
        let mut result = ainxt_tool_runtime::Tool::run(&grep, rt_ctx, input)
            .await
            .map_err(|e| {
                ainxt_tool_runtime::ToolError::execution(
                    ainxt_tool_protocol::ToolId::new("grep").expect("valid"),
                    e.to_string(),
                )
            })?;
...
mod tests {
    ...
    fn tool_metadata() {
        ...
        assert_eq!(ainxt_tool_runtime::Tool::id(&tool).as_str(), "hashline_grep");
