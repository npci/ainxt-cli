...
///
/// Serialized over the wire in the `ainxt.dev/task/kill` ext response
/// (`ainxt-shell::extensions::task::KillTaskResponse`) and deserialized
/// by clients (ainxt-pager), so it derives both serde directions.
...
///
/// Implemented by:
/// - `LocalTerminalBackend` (in ainxt-tools, spawns processes)
/// - `AcpTerminalBackend` (in ainxt-shell, calls ACP protocol)
#[async_trait::async_trait]
