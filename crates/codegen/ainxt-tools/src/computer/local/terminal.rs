...
impl LocalTerminalActor {
    ...
    async fn spawn_persistent_command(
        ...
        // Apply SHELL_ENV_OVERRIDES (TERM=dumb, NO_COLOR, AINXT_AGENT=1, etc.)
        ...
        crate::util::apply_ainxt_agent_marker(&mut cmd);
        ...
        #[cfg(target_os = "linux")]
        if ainxt_sandbox::should_restrict_child_network() {
            unsafe {
                cmd.pre_exec(|| ainxt_sandbox::child_net::install_child_network_filter());
            }
        }
    ...
    async fn poll_all_processes(&mut self) {
        ...
        for task_id in &size_exceeded {
            if let Some(process) = self.processes.get_mut(task_id) {
                ...
                // command, unlike the bg-only max-runtime sweep; matches OOM).
                let result = Ok(process.to_result());
                process.notify_waiters(result);
            }
        }
                ...
                && process.exit_status.is_none()
            {
                ...
                if process.kind == crate::computer::types::TaskKind::Monitor {
                    ...
                    let start_offset = std::fs::metadata(&pipeline_output_file)
                        ...
                        .unwrap_or(0);
                    tokio::spawn(async move {
                        crate::implementations::ainxt_build::monitor::tool::run_monitor_pipeline(
                            &pipeline_task_id,
...
fn spawn_shell_command(
    ...
    // Exec ticket: a token check, not a second policy decision.
    //
    // Authorization happens upstream at the permission chokepoint, which mints
    // a single-use, short-TTL ticket for the exact command it approved. This
    // site redeems it. The point is not to re-decide — it is that any *future*
    // code path which reaches the spawn without going through authorization
    // fails closed here instead of running unchecked. A single-chokepoint
    // design is only as good as the guarantee that nothing else reaches the
    // syscall, and that guarantee decays as a codebase grows.
    //
    // The ticket is keyed on the exact command string the permission layer
    // approved. If any call path ever rewrites the command between
    // authorization and spawn, redemption fails and a legitimate command is
    // refused — so observe mode deliberately still *checks* and warns, rather
    // than skipping the check entirely. Otherwise the first time a mismatch
    // surfaced would be the day a department flipped to `block`, which is the
    // worst possible moment to discover it.
    if let Some(pep) = ainxt_pep::global::active() {
        let engine = ainxt_policy::global::active();
        if engine.enforcement() != ainxt_policy_types::Enforcement::Off
            && pep.redeem_exec_ticket(command).is_none()
        {
            if engine.is_enforcing() {
                return Err(std::io::Error::other(
                    "refusing to spawn: this command was not authorized by the policy \
                     enforcement point (no valid exec ticket)",
                ));
            }
            tracing::warn!(
                "spawn without a valid exec ticket; under `block` this would be refused. \
                 If the command was approved, the string was rewritten between \
                 authorization and spawn and the ticket key no longer matches."
            );
        }
    }
    ...
    let mut cmd = {
        ...
        // The parent (ainxt) process retains network for the LLM API.
    ...
    let mut build_cmd = |with_breakaway: bool| {
        ...
        let inv = ainxt_config::shell::shell_command_argv(command);
        ...
        //     have JOB_OBJECT_LIMIT_BREAKAWAY_OK set — common when ainxt-agent
...
mod tests {
    ...
    async fn test_persistent_shell_env_var_persists() {
        ...
        let result = backend
            .run(make_request("export AINXT_PERSIST_TEST=hello123"))
            .await
            .unwrap();
        assert_eq!(result.exit_code, Some(0));

        let result = backend
            .run(make_request("echo $AINXT_PERSIST_TEST"))
            .await
            .unwrap();
        assert_eq!(result.exit_code, Some(0));
        assert_eq!(
    ...
    async fn test_persistent_shell_clears_gpg_tty() {
        ...
        req.env
            .insert("GPG_TTY".to_string(), "/ainxt-sentinel-tty".to_string());
    ...
    async fn cancelled_wait_does_not_suppress_auto_wake_on_completion() {
        ...
        //    oneshot receiver) long before the task exits. This matches a
