...
//! config helper `ainxt-shell::util::config::resolve_search_tools_enabled`
//! (requirements > env `AINXT_TOOLS_FIND_BFS` / `AINXT_TOOLS_GREP_UGREP` (+
//! `AINXT_FIND_BFS` / `AINXT_GREP_UGREP` aliases, `DISABLE_EMBEDDED_SEARCH_TOOLS`
...
//! (release builds, self-extracted to `~/.ainxt/vendor/<name>-<ver>-<target>`) →
//! `~/.ainxt/vendor/{name}` if a regular file → `which` on the agent `$PATH`.
...
//! function (which tags itself with a `__ainxt_shadow_{name}` marker) or a
...
/// BRE + ignore-files; recursive flags are no-ops on stdin (computer equivalence).
...
// Binaries embedded by build.rs when `AINXT_TOOLS_BUNDLE_{BFS,UGREP}_PATH` is set
// (release pipeline). Self-extracted to `~/.ainxt/vendor` on first use, mirroring
// the ripgrep bundling in `ainxt_build::grep::ripgrep`.
#[cfg(bundle_bfs)]
const BFS_BYTES: &[u8] = include_bytes!(concat!(
    env!("OUT_DIR"),
    "/bundle-bfs/bfs-",
    env!("AINXT_TOOLS_BFS_VER"),
    "-",
    env!("AINXT_TOOLS_BFS_TARGET"),
    ".bin"
));
...
const UGREP_BYTES: &[u8] = include_bytes!(concat!(
    env!("OUT_DIR"),
    "/bundle-ugrep/ugrep-",
    env!("AINXT_TOOLS_UGREP_VER"),
    "-",
    env!("AINXT_TOOLS_UGREP_TARGET"),
    ".bin"
));
...
/// OS binary again. Gated on the `__ainxt_shadow_{name}` marker that
...
fn restore_command(name: &str) -> String {
    format!(
        "if [ -n \"${{__ainxt_shadow_{name}-}}\" ]; then \
           unalias {name} 2>/dev/null || true; \
           unset -f {name} 2>/dev/null || true; \
           unset __ainxt_shadow_{name} 2>/dev/null || true; \
         fi"
    )
}
...
fn resolved_tools() -> &'static ResolvedTools {
    ...
    TOOLS.get_or_init(|| ResolvedTools {
        bfs: resolve_tool("bfs", "AINXT_TOOLS_BFS_PATH", bundled_bfs()),
        ugrep: resolve_tool("ugrep", "AINXT_TOOLS_UGREP_PATH", bundled_ugrep()),
    })
}

/// Write embedded `bytes` to `~/.ainxt/vendor/<versioned_name>` (chmod 755) on
...
fn extract_bundled(versioned_name: &str, bytes: &[u8]) -> std::io::Result<PathBuf> {
    ...
    let dir = crate::util::ainxt_home().join("vendor");
...
fn resolve_tool(bin_name: &str, env_override: &str, bundled: Option<PathBuf>) -> Option<PathBuf> {
    resolve_tool_from(
        ...
        bundled,
        crate::util::ainxt_home().join("vendor").join(bin_name),
        bin_name,
    )
}

/// Resolution order: explicit env path → bundled (self-extracted) →
/// `~/.ainxt/vendor/<bin>` → `which`. Env and vendor only require `is_file()` here
...
/// `__ainxt_shadow_{name}=1` marks this as a harness shadow so `restore_command`
...
fn shell_function(
    ...
    // `local __ainxt_bin` is re-resolved every call. The host hint is trusted
    ...
    // binary. `|| __ainxt_bin=''` keeps the lookup `set -e`-safe (a failed
    ...
    format!(
        "unalias {name} 2>/dev/null || true; \
         {name}() {{ \
           local __ainxt_bin={qpref}; \
           [ -x \"$__ainxt_bin\" ] || __ainxt_bin=$(command -v {bin_name} 2>/dev/null) || __ainxt_bin=''; \
           if [ -z \"$__ainxt_bin\" ]; then command {name} \"$@\"; return; fi; \
           if [[ -z ${{ZSH_VERSION-}} ]] && (( BASH_SUBSHELL > 0 )); then \
             exec -a {name} \"$__ainxt_bin\" {prepend}\"$@\"; \
           else \
             (exec -a {name} \"$__ainxt_bin\" {prepend}\"$@\"); \
           fi; \
         }}; \
         __ainxt_shadow_{name}=1"
    )
}
...
mod tests {
    ...
    fn shell_function_shape() {
        ...
        // Preferred path is the fast-path hint; the shadow execs `$__ainxt_bin`.
        assert!(fn_body.contains("local __ainxt_bin=/tmp/bfs"));
        assert!(fn_body.contains("exec -a find \"$__ainxt_bin\" \"$@\""));
        // Hint is trusted only when executable (`[ -x ]`, not `[ -f ]`), so a
        // non-exec hint falls through instead of hard-failing exec.
        assert!(fn_body.contains("[ -x \"$__ainxt_bin\" ]"));
        assert!(!fn_body.contains("[ -f \"$__ainxt_bin\" ]"));
        ...
        assert!(fn_body.contains("__ainxt_shadow_find=1"));
    ...
    fn shell_function_unresolved_uses_empty_hint() {
        ...
        assert!(fn_body.contains("local __ainxt_bin=''"));
        assert!(fn_body.contains("command -v bfs"));
        assert!(fn_body.contains("command find \"$@\""));
    }
    ...
    fn grep_prepends_ugrep_defaults() {
        ...
        assert!(fn_body.contains("local __ainxt_bin=/tmp/ugrep"));
        assert!(fn_body.contains("\"$__ainxt_bin\" -G --ignore-files --hidden -I"));
    ...
    fn bash_safe_quote_escapes_metacharacters() {
        ...
        assert!(body.contains("local __ainxt_bin='/tmp/evil$(id)'"), "{body}");
    ...
    fn restore_command_is_marker_gated() {
        ...
        assert!(r.contains("if [ -n \"${__ainxt_shadow_find-}\" ]"));
        assert!(r.contains("unalias find"));
        assert!(r.contains("unset -f find"));
        assert!(r.contains("unset __ainxt_shadow_find"));
    }
    ...
    fn build_injection_off_emits_marker_gated_restore_not_function() {
        ...
        assert!(inject.contains("if [ -n \"${__ainxt_shadow_find-}\" ]"));
        assert!(inject.contains("if [ -n \"${__ainxt_shadow_grep-}\" ]"));
    ...
    fn build_injection_on_shadows_resolved_tools() {
        ...
        assert!(inject.contains("__ainxt_shadow_find=1"));
    }
    ...
    fn env_override_accepts_regular_file_without_exec_bit() {
        // `tempfile::tempdir()` (a private, auto-cleaned-up directory)
        // instead of a hand-built, PID-suffixed path directly under the
        // shared temp directory.
        let tmp = tempfile::tempdir().unwrap();
        let bin = tmp.path().join("ainxt-bfs-noexec");
    ...
    fn resolve_tool_precedence() {
        // Real temp files so the is_file() checks pass. `tempfile::tempdir()`
        // (a private, auto-cleaned-up directory) instead of a hand-built,
        // PID-suffixed path directly under the shared temp directory.
        let tmp = tempfile::tempdir().unwrap();
    ...
    /// `AINXT_TOOLS_BUNDLE_{BFS,UGREP}_PATH` at build time). Verifies the embedded
    /// bytes self-extract under `~/.ainxt/vendor` and the extracted `bfs` runs.
    #[cfg(all(bundle_bfs, bundle_ugrep))]
    #[test]
    fn bundled_binaries_extract_and_run() {
        let vendor = crate::util::ainxt_home().join("vendor");
    ...
    /// mode-0644 `AINXT_TOOLS_*_PATH` / vendor copy) must fall through to the OS
