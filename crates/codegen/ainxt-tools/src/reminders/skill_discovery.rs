...
//! after every tool call to check for SKILL.md files in `.ainxt/skills/`,
...
/// Directories that contain skill definitions (`.ainxt/skills/`, `.agents/skills/`,
/// `.claude/skills/`, `.cursor/skills/`). Shared between startup skill discovery
/// and runtime `SkillDiscoveryReminder`.
pub const SKILL_CONFIG_DIRS: &[&str] = &[".ainxt", ".agents", ".claude", ".cursor"];
...
impl SkillDiscoveryReminder {
    ...
    /// (`.ainxt/skills/`, `.agents/skills/`, or `.claude/skills/`).
...
impl Reminder for SkillDiscoveryReminder {
    ...
    async fn collect_reminders(
        ...
        // discovery cannot find these because it looks for `.ainxt/skills/`
        // sub-directories in *ancestor* dirs, and user-scope skills
        // (~/.ainxt/) are outside the git root so the walk breaks early.
