...
use crate::implementations::ainxt_build::task::types::{
...
impl AutoWakeDeliveredIds {
    ...
    ///
    /// Used by the between-turn completion drain in `ainxt-shell` to
...
crate::register_resource!("ainxt_build", "AutoWakeDeliveredIds", AutoWakeDeliveredIds);
...
/// caller includes the text unchanged.
...
/// events from the structured `ainxt.dev/monitor_event` notification, never by
/// parsing this text.
///
...
/// unwrapped via [`split_wrapped_monitor_event`] with unchanged fallback.
pub fn format_monitor_events(
    events: &[crate::implementations::ainxt_build::task::types::MonitorEventNotification],
    task_output_name: Option<&str>,
) -> Option<String> {
    use std::fmt::Write as _;
    ...
    match events {
        ...
        [event] => {
            ...
            let label =
                crate::implementations::ainxt_build::monitor::event::sanitize_monitor_description(
                    label,
                );
            Some(format!(
        ...
        _ => {
            type Event = crate::implementations::ainxt_build::task::types::MonitorEventNotification;
            ...
            for (task_id, group) in &groups {
                let description = group
                    ...
                    .filter(|d| !d.is_empty())
                    .unwrap_or("event");
                let description = crate::implementations::ainxt_build::monitor::event::sanitize_monitor_description(
                    description,
                );
                let _ = write!(
                    buf,
...
/// scope only when it has no recorded owner (legacy / non-ainxt-build backends)
...
///   `disk_pointer_footer` is `None`, the full output is inlined unchanged
...
/// `output` is inlined unchanged -- this notification is the only place
...
/// inlined unchanged; see [`render_completion_output_delivery`].
...
/// - `ainxt-shell`'s `SessionActor` to sweep matching synthetic
...
pub fn consumed_completion_ids(output: &ToolOutput) -> Vec<&str> {
    ...
    match output {
        ...
        | ToolOutput::ChromeNavigate(_)
        | ToolOutput::ChromeReadPage(_)
        | ToolOutput::ChromeInteract(_)
        | ToolOutput::MCP(_)
        | ToolOutput::Skill(_)
    ...
    async fn collect_reminders(
        ...
        if let Some(terminal) = terminal {
            ...
            let goal_loop_active = res
                .get::<crate::implementations::ainxt_build::task::types::GoalLoopActive>()
                .is_some_and(|g| g.0);
            let surface_reminders = !goal_loop_active
                && res
                    .get::<crate::types::resources::Params<
                        crate::implementations::ainxt_build::bash::BashParams,
                    >>()
...
mod tests {
    ...
    /// full output text is inlined unchanged. The bash truncation cap only
    ...
    use ainxt_tool_types::{MultiTaskOutputResult, TaskOutputResult};
    struct MockTerminal {
    ...
    fn shared_with_reminders_disabled(tasks: Vec<TaskSnapshot>) -> SharedResources {
        ...
        res.insert(Terminal(backend));
        res.register_state::<ReportedTaskCompletions>();
        let params = crate::implementations::ainxt_build::bash::BashParams {
    ...
    async fn completions_suppressed_when_goal_loop_active() {
        ...
        res.insert(Terminal(backend));
        res.register_state::<ReportedTaskCompletions>();
        res.insert(crate::implementations::ainxt_build::task::types::GoalLoopActive(true));
        let (tx, mut rx) = tokio::sync::mpsc::unbounded_channel();
        res.insert(SubagentEventSender(tx));
        tokio::spawn(async move {
        ...
        shared
            .lock()
            .await
            .insert(crate::implementations::ainxt_build::task::types::GoalLoopActive(false));
        let second = reminder.collect_reminders(shared, &output).await;
        assert!(
            second.is_empty(),
    ...
    /// inline cap MUST be preserved unchanged. The inline branch is the
    ...
    fn format_monitor_events_single_vs_batched() {
        use crate::implementations::ainxt_build::task::types::MonitorEventNotification;
        ...
        let bare = crate::implementations::ainxt_build::task::types::MonitorEventNotification {
            task_id: "task-9".into(),
    ...
    /// for non-conforming text so the batch falls back to unchanged.
    #[test]
    ...
    fn wrap_monitor_event_round_trips_through_split() {
        use crate::implementations::ainxt_build::monitor::event::wrap_monitor_event;
