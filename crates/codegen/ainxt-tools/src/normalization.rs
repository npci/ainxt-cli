...
/// Resolve `wire_name` in `toolset` and merge the canonical `ainxt.dev/tool` object
...
/// the `ainxt.dev/tool` `_meta` object. Equivalent tools across toolsets emit the
...
pub fn canonical_input(input: &ToolInput) -> Option<serde_json::Value> {
    ...
    Some(match input {
        ...
        | ToolInput::ChromeNavigate(_)
        | ToolInput::ChromeReadPage(_)
        | ToolInput::ChromeClick(_)
        | ToolInput::ChromeType(_)
        | ToolInput::ChromeScreenshot(_)
...
mod tests {
    ...
    fn canonical_omits_absent_options_not_null() {
        let ainxt = parse(serde_json::json!({ "variant" : "ReadFile", "target_file" : "/a" }));
        let g = canonical_input(&ainxt).unwrap();
