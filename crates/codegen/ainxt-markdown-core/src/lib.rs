//! Headless markdown analysis sharing ainxt's exact `pulldown-cmark` config.
//!
...
//! feature set, shared with `ainxt-markdown` so analysis matches what ainxt
//! Build actually renders 1:1.
//!
//! After parsing, ainxt applies [`offset_events`]: only `~~…~~` is strikethrough.
...
/// The exact `pulldown-cmark` option set ainxt uses to render markdown.
///
...
/// Offset event stream from ainxt's parser, with single-tilde strikethrough demoted.
///
...
///
/// Counting matches the `pulldown-cmark` event stream the renderer walks, so a few
...
///
/// Works off raw block source (the only thing carrying closure info), so a unchanged fence line in
...
/// Parse `text` with ainxt's options; count elements and flag structural issues.
