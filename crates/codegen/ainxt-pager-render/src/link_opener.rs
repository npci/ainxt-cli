...
//!
//! Extracted from the `OpenSubscribeUrl` dispatch handler so that any
...
pub fn open_url(url: &str) -> bool {
    ...
    if let Ok(path) = std::env::var("AINXT_TEST_OPEN_URL_FILE") {
        use std::io::Write;
        ...
        {
            tracing::warn!(error = %e, path, "AINXT_TEST_OPEN_URL_FILE write failed");
            return false;
        }
        return true;
    }
    ...
    ainxt_tools::util::detach_std_command(&mut command);
    match command.spawn() {
        Ok(_) => true,
        Err(e) => {
...
///
/// The returned command is TTY-guarded via [`ainxt_tty_utils::detach_std_command`]
...
fn build_open_path_command(path: &std::path::Path) -> std::process::Command {
    ...
    command
        ...
        .stdout(std::process::Stdio::null())
        .stderr(std::process::Stdio::null());
    ainxt_tty_utils::detach_std_command(&mut command);
    command
}
...
/// unchanged — the default arg quoting wraps the whole token and breaks the
...
///
/// Used by the SuperAinxt upsell flow to attribute clicks to `referrer=ainxt-build`,
/// matching the OAuth consent screen and ainxt.dev/cli marketing links regardless of
...
mod tests {
    ...
    fn open_path_command_passes_path_as_a_single_arg() {
        ...
        let path = std::path::Path::new("/tmp/ainxt session/image 1.jpg");
    ...
    fn ensure_query_param_appends_when_missing() {
        let out =
            ensure_query_param("https://ainxt.example.test/subscribe", "referrer", "ainxt-build");
        // The expected value read ".../subscribe" (no query) until 2026-08-29,
        // asserting that a function whose whole job is to append a parameter
        // appends nothing. The suffix was evidently lost in the endpoint rename.
        assert_eq!(
            out,
            "https://ainxt.example.test/subscribe?referrer=ainxt-build"
        );
    }
    ...
    fn ensure_query_param_preserves_existing_value() {
        let out = ensure_query_param(
            "https://ainxt.example.test/subscribe?referrer=other",
            "referrer",
            "ainxt-build",
        );
        assert_eq!(out, "https://ainxt.example.test/subscribe?referrer=other");
    }

    #[test]
    fn ensure_query_param_keeps_other_query_pairs() {
        let out = ensure_query_param(
            "https://ainxt.example.test/subscribe?heavy=1",
            "referrer",
            "ainxt-build",
        );
        assert_eq!(
            out,
            "https://ainxt.example.test/subscribe?heavy=1&referrer=ainxt-build"
        );
    }
    ...
    fn ensure_query_param_preserves_fragment() {
        ...
        // routing (`ainxt.dev/#superainxt`); we still want the referrer attached.
        let out = ensure_query_param("https://ainxt.example.test/#superainxt", "referrer", "ainxt-build");
        assert_eq!(out, "https://ainxt.example.test/?referrer=ainxt-build#superainxt");
    }

    #[test]
    fn ensure_query_param_returns_unchanged_on_parse_failure() {
        let out = ensure_query_param("not a url", "referrer", "ainxt-build");
        assert_eq!(out, "not a url");
    }

    #[test]
    fn ensure_query_param_url_encodes_value() {
        let out = ensure_query_param("https://ainxt.example.test/subscribe", "referrer", "ainxt build");
        assert_eq!(out, "https://ainxt.example.test/subscribe?referrer=ainxt+build");
    }
    ...
    fn browser_unavailable_message_includes_full_url() {
        let url = "https://ainxt.example.test/subscribe";
