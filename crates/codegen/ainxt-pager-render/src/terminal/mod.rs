...
pub enum TerminalName {
    ...
    JetBrains,
    /// ainxt Desktop (Electron app).
    #[strum(to_string = "ainxt Desktop")]
    AinxtDesktop,
...
pub struct TerminalContext {
    ...
    /// Which embedded editor `:terminal` ainxt is running inside, if any.
...
impl TerminalContext {
    ...
    /// keyed off this only fires when a FocusGained actually reaches ainxt, which
    ...
    ///
    /// JediTerm (JetBrains IDEs' own bundled terminal emulator) is the known
    /// offender, on every host OS it ships on — confirmed leaking on macOS,
    /// not just Windows. The pager defaults these sessions to minimal mode
    /// (no mouse capture).
    ...
    pub fn telemetry_snapshot(&self) -> ainxt_telemetry::events::TerminalTelemetry {
        ...
        ainxt_telemetry::events::TerminalTelemetry {
            ...
            clipboard_native_tool: ainxt_shared::clipboard::native_tool_name().to_owned(),
            clipboard_data_control: crate::clipboard::wayland_data_control_label().to_owned(),
        }
    }

    /// Extract terminal info for feedback submissions.
    pub fn feedback_info(&self) -> ainxt_shared::session::FeedbackTerminalInfo {
        use ainxt_shared::session::FeedbackTerminalInfo;
        ...
        FeedbackTerminalInfo {
            ...
            clipboard_native_tool: Some(ainxt_shared::clipboard::native_tool_name().to_owned()),
...
fn detect_terminal_context() -> TerminalContext {
    ...
    // first client, not the current one). Revisit when `ainxt ssh` can
    // forward env vars.
...
/// `ainxt-pager-pty-harness/src/pty.rs` (test-env hygiene — the PTY
...
///
/// `host` is unused now that the leak is confirmed OS-agnostic (it's a bug
/// in JediTerm itself, not in any platform's terminal input decoding), but
/// stays in the signature so a future terminal-specific exception can slot
/// back in without changing every call site.
fn mouse_reporting_leaks(brand: TerminalName, _host: HostOs) -> bool {
...
fn terminal_name_from_term_program(value: &str) -> Option<TerminalName> {
    ...
    match normalized.as_str() {
        ...
        "ainxtdesktop" => Some(TerminalName::AinxtDesktop),
...
///
/// Parsed from `[terminal] alt_screen` in `~/.ainxt/pager.toml` and
