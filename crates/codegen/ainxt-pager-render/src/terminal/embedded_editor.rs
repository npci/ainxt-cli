//! Detects whether ainxt is running inside an editor's embedded `:terminal`
...
/// Which embedded editor `:terminal` ainxt is running inside.
...
/// `HOST_TERMINAL_ENV_VARS` in `ainxt-pager-pty-harness/src/pty.rs`
/// (test-env hygiene).
