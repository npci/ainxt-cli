...
pub struct AnimationConfig {
    ...
    /// Also enabled by the `AINXT_FPS=1` env var. Default: false.
    pub show_fps: bool,
}
...
pub struct RawAnimationConfig {
    ...
    /// Requires a debug build. Also enabled by AINXT_FPS=1 env var. Default: false.
    pub show_fps: bool,
}
...
fn lookup_named_color(name: &str) -> Result<Color, String> {
    // Named colors use the AinxtNight RGB palette. They are quantized via
...
impl RawAppearanceConfig {
    pub fn to_toml_with_comments() -> String {
        ...
        let pager_path = crate::util::display_user_ainxt_path("pager.toml");
        let header = format!(
            "\
# ainxt Pager Appearance Configuration ({pager_path})
...
/// toggles can't interleave and clobber each other (matches the shell's
...
pub fn persist_respect_manual_folds(enabled: bool) -> std::io::Result<()> {
    ...
    if ainxt_config::user_ainxt_home().is_none() {
        return Err(Error::new(
            ErrorKind::NotFound,
            "no user ainxt home resolved; refusing to write a cwd-relative pager.toml \
