...
///
/// `AINXT_FORCE_LEGACY_CONSOLE=1` (or `true`) forces this on regardless of
...
/// Read the `AINXT_FORCE_LEGACY_CONSOLE` escape hatch from the environment.
fn forced_legacy_console_override() -> Option<bool> {
    parse_forced_legacy_console(std::env::var("AINXT_FORCE_LEGACY_CONSOLE").ok().as_deref())
}
...
mod tests {
    ...
    fn to_legacy_glyphs_maps_known_glyphs() {
        ...
        // Glyphs this module doesn't own (em dash, CJK) pass through unchanged.
        assert_eq!(
