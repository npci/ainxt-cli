...
fn resolve_tool_path_with_home(
    ...
    let display_path = target
        .as_deref()
        .map(ainxt_paths::normalize_lexically)
        .unwrap_or_else(|| PathBuf::from(path));
    let relative_to_cwd = target.as_ref().and_then(|_| {
        let cwd = ainxt_paths::normalize_lexically(cwd?);
...
mod tests {
    ...
    fn shorten_path_fish_style() {
        let result = shorten_path("crates/codegen/ainxt-pager/src/views/foo.rs", 25);
    ...
    fn shorten_path_front_truncate() {
        let result = shorten_path(
            "crates/codegen/ainxt-pager/src/views/very_long_filename.rs",
            20,
        );
    ...
    fn expanded_surface_uses_worktree_cwd() {
        let cwd = Path::new("/Users/me/.ainxt/worktrees/foo");
        let path = "/Users/me/.ainxt/worktrees/foo/crates/x/a.rs";
        assert_eq!(
    ...
    fn home_relative_target_preserves_filesystem_spelling_for_io() {
        ...
        assert_eq!(
            ...
            ainxt_paths::normalize_lexically(&home.join("notes.md"))
        );
    }
