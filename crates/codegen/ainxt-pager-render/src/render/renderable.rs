...
///
/// `Line` no longer implements `WidgetRef` as of ratatui 0.30 (the impl was
/// dropped when the crate split into `ratatui-core`); it still implements
/// `Widget` for `&Line<'_>`, so we go through that instead of `WidgetRef`.
