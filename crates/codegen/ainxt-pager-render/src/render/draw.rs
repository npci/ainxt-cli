...
//! [`ainxt_ratatui_inline::Terminal`] whose `flush()` returns `bool` indicating
...
use ainxt_ratatui_inline::LinkSpan;
...
pub type PagerTerminal = ainxt_ratatui_inline::Terminal<CrosstermBackend<TermWriter>>;
...
pub fn spawn_writer_thread() -> (mpsc::Sender<Vec<u8>>, WriterSync, WriterThread) {
    ...
    let test_delay = std::env::var("AINXT_TEST_FRAME_WRITE_DELAY_MS")
        .ok()
    ...
    let handle = std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            let mut writer: Box<dyn std::io::Write> = {
                let tui_out = ainxt_tty_utils::dup_tui_stderr().unwrap_or_else(|_| {
            ...
            while let Ok(data) = rx.recv() {
                ...
                {
                    let _guard = ainxt_shared::stderr::stderr_lock();
