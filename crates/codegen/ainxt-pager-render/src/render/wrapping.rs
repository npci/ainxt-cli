...
/// invariant, consider switching to a two-stage approach that matches
...
pub fn wrap_byte_ranges_matching(text: &str, width: usize) -> Vec<Range<usize>> {
    ...
    // Skip whitespace at the wrap boundary (matches word_wrap_line_with_joiners).
...
///
/// The selection layer's stricter, style-aware twin lives in ainxt-pager
...
mod tests {
    ...
    fn real_markdown_link_underline_does_not_leak_after_wrap() {
        ...
        let (out, _) = ainxt_markdown::render_markdown_ratatui_full(md, md_style, true, None);
    ...
    /// The monotonic `cursor` fast path must produce exactly the same
    ...
    fn table_row_with_emoji_and_em_dash_fills_content_width() {
        ...
        // Matches the markdown table body row "│ ⚠️ warn │ em — dash │".
