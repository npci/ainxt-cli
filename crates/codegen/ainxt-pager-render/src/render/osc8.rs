...
//! frame diff (see `ainxt_ratatui_inline::Terminal::flush_with_links`).
...
/// One path segment without spaces (`main.rs`, `.ainxt`, `@scope`). Leading `.`
/// matches dot-directories and `%` matches percent-encoded segments — ainxt
/// session media lives under `~/.ainxt/sessions/%2F…/images/1.jpg`.
...
/// Relative file path (`images/1.png`, `.ainxt/x.txt`) — one or more `/`-joined
...
/// media lives at `~/.ainxt/sessions/%2F…/images/1.jpg`, which wraps in
...
mod tests {
    ...
    fn local_link_ignores_web_anchor_and_unknown() {
        ...
        assert!(local_link_to_file_target("https://ainxt.example.test", &media).is_none());
    ...
    fn scan_content_x_offset_applied() {
        // Derive the expected width from the input. This asserted `10 + 12`,
        // where 12 was the byte length of an earlier, shorter URL literal; the
        // endpoint rename lengthened the string to 26 and left the 12 behind, so
        // the test demanded a link 14 columns shorter than the text it scanned.
        const URL: &str = "https://ainxt.example.test";
        const X_OFFSET: usize = 10;
        let line = make_line(URL);
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), X_OFFSET as u16, &[], &mut overlay);

        assert_eq!(overlay.links().len(), 1);
        assert_eq!(overlay.links()[0].col_start as usize, X_OFFSET);
        assert_eq!(overlay.links()[0].col_end as usize, X_OFFSET + URL.len());
    }
    ...
    #[test]
    fn scan_detects_ainxt_session_media_path() {
        // Dot-directory (`.ainxt`), percent-encoded session segment, and a
        // trailing sentence period — the shape of `image_gen` output prose.
        let line = make_line("Saved to /Users/alice/.ainxt/sessions/%2Fabc/00000000/images/1.jpg.");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
        ...
        assert_eq!(
            ...
            "file:///Users/alice/.ainxt/sessions/%252Fabc/00000000/images/1.jpg",
        );
    }
    ...
    fn scan_detects_media_path_soft_wrapped_across_rows() {
        ...
        let row0 =
            make_line("Image generated and saved to /Users/alice/.ainxt/sessions/%2FUsers%2Fali");
        ...
        let expected_url = "file:///Users/alice/.ainxt/sessions/%252FUsers%252Fali\
        ...
        assert_eq!(
            ...
            2 + UnicodeWidthStr::width(
                "Image generated and saved to /Users/alice/.ainxt/sessions/%2FUsers%2Fali"
            ) as u16
        );
    ...
    fn scan_wrapped_path_trailing_sentence_period_excluded() {
        ...
        let row0 = make_line("Saved to /Users/me/.ainxt/sessions/%2Fabc/019f3a86/ima");
        ...
        for link in overlay.links() {
            assert_eq!(
                &*resolve_link_target(&link.target)
                    .and_then(|resolved| resolved.osc8_url)
                    .expect("url"),
                "file:///Users/me/.ainxt/sessions/%252Fabc/019f3a86/images/1.jpg"
            );
        }
    ...
    fn scan_file_path_with_dots_and_hyphens() {
        let line = make_line("Reading /tmp/ainxt-impl-summary.md now.");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
        ...
        assert_eq!(
            &*resolve_link_target(&overlay.links()[0].target)
                .and_then(|resolved| resolved.osc8_url)
                .expect("url"),
            "file:///tmp/ainxt-impl-summary.md"
        );
    }
    ...
    fn scan_detects_tilde_file_path() {
        ...
        let raw = "~/Desktop/ainxt-pager-retention-findings-2026-06-06.md";
    ...
    fn scan_single_component_tilde_path_not_linkified() {
        // `~/projects` has a single component — matches the absolute
    ...
    fn scan_relative_path_not_partially_linkified() {
        // A relative path like `crates/codegen/ainxt-pager/src/render` should
        // NOT produce a link for the `/ainxt-pager/src/render` substring.
        let line = make_line("find crates/codegen/ainxt-pager/src/render -name '*.rs'");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
