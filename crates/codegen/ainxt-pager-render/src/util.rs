...
pub use ainxt_config::ainxt_home;

/// Path to `$AINXT_HOME/pager.toml`.
pub fn pager_toml_path() -> PathBuf {
    ainxt_home().join("pager.toml")
}

/// User-facing label for the user ainxt directory (``~/.ainxt`` or ``$AINXT_HOME``).
///
/// Derived from resolved [`ainxt_home()`] vs `ainxt_config::default_ainxt_home()`,
/// not from whether `AINXT_HOME` is set in the environment.
pub fn display_ainxt_home_prefix() -> String {
    if ainxt_home() == ainxt_config::default_ainxt_home() {
        "~/.ainxt".to_string()
    } else {
        "$AINXT_HOME".to_string()
    }
}

/// User-facing path under [`ainxt_home()`], e.g. ``~/.ainxt/config.toml``.
pub fn display_user_ainxt_path(relative: impl AsRef<Path>) -> String {
    let rel = relative.as_ref();
    let prefix = display_ainxt_home_prefix();
...
/// Abbreviate an absolute path for display: prefer [`ainxt_home()`], then `$HOME`.
pub fn abbreviate_path(path: &str) -> Cow<'_, str> {
    let path_buf = Path::new(path);
    let ainxt = ainxt_home();
    if let Ok(rest) = path_buf.strip_prefix(&ainxt) {
        let prefix = display_ainxt_home_prefix();
...
/// True when `path` is under user [`ainxt_home()`] (not project `{cwd}/.ainxt`).
pub fn is_under_user_ainxt_home(path: &Path) -> bool {
    path.starts_with(ainxt_home())
}
...
mod tests {
    ...
    #[test]
    fn display_ainxt_home_prefix_default_install() {
        if std::env::var("AINXT_HOME").is_ok() {
            return;
        }
        assert_eq!(display_ainxt_home_prefix(), "~/.ainxt");
    }

    #[test]
    fn display_user_ainxt_path_joins_relative() {
        let path = display_user_ainxt_path("config.toml");
        assert!(path.ends_with("/config.toml") || path.ends_with("\\config.toml"));
        assert!(path.contains(".ainxt") || path.contains("$AINXT_HOME"));
    }

    #[test]
    fn abbreviate_path_uses_home_when_under_default_ainxt() {
        if let Ok(home) = std::env::var("HOME") {
            if home.is_empty() {
                return;
            }
            let full = format!("{home}/.ainxt/memory/MEMORY.md");
