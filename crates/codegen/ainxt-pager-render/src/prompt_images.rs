...
use ainxt_ratatui_textarea::ElementId;
...
pub fn decode_image_dimensions(bytes: &[u8]) -> Option<(u32, u32)> {
    ...
    ainxt_tools::util::image_validate::validate_image_bytes_unrestricted(bytes, false)
        .ok()
        .map(|(w, h, _)| (w, h))
}
...
impl ImageViewerState {
    ...
    pub fn open_from_path(path: &std::path::Path) -> Option<Self> {
        ...
        let mime_type = ainxt_shared::clipboard::mime_from_bytes(&bytes).to_owned();
...
impl VideoViewerState {
    ...
    pub fn open_from_path(path: &std::path::Path) -> Option<Self> {
        ...
        ainxt_tty_utils::detach_std_command(&mut ffmpeg_cmd);
...
use ainxt_shared::clipboard::mime_to_extension;
...
///
/// Delegates to [`ainxt_shared::clipboard::mime_to_extension`].
...
/// Normalize a media path by dropping the Windows `\\?\` unchanged prefix
...
fn read_image_at_path(path: &std::path::Path) -> Option<PastedImage> {
    ...
    let mime_type = ainxt_shared::clipboard::mime_from_bytes(&data);
...
pub fn session_images_dir(
    ...
    let sid = session_id?;
    let info = ainxt_shared::session::info::Info {
        id: sid.clone(),
        cwd: cwd.to_string_lossy().into_owned(),
    };
    Some(ainxt_shared::session::session_dir(&info).join("images"))
}

/// Derive the `mermaid/` cache directory for a session.
///
/// Matches [`session_images_dir`]: rendered diagram PNGs live alongside the
...
pub fn session_mermaid_dir(
    ...
    Some(ainxt_shared::session::session_dir(&info).join("mermaid"))
}
...
///   [`ainxt_shell::session::placeholder_images::load_placeholder_image`]
...
pub fn build_content_blocks_with_workspace(
    ...
    let allowed: Option<Vec<std::path::PathBuf>> =
        workspace_cwd.map(ainxt_shared::placeholder_images::default_allowed_prefixes);
...
pub fn build_content_blocks_with_prefixes(
    ...
    build_content_blocks_with_prefixes_and_caps(
        ...
        images,
        allowed_prefixes,
        ainxt_shared::placeholder_images::MAX_PLACEHOLDER_AGGREGATE_BYTES,
    )
}
...
/// [`ainxt_shell::session::placeholder_images::recover_orphan_placeholders_with_prefixes_and_caps`].
...
pub fn build_content_blocks_with_prefixes_and_caps(
    ...
    // attachment.
    let rewritten_text =
        ainxt_shared::placeholder_images::strip_paths_from_image_placeholders(rewritten_text);
    ...
    for img in &images {
        ...
        blocks.push(ContentBlock::Image(
            ImageContent::new(data, mime_type)
                ...
                .meta(Some(
                    ainxt_shared::placeholder_images::display_number_meta(img.display_number),
                )),
        ));
    }
...
fn resolve_orphan_placeholders(
    ...
    let placeholders = ainxt_shared::placeholder_images::extract_placeholders(&text);
    ...
    for ph in &placeholders {
        ...
        match ainxt_shared::placeholder_images::load_placeholder_image(&ph.path, allowed) {
            Ok(loaded) => {
                ...
                recovered.push(
                    ImageContent::new(data, loaded.mime_type)
                        ...
                        .meta(Some(
                            ainxt_shared::placeholder_images::display_number_meta(
                                ph.display_number,
                            ),
                        )),
                );
...
mod tests {
    ...
    /// caption must survive unchanged instead of being silently
    ...
    fn file_url_single_slash_rejected_at_anchor_gate() {
        ...
        // URL parsing runs.
        let pasted = "file:/tmp/should_not_exist_abc_ainxt_pager.png";
    ...
    fn dropped_path_per_line_partial_resolution_falls_through() {
        ...
        // sees the unchanged string in the prompt rather than only
        // `real_dir` appearing.
        let entries = dropped_paths(&pasted);
        assert!(
            entries.is_empty(),
    ...
    fn dropped_path_nonexistent_bare_path_not_intercepted() {
        ...
        // matches that for the non-image branch.)
        let entries = dropped_paths("/tmp/definitely_does_not_exist_xyz_ainxt_pager.txt");
        assert!(
            entries.is_empty(),
    ...
    fn build_blocks_sets_display_number_meta() {
        ...
        assert_eq!(
            ainxt_shared::placeholder_images::display_number_from_meta(ic.meta.as_ref()),
            Some(3),
    ...
    fn build_blocks_uri_prefers_durable_session_path() {
        ...
        img.session_image_path = Some(PathBuf::from("/Users/test/.ainxt/session/image.png"));
        let blocks = build_blocks_no_workspace("text".into(), vec![img]);
        assert_eq!(blocks.len(), 2);
        if let agent_client_protocol::ContentBlock::Image(ic) = &blocks[1] {
            assert_eq!(
                ic.uri.as_deref(),
                Some("file:///Users/test/.ainxt/session/image.png"),
    ...
    fn build_blocks_orphan_aggregate_cap_inclusive_boundary() {
        ...
        // decode the base64 data and assert exactly equality
