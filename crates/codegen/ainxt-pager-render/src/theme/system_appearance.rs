...
/// in `config.toml`. When `None`, defaults to `AinxtNight` / `AinxtDay`.
///
...
pub fn to_theme_kind(
    ...
    match appearance {
        SystemAppearance::Light => light_theme.unwrap_or(ThemeKind::AinxtDay),
        SystemAppearance::Dark => dark_theme.unwrap_or(ThemeKind::AinxtNight),
    }
}
