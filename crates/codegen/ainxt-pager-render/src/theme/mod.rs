...
//! The default theme is AinxtNight (neutral gray base with TokyoNight accents).
//!
//! ## Color support
//!
//! AinxtNight is defined in `Color::Rgb` (truecolor). At startup,
...
impl ThemeKind {
    ...
    pub fn available() -> &'static [ThemeKind] {
        ...
        const NO_TRUECOLOR: &[ThemeKind] = &[ThemeKind::AinxtNight, ThemeKind::AinxtDay];
    ...
    pub fn display_name(self) -> &'static str {
        match self {
            Self::AinxtNight => "ainxtnight",
            Self::TokyoNight => "tokyonight",
            Self::AinxtDay => "ainxtday",
    ...
    /// when quantized to 256 or 16 colors. AinxtNight uses neutral grays
    /// that survive quantization cleanly.
    pub fn requires_truecolor(self) -> bool {
        match self {
            Self::AinxtNight => false,
    ...
    pub fn from_name(name: &str) -> Option<Self> {
        ...
        match lower.as_str() {
            ...
            "ainxtnight" | "ainxt-night" | "dark" => Some(Self::AinxtNight),
            "tokyonight" | "tokyo-night" | "tokyo" => Some(Self::TokyoNight),
            "ainxtday" | "ainxt-day" | "light" | "day" => Some(Self::AinxtDay),
...
/// `"ainxtnight"` → `"ainxt Night"`). Falls back to `value` unchanged.
pub fn display_name_for_canonical(value: &str) -> &str {
    match value {
        "auto" => "Auto",
        "ainxtnight" => "ainxt Night",
        "ainxtday" => "ainxt Day",
...
impl Theme {
    ...
    ///
    /// Reads the active theme kind (loaded from `~/.ainxt/config.toml` on
    ...
    pub fn current() -> Self {
        ...
        let base = match cache::current_kind() {
            ThemeKind::AinxtNight => Self::ainxtnight(),
            ThemeKind::TokyoNight => Self::tokyonight(),
            ThemeKind::AinxtDay => Self::ainxtday(),
            ...
            // if reached, fall back to AinxtNight.
            ThemeKind::Auto => Self::ainxtnight(),
        };
        ...
        //      (e.g., `TERM=xterm`, `TERM=ansi`, or `AINXT_FORCE_COLOR_LEVEL=basic`),
        //      where naive quantization collapses every dark RGB onto `Color::Black`.
        //   2. Legacy Windows ConHost below TrueColor, kept for equivalence with the
    ...
    ///    on ainxtnight, erasing every state signal). We pin each
    ...
    /// theme (e.g., AinxtNight on a white terminal would show white
    ...
    fn ansi16_chrome_overrides(self, dark: bool) -> Self {
        ...
        Self {
            ...
            // AinxtNight bg field quantizes to Color::Black and these
            // become invisible.
...
pub fn apply_cursor_color() {
    ...
    ainxt_shared::stderr::with_locked_stderr(|stderr| {
...
mod tests {
    ...
    fn is_auto_returns_false_for_concrete_variants() {
        assert!(!ThemeKind::AinxtNight.is_auto());
        assert!(!ThemeKind::AinxtDay.is_auto());
    ...
    fn ansi16_overrides_light_inverts_high_contrast_and_elevated_bg() {
        ...
        use ratatui::style::Color;
        let t = Theme::ainxtday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_state_accents_pin_to_polarity_aware_hue() {
        ...
        use ratatui::style::Color;
        let t_dark = Theme::ainxtnight().ansi16_chrome_overrides(true);
        ...
        let t_light = Theme::ainxtday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_canvas_matching_surfaces_use_theme_polarity() {
        ...
        // the theme disagrees with the terminal profile (e.g. AinxtNight
    ...
    fn scrollbar_thumb_contrasts_with_track_in_all_themes() {
        ...
        for &kind in ThemeKind::ALL {
            let theme = match kind {
                ThemeKind::AinxtNight => Theme::ainxtnight(),
                ThemeKind::AinxtDay => Theme::ainxtday(),
    ...
    #[test]
    fn ansi16_quantize_without_override_collapses_ainxtnight_backgrounds() {
        // Regression-ratchet for the override gate in `Theme::current`:
        // naive `Basic` quantization maps every dark AinxtNight bg field
        ...
        use ratatui::style::Color;
        let q = Theme::ainxtnight().quantized(color_support::ColorLevel::Basic);
        for (name, color) in [
            ("bg_base", q.bg_base),
            ("bg_light", q.bg_light),
            ("bg_dark", q.bg_dark),
    ...
    fn from_name_concrete_variants_still_work() {
        assert_eq!(
            ThemeKind::from_name("ainxtnight"),
            Some(ThemeKind::AinxtNight)
        );
        assert_eq!(ThemeKind::from_name("dark"), Some(ThemeKind::AinxtNight));
        assert_eq!(ThemeKind::from_name("ainxtday"), Some(ThemeKind::AinxtDay));
        assert_eq!(ThemeKind::from_name("light"), Some(ThemeKind::AinxtDay));
        assert_eq!(
    ...
    fn from_str_matches_from_name_for_all_canonicals() {
        ...
        let cases = [
            ...
            ("ainxtnight", ThemeKind::AinxtNight),
            ("ainxt-night", ThemeKind::AinxtNight),
            ("dark", ThemeKind::AinxtNight),
            ("tokyonight", ThemeKind::TokyoNight),
            ("tokyo-night", ThemeKind::TokyoNight),
            ("tokyo", ThemeKind::TokyoNight),
            ("ainxtday", ThemeKind::AinxtDay),
            ("ainxt-day", ThemeKind::AinxtDay),
            ("light", ThemeKind::AinxtDay),
            ("day", ThemeKind::AinxtDay),
