use regex::Regex;
use ainxt_tools::types::{claude_names_for, ainxt_names_for};
...
///   **exact** match against each name (after external→ainxt alias expansion), NOT a regex;
/// - anything else is an **unanchored** regex (also tested against the tool's external
///   alias names, so e.g. `^Bash$` matches the ainxt tool `run_terminal_command`).
///
...
/// plus any ainxt tool names that term aliases (so `"Bash"` also matches
/// `run_terminal_command`), per the shared external-name to ainxt registry in
/// `ainxt-tools`. Empty terms and duplicates are dropped.
fn exact_names(pattern: &str) -> Vec<String> {
    ...
    for term in pattern.split('|') {
        push(term);
        for ainxt_name in ainxt_names_for(term) {
            push(ainxt_name);
        }
    }
    names
...
mod tests {
    ...
    #[test]
    fn claude_bash_matches_ainxt_tool() {
        let m = HookMatcher::new("Bash").unwrap();
        assert!(m.is_match("Bash")); // external alias name
        assert!(m.is_match("run_terminal_command")); // ainxt name
        assert!(!m.is_match("read_file"));
    ...
    #[test]
    fn claude_edit_write_matches_ainxt_tool_exactly() {
        let m = HookMatcher::new("Edit|Write").unwrap();
        assert!(m.is_match("Edit"));
        assert!(m.is_match("Write"));
        assert!(m.is_match("search_replace")); // ainxt equivalent
        assert!(m.is_match("hashline_edit")); // second ainxt alias
        assert!(!m.is_match("read_file"));
    ...
    #[test]
    fn claude_read_matches_ainxt_tool() {
    ...
    #[test]
    fn regex_against_claude_alias_matches_ainxt_tool() {
        // A regex written against an external alias still matches the ainxt tool
