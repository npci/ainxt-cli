...
// (`~/.ainxt/trusted_folders.toml`) is the single authority for whether a repo's
...
/// (`<user_ainxt_home>/trusted-hook-projects`), or `None` when no user ainxt home
/// resolves. Retained only for the one-time migration into folder-trust.
pub fn legacy_trust_file_path() -> Option<PathBuf> {
    Some(ainxt_config::user_ainxt_home()?.join("trusted-hook-projects"))
}
...
pub fn disable_hook(hook_name: &str) -> Result<(), String> {
    let file = disabled_hooks_file_path()
        .ok_or_else(|| "no user ainxt home (set $AINXT_HOME or $HOME)".to_string())?;
...
fn disable_hook_with_file(hook_name: &str, file: &Path) -> Result<(), String> {
    ...
    f.write_all(format!("{hook_name}\n").as_bytes())
        .map_err(|e| format!("failed to write disabled-hooks file: {e}"))?;
    // Explicitly release the file handle as soon as the write completes,
...
fn enable_hook_with_file(hook_name: &str, file: &Path) -> Result<bool, String> {
    ...
    // Explicitly release the file handle now that all lines are written,
    // rather than relying solely on end-of-scope Drop.
    drop(f);
    Ok(true)
}

/// Returns the path to `$AINXT_HOME/disabled-hooks`, or `None` when no user ainxt
/// home resolves.
fn disabled_hooks_file_path() -> Option<PathBuf> {
    Some(ainxt_config::user_ainxt_home()?.join("disabled-hooks"))
}
...
mod tests {
    ...
    fn trust_file_in(dir: &Path) -> PathBuf {
        let ainxt_dir = dir.join(".ainxt");
        std::fs::create_dir_all(&ainxt_dir).unwrap();
        ainxt_dir.join("trusted-hook-projects")
    }
