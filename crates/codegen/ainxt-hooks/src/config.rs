...
pub struct HooksMap {
    ...
    /// Event names present in the JSON but not recognized by ainxt.
...
pub struct RawHandler {
    ...
    /// keys (`AINXT_HOOK_EVENT`, `AINXT_HOOK_NAME`, `AINXT_SESSION_ID`,
    /// `AINXT_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`) are stripped at load
...
pub struct HookSpec {
    ...
    /// `${VAR%pat}`, etc.) are preserved unchanged and resolved at run
    ...
    ///    (`AINXT_HOOK_EVENT`, `AINXT_HOOK_NAME`, `AINXT_SESSION_ID`,
    ///    `AINXT_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`) are stripped at
    ///    load time and a tracing warning is emitted.
    /// 2. Plugin-injected vars merged in by the plugin adapter
    ///    (`ainxt-agent::plugins::hooks_adapter`). The adapter sets
    ///    `AINXT_PLUGIN_ROOT`, `CLAUDE_PLUGIN_ROOT`, `AINXT_PLUGIN_DATA`,
    ...
    ///    (`AINXT_HOOK_EVENT`, `AINXT_HOOK_NAME`, `AINXT_SESSION_ID`,
    ///    `AINXT_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`). These are
            ...
            for (hook_idx, handler) in group.hooks.into_iter().enumerate() {
                ...
                match handler.handler_type.as_str() {
                    "command" => {
                        ...
                        // preserved unchanged and handled by the runner's
...
mod tests {
    ...
    fn source_dir_from_file_path() {
        ...
        let (specs, _) = parse_hook_file(json, Path::new("/home/user/.ainxt/hooks/safety.json"));
        assert_eq!(specs[0].source_dir, PathBuf::from("/home/user/.ainxt/hooks"));
    }
    ...
    fn parse_hook_file_expands_env_var_in_command_from_process_env() {
        let key = "AINXT_HOOKS_PARSE_TEST_CMD_PROC_ENV";
    ...
    fn parse_hook_file_expands_env_var_in_url_from_process_env() {
        let key = "AINXT_HOOKS_PARSE_TEST_URL_PROC_ENV";
    ...
    fn parse_hook_file_preserves_unresolved_env_refs_in_command() {
        let key = "AINXT_HOOKS_PARSE_TEST_NEVER_SET_AT_LOAD_TIME";
    ...
    fn parse_hook_file_preserves_unresolved_env_refs_in_url() {
        let key = "AINXT_HOOKS_PARSE_TEST_URL_NEVER_SET_AT_LOAD_TIME";
        with_env_var(key, None, || {
            let json = format!(
                r#"{{
                    "hooks": {{
    ...
    /// Env values are stored unchanged; references inside them
    ...
    fn parse_hook_file_matcher_is_not_env_expanded() {
        let key = "AINXT_HOOKS_PARSE_TEST_MATCHER_VAR";
        with_env_var(key, Some("expanded_value_should_not_appear"), || {
            ...
            assert!(errors.is_empty(), "unexpected errors: {errors:?}");
            assert_eq!(specs.len(), 1);
            // configured_matcher stores the source string unchanged.
            assert_eq!(
    ...
    fn parse_hook_file_matcher_braced_var_is_not_env_expanded() {
        ...
        let key = "AINXT_HOOKS_PARSE_TEST_MATCHER_BRACED";
        with_env_var(key, Some("expanded_should_not_appear"), || {
            ...
            // Stored value must equal the source pattern unchanged.
    ...
    /// User attempts to set runner-reserved keys (AINXT_HOOK_*,
    /// AINXT_SESSION_ID, AINXT_WORKSPACE_ROOT, CLAUDE_PROJECT_DIR) via
                        ...
                        "hooks": [
                            {
                                ...
                                "command": "echo hi",
                                "env": {
                                    "AINXT_HOOK_EVENT": "spoofed",
                                    "AINXT_HOOK_NAME": "spoofed",
                                    "AINXT_SESSION_ID": "spoofed",
                                    "AINXT_WORKSPACE_ROOT": "/etc",
