...
//! are preserved unchanged. This is important so that:
//!
...
//! unchanged. These forms have shell-specific semantics (notably the
...
//! `ainxt_config::expand_env_vars_in_string`, but with an additional
...
const SENTINEL_PREFIX: &str = "\u{f8ff}__AINXT_HOOKS_MASK_";
...
/// modifier form survives expansion unchanged.
///
...
/// inputs and assert they are preserved unchanged.
...
///
/// Unresolved references are preserved unchanged so this function is safe
...
/// dispatcher's always-set `AINXT_HOOK_*` vars) survive the load-time pass.
///
/// Parameter-expansion-modifier forms (`${VAR:-x}`, `${VAR%pat}`, etc.)
/// are ALSO preserved unchanged; see the module-level rustdoc for why.
...
fn mask_modifier_forms(input: &str, sentinel: &str) -> String {
    ...
    for r in iter_env_var_references(input) {
        ...
        // start of string) and this one unchanged.
        if cursor < r.start {
        ...
        if r.braced && r.has_modifier {
            ...
            // through unchanged so shellexpand can resolve (or leave
            // unresolved).
...
///   the outer modifier body. This matches the legacy parser
...
mod tests {
    ...
    fn extra_takes_precedence_over_process_env() {
        with_env_var(
            "AINXT_HOOKS_ENV_EXPAND_TEST_PRECEDENCE",
            Some("from-process"),
            || {
                let mut extra = HashMap::new();
                extra.insert(
                    "AINXT_HOOKS_ENV_EXPAND_TEST_PRECEDENCE".to_string(),
                    "from-extra".to_string(),
                );
                let out =
                    expand_env_vars_with_extra("${AINXT_HOOKS_ENV_EXPAND_TEST_PRECEDENCE}", &extra);
    ...
    fn falls_back_to_process_env() {
        with_env_var(
            "AINXT_HOOKS_ENV_EXPAND_TEST_FALLBACK",
            Some("/from/proc/env"),
            || {
                let extra = HashMap::new();
                let out =
                    expand_env_vars_with_extra("${AINXT_HOOKS_ENV_EXPAND_TEST_FALLBACK}/x", &extra);
    ...
    fn preserves_unresolved_references() {
        ...
        with_env_var("AINXT_HOOKS_ENV_EXPAND_NEVER_SET", None, || {
            let extra = HashMap::new();
            let input = "${AINXT_HOOKS_ENV_EXPAND_NEVER_SET}/x.sh";
    ...
    /// `${VAR:-default}` must be preserved unchanged, even when `VAR` is
    ...
    fn preserves_default_modifier_when_var_unset() {
        let extra = HashMap::new();
        with_env_var("AINXT_HOOKS_ENV_EXPAND_MODIFIER_UNSET", None, || {
            let input = "${AINXT_HOOKS_ENV_EXPAND_MODIFIER_UNSET:-/default/path.sh}";
            let out = expand_env_vars_with_extra(input, &extra);
            assert_eq!(out, input);
        });
    }
    ...
    /// unchanged -- the shell's `:-` semantics differ from shellexpand's
    ...
    fn preserves_default_modifier_when_var_set() {
        let mut extra = HashMap::new();
        extra.insert(
            "AINXT_HOOKS_DEFAULT_SET".to_string(),
            "/from/extra".to_string(),
        );
        let input = "${AINXT_HOOKS_DEFAULT_SET:-/fallback}";
        let out = expand_env_vars_with_extra(input, &extra);
        assert_eq!(out, input);
    }
    ...
    fn preserves_no_colon_default_modifier() {
        let extra = HashMap::new();
        let input = "${AINXT_HOOKS_NCD-/fallback}";
        let out = expand_env_vars_with_extra(input, &extra);
        assert_eq!(out, input);
    }
    ...
    fn preserves_assignment_modifier() {
        let extra = HashMap::new();
        let input = "${AINXT_HOOKS_ASSIGN:=/assigned/path.sh}";
        let out = expand_env_vars_with_extra(input, &extra);
        assert_eq!(out, input);
    }
    ...
    fn preserves_error_modifier() {
        let extra = HashMap::new();
        let input = "${AINXT_HOOKS_ERR:?error message}";
        let out = expand_env_vars_with_extra(input, &extra);
        assert_eq!(out, input);
    }
    ...
    fn preserves_alternate_modifier() {
        let extra = HashMap::new();
        let input = "${AINXT_HOOKS_ALT:+/used/if/set}";
        let out = expand_env_vars_with_extra(input, &extra);
        assert_eq!(out, input);
    }
    ...
    fn preserves_suffix_strip_modifier() {
        let extra = HashMap::new();
        let input = "${AINXT_HOOKS_SUFFIX%.sh}";
        let out = expand_env_vars_with_extra(input, &extra);
        assert_eq!(out, input);
    }
    ...
    fn preserves_prefix_strip_modifier() {
        let extra = HashMap::new();
        let input = "${AINXT_HOOKS_PREFIX#prefix/}";
        let out = expand_env_vars_with_extra(input, &extra);
        assert_eq!(out, input);
    }
    ...
    fn preserves_substitution_modifier() {
        let extra = HashMap::new();
        let input = "${AINXT_HOOKS_SUB/foo/bar}";
        let out = expand_env_vars_with_extra(input, &extra);
        assert_eq!(out, input);
    }
    ...
    fn preserves_substring_modifier() {
        let extra = HashMap::new();
        let input = "${AINXT_HOOKS_SUBSTR:0:5}";
        let out = expand_env_vars_with_extra(input, &extra);
        assert_eq!(out, input);
    }
    ...
    fn mixed_plain_and_modifier_only_plain_expanded() {
        let mut extra = HashMap::new();
        extra.insert("AINXT_HOOKS_PLAIN".to_string(), "/usr/local".to_string());
        let input = "${AINXT_HOOKS_PLAIN}/${AINXT_HOOKS_DEFER:-/fallback}";
        let out = expand_env_vars_with_extra(input, &extra);
        assert_eq!(out, "/usr/local/${AINXT_HOOKS_DEFER:-/fallback}");
    }
    ...
    fn empty_extra_value_resolves_to_empty_for_plain_form() {
        let mut extra = HashMap::new();
        extra.insert("AINXT_HOOKS_EMPTY".to_string(), "".to_string());
        let out = expand_env_vars_with_extra("[${AINXT_HOOKS_EMPTY}]", &extra);
        assert_eq!(out, "[]");
    }

    /// When the var is set in `extra` but to the empty string, the
    /// modifier-form `${VAR:-default}` is preserved unchanged (so that
    ...
    fn empty_extra_value_does_not_trigger_default() {
        let mut extra = HashMap::new();
        extra.insert("AINXT_HOOKS_EMPTY_MOD".to_string(), "".to_string());
        let input = "${AINXT_HOOKS_EMPTY_MOD:-/fallback}";
        let out = expand_env_vars_with_extra(input, &extra);
        assert_eq!(out, input);
    }
    ...
    fn extra_values_are_not_recursively_expanded() {
        with_env_var(
            "AINXT_HOOKS_RECURSION_BAR",
            Some("should-not-appear"),
            || {
                let mut extra = HashMap::new();
                extra.insert(
                    "AINXT_HOOKS_RECURSION_FOO".to_string(),
                    "$AINXT_HOOKS_RECURSION_BAR".to_string(),
                );
                let out = expand_env_vars_with_extra("${AINXT_HOOKS_RECURSION_FOO}", &extra);
                assert_eq!(out, "$AINXT_HOOKS_RECURSION_BAR");
            },
        );
    }
    ...
    fn mask_helper_handles_unterminated_brace() {
        // No closing brace -- no masking, emit unchanged.
        assert_eq!(
    ...
    /// the unterminated tail is emitted unchanged because the walker
    ...
    fn expand_preserves_pre_existing_old_nul_sentinel_in_extra() {
        ...
        // Output must contain the literal NUL bytes unchanged, NOT
        // `${OLD}`.
    ...
    /// `"\u{f8ff}__AINXT_HOOKS_MASK__\u{f8ff}"`. A user-supplied
    ...
    /// through unchanged when it appears in an extra-env value, even
    ...
    fn expand_preserves_pre_existing_legacy_fixed_sentinel_in_extra() {
        let legacy_sentinel = "\u{f8ff}__AINXT_HOOKS_MASK__\u{f8ff}";
        let mut extra = HashMap::new();
        ...
        // unchanged -- it must NOT be rewritten to `${`.
        assert_eq!(
            out,
    ...
    /// must also pass through unchanged. (Sanity check that the
