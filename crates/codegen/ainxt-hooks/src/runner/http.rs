...
pub async fn run_http_hook(
    ...
    //
    // Unset refs are preserved unchanged, so `validate_hook_url` will
    ...
    // secrets like API tokens) don't land in `~/.ainxt/logs`. Falls
...
mod tests {
    ...
    fn url_extra_env_shadows_process_env() {
        let key = "AINXT_HOOKS_HTTP_TEST_SHADOW";
    ...
    async fn run_http_hook_scrubs_url_from_reqwest_error() {
        ...
        // Use a clearly synthetic value that does not match any real
        // credential format (no ghp_/sk-/etc. prefix) so secret-scanning
        // tools do not flag this test fixture as a leaked credential.
        let secret = "TEST_HOOK_SCRUB_SENTINEL_VALUE_XYZ";
        let mut extra_env = std::collections::HashMap::new();
        ...
        assert_eq!(
            info.url,
            "https://192.0.2.1/check?token=TEST_HOOK_SCRUB_SENTINEL_VALUE_XYZ"
        );
    ...
    /// unchanged and never issues a second request to the target.
    #[tokio::test]
    ...
    /// Unresolved `${VAR}` refs are preserved unchanged by the helper,
    ...
    async fn url_unresolved_var_fails_validation() {
        let key = "AINXT_HOOKS_HTTP_TEST_UNRESOLVED";
