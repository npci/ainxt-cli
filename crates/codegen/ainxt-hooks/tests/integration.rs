//! Integration tests for ainxt-hooks.
//!
...
use ainxt_hooks::discovery::load_hooks;
use ainxt_hooks::dispatcher;
use ainxt_hooks::event::*;
use ainxt_hooks::result::HookDecision;
use ainxt_hooks::runner::RunContext;
...
async fn hook_receives_env_vars() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$AINXT_HOOK_EVENT" > {f}; echo "NAME=$AINXT_HOOK_NAME" >> {f}; echo "SESSION=$AINXT_SESSION_ID" >> {f}; echo '{{"decision":"allow"}}'"#,
...
/// runner-reserved keys (`AINXT_HOOK_EVENT`, `AINXT_HOOK_NAME`,
/// `AINXT_SESSION_ID`, `AINXT_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`)
...
async fn runner_injected_vars_override_extra_env_at_spawn() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$AINXT_HOOK_EVENT" > {f}; echo "NAME=$AINXT_HOOK_NAME" >> {f}; echo "SESSION=$AINXT_SESSION_ID" >> {f}; echo "ROOT=$AINXT_WORKSPACE_ROOT" >> {f}; echo "PROJ=$CLAUDE_PROJECT_DIR" >> {f}; echo "USER_KEY=$USER_KEY" >> {f}; echo '{{"decision":"allow"}}'"#,
        f = output_file.display(),
    );
                    ...
                    "hooks": [
                        {
                            ...
                            "env": {
                                "AINXT_HOOK_EVENT": "spoofed_event",
                                "AINXT_HOOK_NAME": "spoofed_name",
                                "AINXT_SESSION_ID": "spoofed_session",
                                "AINXT_WORKSPACE_ROOT": "/spoofed/root",
    ...
    assert!(
        ...
        "AINXT_HOOK_EVENT must reflect the real event, got:\n{captured}"
    );
    assert!(
        !captured.contains("EVENT=spoofed_event"),
        "spoofed AINXT_HOOK_EVENT must NOT leak through, got:\n{captured}"
    );
    assert!(
        captured.contains(&format!("SESSION={real_session}")),
        "AINXT_SESSION_ID must reflect the real session, got:\n{captured}"
    );
    assert!(
        !captured.contains("SESSION=spoofed_session"),
        "spoofed AINXT_SESSION_ID must NOT leak through"
    );
    assert!(
        captured.contains(&format!("ROOT={real_workspace}")),
        "AINXT_WORKSPACE_ROOT must reflect the real workspace root, got:\n{captured}"
    );
    assert!(
        !captured.contains("ROOT=/spoofed/root"),
        "spoofed AINXT_WORKSPACE_ROOT must NOT leak through"
    );
    assert!(
...
async fn http_hook_url_env_expansion_end_to_end() {
    ...
    // raw_url matches the source string so wire-DTO
