...
Sample hooks for Ainxt. Copy to `~/.ainxt/hooks/` to enable globally, or to `<project>/.ainxt/hooks/` for project-scoped hooks (requires `/hooks-trust`).

> Every `cp` command below is relative to **this directory**. Run them from here:
> ```sh
> cd crates/codegen/ainxt-hooks/examples
> ```
...
**Install:**
```sh
mkdir -p ~/.ainxt/hooks/bin
cp hooks/safe-shell.json ~/.ainxt/hooks/
cp hooks/bin/safe-shell-guard.sh ~/.ainxt/hooks/bin/
chmod +x ~/.ainxt/hooks/bin/safe-shell-guard.sh
```
...
```sh
mkdir -p ~/.ainxt/hooks/bin
cp hooks/no-recursive-grep.json ~/.ainxt/hooks/
cp hooks/bin/no-recursive-grep-guard.py ~/.ainxt/hooks/bin/
chmod +x ~/.ainxt/hooks/bin/no-recursive-grep-guard.py
```
...
Appends session metadata to `~/.ainxt/session-audit.log` — event, session ID, cwd, timestamp.

**Install:**
```sh
mkdir -p ~/.ainxt/hooks/bin
cp hooks/session-log.json ~/.ainxt/hooks/
cp hooks/bin/session-log.sh ~/.ainxt/hooks/bin/
chmod +x ~/.ainxt/hooks/bin/session-log.sh
```
...
Logs all tool calls to `~/.ainxt/tool-activity.log` — tool name, event type, effective tool name, backgrounded status.

**Install:**
```sh
mkdir -p ~/.ainxt/hooks/bin
cp hooks/tool-logger.json ~/.ainxt/hooks/
cp hooks/bin/tool-logger.sh ~/.ainxt/hooks/bin/
chmod +x ~/.ainxt/hooks/bin/tool-logger.sh
```
...
- **Matcher:** regex on tool name. Claude names like `Bash`, `Read`, `Edit` are auto-expanded to also match Ainxt names (`run_terminal_cmd`, `read_file`, `search_replace`)
...
Remove the JSON file from `~/.ainxt/hooks/`. The hook stops running on the next session.