...
use ainxt_fsnotify::{FsConfig, FsEvent, FsEventKind, FsEventSource};
...
fn fake_sl_repo() -> TempDir {
    ...
    {
        use std::os::unix::fs::PermissionsExt;
        fs::set_permissions(&sl_dir, fs::Permissions::from_mode(0o700)).unwrap();
    }
...
async fn shared_dedupes_by_directory() {
    ...
    let Ok(a) = ainxt_fsnotify::shared(path.clone(), FsConfig::default()) else {
        eprintln!("skipping: OS watcher unavailable (resource limit?)");
        return;
    };
    let before = ainxt_fsnotify::stats();
    let b = ainxt_fsnotify::shared(path.clone(), FsConfig::default()).unwrap();
    ...
    let after = ainxt_fsnotify::stats();
    assert_eq!(
    ...
    let c = ainxt_fsnotify::shared(other.path().to_path_buf(), FsConfig::default()).unwrap();
    assert!(!Arc::ptr_eq(&a, &c), "different dirs must not share");
    assert_eq!(
        ainxt_fsnotify::stats().created_total - after.created_total,
        1,
    ...
    drop(a);
    drop(b);
    let d = ainxt_fsnotify::shared(path, FsConfig::default()).unwrap();
    assert_eq!(
        Arc::strong_count(&d),
        1,
...
///
/// ```bash
/// cargo test -p ainxt-fsnotify --test integration \
...
async fn shared_watcher_scaling_demo() {
    ...
    for _ in 0..SESSIONS {
        let Ok(src) = ainxt_fsnotify::shared(path.clone(), FsConfig::default()) else {
            eprintln!("skipping: OS watcher unavailable (resource limit?)");
            return;
        };
        handles.push(src);
