...
//! startup latency, under either strategy (`AINXT_FSNOTIFY_PER_DIR=0|1`).
//!
//! ```bash
//! # Generate a synthetic tree, then measure both strategies against it:
//! cargo run --release -p ainxt-fsnotify --example watch_stats -- gen js /tmp/js-repo
//! AINXT_FSNOTIFY_PER_DIR=0 cargo run --release -p ainxt-fsnotify --example watch_stats -- run /tmp/js-repo 5
//! AINXT_FSNOTIFY_PER_DIR=1 cargo run --release -p ainxt-fsnotify --example watch_stats -- run /tmp/js-repo 5
//! ```
//!
...
//!   "ainxt holds 55k inotify watches" report).
...
use ainxt_fsnotify::{FsConfig, FsEventSource};
...
fn main() {
    ...
    match args.get(1).map(String::as_str) {
        ...
        Some("run") => {
            ...
            let strategy = std::env::var("AINXT_FSNOTIFY_PER_DIR").unwrap_or_default();
