...
struct VcsDirs {
    ...
    /// Whether Sapling suppression is enabled (`AINXT_FSNOTIFY_SAPLING`).
    sapling: bool,
}
...
impl VcsDirs {
    ...
    /// True only for the exact VCS lock files that arm suppression (matches
...
fn read_sl_parent(sl_dir: &Path) -> Option<String> {
    ...
    // Explicitly release the file handle now that the fixed-size header has
    // been read, rather than relying solely on end-of-scope Drop.
    drop(f);
