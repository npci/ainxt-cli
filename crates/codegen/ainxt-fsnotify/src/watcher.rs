...
//!   nothing. `AINXT_FSNOTIFY_PER_DIR=1|0` overrides the platform default.
...
/// Whether Sapling (`.sl`) support is enabled (default on; `AINXT_FSNOTIFY_SAPLING=0`
...
pub(crate) fn sapling_enabled() -> bool {
    !matches!(
        std::env::var("AINXT_FSNOTIFY_SAPLING").ok().as_deref(),
        Some("0") | Some("false")
    )
}
...
/// Resolve the strategy: `AINXT_FSNOTIFY_PER_DIR=1|true` forces per-dir,
...
pub(crate) fn watch_strategy() -> WatchStrategy {
    match std::env::var("AINXT_FSNOTIFY_PER_DIR").ok().as_deref() {
...
/// Per-dir mode's total watch budget (`AINXT_FSNOTIFY_MAX_WATCHES` overrides).
///
...
pub(crate) fn max_watch_budget() -> usize {
    std::env::var("AINXT_FSNOTIFY_MAX_WATCHES")
        .ok()
                        ...
                        if dirs.len() > budget {
                            tracing::warn!(
                                "fs_notify: {} non-ignored dirs exceed watch budget {budget}; \
                                 shedding the deepest (raise with AINXT_FSNOTIFY_MAX_WATCHES)",
                                dirs.len()
                            );
...
mod tests {
    ...
    use ainxt_tracing_macros::teprintln;
    ...
    mod integration {
        ...
        fn test_debouncer_subdirectory() {
            ...
            // Create subdirectory with explicit, restrictive permissions
    ...
    mod config_tests {
        ...
        fn gitignore_cache_is_ignored_handles_sl_like_git() {
            ...
            // With watch_vcs off, even wlock is ignored (matches `.git`).
    ...
    mod helper_tests {
        ...
        fn external_ancestor_sl_arms_in_recursive_root_mode() {
            ...
            // (e.g. `ainxt` run in `crates/codegen`): the production guard must
        ...
        fn sapling_enabled_respects_kill_switch() {
            ...
            impl Drop for Restore {
                fn drop(&mut self) {
                    ...
                    unsafe { std::env::remove_var("AINXT_FSNOTIFY_SAPLING") };
                }
            }
            ...
            for off in ["0", "false"] {
                unsafe { std::env::set_var("AINXT_FSNOTIFY_SAPLING", off) };
                assert!(!sapling_enabled(), "{off:?} must disable Sapling");
            }
            unsafe { std::env::set_var("AINXT_FSNOTIFY_SAPLING", "1") };
