...
1. Call the `task` tool with:
   ...
   - `prompt`: copy the **VERIFIER PROMPT** section below unchanged. If a focus
...
6. BUILD AND TEST:
   Read the repo's AGENTS.md (the root file and any in the
...
8. REVIEW THE CODE:
   ...
   - Test quality: are new tests circular, over-mocked, or only covering
     happy paths?
   - Project-instruction compliance: where the repo's AGENTS.md
...
- Do not invent issues to fill space. If the work genuinely addresses the
  ...
  violations of rules explicitly stated in the repo's AGENTS.md
