//! Build script for bundling ripgrep for the ainxt-shell crate.
//!
//! - If `AINXT_SHELL_BUNDLE_RG_PATH` is set, always bundle it
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("cargo:rerun-if-env-changed=AINXT_SHELL_BUNDLE_RG_PATH");
    println!("cargo:rerun-if-env-changed=AINXT_SHELL_RG_DOWNLOAD_BASE");
    ...
    let path_override = env::var("AINXT_SHELL_BUNDLE_RG_PATH").ok();
    ...
    // AINXT_ROOT/target/tmp (which is read-only inside the sandbox). Outside
    // Bazel, prefer AINXT_ROOT's shared cache dir (monorepo behavior) and fall
    // back to OUT_DIR for standalone checkouts where AINXT_ROOT is not a thing.
    ...
    } else if let Ok(ainxt_root) = env::var("AINXT_ROOT") {
        PathBuf::from(ainxt_root).join("target/tmp/ainxt-shell-bundle-rg")
    } else {
        PathBuf::from(env::var("OUT_DIR")?)
    };
    ...
    // An explicit AINXT_SHELL_BUNDLE_RG_PATH still bundles on Windows (the
    ...
    println!("cargo:rustc-env=AINXT_SHELL_RG_VER={}", RG_VER);
    println!(
        "cargo:rustc-env=AINXT_SHELL_RG_GEN_DIR={}",
        gen_dir.display()
    );
    ...
    if let Some(path) = path_override {
        ...
        println!("cargo:rustc-env=AINXT_SHELL_RG_TARGET=override");
        let _ = fs::remove_file(&dest);
        fs::copy(PathBuf::from(path.clone()), &dest).map_err(|e| {
            format!(
                "Failed copying AINXT_SHELL_BUNDLE_RG_PATH: {e} from path {path} to dest {}",
                dest.display()
            )
        })?;
        return Ok(());
    ...
    let asset_triple = match (target_os.as_str(), target_arch.as_str()) {
        ...
        _ => {
            return Err(format!(
                "Unsupported target for ripgrep bundling: {os}-{arch}. Set AINXT_SHELL_BUNDLE_RG_PATH to a local rg binary for offline or unsupported builds.",
                os = target_os,
                arch = target_arch
            ).into());
        }
    ...
    println!("cargo:rustc-env=AINXT_SHELL_RG_TARGET={}", asset_triple);
    ...
    // internal mirror (e.g. AINXT_SHELL_RG_DOWNLOAD_BASE=http://<mirror>/github/
    // BurntSushi/ripgrep/releases/download). Defaults to the public GitHub
    // releases URL.
    let download_base = env::var("AINXT_SHELL_RG_DOWNLOAD_BASE")
    ...
    let bytes: Vec<u8> = {
        let resp = reqwest::blocking::get(&url).map_err(|e| {
            format!(
                "Failed to download ripgrep: {}\nSet AINXT_SHELL_BUNDLE_RG_PATH to a local rg for offline builds.",
                e
            )
        })?;
        if !resp.status().is_success() {
            return Err(format!(
                "HTTP {} downloading ripgrep. Set AINXT_SHELL_BUNDLE_RG_PATH for offline builds.",
                resp.status()
            )
            .into());
        }
    ...
    if !found {
        return Err(format!(
            "Could not find 'rg' in ripgrep archive {}. Set AINXT_SHELL_BUNDLE_RG_PATH for offline builds.",
            url
        )
        .into());
    }
