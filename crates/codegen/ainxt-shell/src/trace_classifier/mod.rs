...
pub struct TurnLine<'a> {
    ...
    /// Matches what the classifier saw in its `[runtime_state]` line
...
impl Summary {
    pub fn render(&self) -> String {
        // Lead with the common (non-action) class for equivalence with the
...
/// `merge=false`: replace the state entirely. Matches production's
/// `apply_replace`. Split out from the merge path to match the
/// two-function shape in `ainxt-tools` (F29).
...
/// `merge=true`: upsert by id. Matches `apply_merge` in
/// `ainxt-tools` — existing items keep their prior content when
...
/// binary. Wraps a `ainxt_sampler::SamplingClient` and pulls the
/// text content out of the response.
pub struct SamplerClassifierClient {
    inner: ainxt_sampler::SamplingClient,
}

impl SamplerClassifierClient {
    pub fn new(inner: ainxt_sampler::SamplingClient) -> Self {
        Self { inner }
    }
}
...
pub fn build_classifier_request(
    ...
    ConversationRequest {
        ...
        x_ainxt_conv_id: session_id_str.clone(),
        x_ainxt_req_id: Some(format!("{LAZINESS_REQ_ID_PREFIX}{}", uuid::Uuid::new_v4())),
        x_ainxt_session_id: session_id_str,
        x_ainxt_agent_id: Some(ainxt_telemetry::id::agent_id()),
...
pub struct RunArgs {
    ...
    /// ainxt-home directory containing the `auth.json` to consult as
    /// the third API-key fallback. Defaults to
    /// `ainxt_shell::util::ainxt_home::ainxt_home()` when `None`.
    /// Exposed as a CLI flag for testability.
    pub ainxt_home: Option<PathBuf>,
}
...
pub fn parse_trace_file(path: &Path) -> Result<Vec<TurnRecord>> {
    ...
    let result: Result<Vec<TurnRecord>> = serde_json::from_reader(&mut reader)
        .with_context(|| format!("parse trace {} as Vec<TurnRecord>", path.display()));
    // Explicitly release the underlying file handle now that parsing is
    // complete, rather than relying solely on end-of-scope Drop.
    drop(reader);
    result
}

/// Resolve the API key from `--api-key` → `$AINXT_API_KEY` →
...
/// provider command (if `AINXT_AUTH_PROVIDER_COMMAND` is set) is
...
/// the user can act on.
pub async fn resolve_api_key(explicit: Option<&str>, ainxt_home: &Path) -> Result<String> {
    if let Some(k) = explicit.map(str::trim).filter(|s| !s.is_empty()) {
        return Ok(k.to_owned());
    }
    let env_key = std::env::var("AINXT_API_KEY").ok();
    if let Some(k) = env_key.as_deref().map(str::trim).filter(|s| !s.is_empty()) {
        return Ok(k.to_owned());
    }
    if let Some(key) = non_interactive_auth_key(ainxt_home).await? {
        return Ok(key);
    }
    Err(anyhow!(
        "no API key: pass --api-key, set AINXT_API_KEY, or run `ainxt login` to populate \
         <ainxt-home>/auth.json. An expired OIDC token is auto-refreshed when a refresh_token \
         is present; if not, re-login is required."
    ))
}

/// Build an `AuthManager` for `ainxt_home`, install the
/// non-interactive refresher chain, and ask for a usable key. Matches
/// the body of [`crate::auth::try_ensure_fresh_auth`] but accepts an
/// explicit `ainxt_home` so `--ainxt-home` overrides work in test
/// fixtures (the upstream helper hardcodes the global `ainxt_home()`
/// path).
///
/// Returns:
...
async fn non_interactive_auth_key(ainxt_home: &Path) -> Result<Option<String>> {
    use crate::auth::{AuthError, AuthManager, AinxtComConfig};
    ...
    let config = AinxtComConfig::default();
    let auth_provider_command = config.auth_provider_command.clone();
    let manager = std::sync::Arc::new(AuthManager::new(ainxt_home, config));
    ...
    match manager.auth().await {
        ...
        Err(e) => Err(anyhow!(
            "auth.json refresh failed: {e}. Run `ainxt login` to re-authenticate, \
             or pass --api-key / set $AINXT_API_KEY to bypass auth.json."
        )),
    }
}
...
async fn build_sampler_client(
    ...
    model: String,
    api_key: Option<&str>,
    ainxt_home: Option<&Path>,
) -> Result<ainxt_sampler::SamplingClient> {
    let default_home;
    let ainxt_home_path: &Path = match ainxt_home {
        Some(p) => p,
        None => {
            default_home = crate::util::ainxt_home::ainxt_home();
            default_home.as_path()
        }
    };
    let resolved = resolve_api_key(api_key, ainxt_home_path).await?;
    let config = ainxt_sampler::SamplerConfig {
        api_key: Some(resolved),
        base_url,
        model,
        max_completion_tokens: Some(LAZINESS_MAX_OUTPUT_TOKENS),
        ..ainxt_sampler::SamplerConfig::default()
    };
    ainxt_sampler::SamplingClient::new(config).map_err(|e| anyhow!("build SamplingClient: {e}"))
}
...
pub async fn run_with_client(
    ...
    match output {
        Some(path) => {
            ...
            .await;
            // Explicitly release the underlying file handle now that all
            // output has been written, rather than relying solely on
            // end-of-scope Drop.
            drop(sink);
            result
        }
...
mod tests {
    use super::*;
    use ainxt_sampling_types::{AssistantItem, ToolCall, ToolResultItem};
    ...
    /// contains turn_0..N's items plus turn_N's own. This matches the
    ...
    async fn classifier_request_matches_production_shape() {
        ...
        assert_eq!(req.x_ainxt_conv_id.as_deref(), Some("sess-x"));
        assert_eq!(req.x_ainxt_session_id.as_deref(), Some("sess-x"));
        let req_id = req.x_ainxt_req_id.as_deref().expect("req id");
        ...
        assert_eq!(
            req.x_ainxt_agent_id.as_deref(),
            Some(ainxt_telemetry::id::agent_id().as_str())
        );
    }
    ...
    /// OIDC scope (the same scope `ainxt login` writes today and
    /// `AuthManager` reads). `auth_mode: api_key` skips the refresh
    /// path entirely — useful for "plain key, no refresh wanted"
    /// fixtures.
    fn write_auth_json(ainxt_home: &Path, key: &str) {
        let scope = crate::auth::AinxtComConfig::default().auth_scope();
        ...
        std::fs::write(ainxt_home.join("auth.json"), body.to_string()).expect("write auth.json");
    }
    ...
    fn write_empty_auth_json(ainxt_home: &Path) {
        std::fs::write(ainxt_home.join("auth.json"), "{}").expect("write auth.json");
    }
    ...
    fn write_fresh_oidc_auth_json(ainxt_home: &Path, key: &str) {
        let scope = crate::auth::AinxtComConfig::default().auth_scope();
        let body = serde_json::json!({
            scope: {
                "key": key,
    ...
    fn write_expired_oidc_auth_json_no_refresh(ainxt_home: &Path, key: &str) {
        let scope = crate::auth::AinxtComConfig::default().auth_scope();
        let body = serde_json::json!({
            scope: {
                "key": key,
    ...
    async fn resolve_api_key_precedence() {
        ...
        let ainxt_home = tmp.path();
        // No auth.json at all → AuthManager sees nothing on disk.
        write_empty_auth_json(ainxt_home);
        ...
        assert!(
            resolve_api_key(None, ainxt_home).await.is_err(),
            "no flag, no env, no auth.json → error",
        );
        assert!(
            resolve_api_key(Some(""), ainxt_home).await.is_err(),
            "empty flag → error",
        );
        assert!(
            resolve_api_key(Some("   "), ainxt_home).await.is_err(),
        ...
        assert_eq!(
            resolve_api_key(Some("from-flag"), ainxt_home)
                .await
                .expect("ok"),
            "from-flag",
        );

        unsafe { std::env::set_var("AINXT_API_KEY", "from-env") };
        assert_eq!(
            resolve_api_key(None, ainxt_home).await.expect("ok"),
            "from-env"
        );
        assert_eq!(
            resolve_api_key(Some("from-flag"), ainxt_home)
        ...
        assert_eq!(
            resolve_api_key(Some(""), ainxt_home).await.expect("ok"),
            "from-env",
        ...
        unsafe { std::env::set_var("AINXT_API_KEY", "   ") };
        assert!(
            resolve_api_key(None, ainxt_home).await.is_err(),
    ...
    async fn resolve_api_key_falls_back_to_auth_json() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let ainxt_home = tmp.path();
        write_auth_json(ainxt_home, "from-auth-json");
        // `with_no_env` returns sync; we re-implement the clearing
        // inline so we can `.await` the resolver.
        let key =
            with_env_isolated(async { resolve_api_key(None, ainxt_home).await.expect("ok") }).await;
    ...
    async fn resolve_api_key_skips_auth_json_when_env_provided() {
        ...
        let key = with_env_isolated(async {
            unsafe { std::env::set_var("AINXT_API_KEY", "from-env") };
            let resolved = resolve_api_key(None, ainxt_home).await.expect("ok");
    ...
    async fn resolve_api_key_errors_when_all_three_sources_missing() {
        ...
        let msg = with_env_isolated(async {
            let err = resolve_api_key(None, ainxt_home)
                .await
        ...
        assert!(
            msg.contains("--api-key")
                && msg.contains("AINXT_API_KEY")
                && msg.contains("ainxt login")
    ...
    async fn resolve_api_key_skips_empty_auth_json_entry() {
        ...
        // the unified error.
        write_auth_json(ainxt_home, "");
        let err1 = with_env_isolated(async { resolve_api_key(None, ainxt_home).await }).await;
        assert!(err1.is_err(), "empty auth.json entry is treated as absent");

        write_auth_json(ainxt_home, "   ");
        let err2 = with_env_isolated(async { resolve_api_key(None, ainxt_home).await }).await;
        assert!(
            err2.is_err(),
    ...
    async fn resolve_api_key_returns_refreshable_oidc_token() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let ainxt_home = tmp.path();
        write_fresh_oidc_auth_json(ainxt_home, "fresh-oidc-key");
        let key =
            with_env_isolated(async { resolve_api_key(None, ainxt_home).await.expect("ok") }).await;
    ...
    async fn resolve_api_key_returns_error_when_oidc_token_expired_and_no_refresh_token() {
        ...
        let tmp = tempfile::tempdir().expect("tempdir");
        let ainxt_home = tmp.path();
        write_expired_oidc_auth_json_no_refresh(ainxt_home, "stale-oidc-key");
        let err = with_env_isolated(async { resolve_api_key(None, ainxt_home).await }).await;
        assert!(
            err.is_err(),
    ...
    /// Matches `crate::auth::devbox_login::SA_TOKEN_PATH`. Tests that
    ...
    /// * `AINXT_API_KEY` — would return early from `resolve_api_key`.
    /// * `AINXT_AUTH` — inline-JSON credentials override that bypasses
    ///   the on-disk read entirely (`AuthManager::new`).
    /// * `AINXT_AUTH_PATH` — overrides the auth.json path; if set to
    ///   the operator's real `~/.ainxt/auth.json`, the test would read
    ///   live OIDC credentials instead of the scratch fixture.
    /// * `AINXT_AUTH_PROVIDER_COMMAND` — selects an external
    ...
    async fn run_rejects_missing_trace_path() {
        let prev = std::env::var("AINXT_API_KEY").ok();
        unsafe { std::env::remove_var("AINXT_API_KEY") };
        let tmp = tempfile::tempdir().expect("tempdir");
        let args = RunArgs {
            ...
            min_confidence: None,
            include_reasoning: None,
            ainxt_home: Some(tmp.path().to_path_buf()),
        };
        ...
        match prev {
            Some(v) => unsafe { std::env::set_var("AINXT_API_KEY", v) },
            None => unsafe { std::env::remove_var("AINXT_API_KEY") },
        }
    }
