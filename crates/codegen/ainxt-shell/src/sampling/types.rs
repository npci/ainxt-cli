// Re-export all types from the standalone ainxt-sampling-types crate.
// This keeps all existing `crate::sampling::types::*` imports working.
pub use ainxt_sampling_types::types::*;

// `CreateResponseWrapper` and `MessagesRequestWrapper` previously lived
// here. They were moved into `ainxt-sampling-types::types` (and
// are re-exported above via the wildcard) so the new
// `ainxt-sampler` crate can reference them without a circular
// dep on `ainxt-shell`.

// Tests for the types now live in ainxt-sampling-types crate.

use ainxt_tools::types::output::ImageContent as ToolsImageContent;
...
///
/// Lives in the shell (rather than `ainxt-sampling-types` or
/// `ainxt-tools`) so neither of those crates needs to depend on
