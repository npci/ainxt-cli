...
pub use self::types::*;
pub use ainxt_sampler::ApiBackend;
pub use ainxt_sampler::SamplingClient as Client;
pub use ainxt_sampler::{effective_api_backend, parse_api_backend, set_forced_api_backend};

// Re-export async-openai Responses API types under `rs` namespace
pub use async_openai::types::responses as rs;

// ---------------------------------------------------------------------------
// ainxt-sampler re-exports
// ---------------------------------------------------------------------------
//
// The actual streaming / retry / HTTP-client logic lives in the
// `ainxt-sampler` crate. We re-export the public surface here so
...
// `ainxt_sampler::*`. The shell-side `sampling::client::Config`
