...
pub(crate) struct RoleToolNames {
    ...
    /// (the default ainxt-build host's `search_replace` mutator) when `Write`
...
impl RoleToolNames {
    ...
    /// `search_replace` on the default ainxt-build host) instead of the literal
    ...
    pub(crate) fn from_summary(
        ...
        Self::from_parts(
            ...
            get(ToolKind::Search),
            // The default ainxt-build host's pre-spawn describe probe exposes only
...
/// `true` when `name` is safe to splice unchanged into an LLM prompt: a
...
pub(crate) mod tests {
    ...
    #[test]
    fn from_summary_uses_ainxt_build_names() {
        // A ainxt-build toolset: client names match the literal defaults.
    ...
    fn web_tools_fall_back_when_absent_from_the_toolset() {
        ...
        // on the default ainxt-build host (the stock `web_search`/`web_fetch`).
    ...
    #[test]
    fn from_summary_write_falls_back_to_edit_on_default_ainxt_build_host() {
        // Default ainxt-build host: the pre-spawn describe probe exposes only
    ...
    fn from_parent_write_falls_back_to_edit_when_bridge_has_no_write() {
        // Default ainxt-build parent bridge: no `Write`, only `Edit`
    ...
    #[test]
    fn parent_and_summary_renders_agree_on_default_ainxt_build_mutator() {
        ...
        // default ainxt-build host (Edit-only toolset), so a fail-open retry
    ...
    fn apply_is_single_pass_a_name_equal_to_a_token_is_not_re_expanded() {
        ...
        // `EXECUTE_TOOL` (no braces) is a safe name and is kept unchanged.
