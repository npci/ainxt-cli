use super::support::*;
use super::*;
use ainxt_tools::reminders::task_completion::consumed_completion_ids;
use ainxt_tools::types::output::{BashOutput, TextOutput, ToolOutput};
    ...
    local
        .run_until(async {
            let (gateway_tx, _) = tokio::sync::mpsc::unbounded_channel::<
                ainxt_acp_lib::AcpClientMessage,
            >();
...
/// though the helper unit-tests still pass. Matches the call shape
...
/// `TaskCompletionReminder` won't resurface it). Matches the resource access
/// in `SessionActor::mark_completions_reported`.
async fn already_reported(actor: &SessionActor, task_id: &str) -> bool {
    use ainxt_tools::reminders::task_completion::ReportedTaskCompletions;
...
async fn between_turn_drain_suppresses_auto_wake_delivered_subagents() {
    ...
    use ainxt_tools::reminders::task_completion::AutoWakeDeliveredIds;
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
            let (gateway_tx, _) =
...
struct OneTaskTerminal {
    tasks: Vec<ainxt_tools::computer::types::TaskSnapshot>,
}
#[async_trait::async_trait]
impl ainxt_tools::computer::types::TerminalBackend for OneTaskTerminal {
    async fn run(
        &self,
        _: ainxt_tools::computer::types::TerminalRunRequest,
    ) -> Result<
        ainxt_tools::computer::types::TerminalRunResult,
        ainxt_tools::computer::types::ComputerError,
    > {
        unimplemented!()
    }
    ...
    async fn get_task(&self, _: &str) -> Option<ainxt_tools::computer::types::TaskSnapshot> {
        None
    }
    async fn kill_task(&self, _: &str) -> ainxt_tools::computer::types::KillOutcome {
        ainxt_tools::computer::types::KillOutcome::NotFound
    }
    async fn wait_for_completion(
        &self,
        _: &str,
        _: Option<std::time::Duration>,
    ) -> Option<ainxt_tools::computer::types::TaskSnapshot> {
        None
    }
    async fn list_tasks(&self) -> Vec<ainxt_tools::computer::types::TaskSnapshot> {
        self.tasks.clone()
    }
}
fn completed_bash_task(id: &str) -> ainxt_tools::computer::types::TaskSnapshot {
    ainxt_tools::computer::types::TaskSnapshot {
        task_id: id.into(),
