...
#[cfg(test)]
pub(crate) fn noop_observability_bridge() -> ainxt_computer_hub_sdk::ObservabilityBridge {
    ainxt_computer_hub_sdk::ObservabilityBridge::new(
        None,
        ainxt_tool_protocol::SessionId::new("test").expect("valid"),
    )
}
#[cfg(test)]
pub(crate) async fn test_agent_default() -> ainxt_agent::Agent {
...
#[cfg(test)]
pub(crate) async fn test_agent_with_goal_tool() -> ainxt_agent::Agent {
    use ainxt_tools::implementations::ainxt_build::update_goal::UpdateGoalTool;
    use ainxt_tools::registry::types::ToolConfig;
    test_agent_with_tools(vec![ToolConfig::for_tool::<UpdateGoalTool>()]).await
}
/// ainxt-build agent with the real `TodoWriteTool` (id `todo_write`, kind
/// `Plan`) registered, so `tool_for_kind(ToolKind::Plan)` resolves through the
/// live toolset instead of the literal fallback.
#[cfg(test)]
pub(crate) async fn test_ainxt_build_agent_with_todo() -> ainxt_agent::Agent {
    use ainxt_tools::implementations::ainxt_build::todo::TodoWriteTool;
    use ainxt_tools::registry::types::ToolConfig;
...
#[cfg(test)]
pub(crate) async fn test_agent_with_plan_tools() -> ainxt_agent::Agent {
    use ainxt_tools::implementations::ainxt_build::enter_plan_mode::EnterPlanModeTool;
    use ainxt_tools::implementations::ainxt_build::exit_plan_mode::ExitPlanModeTool;
    use ainxt_tools::registry::types::ToolConfig;
...
pub(crate) async fn test_agent_with_tools(
    tools: Vec<ainxt_tools::registry::types::ToolConfig>,
) -> ainxt_agent::Agent {
    test_agent_from_config(
        ainxt_tools::registry::types::ToolServerConfig {
            tools,
            behavior_preset: None,
        },
        ainxt_agent::AgentDefinition::default_ainxt_build(),
        std::sync::Arc::new(ainxt_tools::computer::local::LocalTerminalBackend::new()),
    )
    .await
}
#[cfg(test)]
async fn test_agent_from_config(
    ...
    definition: ainxt_agent::AgentDefinition,
    backend: std::sync::Arc<dyn ainxt_tools::computer::types::TerminalBackend>,
) -> ainxt_agent::Agent {
    use ainxt_tools::computer::local::LocalFs;
    use ainxt_tools::computer::types::AsyncFileSystem;
    ...
    let ctx = SessionContext {
        ...
        session_folder: std::env::temp_dir().join("ainxt-test"),
    ...
    ainxt_agent::Agent::new(
        definition,
        ainxt_agent::PromptContext::default(),
        String::new(),
        tool_bridge,
        ainxt_agent::ReminderPolicy::default(),
        ainxt_agent::CompactionPolicy::default(),
        vec![],
        false,
    )
}
...
pub(crate) async fn create_test_actor_ex(
    ...
    gateway_tx: tokio::sync::mpsc::UnboundedSender<ainxt_acp_lib::AcpClientMessage>,
    persistence_tx: tokio::sync::mpsc::UnboundedSender<PersistenceMsg>,
) -> (
    SessionActor,
    tokio::sync::mpsc::UnboundedReceiver<SessionEvent>,
) {
    let cwd = ainxt_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap();
    let fs = Arc::new(ainxt_workspace::file_system::MockFs::new(
        cwd.to_path_buf(),
    ));
    ...
    let actor = SessionActor {
        ...
        permissions: ainxt_workspace::permission::PermissionHandle::allow_all(),
        tool_context,
