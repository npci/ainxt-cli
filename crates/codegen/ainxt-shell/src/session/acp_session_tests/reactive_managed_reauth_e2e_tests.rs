...
//! `ainxt.dev/mcp/server_status` pushes that clients consuming only `server_status`
...
//! handshake against rmcp 2.1 in `ainxt-mcp/tests/repro_sse_flood.rs`): a
...
use ainxt_mcp::servers::{ClientStateKind, HttpConfig, McpClient};

const MANAGED: &str = "ainxt_com_testconnector";
...
async fn actor_with_proxy(
    proxy_base: &str,
    gw_tx: tokio::sync::mpsc::UnboundedSender<ainxt_acp_lib::AcpClientMessage>,
...
/// Drain all `ainxt.dev/mcp/server_status` pushes currently queued on `gw_rx`.
...
/// non-auth `Ok(is_error)` body (e.g. a 403 policy denial) untouched. Matches
...
async fn entry_b_routes_auth_rejection_but_not_policy_denial() {
    use ainxt_mcp::servers::{is_auth_rejection_message, parse_mcp_tool_name};

    // Managed-prefix gate: only `ainxt_com_*` tools enter entry-B.
