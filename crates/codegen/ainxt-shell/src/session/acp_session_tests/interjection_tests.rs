...
mod interjection_broadcast_tests {
    ...
    /// `handle_interjection` depends on: method `ainxt.dev/session/interjection`
            ...
            .run_until(async {
                ...
                while let Ok(msg) = gateway_rx.try_recv() {
                    if let ainxt_acp_lib::AcpClientMessage::ExtNotification(args) = msg
                        && args.request.method.as_ref() == "ainxt.dev/session/interjection"
                    {
                        payload =
                ...
                let payload = payload.expect("an ainxt.dev/session/interjection broadcast");
                assert_eq!(
