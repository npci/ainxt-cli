...
//! notification the real `send_ainxt_notification` persists — not a
...
fn is_turn_completed(m: &PersistenceMsg) -> bool {
    matches!(
        m,
        PersistenceMsg::Update(crate::session::storage::SessionUpdate::Ainxt(n))
            if matches!(n.update, AinxtSessionUpdate::TurnCompleted { .. })
    )
}
...
fn turn_completed_fields(msgs: &[PersistenceMsg]) -> Option<(String, String, Option<String>)> {
    msgs.iter().find_map(|m| match m {
        PersistenceMsg::Update(crate::session::storage::SessionUpdate::Ainxt(n)) => match &n.update {
    ...
    local
        .run_until(async {
            ...
            *actor
            ...
            // before calling `handle_completion` (matches the Cancel/Shutdown
            ...
            // Limitation: this drives `handle_completion` directly and matches
            ...
            *actor
            ...
            while let Ok(msg) = gateway_rx.try_recv() {
                if let ainxt_acp_lib::AcpClientMessage::ExtNotification(args) = msg
                    && args.request.method.as_ref() == "ainxt.dev/session_notification"
                    && let Ok(v) =
