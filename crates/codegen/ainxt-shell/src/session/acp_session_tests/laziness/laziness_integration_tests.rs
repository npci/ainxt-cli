...
/// model ids) so the test entry matches a realistic catalog row.
fn detector_entry(
    enabled: bool,
    max_nudges: u32,
...
async fn idle_recheck_after_sleep_short_circuits_silently() {
    ...
    // mutation. Matches the real-world race the production code
    // must handle.
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
...
async fn laziness_abort_check_detects_bumps_between_snapshot_and_recheck() {
    ...
    local
        .run_until(async {
            ...
            // Invoke the helper UNDER the state lock — matches the
