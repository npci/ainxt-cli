...
//!
//! Tests mutate `AINXT_GOAL_CLASSIFIER` so they carry `serial`.
...
use ainxt_tools::implementations::ainxt_build::update_goal::UpdateGoalInput;
...
async fn answer_strategist(
    ...
    req: Box<ainxt_tools::implementations::ainxt_build::task::types::SubagentRequest>,
) {
    match behaviour {
...
async fn make_actor(
    ...
    actor.tool_context.cwd =
        ainxt_paths::AbsPathBuf::new(tmp.path().to_path_buf()).expect("abs cwd");
