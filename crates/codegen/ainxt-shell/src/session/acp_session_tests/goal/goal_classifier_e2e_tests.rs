...
use ainxt_tools::implementations::ainxt_build::update_goal::{RejectReason, UpdateGoalInput};
const ENV_FLAG: &str = "AINXT_GOAL_CLASSIFIER";
...
struct MockCoordinator {
    ...
    describe_outcome: StdArc<
        parking_lot::Mutex<
            ainxt_tools::implementations::ainxt_build::task::types::SubagentDescribeOutcome,
        >,
    >,
...
fn seed_channel(actor: &SessionActor, cmds: Vec<UpdateGoalInput>) {
    ...
    *actor.goal_update_rx.borrow_mut() = Some(rx);
    for cmd in cmds {
        tx.send(ainxt_tools::implementations::ainxt_build::update_goal::envelope_for_test(cmd))
            .unwrap();
    }
    drop(tx);
}
...
async fn goal_classifier_stall_early_exit_pauses_with_no_progress() {
    ...
    local
        .run_until(async {
            ...
            let (actor, tmp) = make_actor(Some(coord.tx.clone()), true).await;
            use ainxt_tools::implementations::ainxt_build::update_goal::UpdateGoalAck;
...
async fn goal_classifier_env_override_disables_when_remote_enabled() {
    ...
    assert!(
        !resolved.value,
        "AINXT_GOAL_CLASSIFIER=0 must disable even with remote=true",
    );
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
...
/// uses N=1 for spawn-count equivalence with the legacy single-classifier
...
async fn update_goal_tool_blocks_until_classifier_verdict_when_enabled() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                matches!(ack,
                ainxt_tools::implementations::ainxt_build::update_goal::UpdateGoalAck::ClassifierAchieved
                { .. },),
...
async fn update_goal_tool_returns_immediately_when_classifier_disabled() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                matches!(ack,
                ainxt_tools::implementations::ainxt_build::update_goal::UpdateGoalAck::CompletedWithoutClassifier,)
            );
        })
        .await;
}
...
async fn update_goal_tool_returns_immediately_for_blocked_reason() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                matches!(ack,
                ainxt_tools::implementations::ainxt_build::update_goal::UpdateGoalAck::Accepted
                { .. },)
            );
        })
        .await;
...
fn tool_error_code(err: &ainxt_tool_runtime::ToolError) -> &str {
    err.details
        .as_ref()
...
use ainxt_tools::implementations::ainxt_build::task::types::SubagentDescribeOutcome;
...
async fn resolve_role_override_toolset_incapable_fails_open() {
    ...
    local
        .run_until(async {
            let mut summary =
                ainxt_tools::implementations::ainxt_build::task::types::SubagentTypeSummary {
                    can_read: true,
                    ..Default::default()
                };
...
/// `ainxt-build-plan` (non-strict), and `opencode` (non-strict + unrepresentable
...
async fn resolve_role_override_harness_flavor_representability_gate() {
    ...
    local
        .run_until(async {
            ...
            let honored = ["ainxt-build-plan", "opencode"];
    ...
    local
        .run_until(async {
            ...
            actor.chat_state_handle.push_assistant_response(
                ainxt_sampling_types::conversation::ConversationItem::assistant(SUMMARY),
            );
            let _ = actor.run_verification_stage_for_drain(1, 3).await;
            assert_eq!(
                actor
...
async fn reverification_replays_anchor_and_does_not_overwrite_it() {
    ...
    local
        .run_until(async {
            ...
            actor.chat_state_handle.push_assistant_response(
                ainxt_sampling_types::conversation::ConversationItem::assistant(NOTE),
            );
