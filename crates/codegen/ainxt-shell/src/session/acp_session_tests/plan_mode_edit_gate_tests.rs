...
use super::support::*;
use super::*;
/// Build an actor whose toolset parses ainxt `search_replace` plus the plan
...
async fn build_gate_actor() -> SessionActor {
    ...
    *actor.agent.borrow_mut() = test_agent_with_tools(vec![
        ToolConfig::from_id("AinxtBuild:read_file"),
        ToolConfig::from_id("AinxtBuild:search_replace"),
...
async fn tool_result_text(actor: &SessionActor, call_id: &str) -> String {
    ...
    conv.iter()
        ...
        .find_map(|item| match item {
            ainxt_sampling_types::ConversationItem::ToolResult(tr)
...
/// worst case) still rejects a ainxt edit outside the plan file, without ever
/// reaching the permission layer, and steers the model to `exit_plan_mode`.
#[tokio::test(flavor = "current_thread")]
async fn plan_mode_rejects_ainxt_edit_outside_plan_file_despite_allow_all_permissions() {
