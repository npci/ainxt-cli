...
//! `ainxt-file-utils`. The orphan-rule items (`From<&permission::Decision>`
//! and the doom-loop categorizer) stay here since they need shell-local
//! types.

pub(crate) use ainxt_file_utils::events::tracker::EventTracker;
pub(crate) use ainxt_file_utils::events::types::{
...
///
/// Matches the JSON schema in the classifier prompt; `serde` uses the
...
/// pairing is one-to-one. Matches the `TodoGateReason` pattern.
...
) -> Option<ainxt_sampling_types::PriorTurnInterrupt> {
    use ainxt_sampling_types::PriorTurnInterrupt;
    match category {
...
// Compile-time non-empty guard (matches the laziness const block).
#[allow(clippy::const_is_empty)]
const _: () = assert!(
...
/// committing it would silently run ainxt-build flavor. Distinct from
...
impl GoalRoleModelFailOpenReason {
    ...
    /// until it is listed here (and given an `as_const_str` arm). Matches
...
/// Matches the `GoalPauseReason` → `GoalPauseReasonTelemetry` pattern;
...
/// (`ainxt-file-utils`). The two enums are intentional matches and live in
