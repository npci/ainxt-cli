...
use crate::extensions::notification::SessionUpdate as AinxtSessionUpdate;
use crate::extensions::notification::{
    RetryState, SessionNotification as AinxtSessionNotification, is_reauthable_failure,
};
...
use ainxt_agent::prompt::agents_md::LEGACY_AGENTS_MD_REMINDER_PREFIX;
use ainxt_agent::prompt::skills::SkillsConfig;
use ainxt_sampler::SamplerConfig as SamplingConfig;
use ainxt_sampling_types::truncate_bytes;
use ainxt_tools::computer::local::LocalTerminalBackend;
use ainxt_tools::implementations::BashToolInput;
use ainxt_tools::implementations::ainxt_build::web_fetch::WebFetchConfig;
use ainxt_tools::types::ToolInput;
use ainxt_tools::types::compat::CompatConfig;
use ainxt_tools::types::output::{
    BashOutput, ReadFileOutput, ToolOutput as ToolsToolOutput, ToolRunResult,
};
use ainxt_workspace::file_system::CodebaseIndexManager;
use ainxt_workspace::permission::{
    AccessKind, ClientType, Decision, PermissionEvent, PermissionHandle,
};
use ainxt_workspace::session::file_state::{FileStateHandle, FileStateTracker};
const SESSION_LOG: &str = "ainxt_session";
#[path = "compaction.rs"]
mod compaction;
...
pub(crate) use spawn::*;
/// Client-registered hook gates (the `ainxt.dev/hooks/run` reverse request).
mod hooks;
pub(crate) struct InputItem {
    ...
    /// See [`SessionCommand::Prompt::unchanged`].
...
/// because it embeds unchanged user prose that may contain `{...}`).
///
...
impl ainxt_tools::types::resources::ManagedGatewayToolCaller for ShellManagedGatewayToolClient {
    async fn call_tool(
        &self,
        call_id: &str,
        ...
        ainxt_tools::types::resources::ManagedGatewayToolCallResponse,
        ainxt_tool_runtime::ToolError,
    > {
        let auth_key = self
            .auth_manager
            .get_valid_token()
            ...
            .ok_or_else(|| ainxt_tool_runtime::ToolError::unauthorized("no auth token available"))?;
    ...
    match error {
        crate::session::managed_mcp::ManagedMcpFetchError::Status { status, message } => {
            ...
            let mut err = if status == reqwest::StatusCode::UNAUTHORIZED {
                ainxt_tool_runtime::ToolError::unauthorized(detail)
            } else if status == reqwest::StatusCode::FORBIDDEN {
                ainxt_tool_runtime::ToolError::permission_denied(detail)
            } else {
                let tool_id = ainxt_tool_protocol::ToolId::new(caller)
                    .unwrap_or_else(|_| ainxt_tool_protocol::ToolId::new("use_tool").expect("valid"));
                ainxt_tool_runtime::ToolError::execution(tool_id, detail)
            };
        ...
        crate::session::managed_mcp::ManagedMcpFetchError::Transport(e) => {
            ainxt_tool_runtime::ToolError::network_error(format!(
                "Managed MCP gateway tool call failed: {}",
                e.without_url()
            ))
        }
        crate::session::managed_mcp::ManagedMcpFetchError::NoAuth => {
            ainxt_tool_runtime::ToolError::unauthorized("no auth token available")
        }
    }
}
#[cfg(test)]
mod managed_gateway_error_tests {
    ...
    fn unauthorized_status_maps_to_unauthorized_and_carries_status() {
        ...
        assert_eq!(err.kind, ainxt_tool_runtime::ToolErrorKind::Unauthorized);
    ...
    fn forbidden_status_maps_to_permission_denied_and_carries_status() {
        ...
        assert_eq!(err.kind, ainxt_tool_runtime::ToolErrorKind::PermissionDenied);
        let details = err.details.as_ref().unwrap();
        assert_eq!(
            details.get(HTTP_STATUS_DETAILS_KEY),
    ...
    fn general_status_maps_to_execution_with_caller_tool_id() {
        ...
        assert_eq!(err.kind, ainxt_tool_runtime::ToolErrorKind::Execution);
        let details = err.details.as_ref().unwrap();
        assert_eq!(
            details.get(HTTP_STATUS_DETAILS_KEY),
    ...
    async fn transport_error_maps_to_network_error_without_url() {
        ...
        assert_eq!(err.kind, ainxt_tool_runtime::ToolErrorKind::NetworkError);
...
pub(crate) struct SessionActor {
    ...
    /// `ainxt-sampler`. Threaded into every `SamplerConfig`
    /// reconstructed by `reconstruct_full_config`. `None` when the
    /// session was spawned without an `AuthManager` (BYOK direct
    /// mode, test fixtures).
    pub(crate) attribution_callback: Option<ainxt_sampler::SharedAttributionCallback>,
    ...
    /// of truth (matches `goal_update_tx`), rather than re-resolving the config.
    ...
    pub(crate) chat_state_handle: ainxt_chat_state::ChatStateHandle,
    ...
    pub(crate) compactions_remaining:
        std::cell::Cell<Option<ainxt_sampling_types::CompactionsRemaining>>,
    pub(crate) compaction_at_tokens:
        std::cell::Cell<Option<ainxt_sampling_types::CompactionAtTokens>>,
    ...
    /// sampler itself sends the matching `x-ainxt-doom-loop-check` header.
    pub(crate) doom_loop_recovery: Option<ainxt_sampling_types::DoomLoopRecoveryPolicy>,
    ...
    /// Unchanged mirror-fork override: when `Some`, every turn sends this exact
    ...
    pub(crate) upload_queue:
        std::sync::Arc<std::sync::OnceLock<ainxt_file_utils::queue::UploadQueue>>,
    ...
    pub(crate) agent: std::cell::RefCell<ainxt_agent::Agent>,
    /// Dedup slot for `ainxt.dev/git_head_changed`, shared with the fs-watch
    /// `GitHead` consumer (see `git_head_dedup_key`).
    pub(crate) last_reported_branch: Arc<parking_lot::Mutex<Option<String>>>,
    /// Client opted into `ainxt.dev/gitHeadChanged`. When false (headless/SDK),
    ...
    /// Cached at actor construction (matches `goal_classifier_enabled`);
    ...
    /// Kill-switch (`AINXT_GOAL_USE_CURRENT_MODEL_ONLY` / `[features]
    ...
    pub(crate) pending_classifier_completions: parking_lot::Mutex<
        VecDeque<ainxt_tools::implementations::ainxt_build::update_goal::UpdateGoalInput>,
    >,
    ...
    pub(crate) mcp_announced_servers:
        Mutex<HashMap<String, ainxt_tools::implementations::search_tool::ServerFingerprint>>,
    ...
    pub(crate) extension_registry: ainxt_agent_lifecycle::LocalExtensionRegistry,
    ...
    pub(crate) hook_registry:
        std::cell::RefCell<Option<Arc<ainxt_hooks::discovery::HookRegistry>>>,
    /// Client hooks from `session/new` `_meta["ainxt.dev/hooks"]`; gated in
    ...
    pub(crate) plugin_registry:
        std::cell::RefCell<Option<std::sync::Arc<ainxt_agent::plugins::PluginRegistry>>>,
    /// Shared handle to the agent-level plugin registry.
    /// Used by `/plugins reload` to trigger a rebuild that new sessions see.
    pub(crate) plugin_registry_handle: Option<ainxt_agent::plugins::SharedPluginRegistryHandle>,
    ...
    pub(crate) observability_bridge: ainxt_computer_hub_sdk::ObservabilityBridge,
    ...
    /// Handle to the per-session `ainxt-sampler` actor.
    ///
    ...
    pub(crate) sampler_handle: ainxt_sampler::SamplerHandle,
    /// Cached recipe for constructing this session's [`ainxt_agent::Agent`].
    ///
    /// Populated once at session spawn and then reused by
    /// `handle_rebuild_agent_for_definition` to build a fresh `Agent`
    /// (system prompt, [`ainxt_tools::bridge::ToolBridge`], tool
    ...
    pub(crate) workspace_ops: ainxt_workspace::WorkspaceOps,
...
impl SessionActor {
    ...
    /// Fire-and-forget — failures are logged but do not interrupt the turn.
    async fn send_after_turn_event(&self, payload: ainxt_tool_protocol::turn_hook::AfterTurnPayload) {
        self.workspace_ops
    ...
    fn build_command_availability(
        ...
        slash_commands::CommandAvailability {
            ...
            scheduler: tool_names.iter().any(|n| {
                n == ainxt_tools::implementations::ainxt_build::SCHEDULER_CREATE_TOOL_NAME
            }),
    ...
    async fn send_feedback_notification(&self, request: crate::session::feedback::FeedbackRequest) {
        self.send_ainxt_notification(AinxtSessionUpdate::FeedbackRequest(request.into()))
            .await;
    }
}
impl SessionActor {
    ...
    fn tool_bridge_handle(&self) -> Arc<ainxt_tools::bridge::ToolBridge> {
...
/// The saved JSON enables deterministic re-rendering, `ainxt prompt --json`
/// inspection, and post-hoc debugging of what went into a session's system prompt.
fn save_prompt_context(session_info: &SessionInfo, prompt_context: &ainxt_agent::PromptContext) {
...
) -> Option<ainxt_agent::PromptContext> {
    let dir = crate::session::persistence::session_dir(session_info);
...
mod managed_gateway_descriptor_tests {
    use super::*;
    use ainxt_tools::types::output::{MCPOutput, ToolOutput};
    use ainxt_tools::types::tool::{ToolKind, ToolNamespace};
    #[derive(Debug, Default)]
    struct FixtureMcpTool;
    impl ainxt_tools::types::tool_metadata::ToolMetadata for FixtureMcpTool {
    ...
    impl ainxt_tool_runtime::Tool for FixtureMcpTool {
        type Args = serde_json::Value;
        type Output = ToolOutput;
        fn id(&self) -> ainxt_tool_protocol::ToolId {
            ainxt_tool_protocol::ToolId::new("server__tool").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::ainxt_tool_runtime::ListToolsContext,
        ) -> ainxt_tool_types::ToolDescription {
            ainxt_tool_types::ToolDescription::new("server__tool", "fixture")
        }
        async fn run(
            &self,
            _ctx: ainxt_tool_runtime::ToolCallContext,
            _args: serde_json::Value,
        ) -> Result<ToolOutput, ainxt_tool_runtime::ToolError> {
...
// NOTE: `acp_session_tests/rewrite_zero_turn_prefix_tests.rs` does not exist. The module was
// declared with a `#[path]` to a file that is not in the tree, so `cargo check --all-targets`
// and `cargo test` failed to build this crate at all — taking every other test module in it
// down too. Declaration removed so the suite compiles. `Self::rewrite_zero_turn_prefix`
// (session/acp_session_impl/prompt_build.rs) is therefore UNTESTED; re-add coverage here.
...
mod tool_meta_stamp_tests {
    //! Pin the `ainxt.dev/tool` stamps on the harness emission paths: the early
    ...
    use super::*;
    use tokio::sync::mpsc;
    use ainxt_tools::registry::types::ToolConfig;
    use ainxt_tools::tool_taxonomy::TOOL_META_KEY;
    use ainxt_workspace::permission::PermissionCommand;
    ...
    /// The `ainxt.dev/tool` object from an event's `_meta`, if present.
            ...
            .run_until(async {
                ...
                fixture.actor.agent = std::cell::RefCell::new(
                    test_agent_with_tools(vec![ToolConfig::from_id(
                        "AinxtBuild:read_file".to_string(),
                    )])
                    .await,
                );
                ...
                let t = tool_meta(early.as_ref()).expect("early ToolCall carries ainxt.dev/tool");
                assert_eq!(t["name"], "read_file");
                assert_eq!(t["kind"], "read");
                assert_eq!(t["namespace"], "ainxt_build");
                assert!(t.get("input").is_none(), "identity-only before parse");
                let refined = refined.expect("refinement ToolCallUpdate emitted");
                let t = tool_meta(refined.as_ref()).expect("refinement carries ainxt.dev/tool");
        ...
        local
            .run_until(async {
                ...
                let t = tool_meta(update.meta.as_ref())
                    .expect("permission-request ToolCallUpdate carries ainxt.dev/tool");
                assert_eq!(t["name"], "read_file");
                assert_eq!(t["kind"], "read");
                assert_eq!(t["input"]["path"], "/tmp/stamp.txt");
            })
...
// NOTE: `acp_session_tests/cancel_running_task_tests.rs` does not exist either — same problem,
// same fix. Cancel-running-task behaviour is UNTESTED; re-add coverage here.
#[cfg(test)]
...
mod managed_gateway_tool_tests {
    ...
    use ainxt_tools::types::output::{MCPOutput, ToolOutput};
    use ainxt_tools::types::tool::{ToolKind, ToolNamespace};
    use ainxt_tools::types::tool_metadata::ToolMetadata;
    #[derive(Debug)]
    struct FixtureMcpTool;
    ...
    async fn refresh_snapshot_seeds_only_admitted_gateway_catalog_entries() {
        ...
        let catalog = bridge
            .read_resource::<ainxt_tools::types::resources::ManagedGatewayToolCatalog>()
            .await
