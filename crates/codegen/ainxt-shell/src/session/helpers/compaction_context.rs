...
//! shell-specific types (`ainxt_tools::MemoryBackend`, memory context).
//!
//! The core [`CompactionStateContext`] struct and its builder live in
//! `ainxt_chat_state::compaction_utils`. This module adds system-reminder
//! rendering that requires dependencies not available in `ainxt-chat-state`.
//!
//! The three **common** active-agent sections (background tasks, TODO list,
//! running subagents) are formatted by
//! [`ainxt_compaction::reminder`] so ainxt-chat and ainxt-build stay in lockstep.
...
pub fn to_system_reminder_sync(
    ...
    skills: &[ainxt_tools::implementations::skills::types::SkillInfo],
...
pub async fn to_system_reminder(
    ...
    discovered_agents_md: &[PathBuf],
    skills: &[ainxt_tools::implementations::skills::types::SkillInfo],
    memory_backend: Option<&dyn ainxt_tools::types::memory_backend::MemoryBackend>,
    subagent_tool_names: Option<&SubagentToolNames>,
    mcp_tool_names: Option<&McpToolNames>,
) -> Option<String> {
...
fn to_system_reminder_inner(
    ...
    discovered_agents_md: &[PathBuf],
    skills: &[ainxt_tools::implementations::skills::types::SkillInfo],
    memory_results: &[ainxt_tools::types::memory_backend::MemorySearchResult],
    subagent_tool_names: Option<&SubagentToolNames>,
    mcp_tool_names: Option<&McpToolNames>,
) -> Option<String> {
    ...
    // Skills are intentionally NOT re-listed post-compaction. The listing
    // used to cost tokens on every post-compaction turn for skills the model
    // rarely invoked unprompted; skills remain fully usable via slash
    // commands (`bridge.slash_skills()`) and direct invocation by name. The
    // `skills` parameter is kept (rather than removed) so callers don't need
    // to change and a future config toggle could re-enable this section.
    let _ = skills;
    ...
    if !ctx.connected_mcp_servers.is_empty() {
        use ainxt_tools::implementations::search_tool::format_compaction_server_line;
...
- linear (12 tools)
    ...
    /// unchanged. A fabricated `task-` prefix produces an ID that does not
    ...
    /// render unchanged with id + status; completed/cancelled collapse to counts.
    #[test]
    fn system_reminder_includes_active_todos() {
        ...
        // Done/cancelled items are summarized, not listed unchanged.
