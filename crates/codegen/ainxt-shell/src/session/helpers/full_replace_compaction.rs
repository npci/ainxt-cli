//! ainxt-build's L5 wiring onto the shared full-replace engine
//! (`ainxt_compaction::code_compaction`).
//!
//! The shared engine drives the sample → retry → degenerate/failure
//! classification loop via [`sample_full_replace_summary`](ainxt_compaction::sample_full_replace_summary);
//! this module adapts ainxt-build's transport and telemetry to its two seams:
//!
...
//!
//! The unchanged → fitted → lossy **input ladder** and auto-compaction
//! suppression stay in L5 (`compaction.rs`), driven by the
//! `context_overflow` / `deterministic` flags on
//! [`FullReplaceError`](ainxt_compaction::FullReplaceError).
...
use ainxt_sampling_types::{ConversationItem, HostedTool, ToolSpec};
use ainxt_telemetry::events::{CompactionRetryDegraded, CompactionTrigger};
...
/// [`CompactionSampler`] for ainxt-build's full-replace pass.
///
...
/// else the structured ainxt-build prompt), so the shared `CompactionPrompt`
/// the engine passes is ignored — the engine builds the ainxt-build prompt,
...
impl CompactionSampler for ShellCompactionSampler {
    ...
    async fn sample_compaction(
        ...
        // message (compat short vs structured ainxt-build), ignoring the shared
...
/// Map ainxt-build's [`CompactFailure`] onto the shared engine's
...
/// [`FullReplaceObserver`] that reproduces ainxt-build's per-attempt telemetry:
    ...
    fn on_attempt(&self, _attempt: u32, outcome: &FullReplaceAttemptOutcome<'_>) {
        ...
        match outcome {
            ...
            FullReplaceAttemptOutcome::Degenerate {
                ...
                if *will_retry {
                    ainxt_telemetry::session_ctx::log_event(CompactionRetryDegraded {
                    ...
                    tracing::warn!(
                        session_tag = %crate::util::session_log_tag(&self.session_id),
                        attempt,
                        summary_chars,
