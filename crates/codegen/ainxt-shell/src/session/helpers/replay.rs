...
/// compaction checkpoint file path derived from on-disk session state) in a
/// log record. Values are filtered character-by-character at format time via
/// `Display::fmt`, rather than materialized into a separate sanitized
/// `String` ahead of the call, so a log site can never accidentally
/// interpolate the raw value instead of the sanitized one.
...
pub fn find_latest_compaction_checkpoint(
    ...
    for line in raw_contents.lines() {
        ...
        if env.method != Some("_ainxt.dev/session/update") {
            continue;
        }
        ...
        >(raw_params.get())
            && let AinxtSessionUpdate::CompactionCheckpoint(info) = notification.update
        {
            latest = Some(*info);
        }
    }
        ...
        match update {
            SessionUpdate::Ainxt(notification) => {
                match &notification.update {
                    AinxtSessionUpdate::CompactionCheckpoint(info) => {
                        return self.handle_checkpoint(info, session_dir);
                    }
                    AinxtSessionUpdate::RewindMarker {
                        target_prompt_index,
                        ..
                    } => {
                    ...
                    // Other ainxt notifications are informational — skip them.
                    _ => {}
                }
            }
        ...
        if self.target < info.prompt_index_at_compaction {
            ...
            let bytes = match std::fs::read(&checkpoint_path) {
                ...
                Err(e) if e.kind() == io::ErrorKind::NotFound => {
                    tracing::error!(
                        path = %LogSafe(&checkpoint_path.display().to_string()),
...
mod tests {
    ...
    fn make_rewind_marker(target: usize) -> SessionUpdate {
        SessionUpdate::Ainxt(Box::new(AinxtNotification {
    ...
    fn make_checkpoint(
        ...
        SessionUpdate::Ainxt(Box::new(AinxtNotification {
            session_id: acp::SessionId::new("test"),
            update: AinxtSessionUpdate::CompactionCheckpoint(Box::new(CompactionCheckpointInfo {
