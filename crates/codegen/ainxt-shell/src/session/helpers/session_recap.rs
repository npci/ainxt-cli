...
//!
//! Generation reuses the parent session's conversation prefix unchanged (so the
...
use ainxt_chat_state::{compaction_utils, estimate_conversation_tokens, estimate_item_tokens};
...
/// `conversation[0]` — is reused unchanged and the prompt cache stays warm.
///
...
///    (ainxt/SGLang via ChatCompletions/Responses) keeps reasoning VERBATIM so
...
///    append the instruction after the prefix. Matches compaction's
...
pub(crate) fn build_recap_items(
    ...
    let mut items = if strip_reasoning {
        ainxt_chat_state::compaction_utils::strip_reasoning_blocks(conversation)
    } else {
        conversation
    };
...
/// `max_prompt_length` for current `ainxt-build` / `ainxt-4.5` product backends
...
/// Estimator/serialization slack (matches memory-flush's soft-threshold pad). The
...
/// that unlikely for normal ainxt-build sessions).
///
/// * Fast path — if the whole snapshot already fits, returns
///   `build_recap_items(...)` unchanged (keeps the ainxt prefix KV cache warm;
...
pub(crate) fn budget_recap_items(
    ...
    // unchanged path keeps the ainxt prefix cache warm.
...
mod tests {
    ...
    fn clean_keeps_normal_recap_in_full() {
        ...
        // returned unchanged — never cut mid-sentence.
    ...
    fn build_appends_instruction_user_turn() {
        ...
        // System prompt prefix is preserved unchanged for cache reuse.
    ...
    fn main_turn_count_counts_real_users_only() {
        use std::sync::Arc;
        use ainxt_sampling_types::{ContentPart, SyntheticReason, ToolCall, UserItem};
    ...
    fn mk_tool_call(id: &str, args: &str) -> ainxt_sampling_types::ToolCall {
        use std::sync::Arc;
    ...
    #[test]
    fn budget_over_budget_strips_reasoning_even_on_ainxt() {
        let conv = vec![
            mk_reasoning("r1"),
            ConversationItem::assistant("did stuff"),
            ConversationItem::user("z".repeat(40_000)),
        ];
        // ainxt backend => strip_reasoning=false, but the over-budget branch must
    ...
    #[test]
    fn budget_fast_path_keeps_reasoning_on_ainxt() {
        let conv = vec![
            mk_reasoning("r1"),
            ConversationItem::assistant("did stuff"),
            ConversationItem::user("small"),
        ];
        // Fits under a large window on ainxt (strip_reasoning=false) => unchanged,
        // reasoning kept so the prefix KV cache stays warm.
        let out = budget_recap_items(conv, "system-reminder", false, 256_000);
        assert!(
            out.iter()
                .any(|i| matches!(i, ConversationItem::Reasoning(_))),
            "fits path on ainxt must keep reasoning verbatim"
        );
    }
    ...
    fn budget_threshold_boundary_selects_fast_vs_over_budget() {
        ...
        // Exactly at budget => fast path (`<=`) keeps reasoning unchanged.
        let at = vec![
            mk_reasoning("r"),
