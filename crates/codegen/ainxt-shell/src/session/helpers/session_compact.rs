...
pub use ainxt_chat_state::compaction_utils::{
...
///
/// All other agents (ainxt-build, etc.) continue to use the detailed
...
pub(crate) use ainxt_sampling_types::is_context_length_error;
...
/// engine reuses the session's KV cache (the whole point of the unchanged
...
pub(crate) async fn generate_session_compact(
    ...
    // Use the same backend the client will dispatch a turn on, so a compaction
    // mid-loop can't hit a different endpoint than the turns around it.
    let output = match client.api_backend() {
        ApiBackend::ChatCompletions => {
            ...
            message.x_ainxt_conv_id = Some(sid.clone());
            message.x_ainxt_req_id = Some(format!("ainxt-compact-{}", uuid::Uuid::new_v4()));
            message.x_ainxt_session_id = Some(sid);
            message.x_ainxt_agent_id = Some(ainxt_telemetry::id::agent_id());
            tracing::info!(
                    ...
                    Ok(chunk) => {
                        if let Some(choice) = chunk.choices.first() {
                            ...
                            if let Some(fr) = choice.finish_reason {
                                let sr = ainxt_sampling_types::StopReason::from(fr);
                                truncated =
                                    matches!(sr, ainxt_sampling_types::StopReason::Length);
        ...
        ApiBackend::Responses => {
            let request = ConversationRequest {
                ...
                x_ainxt_conv_id: Some(session_id.to_string()),
                x_ainxt_req_id: Some(format!("ainxt-compact-{}", uuid::Uuid::new_v4())),
                x_ainxt_session_id: Some(session_id.to_string()),
            ...
            loop {
                ...
                match chunk_result {
                    Ok(event) => {
                        if !matches!(
                            &event,
                            ainxt_sampling_types::messages::MessageStreamEvent::Ping
                        ) {
                            last_progress_at = std::time::Instant::now();
                        }
                        match event {
                            ainxt_sampling_types::messages::MessageStreamEvent::ContentBlockDelta {
                                delta: ainxt_sampling_types::messages::StreamDelta::TextDelta {
                                    text,
                                },
                                ..
                            } => {
                            ...
                            ainxt_sampling_types::messages::MessageStreamEvent::MessageDelta {
                                delta,
                                ..
                            } => {
                                if let Some(sr) = delta.stop_reason {
                                    truncated = matches!(
                                        sr, ainxt_sampling_types::messages::StopReason::MaxTokens
                                        |
                                        ainxt_sampling_types::messages::StopReason::ModelContextWindowExceeded
                                    );
                                    stop_reason = Some(
                                        match sr {
                                            ainxt_sampling_types::messages::StopReason::EndTurn => {
                                                "end_turn".to_string()
                                            }
                                            ainxt_sampling_types::messages::StopReason::MaxTokens => {
                                                "max_tokens".to_string()
                                            }
                                            ainxt_sampling_types::messages::StopReason::ToolUse => {
                                                "tool_use".to_string()
                                            }
                                            ainxt_sampling_types::messages::StopReason::StopSequence => {
                                                "stop_sequence".to_string()
                                            }
                                            ainxt_sampling_types::messages::StopReason::Refusal => {
                                                "refusal".to_string()
                                            }
                                            ainxt_sampling_types::messages::StopReason::PauseTurn => {
                                                "pause_turn".to_string()
                                            }
                                            ainxt_sampling_types::messages::StopReason::ModelContextWindowExceeded => {
                                                "model_context_window_exceeded".to_string()
                                            }
                                            ainxt_sampling_types::messages::StopReason::Unknown(
                                                s,
                                            ) => s,
                                        },
                                    );
...
mod compacted_history_shape_tests {
    ...
    /// `ainxt-chat-state`, rendering the system-reminder synchronously (no
    ...
    async fn test_compacted_history_raw_strings() {
        ...
        let formatted_summary =
            ainxt_chat_state::compaction_utils::format_compact_summary_content(compaction_summary);
        assert_eq!(
    ...
    /// Regression guard: ainxt-build must DROP the working
    /// tail post-compaction. A prior change routed ainxt-build to keep `recent_messages`,
    /// which survive only as `Tool call omitted...` stubs (dead tokens). Matches
    /// `summary_before_recent_compaction_with_no_user_query_yields_three_messages` for ainxt-build
    /// (`summary_before_recent = false`).
    #[tokio::test]
    async fn ainxt_build_compaction_drops_working_tail_regression_206460() {
        let conversation = vec![
            ConversationItem::system("You are a helpful assistant."),
            ConversationItem::user(
                "<user_info>\nOS: macos\n</user_info>\n\n<user_query>\nread auth.rs\n</user_query>",
        ...
        assert!(
            ...
            "ainxt-build must drop recent_messages post-compaction",
        );
        let compacted = build_compacted_history(
            "You are a helpful assistant.",
            "<user_info>\nOS: macos\n</user_info>",
        ...
        assert!(
            !compacted
                ...
                .any(|i| matches!(i, ConversationItem::ToolResult(_))
                    || i.text_content() == "Tool call omitted..."),
            "no tail (ToolResult or stub) may leak into the ainxt-build compacted history",
        );
    }
    ...
    fn fallback_minimal_history_has_no_tool_results() {
        use ainxt_chat_state::compaction_utils::validate_compacted_history;
...
mod reasoning_compaction_regression_tests {
    ...
    fn test_config(base_url: &str) -> SamplerConfig {
        SamplerConfig {
            ...
            // Without this, SamplerClient::api_backend() ignores api_backend
            // above and always falls back to Messages (see its doc comment),
            // so every request here 404s against the mock server's
            // registered /v1/chat/completions route.
