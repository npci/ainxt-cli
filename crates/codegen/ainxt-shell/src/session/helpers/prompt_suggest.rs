...
/// prediction.
pub(crate) const DEFAULT_SUGGEST_MODEL: &str = "ainxt-build-0.1";
...
/// (`ainxt-build-0.1`) is API-key-only and excluded from OAuth catalogs, so
...
/// `AINXT_PROMPT_SUGGESTIONS_MODEL` keeps working for models the catalog does
/// not list (matches the pager, which forwards the env value unchecked).
...
/// Reject suggestions with more words than this (matches common
...
/// output, markdown, or over-long text. Matches typical coding-agent suggestion
...
mod tests {
    ...
    fn effective_model_default_requires_catalog() {
        ...
        // OAuth catalogs exclude ainxt-build-0.1 → skip the request entirely,
    ...
    fn effective_model_env_pin_bypasses_catalog_guard() {
        // AINXT_PROMPT_SUGGESTIONS_MODEL is the explicit escape hatch: used
        // unchanged even when the catalog does not list the model (matches
