...
use crate::extensions::notification::{
    SessionNotification as AinxtSessionNotification, SessionUpdate as AinxtSessionUpdate,
};
...
impl GoalNotifySender {
    ...
    /// self-heals the live figure on the next tick. Matches the gateway-only
    ...
    /// latch updates that must not run the `send_ainxt_notification`
    /// rewind-window-close side effect.
    pub(crate) fn send_update(&self, update: AinxtSessionUpdate) {
    ...
    fn dispatch_update(&self, update: AinxtSessionUpdate, persist: bool) {
...
pub(crate) fn build_goal_updated(
    ...
    AinxtSessionUpdate::GoalUpdated {
        ...
        // happened yet — matches the `total_worker_rounds`-style
...
/// drop its goal state.
pub(crate) fn build_goal_cleared() -> AinxtSessionUpdate {
    AinxtSessionUpdate::GoalUpdated {
        goal_id: String::new(),
...
mod tests {
    ...
    fn planning_field(update: &AinxtSessionUpdate) -> Option<bool> {
        match update {
            AinxtSessionUpdate::GoalUpdated { planning, .. } => *planning,
            _ => panic!("expected GoalUpdated"),
        }
    }
    ...
    fn build_goal_updated_verifying_flag_reflects_latch() {
        ...
        let mut o = make_base_orchestration();
        let verifying = |u: &AinxtSessionUpdate| match u {
            AinxtSessionUpdate::GoalUpdated {
                verifying_completion,
                ..
            } => *verifying_completion,
    ...
    fn build_goal_updated_gates_per_model_breakdown_on_two_or_more_models() {
        ...
        let mut o = make_base_orchestration();
        o.live_tokens_by_model = vec![("ainxt-4".into(), 5_000)];
        ...
        // ≥2 distinct models are transmitted unchanged.
        o.live_tokens_by_model = vec![("ainxt-4".into(), 5_000), ("ainxt-3".into(), 3_000)];
        match build_goal_updated(&o, 0, 0) {
            AinxtSessionUpdate::GoalUpdated {
                live_tokens_by_model,
                ..
                ...
                live_tokens_by_model,
                vec![("ainxt-4".to_owned(), 5_000), ("ainxt-3".to_owned(), 3_000)]
            ),
            _ => panic!("expected GoalUpdated"),
        }
    }
