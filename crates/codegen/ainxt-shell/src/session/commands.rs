...
pub enum SessionCommand {
    ...
    SetSessionModel {
        sampling_config: ainxt_sampler::SamplerConfig,
        use_concise: bool,
    ...
    /// rather than the agent-level default (e.g. `ainxt-4.5`).
    ///
    /// Keeps the existing base_url, api_key, and other config — only changes
    /// the `model` field sent in the `x-ainxt-model-override` header and merges
    ...
    GetModelMetadata {
        responds_to: oneshot::Sender<ainxt_chat_state::ModelMetadata>,
    },
    /// Snapshot for `/session-info`.
    GetSessionInfo {
        responds_to: oneshot::Sender<SessionInfoData>,
    },
    /// Snapshot for `/stats`: the session's token/cost usage ledger plus the
    /// active model id, so the pager can render a model+tokens+cost summary
    /// without a round trip through the ainxt.dev backend (unlike `/usage`,
    /// which is a platform billing/credit-limit summary — see
    /// `crate::extensions::billing`).
    GetSessionUsage {
        responds_to: oneshot::Sender<SessionUsageData>,
    },
    ...
    ReloadPlugins {
        registry: Option<std::sync::Arc<ainxt_agent::plugins::PluginRegistry>>,
    },
    /// Re-discover the session's own project hooks (`.ainxt/hooks`,
    ...
    /// Out-of-band history repair (`ainxt.dev/session/repair`): fix tool-pairing
    ...
    RepairHistory {
        dry_run: bool,
        respond_to:
            oneshot::Sender<anyhow::Result<ainxt_chat_state::compaction_utils::HistoryRepairReport>>,
    },
    GetRewindPoints {
    ...
    /// ainxt extension session notification - client-side events to store in persistence
    ...
    RecordSubagentUsage {
        by_model: Vec<(String, ainxt_chat_state::UsageTotals)>,
    ...
    /// mid-session via the `ainxt.dev/session/update_mcp_servers` extension method).
    ...
    /// sends) so a unchanged-fork child can present a byte-identical tool prefix.
    SnapshotToolDefinitions {
        respond_to: oneshot::Sender<Vec<ainxt_sampling_types::ToolSpec>>,
    },
    ...
    KillBackgroundTask {
        task_id: String,
        respond_to: oneshot::Sender<Result<ainxt_tools::types::KillOutcome, String>>,
    },
    DeleteScheduledTask {
        task_id: String,
    ...
    ListTasks {
        respond_to: oneshot::Sender<Option<Vec<ainxt_tools::types::TaskSnapshot>>>,
    },
    ...
    GetHooksList {
        respond_to: oneshot::Sender<ainxt_hooks_plugins_types::HooksListResponse>,
    },
    /// Execute a hooks management action from the pager modal.
    HooksAction {
        action: ainxt_hooks_plugins_types::HooksAction,
        respond_to: oneshot::Sender<ainxt_hooks_plugins_types::ActionOutcome>,
    },
    ...
    /// This session's plugin registry, as served by `ainxt.dev/plugins/list`.
    PluginsList {
        respond_to:
            oneshot::Sender<Option<std::sync::Arc<ainxt_agent::plugins::PluginRegistry>>>,
    },
    ...
    /// from the notification bridge, which does not go through `send_ainxt_notification`).
    ...
    /// serialized mailbox; the rebroadcast of `ainxt.dev/queue/changed` is the
    ...
    /// `ainxt.dev/queue/changed` is the truth signal for every attached client.
    ...
    /// tool-free model call (default `ainxt-build-0.1` when available via
    ...
    /// `ainxt-build` model, low temperature, and capped output tokens.
    RewriteMemoryNote {
        raw_text: String,
    ...
    Interject {
        ...
        /// `ainxt.dev/session/interjection` so the originating pager can dedup its
    ...
    GoalSummaryTurn {
        /// Short instruction appended as a unchanged user message.
        prompt_text: String,
    },
    /// Take turn messages from the chat state actor (proxied from mvp_agent).
    TakeTurnMessages {
        respond_to: oneshot::Sender<Option<ainxt_chat_state::TurnCapture>>,
    },
    ...
    TakeHarnessTraceTurns {
        respond_to:
            oneshot::Sender<Vec<Vec<ainxt_sampling_types::conversation::ConversationItem>>>,
    },
