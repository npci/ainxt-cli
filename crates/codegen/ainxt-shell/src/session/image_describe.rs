...
//! content blocks, the session calls a vision-capable ainxt model
...
use ainxt_chat_state::compaction_utils::{extract_real_user_queries, extract_user_query};
use ainxt_sampling_types::conversation::{ContentPart, ConversationItem, UserItem};
use ainxt_tools::util::truncate::truncate_middle;
/// Per-entry character cap for the conversation outline sent to the
/// vision model. Matches the compat-harness behavior.
...
impl ImageDescribeCache {
    ...
    pub async fn get_or_describe(
        &self,
        client: ainxt_sampler::SamplingClient,
        model: &str,
        raw_bytes: &[u8],
        mime_type: &str,
        outline: Option<&str>,
...
mod tests {
    use super::*;
    use ainxt_sampling_types::conversation::{ConversationItem, UserItem};
    #[test]
    ...
    fn user(text: &str) -> ConversationItem {
        ConversationItem::User(UserItem {
            content: vec![ainxt_sampling_types::conversation::ContentPart::Text {
                text: text.into(),
            }],
            synthetic_reason: None,
            ..Default::default()
