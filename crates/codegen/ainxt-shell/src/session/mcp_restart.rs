...
//! window containing a [`ainxt_mcp::servers::McpClientEventKind::TransportClosed`]
//! or [`ainxt_mcp::servers::McpClientEventKind::HandshakeFailed`] key for a
...
//!    [`ainxt_mcp::servers::start_mcp_server`] sets
...
//!    push and stop.
//! 5. **Already-Empty** — see the [`ainxt_mcp::servers::ClientStateKind::Empty`]
...
use ainxt_mcp::servers::{McpClientEventKind, McpServerName};
...
pub trait RestartActions {
    ...
    /// Push an already-built `ainxt.dev/mcp/server_status` payload to the
...
/// ACP `ainxt.dev/mcp/server_status` notification. Failures are logged and
...
mod tests {
    ...
    struct MockActions {
        ...
        /// Servers with an in-flight restart claim (matches the
    ...
    async fn http_event_does_not_trigger_restart() {
        run_in_local(async {
            let mock = Rc::new(MockActions::new());
            // Intentionally not configured as stdio — matches
