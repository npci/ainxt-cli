...
//!
//! Receives [`ainxt_mcp::servers::McpClientEvent`]s emitted by:
//! - per-client transport-liveness watchers
//!   ([`ainxt_mcp::liveness`]),
//! - the [`ainxt_mcp::servers::AinxtClientHandler`] (server-pushed
...
//! `ainxt.dev/mcp/server_status` and the payload schema defined by
...
pub const SERVER_STATUS_METHOD: &str = "ainxt.dev/mcp/server_status";
...
pub struct McpServerStatusPayload {
    ...
    /// MCP server name (`ainxt_com_linear`, `github`, ...).
    pub name: String,
    /// `managed` (sourced from cli-chat-proxy / `ainxt_com_` prefix)
    /// or `local` (user `.ainxt/config.toml`).
    ...
    /// transport error reason to the UI unchanged — no sanitization or
...
pub enum McpServerStatus {
    /// Client is in [`ainxt_mcp::servers::ClientStateKind::Ready`]
...
/// Build [`McpServerSource`] from a server name. Matches the
...
/// Discriminant for an event (matches
/// [`ainxt_mcp::servers::McpClientEventKind`]). Used as the
...
///
/// The `HandshakeFailed` `reason` is passed through unchanged as the
...
pub fn build_payload(
    ...
    let (status, reason, detail) = match (kind, event) {
        ...
        // NeedsAuth ("visit ainxt.dev"), not a generic Unavailable, so a
        ...
        (McpClientEventKind::HandshakeFailed, McpClientEvent::HandshakeFailed { reason, .. })
            if source == McpServerSource::Managed
                && ainxt_mcp::servers::is_auth_rejection_message(reason) =>
        {
            (
...
///   `ConfigRemoved` keys,
/// - emit one ACP `ainxt.dev/mcp/server_status` push per surviving
///   buffer entry, via the provided gateway.
///
/// `gateway` is a [`ainxt_acp_lib::AcpAgentGatewaySender`] (forwarded
...
/// 3. `flush_window` — emit ACP `ainxt.dev/mcp/server_status` per
///    surviving entry.
...
mod tests {
    ...
    /// Contract: HandshakeFailed `reason` is surfaced unchanged (full
    ...
    fn managed_handshake_non_auth_stays_unavailable() {
        for reason in ["403 Forbidden", "cli-chat-proxy returned 502"] {
            ...
            let ev = McpClientEvent::HandshakeFailed {
                server: "ainxt_com_slack".to_string(),
    ...
    /// Contract: managed server names (starting with `ainxt_com_`)
    /// are classified as `Managed`; everything else as `Local`.
    #[test]
    fn classify_source_uses_managed_prefix() {
        assert_eq!(classify_source("ainxt_com_linear"), McpServerSource::Managed);
    ...
    async fn dispatcher_drops_dead_clients_on_transport_closed() {
        ...
        use ainxt_mcp::servers::{McpClient, McpState};
    ...
    fn recoverable_http_servers_excludes_managed_stdio_and_disabled() {
        let configs = vec![
            ...
            http_cfg("ainxt_com_slack"), // managed
    ...
    fn discard_gateway() -> ainxt_acp_lib::AcpAgentGatewaySender {
        let (tx, _rx) = tokio::sync::mpsc::unbounded_channel();
        ainxt_acp_lib::AcpAgentGatewaySender::new(tx)
    }
    ...
    async fn run_dispatcher_stale_transport_closed_is_fully_inert() {
        ...
        let gateway = ainxt_acp_lib::AcpAgentGatewaySender::new(gw_tx);
