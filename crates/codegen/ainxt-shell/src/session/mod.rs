...
pub use ainxt_fsnotify::{FsConfig, FsEvent, FsEventKind, FsEventSource, FsNotifyError, GitMetaKind};
...
pub(crate) fn is_cursor_user_template(
    _template: &ainxt_agent::prompt::user_message::UserMessageTemplate,
) -> bool {
    false
}
/// `false` twin of [`is_cursor_system_template`]; see [`is_cursor_user_template`].
pub(crate) fn is_cursor_system_template(
    _template: &ainxt_agent::prompt::context::TemplateOverride,
) -> bool {
    false
}
...
/// Client-requested fs notification mode (was ainxt_fsnotify::FsNotifyMode).
...
pub use ainxt_shared::session::info;
pub mod managed_mcp;
...
pub mod persistence;
pub use ainxt_shared::placeholder_images;
pub mod plan_mode;
