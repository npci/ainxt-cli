...
use ainxt_mcp::oauth_config::{McpOAuthConfig, McpOAuthConfigMap};
use ainxt_mcp::servers as inner;
...
pub fn build_config_resolved_event(
    ...
    cwd: &Path,
) -> ainxt_file_utils::events::Event {
    let disabled: Vec<String> = crate::util::config::disabled_mcp_server_names(cwd)
        .into_iter()
        .collect();
    let servers = configs
        .iter()
        .map(|c| ainxt_file_utils::events::McpConfigServer {
        ...
        .collect();
    ainxt_file_utils::events::Event::McpConfigResolved { servers, disabled }
}
