...
//!
//! Hooks registered at `session/new` (`_meta["ainxt.dev/hooks"]`) come in two flavors,
//! both matched by the agent ([`ainxt_hooks::matcher::HookMatcher`], shared with
//! file hooks):
//! - **`PreToolUse` gate**: an awaited reverse *request* `ainxt.dev/hooks/run`; a `deny`
//!   blocks the tool.
//! - **All other events**: fire-and-forget *notifications* `ainxt.dev/hooks/event`,
...
use ainxt_hooks::event::{HookEventEnvelope, HookEventName, HookPayload};
use ainxt_telemetry::events::ClientHookGateOutcome;
...
const HOOK_EVENT_METHOD: &str = "ainxt.dev/hooks/event";
const HOOK_RUN_METHOD: &str = "ainxt.dev/hooks/run";

/// Default per-callback bound for a client's `ainxt.dev/hooks/run` reply; on timeout the gate
...
/// Outcome of the `ainxt.dev/hooks/run` reverse request, before interpreting it as a
    ...
    match outcome {
        ReverseOutcome::Responded(raw) => {
            match serde_json::from_str::<ClientHookResponse>(raw.get()) {
                Ok(resp) => {
                    ...
                    let label = match resp.decision {
                        ...
                        ClientHookDecision::Other => {
                            tracing::warn!(
                                "ainxt.dev/hooks/run returned an unknown decision value; failing open"
                            );
                ...
                Err(err) => {
                    tracing::warn!(%err, "malformed ainxt.dev/hooks/run response; failing open");
                    (
        ...
        ReverseOutcome::Transport(err) => {
            tracing::warn!(%err, "ainxt.dev/hooks/run transport error (no client wired?); failing open");
            (
                ClientHookResponse::default(),
        ...
        ReverseOutcome::Timeout => {
            tracing::warn!("ainxt.dev/hooks/run timed out; failing open");
            (
                ClientHookResponse::default(),
...
/// Whether `group` fires for an event on `tool_name`. Matches the file-hook matcher rule
...
impl SessionActor {
    ...
    pub(super) async fn deny_tool(
        ...
        ainxt_telemetry::session_ctx::log_event(ainxt_telemetry::events::HookBlocked {
    ...
    /// `ainxt.dev/hooks/run` once per matching callback with the shared `envelope` (the
    ...
    pub(super) async fn run_pre_tool_use_client_hook(
        ...
        let tool_name = ainxt_hooks::dispatcher::extract_tool_name(envelope)
            ...
            .map(|(callback_id, timeout)| {
                ...
                async move {
                    ...
                    ainxt_telemetry::session_ctx::log_event(
                        ainxt_telemetry::events::ClientHookGate {
    ...
    /// Issue one `ainxt.dev/hooks/run` reverse request, bounded by a per-callback `timeout`.
    ...
    /// `ainxt.dev/hooks/event` notification to each matching registered callback.
        ...
        local
            .run_until(async {
                ...
                *actor.hook_registry.borrow_mut() = Some(std::sync::Arc::new(
                    ainxt_hooks::discovery::HookRegistry::default(),
                ));
                assert!(actor.hook_event_active(HookEventName::Stop));
