...
use ainxt_file_utils::queue::UploadQueue;
...
pub struct SessionHandle {
    ...
    /// keyed by `tool_call_id`. Matches `current_prompt_id`: the same `Arc` is
    ...
    pub chat_state_handle: ainxt_chat_state::ChatStateHandle,
    ...
    /// `ainxt.dev/codeNavigation.enabled`.  Stored per-session so that in leader
    ...
    /// Exposed so the `ainxt.dev/toggle_plan_mode` handler can toggle plan mode
    ...
    /// Set via `ainxt.dev/debug/arm_auto_compact`.
    pub force_compact: std::sync::Arc<std::sync::atomic::AtomicBool>,
    pub permission_handle: ainxt_workspace::permission::PermissionHandle,
    ...
    pub terminal_backend:
        Option<std::sync::Arc<dyn ainxt_tools::computer::types::TerminalBackend>>,
...
impl SessionHandle {
    ...
    pub(crate) async fn get_model_metadata(&self) -> ainxt_chat_state::ModelMetadata {
            ...
            .is_ok()
        {
            ...
            ainxt_chat_state::ModelMetadata::default()
        }
    }
    ...
    /// Routes through the session actor to the ToolBridge's TerminalBackend.
    pub async fn list_tasks(&self) -> Option<Vec<ainxt_tools::types::TaskSnapshot>> {
        let (tx, rx) = oneshot::channel();
        if self
            .cmd_tx
    ...
    pub async fn get_hooks_list(&self) -> Option<ainxt_hooks_plugins_types::HooksListResponse> {
        let (tx, rx) = oneshot::channel();
        if self
            .cmd_tx
    ...
    /// Snapshot the session's resolved tool schema for unchanged-fork inheritance.
    /// A dead actor or dropped reply fails open to an empty list (child then builds
    /// its own toolset, same as a non-fork spawn).
    pub(crate) async fn snapshot_tool_definitions(&self) -> Vec<ainxt_sampling_types::ToolSpec> {
        let (tx, rx) = oneshot::channel();
        if self
            .cmd_tx
