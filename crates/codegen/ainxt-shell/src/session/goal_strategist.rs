...
//!
//! Matches [`crate::session::goal_planner`] in shape (an `Outcome`, a
...
/// shape matches `GoalPlannerSpawner`; `SpawnError` is reused from the
...
pub(crate) async fn run_goal_strategist(
    ...
    // plan.md safety: snapshot plan.md exactly and arm the restore
    ...
    // Restore plan.md exactly ONCE, regardless of spawn outcome (the
...
mod tests {
    ...
    /// 3-role equivalence: an explicit strategist pair threads `agent_type` as the
    ...
    /// A strategist edit to plan.md is reverted exactly — the WHOLE
    ...
    /// parent (ainxt-build) names, with no placeholder left behind. Guards
    ...
    fn strategist_template_renders_per_agent_type_names() {
        ...
        tool_names.insert(
            ainxt_tools::types::tool::ToolKind::Execute,
            "alt_shell".to_string(),
        );
