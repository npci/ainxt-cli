...
use ainxt_tools::implementations::skills::skill::format_skill_name;
...
// ── ainxt.dev/commands/list ext method ────────────────────────────────
...
pub(super) enum SlashCommandOutcome {
    ...
    ///
    /// The original prompt `blocks` are preserved unchanged — they are NOT
...
///   matching today's ainxt flow that calls our dedicated `skill` tool.
/// - `Passthrough`: leave the prompt unchanged. Some templates use this —
...
pub(super) async fn build_skill_information_for_refs(
    ...
    use ainxt_tools::implementations::skills::skill::{
...
/// `ainxt-tools` so it stays identical to the pager's `LoopCommand` and the
...
mod tests {
    ...
    fn resolve_passthrough_preserves_original_blocks() {
        // External-harness agents: blocks are passed through unchanged.
