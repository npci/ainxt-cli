...
fn read_capped(path: &Path) -> Option<String> {
    ...
    let mut limited = file.take(MAX_READ_BYTES as u64);
    limited.read_to_end(&mut buf).ok()?;
    // Explicitly release the underlying file handle as soon as the capped
    // read completes, rather than relying solely on end-of-scope Drop.
    drop(limited);
