...
/// scratch root (`<temp_dir>/ainxt-goal-<id>`) and the verifier
...
/// can copy it unchanged into spawned verifier prompts without
...
/// Private per-goal scratch root: `<temp_dir>/ainxt-goal-<verifier_id>`.
///
...
pub(crate) fn goal_scratch_root(verifier_id: &str) -> PathBuf {
    std::env::temp_dir().join(format!("ainxt-goal-{verifier_id}"))
}
...
fn copy_no_follow(src: &std::path::Path, dest: &std::path::Path) -> std::io::Result<()> {
    ...
    // Explicitly release both file handles now that the copy is complete,
...
pub struct GoalOrchestration {
    ...
    /// "zero runs allowed" case. Matches the `token_budget:
    ...
    /// most recent `NotAchieved` verdict. Inlined unchanged into every
    ...
    /// the verification stage (matches `planning_in_flight`) so the
...
impl GoalTracker {
    ...
    /// `plan.md` and restores it exactly (and on cancellation),
    ...
    /// the root explicitly first (matches the `complete` / `budget_limit`
    /// cleanup).
...
mod tests {
    ...
    /// public contract — verifier file paths embed it unchanged, and
    /// drift here would silently invalidate the documented
    /// `ainxt-goal-<12 hex chars>` scratch-root format (and the
    ...
    fn serde_skip_fields_reset_on_deserialize() {
        ...
        t.update_live_progress(
            100,
            vec![("ainxt-4".to_owned(), 60), ("ainxt-3".to_owned(), 40)],
            200_000,
            50,
            3,
            10,
    ...
    fn from_snapshot_regenerates_non_canonical_verifier_id() {
        ...
        // A canonical id is kept unchanged.
        let orchestration = make_base_orchestration();
    ...
    /// so the next attempt can resume skeptic 0). Matches the
    ...
    /// `temp_dir()` + `verifier_id`: a `ainxt-goal-<vid>` root with an
    ...
    fn scratch_path_helpers_derive_pinned_layout() {
        ...
        assert_eq!(root, std::env::temp_dir().join("ainxt-goal-vid123"));
        assert_eq!(
            implementer_scratch_dir("vid123"),
            std::env::temp_dir()
                .join("ainxt-goal-vid123")
                .join("implementer"),
        );
        assert_eq!(
    ...
    /// Every terminal goal-ending transition (`complete`, `budget_limit`,
    /// `clear`) removes the private scratch root from disk — matches
