...
//! are consumed unchanged by `templates/goal_verifier_prompt.md`, so the
...
pub(crate) fn build_classifier_evidence_packet(
    ...
    // typed), so it is embedded unchanged — only model-/workspace-derived
...
/// filenames arrive unchanged instead of octal-escaped-and-quoted.
...
/// (workspace is not a git repo, git unavailable, timeout). Matches
    ...
    builder
        ...
        .filter_entry(|entry| {
            ...
            entry
                ...
                .map(|name| {
                    name != ".git"
                        && !ainxt_file_utils::skip_dir_set().contains(name.to_lowercase().as_str())
                })
                .unwrap_or(true)
        });
...
/// still receives the full summary. Matches `GOAL_STRATEGIST_RECOMMENDATION_MAX_CHARS`.
...
/// Tags that, if echoed unchanged by the model inside FINAL_RESPONSE,
...
mod tests {
    use super::*;
    use ainxt_sampling_types::{AssistantItem, UserItem};
    ...
    fn assistant_tool_call_only() -> ConversationItem {
        ConversationItem::Assistant(AssistantItem {
            content: "".into(),
            tool_calls: vec![ainxt_sampling_types::ToolCall {
                id: "call_1".into(),
    ...
    fn user(text: &str) -> ConversationItem {
        ConversationItem::User(UserItem {
            content: vec![ainxt_sampling_types::ContentPart::Text { text: text.into() }],
    ...
    fn evidence_packet_with_file_path_renders_path_not_content() {
        let packet = build_classifier_evidence_packet(
            ...
            Some(Path::new("/home/u/.ainxt/sessions/s1/goal/plan.md")),
            None,
            "I did it.",
        );
        assert_eq!(
            ...
            "OBJECTIVE:\ndo X\n\n\
             ...
             CHANGED_FILES:\n- js/main.js\n\n\
             PLAN_FILE: /home/u/.ainxt/sessions/s1/goal/plan.md\n\n\
             PLAN_CHANGES: (none)\n\n\
             FINAL_RESPONSE:\nI did it.\n",
        );
    }
    ...
    fn evidence_packet_plan_path_with_spaces_and_unicode_round_trips() {
        let plan = "/tmp/ainxt sessions/✓ goal/plan.md";
        let packet = build_classifier_evidence_packet(
            "obj",
            ChangesRef::Unavailable,
            &[],
    ...
    async fn capture_changes_diff_walkdir_skips_all_well_known_directories() {
        let tmp = tempfile::tempdir().unwrap();
        let goal_created_at = now_unix_seconds() - 60;
        for sub in ainxt_file_utils::SKIP_DIR_NAMES {
    ...
    async fn untracked_files_appear_in_changed_files_for_git_layers() {
        ...
        tokio::fs::write(tmp.path().join("brand_new.txt"), b"created by goal\n")
            .await
            .unwrap();
        // Unicode + space: `-z` must deliver the path unchanged, not
