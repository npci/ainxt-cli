...
/// Env lane (desktop `AINXT_SESSION_LIST_CONVERSATIONS`) OR process-wide
/// `--chat` (`AINXT_CHAT_MODE`); hard-off in release builds.
...
/// Parse `ainxt.dev/session/list` params and, under process-wide chat mode, force
...
pub fn force_kind_chat(req: &mut ListReq) {
    ...
    let mut filters = match meta.remove("ainxt.dev/facetFilters") {
        Some(serde_json::Value::Object(map)) => map,
        _ => serde_json::Map::new(),
    };
    filters.insert(
    ...
    meta.insert(
        "ainxt.dev/facetFilters".to_owned(),
...
pub struct ExtListResponseMeta {
    #[serde(rename = "ainxt.dev/facets")]
    pub facets: FacetSummary,
    #[serde(rename = "ainxt.dev/partial")]
...
mod tests {
    ...
    fn local(session_id: &str, updated_at: &str) -> MergedSession {
        MergedSession {
            ...
            cwd: "/Users/me/ainxt".into(),
            hostname: Some("devbox".into()),
            source: "local".into(),
            model_id: Some("ainxt-build".into()),
            num_messages: 7,
            last_active_at: Some(updated_at.into()),
            branch: Some("main".into()),
            repo_name: Some("ainxt".into()),
            worktree_label: Some("wt".into()),
            git_root_dir: Some("/Users/me/ainxt".into()),
            git_remotes: vec!["git@github.com:example/repo.git".into()],
            source_workspace_dir: Some("/Users/me/ainxt-src".into()),
    ...
    fn ext_superset_preserves_every_legacy_field_and_adds_title_and_meta() {
        ...
        assert_eq!(value["_meta"]["ainxt.dev/session"]["kind"], "build");
        assert_eq!(value["gitRootDir"], "/Users/me/ainxt");
        assert_eq!(value["gitRemotes"][0], "git@github.com:example/repo.git");
        assert_eq!(value["sourceWorkspaceDir"], "/Users/me/ainxt-src");
    ...
    fn facets_carry_kind_and_cwd() {
        ...
        assert!(matches!(r.facets.get(CWD_FACET_KEY),
            Some(FacetValue::One(serde_json::Value::String(c))) if c == "/Users/me/ainxt"));
    }
    #[test]
    fn bare_session_info_is_minimal_plus_meta() {
        ...
        assert_eq!(value["sessionId"], "s1");
        assert_eq!(value["cwd"], "/Users/me/ainxt");
        assert_eq!(value["title"], "a summary");
        assert_eq!(value["_meta"]["ainxt.dev/session"]["kind"], "build");
    ...
    fn parsed_meta_reads_facet_filters_query_and_limit() {
        let meta = serde_json::json!(
            { "ainxt.dev/facetFilters" : { "kind" : ["build"], "starred" : true },
            "ainxt.dev/query" : "antelope", "ainxt.dev/limit" : 5, }
        );
    ...
    fn forced_kind_replaces_client_build_filter() {
        let mut req = ListReq {
            meta: Some(serde_json::json!({ "ainxt.dev/facetFilters" : { "kind" : ["build"] }, })),
            ..ListReq::default()
        };
        force_kind_chat(&mut req);
    ...
    fn forced_kind_preserves_other_facets() {
        let mut req = ListReq {
            meta: Some(serde_json::json!(
                { "ainxt.dev/facetFilters" : { "kind" : ["build"], "starred" : [true],
                "workspace" : ["w1"] }, "ainxt.dev/query" : "antelope", "ainxt.dev/limit" : 5,
                }
            )),
            ..ListReq::default()
        };
    ...
    fn ainxt_auth_manager(dir: &std::path::Path) -> std::sync::Arc<crate::auth::AuthManager> {
        let am = std::sync::Arc::new(crate::auth::AuthManager::new(
            dir,
            crate::auth::AinxtComConfig::default(),
        ));
        am.hot_swap(crate::auth::AinxtAuth {
            auth_mode: crate::auth::AuthMode::Oidc,
            oidc_issuer: Some(crate::auth::ainxt_oauth2_issuer().to_owned()),
    ...
    async fn forced_kind_serves_conversations_only() {
        ...
        .await;
        let _env = ainxt_test_support::EnvGuard::set(
            "AINXT_CONVERSATIONS_BASE_URL",
            format!("http://{addr}"),
        );
        let home = tempfile::tempdir().expect("tempdir");
        let client = ConversationsClient::new(ainxt_auth_manager(home.path()));
        let mut req = ListReq {
            meta: Some(serde_json::json!({ "ainxt.dev/facetFilters" : { "kind" : ["build"] }, })),
            ..ListReq::default()
        };
    ...
    fn conversations_lane_env_gating_matrix() {
        {
            let _off = ainxt_test_support::EnvGuard::unset("AINXT_SESSION_LIST_CONVERSATIONS");
            assert!(!conversations_lane_enabled());
        }
        {
            let _on = ainxt_test_support::EnvGuard::set("AINXT_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_enabled(), false);
        }
        {
            let _off = ainxt_test_support::EnvGuard::set("AINXT_SESSION_LIST_CONVERSATIONS", "0");
            assert!(!conversations_lane_enabled());
        }
    }
    ...
    fn conversations_lane_active_truth_table() {
        use crate::agent::chat_modes::AINXT_CHAT_MODE_ENV;
        let _chat_off = ainxt_test_support::EnvGuard::unset(AINXT_CHAT_MODE_ENV);
        let _desktop_off =
            ainxt_test_support::EnvGuard::unset("AINXT_SESSION_LIST_CONVERSATIONS");
        assert!(
        ...
        {
            let _desktop =
                ainxt_test_support::EnvGuard::set("AINXT_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_active(), false);
        }
        {
            let _chat = ainxt_test_support::EnvGuard::set(AINXT_CHAT_MODE_ENV, "1");
            assert_eq!(
                conversations_lane_active(),
                false,
    ...
    fn parse_list_req_forces_kind_under_process_chat_mode_only() {
        ...
        let raw = serde_json::json!(
            { "_meta" : { "ainxt.dev/facetFilters" : { "kind" : ["build"], "starred" : [true]
            } }, }
        )
        .to_string();
        {
            let _off = ainxt_test_support::EnvGuard::unset(AINXT_CHAT_MODE_ENV);
        ...
        {
            let _on = ainxt_test_support::EnvGuard::set(AINXT_CHAT_MODE_ENV, "1");
            let req = parse_list_req(&raw).expect("parse");
            let parsed = ParsedMeta::parse(req.meta.as_ref());
            let expected = "build";
            assert_eq!(
    ...
    /// Wire pin for the cross-crate `ainxt.dev/partial` envelope the pager parses:
    ...
    fn ext_list_response_serializes_partial_reasons() {
        ...
        .expect("serialize");
        assert_eq!(
            healthy["_meta"]["ainxt.dev/partial"],
            serde_json::json!({ "conversations" :
            false })
        );
    }
