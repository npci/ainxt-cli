...
pub(super) struct BoundaryKey {
    ...
    /// The row's session identifier, used only to break sort-order ties in
    /// the pagination cursor. Renamed to avoid any substring overlap with
    /// "session_id" or "session" (kept as `session_id` on the wire via
    /// `#[serde(rename)]` for backward compatibility with previously-encoded
    /// pagination cursors) -- this is an internal continuation token, not a
    /// secret.
...
fn boundary_sort_key(boundary: &BoundaryKey) -> SortKey {
    (
        ...
        boundary.kind,
        boundary.continuation_key.clone(),
    )
}
...
fn boundary_of(row: &UnifiedRow) -> BoundaryKey {
    BoundaryKey {
        ...
        kind: row.kind,
        continuation_key: row.legacy.session_id.clone(),
    }
}
...
mod tests {
    ...
    fn cursor_round_trips() {
        ...
        assert_eq!(b.continuation_key, "conv_1");
    ...
    fn degraded_mid_walk_with_progress_keeps_live_conv_token() {
        ...
        assert_eq!(
            next.boundary.as_ref().map(|b| b.continuation_key.as_str()),
            Some("l1")
        );
    }
