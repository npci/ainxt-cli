...
/// ainxt `SessionUpdate::RewindMarker` discriminant. Appears unchanged in compact
/// JSON, so it doubles as a cheap substring pre-filter.
pub(crate) static REWIND_MARKER: LazyLock<String> = LazyLock::new(|| {
    tagged_discriminant(&AinxtSessionUpdate::RewindMarker {
        target_prompt_index: 0,
        created_at: String::new(),
    })
});

/// ainxt `SessionUpdate::TaskBackgrounded` discriminant.
pub(crate) static TASK_BACKGROUNDED: LazyLock<String> = LazyLock::new(|| {
    tagged_discriminant(&AinxtSessionUpdate::TaskBackgrounded {
...
/// ainxt `SessionUpdate::TaskCompleted` discriminant.
pub(crate) static TASK_COMPLETED: LazyLock<String> = LazyLock::new(|| {
    tagged_discriminant(&AinxtSessionUpdate::TaskCompleted {
