use ainxt_telemetry::events::SessionHarness;
use ainxt_workspace::permission::Decision;
...
pub(crate) fn skill_source_label(skill_path: &str, cwd: &str) -> &'static str {
    ...
    let user = crate::util::ainxt_home::ainxt_home()
        .parent()
...
pub(crate) fn format_hook_name(spec: &ainxt_hooks::config::HookSpec) -> String {
...
/// project dirs contain `/.ainxt/`.)
fn format_hook_source(spec: &ainxt_hooks::config::HookSpec) -> &'static str {
...
impl HookRegInfo {
    pub(crate) fn from_spec(spec: &ainxt_hooks::config::HookSpec) -> Self {
        Self {
...
pub(crate) struct SessionHarnessMetrics {
    ...
    pub permission_mode: ainxt_telemetry::enums::PermissionMode,
    pub mcp_server_names: Vec<String>,
    pub lsp_server_names: Vec<String>,
    pub memory_enabled: bool,
    pub auto_update: Option<bool>,
    pub cwd: String,
    pub skills_config: ainxt_agent::prompt::skills::SkillsConfig,
    /// Resolved vendor-compat config, so recorded skill / AGENTS.md names match
    /// what the session actually discovers.
    pub compat: ainxt_tools::types::compat::CompatConfig,
...
impl SessionHarnessMetrics {
    pub async fn into_event(self, hooks: Vec<HookRegInfo>) -> SessionHarness {
        ...
        let agents_md_dir_names = ainxt_agent::prompt::agents_md::read_agents_config_with_paths(
            &self.cwd,
            self.compat,
        )
        .await
        ...
        .collect();
        let skill_names = ainxt_agent::prompt::skills::list_skills_with_plugins(
            Some(&self.cwd),
            &self.skills_config,
        ...
        SessionHarness {
            ...
            is_git_repo: ainxt_telemetry::context::collect_git_context(&self.cwd).is_git_repo,
