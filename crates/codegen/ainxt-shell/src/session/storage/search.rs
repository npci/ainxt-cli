...
//!
//! Matches the memory system's architecture:
...
//! other concurrently running ainxt processes (which may wipe or downgrade
...
use super::{
    ...
    AINXT_SESSION_UPDATE_METHOD, collect_prompts_from_events,
};
...
/// User-configurable overrides via `~/.ainxt/settings.json` are deferred
/// to a follow-up.
...
///
/// TODO: When multiple ainxt processes run concurrently, they each have
...
fn search_db_path(root_dir: &Path) -> PathBuf {
    ...
    ainxt_sqlite_journal::JournalMode::for_db_path(&path).effective_db_path(&path)
}
...
fn collect_all_indexable_content_single_pass(updates_path: &Path) -> io::Result<(String, u64)> {
    ...
    for line_result in reader.lines() {
        ...
        let (raw_params, is_ainxt) = if let Ok(env) = serde_json::from_str::<RawLinePeek<'_>>(trimmed)
        {
        ...
        // Control events (rewind markers) come from ainxt extensions
        // ("_ainxt.dev/session/update"). Dispatch on source first, then tag.
        if !is_ainxt {
            // ── ACP content events ──────────────────────────────────
            match tag {
...
fn collect_delta_content(updates_path: &Path, offset: u64) -> io::Result<DeltaResult> {
    ...
    for line_result in reader.lines() {
        ...
        match tag {
            Some(t) if is_ainxt && t == *REWIND_MARKER => {
                return Ok(DeltaResult::NeedsFullReread);
            }
            Some(t) if !is_ainxt && t == *USER_MESSAGE_CHUNK => {
                flush_assistant(&mut current_assistant, &mut assistant_texts);
                if let Ok(peek) = serde_json::from_str::<UserContentPeek<'_>>(raw_params)
                    && let Some(content) = peek.update.content
                    && content.content_type == Some("text")
            ...
            Some("agent_message_chunk") if !is_ainxt => {
...
mod tests {
    ...
    fn ainxt_update(session_update_json: &str) -> String {
        format!(
