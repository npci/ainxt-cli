...
pub struct CompactionConfig {
    ...
    /// Locks the context window when `AINXT_DEBUG_CONTEXT_WINDOW` is set.
    ...
    pub compaction_mode: ainxt_chat_state::CompactionMode,
    /// When `true`, feed the summarizer the unchanged conversation instead of the lossy rewrite (the retry loop may still fall back).
