...
/// JSON-RPC wrapper for ainxt extension notifications.
#[derive(Debug, Serialize)]
struct AinxtJsonRpcNotification<'a> {
    method: &'static str,
    params: &'a crate::extensions::notification::SessionNotification,
}

const ACP_SESSION_UPDATE_METHOD: &str = "session/update";
const AINXT_SESSION_UPDATE_METHOD: &str = "_ainxt.dev/session/update";
...
impl ExportedMessage {
    ...
    pub fn from_ainxt_notification(
        notification: &crate::extensions::notification::SessionNotification,
    ) -> Self {
        let wrapper = AinxtJsonRpcNotification {
            method: AINXT_SESSION_UPDATE_METHOD,
            params: notification,
        };
        let content = serde_json::to_string(&wrapper).unwrap_or_else(|_| "{}".to_string());
        ...
        updates
            ...
            .map(|update| match update {
                ...
                SessionUpdate::Ainxt(notification) => {
                    ExportedMessage::from_ainxt_notification(notification)
                }
            })
            .collect()
    }
