...
use crate::extensions::notification::SessionNotification as AinxtSessionNotification;
...
///
/// One-shot ainxt events (RetryState, ImageCompressed, HookExecution,
/// etc.) take the direct `send_ainxt_notification` path for per-event hooks and persistence.
#[derive(Debug, Clone)]
pub(crate) enum SessionNotification {
    Acp(Box<AcpSessionNotification>),
    Ainxt(Box<AinxtSessionNotification>),
}
...
impl From<AinxtSessionNotification> for SessionNotification {
    fn from(n: AinxtSessionNotification) -> Self {
        Self::Ainxt(Box::new(n))
    }
}
...
impl SessionNotification {
    ...
    pub(crate) fn expect_acp(&self) -> &AcpSessionNotification {
        match self {
            Self::Acp(n) => n,
            Self::Ainxt(_) => panic!("expected Acp notification, got Ainxt"),
        }
    }
