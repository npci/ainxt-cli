...
//!
//! Used by both the ACP `ainxt.dev/session/list` handler and the `ainxt sessions`
...
pub async fn fetch_merged(
    ...
    let repo_urls_fut = async {
        cwd.map(|c| {
            ainxt_workspace::session::git::resolve_normalized_remote_urls(std::path::Path::new(
                c,
            ))
        })
        .unwrap_or_default()
...
///
/// Matches the key the session picker UI displays — `last_active_at` with a
...
mod tests {
    ...
    fn all_metadata_fields_populated_together() {
        ...
        assert_eq!(merged[0].repo_name.as_deref(), Some("ainxt"));
