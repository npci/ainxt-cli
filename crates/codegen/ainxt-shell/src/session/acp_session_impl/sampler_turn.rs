...
/// `SamplingError::is_auth_error` in ainxt-sampling-types: 403 is
...
pub(super) fn is_auth_tool_error(err: &ainxt_tool_runtime::ToolError) -> bool {
...
struct SessionTokenAuthGate {
    ...
    /// BYOK status still refresh against cli-chat-proxy / `*.ainxt.dev` without
...
impl SessionTokenAuthGate {
    ...
    fn new(
        ...
        Self {
            ...
            model_byok,
            endpoint_is_first_party: crate::util::is_first_party_ainxt_url(base_url),
        }
    }
    fn active(self) -> bool {
...
pub(super) async fn call_with_auth_retry<F, Fut>(
    ...
    tool_name: &str,
    mut call: F,
) -> Result<ainxt_tools::types::output::ToolRunResult, ainxt_tool_runtime::ToolError>
where
    F: FnMut() -> Fut,
    Fut: std::future::Future<
            Output = Result<
                ainxt_tools::types::output::ToolRunResult,
...
impl SessionActor {
    ...
    /// a unchanged-fork child's tool prefix can never silently drift from what the
    ...
    /// against first-party ainxt hosts).
    ...
    pub(super) async fn reconstruct_full_config(&self) -> SamplingConfig {
        ...
        impl ainxt_sampler::HeaderInjector for TraceContextInjector {
            fn inject(&self, headers: &mut reqwest::header::HeaderMap) {
                if let Some(tp) = ainxt_file_utils::trace_context::current_traceparent()
        ...
        impl ainxt_sampler::BearerResolver for AuthManagerBearerResolver {
        ...
        let cfg = self
            ...
            .get_sampling_config()
            .await
            .unwrap_or_else(|| ainxt_sampling_types::SamplingConfig {
                base_url: String::new(),
                model: String::new(),
        ...
        SamplingConfig {
            ...
            bearer_resolver: if use_bearer_resolver {
                self.auth_manager
                    .as_ref()
                    .map(|am| -> ainxt_sampler::SharedBearerResolver {
    ...
    pub(crate) async fn wire_permission_auto_llm_classifier(self: &Arc<Self>) {
        ...
        let (tx, mut rx) = tokio::sync::mpsc::unbounded_channel::<(
            Vec<ainxt_workspace::permission::ClassifierMessage>,
            ...
            while let Some((messages, respond_to)) = rx.recv().await {
                let result = async {
                    ...
                    let items = messages
                        .into_iter()
                        .map(|m| match m.role {
                            ainxt_workspace::permission::ClassifierMessageRole::System => {
                                ConversationItem::system(m.text)
                            }
                            ainxt_workspace::permission::ClassifierMessageRole::User => {
                    ...
                    let request = ConversationRequest {
                        ...
                        json_schema: Some(
                            ainxt_workspace::permission::classifier_output_json_schema(),
                        ),
                        reasoning_effort: classifier_reasoning_effort,
                        x_ainxt_conv_id: Some(session_id.clone()),
                        x_ainxt_req_id: Some(format!("ainxt-perm-auto-{}", uuid::Uuid::new_v4())),
                        x_ainxt_session_id: Some(session_id),
        ...
        let clf =
            ainxt_workspace::permission::LlmPermissionClassifier::with_channel(tx, prompt_type);
        debug_assert!(
            clf.has_side_query(),
    ...
    /// catalog routing (Tier-1 catalog creds / Tier-2 ainxt-proxy via session token
    /// / `AINXT_API_KEY` / deployment key), gathering the session-local auth context
    ...
    pub(super) async fn resolve_aux_sampler_config(
        &self,
        slug: &str,
    ) -> Option<ainxt_sampler::SamplerConfig> {
        let creds = self.chat_state_handle.get_credentials().await;
        ...
        let disable_api_key_auth = self
            .auth_manager
            .as_ref()
            .map(|am| am.ainxt_com_config().api_key_auth_disabled())
            .unwrap_or(false);
    ...
    async fn resolve_auto_classifier_sampler(
        &self,
        slug: &str,
    ) -> Option<(ainxt_sampler::SamplingClient, String)> {
        ...
        let client = ainxt_sampler::SamplingClient::new(cfg)
            .map_err(|e| {
                tracing::warn!(
                    error = % e,
    ...
    pub(super) async fn prepare_chat_completion(
        &self,
        force_http1: bool,
    ) -> Result<ainxt_sampler::SamplingClient, acp::Error> {
        self.refresh_token_if_expired().await;
        let mut full_config = self.reconstruct_full_config().await;
        full_config.force_http1 = force_http1;
        let sampling_client =
            ainxt_sampler::SamplingClient::new(full_config).map_err(|e| self.to_acp_error(e))?;
        Ok(sampling_client)
    }
    /// Push a fresh `SamplerConfig` into the per-session sampler actor
    /// before each turn. Matches `prepare_chat_completion`'s
    /// auth-refresh + config rebuild, but routes the result to the
    /// `ainxt-sampler` instead of constructing a new
    /// `OaiCompatClient`.
    ///
    /// Behaviour equivalence: we run the same `refresh_token_if_expired()`
    ...
    pub(crate) async fn handle_sampling_failure(
        self: &Arc<Self>,
        error: ainxt_sampler::SamplingErrorInfo,
    ) -> Result<SamplerFailureRecovery, acp::Error> {
        use ainxt_sampler::SamplingErrorKind;
        ...
        let client_version = ainxt_version::VERSION;
        if auth_mode == crate::auth::AuthMode::WebLogin {
            let msg = format!(
                "{detailed_message}\n\n\
                 ...
                 To fix: run `ainxt logout` then `ainxt login` to re-authenticate with OAuth2.\n\n\
        ...
        let error_type = if ainxt_sampling_types::is_context_length_error(&error.message) {
            "context_length"
        } else {
            error.kind.as_str()
        };
    ...
    ///    `send_ainxt_notification(RetryState::Failed)`.
    pub(crate) async fn run_turn_via_sampler(
        ...
        let request_id = ainxt_sampler::RequestId::random();
            ...
            .await
        {
            ...
            Err(rich_err) => {
                self.turn_stream_drained.lock().take();
                let info = ainxt_sampler::SamplingErrorInfo::from(&rich_err);
