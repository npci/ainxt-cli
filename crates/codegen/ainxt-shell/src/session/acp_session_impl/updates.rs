...
//! its buffered/transient/direct variants, ainxt-notification handling, and
...
impl SessionActor {
    ...
    pub(super) async fn record_subagent_usage(
        &self,
        by_model: &[(String, ainxt_chat_state::UsageTotals)],
    ...
    async fn send_update_full(
        ...
        // Per-session cost (USD * 1e10). Gateway-reported costs are used as-is;
        // calls the gateway left unpriced are estimated from the model's
        // published rate (in-house models settle at 0 => free), and the
        // `sessionCostEstimated` flag tells the pager to render the figure as
        // approximate. Piggybacks on the same `_meta` that carries
        // `totalTokens` so no new ACP method is needed — the pager reads it as
        // `sessionCostTicks`. `None` only before the first call completes.
        let session_cost = self
            .chat_state_handle
            .try_get_session_usage()
            .await
            .ok()
            .and_then(|ledger| ledger.totals.displayable_cost());
        ...
        if let Some(cost) = session_cost {
            obj.insert("sessionCostTicks".to_string(), cost.ticks.into());
            // Only stamped when true, so an older pager that ignores the flag
            // simply shows the figure unmarked rather than mislabelling an
            // exact gateway total as an estimate.
            if cost.estimated {
                obj.insert("sessionCostEstimated".to_string(), true.into());
            }
        }
        if is_replay {
    ...
    /// Producer for the **high-frequency streaming path** with an ainxt
    /// extension payload. Routes through `event_tx` -> `ReplayBuffer` ->
    /// `emit_buffered` so chunks get merged + debounced + emitted.
    ///
    /// For one-shot ainxt events (RetryState, ImageCompressed, HookExecution,
    /// AutoCompactCompleted, etc.), use `send_ainxt_notification` instead.
    ///
    /// The frequency-based split (`send_buffered_ainxt_update` vs `send_ainxt_notification`)
    /// matches the ACP-side split between `send_update` (high-frequency,
    /// buffered) and `emit_notification_direct` (low-frequency, direct).
    pub(super) async fn send_buffered_ainxt_update(&self, update: AinxtSessionUpdate) {
        self.close_rewind_window().await;
        let notification = AinxtSessionNotification {
    ...
    /// - **ainxt** (`ToolCallDeltaChunk`) -> inlines a gateway
    ...
    pub(super) async fn emit_buffered(&self, notification: SessionNotification) {
        match notification {
            ...
            SessionNotification::Ainxt(n) => {
                self.log_outbound_ainxt_buffered(&n);
                if self
                    .notifications
                    .gateway_enabled
    ...
    /// Tracing log for buffered ainxt notifications emerging from
    /// emit_buffered. Matches `log_outbound_notification` for ACP.
    /// Visible with `RUST_LOG=acp_event=info`.
    fn log_outbound_ainxt_buffered(&self, notification: &AinxtSessionNotification) {
        if !matches!(
            notification.update,
            AinxtSessionUpdate::ToolCallDeltaChunk { .. }
        ) {
            return;
        }
        tracing::info!(
            target : "acp_event", event = "ainxt_buffered_notification_sent", session_id =
            % self.session_info.id, "Sending buffered ainxt session notification"
        );
    }
    ...
    /// Handle ainxt session notifications - store them in persistence
    ...
    pub(super) async fn handle_ainxt_session_notification(
        &self,
        mut notification: AinxtSessionNotification,
    ) {
        if !matches!(
            notification.update,
            AinxtSessionUpdate::SubagentProgress { .. }
        ) {
            tracing::debug!("storing ainxt session notification");
        }
        {
        ...
        match &notification.update {
            AinxtSessionUpdate::SubagentSpawned {
                ...
                let envelope = self.fire_hook(
                    ainxt_hooks::event::HookEventName::SubagentStart,
                    None,
                    ainxt_hooks::event::HookPayload::SubagentStart {
                ...
                if let Some(registry) = hook_registry_snapshot {
                    ...
                    let _ = ainxt_hooks::dispatcher::dispatch_non_blocking(
                        &registry,
                        ainxt_hooks::event::HookEventName::SubagentStart,
                        &envelope,
                        &ctx,
            ...
            AinxtSessionUpdate::SubagentFinished {
                ...
                let envelope = self.fire_hook(
                    ainxt_hooks::event::HookEventName::SubagentEnd,
                    None,
                    ainxt_hooks::event::HookPayload::SubagentStop {
                        subagent_id: subagent_id.clone(),
        ...
        let _ = self
            ...
            .persistence_tx
            .send(PersistenceMsg::Update(
                crate::session::storage::SessionUpdate::Ainxt(Box::new(notification)),
            ));
    }
    /// Persist an ainxt extension notification to `updates.jsonl` **without** sending it
    ...
    pub(super) fn persist_ainxt_update_only(&self, update: AinxtSessionUpdate) {
        let notification = AinxtSessionNotification {
            session_id: self.session_info.id.clone(),
            update,
        ...
        if self
            ...
            .is_err()
        {
            tracing::warn!("Failed to send ainxt update to persistence channel");
        }
    }
    ...
    pub(super) async fn dispatch_notification_hook(
        ...
        let envelope = self.fire_hook(
            ainxt_hooks::event::HookEventName::Notification,
            None,
            ainxt_hooks::event::HookPayload::Notification {
    ...
    /// Send an ainxt extension notification to the client
    #[tracing::instrument(skip_all)]
    pub(super) async fn send_ainxt_notification(&self, update: AinxtSessionUpdate) {
        self.send_ainxt_notification_with_extra_meta(update, None)
            .await;
    }
    /// [`Self::send_ainxt_notification`] with caller-supplied `_meta` keys merged
    /// into the standard eventId/timestamp meta. Caller keys win on collision.
    #[tracing::instrument(skip_all)]
    pub(super) async fn send_ainxt_notification_with_extra_meta(
        &self,
        ...
        let _ = self
            ...
            .persistence_tx
            .send(PersistenceMsg::Update(
                crate::session::storage::SessionUpdate::Ainxt(Box::new(notification.clone())),
            ));
...
#[cfg(test)]
mod ainxt_event_id_stamping_tests {
    use super::support::create_test_actor;
    use super::*;
    fn persisted_ainxt_event_id(
        prx: &mut tokio::sync::mpsc::UnboundedReceiver<PersistenceMsg>,
    ) -> String {
        loop {
            match prx.try_recv().expect("an ainxt line must be persisted") {
                PersistenceMsg::Update(crate::session::storage::SessionUpdate::Ainxt(notif)) => {
                    return notif
                        .meta
                        .as_ref()
                        .and_then(|m| m.get("eventId"))
                        .and_then(|v| v.as_str())
                        .expect("persisted ainxt lines must carry an eventId")
                        .to_string();
                }
                _ => continue,
            }
    ...
    /// `send_ainxt_notification` (own emission) and
    /// `handle_ainxt_session_notification` (inbound/forwarded, meta-less) —
    ...
    #[tokio::test]
    async fn actor_persisted_ainxt_lines_carry_event_id() {
        let local = tokio::task::LocalSet::new();
        local
            .run_until(async {
                let (gateway_tx, _gateway_rx) =
                    tokio::sync::mpsc::unbounded_channel::<ainxt_acp_lib::AcpClientMessage>();
                let (persistence_tx, mut prx) =
                    tokio::sync::mpsc::unbounded_channel::<PersistenceMsg>();
                let actor = create_test_actor(0, 256_000, 85, gateway_tx, persistence_tx).await;
                actor
                    .send_ainxt_notification(AinxtSessionUpdate::HookAnnotation {
                        message: "own emission".into(),
                    })
                    .await;
                let own_id = persisted_ainxt_event_id(&mut prx);
                assert!(own_id.starts_with("test-actor-"));
                actor
                    .handle_ainxt_session_notification(AinxtSessionNotification {
                        session_id: acp::SessionId::new("test-actor"),
                        update: AinxtSessionUpdate::HookAnnotation {
                            message: "inbound".into(),
                        },
                        meta: None,
                    })
                    .await;
                let inbound_id = persisted_ainxt_event_id(&mut prx);
                assert!(inbound_id.starts_with("test-actor-"));
                assert_ne!(own_id, inbound_id);
                actor.persist_ainxt_update_only(AinxtSessionUpdate::HookAnnotation {
                    message: "persist-only".into(),
                });
                let persist_only_id = persisted_ainxt_event_id(&mut prx);
