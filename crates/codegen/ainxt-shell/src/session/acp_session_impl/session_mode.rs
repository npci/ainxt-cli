...
impl SessionActor {
        ...
        *self.current_prompt_mode.lock() = prompt_mode;
        ...
        if mode.is_plan() {
            ...
            ainxt_telemetry::session_ctx::log_event(
                ainxt_telemetry::events::PlanModeToggled {
                    enabled: true,
                    trigger: ainxt_telemetry::events::PlanModeTrigger::User,
                    turn_in_flight,
    ...
    ///
    /// Matches `handle_session_mode` but driven from `_meta.mode` on the
    ...
    /// first-entry/reentry distinction), or ainxt's reentry/full variant.
