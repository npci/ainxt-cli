...
// ainxt-interjection-core crate so the server-side agent loop can adopt the
...
pub(crate) use ainxt_interjection_core::{InterjectionBuffer, drain_formatted, format_interjection};

/// Shell instantiation of the shared entry type: images are ACP content.
pub(crate) type PendingInterjection = ainxt_interjection_core::PendingInterjection<acp::ImageContent>;
...
/// `ainxt.dev/session/interjection` broadcast, so a live echo would duplicate it.
...
impl SessionActor {
    ...
    pub(super) async fn drain_pending_interjections(&self) -> bool {
        ...
        for PendingInterjection { text, attachments } in entries {
            ...
            // (matches turn-start skills, which replay via `displayText`).
