...
use super::*;
/// Partition the AGENTS.md / Claude.md / `.ainxt/rules/*.md` files returned
/// by `read_agents_config_with_paths` into "workspace" (cwd / repo root /
/// extra workspace user dir) and "user" (`~/.ainxt/`, `~/.claude/`,
/// `~/.ainxt/bundled/`) buckets, mirroring the split between
...
fn partition_rules_by_scope(
    files: Vec<ainxt_agent::prompt::agents_md::AgentConfigFile>,
) -> (
    Vec<ainxt_agent::prompt::user_message::RuleEntry>,
    Vec<ainxt_agent::prompt::user_message::RuleEntry>,
) {
    let home = dirs::home_dir().map(|p| p.to_string_lossy().to_string());
    let user_prefixes: Vec<String> = match home {
        Some(h) => vec![format!("{h}/.ainxt/"), format!("{h}/.claude/")],
        None => vec![],
    };
    let mut workspace = Vec::new();
    let mut user = Vec::new();
    for f in files {
        let entry = ainxt_agent::prompt::user_message::RuleEntry::from(f);
...
/// end for ainxt ordering), so a failed offload never
...
impl SessionActor {
    ...
    async fn build_templated_user_message(
        &self,
        cwd: &std::path::Path,
        template: ainxt_agent::prompt::user_message::UserMessageTemplate,
    ) -> Option<String> {
        use ainxt_agent::prompt::agents_md::read_agents_config_with_paths;
        use ainxt_agent::prompt::user_message::UserMessageContext;
    ...
    async fn gather_vcs_for_prefix(
        ...
        use ainxt_workspace::file_system::{git_status_short, jj_status};
        use ainxt_workspace::session::git::VcsKind;
    ...
    ) -> Vec<ainxt_agent::prompt::user_message::McpServerEntry> {
        use ainxt_agent::prompt::user_message::McpServerEntry;
    ...
    pub(super) async fn transcribe_user_images(
        ...
        let current_query = crate::session::image_describe::strip_template_context_tags(
            &ainxt_chat_state::compaction_utils::extract_user_query(&original_user_message),
        );
        ...
        let client = ainxt_sampler::SamplingClient::new(sampler_config).map_err(|e| {
