...
/// Prefix on `x_ainxt_req_id` for laziness-classifier sampler calls.
...
pub(crate) const LAZINESS_REQ_ID_PREFIX: &str = "ainxt-laziness-";
        ...
        match item {
            ...
            ConversationItem::User(user) => {
                ...
                for part in &user.content {
                    if let ainxt_sampling_types::ContentPart::Text { text: t } = part {
                        if !text.is_empty() {
                            text.push(' ');
                        }
                        text.push_str(t);
            ...
            ConversationItem::Reasoning(r) => {
                if include_reasoning {
                    let text = ainxt_sampling_types::reasoning_item_text(r);
...
/// Matches the laziness classifier's 400-char field cap; truncation appends the
const CLASSIFIER_TURN_MAX_LEN: usize = ainxt_workspace::permission::CLASSIFIER_TURN_MAX_LEN;
...
pub(crate) fn build_classifier_turns(
    items: &[ConversationItem],
    max_items: usize,
) -> Vec<ainxt_workspace::permission::ClassifierTurn> {
        ...
        match item {
            ConversationItem::User(user) => {
                ...
                if !text.is_empty() {
                    ...
                    let text = ainxt_tools::util::truncate_str_with_marker(
                        &text,
            ...
            ConversationItem::Assistant(assistant) => {
                for tc in &assistant.tool_calls {
                    ...
                    let args = ainxt_tools::util::truncate_str_with_marker(
                        &args,
                        CLASSIFIER_TURN_MAX_LEN,
                    )
                    .into_owned();
