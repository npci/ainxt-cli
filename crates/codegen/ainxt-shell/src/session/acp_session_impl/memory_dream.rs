...
pub(super) struct MemoryFlushSnapshot {
    counts: ainxt_chat_state::ConversationCounts,
...
impl SessionActor {
    ...
    pub(super) async fn register_memory_tools(
        &self,
        bridge: &ainxt_tools::bridge::ToolBridge,
    ) -> Result<(), String> {
        ...
        bridge
            .register_mcp_tools(
                ...
                ainxt_tools::implementations::memory::search_tool::MemorySearchImpl,
                None,
            )
            .await
        ...
        bridge
            .register_mcp_tools(
                ...
                ainxt_tools::implementations::memory::get_tool::MemoryGetImpl,
                None,
            )
            .await
    ...
    pub(super) fn emit_memory_session_summary(
        ...
        ainxt_telemetry::session_ctx::log_event(
            ainxt_telemetry::memory_telemetry::MemorySessionSummary {
    ...
    async fn run_dream_inner(
        ...
        self.send_ainxt_notification(AinxtSessionUpdate::MemoryDreamCompleted {
    ...
    async fn run_dream_model_call(&self, user_message: &str) -> Result<String, acp::Error> {
        ...
        let request = ConversationRequest {
            ...
            x_ainxt_req_id: Some(format!("ainxt-dream-{}", uuid::Uuid::new_v4())),
    ...
    pub(super) async fn run_memory_flush(
        ...
        tracing::info!(target: ainxt_telemetry::memory_log::TARGET, "MEMORY_FLUSH: starting");
        let flush_start = std::time::Instant::now();

        self.send_ainxt_notification(AinxtSessionUpdate::MemoryFlushStarted)
            .await;
        ...
        let result = async {
            ...
            ainxt_telemetry::session_ctx::log_event(
                ainxt_telemetry::memory_telemetry::MemoryFlushStart {
                    session_id: self.session_info.id.to_string(),
            ...
            let request = ConversationRequest {
                ...
                model: Some(model),
                x_ainxt_conv_id: Some(session_id.clone()),
                x_ainxt_req_id: Some(format!("ainxt-flush-{}", uuid::Uuid::new_v4())),
                x_ainxt_session_id: Some(session_id.clone()),
                x_ainxt_agent_id: Some(ainxt_telemetry::id::agent_id()),
                ..Default::default()
            };
        ...
        tracing::info!(target: ainxt_telemetry::memory_log::TARGET, outcome = %outcome, "MEMORY_FLUSH: completed");
        ...
        ainxt_telemetry::session_ctx::log_event(
            ainxt_telemetry::memory_telemetry::MemoryFlushComplete {
                session_id: self.session_info.id.to_string(),
                trigger: trigger.to_owned(),
        ...
        let flush_trigger = match trigger {
            "slash_command" => ainxt_telemetry::events::MemoryFlushTrigger::SlashCommand,
            "interval" => ainxt_telemetry::events::MemoryFlushTrigger::Interval,
            "pre_compaction" => ainxt_telemetry::events::MemoryFlushTrigger::PreCompaction,
            _ => ainxt_telemetry::events::MemoryFlushTrigger::UserRequested,
        };
        ainxt_telemetry::session_ctx::log_event(ainxt_telemetry::events::MemoryFlushed {
        ...
        self.send_ainxt_notification(AinxtSessionUpdate::MemoryFlushCompleted {
            result: outcome,
            path: flush_path,
        })
        .await;
    ...
    pub(super) async fn snapshot_memory_flush_state(&self) -> MemoryFlushSnapshot {
        ...
        let chat_history = crate::sampling::conversation_to_chat_messages(
            ainxt_chat_state::compaction_utils::prepare_conversation_for_summarization(conversation),
        );
        MemoryFlushSnapshot {
            counts,
            chat_history,
    ...
    /// LLM call using the `ainxt-build` model.
    ///
    ...
    pub(super) async fn handle_rewrite_memory_note(
        ...
        let request = ConversationRequest {
            items,
            tools: vec![],
            model: Some("ainxt-build".to_owned()),
        ...
        let result = match sampling_client.api_backend() {
            crate::sampling::ApiBackend::ChatCompletions => {
                ...
                let events =
                    ainxt_sampler::stream_chat_completions(raw, meta, request_id, idle_timeout);
                ainxt_sampler::collect_response(events).await
            }
            crate::sampling::ApiBackend::Responses => {
                let (raw, meta, doom_loop) = sampling_client
                    ...
                    .await
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events = ainxt_sampler::stream_responses(
                    raw,
                    meta,
                    request_id,
                    idle_timeout,
            ...
            crate::sampling::ApiBackend::Messages => {
                let (raw, meta) = sampling_client
                    ...
                    .await
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events = ainxt_sampler::stream_messages(raw, meta, request_id, idle_timeout);
                ainxt_sampler::collect_response(events).await
            }
        };
