...
impl SessionActor {
    ...
    /// Emit `ainxt.dev/git_head_changed` after an edit/shell command that may have
    ...
    pub(super) async fn maybe_notify_git_branch(&self) {
        ...
        let (worktree_info, branch) = tokio::join!(
            ainxt_workspace::session::git::get_worktree_info(cwd),
            ainxt_workspace::session::git::get_branch(cwd),
        );
        ...
        if let Ok(raw) = serde_json::value::to_raw_value(&params) {
            let notification = acp::ExtNotification::new("ainxt.dev/git_head_changed", raw.into());
            self.notifications
                .gateway
    ...
    pub(super) async fn outstanding_reply_for_prompt(
        &self,
        prompt_id: &str,
    ) -> Option<ainxt_tools::implementations::ainxt_build::task::types::SubagentOutstandingReply>
    {
    ...
    pub(super) async fn handle_completion(&self, prompt_id: String, result: PromptTurnResult) {
        ...
        if became_idle {
            ...
            self.agent
                ...
                .tool_bridge()
                .update_resource(
                    ainxt_tools::implementations::ainxt_build::task::types::CurrentPromptIdResource(
                        String::new(),
                    ),
                )
                .await;
        ...
        // replayed `_ainxt.dev/session/update` rail so a viewer that re-attaches
    ...
    /// persists + forwards via `send_ainxt_notification`. Retiring
    ...
    pub(super) async fn emit_turn_completed(
        ...
        self.send_ainxt_notification_with_extra_meta(
    ...
    pub(super) fn classify_install_error(
        err: &ainxt_agent::plugins::install_registry::InstallError,
    ) -> String {
