...
pub(crate) struct LazinessDebugLogLine {
    ...
    /// the response failed. Surfaced unchanged so an operator can see
...
impl SessionActor {
    ...
    ///
    /// **Invisibility contract** (matches the memory-flush path):
    ...
    /// sees "ainxt is thinking", streaming token chunks, or any other
    ...
    pub(crate) async fn maybe_fire_laziness_check(self: Arc<Self>) {
        ...
        let request = ConversationRequest {
            ...
            // Don't pass `reasoning_effort` — `ainxt-4.5` (and
            // other tool-flavoured ainxt variants) reject the field at
            ...
            x_ainxt_conv_id: Some(session_id_str.clone()),
            x_ainxt_req_id: Some(format!("{LAZINESS_REQ_ID_PREFIX}{}", uuid::Uuid::new_v4())),
            x_ainxt_session_id: Some(session_id_str),
