...
pub(super) async fn run_session(
    ...
    mut chat_state_event_rx: mpsc::UnboundedReceiver<ainxt_chat_state::ChatStateEvent>,
    ...
    if !session.startup_hints.is_subagent && liveness_watchers_enabled {
        let (event_tx, event_rx) =
            tokio::sync::mpsc::unbounded_channel::<ainxt_mcp::servers::McpClientEvent>();
        {
    ...
    loop {
        tokio::select! {
            ...
            ainxt_telemetry::memory_log::TARGET,
            ...
            } else { tracing::debug!(target : ainxt_telemetry::memory_log::TARGET,
            ...
            ainxt_telemetry::memory_log::TARGET, "MEMORY_DREAM_CHECK: timer fired");
            ...
            Some(ainxt_chat_state::ChatStateEvent::ConversationReset { new_len }) => {
            ...
            Some(ainxt_chat_state::ChatStateEvent::ImageBudget { body_bytes, trigger_bytes,
            reclaim_target_bytes, inline_images, needs_image_compaction, evicted,
            body_bytes_after, }) => {
            ainxt_telemetry::unified_log::info("shell.image_budget", Some(session
            ...
            })),); } Some(ainxt_chat_state::ChatStateEvent::PromptIndexChanged { .. }) |
            Some(ainxt_chat_state::ChatStateEvent::TokensUpdated { .. }) => {} None => {} }
            ...
            .fire_hook(ainxt_hooks::event::HookEventName::SessionEnd, None,
            ainxt_hooks::event::HookPayload::SessionEnd { reason : "channel_closed"
            ...
            ainxt_hooks::dispatcher::dispatch_non_blocking(& registry,
            ainxt_hooks::event::HookEventName::SessionEnd, & envelope, & ctx,). await;
            session.send_hook_execution("session_end", None, None, & results). await; }
            let envelope = session.fire_hook(ainxt_hooks::event::HookEventName::Stop,
            None, ainxt_hooks::event::HookPayload::Stop { reason : "channel_closed"
            .to_string(), },); if let Some(registry) = session.hook_registry.borrow()
            .clone() { let ctx = session.hook_run_ctx(); let results =
            ainxt_hooks::dispatcher::dispatch_non_blocking(& registry,
            ainxt_hooks::event::HookEventName::Stop, & envelope, & ctx,). await;
            ...
            ainxt_telemetry::memory_log::TARGET, result = ? result, tool_searches =
            ...
            session.send_ainxt_notification(AinxtSessionUpdate::MemorySessionSaved { path :
            ...
            ainxt_telemetry::session_ctx::log_event(ainxt_telemetry::events::SessionEnded
            ...
            ainxt_file_utils::trace_context::link_current_span_to_meta(& meta); } let
            ...
            .update_credentials(ainxt_chat_state::Credentials { api_key : r.api_key,
            ...
            SessionCommand::GetSessionUsage { responds_to } => { let usage = session
            .build_session_usage(). await; let _ = responds_to.send(usage); }
            ...
            .send(ainxt_hooks_plugins_types::HooksListResponse { hooks, project_trusted,
            ...
            .send_ainxt_notification(AinxtSessionUpdate::PluginUpdatesInstalled { updates },)
            ...
            .push_capped(ainxt_tools::implementations::ainxt_build::task::types::MonitorEventNotification
            ...
            ainxt_agent::prompt::skills::list_skills_with_plugins(Some(& cwd), &
            ...
            SessionCommand::AinxtSessionNotification { notification } => { session
            .handle_ainxt_session_notification(notification). await; }
            ...
            .send(ainxt_mcp::servers::McpClientEvent::ConfigDiff { added : diff.added
            ...
            .emit(ainxt_file_utils::events::Event::McpServerToggled { server_name :
            ...
            .forward_fire_and_forget(acp::ExtNotification::new("ainxt.dev/mcp/tools_changed",
            ...
            session.fire_hook(ainxt_hooks::event::HookEventName::SessionStart, None,
            ainxt_hooks::event::HookPayload::SessionStart { source, model_id : None,
            agent_type : None, },); if let Some(registry) = session.hook_registry
            .borrow().clone() { let ctx = session.hook_run_ctx(); let results =
            ainxt_hooks::dispatcher::dispatch_non_blocking(& registry,
            ainxt_hooks::event::HookEventName::SessionStart, & envelope, & ctx,).
            ...
            .fire_hook(ainxt_hooks::event::HookEventName::SessionEnd, None,
            ainxt_hooks::event::HookPayload::SessionEnd { reason : "shutdown"
            .to_string(), turn_count : None, tool_call_count : None, },); if let
            Some(registry) = session.hook_registry.borrow().clone() { let ctx = session
            .hook_run_ctx(); let results =
            ainxt_hooks::dispatcher::dispatch_non_blocking(& registry,
            ...
            session.send_hook_execution("session_end", None, None, & results). await; }
            let envelope = session.fire_hook(ainxt_hooks::event::HookEventName::Stop,
            None, ainxt_hooks::event::HookPayload::Stop { reason : "shutdown"
            .to_string(), },); if let Some(registry) = session.hook_registry.borrow()
            .clone() { let ctx = session.hook_run_ctx(); let results =
            ainxt_hooks::dispatcher::dispatch_non_blocking(& registry,
            ainxt_hooks::event::HookEventName::Stop, & envelope, & ctx,). await;
...
pub(super) fn turn_texts_for_feedback(
    conversation: &[ainxt_sampling_types::ConversationItem],
    turn_number: usize,
    ...
    let extracted = ainxt_chat_state::compaction_utils::extract_user_query(&raw);
