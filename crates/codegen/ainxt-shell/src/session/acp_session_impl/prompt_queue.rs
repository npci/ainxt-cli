...
impl SessionActor {
    ...
    pub(super) fn broadcast_queue_changed(&self, state: &State) {
        ...
        tracing::debug!(
            ...
            session = self.session_info.id.0.as_ref(),
            "broadcasting ainxt.dev/queue/changed to subscribers",
        );
    ...
    ///
    /// Matches [`handle_remove_queued_prompt`]'s versioned/owner gate and
    ...
    ///
    /// Always re-broadcasts `ainxt.dev/queue/changed` so every client reconciles
    ...
    /// 3. Re-broadcast `ainxt.dev/queue/changed` so every subscriber renders the
    ...
    /// (matches the pager's local-row edit semantics). Other non-text
