...
impl SessionActor {
    ...
    /// Cancel the running turn for a send-now prompt: Ctrl+C equivalence (kills the
    ...
    pub(super) async fn cancel_running_task(
        ...
        let (running_task, pending_inputs, rewound_input, had_queued_user_prompt) = {
            ...
            //   user's next queued prompt) and rebroadcasts `ainxt.dev/queue/changed`.
        ...
        if rewound_input.is_none() {
            ...
            // (matches the cancel-rate skip above).
