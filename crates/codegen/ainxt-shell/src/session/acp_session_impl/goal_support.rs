    ...
    pub(super) fn into_parts(
        ...
        Option<
            tokio::sync::oneshot::Sender<
                ainxt_tools::implementations::ainxt_build::update_goal::UpdateGoalAck,
            >,
        >,
    ) {
        match self {
...
pub(super) fn try_send_ack(
    ack_tx: Option<
        ...
        >,
    >,
    ack: ainxt_tools::implementations::ainxt_build::update_goal::UpdateGoalAck,
) {
...
impl NotAchievedSyntheticReason {
    ...
    /// synthetic (no-sampler) `NotAchieved` path. Matches the bullet
...
/// `Some` only when the planner is enabled (`AINXT_GOAL_PLANNER`) and a plan
...
/// Override with `AINXT_GOAL_REVERIFY_AFTER` (floored at 1).
...
///
/// `block_recap` and `goal_state` are inserted unchanged — pass an
...
/// the discipline; `None` renders the no-plan block exactly unchanged.
///
...
///
/// `plan_pointer` is inlined unchanged; `bail_preface`, `verifier_gaps`
...
mod fold_tokens_by_model_tests {
    ...
    fn mixed_models_sum_marginals_sorted_desc() {
        let records = vec![
            rec(Some("g1"), 0, 100, Some("ainxt-3")),
            rec(Some("g1"), 100, 500, Some("ainxt-4")), // marginal 400
            rec(Some("g1"), 0, 50, Some("ainxt-3")),    // ainxt-3 total 150
        ];
        let out = fold_tokens_by_model(&records, "g1", "cur");
        assert_eq!(
            out,
            vec![("ainxt-4".to_owned(), 400), ("ainxt-3".to_owned(), 150)]
        );
    }
    ...
    fn single_distinct_model_collapses_to_one_entry() {
        let records = vec![
            rec(Some("g1"), 0, 100, Some("ainxt-4")),
            rec(Some("g1"), 0, 200, None), // folds under current = ainxt-4
        ];
        let out = fold_tokens_by_model(&records, "g1", "ainxt-4");
        assert_eq!(out, vec![("ainxt-4".to_owned(), 300)]);
    }
    ...
    fn other_goal_records_excluded() {
        let records = vec![
            rec(Some("g1"), 0, 100, Some("ainxt-4")),
            rec(Some("g2"), 0, 999, Some("ainxt-4")),
            rec(None, 0, 999, Some("ainxt-4")),
        ];
        let out = fold_tokens_by_model(&records, "g1", "cur");
        assert_eq!(out, vec![("ainxt-4".to_owned(), 100)]);
    }

    #[test]
    fn last_below_anchor_does_not_underflow() {
        let records = vec![rec(Some("g1"), 500, 100, Some("ainxt-4"))];
    ...
    fn captured_model_survives_mid_goal_current_model_switch() {
        // A record captured `ainxt-4` at spawn keeps it even though the
        // current model at aggregation time is `ainxt-3`.
        let records = vec![rec(Some("g1"), 0, 100, Some("ainxt-4"))];
        let out = fold_tokens_by_model(&records, "g1", "ainxt-3");
        assert_eq!(out, vec![("ainxt-4".to_owned(), 100)]);
    }
...
pub(crate) fn goal_slash_and_harness_available(goal_enabled: bool, tool_names: &[String]) -> bool {
    use ainxt_tools::implementations::ainxt_build::UPDATE_GOAL_TOOL_NAME;
...
impl SessionActor {
    ...
    pub(super) async fn maybe_run_goal_planner(&self, objective: &str) {
        ...
        let parent_prompt_id = self
            ...
            .clone();
        // The planner is a unchanged mirror-child fork: its request prefix matches
        ...
        // ignored on this path (breadcrumb below for observability equivalence with the
        ...
        // toolset (the exact tools the unchanged mirror sends) — honoring renames
    ...
    /// drives the turn. Matches the `CurrentPromptIdResource` push.
    ///
    /// Also matches the value into `tool_context.goal_loop_active_gate`, the
    ...
    pub(super) async fn set_goal_loop_active_resource(&self, active: bool) {
        ...
        self.agent
            ...
            .tool_bridge()
            .update_resource(
                ainxt_tools::implementations::ainxt_build::task::types::GoalLoopActive(active),
            )
            .await;
    }
}
