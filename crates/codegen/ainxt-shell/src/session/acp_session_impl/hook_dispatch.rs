...
) -> ainxt_tool_protocol::turn_hook::TurnHookOutcome {
...
) -> ainxt_tool_protocol::session_event::ToolCallOutcome {
    use crate::session::events::ToolOutcome;
    use ainxt_tool_protocol::session_event::ToolCallOutcome;
    match outcome {
...
pub(super) fn notification_hook_for_update(
    ...
    match update {
        AinxtSessionUpdate::DiffReview { .. } => Some((
            "permission_prompt".into(),
            Some("Diff review requested".into()),
            None,
            Some("info".into()),
        )),
        AinxtSessionUpdate::AutoRecoveryExhausted { error, .. } => Some((
            "agent_error".into(),
            Some(error.clone()),
            None,
            Some("error".into()),
        )),
        AinxtSessionUpdate::RetryState(RetryState::Exhausted { reason, .. }) => Some((
            "agent_error".into(),
            Some(reason.clone()),
            None,
            Some("error".into()),
        )),
        AinxtSessionUpdate::RetryState(RetryState::Failed { message, .. }) => Some((
            "agent_error".into(),
            Some(message.clone()),
            None,
            Some("error".into()),
...
impl SessionActor {
    pub(super) fn hook_run_ctx(&self) -> ainxt_hooks::runner::RunContext<'_> {
        ainxt_hooks::runner::RunContext {
    ...
    pub(super) async fn send_hook_annotation(&self, message: &str) {
        self.send_ainxt_notification(AinxtSessionUpdate::HookAnnotation {
    ...
    pub(super) async fn send_hook_execution(
        ...
        results: &[ainxt_hooks::result::HookRunResult],
    ) {
        if results.is_empty()
            || results
                .iter()
                .all(|r| matches!(r, ainxt_hooks::result::HookRunResult::Skipped { .. }))
        {
            return;
        }
        use crate::extensions::notification::{HookRunEntryDto, HookRunStatusDto};
        use ainxt_hooks::result::HookRunResult;
        ...
        self.send_ainxt_notification(AinxtSessionUpdate::HookExecution {
    ...
    pub(super) async fn dispatch_hook(
        &self,
        event: ainxt_hooks::event::HookEventName,
        payload: ainxt_hooks::event::HookPayload,
        prompt_id: Option<&str>,
        tool_name: Option<&str>,
    ) {
        ...
        let results =
            ainxt_hooks::dispatcher::dispatch_non_blocking(&registry, event, &envelope, &ctx)
                .await;
    ...
    pub(super) async fn emit_hook_executed_telemetry(
        ...
        for r in results {
            let (hook_name, elapsed, outcome) = match r {
                ainxt_hooks::result::HookRunResult::Success {
                    hook_name, elapsed, ..
                } => (
                    hook_name,
                    elapsed,
                    ainxt_telemetry::events::HookOutcome::Success,
                ),
                ainxt_hooks::result::HookRunResult::Failed {
                    hook_name, elapsed, ..
                } => (
                    hook_name,
                    elapsed,
                    ainxt_telemetry::events::HookOutcome::Error,
                ),
                ainxt_hooks::result::HookRunResult::Skipped { .. } => continue,
            };
            ainxt_telemetry::session_ctx::log_event(ainxt_telemetry::events::HookExecuted {
...
mod notification_hook_filter_tests {
    ...
    fn hook_execution_does_not_fire_notification_hook() {
        let update = AinxtSessionUpdate::HookExecution {
    ...
    fn hook_annotation_does_not_fire_notification_hook() {
        let update = AinxtSessionUpdate::HookAnnotation {
    ...
    fn retry_in_progress_does_not_fire_notification_hook() {
        let update = AinxtSessionUpdate::RetryState(RetryState::Retrying {
            attempt: 1,
            max_retries: 3,
            reason: "timeout".into(),
        });
    ...
    fn feedback_request_does_not_fire_notification_hook() {
        let update = AinxtSessionUpdate::FeedbackRequest(FeedbackRequestNotification {
    ...
    fn diff_review_fires_permission_prompt() {
        let update = AinxtSessionUpdate::DiffReview { content: vec![] };
    ...
    fn task_completed_does_not_fire_via_filter() {
        let update = AinxtSessionUpdate::TaskCompleted {
            task_snapshot: ainxt_tools::types::TaskSnapshot {
                task_id: "task-1".into(),
