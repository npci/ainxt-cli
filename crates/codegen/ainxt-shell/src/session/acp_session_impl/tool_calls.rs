...
fn is_interruptible_wait_tool(tool_name: &str, args: &serde_json::Value) -> bool {
    match tool_name {
        ...
        | "get_terminal_command_output" => ainxt_tool_types::task_output_waits_from_json(args),
...
fn interrupted_wait_tool_result_with_msg(args: &serde_json::Value, msg: &str) -> ToolRunResult {
    use ainxt_tool_types::{TaskOutputOutput, TaskOutputResult};
    let task_id = args
        .get("task_ids")
...
pub(super) enum PlanEditGate {
    ...
    Allow,
    /// ainxt-toolset edit outside the plan file (plan-file-only rule).
    RejectNonPlanFile,
}
...
///   `AccessKind::Edit` and is plan-file-only (same as ainxt edits). Deleting
...
impl PlanApprovalOutcome {
    fn from_response(
        resp: &ainxt_tools::implementations::ainxt_build::exit_plan_mode::ExitPlanModeExtResponse,
    ) -> Self {
...
///
/// Uses `ainxt_acp_lib`'s TYPED [`AcpChannelFailure`](ainxt_acp_lib::AcpChannelFailure)
...
fn ext_method_no_client(err: &acp::Error) -> bool {
    matches!(
        ainxt_acp_lib::acp_channel_failure(err),
        Some(ainxt_acp_lib::AcpChannelFailure::SendFailed)
    )
}
...
impl SessionActor {
    /// Merge the canonical `ainxt.dev/tool` identity envelope into a tool-call
    ...
    pub(super) fn stamp_tool_meta(
        ...
        ainxt_tools::normalization::merge_tool_meta(
            &toolset,
    ...
    pub(super) async fn execute_tool_calls(
        ...
        for call in tool_calls.into_iter() {
            ...
            self.observability_bridge
                .emit(
                    ainxt_tool_protocol::session_event::SessionEvent::ToolCallStarted {
                ...
                .await?
            {
                ...
                Err(tool_loop) => {
                        ...
                        crate::session::mcp_servers::parse_mcp_tool_name(&call_name)
                    {
                        ...
                        self.emit_event(ainxt_file_utils::events::Event::McpToolCallCompleted {
        ...
        while let Some((idx, mut result)) = dispatch_rx.recv().await {
                ...
                && server.starts_with(crate::session::managed_mcp::MANAGED_MCP_PREFIX)
            {
                let auth_rejected = match &result {
                    Err(err) => ainxt_mcp::servers::is_auth_rejection_message(&err.to_string()),
                    Ok(tool_result) => {
                        tool_result.output.is_error()
                            && ainxt_mcp::servers::is_auth_rejection_message(
            ...
            // Feed the risk ledger the one thing the authorization path cannot
            // know: whether the command actually worked. Failure-loop detection
            // — the brute-force control — is inert without it, because counting
            // invocations alone cannot tell a build loop from a password attack.
            crate::agent::policy_effects::note_tool_outcome(
                &prepared.tool_name,
            ...
            let tool_loop = match result {
                Ok(tool_result) => {
                    ...
                    post_tool_use_result = self
                        .hook_event_active(ainxt_hooks::event::HookEventName::PostToolUse)
                        .then(|| {
                ...
                Err(err) => {
                    ...
                    if self
                        .hook_event_active(ainxt_hooks::event::HookEventName::PostToolUseFailure)
                    {
                        let raw_input: serde_json::Value =
                            serde_json::from_str(&prepared.raw_arguments)
                                .unwrap_or(serde_json::Value::Null);
                        let (tool_input_value, tool_input_truncated) =
                            ainxt_hooks::event::truncate_payload(raw_input);
                        let hook_tool_name = prepared.hook_tool_name();
                        self.dispatch_hook(
                            ainxt_hooks::event::HookEventName::PostToolUseFailure,
                            ainxt_hooks::event::HookPayload::PostToolUseFailure {
            ...
            if let Some(tool_result_value) = post_tool_use_result {
                ...
                let (tool_result_val, tool_result_truncated) =
                    ainxt_hooks::event::truncate_payload(tool_result_value);
                let hook_tool_name = prepared.hook_tool_name();
                self.dispatch_hook(
                    ainxt_hooks::event::HookEventName::PostToolUse,
                    ainxt_hooks::event::HookPayload::PostToolUse {
                        tool_name: hook_tool_name.to_owned(),
                        tool_use_id: prepared.call_id.clone(),
                        tool_input: tool_input_value,
            ...
            self.observability_bridge
                .emit(
                    ainxt_tool_protocol::session_event::SessionEvent::ToolCallCompleted {
                ...
                .await;
            let (ext_file_path, ext_parameters) = if ainxt_telemetry::external::is_active() {
            ...
            ainxt_telemetry::session_ctx::log_event(
                ainxt_telemetry::events::ToolCallCompleted {
                    tool_name: prepared.tool_name.clone(),
                    outcome: tool_outcome,
                    duration_ms,
    ...
    pub(crate) async fn prepare_tool_call(
        ...
        if self.hook_event_active(ainxt_hooks::event::HookEventName::PreToolUse) {
            let (hook_tool_input, hook_tool_input_truncated) =
                ainxt_hooks::event::truncate_payload(raw_input.clone());
            let envelope = self.make_hook_envelope(
                ainxt_hooks::event::HookEventName::PreToolUse,
                None,
                ainxt_hooks::event::HookPayload::PreToolUse {
            ...
            if let Some(registry) = hook_registry_snapshot {
                ...
                let pre_result =
                    ainxt_hooks::dispatcher::dispatch_pre_tool_use(&registry, &envelope, &ctx)
                        .await;
                ...
                .await;
                if let ainxt_hooks::result::HookDecision::Deny { reason, hook_name } =
                    pre_result.decision
                {
                    return Ok(Err(self
                        .deny_tool(
        ...
        if !plan_file_auto_approve {
            ...
            let (telemetry_access_kind, _access_detail) = match &access_kind {
                ainxt_workspace::permission::AccessKind::Read(p) => (
                    ainxt_telemetry::events::AccessKind::Read,
                    p.clone().unwrap_or_default(),
                ),
                ainxt_workspace::permission::AccessKind::Edit(p) => {
                    (ainxt_telemetry::events::AccessKind::Edit, p.clone())
                }
                ainxt_workspace::permission::AccessKind::Bash(cmd) => {
                    (ainxt_telemetry::events::AccessKind::Bash, cmd.clone())
                }
                ainxt_workspace::permission::AccessKind::Grep { path, glob } => (
                    ainxt_telemetry::events::AccessKind::Grep,
                    path.clone().or_else(|| glob.clone()).unwrap_or_default(),
                ),
                ainxt_workspace::permission::AccessKind::MCPTool { name, .. } => {
                    (ainxt_telemetry::events::AccessKind::Mcp, name.clone())
                }
                ainxt_workspace::permission::AccessKind::WebFetch(u) => {
                    (ainxt_telemetry::events::AccessKind::Web, u.clone())
                }
                ainxt_workspace::permission::AccessKind::WebSearch(q) => {
                    (ainxt_telemetry::events::AccessKind::Web, q.clone())
                }
            };
            ...
            ainxt_telemetry::session_ctx::log_event(
                ainxt_telemetry::events::PermissionPrompted {
                    tool_name: call.function.name.clone(),
            ...
            self.events.permission_resolved(
                ...
                match &decision {
                    Decision::Allow | Decision::Ask => {
                        ainxt_file_utils::events::types::PermissionDecision::Allow
                    }
                    Decision::Reject(_) | Decision::PolicyDeny(_) => {
                        ainxt_file_utils::events::types::PermissionDecision::Deny
                    }
                    Decision::Cancelled => {
                        ainxt_file_utils::events::types::PermissionDecision::Cancelled
                    }
                    Decision::FollowupMessage(_) => {
                        ainxt_file_utils::events::types::PermissionDecision::Followup
                    }
                },
                perm_start,
            );
            ...
            let (decision_outcome, _reject_reason) = match &decision {
                Decision::Allow | Decision::Ask => {
                    (ainxt_telemetry::events::PermissionOutcome::Allow, None)
                }
                Decision::Reject(reason) | Decision::PolicyDeny(reason) => (
                    ainxt_telemetry::events::PermissionOutcome::Deny,
                    Some(reason.to_string()),
                ),
                Decision::Cancelled => (
                    ainxt_telemetry::events::PermissionOutcome::Cancelled,
                    None,
                ),
                Decision::FollowupMessage(_) => (
                    ainxt_telemetry::events::PermissionOutcome::Followup,
                    None,
                ),
            };
            tracing::info_span!(
            ...
            .in_scope(|| {});
            ainxt_telemetry::session_ctx::log_event(
                ainxt_telemetry::events::PermissionDecisionPayload {
                    tool_name: call.function.name.clone(),
                    access_kind: telemetry_access_kind,
            ...
            match decision {
                Decision::PolicyDeny(ref reason) | Decision::Reject(ref reason) => {
                    ...
                    let (tool_input_value, tool_input_truncated) =
                        ainxt_hooks::event::truncate_payload(raw_input.clone());
                    self.dispatch_hook(
                        ainxt_hooks::event::HookEventName::PermissionDenied,
                        ainxt_hooks::event::HookPayload::PermissionDenied {
                            tool_name: resolved_tool_name.clone(),
    ...
    /// Issue the `ainxt.dev/exit_plan_mode` reverse-request and await the user's
    ...
    pub(super) async fn request_plan_approval(
        ...
        ainxt_tools::implementations::ainxt_build::exit_plan_mode::ExitPlanModeExtResponse,
        acp::Error,
    > {
        use agent_client_protocol::Client as _;
        use ainxt_tools::implementations::ainxt_build::exit_plan_mode::{
    ...
    /// Default mode. Matches the mid-turn exit so the resume re-park
        ...
        if deactivated {
            ...
            *self.turn_prompt_mode.lock() = PromptMode::Agent;
            ...
            self.enqueue_current_mode_update(acp::SessionModeId::new(
                ainxt_tools::types::SessionMode::Default.as_id(),
            ));
        }
    }
        ...
        let (title, kind, locations, content) = match tool_call_input {
            ...
            ToolInput::ReadFile(read_file) => (
                ...
                vec![
                    acp::ToolCallLocation::new(read_file.path).line(
                        ainxt_tools::normalization::norm_offset_i64(read_file.offset)
                            .map(|l| l as u32),
                    ),
                ],
                Vec::new(),
            ...
            ToolInput::WaitTasks(wait) => (
                format!(
                    ...
                    wait.task_ids.len(),
                    match wait.mode {
                        ainxt_tool_types::WaitMode::WaitAny => "wait_any",
                        ainxt_tool_types::WaitMode::WaitAll => "wait_all",
                    }
                ),
                acp::ToolKind::Other,
                vec![],
            ...
            ToolInput::Skill(skill) => {
                ainxt_telemetry::session_ctx::log_event(
                    ainxt_telemetry::events::SkillDispatched {
    ...
    async fn handle_tool_parse_error(
        ...
        call_id: &str,
        function_name: &str,
        err: ainxt_tool_runtime::ToolError,
        raw_arguments: &str,
        model_id: &str,
    ) -> Result<(), acp::Error> {
        tracing::error!(
    ...
    /// `ainxt_tools::reminders::task_completion::consumed_completion_ids`,
    ...
    fn record_git_pr_signals(&self, effective_tool_name: &str, result: &ToolRunResult) {
        use ainxt_telemetry::enums::PrCreationSource;
        use ainxt_tools::util::git_detect;
        match &result.output {
            ainxt_tools::types::output::ToolOutput::Bash(b) if b.exit_code == 0 => {
                ...
                if ops.pr_merged {
                    ...
                    ainxt_telemetry::session_ctx::log_event(
                        ainxt_telemetry::events::PrMerged {},
                    );
                }
            }
            ainxt_tools::types::output::ToolOutput::MCP(m)
    ...
    fn record_pr_created(
        &self,
        pr: ainxt_tools::util::git_detect::PrRef,
        source: ainxt_telemetry::enums::PrCreationSource,
    ) {
        self.signals_handle()
    ...
    pub(super) async fn handle_bridge_tool_success(
        ...
        let consumed_ids =
            ainxt_tools::reminders::task_completion::consumed_completion_ids(&result.output);
        if !consumed_ids.is_empty() {
        ...
        if matches!(
            ...
            ToolsToolOutput::SearchReplace(
                ainxt_tools::types::output::SearchReplaceOutput::EditsApplied(_)
            ) | ToolsToolOutput::Bash(_)
        ) {
            self.maybe_notify_git_branch().await;
        }
        if let ainxt_tools::types::output::ToolOutput::Bash(ref b) = result.output
            && b.was_bare_echo
        {
            ...
            acp_tool_update(&result.output, call_id, path_rewriter.as_ref(), tool_meta)
        {
            ...
            if matches!(
                &result.output,
                ainxt_tools::types::output::ToolOutput::EnterPlanMode(_)
                    | ainxt_tools::types::output::ToolOutput::ExitPlanMode(_)
            ) {
        ...
        let extraction = if !self.is_cursor_harness()
            ...
            ainxt_tools::util::base64_images::extract_base64_images(prompt_text)
        } else {
            ainxt_tools::util::base64_images::ExtractionResult {
                text: prompt_text,
                images: Vec::new(),
            }
        };
        ...
        if !extracted_images.is_empty() {
            ...
            if let Some((notice, notes)) = crate::session::image_normalize::dropped_to_envelope(
                ...
                self.send_ainxt_notification(AinxtSessionUpdate::ImageDropped { notes })
                    .await;
            }
    ...
    /// Translate one [`ainxt_sampler::SamplingEvent`] from the
    ...
    pub(crate) async fn handle_sampling_event(
        self: &Arc<Self>,
        event: ainxt_sampler::SamplingEvent,
    ) {
        use ainxt_sampler::{SamplingChannel, SamplingEvent};
        match event {
            ...
            SamplingEvent::ToolCallDelta {
                ...
                self.send_buffered_ainxt_update(AinxtSessionUpdate::ToolCallDeltaChunk {
                    tool_call_id: id,
                    tool_index,
                    name,
                    arguments_delta,
            ...
            SamplingEvent::Retrying {
                ...
                if kind == ainxt_sampler::SamplingErrorKind::DoomLoopDetected {
                ...
                self.send_ainxt_notification(AinxtSessionUpdate::RetryState(
...
fn execute_tool_call_parts(
    ...
    let display = ainxt_tools::util::strip_redundant_session_cd(command, cwd);
    (
...
mod plan_mode_edit_gate_tests {
    ...
    use ainxt_workspace::permission::AccessKind;
    ...
    fn search_replace(path: &str) -> ToolInput {
        use ainxt_tools::implementations::ainxt_build::search_replace::SearchReplaceInput;
    ...
    fn write(path: &str) -> ToolInput {
        use ainxt_tools::implementations::opencode::write::WriteInput;
        ToolInput::Write(WriteInput {
            file_path: path.into(),
            content: "x".into(),
        })
    }
    /// ainxt edit tools are plan-file-only while plan mode is active — the
    /// enforcement that makes plan mode read-only even under always-approve.
    #[test]
    fn ainxt_edits_outside_plan_file_rejected() {
        ...
        assert_eq!(
            ...
            PlanEditGate::RejectNonPlanFile,
            "ainxt tools get no markdown exception — plan file only"
        );
    }
    ...
    fn apply_patch_rejected_in_plan_mode() {
        use ainxt_tools::implementations::codex::apply_patch::ApplyPatchInput;
        let t = active_tracker();
        assert_eq!(
            gate(
                &t,
...
mod plan_approval_helper_tests {
    ...
    use ainxt_tools::implementations::ainxt_build::exit_plan_mode::ExitPlanModeExtResponse;
    ...
    fn ext_method_no_client_defaults_false_for_untagged_error() {
        assert!(!ext_method_no_client(&ainxt_acp_lib::acp_internal_error(
...
mod wait_interrupt_tests {
    ...
    use ainxt_tools::types::output::ToolOutput;
    use ainxt_tool_types::TaskOutputOutput;
    ...
    async fn pending_interjection_aborts_in_flight_wait() {
        ...
        use ainxt_interjection_core::PendingInterjection;
