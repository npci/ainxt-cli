...
impl SessionActor {
    ...
    pub(super) fn do_hooks_trust_project(cwd: &str) -> Result<std::path::PathBuf, String> {
        let root =
            ainxt_workspace::session::git::find_git_root_from_path(std::path::Path::new(cwd))
                .map_err(|_| {
    ...
    pub(super) async fn reseed_mcp_output_cap(&self) {
        ...
        let existing = resources
            .get::<ainxt_tools::types::resources::TruncationCfg>()
            .map(|c| c.0.clone());
        match (resolved, existing) {
            (resolved, Some(mut cfg)) => {
                if cfg.mcp_max_output_bytes != resolved {
                    cfg.mcp_max_output_bytes = resolved;
                    resources.insert(ainxt_tools::types::resources::TruncationCfg(cfg));
                }
            }
            (Some(v), None) => {
                resources.insert(ainxt_tools::types::resources::TruncationCfg(
    ...
    pub(super) async fn handle_hooks_action(
        ...
        use ainxt_hooks_plugins_types::{ActionOutcome, HooksAction, OutcomeStatus};
        ...
        match action {
            ...
            HooksAction::Add { path } => {
                ...
                // CWE-427: add_hooks_path() validates path is under ~/.ainxt/.
            ...
            HooksAction::Disable { hook_name } => {
                match ainxt_hooks::trust::disable_hook(&hook_name) {
                    Ok(()) => ActionOutcome {
                        status: OutcomeStatus::Success,
            ...
            HooksAction::Enable { hook_name } => {
                match ainxt_hooks::trust::enable_hook(&hook_name) {
            ...
            HooksAction::ToggleSource {
                ...
                for name in &hook_names {
                    let ok = if disable {
                        ainxt_hooks::trust::disable_hook(name).is_ok()
                    } else {
                        ainxt_hooks::trust::enable_hook(name).is_ok()
                    };
                    if ok {
                        toggled += 1;
                    }
    ...
    pub(super) async fn handle_plugins_action(
        ...
        use ainxt_hooks_plugins_types::{ActionOutcome, OutcomeStatus, PluginsAction};
        ...
        match action {
            ...
            PluginsAction::Install { source } => {
                ...
                let install_source =
                    ainxt_agent::plugins::git_install::parse_install_source(&source, cwd);
                let registry = ainxt_agent::plugins::InstallRegistry::load();
                match ainxt_agent::plugins::git_install::install_from_source(
                    &install_source,
                    &registry,
                ) {
                    Ok(result) => {
                        let repo = ainxt_agent::plugins::git_install::build_installed_repo(
                            &result,
                            &install_source,
                        );
            ...
            PluginsAction::Uninstall {
                ...
                let mut registry = ainxt_agent::plugins::InstallRegistry::load();
                match registry.find_plugin(plugin_name) {
                    ...
                    Some((repo_key, repo, _plugin)) => {
                        ...
                        if let Err(e) =
                            ainxt_agent::plugins::git_install::remove_repo_path(&repo_path)
                        {
            ...
            PluginsAction::Update { plugin_id } => {
                ...
                for (key, repo) in &repos_to_update {
                    match ainxt_agent::plugins::git_install::update_repo(key, repo) {
                        Ok(status) => {
                            use ainxt_agent::plugins::git_install::UpdateStatus;
                            match status {
    ...
    pub(super) async fn reload_hooks_impl(self: &std::sync::Arc<Self>) -> String {
        ...
        *self.hook_load_errors.borrow_mut() = errors.iter().map(|e| e.to_string()).collect();
        ...
        if let Some(ref pr) = plugin_registry_snapshot {
            for plugin in pr.active_plugins() {
                if let Some(ref hooks_path) = plugin.hooks_path {
                    let (specs, warnings) =
                        ainxt_agent::plugins::hooks_adapter::parse_plugin_hooks(
                            hooks_path,
                            &plugin.name,
                            &plugin.root_str(),
                            &plugin.data_dir_str(),
                ...
                if let Some(ref inline_value) = plugin.inline_hooks {
                    let (specs, warnings) =
                        ainxt_agent::plugins::hooks_adapter::parse_plugin_hooks_from_value(
                            inline_value,
                            &plugin.name,
                            &plugin.root_str(),
                            &plugin.data_dir_str(),
        ...
        // no Ref guard is alive across the suspension point.
        {
            ...
            self.send_ainxt_notification(AinxtSessionUpdate::HooksChanged {
                hooks,
                project_trusted,
                load_errors,
            })
    ...
    pub(super) async fn reload_plugins_impl(
        self: &Arc<Self>,
        handle: &ainxt_agent::plugins::SharedPluginRegistryHandle,
        force: bool,
    ) -> String {
        let session_cwd = std::path::Path::new(&self.session_info.cwd);

        let sid = self.session_info.id.0.as_ref();
        ainxt_telemetry::unified_log::info("reload_plugins_impl: start", Some(sid), None);
    ...
    pub(crate) fn preserve_session_plugin_dirs(
        &self,
        incoming: Option<std::sync::Arc<ainxt_agent::plugins::PluginRegistry>>,
    ...
    pub(super) async fn apply_plugin_registry_snapshot(
        self: &Arc<Self>,
        new_registry_snapshot: Option<std::sync::Arc<ainxt_agent::plugins::PluginRegistry>>,
    ) -> (usize, bool, usize) {
        let sid = self.session_info.id.0.as_ref();
        let session_cwd = std::path::Path::new(&self.session_info.cwd);
        ...
        *self.plugin_registry.borrow_mut() = new_registry_snapshot.clone();
        ...
        if let Some(ref new_registry) = new_registry_snapshot {
            ...
            hooks_reloaded = new_specs.len();
            {
                ...
                if let Some(ref mut arc_reg) = *reg {
                    ...
                    let (mut new_reg, _) =
                        ainxt_hooks::discovery::load_hooks_from_sources(&[], &[]);
        ...
        // non-deterministic). Matches the `UpdateMcpServers` command handler.
        ...
        let mcp_changed = if let Some(diff) = mcp_diff {
            ...
            {
                let _ = tx.send(ainxt_mcp::servers::McpClientEvent::ConfigDiff {
        ...
        // BorrowMutError panics when send_ainxt_notification dispatches
        ...
        let t_notify = std::time::Instant::now();
        {
            ...
            self.send_ainxt_notification(AinxtSessionUpdate::PluginsChanged { plugins })
                .await;
        }
