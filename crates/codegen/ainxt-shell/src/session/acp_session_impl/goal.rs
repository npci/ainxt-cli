...
impl RoleCapability {
    ...
    fn is_satisfied(
        self,
        summary: &ainxt_tools::implementations::ainxt_build::task::types::SubagentTypeSummary,
    ) -> bool {
        match self {
...
impl SessionActor {
    ...
    pub(super) async fn drain_goal_updates_with_extra(
        ...
        extra: Vec<ainxt_tools::implementations::ainxt_build::update_goal::UpdateGoalEnvelope>,
    ) {
    ...
    /// used when Guard 3's in-flight short-circuit fires. Matches the
    ...
    /// Returns the stage outcome unchanged; the caller applies the
    /// lifecycle effects.
    ...
    pub(crate) async fn resolve_goal_role_override(
        ...
        //    silently run ainxt-build flavor. Non-strict names (ainxt-build
        //    family) run the default flavor and pass; unresolvable names fall
        //    through to the describe `Unknown` arm below.
        if ainxt_agent::config::is_strict_harness_agent_type(&pair.agent_type)
    ...
    pub(super) async fn resolve_goal_tool_names(&self) -> GoalToolNames {
        ...
        GoalToolNames {
            ...
            // names the tool the active harness exposes. Matches
    ...
    /// agrees with `from_summary` — `search_replace` on the default ainxt-build
    /// host, not the literal `write`). The `{TOOLSET_TOOLS}` block is empty on
    /// this path (no per-role toolset enumeration); on the default ainxt-build
    ...
    pub(super) async fn prune_prior_goal_continuation_directives(&self) {
        use ainxt_sampling_types::conversation::SyntheticReason;
