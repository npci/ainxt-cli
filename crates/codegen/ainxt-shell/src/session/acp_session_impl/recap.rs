...
impl SessionActor {
    ...
    pub(super) async fn handle_side_question(&self, question: &str) -> Result<String, String> {
        ...
        let mut items: Vec<ConversationItem> =
            ainxt_chat_state::compaction_utils::strip_reasoning_blocks(
        ...
        let request = ConversationRequest {
            ...
            temperature: None,
            x_ainxt_conv_id: Some(btw_session_id.clone()),
            x_ainxt_req_id: Some(format!("ainxt-btw-{}", uuid::Uuid::new_v4())),
            x_ainxt_session_id: Some(parent_session_id.clone()),
    ...
    /// (reusing the prompt prefix unchanged so the provider cache stays warm),
    ...
    pub(super) async fn handle_recap(&self, auto: bool) {
        ...
        // keeps reasoning unchanged so the prefix matches the last turn and the
        // provider's prefix KV cache stays warm. Matches compaction's
        ...
        let x_ainxt_conv_id = format!("recap-{}", uuid::Uuid::new_v4());
        let x_ainxt_req_id = format!("ainxt-recap-{}", uuid::Uuid::new_v4());
        ...
        let request = ConversationRequest {
            ...
            model: Some(model.clone()),
            temperature: None,
            x_ainxt_conv_id: Some(x_ainxt_conv_id.clone()),
            x_ainxt_req_id: Some(x_ainxt_req_id.clone()),
            x_ainxt_session_id: Some(self.session_info.id.to_string()),
            x_ainxt_agent_id: Some(ainxt_telemetry::id::agent_id()),
            ..Default::default()
        };
        ...
        // Applies to manual `/recap` too: spinner-less clients (e.g. ainxt
    ...
    fn persist_recap_request_artifact(
        ...
        let artifact = RecapRequestFile {
            ...
            x_ainxt_req_id: x_ainxt_req_id.to_owned(),
            x_ainxt_conv_id: x_ainxt_conv_id.to_owned(),
            strip_reasoning,
    ...
    /// (`AINXT_PROMPT_SUGGESTIONS_MODEL`) > `[models] prompt_suggestion`
    /// (config.toml) > remote `prompt_suggestion_model` (remote settings) >
    /// (config.toml) > remote `prompt_suggestion_model` (remote settings) >
    /// [`prompt_suggest::DEFAULT_SUGGEST_MODEL`] (`ainxt-build-0.1`). Every
    ...
    /// `ainxt-build-0.1` for OAuth users) the request is **skipped
    ...
    /// reasoning_effort are left unset — matches [`Self::handle_recap`]: the
    /// proxy may inject provider defaults, a small token cap silently empties
    /// a reasoning model's response, and some models (e.g. `ainxt-build`)
    ...
    pub(super) async fn handle_suggest_prompt(
        ...
        let request = ConversationRequest {
            ...
            model: Some(model),
            temperature: None,
            x_ainxt_conv_id: Some(format!("promptsuggest-{}", uuid::Uuid::new_v4())),
            x_ainxt_req_id: Some(format!("ainxt-promptsuggest-{}", uuid::Uuid::new_v4())),
            x_ainxt_session_id: Some(self.session_info.id.to_string()),
            x_ainxt_agent_id: Some(ainxt_telemetry::id::agent_id()),
            ..Default::default()
        };
        ...
        tracing::debug!(
            raw_preview = %ainxt_tools::util::truncate_str(raw.trim(), 60),
