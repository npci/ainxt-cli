...
impl TokenUsageCategory {
    ...
    pub fn skills_listing(text: &str, skill_count: usize) -> Self {
        Self {
            ...
            tokens: ainxt_token_estimation::estimate_tokens(text),
...
pub struct ContextInfo {
    ...
    /// always matches the actual trigger (e.g. 65 for ainxt-build in remote settings).
...
impl ContextInfo {
    ...
    pub fn from_notification(used: u64, total: u64) -> Self {
        Self {
            used,
            total,
            usage_pct: ainxt_token_estimation::usage_percentage_u8(used, total),
            free_tokens: ainxt_token_estimation::free_tokens(total, used),
...
pub struct SessionInfoData {
    /// Agent definition name for this session (e.g. `ainxt-build`).
...
/// Unified session usage data returned by `GetSessionUsage`.
///
/// Backs `/stats`: the CLI-local counterpart to `/usage` (platform billing/
/// credit limits — see `crate::extensions::billing`). Everything here comes
/// from the session's own `UsageLedger` (token counts plus the
/// gateway-reported cost — see `ainxt_chat_state::usage`), so it works
/// identically for OAuth, API-key, and offline/self-hosted deployments with no
/// extra backend round trip. Cost is never computed locally: the gateway is
/// its single source, and an unpriced call renders as unavailable.
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct SessionUsageData {
    /// Active model id (routing slug), e.g. `claude-sonnet-4-6`.
    pub model: Option<String>,
    /// Display name resolved from the model catalog, when available.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub model_display_name: Option<String>,
    /// Token + cost totals for the whole session, plus a per-model breakdown
    /// when more than one model served this session (e.g. after `/model`).
    pub usage: crate::extensions::notification::PromptUsage,
}
...
pub fn is_coding_model_slug(model: &str) -> bool {
    matches!(model, "ainxt-build" | "ainxt-4.5")
}
...
/// Full wire response for `ainxt.dev/session/info`.
///
...
/// Full wire response for `ainxt.dev/session/usage` (`/stats`).
///
/// Wraps `SessionUsageData` with the session id, mirroring
/// `SessionInfoResponse`'s shape so both extension methods are handled the
/// same way on the client.
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct SessionUsageResponse {
    pub session_id: String,
    #[serde(flatten)]
    pub data: SessionUsageData,
}
...
pub struct StartupHints {
    ...
    /// Leading conversation items to preserve unchanged across compaction (the
    ...
    /// inherited System at `conversation[0]`: the unchanged parent copy already
...
mod tests {
    ...
    fn should_show_model_fingerprint_truth_table() {
        ...
        assert!(should_show_model_fingerprint(false, "ainxt-build"));
        assert!(should_show_model_fingerprint(false, "ainxt-4.5"));
    ...
    fn desktop_client_type_deserializes_and_round_trips() {
        ...
        let submission = input.to_submission(Some("ainxt-3".into()), None, None, Some(5));
        assert_eq!(
