...
pub struct ParsedPrompt {
    ...
    /// ainxt mode may include editor open/focus metadata; the compat mode does not.
    /// Empty string when there is no context.
    pub context: String,
    /// The user's query, already wrapped in `<user_query>` tags
    /// (or raw when unchanged).
    pub query: String,
...
impl ParsedPrompt {
    ...
    ///
    /// Layout:
    /// - **ainxt mode:** `<user_query>` + `<skill_information>` + context
...
///
/// When `is_cursor` is false, produces original ainxt-format output:
...
{}
    ...
    if !resource_links.is_empty() {
        ...
        context.push_str(&render_resource_links_ainxt(resource_links));
    }
    (context, query)
}
...
/// ainxt-format resource links: `<focused_files>` / `<open_files>` with
/// metadata inside a `<system-reminder>` wrapper.
fn render_resource_links_ainxt(resource_links: &[acp::ResourceLink]) -> String {
...
mod tests {
    ...
    /// Shorthand: render + assemble for ainxt mode.
    fn render_ainxt(
        message: &str,
        embedded: Vec<String>,
    ...
    #[test]
    fn test_ainxt_render_plain_message() {
        let result = render_ainxt("hello", vec![], vec![], &[], false);
        assert_eq!(result, "<user_query>\nhello\n</user_query>");
    }
    #[test]
    fn test_ainxt_render_with_attachments_uses_system_reminder_wrapper() {
        let result = render_ainxt(
            "check this",
        ...
        assert!(
            ...
            "ainxt should start with <user_query>, got: {result}"
        );
    }
    #[test]
    fn test_ainxt_render_user_query_first() {
        let link = acp::ResourceLink::new("doc.md", "file:///doc.md")
            .title(Some("My Doc".into()))
            .size(Some(1024));
        let result = render_ainxt("hello", vec![], vec![], &[link], false);
        let uq_pos = result.find("<user_query>").unwrap();
        let rr_pos = result.find("Referenced resources:").unwrap();
        assert!(
            uq_pos < rr_pos,
            "ainxt: <user_query> ({uq_pos}) should come before resource links ({rr_pos})\ngot: {result}"
        );
        assert!(result.contains("<system-reminder>"));
    }
    #[test]
    fn test_ainxt_render_resource_links_use_focused_files_format() {
        let links = vec![
        ...
        let result = render_ainxt("hello", vec![], vec![], &links, false);
