...
/// sanitizer shared with `ainxt-mcp` so the advertised folder matches disk.
...
mod tests {
    ...
    fn sanitize_replaces_unsafe_chars_and_never_empty() {
        assert_eq!(
            sanitize_descriptor_segment("ainxt_com_linear"),
            "ainxt_com_linear"
        );
    ...
    fn server_dir_is_joined_under_root() {
        let root = Path::new("/home/u/.ainxt/projects/enc/mcps");
