//! Goal planner subagent runner. Matches [`crate::session::goal_classifier`]
...
///
/// [`SubagentRuntimeOverrides::harness_agent_type`]: ainxt_tools::implementations::ainxt_build::task::types::SubagentRuntimeOverrides::harness_agent_type
...
pub(crate) struct RoleSpawnOverride {
    ...
    /// Resolved harness `agent_type` (e.g. `"ainxt-build-plan"`)
...
mod tests {
    ...
    fn planner_template_default_render_preserves_wording_and_has_no_placeholders() {
        ...
        // (ainxt-build) tool names; guards against accidental wording drift.
    ...
    /// 3-role equivalence: an explicit planner pair threads `agent_type` as the
    ...
    async fn fail_open_retry_emits_spawn_failed_event() {
        ...
        let writer = ainxt_file_utils::events::EventWriter::open(dir.path());
        let ov = RoleSpawnOverride {
            model: Some("m".into()),
            agent_type: Some("t".into()),
        };
