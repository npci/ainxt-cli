...
pub fn prompt_history_path(cwd: &str) -> PathBuf {
    crate::util::ainxt_home::sessions_cwd_dir(cwd).join(PROMPT_HISTORY_FILE)
}
...
pub fn append_prompt(cwd: &str, entry: &PromptEntry) -> io::Result<()> {
    ...
    crate::util::ainxt_home::ensure_sessions_cwd_dir(cwd)?;
...
pub fn truncate_if_needed(cwd: &str) -> io::Result<()> {
    ...
    // Explicitly release the underlying file handle now that the content
    // has been read, rather than relying solely on end-of-scope Drop.
    drop(reader);

    let entries: Vec<PromptEntry> = content
        .lines()
...
mod tests {
    ...
    async fn test_append_prompt_async_round_trips_for_session() {
        ...
        // Matches the submit path: awaiting the wrapper makes the append immediately loadable.
