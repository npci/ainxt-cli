...
use crate::extensions::notification::{SessionNotification, SessionUpdate as AinxtSessionUpdate};

/// Shared per-session map of open reverse-requests, keyed by `tool_call_id`.
///
/// Matches the `current_prompt_id` signal on
...
pub enum PendingKind {
    ...
    Question,
    /// `ainxt.dev/exit_plan_mode` plan approval.
    PlanApproval,
}

/// Whether a blocking plan-approval reverse-request is parked in `pending`.
///
/// The resume re-park issues `ainxt.dev/exit_plan_mode` from a detached task
...
fn broadcast(gateway: &GatewaySender, session_id: &acp::SessionId, update: AinxtSessionUpdate) {
...
impl PendingInteractionGuard {
    ...
    pub fn new(
        ...
        broadcast(
            &gateway,
            &session_id,
            AinxtSessionUpdate::PendingInteraction {
    ...
    fn drop(&mut self) {
        ...
        if removed {
            broadcast(
                &self.gateway,
                &self.session_id,
                AinxtSessionUpdate::InteractionResolved {
