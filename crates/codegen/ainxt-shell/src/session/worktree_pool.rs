...
//!   under `~/.ainxt/worktree_pool/<instance_id>/` with a `.pid` liveness
...
use ainxt_fast_worktree::{WorktreeBuilder, WorktreeSync};

const WORKTREE_POOL_LOG: &str = "ainxt_worktree_pool";
use crate::util::config::PoolConfig;
use crate::util::ainxt_home::ainxt_home;

use ainxt_tty_utils::git_command;
...
/// `~/.ainxt/worktree_pool/<instance_id>/` with a `.pid` file recording
...
impl WorktreePool {
    ...
    /// `~/.ainxt/worktree_pool/<instance_id>/`. A `.pid` file is written so
...
pub struct AdoptableWorktree {
    /// Current path: `~/.ainxt/worktree_pool/<old_instance>/<pool_id>/`
...
/// `~/.ainxt/worktree_pool/`, reads each `.pid` file, and checks
...
/// `CleanAll` mode. Placed under `~/.ainxt/worktree_pool/<instance_id>/<uuid>/`.
async fn create_pooled_worktree(
    ...
    let result = tokio::task::spawn_blocking(move || {
        WorktreeBuilder::new(&source, &dest_clone)
            .creation_mode(ainxt_fast_worktree::CreationMode::GitCheckout)
            .parallelism(parallelism)
            .worktree_kind(ainxt_fast_worktree::WorktreeKind::Pool)
            .create()
    })
    .await?;
...
fn schedule_cleanup(path: PathBuf) {
    ...
    tokio::task::spawn(async move {
        ...
        if path.exists() {
            ...
            let result = tokio::task::spawn_blocking(move || {
                ainxt_fast_worktree::remove_worktree(&path_clone)
            })
            .await;
...
async fn remove_worktree_registration(path: &Path) {
    ...
    let result =
        tokio::task::spawn_blocking(move || ainxt_fast_worktree::remove_worktree(&path_owned)).await;
