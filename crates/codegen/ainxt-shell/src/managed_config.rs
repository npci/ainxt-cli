...
pub const MANAGED_ARTIFACT_FILES: [&str; 3] = [
    ainxt_config::MANAGED_CONFIG_FILENAME,
    ainxt_config::REQUIREMENTS_FILENAME,
    ainxt_config::signed_policy::SIGNATURE_SIDECAR_FILE,
];
...
fn remove_managed_config_files(home: &std::path::Path) {
    ...
    if artifacts_removed {
        remove_synced_file(
            home,
            ainxt_config::MANAGED_CONFIG_CACHE_FILE,
    ...
    let atomic_write_tmp_prefixes = [
        format!("{}.", ainxt_config::MANAGED_CONFIG_CACHE_FILE),
        format!(
            "{}.",
            ainxt_config::signed_policy::SIGNATURE_SIDECAR_FILE
        ),
...
/// would just 401).
fn eligible_team_principal(auth: AinxtAuth) -> Option<AinxtAuth> {
...
/// config is a ainxt.dev feature with one ainxt.dev auth.
fn read_active_team_auth() -> Option<AinxtAuth> {
    let home = crate::util::ainxt_home::ainxt_home();
...
/// files (TUI tick vs `ainxt login` vs prefetch). `None` on contention — the
...
#[derive(Clone, Copy)]
enum SyncBudget {
    /// Background loop and explicit `ainxt setup`; runs retries to completion.
    Standard,
...
/// Base is 1s; `AINXT_DEPLOYMENT_CONFIG_BACKOFF_MS` overrides it for tests.
fn retry_backoff(attempt: u32) -> std::time::Duration {
    let base = std::env::var("AINXT_DEPLOYMENT_CONFIG_BACKOFF_MS")
        .ok()
...
/// Map a `reqwest` send failure to a `ManagedConfigError` via the shared `ainxt-http` classifier.
...
/// Override with `AINXT_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn managed_config_sync_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("AINXT_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS")
...
/// snapshots, and OTel: the **server** AinxtBuildDeployment UUID (the id
...
/// Resolve deployment key from `AINXT_DEPLOYMENT_KEY` env var, then config files.
...
/// Callers doing auto-fetch should check this; explicit user actions (ainxt setup) skip it.
pub fn is_fetch_enabled() -> bool {
    if let Some(v) = crate::agent::config::env_bool("AINXT_MANAGED_CONFIG") {
        return v;
    }
...
/// Fetch managed config + requirements and write to `~/.ainxt/`, trying the
...
async fn sync_bounded(
    budget: SyncBudget,
    team_override: Option<AinxtAuth>,
...
/// read-only `ainxt setup --json` both build on this.
...
fn apply_fetched(
    ...
    let verified = if ainxt_config::signed_policy::verification_active() {
    ...
    if let Some(verified) = verified {
        clear_squatting_dir(&home.join(ainxt_config::signed_policy::SIGNATURE_SIDECAR_FILE));
        ainxt_config::signed_policy::write_sidecar(&home, &verified.sidecar)?;
    }
    ...
    clear_squatting_dir(&home.join(ainxt_config::MANAGED_CONFIG_CACHE_FILE));
...
/// Whether the credential a fetch used is still present. Matches the
...
/// Post-login hook for `ainxt login` and the ACP/TUI authenticate flow: clear any
...
pub async fn post_login_sync(authenticated: Option<AinxtAuth>) -> ManagedConfigSync {
    clear_orphan();
    if !is_fetch_enabled() {
...
/// Whether a credential exists that `ainxt setup` could install config for.
...
pub async fn ensure_managed_policy_present(
    ...
    let team = tokio::time::timeout(SESSION_START_AUTH_DEADLINE, auth_manager.auth())
        ...
        .ok()
        .and_then(Result::ok)
        .filter(AinxtAuth::is_team_principal);
    if !has_principal() {
        return;
    }
...
/// are the non-forgeable layers. Recovery: reconnect / `ainxt setup`; ceasing to serve
...
/// Outcome of the `ainxt setup` sync. The caller renders it — CLI presentation
/// and exit codes stay out of the library.
#[derive(Debug)]
pub enum SetupOutcome {
    /// Config was written to `~/.ainxt`.
    Installed,
...
/// Result of `ainxt setup --json`: what the server serves for the current
/// principal, unchanged. `managed_config` may embed the enforced deployment key,
/// exactly as `ainxt setup` would write it to disk.
...
pub struct SetupReport {
    ...
    /// TOML documents exactly as `ainxt setup` would install them.
    pub managed_config: Option<String>,
    pub requirements: Option<String>,
    pub fail_closed: bool,
}

/// Fetches the report behind `ainxt setup --json` without writing anything:
...
pub async fn fetch_setup_report() -> Result<SetupReport, ManagedConfigError> {
    ...
    // Match the installer's trust decision: a payload `ainxt setup` would refuse
    // is reported as an error, not printed as installable config.
    if source.is_some()
        && ainxt_config::signed_policy::verification_active()
...
/// Run the `ainxt setup` sync for the current principal. The caller must check
