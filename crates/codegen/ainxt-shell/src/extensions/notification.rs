...
/// ainxt-specific session notification (parallel to acp::SessionNotification)
/// This wraps an AinxtSessionUpdate with session context for persistence and replay.
...
impl PromptUsage {
    ...
    pub fn project_from_ledger(
        ledger: Option<&ainxt_chat_state::UsageLedger>,
        incomplete: bool,
    ) -> Option<Self> {
    ...
    ///
    /// `approx_cost_usd_ticks` is left alone: it never claimed to be exact, so
    /// there is no false precision to protect a consumer from, and clearing it
    /// would take away the only budget signal a user gets on exactly the paths
    /// where the gateway reported nothing. An incomplete bill can still
    /// under-count tokens, which is why the estimate stays labelled approximate
    /// alongside `usage_is_incomplete`.
    ...
    fn is_token_empty(&self) -> bool {
        ...
        let PromptUsageModel {
            ...
            cached_read_tokens,
            cache_write_tokens: _, // subset of input_tokens, not extra spend
            ...
            approx_cost_usd_ticks: _, // derived from the token counts above
            cost_is_partial: _,
...
pub struct PromptUsageModel {
    ...
    /// Prompt tokens written to the cache. A subset of `input_tokens`, never
    /// an addition to it, so summing the columns would double-count.
    #[serde(default)]
    ...
    /// **Approximate** total cost in the same ticks, covering *every* call:
    /// the gateway's own figures plus a published-rate estimate for the calls
    /// it did not price. Present only when at least one call had to be
    /// estimated, so its presence is itself the "this is not billable" signal.
    ///
    /// Deliberately a complete total rather than the estimated remainder: it is
    /// the number a UI shows, and it must stay meaningful after
    /// [`PromptUsage::scrub_untrustworthy_costs`] clears the partial
    /// `cost_usd_ticks` beside it. Reconciliation must use `cost_usd_ticks`
    /// and ignore this field entirely.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub approx_cost_usd_ticks: Option<i64>,
...
impl From<&ainxt_chat_state::UsageTotals> for PromptUsageModel {
    fn from(t: &ainxt_chat_state::UsageTotals) -> Self {
        ...
        let ainxt_chat_state::UsageTotals {
            input_tokens,
            output_tokens,
            cached_read_tokens,
            cache_write_tokens,
            ...
            api_duration_ms,
            cost_usd_ticks,
            estimated_cost_usd_ticks: _, // folded into the approximate total below
            cost_missing_calls,
        } = *t;
        // Emit the approximate total only when something actually had to be
        // estimated. When the gateway priced every call, `cost_usd_ticks` is
        // already the whole answer and a second near-identical field would
        // only invite a consumer to double-count.
        let approx_cost_usd_ticks = t
            .displayable_cost()
...
impl From<&ainxt_chat_state::UsageLedger> for PromptUsage {
    fn from(ledger: &ainxt_chat_state::UsageLedger) -> Self {
        let mut usage = Self {
...
///
/// `total_cost_usd` keeps its frozen meaning: gateway-priced spend only, under
/// the existing trust gate. A locally estimated figure is published beside it as
/// `estimated_total_cost_usd` (plus exact ticks) so a script written against the
/// old shape can never pick up an approximation by accident — opting in means
/// reading the differently-named key.
pub fn project_result_usage(result: &mut serde_json::Value, usage: &PromptUsage) {
    ...
    result["usage"] = serde_json::json!({
        // `input_tokens` keeps its frozen meaning (full − cache_read) so the
        // documented identity `uncached + cache_read + output = total_tokens`
        // still holds for existing consumers. Cache writes are reported
        // alongside as a new key rather than subtracted here.
    ...
    // The approximate total is published even when the exact one is withheld —
    // that is precisely when it is useful — but always under its own keys, so
    // `total_cost_usd` never carries a locally computed number.
    if let Some(ticks) = approx_cost_usd_ticks {
        result["estimated_total_cost_usd"] = serde_json::json!(ticks_to_usd(ticks));
        result["estimated_total_cost_usd_ticks"] = serde_json::json!(ticks);
    }
    if !usage.model_usage.is_empty() {
        ...
        for (name, m) in &usage.model_usage {
            ...
            // Same split as the totals: `costUSD` stays gateway-only, the
            // approximation gets its own key.
            if let Some(ticks) = approx_cost_usd_ticks {
                entry["estimatedCostUSD"] = serde_json::json!(ticks_to_usd(ticks));
            }
...
pub enum SessionUpdate {
    ...
    HooksChanged {
        hooks: Vec<ainxt_hooks_plugins_types::HookInfo>,
    ...
    ///
    /// Emitted by the `ainxt.dev/recap` ext method: on demand via the `/recap`
    ...
    GoalUpdated {
        ...
        /// depth.
        ///
        /// This is a live, active-subagent-window field (it matches
    ...
    /// outcome. Rides the persisted `_ainxt.dev/session/update` rail (unlike the
    /// fire-and-forget `ainxt.dev/session/prompt_complete` notification), so a
...
/// detailed migration guidance (`ainxt logout` / `ainxt login`) in the
/// message, so we surface that unchanged instead of the generic prompt.
...
pub struct CompactionSegmentFile {
    pub items: Vec<ainxt_sampling_types::ConversationItem>,
    /// Curated summary, analysis tags already stripped.
    pub summary: String,
    pub detail: ainxt_chat_state::CompactionDetail,
...
pub struct CompactionRequestFile {
    ...
    /// Note: this is a per-artifact ID, not the model API's `x_ainxt_req_id`
    ...
    /// or `"detailed"` (10-section structured prompt for ainxt-build and similar agents).
    ...
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub attempt_details: Vec<ainxt_chat_state::compaction_utils::CompactionAttempt>,
}
...
pub struct RecapRequestFile {
    ...
    /// API's `x_ainxt_req_id` (also recorded below for proxy correlation).
    pub request_id: String,
    ...
    pub model: String,
    /// Sampling request id sent to the proxy (`ainxt-recap-{uuid}`).
    pub x_ainxt_req_id: String,
    /// Sampling conversation id (`recap-{uuid}`).
    pub x_ainxt_conv_id: String,
    ...
    /// unchanged for prompt-cache warmth).
...
mod tests {
    ...
    fn recap_request_file_roundtrips() {
        let artifact = RecapRequestFile {
            ...
            x_ainxt_req_id: "ainxt-recap-abc".into(),
            x_ainxt_conv_id: "recap-abc".into(),
        ...
        assert_eq!(parsed.x_ainxt_req_id, "ainxt-recap-abc");
        assert_eq!(
    ...
    fn compaction_request_file_v2_roundtrips_attempt_details() {
        use ainxt_chat_state::compaction_utils::CompactionAttempt;
    ...
    fn memory_flush_completed_with_path_roundtrips() {
        let update = SessionUpdate::MemoryFlushCompleted {
            result: "written".into(),
            path: Some("/home/user/.ainxt/memory/ws/sessions/log.md".into()),
        };
        let json_str = serde_json::to_string(&update).unwrap();
        let parsed: SessionUpdate = serde_json::from_str(&json_str).unwrap();
        assert_eq!(update, parsed);
    ...
    fn memory_dream_completed_roundtrips() {
        let update = SessionUpdate::MemoryDreamCompleted {
            ...
            path: Some("/home/user/.ainxt/memory/ws/MEMORY.md".into()),
        };
        let json_str = serde_json::to_string(&update).unwrap();
        let parsed: SessionUpdate = serde_json::from_str(&json_str).unwrap();
        assert_eq!(update, parsed);
    ...
    fn memory_session_saved_roundtrips() {
        let update = SessionUpdate::MemorySessionSaved {
            path: "/home/user/.ainxt/memory/ws/sessions/2026-01-15-fix-auth-abc12345.md".into(),
        };
        let json_str = serde_json::to_string(&update).unwrap();
        let parsed: SessionUpdate = serde_json::from_str(&json_str).unwrap();
        assert_eq!(update, parsed);
    ...
    fn memory_files_variant_roundtrips_through_json() {
        let update = SessionUpdate::MemoryFiles {
            files: vec![
                MemoryFileInfo {
                    path: "/home/user/.ainxt/memory/MEMORY.md".into(),
                    source: "global".into(),
                    size_bytes: 1024,
                    modified_epoch_secs: Some(1_700_000_000),
                },
                MemoryFileInfo {
                    path: "/project/.ainxt/memory/MEMORY.md".into(),
    ...
    fn make_goal_updated_full() -> SessionUpdate {
        SessionUpdate::GoalUpdated {
            ...
            live_tokens_by_model: vec![("ainxt-4".into(), 6_000), ("ainxt-3".into(), 4_000)],
    ...
    fn goal_updated_serializes_snake_case_tag() {
        ...
        assert_eq!(json["live_tokens_by_model"][0][0], "ainxt-4");
    ...
    fn model_changed_serializes_snake_case_with_optional_effort() {
        ...
        assert_eq!(json["model_id"], "ainxt-4");
        ...
        assert_eq!(json["sessionUpdate"], "model_changed");
        assert_eq!(json["model_id"], "ainxt-3");
        assert!(
    ...
    fn model_changed_envelope_carries_session_id_at_top_level() {
        ...
        assert_eq!(json["update"]["model_id"], "ainxt-4");
    }
    ...
    fn turn_completed_serializes_snake_case_tag_and_fields() {
        // Matches the SubagentProgress convention: `rename_all = "snake_case"`
