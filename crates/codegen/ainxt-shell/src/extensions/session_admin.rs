...
//! lifecycle:
//!
//! - `ainxt.dev/session/rename`                  rename a session locally + remote
//! - `ainxt.dev/session/delete`                  delete a session locally + remote
//! - `ainxt.dev/session/update_mcp_servers`      mid-session MCP server swap
//! - `ainxt.dev/session/fork`                    fork a session into a new one
//! - `ainxt.dev/internal/reload_all_mcp_servers` config hot-reload, all sessions
//! - `ainxt.dev/internal/reload_project_mcp_servers` config hot-reload, cwd-scoped
//! - `ainxt.dev/internal/reload_skills`          skills file watcher fan-out
//! - `ainxt.dev/internal/reload_models`          model list hot-reload from config.toml
//! - `ainxt.dev/internal/reload_models_cache`    model catalog hot-reload from disk cache
//! - `ainxt.dev/internal/auth_cleared`           auth hot-clear cleanup
//! - `ainxt.dev/plugins/reload`                  rebuild shared plugin registry
//! - `ainxt.dev/commands/list`                   list slash commands
...
use ainxt_telemetry::id::agent_id;

#[tracing::instrument(skip_all, fields(method = %args.method))]
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "ainxt.dev/session/rename" => handle_session_rename(agent, args).await,
        "ainxt.dev/session/delete" => handle_session_delete(agent, args).await,
        "ainxt.dev/session/update_mcp_servers" => handle_update_mcp_servers(agent, args).await,
        "ainxt.dev/session/fork" => handle_session_fork(agent, args).await,
        "ainxt.dev/internal/reload_all_mcp_servers" => handle_reload_all_mcp_servers(agent).await,
        "ainxt.dev/internal/reload_project_mcp_servers" => {
            handle_reload_project_mcp_servers(agent, args).await
        }
        "ainxt.dev/internal/reload_skills" => handle_reload_skills(agent),
        "ainxt.dev/internal/reload_models" => handle_reload_models(agent),
        "ainxt.dev/internal/reload_models_cache" => handle_reload_models_cache(agent),
        "ainxt.dev/internal/auth_cleared" => handle_auth_cleared(agent),
        "ainxt.dev/plugins/reload" => handle_plugins_reload(agent).await,
        "ainxt.dev/commands/list" => handle_commands_list(agent, args).await,
...
async fn rename_chat_conversation(
    ...
    client
        ...
        .map_err(|e| match e {
            ConvError::NoOauth => acp::Error::invalid_request()
                .data("chat session rename requires ainxt OAuth credentials"),
    ...
    tracing::info!(
        conversation_tag = %crate::util::session_log_tag(conversation_id),
        title = %title,
...
async fn handle_session_delete(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    // Mirrored by the `ainxt sessions delete <id>` CLI path.
...
async fn soft_delete_chat_conversation(agent: &MvpAgent, conversation_id: &str) -> ExtResult {
    ...
    client
        ...
        .map_err(|e| match e {
            ConvError::NoOauth => acp::Error::invalid_request()
                .data("chat session delete requires ainxt OAuth credentials"),
            other => acp::Error::internal_error()
...
/// hot-reload watcher when `<cwd>/.ainxt/config.toml`,
...
/// Hot-reload the model catalog from `~/.ainxt/models_cache.json` after an
...
async fn handle_plugins_reload(agent: &MvpAgent) -> ExtResult {
    ...
    // Explicit desktop `ainxt.dev/plugins/reload`: force a full local-install re-copy.
    agent
...
async fn handle_commands_list(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    // that `ainxt.dev/commands/list` (the pull used by ainxt-desktop after session
    ...
    let plugin_reg = if let Some(cwd_str) = &req.cwd {
        ...
        // Fresh discovery for *this* cwd (includes .ainxt/plugins under it, plus
