//! `ainxt.dev/share_session` extension handler.
//!
...
async fn handle_share_session(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    let auth = require_ainxt_auth_for_share(&agent.auth_manager)?;
...
async fn upload_share_data_to_gcs(
    ...
    let share_tag = crate::util::session_log_tag(session_id);
    let data_json = match serde_json::to_vec(messages) {
        Ok(json) => json,
        Err(e) => {
            tracing::warn!(
                share_session_tag = %share_tag,
                error = %e,
    ...
    if let Err(e) = ainxt_file_utils::gcs::upload_bytes_signed(
...
fn require_ainxt_auth_for_share(
    auth_manager: &crate::auth::AuthManager,
) -> Result<crate::auth::AinxtAuth, acp::Error> {
    super::auth_gate::require_ainxt_auth(
        auth_manager,
        "Authentication required to share session",
        "Share session is disabled. Run `ainxt login` to authenticate.",
    )
}
...
mod tests {
    ...
    use crate::auth::{AuthMode, AinxtAuth};
    ...
    fn make_auth_manager_with_token_expiring_in(
        ...
        Arc<crate::auth::AuthManager>,
        tempfile::TempDir,
        crate::auth::OAuth2IssuerGuard,
    ) {
        // Returned to the caller so it outlives the assertions: the issuer must
        // stay configured while `require_ainxt_auth_for_share` runs, not just
        // while the token is built.
        let issuer = crate::auth::OAuth2IssuerGuard::set_test_issuer();
        ...
        // The issuer must be BOTH set on the token and configured for the build:
        // only an OIDC token whose issuer matches `AINXT_OAUTH2_ISSUER` (or the
        // local-dev equivalent) returns true from `is_ainxt_auth()`, and that is
        // what `require_ainxt_auth_for_share` gates on.
        //
        // Naming the issuer alone used to be enough, because it was a compiled
        // constant. It is now operator configuration (R42), so the guard is the
        // half that makes this a first-party token rather than just a token with
        // a plausible-looking issuer string.
        let auth = AinxtAuth {
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some(crate::auth::OAuth2IssuerGuard::TEST_ISSUER.to_string()),
            key: "test-key".into(),
    ...
    fn share_works_outside_the_5m_early_invalidation_window() {
        let (mgr, _dir, _issuer) = make_auth_manager_with_token_expiring_in(Duration::minutes(10));
        assert!(mgr.current().is_some());
        assert!(require_ainxt_auth_for_share(&mgr).is_ok());
    }

    #[test]
    fn share_succeeds_inside_the_5m_early_invalidation_window() {
        let (mgr, _dir, _issuer) = make_auth_manager_with_token_expiring_in(Duration::seconds(1));
        ...
        let res = require_ainxt_auth_for_share(&mgr);
        assert!(
            res.is_ok(),
            "require_ainxt_auth_for_share must succeed for a still-valid buffered ainxt token"
        );
    }
    ...
    fn share_fails_with_no_auth_at_all() {
        ...
        assert!(require_ainxt_auth_for_share(&mgr).is_err());
    }

    #[test]
    fn share_rejects_non_ainxt_auth_with_actionable_ainxt_login_message() {
        let dir = tempdir().expect("tempdir");
        let mgr = Arc::new(crate::auth::AuthManager::new(
            dir.path(),
            AinxtComConfig::default(),
        ...
        // API key is the simplest non-ainxt credential (External and enterprise OIDC
        // are also rejected the same way).
        let non_ainxt = AinxtAuth {
            auth_mode: AuthMode::ApiKey,
            key: "ainxt-test-key".into(),
            create_time: Utc::now(),
            ..Default::default()
        };
        mgr.hot_swap(non_ainxt);

        let err = require_ainxt_auth_for_share(&mgr)
            .expect_err("non-ainxt accounts (API key, External, enterprise IdP) must be rejected");

        // This is the key assertion the review asked for: we must test the *exact*
        // actionable data string for the non-ainxt path (distinct from the generic
