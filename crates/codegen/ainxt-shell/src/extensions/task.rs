...
use ainxt_tools::types::{KillOutcome, TaskSnapshot};
...
/// Wire DTO for the `ainxt.dev/task/kill` ext request.
///
/// `pub` (with both serde directions) so ACP clients (ainxt-pager) build
...
/// Wire DTO for the `ainxt.dev/task/kill` ext response payload (nested under
...
/// Wire DTO for the `ainxt.dev/subagent/cancel` ext request.
///
/// `pub` (with both serde directions) so ACP clients (ainxt-pager) build
...
/// Wire DTO for the `ainxt.dev/subagent/cancel` response payload (under `result` in
...
/// Handle `ainxt.dev/scheduler/*` extension methods.
...
/// Handle `ainxt.dev/subagent/*` extension methods.
pub async fn handle_subagent(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "ainxt.dev/subagent/list_running" => {
...
mod tests {
    ...
    // ── ainxt.dev/subagent/cancel outcome wire DTO ──────────────────────────
