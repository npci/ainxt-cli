//! ACP extension handler for bulk session updates (`ainxt.dev/session/updates`).
//!
...
//! the update type (`"session/update"` for ACP, `"_ainxt.dev/session/update"` for
//! ainxt extensions) and extract the notification payload from `params`.
...
/// Send updates as chunked `_ainxt.dev/session/updates/chunk` notifications.
    ...
    for (chunk_idx, chunk) in lines.chunks(chunk_size).enumerate() {
        ...
        if let Ok(raw) = serde_json::value::to_raw_value(&params) {
            gateway.forward_fire_and_forget(acp::ExtNotification::new(
                "ainxt.dev/session/updates/chunk",
...
mod tests {
    ...
    fn dummy_gateway() -> ainxt_acp_lib::AcpAgentGatewaySender {
    ...
    fn make_request(
        ...
        acp::ExtRequest::new("ainxt.dev/session/updates", std::sync::Arc::from(raw))
    }
    ...
    async fn handle_tail_request_with_rewind_returns_only_live_timeline() {
        ...
        std::fs::write(
            ...
            [
                ...
                r#"{"timestamp":5,"method":"_ainxt.dev/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"rewind_marker","target_prompt_index":1}}}"#,
    ...
    async fn handle_falls_back_to_id_lookup_for_divergent_cwd() {
        ...
        // Clean up the dir written under the real ainxt home.
        let _ = std::fs::remove_dir_all(&child_dir);
    }

    fn capturing_gateway() -> (
        ainxt_acp_lib::AcpAgentGatewaySender,
        tokio::sync::mpsc::UnboundedReceiver<ainxt_acp_lib::AcpClientMessage>,
    ) {
        let (tx, rx) = tokio::sync::mpsc::unbounded_channel();
        (ainxt_acp_lib::AcpAgentGatewaySender::new(tx), rx)
    }
    ...
    fn ainxt_rewind(target: usize) -> String {
        format!(
            r#"{{"timestamp":0,"method":"_ainxt.dev/session/update","params":{{"sessionId":"s","update":{{"sessionUpdate":"rewind_marker","target_prompt_index":{target}}}}}}}"#
        )
    }
