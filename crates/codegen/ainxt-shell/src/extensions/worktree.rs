//! Handler for ainxt.dev/git/worktree/* extension methods.
...
const WORKTREE_EXT_LOG: &str = "ainxt_worktree";
...
impl WorktreeNotificationSender for GatewayWorktreeNotifier {
    async fn send_worktree_status(&self, progress: WorktreeStatus) {
        ...
        let notification = acp::ExtNotification::new("ainxt.dev/git/worktree/status", params.into());
...
pub async fn handle(
    ...
    match args.method.as_ref() {
        ...
        "ainxt.dev/git/worktree/create_from_worktree" => {
            let mut req =
            ...
            let result = ops
                .dispatch(
                    &ainxt_workspace::workspace_ops::PrepareWorktreeFromWorktreeReq {
                        inner: req.clone(),
                    },
                    None,
                )
        ...
        "ainxt.dev/git/worktree/create_from_worktree_sync" => {
            ...
            let resolved_root = ops
                .dispatch(
                    &ainxt_workspace::workspace_ops::GitResolveRootReq {
            ...
            let result = ops
                .dispatch(
                    &ainxt_workspace::workspace_ops::CreateWorktreeFromWorktreeSyncReq {
                        inner: req.into_wire(),
                    },
                    None,
                )
        ...
        "ainxt.dev/git/worktree/resume_session" => {
            ...
            let agent_id = ainxt_telemetry::id::agent_id();
        ...
        "ainxt.dev/git/worktree/list" => {
            let req: ainxt_workspace::workspace_ops::WorktreeListReq =
        ...
        "ainxt.dev/git/worktree/show" => {
            let req = serde_json::from_str::<ShowWorktreeRequest>(args.params.get())?;
            let op = ainxt_workspace::workspace_ops::WorktreeShowReq {
        ...
        "ainxt.dev/git/worktree/gc" => {
            ...
            let op = ainxt_workspace::workspace_ops::WorktreeGcReq {
                dry_run: req.dry_run,
                max_age_secs,
                force: req.force,
            };
        ...
        "ainxt.dev/git/worktree/db/stats" => {
            let result = ops
                .dispatch(
                    &ainxt_workspace::workspace_ops::WorktreeDbStatsReq {},
                    None,
                )
                .await
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
        ...
        "ainxt.dev/git/worktree/db/rebuild" => {
            let result = ops
                .dispatch(
                    &ainxt_workspace::workspace_ops::WorktreeDbRebuildReq {},
                    None,
                )
                .await
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
        ...
        "ainxt.dev/git/worktree/db/path" => {
            let result = ops
                .dispatch(
                    &ainxt_workspace::workspace_ops::WorktreeDbPathReq {},
                    None,
                )
                .await
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
...
mod tests {
    ...
    fn list_request_with_filters() {
        let json = r#"{"repo": "ainxt", "type": ["session", "fork"], "includeAll": true}"#;
        let req: ListWorktreeRequest = serde_json::from_str(json).unwrap();
        assert_eq!(req.repo.as_deref(), Some("ainxt"));
    ...
    fn db_path_response_serializes() {
        let resp = WorktreeDbPathResponse {
            path: "/home/user/.ainxt/worktrees.db".into(),
        };
        let json = serde_json::to_string(&resp).unwrap();
        assert!(json.contains("\"path\":\"/home/user/.ainxt/worktrees.db\""));
    }
