...
mod tests {
    ...
    fn test_auth() -> crate::auth::AinxtAuth {
    ...
    fn status_only_reports_bundled_cache_not_higher_priority_sources() {
        ...
        std::fs::create_dir_all(project_root.join(".ainxt/personas")).unwrap();
        std::fs::create_dir_all(project_root.join(".ainxt/roles")).unwrap();
        std::fs::write(
            project_root.join(".ainxt/personas/researcher.toml"),
            "instructions = \"project persona\"\n",
        )
        .unwrap();
        std::fs::write(
            project_root.join(".ainxt/roles/reviewer.toml"),
