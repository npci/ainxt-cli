//! `ainxt.dev/billing` extension handler.
//!
//! Fetches the authenticated user's ainxt billing configuration
...
/// Current billing configuration for ainxt coding credits.
///
/// Carries both the newer credits-config fields (`credit_usage_percent`,
/// `current_period`) and the deprecated `AinxtBuildBillingConfig` fields
...
/// against both the new `GetAinxtCreditsConfig` and the legacy
/// `GetAinxtBuildBillingConfig` backend responses.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct BillingConfig {
    ...
    /// (`GetAinxtCreditsConfig.prepaid_balance`); absent in the legacy billing
    /// shape.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub prepaid_balance: Option<Cent>,
    /// Whether this user is on unified usage billing (shared weekly/monthly
    /// pool). From `AinxtCreditsConfig.is_unified_billing_user`, which billing
    /// sets from remote settings `unified_consumer_billing_enabled`. `None` when
    /// absent (legacy `GetAinxtBuildBillingConfig` shape or older servers).
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// Top-level response (primarily from `GET /rest/ainxt/credits` + auto-topup-rule).
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BillingConfigResponse {
    ...
    /// User-friendly subscription tier name (e.g. "SuperAinxt Heavy").
...
///
/// Keeps history to a count + the most recent period so `~/.ainxt/logs/unified.jsonl`
...
async fn handle_get_billing(agent: &MvpAgent) -> ExtResult {
    let auth = super::auth_gate::require_ainxt_auth(
        &agent.auth_manager,
        "Authentication required to fetch billing data",
        "Billing data requires auth with ainxt.dev. Run `ainxt login` to authenticate.",
    )?;
    ...
    // forwards to the backend `GetAinxtCreditsConfig`.
    ...
    let credits_resp = crate::http::shared_client()
        ...
        .header(
            "X-Ainxt-Token-Auth",
            crate::auth::AinxtComConfig::default().token_header,
        )
    ...
    // Every prompt / /usage / poll path hits `ainxt.dev/billing`; log the fetched
...
async fn handle_get_auto_topup_rule(agent: &MvpAgent) -> ExtResult {
    let auth = super::auth_gate::require_ainxt_auth(
        ...
        "Auto top-up data requires auth with ainxt.dev. Run `ainxt login` to authenticate.",
    )?;
    ...
    // Return the upstream response body unchanged (as a JSON value) so /usage
...
mod tests {
    ...
    fn billing_unified_log_ctx_includes_credits_and_collapses_history() {
        ...
        assert_eq!(ctx["subscriptionTier"], "SuperAinxt");
    ...
    fn billing_config_deserializes_credits_config_shape() {
        // Newer `GetAinxtCreditsConfig` response: percentage-based usage,
        ...
        let json = serde_json::json!({
            "config": {
                ...
                "productUsage": [
                    {"product": "PRODUCT_AINXT_BUILD", "usagePercent": 61.2}
                ],
                "history": [
                    {
                        "period": {
