//! `ainxt.dev/memory/flush`, `ainxt.dev/memory/rewrite`, and `ainxt.dev/compact_conversation`
//! extension handlers.
//!
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        m if m.starts_with("ainxt.dev/compact_conversation") => handle_compact(agent, args).await,
        "ainxt.dev/memory/flush" => handle_flush(agent, args).await,
        "ainxt.dev/memory/rewrite" => handle_rewrite(agent, args).await,
