//! `ainxt.dev/plugins/*` extension handlers.
//!
...
pub fn loaded_plugin_to_info(plugin: &ainxt_agent::plugins::LoadedPlugin) -> PluginInfo {
    use ainxt_agent::plugins::discovery::PluginScope as AgentScope;
...
fn origin_to_dto(origin: &ainxt_agent::plugins::PluginOrigin) -> PluginOrigin {
    use ainxt_agent::plugins::PluginOrigin as AgentOrigin;
    match origin {
        AgentOrigin::CliOverride => PluginOrigin::CliOverride,
        AgentOrigin::ProjectAinxt => PluginOrigin::ProjectAinxt,
        AgentOrigin::ProjectClaude => PluginOrigin::ProjectClaude,
        AgentOrigin::UserAinxt => PluginOrigin::UserAinxt,
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "ainxt.dev/plugins/action" => {
            let req: ainxt_hooks_plugins_types::PluginsActionRequest = super::parse_params(args)?;
            let sid = acp::SessionId::new(req.session_id);
...
mod tests {
    use super::*;
    use ainxt_agent::plugins::PluginOrigin as AgentOrigin;
    use ainxt_agent::plugins::discovery::{PluginId, PluginScope as AgentScope};

    fn make_loaded_plugin(origin: AgentOrigin) -> ainxt_agent::plugins::LoadedPlugin {
        let root = std::path::PathBuf::from("/tmp/test-plugin");
        ainxt_agent::plugins::LoadedPlugin {
    ...
    fn info_carries_origin_and_marketplace_display_name() {
        let plugin = make_loaded_plugin(AgentOrigin::MarketplaceInstall {
            source_name: Some("ainxt Official".to_string()),
        ...
        assert_eq!(info.marketplace_source.as_deref(), Some("ainxt Official"));
    }
