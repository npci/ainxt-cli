...
/// Request for `ainxt.dev/git/suggest_commit_message`.
///
/// The client may pass the staged `diff` text directly; if omitted (or blank)
/// the server gathers the staged diff itself via the existing git diff op
/// (`HEAD`→`staged`, patch text). Everything else mirrors the neighboring
/// git request structs (session-based git-root resolution).
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct GitSuggestCommitMessageRequest {
    #[serde(default)]
    pub session_id: Option<acp::SessionId>,
    #[serde(default)]
    pub git_root: Option<String>,
    /// Optional staged diff text supplied by the client. When absent, the
    /// server gathers it from the staged tree.
    #[serde(default)]
...
/// Request for ainxt.dev/git/checkout_commit extension method.
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
...
async fn resolve_git_root(
    ...
    if let Some(sid) = session_id {
        if let Some(cwd) = agent.get_session_cwd(sid) {
            let result = ops
                .dispatch(
                    &ainxt_workspace::workspace_ops::GitResolveRootReq { cwd },
                    None,
                )
                .await
                .map_err(|e| {
...
pub async fn handle(
    ...
    if let Some(git_root) = try_resolve_git_root(agent, ops, args).await {
        let vcs_kind = ops
            .dispatch(
                &ainxt_workspace::workspace_ops::DetectVcsKindReq {
                    path: git_root.clone(),
                },
                None,
            )
            .await
            .unwrap_or(ainxt_workspace::session::git::VcsKind::Git);
        if vcs_kind.is_jj()
            && let Some(result) =
    ...
    match args.method.as_ref() {
        ...
        "ainxt.dev/git/status" => {
            ...
            if let Some(ref git_root) = git_root {
                let current_commit = ops
                    .dispatch(
                        &ainxt_workspace::workspace_ops::GitCurrentCommitReq {
        ...
        "ainxt.dev/git/suggest_commit_message" => {
            let req = parse_params::<GitSuggestCommitMessageRequest>(args)?;
            let git_root = resolve_git_root(agent, ops, req.git_root, req.session_id.as_ref())
                .await
                .ok();
            // Source the staged diff: prefer the client-supplied text, else
            // gather it server-side (HEAD → staged, patch text).
            ...
            // One-shot completion using the session's active sampler + model.
            // No model id is hardcoded; if no sampler is available the client
            // build fails and we surface a "couldn't suggest" error.
            let sampling_config = agent.models_manager.sampling_config();
            let model = sampling_config.model.clone();
            if model.trim().is_empty() {
                return to_ext_response::<serde_json::Value>(Err(anyhow::anyhow!(
                    "couldn't suggest a commit message: no model configured"
                )));
            }
            let client = match crate::sampling::Client::new(sampling_config) {
                Ok(c) => c,
                Err(e) => {
                    return to_ext_response::<serde_json::Value>(Err(anyhow::anyhow!(
                        "couldn't suggest a commit message: {e}"
                    )));
                }
            };
            match crate::session::helpers::commit_message::suggest_commit_message(
                diff, client, &model,
            )
            .await
            {
                Ok(message) => to_ext_response(Ok(serde_json::json!({ "message": message }))),
                Err(e) => to_ext_response::<serde_json::Value>(Err(anyhow::anyhow!(
                    "couldn't suggest a commit message: {e}"
                ))),
            }
        }
        "ainxt.dev/git/checkout" => {
        ...
        "ainxt.dev/git/checkout_session_head" => {
            ...
            let result = ops
                .dispatch(
                    &ainxt_workspace::workspace_ops::GitCheckoutCommitReq {
                        git_root: git_root.clone(),
                        head_commit,
