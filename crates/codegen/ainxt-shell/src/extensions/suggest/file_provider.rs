...
struct SplitToken<'a> {
    ...
    match_prefix: &'a str,
    /// Unchanged request-text slice kept in front of every completion, so
...
mod tests {
    ...
    async fn suggest_var_prefix_lists_expansion_inserts_raw() {
        ...
        let _env = ainxt_test_support::EnvGuard::set("AINXT_SUGGEST_TEST_DIR", tmp.path());
        let ctx = SuggestContext::new(
            "cat $AINXT_SUGGEST_TEST_DIR/no".into(),
            "cat $AINXT_SUGGEST_TEST_DIR/no".len(),
            "/ignored".into(),
        );
        let results = FilePathProvider.suggest(&ctx).await;
        assert_eq!(results.len(), 1);
        assert_eq!(
            results[0].token_text.as_deref(),
            Some("$AINXT_SUGGEST_TEST_DIR/notes.md")
        );
    }
