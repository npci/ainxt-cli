...
pub(super) fn parse_current_token(prefix: &str) -> CurrentToken {
    ...
    // (balanced, kept unchanged) — only closers the component consumed
...
/// Build the replacement for the whole token: the user's unchanged directory
...
mod tests {
    ...
    /// raw dir keeps the user's quoting unchanged.
    #[test]
    ...
    /// unchanged) and must not force quote rendering on the component.
    #[test]
