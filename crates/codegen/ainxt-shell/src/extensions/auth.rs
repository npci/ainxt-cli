//! `ainxt.dev/auth/*` and legacy `ainxt.dev/{get,set}ApiKey` extension handlers.
//!
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "ainxt.dev/auth/getBearerToken" => handle_get_bearer_token(agent).await,
        "ainxt.dev/getApiKey" => handle_get_api_key(),
        "ainxt.dev/setApiKey" => handle_set_api_key(args),
        "ainxt.dev/auth/submit_code" => handle_submit_code(agent, args),
        "ainxt.dev/auth/get_url" => handle_get_url(agent).await,
        "ainxt.dev/auth/logout" => handle_logout(agent, args).await,
        "ainxt.dev/auth/info" => handle_info(agent),
        "ainxt.dev/auth/check_subscription" => handle_check_subscription(agent).await,
...
fn handle_get_api_key() -> ExtResult {
    let key = crate::agent::auth_method::read_ainxt_api_key_env().ok();
...
fn handle_set_api_key(args: &acp::ExtRequest) -> ExtResult {
    ...
    if let Some(k) = key {
        if k.is_empty() {
            crate::auth::clear_api_key(&ainxt_home)
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
            // SAFETY: ext_method is single-threaded per agent
            unsafe { std::env::remove_var("AINXT_API_KEY") };
        } else {
            crate::auth::store_api_key(&ainxt_home, k)
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
            // SAFETY: ext_method is single-threaded per agent
            unsafe { std::env::set_var("AINXT_API_KEY", k) };
        }
    } else {
        crate::auth::clear_api_key(&ainxt_home)
            .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
...
fn handle_info(agent: &MvpAgent) -> ExtResult {
    ...
    struct AuthInfoResponse {
        ...
        /// `ainxt-asset://` URL resolved by the Electron protocol handler,
    ...
    // Return a ainxt-asset:// URL that the Electron renderer resolves at
    ...
    let profile_image_url = match raw_asset_id.as_deref().filter(|k| !k.is_empty()) {
        ...
        Some(key) => Some(format!("ainxt-asset:///{key}")),
        None => None,
    };
