//! `ainxt.dev/feedback`, `ainxt.dev/feedback/dismiss`, `ainxt.dev/btw`, and `ainxt.dev/review/*`
//! extension handlers.
//!
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "ainxt.dev/feedback" | "ainxt.dev/feedback/dismiss" => {
            tracing::info!("handling user feedback");
            handle_feedback(agent, args).await
        }
        m if m.starts_with("ainxt.dev/review") => {
...
/// Handle `ainxt.dev/btw` -- a side question that doesn't interrupt the current turn.
...
async fn handle_feedback(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    if !agent.cfg.borrow().is_feedback_enabled() {
        return Err(acp::Error::internal_error().data(
            "Feedback is disabled. To enable, set AINXT_FEEDBACK_ENABLED=true or \
...
///
/// Methods:
/// - `ainxt.dev/review/comment`: record a new inline code comment to cloud storage
/// - `ainxt.dev/review/comment/delete`: record a tombstone event for a deleted comment
async fn handle_review(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "ainxt.dev/review/comment" => {
        ...
        "ainxt.dev/review/comment/delete" => {
