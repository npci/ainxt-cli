...
//! using the ainxt-codebase-graph index.
//!
//! ## Extension Methods
//!
//! | Method | Description |
//! |--------|-------------|
//! | `ainxt.dev/code/goto-definition` | Definition location(s) for symbol at position |
//! | `ainxt.dev/code/goto-references` | Reference location(s) for symbol at position |
//! | `ainxt.dev/code/find-definitions` | All definitions of a symbol by name |
//! | `ainxt.dev/code/find-references` | All references to a symbol by name |
//! | `ainxt.dev/code/status` | Indexing status |
...
/// Reason string for the `ainxt.dev/code/status` response.
///
...
pub async fn handle(
    ...
    match args.method.as_ref() {
        "ainxt.dev/code/goto-definition" => {
        ...
        "ainxt.dev/code/goto-references" => {
            let req: GotoRequest = serde_json::from_str(args.params.get())
                .map_err(|e| acp::Error::invalid_params().data(format!("invalid params: {e}")))?;
            let cwd = resolve_cwd(agent, req.cwd.clone(), req.session_id.as_ref())?;
            let was_newly_started =
        ...
        "ainxt.dev/code/find-definitions" => {
        ...
        "ainxt.dev/code/find-references" => {
            let req: FindSymbolRequest = serde_json::from_str(args.params.get())
                .map_err(|e| acp::Error::invalid_params().data(format!("invalid params: {e}")))?;
            let cwd = resolve_cwd(agent, req.cwd.clone(), req.session_id.as_ref())?;
            let was_newly_started =
...
fn to_code_nav_ext_response(resp: ainxt_workspace::workspace_ops::CodeNavResponse) -> ExtResult {
    let symbol = resp
        .locations
        .first()
...
fn eligibility_error(reason: CodeNavEligibility) -> acp::Error {
    let msg = match reason {
        CodeNavEligibility::ClientNotWeb => {
            "code navigation is currently only enabled for ainxt-web clients"
        }
        CodeNavEligibility::CapabilityNotAdvertised => {
            "client must advertise ainxt.dev/codeNavigation.enabled to use code navigation"
        }
