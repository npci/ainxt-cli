...
//!
//! - `ainxt.dev/mcp/list` — list available MCP servers (agent-scoped or session-annotated)
//! - `ainxt.dev/mcp/call` — invoke an MCP tool directly, outside the LLM loop
//! - `ainxt.dev/mcp/servers_updated` — notification pushed when managed configs resolve
//! - `ainxt.dev/mcp/server_status` — per-server delta pushed by the
...
// rmcp is quarantined in ainxt-mcp; see that crate's docs.
use ainxt_mcp::rmcp;
...
/// Agent-only `ainxt.dev/mcp/*` ACP method/notification names.
///
/// Unlike [`wire::MCP_CALL`] (the cross-SDK contract, which stays in
/// `ainxt_mcp::wire`), these methods are private to the agent↔client channel and
...
pub mod mcp_methods {
    ...
    pub const PREFIX: &str = "ainxt.dev/mcp/";

    pub const LIST: &str = "ainxt.dev/mcp/list";
    pub const READ_RESOURCE: &str = "ainxt.dev/mcp/read_resource";
    pub const AUTH_STATUS: &str = "ainxt.dev/mcp/auth_status";
    pub const AUTH_TRIGGER: &str = "ainxt.dev/mcp/auth_trigger";
    pub const TOGGLE: &str = "ainxt.dev/mcp/toggle";
    pub const TOGGLE_TOOL: &str = "ainxt.dev/mcp/toggle_tool";
    pub const UPSERT: &str = "ainxt.dev/mcp/upsert";
    pub const DELETE: &str = "ainxt.dev/mcp/delete";

    pub const SERVERS_UPDATED: &str = "ainxt.dev/mcp/servers_updated";
    pub const TOOLS_CHANGED: &str = "ainxt.dev/mcp/tools_changed";
    pub const INIT_PROGRESS: &str = "ainxt.dev/mcp/init_progress";
}
...
// Re-export the `ainxt.dev/mcp/server_status` schema +
// method constant from the dispatcher module so external callers
// have a single import point alongside the other `ainxt.dev/mcp/*`
// types.
//
...
pub async fn notify_servers_updated(
    gateway: &ainxt_acp_lib::AcpAgentGatewaySender,
    ...
    if let Ok(params) = serde_json::value::to_raw_value(&payload) {
        ...
        tracing::info!("Sent ainxt.dev/mcp/servers_updated notification to client");
    }
}

// ── Dispatch ────────────────────────────────────────────────────────

/// Inbound `ainxt.dev/mcp/*` methods this agent services, resolved from the wire string.
///
...
pub async fn build_mcp_status(
    ...
    tool_bridge: &Arc<ainxt_tools::bridge::ToolBridge>,
    event_writer: Option<&ainxt_file_utils::events::EventWriter>,
) -> McpStatusSnapshot {
    ...
    for client in &clients {
        ...
        if let Some(ew) = event_writer {
            ew.emit(ainxt_file_utils::events::Event::McpHealthCheck {
...
pub async fn init_agent_mcp_pool(mcp_state: &Arc<TokioMutex<McpState>>, cwd: &std::path::Path) {
    ...
    let noop = ainxt_file_utils::events::EventWriter::noop();
...
//
// Implements the `McpResourceProvider` trait from ainxt-tools so that
// `ListMcpResources` / `FetchMcpResource` tools can access MCP
// servers without depending on `ainxt-mcp` directly.
...
impl ainxt_tools::types::resources::McpResourceProvider for McpStateResourceProvider {
    async fn list_resources(
        &self,
        server: Option<String>,
    ) -> Result<Vec<ainxt_tools::types::resources::McpResourceInfo>, String> {
            ...
            match mcp_service.list_all_resources().await {
                Ok(all_resources) => {
                    for r in all_resources {
                        resources.push(ainxt_tools::types::resources::McpResourceInfo {
                            uri: r.uri.clone(),
    ...
    async fn read_resource(
        ...
        server: String,
        uri: String,
    ) -> Result<ainxt_tools::types::resources::McpResourceReadResult, String> {
        let client = {
            let state = self.0.lock().await;
            Arc::clone(
                state
        ...
        match first {
            rmcp::model::ResourceContents::TextResourceContents {
                ...
                text,
                ..
            } => Ok(ainxt_tools::types::resources::McpResourceReadResult {
                uri: content_uri,
                name: None,
                description: None,
                mime_type,
                content: Some(ainxt_tools::types::resources::McpResourceContent::Text(
                    text,
                )),
            }),
            rmcp::model::ResourceContents::BlobResourceContents {
                ...
                description: None,
                mime_type,
                content: Some(ainxt_tools::types::resources::McpResourceContent::Blob(
                    blob.into_bytes(),
                )),
            }),
...
struct McpAuthTriggerResponse {
    ...
    /// failures with no detail; surfaced unchanged by the TUI.
    #[serde(skip_serializing_if = "Option::is_none")]
    error: Option<String>,
}
...
mod tests {
    ...
    /// The emit-only reverse method (`ainxt.dev/mcp/sdk_call`) shares the `ainxt.dev/mcp/`
    ...
    fn inbound_sdk_call_has_no_forward_route() {
        ...
        assert_eq!(
            ...
            None,
            "inbound ainxt.dev/mcp/sdk_call must not resolve to a forward handler"
        );
    ...
    /// refactor than this test warrants. Instead this test matches the exact
    ...
    async fn handle_list_parallel_join_pattern_regression() {
        ...
        async fn run(cache: bool) -> (Duration, bool, usize) {
            ...
            // followed by `build_mcp_status`. Matches the closure in
            // `handle_list`.
            let session_fut = {
    ...
    fn test_mcp_list_response_serialization() {
        let resp = McpListResponse {
            servers: vec![
                McpServerEntry {
                    name: "ainxt_com_linear".to_string(),
                    display_name: None,
                    source: McpServerSource::Managed,
                    config: McpServerConfig::Http {
                        ...
                        scope_name: Some("ainxt CLI".to_string()),
                    },
                    source_label: None,
                    session: None,
                },
        ...
        assert_eq!(json["servers"][0]["scopeName"], "ainxt CLI");
    ...
    fn disabled_managed_http_rows_keep_non_gateway_placeholder_config() {
        let entry = disabled_server_placeholder_entry("ainxt_com_slack");
        assert_eq!(entry.source, McpServerSource::Managed);
        assert!(matches!(entry.config, McpServerConfig::Stdio { .. }));
    }

    /// Matches `handle_list` set construction: catalog names from
    ...
    fn disabled_placeholders_match_handle_list_catalog_relationships() {
        ...
        append_disabled_like_handle_list(
            &mut servers,
            &["ainxt_com_slack", "ainxt_mcp_linear"],
            true,
        );
        assert!(
            servers.is_empty(),
        ...
        let mut servers = build_mcp_catalog_with_gateway_tools(&[], &[], None, &Default::default());
        append_disabled_like_handle_list(&mut servers, &["ainxt_com_slack"], false);
        assert_eq!(servers.len(), 1);
        assert_eq!(servers[0].name, "ainxt_com_slack");
    ...
    fn test_disabled_session_state_serialization() {
        let entry = McpServerEntry {
            name: "ainxt_com_slack".to_string(),
            display_name: None,
            source: McpServerSource::Managed,
            source_label: None,
            config: McpServerConfig::Http {
