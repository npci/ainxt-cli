...
fn discover_auto_sources(cwd: &str, skills: &[SkillInfo]) -> Vec<(String, usize)> {
    ...
    for subdir in &subdirs {
        try_add_source(ainxt_home.join(subdir), None);
    }
...
pub async fn handle(
    ...
    match args.method.as_ref() {
        "ainxt.dev/skills/add" => {
            ...
            .await
            {
                ainxt_telemetry::session_ctx::log_event(
                    ainxt_telemetry::events::SkillAdded {
                        added_count: 0,
                        total_skills: 0,
                        success: false,
                    },
            ...
            ainxt_telemetry::session_ctx::log_event(ainxt_telemetry::events::SkillAdded {
        ...
        "ainxt.dev/skills/remove" => {
            ...
            {
                ainxt_telemetry::session_ctx::log_event(
                    ainxt_telemetry::events::SkillRemoved { success: false },
                );
            ...
            ainxt_telemetry::session_ctx::log_event(ainxt_telemetry::events::SkillRemoved {
                success: true,
            });
