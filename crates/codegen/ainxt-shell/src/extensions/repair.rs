//! `ainxt.dev/session/repair` — out-of-band recovery for sessions bricked by
...
//!
//! Repairs via [`ainxt_chat_state::compaction_utils::repair_history`]. Resident
...
use ainxt_chat_state::compaction_utils::HistoryRepairReport;
...
/// Response payload for `ainxt.dev/session/repair`.
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "ainxt.dev/session/repair" => handle_session_repair(agent, args).await,
...
/// back atomically. `ainxt_root` is injectable for tests.
async fn repair_on_disk(ainxt_root: &std::path::Path, session_id: &str, dry_run: bool) -> ExtResult {
    let summary = crate::session::persistence::find_summary_by_session_id_in_root(
        session_id,
        &ainxt_root.join("sessions"),
    )
    .ok_or_else(|| {
    ...
    let storage = JsonlStorageAdapter::with_root(ainxt_root.to_path_buf());
    ...
    let report = ainxt_chat_state::compaction_utils::repair_history(&mut chat_history);
