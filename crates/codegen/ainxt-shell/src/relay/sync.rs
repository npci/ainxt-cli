...
/// Format: https://ainxt.example.test/build/{sessionId}
pub fn build_share_url(session_id: &str) -> String {
    let base_url =
        std::env::var("AINXT_CODE_WEB_URL").unwrap_or_else(|_| "https://ainxt.example.test".to_string());
...
async fn relay_sync_task(
    ...
    loop {
        tokio::select! {
            _ = cancel.cancelled() => {
                tracing::debug!(session_tag = %crate::util::session_log_tag(&session_id), "RelaySync: cancel received");
                break;
            }
            ...
            msg = from_relay_rx.recv() => {
                match msg {
                    ...
                    None => {
                        tracing::debug!(session_tag = %crate::util::session_log_tag(&session_id), "RelaySync: relay channel closed");
                        break;
                    }
                }
            }
            ...
            msg = rx.recv() => {
                match msg {
                    ...
                    Some(RelaySyncMsg::Shutdown) => {
                        tracing::debug!(session_tag = %crate::util::session_log_tag(&session_id), "RelaySync: shutdown requested");
                        break;
                    }
                    None => {
                        tracing::debug!(session_tag = %crate::util::session_log_tag(&session_id), "RelaySync: channel closed");
                        break;
                    }
                }
            }
    ...
    tracing::debug!(session_tag = %crate::util::session_log_tag(&session_id), "RelaySync task ended");
}
    ...
    match method {
        Some("initialize") => {
            ...
            if let Some(request_id) = id {
                tracing::debug!(session_tag = %crate::util::session_log_tag(session_id), "RelaySync: received initialize request from relay");
                ...
                *initialized = true;
                ...
                let upsert = json!({
                    "jsonrpc": "2.0",
                    "method": "_ainxt.dev/session/upsert",
                    "params": {
                        "sessionId": session_id,
                        "cwd": cwd,
                    }
                ...
                if to_relay_tx.send(upsert.to_string()).is_err() {
                    tracing::warn!(session_tag = %crate::util::session_log_tag(session_id), "RelaySync: failed to send session/upsert");
                }
        ...
        Some("_ainxt.dev/relay/initialized") => {
            tracing::debug!(session_tag = %crate::util::session_log_tag(session_id), "RelaySync: relay confirmed TUI sync mode");
        }
        Some(other) => {
            tracing::debug!(
                session_tag = %crate::util::session_log_tag(session_id),
        ...
        None => {
            tracing::trace!(session_tag = %crate::util::session_log_tag(session_id), "RelaySync: received message without method");
        }
    }
...
mod tests {
    ...
    fn test_build_share_url_default() {
        ...
        assert_eq!(url, "https://ainxt.example.test/build/test-session-123");
    }
    ...
    fn test_build_share_url_with_uuid() {
        ...
        assert_eq!(
            url,
            "https://ainxt.example.test/build/01937d8a-1234-7abc-9def-0123456789ab"
        );
    }
