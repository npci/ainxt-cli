...
use ainxt_agent::plugins::git_install::{self, UpdateStatus};
use ainxt_agent::plugins::install_registry::{
    InstallError, InstallKind, InstallRegistry, InstalledRepo, MarketplaceProvenance,
};
use ainxt_plugin_marketplace::git::{self, SourceCacheLease};
use ainxt_plugin_marketplace::{
...
/// When `keep_data` is true, `~/.ainxt/plugin-data/<id>/` is preserved.
...
fn marketplace_root_for_provenance(
    provenance: &ainxt_agent::plugins::install_registry::MarketplaceProvenance,
                ...
                if registered.is_empty() {
                    write!(
                        ...
                        "Unknown marketplace \"{qualifier}\". No marketplaces are registered; \
                         add one with `ainxt plugin marketplace add`."
                    )
                } else {
            ...
            Self::NameNotFound {
                ...
                write!(
                    ...
                    "No marketplace plugin named \"{name}\" in any registered marketplace.\n\
                     Install a local directory with `ainxt plugin install ./{name}`, or add a \
                     source with `ainxt plugin marketplace add`."
                )?;
            ...
            Self::NameAmbiguous { name, candidates } => {
                ...
                write!(
                    ...
                    "Multiple marketplaces provide a plugin named \"{name}\":\n{list}\n\
                     Pin one with `ainxt plugin install {name}@<qualifier>`."
                )
            }
            Self::PartialScan {
                name,
                ...
                write!(
                    ...
                    "Couldn't scan every marketplace while resolving \"{name}\", so it can't be \
                     ...
                     Retry, or pin the source explicitly with `ainxt plugin install {name}@<qualifier>`."
                )
            }
            Self::Sync {
                source_display,
...
fn filter_sources_by_allowlist(
    ...
    allowlist: &ainxt_workspace::permission::resolution::MarketplaceAllowlist,
...
/// `~/.ainxt/` and `~/.claude/`. Returns `true` if removed from at least one file.
pub fn try_remove_source_from_json_files(source_url_or_path: &str) -> bool {
    // Resolve user ainxt via user_ainxt_home() (None when no home resolves) and
    // home separately, so removal still runs from $AINXT_HOME when no home dir
    // exists, and never touches a cwd-relative .ainxt.
    let home = dirs::home_dir();
    let ainxt = ainxt_config::user_ainxt_home();

    let mut settings_candidates: Vec<std::path::PathBuf> = Vec::new();
    if let Some(ref ainxt) = ainxt {
        settings_candidates.push(ainxt.join("settings.local.json"));
        settings_candidates.push(ainxt.join("settings.json"));
    }
    ...
    if let Some(ref ainxt) = ainxt {
        known_candidates.push(ainxt.join("plugins").join("known_marketplaces.json"));
    }
    if let Some(ref home) = home {
...
mod tests {
    ...
    fn registered_source_label_uses_addressable_qualifier() {
        assert_eq!(
            registered_source_label(&git_source(
                "ainxt Official",
                "https://github.com/ainxt-org/plugin-marketplace.git"
            )),
            "ainxt Official (ainxt-org/plugin-marketplace)"
        );
        assert_eq!(
    ...
    fn candidate_label_includes_pin_hint() {
        assert_eq!(
            ...
            "ainxt Official (pin: sentry@ainxt-org/plugin-marketplace)"
        );
        assert_eq!(
    ...
    fn unknown_qualifier_error_lists_registered_marketplaces() {
        let err = MarketplaceInstallError::UnknownQualifier {
            ...
            registered: vec![
                "ainxt Official (ainxt-org/plugin-marketplace)".into(),
        ...
        assert!(
            msg.contains("  - ainxt Official (ainxt-org/plugin-marketplace)"),
            "{msg}"
        );
    ...
    fn ambiguous_qualifier_error_lists_source_names() {
        let err = MarketplaceInstallError::AmbiguousQualifier {
            qualifier: "ainxt-org/plugin-marketplace".into(),
    ...
    fn name_not_found_error_hints_local_dir_and_add_source() {
        ...
        let msg = err.to_string();
        assert!(msg.contains("ainxt plugin install ./sentry"), "{msg}");
        assert!(msg.contains("ainxt plugin marketplace add"), "{msg}");
    ...
    fn name_ambiguous_error_lists_candidates_and_pin_hint() {
        let err = MarketplaceInstallError::NameAmbiguous {
            name: "sentry".into(),
            candidates: vec!["ainxt Official (pin: sentry@ainxt-org/plugin-marketplace)".into()],
        };
        let msg = err.to_string();
        assert!(
        ...
        assert!(
            msg.contains("  - ainxt Official (pin: sentry@ainxt-org/plugin-marketplace)"),
            "{msg}"
        );
        assert!(
            msg.contains("ainxt plugin install sentry@<qualifier>"),
            "{msg}"
        );
    }
    ...
    const OFFICIAL_URL: &str = "https://github.com/ainxt-org/plugin-marketplace.git";

    #[test]
    fn plan_install_qualifier_unknown_lists_registered_labels() {
        let sources = [
            git_source("ainxt Official", OFFICIAL_URL),
        ...
        match err {
            MarketplaceInstallError::UnknownQualifier {
                ...
                assert_eq!(
                    registered,
                    vec![
                        "ainxt Official (ainxt-org/plugin-marketplace)".to_string(),
    ...
    fn plan_install_qualifier_ambiguous_lists_source_names() {
        let sources = [
            ...
            git_source("Mirror B", "git@github.com:ainxt-org/plugin-marketplace.git"),
        ];
        let err = plan_install(
            &sources,
            "sentry",
            Some("ainxt-org/plugin-marketplace"),
            |_| Ok(Vec::new()),
        )
        .expect_err("two sources share the owner/repo");
        match err {
            MarketplaceInstallError::AmbiguousQualifier { qualifier, sources } => {
                assert_eq!(qualifier, "ainxt-org/plugin-marketplace");
                assert_eq!(
                    sources,
    ...
    fn plan_install_qualifier_not_found_when_scan_lacks_name() {
        let sources = [git_source("ainxt Official", OFFICIAL_URL)];
        let err = plan_install(
            &sources,
            "sentry",
            Some("ainxt-org/plugin-marketplace"),
        ...
        match err {
            MarketplaceInstallError::QualifiedNameNotFound {
                ...
                assert_eq!(source_display, "ainxt Official");
            }
    ...
    fn marketplace_allowlist(
        urls: &[&str],
    ) -> ainxt_workspace::permission::resolution::MarketplaceAllowlist {
        ainxt_workspace::permission::resolution::MarketplaceAllowlist {
    ...
    fn filter_sources_by_allowlist_unrestricted_passes_everything() {
        let allowlist = ainxt_workspace::permission::resolution::MarketplaceAllowlist::default();
        let sources = vec![
    ...
    fn resolve_qualified_source_name_with_matches_git_owner_repo() {
        ...
        let name = resolve_qualified_source_name_with(&sources, "ainxt-org/plugin-marketplace")
            .expect("qualifier should match the official source");
        assert_eq!(name, "ainxt Official");
    }
