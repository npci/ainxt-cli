//! `ainxt mcp doctor` -- runtime health check for MCP servers.
...
fn discover_servers(cwd: &Path) -> (Vec<ConfigSourceStatus>, Vec<DiscoveredServer>) {
    ...
    let user_config = ainxt_home.join("config.toml");
    if user_config.is_file() {
        sources.push(ConfigSourceStatus {
            path: "~/.ainxt/config.toml".to_string(),
...
// ── Managed (ainxt.dev) server discovery ─────────────────────────

const MANAGED_SOURCE_LABEL: &str = "ainxt.dev";
...
/// Discover managed `ainxt_com_*` servers if the user has ainxt auth on disk.
async fn try_discover_managed_servers() -> (ConfigSourceStatus, Vec<DiscoveredServer>) {
    let ainxt_home = ainxt_tools::util::ainxt_home::ainxt_home();
    let ainxt_com_config = AinxtComConfig::default();
    let auth_manager = Arc::new(crate::auth::AuthManager::new(&ainxt_home, ainxt_com_config));
    ...
    if !snapshot.is_managed_mcp_eligible() {
        return managed_skipped(format!("{:?} auth (not ainxt OIDC)", snapshot.auth_mode));
    }

    let token = match auth_manager.get_valid_token().await {
        Ok(key) => key,
        Err(_) => return managed_skipped("auth expired — run `ainxt login`"),
    };
...
async fn check_tools_list(service: &mcp_servers::McpService) -> Check {
    use ainxt_mcp::rmcp::model::PaginatedRequestParams;
    match service
...
pub async fn run_doctor(cwd: &Path, name_filter: Option<&str>) -> DoctorReport {
    ...
    // Folder-trust gate: `ainxt mcp doctor` actually STARTS each server
    ...
    //
    // `remote = None` is intentional: standalone `ainxt mcp doctor` has no loaded
...
pub fn print_report(report: &DoctorReport) {
    ...
    if report.servers.is_empty() {
        ...
        println!("  Run `ainxt mcp add --help` to get started.");
        println!();
        return;
    }
    ...
    println!(
        ...
        if report.failing_count > 0 {
            " Run `ainxt mcp doctor --json` for full diagnostics."
        } else {
            ""
        }
    );
