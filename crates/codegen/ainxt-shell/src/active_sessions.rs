//! Tracks open TUI sessions in `~/.ainxt/active_sessions.json` for crash
...
pub fn try_unregister(session_id: &acp::SessionId) -> io::Result<bool> {
    try_unregister_in(&crate::util::ainxt_home::ainxt_home(), session_id)
}

/// Remove entries with dead PIDs and return them.
pub fn collect_crashed() -> io::Result<Vec<ActiveSession>> {
    collect_crashed_in(&crate::util::ainxt_home::ainxt_home())
}
...
///
/// `data_path`/`tmp_path`/`lock_path` are always `root.join(<fixed filename>)`
/// (see `with_locked_state`/`try_with_locked_state`/`list_in`), so this can
/// only trip if the caller-supplied `root` itself contains `..` -- callers
/// pass either `ainxt_home()` or, in tests, a freshly created `TempDir`,
/// neither of which ever does. Kept as an explicit, enforced invariant
/// (rather than one that's merely true by construction) directly at the
/// file read/write sites.
fn has_no_traversal_component(path: &Path) -> bool {
    !path
        .components()
...
fn read_data_file(path: &Path) -> io::Result<Vec<ActiveSession>> {
    if !has_no_traversal_component(path) {
        return Err(io::Error::new(
            ...
            format!("refusing to read path containing '..': {}", path.display()),
        ));
    }
    match fs::read(path) {
...
fn write_data_file_atomic(
    ...
    if !has_no_traversal_component(tmp_path) || !has_no_traversal_component(data_path) {
        return Err(io::Error::new(
            io::ErrorKind::InvalidInput,
            "refusing to write path containing '..'",
        ));
    }
