...
use ainxt_sampler::SamplerConfig;
use ainxt_tools::implementations::ainxt_build;
use ainxt_tools::registry::types::ToolConfig;

/// Production ainxt-build foreground command-timeout ceiling (seconds). The
...
impl BashToolConfig {
    ...
    pub fn to_bash_params_json(
        ...
        // production ainxt-build opts up to 10h by sending it explicitly
...
pub struct WebFetchToolConfig {
    ...
    /// this URL. Resolution: TOML > `AINXT_WEB_FETCH_PROXY` env > remote settings > None.
...
impl WebFetchToolConfig {
    ...
    pub fn resolve_params(
    ...
    ) -> ainxt_tools::implementations::ainxt_build::web_fetch::WebFetchParams {
        ...
        let proxy_endpoint = self
            ...
            .as_ref()
            .cloned()
            .or_else(|| env_string("AINXT_WEB_FETCH_PROXY"))
...
/// from `ainxt_tools::registry::types::ToolsetConfig` which holds
...
///
/// No bundled default: the model comes from the gateway-sourced base config. An
/// empty model is passed through unchanged (web search no-ops / fails loudly)
/// rather than being substituted with a fabricated slug.
...
///
/// Selects between the standard `AinxtBuild` toolset (`read_file`,
/// `search_replace`, `grep`) and the anchor-based `AinxtBuildHashline`
        ...
        match self {
            ...
            Self::Hashline => {
                ...
                Ok(vec![
                    ...
                    ToolConfig {
                        id: "AinxtBuildHashline:hashline_edit".to_owned(),
                        params: params_map.clone(),
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
                    ...
                    ToolConfig {
                        id: "AinxtBuildHashline:hashline_grep".to_owned(),
                        params: params_map,
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
...
mod tests {
    ...
    fn standard_toolset_configs() {
        ...
        assert!(ids.contains(&"AinxtBuild:read_file"));
        assert!(ids.contains(&"AinxtBuild:search_replace"));
        assert!(ids.contains(&"AinxtBuild:grep"));
    }
    ...
    fn hashline_toolset_configs() {
        ...
        assert_eq!(configs.len(), 3);
        let ids: Vec<&str> = configs.iter().map(|c| c.id.as_str()).collect();
        assert!(ids.contains(&"AinxtBuildHashline:hashline_read"));
        assert!(ids.contains(&"AinxtBuildHashline:hashline_edit"));
        assert!(ids.contains(&"AinxtBuildHashline:hashline_grep"));
    }
    ...
    fn file_toolset_override_never_grants_edit_to_read_only_toolsets() {
        ...
        for mut def in [
            ainxt_agent::config::AgentDefinition::plan(),
            ainxt_agent::config::AgentDefinition::explore(),
        ] {
            ...
            assert!(
                !def.tool_config
                    .tools
                    .iter()
                    .any(|t| t.id == "AinxtBuild:search_replace"),
            ...
            assert!(
                ids.contains(&"AinxtBuildHashline:hashline_read"),
                "{name}: {ids:?}"
            );
            assert!(!ids.contains(&"AinxtBuild:read_file"), "{name}: {ids:?}");
            // ...but never grants the edit slot.
            assert!(
                !ids.contains(&"AinxtBuildHashline:hashline_edit"),
    ...
    fn max_timeout_secs_defaults_to_production_10h() {
        ...
        assert_eq!(
            ...
            "production ainxt-build must set the 10h foreground ceiling"
        );
    }
