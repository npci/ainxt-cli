...
pub enum ConfigChangeEvent {
    AuthChanged,
    GlobalConfigChanged,
    /// `~/.ainxt/models_cache.json` changed — the on-disk `/v1/models`
    /// catalog cache was rewritten, possibly by **another** ainxt process
    /// sharing the same `~/.ainxt` (the writer may also be this process;
...
/// Watches `~/.ainxt/` for `auth.json`, `config.toml`, and `models_cache.json`
/// changes, plus any extra paths (project `.ainxt/config.toml`, `.mcp.json`,
...
impl ConfigFileWatcher {
    ...
    /// `<cwd>/.ainxt/`. Use [`Self::watch_path`] later to register additional
    ...
    pub fn start(
        ...
        let ainxt_home_buf = ainxt_home.to_path_buf();
        ...
        let mut debouncer = new_filtered_debouncer(debounce, move |res: DebounceEventResult| {
            ...
            for event in events {
                ...
                let change = match name {
                    Some("auth.json") if parent == Some(ainxt_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::AuthChanged)
                    }
                    Some("config.toml") if parent == Some(ainxt_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::GlobalConfigChanged)
                    }
                    Some("models_cache.json") if parent == Some(ainxt_home_buf.as_path()) => {
        ...
        debouncer
            .watcher()
            .watch(ainxt_home, RecursiveMode::NonRecursive)
            .map_err(|e| {
                tracing::warn!(
                    path = %ainxt_home.display(),
                    error = %e,
                    "failed to watch ainxt home directory"
                )
            })
            .ok()?;
        ...
        // includes `<cwd>/.ainxt/config.toml` so the loop above
        // watches `<cwd>/.ainxt/`), the call below installs a
    ...
    /// Register `<cwd>/` and `<cwd>/.ainxt/` as **non-recursive** watch
    ...
    /// `<cwd>/.mcp.json` and `<cwd>/.ainxt/config.toml` trigger a
    ...
    /// `<cwd>/.ainxt/`) previously registered for `cwd` via
...
///
/// **Known limitation:** if `<cwd>/.ainxt/` does not yet
/// exist at session-open time, the `.ainxt/` watch fails ENOENT and is
/// swallowed at `debug!`. A later `mkdir <cwd>/.ainxt/` followed by a
/// write to `<cwd>/.ainxt/config.toml` will NOT be observed — the
...
fn watch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let ainxt_dir = cwd.join(".ainxt");
    if let Err(e) = debouncer
        .watcher()
        .watch(&ainxt_dir, RecursiveMode::NonRecursive)
    {
        log_watch_error(
            &e,
            "failed to watch project .ainxt directory (non-recursive)",
        );
    }
}
...
fn unwatch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let ainxt_dir = cwd.join(".ainxt");
    if let Err(e) = debouncer.watcher().unwatch(&ainxt_dir) {
        tracing::debug!(error = %e, "failed to unwatch project .ainxt directory");
    }
}

/// Log a `notify` watch failure, distinguishing the benign
/// "directory doesn't exist yet" case (logged at `debug!` — it's
/// expected for a freshly-opened session whose `<cwd>/.ainxt/` hasn't
...
/// Watches skill directories (`~/.ainxt/skills/`, `<repo>/.ainxt/skills/`, etc.)
...
/// Matches discovery (`find_skill_paths`/`find_command_paths`).
...
/// recursively: `ainxt_home` (`~/.ainxt`, or `$AINXT_HOME`) or a known vendor dir
/// directly under `$HOME` ([`HOME_VENDOR_DIRS`]).
///
/// These hold large non-skill trees — `~/.ainxt` alone has `worktrees/`,
...
fn is_global_config_dir(dir: &Path, ainxt_home: &Path) -> bool {
    #[allow(deprecated)]
    let home = std::env::home_dir();
    is_global_config_dir_impl(dir, ainxt_home, home.as_deref())
}
...
const HOME_VENDOR_DIRS: &[&str] = &[".ainxt", ".agents", ".claude", ".cursor"];

/// Testable core of [`is_global_config_dir`] with `$HOME` injected.
fn is_global_config_dir_impl(dir: &Path, ainxt_home: &Path, home: Option<&Path>) -> bool {
    let canon = |p: &Path| dunce::canonicalize(p).unwrap_or_else(|_| p.to_path_buf());
    if canon(dir) == canon(ainxt_home) {
        return true;
    }
...
/// `~/.ainxt` these exist at startup; later creation is caught on restart).
...
impl SkillsFileWatcher {
    ...
    ///
    /// Uses [`collect_skill_config_dirs`](ainxt_agent::prompt::skills::collect_skill_config_dirs)
    ...
    pub fn start(
        ...
        let dirs_to_watch = ainxt_agent::prompt::skills::collect_skill_config_dirs(
            cwd,
            monorepo_user_dir,
            &ainxt_home,
            config_paths,
        ...
        for dir in &dirs_to_watch {
            if is_global_config_dir(dir, &ainxt_home) {
...
mod tests {
    ...
    /// `is_global_config_dir` must scope down only ainxt_home and the known
    ...
    /// recursively watched.
    #[test]
    fn is_global_config_dir_matches_only_ainxt_home_and_vendor_dirs() {
        let home = TempDir::new().unwrap();
        let home = home.path();
        let ainxt_home = home.join(".ainxt");

        let g = |dir: &Path| is_global_config_dir_impl(dir, &ainxt_home, Some(home));

        // ainxt_home and vendor dirs directly under $HOME: scoped (global).
        assert!(g(&ainxt_home));
        ...
        assert!(!g(&home.join("repo").join(".ainxt")));
    }

    /// Regression for the ~/.ainxt inotify-exhaustion / worktree-noise bug: a
    /// `SKILL.md` under a sibling subtree (e.g. `~/.ainxt/worktrees/`) must not
    ...
    /// A write to `<ainxt_home>/models_cache.json` must surface as
    /// `ConfigChangeEvent::ModelsCacheChanged` so a long-running leader can
    /// hot-load a catalog fetched by another ainxt process.
    #[test]
    #[cfg_attr(
        target_os = "macos",
        ignore = "flaky on macOS: FSEvents does not reliably deliver events in test harness"
    ...
    /// A write to `<cwd>/.ainxt/config.toml` must surface as
    ...
    fn project_cwd_toml_triggers_reload() {
        let ainxt_home = TempDir::new().unwrap();
        let cwd = TempDir::new().unwrap();
        let project_ainxt = cwd.path().join(".ainxt");
        fs::create_dir_all(&project_ainxt).unwrap();
        // Seed the file before the watcher starts so we observe the
        // modification rather than the creation event.
        fs::write(project_ainxt.join("config.toml"), "").unwrap();
        ...
        fs::write(
            project_ainxt.join("config.toml"),
            "[mcp_servers.x]\ncommand = \"/bin/true\"",
        )
        .unwrap();
        ...
        assert!(
            found,
            "expected ProjectConfigChanged for <cwd>/.ainxt/config.toml within 2s"
        );
    }
    ...
    /// `start` must light up `<new_cwd>/.ainxt/config.toml` writes
    ...
    fn watch_path_dynamic_registration() {
        ...
        let project_ainxt = new_cwd.path().join(".ainxt");
        fs::create_dir_all(&project_ainxt).unwrap();
        fs::write(project_ainxt.join("config.toml"), "").unwrap();
