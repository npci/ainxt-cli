...
use crate::auth::{AinxtAuth, read_auth_json};
...
pub enum ConfigUpdate {
    ...
    ///
    /// 1. The global `[mcp_servers]` table in `~/.ainxt/config.toml`
    ///    changed.
    ...
    ///
    /// Project-scoped changes (`<cwd>/.ainxt/config.toml`,
    ...
    /// (`<cwd>/.ainxt/config.toml`, `<cwd>/.mcp.json`, or
    ...
    /// ainxt process sharing the home dir). The agent should consult the cache
    /// thrash unrelated sessions.
    ProjectMcpServersChanged {
        /// The project root whose `.ainxt/`, `.mcp.json`, or
    ...
    ModelsChanged,
    /// `~/.ainxt/models_cache.json` was rewritten on disk (possibly by another
    ...
    ModelsCacheChanged,
    /// Updated UI settings — agent broadcasts `ainxt.dev/config_changed` to IPC clients.
    Ui {
        theme: Option<String>,
        yolo: bool,
...
impl ConfigReloader {
    ...
    pub async fn run(
        ...
        loop {
            ...
            if has_auth {
                ...
                match result {
                    Ok(Err(e)) => {
                        ...
                        let path = self.ainxt_home.join("auth.json");
            ...
            // same tick (e.g. `~/.ainxt/config.toml` AND
    ...
    fn reload_auth(&mut self) -> anyhow::Result<()> {
        let auth_path = self.ainxt_home.join("auth.json");
    ...
    fn reload_config(&mut self) -> anyhow::Result<()> {
        ...
        // config (`~/.ainxt/config.toml`) via toml::Value. Project-
        // scoped changes (`<cwd>/.ainxt/config.toml`,
...
/// |----------------------------|-------------------------|-------------------|
/// | `ProjectConfigChanged`     | `<cwd>/.ainxt/config.toml` | `<cwd>`           |
...
/// for `.ainxt/config.toml`, `find_mcp_json_files` for `.mcp.json`) so
...
/// (`~/.ainxt/config.toml`, `~/.claude.json`, `~/.cursor/mcp.json`)
...
/// `ConfigUpdate::Skills`) and `ainxt inspect` (via the `crate::config`
...
pub(crate) fn parse_skills_config(
    config: &toml::Value,
) -> ainxt_agent::prompt::skills::SkillsConfig {
    config
        .get("skills")
...
fn parse_compat_config(config: &toml::Value) -> ainxt_tools::types::compat::CompatConfigToml {
    config
        .get("compat")
        .and_then(|v| v.clone().try_into().ok())
        .unwrap_or_default()
...
mod tests {
    ...
    fn make_auth(key: &str) -> AinxtAuth {
        AinxtAuth {
            key: key.to_string(),
    ...
    async fn reloader_detects_new_auth_key() {
        ...
        assert!(
            matches!(update, ConfigUpdate::Auth(a) if a.key == "new-key"), // a is Box<AinxtAuth>, Deref coercion
    ...
    /// The hash must reflect ancestor `.ainxt/config.toml` and `.mcp.json`
    ...
    fn hash_project_mcp_config_covers_ancestors() {
        ...
        std::fs::write(tmp.path().join(".ainxt").join("config.toml"), "x = 1").unwrap();
        let h3 = hash_project_mcp_config(&child).expect("readable");
        assert_ne!(
            h2, h3,
            "ancestor .ainxt/config.toml create must change the hash"
        );
    }
    ...
    fn parse_skills_config_with_paths() {
        let config: toml::Value = toml::from_str(
            r#"
[skills]
paths = ["/home/user/.ainxt/skills"]
ignore = ["/tmp"]
"#,
        )
        .unwrap();
        let skills = parse_skills_config(&config);
        assert_eq!(skills.paths, vec!["/home/user/.ainxt/skills".to_string()]);
...
theme = "dark"
yolo = true
fork_secondary_model = "ainxt-4.5"
"#,
        )
        .unwrap();
        let (theme, yolo, fork) = extract_ui_fields(&config);
        ...
        assert!(yolo);
        assert_eq!(fork.as_deref(), Some("ainxt-4.5"));
    }
...
"#,
    ...
    /// `ProjectConfigChanged` (`<cwd>/.ainxt/config.toml`) and
    ...
    fn collect_project_cwds_excludes_home_claude_json() {
        let batch = vec![
            ...
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/x/.ainxt/config.toml"),
            },
        ];
    ...
    fn collect_project_cwds_dedupes_and_extracts() {
        let batch = vec![
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/a/.ainxt/config.toml"),
            },
            ConfigChangeEvent::McpConfigChanged {
                path: PathBuf::from("/repo/a/.mcp.json"),
            },
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/b/.ainxt/config.toml"),
            },
        ];
        let cwds = collect_project_cwds(&batch);
        assert_eq!(
