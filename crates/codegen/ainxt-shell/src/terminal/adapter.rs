//! AcpTerminalAdapter: implements `ainxt-tools::TerminalBackend` using ACP gateway calls.
//!
//! This adapter enables bash tool execution over ACP (remote execution).
//! It translates ainxt-tools' `TerminalBackend` trait into ACP protocol calls:
...
fn wrap_command(command: &str) -> Result<String, ComputerError> {
    // On Windows the ACP client (ainxt-desktop) spawns with `shell: true`
...
/// Wraps ainxt-shell's ACP gateway to satisfy ainxt-tools' TerminalBackend.
