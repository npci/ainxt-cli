...
/// after we stop waiting. Matches the bound the `ainxt-tools` local terminal
...
fn notification_interval() -> Duration {
    std::env::var("AINXT_TERMINAL_NOTIFICATION_INTERVAL_MS")
        .ok()
...
#[async_trait::async_trait]
impl SessionNotificationSender for ainxt_acp_lib::AcpAgentGatewaySender {
    async fn session_notification(
        &self,
        notification: acp::SessionNotification,
    ) -> Result<(), acp::Error> {
...
pub async fn create_terminal(
    ...
    // Non-empty args → spawn program directly (argv preserved unchanged).
