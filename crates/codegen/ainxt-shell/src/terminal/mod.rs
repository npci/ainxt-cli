...
/// Resolved absolute path to bash. On Unix uses the `ainxt_config` shell
/// resolution cascade (`$AINXT_SHELL` > `$SHELL` > `which` > common dirs >
...
pub fn default_shell_path() -> &'static str {
    #[cfg(unix)]
    {
        ainxt_config::shell::unix_shell_path(ainxt_config::shell::UnixShellKind::Bash)
    }
    #[cfg(not(unix))]
    {
        "/bin/bash"
...
/// Delegates to the canonical implementation in `ainxt-tools`.
pub use ainxt_tools::util::pager_env;
...
/// tool output. Used when the client sets `ainxt.dev/bashOutputNoColor: true` (e.g.
/// ainxt-pager which renders its own UI and doesn't need raw ANSI codes).
///
