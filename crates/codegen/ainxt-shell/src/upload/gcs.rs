...
//! `StorageClient` constructed inside `ainxt_file_utils::gcs::*` helpers.
//!
...
//! to an `ainxt_file_utils::gcs::*` helper.
...
/// shell-side adapter that lets `ainxt_file_utils::gcs::*` helpers wire
...
///
/// ```ignore
/// ainxt_file_utils::gcs::upload_bytes(
...
/// `AINXT_TELEMETRY_GCS_BUCKET`; `None` disables trace uploads until a bucket
/// is configured.
pub(crate) const SESSION_TRACES_BUCKET: Option<&str> =
    option_env!("AINXT_SESSION_TRACES_BUCKET_DEFAULT");
...
pub(crate) async fn upload_to_auth_diagnostics(
    ...
    match ainxt_file_utils::gcs::upload_bytes(
