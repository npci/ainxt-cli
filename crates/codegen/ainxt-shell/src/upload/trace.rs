...
use url::Url;
use ainxt_file_utils::queue::{EnqueueOutcome, TraceExportSource, UploadQueue, UploadRetryPolicy};
use ainxt_workspace::permission::PermissionEvent;
...
pub(crate) fn spawn_trace_upload<T: Serialize + Send + 'static>(
    ...
    tokio::spawn(async move {
        ...
        let ok = ainxt_file_utils::gcs::upload_bytes(
            &gcs_config,
            &object_path,
            &bytes,
            "application/json",
...
fn record_upload_failure(ctx: &PromptTraceContext, f: UploadFailure<'_>) {
    ...
    macro_rules! log_failure {
        ($level:ident) => {
            ...
            .status_code, bytes = ? f.bytes, session_tag = %crate::util::session_log_tag(ctx.session_info.id.0.as_ref()),
    ...
    if level == UploadFailureLogLevel::Warn {
        ainxt_telemetry::unified_log::warn(&msg, sid, log_ctx);
    } else {
        ainxt_telemetry::unified_log::error(&msg, sid, log_ctx);
    }
}
...
pub(crate) fn local_sandbox_telemetry() -> Option<LocalSandboxTelemetry> {
    let profile = ainxt_sandbox::configured_profile_name()?;
    Some(LocalSandboxTelemetry {
        profile: profile.to_owned(),
        applied: ainxt_sandbox::is_active(),
    })
}
...
fn classify_workspace(cwd: &str) -> String {
    ...
    if path.ancestors().any(|p| p.join(".git").exists()) {
        "git".to_owned()
    } else if ainxt_file_utils::workspace_classifier::is_project_dir(path) {
        "project".to_owned()
    } else {
...
pub(crate) async fn upload_metadata(ctx: &PromptTraceContext, metadata: PromptMetadata) {
    ...
    let metadata_json = match serde_json::to_vec_pretty(&metadata) {
        ...
        Err(e) => {
            tracing::warn!(
                session_tag = %crate::util::session_log_tag(ctx.session_info.id.0.as_ref()), turn_number = ctx.turn_number,
...
pub(crate) async fn upload_subagent_metadata(
    ...
    if let Err(e) =
        ainxt_file_utils::gcs::upload_bytes(&config, &gcs_path, &json, "application/json").await
    {
        tracing::warn!(
...
pub(crate) async fn upload_images(
    ...
    tracing::info!(
        session_tag = %crate::util::session_log_tag(ctx.session_info.id.0.as_ref()), turn_number = ctx.turn_number, image_count,
...
pub(crate) async fn upload_plugin_state(
    ctx: &PromptTraceContext,
    registry: Option<&ainxt_agent::plugins::PluginRegistry>,
) {
    use ainxt_agent::plugins::discovery::PluginScope;
...
use ainxt_file_utils::gcs::upload_bytes;
...
pub(crate) async fn upload_artifact_to_gcs(
    ...
    match upload_bytes(&config, gcs_path, content, content_type).await {
        ...
        Err(e) => {
            let status_code = e
                .downcast_ref::<ainxt_file_utils::storage_client::HttpUploadError>()
                .map(|e| e.status_code);
...
pub(crate) async fn upload_unified_log(ctx: &PromptTraceContext, wait: UploadWait) {
    ...
    let log_bytes = match tokio::task::spawn_blocking(move || {
        ainxt_telemetry::unified_log::snapshot_session_log(&session_id)
    })
    .await
    {
        Ok(Some(bytes)) => bytes,
    ...
    .await;
    let full_log_bytes =
        tokio::task::spawn_blocking(ainxt_telemetry::unified_log::snapshot_log).await;
    let user_id = ctx
        .auth_manager
        .current_or_expired()
        .map(|a| a.user_id)
...
pub(crate) async fn upload_turn_messages(
    ctx: &PromptTraceContext,
    _capture: ainxt_chat_state::TurnCapture,
    _wait: UploadWait,
) -> bool {
    super::manifest::skip_artifact(
        &ctx.artifact_tracker,
...
pub(crate) fn build_chat_history_session_state(
    _messages: &[ainxt_sampling_types::conversation::ConversationItem],
...
impl TraceExportSource for DynamicResolver {
    ...
    fn proxy_attribution(
        &self,
    ) -> Option<Arc<dyn ainxt_file_utils::storage_client::Auth401AttributionCallback>> {
        ainxt_file_utils::gcs::StorageConfig::proxy_attribution(&self.with_auth())
    }
    fn proxy_credentials(&self) -> Option<Arc<dyn ainxt_auth::AuthCredentialProvider>> {
        ainxt_file_utils::gcs::StorageConfig::proxy_credentials(&self.with_auth())
    }
    fn proxy_http_client(&self) -> Option<reqwest::Client> {
        ainxt_file_utils::gcs::StorageConfig::proxy_http_client(&self.with_auth())
    }
...
pub(crate) fn spawn_startup_spill_reconcile(
    ...
    tokio::spawn(async move {
        match queue {
            Some(queue) => {
                let report =
                    ainxt_workspace::recovery::run_startup_recovery(&ainxt_home, &queue).await;
                tracing::info!(?report, "startup spill recovery complete");
                queue.cleanup_orphans(ainxt_file_utils::queue::DEFAULT_MAX_AGE);
            }
            None => {
                let purged = tokio::task::spawn_blocking(move || {
                    ainxt_workspace::recovery::purge_spilled_items(&ainxt_home)
                })
                .await;
                match purged {
                    Ok(removed) => {
...
pub(crate) fn spawn_purge_stale_upload_scratch() {
    {
        ...
        let dir = crate::util::ainxt_home::ainxt_home()
            .join("upload_queue")
            .join("scratch");
...
pub(crate) fn spawn_upload_queue(
    ...
    let queue = UploadQueue::spawn(ainxt_home, resolver, UploadRetryPolicy::default());
...
pub(crate) async fn upload_trace_artifact_blocking(
    ...
    let queue_result = if let Some(queue) = &ctx.upload_queue {
            ...
            .await
        {
            ...
            Err(e)
                if e.downcast_ref::<ainxt_file_utils::queue::QueueClosed>()
                    .is_some() =>
            {
                tracing::debug!(
...
mod tests {
    ...
    fn dynamic_resolver_refreshes_proxy_token() {
        ...
        let ainxt_com_config = AinxtComConfig::default();
        let scope = ainxt_com_config.auth_scope();
        let initial_auth = AinxtAuth {
    ...
    async fn resolve_async_falls_back_when_no_refresher() {
        ...
        let auth_json = serde_json::to_string_pretty(&store).unwrap();
        std::fs::write(dir.path().join("auth.json"), &auth_json).unwrap();
        let auth_manager = Arc::new(crate::auth::AuthManager::new(dir.path(), ainxt_com_config));
        let resolver = DynamicResolver {
            auth_manager,
            base_config: TraceExportConfig {
                bucket_url: None,
    ...
    async fn resolve_async_drives_refresh_chain_when_token_expired() {
        ...
        impl crate::auth::refresh::TokenRefresher for FreshRefresher {
            async fn refresh(
                ...
                crate::auth::refresh::RefreshOutcome::Success(Box::new(crate::auth::AinxtAuth {
    ...
    async fn proactive_refresh_makes_trace_resolve_a_cache_hit() {
        ...
        let ainxt_com_config = AinxtComConfig::default();
        let auth_manager = Arc::new(crate::auth::AuthManager::new(dir.path(), ainxt_com_config));
        auth_manager.hot_swap(AinxtAuth {
            key: "expired-oidc".into(),
    ...
    fn dynamic_resolver_preserves_token_when_auth_unavailable() {
        use crate::session::repo_changes::UploadMethod;
        let dir = tempfile::tempdir().unwrap();
        let ainxt_com_config = crate::auth::AinxtComConfig::default();
        let auth_manager = Arc::new(crate::auth::AuthManager::new(dir.path(), ainxt_com_config));
        let base_config = TraceExportConfig {
            bucket_url: None,
            service_account_key: None,
    ...
    fn dynamic_resolver_supplies_proxy_credentials_and_attribution() {
        use crate::session::repo_changes::UploadMethod;
        use ainxt_file_utils::queue::TraceExportSource;
        let dir = tempfile::tempdir().unwrap();
        let auth_manager = Arc::new(crate::auth::AuthManager::new(
            dir.path(),
            crate::auth::AinxtComConfig::default(),
        ));
        let base_config = TraceExportConfig {
            bucket_url: None,
            service_account_key: None,
    ...
    async fn dynamic_resolver_auth_recovery_wakes_on_already_rotated_token() {
        ...
        auth_manager.hot_swap(crate::auth::AinxtAuth {
    ...
    /// entry. Matches how the trace viewer extracts `chat_history.jsonl`.
    ...
    fn home_project_dir() -> Option<tempfile::TempDir> {
        ...
        if !ainxt_file_utils::workspace_classifier::is_project_dir(&home.join("probe")) {
            return None;
        }
