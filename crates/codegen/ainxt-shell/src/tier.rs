...
//! server zero-limits on the image and voice endpoints. Everything else
//! (SuperAinxt, SuperAinxt Heavy/Lite, X Premium/+, and any unknown future name)
...
mod tests {
    ...
    fn unrestricted_names() {
        assert!(!is_restricted_tier_name("SuperAinxt"));
        assert!(!is_restricted_tier_name("SuperAinxt Heavy"));
        assert!(!is_restricted_tier_name("superainxt_lite"));
