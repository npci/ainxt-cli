//! Data APIs for `ainxt models`. Clients own display.
...
/// Status for the `ainxt models` banner (display order ≠ sampling priority; see [`AuthStatus::resolve`]).
#[derive(Debug, PartialEq, Eq)]
pub enum AuthStatus {
    ApiKey,
    /// Auth host from `ainxt_ws_origin` (scheme stripped).
    LoggedIn(String),
...
impl AuthStatus {
    ...
    /// [`crate::agent::auth_method::should_advertise_ainxt_api_key`] so
    ...
    pub fn resolve(agent_config: &AgentConfig) -> Self {
        ...
        if agent_config.create_auth_manager().current().is_some() {
            let origin = &agent_config.ainxt_com_config.ainxt_ws_origin;
            let host = origin
                .strip_prefix("https://")
        ...
        if crate::agent::auth_method::should_advertise_ainxt_api_key(
            agent_config.ainxt_com_config.api_key_auth_disabled(),
            models.values(),
...
mod tests {
    ...
    ///
    /// Uses `AINXT_AUTH_PATH` (not `AINXT_HOME`) so a OnceLock-cached real home
    ...
    fn isolate_auth_sources() -> (tempfile::TempDir, [EnvGuard; 7]) {
        ...
        let guards = [
            EnvGuard::unset(AINXT_API_KEY_ENV_VAR),
            EnvGuard::unset(LEGACY_AINXT_API_KEY_ENV_VAR),
            EnvGuard::unset("AINXT_AUTH"),
            EnvGuard::set("AINXT_AUTH_PATH", auth_path.to_str().unwrap()),
            EnvGuard::unset("AINXT_DEPLOYMENT_KEY"),
            EnvGuard::unset("AINXT_WS_ORIGIN"),
            EnvGuard::unset("AINXT_DISABLE_API_KEY_AUTH"),
        ];
        (dir, guards)
    }
    ...
    fn resolve_api_key_env() {
        ...
        let _key = EnvGuard::set(AINXT_API_KEY_ENV_VAR, "ainxt-test-key");
    ...
    fn resolve_legacy_api_key_env() {
        let (_dir, _g) = isolate_auth_sources();
        let _key = EnvGuard::set(LEGACY_AINXT_API_KEY_ENV_VAR, "legacy-key");
        assert_eq!(AuthStatus::resolve(&Config::default()), AuthStatus::ApiKey);
    }
    ...
    fn resolve_oauth_session() {
        ...
        let _auth = EnvGuard::set("AINXT_AUTH", &json);
        // isolate_auth_sources() unsets AINXT_WS_ORIGIN for isolation; this
        // test is specifically about the LoggedIn(host) display, so it needs
        // its own value re-set after that (AinxtComConfig::default() falls
        // back to the deliberately-empty PROD_WS_ORIGIN otherwise -- audit
        // risk R42, see ainxt-env/src/lib.rs).
        let _ws_origin = EnvGuard::set("AINXT_WS_ORIGIN", "https://ainxt.dev");

        assert_eq!(
            AuthStatus::resolve(&Config::default()),
            AuthStatus::LoggedIn("ainxt.dev".to_owned())
        );
    }
