//! Built-in files extracted to `~/.ainxt/` on startup.
...
const IMAGE_SKILL_MD: &str = include_str!("../skills/image/SKILL.md");
...
///
/// These directories under `~/.ainxt/skills/` will be deleted on startup
...
pub(crate) fn is_extracted_bundled_skill(
    ...
    BUNDLED_SKILLS.iter().any(|&(n, _)| n == name)
        && path == ainxt_home.join("skills").join(name).join("SKILL.md")
}

/// Resolve the content for a skill, applying any name-specific transforms.
fn resolve_skill_content(name: &str, raw: &str, ainxt_home: &std::path::Path) -> String {
    match name {
        // Help skill needs path substitution so absolute paths work.
        "help" => {
            let ainxt_home_str = format!("{}/", ainxt_home.to_string_lossy());
            raw.replace("~/.ainxt/", &ainxt_home_str)
        }
        _ => raw.to_string(),
    }
}

/// Extract bundled files to `~/.ainxt/` on startup.
///
...
pub fn extract_bundled_files(ainxt_home: &std::path::Path) {
    ...
    remove_legacy_bundled_skills(ainxt_home);

    let version = ainxt_version::VERSION;
    let marker = ainxt_home.join(".metadata_version");
        ...
        && existing.trim() == version
    {
        ...
        extract_missing_skills(ainxt_home);
        return;
    }
    ...
    for stale in &["CHANGELOG.json", "CHANGELOG.md"] {
        let _ = std::fs::remove_file(ainxt_home.join(stale));
    }

    for &(filename, content) in BUNDLED_FILES {
        if let Err(e) = std::fs::write(ainxt_home.join(filename), content) {
    ...
    for &(name, raw) in BUNDLED_SKILLS {
        let skill_dir = ainxt_home.join("skills").join(name);
        let _ = std::fs::create_dir_all(&skill_dir);
        let content = resolve_skill_content(name, raw, ainxt_home);
...
fn extract_missing_skills(ainxt_home: &std::path::Path) {
    for &(name, raw) in BUNDLED_SKILLS {
        let skill_md = ainxt_home.join("skills").join(name).join("SKILL.md");
        if skill_md.exists() {
            continue;
        }
...
fn remove_legacy_bundled_skills(ainxt_home: &std::path::Path) {
    remove_legacy_skills(ainxt_home, LEGACY_BUNDLED_SKILL_NAMES, BUNDLED_SKILLS);
}
...
fn remove_legacy_skills(
    ...
    for name in legacy_names {
        ...
        let dir = ainxt_home.join("skills").join(name);
        if dir.exists() {
...
mod tests {
    ...
    fn version_bump_re_extracts_all_files() {
        ...
        for name in ["check-work", "image", "code-review"] {
    ...
    async fn help_skill_discovered_by_skill_pipeline() {
        ...
        std::fs::create_dir_all(workspace.join(".ainxt").join("skills").join("help")).unwrap();
        std::fs::copy(
            home.join("skills/help/SKILL.md"),
            workspace.join(".ainxt/skills/help/SKILL.md"),
        )
        .unwrap();
