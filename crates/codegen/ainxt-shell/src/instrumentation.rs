//! Shim — see `ainxt_telemetry::instrumentation` for the implementation.
//!
...
//!   (i.e. `ainxt_shell::instrumentation_timer!`). Keeping the macro here
...
pub use ainxt_telemetry::instrumentation::{
...
///
/// Stays in shell so callers can keep calling `ainxt_shell::instrumentation::finalize_and_exit`.
...
///
/// Macro stays in shell so `$crate` continues to resolve to `ainxt_shell`
/// for the 12+ existing call sites that spell it as
/// `crate::instrumentation_timer!(...)` or `ainxt_shell::instrumentation_timer!(...)`.
/// The macro body delegates to types and functions in
/// `ainxt_telemetry::instrumentation`.
#[macro_export]
