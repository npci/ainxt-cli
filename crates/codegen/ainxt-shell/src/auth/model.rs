...
mod tests {
    ...
    fn is_ainxt_auth_matrix() {
        let issuer = crate::auth::config::ainxt_oauth2_issuer();
        ...
        assert!(with_issuer(AuthMode::Oidc, Some(issuer)).is_ainxt_auth());
        assert!(with_issuer(AuthMode::External, Some(issuer)).is_ainxt_auth());
        ...
        assert!(!with_issuer(AuthMode::ApiKey, Some(issuer)).is_ainxt_auth());
        assert!(!with_issuer(AuthMode::WebLogin, Some(issuer)).is_ainxt_auth());
    }
    ...
    fn is_session_auth_requires_first_party_for_external() {
        ...
        assert!(with_issuer(AuthMode::External, Some(issuer)).is_session_auth());
        ...
        // Plain API keys never do.
        assert!(!with_issuer(AuthMode::ApiKey, Some(issuer)).is_session_auth());
    }
