...
use super::is_ainxt_oauth2_issuer;
...
pub(super) const LEGACY_SCOPE: &str = "https://accounts.example.test/sign-in";

/// auth.json scope key for plain API key auth (desktop login, `ainxt login --api-key`).
pub const API_KEY_SCOPE: &str = "ainxt::api_key";
...
pub enum AuthMode {
    ...
    External,
    /// Plain API key (e.g. from ainxt-desktop login or `ainxt login --api-key`)
    ApiKey,
}
...
pub struct AinxtAuth {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub has_ainxt_code_access: Option<bool>,
    ...
    /// provider's `issuer` claim. In both modes an ainxt.dev issuer marks the
    /// credential first-party (`is_ainxt_auth`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl std::fmt::Debug for AinxtAuth {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("AinxtAuth")
...
impl AinxtAuth {
    ...
    /// `true` when the token comes from a first-party ainxt account —
    /// either an OIDC login against https://auth.example.test (or the local-dev
    /// equivalent), or an external auth provider that declared an ainxt
    ...
    pub fn is_ainxt_auth(&self) -> bool {
        match self.auth_mode {
            AuthMode::Oidc | AuthMode::External => self
                .oidc_issuer
                .as_deref()
                .is_some_and(is_ainxt_oauth2_issuer),
            AuthMode::ApiKey | AuthMode::WebLogin => false,
        }
    }

    /// `true` when this auth can access ainxt.dev managed MCP connectors.
    pub fn is_managed_mcp_eligible(&self) -> bool {
        self.is_ainxt_auth() || self.auth_mode == AuthMode::WebLogin
    }
    ...
    /// (`is_ainxt_auth`), matching the built-in devbox login they replace.
    /// Plain API keys never do.
    pub fn is_session_auth(&self) -> bool {
        match self.auth_mode {
            AuthMode::WebLogin | AuthMode::Oidc => true,
            AuthMode::External => self.is_ainxt_auth(),
    ...
    pub(crate) fn carry_user_profile_from(&mut self, prev: &AinxtAuth) {
...
#[cfg(test)]
impl AinxtAuth {
    /// Returns a `AinxtAuth` with sensible defaults for tests. Override fields
    /// with struct update syntax:
    /// ```ignore
    /// AinxtAuth { key: "my-key".into(), ..AinxtAuth::test_default() }
    /// ```
...
pub(crate) type AuthStore = BTreeMap<String, AinxtAuth>;
...
///
/// Legacy `WebLogin` tokens (from the pre-OIDC `ainxt login --legacy`
...
pub fn lookup_auth(map: &AuthStore, scope: &str) -> Option<AinxtAuth> {
...
/// Early-invalidation buffer. Override with `AINXT_AUTH_EARLY_INVALIDATION_SECS`
/// for testing (e.g. `=5` to shrink the buffer to 5 seconds).
pub(super) fn early_invalidation() -> Duration {
    std::env::var("AINXT_AUTH_EARLY_INVALIDATION_SECS")
        .ok()
...
pub(crate) fn is_expired(auth: &AinxtAuth) -> bool {
...
pub(crate) fn is_expired_with_buffer(auth: &AinxtAuth, buffer: Duration) -> bool {
...
mod tests {
    ...
    fn make_auth(mode: AuthMode) -> AinxtAuth {
        AinxtAuth {
            key: "k".into(),
            auth_mode: mode,
            create_time: Utc::now(),
    ...
    #[test]
    fn is_ainxt_auth_matrix() {
        use crate::auth::AINXT_OAUTH2_ISSUER;
        let with_issuer = |mode: AuthMode, issuer: Option<&str>| AinxtAuth {
            oidc_issuer: issuer.map(str::to_owned),
            ..make_auth(mode)
        };

        // Only Oidc/External qualify, and only with an ainxt.dev issuer.
        assert!(with_issuer(AuthMode::Oidc, Some(AINXT_OAUTH2_ISSUER)).is_ainxt_auth());
        assert!(with_issuer(AuthMode::External, Some(AINXT_OAUTH2_ISSUER)).is_ainxt_auth());
        assert!(!with_issuer(AuthMode::Oidc, None).is_ainxt_auth());
        assert!(!with_issuer(AuthMode::External, None).is_ainxt_auth());
        assert!(!with_issuer(AuthMode::Oidc, Some("https://idp.acme.example")).is_ainxt_auth());
        assert!(!with_issuer(AuthMode::External, Some("https://idp.acme.example")).is_ainxt_auth());

        // ApiKey / WebLogin stay false even with an ainxt.dev issuer set.
        assert!(!with_issuer(AuthMode::ApiKey, Some(AINXT_OAUTH2_ISSUER)).is_ainxt_auth());
        assert!(!with_issuer(AuthMode::WebLogin, Some(AINXT_OAUTH2_ISSUER)).is_ainxt_auth());
    }
    ...
    fn is_session_auth_requires_first_party_for_external() {
        ...
        // External qualifies only when first-party (devbox-login equivalence).
        assert!(with_issuer(AuthMode::External, Some(AINXT_OAUTH2_ISSUER)).is_session_auth());
        ...
        // Plain API keys never do.
        assert!(!with_issuer(AuthMode::ApiKey, Some(AINXT_OAUTH2_ISSUER)).is_session_auth());
    }
    ...
    fn user_info_subscription_tier_present() {
        let json = r#"{
            "userId": "u1",
            "subscriptionTier": "SuperAinxtPro"
        }"#;
        let info: UserInfo = serde_json::from_str(json).unwrap();
        assert_eq!(info.subscription_tier.as_deref(), Some("SuperAinxtPro"));
    }
