...
#[non_exhaustive]
pub enum AuthError {
    #[error("Not logged in. Run `ainxt login`.")]
    NotLoggedIn,

    /// Token expired and no refresh authority available.
    #[error("Token expired. Run `ainxt login` to re-authenticate.")]
    TokenExpiredNoRefresh,

    /// Server rejected the token (401) with no recovery path.
    #[error("Authentication rejected by server. Run `ainxt login` to re-authenticate.")]
    ...
    #[error("{message} Run `ainxt login` to sign in with the required team.")]
    PinnedTeamMismatch { message: String },

    /// Cached API-key session rejected because API-key auth is disabled.
    #[error("API-key auth is disabled by your administrator. Run `ainxt login` to authenticate.")]
    ApiKeyAuthDisabled,
...
/// classify a permanent failure as retryable (matches the dedicated
    ...
    pub(crate) fn user_message(self) -> &'static str {
        match self {
            Self::RefreshTokenRejected => {
                "Your session has expired. Run `ainxt login` to sign in again."
            }
            Self::ClientRejected => {
                "Authentication is temporarily unavailable. Run `ainxt login` if this persists."
            }
            Self::Other => {
                "Authentication could not be refreshed. Run `ainxt login` to sign in again."
            }
        }
    }
}
