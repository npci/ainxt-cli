...
use crate::auth::config::AinxtComConfig;
use crate::auth::error::AuthError;
use crate::auth::token_type::TokenType;
use ainxt_telemetry::events::ManualAuthSurface;

#[cfg(test)]
use super::model::UserInfo;
use super::model::{
    AuthMode, AinxtAuth, early_invalidation, is_expired, is_expired_with_buffer, lookup_auth,
    token_suffix,
};
...
pub struct AuthManager {
    ...
    inner: Arc<RwLock<Option<AinxtAuth>>>,
    path: PathBuf,
    scope: String,
    ...
    power_listener: parking_lot::Mutex<Option<ainxt_system_power::SystemPowerListener>>,
...
enum LockOutcome {
    Held(AuthFileLock),
    Adopted(Box<AinxtAuth>),
}

// ── Construction + builders ──────────────────────────────────────────

impl AuthManager {
    pub fn new(ainxt_home: &Path, ainxt_com_config: AinxtComConfig) -> Self {
        let scope = ainxt_com_config.auth_scope();
        let proxy_base_url =
        ...
        ainxt_telemetry::unified_log::info(
            ...
            Some(serde_json::json!({
                "scope": &scope,
                "ainxt_home": ainxt_home.display().to_string(),
                "HOME": std::env::var("HOME").unwrap_or_else(|_| "(unset)".into()),
                "AINXT_HOME": std::env::var("AINXT_HOME").unwrap_or_else(|_| "(unset)".into()),
                "AINXT_AUTH_PATH": std::env::var("AINXT_AUTH_PATH").unwrap_or_else(|_| "(unset)".into()),
                "AINXT_AUTH": std::env::var("AINXT_AUTH").map(|_| "(set)".to_string()).unwrap_or_else(|_| "(unset)".into()),
            })),
        );

        // AINXT_AUTH: inline JSON credentials (highest priority, read-only).
        if let Ok(inline_json) = std::env::var("AINXT_AUTH") {
            if let Ok(auth) = serde_json::from_str::<AinxtAuth>(&inline_json) {
            ...
            tracing::warn!("AINXT_AUTH set but failed to parse as JSON, falling back to file");
        }

        // AINXT_AUTH_PATH: custom file path (overrides default $AINXT_HOME/auth.json).
        let path = std::env::var("AINXT_AUTH_PATH")
            .map(PathBuf::from)
            .unwrap_or_else(|_| ainxt_home.join("auth.json"));
    ...
    /// else allowed.
    pub(crate) fn cached_token_policy_error(&self, auth: &AinxtAuth) -> Option<AuthError> {
        ...
        let policy = crate::auth::oidc::login_principal_policy(&self.ainxt_com_config)?;
    ...
    fn vet_cached(&self, auth: AinxtAuth) -> Option<AinxtAuth> {
    ...
    pub(crate) fn current(&self) -> Option<AinxtAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    #[inline]
    pub(crate) fn with_inner_write<R>(&self, f: impl FnOnce(&mut Option<AinxtAuth>) -> R) -> R {
    ...
    #[inline]
    pub(crate) fn with_inner_read<R>(&self, f: impl FnOnce(Option<&AinxtAuth>) -> R) -> R {
    ...
    pub(crate) fn current_or_expired(&self) -> Option<AinxtAuth> {
    ...
    pub(crate) fn current_wire_valid(&self) -> Option<AinxtAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    pub(crate) fn expired_auth(&self) -> Option<AinxtAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    fn is_token_expired(&self, auth: &AinxtAuth) -> bool {
    ...
    fn is_token_hard_expired(&self, auth: &AinxtAuth) -> bool {
        self.token_expired_with_buffer(auth, Duration::zero())
    }

    fn token_expired_with_buffer(&self, auth: &AinxtAuth, buffer: Duration) -> bool {
        if auth.expires_at.is_some() {
            return is_expired_with_buffer(auth, buffer);
        }
        if auth.auth_mode == AuthMode::External
            && let Some(ttl) = self.ainxt_com_config.auth_token_ttl
        {
    ...
    ///
    /// Returns the input `AinxtAuth` BEFORE enrichment lands; callers
    /// needing the post-enrichment view re-read `current()`.
    pub(crate) async fn update(self: &Arc<Self>, auth: AinxtAuth) -> std::io::Result<AinxtAuth> {
        ...
        match &write_result {
            Ok(()) => ainxt_telemetry::unified_log::info(
            ...
            Err(e) => ainxt_telemetry::unified_log::error(
    ...
    fn spawn_user_info_enrichment(self: &Arc<Self>, auth: AinxtAuth) {
        enrichment::spawn(Arc::clone(self), auth);
    }

    /// Blocking `/user` enrichment for login flows that exit before the background task lands.
    pub(crate) async fn enrich_auth_inline(&self, auth: &mut AinxtAuth) {
        enrichment::enrich_inline(self, auth).await;
    }

    pub(crate) fn ainxt_com_config(&self) -> &AinxtComConfig {
        &self.ainxt_com_config
    }
    ...
    pub(crate) fn run_external_refresh_command(&self, command: &str) -> Option<AinxtAuth> {
    ...
    pub(crate) fn hot_swap(&self, new_auth: AinxtAuth) {
    ...
    fn try_adopt_disk_token(&self, reason: RefreshReason, msg: &str) -> Option<AinxtAuth> {
    ...
    fn inner_auth_or_external_default(&self) -> AinxtAuth {
        self.inner.read().clone().unwrap_or_else(|| AinxtAuth {
    ...
    #[cfg(test)]
    fn persist_and_swap(&self, auth: AinxtAuth) -> Option<AinxtAuth> {
    ...
    pub(crate) fn read_disk_auth(&self) -> Option<AinxtAuth> {
    ...
    fn read_disk_auth_silent(&self) -> Option<AinxtAuth> {
    ...
    /// matches the `current_or_expired()` bearer the senders put on the wire; a
    ...
    pub(crate) fn read_disk_auth_with_state(&self) -> (Option<AinxtAuth>, DiskAuthState) {
    ...
    fn observe_disk_state(
        ...
        match new_state {
            // Recovery (or first observation in AINXT_AUTH mode).
            DiskAuthState::Ok => {
                ainxt_telemetry::unified_log::info(
    ...
    pub async fn auth(self: &Arc<Self>) -> Result<AinxtAuth, AuthError> {
    ...
    async fn auth_dispatch(self: &Arc<Self>) -> Result<AinxtAuth, AuthError> {
        // Snapshot inner ONCE for dispatch atomicity (closes a TOCTOU
        // where a concurrent `clear()` raced `token_type()` + `inner.read()`).
        let snapshot: Option<AinxtAuth> = self.with_inner_read(|inner| inner.cloned());
        ...
        if let Some(err) = self.permanent_failure() {
            ...
            if !self.ainxt_com_config.blocks_automatic_oidc()
        ...
        let result = match token_type {
            ...
            TokenType::LegacySession => {
                ...
                // assumption that a sibling process (`ainxt login` from
        ...
        // preferred_method=api_key forbids automatic OIDC mint.
        if result.is_err()
            && !self.ainxt_com_config.blocks_automatic_oidc()
            && self.is_devbox_environment()
            && let Ok(auth) = self.try_devbox_recovery().await
        {
            return Ok(auth);
    ...
    pub(crate) async fn try_devbox_recovery(self: &Arc<Self>) -> Result<AinxtAuth, AuthError> {
        if self.ainxt_com_config.blocks_automatic_oidc() {
            tracing::debug!(
        ...
        ainxt_telemetry::unified_log::info("auth: devbox recovery starting", None, None);
    ...
    pub(crate) async fn refresh_chain(
        ...
        //    traffic. Matches `auth()` so callers routing through
    ...
    fn check_refresh_deferral(&self, reason: RefreshReason) -> Result<(), AuthError> {
        ...
        // Dark wake (see `ainxt_system_power::PowerState` for the canonical
    ...
    pub(crate) fn is_different_token(&self, candidate: &AinxtAuth) -> bool {
        let current_key = self.inner.read().as_ref().map(|a| a.key.clone());
    ...
    #[cfg(test)]
    pub(crate) fn manual_auth_last_emit(&self) -> Option<ainxt_telemetry::events::ManualAuth> {
...
pub(crate) fn compute_proactive_sleep(this: &AuthManager) -> StdDuration {
    ...
    match this.inner.read().as_ref().and_then(|a| a.expires_at) {
        ...
        // `[ainxt_com] auth_token_ttl` to drive a real schedule.
...
///
/// Static fallthrough (`AINXT_API_KEY` / `auth.json` `ainxt::api_key`) makes voice
...
impl ainxt_tools::types::ApiKeyProvider for SharedAuthKeyProvider {
...
fn prefers_static_api_key(am: &AuthManager) -> bool {
    matches!(
        am.ainxt_com_config.preferred_method,
        Some(super::config::PreferredAuthMethod::ApiKey)
    )
}

/// Env → `auth.json` `ainxt::api_key`. Off under kill-switch or `preferred_method = oidc`.
fn resolve_static_api_key(am: &AuthManager) -> Option<String> {
    if am.ainxt_com_config.api_key_auth_disabled() {
        return None;
    }
    if matches!(
        am.ainxt_com_config.preferred_method,
    ...
    non_empty_key(crate::agent::auth_method::read_ainxt_api_key_env().ok())
...
impl AuthManager {
    /// `ainxt::api_key` from this manager's auth file, memoized on
...
) -> ainxt_tools::types::SharedApiKeyProvider {
