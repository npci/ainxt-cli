...
use crate::auth::model::AinxtAuth;
use crate::auth::token_type::TokenType;
use ainxt_telemetry::events::{AuthTokenKind, ManualAuth, ManualAuthReason, ManualAuthSurface};
...
impl RejectedAuth {
    pub(crate) fn capture(auth: Option<&AinxtAuth>) -> Self {
        Self {
...
impl ManualAuthTracker {
    ...
    pub(crate) fn record(
        ...
        ainxt_telemetry::session_ctx::log_event(event);
    }
...
impl UnauthorizedRecovery {
    ...
    pub async fn next(&mut self) -> Result<AinxtAuth, AuthError> {
    ...
    async fn resolve_next(&mut self) -> Result<AinxtAuth, AuthError> {
    ...
    async fn next_step_loop(&mut self) -> Result<AinxtAuth, AuthError> {
        loop {
            match self.step {
                ...
                RecoveryStep::DevboxRecovery => {
                    ...
                    if !self.auth_manager.ainxt_com_config().blocks_automatic_oidc()
    ...
    async fn try_reload_from_disk(&self) -> Option<AinxtAuth> {
        let _lock = self
            .auth_manager
        ...
        let Some(disk_auth) = self.auth_manager.read_disk_auth() else {
            ...
            ainxt_telemetry::unified_log::debug("auth recovery: no disk entry", None, None);
            return None;
        };
    ...
    fn fresh_mint_guard(&self) -> Option<AinxtAuth> {
    ...
    async fn try_refresh_from_authority(&self) -> Result<AinxtAuth, AuthError> {
    ...
    fn is_different_token(&self, candidate: &AinxtAuth) -> bool {
...
mod tests {
    ...
    use crate::auth::model::{AuthMode, AinxtAuth};
    ...
    fn rejected_cred() -> Option<AinxtAuth> {
        Some(AinxtAuth {
    ...
    impl TokenRefresher for OkRefresher {
        async fn refresh(&self, _reason: crate::auth::manager::RefreshReason) -> RefreshOutcome {
            ...
            RefreshOutcome::Success(Box::new(AinxtAuth {
    ...
    fn mgr() -> (tempfile::TempDir, Arc<AuthManager>) {
        ...
        let m = Arc::new(AuthManager::new(dir.path(), AinxtComConfig::default()));
        (dir, m)
    }
    ...
    async fn recover_with_ok_refresher(m: &Arc<AuthManager>) -> (Result<AinxtAuth, AuthError>, u32) {
        let calls = Arc::new(AtomicU32::new(0));
        m.set_refresher(Arc::new(OkRefresher {
            calls: calls.clone(),
        }));
    ...
    async fn fresh_mint_guard_wins_over_cached_permanent_failure() {
        ...
        // verdict is cached for it — matches `auth()`'s wire-valid grace arm;
    ...
    async fn reload_from_disk_picks_up_different_token() {
        ...
        let scope = m.ainxt_com_config().auth_scope();
        let fresh = AinxtAuth {
    ...
    async fn reload_from_disk_rejects_expired_different_token() {
        ...
        let scope = m.ainxt_com_config().auth_scope();
        let expired_different = AinxtAuth {
    ...
    /// than a fresh key per call) matters for correctness here: some tests
    /// build the same token twice via `team_jwt(...)` — once to seed state,
    /// once to assert equality/inequality against it — and need identical
    /// output both times for those assertions to be meaningful.
