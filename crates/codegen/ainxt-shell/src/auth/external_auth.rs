...
pub(crate) struct ExternalAuthOutput {
    ...
    /// Token issuer. An ainxt issuer marks the credential as first-party;
    /// see [`AinxtAuth::is_ainxt_auth`].
    #[serde(default)]
    pub issuer: Option<String>,
}

/// Parse process output (stdout) into a `AinxtAuth`. Accepts bare token or JSON.
pub(crate) fn parse_output(output: &std::process::Output) -> anyhow::Result<AinxtAuth> {
...
pub(crate) fn run_external_auth_sync(command: &str, is_refresh: bool) -> Option<AinxtAuth> {
...
pub(crate) fn refresh_with_command(command: &str, prev_auth: &AinxtAuth) -> Option<AinxtAuth> {
...
mod tests {
    ...
    #[test]
    fn parse_output_issuer_claim_enables_ainxt_auth() {
        ...
        // ainxt.dev issuer claim → first-party session (relay-eligible).
        let auth = parse_output(&ok(
            r#"{"access_token":"t","expires_in":900,"issuer":"https://auth.example.test"}"#,
        ))
        .unwrap();
        assert_eq!(auth.oidc_issuer.as_deref(), Some("https://auth.example.test"));
        assert!(auth.is_ainxt_auth());

        // Non-ainxt.dev issuer is stored but stays third-party.
        let auth = parse_output(&ok(
    ...
    #[test]
    fn sync_sets_ainxt_auth_expired_env_on_refresh() {
        let auth = run_external_auth_sync("echo $AINXT_AUTH_EXPIRED", true).unwrap();
