...
use crate::util::ainxt_auth_credentials::AinxtAuthCredentials;
...
fn api_key_id_for(auth: Option<&crate::auth::AinxtAuth>) -> Option<String> {
    auth.filter(|a| matches!(a.auth_mode, crate::auth::AuthMode::ApiKey))
        .map(|a| ainxt_telemetry::config::deployment_id_from_key(&a.key))
}
...
pub struct ShellAuthCredentialProvider {
    ...
    static_credentials: AinxtAuthCredentials,
}
impl ShellAuthCredentialProvider {
    pub(crate) fn new(
        ...
        let mut static_credentials = AinxtAuthCredentials::new(None);
...
///
/// - `"ainxt-shell"`   — classic ainxt CLI / TUI (ainxt-shell)
/// - `"ainxt-pager"`   — new ainxt Pager / TUI (ainxt-pager)
/// - `"ainxt-desktop"` — ainxt Desktop app
/// - `"ainxt-extension"` — VS Code / browser extension
///
/// The factory forwards both:
/// - `x-ainxt-client-version`   (e.g. "0.1.210-alpha.5 (279ffacddb)")
/// - `x-ainxt-client-identifier`
///
/// to the underlying `ainxt-file-utils::StorageClient` via
...
///   - `user_token` (normal OIDC users via com scope) → sends Bearer + `X-Ainxt-Token-Auth`
...
pub fn build_storage_client_for_proxy(
...
) -> ainxt_file_utils::storage_client::StorageClient {
    ...
    if let Some(am) = auth_manager {
        ...
        let bridge: Arc<dyn ainxt_file_utils::storage_client::Auth401AttributionCallback> =
            Arc::new(StorageClientAttributionBridge::new(am, session_id));
        ainxt_file_utils::storage_client::StorageClient::with_provider(
            proxy_base_url,
            http_client,
            provider,
        )
        .with_client_identity(ainxt_version::VERSION, client_identifier)
        .with_client_mode(crate::http::process_client_mode())
        .with_attribution(bridge)
    } else {
        let mut creds = AinxtAuthCredentials::new(user_token);
...
/// Bridge that lets `StorageClient` (which lives in ainxt-file-utils)
...
impl ainxt_file_utils::storage_client::Auth401AttributionCallback for StorageClientAttributionBridge {
...
impl HttpAuth for OtelAuthCredentialProvider {
    fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        ...
        let mut creds = AinxtAuthCredentials::new(None);
...
pub fn sync_external_otel_identity() {
    if let Some(provider) = OTEL_PROVIDER.get() {
        ...
        ainxt_telemetry::external::set_identity(
            ainxt_telemetry::external::IdentityAttrs::from_snapshot(&snapshot),
        );
    }
}
...
/// `ainxt-pager` and `ainxt-tui` need at tracing init time.
///
...
pub fn build_default_otel_layer_config() -> ainxt_telemetry::otel_layer::OtelLayerConfig {
    let endpoints = crate::agent::config::EndpointsConfig::default();
    let ainxt_com_config = crate::auth::AinxtComConfig::default();
    let exporter = ainxt_telemetry::otel_layer::OtelExporterConfig {
    ...
    let token_header_value = ainxt_com_config.token_header.clone();
    let ainxt_home = crate::util::ainxt_home::ainxt_home();
    let bootstrap = Arc::new(AuthManager::new(&ainxt_home, ainxt_com_config));
    let provider = Arc::new(OtelAuthCredentialProvider::new(bootstrap));
    let _ = OTEL_PROVIDER.set(provider.clone());
    ainxt_telemetry::otel_layer::OtelLayerConfig {
...
mod tests {
    ...
    use crate::auth::AinxtComConfig;
    use crate::auth::manager::AuthManager;
    use chrono::{Duration as ChronoDuration, Utc};
    use std::sync::Mutex;
    use ainxt_auth::AuthCredentialProvider;
    /// Serializes tests that pin `AINXT_AUTH_EARLY_INVALIDATION_SECS`, since
    /// env vars are process-global and parallel tests would race.
    static EARLY_INVALIDATION_LOCK: Mutex<()> = Mutex::new(());
    /// RAII guard: pins `AINXT_AUTH_EARLY_INVALIDATION_SECS` to the production
    ...
    impl EarlyInvalidationGuard {
        fn pin_to_default() -> Self {
            ...
            let previous = std::env::var("AINXT_AUTH_EARLY_INVALIDATION_SECS").ok();
            unsafe { std::env::set_var("AINXT_AUTH_EARLY_INVALIDATION_SECS", "300") };
            Self {
                _lock: lock,
                previous,
            }
        ...
        fn drop(&mut self) {
            unsafe {
                match self.previous.take() {
                    Some(prev) => std::env::set_var("AINXT_AUTH_EARLY_INVALIDATION_SECS", prev),
                    None => std::env::remove_var("AINXT_AUTH_EARLY_INVALIDATION_SECS"),
                }
            }
        }
    }
    fn make_auth(key: &str, expires_in: ChronoDuration) -> AinxtAuth {
        AinxtAuth {
            key: key.to_string(),
    ...
    fn make_manager(dir: &tempfile::TempDir, initial: Option<AinxtAuth>) -> Arc<AuthManager> {
    ...
    fn snapshot_populates_tenant_id_per_auth_mode() {
        use ainxt_telemetry::config::deployment_id_from_key;
        let _guard = EarlyInvalidationGuard::pin_to_default();
        let dir = tempfile::tempdir().unwrap();
        let dep = ShellAuthCredentialProvider::new(
            make_manager(&dir, None),
            Some("ainxt-token-EX".into()),
            None,
        )
        .snapshot();
        assert_eq!(
            dep.deployment_id.as_deref(),
            Some(deployment_id_from_key("ainxt-token-EX").as_str())
        );
        assert!(dep.api_key_id.is_none());
        let api_auth = AinxtAuth {
    ...
    /// user token, matching the precedence in `AinxtAuthCredentials::apply`.
