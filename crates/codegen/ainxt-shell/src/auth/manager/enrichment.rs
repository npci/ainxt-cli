...
use crate::auth::model::{AinxtAuth, UserInfo, lookup_auth};
...
pub(super) fn spawn(manager: Arc<AuthManager>, auth: AinxtAuth) {
...
async fn fetch_user_info(manager: &AuthManager, key: &str, log_label: &str) -> Option<UserInfo> {
    ...
    let token_header = &manager.ainxt_com_config.token_header;
    ...
    let response = http_client
        ...
        .header("X-Ainxt-Token-Auth", token_header.as_str())
...
pub(super) async fn enrich_inline(manager: &AuthManager, auth: &mut AinxtAuth) {
...
async fn run_user_info_enrichment(manager: &AuthManager, auth: AinxtAuth) {
    let started = std::time::Instant::now();
...
pub(super) fn apply_user_info_enrichment(disk: &mut AinxtAuth, user_info: UserInfo) {
