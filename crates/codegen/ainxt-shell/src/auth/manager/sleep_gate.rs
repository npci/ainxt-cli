...
impl SleepGate {
    pub(super) fn raise(&self) {
        *self.raised_at.write() = Some(GateRaise::now());
        ainxt_telemetry::unified_log::warn("auth.sleep.gate_set", None, None);
    }
...
impl AuthManager {
    ...
    /// [`ainxt_system_power::PowerState`] for the canonical explanation of what a
    ...
    pub(crate) fn is_dark_wake(&self) -> bool {
        ...
        matches!(
            ainxt_system_power::current_power_state(),
            ainxt_system_power::PowerState::DarkWake
        )
    }
    ...
    /// Test hook: simulate an IdP refresh entering flight (matches
    /// [`InFlightGuard::new`]).
    #[cfg(test)]
    ...
    /// Test hook: simulate an in-flight IdP refresh finishing (matches
    ...
    pub fn start_system_power_listener(self: &Arc<Self>) {
        ...
        let listener = ainxt_system_power::SystemPowerListener::start(move |event| {
            if let Some(this) = weak.upgrade() {
                let imminent = matches!(event, ainxt_system_power::PowerEvent::WillSleep);
