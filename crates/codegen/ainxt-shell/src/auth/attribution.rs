...
//!
//! 1. [`ainxt_telemetry::unified_log::warn`] for the local
//!    `~/.ainxt/logs/unified.jsonl` file (best-effort; ships to GCS
...
//!
//! [`ainxt_sampler`] is intentionally decoupled from this crate. It
//! invokes the trait [`ainxt_sampler::Auth401AttributionCallback`] at
//! its six 401 arms; this module provides [`ShellAttribution`], the
//! concrete impl that the shell wires into
//! [`ainxt_sampler::SamplerConfig::attribution_callback`] at every
...
use ainxt_sampler::{Auth401AttributionCallback, SamplingConsumer};
use ainxt_tools::{Auth401AttributionCallback as ToolAuth401AttributionCallback, ToolConsumer};
...
impl ShellAttribution {
    ...
    /// [`ainxt_sampler::SamplerConfig::attribution_callback`].
    ///
    /// (Returns `Arc<dyn Trait>` rather than `Self` because the
    /// `ainxt_sampler::SamplerConfig` field expects exactly that;
    ...
    /// `Arc<dyn ainxt_tools::Auth401AttributionCallback>` for the
...
impl Auth401AttributionCallback for ShellAttribution {
    fn record_401(&self, consumer: SamplingConsumer, sent_bearer_prefix: Option<&str>) {
        ...
        // to `ainxt_sampler::SENT_BEARER_PREFIX_LEN` characters
...
/// in `ainxt-tools` emits a 401 attribution event through this
...
/// All 401 emit sites in `ainxt-shell` go through this helper -- the
...
pub(crate) fn record_auth_401(
    ...
    // Sink 1 -- local file (~/.ainxt/logs/unified.jsonl) + scrubbed
...
/// and the mint/expiry fields from the resulting `AinxtAuth`.
///
...
fn compute_attribution_payload(
    ...
    // Single read-lock acquisition: pull the live `AinxtAuth` (or
    ...
    // `ainxt_com_config.auth_token_ttl` when `expires_at` is `None`
...
mod tests {
    ...
    /// nothing from a developer's actual `~/.ainxt/auth.json` leaks in.
    ...
    fn fresh_auth(key: &str) -> AinxtAuth {
        AinxtAuth {
            key: key.to_string(),
            create_time: Utc::now(),
    ...
    /// `ShellAttribution` implements `ainxt_tools::Auth401AttributionCallback`
