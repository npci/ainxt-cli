...
use super::super::config::{ForceLoginTeam, AinxtComConfig, OAuth2ProviderConfig, OidcAuthConfig};
use super::super::{AuthMode, AinxtAuth};
use base64::Engine;
...
pub fn is_configured(config: &AinxtComConfig) -> bool {
    config.oidc.is_some()
}
...
pub(crate) fn login_principal_policy(cfg: &AinxtComConfig) -> Option<ForceLoginTeam> {
...
pub(super) async fn discover(issuer: &str) -> anyhow::Result<Discovery> {
    if issuer.trim().is_empty() {
        return Err(anyhow::Error::new(OidcError::NotConfigured));
    }
...
mod tests {
    ...
    /// (`peek_access_token_principal` decodes claims without checking them),
    /// so the exact key value doesn't matter. Random-but-stable (rather than
    /// a fresh key per call) so tests that build the same token twice for
    ...
    fn authorize_url_includes_team_principal_params() {
        let config = OidcAuthConfig {
            ...
            client_id: TEST_CLIENT_ID.into(),
            scopes: vec!["offline_access".into(), "ainxt-cli:access".into()],
            audience: None,
        };
        ...
        assert!(url.contains("referrer=ainxt-build"));
        assert_eq!(
            url.matches("referrer=").count(),
            1,
            "expected exactly one referrer param, got: {url}"
    ...
    fn authorize_url_uses_oauth2_referrer_override_once() {
        ...
        let oauth2 = OAuth2ProviderConfig {
            ...
            principal_type: None,
            principal_id: None,
            referrer: Some("ainxt-desktop".into()),
        };
        let discovery = Discovery {
            authorization_endpoint: "https://auth.example.test/authorize".into(),
            token_endpoint: "https://auth.example.test/token".into(),
        ...
        assert!(url.contains("referrer=ainxt-desktop"));
        assert!(!url.contains("referrer=ainxt-build"));
        assert_eq!(
            url.matches("referrer=").count(),
            1,
    ...
    fn peek_access_token_principal_matrix() {
        ...
        let team_jwt = make_jwt(serde_json::json!(
            { "sub" : "user-42", "iss" : "https://auth.example.test", "aud" : "test-client",
            "exp" : 9999999999u64, "iat" : 1000000000u64, "scope" :
            "offline_access ainxt-cli:access api:access", "principal_type" : "Team",
