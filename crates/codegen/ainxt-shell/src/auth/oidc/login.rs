...
use super::super::config::{AinxtComConfig, OidcAuthConfig};
use super::super::{AuthManager, AinxtAuth};
use super::protocol::{
    OidcError, build_authorize_url, build_ainxt_auth, discover, enforce_login_principal,
...
fn callback_response(result: &CallbackResult) -> (StatusCode, Html<String>) {
    let (title, message) = match result {
        Ok(_) => (
            "Signed in",
            "You can close this window and return to ainxt.",
        ),
...
pub async fn run_login_flow_with_config(
    ...
    let oauth2 = auth_manager.ainxt_com_config().oauth2.as_ref();
    ...
    if has_client_ui {
        ...
        eprintln!();
        let provider_label = if super::super::config::is_ainxt_oauth2_issuer(&oidc.issuer) {
            "ainxt".to_owned()
        } else {
            oidc.issuer.clone()
        };
    ...
    let principal_policy = login_principal_policy(auth_manager.ainxt_com_config());
    ...
    let mut auth = build_ainxt_auth(tokens, user_info, &oidc.issuer, &oidc.client_id);
...
mod tests {
    ...
    async fn full_login_flow_via_race() {
        ...
        let auth_manager = Arc::new(
            AuthManager::new(temp_dir.path(), AinxtComConfig::default())
        ...
        .await
        .unwrap();
        let auth = build_ainxt_auth(tokens, user_info, &oidc_cfg.issuer, &oidc_cfg.client_id);
