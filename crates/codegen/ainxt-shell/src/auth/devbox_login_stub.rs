...
//! matches the real module: `is_devbox_environment()` is always `false`, which
//! short-circuits every auto-recovery/migration call site, and the entry
//! points that can still be reached directly (`ainxt login --devbox`) return a
//! descriptive error.

use super::manager::AuthManager;
use super::model::AinxtAuth;
...
pub(crate) async fn mint_devbox_auth(_auth_manager: &AuthManager) -> anyhow::Result<AinxtAuth> {
    anyhow::bail!(UNAVAILABLE)
}

/// Unreachable in practice (guarded by [`is_devbox_environment`]); errors
/// defensively if called.
pub(super) async fn mint_devbox_auth_raw() -> anyhow::Result<AinxtAuth> {
    anyhow::bail!(UNAVAILABLE)
}

/// `ainxt login --devbox` entry point: always errors in this build.
pub async fn run_devbox_login(_config: &crate::agent::config::Config) -> anyhow::Result<AinxtAuth> {
    anyhow::bail!(UNAVAILABLE)
}
