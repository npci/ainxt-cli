...
use crate::auth::{AuthChannels, AuthManager, AuthMode, AuthUrlInfo, AuthUrlMode, AinxtAuth};
...
pub enum DeviceCodeError {
    #[error(
        "Device-code login is not available for this deployment. \
         Try `ainxt login` or set AINXT_API_KEY instead."
    )]
    NotEnabled,
    #[error(transparent)]
...
/// `x-ainxt-client-surface` header so device-flow metrics can separate logins a
...
pub async fn request_device_code(
    ...
    let resp = with_alpha_test_key(
        client
            ...
            .header("x-ainxt-client-surface", surface.as_str())
            .form(&[
                ("client_id", client_id),
                ("scope", scope_str.as_str()),
                ("referrer", "ainxt-build"),
            ]),
        &url,
    )
    .send()
...
///
/// On success, persists credentials to `~/.ainxt/auth.json` and returns
/// the authenticated `AinxtAuth`.
///
...
pub async fn complete_device_code_login(
    ...
    loop {
        ...
        if tokio::time::Instant::now() > deadline {
            anyhow::bail!("Device code expired. Run `ainxt login --device-auth` again.");
        }
...
pub async fn run_device_code_login_channels(
    ...
    // `ainxt.dev/auth/get_url` isn't blocked on a slow/hanging browser launch
...
pub(crate) mod tests {
    ...
    use crate::auth::{AuthMode, AinxtComConfig};

    /// A random HS256 signing key for test-only JWTs, generated once per test
    /// binary run and reused by every call in this module.
    ///
    /// Not a secret: nothing in production ever verifies these tokens'
    /// signatures (`peek_access_token_principal` decodes claims without
    /// checking them), so the exact key value doesn't matter — it just needs
    /// to exist. Random-but-stable (rather than a fresh key per call) so
    /// tests that build the same token twice for comparison still see equal
    /// output, while no static literal exists in source for a scanner to
    /// flag as a hardcoded secret.
    ...
    fn auth_manager_with_ainxt_home(
        ainxt_home: &std::path::Path,
        proxy_base_url: &str,
    ) -> Arc<AuthManager> {
        Arc::new(
            AuthManager::new(ainxt_home, AinxtComConfig::default())
    ...
    fn build_auth_persists_credentials_without_proxy_fetch() {
        ...
        let ainxt_home = temp_dir.path().join(".ainxt");
        std::fs::create_dir_all(&ainxt_home).unwrap();
        let auth_manager = auth_manager_with_ainxt_home(&ainxt_home, "http://127.0.0.1:9");
        let tokens = super::TokenOk {
            access_token: "access-token".to_string(),
            refresh_token: Some("refresh-token".to_string()),
            expires_in: Some(900),
            scope: Some("openid email offline_access ainxt-cli:access".to_string()),
            id_token: Some(
    ...
    fn build_auth_seeds_team_metadata_from_access_token() {
        ...
        let claims = serde_json::json!({
            ...
            "exp": 9999999999u64,
            "iat": 1000000000u64,
            "scope": "offline_access ainxt-cli:access team:read",
            "principal_type": "Team",
        ...
        let tokens = super::TokenOk {
            ...
            expires_in: Some(900),
            scope: Some("offline_access ainxt-cli:access team:read".to_owned()),
            id_token: None,
        };
    ...
    /// persist nothing.
    fn assert_build_auth_rejected(cfg: AinxtComConfig, token_principal: &str, expected_err: &str) {
        ensure_crypto_provider();
        let temp_dir = tempfile::tempdir().unwrap();
        let ainxt_home = temp_dir.path().join(".ainxt");
        std::fs::create_dir_all(&ainxt_home).unwrap();
        let auth_manager =
            Arc::new(AuthManager::new(&ainxt_home, cfg).with_proxy_base_url("http://127.0.0.1:9"));
        ...
        assert!(
            !ainxt_home.join("auth.json").exists(),
    ...
    async fn run_poll(
        responses: Vec<(u16, serde_json::Value)>,
    ) -> anyhow::Result<(super::AinxtAuth, bool)> {
        let (issuer, server) = spawn_token_server(responses).await;
        let temp_dir = tempfile::tempdir().unwrap();
        let auth_manager = auth_manager_with_ainxt_home(temp_dir.path(), "http://127.0.0.1:9");
