...
use crate::auth::{AuthManager, AinxtAuth, AinxtComConfig, parse_output};
use crate::util::ainxt_home;
...
/// session.
fn is_cached_credential_compatible(auth: &AinxtAuth, ainxt_com_config: &AinxtComConfig) -> bool {
    let expected_issuer = ainxt_com_config
        .oidc
        .as_ref()
        .map(|c| c.issuer.as_str())
        .or_else(|| ainxt_com_config.oauth2.as_ref().map(|c| c.issuer.as_str()));
    ...
    if let Some(policy) = crate::auth::oidc::login_principal_policy(ainxt_com_config) {
...
fn resolve_device_flow(
    ...
    crate::agent::config::BoolFlag::env("AINXT_LOGIN_DEVICE_FLOW")
...
/// ainxt OAuth2 provider supports it. Enterprise OIDC (`oidc=Some`) always uses
/// the loopback flow, mirroring `run_auth_flow_inner`'s precedence.
async fn cli_should_use_device(
    config: &AinxtComConfig,
    login_override: LoginTransportOverride,
) -> bool {
    !crate::auth::oidc::is_configured(config) && should_use_device_flow(login_override).await
}

/// Whether interactive ainxt OAuth2 login uses the RFC 8628 device flow (vs loopback).
///
/// Precedence: CLI (`--oauth`/`--device-auth`) > `AINXT_LOGIN_DEVICE_FLOW` env >
/// `[auth] login_device_flow` config > `ainxt_build_login_device_flow` remote feature flag > loopback.
async fn should_use_device_flow(login_override: LoginTransportOverride) -> bool {
    ...
    let resolved = if login_override.as_cli_bool().is_some() {
        ...
        let env = crate::agent::config::env_bool("AINXT_LOGIN_DEVICE_FLOW");
...
/// How login presents itself; surfaced to the TUI via `ainxt.dev/auth/get_url`.
...
impl AuthUrlMode {
    /// Wire string for the `ainxt.dev/auth/get_url` ACP response.
...
async fn run_external_auth_provider(
    ...
    // Verify the team pin before any persist (equivalence with the OIDC / device-code
    // completion paths). A mismatch fails the login and writes nothing.
    let principal_policy =
        crate::auth::oidc::login_principal_policy(auth_manager.ainxt_com_config());
...
pub async fn run_auth_flow_with_stderr_bridge(
    auth_manager: &Arc<AuthManager>,
    ainxt_com_config: &AinxtComConfig,
...
async fn run_auth_flow_inner(
    ...
    tracing::info!(
        has_oidc = ainxt_com_config.oidc.is_some(),
        has_oauth2 = ainxt_com_config.oauth2.is_some(),
        has_external_auth = ainxt_com_config.auth_provider_command.is_some(),
        reauth,
    ...
    if reauth {
        auth_manager.clear()?;
        // Also remove the legacy accounts.ainxt.dev scope so stale tokens
    ...
    if !force_interactive && let Some(auth) = auth_manager.current() {
        if is_cached_credential_compatible(&auth, ainxt_com_config) {
    ...
    if !force_interactive && !reauth && auth_manager.is_expired() {
        ...
        if disk_auth.as_ref().is_some_and(|d| {
            !crate::auth::is_expired(d) && is_cached_credential_compatible(d, ainxt_com_config)
        }) {
            ainxt_telemetry::unified_log::info(
    ...
    if let Some(ref cmd) = ainxt_com_config.auth_provider_command {
    ...
    // `ainxt login --devbox` uses run_devbox_login and is not gated here.
    if !ainxt_com_config.blocks_automatic_oidc()
    ...
    // ainxt OAuth2 also defaults to loopback; the device flow (robust on
    // remote/SSH where the loopback redirect can't reach the CLI) is opt-in via
    // --device-auth / AINXT_LOGIN_DEVICE_FLOW / [auth] login_device_flow.
    if crate::auth::oidc::is_configured(ainxt_com_config) {
        return crate::auth::oidc::run_login_flow(ainxt_com_config, auth_manager, channels).await;
    }

    if let Some(ref oauth2_cfg) = ainxt_com_config.oauth2 {
    ...
    tracing::error!(
        "auth: no OAuth2 configuration available (neither enterprise OIDC nor ainxt OAuth2 configured)"
    );
    anyhow::bail!(
        "No OAuth2 configuration available. Run `ainxt login` to authenticate, or contact your administrator if you use enterprise SSO."
    )
}
...
pub async fn try_ensure_fresh_auth(ainxt_com_config: &AinxtComConfig) -> Option<AinxtAuth> {
    let ainxt_home = ainxt_home::ainxt_home();
    let auth_manager = std::sync::Arc::new(AuthManager::new(&ainxt_home, ainxt_com_config.clone()));

    // auth() handles cached-valid (fast path), OIDC refresh, external
    // binary -- all through refresh_chain (single mutation point).
    auth_manager.configure_refresher(ainxt_com_config.auth_provider_command.clone(), None);
    match auth_manager.auth().await {
        Ok(auth) => Some(auth),
        Err(e) => {
...
pub(crate) async fn try_ensure_session_noninteractive(
    ...
    if let Some(auth) = try_ensure_fresh_auth(ainxt_com_config).await {
        return Some(auth);
    }
    let ainxt_home = ainxt_home::ainxt_home();
    let auth_manager = Arc::new(AuthManager::new(&ainxt_home, ainxt_com_config.clone()));
    ...
    mint_session_noninteractive(&auth_manager, ainxt_com_config).await
}
...
fn expired_refreshable_session(auth_manager: &AuthManager) -> Option<AinxtAuth> {
    auth_manager
        .current_or_expired()
        .filter(|a| a.is_ainxt_auth() && a.refresh_token.is_some())
}
...
async fn mint_session_noninteractive(
    ...
    if let Some(cmd) = ainxt_com_config.auth_provider_command.as_deref() {
...
async fn persist_or_use_minted(auth_manager: &AuthManager, new_auth: AinxtAuth) -> AinxtAuth {
...
fn report_signed_in(auth: &AinxtAuth) {
    eprint!("\r\x1b[K");
    match auth.email {
...
pub async fn ensure_authenticated_or_noninteractive(
    ...
    message_prefix: Option<&str>,
) -> anyhow::Result<Option<AinxtAuth>> {
    if has_noninteractive_auth {
        Ok(try_ensure_fresh_auth(ainxt_com_config).await)
    } else {
        ensure_authenticated(ainxt_com_config, false, message_prefix)
            .await
            .map(Some)
    }
}

/// Unified `ainxt login` handler for CLI entry points (tui, pager).
///
/// Precedence: `--oauth` forces loopback, `--device-auth` forces device,
/// otherwise `AINXT_LOGIN_DEVICE_FLOW` env / `[auth] login_device_flow` config /
...
pub async fn run_cli_login(
    ...
    // oauth2=None) always uses the loopback flow; only the ainxt OAuth2 provider
    // supports the device flow. Without this guard, `ainxt login` on an
    ...
    let authenticated = if devbox {
    ...
    } else if cli_should_use_device(&config.ainxt_com_config, login_override).await {
        if config.ainxt_com_config.oauth2.is_none() {
            // No OIDC and no oauth2 here, so `--oauth` can't help.
            anyhow::bail!("Sign-in is not available for this deployment. Set AINXT_API_KEY instead.");
        }
        let ainxt_home = ainxt_home::ainxt_home();
        let auth_manager = Arc::new(AuthManager::new(&ainxt_home, config.ainxt_com_config.clone()));
        ...
        if device_auth && crate::auth::oidc::is_configured(&config.ainxt_com_config) {
            eprintln!(
    ...
    // config was actually applied; `ainxt setup` reports the no-config case.
...
pub struct LogoutResult {
    ...
    /// `true` if `AINXT_API_KEY` / `AINXT_CODE_AINXT_API_KEY` env var is set.
...
pub fn perform_logout(
    ...
    if was_logged_in {
        ...
        // equivalence). Clear the external OTEL identity attrs FIRST so any
        ...
        ainxt_telemetry::external::set_identity(
            ainxt_telemetry::external::IdentityAttrs::default(),
        );
        ainxt_telemetry::external::flush();
    ...
    Ok(LogoutResult {
        was_logged_in,
        email,
        api_key_still_set: crate::agent::auth_method::has_ainxt_api_key_env(),
    })
}

/// `ainxt logout` CLI handler. Calls [`perform_logout`] and formats
/// the result to stderr.
pub fn run_cli_logout(config: &crate::agent::config::Config) -> anyhow::Result<()> {
    let ainxt_home = ainxt_home::ainxt_home();
    let auth_manager = AuthManager::new(&ainxt_home, config.ainxt_com_config.clone());
    ...
    if !result.was_logged_in {
        ...
        if result.api_key_still_set {
            eprintln!("You are authenticated via AINXT_API_KEY (environment variable).");
        }
        return Ok(());
    }
    ...
    if result.api_key_still_set {
        eprintln!("AINXT_API_KEY is still set and will be used for authentication.");
    }
    Ok(())
}
...
mod tests {
    ...
    use crate::auth::config::AINXT_OAUTH2_ISSUER;
    use crate::env::EnvVarGuard;
    use chrono::Utc;

    /// Run `f` with `AINXT_LOGIN_DEVICE_FLOW` set to `value` (unset for `None`).
    ...
    fn with_device_flow_env<T>(value: Option<bool>, f: impl FnOnce() -> T) -> T {
        let _guard = match value {
            Some(true) => EnvVarGuard::set("AINXT_LOGIN_DEVICE_FLOW", "true"),
            Some(false) => EnvVarGuard::set("AINXT_LOGIN_DEVICE_FLOW", "false"),
            None => EnvVarGuard::remove("AINXT_LOGIN_DEVICE_FLOW"),
        };
        f()
    }

    // A ainxt.dev first-party (ainxt.dev-issuer) OIDC session — `is_ainxt_auth()` true.
    fn oidc_session(key: &str, refresh: Option<&str>) -> AinxtAuth {
        AinxtAuth {
            key: key.into(),
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some(AINXT_OAUTH2_ISSUER.to_string()),
    ...
    fn expired_refreshable_session_gate() {
        ...
        let mgr = AuthManager::new(dir.path(), AinxtComConfig::default());
        ...
        // Third-party external (no ainxt.dev issuer) stays excluded.
        mgr.hot_swap(AinxtAuth {
            oidc_issuer: None,
            auth_mode: AuthMode::External,
            expires_at: Some(Utc::now() - chrono::Duration::hours(1)),
    ...
    async fn persist_or_use_minted_returns_token_when_save_fails() {
        ...
        // Read-only ainxt_home: reading a missing auth.json succeeds (empty), but
        // writing fails — exercising the save-failure path.
        let dir = tempfile::tempdir().unwrap();
        std::fs::set_permissions(dir.path(), std::fs::Permissions::from_mode(0o500)).unwrap();
        let mgr = Arc::new(AuthManager::new(dir.path(), AinxtComConfig::default()));
    ...
    async fn mint_session_noninteractive_uses_external_provider() {
        let dir = tempfile::tempdir().unwrap();
        let cfg = AinxtComConfig {
            auth_provider_command: Some("printf '%s' ainxt-ext-token".to_string()),
        ...
        assert_eq!(auth.map(|a| a.key), Some("ainxt-ext-token".to_string()));
    }

    /// External-provider output is team-pinned before persist (equivalence with OIDC
    ...
    async fn external_reauth_without_prev_auth_enriches_inline() {
        ...
        let dir = tempfile::tempdir().unwrap();
        let mgr = Arc::new(
            AuthManager::new(dir.path(), AinxtComConfig::default())
    ...
    async fn preresolved_bypasses_resolver_and_is_never_cli() {
        ...
        {
            let _guard = EnvVarGuard::set("AINXT_LOGIN_DEVICE_FLOW", "false");
            assert!(
        ...
        {
            let _guard = EnvVarGuard::set("AINXT_LOGIN_DEVICE_FLOW", "true");
            assert!(
    ...
    async fn enterprise_oidc_never_uses_device_flow() {
        // oidc=Some, oauth2=None: `ainxt login` must use loopback, not device —
        ...
        // The ainxt OAuth2 provider (oidc=None, oauth2=Some) does use device.
        let ainxt = AinxtComConfig::default();
        assert!(ainxt.oauth2.is_some() && ainxt.oidc.is_none());
        assert!(cli_should_use_device(&ainxt, LoginTransportOverride::ForceDevice).await);
    }
    ...
    fn oidc_auth(issuer: &str) -> AinxtAuth {
        AinxtAuth {
    ...
    /// A random HS256 signing key for test-only JWTs, generated once per test
    /// binary run and reused by every call in this module.
    ///
    /// Not a secret: nothing in production verifies these tokens' signatures
    /// here, so the exact key value doesn't matter. Random-but-stable (rather
    /// than a fresh key per call) so tests that build the same token twice
    /// for comparison still see equal output.
    fn test_signing_key() -> jsonwebtoken::EncodingKey {
        static KEY: std::sync::LazyLock<[u8; 16]> =
            std::sync::LazyLock::new(|| *uuid::Uuid::new_v4().as_bytes());
    ...
    fn pinned_cfg(team: &str) -> AinxtComConfig {
        AinxtComConfig {
    ...
    async fn run_auth_flow_uses_valid_disk_token_when_expired() {
        ...
        let valid_disk = AinxtAuth {
            key: "fresh-token-from-disk".into(),
            auth_mode: AuthMode::Oidc,
            expires_at: Some(Utc::now() + chrono::Duration::hours(1)),
            refresh_token: Some("new-rt".into()),
            oidc_issuer: Some(AINXT_OAUTH2_ISSUER.into()),
    ...
    async fn run_auth_flow_falls_through_when_no_refresh_token() {
        ...
        let mut cfg = AinxtComConfig::default();
        cfg.oauth2.as_mut().unwrap().issuer = "http://127.0.0.1:1".into();

        let writer = Arc::new(
            AuthManager::new(dir.path(), cfg.clone()).with_proxy_base_url("http://127.0.0.1:1"),
        );
        let expired_no_rt = AinxtAuth {
        ...
        // AINXT_LOGIN_DEVICE_FLOW / the real config file (the CLI override
    ...
    fn extract_url_from_external_provider_stderr() {
        ...
        assert_eq!(
            extract(
                "Visit the following link to sign into ainxt: https://auth.example.com/login?code=abc"
            ),
    ...
    /// CLI `ainxt login` passes `on_stderr=None`; stderr must be inherited so
