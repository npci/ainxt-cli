...
/// Parse a `ainxt.dev/queue/{remove,reorder,clear,edit,interject}` ext-notification's
...
pub(super) fn parse_queue_edit_command(
    ...
    match method {
        ...
        "ainxt.dev/queue/clear" => Some(SessionCommand::ClearQueue { owner }),
...
mod tests {
    ...
    /// Each `ainxt.dev/queue/*` ext-notification maps to the
    ...
    fn parse_queue_edit_command_maps_each_method() {
        ...
        match parse_queue_edit_command("ainxt.dev/queue/remove", &p, Some("ainxt-tui".into())) {
            Some(SessionCommand::RemoveQueuedPrompt {
                id,
                expected_version,
                owner,
                ...
                assert_eq!(owner.as_deref(), Some("ainxt-tui"));
            }
        ...
        match parse_queue_edit_command("ainxt.dev/queue/remove", &p, None) {
            Some(SessionCommand::RemoveQueuedPrompt {
                expected_version, ..
        ...
        match parse_queue_edit_command("ainxt.dev/queue/reorder", &p, None) {
            Some(SessionCommand::ReorderQueue { ordered_ids }) => {
        ...
        match parse_queue_edit_command("ainxt.dev/queue/edit", &p, Some("ainxt-vscode".into())) {
            Some(SessionCommand::EditQueuedPrompt {
                id,
                new_text,
                editor,
                ...
                assert_eq!(editor.as_deref(), Some("ainxt-vscode"));
            }
        ...
        // interject: id + expectedVersion + owner (matches remove).
        let p = serde_json::json!({
            "sessionId": "s1", "id": "p10", "expectedVersion": 2
        });
        match parse_queue_edit_command("ainxt.dev/queue/interject", &p, Some("ainxt-tui".into())) {
            Some(SessionCommand::InterjectQueuedPrompt {
                id,
                expected_version,
                owner,
        ...
        match parse_queue_edit_command("ainxt.dev/queue/interject", &p, None) {
        ...
        assert!(
            parse_queue_edit_command("ainxt.dev/queue/interject", &serde_json::json!({}), None)
                .is_none()
        );

        // unknown method → None.
        assert!(
            parse_queue_edit_command("ainxt.dev/queue/bogus", &serde_json::json!({}), None).is_none()
        );
        // remove without id → None (can't target an entry).
        assert!(
            parse_queue_edit_command("ainxt.dev/queue/remove", &serde_json::json!({}), None).is_none()
        );
    }
}
