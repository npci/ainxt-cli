...
//! two ways:
//!
//!   - request/response `ainxt.dev/sessions/list` → `{ "sessions": [RosterEntry, …] }`
//!   - broadcast notification `ainxt.dev/sessions/changed` →
...
///
/// Matches the design's `SessionActivity` at dashboard granularity. A full
...
/// Response payload for `ainxt.dev/sessions/list`.
#[derive(Serialize, Deserialize, Clone, Debug, Default)]
pub struct RosterListResponse {
    pub sessions: Vec<RosterEntry>,
}

/// Params payload for the `ainxt.dev/sessions/changed` broadcast notification.
#[derive(Serialize, Deserialize, Clone, Debug, Default)]
...
pub const SESSIONS_LIST_METHOD: &str = "ainxt.dev/sessions/list";
pub const SESSIONS_CHANGED_METHOD: &str = "ainxt.dev/sessions/changed";
