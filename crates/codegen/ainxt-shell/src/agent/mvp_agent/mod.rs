...
use ainxt_telemetry::id::{agent_id, agent_instance_id};
use ainxt_telemetry::session_ctx::log_event;
use ainxt_workspace::file_system::{AcpSessionFs, CodebaseIndexManager, LocalFs};
use ainxt_workspace::permission::{ClientType, PermissionEvent};
...
use ainxt_paths::AbsPathBuf;
use ainxt_workspace::session::git::GitDiscoveryResult;
use ainxt_hunk_tracker::HunkTrackerActor;
/// Hard-error message for legacy Direct hub-bind sessions (`ainxt.dev/cloud_server_id`).
...
pub(crate) fn reject_direct_hub_cloud_meta(
    ...
    if session_meta.and_then(|m| m.get("ainxt.dev/cloud_server_id")).is_some() {
...
/// If `persist_data` is provided, it will be included in the meta under `ainxt.dev/persist`.
...
pub(crate) fn resolve_subscription_tier_for_telemetry(
    ...
    auth: Option<&crate::auth::AinxtAuth>,
) -> Option<String> {
...
/// `/user` already reports a newly qualifying tier (e.g. `SuperAinxtPro`). In
...
pub(crate) fn jwt_claim_matches_user_subscription_tier(
    ...
    match user_subscription_tier {
        "AinxtPro" => jwt_claim == "superainxt",
        "XBasic" => jwt_claim == "x_basic",
        "XPremium" => jwt_claim == "x_premium",
        "XPremiumPlus" => jwt_claim == "x_premium_plus",
        "SuperAinxtPro" => jwt_claim == "superainxt_heavy",
        "SuperAinxtLite" => jwt_claim == "superainxt_lite",
        _ => false,
    }
}
...
/// `_meta["ainxt.dev/session"].kind` → [`SessionKind`]; absent/unknown/malformed → `Build`.
fn parse_session_kind(
    meta: Option<&acp::Meta>,
) -> crate::session::unified_list::SessionKind {
    use crate::session::unified_list::SessionKind;
    use serde::Deserialize;
    meta.and_then(|m| m.get("ainxt.dev/session"))
...
pub(crate) const SESSION_PLUGIN_DIRS_CAPABILITY_KEY: &str = "ainxt.dev/pluginDirs";
...
/// Used to stamp `ainxt.dev/leaderClientId` onto replay notifications so the leader can
...
fn mark_as_replay(
    ...
    if let Some(persist) = persist_data {
        obj.insert("ainxt.dev/persist".to_string(), persist.clone());
    }
}
...
pub(crate) struct PromptResponseMetaArgs<'a> {
    ...
    pub total_tokens: u64,
    pub model_id: &'a str,
    pub last_turn_usage: Option<&'a ainxt_sampling_types::TokenUsage>,
...
/// Typed payload for the `ainxt.dev/settings/update` notification sent to pager
...
fn announcements_push_payload(
    stored: Option<&[ainxt_announcements::RemoteAnnouncement]>,
    last_emitted: &[ainxt_announcements::RemoteAnnouncement],
    now: chrono::DateTime<chrono::Utc>,
    mode: AnnouncementsPushMode,
) -> Option<Vec<ainxt_announcements::RemoteAnnouncement>> {
    let current = ainxt_announcements::filter_expired_at(
...
/// Override with `AINXT_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn announcements_refresh_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("AINXT_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS")
...
/// gates fails.  Used in `ainxt.dev/code/status` responses and to generate
...
pub enum CodeNavEligibility {
    ...
    ClientNotWeb,
    /// Client did not advertise `ainxt.dev/codeNavigation.enabled`.
...
pub struct MvpAgent {
    ...
    /// keyed by SessionId, matches `sessions` lifecycle.
    ///
    ...
    >,
    /// LEADER-SAFE(per-session): keyed by SessionId. Matches `sessions` lifecycle.
    ...
    /// ainxt.dev chat-product catalog (`/rest/modes`) for chat sessions; distinct
    ...
    /// Whether the current client advertised `ainxt.dev/codeNavigation.enabled`.
    ...
    /// Whether the current client advertised `ainxt.dev/folderTrust.interactive` (it
    ...
    /// owned (matches the `DECISIONS` cache, but not a process global), captured
    ...
    /// list. Set by `enforce_ainxt_code_access`; defaults to `true` (API-key and
    ...
    /// **non-recursive** watch on `<cwd>/` and `<cwd>/.ainxt/`).
    ///
    ...
    session_index_claims: RefCell<
        HashMap<acp::SessionId, std::sync::Arc<ainxt_codebase_graph::IndexManagerHandle>>,
    >,
    ...
    subagent_event_tx: tokio::sync::mpsc::UnboundedSender<
        ainxt_tools::implementations::ainxt_build::task::types::SubagentEvent,
    >,
    ...
    monitor_event_buffer: ainxt_tools::implementations::ainxt_build::task::types::MonitorEventBuffer,
    ...
    subagent_roles: std::collections::HashMap<
        String,
        ainxt_subagent_resolution::config::SubagentRole,
    >,
    subagent_personas: std::collections::HashMap<
        String,
        ainxt_subagent_resolution::config::SubagentPersona,
    >,
    ...
    pub(crate) plugin_registry_handle: ainxt_agent::plugins::SharedPluginRegistryHandle,
    ...
    /// cwd→git root plus user/marketplace dirs and stalled ainxt-desktop's first
    ...
    workspace_ops: RefCell<Option<ainxt_workspace::WorkspaceOps>>,
    ...
    /// Announcements list last actually emitted via `ainxt.dev/announcements/update`
    ...
    last_emitted_announcements: RefCell<Vec<ainxt_announcements::RemoteAnnouncement>>,
...
pub(crate) fn inherited_harness_template(
    current: &ainxt_agent::prompt::user_message::UserMessageTemplate,
    pinned_model_agent_type: Option<&str>,
    cwd: &std::path::Path,
) -> Option<ainxt_agent::prompt::user_message::UserMessageTemplate> {
    ...
    if !ainxt_agent::config::is_strict_harness_agent_type(agent_type) {
        return None;
    }
    let harness = ainxt_agent::discovery::by_name_in_cwd(agent_type, cwd)?;
...
/// unchanged — compatible stock switches (e.g. `ainxt-build` →
/// `ainxt-build-plan`) intentionally preserve the session's original ACP
/// `agentProfile`.
...
/// default wire format and toolset, so switching e.g. `ainxt-build` →
/// `ainxt-build-plan` doesn't require rebuilding the harness and would
...
pub(crate) fn harnesses_are_compatible(active: &str, required: &str) -> bool {
    use ainxt_agent::config::is_strict_harness_agent_type;
    match (
...
use ainxt_chat_state::conversation_util::replace_or_insert_system_head;
...
/// unchanged, else the agent template with `_meta.rules` folded into
...
pub(crate) fn warn_on_missing_parent_session_for_validate_type(
    ...
    if !parent_session_present {
        tracing::warn!(
            parent_session_tag = %crate::util::session_log_tag(parent_session_id),
...
///
/// Headers injected:
///  - `x-ainxt-client-version` -- required by the proxy's version-gate check.
///    Uses `client_version` when provided, otherwise falls back to cli-chat-proxy
///    compile-time `CARGO_PKG_VERSION`.
///  - `X-Ainxt-Token-Auth` / `x-authenticateresponse` -- required by the
...
fn inject_proxy_headers(
    ...
    headers
        .entry("x-ainxt-client-version".to_string())
        .or_insert_with(|| {
            client_version
                .map(String::from)
...
/// Parse the client-advertised `ainxt.dev/hunkTracker.mode` string. Case-insensitive
/// and trimmed. Absent/blank/`off`/`disabled` => `None`; unknown => `AllDirty`.
fn resolve_hunk_tracking_mode(
    mode_str: Option<&str>,
) -> Option<ainxt_hunk_tracker::TrackingMode> {
    ...
    Some(
        serde_json::from_value(serde_json::Value::String(mode.to_ascii_lowercase()))
            .unwrap_or(ainxt_hunk_tracker::TrackingMode::AllDirty),
    )
}
...
struct HunkTrackingPlan {
    ...
    actor_mode: Option<ainxt_hunk_tracker::TrackingMode>,
}
...
impl MvpAgent {
    ...
    /// - ainxt updates (`"_ainxt.dev/session/update"`) → `ExtNotification`.
    ///
    ...
    fn forward_raw_replay_line(
        ...
        completions: &mut Vec<
            tokio::sync::oneshot::Receiver<ainxt_acp_lib::AcpResult<()>>,
        >,
        mark_replay: bool,
        ...
        let is_ainxt = method == "_ainxt.dev/session/update";
        if is_ainxt {
            if target_client_id.is_none() && !mark_replay {
                ...
                >(raw_params.get()) else {
                    tracing::debug!(
                        "replay: skipping ainxt update with unparseable params"
                    );
                    return;
                };
                if let Some(obj) = params.as_object_mut() {
                    ...
                    if let Some(m) = meta.as_object_mut() {
                        ...
                        if let Some(pd) = persist_data {
                            m.insert("ainxt.dev/persist".to_string(), pd.clone());
                        }
                        if let Some(tid) = target_client_id {
                            m.insert("ainxt.dev/leaderClientId".to_string(), tid.clone());
                        }
                    }
                }
            ...
            if let Some(tid) = target_client_id {
                stamp_meta_value(&mut notification.meta, "ainxt.dev/leaderClientId", tid);
            }
    ...
    pub(super) async fn replay_session_updates(
        ...
        let Some(updates_path) = updates_file_path.clone() else {
            tracing::warn!(
                session_tag = %crate::util::session_log_tag(session_id.0.as_ref()),
                "replay: no updates file path"
            );
        ...
        tracing::info!(
            session_tag = %crate::util::session_log_tag(session_id.0.as_ref()),
            updates_count, end_offset, file_size,
            "replay: completed"
        );
    ...
    pub(super) fn replay_session_updates_from_offset_enqueue(
        ...
        target_client_id: Option<&serde_json::Value>,
        mark_replay: bool,
    ) -> Vec<tokio::sync::oneshot::Receiver<ainxt_acp_lib::AcpResult<()>>> {
    ...
    pub(super) fn reconcile_stale_background_tasks(
        ...
        for task in &orphaned {
            let snapshot = ainxt_tools::types::TaskSnapshot {
    ...
    ///
    /// Non-ainxt auth (API keys, enterprise) always passes. For ainxt OAuth2
    /// users, reads `allow_access` from remote settings. Defaults to
    /// `false` (blocked) when remote settings are unavailable.
    pub(super) async fn enforce_ainxt_code_access(&self, auth: &crate::auth::AinxtAuth) {
        ...
        if !allow {
            tracing::info!(
                "auth: user blocked by allow_access (remote settings ainxt_build_access_gate)"
            );
            self.retry_subscription_check().await;
        }
    }
    /// Single-shot subscription check called by the pager's "Check
    /// subscription" button (`ainxt.dev/auth/check_subscription`). The pager
            ...
            .map(|auth| {
                let gate = if !self.tier_allowed.get() && gate.is_none() {
                    ...
                    Some(crate::auth::GateInfo {
                        message,
                        url: std::env::var("AINXT_SUBSCRIBE_URL")
                            .ok()
                            .filter(|s| !s.trim().is_empty())
                            .or_else(|| Some("https://ainxt.example.test/subscribe".to_string())),
    ...
    pub(super) async fn maybe_fetch_post_auth_settings(&self) {
        ...
        let is_ainxt_auth = auth.is_ainxt_auth();
        ...
        tracing::info!("post-auth remote_settings fetch succeeded");
        {
                ...
                && cfg.mode != crate::agent::config::AgentMode::Generic
            {
                ...
                if cfg.storage_mode == StorageMode::Writeback && !is_ainxt_auth {
    ...
    /// Fire-and-forget `ainxt.dev/settings/update` from the current remote snapshot.
        ...
        if let Ok(params) = serde_json::value::to_raw_value(&payload) {
            self.gateway
                .forward_fire_and_forget(
                    acp::ExtNotification::new("ainxt.dev/settings/update", params.into()),
                );
        }
    }
    ...
    /// plugin changed already gets. Matches the MCP fan-out in
    ...
    /// cached_token, ainxt.dev/oidc); the cheap pre-checks below absorb repeats
    ...
    ///    against `~/.ainxt/bundled/` and the manifest.
...
/// absent — matching the `ainxt_build_access_gate` flag's server-side default.
///
/// Used by both `enforce_ainxt_code_access` (initial login gate) and
