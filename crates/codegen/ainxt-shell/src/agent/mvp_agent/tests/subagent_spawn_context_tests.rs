...
async fn subagent_spawn_context_inherits_parent_permission_handle() {
    ...
    local
        .run_until(async {
            ...
            let cwd = ainxt_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp"))
                .expect("absolute cwd");
            let (permission_handle, _events_rx) =
                ainxt_workspace::permission::spawn_permission_manager(
                    sid.clone(),
                    gateway,
                    cwd,
                    ainxt_workspace::permission::types::ClientType::Generic,
            ...
            for access in [
                ainxt_workspace::permission::AccessKind::Read(Some(".env".into())),
                ainxt_workspace::permission::AccessKind::Bash("cat .env".into()),
            ] {
                ...
                assert!(
                    matches!(
                        decision,
                        ainxt_workspace::permission::Decision::PolicyDeny(_)
                    ),
