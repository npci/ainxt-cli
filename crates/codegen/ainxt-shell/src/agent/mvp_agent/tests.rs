...
/// genuinely needs to log in, so the fallthrough is interactive `ainxt.dev`.
#[tokio::test(flavor = "current_thread")]
#[serial_test::serial]
async fn cached_token_fallthrough_falls_to_ainxt_com_without_credentials() {
    use crate::agent::auth_method::{
        AINXT_COM_METHOD_ID, LEGACY_AINXT_API_KEY_ENV_VAR, AINXT_API_KEY_ENV_VAR,
    };
    use ainxt_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("AINXT_DISABLE_API_KEY_AUTH");
    ...
    assert_eq!(
        agent
            ...
            .map(|id| id.0.as_ref()),
        Some(AINXT_COM_METHOD_ID),
        "no API-key creds and no kill switch -> interactive ainxt.dev login",
    );
}
