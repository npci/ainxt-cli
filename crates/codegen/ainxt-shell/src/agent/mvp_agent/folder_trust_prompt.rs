...
//! (`ainxt.dev/folder_trust/request`) that asks a GUI client (ainxt-desktop) to decide
...
//! `ainxt.dev/folderTrust.interactive` AND the folder-trust feature flag is on AND the
//! verdict is [`ainxt_workspace::folder_trust::TrustOutcome::Prompt`]. No
...
//! `.ainxt/lsp.json` servers start on the NEXT session open (the durable grant
...
/// ACP `ainxt.dev/folder_trust/request` payload (agent → GUI client). Serialized as
...
/// ACP `ainxt.dev/folder_trust/request` response (GUI client → agent).
...
impl MvpAgent {
    /// Parse the `ainxt.dev/folderTrust.interactive` capability from an initialize
    /// request. Returns `false` if absent or not `true`. Matches
    ...
    pub(crate) fn parse_interactive_trust_capability(init: &acp::InitializeRequest) -> bool {
        init.client_capabilities
            .meta
            .as_ref()
            .and_then(|m| m.get("ainxt.dev/folderTrust"))
    ...
    /// `ainxt.dev/folderTrust.interactive` AND [`folder_trust::prompt_warranted`]
    ...
    pub(crate) fn maybe_spawn_interactive_trust_prompt(
        ...
        let key = ainxt_workspace::trust::workspace_key(cwd);
        ...
        let targets: Vec<ReloadTarget> = self
            ...
            .values()
            .filter(|h| {
                ainxt_workspace::trust::workspace_key(std::path::Path::new(&h.info.cwd)) == key
            })
            .map(|h| ReloadTarget {
        ...
        tokio::task::spawn_local(async move {
            ...
            let ext_request = acp::ExtRequest::new("ainxt.dev/folder_trust/request", raw_params.into());
...
struct ReloadAfterGrant<'a> {
    ...
    plugin_handle: &'a ainxt_agent::plugins::SharedPluginRegistryHandle,
    ...
    proxy_url: &'a str,
    compat: &'a ainxt_tools::types::CompatConfig,
...
async fn reload_project_servers_after_grant(ctx: ReloadAfterGrant<'_>) {
    ...
    for target in ctx.targets {
        ...
        // on the folder-trust verdict (matches `broadcast_plugin_registry_to_sessions`);
        ...
        // The session's OWN project hooks (`.ainxt/hooks`, `.cursor/hooks.json`),
...
mod tests {
    ...
    fn parse_interactive_trust_capability_present_and_true() {
        ...
        meta.insert(
            "ainxt.dev/folderTrust".to_string(),
