...
use super::*;
impl MvpAgent {
    /// Image-description model from gateway-sourced config. No bundled default:
    /// empty when unset (the feature falls back to the session model).
    ...
    /// Session-summary model from gateway-sourced config. No bundled default:
    /// empty when unset (`build_summary_client` falls back to the session model).
    ...
    pub(super) fn build_summary_client(
        ...
        // No bundled default: an unset session-summary model falls back to the
        // session (primary) model rather than a fabricated slug.
        let mut slug = self.resolve_session_summary_model();
        if slug.is_empty() {
            slug = primary.model.clone();
        }
    ...
    fn has_proxy_credentials(&self) -> bool {
        self.cfg.borrow().endpoints.deployment_key.is_some()
            || self.auth_manager.current_or_expired().is_some_and(|a| a.is_ainxt_auth())
    }
    ...
    pub(super) fn current_or_buffered_auth(&self) -> Option<crate::auth::AinxtAuth> {
        self.auth_manager
            .current()
            .or_else(|| {
    ...
    /// Requires feature flag AND ainxt authentication (OIDC or legacy WebLogin).
    ...
    /// must not block the ACP response (ainxt-desktop sends `initialize` immediately).
    ...
    /// plus user/marketplace walks stalled ainxt-desktop's first `initialize`),
    ...
    pub(super) fn ensure_telemetry_client(&self) {
        ...
        if !mode.is_disabled() {
            let Some(auth) = self
                ...
                .current()
                .filter(|a| {
                    a.is_ainxt_auth() || a.auth_mode == crate::auth::AuthMode::ApiKey
                }) else {
                return;
            };
            ...
            let (user_id, team_id) = if auth.is_ainxt_auth() {
                (Some(auth.user_id), auth.team_id)
            } else {
                (None, auth.team_id)
            };
            ainxt_telemetry::client::init_if_needed(
                cfg.telemetry.clone(),
                mode,
                user_id,
                team_id,
                ...
                ainxt_version::VERSION.to_owned(),
                subscription_tier,
    ...
    ///
    /// Used by the `ainxt.dev/commands/list` ext method and the
    ...
    /// Return the current AinxtAuth credentials, if authenticated and not expired.
    pub(crate) fn current_auth(&self) -> Option<crate::auth::AinxtAuth> {
    ...
    pub(crate) fn plugin_registry_handle(
        &self,
    ) -> &ainxt_agent::plugins::SharedPluginRegistryHandle {
    ...
    /// discovery on ainxt-desktop Windows).
    fn ensure_local_workspace_ops(
        &self,
    ) -> Result<ainxt_workspace::WorkspaceOps, acp::Error> {
        ...
        let workspace_identity = self
            ...
            .map(|a| match a.team_id.filter(|t| !t.is_empty()) {
                Some(team) => {
                    ainxt_workspace::WorkspaceIdentity::team(a.user_id, team)
                }
                None => {
                    ainxt_workspace::WorkspaceIdentity::new(
                        a.user_id,
                        a.principal_type,
                        a.principal_id,
                    )
            ...
            .unwrap_or_default();
        let ops = match ainxt_workspace::handle::WorkspaceHandle::new_minimal(
            cwd.to_path_buf(),
            workspace_identity,
            project_lsp_trusted,
        ) {
            Ok(handle) => ainxt_workspace::WorkspaceOps::local(handle),
            Err(e) => {
    ...
    ///     `ainxt.dev`, `oidc`) -- even if the in-memory token is currently
    ///     expired or missing.
    ///
    /// Returns `ApiKey` only when the auth method is BYOK (`ainxt.api_key`) or
    ...
    pub(crate) fn auth_type(&self) -> ainxt_chat_state::AuthType {
    ...
    /// When `cached_token` cannot proceed, prefer non-interactive `ainxt.api_key`
    /// iff `should_advertise_ainxt_api_key`; otherwise `ainxt.dev`. Returns `None`
    ...
    pub(super) fn cached_token_fallthrough_method_id(
        ...
        let preferred = self.cfg.borrow().ainxt_com_config.preferred_method;
        let id = auth_method::method_id_after_cached_token_unavailable(
            auth_method::should_advertise_ainxt_api_key(
                self.cfg.borrow().ainxt_com_config.api_key_auth_disabled(),
    ...
    /// `use_oauth` only when the target is interactive `ainxt.dev`. When
    ...
    pub(super) async fn authenticate_after_cached_token_unavailable(
        ...
        let meta = if method_id.0.as_ref() == auth_method::AINXT_COM_METHOD_ID {
    ...
    pub(super) async fn refresh_remote_settings(&self, auth: &crate::auth::AinxtAuth) {
        if !crate::util::config::resolve_remote_fetch_enabled() {
            tracing::debug!("post-auth settings refresh skipped: remote_fetch disabled");
            return;
        }
        let is_ainxt = auth.is_ainxt_auth();
        ...
        let (
            ...
            let ainxt_user_id = is_ainxt.then(|| user_id.clone());
            let ainxt_team_id = is_ainxt.then(|| team_id.clone()).flatten();
    ...
    pub(super) fn apply_polled_announcements(
        ...
        pre_fetch: Option<Vec<ainxt_announcements::RemoteAnnouncement>>,
    ) {
        let mut cfg = self.cfg.borrow_mut();
    ...
    /// funnels through here. Emits `ainxt.dev/announcements/update` and advances
    ...
    pub(super) fn emit_announcements(&self, mode: AnnouncementsPushMode) {
        ...
        let accepted = self
            ...
            .forward_fire_and_forget(
                acp::ExtNotification::new("ainxt.dev/announcements/update", params.into()),
            );
        if !accepted {
            return;
        }
    ...
    /// Next generation for an `ainxt.dev/announcements/update` push. Strictly
    ...
    pub(super) async fn send_model_auto_switched(
        ...
        if let Ok(params) = serde_json::value::to_raw_value(&notification) {
            let _ = self
                .gateway
                .ext_notification(
                    acp::ExtNotification::new("ainxt.dev/session_notification", params.into()),
                )
                .await;
        }
    }
    ...
    pub(crate) fn prepare_sampling_config_for_model(
        ...
        if matches!(preferred, Some(crate ::auth::PreferredAuthMethod::Oidc))
            ...
            && credentials.auth_type == ainxt_chat_state::AuthType::ApiKey
        {
            credentials.api_key = None;
            credentials.auth_type = ainxt_chat_state::AuthType::SessionToken;
        }
        ...
        if !has_session_key && credentials.auth_type == ainxt_chat_state::AuthType::ApiKey
    ...
    /// Whether the current session is a personal ainxt.dev account on a gated
    /// tier (free / X Basic). The image tools stay advertised to the model but
    /// are flagged tier-restricted so they short-circuit at call time with the
    /// SuperAinxt upsell prose (see `ImageGenConfig`/`VideoGenConfig`'s
    /// `tier_restricted`).
    ///
    ...
    /// authoritatively zero-limits image generation for free & X Basic (429), so this
    ...
    ///
    /// Matches the pager's cosmetic slash-command gate
    ...
    fn is_tier_restricted_capability(&self) -> bool {
        ...
        if !auth.is_ainxt_auth() || auth.team_id.is_some() {
            return false;
        }
        let tier = self
            .cfg
    ...
    ///
    /// Both BYOK and session (OAuth) users go direct to `ainxt_api_base_url`.
    ...
    pub(super) fn prepare_image_gen_config(
        &self,
    ) -> ainxt_tools::implementations::ainxt_build::image_gen::ImageGenConfig {
        ...
        let cfg = self.cfg.borrow();
        let base_url = cfg.endpoints.ainxt_api_base_url.clone();
        let version = cfg
            .client_version
            .clone()
            .unwrap_or_else(|| ainxt_version::VERSION.to_string());
        let alpha_test_key = cfg.endpoints.alpha_test_key.clone();
        let mut headers = indexmap::IndexMap::new();
        headers.insert("user-agent".to_string(), format!("ainxt-build/{version}"));
    ...
    pub(super) fn prepare_app_builder_deployer_config(
        &self,
    ) -> ainxt_tools::implementations::ainxt_build::deploy_app::AppBuilderDeployerConfig {
        use ainxt_tools::implementations::ainxt_build::deploy_app::AppBuilderDeployerConfig;
        AppBuilderDeployerConfig::Disabled
    }
    /// Build video generation config. Video tools call the ainxt API directly.
    pub(super) fn prepare_video_gen_config(
        &self,
    ) -> ainxt_tools::implementations::ainxt_build::video_gen::VideoGenConfig {
        use ainxt_tools::implementations::ainxt_build::video_gen::VideoGenConfig;
    ...
    ///
    /// Enabled gate: `disable_web_search` kill-switch > `AINXT_WEB_FETCH` env >
    /// remote settings `web_fetch_enabled` > default (false).
    ///
    /// Params resolution (TOML > env > remote settings > default):
    /// - `proxy_endpoint`: `[toolset.web_fetch] proxy_endpoint` > `AINXT_WEB_FETCH_PROXY` > remote settings > None
    /// - `allowed_domains`: `[toolset.web_fetch] allowed_domains` > remote settings > built-in defaults
    pub(super) fn prepare_web_fetch_config(
        &self,
    ) -> ainxt_tools::implementations::ainxt_build::web_fetch::WebFetchConfig {
        use ainxt_tools::implementations::ainxt_build::web_fetch::WebFetchConfig;
        let cfg = self.cfg.borrow();
    ...
    pub fn with_models(
        ...
        let has_ainxt_auth = auth_manager
            .current_or_expired()
            .is_some_and(|a| a.is_ainxt_auth());
        let relay_sync_enabled = tui_mode && relay_config_enabled && has_ainxt_auth;
        ...
        if relay_sync_enabled {
            tracing::info!("[ainxt] Relay sync: ENABLED");
        } else if tui_mode && relay_config_enabled && !has_ainxt_auth {
            tracing::info!(
                "[ainxt] Relay sync: DISABLED (no auth - run 'ainxt login' first)"
            );
        ...
        let instance = Self {
            ...
            plugin_registry_handle: ainxt_agent::plugins::SharedPluginRegistryHandle::new(
                None,
            ...
            monitor_event_buffer: ainxt_tools::implementations::ainxt_build::task::types::MonitorEventBuffer::default(),
        ...
        instance
            .auth_manager
            .configure_refresher(
                instance.cfg.borrow().ainxt_com_config.auth_provider_command.clone(),
    ...
    /// Handle `ainxt.dev/internal/evict_sessions` — the leader server tells us a
    ...
    pub async fn kill_background_task(
        ...
        session_id: &str,
        task_id: &str,
    ) -> Result<ainxt_tools::types::KillOutcome, String> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    /// `ainxt.dev/subagent/cancel`. Active/pending → cancelled (a finish follows);
    /// already-finished → its terminal status; unknown id → `NotFound`.
    pub fn cancel_subagent(
        &self,
        subagent_id: &str,
    ) -> ainxt_tools::implementations::ainxt_build::task::types::SubagentCancelOutcome {
    ...
    pub async fn list_tasks(
        &self,
        session_id: &str,
    ) -> Option<Vec<ainxt_tools::types::TaskSnapshot>> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    /// 2. Config file/env enables it ([relay] enabled or AINXT_RELAY_SYNC_ENABLED)
            ...
            while state_rx.changed().await.is_ok() {
                ...
                if let Ok(params) = serde_json::value::to_raw_value(&notification) {
                    let ext_notification = acp::ExtNotification::new(
                        "ainxt.dev/session_notification",
                        params.into(),
                    );
    ...
    /// Get hooks list for a session (for `ainxt.dev/hooks/list` extension).
    pub async fn list_hooks(
        &self,
        session_id: &acp::SessionId,
    ) -> Option<ainxt_hooks_plugins_types::HooksListResponse> {
        let handle = self.get_session_handle(session_id)?;
        handle.get_hooks_list().await
    }
    /// Execute a hooks management action (for `ainxt.dev/hooks/action`).
    pub async fn execute_hooks_action(
        &self,
        session_id: &acp::SessionId,
        action: ainxt_hooks_plugins_types::HooksAction,
    ) -> Option<ainxt_hooks_plugins_types::ActionOutcome> {
        if matches!(action, ainxt_hooks_plugins_types::HooksAction::Untrust)
            && let Some(cwd) = self.get_session_cwd(session_id)
        {
            self.interactive_trust_prompted
                .borrow_mut()
                .remove(&ainxt_workspace::trust::workspace_key(&cwd));
        }
        let handle = self.get_session_handle(session_id)?;
        handle.execute_hooks_action(action).await
    }
    /// Execute a plugins management action (for `ainxt.dev/plugins/action`).
    pub async fn execute_plugins_action(
        &self,
        session_id: &acp::SessionId,
        action: ainxt_hooks_plugins_types::PluginsAction,
    ) -> Option<ainxt_hooks_plugins_types::ActionOutcome> {
        let is_reload = matches!(action, ainxt_hooks_plugins_types::PluginsAction::Reload);
        let handle = self.get_session_handle(session_id)?;
        let outcome = handle.execute_plugins_action(action).await;
        let succeeded = matches!(
            outcome.as_ref().map(| o | & o.status),
            Some(ainxt_hooks_plugins_types::OutcomeStatus::Success)
        );
    ...
    /// Get a snapshot of the shared plugin registry (for `ainxt.dev/plugins/list`).
    pub fn plugin_registry_snapshot(
        &self,
    ) -> Option<std::sync::Arc<ainxt_agent::plugins::PluginRegistry>> {
    ...
    pub(super) fn trace_upload_config_snapshot(
        ...
        let auth_token = if cfg.endpoints.deployment_key.is_none() {
            self.auth_manager
                .current_or_expired()
                .filter(|auth| auth.is_ainxt_auth())
                .map(|auth| auth.key)
        } else {
            None
        };
    ...
    /// Build the `ainxt.dev/sessionConfig` and `ainxt.dev/sessionDetail` `_meta` values
    ...
    pub(super) fn session_config_meta(
        ...
        let detail = session_config::AinxtSessionDetail::build(
    ...
    pub(super) async fn upload_harness_trace_turns(
        ...
        model: &str,
        turns: Vec<Vec<ainxt_sampling_types::conversation::ConversationItem>>,
    ) {
    ...
    pub(super) async fn build_harness_trace_uploads(
        ...
        base: u64,
        turns: Vec<Vec<ainxt_sampling_types::conversation::ConversationItem>>,
    ) -> Vec<(PromptTraceContext, PromptMetadata, ainxt_chat_state::TurnCapture)> {
        ...
        for (offset, items) in turns.into_iter().enumerate() {
            ...
            let metadata = PromptMetadata {
                ...
                agent_type: None,
                shell_version: Some(ainxt_version::VERSION.to_string()),
                workspace_type: None,
                sandbox: local_sandbox_telemetry(),
            };
            let capture = ainxt_chat_state::TurnCapture {
                messages: items,
    ...
    pub(crate) async fn get_trace_context(
        ...
        let upload_method = match upload_method {
            ...
            None => {
                ...
                ainxt_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::TraceUploadSkipped {
        ...
        let queue = session_handle
            .upload_queue
            .get_or_init(|| {
                let ainxt_home = crate::util::ainxt_home::ainxt_home();
    ...
    /// 5. `AINXT_AGENT` env var.
    /// 6. Built-in default agent.
    ///
    /// `AINXT_AGENT` and an explicit `[agent] name` bypass step 1.
    /// Strict-harness classification is structural — see
    /// [`ainxt_agent::config::is_strict_harness_agent_type`].
    ///
    ...
    pub fn resolve_agent_definition(
        ...
        acp_agent_profile: Option<ainxt_agent::AgentDefinition>,
        model_agent_type: Option<&str>,
    ) -> ainxt_agent::AgentDefinition {
        use ainxt_agent::AgentDefinition;
        let ainxt_agent_env_set = std::env::var("AINXT_AGENT")
            .ok()
            .is_some_and(|s| !s.trim().is_empty());
        let config_agent_explicitly_set = agent_config.name.is_some();
        let model_requires_strict_harness = model_agent_type
            .is_some_and(ainxt_agent::config::is_strict_harness_agent_type);
        if !ainxt_agent_env_set && !config_agent_explicitly_set
            && model_requires_strict_harness && let Some(required) = model_agent_type
            && let Some(def) = ainxt_agent::discovery::by_name_in_cwd(required, cwd)
        {
            tracing::info!(
        ...
        if let Some(ref name) = agent_config.name {
            ...
            if let Some(def) = ainxt_agent::discovery::by_name_in_cwd(name, cwd) {
                return def;
            }
            tracing::warn!(
                agent_name = % name,
        ...
        let agent_name = std::env::var("AINXT_AGENT").ok();
        let resolved = match agent_name.as_deref() {
            Some("browser-use") | Some("browser_use") => AgentDefinition::browser_use(),
            Some("ainxt-build-concise") | Some("ainxt_build_concise") => {
                AgentDefinition::ainxt_build_concise()
            }
            Some(path) if std::path::Path::new(path).is_absolute() => {
                match AgentDefinition::from_file(path) {
                    ...
                    Err(e) => {
                        ...
                        AgentDefinition::ainxt_build_plan()
                    }
                }
            }
            Some(name) => {
                ainxt_agent::discovery::by_name_in_cwd(name, cwd)
                    .unwrap_or_else(AgentDefinition::ainxt_build_plan)
            }
            None => AgentDefinition::ainxt_build_plan(),
        };
        if !ainxt_agent_env_set && !config_agent_explicitly_set
            && model_requires_strict_harness && let Some(required) = model_agent_type
            && resolved.name != required
        {
            ...
            if let Some(def) = ainxt_agent::discovery::by_name_in_cwd(required, cwd) {
                return def;
            }
            tracing::warn!(
    ...
    pub(super) async fn spawn_and_register_session(
        ...
        let fs_notify_config = init
            ...
            .meta
            .as_ref()
            .and_then(|m| m.get("ainxt.dev/fs_notify"))
            .and_then(|v| {
                use crate::session::{ClientFsConfig, ClientFsMode};
                use ainxt_fsnotify::FsConfig;
        ...
        let fs: Arc<dyn ainxt_workspace::file_system::AsyncFileSystem> = if use_acp_fs {
        ...
        let hunk_plan = plan_hunk_tracking(
            init
                ...
                .meta
                .as_ref()
                .and_then(|m| m.get("ainxt.dev/hunkTracker"))
        ...
        let incremental_bash_output = init
            ...
            .meta
            .as_ref()
            .and_then(|m| m.get("ainxt.dev/incrementalBashOutput"))
            .and_then(|v| v.as_bool())
            .unwrap_or(false);
        let no_color = init
            .client_capabilities
            .meta
            .as_ref()
            .and_then(|m| m.get("ainxt.dev/bashOutputNoColor"))
            .and_then(|v| v.as_bool())
            .unwrap_or(false);
        ...
        let (hunk_tracker_handle, hunk_event_rx) = match hunk_plan.actor_mode {
            ...
            None => (ainxt_hunk_tracker::HunkTrackerHandle::noop(), None),
        };
        let has_ainxt_auth = self.auth_manager.current().is_some_and(|a| a.is_ainxt_auth());
        let loc_tracking_enabled = hunk_tracking_enabled && has_ainxt_auth
            && (self
                .cfg
                .borrow()
                .remote_settings
                ...
                .unwrap_or(false)
                || std::env::var("AINXT_LOC_TRACKING")
        ...
        let loc_aggregate_rx = match hunk_event_rx {
            Some((hunk_event_rx, loc_cancel)) if loc_tracking_enabled => {
                ...
                let loc_writer = ainxt_hunk_tracker::JsonlHunkRecordWriter::new(loc_path);
                let loc_ctx = ainxt_hunk_tracker::LocSinkContext {
                    session_id: session_info.id.0.to_string(),
                    agent_id: agent_id(),
                ...
                tokio::spawn(
                    ainxt_hunk_tracker::run_loc_sink(
                        hunk_event_rx,
                        loc_writer,
                        loc_ctx,
                        loc_cancel,
        ...
        let mut session_env = ainxt_workspace::permission::claude_settings::load_claude_env_with_project(
            cwd.as_path(),
            project_env_trusted,
        );
        let envrc = match preloaded_envrc {
            Some(env) => env,
            None => {
                ainxt_workspace::envrc::load_envrc_or_empty_when_trusted(
                    cwd.as_path(),
        ...
        let pinned_model: Option<(acp::ModelId, ModelEntry)> = match &agent_definition
            .model
        {
            ainxt_agent::config::ModelOverride::Override(id) => {
            ...
            ainxt_agent::config::ModelOverride::Inherit => None,
        };
        ...
        if lsp_tools_enabled && tool_ctx.lsp.is_none() {
            ...
            let (
                ...
                .unzip();
            let sourced = ainxt_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
            ...
            if servers.is_empty() {
                let user_path = ainxt_tools::util::ainxt_home::ainxt_home()
                    .join("lsp.json");
                let project_path = tool_ctx.cwd.as_path().join(".ainxt").join("lsp.json");
                tracing::warn!(
        ...
        let (mut handle, permission_events_rx, agent_system_prompt, session_thread) = {
            ...
            let credentials = ainxt_chat_state::Credentials {
            ...
            let attribution_callback: Option<
                ainxt_sampler::SharedAttributionCallback,
            > = Some(
            ...
            let agent_hook_registry_override = agent_definition
                ...
                .and_then(|hooks_config| {
                    ...
                    let (specs, errors) = ainxt_hooks::config::parse_hooks_from_value_with_dir(
                        &hooks_val,
                    ...
                    let git_root = ainxt_workspace::session::git::find_git_root_from_path(
                            cwd,
                        )
                        .ok();
            ...
            let git_head_changed = init
                ...
                .meta
                .as_ref()
                .and_then(|m| m.get("ainxt.dev/gitHeadChanged"))
                .and_then(|v| v.as_bool());
            ...
            spawn_session_on_thread(
                    ...
                    ainxt_agent::prompt::context::PromptAudience::Primary,
                    None,
                    None,
                    disable_web_search,
        ...
        if let Some(mut loc_rx) = loc_aggregate_rx {
            ...
            tokio::spawn(async move {
                while let Some(agg) = loc_rx.recv().await {
                    match agg {
                        ainxt_hunk_tracker::LocAggregate::LinesChanged {
                            author_type,
                            lines_added,
                            lines_removed,
                            file_path,
                            ...
                            let is_agent = author_type
                                == ainxt_hunk_tracker::AuthorType::Agent;
                            signals
                                .record_loc_change(
                                    is_agent,
                                    lines_added,
                        ...
                        ainxt_hunk_tracker::LocAggregate::LinesReverted {
