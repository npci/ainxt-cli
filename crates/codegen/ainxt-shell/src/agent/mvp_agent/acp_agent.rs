...
impl acp::Agent for MvpAgent {
    ...
    async fn initialize(
        ...
        ainxt_telemetry::unified_log::info("agent initialized", None, None);
        ...
        {
            let root = crate::util::ainxt_home::ainxt_home();
        ...
        CLEANUP_PERMISSIONS_ONCE
            .call_once(|| {
                tokio::task::spawn(
                    ainxt_workspace::permission::cleanup_stale_permission_state(
        ...
        ainxt_workspace::trust::migrate_legacy_hook_trust();
        ...
        if !self.tier_allowed.get() && let Some(auth) = self.auth_manager.current() {
            self.enforce_ainxt_code_access(&auth).await;
        }
        ...
        if client_type == ClientType::Generic {
            match client_identifier.as_deref() {
                Some("ainxt-web") => client_type = ClientType::AinxtWeb,
                Some("nebula") => client_type = ClientType::Nebula,
                Some("ainxt-code-extension") => client_type = ClientType::Extension,
                Some("ainxt-desktop") => client_type = ClientType::Desktop,
                _ => {}
            }
        }
        *self.client_type.borrow_mut() = client_type;
        ...
        tracing::info!(
            ...
            "code-nav capability initialized from initialize request; \
             index will start lazily on first ainxt.dev/code/* request if eligible"
        );
        ...
        *self.buffering_settings.borrow_mut() = buffering_settings;
        ...
        if !self.cfg.borrow().ainxt_com_config.api_key_auth_disabled()
            && auth_method::read_ainxt_api_key_env().is_err()
            && let Some(api_key) = crate::auth::read_api_key(
                &crate::util::ainxt_home::ainxt_home(),
            )
        {
            unsafe { std::env::set_var("AINXT_API_KEY", &api_key) };
            tracing::info!("auth: loaded API key from auth.json (ainxt::api_key scope)");
            ainxt_telemetry::unified_log::info(
                "auth: loaded API key from auth.json (ainxt::api_key scope)",
                None,
                None,
            );
        }
        ...
        let has_external_api_key = auth_method::should_advertise_ainxt_api_key(
            disable_api_key_auth,
        ...
        let (
            ...
            let cfg = self.cfg.borrow();
            let issuer = cfg.ainxt_com_config.oidc.as_ref().map(|o| o.issuer.clone());
            (
                cfg.ainxt_com_config.auth_provider_label.clone(),
                cfg.ainxt_com_config.auth_provider_command.is_some(),
                cfg.ainxt_com_config.oidc.is_some(),
                issuer,
            )
        };
        if has_enterprise_oidc {
            ...
            tracing::info!(
                ...
                "auth: advertising ainxt.dev auth method",
            );
        }
        let preferred_method = self.cfg.borrow().ainxt_com_config.preferred_method;
        ...
        ainxt_telemetry::unified_log::info(
            ...
            Some(
                serde_json::json!(
                    { "ainxt_home" : crate ::util::ainxt_home::ainxt_home().display()
        ...
        debug_assert!(
            ...
            Some(auth_method::AuthMethodKind::AinxtApiKey)),
            "BYOK invariant violated: ainxt.api_key MUST be auth_methods.first() \
            ...
            acp::InitializeResponse::new(acp::ProtocolVersion::V1)
                .agent_capabilities(
                    acp::AgentCapabilities::new()
                        ...
                        .meta(
                            serde_json::json!(
                                { "ainxt.dev/fs_notify" : true, "ainxt.dev/hooks" : { "blockingEvents"
                                : [ainxt_hooks::event::HookEventName::PreToolUse],
                                "decisions" : ["deny"], }, }
                            )
                                .as_object()
                                .cloned(),
                ...
                .meta({
                    let metadata = parse_json_object_env("AINXT_AGENT_METADATA");
                    serde_json::json!(
                        { "ainxtShell" : true, "defaultAuthMethodId" :
                        default_auth_method_id_wire, (ainxt_mcp::wire::MCP_SDK) :
                        ...
                        ainxt_version::VERSION, "agentId" : agent_id(),
    ...
    async fn authenticate(
        ...
        if let Some(preferred) = self.cfg.borrow().ainxt_com_config.preferred_method {
        ...
        match arguments.method_id.0.as_ref() {
            auth_method::AINXT_API_KEY_METHOD_ID => {
                if self.cfg.borrow().ainxt_com_config.api_key_auth_disabled() {
                ...
                if sampling_config.api_key.is_none() {
                    if let Ok(api_key) = auth_method::read_ainxt_api_key_env() {
                        ...
                        .any(|m| m.has_own_credentials())
                    {
                        ...
                        return Err(
                            acp::Error::auth_required()
                                .data(
                                    "Set AINXT_API_KEY or add api_key/env_key to config.toml.",
                                ),
                        );
                    }
                }
                ...
                log_event(ainxt_telemetry::events::Login {
            ...
            auth_method::CACHED_TOKEN_AUTH_METHOD_ID => {
                ...
                let pin_blocks_oidc_mint = matches!(
                    self.cfg.borrow().ainxt_com_config.preferred_method, Some(crate
            ...
            auth_method::AINXT_COM_METHOD_ID | auth_method::OIDC_METHOD_ID => {
                let ainxt_ctx = self.auth_manager.ainxt_com_config();
                let auth_meta = AuthRequestMeta::from_json(arguments.meta.as_ref());
                tracing::info!(
                ...
                let use_oidc = self.cfg.borrow().resolve_ainxt_oauth(cli_oauth);
                tracing::debug!(
                    ...
                    })?;
                {
                    ...
                    tracing::debug!(
                        "auth: ainxt.dev/oidc handler set api_key (SessionToken)"
                    );
                    ainxt_telemetry::unified_log::debug(
                        "auth: ainxt.dev/oidc handler set api_key (SessionToken)",
                        None,
    ...
    async fn new_session(
        ...
        ainxt_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::SessionStarted {
        ...
        if product_analytics || ainxt_telemetry::external::is_active() {
            ...
            let perm = if session_yolo_mode {
                ainxt_telemetry::enums::PermissionMode::AlwaysApprove
            } else if session_auto_mode
                && crate::util::config::auto_permission_mode_enabled_from_disk()
            {
                ainxt_telemetry::enums::PermissionMode::Auto
            } else {
                ainxt_telemetry::enums::PermissionMode::Ask
            };
            tokio::spawn(async move {
                let git = ainxt_telemetry::context::collect_git_context(&cwd_str);
                let ev = ainxt_telemetry::events::SessionNew {
                    session_id: sid,
                ...
                ainxt_telemetry::session_ctx::log_event_dual(product_analytics, ev);
            });
        }
        ...
        let (git_root, is_git_repo, discovery_failed) = match ainxt_workspace::session::git::discover_git_root(
            cwd.as_path(),
        ) {
        ...
        if let Some(obj) = meta.as_object_mut() {
            obj.insert("ainxt.dev/sessionConfig".to_string(), session_config_value);
            obj.insert("ainxt.dev/sessionDetail".to_string(), session_detail_value);
        }
        Ok(
    ...
    async fn load_session(
        ...
        let persist_data = arguments
            .meta
            .as_ref()
            .and_then(|m| m.get("ainxt.dev/persist"))
            .cloned();
            ...
            Some(s) => {
                match s.state {
                    crate::session::plan_mode::PlanModeState::Inactive => {
                        ainxt_telemetry::events::PlanModeState::Inactive
                    }
                    crate::session::plan_mode::PlanModeState::Pending => {
                        ainxt_telemetry::events::PlanModeState::Pending
                    }
                    crate::session::plan_mode::PlanModeState::Active
                    | crate::session::plan_mode::PlanModeState::ExitPending => {
                        ainxt_telemetry::events::PlanModeState::Active
                    }
                }
            }
            None => ainxt_telemetry::events::PlanModeState::Inactive,
        };
        ...
        let restore_code_requested = request_meta
            .as_ref()
            .and_then(|m| m.get("ainxt.dev/restore_code"))
            .and_then(|v| v.as_bool())
        ...
        if restore_code_requested && registry_client_for_restore.is_none() {
            ainxt_workspace::session::git::warn_registry_disabled_restore(
                session_id.0.as_ref(),
            );
        }
        let restore_checkout_allowed = ainxt_workspace::session::git::restore_code_checkout_allowed(
            cwd.as_path(),
        ...
        {
            tracing::warn!(
                target : ainxt_workspace::session::git::RESTORE_CODE_LOG, session_id =
                % session_id.0, supplied_cwd = % cwd.as_str(), persisted_cwd = % summary
                .info.cwd, target_sha = % target_sha,
                "restore_code: skipping session HEAD checkout — supplied cwd is neither a ainxt worktree nor the session's persisted cwd (refusing to detach the source repo)"
            );
            ainxt_telemetry::unified_log::warn(
        ...
        if restore_code_requested && restore_checkout_allowed
            && let Some(ref target_sha) = summary.head_commit
        {
            use ainxt_workspace::session::git::RestoreKind;
            let outcome = ainxt_workspace::session::git::checkout_session_commit(
                    cwd.as_path(),
                    target_sha,
                    true,
                    session_id.0.as_ref(),
        ...
        let load_envrc = {
            let skip_envrc = request_meta
                .as_ref()
                .and_then(|m| m.get("ainxt.dev/skip_envrc"))
                .and_then(|v| v.as_bool())
                .unwrap_or(false);
            if skip_envrc {
                false
        ...
        let preloaded_envrc = ainxt_workspace::envrc::load_envrc_or_empty_when_trusted(
            cwd.as_path(),
        ...
        let prompt_display_cwd = request_meta
            .as_ref()
            .and_then(|m| m.get("ainxt.dev/display_cwd"))
            .and_then(|v| v.as_str())
            .map(|s| s.to_string())
        ...
        let is_ainxt_build = persisted_model.0.starts_with("ainxt-build");
        let same_family_fallback = if is_ainxt_build {
            available.keys().find(|id| id.0.starts_with("ainxt-build")).cloned()
        } else {
            available.keys().find(|id| !id.0.starts_with("ainxt-build")).cloned()
        };
        ...
        if let Some(persist) = persist_data {
            response_meta_map.insert("ainxt.dev/persist".to_string(), persist);
        }
        ...
        if summary.head_commit.is_some() && let Some(ref cwd) = session_cwd
            && summary
                ...
                .as_deref()
                .is_none_or(|root| {
                    ainxt_workspace::session::git::find_git_root_from_path(
                ...
                })
        {
            ...
            let current_head = ainxt_workspace::session::git::git_cli(
                    cwd_path,
                    &["rev-parse", "HEAD"],
                )
                .await
                .ok();
            if let Some(divergence) = ainxt_workspace::session::git::detect_head_divergence(
            ...
            .and_then(|h| h.current_prompt_id.lock().ok().and_then(|g| g.clone()))
        {
            response_meta_map
                .insert(
                    "ainxt.dev/runningPromptId".to_string(),
        ...
        response_meta_map.insert("ainxt.dev/sessionConfig".to_string(), session_config_value);
        response_meta_map.insert("ainxt.dev/sessionDetail".to_string(), session_detail_value);
        ...
        if self.product_analytics_enabled() {
            log_event(ainxt_telemetry::events::SessionLoad {
                session_id: session_id.0.to_string(),
    ...
    async fn prompt(
        ...
        if let Some(meta) = arguments.meta.as_ref() {
            ainxt_file_utils::trace_context::link_current_span_to_meta(
        ...
        handle
            ...
            .send(SessionCommand::Prompt {
                ...
                verbatim,
                traceparent: ainxt_file_utils::trace_context::current_traceparent(),
                json_schema,
                send_now,
                respond_to: tx,
                persist_ack: None,
            ...
            self.gateway
                .forward_fire_and_forget(
                    acp::ExtNotification::new(
                        "ainxt.dev/session/prompt_complete",
                        params.into(),
                    ),
                );
        }
        ...
        match stop_result {
            Ok(turn_ok) => {
                ...
                let turn_messages: Option<ainxt_chat_state::TurnCapture> = {
                    let (tx, rx) = oneshot::channel();
                    if handle
                        .cmd_tx
                ...
                if let Some(ctx) = trace_context {
                        ...
                        && let Some(client) = self.session_registry_client()
                    {
                        ...
                        tokio::spawn(async move {
                            ...
                            if first_prompt.is_some() || summary.is_some() {
                                ...
                                tracing::debug!(
                                    session_id = % reg_req.owner_id, has_summary = upd_req
                                    .summary.is_some(), "session registry post-register update"
                                );
                                if let Err(e) = client
                                    .update(&reg_req.owner_id, &upd_req)
                                    .await
                                {
                                    tracing::warn!(
                                        error = % e,
                    ...
                    async fn advance_last_turn(
                        ...
                        let repo_head_at_end = ainxt_tty_utils::git_command()
                            .current_dir(&cwd)
                            .args(["rev-parse", "HEAD"])
                            .output()
                            .ok()
                    ...
                    }
                    {
                        ...
                        tokio::spawn(async move {
                            let head = ainxt_workspace::session::git::get_current_commit(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let branch = ainxt_workspace::session::git::get_branch(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let _ = cmd_tx
    ...
    async fn ext_method(
        ...
        if let Some(meta) = &request_meta {
            ainxt_file_utils::trace_context::link_current_span_to_meta(meta);
        }
        ...
        let result = match method.as_ref() {
            "ainxt.dev/getApiKey" | "ainxt.dev/setApiKey" => {
                crate::extensions::auth::handle(self, &args).await
            }
            "ainxt.dev/session/info" | "ainxt.dev/session/usage" | "ainxt.dev/session/close"
            | "ainxt.dev/session/list" | "ainxt.dev/sessions/list" => {
                crate::agent::handlers::session::handle(self, &args).await
            }
            "ainxt.dev/workspaces/list" => {
                crate::agent::handlers::workspaces::handle(self, &args).await
            }
            "ainxt.dev/session/updates" => {
                crate::extensions::session_updates::handle(&args, &self.gateway).await
            }
            "ainxt.dev/session/load_history" => {
            ...
            "ainxt.dev/session/resolve_local_for_worktree_resume"
            | "ainxt.dev/session/rehydrate" => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::worktree::handle(self, &ops, &args).await
            }
            "ainxt.dev/session/rename" | "ainxt.dev/session/delete"
            | "ainxt.dev/session/update_mcp_servers" | "ainxt.dev/session/fork"
            | "ainxt.dev/internal/reload_all_mcp_servers"
            | "ainxt.dev/internal/reload_project_mcp_servers" | "ainxt.dev/internal/reload_skills"
            | "ainxt.dev/internal/reload_models" | "ainxt.dev/internal/reload_models_cache"
            | "ainxt.dev/internal/auth_cleared" | "ainxt.dev/plugins/reload"
            | "ainxt.dev/commands/list" => {
                crate::extensions::session_admin::handle(self, &args).await
            }
            "ainxt.dev/session/repair" => crate::extensions::repair::handle(self, &args).await,
            "ainxt.dev/memory/flush" | "ainxt.dev/memory/rewrite" => {
            ...
            "ainxt.dev/interject" => crate::extensions::interject::handle(self, &args).await,
            "ainxt.dev/feedback" | "ainxt.dev/feedback/dismiss" | "ainxt.dev/btw" => {
                crate::extensions::feedback::handle(self, &args).await
            }
            "ainxt.dev/recap" => crate::extensions::recap::handle(self, &args).await,
            "ainxt.dev/cloud/terminate" => {
                crate::extensions::auth_gate::require_ainxt_auth(
                    &self.auth_manager,
                    "Authentication required",
                    "Run `ainxt login` to authenticate.",
                )?;
            ...
            "ainxt.dev/billing" => crate::extensions::billing::handle(self, &args).await,
            "ainxt.dev/auto-topup-rule" => {
                crate::extensions::billing::handle(self, &args).await
            }
            "ainxt.dev/share_session" => crate::extensions::share::handle(self, &args).await,
            "ainxt.dev/privacy/setCodingDataRetention" => {
                crate::extensions::privacy::handle(self, &args).await
            }
            "ainxt.dev/rollout/survey" => {
            ...
            "ainxt.dev/suggest" => crate::extensions::suggest::handle(self, &args).await,
            "ainxt.dev/suggestPrompt" => crate::extensions::suggest::handle(self, &args).await,
            s if s.starts_with("ainxt.dev/auth/") => {
                crate::extensions::auth::handle(self, &args).await
            }
            s if s.starts_with("ainxt.dev/session_summaries/") => {
                crate::agent::handlers::session::handle(self, &args).await
            }
            s if s.starts_with("ainxt.dev/git/worktree/") => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::worktree::handle(self, &ops, &args).await
            }
            s if s.starts_with("ainxt.dev/git/") => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::git::handle(self, &ops, &args).await
            }
            s if s.starts_with("ainxt.dev/compact_conversation") => {
                crate::extensions::memory::handle(self, &args).await
            }
            s if s.starts_with("ainxt.dev/plugins/") => {
                crate::extensions::plugins::handle(self, &args).await
            }
            s if s.starts_with("ainxt.dev/marketplace/") => {
                crate::extensions::marketplace::handle(self, &args).await
            }
            s if s.starts_with("ainxt.dev/hooks/") => {
                crate::extensions::hooks::handle(self, &args).await
            }
            s if s.starts_with("ainxt.dev/hunk-tracker/") => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::hunk_tracker::handle(self, &ops, &args).await
            }
            s if s.starts_with("ainxt.dev/pr/") => {
            ...
            s if s.starts_with("ainxt.dev/review") => {
                crate::extensions::feedback::handle(self, &args).await
            }
            s if s.starts_with("ainxt.dev/debug/") => {
                crate::extensions::debug::handle(self, &args).await
            }
            s if s.starts_with("ainxt.dev/rewind") => {
    ...
    async fn ext_notification(
        ...
        if args.method.as_ref() == "ainxt.dev/yolo_mode_changed"
        ...
        if args.method.as_ref() == "ainxt.dev/permissions/reset" {
        ...
        if args.method.as_ref() == "ainxt.dev/internal/evict_sessions" {
            self.handle_evict_sessions(&args.params).await;
        }
        if args.method.as_ref() == "ainxt.dev/toggle_plan_mode"
            && let Ok(params) = serde_json::from_str::<
                serde_json::Value,
            >(args.params.get())
        {
        ...
        if matches!(
            args.method.as_ref(), "ainxt.dev/queue/remove" | "ainxt.dev/queue/reorder" |
            "ainxt.dev/queue/clear" | "ainxt.dev/queue/edit" | "ainxt.dev/queue/interject"
        )
            && let Ok(params) = serde_json::from_str::<
                serde_json::Value,
            >(args.params.get())
        ...
        if args.method.as_ref() == "ainxt.dev/terminal/pty/input"
            && let Ok(params) = serde_json::from_str::<
                serde_json::Value,
            >(args.params.get())
        {
        ...
        if args.method.as_ref() == "_ainxt.dev/session/update" {
            if let Ok(notification) = serde_json::from_str::<
                SessionNotification,
            >(args.params.get()) {
                tracing::info!(
                    "Storing ainxt session notification: session_id={}", notification
                    .session_id.0
                );
                    ...
                    .get(&notification.session_id)
                {
                    let _ = handle
                        .cmd_tx
                        .send(crate::session::SessionCommand::AinxtSessionNotification {
                            notification,
                        });
                } else {
                    tracing::warn!(
                        "Received ainxt session notification for unknown session: {}",
                        notification.session_id.0
                    );
                }
            } else {
                tracing::warn!("Failed to parse ainxt session notification params");
            }
        }
        if args.method.as_ref() == "ainxt.dev/telemetry/non_git_decision" {
            ...
            >(args.params.get()) {
                ...
                ainxt_telemetry::session_ctx::log_event(ainxt_telemetry::events::NonGitDecisionEvent {
        ...
        if args.method.as_ref() == "ainxt.dev/telemetry/multi_agent_followup" {
            #[derive(serde::Deserialize)]
            ...
            >(args.params.get()) {
                ...
                ainxt_telemetry::session_ctx::log_event(ainxt_telemetry::events::MultiAgentFollowup {
                    ...
                    other_agents: params
                        .other_agents
                        .into_iter()
                        .map(|(l, s, m)| ainxt_telemetry::events::AgentInfo {
                            label: l.to_string(),
                            session_id: s,
                            model_id: m,
                        })
        ...
        if args.method.as_ref() == "ainxt.dev/telemetry/multi_agent_apply" {
            #[derive(serde::Deserialize)]
            ...
            >(args.params.get()) {
                ...
                ainxt_telemetry::session_ctx::log_event(ainxt_telemetry::events::MultiAgentApply {
        ...
        if args.method.as_ref() == "ainxt.dev/telemetry/multi_agent_discard" {
            #[derive(serde::Deserialize)]
            ...
            >(args.params.get()) {
                ...
                ainxt_telemetry::session_ctx::log_event(ainxt_telemetry::events::MultiAgentDiscard {
                    discarded_agents: params
                        .discarded_agents
                        .into_iter()
                        .map(|(l, s, m)| ainxt_telemetry::events::AgentInfo {
        ...
        if args.method.as_ref() == ainxt_telemetry::unified_log::LOG_METHOD
            && let Ok(params) = serde_json::from_str::<
                ainxt_telemetry::unified_log::LogNotificationParams,
            >(args.params.get())
        {
            ainxt_telemetry::unified_log::ingest_client_entries(
                params.src,
                &params.entries,
            );
        }
