...
impl MvpAgent {
    ...
    /// session end — a terminal/explicit close (`ainxt.dev/session/close`). It must
    ...
    /// (`ainxt.dev/session/close`). Finalizes the cloud replica (genuine session
    ...
    /// `ainxt.dev/sessions/changed` notification with the session in `removed` so
    ...
    /// `ainxt.dev/sessions/changed` notification with the current entry in
    ...
    /// Emit an `ainxt.dev/sessions/changed` upsert for a resident session with an
    ...
    /// Fan an `ainxt.dev/sessions/changed` delta out to every attached client.
    ///
    ...
    /// the `ainxt.dev/sessions/changed` special-case in `leader/server.rs`).
