...
use crate::auth::{AuthManager, AinxtAuth, AinxtComConfig};
use crate::remote::{FetchModelsResult, fetch_models_blocking};
use crate::sampling::SamplerConfig as SamplingConfig;
use globset::{Glob, GlobSet, GlobSetBuilder};
use ainxt_sampling_types::{ReasoningEffort, ReasoningEffortOption};

/// User-facing error when the gateway supplies no models. There are no bundled
/// defaults, so an empty gateway catalog is a hard failure for interactive boot.
pub(crate) const NO_MODELS_ERROR: &str =
    "No models available from the ainxt gateway. Check your connection and run `ainxt login`.";
...
impl ModelFetchAuth {
    ...
    ///
    /// A `deployment_key` outranks an ambient `AINXT_API_KEY` so a stray env key
    ...
    pub(crate) fn resolve(endpoints: &config::EndpointsConfig, has_cached_session: bool) -> Self {
        if endpoints.has_custom_endpoint() {
            ...
            Self::Deployment
        } else if crate::agent::auth_method::has_ainxt_api_key_env() {
            Self::ApiKey
        } else {
            Self::Session
        }
...
struct Inner {
    ...
    gateway: RwLock<Option<ainxt_acp_lib::AcpAgentGatewaySender>>,
...
impl Default for ModelsManager {
    fn default() -> Self {
        ...
        let auth_manager = Arc::new(AuthManager::new(&ainxt_home, AinxtComConfig::default()));
        Self::new(
            None,
            IndexMap::new(),
...
impl ModelsManager {
    ...
    pub fn from_config(
        ...
        let current_model_id = match resolve_default_model(cfg, &catalog, is_session_auth) {
            Some((current_model_key, current_model, model_source)) => {
                tracing::info!(
                    model_id = %current_model.model,
                    source = %model_source,
                    "default model resolved"
                );
                acp::ModelId::new(Arc::from(current_model_key))
            }
            None if has_prefetched => {
                // A real catalog fetch happened but yielded nothing selectable.
                // The catalog is gateway-only: surface a clear error instead of
                // inventing a bundled default model.
                return Err(NO_MODELS_ERROR.to_string());
            }
            None => {
                // No real fetch yet (first run defers to the async fetch). Leave
                // the current model unset; `reselect_default_model` picks one
                // once the gateway catalog arrives.
                tracing::info!(
                    "no model catalog yet; deferring default selection to async fetch"
                );
                acp::ModelId::new(Arc::from(String::new()))
            }
        };
    ...
    pub(crate) fn set_gateway(&self, gateway: ainxt_acp_lib::AcpAgentGatewaySender) {
    ...
    pub fn apply_config(&self, new_config: config::Config) {
        ...
        *self.inner.models.write() = new_catalog;
        ...
        // Push the new catalog to connected clients (`ainxt.dev/models/update`).
    ...
    pub fn model_compactions_remaining(
        &self,
        model_id: &str,
    ) -> Option<ainxt_sampling_types::CompactionsRemaining> {
        self.inner
            .models
            .read()
            .get(model_id)
    ...
    pub fn model_compaction_at_tokens(
        &self,
        model_id: &str,
    ) -> Option<ainxt_sampling_types::CompactionAtTokens> {
        self.inner
            .models
            .read()
            .get(model_id)
    ...
    ///
    /// `model_id` may be a routing slug (`config.model`, e.g. `ainxt-4.5`)
    /// OR a catalog key; the catalog map is keyed by the config key, which can
    /// differ from the slug for custom/enterprise ids (e.g. key `enterprise-ainxt-build`
    /// → slug `ainxt-4.5`). Resolve to the catalog key first so a slug
    ...
    /// Hot-reload the catalog from `~/.ainxt/models_cache.json` after an
    ...
    /// When another ainxt process sharing `~/.ainxt` (a `--no-leader` run, a
    /// newer client, ainxt-desktop) fetches a fresher `/v1/models` catalog and
    ...
    /// `ModelsCacheManager` path is fixed to `ainxt_home()`, a process-wide
    /// `OnceLock`).
    fn reload_from_cache_manager(&self, cache: &ModelsCacheManager) {
        ...
        // Capture whether this is the first real catalog (matches
        ...
        *self.inner.etag.write() = cached.etag;
        ...
        // Recompute the prompt-block flag (matches `apply_refresh_result`) so
    ...
    pub fn sampling_config(&self) -> SamplingConfig {
            ...
            .or_else(|| all_models.values().next())
        {
            Some(m) => m,
            None => {
                // No bundled default exists: the catalog is gateway-only. If it
                // is empty here, sampling was reached before a real catalog
                // arrived. Build a minimal entry from the current model id (the
                // gateway slug) rather than inventing one; an empty id will
                // fail the request loudly instead of routing to a fake model.
                tracing::error!(
                    "no models available in catalog; the ainxt gateway returned no models"
                );
                fallback = ModelEntry::fallback(current_model_id.0.as_ref(), &config.endpoints);
                &fallback
            }
        };
    ...
    fn reselect_current_model_if_missing(&self, config: &config::Config) {
        ...
        let Some((key, _, source)) = resolved else {
            // Empty gateway catalog: leave the current selection alone rather
            // than crash. The picker stays empty and callers log the error.
            tracing::error!("no models available from the ainxt gateway; keeping current selection");
            return;
        };
...
impl ModelsCacheManager {
    fn new() -> Self {
        Self {
            path: crate::util::ainxt_home::ainxt_home().join(MODELS_CACHE_FILE),
            ttl: CACHE_TTL,
        }
    }
    ...
    fn load_fresh(
        ...
        if cache.ainxt_version.as_deref() != Some(ainxt_version::VERSION) {
    ...
    fn persist(
        ...
        let cache = ModelsCache {
            fetched_at: Utc::now(),
            ainxt_version: Some(ainxt_version::VERSION.to_string()),
...
/// routing slug (e.g. "Auto" and "ainxt" both route to `ainxt-build`)
...
pub(crate) fn prefetch_models_and_settings_blocking(
    ...
    // Settings need a ainxt.dev session; skip for BYOK.
    ...
    match fetch_models_blocking(endpoints, auth, fetch_auth) {
        Ok(FetchModelsResult { models, etag }) if !models.is_empty() => {
            let api_base_url_override = match fetch_auth {
                ModelFetchAuth::ApiKey => Some(endpoints.ainxt_api_base_url.clone()),
                _ => None,
            };
...
fn resolve_prefetch_env_with_auth(auth: Option<AinxtAuth>) -> Option<PrefetchEnv> {
    let _timer = crate::instrumentation_timer!("startup.early_prefetch_launch");
    // Config-aware (not env-only) so the prefetch can't leak the bearer to api.ainxt.dev.
...
/// explicit off switch must hold even when a stray login, `AINXT_API_KEY`, or
...
fn resolve_prefetch_env(ainxt_com_config: Option<AinxtComConfig>) -> Option<PrefetchEnv> {
    let ainxt_home = crate::util::ainxt_home::ainxt_home();
    let auth_manager = AuthManager::new(&ainxt_home, ainxt_com_config.unwrap_or_default());
...
pub fn start_early_prefetch_with_auth(auth: Option<AinxtAuth>) -> Option<EarlyPrefetchHandle> {
...
pub fn start_early_prefetch(ainxt_com_config: Option<AinxtComConfig>) -> Option<EarlyPrefetchHandle> {
    let env = resolve_prefetch_env(ainxt_com_config)?;
    Some(spawn_prefetch_thread(env))
}
...
///
/// Sessions persist the routing slug (`[model.X].model`, e.g. `ainxt-4.5`);
/// the catalog and `/model` picker use config keys (e.g. `enterprise-ainxt-build`).
...
/// Pick the default model: CLI > env > config > remote-settings hint.
///
/// Returns `None` when the catalog has no selectable model — there is NO
/// bundled fallback. Callers (bootstrap) surface a clear "no models from the
/// gateway" error; long-running managers leave the current selection alone.
pub(crate) fn resolve_default_model(
    cfg: &config::Config,
    catalog: &IndexMap<String, ModelEntry>,
    is_session_auth: bool,
) -> Option<(String, ModelEntry, config::ConfigSource)> {
    ...
    // First selectable model, preferring auth-visible entries. Returns `None`
    // when nothing is selectable — there is no bundled fallback model.
    let first_or_fallback = || -> Option<(String, ModelEntry)> {
        if let Some((key, first)) = visible.first() {
            return Some((key.clone(), first.clone()));
        }
        ...
        tracing::warn!(
            "no selectable models available from the ainxt gateway; catalog is empty"
        );
        None
    };
    ...
    match &model_pref {
        None => {
            ...
            Some((key, first, config::ConfigSource::Default))
        }
        Some(pref) => {
            let found = visible
            ...
            if let Some((key, entry)) = found {
                Some((key.clone(), entry.clone(), pref.source))
            } else {
                        ...
                        .or_else(|| visible.iter().find(|(_, m)| m.model == prev))
                {
                    ...
                    return Some((key.clone(), entry.clone(), config::ConfigSource::Config));
                }
                let (key, first) = first_or_fallback()?;
                Some((key, first, config::ConfigSource::Default))
            }
...
pub fn resolve_model_catalog(
    ...
    // must not stamp `none` onto ainxt-4.5, which only offers low/medium/high).
...
mod tests {
    ...
    fn test_manager() -> ModelsManager {
        ...
        let tmp = std::env::temp_dir().join("ainxt-test-models-manager");
        let auth_manager = Arc::new(AuthManager::new(&tmp, AinxtComConfig::default()));
        ModelsManager::new(
            None,
            IndexMap::new(),
            acp::ModelId::new("default"),
    ...
    fn default_model_honors_allowlist_when_no_default_set() {
        ...
        let (_key, entry, _src) = resolve_default_model(&cfg, &catalog, true)
            .expect("test catalog has a selectable model");
        assert!(
            entry.info.user_selectable,
    ...
    async fn set_current_model_id_change_fires_watch_to_all_subscribers() {
        ...
        mgr.set_current_model_id(acp::ModelId::new("ainxt-4"));
    ...
    async fn model_switch_generation_snapshot_reflects_current_state() {
        ...
        mgr.set_current_model_id(acp::ModelId::new("ainxt-3"));
    ...
    fn current_reasoning_effort_seeded_from_config() {
        let tmp = std::env::temp_dir().join("ainxt-test-models-manager-seed");
        let auth_manager = Arc::new(AuthManager::new(&tmp, AinxtComConfig::default()));
    ...
    fn reasoning_effort_override_skips_models_that_do_not_offer_level() {
        use indexmap::IndexMap;
        use ainxt_sampling_types::ReasoningEffortOption;
        ...
        let mut no_none = ModelEntry {
            info: config::ModelInfo::fallback("ainxt-4.5"),
            api_key: None,
            env_key: None,
            api_base_url: None,
        };
        ...
        prefetched.insert("ainxt-4.5".to_string(), no_none);
        ...
        let catalog = resolve_model_catalog(&cfg, Some(prefetched));
        assert_eq!(
            catalog["ainxt-4.5"].info.reasoning_effort,
            Some(ReasoningEffort::High),
    ...
    fn config_menu_only_model_derives_support_and_default() {
        ...
        let tmp = std::env::temp_dir().join("ainxt-test-models-manager-menu-only");
        let auth_manager = Arc::new(AuthManager::new(&tmp, AinxtComConfig::default()));
        let mgr = ModelsManager::new(
            None,
            catalog,
    ...
    fn first_apply_refresh_reselects_default_model() {
        let mgr = test_manager();
        let mut cfg = config::Config::default();
        cfg.models.default = Some("ainxt-3".to_string());

        assert!(!mgr.has_fetched_real_catalog());

        let prefetched = make_prefetched(&["ainxt-3", "ainxt-4"]);
        mgr.apply_refresh_result(&cfg, Some(prefetched), None);

        assert!(mgr.has_fetched_real_catalog());
        assert_eq!(mgr.current_model_id().0.as_ref(), "ainxt-3");
    }
    ...
    fn subsequent_refresh_reselects_when_model_removed() {
        ...
        // Second refresh with ainxt-4 removed.
        let prefetched = make_prefetched(&["ainxt-3", "ainxt-4.5"]);
        mgr.apply_refresh_result(&cfg, Some(prefetched), None);
    ...
    fn apply_config_honors_new_preferred_model() {
        ...
        *mgr.inner.cfg.write() = stale_cfg;
        ...
        new_cfg.models.default = Some("ainxt-3".to_string());
    ...
    fn apply_config_falls_back_when_preferred_not_in_catalog() {
        ...
        let mut new_cfg = config::Config::default();
        new_cfg.models.default = Some("ainxt-nonexistent".to_string());
        mgr.apply_config(new_cfg);
    ...
    fn auth_refresh_then_config_reload_preserves_user_model() {
        ...
        *mgr.inner.etag.write() = None;
        ...
        let prefetched = make_prefetched(&["ainxt-3", "ainxt-4"]);
        mgr.apply_refresh_result(&cfg, Some(prefetched), None);
        assert_eq!(mgr.current_model_id().0.as_ref(), "ainxt-4");

        // Config reload with persisted preference.
        let mut new_cfg = config::Config::default();
        new_cfg.models.default = Some("ainxt-4".to_string());
        mgr.apply_config(new_cfg);
        assert_eq!(mgr.current_model_id().0.as_ref(), "ainxt-4");
    }
    ...
    fn reload_from_disk_cache_applies_external_catalog() {
        ...
        cache.persist(
            &make_prefetched(&["ainxt-4.5", "ainxt-4.3"]),
            Some("etag-ext"),
            auth_method,
            &mgr.cache_origin(),
        );
        ...
        assert!(mgr.has_fetched_real_catalog());
        assert!(mgr.models().contains_key("ainxt-4.5"));
        assert!(mgr.models().contains_key("ainxt-4.3"));
    ...
    fn reload_from_disk_cache_ignores_stale_cache() {
        ...
        let stale = ModelsCache {
            ...
            models: make_prefetched(&["ainxt-stale"]),
        };
        cache.atomic_write(&stale);

        mgr.reload_from_cache_manager(&cache);

        assert!(!mgr.models().contains_key("ainxt-stale"));
    ...
    fn reload_from_disk_cache_ignores_auth_method_mismatch() {
        ...
        cache.persist(
            &make_prefetched(&["ainxt-other-auth"]),
            Some("etag-x"),
            other,
            &mgr.cache_origin(),
        );
        ...
        assert!(!mgr.models().contains_key("ainxt-other-auth"));
    }
    ...
    fn reload_from_disk_cache_ignores_origin_mismatch() {
        ...
        let auth_method = mgr.inner.fetch_auth.read().cache_auth_method();
        cache.persist(
            &make_prefetched(&["ainxt-other-origin"]),
            Some("etag-y"),
            auth_method,
        ...
        assert!(!mgr.models().contains_key("ainxt-other-origin"));
        assert!(mgr.inner.etag.read().is_none());
    }
    ...
    fn reload_from_disk_cache_ignores_legacy_cache_without_origin() {
        ...
        let legacy = ModelsCache {
            ...
            models: make_prefetched(&["ainxt-legacy"]),
        };
        cache.atomic_write(&legacy);

        mgr.reload_from_cache_manager(&cache);

        assert!(!mgr.models().contains_key("ainxt-legacy"));
    }
    ...
    fn clear_resets_has_fetched_real_catalog() {
        ...
        let prefetched = make_prefetched(&["ainxt-4.5", "ainxt-4.3"]);
        mgr.apply_refresh_result(&cfg, Some(prefetched), None);
        let first_available = mgr.available().keys().next().unwrap().clone();
        assert_eq!(
            mgr.current_model_id().0.as_ref(),
    ...
    fn resolve_custom_endpoint_always_wins() {
        let _key = EnvGuard::set("AINXT_API_KEY", "test-key");
    ...
    fn resolve_falls_back_to_session_when_nothing_set() {
        let _unset = EnvGuard::unset("AINXT_API_KEY");
        let _unset_legacy = EnvGuard::unset("AINXT_CODE_AINXT_API_KEY");
        let endpoints = config::EndpointsConfig::default();
        assert_eq!(
            ModelFetchAuth::resolve(&endpoints, false),
            ModelFetchAuth::Session,
    ...
    /// `deployment_key` outranks a stray `AINXT_API_KEY`, but session wins over both.
    #[test]
    #[serial]
    fn resolve_deployment_key_outranks_ambient_api_key() {
        let _key = EnvGuard::set("AINXT_API_KEY", "stray-env-key");
        let endpoints = config::EndpointsConfig {
            deployment_key: Some("deploy-key".to_owned()),
            ..config::EndpointsConfig::default()
        };
        assert_eq!(
            ModelFetchAuth::resolve(&endpoints, false),
            ModelFetchAuth::Deployment,
            "managed deployment_key should outrank an ambient AINXT_API_KEY",
        );
        assert_eq!(
            ModelFetchAuth::resolve(&endpoints, true),
            ModelFetchAuth::Session,
    ...
    /// once — session auth, ambient `AINXT_API_KEY`, `deployment_key`, AND a
    ...
    fn build_prefetched_map_distinct_ids_same_slug() {
        let entries = vec![
            make_entry_config_with_id(Some("auto"), "ainxt-build", Some("Auto")),
            make_entry_config_with_id(Some("ainxt-build"), "ainxt-build", Some("ainxt")),
            make_entry_config_with_id(
                Some("ainxt-composer-2.5-fast"),
                "ainxt-composer-2.5-fast",
                Some("ainxt Fast"),
            ),
        ];
        ...
        assert!(map.contains_key("ainxt-build"));
        assert!(map.contains_key("ainxt-composer-2.5-fast"));
        assert_eq!(
            map["auto"].info.model, "ainxt-build",
            "auto entry should still route to ainxt-build"
        );
        assert_eq!(map["ainxt-build"].info.model, "ainxt-build");
    }
    ...
    fn build_prefetched_map_duplicate_id_overwrites() {
        let entries = vec![
            make_entry_config_with_id(Some("ainxt-build"), "ainxt-build", Some("First")),
            make_entry_config_with_id(Some("ainxt-build"), "ainxt-build", Some("Second")),
        ];
        let map = build_prefetched_map(entries, None);

        assert_eq!(map.len(), 1, "duplicate id: second overwrites first");
        assert_eq!(map["ainxt-build"].info.name.as_deref(), Some("Second"));
    }
    ...
    fn resolve_default_model_prefers_id_over_model_slug() {
        ...
        catalog.insert(
            "auto-ainxt-build".to_string(),
            make_model_entry("ainxt-build"),
        );
        catalog.insert("ainxt-build".to_string(), make_model_entry("ainxt-build"));

        let mut cfg = config::Config::default();
        cfg.models.default = Some("ainxt-build".to_string());

        let (key, _, _) = resolve_default_model(&cfg, &catalog, true)
            .expect("test catalog has a selectable model");
        assert_eq!(key, "ainxt-build", "must match id, not first slug hit");
    }
    ...
    fn resolve_catalog_key_maps_routing_slug_to_config_key() {
        ...
        models.insert(
            "enterprise-ainxt-build".to_string(),
            make_model_entry("ainxt-4.5"),
        );
        models.insert("ainxt-4.3".to_string(), make_model_entry("ainxt-4.3"));

        let persisted = acp::ModelId::new("ainxt-4.5");
        let key = resolve_catalog_key(&models, &persisted).expect("slug must resolve");
        assert_eq!(key.0.as_ref(), "enterprise-ainxt-build");
    }

    #[test]
    fn resolve_catalog_key_prefers_exact_key_match() {
        let mut models = IndexMap::new();
        models.insert("ainxt-4.5".to_string(), make_model_entry("ainxt-4.5"));

        let persisted = acp::ModelId::new("ainxt-4.5");
        let key = resolve_catalog_key(&models, &persisted).expect("exact key must resolve");
        assert_eq!(key.0.as_ref(), "ainxt-4.5");
    }
    ...
    fn resolve_catalog_key_last_slug_match_wins() {
        let mut models = IndexMap::new();
        models.insert(
            "default-ainxt-build".to_string(),
            make_model_entry("ainxt-4.5"),
        );
        models.insert("user-ainxt-build".to_string(), make_model_entry("ainxt-4.5"));

        let persisted = acp::ModelId::new("ainxt-4.5");
        let key = resolve_catalog_key(&models, &persisted).expect("slug must resolve");
        assert_eq!(key.0.as_ref(), "user-ainxt-build");
    }
    ...
    fn selectable_prefers_available_identity_over_non_selectable_exact_key() {
        let mut models = IndexMap::new();
        models.insert("ainxt-build".to_string(), make_model_entry("ainxt-build"));
        models.insert(
            "enterprise-ainxt-build".to_string(),
            make_model_entry("ainxt-build"),
        );
        ...
        let available = test_available_keys(&["enterprise-ainxt-build", "ainxt-4.3"]);

        let persisted = acp::ModelId::new("ainxt-build");
        assert_eq!(
    ...
    fn selectable_prefers_exact_key_over_later_slug_match() {
        let mut models = IndexMap::new();
        models.insert("ainxt-build".to_string(), make_model_entry("ainxt-4.5"));
        models.insert("other".to_string(), make_model_entry("ainxt-build"));

        let available = test_available_keys(&["ainxt-build", "other"]);

        let persisted = acp::ModelId::new("ainxt-build");
        let key = selectable_catalog_key_for_persisted(&models, &available, &persisted)
            .expect("exact selectable key must win");
        assert_eq!(key.0.as_ref(), "ainxt-build");
    }
