//! ainxt.dev chat-product model catalog: caches `/rest/modes` and maps modes to
...
/// ~54 min, matching ainxt-web's refetch cadence.
...
pub const AINXT_CHAT_MODE_ENV: &str = "AINXT_CHAT_MODE";
...
impl ChatModesManager {
    ...
    /// The active ainxt.dev identity, or `None` when unauthenticated. Modes are
...
/// Maps ainxt.dev modes → `SessionModelState`: keeps only `available` modes,
...
mod tests {
    ...
    fn name_falls_back_to_id_when_title_blank() {
        ...
        let state = modes_to_model_state(&resp);
        assert_eq!(state.available_models[0].name, "ainxt-4.5");
    }
}
