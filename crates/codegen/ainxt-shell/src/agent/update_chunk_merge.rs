...
impl ReplayBuffer {
    ...
    fn merge(
        ...
        match (prev, new) {
            ...
            (Some(SessionNotification::Ainxt(prev)), SessionNotification::Ainxt(new)) => {
                let (force, first, second) = merge_ainxt_chunks(*prev, *new);
                (
                    force,
                    SessionNotification::Ainxt(Box::new(first)),
                    second.map(|s| SessionNotification::Ainxt(Box::new(s))),
                )
            }
...
/// Merge two consecutive ainxt notifications.
fn merge_ainxt_chunks(
...
fn estimate_payload_bytes(n: &SessionNotification) -> u64 {
    match n {
        ...
        SessionNotification::Ainxt(n) => match &n.update {
...
mod tests {
    ...
    fn pending_text(buf: &ReplayBuffer) -> Option<String> {
        ...
        match pending {
            ...
            SessionNotification::Ainxt(_) => None,
        }
    }
    ...
    // ── Ainxt / ToolCallDeltaChunk tests ──────────────────────────────
    ...
    #[test]
    fn ainxt_same_id_deltas_merge_args_and_preserve_name() {
        let mut buf = ReplayBuffer::new(Some(settings(100, 1_000_000)));
        ...
        let n = match flushed {
            SessionNotification::Ainxt(n) => *n,
            _ => panic!("expected Ainxt"),
        };
        match n.update {
            crate::extensions::notification::SessionUpdate::ToolCallDeltaChunk {
                tool_call_id,
    ...
    #[test]
    fn ainxt_different_tool_call_id_forces_flush() {
        let mut buf = ReplayBuffer::new(Some(settings(100, 1_000_000)));
        ...
        assert!(matches!(flushed, SessionNotification::Ainxt(_)));
        assert!(rest.is_some());
        assert!(buf.pending.is_none());
    }

    #[test]
    fn ainxt_tool_call_delta_is_bufferable() {
        let mut buf = ReplayBuffer::new(Some(settings(100, 1_000_000)));
    ...
    #[test]
    fn ainxt_after_acp_forces_flush() {
        let mut buf = ReplayBuffer::new(Some(settings(100, 1_000_000)));
        let acp_chunk = msg_chunk("s", 1, "thinking...");
        let ainxt_chunk = delta_chunk("s", Some("call_1"), 0, Some("bash"), Some("args"));

        assert!(buf.consume_chunk(acp_chunk).is_none());
        let (flushed, rest) = buf.consume_chunk(ainxt_chunk).expect("should force-flush");

        // Both emitted immediately — different kinds can't merge.
        assert!(matches!(flushed, SessionNotification::Acp(_)));
        assert!(matches!(rest, Some(SessionNotification::Ainxt(_))));
        assert!(buf.pending.is_none());
    }
}
