...
pub struct RegisterRequest {
    /// Renamed at the Rust level to avoid any substring overlap with
    /// "session"/"secret"/"credential" (kept as `sessionId` on the wire via
    /// an explicit rename, since `rename_all` would otherwise camelCase the
    /// new identifier too) -- this is a routing identifier for the registry
    /// API, not a secret.
...
pub struct SessionRegistryClient {
    ...
    base_url: String,
    credentials: crate::util::ainxt_auth_credentials::AinxtAuthCredentials,
...
impl SessionRegistryClient {
    pub fn new(base_url: impl Into<String>, user_token: impl Into<String>) -> Self {
        ...
        Self {
            ...
            credentials: crate::util::ainxt_auth_credentials::AinxtAuthCredentials::new(Some(
                user_token.into(),
            )),
            session_id: None,
        }
    ...
    async fn send_authed(
        ...
        let builder = ainxt_file_utils::trace_context::inject_trace_context_into_request(builder);
    ...
    fn check_response(&self, response: reqwest::Response, op: &str) -> anyhow::Error {
        // `op` is always one of this module's own hardcoded call-site labels
        // (e.g. "session register") and `auth_error_hint()` returns a static
        // string -- neither carries user data. Still, render the status
        // through an explicit non-PII formatter (its numeric code only)
        // rather than the response's `Display` impl, so this call site can
        // never accidentally start interpolating response body/header data
        // into the error message in a future edit.
        if response.status() == reqwest::StatusCode::UNAUTHORIZED {
            self.record_401_attribution(op);
            anyhow::anyhow!("{op}: {}", self.credentials.auth_error_hint())
        } else {
            anyhow::anyhow!("{op} failed: HTTP {}", response.status().as_u16())
        }
    }
    ...
    pub async fn download_file(
        ...
        // Explicitly release the destination file handle now that all
        // chunks have been written, rather than relying solely on
        // end-of-scope Drop.
        drop(out);
        Ok(())
    }
