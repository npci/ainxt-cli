...
use ainxt_acp_lib::AcpAgentGatewaySender as GatewaySender;
use ainxt_tools::implementations::ainxt_build::task::types::*;
use ainxt_workspace::file_system::AsyncFileSystem;
use ainxt_hunk_tracker::HunkTrackerHandle;
...
pub(crate) struct SubagentTracker {
    ...
    /// Matches `SubagentRequest::surface_completion`.
    ...
    pub color: Option<ainxt_agent::config::AgentColor>,
}
...
pub(crate) struct SubagentSpawnContext {
    ...
    pub lsp: Option<std::sync::Arc<dyn ainxt_tools::implementations::lsp::LspBackend>>,
    #[expect(
        dead_code,
        reason = "unused in production; remove expect when wired or delete the item"
    )]
    ...
    pub sampling_config: ainxt_sampler::SamplerConfig,
    ...
    pub auth: Option<crate::auth::AinxtAuth>,
    ...
    pub parent_terminal_backend: Option<Arc<dyn ainxt_tools::computer::types::TerminalBackend>>,
    ...
    pub parent_notification_handle:
        Option<ainxt_tools::notification::types::ToolNotificationHandle>,
    ...
    pub web_search_sampling_config: Option<ainxt_sampler::SamplerConfig>,
    /// Resolved config for web fetch.
    pub web_fetch_config: ainxt_tools::implementations::ainxt_build::web_fetch::WebFetchConfig,
    /// Image generation config (parent-inherited).
    pub image_gen_config: ainxt_tools::implementations::ainxt_build::image_gen::ImageGenConfig,
    /// Resolved config for video generation.
    pub video_gen_config: ainxt_tools::implementations::ainxt_build::video_gen::VideoGenConfig,
    ...
    pub parent_chat_state: Option<ainxt_chat_state::ChatStateHandle>,
    ...
    pub plugin_registry: Option<std::sync::Arc<ainxt_agent::plugins::PluginRegistry>>,
    ...
    pub file_tool_overrides: Option<Vec<ainxt_tools::registry::types::ToolConfig>>,
    ...
    pub hook_registry: Option<std::sync::Arc<ainxt_hooks::discovery::HookRegistry>>,
    #[expect(
        dead_code,
        reason = "unused in production; remove expect when wired or delete the item"
    )]
    pub hook_workspace_root: String,
    pub permission_handle: Option<ainxt_workspace::permission::PermissionHandle>,
    pub worktree_type: crate::util::config::WorktreeType,
    pub api_key_provider: Option<ainxt_tools::types::SharedApiKeyProvider>,
    pub image_description_model: String,
    /// Dual-mode workspace operations handle.
    pub workspace_ops: ainxt_workspace::WorkspaceOps,
    ...
    pub attribution_callback: Option<ainxt_sampler::SharedAttributionCallback>,
    /// Parent session's agent name (e.g. "ainxt-build").
    ...
    /// unchanged mirror-fork sends the parent's exact tool prefix for cache reuse.
    pub parent_tool_snapshot: Option<Vec<ainxt_sampling_types::ToolSpec>>,
    /// Pre-discovered skills from the parent session, captured at spawn time.
    pub parent_skills: Option<Vec<ainxt_tools::implementations::skills::types::SkillInfo>>,
    /// Parent's skills config for the child's SkillManager.
    pub parent_skills_config: ainxt_agent::prompt::skills::SkillsConfig,
    ...
    pub parent_compat: ainxt_tools::types::compat::CompatConfig,
    /// Shared set of IDs delivered via auto-wake synthetic prompts.
    pub auto_wake_delivered:
        Option<ainxt_tools::reminders::task_completion::AutoWakeDeliveredIds>,
...
impl SubagentSpawnContext {
    ...
    fn apply_session_cli_overrides(&self, def: &mut ainxt_agent::config::AgentDefinition) {
        if let Some(ref cfg) = self.agent_config {
            cfg.cli_agent_overrides.apply_to_subagent_definition(def);
        }
    }
    /// Subagent unchanged-input flag, mirroring `Config::resolve_compaction_verbatim_input` (env > config > remote settings > default `true`).
    pub fn resolve_compaction_verbatim_input(&self) -> bool {
        crate::agent::config::BoolFlag::env("AINXT_COMPACTION_VERBATIM_INPUT")
            .config(
                self.agent_config
                    .as_ref()
    ...
    /// and its directory deleted. Resolution matches the other subagent gates
    ...
    pub fn resolve_subagent_worktree_snapshot_enabled(&self) -> bool {
        crate::agent::config::BoolFlag::env("AINXT_SUBAGENT_WORKTREE_SNAPSHOT")
            .config(
                self.agent_config
                    .as_ref()
...
use ainxt_subagent_resolution::ResumeSourceData;
...
use ainxt_subagent_resolution::resolve_effective_overrides;
...
async fn resolve_subagent_sampling_config(
    agent_name: &str,
    agent_model: &ainxt_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (ainxt_sampler::SamplerConfig, acp::ModelId) {
    use ainxt_agent::config::ModelOverride;
...
async fn resolve_effective_model_config(
    ...
    subagent_type: &str,
    definition_model: &ainxt_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (ainxt_sampler::SamplerConfig, acp::ModelId) {
...
fn log_subagent_model_resolution(
    agent_name: &str,
    priority: &str,
    resolved: &ainxt_sampler::SamplerConfig,
    resolved_id: &acp::ModelId,
    parent: &ainxt_sampler::SamplerConfig,
) {
    ...
    if let Some(ref chat_state) = ctx.parent_chat_state {
        if let Some(cfg) = chat_state.get_sampling_config().await {
            ...
            let auth_scheme = crate::agent::config::try_resolve_model_credentials(&cfg.model, None)
                .map(|r| r.auth_scheme)
                .unwrap_or_default();
            let inherited = ainxt_sampler::SamplerConfig {
                ...
                api_backend_explicit: cfg.api_backend_explicit,
                // A subagent inherits the parent's model, so it must inherit
                // that model's capability profile too — otherwise the child
                // would send extensions the endpoint rejects.
                capabilities: crate::agent::config::resolve_model_capabilities(&cfg.model),
                auth_scheme,
                extra_headers,
...
fn resolve_model_override_to_config(
    model_id: &str,
    ctx: &SubagentSpawnContext,
) -> Option<(ainxt_sampler::SamplerConfig, acp::ModelId)> {
...
pub(crate) fn resume_inherited_prefix_len(
    conversation: &[ainxt_sampling_types::conversation::ConversationItem],
) -> usize {
...
struct InitialContext {
    ...
    conversation: Vec<ainxt_sampling_types::conversation::ConversationItem>,
    /// True only for a unchanged mirror-fork (parent items copied exactly).
...
fn forked_initial_context(
    mut items: Vec<ainxt_sampling_types::conversation::ConversationItem>,
) -> InitialContext {
    ...
    let (conversation, prefix_len) =
        ainxt_subagent_resolution::context::normalize_forked_context(items);
    if prefix_len < 2 {
        return InitialContext {
            source: InitialContextSource::New,
...
/// A unchanged mirror requires a coherent tail: the conversation must end on a
...
fn conversation_tail_is_complete(
    items: &[ainxt_sampling_types::conversation::ConversationItem],
) -> bool {
    use ainxt_sampling_types::conversation::ConversationItem;
    matches!(
...
///
/// Unchanged mirror (the cache-preserving path): when the parent fits the child
...
fn verbatim_or_normalize_fork(
    items: Vec<ainxt_sampling_types::conversation::ConversationItem>,
    ...
    let estimated_tokens = ainxt_chat_state::estimate_conversation_tokens(&items);
    ...
    let (conversation, prefix_len) =
        ainxt_subagent_resolution::context::normalize_forked_context(filtered);
    InitialContext {
        source: InitialContextSource::Forked,
        copy_error: None,
        prefix_len: Some(prefix_len),
...
/// A unchanged mirror-fork inherits items as-is and never normalizes, so it reports
    ...
    if let Some(source) = resume_source {
        ...
        return match storage.copy_session_data_sync(
            ...
            Ok(result) => {
                ...
                let estimated_tokens = ainxt_chat_state::estimate_conversation_tokens(&conversation);
...
fn filter_pool_by_inheritance(
    ...
    inheritance: &ainxt_agent::config::McpInheritance,
) -> Option<crate::session::mcp_servers::SharedMcpPool> {
    use ainxt_agent::config::McpInheritance;
    match inheritance {
...
fn resolve_agent_definition(
    subagent_type: &str,
    ctx: &SubagentSpawnContext,
) -> Option<ainxt_agent::config::AgentDefinition> {
    let mut def = ainxt_agent::discovery::by_name_in_cwd_with_plugins(
        subagent_type,
        &ctx.parent_cwd,
...
pub(crate) struct SubagentValidationContext {
    pub parent_cwd: PathBuf,
    pub plugin_registry: Option<Arc<ainxt_agent::plugins::PluginRegistry>>,
...
pub(crate) fn validate_subagent_type(
    ...
    let resolves = ctx.cli_agent_names.iter().any(|n| n == subagent_type)
        || ainxt_agent::discovery::by_name_in_cwd_with_plugins(
            subagent_type,
            &ctx.parent_cwd,
            ctx.plugin_registry.as_deref(),
        )
        .is_some();
    if !resolves {
        let mut available: Vec<String> = ainxt_agent::discovery::all_subagents_with_plugins(
            &ctx.parent_cwd,
            &ctx.subagent_toggle,
            ctx.plugin_registry.as_deref(),
        )
...
/// agent definition.
///
/// The harness flavor (alternate vs ainxt-build) normally follows the PARENT
/// agent: `AinxtBuildOrchestrator` parents give children
...
/// (so a ainxt-build session can run an alternate-harness verifier and vice-versa);
...
fn resolve_subagent_toolset(
    ...
    ctx: &SubagentSpawnContext,
    definition: &mut ainxt_agent::config::AgentDefinition,
) {
...
/// this is NOT a exactly equivalent of the finalize-time `kind_to_name`
...
fn summarize_tool_config(
    config: &ainxt_tools::registry::types::ToolServerConfig,
) -> SubagentTypeSummary {
    use std::collections::HashMap;
...
fn resolve_subagent_permission_mode(
    requested: ainxt_agent::config::PermissionMode,
    is_plugin: bool,
    policy_block: Option<&'static str>,
) -> ainxt_agent::config::PermissionMode {
    use ainxt_agent::config::PermissionMode;
    if is_plugin {
...
fn resolve_subagent_source_repo(ctx: &SubagentSpawnContext) -> std::path::PathBuf {
    ...
    ainxt_workspace::session::git::find_main_repo_root_from_path(&source_cwd)
...
/// auto-backgrounded (non-destructively). Env override: `AINXT_SUBAGENT_AWAIT_BUDGET_MS`.
const SUBAGENT_AWAIT_BUDGET: std::time::Duration = std::time::Duration::from_secs(600);
fn subagent_await_budget() -> std::time::Duration {
    std::env::var("AINXT_SUBAGENT_AWAIT_BUDGET_MS")
        .ok()
...
fn cancellation_error_message(
    category: Option<ainxt_file_utils::events::types::CancellationCategory>,
...
/// Also suppressed while the parent's goal loop is active (matches the bash
...
fn inject_subagent_completed_prompt(
    ...
    request: &SubagentRequest,
    auto_wake_delivered: &Option<ainxt_tools::reminders::task_completion::AutoWakeDeliveredIds>,
    ...
    let message = ainxt_tools::reminders::task_completion::format_subagent_completion(
        &summary,
        Some(task_output_tool_name),
    );
    let wrapped = ainxt_tools::reminders::wrap_reminder(&message);
...
fn emit_subagent_notification(
    ...
    if let Some(cmd_tx) = parent_cmd_tx {
        let _ = cmd_tx.send(SessionCommand::AinxtSessionNotification {
...
/// `ainxt.dev/subagent/list_running` on reconnect.
    ...
    tokio::task::spawn_local(async move {
        ...
        loop {
            ...
            if let Some(ref cmd_tx) = parent_cmd_tx {
                let _ = cmd_tx.send(SessionCommand::AinxtSessionNotification { notification });
            }
            if let Some(params) = params {
                let ext_notification =
                    acp::ExtNotification::new("ainxt.dev/session_notification", params.into());
...
pub(crate) struct SubagentMeta {
    ...
    /// True only for a summarized (normalized) fork; false for unchanged
