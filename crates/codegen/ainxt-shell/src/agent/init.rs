...
fn resolve_config(cfg: &AgentConfig, auth_manager: &AuthManager) -> AgentConfig {
    ...
    if cfg.remote_settings.is_none()
        && let Some(handle) =
            crate::agent::models::start_early_prefetch(Some(cfg.ainxt_com_config.clone()))
    {
        match handle.join() {
            Ok(result) => {
    ...
    // env var > remote settings > Local. Skip remote settings for Generic (ainxt -p, subagents).
    ...
    // Writeback talks to the code backend; requires ainxt.dev auth.
    if cfg.storage_mode == StorageMode::Writeback
        && !auth_manager.current().is_some_and(|a| a.is_ainxt_auth())
    {
        tracing::info!("Writeback is disabled: requires auth with ainxt.dev");
...
fn init_process(cfg: &AgentConfig, auth_manager: &AuthManager) {
    ...
    INIT.call_once(|| {
        if !cfg!(test) {
            ...
            // Signed policy overlay refresh. Alongside the managed-config sync
            // because it is the same shape of problem: a signed artifact the
            // gateway may update between restarts. Startup never waits on it —
            // the policy already on disk is valid and enforcing, so a gateway
            // that is down costs currency, not protection.
            ainxt_policy_fetch::spawn_sync(
        ...
        crate::builtin::extract_bundled_files(&ainxt_home);
        ...
        if cfg.resolve_official_marketplace_auto_register().value {
            crate::extensions::marketplace::ensure_official_marketplace_source(&ainxt_home);
        }
...
pub fn update_telemetry_config(config: &AgentConfig, auth_manager: &AuthManager) {
    let ainxt_auth = auth_manager.current().filter(|a| a.is_ainxt_auth());
    let user_id = ainxt_auth.as_ref().map(|a| a.user_id.clone());
    let team_id = ainxt_auth.as_ref().and_then(|a| a.team_id.clone());
