...
///
/// Always true except under the admin kill switch — so a fresh install with
/// no credential configured anywhere still starts straight into the app
/// instead of being forced into interactive login; [`resolve_credentials`]
/// already handles "nothing configured" cleanly at request time, so nothing
/// downstream needs this to be conditional on a real credential existing.
///
...
/// method is never advertised, so `AINXT_API_KEY` can't bypass a deployment's
/// forced IdP login.
///
/// [`resolve_credentials`]: crate::agent::config::resolve_credentials
...
mod tests {
    ...
    fn enterprise_byok_config_does_not_require_login() {
        ...
        // Without the env var present at all — the predicate must still hold,
        // so a completely unconfigured session still doesn't require login.
        {
            ...
            assert_eq!(
                first_kind(&built.methods),
                Some(AuthMethodKind::AinxtApiKey),
                "unconfigured session: ainxt.api_key must still be auth_methods.first()",
            );
        }
