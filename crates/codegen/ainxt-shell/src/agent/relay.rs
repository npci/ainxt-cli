...
//! connection to the ainxt.dev relay server with automatic reconnection.
...
/// Config for the ainxt.dev WebSocket relay. Fields are private so the only
...
impl RelayConfig {
    /// Session gate: builds only for a ainxt.dev first-party session
    /// (`is_ainxt_auth`: ainxt.dev-issuer OIDC or external credential) with a
    /// non-empty bearer. BYOK/ApiKey, non-ainxt.dev issuers (enterprise OIDC,
    ...
    pub(crate) fn for_session(
        ...
        if !session.is_ainxt_auth() || session.key.is_empty() {
            return None;
        }
        let _ = alpha_test_key;
        Some(Self {
            ws_url: ctx.ainxt_ws_url.clone(),
            ws_origin: ctx.ainxt_ws_origin.clone(),
...
///
/// The relay maintains a persistent WebSocket connection to ainxt.dev with
...
async fn attempt_auth_recovery(
    ...
    let Some(ref am) = config.auth_manager else {
        teprintln!("Authentication required. Run `ainxt login` to re-authenticate.");
        cancel.cancel();
        return false;
    };
    ...
    loop {
        ...
        match connect_to_relay(&config, proxy_url.as_deref(), &cancel).await {
            Ok(ws) => {
                ...
                tprintln!("Disconnected from ainxt WebSocket server");
...
fn build_relay_request(config: &RelayConfig) -> anyhow::Result<axum::http::Request<()>> {
    ...
    req.headers_mut().insert(
        ...
        axum::http::header::HeaderValue::from_static(ainxt_version::VERSION),
    );
    req.headers_mut().insert(
    ...
    let read_from_ws = async move {
        loop {
            tokio::select! {
                ...
                ainxt_telemetry::unified_log::warn("relay: read liveness timeout, reconnecting",
...
mod tests {
    ...
    /// Helper to create a test AinxtAuth with the given key.
    fn test_auth(key: &str) -> AinxtAuth {
        AinxtAuth {
            key: key.to_string(),
    ...
    #[test]
    fn for_session_builds_only_for_ainxt_issuer() {
        use crate::auth::AINXT_OAUTH2_ISSUER;
        let cfg = AinxtComConfig::default();
        let builds = |a: &AinxtAuth| RelayConfig::for_session(a, &cfg, None, None).is_some();
        let ainxt = AinxtAuth {
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some(AINXT_OAUTH2_ISSUER.to_string()),
            ..test_auth("ainxt-bearer")
        };
        assert!(ainxt.is_ainxt_auth(), "precondition: is_ainxt_auth");
        assert!(builds(&ainxt));
        let external_ainxt = AinxtAuth {
            auth_mode: AuthMode::External,
            oidc_issuer: Some(AINXT_OAUTH2_ISSUER.to_string()),
            ..test_auth("ext-bearer")
        };
        assert!(external_ainxt.is_ainxt_auth(), "precondition: is_ainxt_auth");
        assert!(builds(&external_ainxt));
        assert!(!builds(&AinxtAuth {
            key: String::new(),
            ..ainxt.clone()
        }));
        assert!(!builds(&AinxtAuth {
    ...
    /// Helper: write a AinxtAuth to disk under the given scope.
    fn write_test_auth_to_disk(dir: &std::path::Path, scope: &str, auth: &AinxtAuth) {
    ...
    async fn auth_recovery_refreshes_and_heals_missing_auth_json() {
        ...
        let mut config = RelayConfig::for_session(&expired_session, &cfg, None, Some(am.clone()))
            .expect("ainxt.dev OIDC session is relay-eligible");
        let cancel = CancellationToken::new();
