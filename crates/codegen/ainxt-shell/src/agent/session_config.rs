...
mod tests {
    ...
    fn options_have_one_selected_model_and_a_mode_per_effort() {
        let models = [
            model("ainxt-build", "ainxt"),
            model("ainxt-4.5", "ainxt 4.5"),
        ];
        let current = acp::ModelId::from("ainxt-build");
        ...
        assert_eq!(selected_models[0].id, "ainxt-build");
    ...
    fn none_effort_is_not_a_user_selectable_mode() {
        ...
        let models = [model("ainxt-build", "ainxt")];
        let current = acp::ModelId::from("ainxt-build");
        let opts = build_session_config_options(
            &models,
            &current,
    ...
    fn model_label_falls_back_to_id_when_name_empty() {
        let models = [model("ainxt-build", "")];
        let current = acp::ModelId::from("ainxt-build");
        let opts = build_session_config_options(&models, &current, &[], None);
        assert_eq!(opts[0].label, "ainxt-build");
    }

    #[test]
    fn session_config_option_serializes_camel_case() {
        let opt = SessionConfigOption {
            id: "ainxt-build".to_string(),
            category: "model".to_string(),
            label: "ainxt".to_string(),
            description: None,
            selected: true,
        };
        let v = serde_json::to_value(&opt).expect("serialize");
        assert_eq!(v["id"], "ainxt-build");
        assert_eq!(v["category"], "model");
        assert_eq!(v["label"], "ainxt");
        assert_eq!(v["selected"], true);
        assert!(v.get("description").is_none());
    }

    #[test]
    fn ainxt_session_detail_serializes_camel_case() {
        let detail = AinxtSessionDetail::build(
            "sess-1".to_string(),
            "/Users/me/ainxt".to_string(),
        ...
        assert_eq!(v["cwd"], "/Users/me/ainxt");
        assert_eq!(v["currentModelId"], "ainxt-build");
