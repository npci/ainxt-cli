...
const CLIENT_VERSION_HEADER: &str = "x-ainxt-client-version";

// ============================================================================
// Turn delta wire types (local to ainxt-shell until cli-chat-proxy catches up)
// ============================================================================
...
impl FeedbackClient {
    pub fn new(base_url: impl Into<String>, user_token: Option<String>) -> Self {
        ...
        let credentials = crate::util::ainxt_auth_credentials::AinxtAuthCredentials::new(user_token);
    ...
    fn build_middleware_client(
        ...
        reqwest_middleware::ClientBuilder::new(http.clone())
            .with(ainxt_auth::AuthRetryMiddleware::new(provider, 0))
            .build()
    }

    fn make_auth_provider(
        credentials: &crate::util::ainxt_auth_credentials::AinxtAuthCredentials,
    ) -> Arc<dyn ainxt_auth::AuthCredentialProvider> {
        if let Some(am) = credentials.auth_manager() {
            ...
            Arc::new(ainxt_auth::StaticAuthCredentialProvider::new(
    ...
    fn add_common_headers(&self, builder: RequestBuilder) -> RequestBuilder {
        let builder = builder
            .header(CLIENT_VERSION_HEADER, ainxt_version::VERSION)
            .header(
        ...
        if self.credentials.deployment_key.is_none() {
            builder.header("X-Ainxt-Token-Auth", "ainxt-cli")
        } else {
            builder
        }
    }
    ...
    async fn send_json<T: DeserializeOwned>(
        ...
        let request = ainxt_file_utils::trace_context::inject_trace_context_into_request(request);
...
mod tests {
    ...
    fn test_signals_to_update() {
        let signals = crate::session::signals::SessionSignals {
            ...
            models_used: vec!["ainxt-3".to_string()],
            primary_model_id: Some("ainxt-3".to_string()),
        ...
        assert_eq!(update.primary_model_id, Some("ainxt-3".to_string()));
