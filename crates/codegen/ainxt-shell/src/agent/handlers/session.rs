...
use crate::session::{
    ...
    SessionUsageData, SessionUsageResponse,
};

/// Matches the display title (`generated_title`, else `session_summary`) into
...
/// Router for ainxt.dev/session/* and ainxt.dev/session_summaries/* methods.
pub async fn handle(
    agent: &MvpAgent,
    args: &acp::ExtRequest,
) -> Result<acp::ExtResponse, acp::Error> {
    match args.method.as_ref() {
        "ainxt.dev/session/info" => handle_session_info(agent, args).await,
        "ainxt.dev/session/usage" => handle_session_usage(agent, args).await,
        "ainxt.dev/session/close" => handle_session_close(agent, args).await,
        "ainxt.dev/session/list" => handle_session_list(agent, args).await,
        "ainxt.dev/sessions/list" => handle_roster_list(agent, args).await,
        m if m.starts_with("ainxt.dev/session_summaries/") => {
...
/// `ainxt.dev/sessions/list` — the FleetView roster. Returns every
...
/// `ainxt.dev/sessions/changed` broadcast.
...
/// `ainxt.dev/session/usage` — the `/stats` data source: active model plus
/// the session's own token/cost ledger (`SessionUsageData`). Distinct from
/// `ainxt.dev/billing` (`/usage`, platform credit-limit summary — see
/// `crate::extensions::billing`): this never leaves the local process, so it
/// works identically for OAuth, API-key, and self-hosted deployments.
async fn handle_session_usage(
    ...
    let _ = session
        .cmd_tx
        .send(SessionCommand::GetSessionUsage { responds_to: tx });
    let data = rx.await.unwrap_or_else(|_| SessionUsageData {
        model: None,
        model_display_name: None,
        usage: Default::default(),
    });

    let response = SessionUsageResponse { session_id, data };
...
async fn handle_session_close(
    ...
    if existed {
        ...
        tracing::info!(session_id = %req.session_id, "session closed via ainxt.dev/session/close");
    } else {
...
async fn handle_session_summaries(
    ...
    match args.method.as_ref() {
        "ainxt.dev/session_summaries/session_list" => {
        ...
        "ainxt.dev/session_summaries/workspace_list" => {
            tracing::debug!("ainxt/session_summaries/workspace_list is working");
        ...
        "ainxt.dev/session_summaries/workspace_list_recent" => {
