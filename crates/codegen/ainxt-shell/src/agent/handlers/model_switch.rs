...
use ainxt_sampling_types::parse_reasoning_effort_meta;
...
pub(crate) async fn apply(
    ...
    let mut pending_rebuild_definition: Option<ainxt_agent::AgentDefinition> = None;
    {
        ...
        tracing::info!(
            ...
            model_id = % model_id.0, ? required_agent_type,
        ...
        if is_mismatch && turn_count > 0 {
            tracing::warn!(
                session_tag = %crate::util::session_log_tag(session_id.0.as_ref()),
                model_id = % model_id.0, active_agent = ?
                active_agent_type, required_agent = % required, turn_count,
                "set_session_model: agent type mismatch rejected"
            );
            ainxt_telemetry::session_ctx::log_event(ainxt_telemetry::events::ModelSwitched {
        ...
        if is_mismatch && turn_count == 0 {
            ...
            let resolved = ainxt_agent::discovery::by_name_in_cwd_with_plugins(
                required,
                cwd,
    ...
    if let Some(eff) = effort_override {
            ...
            .model_supports_reasoning_effort(model_id.0.as_ref())
        {
            ...
            tracing::warn!(
                session_tag = %crate::util::session_log_tag(session_id.0.as_ref()),
                model_id = % model_id.0, effort = % eff,
