...
/// `tracing::` log call sites that embed externally influenced data (e.g. the
/// status line returned by an untrusted HTTP CONNECT proxy) in a log record.
/// Values are filtered character-by-character at format time via
...
async fn open_connect_tunnel(
    ...
    debug!(status_line = %LogSafe(status_line.trim()), "Proxy CONNECT response");
