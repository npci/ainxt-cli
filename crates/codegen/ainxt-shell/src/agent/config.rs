...
use crate::auth::{AuthManager, AinxtComConfig, OidcAuthConfig};
...
use std::sync::Arc;
use ainxt_agent::prompt::skills::SkillsConfig;
use ainxt_sampler::{AuthScheme, SamplerConfig};
...
pub const DEFAULT_AGENT_TYPE: &str = "ainxt-build-plan";
...
// These are the PRIMARY compiled endpoint defaults — the ones the model catalog
// and chat path actually resolve through. `ainxt-env`'s table is a separate,
// parallel set; fixing only that one left the real defaults untouched and the
// CLI still fetched `https://api.ainxt.dev/v1/models` while ignoring
// `AINXT_API_BASE_URL`, producing an empty catalog against an unreachable host.
//
// `api.ainxt.dev` and `assets.ainxt.dev` are NOT REGISTERED (no nameservers as
// of 2026-08-04). Compiling an unregistered domain in as the default API base
// means whoever registers it owns the default endpoint of every shipped binary.
// Build-time `AINXT_BUILD_*` overrides bake in real infrastructure; the fallback
// is a local gateway for the API (usable, unhijackable) and the RFC 2606
// `.invalid` TLD for the asset server (reserved, unregisterable, fails fast).
/// Default base URL for the cli chat proxy (ainxt backend).
pub const CLI_CHAT_PROXY_BASE_URL_DEFAULT: &str =
    match option_env!("AINXT_BUILD_CLI_CHAT_PROXY_BASE_URL") {
        Some(v) => v,
        None => "http://localhost:8000/ainxt/v1/api",
    };
/// Default base URL for the ainxt model API. Override with `AINXT_API_BASE_URL`.
pub const AINXT_API_BASE_URL_DEFAULT: &str = match option_env!("AINXT_BUILD_API_BASE_URL") {
    Some(v) => v,
    None => "http://localhost:8000/ainxt/v1/api",
};
/// Default base URL for the asset server (profile images, etc.).
pub const ASSET_SERVER_URL_DEFAULT: &str = match option_env!("AINXT_BUILD_ASSET_SERVER_URL") {
    Some(v) => v,
    None => "https://assets.ainxt.invalid",
};
...
pub struct EndpointsConfig {
    ...
    /// Base URL for the public ainxt API.
    pub ainxt_api_base_url: String,
    ...
    /// Env: `AINXT_MODELS_BASE_URL`. Enables custom endpoint mode.
    /// List URL defaults to `{models_base_url}/models`.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub models_base_url: Option<String>,
    /// Env: `AINXT_MODELS_LIST_URL`. Overrides the default `{base}/models` list URL.
    #[serde(alias = "models_endpoint", skip_serializing_if = "Option::is_none")]
    pub models_list_url: Option<String>,
    /// Env: `AINXT_FEEDBACK_BASE_URL`. Where feedback submissions go.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub feedback_base_url: Option<String>,
    /// Env: `AINXT_TRACE_UPLOAD_URL`. Where trace uploads go.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_url: Option<String>,
    /// Env: `AINXT_TRACE_UPLOAD_BUCKET`. Direct bucket (`gs://` or `s3://`), bypasses proxy.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_bucket: Option<String>,
    /// Env: `AINXT_TRACE_UPLOAD_REGION`. AWS region (S3 only).
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_region: Option<String>,
    /// Env: `AINXT_TRACE_UPLOAD_CREDENTIALS_FILE`. Path to GCS SA key or AWS credentials file.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// Env: `AINXT_TRACE_UPLOAD_ENDPOINT_URL`. Custom S3-compatible endpoint.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_endpoint_url: Option<String>,
    /// Env: `AINXT_DEPLOYMENT_KEY`. Management API key for enterprise deployments.
    /// Sent on telemetry and service requests for deployment-level attribution.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub deployment_key: Option<String>,
    /// Env: `AINXT_MANAGED_CONFIG_URL`. Override the managed config endpoint.
    ...
    /// favor of `AINXT_INTERNAL_OTLP_TRACES_ENDPOINT`, and ignored by the internal
    /// pipeline when `AINXT_EXTERNAL_OTEL` is set (the standard `OTEL_*` vars then
    /// route the external stream only).
    #[serde(skip_serializing_if = "Option::is_none")]
    pub otel_exporter_otlp_endpoint: Option<String>,
    /// Env: `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT`. Full traces endpoint, used
    /// unchanged; overrides `otel_exporter_otlp_endpoint`. Same legacy/deprecation
    ...
    /// Env: `AINXT_INTERNAL_OTLP_TRACES_ENDPOINT`. Full INTERNAL traces endpoint,
    /// used unchanged. Dev/debug repoint of the internal span firehose (replaces
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub ainxt_internal_otlp_traces_endpoint: Option<String>,
    /// Env: `AINXT_INTERNAL_OTLP_HEADERS`. `k=v,k2=v2` extra headers for the
    /// internal export (debug). Wins over the legacy `OTEL_EXPORTER_OTLP_HEADERS`.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub ainxt_internal_otlp_headers: Option<String>,
    /// External-OTEL master switch, captured at construction via
    /// [`external_otel_master_switch_resolved`] — the same layered resolution
    /// (requirement pin > `AINXT_EXTERNAL_OTEL` env > `[telemetry].otel_enabled`
...
pub(crate) fn default_asset_server_url() -> String {
    std::env::var("AINXT_ASSET_SERVER_URL").unwrap_or_else(|_| ASSET_SERVER_URL_DEFAULT.to_owned())
}
...
/// format, shared with `AINXT_INTERNAL_OTLP_HEADERS`): split on `,`,
...
impl EndpointsConfig {
    ...
    /// the public default. NEVER falls back to `ainxt_api_base_url` — that is the
    ...
    /// cli-chat-proxy, never `ainxt_api_base_url`.
    ...
    /// Managed deployment-config URL (`ainxt setup`): explicit `managed_config_url`,
    /// else `proxy_url` + `/deployment/config`. Never `ainxt_api_base_url`, so the
    ...
    /// 1. `ainxt_internal_otlp_traces_endpoint` (unchanged)
    /// 2. legacy `otel_exporter_otlp_traces_endpoint` (unchanged) >
    ///    `otel_exporter_otlp_endpoint` + `/v1/traces` — ONLY when the
    ///    external-OTEL master switch is unset (back-compat; deprecated)
    /// 3. `proxy_url` + `/traces`.
    /// Uses the proxy default (not the `ainxt_api_base_url` fallback) so
    /// telemetry reports to ainxt even when inference is overridden. When the
    ...
    pub fn resolve_otlp_traces_endpoint(&self) -> String {
        if let Some(full) = blank_as_unset(&self.ainxt_internal_otlp_traces_endpoint) {
            ...
            && let Some(legacy) = self.legacy_internal_otlp_traces_endpoint()
        {
            tracing::warn!(
                "Repointing the internal trace pipeline via OTEL_EXPORTER_OTLP_ENDPOINT / \
                 ...
                 AINXT_INTERNAL_OTLP_TRACES_ENDPOINT instead — the standard OTEL_* vars will \
    ...
    /// `otel_exporter_otlp_traces_endpoint` unchanged, else
    ...
    /// Extra headers for the INTERNAL export: `ainxt_internal_otlp_headers`
    ...
    pub fn resolve_otlp_headers(&self) -> Vec<(String, String)> {
        if let Some(headers) = blank_as_unset(&self.ainxt_internal_otlp_headers) {
    ...
    /// `ainxt_internal_otlp_*` override shadowed that half.
    ///
    ...
    pub fn internal_otlp_consumed_standard_vars(&self) -> bool {
        ...
        let endpoint_consumed = blank_as_unset(&self.ainxt_internal_otlp_traces_endpoint).is_none()
            && self.legacy_internal_otlp_traces_endpoint().is_some();
        let headers_consumed = blank_as_unset(&self.ainxt_internal_otlp_headers).is_none()
    ...
    /// still honored by the internal pipeline even with `AINXT_EXTERNAL_OTEL`
    ...
    pub fn resolve_models_list_url(&self) -> String {
        ...
        let base = self
            ...
            .clone()
            .unwrap_or_else(|| self.proxy_url());
        models_url_for_base(&base)
    }
}
/// Append the `/models` catalog path segment to a resolved base URL.
///
/// Shared by [`EndpointsConfig::resolve_models_list_url`] so the aux-endpoint
/// tests can assert the catalog path independently of which base URL
/// (proxy or `models_base_url` override) fed into it.
fn models_url_for_base(base: &str) -> String {
    format!("{base}/models")
}
impl Default for EndpointsConfig {
    fn default() -> Self {
        let gateway_api_base = std::env::var("AINXT_GATEWAY_URL").ok().map(|g| {
            let g = g.trim_end_matches('/');
            if g.ends_with("/ainxt/v1/api") { g.to_owned() } else { format!("{g}/ainxt/v1/api") }
        });
        Self {
            cli_chat_proxy_base_url: std::env::var("AINXT_CLI_CHAT_PROXY_BASE_URL")
                .ok()
                .or_else(|| gateway_api_base.clone()),
            ainxt_api_base_url: std::env::var("AINXT_API_BASE_URL")
                .ok()
                .or_else(|| gateway_api_base.clone())
                .unwrap_or_else(|| AINXT_API_BASE_URL_DEFAULT.to_owned()),
            alpha_test_key: None,
            models_base_url: env_string("AINXT_MODELS_BASE_URL"),
            models_list_url: env_string("AINXT_MODELS_LIST_URL"),
            feedback_base_url: env_string("AINXT_FEEDBACK_BASE_URL"),
            trace_upload_url: env_string("AINXT_TRACE_UPLOAD_URL"),
            trace_upload_bucket: env_string("AINXT_TRACE_UPLOAD_BUCKET"),
            trace_upload_region: env_string("AINXT_TRACE_UPLOAD_REGION"),
            trace_upload_credentials_file: env_string("AINXT_TRACE_UPLOAD_CREDENTIALS_FILE"),
            trace_upload_credentials: None,
            trace_upload_endpoint_url: env_string("AINXT_TRACE_UPLOAD_ENDPOINT_URL"),
            // ainxt token auth: the proxy/service plane authenticates with a
            // bare `Authorization: Bearer <token>`. Prefer an explicit
            // AINXT_DEPLOYMENT_KEY, else the user's AINXT_TOKEN, else the token
            // stored by `ainxt login` (~/.ainxt/credentials.json) — so a single
            // credential drives both the inference and proxy planes and startup
            // stays non-interactive (no OAuth browser flow).
            deployment_key: env_string("AINXT_DEPLOYMENT_KEY")
                .or_else(|| env_string("AINXT_TOKEN"))
                .or_else(|| {
                    crate::auth::gateway_auth::load_credentials()
                        .map(|c| c.access_token)
                        .filter(|t| !t.is_empty())
                }),
            managed_config_url: env_string("AINXT_MANAGED_CONFIG_URL"),
            ...
            ainxt_internal_otlp_traces_endpoint: env_string("AINXT_INTERNAL_OTLP_TRACES_ENDPOINT"),
            ainxt_internal_otlp_headers: env_string("AINXT_INTERNAL_OTLP_HEADERS"),
...
pub use ainxt_config_types::{BoolFlag, ConfigSource, LazinessDetectorPerModelConfig, Resolved};
...
fn resolve_compaction_mode_from(
    ...
    config: Option<&str>,
    remote: Option<&str>,
) -> ainxt_chat_state::CompactionMode {
    use ainxt_chat_state::CompactionMode;
...
/// Controls the per-turn unchanged detail in `segments` mode (default `verbose`).
fn resolve_compaction_detail_from(
    env: Option<&str>,
    config: Option<&str>,
    remote: Option<&str>,
) -> ainxt_chat_state::CompactionDetail {
    use ainxt_chat_state::CompactionDetail;
...
fn resolve_compat_cell(
    ...
    resolve_compat_cell_with_env(ainxt_config::env_bool(env), cfg, remote, default)
}
...
pub(crate) fn compat_config_cell(
    ...
    cell: ainxt_tools::types::compat::CompatCell,
...
pub(crate) use ainxt_telemetry::config::env_telemetry_mode;
pub use ainxt_telemetry::config::{TelemetryConfig, TelemetryMode};
...
impl PluginsConfig {
    ...
    /// Native `.ainxt/config.toml` entries already present take precedence:
    ...
    pub fn merge_claude_enabled_plugins(&mut self, _cwd: Option<&std::path::Path>) {
        ...
        for path in &paths {
            let (claude_enabled, claude_disabled) =
                ainxt_agent::plugins::marketplace::load_enabled_disabled_plugins(path);
    ...
    pub fn to_discovery_config(&self) -> ainxt_agent::plugins::discovery::DiscoveryConfig {
        ainxt_agent::plugins::discovery::DiscoveryConfig {
...
pub struct ModelsConfig {
    ...
    /// Force a single API backend (wire protocol / endpoint) for ALL models,
    /// overriding each model's own configured `api_backend`: `"messages"`,
    /// `"chat_completions"`, or `"responses"`. Config-file form of the
    /// `--api-backend` flag / `AINXT_API_BACKEND` env. Leave unset to let
    /// each model use its own backend (bundled/gateway models still default
    /// to Messages; a `[model.<id>]` entry with an explicit `api_backend`
    /// uses that). See `ainxt_sampler::effective_api_backend`.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// Model that judges whether a turn is finished before the turn-end cutoff
    /// (`turn_end_judge`). Unset = the session's own model.
    ///
    /// Worth pointing at a small local instruct model rather than the coding
    /// model: the judge task is one short JSON object, small models hold that
    /// contract more reliably than a large model inclined to narrate, a
    /// different model family is less prone to endorsing its own completion
    /// claims, and a judge call on a separate route does not evict the coding
    /// model's KV prefix cache mid-session.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// Unset = remote pin, then the client hint / built-in `ainxt-build-0.1`
...
pub struct RemoteConfig {
    /// Shared secret for the remote/relay connection. Never serialized back
    /// out: this struct is only ever deserialized *from* `config.toml`
    /// (its containing `Config::remote` field is itself
    /// `#[serde(skip_serializing)]`), but the field is hardened with its
    /// own `skip_serializing` too so the guarantee holds even if this
    /// struct is ever serialized directly (e.g. in a future debug dump).
...
/// (`ainxt workspace` / leader `with_default_hub_url`). Does **not** enable
/// agent-side harness/client connections or alter local session behavior.
///
/// ```toml
/// [hub]
/// url = "wss://hub.ainxt.dev/ws"
/// ```
#[derive(Clone, Debug, Default, Serialize, Deserialize)]
#[serde(default)]
pub struct HubConfig {
    /// Hub WebSocket URL (`ws://` or `wss://`) used as the leader default for
    /// `ainxt workspace start` when the CLI does not pass `--hub-url`.
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl SandboxSettingsConfig {
    ...
    pub fn resolve_profile(
        ...
        resolve_string_flag(cli_arg, "AINXT_SANDBOX", self.profile.as_deref(), None)
    ...
    pub fn resolve_auto_allow_bash(&self, requirement: Option<bool>) -> Resolved<bool> {
        BoolFlag::env("AINXT_SANDBOX_AUTO_ALLOW_BASH")
...
impl SuggestionsConfig {
    pub fn resolve_enabled(
        ...
        BoolFlag::env("AINXT_SUGGESTIONS")
            .config(self.enabled)
    ...
    pub fn resolve_ai_enabled(
        ...
        BoolFlag::env("AINXT_SUGGESTIONS_AI")
    ...
    pub fn resolve_ai_model(&self) -> String {
        resolve_string_flag(
            None,
            "AINXT_SUGGESTIONS_AI_MODEL",
            self.ai_model.as_deref(),
            None,
        )
        .map(|r| r.value)
        .unwrap_or_else(|| "ainxt-build".to_owned())
    }
}
...
///
/// These supplement the built-in scan locations (`.ainxt/skills/`,
/// `.agents/skills/`, `~/.ainxt/skills/`). They're written by `/import-claude`
...
pub struct Config {
    ...
    /// Warnings from `[model.*]` parsing; surfaced by `ainxt inspect`.
    #[serde(skip)]
    pub model_override_warnings: Vec<super::config_model_override_parse::ModelOverrideWarning>,
    pub ainxt_com_config: AinxtComConfig,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// Typed as `AinxtComConfig` (same schema) so sub-field typos are caught.
    #[serde(default, skip_serializing)]
    pub auth: Option<AinxtComConfig>,
    /// `[desktop]` section — owned by ainxt-desktop (Electron app), opaque to the CLI agent.
    #[serde(default, skip_serializing)]
    pub desktop: Option<toml::Value>,
    /// Top-level `announcements` array — consumed by `resolve_announcements`.
    #[serde(default, skip_serializing)]
    pub announcements: Vec<ainxt_announcements::RemoteAnnouncement>,
    ...
    /// `[marketplace]` — also read by `ainxt_plugin_marketplace::load_sources()`.
    #[serde(default, skip_serializing)]
    ...
    /// Defaults to reading from AINXT_STORAGE_MODE env var.
    #[serde(skip)]
    ...
    #[serde(skip)]
    pub cli_agents: Vec<ainxt_agent::config::AgentDefinition>,
    #[serde(skip)]
    pub cli_agent_overrides: CliAgentOverrides,
    /// Whether subagent (task tool) support is enabled. Enabled by default;
    /// disabled only via `AINXT_SUBAGENTS=0` or `[subagents] enabled = false`.
    /// Not remotely gated.
    #[serde(skip)]
    ...
    /// and `.ainxt/roles/*.toml` file discovery.
    #[serde(skip)]
    pub subagent_roles:
        std::collections::HashMap<String, ainxt_subagent_resolution::config::SubagentRole>,
    #[serde(skip)]
    pub subagent_personas:
        std::collections::HashMap<String, ainxt_subagent_resolution::config::SubagentPersona>,
    ...
    pub zdr_video_output_s3:
        Option<ainxt_tools::implementations::ainxt_build::video_gen::ZdrVideoOutputS3Config>,
    ...
    /// Image describe model (`ainxt-build` default via `ModelOverrideConfig::resolve`).
    #[serde(skip)]
    pub image_description_model: Option<String>,
    /// Turn-end judge model pin (`env AINXT_JUDGE_MODEL > [models] judge`).
    /// `None` = judge with the session's own model.
    #[serde(skip)]
...
pub struct CliAgentOverrides {
    ...
    pub permission_rules: Vec<ainxt_workspace::permission::types::PermissionRule>,
    pub max_turns: Option<u32>,
    pub permission_mode: Option<ainxt_agent::config::PermissionMode>,
}
impl CliAgentOverrides {
    ...
    pub fn apply_to_definition(&self, def: &mut ainxt_agent::config::AgentDefinition) {
    ...
    pub fn apply_to_subagent_definition(&self, def: &mut ainxt_agent::config::AgentDefinition) {
...
pub use ainxt_agent::config::AgentDefinition;
pub use ainxt_agent::config::Effort;
pub use ainxt_agent::config::PermissionMode;
pub use ainxt_shared::ui_config::{ContextualHints, UiConfig};
...
/// ```toml
/// [agent]
/// # Use a named agent (looked up via discovery: .ainxt/agents/, ~/.ainxt/agents/, built-ins)
...
/// 4. `AINXT_AGENT` env var
/// 5. Default `ainxt-build` agent
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
#[serde(default)]
pub struct AgentSelectionConfig {
    /// Name of a built-in or discovered agent definition.
    /// Looked up via `ainxt_agent::discovery::by_name_in_cwd()`.
    /// Examples: "ainxt-build", "browser-use", or a custom agent name.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub name: Option<String>,
    /// Path to an agent definition file (.md with YAML frontmatter).
    /// When set, the agent is loaded from this file.
    /// Supports environment variable expansion (e.g., `$HOME/.ainxt/agents/my-agent.md`).
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl Default for Config {
    fn default() -> Self {
        ...
        let mut cfg = Self {
            ...
            ainxt_com_config: AinxtComConfig::default(),
            shortcuts: None,
            hints: None,
            ui: UiConfig::default(),
            ...
            client_version: Some(ainxt_version::VERSION.to_string()),
            ...
            // No bundled default: the web-search model comes from the gateway
            // config (`[models].web_search` / remote settings). Empty means
            // "unset" and web search no-ops until a real model is resolved.
...
impl Config {
    ...
    pub fn create_auth_manager(&self) -> AuthManager {
        AuthManager::new(
            &crate::util::ainxt_home::ainxt_home(),
            self.ainxt_com_config.clone(),
        )
    ...
    pub fn new_from_toml_cfg(raw_config: &toml::Value) -> Result<Self, String> {
        ...
        if config.ainxt_com_config.oidc.is_none() {
            config.ainxt_com_config.oidc = OidcAuthConfig::from_env();
        }
        if config.ainxt_com_config.oidc.is_none() && config.ainxt_com_config.oauth2.is_none() {
            config.ainxt_com_config.oauth2 = crate::auth::OAuth2ProviderConfig::from_env();
        }
        ...
        config.judge_model = model_overrides.judge;
    ...
    pub fn resolve_runtime_fields(&mut self, ctx: &RuntimeResolutionContext<'_>) {
        ...
        self.auto_wake_enabled = BoolFlag::env("AINXT_AUTO_WAKE")
    ...
    /// If the TOML contains `[auth]`, copy its contents under `[ainxt_com_config]`.
    /// `[ainxt_com_config]` takes precedence if both are present (explicit wins).
    ///
    /// This lets customers write the shorter `[auth.oidc]` instead of `[ainxt_com_config.oidc]`.
    fn expand_auth_alias(raw_config: &toml::Value) -> toml::Value {
        ...
        {
            if let Some(gcc) = table.get_mut("ainxt_com_config") {
                ...
                table.insert("ainxt_com_config".to_owned(), auth);
            }
        }
        config
    }
    fn apply_env_overrides(&mut self) {
        ...
        if let Some(mode) = env_telemetry_mode("AINXT_TELEMETRY_ENABLED") {
    ...
    /// config.toml key, or `AINXT_TWO_PASS_COMPACTION` env.
    ...
    pub(crate) fn resolve_trace_upload(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("AINXT_TELEMETRY_TRACE_UPLOAD")
    ...
    pub(crate) fn trace_upload_decision_debug(&self) -> serde_json::Value {
        ...
        serde_json::json!(
            ...
            "in_env_trace_upload" : std::env::var("AINXT_TELEMETRY_TRACE_UPLOAD").ok(),
            "in_env_telemetry_enabled" : std::env::var("AINXT_TELEMETRY_ENABLED").ok(),
    ...
    pub(crate) fn resolve_feedback(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("AINXT_FEEDBACK_ENABLED")
    ...
    pub(crate) fn resolve_two_pass_compaction(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("AINXT_TWO_PASS_COMPACTION")
            .config(self.features.two_pass_compaction)
            .feature_flag(ff)
            .default(false)
            .resolve()
    }
    /// Server-side doom-loop check policy (the `x-ainxt-doom-loop-check`
    ...
    /// `AINXT_DOOM_LOOP_RECOVERY` > TOML `enabled` > remote `enabled` >
    ...
    pub(crate) fn resolve_doom_loop_recovery(
        &self,
    ) -> Option<ainxt_sampling_types::DoomLoopRecoveryPolicy> {
        use ainxt_sampling_types::DoomLoopRecoveryPolicy as Policy;
        let remote = self
            .remote_settings
            .as_ref()
            .and_then(|s| s.doom_loop_recovery.as_ref());
        let enabled = BoolFlag::env("AINXT_DOOM_LOOP_RECOVERY")
    ...
    /// Gate first-run auto-registration of the official ainxt marketplace source.
    /// Precedence: env `AINXT_OFFICIAL_MARKETPLACE_AUTO_REGISTER` > remote settings >
    ...
    pub(crate) fn resolve_official_marketplace_auto_register(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("AINXT_OFFICIAL_MARKETPLACE_AUTO_REGISTER")
            .feature_flag(ff)
            .default(false)
            .resolve()
    }
    ...
    pub(crate) fn resolve_ask_user_question(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("AINXT_ASK_USER_QUESTION")
    ...
    ///
    /// Precedence: requirements > `AINXT_VOICE_MODE` > config/managed
    ...
    /// `image_gen` tool gate. Default on; gated only by the `AINXT_IMAGE_GEN`
    /// env var and managed-config requirement pin.
    pub(crate) fn resolve_image_gen(&self) -> Resolved<bool> {
        BoolFlag::env("AINXT_IMAGE_GEN")
    ...
    pub(crate) fn resolve_image_edit(&self) -> Resolved<bool> {
        use ainxt_tools::implementations::ainxt_build::IMAGE_EDIT_TOOL_NAME;
        ...
        BoolFlag::env("AINXT_IMAGE_EDIT").default(true).resolve()
    }
    /// Optional ainxt image model override for `image_gen`. When set (non-empty),
    /// `image_gen` calls this model slug instead of the default quality model.
    /// Precedence: env `AINXT_IMAGE_GEN_MODEL_OVERRIDE` > `[features]
    /// image_gen_model_override` config > remote settings `image_gen_model_override`.
    /// `None` → default model (`ainxt-image-quality`).
    pub(crate) fn resolve_image_gen_model_override(&self) -> Option<String> {
        resolve_string_flag(
            None,
            "AINXT_IMAGE_GEN_MODEL_OVERRIDE",
    ...
    pub(crate) fn resolve_goal_classifier_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("AINXT_GOAL_CLASSIFIER")
    ...
    pub(crate) fn resolve_goal_planner_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("AINXT_GOAL_PLANNER")
    ...
    pub(crate) fn resolve_goal_summary_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("AINXT_GOAL_SUMMARY")
    ...
    pub(crate) fn resolve_goal_use_current_model_only(&self) -> Resolved<bool> {
        BoolFlag::env("AINXT_GOAL_USE_CURRENT_MODEL_ONLY")
    ...
    pub(crate) fn resolve_backend_tools(&self) -> Resolved<bool> {
        BoolFlag::env("AINXT_BACKEND_SEARCH")
            .config(self.features.backend_tools)
            .default(true)
            .resolve()
    }
    /// Resolve the mode (env `AINXT_COMPACTION_MODE` > config > remote settings >
    /// default, unrecognized falling through) and, for `Segments`, attach the
    /// separately-resolved detail level.
    pub(crate) fn resolve_compaction_mode(&self) -> ainxt_chat_state::CompactionMode {
        resolve_compaction_mode_from(
            env_string("AINXT_COMPACTION_MODE").as_deref(),
    ...
    /// Resolve unchanged-input flag: env `AINXT_COMPACTION_VERBATIM_INPUT` > config > remote settings > default `true`.
    pub(crate) fn resolve_compaction_verbatim_input(&self) -> bool {
        BoolFlag::env("AINXT_COMPACTION_VERBATIM_INPUT")
    ...
    /// Precedence: env `AINXT_COMPACTION_DETAIL`, then config
    /// `features.compaction_detail`, then remote settings
    /// `remote_settings.compaction_detail`, then default (`verbose`). Drives the
    /// `segments` unchanged detail level.
    fn resolve_compaction_detail(&self) -> ainxt_chat_state::CompactionDetail {
        resolve_compaction_detail_from(
            env_string("AINXT_COMPACTION_DETAIL").as_deref(),
    ...
    pub fn resolve_cancel_rewind(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("AINXT_CANCEL_REWIND")
            .config(self.features.cancel_rewind)
            .feature_flag(ff)
            .default(true)
            .resolve()
    }
    /// Resolve whether to use ainxt's default OAuth2 (ainxt auth.ainxt.dev).
    ///
    /// Enterprise OIDC (`oidc` in config.toml) always wins — this only gates
    /// the default ainxt OAuth2 fallback when no enterprise OIDC is configured.
    ///
    /// Priority: `--oauth` > AINXT_OAUTH_ENABLED env > default (true = OAuth).
    pub fn resolve_ainxt_oauth(&self, cli_oidc: Option<bool>) -> Resolved<bool> {
        BoolFlag::env("AINXT_OAUTH_ENABLED")
            .cli(cli_oidc)
            .default(true)
            .resolve()
    }
    ...
    /// [`resolve_mcp_auto_restart`]; matches
    ...
    /// `ainxt.dev/mcp/server_status` push.
    ///
    /// Thin delegate to the canonical
    /// [`resolve_mcp_push_server_status`] free function — matches the
    ...
    /// narrow non-recursive watches for `<cwd>/` and `<cwd>/.ainxt/`.
    ///
    /// Thin delegate to the canonical
    /// [`resolve_mcp_recursive_config_watch`] free function — matches
...
///
/// `requirement > cli > env (AINXT_MCP_LIVENESS_WATCHERS) > config >
...
pub fn resolve_mcp_liveness_watchers(
    ...
    BoolFlag::env("AINXT_MCP_LIVENESS_WATCHERS")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
/// `BoolFlag` precedence:
///
/// `requirement > cli > env (AINXT_MCP_AUTO_RESTART) > config >
/// managed > feature_flag > default (true)`.
///
/// Matches [`resolve_mcp_liveness_watchers`]. Both
/// `Config::resolve_mcp_auto_restart` and
/// `util::config::resolve_mcp_auto_restart` delegate here so the
/// precedence is single-sourced.
///
/// Recovery is on by default; opt out via `AINXT_MCP_AUTO_RESTART=false`,
...
pub fn resolve_mcp_auto_restart(
    ...
    BoolFlag::env("AINXT_MCP_AUTO_RESTART")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
///
/// `requirement > cli > env (AINXT_MCP_PUSH_SERVER_STATUS) > config >
/// managed > feature_flag > default (true)`.
///
...
/// `ainxt.dev/mcp/server_status` is wired default-on, with this
...
pub fn resolve_mcp_push_server_status(
    ...
    BoolFlag::env("AINXT_MCP_PUSH_SERVER_STATUS")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
/// [`resolve_mcp_liveness_watchers`]:
///
/// `requirement > cli > env (AINXT_MCP_RECURSIVE_CONFIG_WATCH) >
...
/// operators flip this flag (e.g. via `AINXT_MCP_RECURSIVE_CONFIG_
...
pub fn resolve_mcp_recursive_config_watch(
    ...
    BoolFlag::env("AINXT_MCP_RECURSIVE_CONFIG_WATCH")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
impl SyncBoolFlag {
    ...
    /// Either-direction env resolver (typically `AINXT_*`). Returns
...
pub fn is_telemetry_disabled_sync() -> bool {
    !SyncBoolFlag::new(telemetry_enabled_from_toml)
        ...
        .enable_env(ainxt_telemetry_env_enabled)
        .resolve()
}
...
pub fn is_error_reporting_disabled_sync() -> bool {
    !SyncBoolFlag::new(error_reporting_enabled_from_toml)
        ...
        .enable_env(|| env_bool("AINXT_ERROR_REPORTING"))
...
/// `AINXT_TELEMETRY_ENABLED` resolved through `TelemetryMode::parse` so the
/// extended string forms (e.g. `"session_metrics"`) are accepted.
fn ainxt_telemetry_env_enabled() -> Option<bool> {
    env_telemetry_mode("AINXT_TELEMETRY_ENABLED").map(|m| !m.is_disabled())
}
/// Load `~/.ainxt/requirements.toml` standalone so the admin pin can beat
/// env vars. The merged config layer can't express that — last-merge-wins
/// loses provenance.
pub(crate) fn read_requirements_toml() -> Option<toml::Value> {
    let path = crate::util::ainxt_home::ainxt_home().join("requirements.toml");
    let content = std::fs::read_to_string(&path).ok()?;
    toml::from_str(&content).ok()
}
/// Resolve the external-OTEL master switch exactly the way the external
/// stream's activation does: **requirement pin > `AINXT_EXTERNAL_OTEL` env >
...
pub(crate) fn external_otel_master_switch_resolved() -> bool {
    external_otel_master_switch_from(
        ainxt_config::load_merged_requirements().as_ref(),
        env_bool("AINXT_EXTERNAL_OTEL"),
...
/// by `ainxt setup`) sit under the env vars, requirements pins are applied on
...
pub fn resolve_external_otel_config(
    client: ainxt_telemetry::external::config::ExternalClientInfo,
) -> Option<ainxt_telemetry::external::ExternalOtelConfig> {
...
pub(crate) fn resolve_external_otel_config_with(
    ...
    let file_cfg: Option<ainxt_telemetry::external::ExternalOtelFileConfig> = effective_config
        .and_then(|cfg| cfg.get("telemetry"))
        .map(|t| ainxt_telemetry::external::ExternalOtelFileConfig {
    ...
    let getenv_pinned = |name: &str| -> Option<String> {
        let pin = match name {
            ainxt_telemetry::external::config::ENV_MASTER_SWITCH => req_enabled,
    ...
    let mut resolved = ainxt_telemetry::external::ExternalOtelConfig::resolve_with(
        getenv_pinned,
        file_cfg.as_ref(),
    )?;
...
pub fn apply_external_otel_remote_policy(settings: Option<&crate::util::config::RemoteSettings>) {
    ...
    let policy = ainxt_telemetry::external::ExternalOtelRemotePolicy {
    ...
    if policy.force_disable || policy.lock_content_gates {
        ainxt_telemetry::external::apply_remote_policy(policy);
    }
}
...
fn managed_settings_env_flag(key: &str) -> Option<bool> {
    let path = ainxt_config::claude_managed_settings_path()?;
    let content = std::fs::read_to_string(&path).ok()?;
    let json: serde_json::Value = serde_json::from_str(&content).ok()?;
    ainxt_workspace::permission::resolution::json_env_flag(json.get("env"), key)
}
/// Assemble the final model map. Priority (highest wins):
/// config.toml `[model.*]` > prefetched (remote). There are NO bundled
/// defaults: the catalog is sourced only from the ainxt gateway. When
/// `prefetched` is `None`/empty and there are no `[model.*]` entries, the
/// resolved catalog is empty and callers surface a clear error.
pub fn resolve_model_list(
    ...
    if let Some(mut prefetched) = prefetched {
        tracing::debug!(model_count = prefetched.len(), "loaded prefetched models");
...
/// Test-only fixture: a single-entry catalog keyed by the test model slug.
///
/// Production has no bundled defaults (the catalog is gateway-only). This exists
/// solely so tests that previously exercised the removed bundled catalog can
/// still build a representative entry.
#[cfg(test)]
pub(crate) fn default_model_entries(endpoints: &EndpointsConfig) -> IndexMap<String, ModelEntry> {
    let slug = crate::models::TEST_MODEL;
    let config = ModelEntryConfig {
        id: None,
        model: slug.to_owned(),
        base_url: endpoints.resolve_inference_base_url(),
        api_base_url: Some(endpoints.ainxt_api_base_url.clone()),
        name: Some(slug.to_owned()),
        description: None,
        context_window: NonZeroU64::new(500_000).expect("500000 is non-zero"),
    ...
    out.insert(slug.to_owned(), ModelEntry::from_config_entry(&config));
    out
}
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ModelEntryConfig {
    ...
    pub model: String,
    /// The base URL of the model. e.g. "https://api.ainxt.dev/v1"
    pub base_url: String,
    ...
    /// If not set, falls back to env_key, then AINXT_API_KEY.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// If not set, falls back to AINXT_API_KEY.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// Can also be set via the `AINXT_MAX_RETRIES` environment variable.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub max_retries: Option<u32>,
    ...
    /// Without this set, ainxt API models send args as one delta
...
pub struct ConfigModelOverride {
    ...
    /// `[model.<id>.capabilities]` — withhold Messages extensions this
    /// endpoint cannot parse. Absent means auto-detect from the model id.
    pub capabilities: Option<ainxt_sampling_types::ModelCapabilities>,
    #[serde(default)]
    pub extra_headers: IndexMap<String, String>,
...
impl ConfigModelOverride {
    pub(crate) fn apply(
        ...
        if let Some(ref v) = self.api_backend {
            ...
            entry.info.api_backend_explicit = true;
        }
        if let Some(ref v) = self.capabilities {
            entry.info.capabilities = Some(v.clone());
        }
...
pub struct ModelInfo {
    ...
    /// The routing slug sent in API requests.
    pub model: String,
    /// The base URL of the model (session endpoint). e.g. "https://api.ainxt.dev/v1"
    pub base_url: String,
    ...
    pub top_p: Option<f32>,
    pub api_backend: ApiBackend,
    /// True when a user's own `[model.<id>]` TOML entry explicitly set
    /// `api_backend` (via `ConfigModelOverride::apply`), as opposed to a
    /// bundled default or gateway-catalog value the dispatch layer forces to
    /// Messages absent a global override. Lets `sampling_config_for_model`
    /// respect a direct/BYOK model's own wire protocol (e.g. a local Ollama
    /// server declared with `api_backend = "chat_completions"`) without
    /// changing behavior for gateway-routed models, whose catalog
    /// `api_backend` may not describe the gateway's own transport at all.
    #[serde(default)]
    pub api_backend_explicit: bool,
    /// Which optional Messages features this model's endpoint accepts.
    /// `None` — the usual case — means "derive it from the model id"
    /// (`ModelCapabilities::for_model`): fully capable for a first-party model,
    /// core-only for an in-house / self-hosted one. Set it explicitly with
    /// `[model.<id>.capabilities]` in config.toml, or have the gateway advertise
    /// it per model in the served catalog.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub capabilities: Option<ainxt_sampling_types::ModelCapabilities>,
    ...
    /// Always has a value; defaults to `"ainxt-build-plan"` when the server
...
impl ModelInfo {
    ...
    pub fn fallback(slug: &str) -> Self {
        ModelInfo {
            ...
            id: None,
            // Unknown slug: leave the profile unset so it is derived from the
            // slug at dispatch (`sampling_config_for_model`).
            capabilities: None,
            model: slug.to_owned(),
            base_url: String::new(),
            name: None,
...
impl ModelEntry {
    ...
    /// resolves to a non-empty value, or brings its own credential via an
    /// auth-bearing `extra_headers` entry (`x-api-key` / `authorization`).
    /// Probes `std::env::var` at call time — result is not stable across env changes.
    pub fn has_own_credentials(&self) -> bool {
        self.own_credential().is_some() || self.has_auth_extra_headers()
    }
    /// `true` if this model's own config sets an auth-bearing extra header,
    /// i.e. it brings its own credential without `api_key`/`env_key`.
    fn has_auth_extra_headers(&self) -> bool {
        self.info
            .extra_headers
            .keys()
            .any(|k| k.eq_ignore_ascii_case("x-api-key") || k.eq_ignore_ascii_case("authorization"))
    }
}
...
/// codebase_indexing = ["/Users/*/ainxt*", "!/Users/*/old-*"]           # globs, ! to exclude
/// ```
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(untagged)]
...
/// config. Matches the remote tolerance in `util::config::remote`.
...
pub struct AutoModeConfig {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub prompt_type: Option<ainxt_workspace::permission::ClassifierPromptType>,
...
pub struct Features {
    ...
    /// Show a blocking warning when ainxt starts outside a Git repository.
    /// Default: false. Used as the local fallback when the `non_git_warning` remote settings
    /// flag in `ainxt_build_settings` is absent. When the remote flag is present it takes
    ...
    /// `image_gen` ainxt image model override. `None`/empty = defer to remote settings
    /// (`image_gen_model_override`) / env / default (`ainxt-image-quality`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// env (`AINXT_COMPACTION_MODE`). Parsed via `CompactionMode::parse`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub compaction_mode: Option<String>,
    /// `none` | `minimal` | `balanced` | `verbose` (default). `None` = defer to
    /// env (`AINXT_COMPACTION_DETAIL`). The `segments` unchanged detail level.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub compaction_detail: Option<String>,
    /// Feed the summarizer the unchanged conversation instead of the lossy rewrite; `None` = defer to env/remote settings/default (true).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// termination and pushes `ainxt.dev/mcp/server_status` updates to
    ...
    /// `AINXT_MCP_AUTO_RESTART` to opt out).
    ///
    /// Resolved via [`Config::resolve_mcp_auto_restart`].
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub mcp_auto_restart: Option<bool>,
    /// Pager-side subscription to the `ainxt.dev/mcp/server_status` push.
    ///
    ...
    /// back to the legacy `ainxt.dev/mcp/tools_changed` debounced refetch
...
pub struct ResolvedCredentials {
    pub api_key: Option<String>,
    pub base_url: String,
    pub auth_type: ainxt_chat_state::AuthType,
    pub auth_scheme: AuthScheme,
}
...
/// Priority: model api_key/env_key > session token > AINXT_API_KEY.
///
...
pub fn resolve_credentials(model: &ModelEntry, session_key: Option<&str>) -> ResolvedCredentials {
    ...
    let (api_key, base_url, auth_type) = if let Some(key) = model.own_credential() {
        (
            Some(key),
            info.base_url.clone(),
            ainxt_chat_state::AuthType::ApiKey,
        )
    } else if let Some(key) = session_key {
        (
            Some(key.to_owned()),
            info.base_url.clone(),
            ainxt_chat_state::AuthType::SessionToken,
        )
    } else if let Ok(key) = crate::agent::auth_method::read_ainxt_api_key_env() {
        let url = model
            .api_base_url
            .clone()
            .unwrap_or_else(|| info.base_url.clone());
        (Some(key), url, ainxt_chat_state::AuthType::ApiKey)
    } else {
...
/// `disable_api_key_auth` at the credential seam: swap a first-party ainxt API
/// key for the IdP session (absent => request fails => forces login). BYOK
/// (non-ainxt `base_url`) is untouched; no-op when the switch is off.
pub fn enforce_disable_api_key_auth(
    ...
    if disable_api_key_auth
        && creds.auth_type == ainxt_chat_state::AuthType::ApiKey
        && crate::util::is_first_party_ainxt_url(&creds.base_url)
    {
        creds.auth_type = ainxt_chat_state::AuthType::SessionToken;
        creds.api_key = session_key.map(str::to_owned);
        ainxt_telemetry::unified_log::debug(
...
pub use ainxt_telemetry::config::deployment_id_from_key;
...
pub fn try_resolve_model_credentials(
    ...
    enforce_disable_api_key_auth(
        &mut credentials,
        cfg.ainxt_com_config.api_key_auth_disabled(),
        session_key,
    );
    Some(credentials)
}
/// The capability profile for `model_id`, for callers that hold only a model id
/// (a subagent inheriting its parent's live config, a turn rebuilding its
/// sampler config) rather than a resolved [`ModelEntry`].
///
/// Prefers the explicit profile on the catalog entry — `[model.<id>.capabilities]`
/// or a gateway-advertised one — and falls back to deriving it from the id, so
/// a self-hosted model still gets the core-only profile when the catalog is
/// unreachable. Mirrors [`try_resolve_model_credentials`]'s resolution shape.
pub fn resolve_model_capabilities(model_id: &str) -> ainxt_sampling_types::ModelCapabilities {
    let from_catalog = (|| {
        let raw = crate::config::load_effective_config().ok()?;
        let cfg = Config::new_from_toml_cfg(&raw).ok()?;
        let models = resolve_model_list(&cfg, None);
        find_model_by_id(&models, model_id)?.info().capabilities.clone()
    })();
    from_catalog
        .unwrap_or_else(|| ainxt_sampling_types::ModelCapabilities::for_model(model_id))
}
...
pub fn resolve_aux_model_sampling_config(
    ...
    let ainxt_bearer = session_key
        .map(|s| s.to_owned())
        .or_else(|| crate::agent::auth_method::read_ainxt_api_key_env().ok())
        .or_else(|| endpoints.deployment_key.clone());
    if let Some(bearer) = ainxt_bearer {
...
/// on BYOK / non-proxy routes for internal slugs like `ainxt-build`).
...
pub fn resolve_chat_state_auth_type(
    model_id: &str,
    session_key: Option<&str>,
    fallback: ainxt_chat_state::AuthType,
) -> ainxt_chat_state::AuthType {
...
pub fn sampling_config_for_model(
    ...
    // Fully capable unless the catalog entry or `[model.<id>.capabilities]`
    // explicitly narrows it. Deliberately not inferred from the model id: the
    // gateway fronting a self-hosted model may implement the full Anthropic
    // surface, and withholding on suspicion costs reasoning quality and cache
    // hits for no gain.
    let capabilities = info.capabilities.clone().unwrap_or_default();
    if capabilities.is_narrowed() {
        tracing::debug!(
            model = %model_name,
            thinking = capabilities.thinking,
            prompt_caching = capabilities.prompt_caching,
            output_config = capabilities.output_config,
            system_blocks = capabilities.system_blocks,
            "model capability profile explicitly narrowed; withholding Messages extensions"
        );
    }
    // Thinking on by default for a Messages endpoint, matching the TypeScript
    // CLI, which sends `thinking: {type: "adaptive"}` to every model it talks to
    // and sustains multi-step tool loops that this one was abandoning.
    //
    // `thinking` is derived solely from `reasoning_effort`, and a catalog entry
    // advertising no effort menu leaves that `None`
    // (`derive_reasoning_effort_fields` returns early on an empty list). The
    // result was reasoning models silently running with thinking off unless
    // someone passed `--reasoning-effort`, which nothing told them to do.
    //
    // The effort *value* does not pick the thinking mode — any `Some` yields
    // adaptive — so this is "on", not a strength claim; `Medium` is the enum's
    // own default. Opt out per model with `[model.<id>.capabilities] thinking =
    // false`, which also suppresses the wire field.
    let reasoning_effort = info.reasoning_effort.or_else(|| {
        (matches!(api_backend, ApiBackend::Messages) && capabilities.thinking)
            .then_some(ReasoningEffort::Medium)
    });
    if reasoning_effort != info.reasoning_effort {
        tracing::debug!(
            model = %model_name,
            effort = ?reasoning_effort,
            "no reasoning effort on the catalog entry; defaulting so a Messages              model gets thinking (matches the TypeScript CLI)"
        );
    }
    SamplerConfig {
        ...
        top_p,
        api_backend,
        api_backend_explicit: info.api_backend_explicit,
        capabilities,
        ...
        // Attestation: report the live enforcement posture and policy version
        // on every request, so the gateway can refuse to serve a client that
        // is not enforcing. Read fresh per request — the policy engine is
        // hot-swappable, and a cached posture would misreport after an overlay
        // narrowed it.
        header_injector: Some(std::sync::Arc::new(
            crate::agent::policy_attest::PolicyAttestation,
        )),
    }
}
...
///
/// * cli-chat-proxy bases get `X-Ainxt-Token-Auth` and
///   `x-authenticateresponse` headers (matches the inline match in the legacy
...
pub fn inject_url_derived_headers(
    ...
    if crate::util::is_cli_chat_proxy_url(base_url) {
        headers
            .entry("X-Ainxt-Token-Auth".to_string())
            .or_insert_with(|| "ainxt-cli".to_string());
        headers
...
pub fn resolve_web_search_sampling_config(
    ...
    let resolved = if let Some(entry) = find_model_by_id(models, model_id).cloned() {
        ...
        // No bundled default web-search model: if the configured model is not
        // in the gateway-sourced catalog, web search is disabled.
        None
    };
    if resolved.is_none() {
        tracing::warn!(
...
mod tests {
    ...
    fn main_cli_tools_override_preserves_profile_injection_policy() {
        ...
        let mut cases = vec![(AgentDefinition::default_ainxt_build(), true)];
    ...
    fn auto_mode_config_parses_from_toml_and_json_equivalently() {
        use ainxt_workspace::permission::ClassifierPromptType;
        let toml_src = r#"
enabled = true
prompt_type = "no_user_tool_prefix"
classifier_model = "ainxt-4.5"
...
"#;
        ...
        let json = serde_json::json!(
            ...
            : "ainxt-4.5", "reasoning_effort" : "low" }
        );
        ...
        for cfg in [&from_toml, &from_json] {
            ...
            assert_eq!(cfg.classifier_model.as_deref(), Some("ainxt-4.5"));
    ...
    fn inject_url_derived_headers_adds_proxy_headers_for_cli_chat_proxy_url() {
        ...
        // PROD_CLI_CHAT_PROXY_BASE_URL is deliberately empty in the OSS build
        // (audit risk R42); is_cli_chat_proxy_url() always trusts loopback
        // regardless of env config, which is what this test needs to exercise.
        inject_url_derived_headers(&mut headers, None, "http://127.0.0.1:9");
        assert_eq!(
            headers.get("X-Ainxt-Token-Auth").map(String::as_str),
            Some("ainxt-cli")
        );
        assert_eq!(
    ...
    fn inject_url_derived_headers_skips_proxy_headers_for_external_url() {
        let mut headers = IndexMap::new();
        inject_url_derived_headers(&mut headers, None, "https://api.ainxt.dev/v1");
        assert!(headers.get("X-Ainxt-Token-Auth").is_none());
    ...
    fn inject_url_derived_headers_does_not_overwrite_existing_entries() {
        let mut headers = IndexMap::new();
        headers.insert("X-Ainxt-Token-Auth".to_string(), "caller-set".to_string());
    ...
    fn new_from_toml_cfg_restores_web_search_and_session_summary_models() {
        ...
        // Gateway-only: an empty config has NO bundled defaults for the
        // auxiliary role models. web_search is empty; the others are None.
        assert_eq!(
            cfg.web_search_model, "",
            "empty config should leave web_search model unset (no bundled default)"
        );
        assert_eq!(
            cfg.session_summary_model, None,
            "empty config should leave session_summary model unset"
        );
        assert_eq!(
            cfg.image_description_model, None,
            "empty config should leave image_description model unset"
        );
    ...
    #[test]
    fn web_search_absent_from_catalog_resolves_to_none() {
        // Gateway-only: there is no hidden bundled web-search model. A model id
        // that is not in the catalog resolves to `None` (web search disabled)
        // instead of synthesizing a fabricated hidden entry.
        let endpoints = EndpointsConfig::default();
        let resolved = resolve_web_search_sampling_config(
            "some-gateway-web-search-model",
            &IndexMap::new(),
            Some("session-token"),
            false,
            None,
        ...
        assert!(
            resolved.is_none(),
            "an unknown web-search model must not synthesize a bundled default"
        );
    }
    #[test]
    fn finalize_image_describe_sampler_none_uses_active_session_model_not_forced_helper() {
        ...
        assert_ne!(cfg.model, "ainxt-build");
    }
    #[test]
    ...
    fn parses_model_api_key() {
        ...
        assert_eq!(model.info.model, "ainxt-4.5");
        assert_eq!(model.info.base_url, "https://api.example.com/v1");
        assert_eq!(model.api_key, Some("sk-test-key-12345".to_string()));
    }
    /// A first-party model keeps every extension — no regression for the
    /// hosted path.
    #[test]
    fn sampling_config_keeps_full_profile_for_first_party_models() {
        for model in ["claude-sonnet-4-6", "gpt-5.4", "ainxt-build"] {
            let entry = test_model_entry(model, "https://api.ainxt.dev/v1", None, None, None);
            ...
            assert_eq!(
                cfg.capabilities,
                ainxt_sampling_types::ModelCapabilities::full(),
                "{model} must keep the full profile"
            );
        }
    }

    /// An explicit `[model.<id>.capabilities]` profile wins over auto-detection
    /// in both directions, so an operator can re-enable a feature their gateway
    /// does support, or disable one on a model the detector thinks is hosted.
    #[test]
    fn explicit_capabilities_override_auto_detection() {
        // Re-enable thinking on an in-house model.
        let mut entry = test_model_entry("local:kimi-k2.7-code", "http://x/v1", None, None, None);
        entry.info.capabilities = Some(ainxt_sampling_types::ModelCapabilities {
            thinking: true,
            ..ainxt_sampling_types::ModelCapabilities::core_only()
        });
        let cfg = sampling_config_for_model(
            &entry,
            resolve_credentials(&entry, None),
        ...
        assert!(cfg.capabilities.thinking, "explicit opt-in must be honored");
        assert!(!cfg.capabilities.prompt_caching);

        // Withhold prompt caching on a model the detector considers hosted.
        let mut entry = test_model_entry("claude-sonnet-4-6", "https://x/v1", None, None, None);
        entry.info.capabilities = Some(ainxt_sampling_types::ModelCapabilities {
        ...
        assert!(!cfg.capabilities.prompt_caching, "explicit opt-out must be honored");
        assert!(cfg.capabilities.thinking);
    }

    /// `[model.<id>]` parses a partial capabilities table and merges it onto the
    /// entry, leaving unlisted features enabled.
    #[test]
    fn config_model_override_applies_partial_capabilities_table() {
        let toml = r#"
            [model.my-local]
            model = "kimi-k2.7"
            base_url = "http://10.0.0.5:8000/v1"
            [model.my-local.capabilities]
            thinking = false
        "#;
        let raw: toml::Value = toml::from_str(toml).expect("toml parses");
        let models = raw.get("model").expect("model table");
        let override_cfg: ConfigModelOverride = models
            .get("my-local")
            .cloned()
            .unwrap()
            .try_into()
        ...
        let caps = override_cfg.capabilities.expect("capabilities parsed");
        assert!(!caps.thinking);
        assert!(
            caps.prompt_caching && caps.output_config && caps.system_blocks,
            "unlisted capability keys must stay enabled: {caps:?}"
        );
    }

    /// A Messages-backend model whose catalog entry advertises no effort menu
    /// must still get thinking. `thinking` is derived only from
    /// `reasoning_effort`, so leaving it `None` ran reasoning models with
    /// thinking off — the TypeScript CLI sends adaptive thinking to the same
    /// model and sustains tool loops this one was abandoning.
    #[test]
    fn messages_model_without_catalog_effort_defaults_to_thinking_on() {
        let mut entry = test_model_entry("kimi-k2.7-code", "http://10.0.0.5:8000/v1", None, None, None);
        ...
        assert_eq!(
            ...
            "a Messages model with no advertised effort must default to one so thinking is sent"
        );
    }

    /// An effort the catalog or config did specify is never overwritten.
    #[test]
    fn explicit_catalog_effort_is_preserved() {
        let mut entry = test_model_entry("kimi-k2.7-code", "http://x/v1", None, None, None);
        entry.info.api_backend = ApiBackend::Messages;
    ...
    /// Only the Messages wire gets this default — Chat Completions carries no
    /// `thinking` field, so inventing an effort there would only add a field
    /// the endpoint did not ask for.
    #[test]
    fn chat_completions_model_gets_no_default_effort() {
        let mut entry = test_model_entry("some-oai-model", "http://x/v1", None, None, None);
    ...
    /// `[model.<id>.capabilities] thinking = false` opts out: no default effort
    /// is invented for a model whose endpoint is known to reject thinking.
    #[test]
    fn thinking_disabled_by_capabilities_suppresses_the_default_effort() {
        let mut entry = test_model_entry("kimi-k2.7-code", "http://x/v1", None, None, None);
        entry.info.api_backend = ApiBackend::Messages;
        entry.info.reasoning_effort = None;
        entry.info.capabilities = Some(ainxt_sampling_types::ModelCapabilities {
    ...
    fn sampling_config_uses_fallback_when_no_model_api_key() {
        ...
        let sampling_config = sampling_config_for_model(
            ...
            ResolvedCredentials {
                ...
                auth_type: ainxt_chat_state::AuthType::ApiKey,
    ...
    fn default_models_dual_endpoint_routing() {
        ...
        for (model_id, entry) in default_model_entries(&endpoints) {
            ...
            assert_eq!(
                api_key_creds.base_url, endpoints.ainxt_api_base_url,
                "{model_id}: ExternalApiKey must route to api.ainxt.dev"
            );
        }
    }
    #[test]
    ...
    fn env_keys_resolve_first_set_wins() {
        let keys = EnvKeys::new(["AINXT_TEST_ENV_KEY_PRIMARY", "AINXT_TEST_ENV_KEY_FALLBACK"]);
        assert_eq!(keys.resolve_value_with(|_| None), None, "none set");
        assert_eq!(
            keys.resolve_value_with(
                |n| (n == "AINXT_TEST_ENV_KEY_FALLBACK").then(|| "from-fallback".into())
            ),
            Some("from-fallback".into())
        );
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "AINXT_TEST_ENV_KEY_PRIMARY" => Some("from-primary".into()),
                "AINXT_TEST_ENV_KEY_FALLBACK" => Some("from-fallback".into()),
                _ => None,
            }),
        ...
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "AINXT_TEST_ENV_KEY_PRIMARY" => Some(String::new()),
                "AINXT_TEST_ENV_KEY_FALLBACK" => Some("from-fallback".into()),
                _ => None,
            }),
            Some("from-fallback".into())
    ...
    fn env_keys_resolve_skips_whitespace_only_value() {
        let keys = EnvKeys::new(["AINXT_TEST_WS_PRIMARY", "AINXT_TEST_WS_FALLBACK"]);
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "AINXT_TEST_WS_PRIMARY" => Some("   ".into()),
                "AINXT_TEST_WS_FALLBACK" => Some("real".into()),
                _ => None,
            }),
            Some("real".into())
        );
        assert_eq!(
            EnvKeys::single("AINXT_TEST_WS_ONLY").resolve_value_with(|_| Some("   ".into())),
            None
        );
        assert_eq!(
            EnvKeys::single("AINXT_TEST_WS_PAD").resolve_value_with(|_| Some("  tok  ".into())),
            Some("  tok  ".into())
        );
    }
    #[test]
    ...
    fn first_own_credential_empty_api_key_falls_through_to_env_key() {
        use ainxt_test_support::EnvGuard;
        let var = "AINXT_TEST_FIRST_OWN_CRED_ENV";
    ...
    fn resolve_credentials_multi_env_key_uses_lc_alias() {
        use ainxt_chat_state::AuthType;
        let primary = "AINXT_TEST_MULTI_ENV_PRIMARY";
        let alias = "AINXT_TEST_MULTI_ENV_LC_ALIAS";
        unsafe {
    ...
    fn resolve_credentials_empty_env_key_falls_through_to_session() {
        use ainxt_chat_state::AuthType;
        use ainxt_test_support::EnvGuard;
        let primary = "AINXT_TEST_EMPTY_ENV_PRIMARY";
        let alias = "AINXT_TEST_EMPTY_ENV_LC_ALIAS";
    ...
    fn resolve_credentials_empty_env_key_falls_through_to_global_key() {
        use crate::agent::auth_method::{LEGACY_AINXT_API_KEY_ENV_VAR, AINXT_API_KEY_ENV_VAR};
        use ainxt_chat_state::AuthType;
        use ainxt_test_support::EnvGuard;
        let sentinel = "ainxt-global-sentinel-key";
        let primary = "AINXT_TEST_EMPTY_ENV_GLOBAL_PRIMARY";
        let alias = "AINXT_TEST_EMPTY_ENV_GLOBAL_ALIAS";
        let _primary = EnvGuard::set(primary, "");
        let _alias = EnvGuard::set(alias, "");
        let _global = EnvGuard::set(AINXT_API_KEY_ENV_VAR, sentinel);
        let _legacy = EnvGuard::unset(LEGACY_AINXT_API_KEY_ENV_VAR);
        let mut model = test_model_entry("m", "https://inference.example/v1", None, None, None);
        model.env_key = Some(EnvKeys::new([primary, alias]));
        assert!(!model.has_own_credentials());
        let creds = resolve_credentials(&model, None);
    ...
    fn proxy_messages_models_use_bearer_auth_scheme() {
        ...
        // (audit risk R42); is_cli_chat_proxy_url() always trusts loopback
        // regardless of env config, which is what this test needs to exercise.
        let mut model = test_model_entry("ainxt-4.5", "http://127.0.0.1:9", None, None, None);
    ...
    fn resolve_credentials_no_session_key_returns_api_key() {
        let model = test_model_entry("m", "https://example.com/v1", None, None, None);
        let creds = resolve_credentials(&model, None);
        assert_eq!(creds.auth_type, ainxt_chat_state::AuthType::ApiKey);
    }
    fn api_key_creds(base_url: &str) -> ResolvedCredentials {
        ResolvedCredentials {
            api_key: Some("ainxt-secret".to_string()),
            base_url: base_url.to_string(),
            auth_type: ainxt_chat_state::AuthType::ApiKey,
            auth_scheme: Default::default(),
        }
    }
    /// `disable_api_key_auth` kill switch (Claude `forceLoginMethod` equivalence).
    #[test]
    fn enforce_disable_api_key_auth_blocks_first_party_only() {
        use ainxt_chat_state::AuthType;
        let mut creds = api_key_creds("https://api.ainxt.dev/v1");
        enforce_disable_api_key_auth(&mut creds, false, Some("session-jwt"));
        assert_eq!(creds.auth_type, AuthType::ApiKey);
        assert_eq!(creds.api_key.as_deref(), Some("ainxt-secret"));
        let mut creds = api_key_creds("https://api.ainxt.dev/v1");
        ...
        let mut creds = ResolvedCredentials {
            ...
            ..api_key_creds("https://api.ainxt.dev/v1")
        };
        enforce_disable_api_key_auth(&mut creds, true, Some("session-jwt"));
        assert_eq!(creds.auth_type, AuthType::SessionToken);
    }
    ...
    /// (non-ainxt.dev) own keys are preserved. (`try_resolve_model_credentials`
    ...
    fn try_resolve_model_credentials_swaps_first_party_own_key_under_kill_switch() {
        ...
        assert_eq!(creds.api_key.as_deref(), Some("ainxt-model-key"));
        enforce_disable_api_key_auth(&mut creds, true, Some("session-jwt"));
        assert_eq!(
            creds.auth_type,
    ...
    fn x_api_key_auth_scheme_flows_from_config_to_sampler() {
        ...
        assert_eq!(config.api_backend, ApiBackend::Messages);
        let client = ainxt_sampler::SamplingClient::new(config).expect("client should build");
    ...
    fn byok_from_lookup_classifies_all_states() {
        ...
        let session = test_model_entry("m", "https://api.ainxt.dev/v1", None, None, None);
        assert_eq!(
    ...
    fn user_override_adds_api_key_to_default_model() {
        let dm = crate::models::default_model();
        // Gateway-only: the base entry comes from the prefetched catalog; the
        // config overlay adds an api_key and must inherit the gateway base_url.
        ...
        .unwrap();
        let cfg = Config::new_from_toml_cfg(&raw_config).expect("config should parse");
        let prefetched = default_model_entries(&cfg.endpoints);
        let expected_base_url = prefetched
            .get(dm)
            .expect("prefetched default present")
            .info
            .base_url
            .clone();
        ...
        assert_eq!(
            model.info.base_url, expected_base_url,
            "base_url should inherit from the gateway catalog entry, not be stale"
        );
    }
    #[test]
    ...
    fn user_override_parses_compaction_at_tokens_from_toml() {
        use ainxt_sampling_types::CompactionAtTokens;
        let dm = crate::models::default_model();
        let raw_config: toml::Value = toml::from_str(&format!(
            r#"
            [model."{dm}"]
    ...
    fn user_override_parses_compactions_remaining_from_toml() {
        use ainxt_sampling_types::CompactionsRemaining;
        let dm = crate::models::default_model();
        let raw_config: toml::Value = toml::from_str(&format!(
            r#"
            [model."{dm}"]
    ...
    fn compaction_mode_precedence_env_over_config_over_remote_over_default() {
        ...
        assert_eq!(
            ...
            CompactionMode::Segments(ainxt_chat_state::CompactionDetail::default())
        );
        assert_eq!(
    ...
    fn sampling_config_context_window_from_entry_or_default() {
        let model = test_model_entry("any-model", "https://api.ainxt.dev/v1", None, None, None);
        let config = sampling_config_for_model(
            &model,
            resolve_credentials(&model, None),
            None,
        ...
        let mut model = test_model_entry("any-model", "https://api.ainxt.dev/v1", None, None, None);
    ...
    /// ainxt-shell/src/agent/models.rs:857 for any BYOK Claude config.
    #[test]
    ...
    #[test]
    fn model_agent_type_defaults_to_ainxt_build() {
        let raw_config: toml::Value = toml::from_str(
            r#"
            [model.my-model]
            model = "my-model"
    ...
    fn hidden_model_excluded_from_acp_but_kept_in_catalog() {
        ...
        let raw_config: toml::Value = toml::from_str(
            ...
            [model.visible-model]
            model = "visible-model"
            base_url = "https://api.ainxt.dev/v1"
            context_window = 200000
    ...
    fn invalid_glob_is_rejected_by_validation() {
        ...
        assert!(ModelGlobSet::compile(Some(&vec!["ainxt[".to_string()])).is_err());
        let raw: toml::Value = toml::from_str(
            r#"
            [models]
            allowed_models = ["ainxt["]
            "#,
        )
        .unwrap();
    ...
    #[test]
    fn auth_alias_maps_to_ainxt_com_config() {
        let raw: toml::Value = toml::from_str(
            r#"
            [auth.oidc]
        ...
        let oidc = cfg.ainxt_com_config.oidc.expect("oidc should be set");
        assert_eq!(oidc.issuer, "https://example.okta.com");
        assert_eq!(oidc.client_id, "test-id");
    }
    #[test]
    fn ainxt_com_config_still_works() {
        let raw: toml::Value = toml::from_str(
            r#"
            [ainxt_com_config.oidc]
            issuer = "https://example.okta.com"
            client_id = "test-id"
            "#,
        )
    ...
    fn disable_api_key_auth_parses_from_auth_alias() {
        ...
        assert_eq!(absent.ainxt_com_config.disable_api_key_auth, None);
        let raw: toml::Value = toml::from_str(
            r#"
            [auth]
        ...
        .unwrap();
        let cfg = Config::new_from_toml_cfg(&raw).expect("config should parse");
        assert_eq!(cfg.ainxt_com_config.disable_api_key_auth, Some(true));
    }
    ...
    fn force_login_team_uuid_parses_string_and_array() {
        ...
        let absent = Config::new_from_toml_cfg(&toml::from_str("").unwrap()).unwrap();
        assert_eq!(absent.ainxt_com_config.force_login_team_uuid, None);
        let raw: toml::Value = toml::from_str(
            r#"
            [auth]
        ...
        let cfg = Config::new_from_toml_cfg(&raw).expect("config should parse");
        assert_eq!(
            cfg.ainxt_com_config.force_login_team_uuid,
    ...
    fn force_login_team_uuid_implies_api_key_auth_disabled() {
        use crate::auth::{ForceLoginTeam, AinxtComConfig};
        let base = AinxtComConfig {
            disable_api_key_auth: None,
            force_login_team_uuid: None,
            ..AinxtComConfig::default()
        };
    ...
    fn e2e_user_overrides_default_model_key_with_custom_endpoint() {
        ...
        assert_eq!(
            ...
            "should route to the user's custom endpoint, not api.ainxt.dev"
        );
    ...
    fn e2e_config_toml_model_overrides_default() {
        ...
        unsafe { std::env::set_var("AINXT_API_KEY", "ainxt-key") };
        let sampling = resolve_sampling(model, None);
        assert_eq!(sampling.base_url, "https://inference.example.com/v1");
        unsafe { std::env::remove_var("AINXT_API_KEY") };
        let sampling = resolve_sampling(model, None);
        assert_eq!(sampling.base_url, "https://inference.example.com/v1");
    }
    #[test]
    ...
    fn config_models_default_is_not_overwritten_by_default_models_json() {
        ...
        let resolved = resolve_string_flag(
            None,
            "AINXT_DEFAULT_MODEL_TEST_NONEXISTENT",
            config_default,
    ...
    fn config_models_default_custom_model_is_in_resolved_model_list() {
        ...
        assert!(
            models.contains_key("acme-ainxt"),
            "user-defined model must be in the resolved model list"
        );
        let model = models.get("acme-ainxt").unwrap();
        assert_eq!(model.info.model, "ainxt-4.5");
        assert_eq!(model.info.base_url, "https://inference.example.com/v1");
    }
    #[test]
    fn e2e_default_model_with_session_routes_to_proxy() {
        // Gateway-only: the default model comes from the (prefetched) gateway
        // catalog, not a bundled default. Session auth routes to the resolved
        // inference (cli-chat-proxy) base_url of the catalog entry.
        let endpoints = EndpointsConfig::default();
        let expected_proxy = endpoints.resolve_inference_base_url();
        let (_, models) =
            resolve_models_from_toml("", Some(default_model_entries(&endpoints)));
        let model = models
        ...
        assert_eq!(
            sampling.base_url, expected_proxy,
            "session auth should route to the catalog entry's cli-chat-proxy base_url"
        );
    }
    #[test]
    #[serial]
    fn e2e_default_model_with_external_api_key_routes_to_api_ainxt() {
        // Gateway-only: the default model comes from the (prefetched) catalog.
        // An external API key routes to the entry's api_base_url.
        let endpoints = EndpointsConfig::default();
        let expected_api = endpoints.ainxt_api_base_url.clone();
        let (_, models) =
            resolve_models_from_toml("", Some(default_model_entries(&endpoints)));
        let model = models
            .get(crate::models::default_model())
            .expect("default model should exist");
        unsafe { std::env::set_var("AINXT_API_KEY", "ainxt-external-key") };
        let sampling = resolve_sampling(model, None);
        assert_eq!(sampling.api_key.as_deref(), Some("ainxt-external-key"));
        assert_eq!(
            sampling.base_url, expected_api,
            "external API key should route via the entry's api_base_url"
        );
        unsafe { std::env::remove_var("AINXT_API_KEY") };
    }
    #[test]
    fn e2e_user_config_overrides_prefetched_model() {
        ...
        prefetched.insert(
            dm.to_string(),
            test_model_entry(dm, "https://api.ainxt.dev/v1", None, None, None),
        );
        let (_, models) = resolve_models_from_toml(
            &format!(
                r#"
    ...
    fn e2e_credential_priority_model_key_beats_session_beats_env() {
        // Isolate `AINXT_HOME`: `resolve_sampling` -> `resolve_credentials` ->
        // `read_ainxt_api_key_env` falls back to a stored `credentials.json`
        // access token, which would otherwise resolve to the developer's real
        // `~/.ainxt` login and defeat the final "no credentials" assertion.
        let _home = crate::test_support::isolated_ainxt_home();
        ...
        unsafe { std::env::set_var("AINXT_API_KEY", "env-key") };
        ...
        assert_eq!(
            sampling.base_url, "https://api.ainxt.dev/v1",
    ...
    fn e2e_duplicate_model_field_both_entries_survive() {
        let dm = crate::models::default_model();
        // Gateway-only: the "default" entry is seeded via the prefetched catalog
        // (there is no bundled default); the user entry is added on top.
        let (_, models) = resolve_models_from_toml(
            &format!(
                r#"
            [model.acme-ainxt]
            ...
            Some(default_model_entries(&EndpointsConfig::default())),
        );
        ...
        let user = models.get("acme-ainxt").unwrap();
    ...
    #[test]
    fn e2e_enterprise_custom_endpoint_skips_ainxt_defaults() {
        ...
        assert!(
            ...
            "ainxt default must not leak into enterprise model list"
        );
        assert_eq!(resolved.len(), 1, "only the prefetched enterprise model");
    }
    #[test]
    fn e2e_default_endpoint_no_bundled_defaults() {
        // Gateway-only: the default endpoint no longer injects any bundled
        // models. With no prefetch, the resolved catalog is empty.
        let cfg = Config::default();
        let resolved = resolve_model_list(&cfg, None);
        assert!(
            resolved.is_empty(),
            "no bundled defaults: the catalog is empty without a gateway prefetch"
        );
    }
    #[test]
    fn e2e_acp_model_info_no_dedup_on_model_field() {
        ...
        assert!(
            acp_models.contains_key(&acp::ModelId::new("default-ainxt")),
            "default entry should be addressable by map key"
        );
        assert!(
            acp_models.contains_key(&acp::ModelId::new("acme-ainxt")),
    ...
    fn e2e_enterprise_endpoints_plus_partial_model_override() {
        let dm = crate::models::default_model();
        // Gateway-only: the base model entry (with enterprise base/api URLs)
        // comes from the prefetched catalog; the local `[model.*]` block layers
        // a partial override (api_key) on top.
        let raw: toml::Value = toml::from_str(&format!(
            r#"
            [endpoints]
            cli_chat_proxy_base_url = "https://enterprise-proxy.acme.com/v1"
            ainxt_api_base_url = "https://enterprise-api.acme.com/v1"
    ...
    fn e2e_enterprise_endpoints_only_no_model_override() {
        // Gateway-only: the base model entry (carrying enterprise endpoints)
        // comes from the prefetched catalog rather than a bundled default.
        let raw: toml::Value = toml::from_str(
            r#"
            [endpoints]
            cli_chat_proxy_base_url = "https://enterprise-proxy.acme.com/v1"
        ...
        assert_eq!(
            model.api_base_url.as_deref(),
            Some("https://enterprise-api.acme.com/v1"),
            "default model should use enterprise ainxt_api_base_url"
        );
    }
    ...
    fn unset_endpoint_env_vars() {
        for k in [
            "AINXT_CLI_CHAT_PROXY_BASE_URL",
            "AINXT_API_BASE_URL",
            "AINXT_FEEDBACK_BASE_URL",
            "AINXT_TRACE_UPLOAD_URL",
            "AINXT_MANAGED_CONFIG_URL",
            "AINXT_MODELS_BASE_URL",
            "AINXT_MODELS_LIST_URL",
            "OTEL_EXPORTER_OTLP_ENDPOINT",
            "OTEL_EXPORTER_OTLP_TRACES_ENDPOINT",
            "OTEL_EXPORTER_OTLP_HEADERS",
            "AINXT_INTERNAL_OTLP_TRACES_ENDPOINT",
            "AINXT_INTERNAL_OTLP_HEADERS",
            "AINXT_EXTERNAL_OTEL",
        ] {
    ...
    /// `ainxt_api_base_url` — overriding ONLY inference keeps every aux endpoint on
    /// the proxy; explicit per-service overrides win unchanged.
    #[test]
    #[serial]
    fn aux_endpoints_resolve_to_proxy_never_inference() {
        unset_endpoint_env_vars();
        let inference = "https://inference.acme-corp.example/ainxt/v1";
        let cfg = EndpointsConfig {
            ainxt_api_base_url: inference.to_string(),
        ...
        // The catalog path carries the version segment when the base lacks one —
        // see `models_url_for_base`. The point of this test is that aux endpoints
        // resolve off the PROXY base, not the inference base; the exact path is
        // asserted in `models_url_for_base`'s own tests.
        assert_eq!(cfg.resolve_models_list_url(), models_url_for_base(proxy));
        assert_eq!(
        ...
        assert_eq!(cfg.ainxt_api_base_url, inference);
    ...
    /// REGRESSION: the managed-config URL never follows `ainxt_api_base_url`
    ...
    fn loader_managed_config_url_never_follows_inference_endpoint() {
        ...
        let cfg = Config::new_from_toml_cfg(
            &toml::from_str(
                r#"[endpoints]
                ainxt_api_base_url = "https://inference.acme-corp.example/ainxt/v1""#,
            )
            .unwrap(),
        )
    ...
    fn resolve_feedback_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("AINXT_FEEDBACK_ENABLED") };
        unsafe { std::env::remove_var("AINXT_TELEMETRY_ENABLED") };
        let cfg = Config::default();
    ...
    fn resolve_session_recap_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("AINXT_SESSION_RECAP") };
        let cfg = Config::default();
    ...
    fn resolve_session_recap_env_off_overrides_default() {
        unsafe { std::env::set_var("AINXT_SESSION_RECAP", "0") };
        let cfg = Config::default();
        let r = cfg.resolve_session_recap();
        assert!(!r.value);
    ...
    fn resolve_two_pass_compaction_precedence() {
        unsafe { std::env::remove_var("AINXT_TWO_PASS_COMPACTION") };
        ...
        assert!(r.value);
        assert_eq!(r.source, ConfigSource::Config);
        unsafe { std::env::set_var("AINXT_TWO_PASS_COMPACTION", "0") };
        let r = config_over_remote.resolve_two_pass_compaction();
    ...
    fn resolve_doom_loop_recovery_precedence() {
        ...
        unsafe { std::env::remove_var("AINXT_DOOM_LOOP_RECOVERY") };
        let default_cfg = Config::default();
        assert!(
        ...
        unsafe { std::env::set_var("AINXT_DOOM_LOOP_RECOVERY", "0") };
        assert!(
    ...
    fn resolve_feedback_env_overrides_all() {
        unsafe { std::env::set_var("AINXT_FEEDBACK_ENABLED", "true") };
        let mut cfg = Config::default();
    ...
    fn resolve_trace_upload_disabled_when_telemetry_off_despite_remote_flag() {
        unsafe { std::env::remove_var("AINXT_TELEMETRY_ENABLED") };
        unsafe { std::env::remove_var("AINXT_TELEMETRY_TRACE_UPLOAD") };
        let mut cfg = Config::default();
    ...
    fn resolve_goal_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("AINXT_GOAL") };
        let cfg = Config::default();
    ...
    fn resolve_goal_env_overrides_config() {
        unsafe { std::env::set_var("AINXT_GOAL", "1") };
        let mut cfg = Config::default();
    ...
    fn resolve_ask_user_question_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("AINXT_ASK_USER_QUESTION") };
        let cfg = Config::default();
    ...
    fn resolve_ask_user_question_env_overrides_remote_settings() {
        unsafe { std::env::set_var("AINXT_ASK_USER_QUESTION", "1") };
        let cfg = Config {
            remote_settings: Some(crate::util::config::RemoteSettings {
    ...
    fn resolve_image_gen_model_override_remote_settings_or_config() {
        unsafe { std::env::remove_var("AINXT_IMAGE_GEN_MODEL_OVERRIDE") };
        ...
        assert_eq!(
            with(None, Some("ainxt-image")).resolve_image_gen_model_override(),
            Some("ainxt-image".to_owned())
        );
        assert_eq!(
            with(Some("ainxt-image-pro"), Some("ainxt-image"))
                .resolve_image_gen_model_override(),
            Some("ainxt-image-pro".to_owned())
        );
    }
    #[test]
    #[serial]
    fn imagine_tools_disabled_gates_image_edit() {
        unsafe { std::env::remove_var("AINXT_IMAGE_EDIT") };
        ...
        unsafe { std::env::set_var("AINXT_IMAGE_EDIT", "1") };
    ...
    fn clear_goal_envs() {
        unsafe {
            std::env::remove_var("AINXT_GOAL");
            std::env::remove_var("AINXT_GOAL_CLASSIFIER");
            std::env::remove_var("AINXT_GOAL_PLANNER");
            std::env::remove_var("AINXT_GOAL_SUMMARY");
            std::env::remove_var("AINXT_GOAL_VERIFIER_N");
            std::env::remove_var("AINXT_GOAL_CLASSIFIER_MAX");
            std::env::remove_var("AINXT_GOAL_STRATEGIST_EVERY");
            std::env::remove_var("AINXT_GOAL_REVERIFY_AFTER");
        }
    }
    ...
    fn resolve_goal_classifier_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("AINXT_GOAL_CLASSIFIER", "0") };
        let r = cfg_with_goal_and_remote(true, remote_classifier(true))
            .resolve_goal_classifier_enabled(true);
        assert!(!r.value);
        assert_eq!(r.source, ConfigSource::Env);
        unsafe { std::env::set_var("AINXT_GOAL_CLASSIFIER", "1") };
    ...
    fn resolve_goal_planner_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("AINXT_GOAL_PLANNER", "0") };
        let r =
            cfg_with_goal_and_remote(true, remote_planner(true)).resolve_goal_planner_enabled(true);
        assert!(!r.value);
        assert_eq!(r.source, ConfigSource::Env);
        unsafe { std::env::set_var("AINXT_GOAL_PLANNER", "1") };
    ...
    fn resolve_goal_summary_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("AINXT_GOAL_SUMMARY", "0") };
        let r =
...
"#,
    ...
    const GOAL_USE_CURRENT_ENV: &str = "AINXT_GOAL_USE_CURRENT_MODEL_ONLY";
...
[goal]
enabled = true
planner_model = { model = "ainxt-build", agent_type = "ainxt-build-plan" }

[goal.strategist_model]
model = "ainxt-composer-2.5-fast"
agent_type = "cursor"

[[goal.skeptic_models]]
model = "ainxt-build"
agent_type = "ainxt-build-plan"
...
"#;
        ...
        let cfg = Config::new_from_toml_cfg(&raw).unwrap();
        assert_eq!(cfg.goal.planner_model.as_ref().unwrap().model, "ainxt-build");
        assert_eq!(
            cfg.goal.strategist_model.as_ref().unwrap().agent_type,
            "cursor"
        );
        assert_eq!(cfg.goal.skeptic_models.len(), 2);
        assert_eq!(cfg.goal.skeptic_models[0].model, "ainxt-build");
        assert_eq!(
...
planner_model = { agent_type = "ainxt-build-plan" }
"#;
        let raw: toml::Value = toml::from_str(toml_str).unwrap();
        let cfg = Config::new_from_toml_cfg(&raw)
...
"#;
        ...
        assert_eq!(cfg.goal.skeptic_models.len(), 2);
        assert_eq!(cfg.goal.skeptic_models[0].model, "ainxt-build");
        assert_eq!(cfg.goal.skeptic_models[1].model, "ainxt-composer-2.5-fast");
    }
...
classifier_max_runs = 6
planner_model = { model = "ainxt-build", agent_type = "ainxt-build-plan" }
strategist_model = { model = "ainxt-composer-2.5-fast", agent_type = "cursor" }
...
"#,
        ...
        let ainxt_build = crate::util::config::GoalRoleModel {
            model: "ainxt-build".into(),
            agent_type: "ainxt-build-plan".into(),
        };
        let composer = crate::util::config::GoalRoleModel {
            model: "ainxt-composer-2.5-fast".into(),
        ...
        assert_eq!(
            planner.value,
            GoalRoleModelChoice::Explicit(ainxt_build.clone())
        );
        ...
        assert_eq!(
            ...
            vec![
                GoalRoleModelChoice::Explicit(ainxt_build),
    ...
    fn config_warns_on_section_typo() {
        // Isolate `AINXT_HOME`: `Config::new_from_toml_cfg` merges the
        // effective config over disk state under `ainxt_home()`; without
        // this, a developer's real `~/.ainxt/config.toml` (e.g. a real
        // `deployment_key`) can leak into `config.endpoints` and defeat the
        // "typo'd section is ignored" assertion below.
        let _home = crate::test_support::isolated_ainxt_home();
        let raw: toml::Value = toml::from_str(
            r#"
            [endpoint]
            deployment_key = "ainxt-token-test"
        "#,
        )
        .unwrap();
    ...
    fn internal_otlp_test_config() -> EndpointsConfig {
        EndpointsConfig {
            ...
            ainxt_internal_otlp_traces_endpoint: None,
            ainxt_internal_otlp_headers: None,
            external_otel_master_switch: false,
            ..Default::default()
        }
    }
    /// `ainxt_internal_otlp_traces_endpoint` wins over the legacy `OTEL_*`
    /// fields regardless of the master switch.
    #[test]
    fn internal_otlp_endpoint_ainxt_internal_wins_regardless_of_switch() {
        for switch in [false, true] {
            let cfg = EndpointsConfig {
                ainxt_internal_otlp_traces_endpoint: Some(
            ...
            assert_eq!(
                ...
                "switch={switch}: AINXT_INTERNAL_OTLP_TRACES_ENDPOINT must win verbatim (trailing / trimmed)"
            );
        }
    }
    ...
    fn internal_otlp_consumed_standard_vars_cases() {
        ...
        for case in cases {
            let cfg = EndpointsConfig {
                ...
                ainxt_internal_otlp_traces_endpoint: case
                    .internal_ep
                    .then(|| "https://internal.example/traces".to_string()),
                ainxt_internal_otlp_headers: case.internal_headers.then(|| "ik=iv".to_string()),
                ..internal_otlp_test_config()
            };
            assert_eq!(
    ...
    /// Headers precedence: `ainxt_internal_otlp_headers` wins; legacy
    /// `otel_exporter_otlp_headers` only when the master switch is unset.
    #[test]
    fn internal_otlp_headers_precedence() {
        for switch in [false, true] {
            let cfg = EndpointsConfig {
                ainxt_internal_otlp_headers: Some("x-debug=1".to_string()),
    ...
    fn ext_client() -> ainxt_telemetry::external::config::ExternalClientInfo {
        ainxt_telemetry::external::config::ExternalClientInfo::default()
    }
    #[test]
    ...
    fn external_otel_requirements_pin_wins_over_env() {
        ...
        assert!(
            resolve_external_otel_config_with(
                None,
                Some(&req),
                ext_env(&[("AINXT_EXTERNAL_OTEL", "1"), ("OTEL_LOGS_EXPORTER", "otlp"),]),
                ext_client(),
                false,
            )
            .is_none()
    ...
    /// (managed config / requirements — no `AINXT_EXTERNAL_OTEL` env var) must
    ...
    fn clear_runtime_env_vars() {
        unsafe {
            std::env::remove_var("AINXT_SUBAGENTS");
            std::env::remove_var("AINXT_RESPECT_GITIGNORE");
            std::env::remove_var("AINXT_WEB_SEARCH_MODEL");
            std::env::remove_var("AINXT_SESSION_SUMMARY_MODEL");
            std::env::remove_var("AINXT_CURSOR_SKILLS_ENABLED");
            std::env::remove_var("AINXT_CURSOR_RULES_ENABLED");
            std::env::remove_var("AINXT_CURSOR_AGENTS_ENABLED");
            std::env::remove_var("AINXT_CLAUDE_SKILLS_ENABLED");
            std::env::remove_var("AINXT_CLAUDE_RULES_ENABLED");
            std::env::remove_var("AINXT_CLAUDE_AGENTS_ENABLED");
        }
    }
    fn clear_managed_mcp_env_vars() {
        unsafe {
            std::env::remove_var("AINXT_MANAGED_MCPS_ENABLED");
            std::env::remove_var("AINXT_MANAGED_MCP_GATEWAY_TOOLS_ENABLED");
        }
    }
...
"#,
    ...
    fn resolve_raw_compat_sessions_load_failure_allows_env_override() {
        let _env = isolate_compat_env();
        let _codex = EnvGuard::set("AINXT_CODEX_SESSIONS_ENABLED", "true");
        let resolved = resolve_compat_sessions_from_raw(Err(()), None);
        assert!(!resolved.cursor.sessions);
        assert!(!resolved.claude.sessions);
        assert!(resolved.codex.sessions);
    ...
    fn resolve_compat_env_sessions_disable_independently() {
        ...
        for (vendor, env_var) in [
            (CompatVendor::Cursor, "AINXT_CURSOR_SESSIONS_ENABLED"),
            (CompatVendor::Claude, "AINXT_CLAUDE_SESSIONS_ENABLED"),
            (CompatVendor::Codex, "AINXT_CODEX_SESSIONS_ENABLED"),
        ] {
    ...
    fn resolve_compat_precedence_and_reserved_codex_hook() {
        ...
        let _session = EnvGuard::set("AINXT_CURSOR_SESSIONS_ENABLED", "true");
        let _hook = EnvGuard::set("AINXT_CODEX_HOOKS_ENABLED", "true");
        let resolved = resolve_compat_config(&config, Some(&remote));
        assert!(resolved.cursor.sessions);
        assert!(resolved.codex.hooks);
    }
    ...
    fn resolve_runtime_fields_compat_asymmetric_sources() {
        let _env = isolate_compat_env();
        let _cursor = EnvGuard::set("AINXT_CURSOR_SESSIONS_ENABLED", "false");
        let raw: toml::Value =
    ...
    fn resolve_runtime_fields_interactive_defaults() {
        ...
        // Gateway-only: no bundled defaults for the auxiliary role models.
        assert_eq!(cfg.web_search_model, "");
    ...
    fn resolve_runtime_fields_gitignore_from_env() {
        clear_runtime_env_vars();
        unsafe { std::env::set_var("AINXT_RESPECT_GITIGNORE", "0") };
        let raw = empty_config();
        let mut cfg = Config::new_from_toml_cfg(&raw).unwrap();
        cfg.resolve_runtime_fields(&RuntimeResolutionContext {
            raw_config: &raw,
...
"#,
    ...
    fn is_telemetry_explicitly_disabled_sync_env_signals() {
        unsafe { std::env::set_var("AINXT_TELEMETRY_ENABLED", "0") };
        unsafe { std::env::remove_var("DISABLE_TELEMETRY") };
        assert!(is_telemetry_explicitly_disabled_sync());
        unsafe { std::env::set_var("AINXT_TELEMETRY_ENABLED", "1") };
        assert!(!is_telemetry_explicitly_disabled_sync());
        unsafe { std::env::remove_var("AINXT_TELEMETRY_ENABLED") };
...
"#,
        ...
        ainxt_config::apply_version_overrides(&mut value, &v).unwrap();
        let cfg = Config::new_from_toml_cfg(&value).unwrap();
        assert_eq!(cfg.models.default.as_deref(), Some("ainxt-4.5"));
    }
    /// Reproduce the enterprise managed config bug: [model.ainxt-build] sets
    /// context_window=500k for model="ainxt-4.5", but
    /// [models].default="ainxt-4.5" resolves to the bare
    /// prefetched entry (256k) because Layer 3 only overrides key
    /// "ainxt-build", not key "ainxt-4.5".
    ///
    ...
    fn slug_propagation_enterprise_managed_config_key_mismatch() {
        ...
        prefetched.insert("ainxt-4.5".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
        let by_key = resolved
            .get("ainxt-build")
            .expect("ainxt-build key must exist");
        assert_eq!(by_key.info.context_window.get(), 500_000);
        assert_eq!(by_key.info.model, "ainxt-4.5");
        let by_latest = resolved.get("ainxt-4.5").expect("ainxt-4.5 key must exist");
        assert_eq!(
            by_latest.info.context_window.get(),
            500_000,
            "BUG: prefetched 'ainxt-4.5' should inherit 500k from \
             sibling 'ainxt-build' (same model slug), not stay at {default_cw}"
        );
    }
    ...
    fn slug_propagation_inherits_api_backend_but_not_agent_type() {
        ...
        let mut prefetched = IndexMap::new();
        let mut entry =
            test_model_entry("ainxt-4.5", "https://test.example.com/v1", None, None, None);
        entry.info.context_window = NonZeroU64::new(default_cw).unwrap();
        entry.info.agent_type = default_agent_type();
        entry.info.api_backend = ApiBackend::default();
        prefetched.insert("ainxt-4.5".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
        let latest = resolved.get("ainxt-4.5").unwrap();
        assert_eq!(
    ...
    fn global_extra_headers_apply_to_model_without_override() {
        let dm = crate::models::default_model();
        // Gateway-only: seed the model via the prefetched catalog so the global
        // header default has an entry to apply to (no bundled default exists).
        let (_, models) = resolve_models_from_toml(
            r#"
            [models]
    ...
    fn resolve_model_list_config_reasoning_efforts_beats_remote() {
        ...
        .unwrap();
        let cfg = Config::new_from_toml_cfg(&raw_config).expect("config should parse");
        let mut entry = prefetch_model_entry("ainxt-x", 200_000, ApiBackend::default());
        entry.info.reasoning_efforts = vec![ReasoningEffortOption {
            id: "high".to_string(),
            value: ReasoningEffort::High,
            label: "High".to_string(),
        ...
        let mut prefetched = IndexMap::new();
        prefetched.insert("ainxt-x".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
    ...
    #[test]
    fn resolve_model_list_keeps_default_context_window_when_no_donor() {
        // Gateway-only: there are no bundled defaults to inherit a context
        // window from. A prefetched model that omits its context window keeps
        // the parser default rather than borrowing from a removed bundled entry.
        let cfg = Config::default();
        let default_cw = DEFAULT_CONTEXT_WINDOW;
        let entry = prefetch_model_entry("ainxt-build", default_cw, ApiBackend::default());
        let mut prefetched = IndexMap::new();
        prefetched.insert("ainxt-build".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
        let entry = resolved.get("ainxt-build").expect("model must exist");
        assert_eq!(
            entry.info.context_window.get(),
            default_cw,
            "no bundled default exists, so context_window stays at DEFAULT_CONTEXT_WINDOW"
        );
    }
    #[test]
    fn resolve_model_list_does_not_override_explicitly_set_context_window() {
        ...
        let entry = prefetch_model_entry("ainxt-build", explicit_cw, ApiBackend::default());
        let mut prefetched = IndexMap::new();
        prefetched.insert("ainxt-build".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
        let entry = resolved.get("ainxt-build").expect("model must exist");
    ...
    #[test]
    fn resolve_model_list_keeps_prefetched_agent_type_and_api_backend() {
        // Gateway-only: with no bundled defaults, a prefetched model keeps its
        // own agent_type / api_backend — there is no donor to inherit from.
        let cfg = Config::default();
        let default_cw = DEFAULT_CONTEXT_WINDOW;
        let entry = prefetch_model_entry("ainxt-build", default_cw, ApiBackend::default());
        let mut prefetched = IndexMap::new();
    ...
    #[test]
    fn resolve_model_list_empty_when_no_prefetch() {
        // Gateway-only: a prefetched entry is present in the catalog, but with
        // NO prefetch the catalog is empty — there are no bundled defaults.
        let cfg = Config::default();
        let mut defs = default_model_entries(&EndpointsConfig::default());
        let mut p = IndexMap::new();
        if let Some(e) = defs.shift_remove("ainxt-build") {
            p.insert("ainxt-build".to_string(), e);
        }
        let resolved = resolve_model_list(&cfg, Some(p));
        assert!(resolved.contains_key("ainxt-build"));
        let no_p = resolve_model_list(&cfg, None);
        assert!(
            no_p.is_empty(),
            "with no prefetch and no config models, the catalog must be empty"
        );
    }
    #[test]
    ...
    fn resolve_model_list_prefetch_replaces_bundled_entirely() {
        let cfg = Config::default();
        let mut p = IndexMap::new();
        let e = prefetch_model_entry("ainxt-4.5", 500_000, ApiBackend::Responses);
        p.insert("ainxt-4.5".to_string(), e);
        let resolved = resolve_model_list(&cfg, Some(p));
        assert!(resolved.contains_key("ainxt-4.5"));
        assert!(!resolved.contains_key("ainxt-build"));
    }
    #[test]
    ...
    /// Regression: enterprise managed config aliases ainxt-build to their own
    /// endpoint with env_key. The bundled ainxt-build has supported_in_api=false.
    ...
    fn byok_config_overlay_visible_to_api_key_users() {
        ...
        let cfg = Config::new_from_toml_cfg(&raw).expect("config should parse");
        let resolved = resolve_model_list(&cfg, None);
        let entry = resolved.get("ainxt-build").expect("ainxt-build must exist");
        assert!(
    ...
    #[test]
    fn plain_config_overlay_preserves_gateway_visibility() {
        // Gateway-only: the base entry (supported_in_api=false) comes from the
        // prefetched catalog; a non-BYOK config overlay must preserve it.
        let raw: toml::Value = toml::from_str(
            r#"
            [model.ainxt-build]
        ...
        assert!(
            !entry.visible_for_auth(false),
            "non-BYOK config overlay must preserve the gateway supported_in_api=false"
        );
    }
    #[test]
    #[serial]
    fn mcp_liveness_watchers_default_is_true() {
        unsafe { std::env::remove_var("AINXT_MCP_LIVENESS_WATCHERS") };
    ...
    fn mcp_liveness_watchers_requirement_wins_over_everything() {
        unsafe { std::env::set_var("AINXT_MCP_LIVENESS_WATCHERS", "true") };
    ...
    fn mcp_liveness_watchers_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("AINXT_MCP_LIVENESS_WATCHERS", "false") };
    ...
    fn mcp_auto_restart_default_is_true() {
        unsafe { std::env::remove_var("AINXT_MCP_AUTO_RESTART") };
    ...
    fn mcp_auto_restart_requirement_wins_over_everything() {
        unsafe { std::env::set_var("AINXT_MCP_AUTO_RESTART", "false") };
    ...
    fn mcp_auto_restart_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("AINXT_MCP_AUTO_RESTART", "true") };
    ...
    fn mcp_push_server_status_default_is_true() {
        unsafe { std::env::remove_var("AINXT_MCP_PUSH_SERVER_STATUS") };
    ...
    fn mcp_push_server_status_requirement_wins_over_everything() {
        unsafe { std::env::set_var("AINXT_MCP_PUSH_SERVER_STATUS", "true") };
    ...
    fn mcp_push_server_status_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("AINXT_MCP_PUSH_SERVER_STATUS", "false") };
    ...
    fn mcp_recursive_config_watch_default_is_true() {
        unsafe { std::env::remove_var("AINXT_MCP_RECURSIVE_CONFIG_WATCH") };
    ...
    fn mcp_recursive_config_watch_requirement_wins_over_everything() {
        unsafe { std::env::set_var("AINXT_MCP_RECURSIVE_CONFIG_WATCH", "true") };
    ...
    fn mcp_recursive_config_watch_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("AINXT_MCP_RECURSIVE_CONFIG_WATCH", "false") };
