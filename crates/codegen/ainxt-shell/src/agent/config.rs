...
"#;
    ...
    /// An in-house model gets the core-only profile with no configuration at
    /// all — the default that makes a self-hosted endpoint work out of the box.
    #[test]
    fn sampling_config_auto_detects_core_only_for_in_house_models() {
        for model in ["local:kimi-k2.7-code", "moonshotai/Kimi-K2.7-Code", "gpt-oss-120b"] {
            let entry = test_model_entry(model, "http://10.0.0.5:8000/v1", None, None, None);
            ...
            assert_eq!(
                ...
                "{model} must default to the core-only profile"
            );
        }
    }
