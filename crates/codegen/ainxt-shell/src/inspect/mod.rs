//! `ainxt inspect` — configuration introspection.
//!
//! Shows everything ainxt discovers in the current directory: project
...
use ainxt_tools::util::truncate::estimate_tokens;
...
/// Enterprise login-hardening policy resolved from `[ainxt_com_config]`
/// (TOML + env). Surfaced so admins can verify the deployment loaded it.
/// The team pin is admin policy, not a secret, so it is shown unchanged.
#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct LoginPolicyReport {
    /// Raw `disable_api_key_auth` knob (env `AINXT_DISABLE_API_KEY_AUTH`).
...
pub struct ConfigSources {
    ...
    /// .ainxt/config.toml files). Driven from the same resolvers used at runtime
...
/// A single config layer entry for `ainxt inspect`.
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
...
async fn build_report(cwd: &Path) -> InspectReport {
    ...
    let trust_store = ainxt_agent::plugins::TrustStore::load();
    ...
    let discovered_plugins = ainxt_agent::plugins::discover_plugins(
        Some(cwd),
        &plugin_config,
        &trust_store,
        project_trusted,
    ...
    let plugin_registry = ainxt_agent::plugins::PluginRegistry::from_discovered(
    ...
    InspectReport {
        ainxt_version: ainxt_version::VERSION.to_string(),
...
fn instruction_file_type(
    ...
    if has_rules_directory(file_path, ".ainxt")
...
async fn list_instructions(cwd: &Path) -> Vec<InstructionFile> {
    ...
    let configs = ainxt_agent::prompt::agents_md::read_agents_config_with_paths(
        &cwd.display().to_string(),
        ainxt_agent::prompt::skills::CompatConfig::default(),
    )
    .await;

    let ainxt_home = Some(crate::util::ainxt_home::ainxt_home());
    ...
    // `ainxt_agent::prompt::agents_md::read_agents_config_with_paths`
...
/// which handles both ainxt TOML and vendor settings fallback in one codepath.
async fn list_permissions(cwd: &Path) -> PermissionsReport {
    use ainxt_workspace::permission::resolution;
...
/// (`[ainxt_com_config]`, the `[auth]` alias, and env overrides) so admins can
...
fn login_policy_report(config: Option<&crate::agent::config::Config>) -> LoginPolicyReport {
    let ainxt_com_config = config
        .map(|c| c.ainxt_com_config.clone())
        .unwrap_or_default();
    LoginPolicyReport {
        api_key_auth_disabled: ainxt_com_config.api_key_auth_disabled(),
        disable_api_key_auth: ainxt_com_config.disable_api_key_auth,
        force_login_team_uuid: ainxt_com_config.force_login_team_uuid,
    }
}
...
fn list_hooks(
    ...
    discovered_plugins: &[ainxt_agent::plugins::DiscoveredPlugin],
) -> Vec<HookEntry> {
    let all_on = ainxt_tools::types::compat::CompatConfig::default();
    ...
    let (registry, _errors) =
        ainxt_hooks::discovery::load_hooks_from_sources(&global_sources, &project_sources);
    ...
    let mut entries: Vec<HookEntry> = registry
        ...
        .into_iter()
        .map(|h| {
            let is_user_scope = h.source_dir.starts_with(&ainxt_home)
...
async fn list_skills(
    cwd: &Path,
    plugin_registry: &ainxt_agent::plugins::PluginRegistry,
    skills_config: &ainxt_agent::prompt::skills::SkillsConfig,
) -> Vec<SkillEntry> {
    // Discover with all vendors ON so inspect shows the full set.
    let skills = ainxt_agent::prompt::skills::list_skills_with_plugins(
    ...
    skills
        .into_iter()
        .map(|s| {
            let source = skill_entry_source(&s, &ainxt_home);
...
/// `<ainxt_home>/skills/<name>/SKILL.md` and discovered as user skills, so a
...
/// discovery: deployed pagers parse `ainxt.dev/skills/list` into a typed
...
fn skill_entry_source(
    s: &ainxt_agent::prompt::skills::SkillInfo,
    ainxt_home: &Path,
) -> ConfigSource {
    use ainxt_tools::implementations::skills::types::SkillScope;
    ...
    match s.scope {
        ...
        SkillScope::User => {
            if crate::builtin::is_extracted_bundled_skill(&s.name, &path, ainxt_home) {
...
fn list_agents(
    ...
    let agents = ainxt_agent::discovery::all_subagents_with_plugins(
        cwd,
        &HashMap::new(),
        Some(plugin_registry),
    );
...
fn list_plugins(discovered: &[ainxt_agent::plugins::DiscoveredPlugin]) -> Vec<PluginEntry> {
    discovered
        .iter()
        .map(|p| {
            let scope = match p.scope {
                ainxt_agent::plugins::PluginScope::CliOverride => Scope::Cli,
                ainxt_agent::plugins::PluginScope::Project => Scope::Project,
                ainxt_agent::plugins::PluginScope::User => Scope::User,
                ainxt_agent::plugins::PluginScope::ConfigPath => Scope::Config,
            };
            PluginEntry {
                ...
                provides: PluginProvides {
                    ...
                    skills: ainxt_agent::plugins::registry::skill_md_paths(&p.skill_dirs).len(),
...
fn list_marketplaces(git_root: Option<&Path>) -> Vec<MarketplaceEntry> {
    ...
    ainxt_agent::plugins::marketplace::resolve(root)
        .into_iter()
...
fn list_lsp_servers(
    ...
    let servers = ainxt_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
        cwd,
        &plugin_lsp_paths,
        &inline_values,
        &plugin_names,
    ...
    // clone so the listing matches the live gate. `remote = None` matches
    // `ainxt mcp doctor` (no loaded RemoteSettings in a standalone command).
    crate::agent::folder_trust::resolve_and_record(cwd, None, false);
...
/// `.ainxt/config.toml` files (via `find_project_configs`). The macOS MDM
...
fn list_config_sources(cwd: &Path) -> ConfigSources {
    ...
    // User managed
    if let Some(home) = crate::config::user_ainxt_home() {
...
fn print_human(r: &InspectReport) {
    println!();
    println!("  Environment");
    println!("  {TREE} Version: {} [{}]", r.ainxt_version, r.channel);
    ...
    if r.mcp_servers.is_empty() {
        ...
        println!("  {TREE} (none) \u{2014} see `ainxt mcp add --help`");
    } else {
        print_columns(
            "MCP Servers",
            &r.mcp_servers,
...
mod tests {
    use super::*;
    use ainxt_agent::prompt::skills::{SkillInfo, SkillsConfig};
    use ainxt_tools::implementations::skills::types::SkillScope;
    ...
    fn vendor_rule_paths_select_rules_compatibility_cells() {
        ...
        for path in ["/repo/.ainxt/rules/team.md", r"C:\repo\.ainxt\rules\team.md"] {
    ...
    fn requirements_layer_contributes_requires_non_empty_post_strip_table() {
        ...
        let path = "/home/u/.ainxt/requirements.toml";
    ...
    fn model_override_warnings_inspect_smoke() {
        ...
        assert!(cfg.config_models.contains_key("ainxt-4.5"));

        let human = render_model_override_warnings(&warnings);
        assert!(human.contains("Model Overrides"), "{human}");
        assert!(
            human.contains("[model.\"ainxt-4.5\"] send_compactions_remaining"),
            "{human}"
        );
        assert!(
            human.contains("[model.\"ainxt-4.5\"] reasoning_effort"),
            "{human}"
        );
        ...
        assert_eq!(alias_warning["modelKey"], "ainxt-4.5");
    ...
    fn skill_entry_source_maps_scopes() {
        let home = Path::new("/home/u/.ainxt");

        let s = skill_fixture("a", "/repo/.ainxt/skills/a/SKILL.md", SkillScope::Local);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::Project { .. }
        ));

        let s = skill_fixture("b", "/repo/.ainxt/skills/b/SKILL.md", SkillScope::Repo);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::Project { .. }
        ));

        let s = skill_fixture("c", "/home/u/.ainxt/skills/c/SKILL.md", SkillScope::User);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::User { .. }
        ));
        ...
        let s = skill_fixture(
            "d",
            "/home/u/.ainxt/server-skills/d/SKILL.md",
            SkillScope::Server,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
        ...
        let s = skill_fixture("e", "/home/u/.ainxt/bundled/e/SKILL.md", SkillScope::Bundled);
        assert!(matches!(
            skill_entry_source(&s, home),
    ...
    /// path `<ainxt_home>/skills/<name>/SKILL.md`; a same-named skill anywhere
    ...
    fn skill_entry_source_relabels_extracted_bundled_skills() {
        ...
        let s = skill_fixture(
            "help",
            "/home/u/.ainxt/skills/help/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
        ...
        let s = skill_fixture("help", "/repo/.ainxt/skills/help/SKILL.md", SkillScope::Repo);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::Project { .. }
        ));

        // Bundled name in a user dir outside <ainxt_home>/skills: stays user.
        let s = skill_fixture(
            "help",
        ...
        // Bundled frontmatter name in a different dir under <ainxt_home>/skills:
        // not the extracted copy — stays user.
        let s = skill_fixture(
            "help",
            "/home/u/.ainxt/skills/my-tools/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
        ...
        // Non-bundled name under <ainxt_home>/skills: stays user.
        let s = skill_fixture(
            "my-skill",
            "/home/u/.ainxt/skills/my-skill/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
    ...
    async fn list_skills_honors_skills_config() {
        ...
        // Test-unique names: discovery also reads this machine's real ~/.ainxt dirs.
        ...
        let registry = ainxt_agent::plugins::PluginRegistry::from_discovered(vec![], &[], &[]);
