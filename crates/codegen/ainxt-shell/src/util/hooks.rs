...
use ainxt_hooks::discovery::HookSource;
...
pub fn discover_hook_source_paths(
    ...
    // sources. Native .ainxt/hooks/ directories are still scanned (they hold
    ...
    // user_ainxt_home() is None when no home resolves, so inspect lists the same
    // sources a live session loads, instead of a cwd-relative .ainxt.
    ...
    if let Some(ref ainxt) = ainxt {
        global.push(ainxt.join("hooks"));
    }

    let custom_paths: Vec<PathBuf> = ainxt
        .as_ref()
    ...
    if let Some(root) = git_root {
        ...
        project.push(root.join(".ainxt").join("hooks"));
        if !skip_cursor {
...
pub fn discover_hooks(
    ...
    ainxt_hooks::discovery::HookRegistry,
    Vec<ainxt_hooks::error::HookError>,
) {
