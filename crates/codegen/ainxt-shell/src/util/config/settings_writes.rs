...
/// canonical theme name (`ainxtnight`, `tokyonight`, `auto`, etc.).
...
///
/// Caller must validate against the model catalog. Empty string restores the
/// "no opinion" sentinel (there is no bundled default model). Length >
/// [`MAX_DEFAULT_MODEL_LEN`] → `Err`.
