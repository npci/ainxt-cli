...
use ainxt_tools::types::compat::{CompatConfig, CompatConfigToml};

pub use ainxt_mcp::oauth_config::{McpOAuthConfig, McpOAuthConfigMap};
// MCP server config value types extracted to `ainxt-config-types` (config
// dependency inversion); re-exported so `crate::util::config::*` paths keep working.
pub use ainxt_config_types::{McpJsonOAuthBlock, McpServerConfig, McpServerTransportConfig};
...
pub use ainxt_config_types::{McpConfig, RelaySyncConfig};
// Worktree-pool config value type extracted; re-exported to keep paths stable.
pub use ainxt_config_types::PoolConfig;
...
/// Walks from cwd up to the git repo root checking `.ainxt/config.toml` at each level.
/// Project-scoped `.ainxt/config.toml` entries override global `~/.ainxt/config.toml`
...
/// `.ainxt/config.toml`, otherwise user (global config, `~/.claude.json`,
...
/// Load MCP servers with project-scoped overrides from `.ainxt/config.toml`.
///
/// Merge strategy:
/// 1. Load MCP servers from global `~/.ainxt/config.toml`
/// 2. Walk from git repo root down to `cwd`, loading `.ainxt/config.toml` at each level
        ...
        if let Ok(root) = crate::config::load_config_file(config_path) {
            ...
            if !project_servers.is_empty() {
                tracing::info!(
                    ...
                    "Loaded project-scoped MCP servers from .ainxt/config.toml"
                );
...
///
/// Uses the top-level `disabled_mcp_servers` array in `~/.ainxt/config.toml`.
...
/// Upsert an MCP server entry in `~/.ainxt/config.toml`.
///
...
/// config file, e.g. a project-scoped `.ainxt/config.toml`.
...
/// Delete an MCP server entry from `~/.ainxt/config.toml`.
///
...
/// config file, e.g. a project-scoped `.ainxt/config.toml`. OAuth credential
...
pub async fn delete_mcp_server_config_at(
    ...
    if let Ok(mut cred_store) = ainxt_mcp::credentials::McpCredentialStore::load_default() {
...
/// project-scoped `.ainxt/config.toml` files.
...
// `.mcp.json` discovery moved to `ainxt-workspace` (client-side, shared with
// the folder-trust gate); re-exported so `crate::util::config::*` paths keep working.
pub use ainxt_workspace::project_config::{
...
/// (`"user"` or `"project"`).
///
/// Overlays project-scoped `.ainxt/config.toml` files from `cwd` up to the
...
fn config_path() -> PathBuf {
    crate::util::ainxt_home::ainxt_home().join("config.toml")
}

/// Path to the user-level config file (`~/.ainxt/config.toml`).
pub fn user_config_path() -> PathBuf {
    config_path()
}

/// Path to a project-level config file (`<dir>/.ainxt/config.toml`).
pub fn project_config_path(dir: &std::path::Path) -> PathBuf {
    dir.join(".ainxt").join("config.toml")
}
...
"#;
    ...
    fn skills_config_parses_paths_and_ignore() {
        let root = toml::from_str::<TomlValue>(
            r#"
[skills]
paths = ["~/.ainxt/skills", "~/.ainxt/skills/special/SKILL.md"]
ignore = ["~/.ainxt/skills/noisy/SKILL.md"]
"#,
        )
        .unwrap();
        ...
        assert_eq!(
            cfg.paths,
            vec!["~/.ainxt/skills", "~/.ainxt/skills/special/SKILL.md"]
        );
        assert_eq!(cfg.ignore, vec!["~/.ainxt/skills/noisy/SKILL.md"]);
    }
...
"#;
    ...
    fn mcp_json_env_var_default_value() {
        ...
        std::fs::write(
            ...
            r#"{
                "mcpServers": {
                    "api": {
                        "url": "${AINXT_TEST_MCP_UNSET_VAR_12345:-https://fallback.example.com}/mcp"
                    }
                }
            }"#,
        )
    ...
    fn mcp_json_all_toml_names_includes_disabled() {
        ...
        std::fs::create_dir_all(&ainxt_dir).unwrap();
        std::fs::write(
            ainxt_dir.join("config.toml"),
            r#"
