...
pub fn resolve_zdr_access_enabled(
    ...
    BoolFlag::env("AINXT_ZDR_ACCESS_ENABLED")
...
/// fetches from ainxt backends are allowed, including the deployment-config sync
...
/// tier stays fail-open: it is a preference, not deployment policy. Matches
...
mod tests {
    ...
    fn remote_fetch_system_and_mdm_tiers_follow_the_walk() {
        ...
        // system managed layer (matches effective_config merge order), and
        ...
        // beats both (matches requirements_layers apply order).
        let mut layers = empty_layers();
        layers.user_requirements = Some(features_remote_fetch(true));
    ...
    fn remote_fetch_layer_load_failure_still_honors_policy_pins() {
        ...
        // Precedence matches the Ok-arm walk: requirements > managed > system managed.
        assert!(remote_fetch_enabled_from_policy_layers(
            Some(&on),
            Some(&off),
            Some(&off)
