pub const ENV_SYSTEM_PROMPT_LABEL: &str = "AINXT_SYSTEM_PROMPT_LABEL";

pub const DEFAULT_SYSTEM_PROMPT_LABEL: &str = ainxt_agent::DEFAULT_SYSTEM_PROMPT_LABEL;

/// Resolve system-prompt identity label.
/// Precedence: env → config per-model → `[agent]` → GB per-model → GB global →
/// `"ainxt"`. Empty/whitespace falls through.
///
...
mod system_prompt_label_tests {
    ...
    /// Serialize access to `AINXT_SYSTEM_PROMPT_LABEL` and clear it for tier tests.
