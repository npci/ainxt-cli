...
pub(crate) const ENV_AUTO_PERMISSION_MODE: &str = "AINXT_AUTO_PERMISSION_MODE";

/// Crate-wide serialization lock for tests that mutate
/// `AINXT_AUTO_PERMISSION_MODE`. Every test reading the gate (here and in
...
/// two can't drift. Precedence: requirement > env (`AINXT_AUTO_PERMISSION_MODE`)
...
///
/// requirements > env (`AINXT_AUTO_PERMISSION_MODE`) > `[auto_mode] enabled` in
...
/// layer (matches goal's model resolvers); the gate's own env layer is handled by
...
pub fn auto_mode_classifier_defaults(
    ...
    ainxt_workspace::permission::ClassifierPromptType,
    Option<ainxt_sampling_types::ReasoningEffort>,
) {
    let prompt_type = cfg
        .prompt_type
        .unwrap_or(ainxt_workspace::permission::ClassifierPromptType::Full);
    let reasoning_effort = cfg.reasoning_effort.or_else(|| {
        effective_supports_reasoning_effort.then_some(ainxt_sampling_types::ReasoningEffort::Low)
    });
...
mod auto_permission_mode_gate_tests {
    ...
    // `AINXT_AUTO_PERMISSION_MODE` is process-global; serialize every test that
