...
/// [`ainxt_config::env_bool`] parser (`AINXT_TOOLS_FIND_BFS` /
/// `AINXT_TOOLS_GREP_UGREP`, plus the `AINXT_FIND_BFS` / `AINXT_GREP_UGREP` aliases).
///
/// Pass the **merged** requirements ([`crate::config::load_merged_requirements`])
/// so an org policy in any requirements layer — not only
/// `~/.ainxt/requirements.toml` — is honored. Returns `(find_bfs, grep_ugrep)`,
/// which the caller bakes into a
/// [`ainxt_tools::computer::local::SearchShadowConfig`] on the local terminal
/// backend.
pub fn resolve_search_tools_enabled(
    ...
    let disable = ainxt_config::env_bool("DISABLE_EMBEDDED_SEARCH_TOOLS");
    ...
    let resolve = |primary: &str, alias: &str, key: &str| -> bool {
        let env = ainxt_config::env_bool(primary).or_else(|| ainxt_config::env_bool(alias));
    ...
    (
        resolve("AINXT_TOOLS_FIND_BFS", "AINXT_FIND_BFS", "find_bfs"),
        resolve("AINXT_TOOLS_GREP_UGREP", "AINXT_GREP_UGREP", "grep_ugrep"),
    )
}
...
const ENV_PERSISTENT_SHELL: &str = "AINXT_PERSISTENT_SHELL";
...
mod persistent_local_shell_tests {
    ...
    // AINXT_PERSISTENT_SHELL is process-global (the documented kill-switch a dev
...
/// the shared [`ainxt_config::env_bool`] via `BoolFlag`). The secs env var
/// lives in the tools crate (`RESPONSE_TIMEOUT_ENV`), parsed once there.
const ENV_ASK_USER_QUESTION_TIMEOUT_ENABLED: &str = "AINXT_ASK_USER_QUESTION_TIMEOUT_ENABLED";
...
///
/// Precedence: requirements > env (`AINXT_ASK_USER_QUESTION_TIMEOUT_ENABLED`)
...
fn resolve_ask_user_question_timeout_secs_from_tiers(
    ...
    requirement
        ...
        .or(remote)
        .unwrap_or(
            ainxt_tools::implementations::ainxt_build::ask_user_question::RESPONSE_TIMEOUT
                .as_secs(),
        )
}
...
///
/// Precedence: requirements > env (`AINXT_ASK_USER_QUESTION_TIMEOUT_SECS`,
...
fn resolve_ask_user_question_timeout_secs(
    ...
    resolve_ask_user_question_timeout_secs_from_tiers(
        ...
        ainxt_tools::implementations::ainxt_build::ask_user_question::response_timeout_env_secs(),
...
) -> ainxt_tools::implementations::ainxt_build::ask_user_question::AskUserQuestionParams {
    let requirements = crate::config::load_merged_requirements();
    let layers = match crate::config::ConfigLayers::load() {
        Ok(l) => Some(l),
        Err(e) => {
...
mod ask_user_question_timeout_tests {
    ...
    use ainxt_tools::implementations::ainxt_build::ask_user_question::RESPONSE_TIMEOUT_ENV;
    ...
    fn timeout_secs_tier_precedence() {
        let d = ainxt_tools::implementations::ainxt_build::ask_user_question::RESPONSE_TIMEOUT
            .as_secs();
...
mod tests {
    ...
    // Assumes AINXT_TOOLS_* / DISABLE_EMBEDDED_SEARCH_TOOLS are unset in the test env.
    #[test]
