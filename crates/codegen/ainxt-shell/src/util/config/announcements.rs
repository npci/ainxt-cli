...
/// Re-exported from `ainxt-announcements` for backward compatibility.
pub use ainxt_announcements::RemoteAnnouncement;
...
/// Dev/test override for announcements via `AINXT_ANNOUNCEMENTS_OVERRIDE` (a JSON
...
pub fn announcements_override() -> Option<Vec<RemoteAnnouncement>> {
    let raw = std::env::var("AINXT_ANNOUNCEMENTS_OVERRIDE").ok()?;
    match serde_json::from_str::<Vec<RemoteAnnouncement>>(&raw) {
        Ok(list) => Some(list),
        Err(_) => {
            tracing::warn!("invalid AINXT_ANNOUNCEMENTS_OVERRIDE JSON; ignoring");
            None
        }
    }
}
...
/// `AINXT_ANNOUNCEMENTS_OVERRIDE` env var overrides everything (dev-only escape hatch).
