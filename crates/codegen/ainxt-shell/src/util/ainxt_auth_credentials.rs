...
use std::sync::Arc;
/// Credentials for authenticating with ainxt backend services.
///
...
/// User token (ainxt users) sends `Bearer` + `X-Ainxt-Token-Auth: ainxt-cli`.
/// Deployment key takes precedence when both are present.
#[derive(Clone)]
pub struct AinxtAuthCredentials {
...
impl std::fmt::Debug for AinxtAuthCredentials {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("AinxtAuthCredentials")
            .field(
                "user_token",
...
impl AinxtAuthCredentials {
    ...
    pub fn auth_error_hint(&self) -> &'static str {
        if self.deployment_key.is_some() {
            "Your AINXT_DEPLOYMENT_KEY is invalid or expired. Please contact a team admin."
        } else if self.user_token.is_some() {
            "Your auth token is invalid or expired. Run `ainxt login` to re-authenticate."
        } else {
            "Not authenticated."
        }
    }
    ...
    pub fn resolve(&self) -> AinxtAuthCredentials {
    ...
    /// the bearer.
    pub async fn resolve_async(&self) -> AinxtAuthCredentials {
    ...
    pub fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        let builder = if let Some(ref key) = self.deployment_key {
            ...
            builder
                ...
                .header(
                    obfstr::obfstr!("X-Ainxt-Token-Auth"),
                    obfstr::obfstr!("ainxt-cli"),
                )
        } else {
            builder
        };
...
impl ainxt_auth::HttpAuth for AinxtAuthCredentials {
    fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        AinxtAuthCredentials::apply(self, builder, base_url)
    }
}
#[cfg(test)]
mod tests {
    ...
    fn resolve_returns_token_when_not_expired() {
        ...
        let creds = AinxtAuthCredentials::new(None).with_auth_manager(mgr);
    ...
    fn resolve_returns_token_during_early_invalidation_window() {
        ...
        let creds = AinxtAuthCredentials::new(None).with_auth_manager(mgr.clone());
    ...
    fn resolve_returns_static_token_when_no_auth_manager() {
        let creds = AinxtAuthCredentials::new(Some("static-token".into()));
