...
//!       [--api-key <key> | $AINXT_API_KEY | <ainxt-home>/auth.json] \
//!       [--min-confidence 0.7] \
//!       [--include-reasoning true] \
//!       [--ainxt-home <path>]
//!
//! The binary name is `trace_classify` (underscore) — that's the file
//! name in `src/bin/`, which cargo's auto-discovery uses unchanged.
...
use clap::Parser;
use ainxt_shell::trace_classifier::{RunArgs, run, validate_min_confidence};
...
struct Cli {
    ...
    /// has access to.
    #[arg(long, default_value = "ainxt-4.5")]
    model: String,

    /// Sampler base URL.
    #[arg(long, default_value = "https://api.example.test/v1")]
    api_base_url: String,

    /// API key. Overrides `$AINXT_API_KEY` when set; falls back to
    /// `$AINXT_API_KEY`, then `<ainxt-home>/auth.json` (`ainxt::api_key`
    ...
    /// shell uses (`$AINXT_HOME` or `~/.ainxt`). Exposed primarily for
    /// tests / sandboxed invocations.
    #[arg(long)]
    ainxt_home: Option<PathBuf>,
}
...
mod tests {
    ...
    fn cli_parses_minimal_args() {
        ...
        assert_eq!(cli.api_base_url, "https://api.example.test/v1");
        assert!(cli.output.is_none());
        assert!(cli.api_key.is_none());
        assert!(cli.min_confidence.is_none());
        assert!(cli.include_reasoning.is_none());
        assert!(cli.ainxt_home.is_none());
    }
    ...
    #[test]
    fn cli_ainxt_home_override_parses() {
        ...
        .expect("parse");
        assert_eq!(cli.ainxt_home, Some(PathBuf::from("/tmp/scratch-ainxt")));
    }
    ...
    fn cli_defaults_match_documented_values() {
        ...
        assert_eq!(by_id("model"), vec!["ainxt-4.5"]);
        assert_eq!(by_id("api_base_url"), vec!["https://api.example.test/v1"]);
