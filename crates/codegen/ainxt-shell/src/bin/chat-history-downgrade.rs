...
use clap::Parser;
use ainxt_shell::sampling::conversation::{ConversationItem, conversation_item_to_chat_message};
use ainxt_shell::sampling::types::{ChatRequestMessage, Role};
...
/// Restrict `path` to owner-only read/write (0600) on Unix. Best-effort:
/// a failure here (e.g. unsupported filesystem) must not abort the
/// conversion, so errors are logged and swallowed.
#[cfg(unix)]
fn set_owner_only_permissions(path: &std::path::Path) {
    use std::os::unix::fs::PermissionsExt;
    if let Err(e) = std::fs::set_permissions(path, std::fs::Permissions::from_mode(0o600)) {
        eprintln!("warning: failed to set owner-only permissions on {path:?}: {e}");
    }
}
...
fn main() -> anyhow::Result<()> {
    ...
    // The output file carries the same chat/reasoning content as the
    // input, which may include user prompts and PII -- lock it down to
    // owner-only (0600) on Unix rather than leaving it at the process's
    // ambient umask-derived default.
    set_owner_only_permissions(&args.output);
    ...
    writer.flush()?;
    // Explicitly release the buffered writer (and its underlying file
    // handle) once flushed, rather than relying solely on end-of-scope Drop.
    drop(writer);
...
mod tests {
    ...
    /// Create `path` (truncating if it exists) with owner-only read/write
    /// (0600) applied atomically at open time via `OpenOptions::mode`,
    /// rather than as a follow-up `set_permissions()` call. Mirrors the
    /// technique used by `create_owner_only_file` in
    /// `ainxt-codebase-graph::navigation` tests, applied here so these
    /// test fixtures use the same hardening shape as production call
    /// sites that create files carrying sensitive content. Best-effort
    /// mode on non-Unix platforms, which lack POSIX mode bits.
    fn create_owner_only_file(path: &std::path::Path) -> File {
    ...
    fn test_assistant_with_reasoning() {
        let v1 = r#"{"type":"assistant","content":"The answer is 42.","reasoning":{"text":"Let me think..."},"tool_calls":[],"model_id":"ainxt-3"}"#;
        let out = convert_line_for_test(v1);
        let v = v0_value(&out);
        assert_eq!(v["role"], "assistant");
        assert_eq!(v["content"], "The answer is 42.");
        assert_eq!(v["reasoning_content"], "Let me think...");
        assert_eq!(v["model_id"], "ainxt-3");
    }
    ...
    fn test_sibling_reasoning_folds_into_following_assistant() {
        ...
        let a_line = r#"{"type":"assistant","content":"The answer is 42.","tool_calls":[],"model_id":"ainxt-3"}"#;
    ...
    fn test_full_file_roundtrip() {
        ...
        let mut writer = BufWriter::new(create_owner_only_file(&output_path));
        let mut count = 0;
        let mut pending = Vec::new();
        for line in reader.lines() {
