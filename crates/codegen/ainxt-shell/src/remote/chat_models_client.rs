//! ainxt.dev chat-product model catalog (`POST /rest/modes`) — the models
//! ainxt-web's chat picker shows, distinct from the CLI `/v1/models` build
...
const AINXT_WEB_URL: &str = "https://ainxt.example.test";
...
pub enum ChatModelsError {
    #[error("no ainxt.dev credentials")]
    NoAuth,
...
impl ChatModelsClient {
    pub fn new(auth: Arc<AuthManager>) -> Self {
        let base_url = std::env::var("AINXT_MODES_BASE_URL")
            .ok()
            .filter(|s| !s.is_empty())
            .or_else(|| {
                std::env::var("AINXT_CONVERSATIONS_BASE_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .or_else(|| {
                std::env::var("AINXT_CODE_WEB_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .unwrap_or_else(|| AINXT_WEB_URL.to_string());
        Self {
    ...
    /// Gated only on a valid ainxt.dev bearer — deliberately NOT `is_ainxt_auth()`
    ...
    pub async fn list_modes(&self, locale: &str) -> Result<ListModesResponse, ChatModelsError> {
        ...
        let mut builder = self
            ...
            .header(
                "X-Ainxt-Token-Auth",
                self.auth.ainxt_com_config().token_header.clone(),
            )
