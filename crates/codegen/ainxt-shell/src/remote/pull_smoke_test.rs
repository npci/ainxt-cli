...
//!
//! Run with: `cargo test -p ainxt-shell -- pull_smoke --ignored --nocapture`
...
mod tests {
    ...
    fn load_prod_auth() -> Option<AinxtAuth> {
        let path = crate::util::ainxt_home::ainxt_home().join("auth.json");
        let contents = std::fs::read_to_string(&path).ok()?;
        let store: BTreeMap<String, AinxtAuth> = serde_json::from_str(&contents).ok()?;
    ...
    async fn smoke_push_pull_round_trip() {
        ...
        let auth = load_prod_auth().expect("No auth.json — run `ainxt login`");
