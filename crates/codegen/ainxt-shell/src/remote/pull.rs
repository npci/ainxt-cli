...
pub async fn pull_session_to_local(
    ...
    let cwd = match remote.cwd.as_ref() {
        ...
        None => {
            tracing::warn!(
                ...
                "Cannot pull session: backend has cwd=null"
            );
    ...
    tracing::info!(
        session_tag = %crate::util::session_log_tag(session_id),
        %cwd, num_messages, "Pulled session from backend"
    );
...
pub(crate) mod hydrate {
    ...
    fn write_summary(
        ...
        let summary = Summary {
            ...
            head_branch: None,
            request_id: None,
            // Record the *local* ainxt_home (where this hydrated copy lives),
            ...
            sandbox_profile: ainxt_sandbox::configured_profile_name().map(String::from),
    ...
    pub(super) fn write_updates(
        ...
        // Explicitly release the buffered writer (and the underlying file
        // handle) now that the flush succeeded, rather than relying solely
        // on end-of-scope Drop.
        drop(w);
        Ok(())
    }
    ...
    impl ChatReducer {
        ...
        fn process(
            ...
            match update {
                ...
                SessionUpdate::Ainxt(n) => self.handle_ainxt(&n.update),
            }
        }
        ...
        fn handle_ainxt(
            ...
            use crate::extensions::notification::SessionUpdate as AinxtUpdate;

            match update {
                AinxtUpdate::CompactionCheckpoint(_) => {
                    self.reset();
    ...
    const REPLAYABLE_METHODS: &[&str] = &["session/update", "_ainxt.dev/session/update"];
