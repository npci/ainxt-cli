...
use crate::auth::{AinxtAuth, AinxtComConfig};
...
const AINXT_CODE_BACKEND_URL: &str = "https://code.example.test";
const DEFAULT_TIMEOUT: Duration = Duration::from_secs(30);
const AINXT_CODE_WEB_URL: &str = "https://ainxt.example.test";
/// Build a share URL from a permission ID
pub fn share_url(permission_id: &str) -> String {
    let web_url =
        std::env::var("AINXT_CODE_WEB_URL").unwrap_or_else(|_| AINXT_CODE_WEB_URL.to_string());
...
fn add_cli_chat_proxy_headers_blocking(
    ...
    let mut builder = builder
        ...
        .header("X-Ainxt-Token-Auth", AinxtComConfig::default().token_header)
...
async fn add_bundle_fetch_headers(
    ...
    let mut credentials = crate::util::ainxt_auth_credentials::AinxtAuthCredentials::new(
    ...
    ainxt_file_utils::trace_context::inject_trace_context_into_request(builder)
}
...
async fn fetch_bundle_inner(
    ...
    let client: reqwest_middleware::ClientWithMiddleware = if let Some(am) = auth_manager {
        let provider: std::sync::Arc<dyn ainxt_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
...
impl BackendClient {
    ...
    pub fn new() -> Self {
        ...
        Self {
            ...
            reqwest_client,
            base_url: std::env::var("AINXT_CODE_BACKEND_URL")
                .unwrap_or_else(|_| AINXT_CODE_BACKEND_URL.to_string()),
            auth_manager: None,
        }
    }
    ...
    /// instead of requiring the caller to pass `&AinxtAuth`.
    pub fn with_auth_manager(mut self, manager: std::sync::Arc<crate::auth::AuthManager>) -> Self {
        let credentials: std::sync::Arc<dyn ainxt_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
                crate::auth::credential_provider::ShellAuthCredentialProvider::new(
                    manager.clone(),
                    None,
    ...
    async fn resolve_auth(&self) -> Result<AinxtAuth, BackendError> {
        let manager = self
            .auth_manager
            .as_ref()
    ...
    /// Must include X-Ainxt-Token-Auth so nginx auth subrequest routes to authenticate_ainxt_cli_token.
    /// See: crates/codegen/ainxt-shell/src/agent/app.rs:run_headless
    async fn auth_header_map(&self) -> Result<reqwest::header::HeaderMap, BackendError> {
        ...
        headers.insert(
            "X-Ainxt-Token-Auth",
            required(&AinxtComConfig::default().token_header, "X-Ainxt-Token-Auth")?,
        );
        ...
        if let Ok(v) = HeaderValue::from_str(&crate::http::process_client_identifier()) {
            headers.insert("x-ainxt-client-identifier", v);
        }
        headers.insert(
            crate::http::CLIENT_MODE_HEADER,
        ...
        headers.insert(
            "x-ainxt-client-version",
            HeaderValue::from_static(ainxt_version::VERSION),
        );
        Ok(headers)
    }
    async fn send_with_auth(
        ...
        let builder = ainxt_file_utils::trace_context::inject_trace_context_into_request(
...
/// Fetch `ainxt_build_login_device_flow` from cli-chat-proxy `GET /v1/login-config`.
///
/// Unauthenticated (pre-login); `x-ainxt-agent-id` is the per-install bucketing key.
...
pub async fn fetch_login_device_flow(cli_chat_proxy_base_url: &str) -> Option<bool> {
    let agent_id = tokio::task::spawn_blocking(ainxt_telemetry::id::agent_id)
        .await
        .ok()?;
    ...
    let response = client
        ...
        .header("x-ainxt-agent-id", agent_id)
        .header("x-ainxt-client-version", ainxt_version::VERSION)
        .header(
            "x-ainxt-client-identifier",
            crate::http::process_client_identifier(),
...
impl ListModelsEndpoint {
    fn from_endpoints(
        ...
        if endpoints.has_custom_endpoint() {
            ...
            Self {
                url: format!("{}/models", endpoints.ainxt_api_base_url),
                auth: EndpointAuth::ApiKey,
            }
        } else {
            Self {
...
/// Build a session credential from the token persisted by `ainxt login`.
///
/// Token logins store only the bearer (in `~/.ainxt/credentials.json`, surfaced
/// as `endpoints.deployment_key`); they never write the OAuth `auth.json` that
/// backs the `AuthManager`. This lets the model-catalog fetch present that
/// bearer as a session credential to the gateway. Returns `None` when no token
/// is available.
fn deployment_session_auth(
    endpoints: &crate::agent::config::EndpointsConfig,
) -> Option<AinxtAuth> {
    let key = endpoints
        .deployment_key
        .clone()
        .filter(|k| !k.is_empty())?;
    let stored = crate::auth::gateway_auth::load_credentials();
    Some(AinxtAuth {
        key,
        auth_mode: crate::auth::AuthMode::WebLogin,
        email: stored.and_then(|c| c.email),
        ..AinxtAuth::default()
    })
}
...
pub(crate) fn fetch_models_blocking(
    ...
    // Token logins (`ainxt login`) persist the bearer to
    // `~/.ainxt/credentials.json` and surface it as `endpoints.deployment_key`,
    // but they never populate the OAuth `auth.json` the `AuthManager` reads, so
    // `auth` arrives `None` here. Synthesize a session credential from the
    // stored token so the catalog fetch can authenticate against the gateway.
    let fallback_auth = if auth.is_none() {
        deployment_session_auth(endpoints)
    } else {
        None
    };
    let auth = auth.or(fallback_auth.as_ref());
    match source.auth {
        EndpointAuth::ApiKey => {
            let api_key = crate::agent::auth_method::read_ainxt_api_key_env()
                .or_else(|_| {
                ...
                .map_err(|_| {
                    BackendError::Auth(
                        "No API key for custom models endpoint. Set AINXT_API_KEY.".into(),
                    )
                })?;
...
pub fn parse_remote_model_value(
    ...
    Some(crate::agent::config::ModelEntryConfig {
        ...
        reasoning_efforts: obj
            ...
            .map(|arr| ainxt_sampling_types::parse_reasoning_effort_options(arr))
            .unwrap_or_default(),
        ...
        compactions_remaining: obj
            ...
            .or_else(|| {
                obj
                    ...
                    .and_then(|v| v.as_bool())
                    .map(ainxt_sampling_types::CompactionsRemaining::Dynamic)
            }),
...
mod tests {
                ...
                get(
                    |State(state): State<LoginConfigServerState>, headers: HeaderMap| async move {
                        state.seen.lock().unwrap().push(LoginConfigHeaders {
                            ...
                            agent_id: header_str(&headers, "x-ainxt-agent-id"),
                            client_identifier: header_str(&headers, "x-ainxt-client-identifier"),
                            client_version: header_str(&headers, "x-ainxt-client-version"),
                        });
    ...
    async fn fetch_login_device_flow_sends_only_unauthenticated_headers() {
        ...
        assert!(
            ...
            "must send x-ainxt-agent-id (the bucketing key)"
        );
        assert!(
            h.client_identifier.is_some(),
            "must send x-ainxt-client-identifier"
        );
        assert!(
            h.client_version.is_some(),
            "must send x-ainxt-client-version"
        );
    ...
    fn test_auth_manager() -> Arc<crate::auth::AuthManager> {
        ...
        let mgr = crate::auth::AuthManager::new(dir.path(), crate::auth::AinxtComConfig::default());
    ...
    async fn fetch_subagent_bundle_success() {
        ...
        assert_eq!(headers.token_auth.as_deref(), Some("ainxt-cli"));
    ...
    fn parse_openai_format_uses_id_field() {
        let value = serde_json::json!(
            { "id" : "ainxt-3", "object" : "model", "owned_by" : "ainxt", "context_window" :
            131072 }
        );
        let result = parse_remote_model_value(&value, "https://api.example.test/v1").unwrap();
        assert_eq!(result.model, "ainxt-3");
        assert_eq!(result.base_url, "https://api.example.test/v1");
        assert_eq!(result.name.as_deref(), Some("ainxt-3"));
    }
    #[test]
    ...
    fn parse_reads_reasoning_effort_fields() {
        ...
        let value = serde_json::json!(
            { "model" : "ainxt-4.5", "context_window" : 1_000_000,
        ...
        let value = serde_json::json!(
            { "model" : "ainxt-4.5", "contextWindow" : 1_000_000,
    ...
    fn parse_reads_reasoning_efforts_list() {
        use ainxt_sampling_types::ReasoningEffort;
        let value = serde_json::json!(
            { "model" : "ainxt-4.5", "context_window" : 1_000_000, "reasoning_efforts" :
    ...
    fn parse_remote_model_value_no_laziness_detector_block_yields_default() {
        let value = serde_json::json!(
            { "model" : "ainxt-4", "context_window" : 256_000, }
        );
        let result = parse_remote_model_value(&value, "https://default.url").unwrap();
        assert_eq!(
    ...
    fn parse_remote_model_value_parses_camelcase_key() {
        let value = serde_json::json!(
            { "model" : "ainxt-4", "context_window" : 256_000, "lazinessDetector" : {
    ...
    fn parse_remote_model_value_parses_snake_case_laziness_detector() {
        let value = serde_json::json!(
            { "model" : "ainxt-4", "context_window" : 256_000, "laziness_detector" : {
    ...
    fn parse_remote_model_value_parses_meta_laziness_detector() {
        let value = serde_json::json!(
            { "model" : "ainxt-4", "context_window" : 256_000, "_meta" : {
    ...
    fn parse_reads_show_model_fingerprint_field() {
        let value = serde_json::json!(
            { "model" : "ainxt-build", "context_window" : 256_000,
            "show_model_fingerprint" : true }
        );
        let result = parse_remote_model_value(&value, "https://default.url").unwrap();
        assert!(result.show_model_fingerprint);
        let value = serde_json::json!(
            { "model" : "ainxt-build", "contextWindow" : 256_000, "showModelFingerprint" :
            true }
        );
        let result = parse_remote_model_value(&value, "https://default.url").unwrap();
        assert!(result.show_model_fingerprint);
        let value = serde_json::json!(
            { "model" : "ainxt-build", "context_window" : 256_000, "_meta" : {
    ...
    fn list_url_derived_from_base_url() {
        ...
        assert_eq!(ep.resolve_models_list_url(), "https://api.example.test/v1/models");
    }
    #[test]
    ...
    /// ApiKey → `ainxt_api_base_url` (ApiKey, public default when unset); a custom
    /// models endpoint → that URL unchanged.
    #[test]
    #[serial_test::serial]
    fn models_fetch_endpoint_matches_auth_mode() {
        ...
        // Asserted against the CONSTANT, not a literal. These two assertions
        // spent three weeks failing silently: the 2026-08-04 remediation changed
        // `CLI_CHAT_PROXY_BASE_URL_DEFAULT` to a local gateway -- because
        // `api.ainxt.dev` was found to be unregistered -- and left the tests
        // asserting the old hostname. Referencing the constant means the next
        // change to the default cannot leave this behind.
        let expected_models = format!(
            "{}/models",
            crate::agent::config::CLI_CHAT_PROXY_BASE_URL_DEFAULT
        );
        assert_eq!(session.url, expected_models);
        ...
        assert_eq!(deployment.url, expected_models);
        assert_eq!(deployment.auth, EndpointAuth::Session);
        let api = ListModelsEndpoint::from_endpoints(&cfg, ModelFetchAuth::ApiKey);
        assert_eq!(api.url, "https://inference.acme-corp.example/ainxt/v1/models");
        assert_eq!(api.auth, EndpointAuth::ApiKey);
        // An empty `[endpoints]` table falls back to the compiled default, so
        // assert against the constant for the same reason as above.
    ...
    /// REGRESSION: `ainxt setup` must send the deployment key to
    ...
    fn deployment_config_url_uses_cli_chat_proxy_when_not_overridden() {
        ...
        unsafe { std::env::set_var("AINXT_DEPLOYMENT_KEY", "ainxt-token-ENTERPRISE") };
        ...
        unsafe { std::env::remove_var("AINXT_DEPLOYMENT_KEY") };
    }
    #[derive(Clone)]
