...
/// Body for `PUT /rest/app-chat/conversations/{id}` (ainxt-web `chatUpdateConversation`).
...
impl ConversationsClient {
    pub fn new(auth: Arc<AuthManager>) -> Self {
        let base_url = std::env::var("AINXT_CONVERSATIONS_BASE_URL")
            .ok()
            .filter(|s| !s.is_empty())
            .or_else(|| {
    ...
    async fn require_ainxt_auth(&self) -> Result<AinxtAuth, ConvError> {
    ...
    pub async fn list_conversations(
        ...
        let auth = self.require_ainxt_auth().await?;
