...
pub enum ClientMode {
    /// Headless mode (ainxt agent, ainxt agent headless) - uses websocket relay.
    /// Leader connects to websocket relay once and forwards messages.
    Headless,
    /// Stdio mode (ainxt agent stdio, ainxt -p) - uses local IPC.
...
pub struct ClientCapabilities {
    ...
    /// Whether this client has advertised `ainxt.dev/codeNavigation.enabled`.
...
pub struct LeaderCapabilities {
    ...
    /// (driven by `ainxt update`). Old leaders default to `false`, so a new
...
pub enum ControlCommand {
    ...
    /// `ainxt update`). The leader stops admitting new turns, waits a bounded
    ...
    ///
    /// `to_version` is the version `ainxt update` just installed; the leader uses
...
mod tests {
    ...
    fn workspace_status_payload_roundtrip() {
        let payload = ControlPayload::WorkspaceStatus {
            ...
            uptime_ms: 4200,
            active_tool_calls: 2,
            sessions: vec!["ainxt-a".into(), "ainxt-b".into()],
            pid: 4242,
        };
