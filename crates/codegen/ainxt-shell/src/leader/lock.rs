...
use fs2::FileExt;
use ainxt_workspace::util::is_lock_contended;
...
pub fn compute_ws_url_suffix(ws_url: &str) -> String {
    ...
    // `ainxt_ws_url` is always the *relay* endpoint (see
...
/// `~/.ainxt/leader.sock` — e.g. run a local branch build's leader without
...
/// bypassed entirely.
pub const LEADER_SOCKET_ENV: &str = "AINXT_LEADER_SOCKET";
...
pub fn lock_path_for_ws_url(ws_url: &str) -> PathBuf {
    resolve_lock_path(leader_socket_override(), &ainxt_home(), ws_url)
}
...
pub fn socket_path_for_ws_url(ws_url: &str) -> PathBuf {
    resolve_socket_path(leader_socket_override(), &ainxt_home(), ws_url)
}
...
impl LeaderLock {
    /// Create a new LeaderLock using the default paths in ainxt home.
...
mod tests {
    ...
    fn override_socket_path_wins_over_ws_url_derivation() {
        let root = Path::new("/home/u/.ainxt");
        let override_sock = PathBuf::from("/home/u/.ainxt/leader-branch.sock");

        // With an override, the path is taken unchanged and the WS-URL suffix is
        ...
        assert_eq!(
            ...
            PathBuf::from("/home/u/.ainxt/leader-branch.lock")
        );
    }
