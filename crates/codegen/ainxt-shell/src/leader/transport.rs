...
//!   into `\\.\pipe\ainxt-leader-<hash>` so callers keep their path-based API.
//!
#[cfg(unix)]
...
mod windows_impl {
    ...
    impl LeaderListener {
        ...
        /// Wait for the next incoming connection. Matches
    ...
    /// Deterministic leaf name (`ainxt-leader-<hash>`) for a filesystem path.
    ///
    ...
    fn pipe_leaf_name(path: &Path) -> std::ffi::OsString {
        ...
        std::ffi::OsString::from(format!("ainxt-leader-{hash:016x}"))
    }
    ...
    mod tests {
        ...
        fn pipe_name_is_deterministic() {
            let a = path_to_pipe_name(Path::new("/tmp/ainxt.sock"));
            let b = path_to_pipe_name(Path::new("/tmp/ainxt.sock"));
            assert_eq!(a, b);
        }
        ...
        fn pipe_name_has_correct_prefix() {
            ...
            assert!(s.starts_with(r"\\.\pipe\ainxt-leader-"), "got: {s}");
        }
        ...
        fn pipe_name_is_bounded() {
            ...
            // \\.\pipe\ainxt-leader- (20 chars) + 16 hex chars = 36 total
        ...
        async fn listener_is_ready_tracks_pipe_lifecycle() {
            ...
            let path =
                std::env::temp_dir().join(format!("ainxt-ready-probe-{}.sock", std::process::id()));
