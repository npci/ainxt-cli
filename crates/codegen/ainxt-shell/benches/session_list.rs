...
//!
//! Run: `cargo bench -p ainxt-shell --bench session_list`
...
use ainxt_fast_worktree::{ListFilter, WorktreeDb, WorktreeKind, WorktreeRecord, WorktreeStatus};
use ainxt_shell::session::info::Info;
use ainxt_shell::session::persistence::Summary;
use ainxt_shell::session::storage::{JsonlStorageAdapter, StorageAdapter};
use ainxt_shell::session::unified_list::{ListReq, UnifiedListResult, build_unified_list};
...
impl Fixture {
    fn new(home: TempDir) -> Self {
        ...
        #[cfg(unix)]
        {
            ...
            fs::set_permissions(&sessions_root, fs::Permissions::from_mode(0o700)).unwrap();
        }
        ...
        for workspace_index in 0..WORKSPACE_COUNT {
            ...
            let encoded = ainxt_shell::util::ainxt_home::encode_cwd_dirname(&cwd);
            let cwd_dir = sessions_root.join(encoded);
            fs::create_dir(&cwd_dir).expect("create encoded cwd directory");
            #[cfg(unix)]
            {
                use std::os::unix::fs::PermissionsExt;
                fs::set_permissions(&cwd_dir, fs::Permissions::from_mode(0o700)).unwrap();
            }
    ...
    fn assert_correct(&self, runtime: &tokio::runtime::Runtime) {
        let discovered = ainxt_shell::session::worktree::candidate_worktree_cwds_for_same_repo(
...
fn create_same_repo_cwds(home: &Path) -> SameRepoTopology {
    ...
    repo.remote(
        "origin",
        "https://github.com/ainxt-org/session-list-benchmark.git",
    )
    ...
    let worktree_base = ainxt_shell::session::worktree::worktree_base_dir(&main);
    ...
    for (index, (path, label)) in linked.iter().enumerate().take(DB_OVERLAP_WORKTREE_COUNT) {
        db.register(&WorktreeRecord {
            ...
            metadata: Some(ainxt_shell::session::worktree::build_label_metadata(
                label, false,
            )),
        })
...
fn write_summary(
    ...
    let summary = Summary {
        ...
        git_remotes: vec!["git@github.com:ainxt-org/benchmark.git".to_owned()],
...
fn bench_session_list(c: &mut Criterion) {
    ...
    unsafe {
        std::env::set_var("AINXT_HOME", home.path());
    }
    assert_eq!(ainxt_shell::util::ainxt_home::ainxt_home(), home.path());
