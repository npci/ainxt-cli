...
- The legacy `agent` command is now kept in sync with `ainxt` after running `ainxt update`.
- Headless (`ainxt -p`) runs now wait for background tasks and subagents to finish before exiting.
