...
- **ainxt sessions delete <id>** command now lets you permanently remove a session from the CLI.
...
- **ainxt quit** no longer hangs when background git or network tasks are slow.
...
- Session lists (welcome screen, /resume, ainxt sessions list) are now sorted by the same activity time shown in the UI.
