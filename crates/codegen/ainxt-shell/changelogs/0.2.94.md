...
- **ainxt inspect** now lists skills from [skills].paths and correctly labels bundled vs user skills.
- **--minimal** and **--fullscreen** choices are now remembered for future plain ainxt launches.

## Bug Fixes

- **Queued bash commands** promoted at turn end now render their output instead of disappearing.
- **Xcode / Foundation ACP clients** can now drive ainxt agent stdio without silent parse drops on session/* calls.
...
- **ainxt --minimal** now supports native drag-select on classic Windows conhost terminals.
