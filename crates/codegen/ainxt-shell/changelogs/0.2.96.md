...
- **ainxt.dev/pr/status** now reports whether an open PR is in the merge queue.
...
- **`ainxt worktree ls`** now works as a short alias for `ainxt worktree list`.
- **MCP tool output truncation** can now be set per-repo in `.ainxt/config.toml`.
...
- **Resumed ainxt.dev chats** now use the conversation's last model instead of the gateway default.
