...
- **`ainxt mcp add`** now accepts positional arguments (e.g. `ainxt mcp add filesystem -- npx ...`), supports --scope project, and adds -e/-H flags for env/headers.
...
- **`ainxt update`** no longer downloads the same binary twice when multiple updaters or leader checks run concurrently.
- **Background task IDs** after /compact are now shown unchanged so the model can reference them correctly in later tool calls.
...
- **Fixed git clone failures** on Windows when the CLI tries to clone marketplace plugins into ~/.ainxt.
