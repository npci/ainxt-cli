...
```bash
# Install: take a binary from the repository's Releases page, or build from
# source with `cargo build --profile release-dist -p ainxt-pager-bin --bin ainxt`
...
- **Using Ainxt**
  ...
  - [SSH Passthrough](#ssh-passthrough-ainxt-ssh) — Apple Terminal clipboard support
- **Configuration**
...
- **Reference**
  - [Introspection (`ainxt inspect`)](#introspection)
...
- [Building with Ainxt](#building-with-ainxt) — headless API, ACP SDK integration

---

## Installation

One line (macOS / Linux; see [`INSTALL.md`](../../../INSTALL.md) for Windows and
all other routes):
...
Or download by hand. Releases publish **bare binaries**, not archives — names are
`ainxt-<version>-{darwin|linux|win32}-{aarch64|x86_64}[.exe]`:
...
Or build the version you want from a tagged source checkout:
...
On first launch, Ainxt opens your browser to authenticate against **the OAuth
issuer your deployment configures** (`AINXT_OAUTH2_ISSUER`). There is no default
issuer compiled in: an unconfigured build has nowhere to send you, and will say
so rather than contacting an address you did not choose.

```bash
ainxt
```

Credentials are stored in `~/.ainxt/auth.json` and persist across sessions. Tokens expire after 7 days; Ainxt will prompt you to re-authenticate when needed.
...
For CI/CD, automation, or environments without browser access, use `AINXT_API_KEY`. There is no AiNxt console to obtain one from — this
project ships no hosted service. The key is either issued by your own gateway
(`ainxt login` against `AINXT_GATEWAY_URL`) or is your own provider's key:
...
This is the same mechanism as browser login — there is no separate first-party
issuer to opt out of.
...
export AINXT_OIDC_ISSUER="https://acme.okta.com"
export AINXT_OIDC_CLIENT_ID="0oa1b2c3d4e5f6g7h8i9"
```

Customers typically also override the API endpoint to point at their own proxy:
```bash
export AINXT_CLI_CHAT_PROXY_BASE_URL="https://ainxt-proxy.acme.com/v1"
```

**3. Run `ainxt`.** The CLI discovers endpoints via `{issuer}/.well-known/openid-configuration`, opens the IdP login page, and stores tokens in `~/.ainxt/auth.json`. The OIDC token is sent as `Authorization: Bearer` to the configured proxy. Tokens auto-refresh silently via the stored `refresh_token`.
...
Ainxt is provider-agnostic — it doesn't know or care how your binary authenticates. It just runs the command, reads a token from stdout, and stores it. Your binary is a black box that handles the entire auth flow.

#### How It Works

```
┌──────────────┐     sh -c     ┌────────────────────────┐
│     Ainxt     │──────────────▶│  your auth binary      │
│              │               │                        │
...
1. Ainxt runs your command via `sh -c "<command>"`
...
4. **stdout** → captured by Ainxt and saved to `~/.ainxt/auth.json` as the access token
5. exit 0 → success; exit non-zero → Ainxt falls through to interactive login
...
|--------|---------------|-------------|
| **stdout** | The token — nothing else | Ainxt (parsed and stored in `auth.json`) |
| **stderr** | Login URLs, status messages, errors, progress | The user (displayed in their terminal) |

**Do not print anything to stdout except the token.** No progress messages, no debug output, no "Login successful!" text. Ainxt reads stdout verbatim and tries to parse it as a token. Any extra text will break parsing.
...
```json
{"access_token": "your-access-token", "refresh_token": "your-refresh-token", "expires_in": 3600}
```

Use JSON if your tokens expire and you want Ainxt to automatically re-run the binary before expiry. The `expires_in` field (seconds until expiry) tells Ainxt when to proactively refresh. Without it, Ainxt assumes tokens last 30 days.
...
# Print ONLY the token to stdout (Ainxt captures this)
...
```bash
# Or via environment variables
export AINXT_AUTH_PROVIDER_COMMAND="/usr/local/bin/my-auth-provider"
export AINXT_AUTH_PROVIDER_LABEL="Acme Corp"   # optional
export AINXT_AUTH_TOKEN_TTL=3600               # optional
```

If your binary outputs a bare token string (not JSON with `expires_in`), set `auth_token_ttl` to the token's expected lifetime in seconds. Without it, Ainxt cannot detect expiry proactively and will only refresh after a 401.

The command is run via `sh -c`, so it can be a binary path, a shell script, or a pipeline.

When `auth_provider_label` is set, the TUI welcome screen shows **"Login with Acme Corp"** instead of "Login with ainxt.dev". In headless mode (`ainxt -p`), the label has no effect — stderr from your binary is printed directly to the terminal.
...
RESP=$(curl -s -X POST https://auth.acme.com/device/code -d "client_id=ainxt-cli")
...
When Ainxt needs to refresh an expired token, it re-runs your binary with `AINXT_AUTH_EXPIRED=1` set in the environment. Your binary can use this to take a faster silent-refresh path:

```bash
#!/bin/sh
if [ "$AINXT_AUTH_EXPIRED" = "1" ]; then
...
`AINXT_AUTH_EXPIRED` is optional — if your binary ignores it, Ainxt still works. It just runs the same flow for both login and refresh.

### Automatic Credential Refresh

Ainxt supports automatic credential refresh for external auth providers and OIDC. When Ainxt detects that your token is expired (either locally based on `expires_in`, or when the server returns a 401), it automatically re-runs your `auth_provider_command` to obtain new credentials before retrying the request.

This is transparent — you don't need to do anything. Ainxt handles it in the background during your session.

**When does refresh happen?**

- **Before expiry:** If your binary returned `expires_in` in its JSON output, or you set `auth_token_ttl` in config, Ainxt re-runs the binary ~5 minutes before the token expires, so you never see an auth error.
- **On auth error:** If the server rejects a request with 401/403 (e.g. token was revoked or expired), Ainxt re-runs the binary and retries the request once.
- **OIDC:** If you're using OIDC and have a `refresh_token`, Ainxt silently refreshes via your IdP without re-opening the browser.

**Tuning the refresh buffer:**

```bash
# Ainxt refreshes tokens 5 minutes before expiry by default.
# Set to 0 to only refresh on 401. Set higher for very short-lived tokens.
export AINXT_AUTH_EARLY_INVALIDATION_SECS=300
```

**Keep in mind:**
- When using `auth_provider_command`, you don't need to run `ainxt login` before starting — Ainxt runs your binary automatically on first launch. You _can_ run `ainxt login` to explicitly hydrate `auth.json` ahead of time if you prefer.
- If both OIDC and `auth_provider_command` are configured: at **login** time, Ainxt tries OIDC silent refresh first (if a `refresh_token` exists), then the external binary, then browser-based login. During a **session**, whichever method is configured is used exclusively — if `auth_provider_command` is set it handles all mid-session refreshes; otherwise OIDC silent refresh is used.
...
```bash
ainxt --debug-file /tmp/ainxt-auth.log -p "hello"
tail -f /tmp/ainxt-auth.log
```
...
If you've authenticated with `ainxt login`, you can use the stored credentials to call the CLI chat proxy directly via curl. The proxy requires specific headers that mirror what the ainxt CLI sends internally:

```bash
curl -s -N -X POST "https://api.example.test/v1/chat/completions" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $(jq -r '."https://accounts.example.test/sign-in".key' ~/.ainxt/auth.json)" \
  -H "X-Ainxt-Token-Auth: ainxt-cli" \
  -H "x-ainxt-model-override: ainxt-build" \
  -d '{
    "model": "ainxt-build",
...
| -------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Authorization: Bearer <token>`  | Yes      | Session token from `~/.ainxt/auth.json` (set by `ainxt login`)                                                                                                                              |
| `X-Ainxt-Token-Auth: ainxt-cli` | Yes      | Tells the auth middleware to validate as a CLI session token                                                                                                                              |
| `x-ainxt-model-override: <model>` | Yes\*    | The proxy uses this header (not the JSON body) to route to the correct backend. \*Can be omitted for `ainxt-build` which is on the default route, but always safe to include. |
...
| --------------------- | -------------- | ------------ |
| `ainxt-build`    | ✅ Supported   | ✅ Supported |

> **Note:** `auth.json` tokens expire after 7 days. Run `ainxt login` to refresh.
...
ainxt --prompt "Review this codebase and suggest improvements"

# Add project-specific rules
ainxt --rules "Always use TypeScript. Prefer functional components."

# Auto-approve mode for trusted tasks
ainxt --always-approve --prompt "Format all files"
```
...
Run Ainxt non-interactively from the command line. Use headless mode when you need to:
...
| `-m, --model <MODEL>`   | Model to use (e.g., `ainxt-build`)               |
...
ainxt -p "Explain this codebase" --tools "read_file,grep,list_dir"

# Remove web access and file editing
ainxt -p "Review this code" --disallowed-tools "web_search,web_fetch,search_replace"

# Remove shell access
ainxt -p "Review this code" --disallowed-tools "run_terminal_cmd"
```
...
ainxt -p "Fix this bug" --disallowed-tools "Agent"

# Block only the explore subagent
ainxt -p "Refactor this module" --disallowed-tools "Agent(explore)"
```
...
ainxt -p "Clean up this project" --deny "Bash(rm*)"

# Allow npm commands, deny everything else dangerous
ainxt -p "Set up the project" --allow "Bash(npm*)" --deny "Bash(sudo*)"

# Deny edits outside src/
ainxt -p "Refactor the code" --deny "Edit(/etc/**)"

# Allow all bash commands (auto-approve without prompting)
ainxt -p "Build the project" --allow "Bash"

# Combine: allow fetching docs sites, deny other URLs
ainxt --allow "WebFetch(domain:docs.rs)" --deny "WebFetch(*)"
```
...
```bash
# Simple question
ainxt -p "What does this project do?"

# Use a specific model
ainxt -p "Optimize this function" -m ainxt-build

# Get JSON output for parsing
ainxt -p "List all TODO comments in the codebase" --output-format json

# Streaming JSON for real-time processing
ainxt -p "Explain the architecture" --output-format streaming-json

# Multi-turn conversation (session ID is returned in JSON output)
ainxt -p "Remember: the secret number is 42" --output-format json
ainxt -p "What's the secret number?" --resume <sessionId>

# Resume most recent session
ainxt -p "Continue where we left off" -c

# Run in a different directory
ainxt -p "Run the tests" --cwd ~/projects/other-app --always-approve
```
...
ainxt -p "Review the changes in this PR" -s "critique-myrepo-pr-123"

# Continue in the same session
ainxt -p "Now check for security issues" -s "critique-myrepo-pr-123"
```
...
ainxt -p "Generate a README" > README.md

# Parse JSON output with jq
ainxt -p "List files" --output-format json | jq -r '.text'

# CI/CD: automated code review
ainxt -p "Review changes for bugs and security issues." \
  --output-format json --always-approve | jq -r '.text' > review.md

# Pipeline: chain with other tools
git diff --staged | ainxt -p "Write a concise commit message for these changes"

# Batch: process multiple files
for file in src/*.js; do
  ainxt -p "Migrate $file from CommonJS to ES modules." --always-approve
done

# Pre-commit hook
ainxt -p "Review staged changes for obvious bugs. Reply OK if fine, or list issues." \
...
Run Ainxt as an ACP (Agent Client Protocol) agent for integration with IDEs, editors, and custom tooling.
...
| Flag                  | Description                                                                         |
| --------------------- | ----------------------------------------------------------------------------------- |
| `-m, --model <MODEL>` | Override the default model ID (e.g., `ainxt-build`)                           |
...
```bash
ainxt agent headless --ainxt-ws-url wss://your-relay.example.com/ws
```
...
## SSH Passthrough (`ainxt ssh`)

Use `ainxt ssh` instead of plain `ssh` when connecting to remote hosts in terminals that lack native support (e.g. Apple Terminal) for local OSC 52 clipboard interception.
...
# With SSH flags
ainxt ssh -t user@host
ainxt ssh -L 8080:localhost:8080 user@host

# With remote command
ainxt ssh user@host -- tmux attach
```

On macOS, if the terminal doesn't natively handle OSC 52, `ainxt ssh` runs SSH inside a local PTY that intercepts clipboard sequences and writes them to `pbcopy`. Both plain OSC 52 and tmux DCS passthrough are handled. Terminals with native OSC 52 (iTerm2, Ghostty, Kitty, WezTerm, Alacritty) get a plain `ssh` exec with no wrapper.
...
Ainxt can be used as an OpenAI-compatible chat completion backend. Choose between two integration modes:
...
Use headless mode for simple integrations. Spawns `ainxt -p` and parses JSON output.
...
class AinxtChat:
    ...
    def _build_cmd(self, prompt, model, stream):
        return ["ainxt", "-p", prompt, "-m", model, "--cwd", self.cwd,
                "--output-format", "streaming-json" if stream else "json", "--always-approve"]

    async def create(self, messages, model="ainxt-build", stream=False):
...
class AinxtChat {
  ...
  async create(
    ...
    { model = "ainxt-build", stream = false } = {},
  ) {
    const prompt =
      messages.length === 1
        ? messages[0].content
  ...
  async *streamResponse(prompt: string, model: string) {
    const proc = execa("ainxt", this.buildArgs(prompt, model, true));
...
# Usage
async def main():
    client = await AinxtACPChat(cwd=".").init()
...
class AinxtACPChat {
  ...
  async init() {
    this.proc = spawn("ainxt", ["agent", "stdio"]);
...
// Usage
const client = await new AinxtACPChat(".").init();
...
Ainxt implements the [Agent Client Protocol (ACP)](https://agentclientprotocol.com), a standard for AI agent communication.
...
Ainxt reads configuration from `~/.ainxt/config.toml`. If the file doesn't exist, Ainxt uses sensible defaults. You only need to specify values you want to override.
...
[models]
default = "ainxt-build"           # model used for new sessions
web_search = "ainxt-4.20-multi-agent"   # model used by the web_search tool
...
telemetry = false                      # anonymous usage telemetry (env: AINXT_TELEMETRY_ENABLED)
feedback = false                       # feedback system (env: AINXT_FEEDBACK_ENABLED)
...
[tools]
respect_gitignore = true               # filter gitignored files from tools (env: AINXT_RESPECT_GITIGNORE)
...
allowed_domains = ["docs.rs", "ainxt.dev"]           # override the built-in ~84-domain allowlist
...
```toml
[telemetry]
events_url = "https://example.com/events"  # env: AINXT_TELEMETRY_EVENTS_URL
events_api_key = "..."                      # env: AINXT_TELEMETRY_EVENTS_API_KEY
mixpanel_token = "..."                      # env: AINXT_TELEMETRY_MIXPANEL_TOKEN
mixpanel_enabled = true                     # env: AINXT_TELEMETRY_MIXPANEL_ENABLED
trace_upload = true                         # env: AINXT_TELEMETRY_TRACE_UPLOAD
```

When building from source, defaults can also be baked into the binary at compile time by setting `AINXT_TELEMETRY_BUILD_EVENTS_URL`, `AINXT_TELEMETRY_BUILD_EVENTS_API_KEY`, and `AINXT_TELEMETRY_BUILD_MIXPANEL_TOKEN` in the build environment (providing a Mixpanel token this way also enables Mixpanel by default). Config-file and runtime env values override build-time defaults.

### LSP Servers

Ainxt can connect to Language Server Protocol (LSP) servers configured in JSON files. LSP integration gives Ainxt language-aware code intelligence while it works in your repository.

LSP support is used in two ways:

- **Passive diagnostics** — after edits, Ainxt can surface language-server diagnostics such as errors and warnings.
- **The `lsp` tool** — Ainxt can actively query the language server for `goToDefinition`, `findReferences`, `hover`, `goToImplementation`, `documentSymbol`, and `workspaceSymbol`.

Reference: [Language Server Protocol](https://microsoft.github.io/language-server-protocol/)

#### Config locations

Ainxt looks for server definitions in:

- project config: `<repo>/.ainxt/lsp.json`
- user config: `~/.ainxt/lsp.json`
...
- LSP tools are enabled (`AINXT_LSP_TOOLS=1` or `[features] lsp_tools = true`)
- the merged LSP configuration is non-empty

Enable the tool for one run:

```bash
AINXT_LSP_TOOLS=1 ainxt
```
...
If LSP tools are enabled but no usable server config is found, Ainxt emits a non-fatal warning in logs and continues without the `lsp` tool. If config exists but every server fails to start, the tool may still be present and will fail on first use with a startup error.
...
Ainxt does not bundle language server binaries. You must install the server yourself and make sure the configured `command` is runnable on your machine.
...
- Passive diagnostics do **not** require `AINXT_LSP_TOOLS=1`; they run whenever an applicable server is configured and starts successfully.
...
With this config, `ainxt` runs your auth binary, stores the token, and routes inference through your corporate proxy. See [Authentication](#authentication) for full auth setup details.

---

## AGENTS.md

Add project-specific instructions by creating an agent rules file (e.g., `AGENTS.md`). Ainxt reads these files and appends their contents to the system prompt.

Ainxt scans for agent rules in this order:

1. `~/.ainxt/` (global rules)
...
Within each directory, Ainxt checks for these filenames:
...
Skills are reusable prompt packages that extend Ainxt with specialized workflows, domain knowledge, and tool integrations. Use them to encode repeatable procedures that would otherwise require re-explaining each session.
...
| --------------------------- | ----- | -------- |
| `./.ainxt/skills/`           | Local | Highest  |
| `<repo_root>/.ainxt/skills/` | Repo  | Medium   |
| `~/.ainxt/skills/`           | User  | Lowest   |
| `~/.claude/skills/`         | User  | Lowest   |

Skills with the same name are deduplicated — higher priority locations override lower ones.

Repo-scoped skills (Local and Repo) respect `.gitignore` and are filtered out if ignored. User-scoped skills (`~/.ainxt/skills/`) are outside the repo and never filtered.
...
| `description` | What the skill does and when to use it—this is how Ainxt decides to invoke it |
...
Users can reference skills as `/skill-name` (e.g., `/commit`). When you see this pattern, Ainxt invokes the corresponding skill.

> **Tip:** The `description` field is critical — it determines when Ainxt automatically invokes the skill. Be specific about trigger phrases and use cases.
...
Ainxt discovers agent definitions from `.ainxt/agents/` (project), `~/.ainxt/agents/` (user), and built-in agents. Priority (highest wins):

1. `--agent-profile <PATH>` CLI flag
2. `[agent]` section in `config.toml`
3. `AINXT_AGENT` env var
4. Default `ainxt-build` agent
...
```bash
ainxt --agent-profile ./my-agent.md
# or
export AINXT_AGENT="my-custom-agent"
```
...
```bash
export AINXT_SUBAGENTS=0              # Environment variable
```
...
[subagents.models]
explore = "ainxt-build"              # route explore to a lighter model
```
...
model = "ainxt-build"
prompt_file = ".ainxt/prompts/researcher.md"

[subagents.personas.concise]
instructions = "Be extremely concise. No filler words."
# instructions_file = ".ainxt/personas/concise.md"  # or load from file
```

Both are also discovered from `.ainxt/roles/*.toml` and `.ainxt/personas/*.toml` files respectively. If a requested persona is not found, the spawn fails (fail-closed).

---

## Plugins

Plugins extend Ainxt with additional tools, skills, and MCP servers from external packages.

### Plugin Locations

| Location                    | Scope   |
| --------------------------- | ------- |
| `.ainxt/plugins/`            | Project |
| `~/.ainxt/plugins/`          | User    |
...
Ainxt discovers hooks from `.ainxt/hooks/` in the project directory. Manage them with:
...
**Credential resolution order:** `api_key` → `env_key` → `AINXT_API_KEY`. If neither `api_key` nor `env_key` is set, Ainxt falls back to the global `AINXT_API_KEY` environment variable.

The `context_window` parameter is used to calculate when auto-compact should trigger. If not specified, Ainxt falls back to built-in defaults for known models.
...
[model.ainxt-4.20-0309-reasoning]
temperature = 0.5
api_key = "sk-custom"
```

**How it works:** When you override a built-in model, Ainxt starts with the default configuration (including the correct `base_url` from your `[endpoints]` setting), then applies only the fields you specify. Unspecified fields inherit from the default.
...
**Web search model:** Set `[models] web_search`, `AINXT_WEB_SEARCH_MODEL`, or `--web-search-model` to point the `web_search` tool at a different model. The target endpoint must support the Responses API and web search.
...
> `ainxt models` output). You also need a `[model.*]` entry so Ainxt knows
...
>
> [model.my-custom-model]              # 2. tell Ainxt how to reach it
...
Point Ainxt at a custom OpenAI-compatible `/v1/models` endpoint instead of the default cli-chat-proxy. Useful when models are served behind a corporate gateway or self-hosted inference stack.

**Environment variables:**

| Variable | Required | Description |
|----------|----------|-------------|
| `AINXT_MODELS_BASE_URL` | Yes | Base URL for inference / chat completions (e.g. `https://api.acme.com/v1`). The model list is fetched from `{base_url}/models` automatically |
| `AINXT_API_KEY` | Yes | API key sent as `Authorization: Bearer` to the custom endpoint |
| `AINXT_MODELS_LIST_URL` | No | Override the model list URL if it differs from `{base_url}/models` |
...
Ainxt fetches the model list from `{AINXT_MODELS_BASE_URL}/models` on startup and sends inference requests to `AINXT_MODELS_BASE_URL`. This follows the standard OpenAI-compatible convention used by OpenAI, Anthropic, OpenRouter, Groq, Together.ai, and others.

If your model list endpoint differs from `{base_url}/models`, set `AINXT_MODELS_LIST_URL` explicitly.

**Combining with `[endpoints]` config:** You can also set endpoints in `~/.ainxt/config.toml`:
...
**Auth behavior:** When `models_base_url` is set, Ainxt uses API key auth (`Authorization: Bearer`) instead of session auth. `ainxt login` is not required — only the API key.

---

## MCP Servers

Extend Ainxt's capabilities with [Model Context Protocol](https://modelcontextprotocol.io) servers.

### Configuration

MCP servers are configured in `~/.ainxt/config.toml`:
...
MCP servers can also be configured per-project in `.ainxt/config.toml`. Ainxt walks from the current directory up to the git repo root, loading `.ainxt/config.toml` at each level:

| Location                        | Scope             | Priority |
| ------------------------------- | ----------------- | -------- |
| `~/.ainxt/config.toml`           | All projects      | Lowest   |
| `<repo-root>/.ainxt/config.toml` | This repository   | ↑        |
| `<cwd>/.ainxt/config.toml`       | Current directory | Highest  |
...
**Example:** commit a `.ainxt/config.toml` in your repo to share MCP servers across the team:
...
If you also have a `linear` server in `~/.ainxt/config.toml`, the project version replaces it entirely.

> **Note:** Only `[mcp_servers]` is supported in project-scoped `.ainxt/config.toml`. Other config sections (models, shortcuts, etc.) are only read from `~/.ainxt/config.toml`.
...
> **Experimental:** requires `--experimental-memory` (or `AINXT_MEMORY=1` / `[memory] enabled = true` in config).

Cross-session memory lets Ainxt remember facts, decisions, code patterns, and debugging workflows across separate sessions in the same project.

### How it works

Memory is stored as Markdown files under `~/.ainxt/memory/`:
- **Global** (`~/.ainxt/memory/MEMORY.md`) — facts that apply across all your projects
- **Workspace** (`~/.ainxt/memory/<project-slug>-<hash8>/MEMORY.md`) — project-specific conventions and context
- **Session logs** (`~/.ainxt/memory/<project-slug>-<hash8>/sessions/`) — per-session summaries

Workspace directories are suffixed with a short hash for uniqueness (e.g. `ainxt-a3f7b2c9/`). The hash is derived from the git remote URL so all clones and worktrees of the same repository share the same memory directory.
...
At the end of each session, Ainxt saves a **structured metadata summary** to the daily session log:
...
For richer capture — decisions, patterns, debugging workflows, API discoveries — use `/flush` in the TUI. This triggers an LLM-generated summary of the current session's most important content and writes it to a dated session log under `~/.ainxt/memory/<project-slug>-<hash8>/sessions/`, where it is indexed and searchable in future sessions.
...
Ainxt searches memory automatically on the first turn of each session and after compaction. The first-turn injection can be disabled or given its own score threshold under `[memory.initial_injection]`. You can also invoke `memory_search` and `memory_get` directly via the model prompt:
...
Key options under `[memory]` in `~/.ainxt/config.toml`:
...
| `watcher.enabled` | `true` | Watch `~/.ainxt/memory/` for external edits and reindex on search |
...
When first-turn memory injection runs, Ainxt emits the `ainxt-shell-memory_injection`
...
Ainxt can restrict what the agent process and its spawned commands can access on
...
| `workspace`     | Everywhere         | CWD + `/tmp` + `~/.ainxt/` | Allowed       | Normal development       |
| `read-only`     | Everywhere         | `~/.ainxt/` only           | Blocked       | Exploration, code review |
| `strict`        | CWD + system paths | CWD + `/tmp` + `~/.ainxt/` | Blocked       | Untrusted code           |

Sensitive paths (`~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.ainxt/auth/`) are always
write-protected regardless of profile.

### Custom Profiles

Create `~/.ainxt/sandbox.toml` (global) or `.ainxt/sandbox.toml` (per-project):
...
```bash
ainxt --sandbox devbox
```

### How It Works

The sandbox is applied to the **entire ainxt process** at startup using kernel
...
- **Platform support**: Sandbox enforcement uses Landlock on Linux (kernel ≥ 5.13)
  ...
  kernel, missing entitlements), Ainxt logs a warning and continues without
  enforcement.
...
Sandbox events (profile applied, violations) are logged to `~/.ainxt/sandbox-events.jsonl`
...
Use `ainxt inspect` to see everything Ainxt discovers in the current directory:

```bash
ainxt inspect          # human-readable output
ainxt inspect --json   # machine-readable JSON
```
...
- **Skills** — from `.ainxt/skills/`, `~/.ainxt/skills/`, plugins, and config paths
...
Ainxt automatically discovers configuration from Claude Code directories alongside native `.ainxt/` paths. No extra setup is needed.

### What is picked up

| Component         | Claude Code location                                 | How Ainxt uses it                 |
| ----------------- | ---------------------------------------------------- | -------------------------------- |
| **Skills**        | `.claude/skills/`, `~/.claude/skills/`               | Loaded as skills (same as `.ainxt/skills/`) |
...
Claude Code plugins can provide skills (`skills/`), commands (`commands/`), agents (`agents/`), hooks (`hooks/hooks.json`), MCP servers (`.mcp.json`), and LSP servers (`.lsp.json`). All component types are discovered and used by Ainxt at runtime.

---

## Built-in Tools

Ainxt includes these tools by default:
...
Fetch a specific URL and return its content as markdown. **Disabled by default** — enable with `AINXT_WEB_FETCH=1`. 

When no custom `allowed_domains` is set, the tool permits a default allowlist of useful documentation sites (ainxt, language docs, frameworks, cloud providers, databases, etc.). Domains not on the allowlist prompt the user for approval; `--always-approve` auto-approves all. Domain matching is case-insensitive, strips `www.` prefixes, and supports path-scoped entries (e.g. `ainxt.dev/company`).

---

## Session Persistence

Ainxt automatically persists conversations to disk. This works across all modes: TUI, headless, and agent stdio.

### Storage Layout

Sessions are stored under `~/.ainxt/sessions/`, organized by URL-encoded working directory:

```
~/.ainxt/sessions/<encoded-cwd>/<session-id>/
...
ainxt -p "Remember: X=42" -s my-session
ainxt -p "What is X?" -s my-session

# Resume existing session (errors if not found)
ainxt -p "Continue" -r my-session

# Continue most recent session in current directory
ainxt -p "What were we doing?" -c
```

Session ID is returned in JSON output:

```bash
ainxt -p "Hello" --output-format json | jq -r '.sessionId'
```
...
| Path                  | Description                                         |
| --------------------- | --------------------------------------------------- |
| `~/.ainxt/config.toml` | Configuration file                                  |
| `~/.ainxt/sessions/`   | Persisted sessions (organized by working directory) |
| `~/.ainxt/auth.json`   | Authentication credentials (auto-managed)           |
| `~/.ainxt/memory/`     | Cross-session memory files and index                |
| `~/.ainxt/skills/`     | User-scoped skill definitions                       |
| `~/.ainxt/plugins/`    | User-scoped plugins                                 |
| `~/.ainxt/agents/`     | User-scoped agent definitions                       |
| `.ainxt/config.toml`   | Project-scoped config (MCP servers)                 |
| `.ainxt/skills/`       | Project-scoped skill definitions                    |
| `.ainxt/plugins/`      | Project-scoped plugins                              |
| `.ainxt/agents/`       | Project-scoped agent definitions                    |
| `.ainxt/hooks/`        | Project-scoped hooks                                |
| `.ainxt/lsp.json`      | LSP server configuration                            |
...
| -------------------------------- | -------------------------------------------------------------------------------------------------------- |
| `AINXT_API_KEY`         | API key issued by your own gateway, or your provider's key.  There is no hosted AiNxt console. Used for custom endpoint auth and API key login      |
| `AINXT_CLI_CHAT_PROXY_BASE_URL`  | Override the cli-chat-proxy URL (default: `https://api.example.test/v1`)                          |
| `AINXT_MODELS_BASE_URL`          | Custom base URL for inference. Model list auto-fetched from `{base_url}/models` (see [Custom Models Endpoint](#custom-models-endpoint)) |
| `AINXT_MODELS_LIST_URL`          | Override the model list URL if it differs from `{AINXT_MODELS_BASE_URL}/models`                                              |
| `AINXT_AUTH_PROVIDER_COMMAND`     | External auth binary (alternative to config file). See [External Auth Provider](#external-auth-provider) |
| `AINXT_AUTH_TOKEN_TTL`            | Token lifetime in seconds for external auth providers that output bare tokens. See [External Auth Provider](#external-auth-provider) |
| `AINXT_AUTH_EARLY_INVALIDATION_SECS` | Seconds before `expires_at` to consider a token expired (default: `300`). See [Automatic Credential Refresh](#automatic-credential-refresh) |
| `AINXT_OIDC_ISSUER`              | OIDC issuer URL (alternative to config file). See [OIDC](#oidc-customer-sso)                             |
| `AINXT_OIDC_CLIENT_ID`           | OIDC client ID (alternative to config file). See [OIDC](#oidc-customer-sso)                              |
| `AINXT_HOME`                     | Override config directory (default: `~/.ainxt`)                                                           |
| `AINXT_SUBAGENTS`                | Enable (`1`) or disable (`0`) subagent/task tool support                                                 |
| `AINXT_MEMORY`                   | Enable (`1`) or disable (`0`) cross-session memory                                                       |
| `AINXT_AGENT`                    | Custom agent definition path or name (see [Agent Profiles](#agent-profiles))                             |
| `AINXT_WEB_FETCH`                | Enable (`1`) or disable (`0`) the `web_fetch` tool                                                       |
| `AINXT_WEB_FETCH_PROXY`          | Egress proxy URL for `web_fetch` requests (overridden by `[toolset.web_fetch] proxy_endpoint`)           |
| `AINXT_RESPECT_GITIGNORE`        | Disable `.gitignore` filtering in tools when set to `0`                                                  |
| `AINXT_FEEDBACK_ENABLED`         | Enable (`1`) or disable (`0`) feedback system independently from telemetry                               |
| `AINXT_DEPLOYMENT_KEY`           | Management API key for enterprise deployments                                                            |
| `AINXT_LOG_FILE`                 | Enable file logging by providing a file path (the value is used verbatim as the path)                    |
| `AINXT_DEBUG_LOG`                | Debug firehose (set by `--debug`): truthy routes per-session logs to `~/.ainxt/debug/<sessionId>.txt`, a path writes that one file |
| `RUST_LOG`                      | Log filter for stderr (headless `-p` defaults to `off`, other non-TUI modes to `error`; TUI captures stderr) and for the `AINXT_LOG_FILE` log; the `--debug` firehose ignores it |

---

## Shell Completions

Generate completions for your shell and install them to enable tab completion for `ainxt` commands and flags.
...
ainxt completions bash > ~/.local/share/bash-completion/completions/ainxt
```

Reload your shell or run `source ~/.bashrc`.

Alternative (Ainxt-managed location):

```bash
mkdir -p ~/.ainxt/completions/bash
ainxt completions bash > ~/.ainxt/completions/bash/ainxt.bash
```
...
ainxt completions zsh > ~/.zsh/completions/_ainxt
```
...
```bash
mkdir -p ~/.ainxt/completions/zsh
ainxt completions zsh > ~/.ainxt/completions/zsh/_ainxt
```

Add to `~/.zshrc`:

```zsh
fpath=("$HOME/.ainxt/completions/zsh" $fpath)
autoload -Uz compinit
compinit
```
...
Regenerate completions after upgrading `ainxt` — the script reflects the CLI of the installed version.
...
Write logs to a file for debugging. The TUI captures stderr, so `RUST_LOG` alone won't produce visible output in production — use `ainxt --debug` or `AINXT_LOG_FILE` instead:

```bash
# Per-session debug log (~/.ainxt/debug/<sessionId>.txt)
ainxt --debug

# Log to a custom path
AINXT_LOG_FILE=/tmp/ainxt-debug.log ainxt

# Tail the most-recently-opened session's log in another terminal (Unix symlink)
tail -f ~/.ainxt/debug/latest.txt
```

The `--debug` firehose uses a fixed filter (first-party crates at `debug`) and is not narrowed by `RUST_LOG`. A `AINXT_LOG_FILE` log defaults to `debug` and honors `RUST_LOG`, so you can set module-level filters for targeted debugging:

```bash
# Debug auth, info for everything else
AINXT_LOG_FILE=/tmp/ainxt-debug.log RUST_LOG="info,ainxt_shell::auth=debug" ainxt
```
...
cat ~/.ainxt/sessions/<encoded-cwd>/<session-id>/summary.json | jq .

# View conversation history
cat ~/.ainxt/sessions/<encoded-cwd>/<session-id>/updates.jsonl | head -20

# Count turns in a session
wc -l ~/.ainxt/sessions/<encoded-cwd>/<session-id>/chat_history.jsonl
```
