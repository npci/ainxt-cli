...
use ainxt_shell::cpu_profile::ControlErrorCode;
...
async fn setup_control_test_server(
    temp: &TempDir,
) -> (std::path::PathBuf, ainxt_shell::leader::ServerHandle) {
...
async fn test_session_new_valid_default_model_injected() {
    ...
    let meta = &json["params"]["_meta"];
    assert_eq!(meta["modelId"], "ainxt-3-fast");
...
/// `ainxt.dev/session_notification` (carrying the `ModelChanged` update) out
/// to **every** subscriber of that session — not just the invoker — so
/// the follower client matches the new model in its UI.
///
...
async fn test_set_model_broadcasts_to_session_subscribers() {
    ...
    invoker
        .send(format!(
            r#"{{"jsonrpc":"2.0","id":42,"method":"session/setModel","params":{{"sessionId":"{}","modelId":"ainxt-4"}}}}"#,
            shared_sid
        ))
        .unwrap();
    let received = acp_rx.recv().await.unwrap();
    ...
    assert_eq!(json["params"]["modelId"], "ainxt-4");
    ...
    let broadcast = format!(
        r#"{{"jsonrpc":"2.0","method":"ainxt.dev/session_notification","params":{{"sessionId":"{}","update":{{"sessionUpdate":"model_changed","model_id":"ainxt-4","reasoning_effort":"high"}}}}}}"#,
        shared_sid
    );
    response_tx.send(broadcast.clone()).unwrap();
    let response = format!(
        r#"{{"jsonrpc":"2.0","result":{{"meta":{{"model":"ainxt-4"}}}},"id":"{}"}}"#,
        setmodel_ns_id
    );
    response_tx.send(response).unwrap();
    ...
    assert_eq!(inv2["result"]["meta"]["model"], "ainxt-4");
...
async fn test_extension_method_roundtrip() {
    ...
    let ext_call = r#"{"jsonrpc":"2.0","id":50,"method":"_ainxt.dev/search/fuzzy/open","params":{"sessionId":"sess-123","hidden":false}}"#;
    ...
    assert_eq!(json["method"], "_ainxt.dev/search/fuzzy/open");
...
async fn test_session_ownership_cleanup_on_disconnect() {
    use ainxt_shell::leader::run_leader_server;
    ...
    tokio::spawn(async move {
        ...
        let (shutdown_tx, _shutdown_rx) =
            tokio::sync::watch::channel(ainxt_shell::leader::ShutdownReason::Manual);
        ...
        let _ = run_leader_server(
            ...
            client_count,
            agent_busy,
            ainxt_shell::agent::activity::AgentActivity::default(),
            ready_rx,
    ...
    assert_eq!(eviction_json["method"], "ainxt.dev/internal/evict_sessions");
    ...
    let probe = r#"{"jsonrpc":"2.0","method":"ainxt.dev/probe","params":{"ping":true}}"#;
...
/// Verify that an `ainxt.dev/code/status` extension request is forwarded to the
...
async fn test_code_status_ext_request_forwarded_to_agent() {
    ...
    // Send ainxt.dev/code/status with a sessionId — the leader must forward it to the agent.
    let status_req = r#"{"jsonrpc":"2.0","id":42,"method":"extensions/ext","params":{"method":"ainxt.dev/code/status","params":{"sessionId":"sess-web-1","cwd":"/repo"}}}"#;
    ...
    assert_eq!(json["params"]["method"], "ainxt.dev/code/status");
...
async fn test_raw_registration_handshake_not_ready_then_ready() {
    ...
    tokio::spawn(async move {
        ...
        let _ = run_leader_server(
            ...
            watch::channel(ainxt_shell::leader::ShutdownReason::Manual).0,
            None,
            control_state,
        )
        .await;
...
/// Integration test: a connected client receives `ainxt.dev/leader/version_mismatch`
...
async fn test_version_mismatch_notification_sent_to_client() {
    ...
    use tokio::sync::{mpsc, watch};
    use tokio_util::sync::CancellationToken;
    use ainxt_shell::leader::{ClientCapabilities, ClientMode, LeaderClient, run_leader_server};
...
async fn test_auto_update_shutdown_reason_reaches_client() {
    use ainxt_shell::leader::{ClientCapabilities, ClientMode, LeaderClient, ShutdownReason};
    ...
    tokio::time::timeout(
        ...
        disconnect_rx.wait_for(|r| *r != ainxt_shell::leader::DisconnectReason::Connected),
    )
    .await
    ...
    assert_eq!(
        *disconnect_rx.borrow(),
        ainxt_shell::leader::DisconnectReason::LeaderShutdown
    );
}
...
async fn test_leader_code_nav_isolation_end_to_end() {
    ...
    // Web client sends ainxt.dev/code/status (the primary non-starting code-nav call).
    let status_with_session = r#"{"jsonrpc":"2.0","id":10,"method":"extensions/ext","params":{"method":"ainxt.dev/code/status","params":{"sessionId":"web-session","cwd":"/repo"}}}"#;
    ...
    assert_eq!(status_json["params"]["method"], "ainxt.dev/code/status");
...
async fn test_lock_released_before_connect_prevents_deadlock() {
    ...
    // Spawner acquires the file lock (matches connect_or_spawn).
...
async fn setup_persistent_test_server(
    ...
    tokio::spawn(async move {
        ...
        let _ = run_leader_server(
            ...
            tokio::sync::watch::channel(false).0,
            tokio::sync::watch::channel(ainxt_shell::leader::ShutdownReason::Manual).0,
            None,
            control_state,
        )
        .await;
...
///
/// This binary does not sandbox AINXT_HOME, so on a dev machine these entries
/// land in the real `~/.ainxt` log — accepted: the server already writes
...
fn orphan_log_count(request_id: &str) -> usize {
    let Some(bytes) = ainxt_telemetry::unified_log::snapshot_log() else {
        return 0;
    };
...
async fn wait_for_client_disconnected_log(client_id: u64) -> bool {
    let deadline = tokio::time::Instant::now() + Duration::from_secs(5);
    loop {
        let seen = ainxt_telemetry::unified_log::snapshot_log().is_some_and(|bytes| {
...
async fn test_hung_agent_leaves_transport_healthy_and_forwards_cancel() {
    ...
    assert_eq!(
        *client.disconnect_reason().borrow(),
        ainxt_shell::leader::DisconnectReason::Connected,
...
async fn test_sever_mid_rpc_orphans_response_and_replay_recovers() {
    ...
    assert_eq!(evict_json["method"], "ainxt.dev/internal/evict_sessions");
