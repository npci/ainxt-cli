//! Shared helpers for ainxt-shell integration tests.

use ainxt_shell::sampling::{ApiBackend, Client, SamplerConfig};
...
pub fn test_sampler_config(
    ...
    // Shell `Client` is `ainxt_sampler::SamplingClient`, which takes a
    ...
    SamplerConfig {
        ...
        top_p: None,
        api_backend,
        // Without this, SamplerClient::api_backend() ignores the
        // `api_backend` param above and always falls back to Messages,
        // so callers requesting Responses/ChatCompletions 404 against
        // the mock server's actually-registered route.
