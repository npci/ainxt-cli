...
use ainxt_shell::auth::{AuthManager, AuthMode, AinxtAuth, AinxtComConfig};
use ainxt_shell::heap_profile::{
...
use ainxt_test_support::{EnvGuard, MockInferenceServer};
use ainxt_version::TEST_VERSION_ENV;
...
fn seed_auth_json(home: &Path, token: &str) {
    let scope = AinxtComConfig::default().auth_scope();
    let auth = AinxtAuth {
        key: token.to_owned(),
...
fn assert_storage_auth(uploads: &[ainxt_test_support::mock_server::StorageUpload]) {
    for u in uploads {
        assert_eq!(
...
impl Harness {
    async fn start(remote: RemoteSettings) -> Self {
        ...
        let auth = Arc::new(AuthManager::new(home.path(), AinxtComConfig::default()));
