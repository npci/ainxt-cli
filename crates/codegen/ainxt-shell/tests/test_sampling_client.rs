...
use ainxt_shell::sampling::{
    ApiBackend, Client, ConversationItem, ConversationRequest, ConversationToolChoice,
    SamplingError, ToolCall, ToolSpec, rs,
};
use ainxt_shell::session::storage::JsonlStorageAdapter;
use ainxt_test_support::sse::responses_api_reasoning_and_text_events;
use ainxt_test_support::{MockInferenceServer, ScriptedResponse, SseEvent};
...
async fn test_chat_completions_streaming_tool_calls() {
    ...
    server.enqueue_response(
        "/v1/chat/completions",
        ScriptedResponse::sse(chat_completion_tool_call_stream(tool_calls, "ainxt-test")),
    );
    let client = create_test_client(&server.url(), ApiBackend::ChatCompletions);
...
/// Upgrade path, ainxt-build / Responses API: a legacy session whose
...
async fn responses_upgrade_roundtrips_reconstructed_reasoning_as_typed_input() {
    // 1. Seed a legacy ainxt-build chat_history.jsonl (inline reasoning
    ...
    std::fs::write(
        ...
        concat!(
            ...
            r#"{"type":"user","content":[{"type":"text","text":"q1"}]}"#,
            "\n",
            r#"{"type":"assistant","content":"a1","reasoning":{"text":"legacy ainxt-build reasoning","encrypted":"ENC_BLOB_xyz","id":"rs_ainxtbuild_legacy"},"model_id":"ainxt-build"}"#,
            "\n",
        ),
    )
    .unwrap();
    ...
    //    input item with its fields preserved unchanged — the byte-stable
    ...
    assert_eq!(
        ...
        Some("legacy ainxt-build reasoning")
    );
}
...
// TODO(known-red): the outgoing /v1/messages request body carries no
// "thinking" content block for the reconstructed Reasoning item -- the
// legacy-history reconstruction itself (step 2, asserted separately) does
// produce a ConversationItem::Reasoning, but something in the Messages
// serialization path for it isn't reached or isn't matched. Never verified
// on this repo's CI before today (cargo test --workspace was unreachable
// until unrelated masking bugs upstream were fixed); needs its own
// dedicated investigation into the Reasoning -> thinking-block wire path.
#[ignore = "known-red: reconstructed reasoning doesn't reach the wire as a thinking block, unconfirmed root cause"]
#[tokio::test]
async fn messages_upgrade_emits_reconstructed_reasoning_as_thinking_block() {
    ...
    std::fs::write(
        ...
        concat!(
            ...
            r#"{"type":"user","content":[{"type":"text","text":"q1"}]}"#,
            "\n",
            r#"{"type":"assistant","content":"a1","reasoning":{"text":"legacy anthropic thinking","encrypted":"SIGNATURE_abc","id":""},"model_id":"ainxt-4.5"}"#,
            "\n",
        ),
    )
    .unwrap();
...
async fn test_responses_api_streaming_text() {
    ...
    while let Some(event_result) = stream.next().await {
        ...
        use ainxt_shell::sampling::rs::ResponseStreamEvent;
        match event {
    ...
    while let Some(event_result) = stream.next().await {
        ...
        if let ResponseStreamEvent::ResponseCompleted(completed) = event {
            for output in completed.response.output {
                use ainxt_shell::sampling::rs::OutputItem;
                ...
                match output {
                    ...
                    OutputItem::Message(msg) => {
                        for content in &msg.content {
                            use ainxt_shell::sampling::rs::OutputMessageContent;
...
async fn test_stream_error_during_responses_streaming() {
    ...
    // This matches test_stream_error_during_streaming but exercises the
...
async fn test_request_includes_headers() {
    ...
    assert_eq!(request.header("x-ainxt-conv-id"), Some("conv-12345"));
    assert_eq!(request.header("x-ainxt-req-id"), Some("req-67890"));
}
...
/// `SamplerConfig::doom_loop_recovery` puts `x-ainxt-doom-loop-check` on the
...
async fn test_doom_loop_check_enabled_sends_header_and_absorbs_check_event() {
    use ainxt_sampling_types::doom_loop::{DOOM_LOOP_CHECK_EVENT_TYPE, SAMPLE_CHECK_EVENT_DATA};
    ...
    assert_eq!(logged.header("x-ainxt-doom-loop-check"), Some("true"));
}
...
async fn test_doom_loop_check_disabled_sends_no_header_and_drops_check_frames() {
    ...
    let logged = server.requests().pop().unwrap();
    assert!(logged.path.contains("/responses"));
    assert_eq!(logged.header("x-ainxt-doom-loop-check"), None);
}
...
async fn test_responses_backend_hits_responses_endpoint_not_chat_completions() {
    ...
    // The session-level dispatch in `ainxt-sampler` selects the
