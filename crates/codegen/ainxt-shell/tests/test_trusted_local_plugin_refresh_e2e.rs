...
//! 4. Smoke-validate session JSON under `$AINXT_HOME/sessions/` after exit.
//!
//! Requires a built `ainxt` binary (`AINXT_BINARY` or cargo-built pager) for the
//! ignored headless test.
//!
//! ```bash
//! cargo test -p ainxt-shell --test test_trusted_local_plugin_refresh_e2e
//! cargo test -p ainxt-shell --test test_trusted_local_plugin_refresh_e2e -- --ignored
//! ```
...
use ainxt_agent::plugins::SharedPluginRegistryHandle;
use ainxt_agent::plugins::discovery::DiscoveryConfig;
use ainxt_agent::plugins::git_install::{InstallSource, install_from_source};
...
fn trusted_local_refresh_surfaces_new_agent_via_discovery() {
    ...
    let _ainxt_guard = EnvVarGuard::set("AINXT_HOME", &ainxt_home);

    // Live source: a user-home local plugin (matches a `~/.claude` local tree).
    let source = home
        .join(".claude")
    ...
    let mut registry = InstallRegistry::empty(ainxt_home.join("installed-plugins"));
    ...
    // ainxt_home() caches the first AINXT_HOME per process; a separate test
    ...
    assert_eq!(
        plugin.scope,
        ainxt_agent::plugins::PluginScope::CliOverride
    );
...
#[tokio::test]
#[ignore = "requires pre-built ainxt binary; run with --ignored"]
#[serial]
async fn headless_session_refreshes_trusted_local_plugin_and_writes_session_json() {
    ...
    // The spawned binary gets HOME/AINXT_HOME via `cmd.env` below; this global env
    // is only for the in-process post-run discovery assertion (which resolves the
    // registry via ainxt_home()). `#[serial]` keeps it from racing other tests.
    let _home_guard = EnvVarGuard::set("HOME", &home);
    let _ainxt_guard = EnvVarGuard::set("AINXT_HOME", &ainxt_home);
    ...
    // Smoke: session storage under AINXT_HOME/sessions has JSON artifacts.
