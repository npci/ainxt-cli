...
use ainxt_acp_lib::{AcpAgentGatewaySender, AcpClientMessage};
use ainxt_paths::AbsPathBuf;
use ainxt_workspace::permission::types::{
...
/// Shared `AINXT_HOME` for the entire test binary. The `OnceLock` in
/// `ainxt-config` only allows one value per process, so all tests share
...
fn test_home() -> &'static PathBuf {
    ...
    HOME.get_or_init(|| {
        ...
        unsafe { std::env::set_var("AINXT_HOME", &path) };
        path
    })
}
...
async fn request(handle: &PermissionHandle, access: AccessKind, id: &str) -> Decision {
    ...
    let cmd = PermissionCommand::Request {
        ...
        // This test drives the actor directly instead of going through
        // `PermissionHandle::request`, which is where the enforcement point is
        // normally evaluated. `false` is the neutral value — no human forced —
        // and preserves the behaviour this test asserted before the field existed.
...
async fn mcp_tool_grant_persists_and_short_circuits_next_request() {
    run_actor_test(ClientType::AinxtPager, |handle, gw, cwd| async move {
...
async fn fallback_client_plain_allow_always_persists_mcp_tool() {
    // Regression: Generic / AinxtWeb / Extension clients
...
async fn dont_ask_policy_denies_without_prompting() {
    use ainxt_workspace::permission::types::{PermissionConfig, PromptPolicy};
