...
//! - Session resume
//!
//! Each test spawns a real `ainxt agent stdio` process, speaks the full ACP
...
//! Run locally:
//! ```bash
//! cargo test -p ainxt-shell --test test_agent_type_invariant -- --ignored
//! ```
...
/// - `model-a`: no agent_type (→ "ainxt-build")
/// - `model-b`: no agent_type (→ "ainxt-build")
...
/// `ainxt-build` harness. The system prompt sent to the LLM should contain
/// the ainxt-build identity string.
#[tokio::test]
#[ignore]
async fn test_default_model_uses_ainxt_build_harness() {
    with_local_set(|| async {
        ...
        assert!(
            sys_prompt.contains("ainxt") || sys_prompt.contains("ainxt"),
            "default model should use ainxt-build harness\nsystem prompt preview: {}",
...
async fn test_session_resume_preserves_harness() {
    with_local_set(|| async {
        ...
        let original_has_ainxt =
            original_sys_prompt.contains("ainxt") || original_sys_prompt.contains("ainxt");
        let resumed_has_ainxt =
            resumed_sys_prompt.contains("ainxt") || resumed_sys_prompt.contains("ainxt");
        assert_eq!(
            original_has_ainxt,
            resumed_has_ainxt,
            "resumed session should use the same harness as the original\n\
             original identity markers: ainxt={original_has_ainxt}\n\
             resumed identity markers: ainxt={resumed_has_ainxt}\n\
...
/// default to `"ainxt-build"`. This exercises the serde default.
#[tokio::test]
#[ignore]
async fn test_model_without_agent_type_defaults_to_ainxt_build() {
    with_local_set(|| async {
            ...
            assert!(
                sys_prompt.contains("ainxt") || sys_prompt.contains("ainxt"),
                "model without agent_type should default to ainxt-build harness\nsystem prompt preview: {}",
                & sys_prompt[..sys_prompt.len().min(500)]
            );
        })
        .await;
}
/// The `AINXT_AGENT` escape hatch should override the model's agent_type.
/// Setting `AINXT_AGENT=ainxt-build` with an alternate-agent model should use
/// ainxt-build harness.
#[tokio::test]
#[ignore]
async fn test_ainxt_agent_env_overrides_model_agent_type() {
    with_local_set(|| async {
            ...
            cmd.args(["agent", "stdio"])
                ...
                .kill_on_drop(true);
            ainxt_test_support::env::test_env_cmd_tokio(
                &mut cmd,
                &server.url(),
                home.path(),
            );
            cmd.env("AINXT_AGENT", "ainxt-build");
            let mut child = cmd.spawn().expect("spawn ainxt");
            ...
            let incoming = ainxt_acp_lib::LineBufferedRead::spawn_local(incoming);
            ...
            assert!(
                sys_prompt.contains("ainxt") || sys_prompt.contains("ainxt"),
                "AINXT_AGENT=ainxt-build should override cursor model's agent_type\nsystem prompt preview: {}",
                & sys_prompt[..sys_prompt.len().min(500)]
            );
        })
        .await;
