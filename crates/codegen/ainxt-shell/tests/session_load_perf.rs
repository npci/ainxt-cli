...
//! `AINXT_PERF_SESSION_SRC=/path/to/<session-dir>`.
//!
//! Run:
//!   cargo test -p ainxt-shell --test session_load_perf -- --nocapture
//!   cargo test -p ainxt-shell --test session_load_perf full_session_load_e2e -- --ignored --nocapture
...
use ainxt_shell::session::info::Info;
use ainxt_shell::session::storage::{
    JsonlStorageAdapter, StorageAdapter, load_updates_for_replay_at,
};
use ainxt_workspace::session::file_state::{FileSnapshot, FlexiblePath, RewindPoint};
...
impl GenOpts {
    fn from_env() -> Self {
        ...
        let scale = g("AINXT_PERF_SCALE", 1).max(1);
        Self {
            turns: g("AINXT_PERF_TURNS", 80) * scale,
            acu_per_turn: g("AINXT_PERF_ACU_PER_TURN", 15),
            catalog_commands: g("AINXT_PERF_CATALOG_COMMANDS", 64),
            catalog_desc_len: g("AINXT_PERF_CATALOG_DESC_LEN", 320),
            agent_chunks_per_turn: g("AINXT_PERF_AGENT_CHUNKS_PER_TURN", 8),
            agent_chunk_len: g("AINXT_PERF_AGENT_CHUNK_LEN", 2000),
            rewind_points: g("AINXT_PERF_REWIND_POINTS", 60) * scale,
            files_per_rewind: g("AINXT_PERF_FILES_PER_REWIND", 40),
            file_content_len: g("AINXT_PERF_FILE_CONTENT_LEN", 8000),
        }
    }
}
...
/// `Info` and the session directory path. Uses `AINXT_PERF_SESSION_SRC` if set
...
async fn prepare_session(root: &Path, cwd: &Path, opts: &GenOpts) -> (Info, PathBuf) {
    ...
    if let Ok(src) = std::env::var("AINXT_PERF_SESSION_SRC") {
...
/// `(len, content_hash)` fingerprint of a file, for asserting it is exactly
...
/// `--ignored` (optionally `AINXT_PERF_SESSION_SRC=...`) to get the numbers.
...
fn generate_or_restore_rewind(path: &Path, opts: &GenOpts) {
    if std::env::var("AINXT_PERF_SESSION_SRC").is_ok() {
        return;
    }
...
use ainxt_shell::agent::mvp_agent::MvpAgent;
...
async fn full_session_load_e2e() {
    ...
    let server = ainxt_test_support::MockInferenceServer::start()
        .await
        .unwrap();
    ...
    let cwd = TempDir::new().unwrap();
    let opts = GenOpts::from_env();
    let instr_log = ainxt_home.path().join("instr.jsonl");

    // SAFETY: single-threaded current-thread runtime; set before any agent code
    // reads these process-globals (ainxt_home()/instrumentation mode are OnceLock).
    unsafe {
        std::env::set_var("AINXT_HOME", ainxt_home.path());
        std::env::set_var("AINXT_INSTRUMENTATION", "log");
        std::env::set_var("AINXT_INSTRUMENTATION_LOG", &instr_log);
        std::env::set_var("AINXT_CLI_CHAT_PROXY_BASE_URL", server.url());
        std::env::set_var("AINXT_API_BASE_URL", server.url());
        std::env::set_var("AINXT_API_KEY", "test-key-for-ci");
        std::env::set_var("AINXT_TELEMETRY_ENABLED", "false");
        std::env::set_var("AINXT_FEEDBACK_ENABLED", "false");
        std::env::set_var("AINXT_TRACE_UPLOAD", "false");
    }
    ...
    let _ = tracing_subscriber::registry()
        .with(ainxt_shell::instrumentation::layer::<Registry>())
        .try_init();

    let (info, dir) = prepare_session(ainxt_home.path(), cwd.path(), &opts).await;
    let updates_path = dir.join("updates.jsonl");
    let rewind_path = dir.join("rewind_points.jsonl");
    eprintln!(
    ...
    local
        .run_until(async move {
            ...
            if let Some(method) = init.auth_methods.iter().find(|m| &*m.id().0 == "ainxt.api_key") {
                let _ = client_conn
            ...
            let _ = ainxt_shell::instrumentation::finalize();
            ...
            // C1: the lazy rewind file must be exactly unchanged by a load.
            assert_eq!(
