...
//!
//! This test spawns a real `ainxt agent stdio` process, seeds an orphaned
...
//! ```bash
//! cargo test -p ainxt-shell --test test_subagent_orphan_reconcile -- --ignored
//! ```
...
async fn resume_reconciles_orphaned_running_subagent() {
    with_local_set(|| async {
        ...
        // leaves behind.
        // AinxtStdioClient sets HOME=<temp>; the binary uses <HOME>/.ainxt as AINXT_HOME.
        let ainxt_home = shared_home.path().join(".ainxt");
        let session_dir = locate_session_dir(&ainxt_home, session_id.0.as_ref());
