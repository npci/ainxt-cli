...
//!
//! Each test spawns a real `ainxt agent stdio` process against a mock
//! inference server and asserts on the persisted `summary.json`.
//!
//! Run locally:
//! ```bash
//! cargo test -p ainxt-shell --test test_summary_reasoning_effort -- --ignored
//! ```
...
/// Find `summary.json` for `session_id` under `<home>/.ainxt/sessions/` and
...
fn read_summary(home: &std::path::Path, session_id: &str) -> serde_json::Value {
    let sessions_root = home.join(".ainxt").join("sessions");
...
async fn test_fresh_session_persists_reasoning_effort() {
    with_local_set(|| async {
        ...
        let ainxt_dir = home.path().join(".ainxt");
        std::fs::create_dir_all(&ainxt_dir).expect("create .ainxt dir");
        std::fs::write(
...
"#,
        ...
        let client = AinxtStdioClient::spawn_with_home(&server, workdir.path(), home).await;
