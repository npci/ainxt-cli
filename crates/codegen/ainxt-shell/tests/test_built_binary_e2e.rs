//! Built-binary end-to-end tests for the ainxt (ainxt-pager) binary.
//!
//! These tests verify that the built ainxt binary works end-to-end against a mock
...
//!
//! The tests exercise:
//! - **Smoke** (`ainxt --version`): binary loads without crashing
//! - **ACP stdio** (`ainxt agent stdio`): full protocol lifecycle via ClientSideConnection
//!
//! Tests are `#[ignore]`d by default — they require a pre-built binary.
//!
//! Run locally (auto-builds the binary if not already present):
//! ```bash
//! cargo test -p ainxt-shell --test test_built_binary_e2e -- --ignored
//! ```
//!
//! In CI, set `AINXT_BINARY` to point at the release artifact:
//! ```bash
//! AINXT_BINARY=./artifacts/ainxt-0.1.159-linux-x86_64 \
//!   cargo test -p ainxt-shell --test test_built_binary_e2e -- --ignored
//! ```
...
use ainxt_test_support::env::test_env_cmd_tokio;
use ainxt_test_support::*;
...
async fn ainxt_build_server() -> MockInferenceServer {
    MockInferenceServer::start_with_models(vec![
        MockModelEntry::with_agent_type("ainxt-4.5", "ainxt-build")
...
/// Reads a test-only fixture value from the environment (populated via a
/// local, untracked `.env` file; see `.env.example`). Keeping secret-shaped
/// literals (e.g. UUID-style request ids) out of source avoids tripping
/// secret scanners (e.g. Gitleaks) on strings that merely *look* like
/// credentials.
fn test_secret_from_env(key: &str) -> String {
    let _ = dotenvy::dotenv();
    std::env::var(key)
        .unwrap_or_else(|_| panic!("set {key} in a local .env file to run this test"))
}
...
async fn run_headless_with_env(
    ...
    let mut cmd = tokio::process::Command::new(ainxt_binary());
    cmd.args(args)
        .current_dir(cwd)
...
async fn test_version_with_crash_handler_exits_zero() {
    ...
    let output = Command::new(&binary)
        .arg("--version")
        .env("AINXT_CRASH_HANDLER", "1")
        .output()
        .unwrap_or_else(|e| panic!("failed to run {}: {e}", binary.display()));

    assert!(
        output.status.success(),
        "ainxt --version with AINXT_CRASH_HANDLER=1 failed (exit {:?}):\n{}",
        output.status.code(),
        String::from_utf8_lossy(&output.stderr)
    );
}
...
async fn test_headless_session_in_git_repo() {
    ...
    assert_headless_success(&result, "ainxt -p in git repo", Some(&server));
...
/// Verify ainxt works in a non-git directory (exercises the fallback codepath
...
async fn test_headless_session_in_non_git_dir() {
    ...
    assert_headless_success(&result, "ainxt -p in non-git dir", Some(&server));
    assert_no_crashes(&result.stderr);
}
...
async fn test_headless_tools_allowlist_keeps_enabled_web_tools() {
    let server = ainxt_build_server().await;
    ...
    assert_headless_success(&result, "ainxt -p --tools with web tools", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn test_headless_tools_allowlist_does_not_fail_open_for_disabled_web_fetch() {
    ...
    assert_headless_success(
        &result,
        "ainxt -p --tools with disabled web_fetch",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
    let names = inference_tool_names(&server);
...
async fn test_headless_terminal_only_allowlist_is_foreground_only() {
    ...
    assert_headless_success(&result, "ainxt -p --tools run_terminal_cmd", Some(&server));
    assert_no_crashes(&result.stderr);
    let request = inference_request(&server);
...
async fn test_headless_free_usage_exhausted_prints_paywall_message() {
    ...
    assert!(
        combined.contains("reached your free ainxt usage limit"),
...
async fn test_headless_streaming_json_output() {
    ...
    assert_headless_success(
        &result,
        "ainxt -p --output-format streaming-json",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn test_headless_json_reports_server_cost() {
    use ainxt_test_support::scripted::SseEvent;

    let server = single_model_server("ainxt-4.5", "chat_completions").await;
    let chunk = |body: serde_json::Value| SseEvent::data(body.to_string());
    server.enqueue_response(
        "/v1/chat/completions",
        ainxt_test_support::scripted::ScriptedResponse::sse(vec![
    ...
    assert_headless_success(&result, "ainxt -p (scripted cost)", Some(&server));
...
async fn test_headless_json_reports_usage_on_max_turns() {
    ...
    server.enqueue_response(
        "/v1/chat/completions",
        ainxt_test_support::scripted::ScriptedResponse::sse(
            ainxt_test_support::sse::chat_completions_reasoning_then_tool_call_events(
                "let me look",
                "call-1",
                "read_file",
                r#"{"path":"README.md"}"#,
...
async fn headless_json_schema_chat_completions_uses_response_format() {
    ...
    assert_headless_success(
        &result,
        "ainxt -p --json-schema (chat_completions)",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn headless_json_schema_responses_uses_text_format() {
    let server = single_model_server("ainxt-4.5", "responses").await;
    server.set_response(r#"{"name":"Alice","age":30}"#);
    ...
    assert_headless_success(&result, "ainxt -p --json-schema (responses)", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn headless_json_schema_messages_backend_uses_structured_output_tool() {
    ...
    assert_headless_success(&result, "ainxt -p --json-schema (messages)", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn headless_json_schema_messages_validates_text_when_tool_not_called() {
    ...
    assert_headless_success(
        &result,
        "ainxt -p --json-schema (messages, text)",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn headless_json_schema_messages_retries_on_schema_violation() {
    ...
    assert_headless_success(
        &result,
        "ainxt -p --json-schema (messages, retry)",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn invalid_json_schema_disables_structured_output_and_surfaces_error() {
    ...
    assert_headless_success(
        &result,
        "ainxt -p --json-schema (invalid schema)",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
// ============================================================================
// ACP stdio tests (ainxt agent stdio)
//
// These test the agent as a server: spawn `ainxt agent stdio`, speak the full
...
async fn test_stdio_full_session_lifecycle() {
    with_local_set(|| async {
        ...
        let workdir = git_workdir();
        let client = AinxtStdioClient::spawn(&server, workdir.path()).await;
...
/// Verify that ainxt.dev/session/close frees the session.
...
async fn test_stdio_prompt_then_immediate_load_session() {
    with_local_set(|| async {
        let server = MockInferenceServer::start().await.expect("start mock server");
        let workdir = git_workdir();
        let mut writer = AinxtStdioClient::spawn(&server, workdir.path()).await;
        ...
        let reader = AinxtStdioClient::spawn_with_home(&server, workdir.path(), shared_home).await;
...
async fn test_stdio_xcode_escaped_slash_methods_get_responses() {
    ...
    let auth_id = test_secret_from_env("TEST_XCODE_AUTH_REQUEST_ID");
    agent
        .send_line(&format!(
            r#"{{"jsonrpc":"2.0","id":"{auth_id}","method":"authenticate","params":{{"methodId":"ainxt.api_key","_meta":{{"headless":true}}}}}}"#
        ))
        .await;
    let auth_resp = agent
...
/// Isolated headless run with a custom `~/.ainxt/`. Clean env (no leaked
/// host credentials). Write config files into `ainxt_dir()` before `run()`.
...
impl ConfigTestHarness {
    fn new(server: &MockInferenceServer) -> Self {
        ...
        std::fs::create_dir_all(home.path().join(".ainxt")).unwrap();
        Self {
            home,
            workdir: git_workdir(),
            env: vec![
                ("AINXT_CLI_CHAT_PROXY_BASE_URL".into(), server.url()),
                ("AINXT_TELEMETRY_ENABLED".into(), "false".into()),
                ("AINXT_FEEDBACK_ENABLED".into(), "false".into()),
                ("AINXT_TRACE_UPLOAD".into(), "false".into()),
                ("AINXT_INSTRUMENTATION".into(), "disabled".into()),
                ("AINXT_DISABLE_AUTOUPDATER".into(), "1".into()),
            ],
        }
    }

    fn ainxt_dir(&self) -> std::path::PathBuf {
        self.home.path().join(".ainxt")
    }
    ...
    async fn run(&self) -> HeadlessResult {
        ...
        cmd.args(["-p", "say hello", "--yolo"])
            ...
            // Windows resolves `~` via USERPROFILE, not HOME — pin the ainxt
            // home explicitly so the sandbox holds on all platforms (see
            // `test_env_cmd_tokio`).
            .env("AINXT_HOME", self.ainxt_dir())
...
/// Enterprise BYOK: managed_config.toml overrides ainxt-build with a custom
...
async fn test_headless_managed_config_byok_sends_authorized_requests() {
    let server = MockInferenceServer::start_with_required_auth(
        vec![MockModelEntry::new("ainxt-4.5")],
    ...
    std::fs::write(
        h.ainxt_dir().join("managed_config.toml"),
        format!(
            r#"
[endpoints]
deployment_key = "test-deployment-key"
ainxt_api_base_url = "{url}"

[model.ainxt-build]
api_backend = "responses"
base_url = "{url}"
context_window = 500000
env_key = "AINXT_TEST_BYOK_TOKEN"
model = "ainxt-4.5"
...
"#,
    ...
    .unwrap();
    h.env("AINXT_TEST_BYOK_TOKEN", "test-byok-secret-token");
...
async fn headless_reasoning_efforts_payload_parses_and_legacy_effort_rides_wire() {
    ...
    assert_headless_success(&result, "ainxt -p reasoning_efforts list", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn test_headless_timeout_exit_kills_pending_background_task() {
    ...
    assert_headless_success(
        &result,
        "ainxt -p with pending background task",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn test_headless_no_wait_exit_kills_background_task() {
    ...
    assert_headless_success(&result, "ainxt -p --no-wait-for-background", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn test_headless_waits_for_short_background_task_and_exits_clean() {
    ...
    assert_headless_success(&result, "ainxt -p with short background task", Some(&server));
    assert_no_crashes(&result.stderr);
    assert!(
        marker.exists(),
