...
mod imp {
    ...
    /// # Safety
    ///
    /// `ctx` must be the third argument passed by the kernel to a `SA_SIGINFO`
    /// signal handler — a valid pointer to a `ucontext_t` (or platform
    /// equivalent). Passing any other pointer is undefined behaviour.
    unsafe fn extract_pc_and_fp(ctx: *mut libc::c_void) -> (usize, usize) {
        ...
        // SAFETY: on Linux x86_64 the kernel guarantees `ctx` points to a
        // valid `ucontext_t` when the handler was registered with `SA_SIGINFO`.
        unsafe {
        ...
        // SAFETY: on Linux aarch64 the kernel guarantees `ctx` points to a
        // valid `ucontext_t` when the handler was registered with `SA_SIGINFO`.
        unsafe {
            let uc = ctx as *const libc::ucontext_t;
    ...
    /// Stops at the first invalid (null, misaligned, suspiciously small, or
    /// suspiciously large) frame pointer to limit exposure to a corrupted stack.
    unsafe fn walk_frame_pointers(initial_fp: usize, out: &mut [usize], max: usize) -> usize {
        let mut fp = initial_fp;
        let mut count = 0;

        // Upper-bound guard: reject addresses in the top 64 KiB of the address
        // space (kernel/guard pages on all supported platforms). This catches
        // obviously invalid high addresses that pass the zero-page check but
        // would fault on dereference.
        let addr_ceiling = usize::MAX - 0xFFFF;

        while count < max {
            // Validate: non-null, not in the zero page, not in the top guard
            // region, and pointer-aligned. Any failure means the frame chain
            // is corrupt — stop rather than fault.
            if fp == 0
                || fp < 4096
                || fp > addr_ceiling
                || !fp.is_multiple_of(core::mem::size_of::<usize>())
            {
                break;
            }
            ...
            //   [fp+8] = return address
            //
            // SAFETY: `fp` has passed the alignment and range checks above.
            // We are in a signal handler for a fatal fault — the stack may be
            // partially corrupted. A secondary fault here is caught by
            // `SA_RESETHAND` (the handler is reset to `SIG_DFL` on first
            // delivery), so a bad read terminates the process rather than
            // looping. The crash PC blob is already written to disk before
            // this walk begins, so a secondary fault still produces a report.
    ...
    /// `AtomicBool` instead of `static mut bool` so concurrent signal
    /// deliveries on different threads cannot data-race on this flag.
    ...
    fn save_termios() {
        // SAFETY: `ORIGINAL_TERMIOS` is written only here, called once at
        // install time before any signal handler is registered. No concurrent
        // writer exists at this point.
        unsafe {
            let termios = &mut *std::ptr::addr_of_mut!(ORIGINAL_TERMIOS);
            if libc::tcgetattr(0, termios) == 0 {
                HAS_TERMIOS.store(true, Ordering::Relaxed);
            }
        }
    }
    ...
    unsafe fn restore_termios_and_reraise(sig: libc::c_int) {
        unsafe {
            // SAFETY: `ORIGINAL_TERMIOS` is written once in `save_termios`
            // before any signal handler is registered. Reading it here (in the
            // signal handler) is safe because no concurrent writer exists after
            // install time. `HAS_TERMIOS` is now an `AtomicBool` so the load
            // is race-free.
            if HAS_TERMIOS.load(Ordering::Relaxed) {
    ...
    unsafe fn register_crash_signals(
        ...
        // SAFETY: `handler` is a valid `sa_sigaction`-compatible function
        // pointer (enforced by the caller's type signature). `std::mem::zeroed`
        // is valid for `libc::sigaction` — all-zero is a defined initial state
        // for that C struct. `libc::sigemptyset` and `libc::sigaction` are
        // async-signal-safe and correct to call here.
        unsafe {
            let mut sa: libc::sigaction = std::mem::zeroed();
    ...
    /// # Safety
    ///
    /// Must only be called from a signal handler context. `info` and `ctx`
    /// must be the `siginfo_t` and `ucontext_t` pointers passed by the kernel.
    /// Only async-signal-safe operations are used inside.
    unsafe fn write_crash_blob(
        sig: libc::c_int,
        info: *mut libc::siginfo_t,
        ctx: *mut libc::c_void,
    ) {
        // SAFETY: `CRASH_BUF` and `APP_VERSION` are `static mut` arrays written
        // only at install time (before any signal handler fires) and read only
        // here in the signal handler. `SA_RESETHAND` ensures at most one handler
        // invocation writes to them, preventing concurrent writes. The
        // `addr_of_mut!` / `addr_of!` macros avoid creating references to
        // `static mut`, which would be undefined behaviour.
        unsafe {
            let fd = CRASH_FD.load(Ordering::Relaxed);
            if fd >= 0 {
                // SAFETY: `info` is non-null only when the kernel provided it
                // (checked before each field access below).
    ...
    pub fn install(crash_dir: &Path, ainxt_version: &str) -> bool {
        ...
        // SAFETY: `APP_VERSION` is written here, once, before any signal
        // handler is registered. No concurrent writer exists at this point.
        unsafe {
            let version = &mut *std::ptr::addr_of_mut!(APP_VERSION);
            version.fill(0);
            let copy_len = ainxt_version.len().min(format::VERSION_STRING_LEN);
            version[..copy_len].copy_from_slice(&ainxt_version.as_bytes()[..copy_len]);
        }

        save_termios();
        setup_alt_stack();
        // SAFETY: `crash_handler_basic` is a valid `sa_sigaction` function pointer.
    ...
    pub fn enable_terminal_escape_restore() {
        // SAFETY: both `crash_handler` and `terminal_restore_handler` are
        // valid `sa_sigaction` function pointers.
        unsafe {
    ...
    pub fn disable_terminal_escape_restore() {
        // SAFETY: both `crash_handler_basic` and `terminal_restore_handler_basic`
        // are valid `sa_sigaction` function pointers.
        unsafe {
            register_crash_signals(if CRASH_FD.load(Ordering::Relaxed) >= 0 {
                crash_handler_basic
            } else {
...
mod tests {
    ...
    fn full_install_replaces_minimal_handler() {
        ...
        let dir = std::env::temp_dir().join("ainxt-crash-handler-test-replace");
