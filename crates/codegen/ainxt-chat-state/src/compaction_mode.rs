...
//! lossy summary dropped. In `ainxt-chat-state` so flag resolution and the
...
/// `Segments` carries its unchanged detail level inline, since detail is
...
impl CompactionMode {
    ...
    /// resolver attach the separately-resolved `AINXT_COMPACTION_DETAIL`.
    ...
    pub fn transcript_hint(self, location: Option<&str>) -> Option<String> {
        ...
        Some(match self {
            ...
            // Wording matches the segment-store continuation note.
