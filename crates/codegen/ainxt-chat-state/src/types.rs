...
/// emitter in `ainxt-shell` and the upsert/detection here — a drift would
...
pub enum AuthType {
    /// From AuthManager (ainxt login, OIDC, external binary). Refreshable.
    #[default]
    SessionToken,
    /// From user config ([model.*] api_key, env_key, AINXT_API_KEY). Not refreshable.
    ApiKey,
}
...
/// `ainxt_sampling_types::SamplingConfig` (which is secret-free).
