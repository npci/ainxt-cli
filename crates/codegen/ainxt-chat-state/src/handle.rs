...
impl ChatStateHandle {
    ...
    /// Out-of-band history repair (`ainxt.dev/session/repair`); see
    ...
    pub async fn get_credentials(&self) -> Credentials {
        // Diagnostic-only tag for the `query()` dead-actor log line below;
        // free-form and independent of the `ChatStateCommand` variant name,
        // so it can be worded to avoid echoing "Credentials" into a log
        // record even though this accessor never actually logs the
        // credential *value* itself (only that the query round-tripped).
        self.query("FetchAuthState", |reply| ChatStateCommand::GetCredentials {
            reply,
        })
        .await
        .unwrap_or_default()
