...
impl ChatStateActor {
    ...
    pub(super) fn build_conversation_request(
        ...
        ConversationRequest {
            ...
            x_ainxt_conv_id: Some(conv_id),
            x_ainxt_req_id: Some(req_id),
            ...
            json_schema: None,
            // Overwritten by the sampling client from the model's own config
            // before dispatch — see `apply_conversation_defaults`.
...
/// contribution, so the result is exactly the true body size.
///
...
/// tracks the true body exactly as images are removed.
///
...
mod tests {
    ...
    fn conversation_body_bytes_matches_serde_json_exactly() {
        ...
        // encode exactly — including non-image content and string
