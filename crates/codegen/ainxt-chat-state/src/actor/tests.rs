...
use tokio::sync::mpsc;
use ainxt_sampling_types::{ConversationItem, SamplingConfig};
...
async fn update_sampling_config_is_queryable() {
    ...
    assert_eq!(config.model, "ainxt-3");
...
async fn build_request_includes_all_messages() {
    ...
    assert_eq!(request.x_ainxt_conv_id, Some("conv-1".to_string()));
    assert_eq!(request.x_ainxt_req_id, Some("req-1".to_string()));
}
...
async fn build_request_with_tool_definitions() {
    use ainxt_sampling_types::ToolSpec;
...
async fn build_request_uses_sampling_config() {
    ...
    assert_eq!(request.model, Some("ainxt-3".to_string()));
...
/// Simulates the exact sequence that `ainxt-shell`'s `execute_tool_calls`
...
async fn parallel_tool_calls_accept_first_reject_second_skip_third() {
    ...
    let assistant_with_tools =
        ConversationItem::Assistant(ainxt_sampling_types::AssistantItem {
...
async fn has_dangling_tool_calls_reflects_unanswered_calls() {
    ...
    // stays dangling (matches a turn parked mid-tool / on a permission prompt).
    h.handle
...
async fn get_first_user_text_image_first_returns_none() {
    use ainxt_sampling_types::{ContentPart, UserItem};
...
async fn fresh_subagent_bootstrap_has_system_message_after_replace() {
    ...
    // Build the system prompt and inject it (matches acp_session.rs lines 1287-1295).
...
async fn forked_subagent_bootstrap_replaces_parent_system_message() {
    ...
    // (matches acp_session.rs lines 1287-1295).
...
async fn get_last_model_metadata_returns_both_fields() {
    let h = TestHarness::with_conversation(vec![
        ...
        ConversationItem::Assistant(ainxt_sampling_types::AssistantItem {
            content: "hello".into(),
            tool_calls: vec![],
            model_id: Some("ainxt-4.5".into()),
            model_fingerprint: Some("fp_abc123".into()),
            reasoning_effort: None,
        }),
    ]);
    let meta = h.handle.get_last_model_metadata().await;
    assert_eq!(meta.resolved_model_id.as_deref(), Some("ainxt-4.5"));
...
async fn sampling_config_survives_compaction_replacement() {
    use ainxt_sampling_types::ApiBackend;
    ...
    assert_eq!(pre.model, "ainxt-build");
    ...
    assert_eq!(pre_meta.resolved_model_id.as_deref(), Some("ainxt-4.5"));
...
/// If that model slug is e.g. "ainxt-build" and not in the ModelState
...
async fn model_metadata_lost_after_compaction_then_recovered_on_next_turn() {
    ...
    h.handle
        .push_assistant_response(ConversationItem::Assistant(
            ainxt_sampling_types::AssistantItem {
...
async fn context_window_downgrade_triggers_auto_compact() {
    ...
    // Initial config: 500k context, Responses backend (matches ainxt-4.5)
    let config = SamplingConfig {
    ...
    assert_eq!(post.model, "ainxt-4.5", "model slug must not change");
    assert_eq!(
        post.api_backend,
        ApiBackend::Responses,
...
fn serialize_via_public_api(
    req: &ainxt_sampling_types::ConversationRequest,
) -> serde_json::Value {
    ...
    ainxt_sampling_types::patch_reasoning_text_types(&mut body);
...
fn assert_prefix_stable_pair(
    base: &ainxt_sampling_types::ConversationRequest,
    extended: &ainxt_sampling_types::ConversationRequest,
    label: &str,
) {
...
async fn prefix_stable_after_image_pruning() {
    ...
    let h = TestHarness::with_conversation(vec![
        ConversationItem::system("sys"),
        ConversationItem::User(ainxt_sampling_types::UserItem {
            content: vec![
                ContentPart::Text {
    ...
    h.handle
        .push_assistant_response(ConversationItem::assistant("a2"));
    h.handle
        .push_user_message(ConversationItem::User(ainxt_sampling_types::UserItem {
            content: vec![
                ContentPart::Text {
                    text: "new image".into(),
                },
...
async fn prefix_stable_with_backend_tool_calls() {
    use ainxt_sampling_types::{BackendToolCallItem, BackendToolKind, rs};
...
// ============================================================================
// Out-of-band history repair (ainxt.dev/session/repair)
// ============================================================================
