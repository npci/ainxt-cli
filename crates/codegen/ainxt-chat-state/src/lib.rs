//! ainxt-chat-state — Actor-based chat state management for ainxt agents.
//!
//! This crate extracts conversation state management from `ainxt-shell`'s
//! `acp_session.rs` into a standalone actor. It follows the same actor pattern
//! as `ainxt-hunk-tracker`:
//!
//! ```text
//! ┌────────────────┐                  ┌──────────────────────────────────────┐
