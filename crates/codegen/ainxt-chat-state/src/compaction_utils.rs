...
//! no I/O, no actor state. They live in `ainxt-chat-state` so that both
//! this crate and `ainxt-shell` can share them without duplication.
use std::collections::BTreeSet;
use ainxt_sampling_types::{ContentPart, ConversationItem, ToolResultItem};
...
/// Segment-store prep (`segments` mode): keep tool I/O unchanged, strip only images + reasoning.
...
/// Shrink a unchanged conversation to `max_tokens`: drop oldest whole turns (System kept, tool runs unsplit; the last turn is truncated in place rather than dropped).
...
///
/// Stored here (rather than only in `ainxt-shell`) so that query-extraction
...
/// `ActiveSubagentSummary` in ainxt-tools. The mapping between them
/// happens in `run_compact_inner()` (ainxt-shell).
#[derive(Clone)]
...
/// A dependency-free mirror of `TodoStatus` (ainxt-tools), kept here so
...
impl TodoSummaryStatus {
    ...
    /// Matches `TodoStatus::tag()` in ainxt-tools.
...
/// handled by the consumer (e.g. `ainxt-shell`), which has access to
...
impl CompactionStateContext {
    ...
    /// `recent_messages` dropped, all other live state preserved unchanged.
    ///
    ...
    /// model to re-read the same files. ainxt-build retains
    /// `recent_messages` so the model keeps unchanged tool context.
...
/// unchanged even when it quotes `</analysis>`/`<summary>` in a later section.
...
/// turn. Matches `sanitize_evidence` in `goal_classifier.rs`. Closers first so
...
/// `ainxt-chat-state` and `ainxt-shell` share the same format.
...
pub struct CompactedHistoryInput<'a> {
    ...
    /// This preserves project instructions unchanged across compaction.
    ...
    /// When `false` (the default), recent messages come first (ainxt-build
    /// ordering).
    ...
    /// carrier and the ainxt-build summary.
    pub transcript_hint: Option<String>,
    /// Number of summaries generated so far for this user query, *including*
    /// the one being built. Rendered unchanged into the carrier's
    /// "Total summaries generated so far …" footer. Ignored by the ainxt-build
...
/// This is a pure function with no I/O. It matches exactly what
/// `run_compact_inner` in `ainxt-shell` assembles inline, but is
...
pub fn repair_history(items: &mut Vec<ConversationItem>) -> HistoryRepairReport {
    let duplicates_removed = ainxt_sampling_types::dedup_duplicate_tool_results(items);
    let stripped_tool_result_ids = strip_displaced_tool_results(items);
    let synthetic_results_inserted = ainxt_sampling_types::repair_dangling_tool_calls(
        items,
        ainxt_sampling_types::DanglingToolCallReason::HarnessHalted {
...
mod tests {
    use super::*;
    use ainxt_sampling_types::SyntheticReason;
    #[test]
...
actual user question";
    ...
    /// embeds the given path unchanged, so the trained model can re-read the
    ...
    fn call(id: &str) -> ainxt_sampling_types::ToolCall {
        ainxt_sampling_types::ToolCall {
            id: id.into(),
            name: "read_file".to_string(),
            arguments: "{}".into(),
        }
    ...
    async fn build_compacted_history_transcript_hint() {
        ...
        let summary = build_compacted_history(input(Some(
            "/home/user/.ainxt/sessions/abc/updates.jsonl".to_string(),
        )))
        .last()
        .unwrap()
        .text_content();
        assert!(summary.contains("/home/user/.ainxt/sessions/abc/updates.jsonl"));
    ...
    ///
    /// Simulates the exact conversation shape produced by ainxt-shell:
    ///
    ...
    async fn build_compacted_history_multi_turn_with_parallel_tool_calls() {
        use ainxt_sampling_types::{AssistantItem, ToolCall};
        let conversation = vec![
...
The user asked to read main.rs and lib.rs. main.rs prints hello world, lib.rs has an add function. The user then asked to fix a typo in main.rs and run tests. The typo was fixed and tests passed.";
    ...
    fn conversation_item_preserves_reasoning_siblings() {
        use ainxt_sampling_types::{AssistantItem, rs};
        let result = strip_tool_messages_for_conversation_item(vec![
            ConversationItem::system("system"),
    ...
    fn prepare_for_summarization_drops_reasoning_sibling_on_mutated_assistant() {
        use ainxt_sampling_types::{AssistantItem, ToolCall, rs};
        let mk_reasoning = || {
            ConversationItem::Reasoning(rs::ReasoningItem {
                id: "r_123".to_string(),
                summary: vec![rs::SummaryPart::SummaryText(rs::SummaryTextContent {
    ...
    /// The segment view must KEEP unchanged tool I/O (calls + results) — that's
    ...
    /// Unchanged view keeps tool calls (with arguments) and results — no flattening, no dropped results.
    #[test]
    ...
    fn verbatim_reasoning_kept_unless_messages_backend() {
        use ainxt_sampling_types::{ToolCall, rs};
        let mk = || {
            vec![
                ConversationItem::system("sys"),
                ConversationItem::Reasoning(rs::ReasoningItem {
        ...
        assert!(
            ...
            "reasoning must be kept when strip_reasoning = false (ainxt backends)"
        );
    ...
    fn fit_counts_user_images_against_budget() {
        use ainxt_sampling_types::ContentPart;
