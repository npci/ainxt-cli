...
/// Whole-turn-boundary truncation cap for one segment's unchanged section.
...
/// How much per-turn detail lands in the unchanged section. Matches the Python
...
/// Insert thousands separators (matches Python's `{:,}`).
...
/// One unchanged turn: role header, text, and `[tool_request: …]` arg lines.
...
/// `detail == None`) unchanged turns truncated at a whole-turn boundary before
...
/// Stopwords dropped from INDEX keywords (matches the Python implementation).
...
mod tests {
    ...
    /// curated summary, and a detail-specific unchanged section.
    #[test]
    ...
    /// Unchanged turns are dropped at a whole-turn boundary once the byte budget
    ...
    // --- Equivalence with the Python implementation's own test vectors (compaction_utils_test.py) ---
    ...
    /// Matches `TestComputeTurnStats::test_basic_counts` — same turns, same
    ...
    /// Matches `test_error_counting` + `test_last_assistant_excerpt`.
    #[test]
