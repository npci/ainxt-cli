...
use ainxt_config_types::DisplayRefreshSettings;
...
pub struct UiConfig {
    ...
    /// connect time (CLI `--hunk-tracker-mode` / `AINXT_HUNK_TRACKER` override
    ...
    /// Speech-to-text language preference for voice dictation. A ainxt STT
    /// catalog code (`en`, `es`, `ja`, … — see ainxt STT supported languages) or
...
impl Default for UiConfig {
    fn default() -> Self {
        Self {
            ...
            theme: None,
            // Empty string is the "no opinion" sentinel: the fork feature falls
            // back to the session/gateway model when this is unset. No bundled
            // default model exists — the catalog is sourced only from the gateway.
