...
//! image bytes.
//!
//! Both the TUI ([`ainxt_pager::prompt_images`]) and the server-side
...
//! [`ainxt_pager::prompt_images::display_text`]. The shape of this
...
/// position (the two diverge — see `AttachedImages` in `ainxt-tools`).
pub const IMAGE_DISPLAY_NUMBER_META_KEY: &str = "ainxt.dev/imageDisplayNumber";
...
/// [`AttachedImages`](ainxt_tools::types::resources::AttachedImages))
...
///   separator — see [`ainxt_pager::prompt_images::display_text`].
...
/// [`ainxt_pager::prompt_images::read_image_at_path`], which has no
...
fn decode_image_mime(data: &[u8]) -> Option<&'static str> {
    ainxt_tools::util::image_validate::validate_image_bytes_with(data, false)
        .ok()
        .map(|(_, _, mime)| mime)
}
...
mod tests {
    ...
    fn strip_paths_returns_input_unchanged_when_no_placeholders() {
        ...
        // no-match branch returns it unchanged — no allocation. The
    ...
    /// `ainxt_pager::prompt_images::display_text`. A
    ...
    fn attached_image_references_prefers_file_path_over_data() {
        ...
        let img = agent_client_protocol::ImageContent::new("AAAA", "image/png")
            .uri(Some(
                "file:///Users/me/.ainxt/sessions/s/images/image-1.png".into(),
            ))
        ...
        assert_eq!(
            ...
            vec![(
                1,
                "/Users/me/.ainxt/sessions/s/images/image-1.png".to_string()
            )]
        );
    }
