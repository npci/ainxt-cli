...
//! `AINXT_CLIPBOARD_NO_NATIVE_READ=1` disables the in-process read entirely
...
///
/// Matches the `osascript` probes' coercion order (`PNGf` → `TIFF` → `JPEG`)
...
/// the `AINXT_CLIPBOARD_NO_DATA_CONTROL` kill-switch env var is set. The kill
...
mod platform {
    ...
    /// path, and `None` when `AINXT_CLIPBOARD_NO_NATIVE_READ` is set (kill
    ...
    pub(super) fn native_image_read() -> Option<super::ImageData> {
        if std::env::var_os("AINXT_CLIPBOARD_NO_NATIVE_READ").is_some() {
            return None;
        }
        let _guard = PASTEBOARD_LOCK.lock();
        objc2::rc::autoreleasepool(|_| {
    ...
    fn attachments_probe_temp_paths() -> (std::path::PathBuf, std::path::PathBuf, std::path::PathBuf)
    {
        ...
        (
            temp_dir.join("ainxt-clipboard-probe.png"),
            temp_dir.join("ainxt-clipboard-probe.tiff"),
            temp_dir.join("ainxt-clipboard-probe.jpg"),
        )
    }
...
mod platform {
    ...
    /// Memoized read of the `AINXT_CLIPBOARD_NO_DATA_CONTROL` kill switch —
    ...
    fn data_control_kill_switch_set() -> bool {
        ...
        *SET.get_or_init(|| std::env::var_os("AINXT_CLIPBOARD_NO_DATA_CONTROL").is_some())
    }

    /// True when the arboard leg must be skipped entirely: on Wayland the
    /// `AINXT_CLIPBOARD_NO_DATA_CONTROL` kill switch has to stop arboard from
        ...
        LEASE
            .get_or_init(|| {
                if arboard_wayland_bypassed() {
                    tracing::debug!("arboard leg disabled (AINXT_CLIPBOARD_NO_DATA_CONTROL)");
                    return None;
                }
    ...
    fn arboard_read_with_deadline<T: Send + 'static>(
        ...
        if arboard_wayland_bypassed() {
            anyhow::bail!("arboard leg disabled (AINXT_CLIPBOARD_NO_DATA_CONTROL)");
        }
    ...
    const WL_SPEC: ToolSpec = ToolSpec {
        ...
        // return the selection unchanged, which `wayland_readback_matches` compares exactly.
    ...
    /// (matches the lease's timeout ⇒ permanent-degradation stance), so a
    ...
    pub(super) fn wayland_data_control_supported() -> bool {
        ...
        // switch (precedent: AINXT_CLIPBOARD_NO_NATIVE_READ) and non-Wayland
    ...
    /// this matches arboard's own backend selection); `MissingProtocol` /
    ...
    /// --no-newline` returns the selection unchanged (no appended newline), so an
    ...
    mod linux_tests {
        ...
        fn readback_exact_match() {
            ...
            // Unchanged trailing newline must verify (wl-paste --no-newline does not strip it).
        ...
        fn probe_cache_indefinite_stays_undecided_below_cap() {
            ...
            // Cap reached: permanently decided false (matches lease degradation).
            assert!(!apply_probe_outcome(&mut cache, ProbeOutcome::Indefinite));
...
mod tests {
    ...
    #[ignore]
    fn round_trip() {
        let sentinel = format!("ainxt-clipboard-test-{}", std::process::id());
    ...
    mod attachments_protocol_tests {
        ...
        fn parse_furl_multi_path_trailing_newline() {
            ...
            // preserved unchanged — the drop-path parser splits on
            // them.
            assert_eq!(
...
mod macos_probe_smoke {
    ...
    /// `cargo test -p ainxt-shared -- --ignored probe_smoke`.
    #[test]
