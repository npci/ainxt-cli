...
/// Test-only entry point: invokes the private [`install_gateway`] for
/// integration tests that stand up a mock gateway server and must isolate
/// `AINXT_HOME`.
#[doc(hidden)]
pub async fn install_gateway_for_test(
    target: Option<&str>,
    update_config: &UpdateConfig,
) -> Result<()> {
    install_gateway(target, update_config).await
}
