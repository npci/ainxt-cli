...
use ainxt_shell::env::AinxtBuildEnvironment;
use ainxt_shell::util::ainxt_home::ainxt_home;

const TTL_SECONDS_BEFORE_AUTO_UPDATE: Duration = Duration::from_secs(60 * 30);
const NPM_PACKAGE: &str = "@ainxt-official/ainxt";
pub const GH_RELEASE_REPO: &str = "ainxt-org-shared/ainxt-build";

/// Environment variable naming the update origins, comma-separated, in
/// preference order.
pub(crate) const CLI_BASE_URLS_ENV: &str = "AINXT_CLI_BASE_URLS";
...
///
/// **Empty in the open-source build, and that is the point.**
///
/// This was a two-element constant whose first entry was `https://ainxt.dev/cli`
/// — a domain that was never registered (audit risk R42). An update origin is
/// the most dangerous URL in a program: whoever answers it decides what code
/// runs on the next launch. Shipping an unregistered one meant the first person
/// to buy that name owned every installed client. The old fallback's own doc
/// comment listed "DNS hijack" among the reasons a fallback was needed, which
/// was the right worry aimed one position too late.
///
/// A source-available build has no update service behind it, so the honest
/// default is none. With no origins configured the updater reports that it has
/// nowhere to check — all three call sites already handle the empty list and
/// carry the error text for it — and the CLI works normally otherwise. Updating
/// is then whatever the operator chose: a package manager, a tagged release, or
/// the gateway-served channel via [`UpdateConfig::gateway_url`].
///
/// Operators publishing their own builds set `AINXT_CLI_BASE_URLS` to origins
/// **they control**:
/// `AINXT_CLI_BASE_URLS=https://artifacts.example.internal/ainxt/cli`
pub(crate) fn cli_base_urls() -> &'static [&'static str] {
    static URLS: std::sync::OnceLock<Vec<&'static str>> = std::sync::OnceLock::new();
    URLS.get_or_init(|| {
        std::env::var(CLI_BASE_URLS_ENV)
            .ok()
            .map(|raw| {
                raw.split(',')
                    .map(str::trim)
                    .filter(|s| !s.is_empty())
                    // Leaked deliberately, and at most once per process: update
                    // origins are process-lifetime configuration, and producing
                    // `&'static str` keeps every existing call site's signature
                    // unchanged.
...
///
/// Constructed once from `AinxtBuildEnvironment` at startup and threaded through the
/// update call chain so that `auto_update` and `version` never need to know
/// about the `AinxtBuildEnvironment` enum directly.
#[derive(Debug, Clone)]
pub struct UpdateConfig {
    /// Chat API proxy base URL (versioned `https://api.example.test/v1` endpoint).
    pub proxy_base_url: String,
    /// Auth scope key for `~/.ainxt/auth.json`.
    pub auth_scope: String,
    /// Enterprise deployment key (AINXT_DEPLOYMENT_KEY).
    ...
    /// Base URL of the ainxt gateway (`AINXT_GATEWAY_URL`). When set, the
    /// gateway-served update channel becomes the default installer: the CLI
    /// pulls signed builds from `{gateway_url}/ainxt/v1/api/cli/...` instead of
    /// npm/GitHub/GCS. See [`crate::gateway`].
    pub gateway_url: Option<String>,
}

impl UpdateConfig {
    pub fn from_environment(env: &AinxtBuildEnvironment) -> Self {
        Self {
            proxy_base_url: env.cli_chat_proxy_base_url(),
            auth_scope: ainxt_shell::auth::AinxtComConfig::default().auth_scope(),
            deployment_key: None,
            alpha_test_key: None,
            channel: "stable".to_string(),
            npm_registry: None,
            gateway_url: std::env::var("AINXT_GATEWAY_URL")
                .ok()
...
/// not semver, so both are needed to pick the right one.
#[doc(hidden)]
...
pub async fn fetch_latest_version(installer: &str, config: &UpdateConfig) -> Result<String> {
    tracing::debug!(
        ...
        "[auto-update] fetch_latest_version: starting version fetch"
    );
    let result = match installer {
        ...
        "gateway" => crate::gateway::fetch_gateway_manifest(config)
            .await
            .map(|m| m.version),
    ...
    match &result {
        Ok(version) => tracing::debug!(
            installer = %installer,
            version = %version,
            "[auto-update] fetch_latest_version: got latest version"
        ),
        Err(e) => tracing::warn!(
            installer = %installer,
            error = %e,
            "[auto-update] fetch_latest_version: failed to fetch latest version"
        ),
    }
    result
}
...
pub async fn write_version_cache(version: &str, stable_version: Option<&str>) {
    let version_path = ainxt_home().join("version.json");
    let now = time::OffsetDateTime::now_utc();
    let json = AinxtVersion::new(
        version.to_string(),
...
pub async fn is_version_cache_fresh() -> bool {
    ...
    if let Ok(version_str) = fs::read_to_string(&version_path).await
        && let Ok(version) = serde_json::from_str::<AinxtVersion>(&version_str)
...
pub use ainxt_version::installed as get_installed_ainxt_version;

/// Version of the managed ainxt binary currently on disk, read from the
/// `~/.ainxt/bin/ainxt` symlink target (`../downloads/ainxt-<version>-<platform>`)
/// without exec'ing anything.
///
/// Concurrent updaters (TUI background download, leader hourly checker,
/// explicit `ainxt update`) decide staleness from this instead of their own
...
/// link whose target binary was deleted (e.g. manual `~/.ainxt/downloads`
...
pub fn installed_on_disk_version() -> Option<String> {
    #[cfg(unix)]
    {
        let app = ainxt_shell::util::ainxt_home::ainxt_application();
        ...
        version_from_versioned_binary_name(target.file_name()?.to_str()?, "ainxt")
    }
    #[cfg(not(unix))]
    {
        None
...
///
/// Handles the internal layout (`ainxt-0.1.150-macos-aarch64`, including
/// pre-releases: `ainxt-0.1.150-alpha.1-linux-x86_64` → `0.1.150-alpha.1`)
/// and the npm layout without a platform suffix (`ainxt-0.1.150`,
/// `ainxt-0.1.150-alpha.1`): everything between the `{bin_prefix}-` prefix
/// and the first platform-OS component is the version, validated as semver
/// so unknown layouts (`ainxt-latest`, `ainxt-pager-*` when `bin_prefix` is
/// `ainxt`) return `None` instead of garbage.
///
...
///
/// Tries each base URL from [`cli_base_urls`] and returns the first success.
...
/// Read the cached stable version from `~/.ainxt/version.json` (sync, for display).
///
...
pub fn cached_stable_version() -> Option<String> {
    ...
    let gv: AinxtVersion = serde_json::from_str(&content).ok()?;
    gv.stable_version
}
...
pub fn channel_name() -> Option<&'static str> {
    ...
    *NAME.get_or_init(|| {
        ...
        derive_channel(ainxt_version::VERSION, &stable)
    })
}
...
/// `~/.ainxt/version.json` (written by the auto-updater):
...
pub fn channel_label() -> &'static str {
    ...
    LABEL.get_or_init(|| {
        ...
        match derive_channel(ainxt_version::VERSION, &stable) {
            Some("alpha") => " [alpha]",
            Some(_) => " [stable]",
            None => "",
        }
...
mod tests {
    ...
    fn test_is_fresh_rejects_future_timestamp() {
        ...
        let v = AinxtVersion::new("0.1.200".to_string(), None, future);
        assert!(
    ...
    /// symlink-target file name (`ainxt-<version>-<platform>`).
    #[test]
    fn test_version_from_versioned_binary_name() {
        let cases: &[(&str, Option<&str>)] = &[
            ("ainxt-0.2.46-darwin-arm64", Some("0.2.46")),
            ("ainxt-0.1.220-linux-x86_64", Some("0.1.220")),
            ("ainxt-0.2.5-windows-x86_64.exe", Some("0.2.5")),
            ...
            ("ainxt-0.1.220-alpha.4-linux-x86_64", Some("0.1.220-alpha.4")),
            ("ainxt-0.1.220-alpha.4", Some("0.1.220-alpha.4")), // npm layout
            ("ainxt-pager-0.1.5-darwin-arm64", None),           // "pager" is not a version
            ("ainxt-garbage-darwin-arm64", None),               // unparseable version
            ("ainxt-0.2.46", Some("0.2.46")),                   // no platform suffix
            ("other-0.2.46-darwin-arm64", None),               // wrong prefix
            ("ainxt-latest", None),                             // symlink alias, not a version
            ("ainxt", None),                                    // bare name
            ("", None),
        ];
        for (name, expected) in cases {
            assert_eq!(
                version_from_versioned_binary_name(name, "ainxt").as_deref(),
                *expected,
        ...
        // prefix but not under "ainxt".
        assert_eq!(
            version_from_versioned_binary_name("ainxt-pager-0.1.5-darwin-arm64", "ainxt-pager")
                .as_deref(),
            Some("0.1.5")
        );
    }
    ...
    // ──────────────────────────────────────────────────────────────────────
    // AinxtVersion JSON shape — backward compatibility invariants
    // ──────────────────────────────────────────────────────────────────────
    ...
    fn test_version_json_backward_compat() {
        ...
        let v: AinxtVersion = serde_json::from_str(old).unwrap();
        assert_eq!(v.version, "0.1.180");
        assert!(v.stable_version.is_none());

        // New format with stable_version round-trips correctly.
        let now = time::OffsetDateTime::now_utc();
        let new = AinxtVersion::new("0.2.5".to_string(), Some("0.2.3".to_string()), now);
        let json = serde_json::to_string(&new).unwrap();
        let parsed: AinxtVersion = serde_json::from_str(&json).unwrap();
        ...
        assert!(serde_json::from_str::<AinxtVersion>(future).is_ok());

        // Missing required field (checked_at) is rejected.
        let missing = r#"{"version":"0.1.180"}"#;
        assert!(serde_json::from_str::<AinxtVersion>(missing).is_err());
    }
    ...
    fn test_is_fresh_ttl_boundaries() {
        let now = time::OffsetDateTime::now_utc();
        let v = AinxtVersion::new("0.1.200".to_string(), None, now);
    ...
    fn test_update_config_default_channel_is_stable() {
        use ainxt_shell::env::AinxtBuildEnvironment;
        let cfg = UpdateConfig::from_environment(&AinxtBuildEnvironment::Production);
