...
//! exact JSON shape produced by `ainxt update --check --json` for the failure
//! modes that real users have hit in the wild.
//!
//! Seen when a user is behind a corporate npm registry mirror:
//!
//! ```text
//! # Mirror returns 403 for the @ainxt-official scope
...
use ainxt_update::auto_update::check_update_status;

/// Set up a fake `npm` on PATH, set `AINXT_INSTALLER=npm` so the auto-update
...
async fn check_status_surfaces_npm_403_in_error_field() {
    ...
    g.set_stderr(
        "npm error code E403\n\
         npm error 403 403 Forbidden - GET https://registry-mirror.example.invalid/api/npm/js-virtual/@ainxt-official%2fainxt\n\
