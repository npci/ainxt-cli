...
//! its own `OnceLock<AINXT_HOME>`. The helpers below ensure the per-binary
//! initialization is identical: same env-var set, same isolation guarantees,
//! same reset between tests.
//!
//! Matches the AINXT_HOME isolation pattern used in other integration tests.
//!
//! ## Usage
//!
//! ```ignore
...
//! #[serial_test::serial]
//! async fn my_test() {
//!     let _ = test_home();   // initializes AINXT_HOME once per binary
...
// ─────────────────────────────────────────────────────────────────────────────
// AINXT_HOME isolation
// ─────────────────────────────────────────────────────────────────────────────

/// Returns a process-wide test `AINXT_HOME`, initialized exactly once per test
/// binary. Once initialized, `ainxt_config::ainxt_home()` will resolve to
...
pub fn test_home() -> &'static PathBuf {
    ...
    HOME.get_or_init(|| {
        ...
        unsafe {
            ...
            std::env::remove_var("AINXT_MANAGED_BY_NPM");
            std::env::remove_var("AINXT_MANAGED_BY_INTERNAL");
        }
        path
    })
}

/// Wipe state in `AINXT_HOME` between tests so each test sees a clean home.
...
/// Override the version reported by `get_installed_ainxt_version()` for the
/// duration of the test (until [`reset_home`] or process exit).
pub fn set_test_version(v: &str) {
    // SAFETY: tests using this helper must be `#[serial]`.
    unsafe { std::env::set_var("AINXT_TEST_VERSION", v) };
}
...
/// Minimal [`ainxt_update::UpdateConfig`] for install tests.
pub fn make_update_config(channel: &str) -> ainxt_update::UpdateConfig {
    ainxt_update::UpdateConfig {
...
/// Backdate every file in `AINXT_HOME/downloads` by ~2 hours.
///
