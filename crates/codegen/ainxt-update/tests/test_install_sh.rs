...
//!
//! > After any install attempt, `$BIN_DIR/ainxt` resolves to a binary that runs,
...
//!
//! The installer lives in the sibling `ainxt-pager` crate; it is resolved by
...
fn script_path(name: &str) -> Option<PathBuf> {
    dunce::canonicalize(
        Path::new(env!("CARGO_MANIFEST_DIR")).join(format!("../ainxt-pager/scripts/{name}")),
    )
    .ok()
    .filter(|p| p.exists())
}
...
const INSTALLER_BLOCK_START: &str = "# >>> ainxt installer >>>";
...
# A `<artifact>.sha256` fetch (no -o, goes to stdout). install.sh ENFORCES this
# when present, so the fixture must publish the digest of the very bytes it
# serves in `full` mode -- otherwise every install aborts with CHECKSUM
# MISMATCH and both tests in this file fail. They did, from the moment
# integrity verification was added to install.sh until 2026-08-29: the fixture
# was URL-agnostic and answered the .sha256 request with the version string
# below, which install.sh then compared against the real digest. No CI runs in
# this repository, so nothing reported it.
#
# In truncate/garbage modes the served bytes deliberately differ from this
# digest, so the mismatch path is exercised too -- which is what the
# ("truncate", false) / ("garbage", false) cases expect.
case "$url" in
  *.sha256)
    tmpf=$(mktemp)
    printf '%s' '{good}' > "$tmpf"
    if command -v sha256sum >/dev/null 2>&1; then
      sha256sum "$tmpf" | awk '{{print $1}}'
    else
      shasum -a 256 "$tmpf" | awk '{{print $1}}'
    fi
    rm -f "$tmpf"
    exit 0 ;;
esac
...
fn seed_previous_good(home: &Path, platform: &str) -> PathBuf {
    let downloads = home.join(".ainxt").join("downloads");
    let bin = home.join(".ainxt").join("bin");
    std::fs::create_dir_all(&downloads).unwrap();
    std::fs::create_dir_all(&bin).unwrap();
    let prev = downloads.join(format!("ainxt-{platform}"));
    ...
    std::os::unix::fs::symlink(format!("../downloads/ainxt-{platform}"), &link).unwrap();
    dunce::canonicalize(&prev).unwrap()
}

/// Re-resolve `$BIN_DIR/ainxt` from disk and re-run it: the active ainxt must
/// always execute, and never be a `.tmp`/partial file.
fn assert_active_ainxt_runs(home: &Path) {
    let link = home.join(".ainxt").join("bin").join("ainxt");
    assert!(link.is_symlink(), "ainxt must remain a symlink");
    let resolved =
        dunce::canonicalize(&link).unwrap_or_else(|e| panic!("ainxt symlink dangles: {e}"));
    let name = resolved.file_name().unwrap().to_string_lossy().to_string();
    assert!(
        !name.contains(".tmp"),
        "active ainxt must not be a temp file: {name}"
    );
    let ok = Command::new(&resolved)
        ...
        .map(|s| s.success())
        .unwrap_or(false);
    assert!(ok, "active ainxt must run: {}", resolved.display());
}
...
fn run_installer(install_sh: &Path, home: &Path, fakebin: &Path, mode: &str, shell: &str) -> bool {
    ...
    let status = Command::new("/bin/bash")
        ...
        .env("PATH", path_env)
        .env("SHELL", shell)
        .env("AINXT_BIN_DIR", home.join(".ainxt").join("bin"))
        .env("AINXT_CHANNEL", "stable")
        // Pin the artifact origin explicitly. Two reasons:
        //  1. install-enterprise.sh REQUIRES it -- it has no default and no
        //     fallback origin, by design (a hardcoded third-party GCS bucket
        //     used to be the silent fallback; see CLI-026).
        //  2. It keeps this test hermetic: install.sh's other branch resolves
        //     the latest release via api.github.com, and a unit test must not
        //     depend on the network. The fake curl intercepts every request
        //     regardless, and `.invalid` is RFC 6761 reserved so it can never
        //     resolve even if something slipped past the fake.
        .env("AINXT_BASE_URL", "https://artifacts.example.invalid/cli")
        .env("FAKE_MODE", mode)
        .status()
...
fn assert_single_installer_block(path: &Path, preserved: Option<&str>) {
    ...
    assert_eq!(
        n,
        1,
        "{} must contain exactly one ainxt installer block, got {n}:\n{body}",
        path.display()
    );
...
fn run_shell_rc_case(case: &ShellRcCase) {
    ...
    assert_active_ainxt_runs(&home_path);
}

#[test]
fn install_sh_blitz_keeps_ainxt_runnable_under_corruption() {
    ...
    for (mode, expect_ok) in cases {
        ...
        // ainxt always runs (new good binary on success, previous-good on
        // rejection).
        assert_active_ainxt_runs(home.path());
    }
}
