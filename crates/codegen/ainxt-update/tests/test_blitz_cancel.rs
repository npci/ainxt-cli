...
//!
//! > `~/.ainxt/bin/ainxt` resolves to a binary that passes the smoke-test, OR it
...
use ainxt_update::auto_update::install_internal_from_base;
...
/// (`ainxt` and `agent` — see `swap_managed_bin_links`). Returns the
...
/// What the active `ainxt` should resolve to after an install attempt.
...
/// to both `ainxt` and `agent` — `swap_managed_bin_links` moves them together.
...
/// `cargo nextest run -p ainxt-update --run-ignored all`.
#[tokio::test(flavor = "multi_thread")]
#[serial]
...
async fn blitz_fuzz_stress() {
    ...
    let iterations: usize = std::env::var("AINXT_BLITZ_ITERS")
        .ok()
