...
mod tests {
    ...
    fn test_is_cli_chat_proxy_url_accepts_proxy_subpath() {
        // is_cli_chat_proxy_url() itself can no longer be asserted true for
        // any *.ainxt.dev URL: PROD_CLI_CHAT_PROXY_BASE_URL ships empty by
        // design (audit risk R42 -- no compiled trust anchor), so the
        // production value this test used to assume is gone, and only an
        // operator's own AINXT_PRODUCTION_CLI_CHAT_PROXY_BASE_URL or
        // loopback are trusted at runtime. Test the subpath-acceptance
        // logic directly against an explicit trusted base instead, which is
        // what this test is actually for.
        assert!(matches_trusted_base_url(
            "https://api.ainxt.dev/v1/chat/completions/some/deeper/path",
