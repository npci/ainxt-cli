...
//! are published per-version to the CDN at `ainxt.dev/cli/changelogs/`.
//!
...
/// CDN base for all changelogs, resolved through `ainxt_env` so a fork or a
/// self-hosted deployment can serve its own via `AINXT_URL_CHANGELOG_BASE`.
fn changelog_base() -> String {
    ainxt_env::url_changelog_base().into_owned()
}
...
impl ChangelogManager {
    pub fn new() -> Self {
        // Prefer live `$AINXT_HOME` so harness-injected homes (PTY e2e) always
    ...
    /// `ainxt_home()` OnceLock). A seeded `$AINXT_HOME` set on the pager
    /// process is always honoured even if some earlier init path cached a
    /// different home.
    fn from_env_home() -> Self {
        let home = std::env::var_os("AINXT_HOME")
            .map(std::path::PathBuf::from)
            .filter(|p| !p.as_os_str().is_empty())
            .unwrap_or_else(crate::util::ainxt_home::ainxt_home);
        Self {
    ...
    ///
    /// When `AINXT_CHANGELOG_OFFLINE` is set (PTY / integration tests), skip
    ...
    /// `$AINXT_HOME` so harness-injected env always applies.
    ///
    ...
    /// without mutating process-global env (`AINXT_HOME` /
    /// `AINXT_CHANGELOG_OFFLINE`), which races across the parallel test
    ...
    fn fetch_with(&self, offline: bool, base: &str) -> Changelog {
        // No changelog host configured (the default in the open-source build --
        // see R42). Serve whatever is cached and make no network attempt: with
        // an empty base every URL below would be a relative path, and the
        // request is guaranteed to fail after burning the timeout.
        ...
        // any on-disk seed under `$AINXT_HOME` even when offline mode was not
...
fn changelog_offline() -> bool {
    std::env::var_os("AINXT_CHANGELOG_OFFLINE").is_some_and(|v| !v.is_empty() && v != "0")
}
...
mod tests {
    ...
    /// `$AINXT_HOME` env so tests never race the parallel harness.
    ...
    fn cdn_miss_falls_back_to_env_home_disk_cache() {
        let tmp = tempfile::tempdir().unwrap();
        let home = tmp.path().join("ainxt-home-fallback");
        std::fs::create_dir_all(&home).unwrap();
