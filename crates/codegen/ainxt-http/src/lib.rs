...
//! `ainxt_sampler::shared_http` (one HTTP/2 pooled client plus
//! a pool-less HTTP/1.1 fallback shared across every
//! `SamplingClient`). The sampler reads `AINXT_POOL_*` /
//! `AINXT_CONNECT_TIMEOUT_SECS` once, when its shared client is
//! first built, and `AINXT_SAMPLER_SHARED_CLIENT=0` falls back to
...
// `OriginClientInfo` is owned by `ainxt-sampler` so `SamplerConfig` can use
// it without taking a circular dependency on `ainxt-shell`. Re-exported
// under the same path (`crate::http::OriginClientInfo`) so existing call-sites
// compile unchanged. The telemetry engine in `ainxt-telemetry` consumes
// the same type via `ainxt_sampler::OriginClientInfo`. The shell-specific
...
/// `AINXT_CLIENT_VERSION` env vars. Returns `None` when
/// `AINXT_CLIENT_NAME` is unset.
...
/// Process-level client identifier (`AINXT_CLIENT_NAME` env var, default `"ainxt-shell"`).
pub fn process_client_identifier() -> String {
    std::env::var("AINXT_CLIENT_NAME").unwrap_or_else(|_| "ainxt-shell".to_string())
}

/// Header telling cli-chat-proxy whether this process is a single-prompt
/// (`ainxt -p`) run or an interactive session; feeds the `client_mode`
/// metric label.
pub const CLIENT_MODE_HEADER: &str = "x-ainxt-client-mode";

/// One-way latch: set to `"headless"` at startup by the non-TUI entry points
/// (`run_single_turn` for `ainxt -p`, `run_headless_inner` for
/// `ainxt agent [headless]`), `"interactive"` otherwise.
...
/// Apply the process-wide TLS policy to a client builder.
///
...
/// all ainxt HTTP clients must connect to servers with valid certificates.
pub fn apply_tls_policy(builder: reqwest::ClientBuilder) -> reqwest::ClientBuilder {
    builder
}

/// Blocking-client variant of [`apply_tls_policy`].
///
/// TLS certificate and hostname verification are always enforced.
pub fn apply_tls_policy_blocking(
...
pub fn shared_client() -> reqwest::Client {
    ...
    CLIENT
        .get_or_init(|| {
            ...
            apply_tls_policy(reqwest::Client::builder())
...
pub fn with_auth_retry(
    ...
    credentials: std::sync::Arc<dyn ainxt_auth::AuthCredentialProvider>,
) -> reqwest_middleware::ClientWithMiddleware {
    reqwest_middleware::ClientBuilder::new(client)
        .with(ainxt_auth::AuthRetryMiddleware::new(credentials, 1))
        .build()
}
...
///
/// Matches `shared_client()`'s pool self-healing for the same reason: this client
...
pub fn shared_blocking_client() -> reqwest::blocking::Client {
    ...
    BLOCKING_CLIENT
        .get_or_init(|| {
            ...
            apply_tls_policy_blocking(reqwest::blocking::Client::builder())
                .connect_timeout(std::time::Duration::from_secs(30))
...
mod tests {
    ...
    fn origin_client_info_from_meta_extracts_identifier_and_version() {
        let meta = serde_json::json!({
            "clientIdentifier": "ainxt-desktop",
            "clientVersion": "1.2.3",
        })
        .as_object()
        .cloned()
        ...
        assert_eq!(
            ...
            Some(OriginClientInfo {
                product: "ainxt-desktop".to_string(),
    ...
    fn session_user_agent_string_renders_expected_variants() {
        ...
        assert!(with_version.starts_with("ainxt-desktop/1.2.3 ainxt-shell/"));
        ...
        assert!(without_version.starts_with("ainxt-web ainxt-shell/"));
        assert!(!without_version.starts_with("ainxt-web/"));
    }
    ...
    fn user_agent_render_collapses_duplicate_origin_and_agent_identity() {
        ...
        assert_eq!(ua.render(), "ainxt-shell/0.1.171 (macos; aarch64)");
    }
}
