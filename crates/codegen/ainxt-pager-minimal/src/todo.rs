...
//! [`todo_panel_lines`] + [`render_todo_panel`]. Matches the full-TUI `TodoPane`
...
use ainxt_shell::tools::TodoStatus;
...
/// todos. Matches the full-TUI `TodoPane`'s glyphs/colors.
    ...
    let mut lines: Vec<Line<'static>> = todos
        ...
        .map(|t| {
            let (glyph, style) = match t.status {
                ...
                TodoStatus::Completed => (ainxt_pager::glyphs::check_mark(), theme.muted()),
                TodoStatus::Cancelled => (
                    ainxt_pager::glyphs::ballot_x(),
...
mod tests {
    ...
    use ainxt_shell::tools::{TodoItem, TodoPriority};
