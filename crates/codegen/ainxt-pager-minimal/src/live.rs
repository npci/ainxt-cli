...
use ainxt_pager::app::PagerTerminal;
use ainxt_pager::app::app_view::{ActiveView, AppView};
use ainxt_pager::minimal_api;
use ainxt_pager::render::Renderable;
use ainxt_pager::scrollback::state::ScrollbackState;
use ainxt_pager::scrollback::wrappers::EntryRenderer;
use ainxt_pager::theme::Theme;
use ainxt_pager::views::prompt_widget::PromptStyle;
use ainxt_pager::views::turn_status;
...
/// welcome box.
pub(super) fn live_left_inset(_appearance: &ainxt_pager::appearance::AppearanceConfig) -> u16 {
    0
}
...
pub(super) fn prompt_style(
    appearance: &ainxt_pager::appearance::AppearanceConfig,
) -> PromptStyle {
    PromptStyle {
        focused: true,
...
pub fn draw_live(app: &mut AppView, terminal: &mut PagerTerminal) {
    ...
    ainxt_pager::render::draw::draw_frame(terminal, cursor, |frame, _link_spans| {
        let area = frame.area();
        ...
        if minimal_api::extensions_modal(agent).is_some() {
            ...
            if let Some(state) = minimal_api::extensions_modal_mut(agent) {
                ainxt_pager::views::extensions_modal::render_extensions_modal(
                    frame.buffer_mut(),
                    area,
                    state,
                    None,
        ...
        (
            ...
            result
                .post_flush_escapes
                .map(ainxt_pager::terminal::overlay::PostFlush::from),
        )
    });
}
fn live_tail_renderer<'a>(
    entry: &'a ainxt_pager::scrollback::entry::ScrollbackEntry,
    theme: &'a Theme,
    appearance: &ainxt_pager::appearance::AppearanceConfig,
    cwd: &'a std::path::Path,
    tick: u64,
...
/// subagents" cue while the agent is idle. Matches the full-TUI computation in
/// `AgentView::draw` (which minimal bypasses).
fn minimal_watchers(agent: &ainxt_pager::app::agent_view::AgentView) -> turn_status::Watchers {
    turn_status::Watchers {
        monitors: agent
            ...
            .values()
            .filter(|t| {
                t.is_monitor && t.status == ainxt_pager::app::agent::BgTaskStatus::Running
            })
            .count(),
...
fn minimal_advance_phase_timer(
    agent: &mut ainxt_pager::app::agent_view::AgentView,
) -> Option<ainxt_pager::acp::tracker::TurnActivity> {
...
fn render_minimal_status(
    buf: &mut Buffer,
    area: Rect,
    agent: &ainxt_pager::app::agent_view::AgentView,
    activity: &Option<ainxt_pager::acp::tracker::TurnActivity>,
...
fn render_idle_hint(buf: &mut Buffer, area: Rect, theme: &Theme) {
    ...
    buf.set_style(area, style);
    let auto = ainxt_pager::app::minimal_auto_set_for_mouse_leak();
    let switch_back = ainxt_pager::app::minimal_show_switch_back_to_fullscreen();
    let hint = match (auto, switch_back) {
        (true, true) => {
            "minimal · auto-set on JetBrains due to mouse reporting issues \
             · /fullscreen to go back · /help"
        }
        (true, false) => {
            "minimal · auto-set on JetBrains due to mouse reporting issues · /help"
        }
...
/// the interject chord — Apple Terminal). Matches the regular TUI's model
...
fn render_prompt_info(
    ...
    use ainxt_pager::views::context_bar::fmt_tokens;
...
/// is a silent arm (no label). Matches the full-TUI shortcuts-bar `PendingHint`,
/// which minimal does not render.
fn minimal_pending_hint(
    pending: &Option<ainxt_pager::app::app_view::PendingAction>,
) -> Option<String> {
...
mod tests {
    use super::*;
    fn agent() -> ainxt_pager::app::agent_view::AgentView {
        minimal_api::test_agent_view(Some("s1"), std::path::PathBuf::from("/tmp"))
    }
    #[test]
    fn tail_height_uses_owning_session_cwd_for_tool_paths() {
        use ainxt_pager::app::agent::AgentState;
        use ainxt_pager::scrollback::RenderBlock;
        use ainxt_pager::scrollback::entry::ScrollbackEntry;
        use ainxt_pager::scrollback::types::DisplayMode;
        ...
        let appearance = super::super::commit::committed_appearance(
            &ainxt_pager::appearance::AppearanceConfig::default(),
        );
        let theme = Theme::current();
    ...
    fn minimal_status_shows_rich_activity_and_idle_hint() {
        use ainxt_pager::acp::tracker::TurnActivity;
        use ainxt_pager::app::agent::AgentState;
        let theme = Theme::current();
        ...
        ainxt_pager::app::set_minimal_show_switch_back_to_fullscreen_for_test(false);
        let a = agent();
        ...
        ainxt_pager::app::set_minimal_show_switch_back_to_fullscreen_for_test(true);
        let mut buf = Buffer::empty(area);
        render_minimal_status(&mut buf, area, &a, &None, None, &theme);
    ...
    fn minimal_status_shows_idle_watching_cue() {
        ...
        a.session.scheduled_tasks.insert(
            "loop-1".to_string(),
            ainxt_pager::app::agent::ScheduledTaskInfo {
    ...
    fn prompt_info_renders_model_context_and_queued() {
        let mut a = agent();
        a.context_state = Some(ainxt_shell::session::ContextInfo {
            used: 276_000,
            total: 2_000_000,
            ..Default::default()
        });
    ...
    fn prompt_info_shows_session_mode_flag() {
        ...
        let render = |a: &ainxt_pager::app::agent_view::AgentView| -> String {
            let mut buf = Buffer::empty(area);
    ...
    fn pending_hint_formats_press_again() {
        ...
        use ainxt_pager::app::app_view::PendingAction;
        use ainxt_pager::input::key::KeyShortcut;
