//! Minimal (scrollback-native) render mode — `ainxt --minimal`.
//!
//! In this mode finalized conversation blocks are printed once into the
//! terminal's *native* scrollback (via `ainxt_ratatui_inline::Terminal::insert_before`,
...
//! # Wiring
//!
//! `ainxt-pager` (the lib) does **not** depend on this crate — that would be
...
//! seam ([`ainxt_pager::minimal_hook`]) of function pointers, and the
//! composition-root binary (`ainxt-pager-bin`) calls [`install`] once at
...
/// Register the minimal-mode render hooks with `ainxt-pager`.
///
...
/// subsequent calls are ignored (see [`ainxt_pager::minimal_hook`]).
pub fn install() {
    ainxt_pager::minimal_hook::install(ainxt_pager::minimal_hook::MinimalHooks { draw });
}
