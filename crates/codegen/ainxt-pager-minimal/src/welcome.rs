...
//! cwd, the model, and a one-line hint. It matches the full-TUI hero box's style
//! (rounded dim border + logo) without its menu/onboarding.
//!
//! It is printed via [`ainxt_ratatui_inline::Terminal::insert_before`] — the same
...
pub fn maybe_commit_welcome(app: &mut AppView, terminal: &mut PagerTerminal) {
    ...
    let border_color =
        ainxt_pager::render::color::blend_color(theme.bg_base, theme.gray_dim, 0.45)
