...
use ainxt_pager::app::app_view::AppView;
...
pub fn pump_transcript(app: &mut AppView) {
    ...
    let appearance = super::commit::committed_appearance(&app.appearance);
    {
        ...
        let prev_thinking = ainxt_pager::appearance::cache::load_show_thinking_blocks();
        ...
        ainxt_pager::appearance::cache::set_show_thinking_blocks(prev_thinking);
    }
...
fn finish_transcript(app: &mut AppView, id: ainxt_pager::app::agent::AgentId, out: String) {
    if out.is_empty() {
        if let Some(agent) = app.agents.get_mut(&id) {
            agent
                .scrollback
                .push_block(ainxt_pager::scrollback::block::RenderBlock::system(
                    "No conversation transcript to view yet",
                ));
        }
        return;
    }
    // Written under the per-user ainxt home rather than the shared, world-
    // readable OS temp directory, since a conversation transcript can
    // contain user/session content.
    let transcript_dir = ainxt_shell::util::ainxt_home::ainxt_home().join("transcripts");
    let _ = std::fs::create_dir_all(&transcript_dir);
    let path = transcript_dir.join(format!("ainxt-transcript-{}.ansi", uuid::Uuid::new_v4()));
        ...
        Err(e) => {
            if let Some(agent) = app.agents.get_mut(&id) {
                agent.scrollback.push_block(
                    ainxt_pager::scrollback::block::RenderBlock::system(format!(
...
mod tests {
    use super::*;
    use ainxt_pager::scrollback::block::RenderBlock;
