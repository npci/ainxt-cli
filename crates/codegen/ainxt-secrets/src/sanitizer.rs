...
/// Vendor API keys with `sk-`/`sk_` prefixes and ainxt (`ainxt-`) keys. `\b`-anchored so
/// `task-`/`disk-`/`risk-` don't fold a stray `sk-`.
static API_KEY_PREFIX_REGEX: LazyLock<Regex> =
    LazyLock::new(|| compile(r"\b(?:sk[-_]|ainxt-)[A-Za-z0-9_-]{20,}"));
...
mod tests {
    ...
    fn no_match_returns_borrowed() {
        ...
        assert!(matches!(redact_secrets("model=ainxt-3"), Cow::Borrowed(_)));
    }
    ...
    fn redacts_known_secret_shapes() {
        let cases = [
            (
                fixture(&["key: ainxt-", "abc123XYZdef456GHIjkl789"]),
                "ainxt api key",
            ),
            (
    ...
    fn redacts_bare_jwt_leaving_no_token() {
        let jwt = test_secret_from_env("TEST_BARE_JWT_SECRET");
        let out = redact_secrets(&format!("deployment key {jwt}"));
        assert!(!out.contains("eyJ"), "bare JWT survived redaction: {out}");
    }

    #[test]
    fn redacts_sensitive_url_query_params() {
        let out = redact_secrets("callback https://ainxt.example.test/cb?code=ABC123XYZ&state=xyz789 failed");
    ...
    fn url_regex_excludes_trailing_punctuation() {
        let out = redact_secrets("see `https://ainxt.example.test/cb?code=ABCD12345`");
