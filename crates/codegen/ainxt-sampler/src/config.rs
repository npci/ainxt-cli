...
//! `ainxt_sampling_types::SamplingConfig` so that the sampler crate
...
/// composing chat-state's `ainxt_sampling_types::SamplingConfig`
/// with `Credentials` (api key, client version).
///
/// URL-derived request headers (e.g. `X-Ainxt-Token-Auth` for the
/// cli-chat-proxy) are
...
pub struct SamplerConfig {
    ...
    /// True when `api_backend` came from a model's own explicit
    /// `[model.<id>].api_backend` config (a direct/BYOK endpoint), as
    /// opposed to a bundled default or gateway-catalog value whose
    /// `api_backend` may not describe the gateway's own transport at all.
    /// `Client::api_backend` only trusts `api_backend` over the Messages
    /// default when this is true — see its doc comment for why.
    #[serde(default)]
    pub api_backend_explicit: bool,
    /// Which optional Messages features this model's endpoint accepts. Injected
    /// onto every `ConversationRequest` the client builds, so a self-hosted
    /// endpoint is never sent an extension it cannot parse. Defaults to fully
    /// capable. See [`ainxt_sampling_types::ModelCapabilities`].
    #[serde(default)]
    pub capabilities: ainxt_sampling_types::ModelCapabilities,
    #[serde(default)]
    pub auth_scheme: AuthScheme,
    /// Extra request headers applied unchanged. The sampler never inspects
    ...
    /// client itself sends the opt-in `x-ainxt-doom-loop-check` header on
