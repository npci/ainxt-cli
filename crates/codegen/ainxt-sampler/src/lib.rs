//! ainxt-sampler - Actor-based sampling layer for ainxt ainxt.
//!
//! This crate extracts the HTTP streaming + retry logic out of
//! `ainxt-shell`'s session actor into a standalone, reusable
//! component built on the same actor pattern as `ainxt-hunk-tracker`.
//!
//! ## Layered API
//!
...
pub use client::{
    ApiBackend, SamplingClient, effective_api_backend, parse_api_backend, set_forced_api_backend,
