...
pub fn stream_responses<'a>(
    ...
    async_stream::stream! {
        ...
        let usage = response.usage.as_ref().map(|u| TokenUsage {
            ...
            // The Responses API reports no cache-write bucket.
        ...
        let mut items = ainxt_sampling_types::response_to_conversation_items(response);
        ainxt_sampling_types::inject_streaming_reasoning_fallback(&mut items, reasoning_acc);
...
mod tests {
    ...
    async fn doom_loop_collector_signals_land_on_completed_response() {
        use ainxt_sampling_types::doom_loop::{
