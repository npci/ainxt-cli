//! HTTP client for the ainxt sampling APIs.
//!
...
use ainxt_sampling_types::error::{parse_error_bytes, try_parse_stream_error};
...
pub use ainxt_sampling_types::ApiBackend;

/// Process-wide forced API backend, resolved once at startup from the layered
/// config (CLI flag `--api-backend` > env `AINXT_API_BACKEND` > config
/// `[models] api_backend`). `None` when none of those were set — callers must
/// not install a default here; see [`effective_api_backend`]. Set via
/// [`set_forced_api_backend`].
static FORCED_API_BACKEND: std::sync::OnceLock<ApiBackend> = std::sync::OnceLock::new();

/// Install the process-wide forced API backend. Call once at startup, and only
/// when the flag/env/config layers actually resolved to a value — an absent
/// override must leave this unset so [`effective_api_backend`] can fall back
/// to each model's own configured backend instead of a hardcoded default.
/// Later calls are ignored (first write wins).
pub fn set_forced_api_backend(backend: ApiBackend) {
    let _ = FORCED_API_BACKEND.set(backend);
}

/// Parse a backend name (case-insensitive) from a flag/config/env value.
pub fn parse_api_backend(s: &str) -> Option<ApiBackend> {
...
/// The API backend explicitly forced for the whole process, if any.
///
/// The AiNxt gateway exposes a single Anthropic-Messages-compatible router
/// (`POST /messages`) for all models and no longer serves `/chat/completions`,
/// so operators can force every dispatch to speak Messages regardless of a
/// model's own configured backend via `--api-backend` / `AINXT_API_BACKEND` /
/// `[models] api_backend`.
///
/// Returns `None` when nothing was explicitly forced — callers (see
/// [`Client::api_backend`]) must then use each model's own configured
/// backend, NOT a hardcoded default. Forcing a single global default here
/// previously broke direct/BYOK models (e.g. a local Ollama server
/// configured with `api_backend = "chat_completions"` in `[model.<id>]`):
/// every request was silently built and parsed as Anthropic Messages
/// regardless, which fails as soon as the model streams anything
/// Messages-specific that a non-Anthropic endpoint doesn't produce (e.g. an
/// unsigned "thinking" block).
pub fn effective_api_backend() -> Option<ApiBackend> {
    if let Some(backend) = FORCED_API_BACKEND.get() {
        return Some(backend.clone());
    }
    std::env::var("AINXT_API_BACKEND")
        .ok()
        .as_deref()
        .and_then(parse_api_backend)
}

/// Process-level fallback for the `x-ainxt-client-identifier` header.
const DEFAULT_CLIENT_IDENTIFIER: &str = "ainxt-shell";

/// Product identifier baked into User-Agent strings.
const AGENT_PRODUCT: &str = "ainxt-shell";
const ANTHROPIC_DEFAULT_MAX_TOKENS: u32 = 128_000;

/// Per-request `x-ainxt-*` headers. Optional fields are skipped when empty/`None`.
struct AinxtRequestHeaders<'a> {
    conv_id: &'a str,
    req_id: &'a str,
    model_id: &'a str,
    session_id: &'a str,
...
impl AinxtRequestHeaders<'_> {
    fn apply(&self, builder: reqwest::RequestBuilder) -> reqwest::RequestBuilder {
        let mut b = builder
            .header("x-ainxt-conv-id", self.conv_id)
            .header("x-ainxt-req-id", self.req_id)
            .header("x-ainxt-model-override", self.model_id)
            .header("x-ainxt-session-id", self.session_id)
            .header("x-ainxt-agent-id", self.agent_id);
        if let Some(idx) = self.turn_idx {
            b = b.header("x-ainxt-turn-idx", idx);
        }
        if let Some(id) = self.deployment_id.filter(|s| !s.is_empty()) {
            b = b.header("x-ainxt-deployment-id", id);
        }
        if let Some(id) = self.user_id.filter(|s| !s.is_empty()) {
            b = b.header("x-ainxt-user-id", id);
        }
        b
    }
}
...
/// Deserialize a Responses API SSE event, with a fallback for ainxt-specific
...
/// when the API emits the ainxt-specific `context_details` field.
    ...
    let mut event = match serde_json::from_str::<rs::ResponseStreamEvent>(data) {
        ...
        Err(first_err) => {
            ...
            if let Ok(mut value) = serde_json::from_str::<serde_json::Value>(data) {
                ...
                // (e.g., ainxt-specific "x_search"). Instead of maintaining a
...
fn apply_terminal_event_overrides(event: &mut rs::ResponseStreamEvent, data: &str) {
    ...
    if let Some(ticks) = ainxt_sampling_types::reported_cost_ticks(
        value
...
pub(crate) const COST_USD_TICKS_METADATA_KEY: &str = "ainxt.cost_usd_ticks";
...
fn extract_model_metadata(headers: &reqwest::header::HeaderMap) -> Option<ResponseModelMetadata> {
    let context_window = headers
        .get("x-ainxt-context-window")
        .and_then(|v| v.to_str().ok())
        .and_then(|s| s.parse::<u64>().ok());

    let max_completion_tokens = headers
        .get("x-ainxt-max-completion-tokens")
        .and_then(|v| v.to_str().ok())
...
struct ClientDefaults {
    ...
    /// The model's context window, used only to clamp the Messages
    /// `max_tokens` fallback — see `default_messages_max_tokens`.
    context_window: u64,
    ...
    capabilities: ainxt_sampling_types::ModelCapabilities,
    stream_tool_calls: bool,
    doom_loop_recovery: Option<ainxt_sampling_types::DoomLoopRecoveryPolicy>,
}
...
fn agent_version() -> String {
    ainxt_version::VERSION.to_string()
}

/// Render a User-Agent string for the given origin client.
///
/// Matches the shell's `user_agent_string_for` but uses sampler-local
...
impl SamplingClient {
    ...
    pub fn new(config: SamplerConfig) -> Result<Self> {
                    ...
                    let header_value = HeaderValue::from_str(api_key).map_err(|_| {
                        tracing::debug!(
                            "Invalid api_key: cannot be converted to a valid HTTP header \
                             (key redacted — check for non-ASCII or control characters)"
                        );
                        SamplingError::Auth(
                            "Invalid api_key: cannot be converted to a valid HTTP header"
                                .to_string(),
        ...
        // Apply all extra headers unchanged. This is the single
        ...
        // Add x-ainxt-client-version header for version gating at the proxy.
        if let Some(client_version) = config.client_version.as_ref()
            && let Ok(header_value) = HeaderValue::from_str(client_version)
        {
            headers.insert(
                HeaderName::from_static("x-ainxt-client-version"),
                header_value,
            );
        }
        ...
        {
            headers.insert(
                HeaderName::from_static("x-ainxt-deployment-id"),
                header_value,
            );
        }
        ...
        {
            headers.insert(HeaderName::from_static("x-ainxt-user-id"), header_value);
        }

        {
            ...
            if let Ok(header_value) = HeaderValue::from_str(&client_id) {
                headers.insert(
                    HeaderName::from_static("x-ainxt-client-identifier"),
                    header_value,
                );
            }
        }
        ...
        // INV-1 (gateway allowlist): under an installed enforcing policy, the
        // configured gateway host must be on the egress allowlist. This is the
        // userspace half of INV-1 — it stops the CLI's own model traffic from
        // being pointed at a non-governed endpoint via config (`/gateway`,
        // `AINXT_GATEWAY_URL`). The `Operator` tier makes this an allowlist-only
        // check: the gateway call must be permitted regardless of context tier,
        // because the gateway itself performs content governance. Guarded by
        // `is_installed()` so unwired binaries and tests are unaffected.
        if ainxt_policy::global::is_installed() {
            let engine = ainxt_policy::global::active();
            let decision = ainxt_policy::egress_guard::check_url(
                &engine,
                &config.base_url,
                ainxt_policy::types::tier::TrustTier::Operator,
            );
            if engine.should_block(decision.verdict) {
                tracing::error!(
                    target: crate::sampling_log::TARGET,
                    event = "gateway_egress_denied",
                    base_url = %config.base_url,
                    rule = decision.rule.as_ref().map(|r| r.to_string()).unwrap_or_default(),
                    reason = %decision.reason,
                );
                return Err(SamplingError::InvalidConfiguration(
                    "configured gateway host is not on the egress allowlist (INV-1)",
                ));
            }
        }
        ...
        let defaults = ClientDefaults {
            ...
            top_p: config.top_p,
            api_backend: config.api_backend,
            api_backend_explicit: config.api_backend_explicit,
            auth_scheme: config.auth_scheme,
    ...
    /// The API backend used for dispatch:
    ///
    /// 1. An explicitly forced backend (see [`effective_api_backend`]) wins
    ///    for every model when set.
    /// 2. Otherwise, if this model's `api_backend` came from its own
    ///    explicit `[model.<id>].api_backend` config
    ///    (`self.defaults.api_backend_explicit`) — a direct/BYOK endpoint,
    ///    e.g. a local Ollama server — that value is used. This is what
    ///    lets switching to a model with a different `api_backend`
    ///    mid-session dispatch correctly.
    /// 3. Otherwise (bundled defaults, gateway-catalog entries — whose
    ///    `api_backend` metadata may not describe the gateway's own
    ///    transport at all), fall back to Messages, matching the gateway's
    ///    single Messages-compatible router.
    pub fn api_backend(&self) -> ApiBackend {
        effective_api_backend().unwrap_or_else(|| {
            if self.defaults.api_backend_explicit {
    ...
    fn format_request_headers(
        ...
        req_headers.push(Self::format_header("x-ainxt-conv-id", x_ainxt_conv_id));
        req_headers.push(Self::format_header("x-ainxt-req-id", x_ainxt_req_id));
        req_headers.push(Self::format_header("x-ainxt-model-override", model_id));
        if include_accept {
    ...
    /// Record the exact bytes of an outgoing request to the sampling log.
    ///
    /// Reads the body off the built `reqwest::Request`, so what lands in the
    /// log is what goes on the socket — not a re-serialization that could
    /// drift from it. Streaming bodies have no in-memory bytes; those are
    /// skipped rather than guessed at.
    fn log_request_body(&self, request: &reqwest::Request, endpoint_name: &str) {
        if let Some(bytes) = request.body().and_then(|b| b.as_bytes()) {
            crate::sampling_log::log_request_body(endpoint_name, &self.defaults.model, bytes);
        }
    }
    ...
    fn build_api_error_message(
        ...
        // Every non-success path funnels through here, so this is the one place
        // the rejection body has to be recorded to sit next to the request that
        // caused it in the sampling log.
        crate::sampling_log::log_error_response(
            endpoint,
            &self.defaults.model,
            status.as_u16(),
            server_message,
    ...
    pub async fn chat_completion(
        ...
        let x_ainxt_conv_id = &payload.x_ainxt_conv_id.clone().unwrap_or_default();
        let x_ainxt_req_id = &payload.x_ainxt_req_id.clone().unwrap_or_default();
        ...
        let ainxt_headers = AinxtRequestHeaders {
            conv_id: x_ainxt_conv_id,
            req_id: x_ainxt_req_id,
            model_id: &model_id,
            session_id: payload.x_ainxt_session_id.as_deref().unwrap_or_default(),
            turn_idx: payload.x_ainxt_turn_idx.as_deref(),
            agent_id: payload.x_ainxt_agent_id.as_deref().unwrap_or_default(),
            deployment_id: payload.x_ainxt_deployment_id.as_deref(),
            user_id: payload.x_ainxt_user_id.as_deref(),
        };
        let http_request = ainxt_headers
    ...
    pub async fn chat_completion_stream(
        ...
        self.log_request_body(&built_request, "chat/completions");
        ...
        if !status.is_success() {
            ...
            let req_headers =
                self.format_request_headers(x_ainxt_conv_id, x_ainxt_req_id, &model_id, true);
    ...
    pub async fn create_response(
        ...
        let x_ainxt_conv_id = request.x_ainxt_conv_id.as_deref().unwrap_or_default();
        let x_ainxt_req_id = request.x_ainxt_req_id.as_deref().unwrap_or_default();
        ...
        let ainxt_headers = AinxtRequestHeaders {
            ...
            req_id: x_ainxt_req_id,
            model_id: &model_id,
            session_id: request.x_ainxt_session_id.as_deref().unwrap_or_default(),
            turn_idx: request.x_ainxt_turn_idx.as_deref(),
            agent_id: request.x_ainxt_agent_id.as_deref().unwrap_or_default(),
            deployment_id: request.x_ainxt_deployment_id.as_deref(),
            user_id: request.x_ainxt_user_id.as_deref(),
        };
        ...
        ainxt_sampling_types::patch_reasoning_text_types(&mut request_body);
        let http_request = ainxt_headers
        ...
        if !status.is_success() {
            ...
            let req_headers =
                self.format_request_headers(x_ainxt_conv_id, x_ainxt_req_id, &model_id, false);
            let server_message = parse_error_bytes(bytes.as_ref());
    ...
    /// gate that adds the opt-in `x-ainxt-doom-loop-check` request header, so
    ...
    pub async fn create_response_stream(
        ...
        // Inject ainxt-specific fields not in async-openai's CreateResponse type.
        if self.defaults.stream_tool_calls {
            request_body["stream_tool_calls"] = serde_json::json!(true);
        }
        // Inject ainxt-specific tools (e.g., x_search) that can't be expressed
        ...
        let mut http_request = ainxt_headers
            .apply(self.post(self.endpoint("responses")))
            .header(ACCEPT, HeaderValue::from_static("text/event-stream"));
        ...
        self.log_request_body(&built_request, "responses");
    ...
    /// The `max_tokens` to send when neither the request nor
    /// `max_completion_tokens` supplies one.
    ///
    /// The Messages wire format requires `max_tokens`, so unlike Chat
    /// Completions (where the field is simply omitted) there has to be a
    /// fallback. It is clamped to the model's context window: a self-hosted
    /// endpoint (vLLM/SGLang/Ollama behind a Messages shim) rejects the whole
    /// request with a 400 when `max_tokens` alone exceeds the model's context,
    /// so an unclamped 128k default makes every request fail for any model
    /// with a smaller window.
    fn default_messages_max_tokens(&self) -> u32 {
        let context_window = u32::try_from(self.defaults.context_window).unwrap_or(u32::MAX);
        let clamped = ANTHROPIC_DEFAULT_MAX_TOKENS.min(context_window);
        if clamped < ANTHROPIC_DEFAULT_MAX_TOKENS {
            tracing::debug!(
                model = %self.defaults.model,
                context_window = self.defaults.context_window,
                max_tokens = clamped,
                "clamped default max_tokens to the model's context window; set \
                 `max_completion_tokens` on the model to control this explicitly"
            );
        }
        clamped
    }
    ...
    fn apply_message_defaults(&self, request: &mut MessagesRequestWrapper) -> Result<()> {
        ...
        if request.inner.max_tokens == 0 {
            request.inner.max_tokens = self
                .defaults
                .max_completion_tokens
                .unwrap_or_else(|| self.default_messages_max_tokens());
        }
    ...
    pub async fn create_message_stream(
        ...
        self.log_request_body(&built_request, "messages");
    ...
    fn apply_conversation_defaults(&self, request: &mut ConversationRequest) -> Result<()> {
        ...
        // The model's capability profile always comes from the client's config:
        // callers construct `ConversationRequest`s without knowing which model
        // will serve them, so a request-level value would only ever be the
        // permissive default. Every dispatch path funnels through here, so this
        // is the one place the profile has to be applied.
        request.capabilities = self.defaults.capabilities.clone();
    ...
    pub async fn conversation_stream_responses(
        ...
        let trace = request.trace.take();
        let x_ainxt_conv_id = request.x_ainxt_conv_id.clone();
        let x_ainxt_req_id = request.x_ainxt_req_id.clone();
        let x_ainxt_session_id = request.x_ainxt_session_id.clone();
        let x_ainxt_turn_idx = request.x_ainxt_turn_idx.clone();
        let x_ainxt_agent_id = request.x_ainxt_agent_id.clone();

        // Collect ainxt-specific tools that can't be expressed via rs::Tool
        // (e.g., x_search). These are injected as raw JSON after serialization.
        let extra_tools = ainxt_sampling_types::extra_raw_tools(&request.hosted_tools);

        let responses_request: rs::CreateResponse = (&request).into();

        let mut wrapper = CreateResponseWrapper::new(responses_request);
        wrapper.x_ainxt_conv_id = x_ainxt_conv_id;
        wrapper.x_ainxt_req_id = x_ainxt_req_id;
        wrapper.x_ainxt_session_id = x_ainxt_session_id;
        wrapper.x_ainxt_turn_idx = x_ainxt_turn_idx;
        wrapper.x_ainxt_agent_id = x_ainxt_agent_id;
...
mod tests {
    use super::*;
    use ainxt_sampling_types::types::ChatRequestMessage;
    ...
    fn minimal_config() -> SamplerConfig {
        SamplerConfig {
            ...
            capabilities: ainxt_sampling_types::ModelCapabilities::default(),
    ...
    fn streaming_chat_request_serializes_correctly() {
        let request = ChatCompletionRequest {
            ...
            reasoning_effort: None,
            x_ainxt_conv_id: None,
            x_ainxt_req_id: None,
            x_ainxt_session_id: None,
            x_ainxt_turn_idx: None,
            x_ainxt_agent_id: None,
            x_ainxt_deployment_id: None,
            x_ainxt_user_id: None,
            trace: None,
        };
    ...
    fn new_with_minimal_config_succeeds() {
        ...
        // `minimal_config()` doesn't mark its `api_backend` as explicit
        // (matching a bundled/gateway-catalog model), so dispatch is forced
        // to Messages (only `/messages`) unless `AINXT_API_BACKEND`
        // overrides it — see `effective_api_backend`.
        assert_eq!(client.api_backend(), ApiBackend::Messages);
    }

    #[test]
    fn api_backend_respects_explicit_per_model_config() {
        // A direct/BYOK model (e.g. a local Ollama server) that explicitly
        // set `api_backend = "chat_completions"` in its `[model.<id>]` entry
        // must actually dispatch as chat_completions, not get silently
        // forced to Messages the way bundled/gateway models are.
        let config = SamplerConfig {
            api_backend: ApiBackend::ChatCompletions,
            api_backend_explicit: true,
            ..minimal_config()
        };
    ...
    #[test]
    fn parse_api_backend_accepts_documented_spellings() {
        assert_eq!(parse_api_backend("messages"), Some(ApiBackend::Messages));
        assert_eq!(
            parse_api_backend("chat_completions"),
            Some(ApiBackend::ChatCompletions)
        );
        assert_eq!(
            parse_api_backend(" Chat-Completions "),
            Some(ApiBackend::ChatCompletions)
        );
        assert_eq!(parse_api_backend("responses"), Some(ApiBackend::Responses));
        assert_eq!(parse_api_backend("grpc"), None);
    }
    ...
    fn new_applies_extra_headers() {
        ...
        cfg.extra_headers
            .insert("x-Ainxt-token-auth".to_string(), "ainxt-cli".to_string());
    ...
    fn user_agent_omits_origin_version_when_absent() {
        ...
        let ua = user_agent_string_for(&origin);
        // No slash between product and the ainxt-shell agent product.
        assert!(ua.starts_with("my-client ainxt-shell/"));
    }

    #[test]
    fn user_agent_collapses_when_origin_matches_agent() {
        let agent_version = ainxt_version::VERSION.to_string();
        let origin = OriginClientInfo {
            product: AGENT_PRODUCT.to_string(),
        ...
        let make = |ticks: i64| {
            format!(
                ...
                "response": {{
                    ...
                    "model": "ainxt-build", "status": "completed", "output": [],
                    "usage": {{
                        "input_tokens": 10,
