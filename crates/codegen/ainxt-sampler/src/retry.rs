...
//!
//! Today CCP's header matches the client's `is_retryable()` logic
...
use ainxt_sampling_types::SamplingError;
...
/// Resolve max API retries: `AINXT_MAX_RETRIES` env > model config > default ([`DEFAULT_MAX_RETRIES`]).
pub fn resolve_max_retries(model_max_retries: Option<u32>) -> u32 {
    let env_override = std::env::var("AINXT_MAX_RETRIES").ok();
