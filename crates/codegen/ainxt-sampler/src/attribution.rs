...
//! it" buckets.
//!
//! `ainxt-sampler` is intentionally decoupled from `ainxt-shell`
...
/// tools live in `ainxt-tools`
/// (`crates/codegen/ainxt-tools/src/implementations/`), have their
/// own HTTP clients that do not flow through `SamplingClient`, and
/// hook into the `ainxt_tools::ApiKeyProvider` trait rather than
/// this enum.
...
/// callbacks across the crate boundary. Matches
/// `ainxt_shell::auth::token_suffix` (which truncates to 12 chars
...
/// `ainxt-shell/src/auth/manager.rs` to match, otherwise the
