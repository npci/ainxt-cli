...
//! payloads come from `ainxt_test_support::sse`.
...
use ainxt_test_support::{SseEvent, sse};
...
fn test_config(base_url: String, model: &str) -> SamplerConfig {
    SamplerConfig {
        ...
        // above entirely and always falls back to Messages (see its doc
        // comment), so every request in this file 404s against the mock
        // server's registered /v1/chat/completions (or /v1/messages,
        // /v1/responses for the tests that override `cfg.api_backend`
        // below) route. `api_backend_explicit: true` makes the configured
        // backend actually take effect.
    ...
    match events.last().unwrap() {
        SamplingEvent::Completed { response, .. } => {
            assert_eq!(
                ...
                Some(ainxt_sampling_types::StopReason::ContentFilter)
            );
        }
        other => panic!("expected Completed, got {other:?}"),
    }
