...
fn profile_from_env() -> ainxt_sandbox::ProfileName {
    ainxt_sandbox::ProfileName::Custom(std::env::var(PROFILE_ENV).expect(PROFILE_ENV))
}
...
fn subprocess_entry() {
    ...
    #[cfg(target_os = "linux")]
    {
        if !ainxt_sandbox::is_inside_bwrap() {
            ...
            match ainxt_sandbox::bwrap_reexec_for_profile(&profile_from_env(), workspace) {
                Some(mut cmd) => {
    ...
    match scenario.as_str() {
        "block_deny" => {
            let mut sandbox = ainxt_sandbox::SandboxManager::new(profile_from_env(), workspace);
...
fn run_deny_case(
    ...
    let support = ainxt_sandbox::SandboxManager::support_info();
    ...
    let deny_list = deny_entries
        ...
        .collect::<Vec<_>>()
        .join(", ");
    fs::create_dir_all(tmp.join(".ainxt")).expect("mkdir .ainxt");
    fs::write(
        tmp.join(".ainxt").join("sandbox.toml"),
