...
//! (best-effort).
//!
//! Equivalence invariant: `validate_deny_glob` accepts/rejects identically on both
...
mod tests {
    ...
    mod linux_expand {
        ...
        fn tmp_tree(tag: &str) -> PathBuf {
            // `tempfile::Builder` (a private directory under the OS temp
            // root, not a hand-built path directly under the shared temp
            // directory) as the parent for this named subdirectory; the
            // `TmpTree` guard below still owns the final removal.
