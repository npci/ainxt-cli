...
//! OS-level sandboxing for ainxt via [nono](https://crates.io/crates/nono).
//!
...
//!
//! ```rust,no_run
//! use ainxt_sandbox::{SandboxManager, ProfileName};
//! use std::path::Path;
//!
...
const BWRAP_ENV_VAR: &str = "__AINXT_INSIDE_BWRAP";
...
/// Zero-permission placeholder (file or dir) under `ainxt_home` used by bwrap bind-over.
///
/// The placeholder name is suffixed with the current PID so concurrent ainxt
...
fn bwrap_blocked_placeholder(name: &str, want_dir: bool) -> Option<PathBuf> {
    ...
    let path = paths::ainxt_home().join(format!("{name}.{}", std::process::id()));
...
mod tests {
    ...
    /// Create a temp workspace whose `.ainxt/sandbox.toml` contains `toml_body`.
    /// Returns the workspace path (caller removes it).
    #[cfg(all(feature = "enforce", unix))]
    fn temp_workspace_with_sandbox_toml(tag: &str, toml_body: &str) -> PathBuf {
        // `tempfile::Builder` (a private directory under the OS temp root,
        // not a hand-built path directly under the shared temp directory)
        // as the parent for this named workspace; callers' existing
        // `remove_dir_all` cleanup still applies to the returned path.
        let ws = tempfile::Builder::new()
            .prefix(&format!("ainxt-{tag}-"))
            .tempdir()
            .unwrap()
            .keep();
        let ainxt = ws.join(".ainxt");
        std::fs::create_dir_all(&ainxt).unwrap();
        std::fs::write(ainxt.join("sandbox.toml"), toml_body).unwrap();
        ws
    }
    ...
    fn bwrap_reexec_uses_dir_placeholder_for_directories() {
        let _g = EnvGuard::remove(BWRAP_ENV_VAR);
        let dir = std::env::temp_dir().join(format!("ainxt-deny-dir-{}", std::process::id()));
        ...
        let args: Vec<String> = cmd
            ...
            .map(|a| a.to_string_lossy().to_string())
            .collect();
        let blocked_dir = paths::ainxt_home()
