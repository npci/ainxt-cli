...
// ── ainxt state directory ────────────────────────────────────────────────────

/// ainxt state directory — always writable (`$AINXT_HOME` or `~/.ainxt`).
pub(crate) fn ainxt_home() -> PathBuf {
...
pub(crate) fn essential_writable_paths(workspace: &Path) -> Vec<PathBuf> {
    let mut paths = vec![workspace.to_path_buf(), ainxt_home()];
    paths.extend(temp_writable_paths());
    paths
}

/// Writable directory paths for the read-only profile (minimal: just ~/.ainxt + temp).
/// Device files are handled separately via `allow_file` in `to_capability_set_with_config`.
#[cfg(all(feature = "enforce", unix))]
pub(crate) fn essential_writable_paths_minimal() -> Vec<PathBuf> {
    let mut paths = vec![ainxt_home()];
    paths.extend(temp_writable_paths());
    paths
}
