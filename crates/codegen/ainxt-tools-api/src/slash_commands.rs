//! Canonical slash-command wording (`/loop`, `/image`, `/video`, `/goal`),
//! shared by every front-end (ainxt shell/pager and other hosts) so
...
/// Canonical name of the image generation tool; gates `/image`.
pub const IMAGE_GEN_TOOL_NAME: &str = "image_gen";

/// Advertised name of the /image command.
pub const IMAGINE_COMMAND_NAME: &str = "image";

/// Canonical name of the image-to-video tool; gates `/video`.
pub const IMAGE_TO_VIDEO_TOOL_NAME: &str = "image_to_video";

/// Advertised name of the /video command.
pub const IMAGINE_VIDEO_COMMAND_NAME: &str = "video";

/// Usage hint shown when `/image` is invoked with no arguments.
...
/// Usage hint shown when `/video` is invoked with no arguments.
...
/// Video workflow guidance injected by `/video`.
