...
//! `ainxt_shell::config::{SubagentRole, SubagentPersona, PersonaIOField}`.
//!
//! Methods that remain in `ainxt-shell` (on `SubagentsConfig`):
...
"#;
        ...
        assert_eq!(
            ...
            Some(".ainxt/personas/concise.md")
        );
        assert_eq!(persona.model.as_deref(), Some("ainxt-3-fast"));
