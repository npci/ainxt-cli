...
//!
//! Extracted from `ainxt-shell/src/agent/subagent/` `resolve_effective_overrides()`.

use std::collections::HashMap;
use std::path::Path;

use serde::de::DeserializeOwned;
use ainxt_tools::implementations::ainxt_build::task::types::SubagentRuntimeOverrides;
use ainxt_tool_types::{SubagentCapabilityMode, SubagentIsolationMode};
...
mod tests {
    use super::*;
    use ainxt_tools::implementations::ainxt_build::task::types::ModelOverrideProvenance;
    ...
    fn explicit_model_overrides_role() {
        let overrides = make_overrides(Some("ainxt-light"), None, None, None, None);
        let role = SubagentRole {
            model: Some("ainxt-3".into()),
            ..Default::default()
        };
        let result =
            resolve_effective_overrides(&overrides, Some(&role), &empty_personas(), None, None);
        assert_eq!(result.model.as_deref(), Some("ainxt-light"));
    }
    ...
    fn role_model_used_when_no_explicit() {
        ...
        let result =
            resolve_effective_overrides(&overrides, Some(&role), &empty_personas(), None, None);
        assert_eq!(result.model.as_deref(), Some("ainxt-3"));
    }
    ...
    fn persona_not_found_error_is_non_fatal() {
        ...
        let overrides = make_overrides(Some("ainxt-3"), Some("missing"), None, None, None);
        let role = SubagentRole {
            model: Some("ainxt-light".into()),
            default_isolation: Some("worktree".into()),
            ..Default::default()
        };
        let result =
    ...
    fn persona_file_error_returns_early_with_defaults() {
        ...
        let overrides = make_overrides(Some("ainxt-3"), Some("broken"), None, None, None);
        let dir = tempfile::tempdir().unwrap();
        let mut personas = HashMap::new();
        personas.insert(
            "broken".to_string(),
