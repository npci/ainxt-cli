...
//!
//! Extracted from `ainxt-shell/src/agent/subagent/` `normalize_forked_context()`.
...
/// Maximum number of complete turns to render unchanged in the background
...
///
/// See also: `ainxt-chat-state::compaction_utils::strip_system_tags` which
...
/// or fewer complete turns, all are included unchanged. If more, the last
/// [`MAX_VERBATIM_TURNS`] are unchanged and earlier turns are summarized.
///
...
pub fn normalize_forked_context(items: Vec<ConversationItem>) -> (Vec<ConversationItem>, usize) {
    ...
    if turns.len() <= MAX_VERBATIM_TURNS {
        // All turns fit - render unchanged.
        for item in &parent_items {
            render_item_to_background(&mut background, item);
        }
    } else {
        // Summarize early turns, keep last MAX_VERBATIM_TURNS unchanged.
...
/// `ainxt-shell/src/session/storage/jsonl.rs` (it truncates to the last
...
mod tests {
    use super::*;
    use ainxt_sampling_types::conversation::{ConversationItem, ToolCall, ToolResultItem};
    ...
    fn reasoning_item(text: &str) -> ConversationItem {
        ConversationItem::Reasoning(ainxt_sampling_types::synthesized_reasoning_item(text))
    }
    ...
    fn three_turns_all_verbatim() {
        ...
        if let ConversationItem::User(u) = &result[1] {
            let text = u
                ...
                .collect::<Vec<_>>()
                .join("");
            // All three turns should be unchanged
    ...
    fn four_turns_early_summarized_last_three_verbatim() {
        ...
        if let ConversationItem::User(u) = &result[1] {
            let text = u
                ...
                .collect::<Vec<_>>()
                .join("");
            // With 4 turns, the last 3 are unchanged and the first 1 is summarized.
            // early_end = turns[turns.len()-3] = turns[1] = end of turn 2.
            // So turns 1-2 are summarized, turns 3-4 are unchanged.
            ...
            // Last 2 turns (3, 4) should be unchanged
            assert!(text.contains("=== Recent turns (verbatim) ==="));
            assert!(text.contains("[User]: Turn 3"));
            assert!(text.contains("[User]: Turn 4"));
            // Turns 1 and 2 should NOT appear unchanged
            assert!(!text.contains("[User]: Turn 1"));
            assert!(!text.contains("[User]: Turn 2"));
        } else {
            panic!("Expected User item");
