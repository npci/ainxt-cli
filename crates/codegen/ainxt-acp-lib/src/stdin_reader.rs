...
//!
//! Every ACP client (VS Code extension, ainxt-desktop, the leader bridge) drives
...
//! observes immediate EOF instead of deadlocking on the lock. This matches what
...
///
/// Yielded lines are **not guaranteed byte-unchanged**: a line the pinned acp
...
/// [`normalize_json_line`], so bytes are unchanged except for the lines that
