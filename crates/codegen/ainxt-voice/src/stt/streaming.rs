...
/// Streaming STT over `wss://api.example.test/v1/stt`.
...
mod tests {
    ...
    fn optional_header_inserted_when_present_skipped_when_empty() {
        let mut req = "wss://api.example.test/v1/stt".into_client_request().unwrap();
        insert_optional_header(&mut req, "x-ainxt-client-identifier", "ainxt-shell");
        insert_optional_header(&mut req, "User-Agent", "");
        assert_eq!(
            req.headers().get("x-ainxt-client-identifier").unwrap(),
            "ainxt-shell"
        );
        assert!(
