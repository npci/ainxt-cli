...
//!
//! ```bash
//! export AINXT_API_KEY=...
//! cargo run -p ainxt-voice --bin voice-probe -- --seconds 5
//! ```
...
async fn main() -> anyhow::Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "info,ainxt_voice=debug".into()),
        )
        .init();

    let args = parse_args(std::env::args().skip(1).collect());

    let auth = std::env::var("AINXT_API_KEY")
        .ok()
        .and_then(StaticVoiceAuth::shared)
        .ok_or_else(|| {
            anyhow::anyhow!("set AINXT_API_KEY (standalone probe has no login session)")
        })?;
    ...
    if args.mic_only {
        ...
        {
            let (bytes, chunks) =
                ainxt_voice::run_mic_only_probe(config.sample_rate, args.seconds)?;
...
fn load_config(path: Option<&std::path::Path>) -> VoiceConfig {
    ...
    let env_base = std::env::var("AINXT_API_BASE_URL").ok();
    ...
    if let Ok(home) = std::env::var("AINXT_HOME")
...
Environment:
  AINXT_API_KEY     required
  RUST_LOG        optional (default info,ainxt_voice=debug)

Reads [voice] from ~/.ainxt/config.toml unless --config PATH is set.
"#
    );
}
