...
//!
//! ```bash
//! cargo test -p ainxt-pager-pty-harness --test prompt_history_durable_quit \
//!   -- --ignored --nocapture
//! ```
...
use ainxt_pager_pty_harness::{ContentController, PtyHarness, keys, pager_binary};
...
/// The per-CWD history file lives at `<home>/.ainxt/sessions/<enc-cwd>/prompt_history.jsonl`.
fn find_prompt_history(home: &Path) -> Result<PathBuf> {
    let root = home.join(".ainxt").join("sessions");
