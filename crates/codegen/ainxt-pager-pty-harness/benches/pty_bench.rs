//! `pty-bench` — PTY benchmark CLI for `ainxt-pager`.
//!
...
//! ```bash
//! cargo bench -p ainxt-pager-pty-harness \
...
//! ```bash
//! PAGER_BINARY=./artifacts/ainxt-${VERSION}-linux-x86_64 \
//!   cargo bench -p ainxt-pager-pty-harness \
//!   --bench pty_bench -- --all \
...
#[command(
    name = "pty-bench",
    about = "PTY benchmark harness for ainxt-pager",
    long_about = None,
)]
struct Cli {
