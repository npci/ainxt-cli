//! Unified PTY harness for ainxt-pager.
//!
...
//!    `ainxt-pager/tests/scenarios/`) — assert screen contents and
...
//! - **`scroll_matrix`**  — `AINXT_SCROLL_LOG` JSONL ingestion for the scroll validation matrix.
...
impl PtyHarness {
    ...
    /// # use ainxt_pager_pty_harness::{PtyHarness, ContentController, pager_binary};
    ...
    /// through ainxt's stdout. Used to reproduce the doubled-line class of bugs
    /// where ainxt's diff renderer never re-asserts a region it didn't write
