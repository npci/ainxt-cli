//! `AINXT_SCROLL_LOG` JSONL parsing, per-stream grouping, and finalize
//! synchronization.
//!
//! Wire schema source of truth: the pager's `ScrollLogRecord` in
//! `ainxt-pager/src/input/scroll_log.rs`. [`ScrollLogLine`] matches it
...
/// Parse a `AINXT_SCROLL_LOG` JSONL file into records.
///
...
/// session (`AINXT_SCROLL_LOG` set at spawn): `stream_start` → `flush`* →
...
mod tests {
    ...
    // in `ainxt-pager/src/input/mouse/tests.rs`.
