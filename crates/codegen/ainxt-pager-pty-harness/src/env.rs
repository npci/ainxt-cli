...
fn local_pager_binary_path() -> Result<PathBuf> {
    Ok(target_dir()?
        .join("debug")
        .join(format!("ainxt-pager{}", std::env::consts::EXE_SUFFIX)))
}
...
fn ensure_local_pager_binary(binary: &std::path::Path) -> Result<()> {
    ...
    let output = cmd
        .output()
        .with_context(|| format!("failed to spawn {cargo} to build ainxt-pager"))?;

    if !output.status.success() {
        bail!(
            "failed to build ainxt-pager (exit {:?})\nstdout:\n{}\nstderr:\n{}",
            output.status.code(),
    ...
    if !binary.exists() {
        bail!(
            "ainxt-pager build completed but binary missing at {}",
            binary.display()
        );
    }
    Ok(())
...
/// 2. `CARGO_BIN_EXE_ainxt-pager` (set by `cargo test`)
/// 3. Build locally via `cargo build -p ainxt-pager-bin` (the composition-
///    root package that owns the `ainxt-pager` binary)
pub fn pager_binary() -> Result<PathBuf> {
    ...
    if let Ok(path) = std::env::var("CARGO_BIN_EXE_ainxt-pager") {
        let p = PathBuf::from(path);
        if p.exists() {
            return Ok(p);
        }
