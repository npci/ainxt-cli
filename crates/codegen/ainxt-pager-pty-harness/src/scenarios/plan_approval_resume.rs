...
//! shell re-issues the `ainxt.dev/exit_plan_mode` reverse-request — a real live ACP
...
/// snapshot if the shell wrote nothing. The shape matches
/// `ainxt_shell::session::plan_mode::PlanModeSnapshot`; we only touch the one
