...
//! elected leader) plus a fixed leader socket beneath its `AINXT_HOME`; clients
...
impl LeaderCluster {
    ...
    /// one leader) and a fixed leader socket under its `AINXT_HOME`.
    pub async fn start(rows: u16, cols: u16) -> Result<Self> {
        let content = ContentController::start()
            .await
            .context("start content controller")?;
        // One shared AINXT_HOME => one leader; the socket lives beneath it so
        // every client (sharing the same env) elects/attaches to the same one.
        let ainxt_home = content.home().join(".ainxt");
        std::fs::create_dir_all(&ainxt_home).context("create ainxt home")?;
        let socket = ainxt_home.join("leader-e2e.sock");
    ...
    /// The cluster's sessions root: `AINXT_HOME/sessions` (layout below is
    /// `sessions/<encoded-cwd>/<session-id>/updates.jsonl`).
    fn sessions_dir(&self) -> PathBuf {
        self.content.home().join(".ainxt").join("sessions")
    }
    ...
    pub fn wait_for_turn_completed(&self, timeout: Duration) -> Result<Value> {
        ...
        loop {
            ...
            // Sync FS poll matches the harness's blocking wait_for_text; a stat
...
mod tests {
    ...
    fn envelope(update_json: &str) -> String {
        format!(
            r#"{{"timestamp":1,"method":"_ainxt.dev/session/update","params":{{"sessionId":"s","update":{update_json}}}}}"#
        )
    }
    ...
    fn parse_update_payloads_unwraps_and_tolerates_torn_trailing_line() {
        let body = format!(
            "{}\n{}\n{{\"timestamp\":2,\"method\":\"_ainxt.dev/sess",
            envelope(
