//! Scripted TUI scenario runner for ainxt-pager.
//!
...
pub struct EnvironmentConfig {
    ...
    #[serde(default)]
    pub args: Vec<String>,
    /// Optional `config.toml` written into the run's isolated `$AINXT_HOME`
...
pub enum ScenarioStep {
    ...
    /// graphics APC sequences (`\x1b_G`). Matches [`AssertOsc52Contains`]: the
...
impl ScriptedScenarioRunner {
    ...
    pub async fn run(&self, scenario: &ScriptedScenario) -> Result<ScriptedRunReport> {
        ...
        if let Some(config_toml) = &scenario.environment.config_toml {
            ...
            fs::create_dir_all(&ainxt_home)
                .with_context(|| format!("create scenario AINXT_HOME {}", ainxt_home.display()))?;
            fs::write(ainxt_home.join("config.toml"), config_toml)
