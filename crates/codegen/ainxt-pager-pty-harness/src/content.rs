...
//! [`MockInferenceServer`] from `ainxt-test-support` and provides the
...
pub use ainxt_test_support::mock_server::LogEntry;
pub use ainxt_test_support::mock_server::MockModelEntry as MockModel;
pub use ainxt_test_support::mock_server::StorageUpload;
// SSE event builders for `enqueue_response` scripts (reasoning turns etc.).
pub use ainxt_test_support::sse;
pub use ainxt_test_support::{ScriptedResponse, SseEvent};
...
impl ContentController {
    ...
    pub async fn start_with_models(models: Vec<MockModel>) -> Result<Self> {
        ...
        // Pre-delegation equivalence, both load-bearing for PTY tests: settings
    ...
    /// Isolated `$HOME` directory that the pager should use (keeps its ~/.ainxt
    ...
    ///
    /// Matches `ainxt_test_support::env::test_env_cmd_tokio`.
    pub fn env_for_pager(&self) -> Vec<(String, String)> {
        ...
        vec![
            ("HOME".into(), home),
            // Explicit AINXT_HOME prevents leaking the real user's
            // config.toml when $HOME alone isn't sufficient (e.g. if
            // AINXT_HOME is set in the test runner's env).
            ("AINXT_HOME".into(), ainxt_home),
            ("AINXT_CLI_CHAT_PROXY_BASE_URL".into(), self.url()),
            ("AINXT_API_BASE_URL".into(), self.url()),
            ("AINXT_API_KEY".into(), "test-key-for-ci".into()),
            ...
            ("AINXT_PROMPT_SUGGESTIONS".into(), "false".into()),
...
mod tests {
    ...
    /// on the SuperAinxt upsell screen and breaks every PTY test.
    #[tokio::test]
    ...
    async fn env_for_pager_shape() {
        ...
        assert_eq!(
            get("AINXT_HOME").as_deref(),
            content.home().join(".ainxt").to_str()
        );
        assert_eq!(get("AINXT_CLI_CHAT_PROXY_BASE_URL"), Some(content.url()));
        assert_eq!(get("AINXT_API_BASE_URL"), Some(content.url()));
        assert_eq!(get("AINXT_API_KEY").as_deref(), Some("test-key-for-ci"));
        assert_eq!(get("AINXT_TELEMETRY_ENABLED").as_deref(), Some("false"));
        assert_eq!(get("AINXT_FEEDBACK_ENABLED").as_deref(), Some("false"));
        assert_eq!(get("AINXT_TRACE_UPLOAD").as_deref(), Some("false"));
        assert_eq!(get("AINXT_PROMPT_SUGGESTIONS").as_deref(), Some("false"));
        assert_eq!(get("AINXT_MAX_RETRIES").as_deref(), Some("0"));
