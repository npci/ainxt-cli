...
//!
//! ```rust,no_run
//! use ainxt_tty_utils::{detach_command, pager_env};
...
//!
//! ```rust,no_run
//! use ainxt_tty_utils::{detach_std_command, pager_env};
//! use std::process::Stdio;
//!
...
/// Resource ceilings applied to a spawned process tree.
///
/// On Windows these are the **only** kernel-level containment available to this
/// tool: there is no Landlock, no Seatbelt, and no seccomp, so the capability
/// enforcement point is otherwise the sole control. A Job Object cannot
/// restrict what a process *reaches*, but it can bound how much it *consumes*,
/// which is what turns the resource-abuse classes — fork bombs, crypto mining,
/// runaway brute-force loops — from unbounded into merely noisy.
///
/// Deliberately generous. A limit that fires during an ordinary `cargo build`
/// or `npm ci` gets raised until it is meaningless, so these are set to bound
/// pathological behaviour and nothing else.
#[derive(Debug, Clone, Copy)]
pub struct ResourceLimits {
    /// Maximum concurrent processes in the tree. Bounds fork bombs and mass
    /// spawning; a parallel build sits well under this.
    pub max_active_processes: u32,
    /// Committed-memory ceiling for the whole job, in bytes.
    pub max_job_memory_bytes: usize,
    /// Optional CPU ceiling as a percentage (1–100).
    ///
    /// `None` by default, and that is a considered choice rather than an
    /// oversight: a legitimate `cargo build -j16` saturates every core, so a
    /// CPU cap is the limit most likely to punish real work. It is available
    /// for profiles that want it (a mining workload is exactly a pegged CPU),
    /// but it is opt-in.
...
impl Default for ResourceLimits {
    fn default() -> Self {
        Self {
            max_active_processes: 512,
            max_job_memory_bytes: 8 * 1024 * 1024 * 1024,
...
impl ProcessGroup {
    ...
    /// Create a teardown handle with explicit resource ceilings.
    ///
    /// On Unix the limits are accepted and ignored — the equivalent controls
    /// there are cgroups and rlimits, which are applied elsewhere, and silently
    /// pretending otherwise would be worse than doing nothing.
    pub fn with_limits(limits: ResourceLimits) -> io::Result<Self> {
        #[cfg(unix)]
        {
            let _ = limits;
            Ok(Self { leader: None })
        ...
        #[cfg(windows)]
        {
            ...
            // CPU rate control is a separate information class, so it needs its
            // own call. Only issued when a cap was actually requested.
            if let Some(percent) = limits.max_cpu_percent {
                ...
                cpu.ControlFlags =
                    JOB_OBJECT_CPU_RATE_CONTROL_ENABLE | JOB_OBJECT_CPU_RATE_CONTROL_HARD_CAP;
                // The field is in hundredths of a percent: 100% == 10_000.
                cpu.Anonymous.CpuRate = u32::from(percent.clamp(1, 100)) * 100;
                ...
                if let Err(e) = result {
                    ...
                    return Err(io::Error::other(format!(
                        "SetInformationJobObject(CpuRateControl): {e}"
                    )));
                }
            }
...
mod tests {
    ...
    fn stderr_redirect_roundtrip_subprocess() {
        let status = std::process::Command::new(std::env::current_exe().unwrap())
            ...
            .env("__AINXT_STDERR_REDIRECT_SUBPROCESS", "1")
            .status()
    ...
    fn stderr_redirect_roundtrip_body() {
        if std::env::var("__AINXT_STDERR_REDIRECT_SUBPROCESS").is_err() {
