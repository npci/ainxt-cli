...
pub enum Event {
    ...
    /// `TODO_GATE_*` discriminator constant in `ainxt-shell::session::events`.
    TodoGateFired {
        fires: u32,
        pending: usize,
        in_progress: usize,
    ...
    /// constants in `ainxt-shell::session::events`.
    ...
    /// matches the classifier event for dashboard symmetry — the
    ...
    /// discriminator constants in `ainxt-shell::session::events`.
    ...
    /// `ainxt-shell::session::events`. Fail-open never pauses the goal.
    ...
    /// enumerated by
    /// `ainxt-shell::session::goal_stop_detector::PATTERN_LABELS`;
...
pub enum InterjectionSource {
    /// Direct `ainxt.dev/interject` while a turn was running (Ctrl+Enter).
    Direct,
...
pub enum RedirectKind {
    /// Mid-turn interjection — Ctrl+O / `ainxt.dev/interject`, or "Send now" on a
...
/// Telemetry mirror of `ainxt-shell`'s `GoalClassifierVerdict`. Two
/// crates due to the orphan rule; the conversion lives in
/// `ainxt-shell/src/session/events.rs`.
#[derive(Debug, Clone, Copy, Serialize)]
#[serde(rename_all = "snake_case")]
...
/// Telemetry mirror of `ainxt-shell`'s `GoalPauseReason`. The two types
