...
use ainxt_circuit_breaker::{Disposition, RetryPolicy};
...
impl ResolvedStorageConfig {
    ...
    /// Bearer this resolved config puts on the wire — `snapshot()` matches
    /// `HttpAuth::apply` for provider-backed configs; the static fallback
    /// matches `AinxtAuthCredentials::apply` precedence (deployment key wins).
...
/// Resolve a `AINXT_UPLOAD_QUEUE_AUTH_PROBE_SECS` override (seconds) into a probe
...
/// `session_id`, no GCS path). Read by `ainxt_workspace::recovery`.
...
struct UploadQueueItem {
    ...
    /// ainxt client version string, stamped on the `gcs_queue_upload` tracing span.
...
pub struct UploadQueue {
    ...
    max_queue_bytes: u64,
    /// ainxt client version string stamped on every `gcs_queue_upload` tracing span.
...
impl UploadQueue {
    ...
    pub fn spawn(
        ...
        Self::spawn_with_concurrency(ainxt_home, resolver, retry_policy, DEFAULT_MAX_CONCURRENT)
    }
    ...
    pub fn spawn_with_concurrency(
        ...
        let queue_dir = ainxt_home.join("upload_queue");
        if let Err(e) = std::fs::create_dir_all(&queue_dir) {
            tracing::warn!(error = % e, "Failed to create upload queue dir");
        }
        if let Some(raw_secs) = std::env::var("AINXT_UPLOAD_QUEUE_AUTH_PROBE_SECS")
            .ok()
            .and_then(|s| s.parse::<u64>().ok())
        {
            match auth_park_probe_override(raw_secs) {
                ...
                None => {
                    tracing::warn!(
                        "Ignoring AINXT_UPLOAD_QUEUE_AUTH_PROBE_SECS={raw_secs}: a zero probe \
        ...
        let max_queue_bytes = std::env::var("AINXT_UPLOAD_QUEUE_MAX_BYTES")
            .ok()
    ...
    /// Set the ainxt client version to stamp on every `gcs_queue_upload` span.
    ...
    ///
    /// Matches [`Self::enqueue`] — same temp-file write, over-budget check and
    ...
    /// discarded and the completion resolves to a non-fatal `Err`. Matches
    ...
    pub async fn enqueue_file_reference(
        ...
        tracing::debug!(
            snapshot_session_tag = %session_log_tag(session_id),
            turn_number,
            gcs_path,
    ...
    match result {
        Ok((url, compression, stored_size)) => {
            ...
            tracing::info!(
                ...
                size_bytes = size,
                used_zstd_compression = compressed,
                outcome = "success",
...
fn session_log_tag(session_id: &str) -> String {
...
fn write_atomic(path: &Path, bytes: &[u8]) -> anyhow::Result<()> {
    ...
    // Explicitly release the file handle before the rename, rather than
    // relying solely on end-of-scope Drop.
    drop(file);
...
/// of entries removed.
pub fn cleanup_orphaned_uploads(ainxt_home: &Path, max_age: Duration) -> u64 {
    let cleaned = cleanup_queue_dir(&ainxt_home.join("upload_queue"), max_age, None);
...
mod tests {
    ...
    /// `enqueue_bytes_blocking` outcome tests. Matches the inline literals used
    ...
    async fn streaming_zstd_handles_incompressible_data() {
        ...
        let content: Vec<u8> = (0..1024)
            .map(|_| {
                ...
                rng ^= rng >> 7;
                rng ^= rng << 17;
                // Intentional truncation: this xorshift64 PRNG is only used
                // to synthesize incompressible test bytes, so we deliberately
                // keep just the low 8 bits of each 64-bit state word rather
                // than the full value. `& 0xFF` makes that truncation
                // explicit (vs. an implicit narrowing `as u8` cast) so it
                // reads as a byte-extraction, not an accidental data-loss bug.
                (rng & 0xFF) as u8
            })
            .collect();
    ...
    /// upload of the owned snapshot (matches `enqueue_file`): completion resolves
