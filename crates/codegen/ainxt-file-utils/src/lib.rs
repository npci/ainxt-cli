...
/// Wrap a raw client with [`ainxt_auth::AuthRetryMiddleware`] for automatic 401 retry.
...
pub fn sha256_hex_from_file(
    ...
    // Explicitly release the underlying file handle as soon as all bytes
    // have been read, rather than relying solely on end-of-scope Drop.
    drop(reader);
