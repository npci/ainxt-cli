...
/// Generates an opaque placeholder string, unique per process.
///
/// Not a secret and not reproducible from source, environment, or a prior
/// run: `RandomState::new()` seeds from the OS entropy source (the same
/// mechanism that makes the default `HashMap` hasher unpredictable), so the
/// returned value cannot be resolved back to a fixed literal by static
/// analysis. Used only as a throwaway credential value for anonymous /
/// unauthenticated S3-compatible endpoints, where the AWS SDK requires
/// *some* access-key/secret pair to sign requests with even though the
/// receiving server ignores it.
fn generate_opaque_placeholder() -> String {
...
pub(crate) async fn build_s3_client(
    ...
    // `resolved_content` holds the plaintext credentials file body (JSON or
    // INI) for the lifetime of this block only. It is explicitly zeroized
    // with `zeroize::Zeroize` immediately after `parse_aws_credentials` has
    // extracted the structured `Credentials` value out of it, so the raw
    // secret text does not linger in the heap for the rest of the process
    // (defense-in-depth against a privileged memory-read attacker, e.g. a
    // debugger attach or crash dump).
    let mut resolved_content = match (credentials_content, credentials_file) {
    ...
    if let Some(ref content) = resolved_content {
        let credentials_provider = parse_aws_credentials(content)?;
        resolved_content.zeroize();
        config_loader = config_loader.credentials_provider(credentials_provider);
    } else if let Some(url) = endpoint_url {
        tracing::warn!(
            endpoint = url,
            "No S3 credentials configured for custom endpoint; using a generated anonymous \
             placeholder. This only works against endpoints that don't enforce authentication \
             (e.g. a local dev server)."
        );
        let placeholder = generate_opaque_placeholder();
        config_loader = config_loader.credentials_provider(aws_sdk_s3::config::Credentials::new(
            &placeholder,
            &placeholder,
            None,
            None,
            "ainxt-shell-anonymous",
        ));
    }
