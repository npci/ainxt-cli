...
//!
//! Directory precedence matches `index::load_index`:
//! `.ainxt-plugin/plugin-index.json` (preferred), then
...
use ainxt_hooks_plugins_types::PluginComponents;
...
mod tests {
    ...
    #[test]
    fn load_catalog_parses_ainxt_plugin_dir() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".ainxt-plugin", BASIC);
    ...
    #[test]
    fn load_catalog_prefers_ainxt_dir_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".ainxt-plugin", BASIC);
        write_catalog(
            dir.path(),
    ...
    fn load_catalog_malformed_returns_none() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".ainxt-plugin", "not json");
        assert!(load_catalog(dir.path()).is_none());
    }
    ...
    fn load_catalog_ignores_unknown_fields() {
        ...
        write_catalog(
            ...
            ".ainxt-plugin",
            r#"{
                "$schema": "https://ainxt.example.test/plugin-index.schema.json",
                "version": 1,
