...
impl MarketplaceRelativePath {
    pub fn parse(input: &str) -> Result<Self, MarketplacePathError> {
        // Single-plugin marketplaces (Claude Code convention) point the
        // plugin's `source` at the marketplace root itself via `"./"` or
        // `"."` — the plugin's manifest lives at the repo root rather than
        // under a `plugins/<name>/` subdirectory. Treat that as a distinct
        // "root" path instead of rejecting it as empty; a fully empty
        // string (no dot at all) is still rejected below.
...
pub struct MarketplaceEntry {
    ...
    /// Relative path within marketplace (e.g., "plugins/ainxt-code-review").
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub components: Option<ainxt_hooks_plugins_types::PluginComponents>,
}
...
mod tests {
    ...
    /// Single-plugin marketplaces (the Claude Code convention) point a
    /// plugin's `source` at the marketplace root itself via `"./"` or `"."`
    /// rather than a `plugins/<name>/` subdirectory. This must parse to a
    /// root marker, not be rejected as an empty path — otherwise the plugin
    /// silently disappears from `scan_marketplace` results and "Install"
    /// has nothing to act on.
    #[test]
    fn marketplace_relative_path_accepts_root_marker() {
        for input in ["./", ".", ".\\"] {
            let path = MarketplaceRelativePath::parse(input)
    ...
    #[test]
    fn marketplace_relative_path_root_marker_join_under_resolves_to_root() {
        let dir = tempfile::tempdir().unwrap();
        let path = MarketplaceRelativePath::parse("./").unwrap();
        let resolved = path.join_under(dir.path()).unwrap();
        assert_eq!(
