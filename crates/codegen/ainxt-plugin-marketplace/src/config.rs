//! Parse marketplace sources from `~/.ainxt/config.toml`.
//!
//! Expected format:
//! ```toml
//! [[marketplace.sources]]
//! name = "ainxt Official"
//! git = "https://github.com/ainxt-org/ainxt-plugin-marketplace.git"
//!
//! [[marketplace.sources]]
//! name = "Local Dev"
//! path = "~/dev/my-plugins"
...
/// instead of `~/.ainxt`/`~/.claude`. Each root is checked for
...
mod tests {
    ...
    fn parse_git_source() {
        ...
        let sources = load_sources(&config);
        assert_eq!(sources.len(), 1);
        assert_eq!(sources[0].name, "ainxt Official");
        assert!(
            matches!(&sources[0].kind, SourceKind::Git { url, branch } if url.contains("ainxt-org") && branch.as_deref() == Some("main"))
        );
    }
