...
//!
//! Catalog file lookup, in order: `.ainxt-plugin/marketplace.json` (preferred),
//! `.ainxt-plugin/plugin.json`, `.claude-plugin/marketplace.json`,
...
///
/// Checks (in order):
/// 1. `.ainxt-plugin/marketplace.json` (preferred ainxt convention)
/// 2. `.ainxt-plugin/plugin.json`
...
mod tests {
    ...
    #[test]
    fn load_index_valid_ainxt_dir() {
        let dir = tempfile::tempdir().unwrap();
        let ainxt_dir = dir.path().join(".ainxt-plugin");
        std::fs::create_dir_all(&ainxt_dir).unwrap();
        std::fs::write(
            ainxt_dir.join("marketplace.json"),
            r#"{"name": "ainxt", "plugins": []}"#,
        )
        .unwrap();
        let result = load_index(dir.path()).unwrap();
        assert!(result.is_some());
        assert_eq!(result.unwrap().name, "ainxt");
    }

    #[test]
    fn load_index_ainxt_dir_takes_precedence_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        for (sub, name) in [(".ainxt-plugin", "ainxt"), (".claude-plugin", "claude")] {
        ...
        assert_eq!(load_index(dir.path()).unwrap().unwrap().name, "ainxt");
    }
