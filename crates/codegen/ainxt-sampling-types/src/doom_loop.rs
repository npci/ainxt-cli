...
//!
//! When the client opts in via the `x-ainxt-doom-loop-check` request header,
...
pub const DOOM_LOOP_CHECK_HEADER: &str = "x-ainxt-doom-loop-check";
...
/// server (unchanged from the server's wire sample; the
...
pub struct DoomLoopSignal {
    ...
    pub channel: String,
    /// The unchanged label; the stable identity used for deduplication and
    /// logging.
    pub raw: String,
}
