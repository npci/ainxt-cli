...
//!
//! TODO: Move from ainxt-shell/src/sampling/error.rs
...
/// Flat error from the ainxt proxy/gateway: `{"code": "...", "error": "..."}`.
#[derive(Debug, Deserialize)]
...
///
/// Some providers wrap the error object in a single-element array —
/// `[{"error": {...}}]` instead of the standard bare `{"error": {...}}`.
/// A one-field struct's derived
/// `Deserialize` also accepts a same-arity JSON array (positionally matching
/// the whole element into that one field), so a bare `from_str` against that
/// shape doesn't fail — it silently produces an all-`None` `ErrorBody`
/// instead, losing the real message. Unwrap a single-element array first so
/// the real object underneath gets parsed normally.
fn try_parse_error(data: &str) -> Option<(String, String)> {
    let unwrapped;
    let data = match serde_json::from_str::<serde_json::Value>(data) {
        Ok(serde_json::Value::Array(arr)) if arr.len() == 1 => {
            unwrapped = arr.into_iter().next().unwrap().to_string();
            unwrapped.as_str()
        }
        _ => data,
    };
...
/// True when an error message indicates a context-window overflow OR a
/// max-output-tokens ceiling violation. Backends report both inconsistently
/// with no stable error code, so we match the message text; both are
/// deterministic (re-sending the same payload always fails the same way —
/// the offending value is either the prompt size or the requested
/// `max_tokens`, and neither changes on retry), so callers must not retry.
///
/// The output-ceiling variant is distinct from a context-window overflow: a
/// model can have a context window far larger than its `max_tokens` output
/// ceiling (e.g. Claude Haiku 4.5: 256K context / 64K output), so a client
/// that only clamps `max_tokens` against context_window can send a value the
/// provider rejects outright. See Anthropic's `invalid_request_error`:
/// `"max_tokens: 128000 > 64000, which is the maximum allowed number of
...
pub fn is_context_length_error(message: &str) -> bool {
    ...
    m.contains("too long for this model")
        ...
        || m.contains("maximum allowed number of output tokens")
        || (m.contains("max_tokens") && m.contains("which is the maximum"))
}
...
mod tests {
    ...
    #[test]
    fn array_wrapped_error_keeps_real_message() {
        // Regression: some providers wrap the error body in a single-element
        // array. A one-field struct's derived Deserialize also accepts a
        // same-arity array (positional matching), so a naive parse
        // "succeeds" with an all-None ErrorBody instead of falling back to
        // the raw text — silently replacing the real message with the
  ...
  "error": {
    ...
    "status": "INVALID_ARGUMENT"
        ...
        assert_eq!(parse_error_bytes(body), "Please pass a valid API key");
    }
    ...
    fn context_length_error_matches_backend_variants() {
        ...
        // Regression: max-output-tokens ceiling errors (distinct from a context-
        // window overflow — Anthropic's `invalid_request_error` for a model whose
        // configured `max_tokens` default exceeds that model's real output
        // ceiling, e.g. Claude Haiku 4.5 at 256K context / 64K output) must be
        // classified the same way: deterministic, never retried.
        for msg in [
            "max_tokens: 128000 > 64000, which is the maximum allowed number of output tokens for claude-haiku-4-5-20251001",
            "invalid_request_error: max_tokens: 128000 > 64000, which is the maximum allowed number of output tokens for claude-haiku-4-5-20251001",
        ] {
            assert!(is_context_length_error(msg), "should match: {msg}");
        }
    ...
    fn serialization_from_rendered_round_trips_display() {
        ...
        // silently desynchronize the strip from the prefix it matches.
        let original =
