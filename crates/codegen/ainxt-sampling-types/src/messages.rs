...
///
/// Field order here is the wire order: `serde` serialises struct fields in
/// declaration order, and the gateway expects `tools`, then `system`, then
/// `messages` at the front of the body. Keep those three first when adding
/// fields — moving them changes what the gateway receives.
...
pub struct MessageDeltaUsage {
    ...
    /// ainxt extension: gateway-computed cost for this request, in USD ticks
    /// (1 USD = 1e10 ticks). The gateway is the only component that prices a
    /// call — the CLI never estimates — so this is the sole cost input.
    /// Absent on stock Anthropic servers and on any gateway predating the
    /// extension; see `ainxt_cost_priced` for how absence is interpreted.
    #[serde(default)]
    pub ainxt_cost_in_usd_ticks: Option<i64>,
    /// ainxt extension: whether `ainxt_cost_in_usd_ticks` is a real price.
    ///
    /// `Some(true)` means the value is authoritative — including a genuine
    /// zero for in-house models. `Some(false)` means the gateway could not
    /// price the call. `None` (field absent) is treated as unpriced, so a
    /// server that never sends cost reports "unavailable", never "free".
    #[serde(default)]
    pub ainxt_cost_priced: Option<bool>,
    /// ainxt extension: true cache-read count, ungated by the server's
    /// `TRACK_CACHE_TOKENS` flag (which may zero the standard
    /// `cache_read_input_tokens`). Display only — never priced locally.
    #[serde(default)]
    pub ainxt_cache_read_tokens: Option<u32>,
    /// ainxt extension: true cache-write (`cache_creation`) count, ungated by
    /// the server's `TRACK_CACHE_TOKENS` flag. Display only.
    #[serde(default)]
    pub ainxt_cache_write_tokens: Option<u32>,
}
