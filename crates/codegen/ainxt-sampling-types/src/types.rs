...
// Re-export ToolDefinition and FunctionTool from ainxt-tools.
...
pub use ainxt_tools::types::definition::{FunctionTool, ToolDefinition};
...
pub struct Usage {
    ...
    /// ainxt extension: request price in USD ticks (1 USD = 1e10 ticks).
...
pub struct ChatCompletionChunk {
    // `id`, `object`, `created`, and `model` are optional in some
    // OpenAI-compatible gateways (they omit them on streamed deltas, or on
    // upstream-error frames). The spec marks them required, but treating them
    // as required breaks deserialization of otherwise-valid chunks, so default
    // them when absent rather than dropping the whole chunk.
    #[serde(default)]
    pub id: String,
    #[serde(default)]
    pub object: String,
...
pub struct SamplingConfig {
    ...
    /// True when `api_backend` came from this model's own explicit
    /// `[model.<id>].api_backend` config (a direct/BYOK endpoint), as
    /// opposed to a bundled default or gateway-catalog value that dispatch
    /// forces to Messages absent this being set. See
    /// `ainxt_sampler::SamplerConfig::api_backend_explicit` /
    /// `ModelInfo::api_backend_explicit`, which this mirrors so the value
    /// survives being persisted/reconstructed via chat-state.
    #[serde(default)]
...
/// custom header fields for ainxt request tracking, similar to
...
pub struct CreateResponseWrapper {
    ...
    /// ainxt-specific tool definitions that can't be expressed via
...
/// Wrapper around `MessagesRequest` that adds custom header fields for ainxt
...
mod tests {
    ...
    fn parse_reasoning_efforts_meta_absent_is_none() {
        ...
        assert!(
            parse_reasoning_efforts_meta(Some(json!({ "agentType": "ainxt" }).as_object().unwrap()))
                .is_none()
        );
    }
