//! Pure data types for the ainxt sampling / chat-completion API layer.
//!
...
//! ainxt agent stack.  It intentionally contains **no I/O** (no HTTP clients,
//! no file system access) so it can be depended on by downstream crates
//! (e.g., `ainxt-chat-state`) without pulling in the full `ainxt-shell`.
...
pub use self::capabilities::{ModelCapabilities, is_in_house_model};
