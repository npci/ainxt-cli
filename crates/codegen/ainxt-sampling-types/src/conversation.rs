...
pub enum HostedTool {
    ...
    /// This is ainxt-specific — not part of the OpenAI Responses API, so it's
...
pub struct ConversationRequest {
    ...
    /// Custom headers for ainxt tracking
    pub x_ainxt_conv_id: Option<String>,
    pub x_ainxt_req_id: Option<String>,
    pub x_ainxt_session_id: Option<String>,
    pub x_ainxt_turn_idx: Option<String>,
    pub x_ainxt_agent_id: Option<String>,
    pub x_ainxt_deployment_id: Option<String>,
    pub x_ainxt_user_id: Option<String>,
    ...
    /// Which optional Messages features this model's endpoint accepts. Defaults
    /// to fully capable; narrowed per model so a self-hosted endpoint is not
    /// sent extensions it cannot parse. See [`crate::ModelCapabilities`].
...
pub struct TokenUsage {
    ...
    ///   into `prompt_tokens` instead — see [`Self::cache_write_tokens`] for the
    ///   separately reported write count.
    #[serde(default)]
    pub cached_prompt_tokens: u32,
    /// Prompt tokens written to the cache (Anthropic
    /// `cache_creation_input_tokens`, billed at ~1.25x the input rate).
    ///
    /// Reporting only — `prompt_tokens` already includes these, so adding
    /// them again would double-count. Zero on backends that do not report
    /// cache writes.
    #[serde(default)]
...
impl From<Usage> for TokenUsage {
    fn from(u: Usage) -> Self {
        ...
        Self {
            ...
            // OpenAI backends report no cache-write bucket.
...
pub struct ConversationResponse {
    ...
    /// only, opt-in via the `x-ainxt-doom-loop-check` header). Empty when the
...
// ---------------------------------------------------------------------------
//
// Part of the ainxt Compaction unification. Lets the shared, transport-agnostic
// engine in `crates/common/ainxt-compaction` operate over ainxt-build's
// `ConversationItem` without depending on this crate — the orphan rule forces
// the impls to live here, next to the type. Matches the harness's
...
impl ainxt_compaction::CompactionItem for ConversationItem {
    fn role(&self) -> ainxt_compaction::CompactionRole {
        use ainxt_compaction::CompactionRole;
        // ainxt-build has no distinct `Developer` role; everything maps onto
    ...
    fn is_compaction_summary(&self) -> bool {
        // ainxt-build has no structural marker that uniquely identifies a prior
        ...
        // (add a dedicated marker) before routing ainxt-build history through
        // the shared `history`/`inter` filter.
        false
    }

    fn attachment_refs(&self) -> Vec<ainxt_compaction::CompactionFileRef> {
        // ainxt-build `UserItem`s carry only `Text`/`Image { url }` content
        // parts — there is no id+name attachment-ref concept like the chat harness's
        // `AinxtTurn`. The full-replace path does not read this; revisit if
        // image attachments need to survive into the `<ainxt_user_queries>`
        // preamble.
        Vec::new()
    }
}

impl ainxt_compaction::CompactionItemFactory for ConversationItem {
...
/// ([stream/messages.rs:340](crates/codegen/ainxt-sampler/src/stream/messages.rs))
...
///
/// This is `pub` so downstream crates (`ainxt-shell`) can re-use it
...
impl From<ChatResponseMessage> for ConversationItem {
    fn from(msg: ChatResponseMessage) -> Self {
        ...
        // ([crates/codegen/ainxt-sampler/src/stream/chat_completions.rs])
...
impl From<ConversationRequest> for ChatCompletionRequest {
    fn from(req: ConversationRequest) -> Self {
        ...
        ChatCompletionRequest {
            ...
            x_ainxt_conv_id: req.x_ainxt_conv_id,
            x_ainxt_req_id: req.x_ainxt_req_id,
            x_ainxt_session_id: req.x_ainxt_session_id,
            x_ainxt_turn_idx: req.x_ainxt_turn_idx,
            x_ainxt_agent_id: req.x_ainxt_agent_id,
            x_ainxt_deployment_id: req.x_ainxt_deployment_id,
            x_ainxt_user_id: req.x_ainxt_user_id,
            trace: None,
        }
    }
}
...
fn build_responses_tools(req: &ConversationRequest) -> Vec<rs::Tool> {
    ...
    for hosted in &req.hosted_tools {
        match hosted {
            ...
            // XSearch is ainxt-specific — not in async_openai's rs::Tool enum.
...
/// Return raw JSON tool definitions for ainxt-specific hosted tools that
...
impl ConversationRequest {
    ...
    pub fn with_conv_id(mut self, conv_id: impl Into<String>) -> Self {
        self.x_ainxt_conv_id = Some(conv_id.into());
        self
    }

    /// Set request ID header
    pub fn with_req_id(mut self, req_id: impl Into<String>) -> Self {
        self.x_ainxt_req_id = Some(req_id.into());
        self
    }
...
/// Stands in for an image withheld by the model's capability profile. Names the
/// flag so the fix is discoverable from the transcript alone.
const IMAGE_OMITTED_PLACEHOLDER: &str =
    "[image omitted: this model's capability profile has images disabled — set \
     `images = true` under [model.<id>.capabilities] to send it]";
...
pub fn build_messages_request(req: &ConversationRequest) -> crate::messages::MessagesRequest {
    ...
    // What this model's endpoint accepts; consulted by the block helpers
    // below and by the request-field gating further down.
    ...
    // Every `tool_use` id emitted so far, so a `tool_result` can be checked for
    // a partner before it goes on the wire.
        ...
        parts
            ...
            .map(|part| match part {
                ...
                ContentPart::Image { url } => {
                    // A text-only endpoint errors on an image block rather than
                    // ignoring it. Name the flag in the placeholder so a dropped
                    // image is visible and actionable in the transcript.
                    if !caps.images {
                        ContentBlock::Text {
    ...
    //
    // `tool_result` blocks must lead the message, so any text block folded in
    // alongside them (an orphaned result recovered as prose — see the ToolResult
    // arm) is moved after them. Stable, so relative order within each group is
    // preserved.
    let flush_tool_results = |pending: &mut Vec<ContentBlock>, msgs: &mut Vec<Message>| {
        if !pending.is_empty() {
            let mut blocks = pending.clone();
            blocks.sort_by_key(|b| !matches!(b, ContentBlock::ToolResult { .. }));
            msgs.push(Message {
    ...
    // output without tools. The ainxt-shell agent does NOT use this path — a
    ...
    // Computed before the item loop because replaying a `thinking` block in
    // history is only valid when the request itself enables thinking — the
    // Reasoning arm below gates on `thinking_config`.
    //
    // Both this and `output_config` are withheld for a model whose endpoint
    // does not implement them; see `ModelCapabilities`.
    let thinking_config = if caps.thinking {
        effort
            .as_ref()
    ...
    let output_config = if caps.output_config && (effort.is_some() || format.is_some()) {
        ...
        match item {
            ...
            ConversationItem::Assistant(a) => {
                ...
                for tc in &a.tool_calls {
                    ...
                    let id = sanitize_tool_call_id(&tc.id);
                    emitted_tool_use_ids.insert(id.clone());
            ...
            ConversationItem::ToolResult(t) => {
                flush_assistant(&mut pending_assistant, &mut messages);
                let content = if t.images.is_empty() || !caps.images {
                    let text = if t.images.is_empty() {
                        t.content.as_ref().to_owned()
                    } else {
                        format!("{}\n{IMAGE_OMITTED_PLACEHOLDER}", t.content.as_ref())
                    };
                ...
                let tool_use_id = sanitize_tool_call_id(&t.tool_call_id);
                if emitted_tool_use_ids.contains(&tool_use_id) {
                    ...
                    // No `tool_use` in this request carries that id, so the
                    // pairing the wire format requires cannot be satisfied and
                    // the endpoint would reject the whole request. Happens when
                    // compaction or a fork boundary drops the assistant turn
                    // that made the call but keeps its result.
                    //
                    // Preserve the output as prose rather than deleting it: the
                    // model still needs what the tool returned, and the text is
                    // all that a re-sent tool_result would have conveyed anyway.
                    tracing::debug!(
                        tool_use_id = %tool_use_id,
                        "tool_result has no matching tool_use in this request; \
                         folding it in as text to keep the request valid"
                    );
                    pending_tool_results.push(ContentBlock::Text {
                        text: format!(
                            "[result of an earlier tool call, no longer in context]\n{}",
                            t.content.as_ref()
                        ),
                        cache_control: None,
                    });
            ...
            //
            // A thinking block is only replayable with the signature the server
            // issued for it: the wire contract has no notion of an unsigned
            // thinking block, so `signature: ""` is invalid input rather than a
            // degraded version of a valid one. An endpoint that streams thinking
            // text without ever sending a `signature_delta` (any Messages-
            // compatible gateway in front of a model that has no signing
            // concept) therefore leaves us holding a block we cannot send back.
            //
            // Dropping it costs only reasoning context in history — the
            // assistant's text and tool calls are carried separately — whereas
            // sending it risks rejecting the whole request. Nothing changes for
            // a server that does sign its thinking blocks.
            ConversationItem::Reasoning(r) => {
                flush_tool_results(&mut pending_tool_results, &mut messages);
                ...
                let signature = r
                    ...
                    .map(str::to_owned)
                    .unwrap_or_default();
                let replayable = !signature.is_empty() && thinking_config.is_some();
                if replayable {
                    ...
                    tracing::debug!(
                        ...
                        request_thinking_enabled = thinking_config.is_some(),
                        "dropping a non-replayable thinking block from history"
                    );
                }
            }
        }
    ...
    // Attach cache_control: {type: "ephemeral"} to last system block, unless the
    // endpoint has no prompt cache to address.
    ...
    // Build system param. An endpoint that only accepts a string `system` gets
    // the blocks joined; a blank line keeps the sections legible to the model.
    let system: Option<SystemParam> = if system_blocks.is_empty() {
        None
    } else if !caps.system_blocks {
        Some(SystemParam::Text(
            system_blocks
                .iter()
                .map(|b| b.text.as_str())
                .collect::<Vec<_>>()
    ...
    // Listed in wire order — see the note on `MessagesRequest`.
    MessagesRequest {
        tools,
        system,
        messages,
...
/// ([crates/codegen/ainxt-sampler/src/stream/messages.rs]) instead
...
mod compaction_item_bridge_tests {
    use super::*;
    use ainxt_compaction::{CompactionItem, CompactionItemFactory, CompactionRole};
    ...
    fn metadata_accessors_are_conservative() {
        // ainxt-build has no structural compaction-summary marker, and no
...
mod tests {
    ...
    fn test_conversation_item_roundtrip() {
        ...
        let assistant = ConversationItem::assistant_with_model("Hi there!", "ainxt-3");
    ...
    fn test_conversation_request_to_chat_completion() {
        ...
        assert_eq!(chat_req.model, Some("ainxt-3".to_string()));
    ...
    fn test_conversation_request_to_responses_api() {
        ...
        assert_eq!(responses_req.model, Some("ainxt-3".to_string()));
    ...
    fn test_responses_api_response_to_conversation_item() {
        ...
        assert_eq!(a.model_id, Some("ainxt-3".to_string()));
        assert_eq!(
    ...
    fn test_assistant_with_content_and_tool_calls() {
        ...
        let assistant = AssistantItem {
            ...
            model_id: Some("ainxt-3".to_string()),
            model_fingerprint: None,
            reasoning_effort: None,
        };
        ...
        assert_eq!(chat_msg.tool_calls.len(), 1);
        assert_eq!(chat_msg.model_id, Some("ainxt-3".to_string()));
    }
    ...
    /// message in its history must NOT forward the broken string unchanged.
    ...
    /// The whole point of the profile: a core-only model gets a request with no
    /// Anthropic extensions at all, while the conversation itself survives.
    #[test]
    fn core_only_profile_strips_every_extension_from_the_request() {
        let mut reasoning = synthesized_reasoning_item("thinking out loud");
        reasoning.encrypted_content = Some("sig-abc".into());
        let mut req = ConversationRequest::from_items(vec![
            ConversationItem::system("you are a coding agent"),
            ConversationItem::system("extra system section"),
            ConversationItem::user("first turn"),
            ConversationItem::Reasoning(reasoning),
            ConversationItem::Assistant(AssistantItem {
                content: "an answer".into(),
                tool_calls: vec![],
                model_id: Some("local:kimi-k2.7-code".into()),
                model_fingerprint: None,
                reasoning_effort: None,
            }),
        ...
        req.reasoning_effort = Some(crate::ReasoningEffort::High);
        req.capabilities = crate::ModelCapabilities::core_only();

        let msg = build_messages_request(&req);
        let json = serde_json::to_value(&msg).unwrap();

        assert!(msg.thinking.is_none(), "thinking must be withheld");
        assert!(
            msg.output_config.is_none(),
            "output_config must be withheld"
        );
        assert!(
            matches!(msg.system, Some(crate::messages::SystemParam::Text(_))),
            "system must be a plain string, got {:?}",
            msg.system
        );
        // Both system sections must still reach the model.
        let system_text = match &msg.system {
            Some(crate::messages::SystemParam::Text(t)) => t.clone(),
            other => panic!("expected text system, got {other:?}"),
        };
        assert!(system_text.contains("you are a coding agent"));
        assert!(system_text.contains("extra system section"));

        // No cache_control anywhere, and no thinking blocks in history.
        let serialized = serde_json::to_string(&json).unwrap();
        assert!(
            !serialized.contains("cache_control"),
            "no cache_control breakpoints for a core-only endpoint"
        );
        assert!(
            !serialized.contains("\"thinking\""),
            "no thinking blocks replayed for a core-only endpoint"
        );
        // The turns themselves survive.
        assert!(serialized.contains("first turn"));
        assert!(serialized.contains("an answer"));
        assert!(serialized.contains("second turn"));
    }

    /// The full profile (the default) must be byte-identical to the old
    /// behavior: extensions present, system as blocks with a cache breakpoint.
    #[test]
    fn full_profile_keeps_every_extension() {
        let mut req = ConversationRequest::from_items(vec![
            ConversationItem::system("you are a coding agent"),
            ConversationItem::user("hello"),
        ]);
        ...
        assert_eq!(
            req.capabilities,
            crate::ModelCapabilities::full(),
            "full must be the default"
        );

        let msg = build_messages_request(&req);
        assert!(msg.thinking.is_some());
        assert!(msg.output_config.is_some());
        let serialized = serde_json::to_string(&serde_json::to_value(&msg).unwrap()).unwrap();
        assert!(
            serialized.contains("cache_control"),
            "prompt-cache breakpoint expected on the full profile"
        );
    }

    /// A tool_result whose tool_use was dropped (compaction, fork boundary)
    /// cannot be paired on the wire and would reject the whole request. It is
    /// folded in as text instead, so the output still reaches the model.
    #[test]
    fn orphaned_tool_result_is_folded_in_as_text() {
        let req = ConversationRequest::from_items(vec![
            ConversationItem::user("read the file"),
            // The assistant turn carrying tool_use `call_gone` is absent.
            ConversationItem::tool_result("call_gone", "fn main() {}"),
            ConversationItem::user("now what?"),
        ]);
        ...
        assert!(
            ...
            "an unpaired tool_result must not go on the wire: {serialized}"
        );
        assert!(
            serialized.contains("fn main"),
            "the tool output must survive as text: {serialized}"
        );
    }

    /// A properly paired tool_use / tool_result round-trips as a real
    /// tool_result block, and the result leads its user message.
    #[test]
    fn paired_tool_result_stays_a_tool_result_and_leads_its_message() {
        let req = ConversationRequest::from_items(vec![
            ConversationItem::user("read the file"),
            ConversationItem::Assistant(AssistantItem {
        ...
        assert_eq!(
            ...
            Some("tool_result"),
            "tool_result must lead the user message: {blocks:?}"
        );
        assert_eq!(
            blocks[0].get("tool_use_id").and_then(|t| t.as_str()),
            Some("call_1")
        );
    }

    /// A text-only endpoint must never receive an image block; the placeholder
    /// names the flag so the omission is visible and fixable.
    #[test]
    fn images_are_replaced_with_a_placeholder_when_withheld() {
        let mut req = ConversationRequest::from_items(vec![ConversationItem::User(UserItem {
            content: vec![
                ContentPart::Text {
        ...
        assert!(
            ...
            "no image payload may reach a text-only endpoint: {serialized}"
        );
        assert!(
            serialized.contains("images = true"),
            "the placeholder must name the flag: {serialized}"
        );
        assert!(
            serialized.contains("what is in this screenshot?"),
            "the user's text must survive"
        );
    }

    /// A vision-capable model still gets the real image block.
    #[test]
    fn images_are_sent_when_the_profile_allows_them() {
        ...
        assert!(req.capabilities.images, "full profile is the default");

        let msg = build_messages_request(&req);
        let serialized = serde_json::to_string(&serde_json::to_value(&msg).unwrap()).unwrap();
        assert!(serialized.contains("base64"), "image must be sent");
    }

    /// Images attached to a tool result (e.g. `read_file` on a PNG) take the
    /// same gate, and the textual result still ships.
    #[test]
    fn tool_result_images_are_withheld_but_the_text_survives() {
        let mut req = ConversationRequest::from_items(vec![
            ConversationItem::user("read the diagram"),
            ConversationItem::Assistant(AssistantItem {
                content: String::new().into(),
                tool_calls: vec![ToolCall {
                    id: "call_1".into(),
                    name: "read_file".into(),
                    arguments: r#"{"path":"d.png"}"#.into(),
                }],
                model_id: None,
                model_fingerprint: None,
                reasoning_effort: None,
        ...
        let msg = build_messages_request(&req);
        let serialized = serde_json::to_string(&serde_json::to_value(&msg).unwrap()).unwrap();
        assert!(!serialized.contains("base64"), "image withheld: {serialized}");
        assert!(serialized.contains("read d.png"), "tool text survives");
        assert!(serialized.contains("images = true"), "flag named");
    }

    /// Collect the `type` of every content block across all messages.
    fn message_block_types(msg: &crate::messages::MessagesRequest) -> Vec<String> {
        let json = serde_json::to_value(msg).unwrap();
        json.get("messages")
    ...
    /// A server that streams thinking *text* but never a `signature_delta`
    /// leaves a Reasoning item with no `encrypted_content`. Replaying it would
    /// put `{"type":"thinking","signature":""}` on the wire, which is not a
    /// degraded valid block — it is invalid input. It must be dropped.
    ///
    /// This only bites from the second turn onward (turn one has no history),
    /// and immediately for a subagent, which starts from a copy of the
    /// parent's conversation.
    #[test]
    fn unsigned_thinking_block_is_not_replayed_in_history() {
        let req = ConversationRequest::from_items(vec![
            ConversationItem::user("first turn"),
            ConversationItem::Reasoning(synthesized_reasoning_item("let me think")),
            ConversationItem::Assistant(AssistantItem {
                content: "an answer".into(),
                tool_calls: vec![],
        ...
        let msg = build_messages_request(&req);
        let types = message_block_types(&msg);
        assert!(
            !types.iter().any(|t| t == "thinking"),
            "unsigned thinking must not be replayed, got blocks: {types:?}"
        );
        assert!(
            types.iter().any(|t| t == "text"),
            "the assistant's text must survive: {types:?}"
        );
    }

    /// A signed thinking block on a request that enables thinking is the
    /// supported case and must still be replayed untouched.
    #[test]
    fn signed_thinking_block_is_replayed_when_thinking_enabled() {
        let mut reasoning = synthesized_reasoning_item("let me think");
        reasoning.encrypted_content = Some("sig-abc123".into());
        let mut req = ConversationRequest::from_items(vec![
            ConversationItem::user("first turn"),
            ConversationItem::Reasoning(reasoning),
            ConversationItem::Assistant(AssistantItem {
        ...
        let msg = build_messages_request(&req);
        assert!(msg.thinking.is_some(), "request must enable thinking");
        let types = message_block_types(&msg);
        assert!(
            types.iter().any(|t| t == "thinking"),
            "signed thinking must be replayed, got: {types:?}"
        );
    }

    /// Even a *signed* thinking block is unsendable when the request does not
    /// enable thinking: blocks in `messages` with top-level `thinking: null` is
    /// the 400 documented in `test_btw_stripped_reasoning_produces_no_thinking_blocks`.
    /// Guard it at the builder instead of relying on every caller to strip.
    #[test]
    fn signed_thinking_block_is_dropped_when_request_disables_thinking() {
        let mut reasoning = synthesized_reasoning_item("let me think");
        reasoning.encrypted_content = Some("sig-abc123".into());
        let req = ConversationRequest::from_items(vec![
            ConversationItem::user("first turn"),
        ...
        let msg = build_messages_request(&req);
        assert!(msg.thinking.is_none(), "request must not enable thinking");
        let types = message_block_types(&msg);
        assert!(
            !types.iter().any(|t| t == "thinking"),
            "thinking blocks must not ride along with thinking: null, got: {types:?}"
        );
    }
    ...
    fn test_btw_stripped_reasoning_produces_no_thinking_blocks() {
        ...
        // `strip_reasoning_blocks` in ainxt-chat-state. Here the assistant
    ...
    fn test_transform_cwd_transforms_tool_call_arguments() {
        ...
        let worktree = "/home/user/.ainxt/worktrees/project/ab-uuid-a";
    ...
    fn test_transform_cwd_worktree_to_root_syncback() {
        ...
        let worktree = "/home/user/.ainxt/worktrees/myproject/fork-a";
    ...
    fn test_transform_cwd_assistant_only_tool_calls_no_content() {
        ...
        let worktree = "/home/user/.ainxt/worktrees/proj/fork-a";
    ...
    fn test_conversation_request_builder() {
        ...
        assert_eq!(req.model, Some("ainxt-3".to_string()));
        assert_eq!(req.temperature, Some(0.5));
        assert_eq!(req.max_output_tokens, Some(1000));
        assert_eq!(req.x_ainxt_conv_id, Some("conv-123".to_string()));
        assert_eq!(req.x_ainxt_req_id, Some("req-456".to_string()));
    ...
    fn test_assistant_item_with_model_id() {
        ...
        .with_model_id("ainxt-3");

        assert_eq!(item.model_id, Some("ainxt-3".to_string()));
    }

    #[test]
    fn test_conversation_item_with_model_id() {
        let item = ConversationItem::assistant("Hello").with_model_id("ainxt-3");
        ...
        let user = ConversationItem::user("Hi").with_model_id("ainxt-3");
    ...
    fn responses_api_conversion_preserves_model_fingerprint() {
        ...
        assert_matches!(item, ConversationItem::Assistant(ref a) => {
            ...
            assert_eq!(a.model_id.as_deref(), Some("ainxt-4.5"));
    ...
    //      (earlier ainxt-build / chat-completions written as v1)
    ...
    #[test]
    fn upgrade_legacy_reasoning_singular_ainxt_build_shape() {
    ...
    fn upgrade_then_fold_through_conversation_to_chat_messages() {
        ...
        // assistant's `reasoning_content`. This matches what the real
    ...
    fn patch_reasoning_text_types_preserves_existing_type() {
        ...
        // Existing discriminators preserved unchanged (no clobber).
        assert_eq!(
    ...
    fn build_responses_input_single_reasoning_sibling_lands_inline() {
        ...
        // The reasoning item must carry encrypted_content unchanged.
        assert_eq!(
