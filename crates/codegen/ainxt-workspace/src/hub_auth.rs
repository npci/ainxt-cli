//! Hub [`AuthProvider`] from `~/.ainxt/auth.json` for the standalone
...
//!
//! The in-leader `ainxt workspace` exposure does NOT use this path — it sources
...
fn default_auth_path() -> anyhow::Result<PathBuf> {
    let ainxt = ainxt_config::user_ainxt_home()
        .ok_or_else(|| anyhow::anyhow!("no user ainxt home (set $AINXT_HOME or $HOME)"))?;
    Ok(ainxt.join("auth.json"))
}
...
fn read_auth_entry(path: &Path) -> anyhow::Result<(String, AuthEntry)> {
    if !path.exists() {
        anyhow::bail!(
            "No auth credentials found at {}. Run `ainxt login` first.",
            path.display()
        );
    }
    ...
    entries
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "no OIDC auth entry found in {}. Run `ainxt login` first.",
                path.display()
            )
        })
}
...
/// the default credential path (`~/.ainxt/auth.json`).
pub fn provider(
    hub_url: &Url,
...
mod tests {
    ...
    /// Reads a test-only secret fixture from the environment (populated via a
    /// local, untracked `.env` file; see `.env.example`). Keeping these
    /// values out of source avoids tripping secret scanners (e.g. Gitleaks)
    /// on strings that merely *look* like credentials.
    ...
    fn read_auth_entry_picks_oidc_entry() {
        ...
        let oidc_key = test_secret_from_env("TEST_OIDC_ENTRY_KEY");
        let path = write_auth_json(
            dir.path(),
            &format!(
                r#"{{
            "legacy": {{ "key": "ainxt-plainkey", "user_id": "u1" }},
            "oidc": {{
                "key": "{oidc_key}",
                "user_id": "u2",
                "refresh_token": "rt",
    ...
    fn read_auth_entry_rejects_non_oidc() {
        ...
        let path = write_auth_json(
            dir.path(),
            r#"{
            "api_key": { "key": "ainxt-plainkey", "user_id": "u1" }
        }"#,
        );
    ...
    fn read_auth_entry_tolerates_extra_fields() {
        let dir = tempfile::tempdir().unwrap();
        let scope_key = test_secret_from_env("TEST_SCOPE_ENTRY_KEY");
        let path = write_auth_json(
            dir.path(),
            &format!(
                r#"{{
    ...
    fn build_oidc_provider_requires_refresh_token() {
        let entry = AuthEntry {
            ...
            user_id: "u1".into(),
            refresh_token: None,
            oidc_issuer: Some("https://auth.example.test".into()),
    ...
    fn write_refreshed_token_updates_jwt_entry() {
        ...
        let path = write_auth_json(
            dir.path(),
            r#"{
            "legacy": { "key": "ainxt-old", "user_id": "u1" },
            "oidc": { "key": "eyJ.old", "user_id": "u2", "refresh_token": "rt-old", "oidc_issuer": "https://auth.example.test" }
        }"#,
        );
        ...
        assert_eq!(updated["legacy"]["key"], "ainxt-old"); // untouched
    }
    ...
    fn write_refreshed_token_targets_exact_scope_key() {
        ...
        let path = write_auth_json(
            dir.path(),
            r#"{
            "zzz": { "key": "eyJ.z", "refresh_token": "rt-z", "oidc_issuer": "https://auth.example.test" },
            "aaa": { "key": "eyJ.a", "refresh_token": "rt-a", "oidc_issuer": "https://auth.example.test" }
        }"#,
        );
    ...
    fn write_refreshed_token_preserves_existing_rt_when_not_rotated() {
        ...
        let path = write_auth_json(
            dir.path(),
            r#"{
            "oidc": { "key": "eyJ.old", "user_id": "u1", "refresh_token": "rt-keep", "oidc_issuer": "https://auth.example.test" }
        }"#,
        );
    ...
    fn provider_loopback_uses_bearer() {
        ...
        let path = write_auth_json(
            dir.path(),
            r#"{ "oidc": { "key": "eyJ.tok", "user_id": "u1", "refresh_token": "rt", "oidc_issuer": "https://auth.example.test", "oidc_client_id": "c1" } }"#,
        );
