...
/// sidecar/temp file path enumerated from the on-disk upload queue) in a log
/// record. Values are filtered character-by-character at format time via
...
static ORPHAN_RECOVERED: LazyLock<IntCounterVec> = LazyLock::new(|| {
    register_int_counter_vec!(
        "ainxt_workspace_orphan_recovered_total",
...
static ORPHAN_LOST: LazyLock<IntCounterVec> = LazyLock::new(|| {
    register_int_counter_vec!(
        "ainxt_workspace_orphan_lost_total",
...
static ORPHAN_EXPIRED: LazyLock<IntCounter> = LazyLock::new(|| {
    register_int_counter!(
        "ainxt_workspace_orphan_expired_total",
    ...
    for sidecar_path in sidecars {
        ...
        let sidecar = match std::fs::read(&sidecar_path) {
            Ok(bytes) => match serde_json::from_slice::<QueueItemSidecar>(&bytes) {
                ...
                Err(e) => {
                    tracing::warn!(
                        sidecar = %LogSafe(&sidecar_path.display().to_string()),
                        error = %e,
        ...
        let actual_sha = ainxt_file_utils::sha256_hex(&bytes);
...
mod tests {
    ...
    use ainxt_file_utils::queue::{TraceExportSource, UploadRetryPolicy, sidecar_path_for};
    ...
    fn write_orphan_pair(
        ...
        let sidecar = QueueItemSidecar {
            ...
            enqueued_at,
            sha256: ainxt_file_utils::sha256_hex(content),
        };
