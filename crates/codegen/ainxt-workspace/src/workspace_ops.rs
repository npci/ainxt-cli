...
use ainxt_computer_hub_sdk::ToolHarness;
use ainxt_tools::types::output::ToolRunResult;
use ainxt_workspace_client::{WorkspaceClient, is_transport_fatal};
pub use ainxt_workspace_types::rpc::WorkspaceRpc;
pub use ainxt_workspace_types::rpc::agents_md::DiscoverAgentsMdReq;
pub use ainxt_workspace_types::rpc::code_nav::{
...
pub use ainxt_workspace_types::rpc::fs::{
...
pub use ainxt_workspace_types::rpc::git::{
...
pub use ainxt_workspace_types::rpc::hooks::{
    HookEventNameWire, HookRegistryReq, HookRegistryWire, HookSpecWire,
};
pub use ainxt_workspace_types::rpc::hunks::{
...
pub use ainxt_workspace_types::rpc::search::{FuzzyChangeReq, FuzzyCloseReq, FuzzyOpenReq};
pub use ainxt_workspace_types::rpc::session::{BeginPromptReq, EndPromptReq, RewindToReq};
pub use ainxt_workspace_types::rpc::skills::DiscoverSkillsReq;
pub use ainxt_workspace_types::rpc::workspace::WorkspaceInfoReq;
pub use ainxt_workspace_types::rpc::worktree::{
...
fn hunk_line_info_to_wire(info: &ainxt_hunk_tracker::types::HunkLineInfo) -> HunkLineInfoWire {
    HunkLineInfoWire {
...
fn hunk_source_to_wire(source: ainxt_hunk_tracker::types::HunkSource) -> HunkSourceWire {
    use ainxt_hunk_tracker::types::HunkSource as S;
    match source {
...
fn hunk_to_wire(hunk: &ainxt_hunk_tracker::types::Hunk) -> HunkWire {
    HunkWire {
...
fn file_content_status_to_wire(
    status: ainxt_hunk_tracker::types::FileContentStatus,
) -> FileContentStatusWire {
    use ainxt_hunk_tracker::types::FileContentStatus as S;
    match status {
...
fn file_content_view_to_wire(
    view: ainxt_hunk_tracker::types::FileContentView,
) -> FileContentViewWire {
    FileContentViewWire {
...
fn file_content_entry_to_wire(entry: ainxt_hunk_tracker::FileContentEntry) -> FileContentEntryWire {
...
fn session_stats_to_wire(stats: &ainxt_hunk_tracker::types::SessionStats) -> SessionStatsWire {
    SessionStatsWire {
...
fn turn_summary_to_wire(turn: ainxt_hunk_tracker::types::TurnSummary) -> TurnSummaryWire {
    TurnSummaryWire {
...
fn session_summary_to_wire(summary: ainxt_hunk_tracker::SessionSummary) -> SessionSummaryWire {
    SessionSummaryWire {
...
fn tracker_action(kind: HunkActionKind) -> ainxt_hunk_tracker::types::HunkAction {
    match kind {
        HunkActionKind::Accept => ainxt_hunk_tracker::types::HunkAction::Accept,
        HunkActionKind::Reject => ainxt_hunk_tracker::types::HunkAction::Reject,
    }
}
...
fn session_tracker(
    ws: &WorkspaceHandle,
    session_id: Option<&str>,
) -> WorkspaceResult<ainxt_hunk_tracker::HunkTrackerHandle> {
    let sid = session_id
...
impl WorkspaceOp for HunkSingleActionReq {
    async fn execute(
        ...
        let hunk_id = ainxt_hunk_tracker::types::HunkId::from_string(self.action.hunk_id.clone());
    ...
    async fn execute(
        ...
        for h in &all_hunks {
            ...
            let is_agent = matches!(
                h.source,
                ainxt_hunk_tracker::types::HunkSource::AgentEdit { .. }
            );
...
/// isn't possible; the round-trip is faithful because the wire type matches the
/// serde shape exactly (the compiled `matcher` is `#[serde(skip)]` either way).
fn hook_registry_to_wire(
    registry: &ainxt_hooks::discovery::HookRegistry,
...
) -> WorkspaceResult<ainxt_hooks::discovery::HookRegistry> {
...
fn resolve_index_for_workspace(
    ...
    std::sync::Arc<ainxt_codebase_graph::IndexManagerHandle>,
    std::path::PathBuf,
)> {
...
fn query_result_to_response(
    result: Result<ainxt_codebase_graph::QueryResult, ainxt_codebase_graph::QueryError>,
) -> CodeNavResponse {
    match result {
...
fn symbol_locations_to_response(
    locations: Vec<ainxt_codebase_graph::SymbolLocation>,
) -> CodeNavResponse {
    CodeNavResponse {
        locations: locations
            .into_iter()
...
///   session's [`FinalizedToolset`](ainxt_tools::registry::types::FinalizedToolset).
...
impl WorkspaceOps {
    ...
    pub fn bind_local_session(
        ...
        hunk_tracker: ainxt_hunk_tracker::HunkTrackerHandle,
    ...
    pub async fn hook_registry(&self) -> WorkspaceResult<ainxt_hooks::discovery::HookRegistry> {
    ...
    ///   [`FinalizedToolset`](ainxt_tools::registry::types::FinalizedToolset)
        ...
        match self {
            ...
            Self::Proxy { client } => {
                if !client.is_connected() {
                    return Err(ainxt_tool_runtime::ToolError::network_error(
                        "The workspace server connection was lost. \
                         Please restart your session to reconnect.",
                    ));
                }
                let tool_id = ainxt_tool_protocol::ToolId::new(name).map_err(|e| {
                    ainxt_tool_runtime::ToolError::custom(
                        "hub_proxy_error",
                        format!("invalid tool name: {e}"),
                    )
                })?;
                let mut ctx = ainxt_tool_runtime::ToolCallContext::default();
                ctx.call_id =
                    ainxt_tool_protocol::ToolCallId::new(call_id.to_owned()).unwrap_or(ctx.call_id);
...
mod tests {
    ...
    /// request types are defined in `ainxt-workspace-types` (and re-exported
    ...
    fn git_op_cwd_uses_explicit_git_root_per_window() {
        ...
        let window_a = std::path::PathBuf::from("/repos/ainxt-main");
        let window_b = std::path::PathBuf::from("/repos/ainxt-main-2");
        assert_eq!(
    ...
    async fn end_local_session_drops_bound_toolset() {
        ...
        let toolset = std::sync::Arc::new(
            ainxt_tools::registry::types::FinalizedToolset::empty_for_test(),
        );
    ...
    /// A `Hunk`'s wire mirror serializes exactly like the heavy type.
    #[test]
    fn hunk_to_wire_serializes_identically() {
        use ainxt_hunk_tracker::types::{Hunk, HunkSource};
    ...
    fn file_content_entry_to_wire_serializes_identically() {
        use ainxt_hunk_tracker::FileContentEntry;
        use ainxt_hunk_tracker::types::FileContentView;
    ...
    /// A `SessionSummary` (with a turn carrying a hunk) matches identically.
    #[test]
    fn session_summary_to_wire_serializes_identically() {
        use std::sync::Arc;
        use ainxt_hunk_tracker::SessionSummary;
        use ainxt_hunk_tracker::types::{Hunk, HunkSource, TurnSummary};
    ...
    fn hook_registry_wire_round_trip_both_directions() {
        let spec = ainxt_hooks::config::HookSpec {
            ...
            event: ainxt_hooks::event::HookEventName::PreToolUse,
            ...
            url_raw: None,
            timeout_ms: 5000,
            source_dir: std::path::PathBuf::from("/home/u/.ainxt/hooks"),
            extra_env: std::collections::HashMap::from([("FOO".to_string(), "bar".to_string())]),
        };
        let mut registry = ainxt_hooks::discovery::HookRegistry::default();
    ...
    fn hook_event_name_wire_covers_all_upstream_variants() {
        use ainxt_hooks::event::HookEventName as E;
    ...
    /// can't depend on `ainxt-hooks`, and `hook_registry_to_wire` only couples
    ...
    fn hook_spec_wire_covers_all_upstream_fields() {
        use ainxt_hooks::config::HookSpec;
