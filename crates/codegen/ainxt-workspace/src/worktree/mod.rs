...
//!
//! Moved from `ainxt-shell/src/session/worktree.rs` into the workspace
...
use ainxt_fast_worktree::{BtrfsDelegate, IgnoredFilesMode, WorkingTreeMode, WorktreeBuilder};

use crate::diag_util::session_log_tag;
...
// Canonical in ainxt-workspace-types; re-exported for existing paths.
...
const WORKTREE_LOG: &str = "ainxt_worktree";

/// Map a [`WorktreeType`] to the fast-worktree crate's `CreationMode`.
pub(crate) fn to_creation_mode(t: WorktreeType) -> ainxt_fast_worktree::CreationMode {
    match t {
        WorktreeType::Linked => ainxt_fast_worktree::CreationMode::Linked,
        WorktreeType::Standalone => ainxt_fast_worktree::CreationMode::Standalone,
        WorktreeType::Git => ainxt_fast_worktree::CreationMode::GitCheckout,
    }
}
...
/// Resolve the ainxt home for worktree paths via the **same** resolver used for
/// `worktrees.db` (`ainxt_fast_worktree::resolve_ainxt_home`), so checkout dirs and
/// the metadata DB always live under the same `.ainxt` tree. That resolver
/// canonicalizes its `$HOME` fallback to match `ainxt_config::ainxt_home()`,
/// so worktree paths also agree with trust/hooks and other ainxt-home paths.
fn ainxt_home() -> std::path::PathBuf {
    ainxt_fast_worktree::resolve_ainxt_home().unwrap_or_else(|_| {
        dirs::home_dir()
...
/// Returns `~/.ainxt/worktrees/<repo_slug>` for the given git root.
///
...
pub fn worktree_base_dir(git_root: &Path) -> std::path::PathBuf {
    ...
    ainxt_home().join("worktrees").join(slug)
}

/// Resolves the worktree base directory (`~/.ainxt/worktrees/<repo_name>`)
/// for a given source path, correctly handling ainxt-managed worktrees.
///
/// When `source_path` is already under `~/.ainxt/worktrees/<repo>/...`, the
...
/// original repo).
///
/// For paths outside the ainxt worktree directory, falls back to
...
pub fn worktree_base_dir_for_source(source_path: &Path) -> Result<std::path::PathBuf> {
    let worktrees_dir = ainxt_home().join("worktrees");
...
/// Walk up from `cwd` (staying within `~/.ainxt/worktrees/`) to its registered
/// worktree record.
///
...
/// The recorded source repo of the ainxt-managed worktree containing `cwd`, if any.
///
/// Thin wrapper over [`worktree_record_for_cwd`] that drops the DB handle;
/// returns `None` (without DB I/O) for paths outside `~/.ainxt/worktrees/`.
...
pub async fn create_worktree_streaming<N: WorktreeNotificationSender>(
    ...
    let git_root = match find_main_repo_root_from_path(source_path) {
        ...
        Err(e) => {
            tracing::warn!(
                target: WORKTREE_LOG,
                session_tag = %session_log_tag(&session_id),
    ...
    // Map WorktreeCopyMode to ainxt_fast_worktree::WorkingTreeMode
    ...
    // Use ainxt-fast-worktree for high-performance worktree creation
    ...
    let report = match tokio::task::spawn_blocking(move || {
        let mut builder = WorktreeBuilder::new(&source_path, &dest_path)
            ...
            .worktree_kind(ainxt_fast_worktree::WorktreeKind::Session)
    ...
    let elapsed_ms = start.elapsed().as_millis() as u64;
    tracing::debug!(
        session_tag = %session_log_tag(&req.session_id),
        path = %absolute_path,
...
pub async fn remove_worktree(
    ...
    match tokio::task::spawn_blocking(move || {
        ainxt_fast_worktree::remove_worktree_with_delegate(&wt_path, delegate)
    })
    .await
    {
        Ok(Ok(report)) => {
...
/// re-registered DB record (create-path equivalence). Returns the rehydrated worktree
...
pub async fn rehydrate_subagent_worktree(
    ...
    let report = tokio::task::spawn_blocking(move || {
        ainxt_fast_worktree::rehydrate_worktree_from_ref(
            &dest,
            &source_repo,
            &snapshot_ref,
            session_id.as_deref(),
...
pub async fn snapshot_subagent_worktree(
    ...
    tokio::task::spawn_blocking(move || -> Result<String> {
        ...
        ainxt_fast_worktree::snapshot_worktree_to_ref(&worktree_path, &ref_name, &message)?;
        ainxt_fast_worktree::transfer_snapshot_to_repo(&worktree_path, &source_repo, &ref_name)?;
        Ok(ref_name)
    })
    .await
...
pub async fn remove_subagent_worktree(worktree_path: &Path) -> Result<()> {
    ...
    tokio::task::spawn_blocking(move || {
        ainxt_fast_worktree::remove_worktree_with_delegate(&worktree_path, delegate)
    })
    .await
...
///
/// When the source path is already inside `~/.ainxt/worktrees/<repo>/`, the
...
async fn cleanup_cancelled_worktree(worktree_path: &str) {
    ...
    let delegate = btrfs_delegate_from_env();
    match tokio::task::spawn_blocking(move || {
        ainxt_fast_worktree::remove_worktree_with_delegate(&path, delegate)
    })
    .await
    {
        Ok(Ok(_)) => {}
...
pub async fn create_worktree_from_worktree_streaming<N: WorktreeNotificationSender>(
    ...
    // Use ainxt-fast-worktree -- it handles copying from any worktree path
    ...
    let builder_result = {
        ...
        tokio::task::spawn_blocking(move || {
            let mut builder = WorktreeBuilder::new(&source_worktree_path, &dest_path)
                ...
                .ignored_files_mode(IgnoredFilesMode::Skip)
                .creation_mode(to_creation_mode(creation_mode))
                .worktree_kind(ainxt_fast_worktree::WorktreeKind::Fork)
                .session_id(session_id_for_builder)
                .metadata(label_metadata);
    ...
    let elapsed_ms = start.elapsed().as_millis() as u64;
    tracing::debug!(
        session_tag = %session_log_tag(&req.new_session_id),
        source = %req.source_worktree_path,
        path = %absolute_path,
        commit = %report.commit,
        copy_mode = ?req.copy_mode,
...
///
/// ACP equivalent of `ainxt -w -r <session_id>` (optionally with `--ref`).
...
/// Response from `ainxt.dev/git/worktree/resume_session`.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Response from `ainxt.dev/session/rehydrate`.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
use ainxt_fast_worktree::{
    DbStats, GcOptions, GcReport, ListFilter, WorktreeDb, WorktreeKind, WorktreeRecord,
    gc_worktrees as fw_gc_worktrees, rebuild_worktree_db, resolve_ainxt_home,
};
...
pub fn worktree_db_rebuild() -> Result<ainxt_fast_worktree::RebuildReport> {
    let home = resolve_ainxt_home()?;
...
pub fn candidate_worktree_cwds_for_same_repo(current_cwd: &std::path::Path) -> Result<Vec<String>> {
    ...
    let db_records = match open_db() {
        Ok(db) => {
            let filter = ainxt_fast_worktree::ListFilter {
...
/// Scan `~/.ainxt/worktrees/<repo_name>/` for subdirectories not tracked
...
pub fn build_candidate_list(
    current_cwd: &str,
    main_repo_root: &str,
    db_records: &[ainxt_fast_worktree::WorktreeRecord],
    fs_paths: &[String],
) -> Vec<String> {
...
mod tests {
    ...
    /// (which stores them unchanged), so this guards the helper RPC timeout
    ...
    async fn snapshot_and_remove_captures_then_deletes() {
        ...
        let ref_name = "refs/ainxt/subagents/dispose-1";
    ...
    async fn snapshot_and_remove_then_rehydrate_round_trips() {
        ...
        let ref_name = "refs/ainxt/subagents/dispose-2";
    ...
    async fn snapshot_standalone_worktree_durable_after_removal_round_trips() {
        ...
        let ref_name = "refs/ainxt/subagents/standalone-1";
    ...
    async fn snapshot_failure_preserves_worktree() {
        ...
        let result = snapshot_and_remove_subagent_worktree(
            &not_a_worktree,
            &not_a_worktree,
            "refs/ainxt/subagents/dispose-3",
        )
        .await;
    ...
    /// Point `AINXT_HOME` at an isolated tempdir (`resolve_ainxt_home` re-reads
    ...
    /// and restores `AINXT_HOME` on drop (before releasing the lock), so the
    ...
    fn worktree_db_fixture(
        ...
        let env = LockedTestEnv::lock().set("AINXT_HOME", &home);
        ...
        let record = WorktreeRecord {
            ...
            status: ainxt_fast_worktree::WorktreeStatus::Alive,
    ...
    async fn cleanup_cancelled_worktree_removes_dir_and_deregisters() {
        ...
        // can't clobber this row (AinxtHomeFixture is not visible across crates).
