...
//! access token from `~/.ainxt/auth.json` does (same identity).
...
use url::Url;
use uuid::Uuid;
use ainxt_computer_hub_sdk::pool::HubConnectionPool;
use ainxt_computer_hub_sdk::{AuthCredential, ToolHarnessBuilder};
...
async fn call_tool(
    harness: &ainxt_computer_hub_sdk::ToolHarness,
    name: &str,
    args: Value,
) -> anyhow::Result<Value> {
...
async fn connect_and_bind(
...
) -> anyhow::Result<ainxt_computer_hub_sdk::ToolHarness> {
    ...
    let metadata = json!({
        "tools": [
            {"id": "AinxtBuild:run_terminal_cmd", "name_override": "run_terminal_command"},
            {"id": "AinxtBuild:read_file"},
        ],
    });
    let tools = harness
