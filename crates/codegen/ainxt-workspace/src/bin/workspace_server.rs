...
//!
//! Reads OIDC credentials from `~/.ainxt/auth.json`, connects to a
//! server, exposes workspace tools, and refreshes tokens
//! automatically.
use clap::Parser;
use std::path::PathBuf;
use url::Url;
use ainxt_workspace::config::WorkspaceServerMetadata;
use ainxt_workspace::daemonize;
use ainxt_workspace::diag_server;
use ainxt_workspace::preview_supervisor::{self, PreviewArgs, PreviewVisibility};
...
const SERVICE_NAME: &str = "prod_ainxt_workspace";
...
fn server_id_startup_error(id: &str) -> Option<String> {
    id.parse::<ainxt_tool_protocol::ServerId>()
        .err()
        .map(|e| format!("{INVALID_SERVER_ID_MARKER} {id:?}: {e}"))
}
#[derive(Parser)]
#[command(name = "ainxt-workspace-server")]
...
struct Args {
    ...
    #[arg(long)]
    capabilities: bool,
    #[arg(long, default_value = "wss://computer-hub.example.test/v1/tools")]
    hub_url: String,
    #[arg(long)]
    ...
    /// `AINXT_WORKSPACE_UPLOAD_QUEUE_ENABLED` env var to `false`) to fall back to
    /// the legacy inline path. Accepts `true`/`false`.
    #[arg(
        long,
        env = "AINXT_WORKSPACE_UPLOAD_QUEUE_ENABLED",
    ...
    /// Confine `ainxt.dev/fs/*` resolution to the workspace root (reject `..`,
    ...
    /// Override with `AINXT_WORKSPACE_CONFINE_FS_TO_ROOT=false` (e.g. local dev).
    #[arg(
        long,
        env = "AINXT_WORKSPACE_CONFINE_FS_TO_ROOT",
        default_value_t = true,
        action = clap::ArgAction::Set,
    )]
...
/// `/usr/local/bin/ainxt-preview-proxy` child (see `cli.rs` for the proxy's
/// flag names). Off by default — when `--preview-enabled` is absent the
/// supervisor is never started and startup is exactly the non-preview
/// path.
...
async fn run(args: Args, cwd: PathBuf) -> anyhow::Result<()> {
    ...
    let donating = ainxt_computer_hub_sdk::DonatingLogLayer::new_inert();
    tracing_subscriber::registry()
        .with(env_filter)
        .with(tracing_subscriber::fmt::layer())
        .with(donating.clone())
        .init();
    let direct_otlp = match std::env::var("AINXT_WORKSPACE_OTLP_ENDPOINT") {
        Ok(endpoint) if !endpoint.is_empty() => {
            match ainxt_tracing::init_fastrace(endpoint.clone(), SERVICE_NAME.to_owned(), None) {
                Ok(()) => {
    ...
    {
        use ainxt_sandbox::{ProfileName, SandboxManager};
        let profile = match std::env::var("AINXT_SANDBOX_PROFILE").ok() {
            Some(val) => {
                let parsed = val
                    .parse::<ProfileName>()
                ...
                if matches!(parsed, ProfileName::Custom(_)) {
                    tracing::warn!(
                        value = % val,
                        "Unrecognized AINXT_SANDBOX_PROFILE, defaulting to workspace"
                    );
            ...
            None if ainxt_sandbox::trust_bwrap_marker_for_devbox() => ProfileName::Devbox,
        ...
        if profile == ProfileName::Off {
            tracing::info!(
                ...
                "Sandbox explicitly disabled via AINXT_SANDBOX_PROFILE=off"
            );
        } else {
            ...
            sandbox.install();
            let active = ainxt_sandbox::is_active();
            ...
            tracing::info!(
                ...
                ainxt_sandbox::should_restrict_child_network(), "{status_msg}"
            );
        }
    }
    let auth_provider = ainxt_workspace::hub_auth::provider(&url, args.auth_config.as_deref())?;
    tracing::info!(hub_url = % url, cwd = % cwd.display(), "Starting workspace server");
    let cwd_display = cwd.display().to_string();
    let session_id = std::env::var("AINXT_SESSION_ID").ok();
    ...
    ainxt_workspace::init_metrics();
    let ws_handle = ainxt_workspace::handle::connect_local_workspace(
        cwd,
        url,
        auth_provider,
        metadata,
    ...
    let grace_budget = ainxt_workspace::handle::termination_grace_from_env();
    ws_handle
        .two_phase_drain(
            grace_budget,
            ainxt_workspace::handle::DrainReason::Sigterm,
        )
        .await;
    ...
    ainxt_computer_hub_sdk::flush_log_layer();
...
mod tests {
    ...
    fn capabilities_flag_parses_and_defaults_off() {
        let args = Args::try_parse_from(["ainxt-workspace-server"]).unwrap();
        assert!(!args.capabilities);
        let args = Args::try_parse_from(["ainxt-workspace-server", "--capabilities"]).unwrap();
        assert!(args.capabilities);
    }
    #[test]
    ...
    fn capabilities_probe_of_legacy_binary_exits_nonzero() {
        ...
        let err = LegacyArgs::try_parse_from(["ainxt-workspace-server", "--capabilities"])
    ...
    fn ready_file_is_accepted_as_a_deprecated_no_op() {
        let args =
            Args::try_parse_from(["ainxt-workspace-server", "--ready-file", "/tmp/x.ready"]).unwrap();
    ...
    fn argv_rejection_exit_code_is_distinct_from_server_id_exit_code() {
        let err = Args::try_parse_from(["ainxt-workspace-server", "--flag-from-the-future"])
            .err()
    ...
    fn preview_flags_parse_and_lower_to_supervisor_config() {
        let args = Args::try_parse_from([
            ...
            "https://ainxt.example.test/preview-auth",
        ...
        assert_eq!(
            ...
            Some("https://ainxt.example.test/preview-auth")
        );
