...
use ainxt_file_utils::gcs::StorageConfig;
use ainxt_file_utils::queue::{EnqueueOutcome, TraceExportSource, UploadQueue};
use ainxt_file_utils::storage_client::Auth401AttributionCallback;
use ainxt_file_utils::{TraceExportConfig, UploadMethod};
use ainxt_auth::{AuthCredentialProvider, CredentialSnapshot};
...
static UPLOAD_QUEUE_PENDING_BYTES: LazyLock<IntGauge> = LazyLock::new(|| {
    register_int_gauge!(
        "ainxt_workspace_upload_queue_pending_bytes",
...
static UPLOAD_QUEUE_PENDING: LazyLock<IntGauge> = LazyLock::new(|| {
    register_int_gauge!(
        "ainxt_workspace_upload_queue_pending",
...
static UPLOAD_OUTCOME_TOTAL: LazyLock<IntCounterVec> = LazyLock::new(|| {
    register_int_counter_vec!(
        "ainxt_workspace_upload_outcome_total",
...
static UPLOAD_FAILED_TOTAL: LazyLock<IntCounterVec> = LazyLock::new(|| {
    register_int_counter_vec!(
        "ainxt_workspace_upload_failed_total",
...
static UPLOAD_SKIPPED_TOTAL: LazyLock<IntCounterVec> = LazyLock::new(|| {
    register_int_counter_vec!(
        "ainxt_workspace_upload_skipped_total",
...
/// Spawn a detached sampler that matches the queue's pending/pending-bytes
...
impl ainxt_auth::visibility::HttpAuth for HubAuthCredentialProvider {
...
mod tests {
    use super::*;
    use ainxt_computer_hub_sdk::auth::AuthCredential;
    ...
    /// `dc_log!` pins the target, honors the level, keeps the message a unchanged
