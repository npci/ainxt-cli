...
impl WorkspaceIdentity {
    ...
    /// `AinxtAuth.team_id`, and for a Team principal `principal_id` *is* the
    /// team id.
...
/// [`AuthIdentity`](ainxt_computer_hub_sdk::AuthIdentity). The two types carry the
...
impl From<ainxt_computer_hub_sdk::AuthIdentity> for WorkspaceIdentity {
    fn from(id: ainxt_computer_hub_sdk::AuthIdentity) -> Self {
...
pub struct WorkspaceEnvironment {
    ...
    /// Version of the `ainxt-workspace` crate that produced the record.
    ...
    /// Sandbox profile name (`$AINXT_SANDBOX_PROFILE`), e.g. `"devbox"`.
...
impl WorkspaceEnvironment {
    ...
    pub(crate) fn capture(
        ...
        Self::assemble(
            ...
            server_id,
            sandbox_id,
            std::env::var("AINXT_SANDBOX_PROFILE").ok(),
            ainxt_sandbox::is_inside_bwrap(),
...
mod tests {
    ...
    fn assemble_populates_every_field() {
        let env = WorkspaceEnvironment::assemble(
            ...
            Some("git@github.com:ainxt-org/example.git".to_string()),
        );
        ...
        assert_eq!(
            ...
            Some("git@github.com:ainxt-org/example.git")
        );
    ...
    fn serializes_with_all_fields_present() {
        let env = WorkspaceEnvironment::assemble(
            ...
            Some("host-1".to_string()),
            Some("/work/repo".to_string()),
            Some("https://github.com/ainxt-org/example".to_string()),
        );
    ...
    fn git_repo_facts_strips_remote_url_credentials() {
        ...
        assert_eq!(url, "https://github.com/ainxt-org/example.git");
    }
}
