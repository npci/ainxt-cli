...
//!
//! Port of common agent auto-permission classifier semantics adapted to ainxt's
...
impl ClassifierContext {
    ...
    /// conservative broadening (only adds matches), not a strict-equivalence claim.
...
/// mapped to ainxt access kinds + known names).
...
/// model is constrained to emit conforming JSON — equivalence with a forced
...
pub fn mcp_access_detail(name: &str, input: &serde_json::Value) -> String {
    ...
    ainxt_tools::util::truncate_line(&format!("{name} {compact}"), MCP_ACCESS_DETAIL_MAX_LEN)
        .into_owned()
}
...
pub fn permission_decision_args(access: &AccessKind, access_detail: Option<&str>) -> String {
    ...
    ainxt_tools::util::truncate_str_with_marker(&raw, CLASSIFIER_TURN_MAX_LEN).into_owned()
}
