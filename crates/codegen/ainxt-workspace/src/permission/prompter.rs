...
use ainxt_file_utils::events::{Event, EventWriter, PermissionDecision};
use ainxt_tools::implementations::ainxt_build::web_fetch::domain_from_url;

const REJECT_ONCE_LABEL: &str = "No, and tell ainxt what to do differently";
...
///        `~/.ainxt/config.toml` via the `Effect::PersistPermissionMode` effect
///     4. Sends the existing `ainxt.dev/yolo_mode_changed` ACP notification so
...
/// (global yolo) option that is prepended for AinxtTUI / AinxtPager / Desktop.
///
...
/// (`AinxtTUI`, `AinxtPager`, `Desktop`) that wire the option id through
...
fn client_supports_enable_always_approve(client_type: ClientType) -> bool {
    matches!(
        client_type,
        ClientType::AinxtTUI | ClientType::AinxtPager | ClientType::Desktop
    )
}
...
/// Canonical definition lives in `ainxt_workspace_types` (so both the
/// permission-validation layer and the MCP transport in `ainxt-mcp` can
...
/// reached `ainxt_workspace::permission::MCP_TOOL_NAME_DELIMITER`.
...
/// `"list_issues"` → `"List Issues"`, `"ainxt_com_notion"` →
/// `"ainxt Com Notion"`, and `"getMyTaskList"` → `"GetMyTaskList"`.
...
/// or scrollback blocks that store the wire name unchanged). Splits on
...
pub struct McpToolPermission {
    ...
    /// e.g. `"Always allow:"`. Matches `BashCommandPermission::prompt_prefix`.
    pub prompt_prefix: String,
    /// Full tool name as the agent called it
    /// (e.g. `"ainxt_com_notion__notion-fetch"`).
    pub tool_name: String,
    /// Server segment (everything before the single `__` separator,
    /// e.g. `"ainxt_com_notion"`). `None` if the tool name has no `__`,
...
impl AcpPrompter {
    pub fn new(
        ...
        // Bash options for AinxtTUI - interactive selection with expandable/contractable terms
    ...
    fn build_options_inner(
        ...
        match access {
            ...
            AccessKind::Bash(bash_command) => {
                // For AinxtTUI clients, use the fancy interactive options with term selection
...
mod tests {
    ...
    fn gate_off_strips_bash_always_allow_keeps_yes_no() {
        let p = prompter_with_gate(ClientType::AinxtPager, false);
    ...
    fn gate_on_includes_bash_always_allow() {
        let p = prompter_with_gate(ClientType::AinxtPager, true);
        let access = AccessKind::Bash("kubectl get pods".to_owned());
        let opts = p.build_options(&access);
        assert!(
            has_option(&opts, "allow-always-command"),
    ...
    fn gate_off_strips_generic_bash_always_and_reject_always() {
        let p = prompter_with_gate(ClientType::AinxtWeb, false);
        let access = AccessKind::Bash("kubectl get pods".to_owned());
        let opts = p.build_options(&access);
    ...
    fn bash_meta_present_only_when_gate_on_for_fancy_clients() {
        ...
        let on = prompter_with_gate(ClientType::AinxtPager, true);
    ...
    fn bash_reject_always_command_maps_selected_words() {
        let p = prompter(ClientType::AinxtPager);
    ...
    fn mcp_prompt_includes_allow_always_with_meta() {
        let p = prompter(ClientType::AinxtTUI);
        let access = AccessKind::MCPTool {
            name: "linear__list".to_owned(),
            input: serde_json::Value::Null,
        };
    ...
    fn mcp_fallback_client_returns_plain_allow_always() {
        // non-TUI clients (Generic / AinxtWeb / Extension / …) see `fallback_options`. The
    ...
    fn mcp_titleize_segment_handles_snake_camel_kebab() {
        ...
        // NOTE: mcp_titleize_segment has no special-case lowercase exception
        // for the "ainxt" brand word (unlike the CLI binary name / about
        // text elsewhere in this codebase) -- it capitalizes every
        // underscore-delimited word uniformly. Asserting on that actual
        // behavior; a branding-consistency pass may want to special-case
        // this later.
        assert_eq!(mcp_titleize_segment("ainxt_com_notion"), "Ainxt Com Notion");
    ...
    /// Same pin as above for `AinxtTUI` and `Desktop` — both client types
    ...
    fn enable_always_approve_is_first_for_tui_and_desktop() {
        for ct in [ClientType::AinxtTUI, ClientType::Desktop] {
            let p = prompter(ct);
