...
use ainxt_tools::implementations::ainxt_build::web_fetch::{
...
fn permission_mode_artifact_str(mode: ainxt_telemetry::enums::PermissionMode) -> &'static str {
    use ainxt_telemetry::enums::PermissionMode;
    match mode {
...
impl PermissionHandle {
    ...
    pub async fn request(
        ...
        // The enforcement point runs before the handle variant is even
        // considered. That placement is the whole guarantee: `AllowAll`, YOLO,
        // session grants, auto-mode and the sandbox bash auto-allow are all
        // strictly downstream from here, so each can only narrow this outcome
        // and none can widen it. Inert when no policy bundle is installed.
        let pep_forced_prompt = match crate::permission::pep_gate::evaluate(
            &access,
            session_id.as_deref(),
            subagent_type.as_deref(),
        ) {
            crate::permission::pep_gate::PepOutcome::Deny(reason) => return Decision::PolicyDeny(reason),
            crate::permission::pep_gate::PepOutcome::ForcedPrompt(_) => true,
            crate::permission::pep_gate::PepOutcome::Proceed => false,
        };

        match self {
            PermissionHandle::AllowAll => {
                // There is no human on this path, so a forced prompt has
                // nowhere to go and degrades to a denial — the same reasoning
                // the Sovereign gate below already applies. The agent sees
                // `PolicyDeny` and adapts.
                if pep_forced_prompt {
                    return Decision::PolicyDeny(
                        "this action requires interactive human approval; \
                         not permitted under allow-all"
                            .to_owned(),
                    );
                }
                // INV-5: even an allow-all handle cannot auto-approve a Sovereign
                // action. There is no human on this path, so it is denied (the
                // agent sees `PolicyDeny` and adapts). Gated on enforcing policy
                // so non-enforcing/OSS contexts behave exactly as before.
                if let Some(action) = crate::permission::sovereign::classify(&access) {
                    let engine = ainxt_policy::global::active();
                    if engine.is_enforcing() && !engine.can_auto_approve(action) {
                        return Decision::PolicyDeny(
                            "sovereign action requires interactive human approval; \
                             not permitted under allow-all (INV-5)"
                                .to_owned(),
                        );
                    }
                }
...
fn spawn_permission_manager_with_pin(
    ...
    let _task = tokio::task::spawn_local(async move {
        ...
        // persist it to ~/.ainxt/sessions/<cwd>/permission.toml. This caused
        // the allow to survive full restarts (new ainxt process, new agent
        ...
        while let Some(cmd) = rx.recv().await {
            match cmd {
                ...
                PermissionCommand::Request {
                    ...
                    // INV-5: a classified Sovereign action can never be
                    // auto-approved. Join it to the existing forced-prompt gates
                    // so YOLO / session grants / auto-mode all fall through to the
                    // interactive human prompt. Gated on enforcing policy so OSS /
                    // dev ergonomics are unchanged (Off/Warn → advisory only).
                    let sovereign_action = crate::permission::sovereign::classify(&access);
                    // The enforcement point's forced prompt joins this existing
                    // gate rather than introducing a parallel one. This is the
                    // disjunction every auto-approve path already consults;
                    // `policy_forced_prompt` is *not* (it is read only by the
                    // MCP arm below, by design), so routing through that one
                    // would silently let Bash / Edit / WebFetch fall through.
                    let sovereign_forced_prompt = pep_forced_prompt || {
                        let engine = ainxt_policy::global::active();
                        engine.is_enforcing()
                            && matches!(sovereign_action, Some(a) if !engine.can_auto_approve(a))
                    };
                    if sovereign_forced_prompt {
                        tracing::info!(
                            tool = %tool_name,
                            ?sovereign_action,
                            "INV-5: sovereign action forced to interactive prompt (auto-approve suppressed)"
                        );
                    }
                    ...
                    if yolo_mode && !shell_forced_prompt && !sovereign_forced_prompt {
                    ...
                    // An active sandbox is not on its own a reason to skip the
                    // human: `nono` has no execute AccessMode, and on Windows
                    // there is no sandbox at all, so "sandboxed therefore safe"
                    // does not hold for exec. Adding the forced-prompt gate
                    // here means the capability decision upstream wins — when
                    // the enforcement point wanted a human, sandbox presence no
                    // longer silently supplies the approval instead.
                    if matches!(&access, AccessKind::Bash(_))
                        && ainxt_sandbox::should_auto_allow_bash()
                        && !policy_forced_prompt
                        && !auto_forced_prompt
                        && !sovereign_forced_prompt
                    {
                    ...
                    let (decision, outcome_str, user_prompted) = match &access {
                        ...
                        _ => {
                            ...
                            let (decision, outcome_str) = match &prompt_outcome {
                                ...
                                PromptOutcome::AllowAlways => {
                                    // Fallback clients (Generic / AinxtWeb /
...
mod tests {
    ...
    /// always-approve option is only offered for `AinxtTUI | AinxtPager | Desktop`).
    ...
    fn manager_with_recording_client_remember(
        ...
        let (gateway, receiver) = ainxt_acp_lib::acp_gateway::<acp::AgentSide, _>(client);
    ...
    // assertions translate unchanged while exercising the new word-based
    // helpers.

    /// Test shim: a script is "safe" iff `evaluate_bash_segments` returns
    /// `AutoAllow` against an empty permission state. Matches the previous
    ...
    mod mcp_pre_decision {
        ...
        fn pre_decision_policy_forced_prompt_overrides_tool_grant_when_gate_off() {
            ...
            // override a session tool-scope grant for MCP (hard floor). Matches
    ...
    async fn auto_classifier_block_prompt_still_offers_always_approve() {
        ...
        local
            .run_until(async {
                ...
                let client = RecordingClient::default();
                let prompts = client.prompts.clone();
                // AinxtPager wires the always-approve option through to its YOLO
                // toggle; it is the option set the auto path prompts under.
                let (mgr, _e) =
                    manager_with_recording_client(&cwd, None, client, ClientType::AinxtPager);
                mgr.set_auto_mode(true);
