...
pub enum ClientType {
    ...
    #[default]
    #[serde(rename = "generic", alias = "ainxt-shell", alias = "ainxt_shell")]
    Generic,
    /// ainxt TUI client - show fancy options with interactive bash term selection
    #[serde(rename = "ainxt-tui", alias = "ainxt_tui")]
    AinxtTUI,
    /// ainxt Web client - identified by clientIdentifier "ainxt-web"
    #[serde(rename = "ainxt_web")]
    AinxtWeb,
    /// Named client (`"nebula"`) — uses the generic permission UI
    #[serde(rename = "nebula")]
    Nebula,
    /// IDE extension client (VS Code and similar) - identified by clientIdentifier "ainxt-code-extension"
    #[serde(rename = "extension")]
    Extension,
    /// ainxt Pager client - TUI-like terminal pager with interactive permission UI.
    /// Treated identically to AinxtTUI for permission options (gets bash highlights +
    /// interactive selection). Reports as "pager" for telemetry attribution.
    ///
    /// Accepts both the hyphenated `"ainxt-pager"` (what the pager actually
    /// sends over the wire, matching `PAGER_CLIENT_TYPE`) and the underscored
    /// `"ainxt_pager"` form for symmetry with the rest of this enum.
    #[serde(rename = "ainxt-pager", alias = "ainxt_pager")]
    AinxtPager,
    /// ainxt Desktop (Electron) client - identified by clientIdentifier "ainxt-desktop".
    /// Uses TUI-style bash permission options (primary command extraction + prefix matching)
    /// but without interactive `<`/`>` word selection.
    #[serde(rename = "ainxt_desktop")]
    Desktop,
}
impl ClientType {
    /// Product token for the `User-Agent` header (e.g. `ainxt-pager`).
    pub fn user_agent_label(&self) -> &'static str {
        match self {
            Self::Generic => "ainxt-shell",
            Self::AinxtTUI => "ainxt-tui",
            Self::AinxtWeb => "ainxt-web",
            Self::Nebula => "nebula",
            Self::Extension => "ainxt-code-extension",
            Self::AinxtPager => "ainxt-pager",
            Self::Desktop => "ainxt-desktop",
        }
    }
    /// Resolve from ACP `clientIdentifier` string (e.g. `"ainxt-web"`, `"ainxt-desktop"`).
    pub fn from_client_identifier(id: Option<&str>) -> Self {
        match id {
            Some("ainxt-web") => Self::AinxtWeb,
            Some("nebula") => Self::Nebula,
            Some("ainxt-code-extension") => Self::Extension,
            Some("ainxt-desktop") => Self::Desktop,
            Some("ainxt-pager") => Self::AinxtPager,
            _ => Self::Generic,
        }
    }
    ...
    pub fn feedback_label(&self) -> &'static str {
        match self {
            Self::AinxtTUI | Self::AinxtPager => "tui",
            Self::AinxtWeb => "web",
            Self::Nebula => "nebula",
...
pub enum PermissionCommand {
    Request {
        ...
        /// Set when the enforcement point requires a human for this action.
        ///
        /// Evaluated once in `PermissionHandle::request` and carried here
        /// rather than recomputed in the actor: `authorize` charges the risk
        /// budget and writes an audit record, so evaluating twice would
        /// double-count every action.
        pep_forced_prompt: bool,
    },
...
impl From<&ainxt_tools::types::ToolInput> for AccessKind {
    fn from(input: &ainxt_tools::types::ToolInput) -> Self {
...
pub enum RequirementSource {
    Unknown,
    /// User-writable `~/.ainxt/requirements.toml` — untrusted for keeping a
...
mod tests {
    ...
    fn hashline_edit_maps_to_edit_access() {
        use ainxt_tools::implementations::ainxt_build_hashline::edit::types::HashlineEditInput;
        use ainxt_tools::types::ToolInput;
    ...
    fn bash_maps_to_bash_access() {
        use ainxt_tools::implementations::ainxt_build::bash::BashToolInput;
        use ainxt_tools::types::ToolInput;
    ...
    fn use_tool_maps_to_mcp_tool_access() {
        use ainxt_tools::implementations::use_tool::UseToolInput;
        use ainxt_tools::types::ToolInput;
    ...
    fn monitor_maps_to_bash_access() {
        use ainxt_tools::implementations::ainxt_build::monitor::types::MonitorInput;
        use ainxt_tools::types::ToolInput;
    ...
    fn web_fetch_maps_to_web_fetch_access() {
        use ainxt_tools::implementations::ainxt_build::web_fetch::WebFetchInput;
        use ainxt_tools::types::ToolInput;
    ...
    fn web_search_maps_to_web_search_access() {
        use ainxt_tools::implementations::ainxt_build::web_search::WebSearchInput;
        use ainxt_tools::types::ToolInput;
    ...
    #[test]
    fn client_type_deserializes_ainxt_shell_as_generic() {
        assert_eq!(
            serde_json::from_value::<ClientType>("ainxt-shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(
            serde_json::from_value::<ClientType>("ainxt_shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(
