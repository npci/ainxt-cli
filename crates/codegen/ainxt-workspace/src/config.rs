...
use std::sync::Arc;
use ainxt_tools::registry::types::{SessionContext, ToolRegistryBuilder, ToolServerConfig};
...
impl SessionTerminalBackend {
    ...
    pub fn local(backend: ainxt_tools::computer::local::LocalTerminalBackend) -> Self {
    ...
    pub fn backend(&self) -> &Arc<dyn ainxt_tools::computer::types::TerminalBackend> {
        &self.backend
    }
...
pub struct WorkspaceBindConfig {
    ...
    pub viewer_ctx: Option<ainxt_tool_runtime::WorkspaceViewerContext>,
    ...
    pub tools: Option<Vec<ainxt_tools_api::ToolConfigEntry>>,
...
pub enum ResolvedToolset {
    ...
    InvalidToolConfig(ainxt_tools::registry::proto_convert::ToolConfigEntryError),
}
...
impl WorkspaceBindConfig {
    ...
    /// [`ainxt_tool_runtime::WorkspaceBindMetadata`] (same type the emitter
    /// serializes); `tool_config` is a consumer-only raw escape hatch read
    /// separately.
    pub fn from_metadata(metadata: &serde_json::Value) -> Self {
        let wire: ainxt_tool_runtime::WorkspaceBindMetadata =
    ...
    pub fn resolve(
        ...
        if let Some(cfg) = &self.tool_config {
            for (idx, tool) in cfg.tools.iter().enumerate() {
                if let Err(err) = ainxt_tools_api::config_validation::validate_name_override(
                    idx,
                    &tool.id,
        ...
        if let Some(tools) = &self.tools {
            ...
            for (idx, entry) in tools.iter().enumerate() {
                ...
                match ainxt_tools::registry::proto_convert::tool_config_from_entry(
                    idx,
                    entry.clone(),
                ) {
            ...
            if !unserved_tool_ids.is_empty() {
                tracing::warn!(
                    ...
                    .manifest_version, running_version = ainxt_version::VERSION,
...
mod bind_config_tests {
    ...
    fn presets_are_never_resolved() {
        for preset in ["explore", "ainxt-computer", "bogus"] {
    ...
    /// Consumer-side equivalence test for the bind-metadata `tools` contract;
    /// pairs with the producer-side pin test in agentic-sampler's
    /// `configs::plane` tests.
    #[test]
    fn tools_entries_resolve_to_tool_server_config() {
        let v = serde_json::json!(
            { "preset" : "explore", "tools" : [{ "id" : "AinxtBuild:grep", "params_json" :
        ...
        assert_eq!(grep.id, "AinxtBuild:grep");
        assert_eq!(
            grep.params,
        ...
        assert_eq!(toolset.tools[1].id, "AinxtBuild:read_file");
    }
    #[test]
...
pub struct WorkspaceConfig {
    ...
    /// Global hook sources (e.g. `~/.claude/settings.json`, `~/.ainxt/hooks/`).
    pub hook_global_sources: Vec<HookSourceConfig>,
    /// Project-scoped hook sources (e.g. `<project>/.ainxt/hooks/`).
    ...
    /// Auth provider for ainxt service calls made from workspace-scoped code.
    ...
    /// `<cwd>/.ainxt/lsp.json`: `false` drops them at load, `true` keeps them. The
    ...
    /// Confine `ainxt.dev/fs/*` / `workspace.fs_*` resolution to the workspace root
...
pub struct WorkspaceServerMetadata {
    ...
    /// Logical sandbox-service session UUID, from the `AINXT_SESSION_ID` env
    ...
    /// unchanged on the diagnostics `/ready` endpoint. Absent for local/legacy
    /// launches.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl WorkspaceServerMetadata {
    ...
    ///
    /// `env_session_id` is the raw `AINXT_SESSION_ID`; empty is normalized to
...
impl WorkspaceConfig {
    ...
    pub fn new_for_proxy(
        ...
        auth_provider: ainxt_computer_hub_sdk::SharedAuthProvider,
...
/// `*.json` hook files. Maps 1:1 to [`ainxt_hooks::discovery::HookSource`]
...
pub enum HookSourceConfig {
    ...
    /// A directory of `*.json` hook files (e.g. `~/.ainxt/hooks/`).
    Directory(PathBuf),
}
