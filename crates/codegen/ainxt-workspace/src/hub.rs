...
impl HubHandle {
    ...
    pub(crate) async fn connect(
        ...
        session_handler_resolver: Option<ainxt_computer_hub_sdk::SessionHandlerResolver>,
        ...
        let mut server_builder = ToolServerBuilder::default()
            ...
            .binary_version(ainxt_version::VERSION)
...
struct CallCompletedGuard {
    ...
    outcome: ainxt_file_utils::events::ToolOutcome,
}
impl CallCompletedGuard {
    fn new(
        ...
        Self {
            ...
            call_id,
            session_id,
            outcome: ainxt_file_utils::events::ToolOutcome::Cancelled,
        }
    }
    fn set_outcome(&mut self, outcome: ainxt_file_utils::events::ToolOutcome) {
...
impl ToolServerHandler for SessionRoutedToolHandler {
    ...
    async fn handle_call(&self, ctx: ToolCallContext, args: Value) -> ToolStream<TypedToolOutput> {
        ...
        let hub_session = ctx
            .extensions
            .get::<ainxt_tool_runtime::SessionContext>()
            .map(|s| s.0.clone());
            ...
            match transport {
                ...
                None => {
                    tracing::warn!(
                        ...
                        "AINXT_HITL_PERMISSION_LIVE set but no hub ToolServer; rejecting guarded tool"
                    );
                    return terminal_only(Err(ToolError::new(
                        ToolErrorKind::PermissionDenied,
        ...
        Box::pin(async_stream::stream! {
            ...
            .set_outcome(ainxt_file_utils::events::ToolOutcome::Success); yield
            ...
            .set_outcome(ainxt_file_utils::events::ToolOutcome::Error); yield
...
fn parse_server_id(id: &str) -> Result<ainxt_tool_protocol::ServerId, ClientError> {
    ainxt_tool_protocol::ServerId::new(id)
...
mod tests {
    ...
    use ainxt_tool_runtime::{SessionContext, ToolCallId};
    ...
    async fn handle_call_terminal_matches_non_streaming_call() {
        ...
        match reference {
            Ok(run_result) => {
                ...
                assert_eq!(
                    typed.model_output,
                    vec![ainxt_tool_runtime::ContentBlock::Text {
    ...
    use ainxt_tools::types::tool_metadata::ToolMetadata as AinxtToolMetadata;
    #[derive(Debug)]
    struct GateStreamingStub;
    impl AinxtToolMetadata for GateStreamingStub {
    ...
    impl ainxt_tool_runtime::Tool for GateStreamingStub {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> ainxt_tool_protocol::ToolId {
            ainxt_tool_protocol::ToolId::new("gate_streaming_stub").expect("valid tool id")
        }
        fn description(
            &self,
            _ctx: &::ainxt_tool_runtime::ListToolsContext,
        ) -> ainxt_tool_types::ToolDescription {
            ainxt_tool_types::ToolDescription::new("gate_streaming_stub", "gate streaming stub")
        }
        async fn run(
            &self,
        ...
        async fn execute(
            ...
            use ainxt_tool_runtime::{ToolProgress, ToolStreamItem};
    ...
    async fn drain_counts<T>(mut stream: ainxt_tool_runtime::ToolStream<T>) -> (usize, usize, bool) {
        let mut progress = 0;
        let mut terminal = 0;
    ...
    fn bg_config() -> ToolServerConfig {
        ToolServerConfig {
            tools: vec![
                ...
                tc("AinxtBuild:monitor", Some(ToolKind::Monitor)),
            ],
    ...
    async fn wait_until(
        ...
        pred: impl Fn(&ainxt_tool_protocol::ToolServerStatusPayload) -> bool,
        timeout: Duration,
    ) -> ainxt_tool_protocol::ToolServerStatusPayload {
    ...
    fn bg_started_notif(task_id: &str) -> ToolNotification {
        use ainxt_tools::notification::types::{
    ...
    fn task_completed_notif(task_id: &str) -> ToolNotification {
        use ainxt_tools::computer::types::{TaskKind, TaskSnapshot};
