...
//! spawns the unchanged `/usr/local/bin/ainxt-preview-proxy` binary as a
...
pub const PREVIEW_PROXY_BIN_PATH: &str = "/usr/local/bin/ainxt-preview-proxy";
...
/// `ainxt_workspace_preview_proxy_restart_total{reason}` — (re)start events the
...
static PREVIEW_PROXY_RESTART_TOTAL: LazyLock<IntCounterVec> = LazyLock::new(|| {
    register_int_counter_vec!(
        "ainxt_workspace_preview_proxy_restart_total",
...
/// Access policy forwarded to the proxy's `--visibility`. Matches the proxy's
...
impl PreviewArgs {
    ...
    /// `ainxt-preview-proxy/src/cli.rs`). Absent options and a false
...
/// Build the unspawned proxy command. Secrets (`AINXT_SERVER_KEY` /
/// `AINXT_SESSION_ID`) reach the proxy by env inheritance — never argv.
fn build_preview_command(cfg: &PreviewArgs) -> io::Result<tokio::process::Command> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        // Raw pre_exec, NOT ainxt_tty_utils::detach_command: the proxy must stay in
...
/// Proxy control path exposing the last-activity stamp (matches
/// `ainxt-preview-proxy`'s `/__control/activity` route).
const PREVIEW_ACTIVITY_PATH: &str = "/__control/activity";

/// Effective proxy `--control-port` when the supervisor didn't pin one (matches
/// the default in `ainxt-preview-proxy/src/cli.rs`).
...
mod tests {
    ...
    fn sample_cfg() -> PreviewArgs {
        PreviewArgs {
            ...
            auth_redirect: Some("https://ainxt.example.test/preview-auth".to_owned()),
            allow_public: true,
    ...
    fn to_argv_maps_every_flag_to_the_proxy_cli_names() {
        // Flag names must match ainxt-preview-proxy/src/cli.rs exactly.
        assert_eq!(
            sample_cfg().to_argv(),
            vec![
                "--preview-port",
    ...
    #[cfg(target_os = "linux")]
    const PDEATHSIG_HELPER_ENV: &str = "AINXT_PDEATHSIG_HELPER";
    #[cfg(target_os = "linux")]
