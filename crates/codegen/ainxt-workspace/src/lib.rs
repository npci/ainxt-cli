...
pub use ainxt_workspace_client::WorkspaceClient;
pub use ainxt_workspace_types::WorkspaceEvent;
pub use ainxt_hunk_tracker::HunkTrackerHandle;
...
/// environment (`AINXT_HOME`, `HOME`, …). nextest isolates each test in its own
...
/// can clobber `AINXT_HOME` mid-test. A single shared lock (used by every
...
mod init_metrics_tests {
    ...
    fn init_metrics_is_idempotent_and_registers_baselines() {
        ...
        assert!(has(
            "ainxt_workspace_toolset_swap_rejected_total",
        ...
        assert!(
            families
                .iter()
                .any(|mf| mf.name() == "ainxt_workspace_env_capture_panic_total")
        );
        assert!(
            families
                .iter()
                .any(|mf| mf.name() == "ainxt_workspace_permission_timeout_total")
        );
    }
}
