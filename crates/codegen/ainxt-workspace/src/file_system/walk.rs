...
/// Walk configuration. Field semantics mirror the `ainxt.dev/fs/list` request.
...
pub async fn read_range(abs: &Path, offset: u64, length: u64) -> std::io::Result<Vec<u8>> {
    ...
    // Explicitly release the underlying file handle now that the capped
    // read is complete, rather than relying solely on end-of-scope Drop.
    drop(limited);
    Ok(chunk)
}
