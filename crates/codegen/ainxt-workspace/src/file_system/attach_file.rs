...
/// `~/.ainxt/sessions/{cwd}/{session_id}/pasted/` so the model can `read_file`
...
///
/// The text is passed through unchanged (no line-number rewriting) since it
...
/// Base directory for Phase-1 session-scoped scratch files.
///
/// Lives under the per-user `ainxt` home directory (`~/.ainxt/scratch/...`)
/// rather than the shared, world-readable OS temp directory, since
/// attachments here can contain user file contents. Namespaced by PID so
/// concurrent test processes (repeated or parallel CI test runs) never
/// collide on identical content-hash paths and race each other's cleanup.
/// The real session_dir lives in shell persistence.
fn session_scratch_root() -> PathBuf {
    ainxt_config::ainxt_home()
        .join("scratch")
        .join(format!("attach-sessions-{}", std::process::id()))
}
...
mod tests {
    ...
    #[tokio::test]
    async fn test_ainxt_render_embedded_resource_uses_file_contents_tag() {
        let _info = test_info("ainxt-text");
        let resource = EmbeddedResource::new(EmbeddedResourceResource::TextResourceContents(
            agent_client_protocol::TextResourceContents::new(
                "const x = 1;\nconst y = 2;\n",
    ...
    #[tokio::test]
    async fn test_ainxt_render_embedded_resource_full_file_uses_is_full_file() {
        let _info = test_info("ainxt-full-file");
        let resource = EmbeddedResource::new(EmbeddedResourceResource::TextResourceContents(
            agent_client_protocol::TextResourceContents::new(
                "fn main() {}\n",
