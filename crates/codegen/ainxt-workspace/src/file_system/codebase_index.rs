...
use ainxt_codebase_graph::{IndexManager, IndexManagerConfig, IndexManagerHandle};

use ainxt_tools::util::ainxt_home::ainxt_home;

/// Get the cache path for a cwd's index.
///
/// Cache is stored in: `~/.ainxt/indexes/{url_encoded_cwd}/goto_index.bin`
...
mod tests {
    ...
    // which maps to `ainxt.dev/code/status` reporting `reason: notStarted`.
    ...
    /// In `ainxt.dev/code/status`, `resolve_index_handle()` calls
