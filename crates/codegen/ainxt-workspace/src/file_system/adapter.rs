//! AcpFsAdapter: implements `ainxt-tools::AsyncFileSystem` using ACP gateway calls.
//!
//! This adapter enables file tool execution over ACP (remote filesystem).
//! It translates ainxt-tools' `AsyncFileSystem` trait into ACP protocol calls:
//!   `read_file()` → read_text_file
//!   `write_file()` → write_text_file
//!   `delete_file()` → not supported by ACP (returns error)
//!
//! Matches the pattern of `AcpTerminalAdapter` for terminal execution.

use std::path::Path;

use agent_client_protocol as acp;
use ainxt_acp_lib::AcpAgentGatewaySender as GatewaySender;
use ainxt_tools::computer::types::{AsyncFileSystem, ComputerError};

/// Wraps ainxt-shell's ACP gateway to satisfy ainxt-tools' AsyncFileSystem.
///
