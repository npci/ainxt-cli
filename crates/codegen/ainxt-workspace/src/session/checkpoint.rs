...
use ainxt_hunk_tracker::{HunkId, HunkTrackerSnapshot, HunkTurnDelta};
...
impl TurnBoundary {
    ...
    /// matches `prompt_index` and the dispatcher ignores it on this path.
...
/// Resolve `workspace_rewind_hunks` from `AINXT_WORKSPACE_REWIND_HUNKS` (default off).
pub(crate) fn rewind_hunks_enabled() -> bool {
    ainxt_config::env_bool("AINXT_WORKSPACE_REWIND_HUNKS").unwrap_or(false)
}
/// Resolve `workspace_rewind_durable` from `AINXT_WORKSPACE_REWIND_DURABLE`
/// (default off). Off ⇒ the legacy in-memory-only path with no disk I/O.
pub(crate) fn rewind_durable_enabled() -> bool {
    ainxt_config::env_bool("AINXT_WORKSPACE_REWIND_DURABLE").unwrap_or(false)
}
impl WorkspaceSession {
    ...
    pub(crate) async fn restore_hunk_checkpoints(&self, target_prompt_index: usize) {
        ...
        let mut file_states: HashMap<std::path::PathBuf, ainxt_hunk_tracker::FileHunkStateSnapshot> =
            HashMap::new();
...
impl WorkspaceHandle {
    ...
    ) -> Option<tokio::task::JoinHandle<ainxt_file_utils::queue::EnqueueOutcome>> {
        match boundary {
            TurnBoundary::Start {
...
mod tests {
    ...
    async fn turn_end_with_durable_flag_off_writes_nothing_to_disk() {
        ...
        handle
            .on_turn_boundary("main", TurnBoundary::rewind_finalize(0))
            .await;
        let store_root = cwd.join(".ainxt").join("rewind-checkpoints");
        assert!(
            !store_root.exists(),
