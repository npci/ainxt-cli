...
/// Directory (under `<cwd>/.ainxt`) holding every session's checkpoint store.
...
pub(crate) struct CheckpointStore {
    /// Per-session store directory: `<cwd>/.ainxt/rewind-checkpoints/<session_id>`.
    dir: PathBuf,
...
impl CheckpointStore {
    ...
    pub(crate) fn with_cap(cwd: &Path, session_id: &str, cap: usize) -> Self {
        // `session_id` is RPC-controlled: never join it unchanged (a `../../etc`
