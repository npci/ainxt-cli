...
use std::sync::Arc;
use ainxt_computer_hub_mcp_adapter::McpBridgeHandle;
use ainxt_mcp::servers::McpState;
use ainxt_tools::notification::types::{ToolNotification, ToolNotificationHandle};
use ainxt_tools::registry::types::{FinalizedToolset, ToolConfig, ToolServerConfig};
use ainxt_hunk_tracker::HunkTrackerHandle;
use ainxt_tool_protocol::ToolId;
use ainxt_tool_runtime::WorkspaceViewerContext;
...
pub struct WorkspaceSession {
    ...
    pub(crate) hunk_checkpoints:
        Arc<tokio::sync::Mutex<HashMap<usize, ainxt_hunk_tracker::HunkTurnDelta>>>,
...
impl WorkspaceSession {
    ...
    pub(crate) async fn toolset_terminal_is_session_owned(&self) -> bool {
        ...
        match res.get::<ainxt_tools::types::resources::Terminal>() {
...
pub struct WorkspaceShared {
    ...
    pub(crate) events: tokio::sync::broadcast::Sender<ainxt_workspace_types::WorkspaceEvent>,
    pub(crate) respect_gitignore: bool,
    pub(crate) memory_config: Option<MemoryConfig>,
    pub(crate) hook_registry: Arc<parking_lot::RwLock<ainxt_hooks::discovery::HookRegistry>>,
    pub(crate) hook_load_errors: Vec<ainxt_hooks::error::HookError>,
    ...
    /// Auth provider for ainxt service calls.
    pub(crate) auth_provider: Option<ainxt_computer_hub_sdk::SharedAuthProvider>,
    ...
    pub(crate) activity_notify_handle:
        arc_swap::ArcSwap<Option<ainxt_tools::notification::types::ToolNotificationHandle>>,
    /// Sink for workspace-originated ext-notifications to the client (e.g.
    /// `ainxt.dev/search/fuzzy/status`). Mode-agnostic: the shell wires it to the
    ...
    pub(crate) local_registry: ainxt_computer_hub_sdk::LocalRegistry,
    ...
    /// Opaque metadata for the tool server registration, forwarded unchanged to
    ...
    pub(crate) lsp: Option<std::sync::Arc<dyn ainxt_tools::implementations::lsp::LspBackend>>,
    ...
    /// (from `AINXT_WORKSPACE_REWIND_ALL_OUTCOMES`, default off).
    pub(crate) workspace_rewind_all_outcomes: bool,
    /// Resolved `$AINXT_WORKSPACE_HOME` — the workspace-owned on-disk state root
    /// (`<ainxt_home>/workspace` by default). The upload queue spills here.
    pub(crate) workspace_home: std::path::PathBuf,
    pub(crate) upload_queue: Option<std::sync::Arc<ainxt_file_utils::queue::UploadQueue>>,
    ...
    /// (`AINXT_WORKSPACE_EVENTS_ENABLED=true`). When `false`, every
    /// [`session_event_writer`](Self::session_event_writer) hands back an
    /// [`EventWriter::noop()`](ainxt_file_utils::events::EventWriter::noop) and
    ...
    /// enabled (`AINXT_WORKSPACE_TOOL_DEFS_ENABLED=true`).
    ...
    pub(crate) session_event_writers:
        Arc<dashmap::DashMap<String, ainxt_file_utils::events::EventWriter>>,
    ...
    pub(crate) inflight_enqueues: dashmap::DashMap<
        (String, u64),
        tokio::task::JoinHandle<ainxt_file_utils::queue::EnqueueOutcome>,
    >,
...
impl WorkspaceShared {
    ...
    pub fn upload_queue(&self) -> Option<&std::sync::Arc<ainxt_file_utils::queue::UploadQueue>> {
    ...
    /// [`EventWriter::noop()`](ainxt_file_utils::events::EventWriter::noop)
    /// WITHOUT touching the cache or the filesystem, so the flag-off path stays
    /// exactly identical to the legacy behaviour. The returned handle is
    /// `Clone + Send + Sync`; callers emit through it directly.
    pub(crate) fn session_event_writer(
        &self,
        session_id: &str,
    ) -> ainxt_file_utils::events::EventWriter {
    ...
    pub(crate) fn session_event_writer_cached(
        &self,
        session_id: &str,
    ) -> Option<ainxt_file_utils::events::EventWriter> {
    ...
    /// Auth provider used for ainxt service calls.
    pub fn auth_provider(&self) -> Option<&ainxt_computer_hub_sdk::SharedAuthProvider> {
    ...
    ///
    /// Returns a clone of the [`ToolServer`](ainxt_computer_hub_sdk::ToolServer)
    ...
    pub fn subscribe_events(
        &self,
    ) -> tokio::sync::broadcast::Receiver<ainxt_workspace_types::WorkspaceEvent> {
...
pub(crate) fn get_or_open_session_writer(
    enabled: bool,
    writers: &dashmap::DashMap<String, ainxt_file_utils::events::EventWriter>,
    workspace_home: &Path,
    session_id: &str,
) -> ainxt_file_utils::events::EventWriter {
    ...
    if let Err(e) = std::fs::create_dir_all(&dir) {
        tracing::warn!(
            session_tag = % session_log_tag(session_id), dir = % dir.display(), error = % e,
...
mod tests {
    ...
    use dashmap::DashMap;
    use ainxt_file_utils::events::{Event, EventWriter};
