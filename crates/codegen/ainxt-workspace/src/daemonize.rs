...
/// (`AINXT_WORKSPACE_TERMINATION_GRACE_MS`, default 45s): a takeover only
...
mod tests {
    ...
    fn pidfile_acquire_errors_on_directory() {
        ...
        // Build with an explicit, restrictive (owner-only) mode via
        // `DirBuilder` rather than `fs::create_dir` + a default umask.
        #[cfg(unix)]
        {
    ...
    fn process_name_matches_own_argv0() {
        ...
        // differently (e.g. Cargo uses `ainxt_workspace-<hash>`), so a
    ...
    fn basename_contains_ignores_directory_components() {
        assert!(basename_contains(
            "/usr/local/bin/ainxt-workspace-server",
            "workspace-server"
        ));
        assert!(basename_contains(
            "C:\\Program Files\\Ainxt-Workspace-Server.exe",
            "workspace-server"
        ));
        assert!(
