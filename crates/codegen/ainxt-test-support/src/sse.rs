...
//!
//! These produce the exact wire format that the ainxt sampling client expects,
...
/// receiver must reconstruct `text` exactly).
...
/// reproduces `text` exactly (newlines and whitespace runs preserved).
...
/// Split `text` into deltas that reconstruct it exactly: the first
...
/// the chunks reconstructs `text` exactly (newlines included).
...
/// event (`ainxt_sampling_types::DOOM_LOOP_CHECK_EVENT_TYPE`). Hardcoded
