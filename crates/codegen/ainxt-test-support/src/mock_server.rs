...
enum ResponseMode {
    ...
    Echo,
    /// Stream a fixed text whose deltas reconstruct it exactly
...
impl MockInferenceServer {
    ...
    /// the user message. Deltas reconstruct the text exactly (newlines
...
mod tests {
    ...
    /// Bearer value the required-auth tests expect. Not a credential: it grants
    /// nothing and is only compared against itself by the in-process mock over
    /// loopback.
    const MOCK_AUTH_FIXTURE: &str = "mock-auth-fixture";
    ...
    async fn required_auth_enforced_in_both_response_modes() {
        ...
        let resp = client
            .post(&url)
            .header("authorization", format!("Bearer {MOCK_AUTH_FIXTURE}"))
            .json(&req_body)
            .send()
            .await
            .unwrap();
