//! Leader-mode (`ainxt agent --leader stdio`) test harness.
//!
...
use ainxt_acp_lib::LineBufferedRead;

use crate::env::ainxt_binary;
...
/// (version-skew) test. Falls back to [`ainxt_binary`]'s resolution.
pub const LEADER_BINARY_ENV: &str = "AINXT_BINARY_LEADER";

/// Env var naming the binary for the second (usually newer) client in a
/// two-binary test. Falls back to [`ainxt_binary`]'s resolution.
pub const CLIENT_BINARY_ENV: &str = "AINXT_BINARY_CLIENT";
...
/// (`AINXT_BINARY_LEADER`, else the shared [`ainxt_binary`] resolution).
pub fn leader_binary() -> PathBuf {
    role_binary(LEADER_BINARY_ENV)
}

/// Binary for the client side of a version-skew test (`AINXT_BINARY_CLIENT`,
/// else the shared [`ainxt_binary`] resolution).
...
impl acp::Client for LeaderAcpClient {
    ...
    async fn ext_notification(&self, args: acp::ExtNotification) -> acp::Result<()> {
        if &*args.method == "ainxt.dev/leader_reconnected" {
            self.capture
                .reconnected_count
                .fetch_add(1, Ordering::SeqCst);
        }
...
/// A `ainxt agent --leader stdio` client subprocess speaking ACP over pipes.
...
impl LeaderStdioClient {
    pub async fn spawn(server: &MockInferenceServer, cwd: &Path, home: &Path) -> Self {
        Self::spawn_with_binary(&ainxt_binary(), server, cwd, home).await
    }
    ...
    pub async fn spawn_with_binary(
        ...
        cmd.args(["agent", "--leader", "stdio"])
            .current_dir(cwd)
            // Hermetic env: the developer's shell may export AINXT_* vars
            // (e.g. AINXT_LEADER_SOCKET pointing at a REAL leader on this
            ...
            .env("AINXT_LEADER_SOCKET", home.join(".ainxt").join("leader.sock"))
            .env("AINXT_CLI_CHAT_PROXY_BASE_URL", server.url())
            .env("AINXT_API_BASE_URL", server.url())
            ...
            // ~/.ainxt/leader.log — keep it chatty for diagnosis.
            .env("RUST_LOG", "ainxt_shell=debug");
    ...
    pub async fn initialize(&self) -> acp::InitializeResponse {
        ...
        let api_key_method = init
            ...
            .expect("ainxt.api_key auth method");
        self.conn
            .authenticate(
...
pub fn leader_lock_path(home: &Path) -> PathBuf {
    home.join(".ainxt").join("leader.lock")
}
...
///
/// The `ainxt.dev/leader_reconnected` ext notification is dropped by the typed
/// `ClientSideConnection` (bare `ainxt.dev/*` methods are rejected by the ACP
...
pub fn leader_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".ainxt").join("leader.log")).unwrap_or_default()
}
