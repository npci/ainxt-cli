//! ACP stdio clients for testing ainxt sessions end-to-end: the typed
//! [`AinxtStdioClient`] (`agent-client-protocol::ClientSideConnection` —
//! authentication, session lifecycle, permissions, notification streaming) and
//! the raw-wire [`RawStdioClient`] (unchanged JSON-RPC lines for shapes the
...
use crate::env::{ainxt_binary, test_env_cmd_tokio};
use crate::headless::stderr_tail;
use crate::mock_server::MockInferenceServer;
use crate::process::spawn_piped_with_stderr_capture;

/// Spawn `ainxt agent stdio` with the canonical hermetic test env: the sandbox
/// from [`test_env_cmd_tokio`] plus the debug-logging kill-list, so the
/// hermeticity setup exists exactly once for the typed ([`AinxtStdioClient`])
...
/// kill-list so a test can still set e.g. `AINXT_DEBUG_LOG=1` explicitly.
fn spawn_agent_process(
    ...
    // controls logging only via `extra_env` / `leading_args` (matches the
    // headless `debug_cmd`).
    for k in [
        "AINXT_DEBUG_LOG",
        "AINXT_LOG_FILE",
    ...
    pub async fn initialize(&self) -> acp::InitializeResponse {
        ...
        let api_key_method = init_resp
            ...
            .unwrap_or_else(|| {
                ...
                panic!(
                    "expected auth method 'ainxt.api_key' but got: {ids:?}\n\
...
/// Drives `ainxt agent stdio` with unchanged newline-delimited JSON-RPC lines.
///
/// Exists for wire shapes the typed [`AinxtStdioClient`] (`ClientSideConnection`,
...
impl RawStdioClient {
    ...
    /// Write `line` unchanged followed by `\n`, and flush.
