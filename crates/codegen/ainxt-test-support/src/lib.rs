...
//! Shared test utilities for ainxt-build crates: mock inference server, SSE
...
//! - [`AinxtStdioClient`] — ACP client that drives `ainxt agent stdio` as a subprocess
//! - [`RawStdioClient`] — raw-wire ACP driver for bytes the typed client can't
//!   produce (Foundation `\/` methods, string UUID ids)
//! - [`leader::LeaderStdioClient`] — ACP client that drives `ainxt agent --leader stdio` (unix)
//! - [`run_headless`] — Run `ainxt -p` against the mock server and capture output
//! - [`git_workdir`] — Create a temp directory with git repo (forces libgit2 init)
//! - [`ainxt_binary`] — Resolve the ainxt binary path (AINXT_BINARY env or cargo_bin)
...
#[cfg(unix)]
pub mod uds_proxy;
pub use acp_client::{AinxtStdioClient, RawStdioClient};
pub use counting_server::spawn_counting_server;
pub use env::{
    CHAT_PROXY_ENV, EnvGuard, FIRST_PARTY_BASE_URL, ainxt_binary, first_party_chat_proxy,
    git_workdir,
};
pub use headless::{
