...
impl AinxtProtoBuilder {
    ...
    fn emit_rerun_if_changed<'a>(
        ...
        for proto in protos {
            // `/dev/stdout` and `/dev/null` are Unix device paths and don't
            // exist on Windows, so use real temp files that work on every OS.
            let scratch = tempfile::TempDir::new()?;
            let dep_path = scratch.path().join("deps.d");
            let discard_path = scratch.path().join("discard.bin");

            let mut command = Command::new(protoc.unwrap_or(Path::new("protoc")));
            command
                .arg(format!("--dependency_out={}", dep_path.display()))
                .arg(format!("--descriptor_set_out={}", discard_path.display()));
            ...
            // Nothing meaningful goes to stdout anymore; a stray write there
            // would otherwise be misread by Cargo as a `cargo:` directive.
            ...
            // The rule's target is the exact --descriptor_set_out path we
            // passed. Strip that instead of matching a literal, since a
            // Windows drive letter like `C:\` would otherwise be mistaken for
            // the rule separator. Fall back to splitting on ": " (colon-space),
            // which a bare drive letter never produces.
            let target_prefix = format!("{}:", discard_path.display());
            let rem = first_line
                .strip_prefix(target_prefix.as_str())
                .or_else(|| first_line.split_once(": ").map(|(_, rest)| rest))
                .with_context(|| {
                    format!("protoc command output must start with the descriptor set path: {output:?}")
                })?;
            for line in iter::once(rem).chain(lines) {
                ...
                // Normalize `\` to `/` first so this still matches on Windows paths.
    ...
    pub fn compile_protos(
        ...
        let file_descriptor_set_path: Option<PathBuf> =
            if let Some(file_descriptor_set_path) = file_descriptor_set_path {
                ...
                let file_descriptor_set_path = tempfile.path().join("ainxt-proto-build.pbbin");
...
pub fn configure() -> AinxtProtoBuilder {
