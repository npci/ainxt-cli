...
Three steps: **install**, **point it at a gateway**, **use it**.

> **Status:** the one-line installers below work as soon as a GitHub Release
> exists for this repository. **No release has been published yet**, so today
> the working path is [Build from source](#option-b--build-from-source), which
> needs only Rust and takes one command. See
> [Publishing a release](#publishing-a-release-maintainers) for what to publish
> to switch the one-liners on.
...
```sh
curl -fsSL <url>/install.sh | AINXT_REQUIRE_CHECKSUM=1 bash
```

```powershell
$env:AINXT_REQUIRE_CHECKSUM=1; irm <url>/install.ps1 | iex
```

**Pin a version:**

```sh
curl -fsSL <url>/install.sh -o install.sh && bash install.sh 0.2.101
```

```powershell
$env:AINXT_VERSION="0.2.101"; irm <url>/install.ps1 | iex
```
...
| Installer: `could not determine the latest release` | No release published yet. Build from source, or set `AINXT_BASE_URL`. |
